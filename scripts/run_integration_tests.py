...
class IntegrationTestRunner:
    ...
    def test_code_generation_workflow(self):
        ...
        for attempt in range(max_attempts):
            ...
            response = requests.get(
                f"{self.base_url}/api/v1/generations/{generation_id}", timeout=10
            )
    ...
    def test_rate_limiting(self):
        ...
        self.log(
            f"Rate limiting: {'active' if rate_limited else 'not triggered (may be disabled in test)'}"
        )
    ...
    def test_api_response_time(self):
        ...
        endpoints = ["/health", "/api/v1/generations", "/metrics"]
