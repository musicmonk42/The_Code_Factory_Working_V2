...
class TestPluginService:
    ...
    async def test_handle_shif_request_error(self, mock_dependencies):
        ...
        service.message_bus.publish.assert_called_with(
            "shif:fix_import_failure",
            {"error": "Fix failed", "path": "/test/file.py"}
        )
...
class TestOmniCoreOmega:
    ...
    def test_create_and_initialize(self, mock_file, mock_fixer, mock_test_gen,
                                  mock_crew, mock_sim, mock_plugin_service,
                                  mock_bus, mock_db):
    ...
    @patch('omnicore_engine.engines.TestGenerationOrchestrator')
    @patch('omnicore_engine.engines.create_import_fixer_engine')
    def test_create_and_initialize_no_config(self, mock_fixer, mock_test_gen,
                                            mock_crew, mock_sim, mock_plugin_service,
                                            mock_bus, mock_db, mock_file):
...
class TestCrewConfigLoading:
    ...
    @patch('omnicore_engine.engines.TestGenerationOrchestrator')
    @patch('omnicore_engine.engines.create_import_fixer_engine')
    def test_load_crew_config_with_agents(self, mock_fixer, mock_test_gen,
                                         mock_crew_class, mock_sim, mock_plugin_service,
                                         mock_bus, mock_db, mock_yaml, mock_file):
