...
class TestSecurityIntegrationManager:
    ...
    def mock_config(self):
        ...
        config.API_ENDPOINT_LIMITS = {
            "/auth/login": {"per_minute": 5}
        }
    ...
    async def test_authenticate_success(self, manager):
        ...
        manager._get_user = AsyncMock(return_value={
            'username': 'testuser',
            'password_hash': 'hash',
            'roles': [UserRole.USER],
            'mfa_enabled': True,
    ...
    async def test_authenticate_invalid_password(self, manager, mock_security_utils):
        ...
        from security_utils import AuthenticationError
    ...
    async def test_authorize_expired_session(self, manager):
        ...
        from security_utils import AuthorizationError
...
class TestSecurityMiddleware:
    ...
    def middleware(self):
        ...
        manager = Mock()
        manager.config = Mock()
        manager.config.API_ENDPOINT_LIMITS = {
            "/api/limited": {"per_minute": 5}
        }
        manager.config.MAX_REQUEST_SIZE_BYTES = 1024
        manager.config.SECURITY_HEADERS = {
            "X-Frame-Options": "DENY"
        }
...
class TestDecorators:
    ...
    async def test_secure_endpoint_with_permission(self):
        ...
        with patch('omnicore_engine.security_integration.get_security_integration_manager') as mock_get:
            manager = Mock()
