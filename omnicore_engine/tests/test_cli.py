...
from omnicore_engine.cli import (
    ...
    EXIT_CODE_POLICY_DENIED,
    EXIT_CODE_FILE_ARGUMENT_ERROR,
...
class TestAuditCommands:
    ...
    async def test_audit_query_command(self):
        ...
        mock_engine.audit.query_audit_records = AsyncMock(return_value=[
            {"uuid": "1", "kind": "test"},
            {"uuid": "2", "kind": "test"}
        ])
    ...
    async def test_audit_replay_command(self):
        ...
        mock_engine.is_initialized = True
        mock_engine.audit = Mock()
        mock_engine.audit.replay_events = AsyncMock(return_value=[
            {"uuid": "1", "sim_id": "sim123"}
        ])
...
class TestOutputFormatting:
    ...
    def test_json_output_format(self):
        ...
        from omnicore_engine.cli import print_output
...
class TestWorkflowCommand:
    ...
    async def test_workflow_command_execution(self):
        ...
        mock_engine.is_initialized = True
        mock_engine.components = {
            'message_bus': Mock(publish=AsyncMock())
        }
...
class TestServeCommand:
    ...
    @patch('sys.argv', ['cli.py', 'serve', '--reload', '--host', '0.0.0.0', '--port', '8080'])
    @patch('omnicore_engine.cli.uvicorn.run')
