...
from watchdog.events import FileSystemEvent, EVENT_TYPE_CREATED, EVENT_TYPE_MODIFIED
import sys
import inspect
    ...
    async def test_handle_plugin_file_event_async_no_load_method(self, handler, mock_registry):
        ...
        try:
            with patch('omnicore_engine.plugin_event_handler.logger') as mock_logger:
                ...
                mock_logger.warning.assert_called()
                assert "does not have an async 'load_from_directory'" in mock_logger.warning.call_args[0][0]
        finally:
            os.unlink(temp_file)
