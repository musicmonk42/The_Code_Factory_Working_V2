...
class EnterpriseSecurityConfig(BaseSettings):
    ...
    DATA_CLASSIFICATION_REQUIRED: bool = Field(
        default=True,
        description="Enforce data classification for all operations"
    )
    ...
    KEY_ROTATION_DAYS: conint(ge=1, le=90) = Field(
        default=30,
        description="Maximum age for encryption keys (days)"
    )
    
    KEY_ROTATION_OVERLAP_DAYS: conint(ge=1, le=30) = Field(
        default=7,
        description="Overlap period for key rotation"
    )
    
    MIN_KEY_LENGTH_BITS: conint(ge=256) = Field(
        default=256,
        description="Minimum encryption key length in bits"
    )
    ...
    REQUIRE_HARDWARE_SECURITY_MODULE: bool = Field(
        default=False,
        description="Require HSM for key storage"
    )
    ...
    BACKUP_ENCRYPTION_ALGORITHM: str = Field(
        ...
        description="Fallback encryption algorithm"
    )
    ...
    REQUIRE_ENCRYPTION_AT_REST: bool = Field(
        default=True,
        description="Mandate encryption for stored data"
    )
    
    REQUIRE_ENCRYPTION_IN_TRANSIT: bool = Field(
        default=True,
        description="Mandate TLS for all communications"
    )
    ...
    MFA_METHODS: List[str] = Field(
        default=["TOTP", "FIDO2", "SMS_BACKUP"],
        description="Allowed MFA methods"
    )
    
    MFA_REMEMBER_DEVICE_DAYS: conint(ge=0, le=30) = Field(
        default=0,
        description="Days to remember trusted device (0=never)"
    )
    ...
    PROGRESSIVE_LOCKOUT: bool = Field(
        default=True,
        description="Increase lockout duration with repeated violations"
    )
    ...
    PRIVILEGED_ACCESS_DURATION_MINUTES: conint(ge=15, le=480) = Field(
        default=60,
        description="Maximum duration for elevated privileges"
    )
    ...
    ABSOLUTE_SESSION_TIMEOUT_MINUTES: conint(ge=60, le=480) = Field(
        default=240,
        description="Maximum session duration regardless of activity"
    )
    ...
    SESSION_TOKEN_LENGTH: conint(ge=32) = Field(
        default=64,
        description="Session token length in bytes"
    )
    ...
    INVALIDATE_SESSION_ON_IP_CHANGE: bool = Field(
        default=True,
        description="Terminate session if IP address changes"
    )
    ...
    DEFAULT_DENY: bool = Field(
        default=True,
        description="Deny access unless explicitly granted"
    )
    
    PRIVILEGE_ESCALATION_REQUIRES_MFA: bool = Field(
        default=True,
        description="Require MFA for privilege escalation"
    )
    ...
    PER_USER_RATE_LIMIT_PER_MINUTE: conint(ge=10) = Field(
        default=60,
        description="Per-user rate limit per minute"
    )
    
    PER_IP_RATE_LIMIT_PER_MINUTE: conint(ge=10) = Field(
        default=100,
        description="Per-IP rate limit per minute"
    )
    ...
    ENABLE_DISTRIBUTED_RATE_LIMITING: bool = Field(
        default=True,
        description="Enable rate limiting across cluster"
    )
    ...
    AUDIT_LOG_INTEGRITY_CHECK: bool = Field(
        default=True,
        description="Use cryptographic signatures for audit logs"
    )
    
    AUDIT_LOG_EVENTS: Set[str] = Field(
        default={
            "AUTHENTICATION", "AUTHORIZATION", "DATA_ACCESS",
            "DATA_MODIFICATION", "DATA_DELETION", "CONFIGURATION_CHANGE",
            "PRIVILEGE_ESCALATION", "SECURITY_VIOLATION", "SYSTEM_ACCESS",
            "KEY_MANAGEMENT", "AUDIT_LOG_ACCESS", "EXPORT_OPERATION"
        },
    ...
    ALLOWED_IP_RANGES: List[str] = Field(
        default=[],
        description="Allowed IP ranges (empty = all)"
    )
    ...
    REQUIRE_SECURE_HEADERS: bool = Field(
        default=True,
        description="Enforce security headers"
    )
    ...
    BREACH_NOTIFICATION_TIMEOUT_HOURS: conint(ge=1, le=72) = Field(
        default=72,
        description="Maximum time to notify of data breach"
    )
    ...
    BACKUP_ENCRYPTION_REQUIRED: bool = Field(
        default=True,
        description="Encrypt all backups"
    )
    ...
    BACKUP_GEOGRAPHIC_DISTRIBUTION: bool = Field(
        default=True,
        description="Distribute backups geographically"
    )
