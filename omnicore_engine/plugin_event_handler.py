...
class PluginEventHandler(FileSystemEventHandler):
    ...
    def _schedule_async_task(self, coro):
        ...
        try:
            if self._loop.is_running():
                ...
                logger.warning("Event loop not running when scheduling async task. Running coroutine directly.")
    ...
    async def _handle_plugin_file_event_async(self, event_src_path: str, event_type: str):
        ...
        try:
            ...
            if event_type == 'modified' and self.last_modified_times.get(real_src_path) == current_mtime:
            ...
            if hasattr(self.registry, 'load_from_directory') and inspect.iscoroutinefunction(self.registry.load_from_directory):
                await self.registry.load_from_directory(self.plugin_dir)
            else:
                logger.warning("PluginRegistry does not have an async 'load_from_directory' method. Plugin reload might not occur.")
                return
            ...
            if found_plugin:
                if hasattr(self.registry, 'db') and self.registry.db and hasattr(self.registry.db, 'save_plugin_legacy'):
                    logger.info(f"Attempting to save {event_type} plugin {found_plugin.meta.kind}:{found_plugin.meta.name} to DB.")
                    await self.registry.db.save_plugin_legacy({
                        ...
                        'code': inspect.getsource(found_plugin.fn) if callable(found_plugin.fn) else str(found_plugin.fn)
                    })
                    logger.info(f"Successfully saved {event_type} plugin {found_plugin.meta.kind}:{found_plugin.meta.name} metadata to DB.")
                else:
                    logger.warning(f"PluginRegistry DB not available or 'save_plugin_legacy' method missing. Plugin '{found_plugin.meta.name}' metadata not persisted on {event_type}.")
            else:
                logger.warning(f"Could not find plugin '{plugin_name_from_file}' in registry after {event_type} event. It might not have been correctly decorated or loaded.")

        except Exception as e:
            logger.error(f"Error during async plugin {event_type} handling for {event_src_path}: {e}", exc_info=True)
