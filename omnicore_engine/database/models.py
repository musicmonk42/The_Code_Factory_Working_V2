...
class AgentState(ArbiterAgentState, Base):
    """
    SQLAlchemy ORM model for storing agent states.
    Includes fields for agent attributes, inventory, and historical data.
    """
    ...
    # New V2 encrypted columns, nullable for graceful migration
    inventory_v2 = Column(String, nullable=True)
    language_v2 = Column(String, nullable=True)
    memory_v2 = Column(String, nullable=True)
    personality_v2 = Column(String, nullable=True)
    custom_attributes_v2 = Column(String, nullable=True)
    ...
    def __repr__(self):
        return f"<AgentState(name='{self.name}', type='{self.agent_type}', x={self.x}, y={self.y})>"

class ExplainAuditRecord(Base):
    """
    SQLAlchemy ORM model for storing audit records.
    Captures details of system events, including encryption and hashing for integrity.
    """
    __tablename__ = "explain_audit"
    ...
    custom_attributes = Column(String, nullable=True)
    rationale = Column(String, nullable=True)
    simulation_outcomes = Column(String, nullable=True)
    tenant_id = Column(String, nullable=True)
    explanation_id = Column(String, nullable=True)
    root_merkle_hash = Column(String, nullable=True)

    def __repr__(self):
        return f"<ExplainAuditRecord(uuid='{self.uuid}', kind='{self.kind}', name='{self.name}')>"

class GeneratorAgentState(AgentState):
    __tablename__ = "generator_agent_state"
    generated_code = Column(String, nullable=True)
    test_results = Column(JSON, nullable=True)
    deployment_config = Column(String, nullable=True)
