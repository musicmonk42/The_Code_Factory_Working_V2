...
def get_or_create_gauge_local(name: str, documentation: str, labelnames: tuple = ()) -> Gauge:
    ...
    try:
        ...
        logger.warning(f"Metric '{name}' already registered with a different type or incompatible labels. Reusing existing.")
        return REGISTRY._names_to_collectors[name]
    except Exception as e:
        logger.error(f"Error getting or creating Gauge '{name}': {e}")
        raise

def get_or_create_histogram_local(name: str, documentation: str, labelnames: tuple = (), buckets: Optional[tuple] = Histogram.DEFAULT_BUCKETS) -> Histogram:
