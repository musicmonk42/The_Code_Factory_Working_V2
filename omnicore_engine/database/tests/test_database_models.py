...
class TestAgentState:
    ...
    def test_agent_state_unique_name_constraint(self, session):
        ...
        agent1 = AgentState(
            name="unique_agent",
            x=0, y=0, energy=100, world_size=100
        )
        agent2 = AgentState(
            name="unique_agent",  # Same name
            x=10, y=10, energy=50, world_size=100
        )
...
class TestModelRelationships:
    ...
    def test_multiple_agent_types_coexist(self, session):
        ...
        generator = GeneratorAgentState(
            name="gen_agent",
            x=10, y=10, energy=100, world_size=100,
            generated_code="code"
        )
        
        sfe = SFEAgentState(
            name="sfe_agent",
            x=20, y=20, energy=100, world_size=100,
            trust_score=0.9
        )
...
class TestModelQueries:
    ...
    def test_filter_audit_records_by_time(self, session):
        ...
        recent_records = session.query(ExplainAuditRecord).filter(
            ExplainAuditRecord.ts > cutoff_time
        ).all()
