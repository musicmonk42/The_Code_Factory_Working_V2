...
class ExplanationRationale(BaseModel):
    ...
    details: Dict[str, Any] = Field(default_factory=dict, description="Additional detailed context or data supporting the reason.")
    explanation_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique ID for this specific explanation instance.")

class SimulationOutcome(BaseModel):
    scenario_name: str = Field(..., description="Name or identifier of the simulation scenario.")
    metrics: Dict[str, Any] = Field(default_factory=dict, description="Key metrics from the simulation outcome.")
    projected_impact: str = Field(..., description="A summary of the projected impact (e.g., 'positive', 'negative', 'neutral').")
    outcome_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique ID for this simulation outcome.")

class ExplainRecord:
    def __init__(self, kind: str, name: str, detail: str, sim_id: Optional[str] = None,
                 agent_id: Optional[str] = None, error: Optional[str] = None, context: Optional[str] = None,
                 custom_attributes: Optional[str] = None, explanation_id: Optional[str] = None,
    ...
    def _hash_record(self) -> str:
        ...
        for key in ['detail', 'context', 'custom_attributes', 'explanation_id', 'root_merkle_hash']:
            if data_to_hash.get(key) is None:
                data_to_hash[key] = ""
        return hashlib.sha256(json.dumps(data_to_hash, sort_keys=True, default=safe_serialize).encode('utf-8')).hexdigest()
...
class EnhancedExplainRecord(ExplainRecord):
    def __init__(self, kind: str, name: str, detail: str, sim_id: Optional[str] = None,
                 agent_id: Optional[str] = None, error: Optional[str] = None, context: Optional[str] = None,
                 custom_attributes: Optional[str] = None, rationale: Optional[str] = None,
                 simulation_outcomes: Optional[str] = None, tenant_id: Optional[str] = None,
                 explanation_id: Optional[str] = None, root_merkle_hash: Optional[str] = None):
        super().__init__(kind, name, detail, sim_id, agent_id, error, context,
                         custom_attributes, explanation_id, root_merkle_hash)
        self.data['rationale'] = rationale
        self.data['simulation_outcomes'] = simulation_outcomes
        self.data['tenant_id'] = hashlib.sha256(tenant_id.encode()).hexdigest() if tenant_id else None
        self.hash = self._hash_record()
...
class AuditRecordSchema(BaseModel):
    ...
    error: Optional[str] = Field(None, description="Brief error message if the event was a failure.")
    context: Optional[str] = Field(None, description="Encrypted string of additional contextual data.")
    custom_attributes: Optional[str] = Field(None, description="Encrypted string of custom, event-specific attributes.")
    rationale: Optional[str] = Field(None, description="Encrypted string of the rationale for the action (for enhanced records).")
    simulation_outcomes: Optional[str] = Field(None, description="Encrypted string of simulated what-if outcomes (for enhanced records).")
    tenant_id: Optional[str] = Field(None, description="Hashed tenant ID for multi-tenancy (for enhanced records).")
    explanation_id: Optional[str] = Field(None, description="ID linking to an AI-generated explanation.")
    root_merkle_hash: Optional[str] = Field(None, description="Merkle root hash for integrity verification.")
    ...
    async def stream_event(self, record: Dict[str, Any]):
        try:
            if self.producer:
                ...
                self.producer.produce(self.topic, json.dumps(record, default=safe_serialize).encode('utf-8'), callback=delivery_report)
...
class AuditProofExporter:
    ...
    async def export_proof_bundle(self, user_id: str, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        try:
            ...
            bundle = {
                'records': [r.model_dump() if isinstance(r, (ExplainRecord, EnhancedExplainRecord)) else r for r in records],
                'merkle_root': merkle_root_hex,
                'decryption_key': decryption_key_value,
                'timestamp': time.time()
            }
            self.logger.info(f"Exported proof bundle for user {user_id}, tenant {tenant_id if tenant_id else 'N/A'}")
            return bundle
        except Exception as e:
...
class AuditHookManager:
    def __init__(self, db: Database, performance_tracker: PluginPerformanceTracker,
    ...
    async def capture_ab_shadow_results(self, kind: str, name: str, version: str) -> Dict[str, Any]:
        try:
            ...
            shadow_exec_time_avg = sum(h.get('execution_time', 0) for h in results) / max(1, len(results))
            
            active_error_rate = sum(h.get('error_rate', 0) for h in active_history) / max(1, len(active_history))
            active_exec_time_avg = sum(h.get('execution_time', 0) for h in active_history) / max(1, len(active_history))
            ...
            self.logger.info(f"Captured A/B and shadow results for {results_dict['plugin_id']}: {results_dict}")
            return results_dict
        except Exception as e:
            self.logger.error(f"Failed to capture A/B and shadow results for {kind}:{name}:{version}: {e}", exc_info=True)
            return {}
...
class ExplainAudit:
    def __init__(self, system_audit_merkle_tree: Optional[Any] = None):
        ...
        try:
            ...
            logger.critical(f"Invalid Fernet key provided to Audit system: {e}. Encryption features for audit will be unavailable or insecure.", exc_info=True)
            self.encrypter = None
        except Exception as e:
            logger.critical(f"Error during Fernet key validation or initialization: {e}. Encryption features for audit will be unavailable or insecure.", exc_info=True)
            self.encrypter = None
        ...
        self.plugin_version_manager = PluginVersionManager(registry=self.plugin_registry, db=self._db_client)
        ...
        self.kafka_streamer = KafkaAuditStreamer(
            bootstrap_servers=self.config.KAFKA_BOOTSTRAP_SERVERS if hasattr(self.config, 'KAFKA_BOOTSTRAP_SERVERS') else 'localhost:9092'
        )
        self.proof_exporter = AuditProofExporter(
            db=self._db_client, encrypter=self.encrypter, merkle_tree=system_audit_merkle_tree
        )
                    ...
                    if not self.web3.is_connected():
                        ...
                        self.safe_create_task(
                            self.feedback_manager.record_feedback(
                                ...
                                details={'type': 'web3_connection_error', 'error': 'Web3 provider not connected'}
                            )
                        )
                else:
                    logger.info("WEB3_PROVIDER_URL not configured in settings. Web3 features disabled.")
            except Exception as e:
    ...
    def encrypt_dict(self, data: Optional[Dict[str, Any]]) -> Optional[str]:
        try:
            ...
            if self.encrypter is None:
                logger.warning("Attempted to encrypt data but Fernet encrypter is not initialized or invalid. Returning plaintext data.")
            ...
            logger.error(f"Encryption failed for data: {self.security_guard.sanitize_data(data)}: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'encryption_error', 'operation': 'encrypt_dict', 'error': str(e)}
                )
            )
            return None
    ...
    def decrypt_str(self, encrypted_data_str: Optional[str]) -> Optional[Any]:
        try:
            if encrypted_data_str is None:
                return None
            if self.encrypter is None:
                logger.warning("Attempted to decrypt data but Fernet encrypter is not initialized or invalid. Returning None.")
                return None

            decrypted_bytes = self.encrypter.decrypt(encrypted_data_str.encode('utf-8'))
            return json.loads(decrypted_bytes.decode('utf-8'))
        except InvalidToken:
            logger.error(f"Decryption failed: Invalid token for data snippet: {encrypted_data_str[:50]}...")
            return None
        except Exception as e:
            logger.error(f"Decryption failed for data snippet: {encrypted_data_str[:50]}...: {e}", exc_info=True)
            return None
    ...
    def safe_create_task(self, coro: Coroutine):
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("Event loop not running, attempting to run coroutine directly. This might block.")
                try:
                    loop.run_until_complete(coro)
                except RuntimeError:
                    asyncio.run(coro)
        except RuntimeError as e:
            logger.warning(f"RuntimeError in safe_create_task (no event loop): {e}. Attempting asyncio.run().")
            try:
                asyncio.run(coro)
    ...
    def add_entry(self, kind: str, name: str, detail: Dict[str, Any], sim_id: Optional[str] = None,
                  agent_id: Optional[str] = None, error: Optional[str] = None,
                  context: Optional[Dict[str, Any]] = None, custom_attributes: Optional[Dict[str, Any]] = None,
                  rationale: Optional[Dict[str, Any]] = None, simulation_outcomes: Optional[List[Dict]] = None,
                  tenant_id: Optional[str] = None, explanation_id: Optional[str] = None,
                  root_merkle_hash: Optional[str] = None):
        ...
        metadata_for_policy = {"kind": kind, "name": name, "detail_summary": str(detail)[:100]}
        ...
        try:
            ...
            try:
                ...
                if current_loop.is_running():
                    allowed, reason = current_loop.run_until_complete(
                        self.policy_engine.should_auto_learn(user_id_for_policy, action_for_policy, metadata_for_policy)
                    )
                else:
            ...
            logger.error(f"Policy check failed for audit entry (deprecated sync call): {self.security_guard.sanitize_data(detail)}: {e}", exc_info=True)

        if not allowed:
            logger.warning(f"Audit record denied by policy: {reason}. Record: {kind}:{name}.")
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.GENERAL,
                    details={'type': 'policy_denial', 'record_kind': kind, 'reason': reason}
                )
            )
            return
        ...
        with self.lock:
            try:
                ...
                if kind in {'plugin_hot_swap', 'plugin_rollback', 'plugin_shadow_deploy_predicted', 'promote_shadow_version', 'ab_shadow_audit'}:
                    record = EnhancedExplainRecord(
                        kind=kind, name=name, detail=encrypted_detail, sim_id=sim_id,
                        agent_id=agent_id, error=error, context=encrypted_context,
                        custom_attributes=encrypted_custom_attributes, rationale=encrypted_rationale,
                        simulation_outcomes=encrypted_simulation_outcomes, tenant_id=tenant_id,
                        explanation_id=explanation_id, root_merkle_hash=current_merkle_root
                    )
                else:
                ...
                try:
                    ...
                    logger.error(f"Audit record schema validation failed for record (UUID: {record.uuid}): {e}")
                    self.safe_create_task(self.feedback_manager.record_feedback(
                        user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                        details={'type': 'audit_schema_validation_failed', 'record_uuid': record.uuid, 'error': str(e), 'record_data_sanitized': self.security_guard.sanitize_data(record.model_dump())}
                    ))
                    return
                ...
                logger.error(f"Failed to process audit record data (sync call, UUID: {record.uuid if 'record' in locals() else 'N/A'}): {self.security_guard.sanitize_data(detail)}: {e}", exc_info=True)
                self.safe_create_task(
                    self.feedback_manager.record_feedback(
                        user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                        details={'type': 'audit_processing_error', 'operation': 'add_entry_sync', 'error': str(e), 'record_kind': kind, 'record_name': name}
                    )
                )

    async def add_entry_async(self, kind: str, name: str, detail: Dict[str, Any], sim_id: Optional[str] = None,
                              agent_id: Optional[str] = None, error: Optional[str] = None,
                              context: Optional[Dict[str, Any]] = None, custom_attributes: Optional[Dict[str, Any]] = None,
                              rationale: Optional[Dict[str, Any]] = None, simulation_outcomes: Optional[List[Dict]] = None,
                              tenant_id: Optional[str] = None, explanation_id: Optional[str] = None,
        ...
        allowed, reason = await self.policy_engine.should_auto_learn(user_id_for_policy, action_for_policy, metadata_for_policy)
        if not allowed:
            logger.warning(f"Audit record denied by policy: {reason}. Record: {kind}:{name}.")
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.GENERAL,
                    details={'type': 'policy_denial', 'record_kind': kind, 'reason': reason, 'record_name': name}
                )
            )
            return
        ...
        with self.lock:
            try:
                ...
                logger.error(f"Failed to process audit record data (async call, UUID: {record.uuid if 'record' in locals() else 'N/A'}): {self.security_guard.sanitize_data(detail)}: {e}", exc_info=True)
                self.safe_create_task(
                    self.feedback_manager.record_feedback(
                        user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                        details={'type': 'audit_processing_error', 'operation': 'add_entry_async', 'error': str(e), 'record_kind': kind, 'record_name': name}
                    )
                )
    ...
    async def _flush_buffer(self):
        ...
        try:
            for record in records_to_flush:
                ...
                if self.config.AUDIT_BLOCKCHAIN_ENABLED and WEB3_AVAILABLE and self.web3 and self.web3.is_connected():
                    try:
                        if self.web3.eth.accounts:
                            ...
                            logger.debug(f"Blockchain audit tx: {tx_hash} for record {record.uuid}.")
                        else:
                            logger.warning("No Web3 accounts available for blockchain audit transaction for record %s.", record.uuid)

                    except Exception as e:
                        logger.error(f"Blockchain transaction failed for record {record.uuid}: {self.security_guard.sanitize_data(record.data)}: {e}", exc_info=True)
                        self.safe_create_task(
                            self.feedback_manager.record_feedback(
                                user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                                details={'type': 'blockchain_tx_failed', 'record_id': record.uuid, 'error': str(e)}
                            )
                        )
                elif self.config.AUDIT_BLOCKCHAIN_ENABLED:
                    logger.warning(f"Blockchain audit enabled but Web3 not fully available or connected for record %s.", record.uuid)

                if self.system_audit_merkle_tree:
                    audit_leaf_content = json.dumps(record.model_dump(), sort_keys=True, default=safe_serialize).encode('utf-8')
                    self.system_audit_merkle_tree.add_leaf(audit_leaf_content)
                    self.system_audit_merkle_tree._recalculate_root()
                    logger.debug(f"Merkle tree updated for record {record.uuid}. New root: {self.system_audit_merkle_tree.get_merkle_root()[:10]}...")

                await self._db_client.save_audit_record(record.model_dump())

                await self.knowledge_graph.add_fact(
                    'AuditRecords', record.uuid, record.model_dump(), source='audit',
                    timestamp=datetime.utcnow().isoformat()
                )
            
            logger.info(f"Flushed {len(records_to_flush)} audit records to DB/KG/Blockchain (if enabled).")
            ...
            logger.error(f"Audit buffer flush failed: {self.security_guard.sanitize_data([r.model_dump() for r in records_to_flush])}: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'audit_flush_error', 'operation': '_flush_buffer', 'error': str(e), 'num_records_failed': len(records_to_flush)}
                )
            )
            raise
    ...
    async def query_audit_records(self, filters: Optional[Dict[str, Any]] = None, use_dream_mode: bool = False) -> List[Dict]:
        ...
        try:
            ...
            allowed, reason = await self.policy_engine.should_auto_learn(user_id_for_policy, action_for_policy, metadata_for_policy)
            if not allowed:
                logger.warning(f"Audit query denied by policy: {reason}. Filters: {self.security_guard.sanitize_data(filters)}.")
                raise ValueError(f"Policy denied: {reason}")
            ...
            for record_data in result_from_db:
                try:
                    ...
                    record_to_process['context'] = self.decrypt_str(record_to_process.get('context'))
                    record_to_process['custom_attributes'] = self.decrypt_str(record_to_process.get('custom_attributes'))
                    record_to_process['rationale'] = self.decrypt_str(record_to_process.get('rationale'))
                    record_to_process['simulation_outcomes'] = self.decrypt_str(record_to_process.get('simulation_outcomes'))
                    
                    AuditRecordSchema(**record_to_process)
                    validated_results.append(record_data)
                except ValidationError as e:
                    logger.warning(f"Invalid audit record encountered during replay (schema validation failed): {e}. Record UUID: {record_data.get('uuid')}. Data snippet: {self.security_guard.sanitize_data(record_data)}.")
                except Exception as e:
                    logger.error(f"Error processing audit record during query for validation/decryption (UUID: {record_data.get('uuid')}): {e}. Data snippet: {self.security_guard.sanitize_data(record_data)}.", exc_info=True)
            
            await self._log_audit('query_audit_records', 'query_' + str(uuid.uuid4()), 'system', {'count': len(validated_results), 'filters': filters})
            return validated_results
        except Exception as e:
            AUDIT_ERRORS.labels(operation='query_audit_records').observe(time.time() - start_time_op)
            logger.error(f"Audit query failed: {self.security_guard.sanitize_data(filters)}: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'audit_error', 'operation': 'query_audit_records', 'error': str(e), 'filters_sanitized': self.security_guard.sanitize_data(filters)}
                )
            )
            raise

    async def replay_events(self, sim_id: str, start_time: float, end_time: float, user_id: str = 'system') -> List[Dict]:
        AUDIT_RECORDS.labels(operation='replay_events').inc()
        start_time_op = time.time()
        try:
            action_for_policy = "replay_events"
            metadata_for_policy = {"sim_id": sim_id, "start_time": start_time, "end_time": end_time}

            allowed, reason = await self.policy_engine.should_auto_learn(user_id, action_for_policy, metadata_for_policy)
            if not allowed:
                logger.warning(f"Event replay denied for user {user_id}: {reason}. Sim ID: {sim_id}.")
                raise ValueError(f"Policy denied: {reason}")
            ...
            for record_data in records_to_replay_raw:
                try:
                    ...
                    logger.error(f"Error processing audit record during replay for validation/decryption (UUID: {record_data.get('uuid')}): {e}. Data snippet: {self.security_guard.sanitize_data(record_data)}.", exc_info=True)
                    self.safe_create_task(
                        self.feedback_manager.record_feedback(
                            user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                            details={'type': 'replay_error', 'record_id': record_data.get('uuid'), 'error': str(e)}
                        )
                    )
            
            await self._log_audit('replay_events', sim_id, user_id, {'count': len(replayed_results), 'replay_filters': db_filters})
            return replayed_results
        except Exception as e:
            AUDIT_ERRORS.labels(operation='replay_events').observe(time.time() - start_time_op)
            logger.error(f"Replay failed: {self.security_guard.sanitize_data(db_filters)}: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'audit_error', 'operation': 'replay_events', 'error': str(e), 'filters_sanitized': self.security_guard.sanitize_data(db_filters)}
                )
            )
            raise
        ...
        try:
            ...
            for record_data in all_records_data:
                try:
                    ...
                    logger.warning(f"Invalid audit record encountered during snapshot validation: {e}. Record UUID: {record_data.get('uuid')}. Data snippet: {self.security_guard.sanitize_data(record_data)}.")
                except Exception as e:
                    logger.error(f"Error processing record for snapshot: {e}. Record UUID: {record_data.get('uuid')}.", exc_info=True)

            if self.encrypter is None:
                raise RuntimeError("Audit system encrypter not initialized. Cannot create snapshot.")
            
            encrypted_records_snapshot = self.encrypter.encrypt(json.dumps(validated_records, default=safe_serialize).encode('utf-8')).decode('utf-8')
            
            await self._db_client.snapshot_audit_state(snapshot_id, encrypted_records_snapshot, user_id)
            
            await self._log_audit('snapshot_audit_state', snapshot_id, user_id, {'records_count': len(validated_records), 'snapshot_size_bytes': len(encrypted_records_snapshot.encode('utf-8'))})
            return snapshot_id
        except Exception as e:
            AUDIT_ERRORS.labels(operation='snapshot_audit_state').observe(time.time() - start_time_op)
            logger.error(f"Audit state snapshot failed: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'audit_error', 'operation': 'snapshot_audit_state', 'error': str(e)}
                )
            )
            raise
    ...
    def _start_flush_task(self):
        ...
        try:
            ...
            if loop.is_running():
                ...
                logger.warning("No running event loop found; periodic audit flush task will not start automatically. Buffer relies on size threshold.")
        except RuntimeError:
            logger.warning("No running event loop found at init; periodic audit flush task will not start automatically. Buffer relies on size threshold.")
        ...
        try:
            if self.system_audit_merkle_tree:
                ...
                audit_leaf_content = json.dumps({
                    ...
                    "details_hash": hashlib.sha256(json.dumps(details, sort_keys=True, default=safe_serialize).encode('utf-8')).hexdigest()
                ...
                self.safe_create_task(
                    self.feedback_manager.record_feedback(
                        user_id="system", feedback_type=FeedbackType.GENERAL,
                        details={'type': 'audit_event_merkle', 'event': event, 'sim_id': sim_id,
                                 'details_hash': hashlib.sha256(json.dumps(details, default=safe_serialize).encode()).hexdigest(),
                                 'merkle_root': self.system_audit_merkle_tree.get_merkle_root() if self.system_audit_merkle_tree else "N/A"}
                    )
                )
            else:
                raise ValueError("system_audit_merkle_tree not available in ExplainAudit. Merkle tree logging for meta-event cannot be performed.")
        except Exception as e:
            logger.error(f"Error in _log_audit: {self.security_guard.sanitize_data(details)}: {e}", exc_info=True)
            self.safe_create_task(
                self.feedback_manager.record_feedback(
                    user_id="system", feedback_type=FeedbackType.BUG_REPORT,
                    details={'type': 'audit_log_error', 'event': event, 'error': str(e)}
                )
            )