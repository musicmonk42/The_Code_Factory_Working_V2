...
class TrackedDict(Mapping):
    ...
    def __getitem__(self, key: str) -> Any:
            ...
            if self._is_metrics:
                if self._metrics_counter is None:
                    self._metrics_counter = get_or_create_counter(
                        'omnicore_scenario_metrics_accessed_total', 'Total accesses to scenario metrics', ['metric_name']
                    )
                ...
                if self._templates_counter is None:
                    self._templates_counter = get_or_create_counter(
                        'omnicore_scenario_templates_accessed_total', 'Total accesses to scenario templates', ['template_name']
                    )
        ...
        try:
            ...
            logger.warning(f"Attempted to access non-existent key '{key}' in {'metrics' if self._is_metrics else 'templates'}")
            raise KeyError(f"Key '{key}' not found in {'metrics' if self._is_metrics else 'templates'}")
