import logging 
import asyncio
from typing import Any, Dict, Optional, List, Type, Coroutine, Callable
...
def get_plugin_metrics() -> dict: 
    ...
    try:
        ...
        return {"error": "Metrics system not available.", "message": "Failed to retrieve plugin metrics"}
...
def get_test_metrics() -> dict: 
    ...
    try:
        ...
        return {"error": "Metrics system not available.", "message": "Failed to retrieve test metrics"}
    except Exception as e:
...
class ExplainableAI:
    ...
    async def explain_event(self, event_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.reasoner:
            self.logger.warning("Explainable AI core is not available. Returning dummy explanation.")
...
class OmniCoreEngine:
    ...
    async def initialize(self):
        ...
        class PluginRegistryComponent(Base):
            ...
            async def health_check(self) -> Dict[str, Any]:
                return {"status": "ok", "plugins_loaded": sum(len(k) for k in self.registry.plugins.values())}

            @property
            def is_healthy(self) -> bool:
                return self.is_started
        
        await self._initialize_component_instance("plugin_registry", PluginRegistryComponent, 
                                                  settings=self.settings, registry=PLUGIN_REGISTRY, observer=start_plugin_observer)
    ...
    async def _initialize_component_instance(self, name: str, component_class: Type[Base], *args, **kwargs) -> Base:
    ...
    async def perform_task(self, task_name: str, **kwargs) -> Any:
        ...
        plugin_instance = PLUGIN_REGISTRY.get_plugin_for_task(task_name) if hasattr(PLUGIN_REGISTRY, 'get_plugin_for_task') else None
        ...
        if plugin_instance:
            try:
                ...
                logger.error(f"Error performing task '{task_name}' via plugin: {e}", exc_info=True)
                return None
