...
class BackpressureManager:
    ...
    async def check_and_notify(self, shard_id: int):
        ...
        should_pause = (current_queue_size / max_size) >= self.threshold or (
            current_hp_queue_size / max_size
        ) >= self.threshold
        ...
        if should_pause and not self.is_paused[shard_id]:
            ...
            try:
                ...
                logger.warning(
                    "Backpressure detected and publish paused.",
                    shard_id=shard_id,
                    queue_size=current_queue_size,
                    hp_queue_size=current_hp_queue_size,
                )
            ...
            try:
                ...
                logger.info(
                    "Backpressure alleviated and publish resumed.",
                    shard_id=shard_id,
                    queue_size=current_queue_size,
                    hp_queue_size=current_hp_queue_size,
                )
                ...
                logger.error(
                    f"Failed to publish backpressure resumption notification or resume publishes: {e}"
                )
