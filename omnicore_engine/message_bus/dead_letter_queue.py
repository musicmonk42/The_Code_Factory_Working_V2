...
class DeadLetterQueue:
    def __init__(self, db: Database, kafka_bridge: Optional['KafkaBridge'], priority_threshold: int):
        self.db = db
    ...
    async def add(self, message: Message, error: str):
        ...
        logger.error("Adding message to DLQ", topic=message.topic, trace_id=message.trace_id, error=full_error)
        ...
        try:
            ...
            logger.error(f"Failed to persist DLQ message to database: {e}", trace_id=message.trace_id)

    async def _process_dlq(self):
        while self.running:
            try:
                message, error, retries = await self.queue.get()
                logger.info("DLQ consumer received message (in-memory).", topic=message.topic, trace_id=message.trace_id, current_retries=retries)
                ...
                if self.kafka_bridge:
                    ...
                    if self.kafka_bridge.circuit.can_attempt():
                        try:
                            ...
                            logger.info("DLQ message published to Kafka bridge.", trace_id=message.trace_id)
                        except Exception as e:
                            self.kafka_bridge.circuit.record_failure()
                            logger.error(f"Failed to publish DLQ message to Kafka: {e}", trace_id=message.trace_id)
                            ...
                            if retries < self.max_retries:
                                ...
                                logger.warning(f"Re-queued DLQ message for retry.", trace_id=message.trace_id, next_retry=retries + 1)
                            else:
                                logger.critical(f"DLQ message failed to process after {self.max_retries} attempts. Dropping message.", trace_id=message.trace_id, error=error)
                    else:
                        logger.warning("Kafka circuit is open. Skipping DLQ message publish to Kafka.", trace_id=message.trace_id)
                elif KAFKA_AVAILABLE and self.kafka_bridge is None:
                    logger.warning("Kafka is available but the Kafka bridge is not initialized. Skipping DLQ publish.")
                ...
                logger.error(f"Unexpected error in DLQ processing loop: {e}. Sleeping before next attempt.", exc_info=True)
