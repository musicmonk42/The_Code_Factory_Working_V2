...
class ShardedMessageBus:
    def __init__(
        ...
        self.queues = [asyncio.PriorityQueue(maxsize=self.max_queue_size) for _ in range(self.shard_count)]
        self.high_priority_queues = [asyncio.PriorityQueue(maxsize=self.max_queue_size) for _ in range(self.shard_count)]

        self.executors = [
            ThreadPoolExecutor(max_workers=self.workers_per_shard, thread_name_prefix=f"msgbus-normal-shard-{i}")
            for i in range(self.shard_count)
        ]
        self.high_priority_executors = [
            ThreadPoolExecutor(max_workers=max(1, self.workers_per_shard // 2), thread_name_prefix=f"msgbus-hp-shard-{i}")
            for i in range(self.shard_count)
        ]
        self.callback_executors = [
            ThreadPoolExecutor(max_workers=getattr(self.config, 'message_bus_callback_workers', 8), thread_name_prefix=f"msgbus-callbacks-{i}")
            for i in range(self.shard_count)
        ]

        self.subscribers: Dict[str, List[Tuple[Callable[[Message], None], Optional[Any]]]] = defaultdict(list)
        self.regex_subscribers: Dict[Pattern, List[Tuple[Callable[[Message], None], Optional[Any]]]] = defaultdict(list)
        ...
        self.kafka_bridge = KafkaBridge(self, self.config, self.kafka_circuit) if getattr(self.config, 'USE_KAFKA', False) else None
        self.redis_bridge = RedisBridge(self, self.config, self.redis_circuit) if getattr(self.config, 'USE_REDIS', False) else None
        
        # Operational components
        self.dlq = DeadLetterQueue(db, self.kafka_bridge, getattr(self.config, 'DLQ_PRIORITY_THRESHOLD', 5))
        self.backpressure_manager = BackpressureManager(self, threshold=getattr(self.config, 'BACKPRESSURE_THRESHOLD', 0.8))
        ...
        try:
            ...
            logger.error("ShardedMessageBus must be initialized within a running asyncio event loop.")
            raise RuntimeError("ShardedMessageBus must be initialized within a running asyncio event loop.")

        self.dispatcher_tasks = []
        self._start_dispatchers()
        if self.kafka_bridge: self.kafka_bridge.start()
        if self.redis_bridge: self.redis_bridge.start()
        if self.guardian: self.guardian.start()
    ...
    def _start_dispatchers(self) -> None:
        ...
        for shard_id in range(self.shard_count):
            task = asyncio.create_task(self._dispatcher_loop(shard_id, self.queues[shard_id], self.executors[shard_id], high_priority=False))
            self.dispatcher_tasks.append(task)
            task = asyncio.create_task(self._dispatcher_loop(shard_id, self.high_priority_queues[shard_id], self.high_priority_executors[shard_id], high_priority=True))
            self.dispatcher_tasks.append(task)
        logger.info("Dispatcher tasks started.", num_tasks=len(self.dispatcher_tasks))

    async def _dispatcher_loop(self, shard_id: int, queue: asyncio.PriorityQueue, executor: ThreadPoolExecutor, high_priority: bool) -> None:
        queue_type = "high_priority" if high_priority else "normal"
        logger.info(f"Starting dispatcher loop.", shard_id=shard_id, queue_type=queue_type)
        while self.running:
            try:
                ...
                with MESSAGE_BUS_DISPATCH_DURATION.labels(shard_id=str(shard_id)).time():
                    await self._dispatch_message_to_subscribers_and_externals(message, self.callback_executors[shard_id])
                queue.task_done()
                logger.debug("Message dispatched and queue task done.", topic=message.topic, trace_id=message.trace_id)
                ...
                logger.error(f"Error in dispatcher loop.", shard_id=shard_id, queue_type=queue_type, error=e)
    ...
    async def _safe_callback_internal(self, callback: Callable[[Message], Any], message: Message, filter: Optional[Any] = None) -> None:
        callback_name = getattr(callback, '__name__', getattr(callback, '__class__', None).__name__ if hasattr(callback, '__class__') else str(callback))
        logger_for_callback = logger.bind(trace_id=message.trace_id, topic=message.topic, callback_name=callback_name)

        if filter and not filter.apply(message.payload):
            logger_for_callback.debug("Message filtered out by subscriber.", filter=filter.__class__.__name__)
            return

        try:
            with MESSAGE_BUS_CALLBACK_LATENCY.labels(topic=message.topic, callback=callback_name).time():
    ...
    async def _dispatch_message_to_subscribers_and_externals(self, message: Message, callback_executor: ThreadPoolExecutor) -> None:
        ...
        for hook in self.post_publish_hooks:
            try:
                ...
                logger_for_dispatch.debug("Post-publish hook executed.", hook=getattr(hook, '__name__', str(hook)))
            except Exception as e:
                logger_for_dispatch.error(f"Error in post-publish hook: {e}", hook=getattr(hook, '__name__', str(hook)))
    ...
    async def _publish_to_shard(self, shard_id: int, message: Message, retries: int = 3) -> bool:
        ...
        logger_for_publish = logger.bind(trace_id=message.trace_id, topic=message.topic, shard_id=shard_id)
        
        policy = self.retry_policies.get(message.topic, self.retry_policies.get("default", RetryPolicy()))
        ...
        async with self.shard_locks[shard_id]:
            for attempt in range(actual_retries + 1):
                try:
                    queue_to_use = self.high_priority_queues[shard_id] if message.priority >= getattr(self.config, 'priority_threshold', 5) else self.queues[shard_id]
                    await queue_to_use.put((message.priority, message))
                    
                    if message.idempotency_key:
                        self.dedup_cache.put(message.idempotency_key, message.trace_id)
                        if self.redis_bridge:
                            await self.redis_bridge.set_dedup_cache(message.idempotency_key, message.trace_id)
                    ...
                    if self.audit_client:
                        await self.audit_client.add_entry_async("message_published", "message_bus", {"topic": message.topic, "trace_id": message.trace_id, "priority": message.priority, "encrypted": message.encrypted, "idempotency_key": message.idempotency_key}, agent_id="ShardedMessageBus")
                    ...
                    logger_for_publish.warning("Message bus queue full. Retrying...", attempt=attempt + 1, max_attempts=actual_retries + 1)
                    if attempt == actual_retries:
                        logger_for_publish.error(f"Message bus queue full for shard {shard_id}, topic {message.topic} after {actual_retries + 1} attempts.")
                        if self.redis_bridge and await self.redis_bridge.publish(message):
                            ...
                            if self.audit_client:
                                await self.audit_client.add_entry_async("message_published_redis_fallback", "message_bus", {"topic": message.topic, "trace_id": message.trace_id, "priority": message.priority}, agent_id="ShardedMessageBus")
                            return True
                        await self.dlq.add(message, f"Failed to publish after {actual_retries + 1} attempts, queue full.")
                        return False
                    ...
                    if self.audit_client:
                        ...
                        await self.audit_client.add_entry_async("message_publish_unexpected_error", "message_bus",
                            {"topic": message.topic, "error": str(e)}, error=str(e), agent_id="ShardedMessageBus")
                    await self.dlq.add(message, f"Unexpected publish error: {e}")
                    return False


    async def publish(
        self, topic: str, payload: Dict[str, Any], priority: int = 0, retries: int = 3, trace_id: Optional[str] = None, idempotency_key: Optional[str] = None,
        encrypt: bool = False, context: Optional[Dict[str, Any]] = None, client_id: Optional[str] = 'default', signature: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if self.audit_client:
                await self.audit_client.add_entry_async("rate_limit_exceeded", "message_bus", {"client_id": client_id}, error=str(e), agent_id="ShardedMessageBus")
            return False
        ...
        try:
            ...
            logger.warning(f"Publish request denied: {e}")
            if self.audit_client:
                await self.audit_client.add_entry_async("message_size_exceeded", "message_bus", {"client_id": client_id}, error=str(e), agent_id="ShardedMessageBus")
            return False
        ...
        if signature:
            ...
            if signing_key is None:
                logger.warning("Message signature provided but MESSAGE_SIGNING_KEY is not configured. Aborting.")
                if self.audit_client:
                    await self.audit_client.add_entry_async("missing_signing_key", "message_bus", {"topic": topic}, error="Missing signing key", agent_id="ShardedMessageBus")
                return False
            if not verify_message(message, signature, signing_key.get_secret_value().encode('utf-8')):
                logger.warning("Message signature verification failed. Aborting publish.")
                if self.audit_client:
                    await self.audit_client.add_entry_async("message_signature_failed", "message_bus", {"topic": topic}, error="Signature mismatch", agent_id="ShardedMessageBus")
                return False
        ...
        if topic.startswith("requests.arbiter"):
            async with aiohttp.ClientSession() as session:
                try:
                    await session.post(self.config.ARBITER_URL + "/events", json={"event_type": topic, "data": payload})
        ...
        async with self._publish_lock:
            ...
            if idempotency_key:
                if self.dedup_cache.get(idempotency_key) is not None:
                    logger_for_publish.info("Duplicate message detected (deduplication cache hit). Skipping publish.", idempotency_key=idempotency_key)
                    return True
            ...
            if encrypt:
                ...
                try:
                    ...
                    if self.audit_client:
                        await self.audit_client.add_entry_async("message_encryption_failed", "message_bus", {"topic": topic, "error": str(e)}, error=str(e), agent_id="ShardedMessageBus")
                    return False
            else:
                processed_payload = json.dumps(payload)

            message = Message(
                topic=topic, payload=processed_payload, priority=priority, timestamp=time.time(),
                trace_id=current_trace_id, encrypted=encrypt, idempotency_key=idempotency_key,
            ...
            for hook in self.pre_publish_hooks:
                try:
                    ...
                    logger_for_publish.error(f"Error in pre-publish hook: {e}. Aborting publish.", hook=getattr(hook, '__name__', str(hook)))
                    if self.audit_client:
                        await self.audit_client.add_entry_async("pre_publish_hook_failed", "message_bus", {"topic": topic, "error": str(e)}, error=str(e), agent_id="ShardedMessageBus")
                    return False
            ...
            if getattr(self.config, 'ENHANCED_PLUGIN_VALIDATION', False):
                try:
                    ...
                    if message.encrypted and self.encryption:
                        ...
                        validation_payload_dict = json.loads(message.payload) # message.payload is JSON string
                    
                    MessageSchema(topic=message.topic, payload=safe_serialize(validation_payload_dict), priority=message.priority, trace_id=message.trace_id, idempotency_key=message.idempotency_key, context=message.context)
                except ValidationError as e:
                    logger_for_publish.error(f"Invalid message payload: {e}")
                    if self.audit_client:
                        await self.audit_client.add_entry_async("message_validation_failed", "message_bus", {"topic": topic, "error": str(e)}, error=str(e), agent_id="ShardedMessageBus")
                    return False
                except Exception as e:
    ...
    async def _subscribe_async(self, topic: Union[str, Pattern], callback: Callable[[Message], None], filter: Optional[Any] = None) -> None:
        async with self._subscriber_lock:
            logger_for_subscribe = logger.bind(topic=str(topic), callback=getattr(callback, '__name__', str(callback)))
    ...
    async def _unsubscribe_async(self, topic: Union[str, Pattern], callback: Callable[[Message], None]) -> None:
        async with self._subscriber_lock:
            ...
            if isinstance(topic, str):
                ...
                self.regex_subscribers[topic] = list(filter(filter_out_callback, self.regex_subscribers[topic]))
                if len(self.regex_subscribers[topic]) < initial_len:
                    logger_for_unsubscribe.info("Unsubscribed callback from regex pattern.")
                else:
                    logger_for_unsubscribe.warning("Callback not found for unsubscribe from regex pattern.")

    async def request(self, topic: str, payload: Any, timeout: float = 5.0, priority: int = 5) -> Any:
        ...
        def handle_reply(message: Message):
            if not response_future.done():
                ...
                logger_for_request.warning("Received late reply for request.", reply_topic=message.topic)

        self.subscribe(reply_topic, handle_reply)
        try:
            await self.publish(topic, payload, trace_id=f"request-response-{reply_topic}", priority=priority)
    ...
    async def _periodic_rebalance_check(self, interval: int = 60) -> None:
        ...
        while self.running and self.dynamic_shards_enabled:
            ...
            avg_load = (sum(queue_loads) + sum(hp_queue_loads)) / (len(queue_loads) + len(hp_queue_loads))
            max_size = self.max_queue_size
            
            if avg_load > max_size * getattr(self.config, 'message_bus_scale_up_threshold', 0.8):
                logger.info(f"Average queue load ({avg_load}) is above threshold. Considering adding a new shard.")
                await self.add_shard()
            elif avg_load < max_size * getattr(self.config, 'message_bus_scale_down_threshold', 0.2) and self.shard_count > 1:
                logger.info(f"Average queue load ({avg_load}) is below threshold. Considering removing a shard.")
                # We can't just remove the last shard; we need to decide which one.
                # For simplicity, remove the shard with the lowest load.
                target_shard_id = min(range(self.shard_count), key=lambda i: queue_loads[i] + hp_queue_loads[i])
    ...
    async def add_shard(self):
        ...
        self.executors.append(ThreadPoolExecutor(max_workers=self.workers_per_shard, thread_name_prefix=f"msgbus-normal-shard-{new_shard_id}"))
        self.high_priority_executors.append(ThreadPoolExecutor(max_workers=self.workers_per_shard, thread_name_prefix=f"msgbus-hp-shard-{new_shard_id}"))
        self.callback_executors.append(ThreadPoolExecutor(max_workers=self.workers_per_shard, thread_name_prefix=f"msgbus-callbacks-{new_shard_id}"))
        self.shard_locks.append(OrderedLock(new_shard_id))

        self.dispatcher_tasks.append(asyncio.create_task(self._dispatcher_loop(new_shard_id, self.queues[-1], self.executors[-1], high_priority=False)))
        self.dispatcher_tasks.append(asyncio.create_task(self._dispatcher_loop(new_shard_id, self.high_priority_queues[-1], self.high_priority_executors[-1], high_priority=True)))
        
        self.hash_ring.add_node_dynamic(str(new_shard_id), partial(self._rebalance_callback, old_shard_count=self.shard_count))
        self.shard_count += 1
    ...
    async def remove_shard(self, shard_id: int):
        ...
        await asyncio.gather(
            self.dispatcher_tasks[shard_id*2], self.dispatcher_tasks[shard_id*2+1], return_exceptions=True
        )
        ...
        self.hash_ring.remove_node_dynamic(str(shard_id), partial(self._rebalance_callback, old_shard_count=self.shard_count))
    ...
    async def adjust_shards(self, target_shard_count: int) -> None:
        ...
        if target_shard_count == self.shard_count:
            return
        logger.info("Adjusting shards.", old_count=self.shard_count, new_count=target_shard_count)
        ...
        if buffered_messages:
            ...
            for priority, message in buffered_messages:
                success = await self.publish(
                    message.topic, message.payload, trace_id=message.trace_id, priority=priority,
                    encrypt=message.encrypted, idempotency_key=message.idempotency_key
                )
                if success:
                    republished_count += 1
            logger.info(f"Re-published {republished_count} messages out of {len(buffered_messages)}.")
    ...
    async def adjust_workers(self, shard_id: int, target_workers: int) -> None:
        if not (0 <= shard_id < self.shard_count):
            logger.error("Invalid shard ID for worker adjustment.", shard_id=shard_id, shard_count=self.shard_count)
            return
        ...
        if target_workers == current_normal_workers and max(1, target_workers // 2) == current_hp_workers:
            return

        logger.info("Adjusting workers for shard.", shard_id=shard_id, old_normal=current_normal_workers, new_normal=target_workers, old_hp=current_hp_workers, new_hp=max(1, target_workers // 2))

        self.executors[shard_id].shutdown(wait=True)
        self.high_priority_executors[shard_id].shutdown(wait=True)
        self.callback_executors[shard_id].shutdown(wait=True)

        self.executors[shard_id] = ThreadPoolExecutor(max_workers=target_workers, thread_name_prefix=f"msgbus-normal-shard-{shard_id}")
        self.high_priority_executors[shard_id] = ThreadPoolExecutor(max_workers=max(1, target_workers // 2), thread_name_prefix=f"msgbus-hp-shard-{shard_id}")
        self.callback_executors[shard_id] = ThreadPoolExecutor(max_workers=getattr(self.config, 'message_bus_callback_workers', 8), thread_name_prefix=f"msgbus-callbacks-{shard_id}")
    ...
    async def configure_for_omnicore(self, engine_type: str) -> 'ShardedMessageBus':
        ...
        if (new_shard_count != self.shard_count or new_workers_per_shard != self.workers_per_shard or
            new_max_queue_size != self.max_queue_size or new_callback_workers != getattr(self.config, 'message_bus_callback_workers', 8)):
            
            logger.info("Detected configuration changes. Re-initializing message bus.",
                        old_config={"shards": self.shard_count, "workers": self.workers_per_shard, "queue_size": self.max_queue_size, "callback_workers": getattr(self.config, 'message_bus_callback_workers', 8)},
                        new_config={"shards": new_shard_count, "workers": new_workers_per_shard, "queue_size": new_max_queue_size, "callback_workers": new_callback_workers})
        ...
        logger.info(f"Message bus configured for {engine_type}.",
                    shard_count=self.shard_count,
                    workers_per_shard=self.workers_per_shard,
    ...
    def _simulation_hook(self, message: Message) -> Message:
        if message.topic.startswith("simulation.") and isinstance(message.payload, dict) and "simulation_id" not in message.payload:
            try:
                ...
                if sim_id:
                    ...
                    logger.debug("Added simulation_id to message payload.", sim_id=sim_id, trace_id=message.trace_id)
            except ImportError:
                logger.warning("Could not import get_current_simulation_id for simulation hook.", trace_id=message.trace_id)
            except Exception as e:
    ...
    async def shutdown(self) -> None:
        ...
        if self.kafka_bridge: await self.kafka_bridge.shutdown()
        if self.redis_bridge: await self.redis_bridge.shutdown()
        ...
        for i, q in enumerate(self.queues): await q.join()
        for i, q in enumerate(self.high_priority_queues): await q.join()
        logger.info("All internal message queues drained.")

        for executor_list in [self.executors, self.high_priority_executors, self.callback_executors]:
        ...
        if self.guardian: await self.guardian.shutdown()
