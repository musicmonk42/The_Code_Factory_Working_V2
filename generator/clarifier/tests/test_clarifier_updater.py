...
mock_fernet_instance.encrypt.side_effect = lambda data: base64.b64encode(b"ENCRYPTED_" + (data if isinstance(data, bytes) else data.encode()))
mock_fernet_instance.decrypt.side_effect = lambda data: base64.b64decode(data).replace(b"ENCRYPTED_", b"")
...
patcher_otel_tracer_provider = patch('opentelemetry.sdk.trace.TracerProvider', return_value=mock_trace_provider)
patcher_otel_batch_processor = patch('opentelemetry.sdk.trace.export.BatchSpanProcessor', return_value=mock_span_processor)
patcher_otel_set_tracer = patch('opentelemetry.trace.set_tracer_provider')
patcher_otel_exporter = patch('opentelemetry.exporter.otlp.proto.grpc.trace_exporter.OTLPSpanExporter', return_value=MagicMock())
...
patcher_clarifier_get_config = patch('generator.clarifier.clarifier.get_config', return_value=mock_config_instance)
patcher_clarifier_get_fernet = patch('generator.clarifier.clarifier.get_fernet', return_value=mock_fernet_instance)
patcher_clarifier_get_logger = patch('generator.clarifier.clarifier.get_logger', return_value=mock_logger)
...
patcher_get_config = patch('generator.clarifier.clarifier_updater.get_config', return_value=mock_config_instance)
patcher_get_fernet = patch('generator.clarifier.clarifier_updater.get_fernet', return_value=mock_fernet_instance)
patcher_get_logger = patch('generator.clarifier.clarifier_updater.get_logger', return_value=mock_logger)
patcher_log_action = patch('generator.clarifier.clarifier_updater.log_action', side_effect=dummy_log_action)
...
patcher_redact_sensitive = patch('generator.clarifier.clarifier_updater.redact_sensitive', side_effect=lambda x: str(x).replace('SECRET123', '[REDACTED_API_KEY]').replace('admin@example.com', '[REDACTED_EMAIL]').replace('user@example.com', '[REDACTED_EMAIL]'))
...
class TestRequirementsUpdater(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_hash_chain_verification(self):
        ...
        hashable_data['prev_hash'] = "genesis_hash_placeholder" # Ensure prev_hash is in hashable data
    ...
    async def test_update_with_correlation_id(self):
        ...
        with patch.object(self.updater, '_infer_updates', AsyncMock(return_value={
            ...
            result = await self.updater.update(
                requirements, ambiguities, answers,
    ...
    async def test_update_error_handling(self):
        ...
        # Make inference fail
        with patch.object(self.updater, '_infer_updates', AsyncMock(side_effect=Exception("Inference failed"))):
...
class TestConvenienceFunction(unittest.IsolatedAsyncioTestCase):
    ...
    def test_update_requirements_with_answers_sync_path(self, MockRequirementsUpdater, mock_asyncio_run):
        ...
        mock_updater_instance.update = AsyncMock(return_value={"features": ["f1"], "clarifications": {}})
