...
mock_fernet_instance.encrypt.side_effect = lambda data: b"ENCRYPTED_" + (data if isinstance(data, bytes) else data.encode())
...
patcher_fernet_class = patch('generator.clarifier.clarifier.Fernet', return_value=mock_fernet_instance)
...
patcher_get_config_clarifier = patch('generator.clarifier.clarifier.get_config', return_value=mock_config_instance)
patcher_get_fernet_clarifier = patch('generator.clarifier.clarifier.get_fernet', return_value=mock_fernet_instance)
patcher_get_logger_clarifier = patch('generator.clarifier.clarifier.get_logger', return_value=mock_logger)
patcher_get_tracer_clarifier = patch('generator.clarifier.clarifier.get_tracer', return_value=(mock_tracer, MockStatus, MockStatusCode))
patcher_get_cb_clarifier = patch('generator.clarifier.clarifier.get_circuit_breaker', return_value=mock_circuit_breaker)
...
patcher_get_config_prompt = patch('generator.clarifier.clarifier_prompt.get_config', return_value=mock_config_instance)
patcher_get_fernet_prompt = patch('generator.clarifier.clarifier_prompt.get_fernet', return_value=mock_fernet_instance)
patcher_get_logger_prompt = patch('generator.clarifier.clarifier_prompt.get_logger', return_value=mock_logger)
patcher_get_tracer_prompt = patch('generator.clarifier.clarifier_prompt.get_tracer', return_value=(mock_tracer, MockStatus, MockStatusCode))
patcher_get_cb_prompt = patch('generator.clarifier.clarifier_prompt.get_circuit_breaker', return_value=mock_circuit_breaker)
...
patcher_get_config_updater = patch('generator.clarifier.clarifier_updater.get_config', return_value=mock_config_instance)
patcher_get_fernet_updater = patch('generator.clarifier.clarifier_updater.get_fernet', return_value=mock_fernet_instance)
patcher_get_logger_updater = patch('generator.clarifier.clarifier_updater.get_logger', return_value=mock_logger)
...
patcher_get_config_user = patch('generator.clarifier.clarifier_user_prompt.get_config', return_value=mock_config_instance)
patcher_get_fernet_user = patch('generator.clarifier.clarifier_user_prompt.get_fernet', return_value=mock_fernet_instance)
patcher_get_logger_user = patch('generator.clarifier.clarifier_user_prompt.get_logger', return_value=mock_logger)
...
patcher_redact_sensitive_user = patch('generator.clarifier.clarifier_user_prompt.redact_sensitive', side_effect=lambda x: x)
patcher_log_action_user = patch('generator.clarifier.clarifier_user_prompt.log_action', return_value=None)
...
patcher_log_action = patch('generator.clarifier.clarifier_updater.log_action', new_callable=AsyncMock)
patcher_send_alert = patch('generator.clarifier.clarifier_updater.send_alert', new_callable=AsyncMock)
patcher_redact_sensitive = patch('generator.clarifier.clarifier_updater.redact_sensitive', side_effect=lambda x: x.replace('SECRET', '[REDACTED]').replace('@', '[REDACTED_EMAIL]'))
...
class TestEndToEndClarification(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        from generator.clarifier.clarifier_user_prompt import get_channel, PROFILE_DIR
    ...
    async def asyncTearDown(self):
        ...
        for f in os.listdir('.'):
            if 'integration' in f and (f.endswith('.tmp') or f.endswith('.json') or f.endswith('.db')):
                try:
                    os.unlink(f)
                except:
                    pass
    ...
    @patch('builtins.input', side_effect=["user's clarification answer", "Markdown, PDF", "yes", "no", "yes", "EU", "no"])
    async def test_full_clarification_workflow(self, mock_input):
        ...
        with patch('generator.clarifier.clarifier.GrokLLM') as MockLLM, \
             patch('generator.clarifier.clarifier.DefaultPrioritizer') as MockPrioritizer:
            ...
            mock_prioritizer.prioritize = AsyncMock(return_value={
                "prioritized": [{
            ...
            with patch.object(self.RequirementsUpdater, '_infer_updates', AsyncMock(return_value={
                "inferred_features": ["login_system", "password_hashing", "session_management"],
                ...
                clarifier.context_manager = SQLiteContextManager(self.temp_db.name, mock_fernet_instance)
    ...
    async def test_prompt_clarifier_integration(self):
        ...
        with patch('generator.clarifier.clarifier.GrokLLM'), \
             patch('generator.clarifier.clarifier.DefaultPrioritizer') as MockPrioritizer, \
            ...
            with patch.object(self.RequirementsUpdater, '_infer_updates', AsyncMock(return_value={
                ...
                # Initialize context manager
                from generator.clarifier.clarifier import SQLiteContextManager
                prompt_clarifier.core_clarifier.context_manager = SQLiteContextManager(self.temp_db.name, mock_fernet_instance)
                ...
                try:
                    ...
                    result = await prompt_clarifier.get_clarifications(ambiguities, requirements, user_context)
    ...
    async def test_concurrent_clarifications(self):
        ...
        with patch('generator.clarifier.clarifier.GrokLLM'), \
            ...
            for i in range(3):
                ...
                from generator.clarifier.clarifier import SQLiteContextManager
                c.context_manager = SQLiteContextManager(f"{self.temp_db.name}_{i}", mock_fernet_instance)
...
class TestComponentInteractions(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_clarifier_to_updater_flow(self):
        ...
        with patch('generator.clarifier.clarifier.GrokLLM'), \
            ...
            with patch.object(RequirementsUpdater, '_infer_updates', AsyncMock(return_value={
