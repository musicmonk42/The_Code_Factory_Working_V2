...
from typing import Dict, Any, List, Callable, Union, Optional, Awaitable
...
def requirements_summary(state: Dict[str, Any], max_length: int = 2000) -> str:
    ...
    if 'features' in reqs:
        summary_parts.append(f"Key Features: {', '.join(reqs['features'][:3])}{'...' if len(reqs['features']) > 3 else ''}")
...
async def llm_summarize(
    ...
    try:
        ...
        await log_audit_event(
            ...
            data={"model": model, "text_length": len(text_to_summarize), "context": context}
        )
        ...
        if not summary:
            logger.warning(f"LLM summarizer returned empty content for model {model}. Falling back to truncation.")
            return text_to_summarize[:max_length]
            
        return summary.strip()

    except Exception as e:
        logger.error(f"LLM-based summarization failed: {e}. Falling back to simple truncation.", exc_info=True)
...
async def ensemble_summarize(
    ...
    if not valid_summaries:
        logger.error("Ensemble summarization failed: No valid summaries returned from any provider.")
...
def refine_from_feedback(
    ...
    if rating < 0.3:
        logger.warning(f"Low rating ({rating}) for summary from {provider_name} on {template_name}. Triggering alert.")
...
class TestSummarizeUtils(unittest.TestCase):
    ...
    def test_code_summary_sync(self):
        ...
        state = {
            ...
            'critique_results': {'semantic_alignment_score': 0.9, 'test_quality_score': 0.8}
        }
    ...
    def test_ensemble_summarize(self, mock_call_llm_api):
        ...
        ensemble_summary = asyncio.run(ensemble_summarize(text, providers=['summarizer_a', 'summarizer_b'], max_length=300))
