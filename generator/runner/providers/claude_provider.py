...
class ClaudeProvider(LLMProvider):
    ...
    async def _api_call(self, model: str, processed_prompt: str, stream: bool, **kwargs):
        ...
        try:
            if model in self.custom_models:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    if stream:
                        logger.warning(
                            f"Streaming not fully supported for custom Claude endpoint '{model}'. Returning full response."
                        )

                    async with session.post(
                        custom["endpoint"], json=payload, headers=headers
                    ) as resp:
                        if resp.status != 200:
                            raise APIConnectionError(
                                f"Custom endpoint error: {resp.status} - {await resp.text()}"
                            )
        ...
        # --- SDK Error Translation ---
            ...
            raise LLMError(
                detail="Rate limit exceeded. Check Anthropic dashboard for limits.",
                provider=self.name,
            ) from e
        except APIConnectionError as e:
            raise LLMError(
                detail="Connection error. Check network or API endpoint status.",
                provider=self.name,
            ) from e
        except AnthropicError as e:
            error_type = type(e).__name__
            raise LLMError(
                detail=f"Anthropic API error ({error_type}): {str(e)}",
                provider=self.name,
            ) from e
        except Exception as e:
            # Catch-all for unexpected errors during the call
            raise LLMError(
                detail=f"Unexpected error in Claude SDK: {str(e)}", provider=self.name
            ) from e
...
if __name__ == "__main__":
    ...
    class TestClaudeProvider(unittest.IsolatedAsyncioTestCase):
        ...
        @patch(
            ...
            return_value=MagicMock(llm_provider_api_key=""),
        )
        async def test_call_non_stream(self, mock_config_load):
            ...
            with patch(
                "anthropic.AsyncAnthropic.messages.create", new_callable=AsyncMock
            ) as mock_create:
    ...
    parser.add_argument(
        "--model", type=str, default="claude-3-haiku-20240307", help="Claude model name"
    )
