...
import aiohttp
from openai import AsyncOpenAI, OpenAIError, APIConnectionError, RateLimitError, AuthenticationError # <-- ADDED SDK ERRORS
...
class OpenAIProvider(LLMProvider):
    ...
    async def _api_call(self, model: str, messages: list, stream: bool, **kwargs):
        ...
        # --- Translate SDK-specific errors to general LLMErrors ---
            ...
            raise LLMError(detail="Rate limit exceeded. Check OpenAI dashboard for limits.", provider=self.name) from e
        except APIConnectionError as e:
            raise LLMError(detail="Connection error. Check network or custom endpoint.", provider=self.name) from e
        except OpenAIError as e:
            raise LLMError(detail=f"OpenAI API error: {str(e)}", provider=self.name) from e
        except Exception as e:
            raise LLMError(detail=f"Unexpected error in OpenAI SDK: {str(e)}", provider=self.name) from e
