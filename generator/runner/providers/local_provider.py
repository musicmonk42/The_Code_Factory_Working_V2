...
PRICING: Dict[str, Dict[str, float]] = (
    {}
)  # User can populate: model: {'input': cost_per_token, 'output': cost_per_token}
...
class LocalProvider(LLMProvider):
    ...
    async def call(
        ...
        model_config = self.custom_models.get(
            model, {"endpoint": "http://localhost:11434/api/generate", "headers": {}}
        )
    ...
    async def count_tokens(self, text: str, model: str) -> int:
        ...
        if model_config and "token_counter" in model_config:
            ...
            try:
                ...
                logger.warning(
                    f"Custom token_counter for model '{model}' failed: {e}. Falling back to default."
                )
