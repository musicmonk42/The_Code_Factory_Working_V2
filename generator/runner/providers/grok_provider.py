    ...
    async def _api_call(self, endpoint: str, headers: Dict[str, str], data: Dict[str, Any], stream: bool, run_id: str):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                logger.warning(f"Grok API call failed (ClientError): {e}. Retrying...", extra={'run_id': run_id})
    ...
    async def health_check(self) -> bool:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get("https://api.x.ai/v1/models", headers=headers, timeout=5) as resp:
