...
from runner.runner_errors import RunnerError, LLMError, ConfigurationError
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        if redis_url:
            try:
                ...
                logger.error(f"CacheManager: Failed to connect to Redis at {redis_url}. Falling back to in-memory. Error: {e}")
                self.redis = None
    ...
    async def get(self, key: str) -> Optional[Any]:
        if self.redis:
            try:
                ...
                logger.error(f"CacheManager: Redis GET failed. Falling back to in-memory. Error: {e}")
...
class CircuitBreaker:
    ...
    async def allow_request(self, provider: str) -> bool:
        ...
        if time.time() - self.last_failure.get(provider, 0) > self.timeout:
            ...
            logger.info(f"CircuitBreaker: State for {provider} is now HALF-OPEN. Allowing trial request.")
            return True
        return False
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                await log_audit_event(
                    action="llm_call",
                    data={'provider': provider, 'model': model, 'input_tokens': input_tokens, 'output_tokens': output_tokens}
                )
