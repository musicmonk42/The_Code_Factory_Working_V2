...
from runner.runner_backends import LocalBackend
...
class TestRunnerIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.patch_save_files = patch(
            'runner.runner_core.Runner._save_files_to_temp_dir', 
        ...
        self.patch_subprocess = patch(
            'runner.runner_core.subprocess_wrapper', 
            new_callable=AsyncMock
        )
        ...
        self.patch_parse_junit = patch(
            'runner.runner_parsers.parse_junit_xml', 
            new_callable=AsyncMock
        )
        ...
        self.patch_log_audit = patch('runner.runner_logging.log_audit_event', new_callable=AsyncMock)
        ...
        self.patch_backend_execute = patch(
            'runner.runner_backends.LocalBackend.execute', 
            new_callable=AsyncMock
        )
    ...
    async def test_full_successful_run(self):
        ...
        self.assertEqual(
            self.mock_log_audit_event.call_args[1]['action'], 
            'TestRunCompleted'
        )
    ...
    async def test_backend_abstraction_conflict(self):
        ...
        self.mock_parse_junit.return_value = TestReportSchema(
            total_tests=1, passed_tests=1, _parser_info=ParserInfo(parser_name='junit', status='success')
        )
        self.mock_parse_coverage.return_value = CoverageReportSchema(
            coverage_percentage=90.0, _parser_info=ParserInfo(parser_name='coverage', status='success')
        )
