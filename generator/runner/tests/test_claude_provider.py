...
def test_register_custom_model(provider: ClaudeProvider) -> None:
    provider.register_custom_model("custom-claude", "https://custom.endpoint.com", {"X-Custom": "header"})
...
async def test_api_call_non_stream_standard(provider: ClaudeProvider, mock_claude_response) -> None:
    with patch.object(provider.client.messages, 'create', new_callable=AsyncMock) as mock_create:
        ...
        result, is_stream = await provider._api_call("claude-3-haiku-20240307", "test prompt", False)
...
async def test_api_call_custom_endpoint(provider: ClaudeProvider) -> None:
    provider.register_custom_model("custom", "https://custom.endpoint.com/v1/messages", {"X-Key": "val"})
...
async def test_count_tokens_success(provider: ClaudeProvider) -> None:
    ...
    with patch.object(provider.sync_client, 'count_tokens', return_value=10, create=True) as mock_count:
...
async def test_count_tokens_fallback(provider: ClaudeProvider) -> None:
    # --- FIX: Add create=True to patch the attribute even if it doesn't exist ---
    with patch.object(provider.sync_client, 'count_tokens', side_effect=RuntimeError("API error"), create=True):
        count = await provider.count_tokens("Hello world", "claude-3-haiku-20240307")
