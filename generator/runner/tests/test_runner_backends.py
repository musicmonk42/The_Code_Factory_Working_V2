...
class TestRunnerBackends(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_local_backend_execute_success(self):
        ...
        with patch(
            "runner.runner_backends.subprocess_wrapper",
            new=AsyncMock(
                return_value={
                    "success": True,
                    "stdout": "1 passed",
            ...
            result = await backend.execute(
                payload, work_dir=Path(self.output_dir), timeout=payload.timeout or 300
            )
    ...
    async def test_docker_backend_execute(self):
        ...
        with patch(
            "runner.runner_backends.DockerClient.from_env",
