...
def mock_external_sdks():
    ...
    with patch('runner.runner_metrics.datadog', MagicMock()) as mock_dd, \
...
def mock_lazy_imports():
    ...
    mock_security_mod = MagicMock(redact_secrets=MagicMock(side_effect=lambda x, **kwargs: x)) # Pass-through
    mock_errors_mod = MagicMock(
        ExporterError=MockExporterError,
        RunnerError=MockRunnerError,
        get_error_codes=lambda: {'EXPORTER_FAILURE': 'E500', 'EXPORTER_UNEXPECTED_ERROR': 'E501'}
    )
...
@pytest.fixture
@pytest.mark.asyncio
async def started_metrics_exporter(mock_config, mock_external_sdks, mock_lazy_imports, clean_prometheus_registry):
    """
...
def test_register_exporter():
    ...
    async def my_exporter(metrics): pass
...
def test_exporter_init_cloudwatch(mock_config, mock_external_sdks, mock_lazy_imports):
    ...
    mock_external_sdks['boto3'].client.assert_called_with('cloudwatch', region_name="us-east-1", aws_access_key_id=None, aws_secret_access_key=None)
    # Test call was made
    mock_external_sdks['cw_client'].get_metric_statistics.assert_called_once()
    assert 'cloudwatch' in m._EXPORTER_REGISTRY

def test_exporter_init_cloudwatch_fail_test_call(mock_config, mock_external_sdks, mock_lazy_imports):
    m._EXPORTER_REGISTRY.clear()
    mock_config.aws_region = "us-east-1"
    ...
    mock_external_sdks['cw_client'].get_metric_statistics.side_effect = BotoClientError({"Error": {}}, "op")
...
async def test_export_all_success(started_metrics_exporter):
    ...
    log_action.assert_has_calls([
        call("MetricsExportAttempt", {"exporter": "custom_json_file", "metric_count": len(metrics_arg)}, extra={'instance_id': 'mock_instance_id'}),
        call("MetricsExportSuccess", {"exporter": "custom_json_file"}, extra={'instance_id': 'mock_instance_id'}),
        call("MetricsExportAttempt", {"exporter": "test_exporter", "metric_count": len(metrics_arg)}, extra={'instance_id': 'mock_instance_id'}),
        call("MetricsExportSuccess", {"exporter": "test_exporter"}, extra={'instance_id': 'mock_instance_id'}),
    ], any_order=True)
...
async def test_export_all_failure_queues_for_retry(started_metrics_exporter):
    ...
    log_action.assert_any_call("MetricsExportFailure", {"error_code": "E500", "detail": "Test export fail"}, extra={'instance_id': 'mock_instance_id'})
...
async def test_retry_loop_success(started_metrics_exporter):
    ...
    assert metric_data[0].labels == {'exporter': 'retry_exporter_success', 'success': 'true'}
...
async def test_retry_loop_max_retries_and_drop(started_metrics_exporter, tmp_path):
    ...
    exporter._failed_exports_queue.append(
        (test_metrics, "retry_exporter_drop", exporter._max_export_retries, past_time, past_time)
    )
    ...
    # Check for log
    log_action.assert_any_call("MetricsExportDropped", {
...
async def test_shutdown_flushes_queue(started_metrics_exporter):
    ...
    exporter._failed_exports_queue.append(
        (test_metrics, "shutdown_flush_exporter", 0, datetime.now(timezone.utc), datetime.now(timezone.utc))
    )
...
async def test_alert_monitor_triggers_anomaly_alert(mock_config, caplog, mock_lazy_imports):
    ...
    cpu_metric_key = m._get_canonical_metric_key('runner_resource_usage_percent', {'resource_type': 'cpu', 'instance_id': 'mock_instance_id'})
    ...
    assert log_payload['std_dev'] == pytest.approx(0.08164965809277232) # stddev of [10.0, 10.1, 9.9]
    assert log_payload['threshold_multiplier'] == 2.0
    assert log_payload['instance_id'] == 'mock_instance_id'
    assert kwargs['extra'] == {'alert_type': 'anomaly', 'metric_name': 'cpu_usage', 'instance_id': 'mock_instance_id'}
    # --- END FIX ---