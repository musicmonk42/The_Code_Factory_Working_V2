...
class FakeTextualApp:
    def __init__(self, *args, **kwargs):
        ...
        self.queue_table = MagicMock(add_columns=MagicMock(), add_row=MagicMock(), clear=MagicMock())
        ...
        self.provenance_tree = MagicMock(clear=MagicMock(),
                                         root=MagicMock(add=MagicMock(),
                                                       add_label=MagicMock(),
                                                       expand=MagicMock()))
        self.coverage_tree = MagicMock(clear=MagicMock(),
                                       root=MagicMock(add=MagicMock(),
                                                     add_label=MagicMock(),
                                                     expand=MagicMock()))
        ...
        self.query_one = MagicMock(side_effect=lambda q, t=None: { # Added t=None for default arg
...
# --- END FIX ---
from runner.runner_logging import logger, log_action, LOG_HISTORY
...
class MockCoverageDetail:
    ...
    def __getattr__(self, name): return self.__dict__.get(name, 0.0)
class MockCoverageReportSchema:
    def __init__(self, **kwargs): self.coverage_details = kwargs.get('coverage_details', {})
...
class MockRunner:
    def __init__(self, config):
        ...
        self.run_tests = AsyncMock(return_value=TaskResult(task_id='mock_id', status='completed', results={'pass_rate': 1.0}, started_at=time.time(), finished_at=time.time()))
        self.enqueue = AsyncMock(return_value=TaskResult(task_id='mock_id', status='enqueued', started_at=time.time()))
...
instance_id: tui_test_instance""")
        ...
        self.patch_config_watcher = patch('runner.runner_config.ConfigWatcher', new=MagicMock(return_value=self.mock_config_watcher))
        ...
        self.patch_metrics_exporter = patch('runner.runner_app.MetricsExporter', new=MockMetricsExporter)
    ...
    async def test_tui_log_handler_redaction(self):
        ...
        with patch.object(log_handler, 'format', return_value="2025 - WARNING - Sensitive: sk-abc123"):
    ...
    async def test_task_submission_success(self):
        ...
        with patch.object(app, 'prompt_for_input_file', new_callable=AsyncMock, return_value=str(self.temp_dir / "README.md")):
            
            # Mock the runner to return a successful result (non-distributed mode)
            app.runner.run_tests.return_value = TaskResult(
                task_id=task_id, status='completed', results={'pass_rate': 0.95, 'coverage_percentage': 0.8},
                tags=['test'], started_at=time.time(), finished_at=time.time()
            )
            ...
            self.assertTrue(any("completed" in c.args[0] for c in app.log_widget.write.call_args_list))
            # --- END FIX ---
            ...
            self.assertTrue(any("95%" in c[0][0] for c in app.pass_rate_label.update.call_args_list))
    ...
    async def test_task_submission_distributed_enqueue(self):
        ...
        config_content = self.config_file.read_text().replace('distributed: false', 'distributed: true')
        ...
        with patch.object(app, 'prompt_for_input_file', new_callable=AsyncMock, return_value=str(self.temp_dir / "README.md")):
            ...
            app.runner.enqueue.return_value = TaskResult(
                task_id=task_id, status='enqueued', tags=['test'], started_at=time.time()
            )
            ...
            # --- FIX: Access the mock call arguments correctly ---
            # *** FIX: Use call_args_list instead of await_args_list ***
            self.assertTrue(any("enqueued" in c.args[0] for c in app.log_widget.write.call_args_list))
            # --- END FIX ---
    ...
    async def test_config_reload(self):
        ...
        new_config = RunnerConfig(
            ...
            log_sinks=[], real_time_log_streaming=False, distributed=False, timeout=300,
            parallel_workers=1, mutation=False, fuzz=False, metrics_interval_seconds=5 # Add missing fields
        )
    ...
    async def test_metrics_display(self):
        ...
        RUN_RESOURCE_USAGE.labels(resource_type='mem', instance_id=instance_id)._value = 50.0 # Added mem
        ...
        with patch('runner.runner_app.RUN_QUEUE.labels', MagicMock(return_value=MagicMock(_value=2))), \
             patch('runner.runner_app.RUN_PASS_RATE', MagicMock(_value=0.95)), \
             patch('runner.runner_app.RUN_RESOURCE_USAGE.labels', MagicMock(side_effect=[
                 MagicMock(_value=75.0), MagicMock(_value=50.0) # CPU, MEM
             ])), \
             patch('runner.runner_app.HEALTH_STATUS.labels', MagicMock(return_value=MagicMock(_value=1))):
            ...
            self.assertTrue(any("Queue size: 2" in c[0][2] for c in app.queue_table.add_row.call_args_list))
            ...
            pass_rate_label_mock.update.assert_called_with(unittest.mock.ANY) # Check content via manual assertion
            self.assertTrue(any("95.00%" in c[0][0] for c in pass_rate_label_mock.update.call_args_list))
    ...
    async def test_error_handling(self):
        ...
        with patch.object(app, 'prompt_for_input_file', new_callable=AsyncMock, return_value=str(self.temp_dir / "README.md")):
            ...
            self.assertTrue(any(error.detail in c.args[0] for c in app.log_widget.write.call_args_list))
            # --- END FIX ---
    ...
    async def test_documentation_display(self):
        ...
        # *** FIX: Use call_args_list instead of await_args_list ***
        self.assertTrue(any("Loaded Markdown documentation" in c.args[0] for c in app.log_widget.write.call_args_list))
        # --- END FIX ---
    ...
    async def test_traceability(self):
        ...
        with patch('runner.runner_app.trace.get_current_span', return_value=mock_span):
            ...
            app.runner.run_tests.return_value = TaskResult(
                task_id=task_id, status='completed', results={'tests': 1}, started_at=time.time(), finished_at=time.time()
            )
