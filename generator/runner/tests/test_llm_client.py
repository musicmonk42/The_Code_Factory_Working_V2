...
import time # Needed for accurate cache key mocking
...
from runner.llm_client import (
    LLMClient, SecretsManager, CacheManager, DistributedRateLimiter, CircuitBreaker,
    call_llm_api, call_ensemble_api, shutdown_llm_client
...
def mock_imports(event_loop):
    ...
    with patch('runner.llm_client.aioredis.from_url', new_callable=MagicMock) as mock_redis_from_url, \
         ...
         patch('runner.llm_client.log_audit_event', new_callable=AsyncMock) as mock_audit, \
        ...
        mock_metrics.LLM_LATENCY_SECONDS.labels = MagicMock(return_value=MagicMock(observe=MagicMock()))
        ...
        mock_metrics.LLM_RATE_LIMIT_EXCEEDED.labels = MagicMock(return_value=MagicMock(inc=MagicMock()))
...
class TestLLMClient:
    ...
    async def test_call_llm_api_non_stream(self, initialized_client, mock_provider, mock_imports):
        ...
        result = await initialized_client.call_llm_api('test_prompt', 'test_model', provider='mock_provider')
    ...
    async def test_call_llm_api_stream(self, initialized_client, mock_provider, mock_imports):
        ...
        mock_provider.count_tokens = AsyncMock(side_effect=[3, 2, 0]) # Input tokens, chunk 1, chunk 2
        initialized_client.rate_limiter.acquire = AsyncMock(return_value=True) 
        
        gen = await initialized_client.call_llm_api('test_prompt', 'test_model', stream=True, provider='mock_provider')
    ...
    async def test_call_ensemble_api(self, initialized_client):
        ...
        async def mock_call_llm_api_internal(prompt, model=None, stream=False, provider=None, **kwargs):
            if provider == 'p1': return {'content': 'consensus_response'}
            if provider == 'p2': return {'content': 'consensus_response'}
            if provider == 'p3': return {'content': 'dissent_response'}
            
        with patch.object(initialized_client, 'call_llm_api', side_effect=mock_call_llm_api_internal) as mock_internal_call:
            ...
            mock_internal_call.side_effect = [Exception('p1 failed'), {'content': 'p2_res'}, {'content': 'p3_res'}]
...
class TestGlobalFunctions:
    ...
    async def test_global_call_llm_api(self, MockLLMClient, mock_config, mock_imports):
        ...
        result = await call_llm_api('global_prompt', 'global_model', False, 'global_provider', mock_config)
