...
from runner.feedback_handlers import (
    FeedbackEvent, Severity, FeedbackSink, FileSink, FeedbackRegistry,
    collect_feedback, register_sink, get_feedback_metrics, shutdown,
    _worker_queue, _worker_stop, _SENTINEL, _registry, # <-- REMOVED _worker_thread
    LoggingSink 
)
...
def test_worker_sentinel():
    ...
    # FIX: Check the variable on the module itself
    assert feedback_handlers._worker_thread is not None and feedback_handlers._worker_thread.is_alive()
    ...
    assert feedback_handlers._worker_thread is None or not feedback_handlers._worker_thread.is_alive()
...
def test_filesink_no_permissions(temp_file):
    ...
    with patch("runner.feedback_handlers.FileSink.emit", side_effect=OSError("Permission denied")) as mock_emit:
        sink = FileSink(str(temp_file))
        register_sink(sink)
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--tb=short", "--cov=runner.feedback_handlers", "--cov-report=term-missing"])