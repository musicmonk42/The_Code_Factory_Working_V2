...
from runner.runner_errors import RunnerError, ExecutionError, TimeoutError, ParsingError, error_codes
...
class TestRunnerCore(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.patch_save_files = patch('runner.runner_core.Runner._save_files_to_temp_dir', new=AsyncMock())
...
<testcase classname="test" name="test_ok" time="0.001"/>
        ...
        mock_success_parser = AsyncMock(return_value=MagicMock(
            model_dump=lambda **kwargs: {'total_tests': 1, 'failures': 0, 'errors': 0, 'pass_rate': 1.0}
        ))
        ...
        with patch('runner.runner_parsers.parse_junit_xml', new=mock_success_parser):
            ...
            with patch('runner.runner_parsers.parse_coverage_xml', return_value=MagicMock(
...
<testcase classname="test" name="test_ok" time="0.001"/>
        ...
        self.assertEqual(RUN_QUEUE.labels(framework='all', instance_id='test_instance')._value.get(), 0)
