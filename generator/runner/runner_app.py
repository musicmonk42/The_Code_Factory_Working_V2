...
class RunnerApp(_TextualAppBase):
    ...
    def _app_config_reload_callback(
        self, new_config: RunnerConfig, diff: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        ...
        if hasattr(self.runner, "_on_config_reload_callback"):
            ...
            self.runner._on_config_reload_callback(
                new_config, diff, runner_instance_ref=self.runner
            )
        else:
        ...
        self.refresh(
            layout=True
        )  # Re-render to pick up new theme/language changes based on new config
    ...
    async def on_shutdown(self) -> None:
        ...
        if (
            hasattr(self, "remote_session")
            and self.remote_session
            and not self.remote_session.closed
        ):
        ...
        if (
            hasattr(self, "metrics_exporter")
            and hasattr(self.metrics_exporter, "shutdown")
            and callable(self.metrics_exporter.shutdown)
        ):
    ...
    async def on_mount(self) -> None:
        ...
        try:
            ...
            lang_translation = gettext.translation(
                "runner", "locale", languages=[self.current_language], fallback=True
            )
            ...
            logger.error(
                f"Error installing gettext for language '{self.current_language}': {e}",
                exc_info=True,
            )
            self.current_language = "en"
            # Fallback to default
            ...
            with TabbedContent(
                ...
                for widget_name, widget_class in self._plugin_widgets.items():
                    try:
                        ...
                        logger.error(
                            f"Error loading plugin widget '{widget_name}': {e}",
                            exc_info=True,
                        )
                        yield TabPane(
                            _(f"Plugin Error: {widget_name}"),
                            id=f"plugin-error-tab-{widget_name.lower()}",
                        )
        ...
        try:
            ...
            self.health_label.update(
                _("Health: [bold]{}[/bold]").format(
                    _("Good")
                    if health_status_value == 1
                    else _("Bad") if health_status_value == 0 else _("Unknown")
                )
            )
        except Exception as e:
    ...
    def _save_workspace_state(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Could not get active tab ID for workspace save: {e}. Defaulting to dashboard-tab."
                )
    ...
    def _load_workspace_state(self):
        ...
        if self.workspace_file.exists():
            try:
                ...
                logger.warning(
                    f"Failed to load workspace state: {e}. Starting with default preferences.",
                    exc_info=True,
                )
                self.current_theme = "dark"
                self.current_language = "en"
            ...
            logger.info(
                f"No workspace file found at {self.workspace_file}. Starting with default preferences."
            )
    ...
    async def start_workflow(self):
        ...
        payload = TaskPayload(
            ...
            code_files={},
            output_path=str(
                self.doc_output_dir.parent
            ),  # Use self.doc_output_dir.parent (which is 'output')
            task_id=task_id,
        )
        try:
    ...
    async def submit_feedback_action(self):
        ...
        try:
            ...
            if hasattr(self.runner, "_tune_from_feedback") and callable(
                self.runner._tune_from_feedback
            ):
                self.runner._tune_from_feedback(score)  # This is sync
                # --- FIX 1.1: Use _log_async ---
                await self._log_async(
                    _(
                        f"[green]Feedback rating {score} submitted and applied to engine tuning.[/green]"
                    )
                )
    ...
    def change_language(self, event: Select.Changed):
        ...
        if new_lang != self.current_language:
            ...
            try:
                ...
                global _
                lang_translation = gettext.translation(
                    "runner", "locale", languages=[new_lang], fallback=True
                )
                lang_translation.install()
                _ = lang_translation.gettext
                ...
                asyncio.create_task(
                    self._log_async(
                        _(
                            f"[red]Error changing language: {e}. Reverting to {self.current_language}.[/red]"
                        )
                    )
                )
    ...
    async def load_plugin(self):
        ...
        try:
            ...
            if self.production_mode and plugin_name not in TRUSTED_PLUGINS:
                ...
                await self._log_async(
                    _(
                        f"[red]Error: Plugin '{plugin_name}' is not in the trusted whitelist for production.[/red]"
                    )
                )
                logger.warning(
                    f"Attempted to load untrusted plugin in production mode: {plugin_name}"
                )
                return
            ...
            try:
                ...
                if hasattr(module, "register_tui_widgets") and callable(
                    module.register_tui_widgets
                ):
                    ...
                    await self._log_async(
                        _(
                            f"[yellow]Plugin '{plugin_name}' has no 'register_tui_widgets' function.[/yellow]"
                        )
                    )
                ...
                if hasattr(module, "register_tui_themes") and callable(
                    ...
                    # --- FIX 1.1: Use _log_async ---
                    await self._log_async(
                        _(f"[blue]Plugin '{plugin_name}' registered TUI themes.[/blue]")
                    )
                else:
                    # --- FIX 1.1: Use _log_async ---
                    await self._log_async(
                        _(
                            f"[yellow]Plugin '{plugin_name}' has no 'register_tui_themes' function.[/yellow]"
                        )
                    )
                ...
                await self._log_async(
                    _(
                        f"[green]Plugin '{plugin_name}' loaded successfully. UI recomposed and refreshed.[/green]"
                    )
                )
                ...
                await self._log_async(
                    _(
                        f"[red]Error: Plugin module '{plugin_name}' not found. Check if it's installed or correctly named.[/red]"
                    )
                )
                logger.error(
                    f"ModuleNotFoundError: Plugin '{plugin_name}' not found.",
                    exc_info=True,
                )
                ...
                await self._log_async(
                    _(
                        f"[red]Error loading plugin '{plugin_name}': Missing expected registration function. {ae}[/red]"
                    )
                )
                logger.error(
                    f"AttributeError loading plugin '{plugin_name}': {ae}",
                    exc_info=True,
                )
            except Exception as e:
                # --- FIX 1.1: Use _log_async ---
                await self._log_async(
                    _(
                        f"[red]An unexpected error occurred loading plugin '{plugin_name}': {e}[/red]"
                    )
                )
                logger.error(
