...
def _setup_mutmut_config(temp_dir_path: Path, code_file_paths: List[Path], test_file_paths: List[Path]) -> None:
    ...
    """
    # FIX Issue 1: Validate that code/ directory exists before writing config
    ...
    # FIX Issue 1: Use TOML array syntax for paths to prevent mutmut from iterating over string characters
    # FIX Issue 2: tests_dir must also use array syntax to prevent mutmut v3+ from iterating over characters
...
# FIX Bug 2: Update mutmut CLI for v3+ compatibility
