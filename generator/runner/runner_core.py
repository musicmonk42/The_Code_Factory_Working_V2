...
from typing import Any, Dict, List, Tuple, Optional, Callable, Union, Awaitable
...
from runner.runner_parsers import (
    register_test_parser, register_coverage_parser, parse_junit_xml, parse_coverage_xml, 
    parse_unittest_output, parse_behave_junit, parse_robot_xml, parse_jest_json, 
...
try:
    from runner.runner_mutation import mutation_test as _mutation_test_func, detect_language as _detect_mutation_lang, fuzz_test as _fuzz_test_func
    HAS_MUTATION_MODULE = True
except ImportError:
    HAS_MUTATION_MODULE = False
    logger.warning("runner/mutation.py not found or dependencies missing. Mutation and Fuzz testing features will be unavailable.")
...
try:
    ...
    logger.warning("Hypothesis not installed. Property-based testing and Hypothesis-based fuzzing will be unavailable.")
...
FRAMEWORKS: Dict[str, Dict[str, Any]] = {
    'pytest': {
        'cmd': ['pytest', '--junitxml=results.xml', '--cov', '--cov-report=xml:cov.xml'],
        ...
        'detect': lambda files: any('pytest' in content for content in files.values()) or any(f.startswith('test_') and f.endswith('.py') for f in files.keys())
    },
    'unittest': {
        ...
        'detect': lambda files: any('unittest' in content for content in files.values()) or any(f.startswith('test_') and f.endswith('.py') for f in files.keys())
    },
    'behave': {
        'cmd': ['behave', '--junit', '--junit-directory', '.', '--format', 'json', '--outfile', 'behave-results.json'],
        'parser': parse_behave_junit,
        'coverage_parser': None,
        'output_files': ['behave-results.json', 'behave.xml'], # Behave can produce JSON or XML
    ...
    'jest': {
        'cmd': ['npm', 'test', '--', '--json', '--outputFile=results.json', '--coverage', '--coverageReporters=json'],
        'parser': parse_jest_json,
        'coverage_parser': parse_istanbul_json,
        'output_files': ['results.json', 'coverage', 'jest-junit.xml'],
        'detect': lambda files: 'jest' in files.get('package.json', '') or any(f.endswith(('.test.js', '.spec.js', '.test.ts', '.spec.ts')) for f in files.keys())
    },
    'mocha': {
        'cmd': ['mocha', '--reporter', 'json', '--full-trace', '--require', 'source-map-support/register'],
        'parser': lambda path: json.loads(path.read_text()), # Mochar is often text based, not specific parser here
        'coverage_parser': parse_istanbul_json,
        'output_files': ['results.json', 'coverage'],
        'detect': lambda files: 'mocha' in files.get('package.json', '') or any(f.endswith(('.test.js', '.spec.js')) for f in files.keys())
    },
    'go test': {
    ...
    'junit': { # Generic JUnit, often used for Java/Maven/Gradle
        ...
        'output_files': ['target/surefire-reports', 'target/site/jacoco'], # Common Maven output dirs
        'detect': lambda files: 'pom.xml' in files.keys() or any(f.endswith('Test.java') for f in files.keys())
    },
    'gradle': {
        'cmd': ['gradle', 'test'],
        ...
        'detect': lambda files: 'build.gradle' in files.keys() or any(f.endswith('Test.java') for f in files.keys())
    },
    'selenium': {
        'cmd': ['pytest', '--driver', 'Chrome', '--url', 'http://example.com'], # Example cmd
...
DOC_FRAMEWORKS = {
    ...
    'javadoc': {
        ...
        'validator_cmd': None,
        'output_dir': 'javadoc',
        'detect': lambda files: any(f.endswith('.java') for f in files.keys()) and 'src' in [Path(f).parts[0] for f in files.keys()]
    },
    'jsdoc': {
        'cmd': ['jsdoc', '-c', 'jsdoc.json', '-d', 'out'],
        'validator_cmd': None,
        'output_dir': 'out',
        'detect': lambda files: 'jsdoc.json' in files.keys() or any(f.endswith(('.js', '.ts')) for f in files.keys())
    },
    'go_doc': {
        'cmd': ['godoc', '-html', '.'], # `godoc` serves, so this would need capturing output or a custom script
        'validator_cmd': None,
...
class Runner(ABC):
    ...
    def __init__(self, config: RunnerConfig):
        ...
        if not backend_cls:
            raise BackendError(error_codes["BACKEND_INIT_FAILURE"], detail=f"Configured backend '{self.config.backend}' is not supported or not installed.", backend_type=self.config.backend)
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=self.config.parallel_workers)
        self.queue: List[PrioritizedTask] = []
        self.provenance_chain: List[Dict[str, Any]] = [] # This is now a log of local hashes, not the full chain
        self.feedback_scores: deque[float] = deque(maxlen=getattr(self.config, 'feedback_score_history_size', 100))
    ...
    async def start_services(self):
        ...
        if self.config.distributed and (self._distributed_task is None or self._distributed_task.done()):
    ...
    async def _write_file_async(self, path: Path, content: str):
        ...
        try:
            ...
            raise SetupError(error_codes["SETUP_FAILURE"], detail=f"Failed to write file to temp dir {path}", task_id="N/A", stage="file_save", cause=e)

    # [NEW] Converted to async
    async def _self_test(self) -> None:
        span = trace.get_current_span()
        try:
            self.config.model_validate(self.config.model_dump()) # Validate config against its Pydantic schema
            span.add_event("Config validated by Pydantic")

            backend_health = self.backend.health()
            if backend_health.get('status') != 'healthy':
                raise BackendError(error_codes["BACKEND_INIT_FAILURE"], detail=f"Backend unhealthy during self-test: {backend_health.get('error', 'unknown error')}", backend_type=self.config.backend)
            span.add_event("Backend health check passed")

            dummy_test_files = {'test_dummy.py': 'import unittest\nclass DummyTests(unittest.TestCase):\n def test_true(self): self.assertTrue(True)'}
            ...
            if test_results.status != 'completed' or not (test_results.results and test_results.results.get('dry_run_result', False)):
                raise TestExecutionError(error_codes["TEST_EXECUTION_FAILED"], detail="Dry-run test failed unexpectedly during self-test.", task_id=dummy_task_payload.task_id, results=test_results.results)
    ...
    async def _monitor_queue(self) -> None:
        ...
        while True:
            try:
                ...
                all_statuses = ['pending', 'running', 'completed', 'failed', 'timed_out', 'enqueued']
                ...
                await asyncio.sleep(self.config.metrics_interval_seconds) # Avoid fast loop on error
    ...
    async def _distributed_worker(self) -> None:
        ...
        if not self.config.dist_url:
            logger.warning("Distributed worker enabled but 'dist_url' is not configured. Disabling distributed worker.")
            return
            ...
            try:
                async with reloading_lock: # Acquire lock to prevent reconfig during send
                    ...
                    for _ in range(min(batch_size, len(self.queue))):
                        ...
                        self._update_task_status(prio_task.task_id, 'enqueued', started_at=time.time())
                    ...
                    try:
                        ...
                        async with session.post(self.config.dist_url + '/enqueue_test_task_batch', json=BatchTaskPayload(tasks=tasks_to_send).model_dump(), timeout=timeout_val) as resp:
                            resp.raise_for_status()
                            dist_response = await resp.json()
                            
                            dist_latency = time.time() - start_dist_latency
                            DISTRIBUTED_LATENCY.labels(instance_id=self.instance_id).observe(dist_latency)
                            logger.info(json.dumps({
                            ...
                            span.add_event("Distributed batch sent", attributes={"response_status": dist_response.get('status')})
                            
                            if dist_response.get('status') == 'success' or dist_response.get('status') == 'enqueued':
                                pass
                            else:
                                error_detail = dist_response.get('message', 'Remote enqueue failed.')
                                ...
                                RUN_ERRORS.labels(error_type='distributed_worker', backend='dist_api_failure', instance_id=self.instance_id).inc()
                                ...
                                for task in tasks_to_send:
                                    ...
                                    self._update_task_status(task.task_id, 'pending', error=DistributedError(error_codes["DISTRIBUTED_COMMUNICATION_ERROR"], detail="Remote enqueue rejected by API", endpoint=self.config.dist_url, cause=None).as_dict())
                        ...
                        RUN_ERRORS.labels(error_type='distributed_worker', backend='network_error', instance_id=self.instance_id).inc()
                        span.set_status(Status(StatusCode.ERROR, f"Network error: {e}"))
                        span.record_exception(e)
                        await self.backend.recover()
                        for task in tasks_to_send:
                            heapq.heappush(self.queue, PrioritizedTask(task.priority, task))
                            self._update_task_status(task.task_id, 'pending', error=DistributedError(error_codes["DISTRIBUTED_COMMUNICATION_ERROR"], detail="Network error sending task batch", endpoint=self.config.dist_url, cause=e).as_dict())
                        ...
                        RUN_ERRORS.labels(error_type='distributed_worker', backend='send_timeout', instance_id=self.instance_id).inc()
                        span.set_status(Status(StatusCode.ERROR, "Send timeout"))
                        for task in tasks_to_send:
                            heapq.heappush(self.queue, PrioritizedTask(task.priority, task))
                            self._update_task_status(task.task_id, 'pending', error=TimeoutError(error_codes["TASK_TIMEOUT"], detail="Distributed send timed out", timeout_seconds=timeout_val).as_dict())
                    except Exception as e:
                        logger.error(json.dumps({
                        ...
                        RUN_ERRORS.labels(error_type='distributed_worker', backend='unexpected_error', instance_id=self.instance_id).inc()
                        span.set_status(Status(StatusCode.ERROR, f"Unexpected error: {e}"))
                        span.record_exception(e)
                        for task in tasks_to_send:
                            heapq.heappush(self.queue, PrioritizedTask(task.priority, task))
                            self._update_task_status(task.task_id, 'pending', error=DistributedError(error_codes["DISTRIBUTED_COMMUNICATION_ERROR"], detail="Unexpected error in distributed worker", cause=e).as_dict())
    ...
    async def _add_provenance(self, entry_data: Dict[str, Any], task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_data_payload = {
                ...
                "framework_used": self.framework_info.get('cmd', 'N/A') if self.framework_info else 'N/A',
                "instance_id": self.instance_id,
                # Log a summary, not the full (potentially huge) data blob
                "result_summary": {k: v for k, v in sanitized_entry_data.items() if k not in ['code_files', 'test_files', 'coverage_details', 'raw_output_summary', 'test_cases']}
            }
            ...
            logger.critical(f"CRITICAL: Failed to log audit event for task {task_id}: {e}. Audit trail is incomplete.", exc_info=True)
    ...
    def _run_feedback(self, results: Dict[str, Any], task_id: str):
        ...
        RUN_PASS_RATE.set(sum(self.feedback_scores) / len(self.feedback_scores) if self.feedback_scores else 0.0)
        ...
        avg_score_for_strategy = sum(self.feedback_model[self.current_strategy]) / len(self.feedback_model[self.current_strategy]) if self.feedback_model[self.current_strategy] else 0.0
        logger.info(json.dumps({
    ...
    def _adjust_strategy(self):
        if not self.feedback_model: return
        ...
        if not eligible_strategies:
            logger.debug("Not enough feedback points for strategy adjustment. Skipping strategy adjustment.")
            return

        best_strategy = max(eligible_strategies, key=lambda k: sum(eligible_strategies[k]) / len(eligible_strategies[k]))
        ...
        if best_strategy != self.current_strategy:
            ...
            if new_strategy_settings:
                ...
                if backend_cls:
                    self.backend = backend_cls(self.config)
                    logger.info(f"Runner backend re-initialized to {self.backend.__class__.__name__} due to strategy change.")
                else:
                    logger.error(f"Strategy '{best_strategy}' requires unavailable backend '{self.config.backend}'. Keeping current backend.")
            else:
                logger.warning(f"No specific settings found for strategy '{best_strategy}'. No config changes applied.")
        ...
        try:
            current_error_count = RUN_ERRORS.labels(error_type='all_errors', backend='any', instance_id=self.instance_id)._value
        except Exception:
            pass
        ...
        if current_error_count > anomaly_error_threshold:
            ...
            try:
                RUN_ERRORS.labels(error_type='all_errors', backend='any', instance_id=self.instance_id).inc(-current_error_count) 
            except Exception:
                pass
    ...
    def _persist_queue(self) -> None:
        ...
        try:
            ...
            RUN_ERRORS.labels(error_type='persistence', backend='queue_save_failed', instance_id=self.instance_id).inc()
    ...
    def _load_persisted_queue(self) -> None:
        ...
        if os.path.exists(queue_file):
            try:
                ...
                for t in tasks_list:
                    heapq.heappush(self.queue, PrioritizedTask(t['priority'], TaskPayload(**t['task'])))
                os.remove(queue_file)
                logger.info(f"Loaded {len(tasks_list)} tasks from persisted queue.")
            except Exception as e:
                logger.error(f"Failed to load persisted queue from '{queue_file}': {e}. Starting with empty queue.", exc_info=True)
                RUN_ERRORS.labels(error_type='persistence', backend='queue_load_failed', instance_id=self.instance_id).inc()
            finally:
                if os.path.exists(queue_file):
                    os.remove(queue_file)
    ...
    async def run_tests(self, task_payload: TaskPayload) -> TaskResult:
        ...
        if task_payload.dry_run:
            ...
            self._update_task_status(task_id, 'completed', results=simulated_results, finished_at=task_result.finished_at)
            return task_result
        
        timeout: int = task_payload.timeout if task_payload.timeout is not None else self.config.timeout
        ...
        try:
            ...
            try:
                ...
                if isinstance(e, SetupError): raise
                raise SetupError(error_codes["SETUP_FAILURE"], detail="Failed to save files to temporary directory.", task_id=task_id, stage="file_save", cause=e)
            ...
            try:
                ...
                raise SetupError(error_codes["SETUP_FAILURE"], detail="Backend setup failed due to unexpected error.", task_id=task_id, stage="backend_setup", cause=e)
            ...
            try:
                test_report = await self.framework_info['parser'](temp_dir_path) # Parsers are now async and return schema
                logger.debug(json.dumps({"event": "parsed_test_results", "task_id": task_id, "results": test_report.model_dump(by_alias=True)}))
            except RunnerError as e: # Catch structured errors from parser
                e.task_id = task_id
                raise e
            except Exception as e: # Catch unexpected errors from parser
                raise ParsingError(error_codes["PARSING_ERROR"], detail=f"Failed to parse test results for framework '{actual_framework_name}'.", task_id=task_id, parser_type="test_results", cause=e)

            parsed_results: Dict[str, Any] = test_report.model_dump(by_alias=True) # Convert to dict for consistency
            ...
            if self.framework_info.get('coverage_parser'):
                ...
                try:
                    ...
                    if coverage_report:
                        ...
                        parsed_results['coverage_details'] = {k: v.model_dump() for k, v in coverage_report.coverage_details.items()} # Convert CoverageDetail to dict
                    ...
                    RUN_ERRORS.labels(error_type='CoverageParsingError', backend=self.config.backend, instance_id=self.instance_id).inc()
            ...
            if getattr(self.config, 'mutation', False) and HAS_MUTATION_MODULE:
                ...
                try:
                    mut_results: Dict[str, Any] = await _mutation_test_func(temp_dir_path, self.config, task_payload.code_files, task_payload.test_files, strategy=getattr(self.config, 'mutation_strategy', 'targeted'), parallel=getattr(self.config, 'mutation_parallel', True), distributed=getattr(self.config, 'mutation_distributed', False))
                    ...
                    RUN_ERRORS.labels(error_type='MutationError', backend=self.config.backend, instance_id=self.instance_id).inc()
                    span.set_status(Status(StatusCode.ERROR, f"Mutation testing failed: {e.error_code}"))
                    span.record_exception(e)
                except Exception as e:
                    logger.error(json.dumps({
                        "event": "mutation_test_failed",
                ...
                logger.warning(json.dumps({"event": "mutation_skipped", "task_id": task_id, "reason": "module unavailable"}))
            ...
            if getattr(self.config, 'fuzz', False) and HAS_MUTATION_MODULE:
                ...
                try:
                    fuzz_results: Dict[str, Any] = await _fuzz_test_func(temp_dir_path, self.config, task_payload.code_files)
                    ...
                    RUN_ERRORS.labels(error_type='FuzzError', backend=self.config.backend, instance_id=self.instance_id).inc()
                    span.set_status(Status(StatusCode.ERROR, f"Fuzz testing failed: {e.error_code}"))
                    span.record_exception(e)
                except Exception as e:
                    logger.error(json.dumps({
                        "event": "fuzz_test_failed",
                ...
                logger.warning(json.dumps({"event": "fuzz_skipped", "task_id": task_id, "reason": "module unavailable"}))
                span.add_event("Fuzz testing skipped: module unavailable")

            # --- Documentation Generation/Validation Phase ---
            doc_validation_status_dict = {'status': 'skipped', 'message': 'Not applicable or no documentation framework detected.'}
            ...
            if actual_doc_framework_name == 'auto':
                ...
                if detected_doc_fw:
                    ...
                    logger.info(f"Auto-detected documentation framework: {actual_doc_framework_name}.")
                else:
                    logger.info("No specific documentation framework detected in project. Skipping doc validation.")
            ...
            if actual_doc_framework_name != 'auto' and actual_doc_framework_name is not None:
                ...
                if doc_framework_info:
                    span.add_event(f"Running doc generation/validation for {actual_doc_framework_name}")
                    ...
                    try:
                        ...
                        if build_result.get('returncode', 0) == 0 and validation_result.get('returncode', 0) == 0:
                            ...
                            logger.info(f"Documentation validation passed for {actual_doc_framework_name} for task {task_id}.")
                            DOC_VALIDATION_STATUS.labels(doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).set(1)
                        else:
                            ...
                            logger.warning(f"Documentation validation failed for {actual_doc_framework_name} for task {task_id}.")
                            DOC_VALIDATION_STATUS.labels(doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).set(0)
                            DOC_GENERATION_ERRORS.labels(error_type='validation_failed', doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).inc()
                    except RunnerError as e: # Catch structured errors from backend.execute
                        doc_validation_status_dict = {'status': 'error', 'error': e.as_dict(), 'message': 'Exception during doc generation/validation.'}
                        logger.error(f"Error during doc generation/validation for {actual_doc_framework_name} for task {task_id}: {e.detail}", exc_info=True)
                        DOC_VALIDATION_STATUS.labels(doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).set(0)
                        DOC_GENERATION_ERRORS.labels(error_type='runtime_error', doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).inc()
                    except Exception as e:
                        doc_validation_status_dict = {'status': 'error', 'error': str(e), 'message': 'Exception during doc generation/validation.'}
                        logger.error(f"Error during doc generation/validation for {actual_doc_framework_name} for task {task_id}: {e}", exc_info=True)
                        DOC_VALIDATION_STATUS.labels(doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).set(0)
                        DOC_GENERATION_ERRORS.labels(error_type='runtime_error', doc_framework_name=actual_doc_framework_name, instance_id=self.instance_id).inc()
                    
                    parsed_results['doc_validation'] = doc_validation_status_dict
                    span.set_attribute("runner.doc_validation_status", doc_validation_status_dict['status'])
                else:
                    logger.warning(f"Documentation framework '{actual_doc_framework_name}' is not recognized or configured. Skipping doc validation.")
                    parsed_results['doc_validation'] = {'status': 'skipped', 'message': f"Unknown doc framework: {actual_doc_framework_name}"}
            ...
            for out_file_pattern in self.framework_info.get('output_files', []):
                for file_in_temp in temp_dir_path.rglob(out_file_pattern):
                    if file_in_temp.is_file():
                        target_path = output_path_actual / 'results' / file_in_temp.relative_to(temp_dir_path)
            ...
            if 'status' in doc_validation_status_dict and doc_validation_status_dict['status'] in ['passed', 'failed'] and doc_framework_info:
                ...
                if final_doc_output_source_dir.exists():
                    ...
                    shutil.copytree(final_doc_output_source_dir, doc_output_dest_dir, dirs_exist_ok=True)
                    logger.info(f"Copied generated documentation from {final_doc_output_source_dir} to {doc_output_dest_dir}.")
            ...
            self._update_task_status(task_id, 'completed', results=final_results, finished_at=time.time())
            ...
            self._update_task_status(task_id, 'timed_out', error=e.as_dict(), finished_at=time.time())
            ...
            logger.error(f"Test run for task {task_id} failed with RunnerError: {e.as_dict()}", exc_info=True)
            span.set_status(Status(StatusCode.ERROR, f"Test run failed: {e.error_code}"))
            span.record_exception(e)
            return self.task_status_map[task_id]
        except Exception as e:
            error_dict = RunnerError(error_codes["UNEXPECTED_ERROR"], f"An unexpected error occurred during test run: {e}", task_id=task_id, cause=e).as_dict()
            self._update_task_status(task_id, 'failed', error=error_dict, finished_at=time.time())
            logger.error(f"Test run for task {task_id} failed with unexpected error: {error_dict}", exc_info=True)
            span.set_status(Status(StatusCode.ERROR, f"Unexpected error: {e}"))
            span.record_exception(e)
            return self.task_status_map[task_id]
        finally:
            if temp_dir_obj:
                try:
                    ...
                    logger.warning(f"Failed to cleanup temporary directory {temp_dir_path}: {cleanup_e}", exc_info=True)
            elif temp_dir_path and temp_dir_path.exists():
                try:
                    shutil.rmtree(temp_dir_path, ignore_errors=True)
                    logger.debug(f"Cleaned up temporary directory (fallback): {temp_dir_path}")
                except Exception as cleanup_e:
                    logger.warning(f"Failed to cleanup temporary directory (fallback) {temp_dir_path}: {cleanup_e}", exc_info=True)
...
async def parallel_runs(runner: "Runner", tasks: List[TaskPayload], priorities: Optional[List[int]] = None) -> List[TaskResult]:
    ...
    completed_task_results_ids: set[str] = set() # Track IDs of completed tasks to avoid re-checking
    ...
    while True:
        ...
        for enq_task_result in enqueued_task_results:
            ...
            if current_status and current_status.status in ['completed', 'failed', 'timed_out']:
        ...
        if time.time() - start_time > max_wait_time:
            logger.warning(f"Parallel runs timed out after {max_wait_time}s. {len(runner.queue)} tasks remaining in queue.")
            break
        
        logger.debug(f"Waiting for parallel tasks to process. Queue size: {len(runner.queue)}. Completed so far: {len(completed_task_results_ids)}")
        # FIX: Use getattr
...
def _on_config_reload_callback(new_config: RunnerConfig, diff: Optional[Dict[str, Any]], runner_instance_ref: Optional["Runner"] = None) -> None:
    """
    ...
    """
    target_runner = runner_instance_ref if runner_instance_ref is not None else globals().get('runner_instance')
        ...
        async def _apply_config_update():
            async with reloading_lock:
                ...
                if 'backend' in (diff or {}):
                    logger.info(f"Backend changed from {target_runner.backend.__class__.__name__} to {new_config.backend}. Re-initializing backend.")
                    try:
                        backend_cls = ALL_BACKENDS.get(new_config.backend)
                        if backend_cls:
                            target_runner.backend = backend_cls(new_config)
                            logger.info(f"Backend successfully re-initialized to {new_config.backend}.")
                        else:
                            logger.error(f"New backend '{new_config.backend}' is not registered or unavailable. Keeping old backend.")
                    except RunnerError as e: # Catch structured errors from backend init
                        logger.error(f"Failed to re-initialize backend to {new_config.backend}: {e.as_dict()}. Keeping old backend.", exc_info=True)
                    except Exception as e:
                        logger.error(f"Failed to re-initialize backend to {new_config.backend}: {e}. Keeping old backend.", exc_info=True)

                if 'parallel_workers' in (diff or {}):
                    logger.info(f"Parallel workers changed to {new_config.parallel_workers}. Re-initializing executor.")
                    target_runner.executor.shutdown(wait=True)
                    target_runner.executor = concurrent.futures.ThreadPoolExecutor(max_workers=new_config.parallel_workers)
                    logger.info("ThreadPoolExecutor re-initialized.")
                
                logger.info("Runner updated with new config settings (e.g., metrics intervals, strategies will adapt).")
...
try:
    ...
    class DummyFastAPI: pass
    api = DummyFastAPI() 
    class DummyClickGroup: pass
    cli = DummyClickGroup()
    API_AVAILABLE = False
...
if API_AVAILABLE:
    ...
    async def startup_event():
        ...
        try:
            ...
            config_watcher = ConfigWatcher('runner.yaml', partial(_on_config_reload_callback, runner_instance_ref=runner_instance))
    ...
    async def shutdown_event():
        ...
        if runner_instance:
            ...
            if hasattr(runner_instance.backend, 'close') and callable(runner_instance.backend.close):
    ...
    @api.post("/run_test_task", summary="Run a single test task immediately or enqueue", response_model=TaskResult)
    async def api_run_test_task(
        payload: TaskPayload = Body(...)
    ):
        global runner_instance
        if runner_instance is None:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Runner service not initialized.")

        if payload.task_id is None:
            payload.task_id = f"api_run_task_{uuid.uuid4()}"

        logger.info(json.dumps({"event": "api_run_task_received", "task_id": payload.task_id, "distributed": runner_instance.config.distributed}))
        ...
        try:
            ...
            logger.error(json.dumps({"event": "api_run_task_failed", "error": e.as_dict()}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=e.as_dict())
        except Exception as e:
            error_dict = RunnerError(error_codes["UNEXPECTED_ERROR"], f"An unexpected API error occurred: {e}", task_id=payload.task_id, cause=e).as_dict()
            logger.error(json.dumps({"event": "api_run_task_failed", "error": error_dict}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error_dict)


    @api.post("/enqueue_test_task", summary="Enqueue a test task for asynchronous execution", response_model=TaskResult)
    async def api_enqueue_test_task(
        payload: TaskPayload = Body(...)
    ):
        global runner_instance
        if runner_instance is None:
        ...
        try:
            ...
            logger.error(json.dumps({"event": "api_enqueue_task_failed", "error": e.as_dict()}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=e.as_dict())
        except Exception as e:
            error_dict = RunnerError(error_codes["UNEXPECTED_ERROR"], f"An unexpected API error occurred: {e}", task_id=payload.task_id, cause=e).as_dict()
            logger.error(json.dumps({"event": "api_enqueue_task_failed", "error": error_dict}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error_dict)


    @api.post("/enqueue_test_task_batch", summary="Enqueue a batch of test tasks for asynchronous execution", response_model=Dict[str, Any])
    async def api_enqueue_test_task_batch(
        batch_payload: BatchTaskPayload = Body(...)
    ):
        global runner_instance
        if runner_instance is None:
            raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Runner service not initialized.")
        ...
        try:
            ...
            return {"status": "batch_enqueued", "task_ids": enqueued_task_ids, "count": len(batch_payload.tasks)}
        except RunnerError as e:
            logger.error(json.dumps({"event": "api_batch_enqueue_failed", "error": e.as_dict()}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=e.as_dict())
        except Exception as e:
            error_dict = RunnerError(error_codes["UNEXPECTED_ERROR"], f"An unexpected API error occurred during batch enqueue: {e}", cause=e).as_dict()
            logger.error(json.dumps({"event": "api_batch_enqueue_failed", "error": error_dict}), exc_info=True)
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error_dict)


    @api.get("/task_status/{task_id}", summary="Get the current status of a test task", response_model=TaskResult)
    async def api_get_task_status(task_id: str):
        ...
        logger.info(json.dumps({"event": "api_get_task_status", "task_id": task_id, "status": status_result.status}))
        return status_result

    @api.get("/metrics_data", summary="Get current Prometheus metrics", response_model=Dict[str, Any])
    ...
    @click.option('--config', default='runner.yaml', help="Path to runner config file.", type=click.Path(exists=True, path_type=Path))
    @click.option('--test-dir', type=click.Path(exists=True, path_type=Path), help="Directory containing test files.")
    @click.option('--code-dir', type=click.Path(exists=True, path_type=Path), help="Directory containing code files.")
    @click.option('--output-path', default='./runner_output', type=click.Path(path_type=Path), help="Path for output results.")
    ...
    def cli_run(config: Path, test_dir: Path, code_dir: Path, output_path: Path, dry_run: bool, timeout: Optional[int], task_id: Optional[str]):
        ...
        async def main_run():
            ...
            test_file_contents = {f.name: f.read_text(encoding='utf-8') for f in test_dir.iterdir() if f.is_file()}
            code_file_contents = {f.name: f.read_text(encoding='utf-8') for f in code_dir.iterdir() if f.is_file()}
    ...
    @click.option('--code-dir', type=click.Path(exists=True, path_type=Path), help="Directory containing code files.")
    @click.option('--output-path', default='./runner_output', type=click.Path(path_type=Path), help="Path for output results.")
    @click.option('--priority', default=0, type=int, help="Priority of the task (lower is higher priority).")
    @click.option('--dry-run', is_flag=True, help="Simulate the test run.")
    @click.option('--timeout', type=int, help="Override default timeout in seconds.")
    @click.option('--task-id', type=str, help="Optional task ID.")
    def cli_enqueue(config: Path, test_dir: Path, code_dir: Path, output_path: Path, priority: int, dry_run: bool, timeout: Optional[int], task_id: Optional[str]):
    ...
    def cli_gui(config: str, prod: bool):
        try:
            ...
            click.echo("Failed to import runner.runner_app. Ensure Textual and its dependencies are installed.", err=True)
            sys.exit(1)
        except Exception as e:
    ...
    def cli_api_server(host: str, port: int):
        try:
            ...
            uvicorn_run("runner.core:api", host=host, port=port, reload=True) # Use string import for uvicorn
        except ImportError:
            click.echo("Failed to import uvicorn or FastAPI. Install with 'pip install uvicorn fastapi'.", err=True)
            sys.exit(1)
        except Exception as e:
