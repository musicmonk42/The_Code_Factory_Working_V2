...
def ensure_init_files(base_dir):
    ...
    if base_path.exists():
        for subdir in base_path.rglob('*'):
            if subdir.is_dir():
                # Check if directory contains Python files (more efficient)
                if next(subdir.glob('*.py'), None) is not None:
                    init_file = subdir / '__init__.py'
                    if not init_file.exists():
                        init_file.write_text('# Auto-generated by conftest.py\\n')

ensure_init_files(code_path)
'''
