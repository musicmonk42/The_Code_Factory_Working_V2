...
def mock_factory_imports(monkeypatch):
    ...
    mock_retry = AsyncMock(
        name="retry_operation", side_effect=lambda func, **kwargs: func()
    )  # Just execute the func

    mock_metrics = {
        "CRYPTO_ERRORS": MagicMock(
            name="CRYPTO_ERRORS",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "KEY_LOAD_COUNT": MagicMock(
            name="KEY_LOAD_COUNT",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "KEY_CLEANUP_COUNT": MagicMock(
            name="KEY_CLEANUP_COUNT",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "SIGN_OPERATIONS": MagicMock(
            name="SIGN_OPERATIONS",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "SIGN_LATENCY": MagicMock(
            name="SIGN_LATENCY",
            labels=MagicMock(return_value=MagicMock(observe=MagicMock())),
        ),
        "VERIFY_OPERATIONS": MagicMock(
            name="VERIFY_OPERATIONS",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "VERIFY_LATENCY": MagicMock(
            name="VERIFY_LATENCY",
            labels=MagicMock(return_value=MagicMock(observe=MagicMock())),
        ),
        "KEY_ROTATIONS": MagicMock(
            name="KEY_ROTATIONS",
            labels=MagicMock(return_value=MagicMock(inc=MagicMock())),
        ),
        "HSM_SESSION_HEALTH": MagicMock(
            name="HSM_SESSION_HEALTH",
    ...
    monkeypatch.setattr(
        "generator.audit_log.audit_crypto.audit_crypto_factory.retry_operation",
        mock_retry,
        raising=False,
    )
    ...
    for name, mock in mock_metrics.items():
        monkeypatch.setattr(
            f"generator.audit_log.audit_crypto.audit_crypto_factory.{name}",
            mock,
            raising=False,
        )
    ...
    monkeypatch.setattr(
        "generator.audit_log.audit_crypto.audit_crypto_factory.CryptoInitializationError",
        Exception,
    )
    # --- END OF FIX ---
...
def mock_crypto_libs(monkeypatch):
    ...
    mocks = {
        "rsa": patch(
            ...
            return_value=mock_rsa_priv_key,
        ),
        "ec": patch(
            "cryptography.hazmat.primitives.asymmetric.ec.generate_private_key",
            return_value=mock_ec_priv_key,
        ),
        "ed25519": patch(
            "cryptography.hazmat.primitives.asymmetric.ed25519.Ed25519PrivateKey.generate",
            return_value=mock_ed_priv_key,
        ),
...
def mock_hsm_full(monkeypatch):
    ...
    monkeypatch.setattr(
        "generator.audit_log.audit_crypto.audit_crypto_provider.get_hsm_pin",
        mock_get_pin,
    )
...
def mock_accessors():
    ...
    fallback_secret_accessor = AsyncMock(
        name="fallback_secret_accessor",
        return_value=b"mock-fallback-secret-key-32bytes!",
    )
...
class TestCryptoProviderABC:
    ...
    async def test_base_class_close(self, mock_accessors, mock_settings):
        ...
        class TestProvider(CryptoProvider):
            ...
            async def verify(self, sig, data, key_id):
                pass

            async def generate_key(self, algo):
                pass

            async def rotate_key(self, old_key_id, algo):
                pass
...
@pytest.mark.usefixtures(
    "mock_settings", "mock_factory_imports", "mock_keystore", "mock_crypto_libs"
)
class TestSoftwareCryptoProvider:
        ...
        with (
            ...
            try:
                ...
                mock_keystore[1].assert_called_once_with(
                    "/tmp/test-keys", b"mock-master-key-for-keystore-32b"
                )
    ...
    async def test_rotate_key_success(self, software_provider, mock_keystore):
        ...
        mock_keystore[0].store_key.assert_any_call(
            ...
            1000.0,
            status="retired",
            retired_at=1234567890.0,
        )

    @pytest.mark.asyncio
    async def test_periodic_rotation_and_cleanup(
        self, software_provider, mock_keystore, mock_factory_imports, mock_crypto_libs
    ):
        ...
        software_provider.keys = {
            active_key: {
                "key_obj": mock_ed_priv_key,
                "algo": "ed25519",
                "creation_time": current_time - 100,
                "status": "active",
            },
            old_active_key: {
                "key_obj": mock_ed_priv_key,
                "algo": "ed25519",
                "creation_time": current_time - 4000,
                "status": "active",
            },  # 4000s > 3600s interval
            retired_key: {
                "key_obj": mock_ed_priv_key,
                "algo": "ed25519",
                "creation_time": 1000.0,
                "status": "retired",
                "retired_at": current_time - 100,
            },
            old_retired_key: {
                "key_obj": mock_ed_priv_key,
                "algo": "ed25519",
        ...
        with patch("asyncio.sleep", AsyncMock(side_effect=asyncio.CancelledError)):
            with patch.object(
                software_provider,
                "rotate_key",
                AsyncMock(return_value="new-key-from-rotate"),
            ) as mock_rotate_call:
                ...
                mock_factory_imports["log_action"].assert_any_call(
                    "key_delete",
                    key_id=old_retired_key,
                    provider="software",
                    status="success",
                )
...
@pytest.mark.usefixtures(
    "mock_settings", "mock_factory_imports", "mock_hsm_full", "mock_crypto_libs"
)
class TestHSMCryptoProvider:
    ...
    async def test_verify_success(self, hsm_provider):
        ...
        mock_pkcs11_session.verify.assert_called_once_with(
            mock_pub_key, b"data", b"sig", mechanism=ANY
        )
    ...
    async def test_rotate_key_success(self, hsm_provider):
        ...
        mock_pkcs11_session.destroy_object.assert_has_calls(
            [call(mock_priv_key_obj), call(mock_pub_key_obj)]
        )
