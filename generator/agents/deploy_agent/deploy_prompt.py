...
prompt_feedback_score = Gauge('deploy_prompt_feedback_score', 'Latest feedback score for generated prompts', ['target', 'variant'])
prompt_tokens_generated = Histogram('deploy_prompt_tokens_generated', 'Number of tokens in generated prompts', ['target', 'variant'])
FEW_SHOT_USAGE = Counter('deploy_prompt_few_shot_usage', 'Number of few-shot examples used', ['target', 'variant'])
TEMPLATE_LOADS = Counter('deploy_prompt_template_loads', 'Number of template loads', ['target', 'variant'])
...
async def optimize_deployment_prompt_text(prompt_text: str) -> str:
    ...
    try:
        ...
        LLM_LATENCY_SECONDS.labels(provider="deploy_prompt", model=SUMMARY_MODEL).observe(time.time() - start_time_summary)
        add_provenance({"action": "summarize_context", "model": SUMMARY_MODEL, "run_id": str(uuid.uuid4())})

        if len(prompt_text) != len(optimized):
            logger.info("Prompt content optimized from %d to %d characters.", len(prompt_text), len(optimized))
        return optimized
        
    except Exception as e:
        LLM_ERRORS_TOTAL.labels(provider="deploy_prompt", model=SUMMARY_MODEL, error_type=type(e).__name__).inc()
        logger.error("Prompt content optimization failed (LLM call): %s. Returning original prompt text.", e, exc_info=True)
...
async def get_commits(repo_path: str, limit: int = 5) -> str:
    ...
    if not os.path.exists(repo_path) or not os.path.isdir(repo_path):
        logger.warning("Repository path does not exist or is not a directory for get_commits: %s", repo_path)
        return "No repository found."
    try:
        cmd = ["git", "log", "-n", str(limit), "--no-merges", "--date=iso", "--pretty=format:%h %ad %s"]
        ...
        if proc.returncode == 0:
            ...
            logger.warning("Git log failed in %s (return code %d): %s", repo_path, proc.returncode, error_msg)
    ...
    for file_name in files_to_check:
        ...
        try:
            ...
            if file_name == 'requirements.txt':
                ...
                modules = re.findall(r'^\s*(?:require|replace)\s+([^\s]+)\s+([^\s]+)', content, re.MULTILINE)
                ...
                deps_list = []
                for match in re.finditer(r'<dependency>\s*<groupId>(.*?)</groupId>\s*<artifactId>(.*?)</artifactId>\s*(?:<version>(.*?)</version>)?\s*<\/dependency>', content):
                    group_id, artifact_id, version = match.groups()
                    deps_list.append({"groupId": group_id, "artifactId": artifact_id, "version": version if version else 'N/A'})
            ...
            logger.error("Unexpected error parsing dependency file %s: %s", file_name, e, exc_info=True)
...
class PromptTemplateRegistry:
    ...
    def _create_environment(self) -> Environment:
        ...
        env = Environment(loader=FileSystemLoader(self.template_dir), autoescape=True, enable_async=True) # Enable async rendering with autoescape for XSS protection
        ...
        env.filters['get_file_content'] = get_file_content # New filter to get specific file content
        env.filters['optimize_deployment_prompt_text'] = optimize_deployment_prompt_text # Use the LLM-based optimizer
        return env
    ...
    def _setup_hot_reload(self):
        ...
        class ReloadHandler(FileSystemEventHandler):
            ...
            def dispatch(self, event):
                ...
                if not event.is_directory and event.src_path.endswith('.jinja'):
                    logger.info("Template file changed: %s. Triggering template reload.", event.src_path)
        ...
        try:
            ...
            logger.warning("Could not start Watchdog observer (likely missing package or environment constraint): %s", e)
...
class DeployPromptAgent:
        ...
        for file in glob.glob(f"{few_shot_dir}/*.json"):
            try:
                ...
                if not isinstance(data, dict) or 'query' not in data or 'example' not in data:
                    logger.warning("Invalid few-shot example format in %s. Must contain 'query' and 'example' keys. Skipping.", file)
                    continue
                examples.append(data)
            except Exception as e:
                logger.error("Failed to load few-shot example from %s: %s", file, e, exc_info=True)
            ...
            try:
                ...
                for file_name in files:
                    ...
                    if file_path.is_file():
                        ...
                        logger.warning("File not found for context gathering: %s at %s. Skipping.", file_name, file_path)
                ...
                if attempt + 1 == attempts:
                    logger.error("Failed to gather context after %d attempts: %s", attempts, e, exc_info=True)
                    raise # Re-raise the final exception
                logger.warning("Failed to gather context (attempt %d/%d): %s. Retrying...", attempt + 1, attempts, e)
                await asyncio.sleep(2 ** attempt) # Exponential backoff
        
        return {} # Should be unreachable due to raise

    async def _read_single_file_for_context(self, file_path: Path, file_name: str) -> Tuple[str, Optional[str]]:
    ...
    async def retrieve_few_shot(self, query: str, top_k: int = 3) -> List[str]:
        ...
        try:
            ...
            logger.info("Retrieved %d few-shot examples for query: '%s...'", len(retrieved_examples), query[:50])
            return retrieved_examples
        except Exception as e:
            logger.error("Few-shot retrieval failed for query '%s...': %s", query[:50], e, exc_info=True)
            return []

    async def optimize_prompt_with_feedback(self, initial_prompt_content: str, target: str, variant: str, llm_model: str = "gpt-4o") -> str:
        """
        Optimizes the prompt based on historical feedback using a meta-LLM.
        """
        feedback_key = f"{target}_{variant}"
        score = self.previous_feedback.get(feedback_key) # Get actual score, not just existence check
        
        if score is not None: # Only optimize if we have feedback
            logger.info("Optimizing prompt with feedback (score: %f) for %s.", score, feedback_key)
            ...
            last_run_str = json.dumps(last_run_context, indent=2)[:1000] if last_run_context else "No specific last run context."
            ...
            try:
                ...
                response_from_meta_llm = await call_llm_api(
                    prompt=meta_prompt, model=llm_model, stream=False # Meta-LLM calls are typically not streamed
                )
                ...
                if optimized_content:
                    ...
                    logger.warning("Meta-LLM returned empty or malformed optimized prompt content. Using original.")
                    return initial_prompt_content

            except Exception as e:
                logger.error("Meta-LLM prompt optimization failed for %s: %s. Returning original prompt.", feedback_key, e, exc_info=True)
                if isinstance(e, LLMError):
                     # Re-raise as LLMError so the caller can distinguish API failures
                     raise
                return initial_prompt_content
        
        logger.debug("No feedback available for '%s'. Using original prompt content without meta-LLM optimization.", feedback_key)
        return initial_prompt_content
    ...
    async def build_deploy_prompt(self, 
                                  ...
                                  model_specific_info: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        with tracer.start_as_current_span("build_deploy_prompt") as span:
            ...
            if self.embedding_model and self.few_shot_examples and model_info.get("few_shot_support", False):
                ...
                if retrieved_few_shots:
                    ...
                    FEW_SHOT_USAGE.labels(target=target, variant=variant).inc(len(retrieved_few_shots)) # Metric for few-shot usage
            ...
            try:
                ...
                optimized_prompt_content = await optimize_deployment_prompt_text(base_prompt_content)
                ...
                optimization_model = model_info.get("optimization_model", "gpt-4o") # Allow specifying opt model
                final_prompt_content = await self.optimize_prompt_with_feedback(optimized_prompt_content, target, variant, optimization_model)
                ...
                try:
                    ...
                    logger.warning("Tiktoken encoding not found for model '%s'. Falling back to 'cl100k_base'.", tokenizer_model_name)
                    tokenizer = tiktoken.get_encoding("cl100k_base")

                prompt_tokens = len(tokenizer.encode(final_prompt_content))
                prompt_tokens_generated.labels(target=target, variant=variant).observe(prompt_tokens)
                
                latency = time.time() - start_time
                LLM_LATENCY_SECONDS.labels(provider="deploy_prompt", model=PROMPT_GEN_MODEL).observe(latency)
                ...
                logger.info("Prompt built successfully for %s (variant: %s). Tokens: %d, Latency: %.2fs", target, variant, prompt_tokens, latency)
                return final_prompt_content

            except Exception as e:
                # Log prompt generation error with context
                error_type = str(type(e).__name__)
                LLM_ERRORS_TOTAL.labels(provider="deploy_prompt", model=PROMPT_GEN_MODEL, error_type=error_type).inc()
                span.set_status(Status(StatusCode.ERROR, f"Prompt generation failed: {e}"))
                span.record_exception(e)
                logger.error("Failed to build prompt for %s (variant: %s): %s", target, variant, e, exc_info=True)
    ...
    async def ab_test_prompts(self, 
        ...
        prompt_strings = await asyncio.gather(*tasks, return_exceptions=True) # Gather results, including exceptions
        ...
        for variant, result in zip(variants, prompt_strings):
            if isinstance(result, Exception):
                logger.error("Failed to build prompt for A/B test variant '%s': %s", variant, result, exc_info=True)
        ...
        if prompts_to_score:
            for variant_to_score, prompt_to_score_string in prompts_to_score:
                ...
                scoring_tasks.append(call_ensemble_api(score_prompt_content, [{"model": SCORING_MODEL}], voting_strategy="majority")) 
            ...
            for i, (variant, _) in enumerate(prompts_to_score): # Iterate through the original prompts_to_score list
                score_response = scoring_responses[i]
                if isinstance(score_response, Exception):
                    logger.warning("Failed to get score for variant '%s': %s", variant, score_response)
                    ...
                    try:
                        ...
                        score_content_cleaned = re.sub(r'```(json)?', '', score_response.get('content', '{}')).strip('`').strip()
                        score_data = json.loads(score_content_cleaned)
                        score = float(score_data.get('score', 0.0)) # Default to 0.0 if score not found or invalid
                        ab_results_final[variant]['score'] = score
                        self.record_feedback(target, variant, score) # Record feedback
                    except (json.JSONDecodeError, ValueError) as e:
                        logger.warning("Failed to parse score JSON for variant '%s': %s. Raw: %s", variant, e, score_response.get('content', '')[:100])
                        ab_results_final[variant]['score'] = 0.0 # Assign lowest score on parse error
                        self.record_feedback(target, variant, 0.0)
    ...
    def record_feedback(self, target: str, variant: str, score: float):
        ...
        self.previous_feedback['last_run'] = {'variant': variant, 'score': score, 'timestamp': time.time()}
        logger.info("Recorded feedback for prompt '%s': %f. Updated last_run context.", feedback_key, score)
...
async def api_generate_prompt(request: Request) -> Response:
    ...
    async with api_semaphore:
        try:
            ...
            logger.error("API /generate_prompt error: %s", e, exc_info=True, extra={'run_id': str(uuid.uuid4())})
...
async def api_ab_test_prompts(request: Request) -> Response:
    ...
    async with api_semaphore:
        try:
            ...
            logger.error("API /ab_test_prompts error: %s", e, exc_info=True, extra={'run_id': str(uuid.uuid4())})
            return web.json_response({'status': 'error', 'message': str(e)}, status=500)
...
if __name__ == "__main__":
    import argparse
    import uvicorn # Typically used to run FastAPI apps, though aiohttp.web.run_app is used here.
...
""")
    ...
    parser.add_argument("--target", default='docker', help='Deployment target (e.g., docker, helm, terraform).')
    parser.add_argument("--files", nargs='+', default=['main.py', 'requirements.txt', 'README.md'], help='List of relevant files in the repository.')
    parser.add_argument("--instructions", default='Generate a minimal, secure configuration for production deployment.', help='Specific instructions for the prompt.')
    parser.add_argument("--variant", default="default", help='Prompt template variant (e.g., default, verbose, secure).')
    parser.add_argument("--repo-path", default=test_repo_path, help='Path to the repository containing files (defaults to temp_test_repo).')
    parser.add_argument("--server", action="store_true", help="Start the aiohttp web server for API endpoints.")
    ...
    parser.add_argument("--ab-test", action="store_true", help="Run A/B test on prompt variants in CLI mode.")
    parser.add_argument("--ab-variants", nargs='+', default=["default"], help="List of variants to A/B test (e.g., default verbose secure).")
    ...
    if args.server:
        ...
        async def run_cli_mode():
            if args.ab_test:
                print(f"Running A/B test for target '{args.target}' with variants: {args.ab_variants}...")
                # Pass model_specific_info for prompt generation.
                dummy_model_info_for_ab_test = {"name": "gpt-4o", "few_shot_support": True, "token_limit": 8000, "optimization_model": "gpt-4o"}
                ...
                for variant, result in ab_results.items():
                    ...
                    print("Prompt (first 500 chars):\n```\n%s%s\n```" % (result['prompt'][:500], '...' if len(result['prompt']) > 500 else ''))
                    print(f"  Prompt Length: {result['length']} characters")
                    print(f"  Prompt Hash: {result['hash']}")
                    print(f"  Rationale: {result['rationale']}")
                    print(f"  Provenance: {result['provenance']}")
                    print(f"  **Meta-LLM Score**: {result['score'] if result['score'] is not None else 'N/A'}")
                print("\n--- End A/B Test Results ---")
            else:
                # Single prompt generation in CLI mode
                print(f"Generating prompt for target '{args.target}' with variant '{args.variant}'...")
                # Pass model_specific_info for prompt generation.
                dummy_model_info_for_cli = {"name": "gpt-4o", "few_shot_support": True, "token_limit": 8000, "optimization_model": "gpt-4o"}
