...
from runner.runner_metrics import LLM_REQUESTS_TOTAL as LLM_CALLS_TOTAL, LLM_ERRORS_TOTAL, LLM_LATENCY_SECONDS # Use central metrics 
# -----------------------------------
...
async def scan_config_for_findings(config_text: str, config_format: str, dangerous_patterns: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    issue_count_gauge.labels(target=config_format, issue_type_category="OverallFindingsCount").set(len(findings))
            ...
            try:
                ...
                try:
                    ...
                    if hadolint_proc.returncode == 0:
                        ...
                        report['lint_status'] = 'warning' if report['build_status'] == 'success' else 'failed'
                ...
                report['security_findings'] = await scan_config_for_findings(config_content, target_type, DANGEROUS_CONFIG_PATTERNS)
                ...
                report['compliance_score'] = 1.0 if total_issues == 0 else max(0.0, 1.0 - (total_issues / 5.0))
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            try:
                ...
                if helm_lint_proc.returncode == 0:
                    ...
                    report['lint_issues'].extend([line for line in report['lint_output'].splitlines() if line.strip() and ("error" in line.lower() or "warning" in line.lower())])
