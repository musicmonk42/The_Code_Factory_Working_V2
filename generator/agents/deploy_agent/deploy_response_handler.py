...
class DockerfileHandler(FormatHandler):
    ...
    def normalize(self, raw: str) -> List[str]:
        ...
        # ✅ INDUSTRY STANDARD: Fix common CMD mistakes for Python/FastAPI apps.
        # LLMs frequently emit `CMD ["app.py"]` or `CMD ["python", "app.py"]` for
        # FastAPI projects whose entry-point is actually `app/main.py`.  Detect
        # and replace those patterns with the correct uvicorn invocation.
        #
        ...
        _is_python_image = any(line.upper().startswith("FROM PYTHON") for line in lines)
        if _is_python_image:
            # JSON exec form:  CMD ["<runtime>", "app.py"]  or  CMD ["app.py"]
            _exec_cmd_re = re.compile(
                r'^CMD\s+\[\s*(?:"python3?"\s*,\s*)?"app\.py"\s*\]$',
                re.IGNORECASE,
            )
            # Shell form:  CMD python[3] app.py
            _shell_cmd_re = re.compile(
                r'^CMD\s+python3?\s+app\.py\s*$',
                re.IGNORECASE,
            )
            _uvicorn_cmd = (
            ...
            lines = [
                _uvicorn_cmd
                if (_exec_cmd_re.match(line) or _shell_cmd_re.match(line))
                else line
                for line in lines
            ]
