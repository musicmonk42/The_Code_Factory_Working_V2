...
# OpenTelemetry setup
trace.set_tracer_provider(TracerProvider(resource=Resource.create({"service.name": "generator-plugin-wrapper"})))
...
class WorkflowInput(BaseModel):
    requirements: Dict[str, Any] = Field(..., description="Natural language requirements from README")
    config: Dict[str, Any] = Field(default_factory=dict, description="Configuration for workflow execution")
    repo_path: str = Field(..., description="The local path to the codebase repository.")
    ambiguities: List[str] = Field(default_factory=list, description="A list of ambiguous statements found in requirements.")
...
class WorkflowOutput(BaseModel):
    status: str = Field(..., description="The final status of the workflow (e.g., 'success', 'failed').")
    correlation_id: str = Field(..., description="The unique ID for tracing the workflow run.")
    final_results: Dict[str, Any] = Field(..., description="A dictionary containing the artifacts from each successful stage.")
...
@retry(
    ...
    before_sleep=lambda retry_state: logger.warning(f"Retrying workflow due to transient error: attempt {retry_state.attempt_number}")
)
async def run_generator_workflow(
    ...
    with tracer.start_as_current_span("generator_workflow", attributes={"correlation_id": correlation_id}) as span:
        try:
            input_data = WorkflowInput(requirements=requirements, config=config, repo_path=repo_path, ambiguities=ambiguities)
            logger.info(f"Starting Generator workflow [Correlation ID: {correlation_id}] for repo: {repo_path}")
            ...
            if clarifier and workflow_state["ambiguities"]:
                with workflow_latency.labels(stage="clarify", correlation_id=correlation_id).time():
                    ...
                    workflow_state["requirements"] = clarified_result.get("requirements", workflow_state["requirements"])
            ...
            with workflow_latency.labels(stage="deploy", correlation_id=correlation_id).time():
                ...
                logger.info(f"Deployment artifact generation stage complete [Correlation ID: {correlation_id}]")
            ...
            with workflow_latency.labels(stage="docgen", correlation_id=correlation_id).time():
                ...
                logger.info(f"Documentation generation stage complete [Correlation ID: {correlation_id}]")

            workflow_success.labels(correlation_id=correlation_id).inc()
            workflow_latency.labels(stage="total", correlation_id=correlation_id).observe(time.time() - start_time)
            logger.info(f"Generator workflow completed successfully [Correlation ID: {correlation_id}]")
            ...
            workflow_errors.labels(correlation_id=correlation_id, stage="unknown", error_type=type(e).__name__).inc()
            logger.critical(f"An unexpected critical error occurred in the workflow: {e} [Correlation ID: {correlation_id}]", exc_info=True)
            span.record_exception(e)
