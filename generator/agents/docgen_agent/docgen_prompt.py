...
async def optimize_prompt_content(prompt_text: str, max_tokens: int) -> str:
    ...
    try:
        ...
        logger.warning(f"Initial prompt exceeds max tokens ({initial_tokens} > {max_tokens}). Optimizing to prevent truncation of core instructions.")
        ...
        for match in matches:
            ...
            try:
                ...
                LLM_CALLS_TOTAL.labels(provider="docgen_prompt", model="gpt-3.5-turbo", task="summarize_context").inc()
                LLM_LATENCY_SECONDS.labels(provider="docgen_prompt", model="gpt-3.5-turbo", task="summarize_context").observe(time.time() - start_time)
                ...
                LLM_ERRORS_TOTAL.labels(provider="docgen_prompt", model="gpt-3.5-turbo", error_type=type(llm_e).__name__).inc()
        ...
        logger.warning("Section summarization was not sufficient. Performing final global summarization.")
        ...
        try:
            ...
            LLM_CALLS_TOTAL.labels(provider="docgen_prompt", model="gpt-3.5-turbo", task="summarize_context_global").inc()
            LLM_LATENCY_SECONDS.labels(provider="docgen_prompt", model="gpt-3.5-turbo", task="summarize_context_global").observe(time.time() - start_time_global)
            add_provenance({"action": "summarize_prompt_context_global", "model": "gpt-3.5-turbo"})
        except Exception as llm_e:
            logger.error(f"Global prompt summarization failed: {llm_e}. Proceeding with truncated text.", exc_info=True)
            LLM_ERRORS_TOTAL.labels(provider="docgen_prompt", model="gpt-3.5-turbo", error_type=type(llm_e).__name__).inc()
        ...
        logger.error(f"Prompt content optimization failed: {e}. Cannot proceed without optimized prompt.", exc_info=True)
...
async def get_dependencies(files_to_check: List[str], repo_path: str) -> str:
    ...
    for file_name in files_to_check:
        ...
        try:
            ...
            logger.error(f"Unexpected error parsing dependency file {file_name}: {e}", exc_info=True)
        ...
        class ReloadHandler(FileSystemEventHandler):
            ...
            def dispatch(self, event):
                if not event.is_directory and event.src_path.endswith('.jinja'):
                    logger.info(f"Template file changed: {event.src_path}. Triggering template reload.")
...
class DocGenPromptAgent:
        ...
        for file in glob.glob(f"{few_shot_dir}/*.json"): # *** FIX: Use full path ***
            try:
                ...
                if not isinstance(data, dict) or 'query' not in data or 'prompt' not in data:
                    logger.warning(f"Invalid few-shot example format in {file}. Must contain 'query' and 'prompt' keys. Skipping.")
                    continue
    ...
    async def gather_context(self, target_files: List[str], repo_path: str) -> Dict[str, Any]: # Added repo_path param
        """
        ...
        for file_name in target_files:
            ...
            if file_path.is_file():
                ...
                logger.warning(f"Target file not found for context gathering: {file_name} at {file_path}. Skipping.")
    ...
    async def retrieve_few_shot(self, query: str, top_k: int = 3) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Retrieved {len(retrieved_prompts)} few-shot examples for query: '{query[:50]}...'")
            return retrieved_prompts
        except Exception as e:
            logger.error(f"Few-shot retrieval failed for query '{query[:50]}...': {e}", exc_info=True)
            return []

    # REFACTORED: Uses call_ensemble_api
    async def enforce_sections(self, prompt_content: str, required_sections: List[str], llm_model: str = "gpt-4o") -> str:
        """
        ...
        try:
            ...
            LLM_CALLS_TOTAL.labels(provider="docgen_prompt", model=llm_model, task="enforce_sections").inc()
            LLM_LATENCY_SECONDS.labels(provider="docgen_prompt", model=llm_model, task="enforce_sections").observe(time.time() - start_time)
            add_provenance({"action": "enforce_prompt_sections", "model": llm_model, "run_id": str(uuid.uuid4())})

            if not enforced_prompt_content:
                LLM_ERRORS_TOTAL.labels(provider="docgen_prompt", model=llm_model, error_type="EmptyLLMResponse").inc()
                raise ValueError("Meta-LLM returned empty content when enforcing sections.")
                
            logger.info(f"Prompt sections enforced successfully by meta-LLM.")
            return enforced_prompt_content
            
        except Exception as e:
            if not isinstance(e, LLMError):
                LLM_ERRORS_TOTAL.labels(provider="docgen_prompt", model=llm_model, error_type=type(e).__name__).inc()
            logger.error(f"Failed to enforce required sections using meta-LLM: {e}", exc_info=True)
            raise RuntimeError(f"Critical error during prompt section enforcement: {e}") from e

    # REFACTORED: Uses call_ensemble_api
    async def optimize_prompt_with_feedback(self, initial_prompt_content: str, doc_type: str, template_name: str, llm_model: str = "gpt-4o") -> str:
        """
        ...
        if score is not None:
            ...
            try:
                ...
                # Add central runner metrics and provenance
                LLM_CALLS_TOTAL.labels(provider="docgen_prompt", model=llm_model, task="optimize_feedback").inc()
                LLM_LATENCY_SECONDS.labels(provider="docgen_prompt", model=llm_model, task="optimize_feedback").observe(time.time() - start_time)
                add_provenance({"action": "optimize_prompt_feedback", "model": llm_model, "run_id": str(uuid.uuid4())})
                ...
                if not isinstance(e, LLMError):
                    LLM_ERRORS_TOTAL.labels(provider="docgen_prompt", model=llm_model, error_type=type(e).__name__).inc()
                logger.error(f"Meta-LLM prompt optimization failed for {feedback_key}: {e}. Returning original prompt.", exc_info=True)
                return initial_prompt_content
        
        logger.debug(f"No feedback available for '{feedback_key}'. Using original prompt content.")
        return initial_prompt_content

    async def get_doc_prompt(self, 
                             doc_type: str, 
                             target_files: List[str], 
                             instructions: Optional[str] = None, 
                             template_name: str = "default", 
                             required_sections: Optional[List[str]] = None,
                             llm_model: str = "gpt-4o") -> str:
        """
        ...
        with tracer.start_as_current_span("get_doc_prompt_pipeline") as span:
            ...
            if self.embedding_model and self.few_shot_examples:
                ...
                if retrieved_few_shots:
                    ...
                    # REFACTORED: Replaced local metric with logger call
                    logger.debug(f"Used {len(retrieved_few_shots)} few-shot examples for {doc_type}/{template_name}")
            ...
            try:
                ...
                optimized_prompt_content = await optimize_prompt_content(base_prompt_content, max_context_tokens)
                ...
                if required_sections:
                    final_prompt_content = await self.enforce_sections(final_prompt_content, required_sections, llm_model)

                try:
                    tokenizer = tiktoken.encoding_for_model(llm_model)
                except KeyError:
                    logger.warning(f"Tiktoken encoding not found for model '{llm_model}'. Falling back to 'cl100k_base'.")
                    tokenizer = tiktoken.get_encoding("cl100k_base")

                prompt_tokens = len(tokenizer.encode(final_prompt_content))
                # REFACTORED: Replaced local metric with logger call
                logger.debug(f"Generated prompt for {doc_type}/{template_name} with {prompt_tokens} tokens.")
                
                latency = time.time() - start_time
                # REFACTORED: Replaced local metric with logger call
                logger.debug(f"Prompt generation latency for {doc_type}/{template_name}: {latency:.2f}s")
                ...
                logger.info(f"Prompt built successfully for {doc_type} (template: {template_name}). Tokens: {prompt_tokens}, Latency: {latency:.2f}s")
                return final_prompt_content

            except Exception as e:
                error_type = str(type(e).__name__)
                # REFACTORED: Replaced local metric with logger call
                logger.error(f"Prompt generation error for {doc_type}: stage=prompt_generation, error={error_type}", exc_info=True)
    ...
    async def ab_test_prompts(self, doc_type: str, target_files: List[str], instructions: Optional[str] = None, template_names: List[str] = ["default"], required_sections: Optional[List[str]] = None, llm_model: str = "gpt-4o") -> Dict[str, Dict[str, Any]]:
        """
        ...
        for i, template_name in enumerate(template_names):
            ...
            if isinstance(current_prompt_string, str) and current_prompt_string.startswith("ERROR:"):
                logger.error(f"Skipping scoring for template '{template_name}' due to prior prompt generation failure.")
        ...
        if prompts_for_scoring:
            ...
            for template_to_score, prompt_to_score_string in prompts_for_scoring:
                ...
                scoring_tasks.append(call_ensemble_api(
            ...
            for i, (template_name_scored, _) in enumerate(prompts_for_scoring):
                ...
                if isinstance(score_response, Exception):
                    logger.warning(f"Failed to get score for template '{template_name_scored}': {score_response}")
                    ...
                    try:
                        ...
                        LLM_CALLS_TOTAL.labels(provider="docgen_prompt", model=scoring_llm_model, task="ab_test_score").inc()
                        add_provenance({"action": "ab_test_prompt_score", "model": scoring_llm_model, "run_id": str(uuid.uuid4())})
                        ...
                        logger.warning(f"Failed to parse score JSON for template '{template_name_scored}': {e}. Raw: {score_content[:100]}")
                        ab_results_final[template_name_scored]['score'] = 0.0
                        self.record_feedback(doc_type, template_name_scored, 0.0)
    ...
    def record_feedback(self, doc_type: str, template_name: str, score: float):
        ...
        logger.info(f"Recorded feedback for prompt '{feedback_key}': {score}. Updated last_run context.")
...
async def api_record_prompt_feedback(request: Request) -> Response:
    ...
    if not all([doc_type, template_name is not None, isinstance(score, (int, float))]):
        raise web.HTTPBadRequest(reason="Missing or invalid 'doc_type', 'template_name', or 'score'.")
...
""")
        ...
        try:
            ...
            subprocess.run(["git", "commit", "-m", "Initial commit for test app"], cwd=test_repo_path, check=True, capture_output=True)
    ...
    parser = argparse.ArgumentParser(description="Documentation Prompt Generation CLI and API Server")
    parser.add_argument("--doc_type", default='README', help='Type of documentation (e.g., README, API_DOCS).')
    parser.add_argument("--target_files", nargs='+', default=['main.py', 'requirements.txt'], help='List of relevant files in the repository.')
    parser.add_argument("--instructions", default='Generate a concise README.', help='Specific instructions for the prompt.')
    parser.add_argument("--template_name", default="default", help='Template name (e.g., default, verbose).')
    parser.add_argument("--required_sections", nargs='+', default=None, help="List of section titles to explicitly enforce.")
    parser.add_argument("--llm_model", default="gpt-4o", help="Specific LLM model to use for prompt optimization/scoring.")
    ...
    parser.add_argument("--ab_test", action="store_true", help="Run A/B test on prompt variants in CLI mode.")
    parser.add_argument("--ab_template_names", nargs='+', default=["default"], help="List of template names to A/B test.")
    # REFACTORED: Added repo_path argument for CLI
    parser.add_argument("--repo_path", default=test_repo_path, help="Path to the repository for context gathering.") 
    ...
    async def run_cli_mode():
        ...
        if args.batch:
            batch_requests = [
                {"doc_type": "README", "target_files": ["main.py"], "instructions": "Very short README."},
                {"doc_type": "API_DOCS", "target_files": ["main.py"], "instructions": "Detailed API docs."}
            ]
            ...
            print(f"Running A/B test for doc_type '{args.doc_type}' with templates: {args.ab_template_names}...")
            ...
            for template_name, result_data in ab_results.items():
                ...
                print(f"  **Meta-LLM Score**: {result_data['score'] if result_data['score'] is not None else 'N/A'}")
            print("\n--- End A/B Test Results ---")
        else:
            print(f"Generating single prompt for doc_type '{args.doc_type}' with template '{args.template_name}'...")
