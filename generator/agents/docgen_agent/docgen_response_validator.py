...
process_calls_total = get_or_create_metric(
    Counter,
                ...
                try:
                    ...
                    with open(lock_file_path, 'w') as lock_file:
                        ...
                        try:
                            ...
                            # Lock held by another process - wait briefly and check if data exists
                            logger.info(f"Another process is downloading NLTK data '{name}', waiting...")
                            time.sleep(2)
                            # Check if data now exists (other process may have completed)
                            try:
                                nltk.data.find(path)
...
class MarkdownPlugin(DocGenPlugin):
    ...
    def validate(self, content: str, schema: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # More reasonable section checking
