...
class LicenseCompliance(CompliancePlugin):
    ...
    def check(self, docs_content: str) -> List[str]:
        issues = []
        license_patterns = [
            r'MIT License', r'Apache License', r'GNU General Public License', r'BSD License',
...
class PluginRegistry:
    ...
    def register(self, plugin: CompliancePlugin):
        ...
        if plugin_name in self.plugins:
            logger.warning(f"Plugin '{plugin_name}' is already registered. Replacing with new instance.")
            ...
            try:
                ...
                if spec and spec.loader:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (issubclass(obj, CompliancePlugin) and 
                            obj is not CompliancePlugin and
                            obj.__module__ == module_name):
                            # Instantiate and register
                            plugin_instance = obj()
                            self.register(plugin_instance)
                            logger.info(f"Loaded plugin '{plugin_instance.name}' from {plugin_file.name}")
...
class DocGenAgent:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(approval_webhook, json=payload, timeout=300) as resp:
                    if resp.status == 200:
                        ...
                        return False, f"Approval service returned an error (HTTP {resp.status})."
    ...
    async def generate_documentation(
        ...
        with tracer.start_as_current_span(
            "docgen_pipeline", 
            attributes={"run_id": run_id, "doc_type": doc_type}
        ) as span:
            logger.info(f"Docgen call started.", extra=log_extra)
            ...
            try:
                ...
                # 4. Call LLM
                logger.info(f"Calling central LLM client for '{doc_type}' documentation.", extra=log_extra)
                ...
                try:
                    if doc_content_str:
                        ...
                        ensemble_summary = await ensemble_summarizers(
                            content=doc_content_str,
                            summarizers=['doc_critique', 'default_summary'], # Use custom + built-in
                            max_length=300,
                            llm_model=llm_model
                        )
                        logger.info(f"Generated documentation summaries.", extra=log_extra)
                        add_provenance({
                            "action": "Doc Summary Generated", 
                            "run_id": run_id, 
                            "summary_hash": hashlib.sha256(summary.encode()).hexdigest(),
                            "ensemble_summary_hash": hashlib.sha256(ensemble_summary.encode()).hexdigest()
                        })
                    else:
                ...
                if human_approval:
                    ...
                    if rating < 0.5: # Refinement threshold
                        logger.info(f"Low rating ({rating}) received. Triggering refinement.", extra=log_extra)
                        try:
                            ...
                            logger.error(f"Refinement from feedback failed: {e}", exc_info=True, extra=log_extra)
                            UTIL_ERRORS.labels(util_name="refine_from_feedback", error_type=type(e).__name__).inc()
    ...
    async def _generate_documentation_streaming(
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Streaming summarization failed: {e}", exc_info=True, extra=log_extra)
    ...
    async def test_integration():
        ...
        with patch('runner.llm_client.call_llm_api', new_callable=AsyncMock) as mock_llm:
            ...
            mock_llm.side_effect = [
                {'content': 'Sample docs content.', 'usage': {'input_tokens': 10, 'output_tokens': 5}}, # For main doc gen
                {'content': 'Critique: Good structure.'} # For summarizer
            ]
            
            # Mock the response validator
            with patch('agents.docgen_agent.ResponseValidator.process_and_validate_response', new_callable=AsyncMock) as mock_validator:
