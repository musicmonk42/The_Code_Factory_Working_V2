...
ERROR_EXPLANATIONS = {
    'python': {
        'E501': {"rationale": "Line too long according to PEP 8.", "link": "https://peps.python.org/pep-0008/#maximum-line-length", "snippet_example": "long_line = 'x' * 100"},
        'W0611': {"rationale": "Unused import.", "link": "https://pylint.pycqa.org/en/latest/user_guide/messages/warning/unused-import.html", "snippet_example": "import os"},
        'E999': {"rationale": "Syntax error.", "link": "https://pycodestyle.pyqa.org/en/latest/intro.html#error-codes", "snippet_example": "if True"}
    },
    'javascript': {
        'no-unused-vars': {"rationale": "Unused variable.", "link": "https://eslint.org/docs/rules/no-unused-vars", "snippet_example": "var x = 1;"},
        'indent': {"rationale": "Incorrect indentation.", "link": "https://eslint.org/docs/rules/indent", "snippet_example": "function(){ return true; }"}
    },
    'typescript': {
        'no-unused-vars': {"rationale": "Unused variable.", "link": "https://eslint.org/docs/rules/no-unused-vars", "snippet_example": "const x: string = 'hello';"},
        'indent': {"rationale": "Incorrect indentation.", "link": "https://eslint.org/docs/rules/indent", "snippet_example": "function(){ return true; }"}
    },
    'go': {
        'golint': {"rationale": "Style issue detected by golangci-lint.", "link": "https://golangci-lint.run/usage/linters/", "snippet_example": "func myfunc() {}"},
        'errcheck': {"rationale": "Unchecked error return value.", "link": "https://golangci-lint.run/usage/linters/#errcheck", "snippet_example": "_, _ = fmt.Println()"}
    },
    'rust': {
        'clippy::unused_variable': {"rationale": "Unused variable detected by Clippy.", "link": "https://rust-lang.github.io/rust-clippy/master/index.html#unused_variable", "snippet_example": "let x = 1;"},
        'clippy::pedantic': {"rationale": "Pedantic style issue.", "link": "https://rust-lang.github.io/rust-clippy/master/index.html#pedantic", "snippet_example": "let mut v = vec![]"}
    },
    'java': {
        'MissingJavadocMethod': {"rationale": "Missing Javadoc for a method.", "link": "https://checkstyle.sourceforge.io/config_javadoc.html#MissingJavadocMethod", "snippet_example": "public void myMethod() {}"}
    }
}
...
def ruff_json(raw_output: str) -> List[Dict]:
    try:
        ...
        return [{
            ...
            'rationale': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('code', ''), {}).get('rationale', ''),
            'link': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('code', ''), {}).get('link', ''),
...
def pylint_json(raw_output: str) -> List[Dict]:
    try:
        ...
        return [{
            ...
            'rationale': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('message-id', ''), {}).get('rationale', ''),
            'link': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('message-id', ''), {}).get('link', ''),
...
def pyright_json(raw_output: str) -> List[Dict]:
    try:
        ...
        return [{
            ...
            'rationale': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('rule', ''), {}).get('rationale', ''),
            'link': ERROR_EXPLANATIONS.get('python', {}).get(issue.get('rule', ''), {}).get('link', ''),
            'snippet': ''
        } for issue in issues]
    except json.JSONDecodeError:
        ...
        for file_report in issues:
            ...
            for issue in file_report.get('messages', []):
                all_issues.append({
                    ...
                    'line': issue.get('line', 0),
                    'column': issue.get('column', 0),
                    'rationale': ERROR_EXPLANATIONS.get('javascript', {}).get(issue.get('ruleId', ''), {}).get('rationale', ''),
                    'link': ERROR_EXPLANATIONS.get('javascript', {}).get(issue.get('ruleId', ''), {}).get('link', ''),
                    'snippet': ''
                })
        return all_issues
    except json.JSONDecodeError:
...
def golangci_lint_json(raw_output: str) -> List[Dict]:
    try:
        ...
        return [{
            ...
            'rationale': ERROR_EXPLANATIONS.get('go', {}).get(issue.get('FromLinter', ''), {}).get('rationale', ''),
            'link': ERROR_EXPLANATIONS.get('go', {}).get(issue.get('FromLinter', ''), {}).get('link', ''),
...
def staticcheck_json(raw_output: str) -> List[Dict]:
    try:
        ...
        return [{
            ...
            'column': issue.get('location', {}).get('column', 0),
            'rationale': ERROR_EXPLANATIONS.get('go', {}).get(issue.get('code', ''), {}).get('rationale', ''),
            'link': ERROR_EXPLANATIONS.get('go', {}).get(issue.get('code', ''), {}).get('link', ''),
        ...
        for issue in issues:
            if issue.get('reason') == 'compiler-message' and issue.get('message', {}).get('code'):
                ...
                all_issues.append({
                    ...
                    'line': line_num,
                    'column': col_num,
                    'rationale': ERROR_EXPLANATIONS.get('rust', {}).get(code_id, {}).get('rationale', ''),
        ...
        for file in issues:
            ...
            for error in file.get('errors', []):
                ...
                all_issues.append({
                    ...
                    'rationale': ERROR_EXPLANATIONS.get('java', {}).get(code, {}).get('rationale', ''),
...
def spotbugs_json(raw_output: str) -> List[Dict]:
    try:
        ...
        for issue in issues:
            ...
            severity = 'low'
            if rank <= 4: severity = 'critical'
            elif rank <= 8: severity = 'high'
            elif rank <= 12: severity = 'medium'
            ...
            all_issues.append({
                ...
                'line': line_num,
                'column': 0,
                'rationale': ERROR_EXPLANATIONS.get('java', {}).get(type_id, {}).get('rationale', ''),
                'link': '',
                'snippet': ''
            })
        return all_issues
...
LINTER_CONFIG: Dict[str, List[Dict[str, Any]]] = {
    ...
    'go': [
        {
            'tool': 'golangci-lint',
            'cmd': ['golangci-lint', 'run', '--out-format=json', '--issues-exit-code=0'],
    ...
    'java': [
        ...
        },
        {
            ...
            'cmd': ['java', '-jar', '/app/spotbugs/lib/spotbugs.jar', '-xml:withMessages', '-json', '-output', 'spotbugs_output.json'],
...
class LintPlugin(ABC):
    @abstractmethod
    async def run_linter(self, tool_cfg: Dict[str, Any], file_path: str, project_dir: str) -> Dict[str, Any]:
        pass

    async def _run_tool(self, cmd: List[str], project_dir: str, tool_name: str, timeout: int, use_container: bool, container_image: Optional[str]) -> Dict[str, Any]:
        if use_container:
            if not shutil.which("docker"):
                # Capability check
                logger.error("Docker is not installed or not in PATH. Cannot run containerized linter.")
                return {'success': False, 'stdout': '', 'stderr': 'Docker not found', 'returncode': 1}
            if not container_image:
                logger.error(f"Configuration error: 'container_image' is not defined for {tool_name}.")
                return {'success': False, 'stdout': '', 'stderr': f"No container image specified for tool", 'returncode': 1}
            try:
                ...
                await asyncio.wait_for(pull_proc.wait(), timeout=300) # 5 minute timeout for pulling
                ...
                return {'success': False, 'stdout': '', 'stderr': 'Docker pull Timeout', 'returncode': 1}
            except Exception as e:
                logger.error(f"An unexpected error occurred during docker pull for '{container_image}': {e}")
            ...
            container_cmd = ["docker", "run", "--rm", "-v", f"{shlex.quote(str(abs_project_dir))}:/app", "-w", "/app", container_image] + cmd
            ...
            if not shutil.which(cmd[0]):
                ...
                return {'success': False, 'stdout': '', 'stderr': f"Tool not found: {cmd[0]}", 'returncode': 127}
            effective_cmd = cmd
        ...
        try:
            ...
            logger.error(f"Command '{effective_cmd[0]}' not found. Ensure it is installed and in the system's PATH.")
            return {'success': False, 'stdout': '', 'stderr': f"Command not found: {effective_cmd[0]}", 'returncode': 127}
        except Exception as e:
            logger.error(f"An unexpected error occurred while running '{tool_name}': {e}. Command: {' '.join(effective_cmd)}")
            return {'success': False, 'stdout': '', 'stderr': str(e), 'returncode': 1}
...
class PythonLintPlugin(LintPlugin):
    async def run_linter(self, tool_cfg: Dict[str, Any], file_path: str, project_dir: str) -> Dict[str, Any]:
                        ...
                        if section_key:
                            ...
                            if tool_cfg['tool'] == 'ruff':
                                if 'select' in current_section:
                                    cmd.extend(['--select', ','.join(current_section['select'])])
                                if 'ignore' in current_section:
                                    cmd.extend(['--ignore', ','.join(current_section['ignore'])])
                    except Exception as e:
        ...
        return await self._run_tool(cmd, project_dir, tool_cfg['tool'], tool_cfg.get('timeout', 60), tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
...
class RustLintPlugin(LintPlugin):
    async def run_linter(self, tool_cfg: Dict[str, Any], file_path: str, project_dir: str) -> Dict[str, Any]:
        ...
        return await self._run_tool(cmd, project_dir, tool_cfg['tool'], tool_cfg.get('timeout', 180), tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
...
class GoLintPlugin(LintPlugin):
    async def run_linter(self, tool_cfg: Dict[str, Any], file_path: str, project_dir: str) -> Dict[str, Any]:
        ...
        if not go_mod_path.exists():
            ...
            init_mod_result = await self._run_tool(init_go_mod_cmd, project_dir, "go_mod_init", 30, tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
            if not init_mod_result['success']:
                logger.error(f"Failed to initialize Go module in {project_dir}: {init_mod_result['stderr']}")
                return {'success': False, 'stdout': '', 'stderr': f"Go mod init failed: {init_mod_result['stderr']}", 'returncode': init_mod_result['returncode']}
            
            # Run go mod tidy to resolve dependencies
            tidy_result = await self._run_tool(["go", "mod", "tidy"], project_dir, "go_mod_tidy", 30, tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
            if not tidy_result['success']:
                 logger.warning(f"Go mod tidy failed (non-critical for single file lint): {tidy_result['stderr']}")

        cmd = tool_cfg['cmd'] + ['./...'] # Analyze all files in the module context

        return await self._run_tool(cmd, project_dir, tool_cfg['tool'], tool_cfg.get('timeout', 120), tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
...
class JavaLintPlugin(LintPlugin):
    async def run_linter(self, tool_cfg: Dict[str, Any], file_path: str, project_dir: str) -> Dict[str, Any]:
        ...
        if tool_cfg['tool'] == 'spotbugs':
            ...
            compile_result = await self._run_tool(compile_cmd, project_dir, "javac_compile", 60, tool_cfg.get('use_container', False), tool_cfg.get('container_image'))
            if not compile_result['success']:
                logger.error(f"Java compilation failed for {file_path}: {compile_result['stderr']}")
                return {'success': False, 'stdout': '', 'stderr': f"Compilation failed: {compile_result['stderr']}", 'returncode': compile_result['returncode']}
            
            # SpotBugs command needs to analyze the compiled classes directory
            cmd = ['java', '-jar', '/app/spotbugs/lib/spotbugs.jar', '-json', '-output', 'spotbugs_output.json', '-effort:max', '-r:medium']
...
async def run_single_lint(tool_cfg: Dict[str, Any], filename: str, content: str, temp_dir: str, language: str, project_dir: str) -> Dict[str, Any]:
    ...
    if not plugin:
        ...
        return {tool_cfg['tool']: {'raw': {'success': False, 'stdout': '', 'stderr': f"No plugin for {language}", 'returncode': 1}, 'parsed': []}}
    ...
    with tracer.start_as_current_span(f"lint.{tool_cfg['tool']}"):
        ...
        if raw_result['stdout'] and (raw_result['success'] or raw_result['returncode'] in [1, 2]):
            ...
            if callable(parser_func):
                ...
                parsed_errors.append({
                    'code': 'PARSER_ERROR', 'severity': 'critical', 'message': f"Parser for {tool_cfg['tool']} is invalid",
                    'file': filename, 'line': 0, 'column': 0, 'rationale': 'The configured parser function could not be executed.', 'link': '', 'snippet': ''
                })
        elif not raw_result['success']:
            logger.error(f"Linter tool {tool_cfg['tool']} failed unexpectedly with return code {raw_result['returncode']}: {raw_result['stderr']}")
            parsed_errors.append({
                'code': 'LINTER_EXEC_ERROR', 'severity': 'critical',
                'message': f"Linter execution failed: {raw_result['stderr'][:500]}", 'file': filename,
                'line': 0, 'column': 0, 'rationale': 'The linter tool encountered an unrecoverable error during execution.', 'link': '', 'snippet': ''
            })
        ...
        for err in parsed_errors:
            ...
            }.get(err.get('code'), "Review the rationale and link for specific guidance on how to fix this issue.")
        
        return {tool_cfg['tool']: {'raw': raw_result, 'parsed': parsed_errors}}

async def run_all_lints_and_checks(code_files: Dict[str, str], temp_dir: str, language: str = 'python', hitl_callback: Optional[Callable] = None, project_dir: Optional[str] = None) -> Dict[str, Any]:
    with tracer.start_as_current_span("run_lints", attributes={"language": language}):
        ...
        if not isinstance(code_files, dict) or not code_files or not all(isinstance(k, str) and isinstance(v, str) for k, v in code_files.items()):
            ...
            logger.error("Invalid code_files: must be a non-empty dictionary with string keys and values.")
            return {'lint_results': {}, 'all_errors': [{'code': 'INVALID_INPUT', 'severity': 'critical', 'message': 'Invalid code_files', 'line': 0, 'column': 0, 'rationale': '', 'link': '', 'snippet': ''}]}
        if language not in LINTER_CONFIG:
            LINT_ERRORS_COUNT.labels(language, 'critical').set(1)
            logger.error(f"Unsupported language: {language}. Supported: {list(LINTER_CONFIG.keys())}")
            return {'lint_results': {}, 'all_errors': [{'code': 'UNSUPPORTED_LANGUAGE', 'severity': 'critical', 'message': f'Unsupported language: {language}', 'line': 0, 'column': 0, 'rationale': '', 'link': '', 'snippet': ''}]}
        
        effective_project_dir = Path(project_dir) if project_dir and Path(project_dir).is_dir() else Path(temp_dir)
        ...
        for tool_cfg in tools:
            ...
            for filename, content in code_files.items():
                tasks.append(run_single_lint(tool_cfg, filename, content, temp_dir, language, str(effective_project_dir)))
        ...
        for res in results_list:
            if isinstance(res, Exception):
                ...
                all_errors_collected.append({
                    'code': 'LINTER_CRASH', 'severity': 'critical', 'message': f"Linter task failed: {str(res)}",
                    'line': 0, 'column': 0, 'rationale': 'A linter process crashed or encountered an unhandled exception.', 'link': '', 'snippet': ''
                })
                continue
        ...
        log_action("All Lints and Checks Completed", {
            "language": language,
...
if __name__ == '__main__':
    ...
    try:
        ...
        effective_project_dir = args.project_dir if args.project_dir and Path(args.project_dir).is_dir() else temp_dir
        ...
        if not Path(effective_project_dir).samefile(source_dir):
            ...
            logger.info(f"Copied files from {args.code_dir} to project context directory {effective_project_dir}")
        ...
        if not code_files:
            ...
            results = asyncio.run(run_all_lints_and_checks(code_files, temp_dir, args.lang, project_dir=effective_project_dir))
            ...
            if results['all_errors']:
                print("\nErrors by Severity:")
                for severity, count in Counter(e['severity'] for e in results['all_errors']).items():
                ...
                for error in results['all_errors']:
                    ...
                    print(f"  Location: Line {error.get('line', 'N/A')}, Column {error.get('column', 'N/A')}")
