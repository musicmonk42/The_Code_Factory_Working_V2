...
try:
    ...
    def count_tokens(prompt, model_name):
        logging.warning("Using dummy count_tokens as runner utility is unavailable. Returning a safe estimate.")
...
# OpenTelemetry Setup
resource = Resource(attributes={"service.name": "codegen-prompt-builder"})
...
if RAG_ENABLED:
    try:
        ...
        knowledge_base = [f"[{lang}] {p}" for lang, practices in BEST_PRACTICES.items() for p in practices]
        ...
        logger.error(f"Failed to initialize RAG components during import: {e}. RAG will be disabled.")
        RAG_ENABLED = False
elif not RAG_LIBRARIES_INSTALLED:
    logger.warning("RAG dependencies (faiss, sentence-transformers) not found. Semantic retrieval will be disabled.")
...
async def initialize_rag_store(redis_client: aioredis.Redis):
    ...
    if not RAG_ENABLED or not redis_client:
        logger.warning("Skipping RAG store initialization because feature is disabled or Redis client is unavailable.")
        return
    ...
    try:
        ...
        try:
            ...
            if "unknown command" in str(e).lower():
                logger.error("Redis server does not support the 'FT.INFO' command. RediSearch module may not be installed or enabled.")
            ...
            await redis_client.execute_command(
                "FT.CREATE", "rag_index", "ON", "HASH", "PREFIX", 1, "rag:",
...
async def retrieve_augmented_context(
    ...
    with tracer.start_as_current_span("retrieve_augmented_context"):
        ...
        if SEARCH_API_KEY:
            try:
                search_query = f"{target_language} " + " ".join(requirements.get('features', [])[:2])
                headers = {"Authorization": f"Bearer {SEARCH_API_KEY}", "Accept-Encoding": "gzip"}
                async with aiohttp.ClientSession() as session:
                    async with session.get(f"https://api.brave.com/search?q={search_query}", headers=headers, timeout=10) as resp:
        ...
        if RAG_ENABLED and redis_client and encoder:
            try:
                ...
                search_query = (
                    f"(@embedding:[VECTOR_RANGE $radius $embedding])=>{{$yield_distance_as: score}}"
                )
                params = {
                ...
                results = await redis_client.execute_command(
                    "FT.SEARCH", "rag_index", search_query,
                ...
                if retrieved_docs:
                    context_snippets.append("Relevant Context (from Redis Vector Store):\n- " + "\n- ".join(retrieved_docs))
            except Exception as e:
                logger.error(f"Failed to use Redis RAG: {e}")
                PROMPT_ERRORS.labels('RedisRAGFailure').inc()
        return "\n\n".join(context_snippets)

async def process_multi_modal_input(input_data: Dict[str, Any]) -> Tuple[Optional[List[str]], Optional[List[str]]]:
    """
    ...
    with tracer.start_as_current_span("process_multi_modal_input"):
        ...
        if not VISION_ENABLED:
            if input_data and input_data.get("image_urls"):
                logger.warning("Multi-modal inputs provided, but feature is disabled by configuration or missing dependencies. Inputs will be ignored.")
            return None, None
        ...
        try:
            ...
            for i, response in enumerate(responses):
                ...
                # Text Detection
                full_text = response.text_annotations[0].description if response.text_annotations else "No text detected."
                ...
                if response.localized_object_annotations:
                    ...
                    diagram_descriptions.append(f"Diagram objects at {url}:\n- " + "\n- ".join(diagram_objects))
        ...
        return (image_descriptions if image_descriptions else None,
...
async def translate_requirements_if_needed(requirements: Dict[str, Any]) -> Dict[str, Any]:
    """
...
async def build_code_generation_prompt(
    ...
    with PROMPT_BUILD_LATENCY.time():
        with tracer.start_as_current_span("build_prompt", attributes={"target_language": target_language, "target_framework": target_framework}):
            logger.info(f"Building prompt for language='{target_language}', framework='{target_framework}'")

            # 1. Input Validation
            if not isinstance(requirements, dict) or 'features' not in requirements or not isinstance(requirements['features'], list):
            ...
            if multi_modal_inputs and (not isinstance(multi_modal_inputs.get('image_urls'), list) or not all(isinstance(url, str) and url.startswith(('http://', 'https://')) for url in multi_modal_inputs.get('image_urls', []))):
                PROMPT_ERRORS.labels('InvalidInput').inc()
            ...
            rag_context = await retrieve_augmented_context(requirements, target_language, redis_client)
            ...
            try:
                ...
                logger.warning(f"Template '{template_name}' not found. Falling back to base.jinja2.")
                template = env.get_template("base.jinja2")  # Assume a generic fallback template exists
            ...
            if enable_meta_llm_critique and META_LLM_API_KEY:
                with tracer.start_as_current_span("meta_llm_critique"):
                    try:
                        ...
                        headers = {"Authorization": f"Bearer {META_LLM_API_KEY}", "Content-Type": "application/json"}
                        # --- Security Change: Use redacted prompt in API call ---
                        data = {"model": META_LLM_MODEL, "messages": [{"role": "user", "content": redacted_critique_prompt}]}
                        # --- End Security Change ---

                        async with aiohttp.ClientSession() as session:
                            async with session.post(META_LLM_API_URL, headers=headers, json=data, timeout=20) as resp:
                                resp.raise_for_status()
                                jr = await resp.json()
            ...
            token_count = count_tokens(prompt, META_LLM_MODEL) # Using META_LLM_MODEL as a representative LLM model name
            # --- End Token Counting Change ---
            
            if token_count > MAX_PROMPT_TOKENS:
                logger.warning(f"Final prompt exceeds token limit ({token_count} > {MAX_PROMPT_TOKENS}).")

            # --- Logging Change: Replace audit_logger.log_action with log_audit_event ---
            log_audit_event("Code Generation Prompt Built", {"prompt_length": len(prompt), "token_count": token_count, "target_language": target_language})
            # --- End Logging Change ---
            return prompt