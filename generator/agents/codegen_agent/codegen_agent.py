...
class AuditLogger(ABC):
    ...
    @abstractmethod
    def log_action(self, action: str, details: Dict[str, Any]): pass
...
class SecurityUtils:
    ...
    def apply_compliance(code: str, rules: Dict[str, Any]) -> List[str]:
        ...
        for banned_import in rules.get('banned_imports', []):
            if re.search(r'\bimport\s+' + re.escape(banned_import) + r'\b', code) or \
               re.search(r'\bfrom\s+' + re.escape(banned_import) + r'\b', code):
        ...
        if required_header and not code.startswith(required_header):
            violations.append(f"Compliance violation: Missing required license header.")
        max_length = rules.get('max_line_length')
        if max_length:
            for i, line in enumerate(code.splitlines()):
                if len(line) > max_length:
                    violations.append(f"Compliance violation: Line {i+1} exceeds max length of {max_length} characters.")
        return violations
...
class FeedbackStore(ABC):
    ...
    @abstractmethod
    async def get_feedback(self, req_hash: str) -> Optional[str]: pass
    @abstractmethod
    async def save_feedback(self, req_hash: str, feedback: str): pass
...
class SQLiteFeedbackStore(FeedbackStore):
    ...
    async def setup(self):
        try:
            ...
            conn.execute("CREATE TABLE IF NOT EXISTS hitl_feedback (req_hash TEXT PRIMARY KEY, feedback TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)")
            conn.close()
            # Cleanup job
            if not os.getenv("CODEGEN_DISABLE_CLEANUP_TASKS", "").lower() in {"1", "true", "yes"}:
    ...
    async def _cleanup_old_feedback(self):
        while True:
            ...
            try:
                conn = sqlite3.connect(self.db_file, check_same_thread=False)
                cursor = conn.cursor()
                cursor.execute("DELETE FROM hitl_feedback WHERE timestamp <= date('now', '-30 days')")
                conn.commit()
                conn.close()
    ...
    async def get_feedback(self, req_hash: str) -> Optional[str]:
        with sqlite3.connect(self.db_file, check_same_thread=False) as conn:
            result = conn.execute("SELECT feedback FROM hitl_feedback WHERE req_hash = ? ORDER BY timestamp DESC LIMIT 1", (req_hash,)).fetchone()
            return result[0] if result else None
    async def save_feedback(self, req_hash: str, feedback: str):
        with sqlite3.connect(self.db_file, check_same_thread=False) as conn:
            conn.execute("INSERT OR REPLACE INTO hitl_feedback (req_hash, feedback) VALUES (?, ?)", (req_hash, feedback))
            conn.commit()
...
class RedisFeedbackStore(FeedbackStore):
    ...
    async def get_feedback(self, req_hash: str) -> Optional[str]:
        if not self._redis: raise RuntimeError("Redis client not initialized.")
...
class CodeGenConfig:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.compliance_rules = config.get('compliance', {'banned_imports': [], 'banned_functions': [], 'max_line_length': 120})
...
async def hitl_review(code_files: Dict[str, str], feedback_store: FeedbackStore, req_hash: str, allow_interactive: bool, redis_client: aioredis.Redis, audit_logger: AuditLogger) -> Tuple[str, Optional[str]]:
    ...
    review_request = {
        ...
        "review_url": os.getenv("REVIEW_SYSTEM_URL", "https://review-system.example.com") + f"/{req_hash}"
    }
    ...
    for i in range(3):
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(review_system_webhook, json=review_request, timeout=10) as resp:
            ...
            # --- Audit/Logging Change: Use log_audit_event ---
            log_audit_event("HITLWebhookFailed", {"req_hash": req_hash, "attempt": i + 1, "error": str(e)})
            # --- End Audit/Logging Change ---
            logger.warning(f"Webhook to review system failed (attempt {i+1}): {e}")
            await asyncio.sleep(5)

    if not webhook_sent:
        logger.error("Failed to send webhook to review system after 3 attempts. Defaulting to rejection.")
    ...
    try:
        message = await asyncio.wait_for(pubsub.get_message(ignore_subscribe_messages=True), timeout=60)
...
if PLUGIN_AVAILABLE:
    @plugin(
        ...
        version="1.0.0",
        params_schema={
            "requirements": {"type": "dict", "description": "The requirements for the code to be generated."},
            "state_summary": {"type": "string", "description": "A summary of the current system state."},
            "config_path_or_dict": {"type": ["string", "dict"], "description": "Path to a YAML config file or a config dictionary."}
        },
        description="Generates code based on requirements, incorporating security scans and human-in-the-loop review.",
        safe=True
    )
    async def generate_code(requirements: Dict[str, Any], state_summary: str, config_path_or_dict: Union[str, Dict[str, Any]]) -> Dict[str, str]:
        """Main async function for code generation with fully pluggable and implemented components."""
        config = CodeGenConfig.from_file(config_path_or_dict) if isinstance(config_path_or_dict, str) else CodeGenConfig(config_path_or_dict)
        ...
        try:
            ...
            logger.warning("Redis not available. Distributed components will operate in-memory or be disabled.")
        ...
        with tracer.start_as_current_span("generate_code_request", attributes={"request.id": request_id, "backend": config.backend}):
            try:
                ...
                with tracer.start_as_current_span("call_llm"):
                    ...
                    if config.ensemble_enabled:
                        ...
                        response = response_dict["content"] if isinstance(response_dict, dict) and "content" in response_dict else str(response_dict)
                ...
                with tracer.start_as_current_span("parse_response_and_scan"):
                    ...
                    code_files = add_traceability_comments(code_files, requirements, requirements.get('target_language', 'python'))
                ...
                if getattr(config, "allow_interactive_hitl", False):
                    with tracer.start_as_current_span("hitl_review"):
                        ...
                        status, feedback = await hitl_review(
                            code_files, feedback_store, req_hash, True, redis_client, JsonConsoleAuditLogger()
                        )
                    if status != 'approved':
                        # --- Audit/Logging Change: Use log_audit_event ---
...
async def health_check():
    ...
    if failed_backends:
        return {"status": status, "details": f"Failed components: {', '.join(failed_backends)}"}
...
async def submit_review(review_submission: Dict[str, Any]):
    ...
    # Here we just log it.
    log_audit_event("HITL Review Submitted", {
        "req_hash": req_hash,
        "status": status,
        "feedback": feedback,
    })
...
if __name__ == "__main__":
    ...
    config_data = {
        ...
        'compliance': {'banned_functions': ['pickle'], 'max_line_length': 120, 'banned_imports': ['os', 'subprocess']}
    }
    with open("prod_config.yaml", "w") as f: yaml.dump(config_data, f)
    if not os.path.exists("templates"): os.makedirs("templates")
    with open("templates/python.jinja2", "w") as f: f.write("Generate a Python script. Requirements: {{ requirements.features }}. Respond ONLY with a valid JSON object with a 'files' key mapping filenames to code strings.")
    with open("templates/base.jinja2", "w") as f: f.write("Generate a generic script. Requirements: {{ requirements.features }}. Respond ONLY with a valid JSON object with a 'files' key mapping filenames to code strings.")

    requirements_data = {"features": ["Implement a function to calculate the nth Fibonacci number."], "target_language": "python"}
    ...
    async def main():
        ...
        generated_code = await generate_code(requirements_data, "Initial state.", "prod_config.yaml")
        print("\n--- Final Output ---")
        for filename, content in generated_code.items(): print(f"File: {filename}\n{content}\n")
    ...
    if os.getenv("CODEGEN_RUN_DEMO") == "1" and os.getenv('OPENAI_API_KEY'):
        ...
        print("Skipping example run: OPENAI_API_KEY environment variable is not set. Cannot run LLM.")
    else:
