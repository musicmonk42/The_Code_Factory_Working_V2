...
from generator.agents.deploy_agent.deploy_validator import (
    ...
    DockerValidator,
    HelmValidator,
    Validator, # FIX: Changed from BaseValidator
    scrub_text,
...
def mock_subprocess_failure():
    ...
    mock_process.communicate = AsyncMock(
        return_value=(b"", b"Error: build failed")
    )
    return mock_process
...
def mock_subprocess():
    ...
    mock_process = MagicMock()
    mock_process.communicate = AsyncMock(return_value=(b'{}', b'')) # Return empty JSON for Trivy/success
    mock_process.returncode = 0
    
    with patch('generator.agents.deploy_agent.deploy_validator.asyncio.create_subprocess_exec', new_callable=AsyncMock) as mock_create_exec:
...
class TestScanConfigForFindings:
    ...
    async def test_scan_privileged_container(self, privileged_pod_yaml):
        ...
        findings = await scan_config_for_findings(privileged_pod_yaml, "yaml", DANGEROUS_CONFIG_PATTERNS)
        
        assert len(findings) > 0
        assert any(
            f["category"] == "PrivilegedContainer" for f in findings
        )
...
"""
    ...
    async def test_scan_with_trivy(self, mock_subprocess, mock_trivy_json_output):
        ...
        mock_process.returncode = 0
        mock_process.communicate = AsyncMock(
            return_value=(mock_trivy_json_output, b"")
        )
...
CMD ["python", "app.py"]
"""
        findings = await scan_config_for_findings(clean_config, "dockerfile", DANGEROUS_CONFIG_PATTERNS)
...
"""
        mock_subprocess.return_value = MagicMock(
            returncode=1,
            communicate=AsyncMock(return_value=(b"", b"Missing FROM"))
        )
    ...
    async def test_fix_dockerfile(self, mock_llm, mock_subprocess):
        ...
        with patch("generator.agents.deploy_agent.deploy_validator.add_provenance") as mock_add_prov:
...
class TestComplianceScoring:
    ...
    async def test_compliance_score_with_issues(self, mock_subprocess, bad_dockerfile):
        ...
        mock_subprocess.return_value = MagicMock(
            returncode=1,
            communicate=AsyncMock(return_value=(b"", b"Multiple errors"))
        )
...
class TestSecurityToolIntegration:
    ...
    async def test_trivy_integration(self, mock_subprocess, mock_trivy_json_output):
        ...
        docker_mock = MagicMock(
            returncode=0,
            communicate=AsyncMock(return_value=(b"Build successful", b""))
        )
...
class TestErrorRecovery:
    ...
    async def test_graceful_tool_failure(self, mock_subprocess):
        ...
        report = await validator.validate("FROM alpine", "docker")
        assert "build_status" in report
        assert report["build_status"] in ["error", "failed", "tool_not_found", "internal_error"]
