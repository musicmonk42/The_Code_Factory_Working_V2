...
def setup_comprehensive_mocking():
    ...
    # Mock LLM clients
    runner_mock.llm_client.call_llm_api = AsyncMock(return_value={
        'content': 'mocked response',
        'usage': {'input_tokens': 100, 'output_tokens': 50},
        'cost_usd': 0.01
    })
    runner_mock.llm_client.call_ensemble_api = AsyncMock(return_value={
    ...
    testgen_prompt_mock._local_regex_sanitize = Mock(side_effect=lambda text: text.replace("@", "[REDACTED_EMAIL]").replace("555", "[REDACTED_PHONE]"))
    ...
    mock_vector_manager.return_value.query_relevant_context = AsyncMock(return_value={'codebase': []})
    ...
    def mock_log_perf_side_effect(template_hash, scores):
        ...
        if template_hash not in mock_tracker_data["performance"]:
            mock_tracker_data["performance"][template_hash] = {"runs": 0, "total_scores": {}, "history": []}
...
class TestModuleImports:
    ...
    def test_basic_imports(self):
        ...
        with patch.dict('sys.modules', mocks):
            ...
            from agents.testgen_agent import testgen_prompt
            from agents.testgen_agent import testgen_response_handler
            from agents.testgen_agent import testgen_validator
...
class TestValidationSystem:
    ...
    async def test_validator_creation(self):
        ...
        with patch.dict('sys.modules', mocks):
            from agents.testgen_agent.testgen_validator import CoverageValidator, validate_test_quality, VALIDATORS
