...
'''
    ...
    async def test_run_tool_success(self, parser):
        ...
        with patch.object(parser, '_run_external_tool') as mock_run:
            ...
            result = await parser._run_tool(['flake8', '--format=json'], 'test.py', 'linter', json_output=True)
...
class TestPublicAPI:
    ...
    async def test_parse_llm_response_success(self):
        ...
        with patch('agents.testgen_agent.testgen_response_handler.DefaultResponseParser._run_external_tool', return_value=""):
            with patch('agents.testgen_agent.testgen_response_handler.DefaultResponseParser._scan_for_security_issues', return_value=""):
