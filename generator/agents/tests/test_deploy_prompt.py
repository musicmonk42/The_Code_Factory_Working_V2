...
""")
        ...
        (examples_dir / "docker_example1.json").write_text(json.dumps({
            "query": "python web application",
            "example": "FROM python:3.9-slim\nWORKDIR /app\nCOPY requirements.txt .\nRUN pip install -r requirements.txt\nCOPY . .\nCMD [\"python\", \"app.py\"]"
        }))
        
        (examples_dir / "docker_example2.json").write_text(json.dumps({
...
def agent(temp_repo):
    ...
    with patch('os.path.exists', return_value=True), \
        ...
        agent = DeployPromptAgent(
            few_shot_dir=str(temp_repo / "few_shot_examples")
        )
...
class TestPromptBuilding:
    ...
    async def test_build_deploy_prompt_basic(self, temp_repo, agent, mock_model_info):
        ...
        with patch('generator.agents.deploy_agent.deploy_prompt.optimize_deployment_prompt_text', new_callable=AsyncMock) as mock_optimize:
    ...
    @pytest.mark.asyncio
    async def test_build_prompt_with_context(self, temp_repo, agent, mock_model_info, sample_context):
    ...
    async def test_build_prompt_with_optimization(self, mock_llm, temp_repo, agent, mock_model_info):
...
class TestTemplateManagement:
    ...
    def test_get_template_missing(self, agent):
        ...
        with pytest.raises(ValueError, match="Required template 'nonexistent_template.jinja' not found"):
...
class TestABTesting:
    ...
    async def test_ab_test_multiple_variants(self, temp_repo, agent):
        ...
        with patch('generator.agents.deploy_agent.deploy_prompt.optimize_deployment_prompt_text', new_callable=AsyncMock) as mock_optimize, \
             patch('generator.agents.deploy_agent.deploy_prompt.call_ensemble_api') as mock_llm:
...
class TestFeedbackSystem:
    ...
    def test_feedback_updates_metric(self, agent):
        ...
        with patch('generator.agents.deploy_agent.deploy_prompt.prompt_feedback_score') as mock_metric:
...
class TestProvenanceTracking:
    ...
    async def test_prompt_includes_provenance(self, temp_repo, agent, mock_model_info):
        ...
        with patch('generator.agents.deploy_agent.deploy_prompt.optimize_deployment_prompt_text', new_callable=AsyncMock) as mock_optimize, \
             patch('generator.agents.deploy_agent.deploy_prompt.add_provenance') as mock_provenance:
