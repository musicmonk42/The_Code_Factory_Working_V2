...
try:
    ...
    logger.info("sentry_sdk not found. External error reporting is disabled. `pip install sentry-sdk` to enable.")
    sentry_sdk = None
...
def scrub_text(text: str) -> str:
    ...
    try:
        ...
        entities = ["PERSON", "EMAIL_ADDRESS", "PHONE_NUMBER", "CREDIT_CARD", "US_SSN", "IP_ADDRESS", "URL", "NRP", "LOCATION", "API_KEY"]
        
        results = analyzer.analyze(text=text, entities=entities, language="en")
        scrubbed_content = anonymizer.anonymize(text=text, analyzer_results=results, anonymizers={"DEFAULT": {"type": "replace", "new_value": "[REDACTED]"}}).text
...
class Policy:
    ...
    validation_suite: List[str] = field(default_factory=lambda: ['coverage', 'mutation', 'stress_performance'])
...
def validate_policy(policy_dict: Dict[str, Any]) -> None:
    ...
    """
    required_fields = {
        'quality_threshold', 'primary_metric', 'validation_suite', 'max_refinements',
        'generation_llm_model', 'critique_llm_model', 'refinement_llm_model', 'self_heal_llm_model',
        'llm_retries', 'retry_wait_min', 'retry_wait_max'
    }
    missing = required_fields - set(policy_dict.keys())
    if missing:
        raise ValueError(f"Missing required policy fields: {', '.join(missing)}.")
    
    if not isinstance(policy_dict['quality_threshold'], (int, float)) or not (0 <= policy_dict['quality_threshold'] <= 100):
        raise ValueError(f"Invalid quality_threshold '{policy_dict['quality_threshold']}'. Must be a number between 0 and 100.")
    if policy_dict['primary_metric'] not in ['coverage_percentage', 'mutation_score', 'stress_performance_score']:
        raise ValueError(f"Invalid primary_metric '{policy_dict['primary_metric']}'. Must be one of: 'coverage_percentage', 'mutation_score', 'stress_performance_score'.")
    if not isinstance(policy_dict['validation_suite'], list) or not all(isinstance(v, str) for v in policy_dict['validation_suite']):
    ...
    if not isinstance(policy_dict['max_refinements'], int) or policy_dict['max_refinements'] < 0:
        raise ValueError(f"Invalid max_refinements '{policy_dict['max_refinements']}'. Must be a non-negative integer.")
    if not isinstance(policy_dict['llm_retries'], int) or policy_dict['llm_retries'] < 1:
        raise ValueError(f"Invalid llm_retries '{policy_dict['llm_retries']}'. Must be a positive integer.")
    if not isinstance(policy_dict['retry_wait_min'], (int, float)) or policy_dict['retry_wait_min'] <= 0:
        raise ValueError(f"Invalid retry_wait_min '{policy_dict['retry_wait_min']}'. Must be a positive number.")
    if not isinstance(policy_dict['retry_wait_max'], (int, float)) or policy_dict['retry_wait_max'] < policy_dict['retry_wait_min']:
        raise ValueError(f"Invalid retry_wait_max '{policy_dict['retry_wait_max']}'. Must be greater than or equal to retry_wait_min.")
    if not all(isinstance(policy_dict[f], str) and policy_dict[f] for f in ['generation_llm_model', 'critique_llm_model', 'refinement_llm_model', 'self_heal_llm_model']):
...
class TestGenAgent:
    ...
    async def _load_code_files(self, target_files: List[str]) -> Dict[str, str]:
        ...
        if errors_found:
            raise ValueError(f"Critical error: Failed to load some code files. Details: {'; '.join(errors_found)}")
    ...
    async def _run_validation_suite(self, code_files: Dict[str, str], test_files: Dict[str, str], language: str, policy: Policy, run_id: str) -> Dict[str, Any]:
        """
        ...
        with tracer.start_as_current_span("run_validation_suite") as span:
            ...
            async def limited_validate(v_type: str):
                async with semaphore:
                    try:
                        return await validate_test_quality(code_files, test_files, language, validation_type=v_type)
                    except Exception as e:
                        logger.error(f"Validation type '{v_type}' failed: {e}", exc_info=True, extra=log_extra)
                        if sentry_sdk:
                            sentry_sdk.capture_exception(e)
                        raise RuntimeError(f"Validation type '{v_type}' failed critically: {e}") from e
            ...
            for v_type, result in zip(policy.validation_suite, results):
                if isinstance(result, Exception):
                    aggregated_report[v_type] = {"error": str(result), "status": "failed_exception"}
                    logger.error(f"Agent run failed validation: ValidationException_{v_type}", extra=log_extra)
                else:
                    aggregated_report[v_type] = result
                    if result.get('status') == 'failed':
                         logger.warning(f"Agent run validation reported failure: ValidationReportedFailed_{v_type}", extra=log_extra)

            span.set_attribute("validation.results", json.dumps(aggregated_report))
            return aggregated_report

    async def _generate_report_markdown(self, history: List[Dict[str, Any]], language: str, final_status: str, run_id: str) -> str:
        """
        ...
        badge_color = "green" if final_status == "success" else "yellow" if final_status == "completed_below_threshold" else "red"
        report_parts.append(f"# Test Generation Report\n\n")
        report_parts.append(f"![Status](https://img.shields.io/badge/status-{final_status.replace('_', '%20')}-{badge_color}.svg)\n")
        ...
        report_parts.append(f"**Full Details:** [Audit Log for Run](https://your.audit.log.platform/run/{run_id})\n\n")
        ...
        for i, step_data in enumerate(history):
            ...
            primary_metric_name = history[0].get('policy', {}).get('primary_metric', 'coverage_percentage')
            # Safely navigate the nested dict structure
            coverage = validation_report.get('coverage', {}).get('metrics', {}).get(primary_metric_name, 'N/A')
            
            report_parts.append(f"## Attempt {i+1} (Action: `{action}`, Primary Metric: {primary_metric_name.replace('_', ' ').title()}: {coverage}%)\n")
            report_parts.append(f"- **Status:** `{step_data.get('status', 'N/A')}`\n")
            
            if action == 'refinement' and 'critique_response' in step_data:
                report_parts.append(f"- **Critique:** \n```text\n{scrub_text(step_data['critique_response'].get('content', 'N/A'))}\n```\n")
            
            report_parts.append("### Validation Results\n")
            for v_type, results in validation_report.items():
                report_parts.append(f"- **{v_type.replace('_', ' ').title()}**: `{json.dumps(results, indent=2)}`\n")
            report_parts.append("\n")
...
"""
            try:
                ...
                logger.warning(f"Failed to generate PlantUML diagram: {e}. Skipping diagram.", exc_info=True, extra=log_extra)
        ...
        try:
            ...
            logger.warning("Git command not found. Cannot fetch changelog for report.", extra=log_extra)
            report_parts.append("\n## Recent Commits\n_Git command not available._\n")
        except subprocess.CalledProcessError as e:
            logger.warning(f"Git log failed (return code {e.returncode}): {e.stderr.strip()}", extra=log_extra)
            report_parts.append(f"\n## Recent Commits\n_Failed to retrieve changelog (Error: {e.stderr.strip()})._\n")
        except Exception as e:
            logger.warning(f"Unexpected error fetching git log: {e}", exc_info=True, extra=log_extra)
    ...
    async def generate_tests(self, target_files: List[str], language: str, policy: Policy) -> Dict[str, Any]:
        ...
        with tracer.start_as_current_span("generate_tests_agent_run") as span:
            ...
            log_extra = {"run_id": run_id, "language": language}
            logger.info(f"Testgen agent run initiated", extra=log_extra)
            start_time = time.time()
            ...
            try:
                ...
                add_provenance({"action": "code_files_loaded", "run_id": run_id, "files_count": len(code_files), "file_names": list(code_files.keys())}, **log_extra)
                ...
                for attempt in range(policy.max_refinements + 1):
                    ...
                    logger.info(f"Starting generation/refinement attempt {attempt + 1}/{policy.max_refinements + 1}.", extra=log_extra)
                    ...
                    step_model = policy.generation_llm_model if attempt == 0 else policy.refinement_llm_model

                    if attempt == 0:
                        span.add_event("Building initial generation prompt.")
                        generation_prompt = build_agentic_prompt('generation', language=language, code_files=code_files)
                        logger.info(f"Calling LLM for initial test generation.", extra=log_extra)
                        llm_response_from_generation = await self._call_llm_with_retry(
                            generation_prompt, language, policy.generation_llm_model, run_id, 'generation'
                        )
                    else:
                        span.add_event(f"Building refinement prompt for attempt {attempt}.")
                        last_step_in_history = history[-1]
                        last_critique_content = last_step_in_history.get('critique_response', {}).get('content', 'No specific critique provided.')
                        last_validation_report = last_step_in_history.get('validation_report', {})
                        last_generated_tests_content = last_step_in_history.get('generated_tests_content', {})

                        refinement_prompt = build_agentic_prompt(
                            'refinement', language=language, code_files=code_files,
                        ...
                        logger.info(f"Calling LLM for refinement attempt {attempt}.", extra=log_extra)
                        llm_response_from_generation = await self._call_llm_with_retry(
                            refinement_prompt, language, policy.refinement_llm_model, run_id, 'refinement'
                        )
                    
                    history.append({"run_id": run_id, "action": step_action, "llm_model": step_model, "status": "in_progress", "policy": policy.__dict__})

                    try:
                        generated_tests_this_attempt = parse_llm_response(llm_response_from_generation.get('content', ''), language=language)
                        if not generated_tests_this_attempt:
                            raise ValueError("Parsed tests are empty after generation/refinement.")
                    except ValueError as e:
                        logger.warning(f"Failed to parse LLM generated tests ({e}). Attempting self-healing.", exc_info=True, extra=log_extra)
                        span.add_event("Parse failure, attempting self-heal.")
                        self_heal_prompt = build_agentic_prompt('self_heal', language=language, generated_tests=llm_response_from_generation.get('content', ''), error_message=str(e))
                        
                        try:
                            healed_llm_response = await self._call_llm_with_retry(
                                self_heal_prompt, language, policy.self_heal_llm_model, run_id, 'self_heal'
                            )
                            generated_tests_this_attempt = parse_llm_response(healed_llm_response.get('content', ''), language=language)
                            if not generated_tests_this_attempt:
                                raise ValueError("Parsed tests are still empty after self-healing.")
                            add_provenance({"action": "self_heal_success", "run_id": run_id, "original_error": str(e)}, **log_extra)
                            logger.info(f"Self-healing successful.", extra=log_extra)
                            span.add_event("Self-healing successful.")
                        except Exception as he:
                            logger.error(f"Self-healing failed: {he}", exc_info=True, extra=log_extra)
                            raise RuntimeError(f"Failed to generate valid tests even after self-healing: {he}") from he
                    ...
                    validation_report = await self._run_validation_suite(code_files, generated_tests_this_attempt, language, policy, run_id)
                    add_provenance({"action": f"validation_report_{attempt}", "run_id": run_id, "report": validation_report}, **log_extra)
                    history[-1]['validation_report'] = validation_report
                    
                    # Safely access the nested primary metric
                    current_metric_value = 0.0
                    try:
                        current_metric_value = validation_report[policy.primary_metric.split('_')[0]]['metrics'][policy.primary_metric]
                    except KeyError:
                        logger.warning(f"Primary metric '{policy.primary_metric}' not found in validation report. Defaulting to 0.", extra=log_extra)

                    logger.info(f"Attempt {attempt + 1} validation completed. {policy.primary_metric}: {current_metric_value:.2f}%.", extra=log_extra)
                    
                    if current_metric_value >= policy.quality_threshold:
                        logger.info(f"Quality threshold met ({policy.primary_metric}: {current_metric_value:.2f}% >= {policy.quality_threshold}%).", extra=log_extra)
                        ...
                        logger.info(f"Quality threshold NOT met. Current {policy.primary_metric}: {current_metric_value:.2f}%.", extra=log_extra)
                        
                        best_metric_so_far = 0.0
                        try:
                            best_metric_so_far = best_validation_report[policy.primary_metric.split('_')[0]]['metrics'][policy.primary_metric]
                        except KeyError:
                    ...
                    if attempt >= policy.max_refinements:
                        logger.warning(f"Max refinements ({policy.max_refinements}) reached. Final score: {current_metric_value:.2f}%.", extra=log_extra)
                    ...
                    critique_prompt = build_agentic_prompt(
                        'critique', language=language, code_files=code_files,
                        generated_tests=generated_tests_this_attempt,
                        validation_feedback=json.dumps(validation_report)
                    )
                    
                    logger.info(f"Calling LLM for critique for attempt {attempt+1}.", extra=log_extra)
                    critique_response = await self._call_llm_with_retry(
                        critique_prompt, language, policy.critique_llm_model, run_id, 'critique'
                    )
                    history[-1]['critique_response'] = critique_response
                    add_provenance({"action": f"critique_response_{attempt}", "run_id": run_id, "response_summary": scrub_text(critique_response.get('content', '')[:200])}, **log_extra)
                    
                else: 
                    logger.warning(f"Loop completed without meeting threshold. Max refinements reached.", extra=log_extra)
                    final_status = "completed_below_threshold"
                ...
                try:
                    final_metric_value = best_validation_report[policy.primary_metric.split('_')[0]]['metrics'][policy.primary_metric]
                except KeyError:
                    pass
                
                logger.info(f"Agent run finished. Final status: {final_status}. Refinements: {attempt}. Final metric '{policy.primary_metric}': {final_metric_value}", extra={**log_extra, "status": final_status, "refinements": attempt, "final_metric_value": final_metric_value})
                
                explainability_report = await self._generate_report_markdown(history, language, final_status, run_id)
                
                span.set_status(Status(StatusCode.OK, f"Agent run completed with status: {final_status}."))
                ...
                logger.error(f"Configuration, validation, or parsing error: {e}", exc_info=True, extra=log_extra)
                span.set_status(Status(StatusCode.ERROR, str(e)))
                span.record_exception(e)
                if sentry_sdk: sentry_sdk.capture_exception(e)
                raise RuntimeError(f"Agent run failed due to configuration or parsing error: {e}") from e
            except RuntimeError as e:
                logger.error(f"LLM operation error or critical validation suite failure: {e}", exc_info=True, extra=log_extra)
                span.set_status(Status(StatusCode.ERROR, str(e)))
                span.record_exception(e)
                if sentry_sdk: sentry_sdk.capture_exception(e)
                raise RuntimeError(f"Agent run failed due to LLM operation or critical validation failure: {e}") from e
            except Exception as e:
                logger.critical(f"Unexpected critical error during agent execution: {e}", exc_info=True, extra=log_extra)
                span.set_status(Status(StatusCode.ERROR, str(e)))
                span.record_exception(e)
                if sentry_sdk: sentry_sdk.capture_exception(e)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run the TestGenAgent to generate tests for code files.")
    parser.add_argument("target_files", nargs='+', help="One or more target code files to generate tests for.")
    parser.add_argument("--language", required=True, help="The programming language of the files (e.g., python, javascript).")
    ...
    try:
        ...
        logger.critical(f"Invalid policy configuration: {e}. Please correct your policy settings.", exc_info=True)
        sys.exit(1)
