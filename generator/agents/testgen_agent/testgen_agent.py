...
'''
    ...
    def _generate_fastapi_tests(self, content: str, file_path: str) -> str:
        ...
        if len(parts) >= 3 and parts[0] == 'generated':
            ...
            parts = parts[2:]
            # After stripping, parts will always have at least 1 element
            # (defensive check for future code changes)
            file_path_obj = Path(*parts) if parts else Path(file_path_obj.name)
        ...
        if not endpoints:
            ...
            for i, endpoint in enumerate(endpoints):
                ...
                if method == 'get':
                    ...
                    lines.append('        # Send POST with empty payload; 422 is valid if required fields are missing.')
                    lines.append('        # Update payload with valid fields from the endpoint schema for stricter tests.')
                    lines.append('        payload = {}')
                    lines.append(f'        response = client.post("{path}", json=payload)')
                    lines.append('        assert response.status_code in (200, 201, 422), f"Expected 200/201/422, got {response.status_code}"')
                elif method in ('put', 'patch'):
                    lines.append('        # Send PUT/PATCH with empty payload; 422 is valid if required fields are missing.')
                    lines.append('        # Update payload with valid fields from the endpoint schema for stricter tests.')
                    lines.append('        payload = {}')
                    lines.append(f'        response = client.{method}("{path}", json=payload)')
                    lines.append('        assert response.status_code in (200, 201, 422), f"Expected 200/422, got {response.status_code}"')
                elif method == 'delete':
