...
class MultiVectorDBManager:
    ...
    def __init__(self, path: str = "./chroma_db"):
        ...
        self.collections = {
            'codebase': self.client.get_or_create_collection(name="codebase", embedding_function=self.embedding_function),
            'tests': self.client.get_or_create_collection(name="tests", embedding_function=self.embedding_function),
            'docs': self.client.get_or_create_collection(name="docs", embedding_function=self.embedding_function),
            'dependencies': self.client.get_or_create_collection(name="dependencies", embedding_function=self.embedding_function),
            'historical_failures': self.client.get_or_create_collection(name="historical_failures", embedding_function=self.embedding_function),
        }
    ...
    async def add_files(self, collection_name: str, files: Dict[str, str]):
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(None, lambda: self.collections[collection_name].add(documents=documents, metadatas=metadatas, ids=ids))
            ...
            logger.error(f"Failed to add files to {collection_name} collection: {e}", exc_info=True)
            raise

    async def query_relevant_context(self, query_text: str, collections: List[str] = None, n_results: int = 3) -> Dict[str, str]:
        """
        ...
        for col_name in collections:
            try:
                results = await asyncio.get_event_loop().run_in_executor(None, lambda: self.collections[col_name].query(query_texts=[query_text], n_results=n_results))
                context_str = ""
...
class AdvancedTemplateTracker:
    ...
    def _load(self) -> Dict:
        ...
        if os.path.exists(self.db_path):
            try:
                ...
                logger.error(f"Failed to decode template performance JSON {self.db_path}: {e}. Initializing new store.")
    ...
    def log_performance(self, template_hash: str, scores: Dict[str, float]):
        ...
        if template_hash not in self.data["performance"]:
            self.data["performance"][template_hash] = {"runs": 0, "total_scores": {}, "history": []}
    ...
    def rollback_template(self, template_name: str, to_version: str):
        ...
        if os.path.exists(version_path):
            ...
            with open(active_path, 'w') as f:
                f.write(content)
            content_hash = self.data["version_hashes"].get(template_name, {}).get(to_version, "unknown")
            logger.info(f"Rolled back {template_name} to version {to_version} with hash {content_hash}.")
            ...
            logger.error(f"Version {to_version} not found for {template_name}. Directory contents: {os.listdir(TEMPLATE_VERSION_DIR)}")
            raise FileNotFoundError(f"Version {to_version} not found for {template_name}.")

    async def auto_evolve_template(self, template_name: str, current_content: str, performance_data: Dict) -> str:
        """
        ...
        try:
            ...
            if not new_content or new_content == current_content:
                ...
                LLM_ERRORS_TOTAL.labels(provider="testgen_prompt", model=model, error_type="EmptyLLMResponse").inc()
            ...
            if not isinstance(e, LLMError):
                LLM_ERRORS_TOTAL.labels(provider="testgen_prompt", model=model, error_type=type(e).__name__).inc()
            return current_content
    ...
    def check_for_regression(self, template_hash: str, new_scores: Dict, primary_metric: str):
        ...
        if new_scores.get(primary_metric, 0) < avg_score * 0.9:  # 10% regression
            ...
            template_name = next((name for name, versions in self.data["version_hashes"].items() if template_hash in versions.values()), None)
            if template_name and self.data["versions"].get(template_name):
                best_version = max(self.data["versions"][template_name], key=lambda v: self.get_avg_score(self.data["version_hashes"][template_name][v], primary_metric))
    ...
    def _setup_hot_reload(self):
        ...
        class TemplateReloadHandler(FileSystemEventHandler):
            ...
            def dispatch(self, event):
                if not event.is_directory and event.src_path.endswith('.jinja') and event.event_type in ('created', 'modified', 'deleted'):
                    logger.info(f"Template file changed: {event.src_path} (Event: {event.event_type}). Triggering reload.")
                    task = asyncio.create_task(self.tracker._reload_templates())
                    task.add_done_callback(lambda fut: logger.error(f"Error in template reload: {fut.exception()}", exc_info=True) if fut.exception() else None)
...
class AdaptivePromptDirector:
    ...
    def _get_template_content(self, template_name: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to load template {template_name}: {e}. Directory contents: {os.listdir(TEMPLATE_DIR)}")
            template_file_name = f"{template_name}.jinja" if not template_name.endswith('.jinja') else template_name
            try:
    ...
    def _select_template(self, candidate_names: List[str], primary_metric: str) -> str:
        ...
        candidate_hashes = {
            ...
            for name in candidate_names if self._get_template_content(name)
        }
        if not candidate_hashes:
            logger.error(f"No valid templates found for candidates: {candidate_names}. Directory: {os.listdir(TEMPLATE_DIR)}")
        ...
        if random.random() < 0.10: # Epsilon-greedy exploration (10% chance)
            ...
            best_hash = max(candidate_hashes, key=lambda h: self.tracker.get_avg_score(h, primary_metric))
            selected_hash = best_hash
        
        logger.debug(f"Selected template {candidate_hashes[selected_hash]} with hash {selected_hash}")
    ...
    def get_rich_context(self, repo_path: str, code_files: Dict[str, str]) -> Dict[str, Any]:
        ...
        contexts = {}
        try:
            commits = subprocess.check_output(["git", "-C", repo_path, "log", "--oneline", "-n", "5"]).decode()
    ...
    def adapt_chain(self, prior_quality: float, threshold: float = 0.8) -> List[str]:
        ...
        """
        chain = ['generation', 'critique', 'refinement'] if prior_quality >= threshold else ['generation', 'critique', 'refinement', 'additional_critique']
...
class AgenticPromptBuilder(abc.ABC):
    ...
    async def _manage_tokens(self, prompt: str) -> str:
        ...
        if tokens > MAX_PROMPT_TOKENS:
            ...
            try:
                ...
                logger.error(f"Failed to summarize prompt for token management: {e}", exc_info=True)
                if not isinstance(e, LLMError):
                    LLM_ERRORS_TOTAL.labels(provider="testgen_prompt", model=model, error_type=type(e).__name__).inc()
    ...
    def _add_explainability(self, prompt: str, rationale: str, context_hashes: Dict[str, str], template_hash: str, chain_steps: List[str]) -> str:
        """
...
class DefaultPromptBuilder(AgenticPromptBuilder):
    ...
    REFACTORED: Uses central runner logging.
    """
    async def get_template_candidates(self, prompt_type: str, language: str, framework: str) -> List[str]:
        """
        ...
        logger.debug(f"Template candidates for {prompt_type}, {language}, {framework}: {valid_candidates}")
    ...
    async def build(self, prompt_type: str, **kwargs) -> str:
        ...
        context_hashes = {k: hashlib.sha256(v.encode()).hexdigest() for k, v in rag_contexts.items()}
        ...
        for key, value in kwargs.items():
            if isinstance(value, str):
                ...
                sanitized_kwargs[key] = {k: self._advanced_sanitize(v) if isinstance(v, str) else v for k, v in value.items()}
        ...
        prompt = self._add_explainability(prompt, rationale, context_hashes, template_hash, chain_steps)
...
def initialize_codebase_for_rag(repo_path: str):
    ...
    logger.info(f"Multi-RAG initialization complete. Indexed files across collections.")
