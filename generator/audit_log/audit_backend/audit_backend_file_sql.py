...
from .audit_backend_core import (
    ...
    compute_hash, ENCRYPTER, COMPRESSION_ALGO, COMPRESSION_LEVEL, TamperDetectionError,
...
class FileBackend(LogBackend):
    ...
    async def _append_single(self, prepared_entry: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"FileBackend WAL write failed for entry {prepared_entry.get('entry_id')}: {e}", exc_info=True)
            ...
            asyncio.create_task(send_alert(f"FileBackend WAL write failed. Potential data loss on crash.", severity="high"))
            ...
            async with aiofiles.open(self.log_file, "r") as f:
                ...
                for line in reversed(lines): # Read from end for more recent entries first
                    ...
                    try:
                        ...
                        logger.warning(f"FileBackend: Malformed JSON entry in log file, skipping: '{stripped_line[:100]}...'. Error: {jde}")
                        BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="MalformedEntry").inc()
                    except Exception as parse_e:
                        logger.warning(f"FileBackend: Unexpected error parsing log entry: '{stripped_line[:100]}...'. Error: {parse_e}")
                        BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="LogParseError").inc()

        except IOError as io_e:
            logger.error(f"FileBackend: I/O error reading log file '{self.log_file}': {io_e}", exc_info=True)
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="FileReadError").inc()
            asyncio.create_task(send_alert(f"FileBackend: Failed to read log file '{self.log_file}'. Check permissions/disk.", severity="critical"))
    ...
    async def _migrate_schema(self):
        ...
        if current_on_disk_version >= self.schema_version:
            logger.info(f"FileBackend schema is already at v{self.schema_version} or newer. No migration needed.")
            return

        logger.info(f"Migrating FileBackend from v{current_on_disk_version} to v{self.schema_version}")
        
        # Create a unique timestamped backup file
        timestamp_str = datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%d%H%M%S')
        backup_file = os.path.normpath(f"{self.log_file}.backup.v{current_on_disk_version}.{timestamp_str}")
        ...
        try:
            ...
            if os.path.exists(self.log_file):
                ...
                logger.info(f"FileBackend migration: No existing log file '{self.log_file}' found to backup. Proceeding with empty new file.")
            ...
            logger.info(f"FileBackend migration completed successfully. Migrated {migrated_count} entries to '{self.log_file}'.")
            ...
            asyncio.create_task(send_alert(f"FileBackend migration failed. Attempting rollback.", severity="critical"))
            ...
            if os.path.exists(backup_file):
                try:
                    ...
                    logger.info(f"FileBackend migration: Rollback successful. Restored '{self.log_file}' from backup.")
                except Exception as rollback_e:
                    logger.critical(f"FileBackend migration rollback failed critically: {rollback_e}", exc_info=True)
                    asyncio.create_task(send_alert(f"CRITICAL: FileBackend migration rollback failed. Data potentially corrupted/lost.", severity="emergency"))
            else:
                logger.warning(f"FileBackend migration: No backup file '{backup_file}' found for rollback.")
    ...
    async def _health_check(self) -> bool:
        ...
        """
        try:
            temp_health_file = os.path.normpath(f"{self.dir_path}/.health_check_{uuid.uuid4()}") # Unique name in the directory
            ...
            logger.warning(f"FileBackend health check failed due to I/O error (e.g., permissions, disk full): {io_e}")
                ...
                async for line in f:
                    ...
                    if stripped_line:
                        try:
                            ...
                            return entry.get("schema_version", 1) # Default to 1 if not present (older schema)
                        except json.JSONDecodeError as jde:
                            logger.warning(f"FileBackend: Could not parse first line of '{self.log_file}' to determine schema version. Assuming v1. Error: {jde}")
                            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="SchemaDetectParseError").inc()
                            return 1 # Malformed line, assume it's old and needs migration
            return 1 # If file is empty after reading
        except IOError as io_e:
            logger.warning(f"FileBackend: Could not read '{self.log_file}' to determine schema version due to I/O error: {io_e}. Assuming v1.")
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="SchemaDetectIOError").inc()
            return 1
        except Exception as e:
            logger.warning(f"FileBackend: Unexpected error determining schema version for '{self.log_file}': {e}. Assuming v1.")
        ...
        try:
            ...
            if os.path.exists(self.log_file):
                try:
                    ...
                    logger.warning(f"FileBackend atomic: Could not read existing log file '{self.log_file}'. Proceeding as if empty for this batch. Error: {io_e}")
                    BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="AtomicReadExistingError").inc()
            ...
            for line in existing_content_lines:
                try:
                    ...
                    logger.warning(f"FileBackend atomic: Malformed existing line during deduplication: '{line.strip()[:100]}...'. Skipping.")
                    BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="AtomicDedupeParseError").inc()
            ...
            for prepared_entry in prepared_entries:
                if prepared_entry["entry_id"] not in existing_entry_ids:
                    ...
                    logger.info(f"FileBackend atomic: Skipping duplicate entry_id '{prepared_entry['entry_id']}' found in batch.")
            ...
            await asyncio.get_event_loop().run_in_executor(None, lambda: os.replace(temp_file, self.log_file))
            logger.debug(f"FileBackend: Atomically flushed {len(new_lines_for_file)} new entries to '{self.log_file}'.")
            ...
            if os.path.exists(self.wal_file):
                try:
                    ...
                    logger.warning(f"FileBackend: Failed to clear WAL file '{self.wal_file}': {io_e}. Will retry later or require manual cleanup.")
                    BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="WALCleanupError").inc()
            ...
            asyncio.create_task(send_alert(f"FileBackend atomic batch write failed. Data might be inconsistent or lost for batch.", severity="high"))
            ...
            if os.path.exists(temp_file):
                try:
                    ...
                    logger.error(f"FileBackend: Failed to cleanup temp file '{temp_file}' after atomic write failure: {cleanup_e}", exc_info=True)
    ...
    async def recover_wal(self):
            ...
            async with aiofiles.open(self.wal_file, "r") as wal_f:
                ...
                async for line in wal_f:
                    ...
                    try:
                        ...
                        logger.warning(f"FileBackend WAL recovery: Malformed JSON entry in WAL file at line {line_num}, skipping. Error: {jde}. Line: '{stripped_line[:100]}...'")
                        BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="WALMalformedEntry").inc()
                        # Consider moving malformed WAL to a quarantine folder for forensics.
                        # For now, just skip.
        except IOError as io_e:
            logger.error(f"FileBackend WAL recovery: I/O error reading WAL file '{self.wal_file}': {io_e}", exc_info=True)
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="WALReadError").inc()
            asyncio.create_task(send_alert(f"FileBackend WAL read failed during recovery. Potential data loss.", severity="high"))
            return # Cannot proceed if WAL cannot be read

        if not wal_entries:
            logger.info("FileBackend WAL file is empty or contains no valid entries after parsing. No recovery action taken.")
            try:
                await asyncio.to_thread(os.remove, self.wal_file) # Clean empty/invalid WAL
            except IOError as io_e:
                logger.warning(f"FileBackend WAL recovery: Failed to remove empty WAL file '{self.wal_file}': {io_e}.")
            return
                    ...
                    async for line in log_f:
                        ...
                        if stripped_line:
                            try:
                                ...
                                logger.warning(f"FileBackend WAL recovery: Malformed main log entry during deduplication, skipping. Error: {jde}. Line: '{stripped_line[:100]}...'")
                                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="MainLogMalformedEntry").inc()
            except IOError as io_e:
                logger.error(f"FileBackend WAL recovery: I/O error reading main log for deduplication: {io_e}", exc_info=True)
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="MainLogReadError").inc()
                asyncio.create_task(send_alert(f"FileBackend main log read failed during WAL recovery deduplication. Data might contain duplicates.", severity="medium"))

        existing_entry_ids = {entry.get("entry_id") for entry in existing_log_entries if "entry_id" in entry}
        ...
        for entry in wal_entries:
            if entry.get("entry_id") and entry["entry_id"] not in existing_entry_ids:
                ...
                logger.debug(f"FileBackend WAL recovery: Skipping duplicate entry_id '{entry.get('entry_id')}' found in WAL.")


        if new_entries_from_wal_to_add:
            try:
                temp_recovery_file = os.path.normpath(f"{self.dir_path}/.tmp_recovery_{uuid.uuid4()}") # Unique temp for recovery
                ...
                async with aiofiles.open(temp_recovery_file, "w") as tmp_f:
                    ...
                    await tmp_f.flush()
                    await asyncio.get_event_loop().run_in_executor(None, lambda: os.fsync(tmp_f.fileno()))
                
                await asyncio.get_event_loop().run_in_executor(None, lambda: os.replace(temp_recovery_file, self.log_file))
                logger.info(f"FileBackend WAL recovery: Successfully appended {len(new_entries_from_wal_to_add)} unique entries to '{self.log_file}'.")
                
                try:
                    await asyncio.to_thread(os.remove, self.wal_file)
                    logger.info(f"FileBackend WAL file '{self.wal_file}' cleared after successful recovery.")
                except IOError as io_e:
                    logger.warning(f"FileBackend WAL recovery: Failed to remove WAL file '{self.wal_file}': {io_e}. Manual cleanup may be required.")
                    BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="WALCleanupError").inc()

            except Exception as recover_write_e:
                logger.error(f"FileBackend WAL recovery failed during atomic write: {recover_write_e}", exc_info=True)
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="WALRecoveryWriteError").inc()
                asyncio.create_task(send_alert(f"CRITICAL: FileBackend WAL recovery write failed. Data inconsistency likely.", severity="emergency"))
                if os.path.exists(temp_recovery_file):
                    try:
                        await asyncio.to_thread(os.remove, temp_recovery_file)
                    except Exception as cleanup_e:
                        logger.error(f"FileBackend: Failed to cleanup temp recovery file after failure: {cleanup_e}", exc_info=True)
        else:
            logger.info("FileBackend WAL recovery: No new unique entries to add to main log or WAL was empty.")
            try:
                await asyncio.to_thread(os.remove, self.wal_file)
            except IOError as io_e:
                logger.warning(f"FileBackend WAL recovery: Failed to remove empty WAL file '{self.wal_file}': {io_e}.")
...
class SQLiteBackend(LogBackend):
    ...
    async def _init_connection(self):
        ...
        def connect_sync():
            try:
                ...
                logger.critical(f"SQLiteBackend: Database error during connection: {db_e}", exc_info=True)
                raise
            except Exception as e:
                logger.critical(f"SQLiteBackend: Unexpected error during connection: {e}", exc_info=True)
                raise
        ...
        try:
            ...
            asyncio.create_task(send_alert(f"SQLiteBackend initialization failed: {e}", severity="critical"))
            raise
    ...
    async def _append_single(self, prepared_entry: Dict[str, Any]) -> None:
        ...
        try:
            ...
            await asyncio.to_thread(
                ...
                (prepared_entry["entry_id"], prepared_entry["encrypted_data"], prepared_entry["timestamp"], prepared_entry["schema_version"], prepared_entry["_audit_hash"])
            )
            # No commit here; commit is handled by the _atomic_context (transaction)
        except sqlite3.Error as db_e:
            logger.error(f"SQLiteBackend: Database error during append for entry '{prepared_entry.get('entry_id')}': {db_e}", exc_info=True)
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="AppendDBError").inc()
            raise # Re-raise to trigger transaction rollback
        except Exception as e:
            logger.error(f"SQLiteBackend: Unexpected error during append for entry '{prepared_entry.get('entry_id')}': {e}", exc_info=True)
    ...
    async def _query_single(self, filters: Dict[str, Any], limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            asyncio.create_task(send_alert(f"SQLiteBackend query failed due to database error: {db_e}", severity="high"))
            ...
            asyncio.create_task(send_alert(f"SQLiteBackend query failed unexpectedly: {e}", severity="high"))
            raise
    ...
    async def _migrate_schema(self):
        ...
        def migrate_sync_logic():
            ...
            try:
                ...
                """)
                logger.debug(f"SQLiteBackend migration: Created temporary table '{temp_table_name}'.")
                ...
                if old_table_name != temp_table_name:
                    ...
                    old_table_exists = False # Old table is the same as the target (already migrated)
                ...
                if old_table_exists:
                    ...
                    for row in old_rows_cursor:
                        ...
                        if self.tamper_detection_enabled:
                            temp_audit_entry_for_hash = original_audit_entry.copy()
                            temp_audit_entry_for_hash.pop("_audit_hash", None)
                            audit_hash = compute_hash(json.dumps(temp_audit_entry_for_hash, sort_keys=True).encode("utf-8"))
                        ...
                        cursor.execute(
                            ...
                            (timestamp, entry_id, self.schema_version, audit_hash, updated_base64_data)
                        )
                        migrated_count += 1
                    
                    logger.info(f"SQLiteBackend migration: Copied {migrated_count} entries from '{old_table_name}'.")
                    cursor.execute(f"DROP TABLE {old_table_name}")
                    logger.debug(f"SQLiteBackend migration: Dropped old table '{old_table_name}'.")
                else:
                    logger.info("SQLiteBackend migration: No old schema table found to copy data from.")
                ...
                logger.debug(f"SQLiteBackend migration: Renamed '{temp_table_name}' to '{new_table_name}'.")

                # 4. Recreate indexes for the new table
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_{new_table_name}_timestamp ON {new_table_name} (timestamp DESC);")
                cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_{new_table_name}_entry_id ON {new_table_name} (entry_id);")
                ...
                if integrity_result and integrity_result[0] != 'ok':
                    raise MigrationError(f"Database integrity check failed after migration: {integrity_result[0]}")
                logger.info("SQLiteBackend migration: Database integrity check passed.")

                cursor.execute("COMMIT")
                logger.info(f"SQLiteBackend migration completed successfully to v{self.schema_version}.")

            except sqlite3.Error as db_e:
                cursor.execute("ROLLBACK")
                logger.error(f"SQLiteBackend migration failed due to database error: {db_e}", exc_info=True)
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="MigrationDBError").inc()
                asyncio.create_task(send_alert(f"SQLiteBackend migration failed due to DB error: {db_e}. Rolling back.", severity="critical"))
                cursor.execute(f"DROP TABLE IF EXISTS {temp_table_name}") # Clean up temp table on rollback
                ...
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="MigrationFailed").inc()
                asyncio.create_task(send_alert(f"SQLiteBackend migration failed unexpectedly: {e}. Rolling back.", severity="critical"))
                cursor.execute(f"DROP TABLE IF EXISTS {temp_table_name}") # Clean up temp table on rollback
    ...
    async def _health_check(self) -> bool:
        ...
        try:
            ...
            integrity_check_result = await asyncio.to_thread(self.conn.execute, "PRAGMA integrity_check;")
            result = await asyncio.to_thread(integrity_check_result.fetchone)
            if result and result[0] != 'ok':
                logger.warning(f"SQLiteBackend health check: Database integrity check failed: {result[0]}")
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="IntegrityCheckFailed").inc()
                asyncio.create_task(send_alert(f"SQLiteBackend database integrity check failed. DB may be corrupted.", severity="high"))
                return False
    ...
    async def _get_current_schema_version(self) -> int:
        ...
        if self.conn is None:
            ...
            try:
                ...
                temp_conn = await asyncio.to_thread(lambda: sqlite3.connect(self.db_file, check_same_thread=False))
                temp_conn.row_factory = sqlite3.Row # Ensure row_factory is set for temp_conn
                cursor = await asyncio.to_thread(temp_conn.cursor)
            except sqlite3.Error as db_e:
                logger.warning(f"SQLiteBackend: Could not connect to '{self.db_file}' to determine schema version. Assuming v1. Error: {db_e}")
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="SchemaDetectDBError").inc()
                return 1
            except Exception as e:
                logger.warning(f"SQLiteBackend: Unexpected error connecting to '{self.db_file}' for schema version. Assuming v1. Error: {e}")
                BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="SchemaDetectError").inc()
                return 1
        else:
        ...
        try:
            ...
            cursor.execute(f"SELECT name FROM sqlite_master WHERE type='table' AND name='logs_v{self.schema_version}';")
            ...
            logger.warning(f"SQLiteBackend: Database error checking schema version: {db_e}. Assuming v1.")
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="SchemaDetectQueryError").inc()
            return 1
        finally:
    ...
    async def _atomic_context(self, prepared_entries: List[Dict[str, Any]]) -> AsyncIterator[None]:
        ...
        try:
            ...
            logger.debug(f"SQLiteBackend: Transaction committed successfully for {len(prepared_entries)} entries.")
            yield # Success
        except Exception as e:
            await asyncio.to_thread(self.conn.rollback)
            logger.error(f"SQLiteBackend transaction rolled back due to error: {e}", exc_info=True)
            BACKEND_ERRORS.labels(backend=self.__class__.__name__, type="TransactionRollback").inc()
            asyncio.create_task(send_alert(f"SQLiteBackend transaction rolled back. Data inconsistency possible for batch.", severity="high"))
