...
class S3Backend(LogBackend):
    ...
    def _validate_params(self):
        ...
        self.old_key_prefix = self.params.get(
            "old_key_prefix", f"audit_logs_v{SCHEMA_VERSION - 1}/"
        )
    ...
    async def _query_single(
        ...
        try:
            ...
            start_date = (
                datetime.datetime.fromisoformat(start_ts_filter)
                .astimezone(datetime.timezone.utc)
                .date()
                if start_ts_filter
                else (current_utc_dt - datetime.timedelta(days=7)).date()
            )
            end_date = (
                datetime.datetime.fromisoformat(end_ts_filter)
                .astimezone(datetime.timezone.utc)
                .date()
                if end_ts_filter
                else current_utc_dt.date()
            )
        except ValueError:
            logger.warning(
                "Invalid timestamp format in filters. Defaulting partition check to last 7 days."
            )
        ...
        for i in range(delta.days + 1):
            ...
            partition_preds.append(
                f"(year = '{date.year}' AND month = '{date.month:02d}' AND day = '{date.day:02d}')"
            )
        ...
        try:
            ...
            if state != "SUCCEEDED":
                raise RuntimeError(
                    f"Athena query failed: {status['QueryExecution']['Status'].get('StateChangeReason', 'N/A')}"
                )
                    ...
                    parsed_results.append(
                        {
                            ...
                            "schema_version": (
                                int(row_dict.get("schema_version"))
                                if row_dict.get("schema_version")
                                and row_dict.get("schema_version").isdigit()
                                else None
                            ),
    ...
    async def _migrate_schema(self) -> None:
        ...
        if current_on_disk_version >= self.schema_version:
            logger.info(
                f"S3Backend schema is already at v{self.schema_version} or newer. No migration needed."
            )
            return

        logger.info(
            f"Migrating S3Backend from v{current_on_disk_version} to v{self.schema_version}"
        )
            ...
            for page in pages:
                for obj in page.get("Contents", []):
                    ...
                    try:
                        ...
                        compressed_encrypted_data_bytes = await asyncio.to_thread(
                            response["Body"].read
                        )
                        ...
                        ts_dt = datetime.datetime.fromisoformat(
                            original_audit_entry["timestamp"].replace("Z", "+00:00")
                        )
                        ...
                        await retry_operation(
                            ...
                            backend_name=self.__class__.__name__,
                            op_name=f"s3_put_object:{new_key}",
                        )
                        ...
                        logger.error(
                            f"Failed to migrate S3 object {old_key}: Decryption failed. Key issue.",
                            exc_info=True,
                        )
                        BACKEND_ERRORS.labels(
                            backend=self.__class__.__name__,
                            type="MigrationDecryptionFail",
                        ).inc()
                        failed_migrations.append(old_key)
                    except Exception as migrate_obj_e:
                        logger.error(
                            f"Failed to migrate S3 object {old_key}: {migrate_obj_e}",
                            exc_info=True,
                        )
                        BACKEND_ERRORS.labels(
                            backend=self.__class__.__name__, type="MigrationObjectError"
                        ).inc()
                        failed_migrations.append(old_key)
    ...
    async def _refresh_athena_partitions(self):
        ...
        logger.info(
            f"S3Backend: Running MSCK REPAIR TABLE for {self.athena_database}.{self.athena_table}"
        )
        try:
            response = await retry_operation(
                # --- FIX: Remove asyncio.to_thread wrapper ---
            ...
            if state != "SUCCEEDED":
                raise RuntimeError(
                    f"MSCK REPAIR TABLE failed with state: {state}. Reason: {status['QueryExecution']['Status'].get('StateChangeReason', 'N/A')}"
                )
            ...
            BACKEND_ERRORS.labels(
                backend=self.__class__.__name__, type="AthenaPartitionRepairError"
            ).inc()
            asyncio.create_task(
                send_alert(
                    f"Failed to refresh Athena partitions for S3Backend: {e}",
                    severity="medium",
                )
            )
    ...
    async def _get_current_schema_version(self) -> int:
        ...
        try:
            paginator = self.s3_client.get_paginator("list_objects_v2")
            pages_current = paginator.paginate(
                Bucket=self.bucket, Prefix=self.key_prefix, MaxKeys=1
            )
            for page in pages_current:
                if page.get("Contents"):
                    return self.schema_version

            pages_old = paginator.paginate(
                Bucket=self.bucket, Prefix=self.old_key_prefix, MaxKeys=1
            )
    ...
    async def _atomic_context(
        ...
        try:
            ...
            timestamp_for_key = datetime.datetime.fromisoformat(
                prepared_entries[0]["timestamp"].replace("Z", "+00:00")
            )
            ...
            compressed_batch_data = zlib.compress(
                batch_data_str.encode("utf-8"), level=COMPRESSION_LEVEL
            )
            ...
            await retry_operation(
                ...
                backend_name=self.__class__.__name__,
                op_name="s3_put_batch_object",
            )
            logger.debug(
                f"S3Backend: Atomically flushed {len(prepared_entries)} entries to {object_key}"
            )
...
class GCSBackend(LogBackend):
    ...
    async def _init_bigquery(self):
        ...
        try:
            await retry_operation(
                ...
                backend_name=self.__class__.__name__,
                op_name="bigquery_create_table",
            )
            logger.info(
                f"BigQuery table '{self.bigquery_dataset}.{self.bigquery_table}' initialized."
            )
        except Exception as e:
    ...
    async def _query_single(
        ...
        try:
            job = await retry_operation(
                ...
                backend_name=self.__class__.__name__,
                op_name="bigquery_start_query",
            )
            results = await retry_operation(
                # --- FIX: Remove asyncio.to_thread wrapper ---
                lambda: job.result(),
                backend_name=self.__class__.__name__,
                op_name="bigquery_get_results",
            )
    ...
    async def _migrate_schema(self) -> None:
        ...
        if current_on_disk_version >= self.schema_version:
            logger.info(
                f"GCSBackend schema is already at v{self.schema_version} or newer. No migration needed."
            )
            return

        logger.info(
            f"Migrating GCSBackend from v{current_on_disk_version} to v{self.schema_version}"
        )
        old_prefix = self.old_key_prefix
        new_prefix = self.key_prefix
            ...
            for blob in blobs:
                ...
                try:
                    ...
                    compressed_encrypted_data_bytes = await retry_operation(
                        ...
                        backend_name=self.__class__.__name__,
                        op_name=f"gcs_download_blob:{old_key}",
                    )
                    ...
                    if migrated_lines:
                        ...
                        await retry_operation(
                            # --- FIX: Remove asyncio.to_thread wrapper ---
                            lambda: new_blob.upload_from_string(
                                new_object_body, content_type="application/gzip"
                            ),
                            backend_name=self.__class__.__name__,
                            op_name=f"gcs_upload_blob:{new_key}",
                        )
                        migrated_count += len(migrated_lines)
                        logger.debug(f"Migrated {old_key} to {new_key}")
                    ...
                    logger.error(
                        f"Failed to migrate GCS object {old_key}: Decryption failed. Key issue.",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="MigrationDecryptionFail"
                    ).inc()
                    failed_migrations.append(old_key)
                except Exception as migrate_obj_e:
                    logger.error(
                        f"Failed to migrate GCS object {old_key}: {migrate_obj_e}",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="MigrationObjectError"
        ...
        try:
            ...
            blobs_current_iter = await retry_operation(
                ...
                lambda: self.client.list_blobs(
                    self.bucket_name, prefix=self.key_prefix, max_results=1
                ),
                backend_name=self.__class__.__name__,
                op_name="gcs_list_blobs_current_prefix",
            )
            ...
            blobs_old_iter = await retry_operation(
                # --- FIX: Remove asyncio.to_thread wrapper ---
                lambda: self.client.list_blobs(
                    self.bucket_name, prefix=self.old_key_prefix, max_results=1
                ),
                backend_name=self.__class__.__name__,
                op_name="gcs_list_blobs_old_prefix",
            )
        ...
        try:
            ...
            await retry_operation(
                ...
                lambda: blob.upload_from_string(
                    compressed_batch_data, content_type="application/gzip"
                ),
                backend_name=self.__class__.__name__,
                op_name="gcs_upload_blob",
            )
            logger.debug(
                f"GCSBackend: Atomically flushed {len(prepared_entries)} entries to {blob_name}"
            )
            ...
            await retry_operation(
                ...
                backend_name=self.__class__.__name__,
                op_name="bigquery_load_job_result",
            )
            ...
            for blob_name in blobs_to_delete:
                try:
                    ...
                    await retry_operation(
                        ...
                        lambda: blob.delete(),
                        backend_name=self.__class__.__name__,
                        op_name=f"gcs_delete_blob:{blob_name}",
                    )
                    deleted_count += 1
                except Exception as delete_e:
                    logger.warning(
                        f"Failed to delete GCS blob {blob_name} during cleanup: {delete_e}",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="GCSCleanupDeleteError"
                    ).inc()
...
class AzureBlobBackend(LogBackend):
    ...
    def _validate_params(self):
        ...
        if not self.blob_prefix.endswith("/"):
            self.blob_prefix += "/"
        self.old_blob_prefix = self.params.get(
            "old_blob_prefix", f"audit_logs_v{SCHEMA_VERSION - 1}/"
        )
        ...
        try:
            ...
            async for blob in blob_iterator:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to process Azure blob {blob.name} during query: {e}",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="AzureBlobQueryError"
                    ).inc()
                    asyncio.create_task(
                        send_alert(
                            f"Failed to query Azure Blob {blob.name}. Data might be corrupted.",
                            severity="medium",
                        )
                    )
                    continue
            ...
            BACKEND_ERRORS.labels(
                backend=self.__class__.__name__, type="AzureBlobQueryListError"
            ).inc()
            asyncio.create_task(
                send_alert(f"AzureBlobBackend query list failed: {e}", severity="high")
            )
            raise
        return entries
    ...
    async def _migrate_schema(self) -> None:
        ...
        if current_on_disk_version >= self.schema_version:
            logger.info(
                f"AzureBlobBackend schema is already at v{self.schema_version} or newer. No migration needed."
            )
            return

        logger.info(
            f"Migrating AzureBlobBackend from v{current_on_disk_version} to v{self.schema_version}"
        )
        ...
        try:
            ...
            for blob in blobs:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to migrate Azure Blob object {old_key}: Decryption failed. Key issue.",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="MigrationDecryptionFail"
                    ...
                    logger.error(
                        f"Failed to migrate Azure Blob object {old_key}: {migrate_obj_e}",
                        exc_info=True,
                    )
                    BACKEND_ERRORS.labels(
                        backend=self.__class__.__name__, type="MigrationObjectError"
    ...
    async def _get_current_schema_version(self) -> int:
        ...
        if self.container_client is None:
            ...
            try:
                ...
                logger.warning(
                    f"Could not establish temporary connection to Azure Blob for schema version check: {e}. Assuming v1."
                )
        ...
        try:
            # Check for objects in current prefix
            blobs_current_iter = target_container_client.list_blobs(
                name_starts_with=self.blob_prefix, results_per_page=1
            )
            try:
            ...
            # Check for objects in old prefix
            blobs_old_iter = target_container_client.list_blobs(
                name_starts_with=self.old_blob_prefix, results_per_page=1
            )
            try:
    ...
    async def _atomic_context(
        ...
        try:
            ...
            timestamp_for_blob_name = datetime.datetime.fromisoformat(
                prepared_entries[0]["timestamp"].replace("Z", "+00:00")
            )
            ...
            await self.container_client.upload_blob(
                ...
                overwrite=True,
                content_settings=ContentSettings(
                    content_type="application/jsonl", content_encoding="gzip"
                ),  # Correct usage
            )
            logger.debug(
                f"AzureBlobBackend: Atomically flushed {len(prepared_entries)} entries to {blob_name}"
            )
            yield
