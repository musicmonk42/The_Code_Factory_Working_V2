...
class SensitiveDataFilter(logging.Filter):
    ...
    SENSITIVE_PATTERNS = [
        ...
        re.compile(
            r"Authorization: Bearer [a-zA-Z0-9\-_.]+", re.IGNORECASE
        ),  # Matches Auth headers
    ...
    def filter(self, record):
        ...
        if hasattr(record, "args") and isinstance(record.args, dict):
            ...
            for k, v in record.args.items():
                if isinstance(v, str):
                    redacted_v = v
                    for pattern in self.SENSITIVE_PATTERNS:
                        redacted_v = pattern.sub(
                            self._get_redacted_replacement(pattern), redacted_v
                        )
        ...
        if (
            ...
            for pattern in self.SENSITIVE_PATTERNS:
                redacted_exc_value = pattern.sub(
                    self._get_redacted_replacement(pattern), redacted_exc_value
                )

            # The type of the exception value is preserved by creating a new instance of the original type
            record.exc_info = (
                record.exc_info[0],
                type(record.exc_info[1])(redacted_exc_value),
                record.exc_info[2],
            )
...
class SimpleCircuitBreaker:
    GAUGE_CIRCUIT_BREAKER_STATE = Gauge(
        "audit_backend_circuit_breaker_state",
        "Circuit breaker state (0=CLOSED, 1=OPEN, 2=HALF_OPEN)",
        ["backend"],
    )
    COUNTER_CIRCUIT_BREAKER_TRIPS = Counter(
        "audit_backend_circuit_breaker_trips_total",
    ...
    def __init__(
        ...
        self.GAUGE_CIRCUIT_BREAKER_STATE.labels(backend=self.backend_name).set(
            0 if self._state == "CLOSED" else (1 if self._state == "OPEN" else 2)
        )

        self.logger = logging.getLogger(f"{__name__}.{backend_name}.CircuitBreaker")
        self.logger.info(
            f"Circuit Breaker initialized for {backend_name} with threshold={failure_threshold}, recovery_timeout={recovery_timeout}s.",
    ...
    def _check_state(self):
        ...
        if self._state == "CLOSED":
            ...
            while self.current_failures and (
                now - self.current_failures[0] > self.recovery_timeout
            ):
        ...
        if self._state == "CLOSED":
            ...
            if time.time() - self.last_state_change_time < self.reset_timeout:
                self.last_state_change_time = (
                    time.time()
                )  # This effectively allows one request per reset_timeout window
                return True
            return False
    ...
    def record_failure(self, error: Exception):
        ...
        if self._state == "HALF_OPEN":
            ...
            self.logger.warning(
                f"Circuit Breaker for {self.backend_name} transitioned from HALF_OPEN to OPEN (trial request failed).",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "cb_half_open_fail",
                },
            )
    ...
    def record_success(self):
        ...
        if self._state == "HALF_OPEN":
            ...
            self.logger.info(
                f"Circuit Breaker for {self.backend_name} successfully reset to CLOSED.",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "cb_reset_success",
                },
            )
        elif self._state == "CLOSED":
            # Only remove stale failures if closed
            while self.current_failures and (
                time.time() - self.current_failures[0] > self.recovery_timeout
            ):
                self.current_failures.popleft()
...
class PersistentRetryQueue(_BaseRetryQueue):
    def __init__(
        ...
        **kwargs,  # Accept extra kwargs
        ...
        self.logger = logging.getLogger(
            f"{__name__}.{self.backend_name}.DLQProcessor"
        )  # Add logger
    ...
    async def reprocess(self, handler: Callable[[dict], Awaitable[bool]]):
        ...
        for item in drained:
            try:
                ...
                self.logger.error(
                    f"DLQ: Handler failed for item {item.get('dlq_item_id')}: {e}",
                    exc_info=True,
                )
                ok = False
            if ok:
    ...
    async def enqueue(self, item_data: Any, failure_reason: str = "unknown_failure"):
        ...
        try:
            ...
            self.logger.warning(
                f"DLQ: Enqueued failed item to retry queue for {self.backend_name}. Queue size: {self.current_size()}",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_enqueue",
                    "dlq_size": self.current_size(),
                    "item_id": dlq_item["dlq_item_id"],
                    "reason": failure_reason,
                },
            )
        except OverflowError:
            self.logger.critical(
                f"DLQ: Retry queue for {self.backend_name} is full ({self.max_queue_size} items). Item dropped. Data loss occurred!",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_drop_full",
                    "dlq_size": self.current_size(),
                    "item_id": dlq_item["dlq_item_id"],
                    "reason": "queue_full",
                },
            ...
            await send_alert(
                f"DLQ for {self.backend_name} is full. Data dropped!",
                severity="emergency",
            )

    async def start_processor(
        self, process_func: Callable[[Any], Awaitable[None]], interval: int = 30
    ):
        ...
        self.logger.info(
            f"DLQ: Started processor shim for {self.backend_name}. Reprocessing must be triggered externally via reprocess().",
            extra={
                "backend_type": self.backend_name,
                "operation": "dlq_processor_start",
            },
        )
    ...
    async def stop_processor(self):
        ...
        self._running = False
        self.logger.info(
            f"DLQ: Stopped processor for {self.backend_name}.",
            extra={
                "backend_type": self.backend_name,
                "operation": "dlq_processor_stop",
            },
        )
...
class FileBackedRetryQueue(PersistentRetryQueue):
    def __init__(
        ...
        **kwargs,
        ...
        self.logger = logging.getLogger(
            f"{__name__}.{backend_name}.FileBackedDLQ"
        )  # Specific logger for this implementation
    ...
    def _handle_reload_task_completion(self, task: asyncio.Task):
        ...
        try:
            if not task.cancelled():
                ...
                if exc:
                    self.logger.error(
                        f"DLQ reload task failed with exception: {exc}",
                        exc_info=exc,
                        extra={
                            "backend_type": self.backend_name,
                            "operation": "dlq_reload_error",
                        },
                    )
        except Exception as e:
            self.logger.error(
                f"Error handling reload task completion: {e}",
    ...
    async def _reload_from_persistence(self):
        ...
        self.logger.info(
            f"DLQ: Reloading items from persistence file '{self.persistence_file}' for {self.backend_name}.",
            extra={"backend_type": self.backend_name, "operation": "dlq_reload_start"},
        )
        if not os.path.exists(self.persistence_file):
            self.logger.info(
                f"DLQ: Persistence file '{self.persistence_file}' not found. Starting with empty DLQ.",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_reload_no_file",
                },
            )
            return
        ...
        try:
            async with aiofiles.open(self.persistence_file, "r", encoding="utf-8") as f:
                ...
                if not content:
                    self.logger.info(
                        f"DLQ: Persistence file '{self.persistence_file}' is empty.",
                        extra={
                            "backend_type": self.backend_name,
                            "operation": "dlq_reload_empty",
                        },
                    )
                    return
                ...
                for item in items:
                    if self.current_size() < self.max_queue_size:
                        ...
                        self.logger.warning(
                            f"DLQ: Queue full during reload for {self.backend_name}. Dropping reloaded item.",
                            extra={
                                "backend_type": self.backend_name,
                                "operation": "dlq_reload_queue_full",
                            },
                        )

            self.DLQ_COUNT_GAUGE.labels(self.backend_name).set(self.current_size())
            self.logger.info(
                f"DLQ: Reloaded {reloaded_count} items. Current size: {self.current_size()}",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_reload_end",
                },
            )

        except json.JSONDecodeError as jde:
            self.logger.error(
                f"DLQ: Malformed persistence file '{self.persistence_file}', cannot reload. Error: {jde}.",
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_reload_malformed",
                },
            )
        except Exception as e:
            self.logger.error(
                f"DLQ: Failed to reload from persistence: {e}",
                exc_info=True,
                extra={
                    "backend_type": self.backend_name,
                    "operation": "dlq_reload_fail",
                },
            )
