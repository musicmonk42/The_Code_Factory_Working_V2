...
def _lazy_log_action(*args, **kwargs):
    ...
    try:
        ...
        if log_action_func:
            ...
            logging.getLogger('audit_utils').warning("LOG_ACTION missing from audit_log module for secure_log.")
    except ImportError:
...
def register_hash_algo(name: str, func: Callable[[bytes, str], str]):
    ...
    with registry_lock:
        ...
        try:
            ...
            if "missing 1 required positional argument: 'algo_name'" in str(e) or "takes 1 positional argument but 2 were given" in str(e):
                # FIX: Corrected error message based on lambda contract change
                raise ValueError(f"Registered hash function '{name}' must accept two positional arguments (data: bytes, algo_name: str). Underlying error: {e}")
    ...
    if algo == 'default_internal' and 'default_internal' not in hash_registry:
         if not _registries_locked:
            register_hash_algo('default_internal', 
             ...
             raise RuntimeError("Cannot compute_hash: 'default_internal' is not registered and registries are locked.")
...
def secure_log(logger_obj: logging.Logger, message: str, level: int = logging.INFO, **kwargs: Any):
    ...
    logger_obj.log(level, redacted_message, extra=redacted_kwargs) # Also log locally, using redacted kwargs
        ...
        if rfc3161ng is None:
            ...
            try:
                ...
                if not TSA_CA_BUNDLE or not os.path.exists(TSA_CA_BUNDLE):
                    ...
                    if IS_PRODUCTION:
                        raise RuntimeError("TSA_CA_BUNDLE is required but missing/invalid in production.")
                    else:
                        logger.warning("Skipping TSA: TSA_CA_BUNDLE environment variable not set or file missing.")
                        # This should be handled by the main exception block below
                        raise Exception("TSA_CA_BUNDLE missing for non-production environment") # Trigger fallback status
