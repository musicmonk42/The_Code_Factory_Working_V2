...
import numpy as np
import requests
from prometheus_client import Counter, Gauge, Histogram, push_to_gateway, REGISTRY, CollectorRegistry, generate_latest
...
def get_metric_name(metric: Union[Counter, Gauge, Histogram]) -> str:
    ...
    try:
        ...
        if isinstance(metric, Counter) or isinstance(metric, Gauge) or isinstance(metric, Histogram):
...
class AuditMetrics:
    ...
    def define_custom_metric(self, name: str, type_: str = 'counter', labels: Optional[List[str]] = None, doc: str = '') -> Any:
        """
        ...
        if not isinstance(type_, str) or type_.lower() not in ['counter', 'gauge', 'histogram']:
        ...
        if full_metric_name in REGISTRY._names_to_collectors or name in custom_metrics:
             logger.warning(f"Metric {full_metric_name} already registered. Returning existing instance.")
    ...
    async def _export_to_cloudwatch(self, metrics_families: List[Any]):
        ...
        try:
            ...
            logger.info(f"Metrics sent to CloudWatch in {len(metric_data)} total points in {len(range(0, len(metric_data), 20))} batches.")
        except Exception as e:
    ...
    async def _send_alert(self, subject: str, message: str, channel: str = 'email'):
        ...
        while alert_queue:
            ...
            if attempt >= ALERT_RETRY_ATTEMPTS:
                logger.critical(f"Alert for '{subject}' failed after {attempt} attempts. Writing to dead-letter log.")
                # Write to a dead-letter log file
                with open("alert_dead_letter.log", "a") as f:
                    # FIX: Use UTC time for log
                    f.write(f"[{datetime.datetime.now(datetime.timezone.utc).isoformat()}] FAILED ALERT: {subject} - {message}\n")
                continue
            ...
            try:
                if channel in ('email', 'all'):
                    await loop.run_in_executor(None, functools.partial(self._send_email_alert_sync, subject, message))
                    success = True
    ...
    async def _send_slack_alert(self, subject: str, message: str):
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None, 
                functools.partial(requests.post, ALERT_SLACK_WEBHOOK, json=payload, timeout=EXPORTER_TIMEOUT)
            )
    ...
    async def _send_pagerduty_alert(self, subject: str, message: str):
        ...
        try:
            ...
            response = await loop.run_in_executor(
                None, 
                functools.partial(requests.post, "https://events.pagerduty.com/v2/enqueue", json=event, timeout=EXPORTER_TIMEOUT)
            )
            response.raise_for_status()
        except Exception as e:
    ...
    async def _self_test_periodically(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            if anomalies:
                alert_message = "Anomalies detected in audit metrics:\n" + "\n".join(anomalies)
                await self._send_alert(subject="Audit Metrics Anomaly Alert (Self-Test)", message=alert_message, channel='all')
            else:
    ...
    def _detect_anomalies(self) -> List[str]:
        ...
        for name, hist_deque in self.metric_history.items():
            ...
            if abs(z_score) > ANOMALY_Z_THRESHOLD:
                anomalies.append(f"Metric '{name}' current value {recent_value:.2f} (Z-score {z_score:.2f}) is anomalous. Mean: {mean:.2f}, Std Dev: {std:.2f}")
...
async def main():
    ...
    logger.info(f"Allowing background monitoring and self-test tasks to run for {SELF_TEST_INTERVAL * 1.5} seconds...")
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info("Program interrupted by user. Initiating shutdown (Note: shutdown is handled by asyncio.run).")
    except Exception as e:
