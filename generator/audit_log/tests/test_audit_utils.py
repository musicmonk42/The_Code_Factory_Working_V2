...
def test_compute_hash_pre_redaction(reset_registries):
    ...
    h_pre = compute_hash(
        data, redaction_mode=HashingModes(pre_redaction=True, post_redaction=False)
    )
...
def test_compute_hash_both_redaction_modes(reset_registries):
    ...
    h_both = compute_hash(
        data, redaction_mode=HashingModes(pre_redaction=True, post_redaction=True)
    )
...
def test_production_registry_overwrite_policy(reset_registries):
    ...
    with patch("generator.audit_log.audit_utils.IS_PRODUCTION", new=True):
        ...
        with pytest.raises(
            ValueError,
            match="Cannot overwrite built-in hash algorithm 'default_internal' in production mode",
        ):
        ...
        with pytest.raises(
            ValueError,
            match="Cannot overwrite built-in provenance logic 'default' in production mode",
        ):
