...
try:
    ...
    from generator.audit_log.audit_log import AuditLog, log_action, initialize_audit_log_instance
...
async def mock_presidio():
    ...
    with patch('generator.audit_log.audit_utils.AnalyzerEngine', create=True) as mock_analyzer_cls, \
         patch('generator.audit_log.audit_utils.AnonymizerEngine', create=True) as mock_anonymizer_cls:
...
async def mock_audit_log_backend():
    ...
    with patch('generator.audit_log.audit_log.get_backend') as mock_backend:
        ...
        mock_backend_inst.query = AsyncMock(return_value=[
...
async def mock_metrics():
    ...
    with patch('generator.audit_log.audit_log.Counter') as mock_counter, \
        ...
        mock_histogram.side_effect = lambda name, *args, **kwargs: mock_metrics.get(name, MagicMock())
        yield mock_metrics
...
class TestAuditLogProto:
    ...
    @pytest.mark.asyncio
    async def test_log_action_unary(self, grpc_channel, mock_presidio, mock_audit_log_backend, mock_metrics, grpc_server):
    ...
    @pytest.mark.asyncio
    async def test_log_stream(self, grpc_channel, mock_presidio, mock_audit_log_backend, mock_metrics, grpc_server):
    ...
    @pytest.mark.asyncio
    async def test_tamper_detection(self, grpc_channel, mock_presidio, mock_audit_log_backend, mock_crypto_provider_factory, mock_metrics, grpc_server, audit_log_instance):
    ...
    async def test_concurrent_log_actions(self, grpc_channel, mock_presidio, mock_audit_log_backend, mock_metrics, grpc_server):
        ...
        try:
            ...
            success_count = sum(1 for r in responses if not isinstance(r, Exception) and hasattr(r, 'status') and r.status == "success")
