...
class TestFileSystemKeyStorageBackend(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.skipif(
        sys.platform == "win32",
        reason="Windows does not support POSIX 0o600 file permissions",
    )
    ...
    async def test_07_delete_key_data(self):
        ...
        await self.backend.store_key_data(
            key_id,
            "payload",
            {"algo": "test", "creation_time": 123, "status": "active"},
        )
        self.assertTrue(os.path.exists(filepath))
    ...
    async def test_08_list_key_metadata(self):
        # Store 3 keys
        await self.backend.store_key_data(
            "key1", "p1", {"algo": "rsa", "creation_time": 1, "status": "active"}
        )
        await self.backend.store_key_data(
            "key2",
            "p2",
            {"algo": "ecdsa", "creation_time": 2, "status": "retired", "retired_at": 3},
        )
        await self.backend.store_key_data(
            "key3", "p3", {"algo": "hmac", "creation_time": 4, "status": "active"}
        )
    ...
    async def test_10_load_key_missing_metadata(self):
        ...
        # Missing 'algo'
        await self.backend.store_key_data(
            key_id, "payload", {"creation_time": 123, "status": "active"}
        )
    ...
    async def test_12_list_key_skips_corrupted(self):
        # Store 2 good keys
        await self.backend.store_key_data(
            "good1", "p1", {"algo": "rsa", "creation_time": 1, "status": "active"}
        )
        await self.backend.store_key_data(
            "good2", "p2", {"algo": "ecdsa", "creation_time": 2, "status": "active"}
        )
...
class TestKeyStore(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_02_store_and_load_key_success(self):
        ...
        self.mock_key_load_count.labels.assert_called_with(
            provider_type="software", status="success"
        )
    ...
    async def test_04_load_key_wrong_master_key(self):
        ...
        self.mock_crypto_errors.labels.assert_called_with(
