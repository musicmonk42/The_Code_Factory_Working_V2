...
from generator.audit_log.audit_backend import (
    ...
    LogBackend, SCHEMA_VERSION, TamperDetectionError, compute_hash,
    _STATUS_OK, _STATUS_ERROR, retry_operation,
    SimpleCircuitBreaker, FileBackedRetryQueue
)
...
def cleanup_test_environment(monkeypatch):
    ...
    mock_settings = {
        "ENCRYPTION_KEYS": [
            {"key_id": "mock_key_1", "key": MOCK_ENCRYPTION_KEY_B64}
        ],
    ...
    for module_to_patch in ["generator.audit_log.audit_backend.audit_backend_core", "generator.audit_log.audit_backend.audit_backend_streaming_backends"]:
        try:
            monkeypatch.setattr(f"{module_to_patch}.settings", MagicMock(
    ...
    monkeypatch.setattr("generator.audit_log.audit_backend.audit_backend_core._is_test_or_dev_mode", lambda: True)
...
async def mock_send_alert():
    ...
    with patch('generator.audit_log.audit_backend.audit_backend_streaming_backends.send_alert', new_callable=AsyncMock) as mock_streaming_alert, \
         patch('generator.audit_log.audit_backend.audit_backend_streaming_utils.send_alert', new_callable=AsyncMock) as mock_utils_alert, \
         patch('generator.audit_log.audit_backend.audit_backend_core.send_alert', new_callable=AsyncMock) as mock_core_alert:
...
async def mock_aiohttp():
    ...
    with patch('aiohttp.ClientSession', new_callable=MagicMock) as mock_session_cls:
        ...
        mock_session_inst.post.side_effect = lambda *args, **kwargs: create_async_context_manager(mock_response_post)
        mock_session_inst.get.side_effect = lambda *args, **kwargs: create_async_context_manager(mock_response_get)
...
async def mock_opentelemetry():
    ...
    with patch('generator.audit_log.audit_backend.audit_backend_core.tracer', new_callable=MagicMock) as mock_tracer:
        mock_span = MagicMock()
...
async def http_backend(mock_aiohttp, cleanup_test_environment):
    ...
    with patch.object(HTTPBackend, '_flush_batch_periodically', new_callable=AsyncMock), \
         patch.object(HTTPBackend, '_health_check_periodically', new_callable=AsyncMock), \
         patch.object(HTTPBackend, '_process_internal_retry_queue', new_callable=AsyncMock), \
...
async def kafka_backend(mock_aiokafka, cleanup_test_environment):
    ...
    with patch.object(KafkaBackend, '_flush_batch_periodically', new_callable=AsyncMock), \
         patch.object(KafkaBackend, '_health_check_periodically', new_callable=AsyncMock), \
         patch.object(KafkaBackend, '_process_internal_retry_queue', new_callable=AsyncMock), \
         patch.object(FileBackedRetryQueue, 'start_processor', new_callable=AsyncMock):
...
async def splunk_backend(mock_aiohttp, cleanup_test_environment):
    ...
    # --- FIX: Patch all background tasks ---
    with patch.object(SplunkBackend, '_flush_batch_periodically', new_callable=AsyncMock), \
         patch.object(SplunkBackend, '_health_check_periodically', new_callable=AsyncMock), \
         patch.object(FileBackedRetryQueue, 'start_processor', new_callable=AsyncMock):
...
async def inmemory_backend(cleanup_test_environment):
    ...
    # --- FIX: Patch all background tasks ---
    with patch.object(InMemoryBackend, '_flush_batch_periodically', new_callable=AsyncMock), \
...
class TestHTTPBackend:
    ...
    @pytest.mark.asyncio
    async def test_flush_batch_success(self, http_backend: HTTPBackend, mock_aiohttp, mock_opentelemetry):
    ...
    async def test_query_e2e_decryption(self, http_backend: HTTPBackend, mock_opentelemetry):
        ...
        with patch.object(http_backend, '_query_single', new_callable=AsyncMock) as mock_query_single:
    ...
    async def test_query_tamper_detection(self, http_backend: HTTPBackend, mock_send_alert):
        ...
        with patch.object(http_backend, '_query_single', new_callable=AsyncMock) as mock_query_single:
            ...
            assert not any(
                "Failed to process log entry" in call.args[0] for call in mock_send_alert.call_args_list
            )
            # --- END FIX ---
    ...
    async def test_flush_fail_internal_retry(self, http_backend: HTTPBackend, mock_aiohttp):
        ...
        with patch.object(http_backend, '_send_batch_chunks', new_callable=AsyncMock) as mock_send_chunks:
...
class TestKafkaBackend:
    ...
    @pytest.mark.asyncio
    async def test_flush_fail_dlq_and_abort(self, kafka_backend: KafkaBackend, mock_aiokafka, mock_send_alert):
...
class TestSplunkBackend:
    ...
    @pytest.mark.asyncio
    async def test_flush_fail_raises(self, splunk_backend: SplunkBackend, mock_aiohttp, mock_send_alert):
        """
        ...
        with patch('generator.audit_log.audit_backend.audit_backend_core.retry_operation', new_callable=AsyncMock) as mock_retry:
