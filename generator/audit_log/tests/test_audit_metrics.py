...
class TestAuditMetrics:
    ...
    async def test_metric_registration_and_emission(self, audit_metrics_instance):
        ...
        assert REGISTRY.get_sample_value('audit_error_types_total', {'type': 'network_issue'}) == 1.0
        assert REGISTRY.get_sample_value('audit_plugin_invocations_total', {'plugin': 'data_enrichment'}) == 1.0
    ...
    async def test_alerting_slack(self, audit_metrics_instance, mock_aiohttp):
        ...
        with patch('requests.post') as mock_post:
            ...
            try:
                ...
                await audit_metrics_instance._send_slack_alert("Test alert", "This is a critical test message")
    ...
    async def test_alerting_retry_failure(self, audit_metrics_instance, mock_aiohttp):
        ...
        mock_fail_response = MagicMock(status_code=500, raise_for_status=MagicMock(side_effect=requests.exceptions.HTTPError))
        
        with patch('requests.post', side_effect=[mock_fail_response, mock_fail_response, mock_fail_response, MagicMock(status_code=200, raise_for_status=MagicMock())]):
            try:
                # FIX 2: Removed unexpected 'severity' keyword argument
    ...
    async def test_concurrent_metric_updates(self, audit_metrics_instance):
        ...
        total_writes = 0
        for i in range(5):
            value = REGISTRY.get_sample_value('audit_log_writes_total', {'action': f'user_action_{i}'})
            assert value == 1.0
            total_writes += value
    ...
    async def test_update_vulnerability_count(self, audit_metrics_instance):
        ...
        value = REGISTRY.get_sample_value('audit_security_vulnerability_count', {'level': 'critical'})
        assert value == 5.0
