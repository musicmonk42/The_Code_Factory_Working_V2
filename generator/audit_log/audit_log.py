...
try:
    ...
    grpc_aio = None
    logging.getLogger(__name__).warning(
        "gRPC protobufs not found. Run 'python -m grpc_tools.protoc...' to generate them. Details: %s",
        e,
    )
...
def _get_encryption_key() -> bytes:
    ...
    if raw:
        ...
        if not _is_test_or_dev_mode():
            ...
            logger.critical(
                "AUDIT_LOG_ENCRYPTION_KEY environment variable not set! Failing fast. Required for production security."
            )
            raise ValueError(
...
def load_users_and_roles(config_path: Optional[str] = None):
    ...
    if not config_path:
        if DEV_MODE:
            logger.warning(
                "AUDIT_LOG_USERS_CONFIG not set. Using hardcoded dummy users in DEV_MODE."
            )
            USERS.update(
                {
                    "user1": {
            ...
            logger.critical(
                "AUDIT_LOG_USERS_CONFIG not set! User configuration is required in non-DEV_MODE."
            )
...
def _safe_create_metric(metric_cls, name, description, labelnames=None):
    """Create a Prometheus metric, handling duplicate registration gracefully.

    If a metric with the same name already exists but with different labels,
    unregister the old one and create a new one with the correct labels.
    """
    kwargs = {}
    if labelnames:
    ...
    try:
        ...
        if existing is not None:
            existing_labels = getattr(existing, "_labelnames", None)
            expected_labels = tuple(labelnames) if labelnames else ()
            if existing_labels != expected_labels:
                # Conflicting metric with different labels; unregister and recreate
                try:
...
LOG_QUERIES = _safe_create_metric(Counter, "audit_log_queries_total", "Total queries performed on the audit log")
# The original LOG_ERRORS labels were ['type', 'user', 'action'].
LOG_ERRORS = _safe_create_metric(
    Counter,
    "audit_log_errors_total",
    "Total errors in audit log operations",
    ["type", "user", "action"],
)
LOG_LATENCY = _safe_create_metric(Histogram, "audit_log_latency_seconds", "Latency of audit log operations", ["op"])
TAMPER_ALERTS = _safe_create_metric(Counter, "audit_tamper_alerts_total", "Total tamper detections")
SELF_HEAL_EVENTS = _safe_create_metric(
    Counter, "audit_self_heal_events_total", "Total self-healing events", ["type"]
)
DOC_GEN_ACTIONS = _safe_create_metric(
    Counter,
    "audit_doc_gen_actions_total",
    "Total documentation generation actions",
    ["generator"],
)
RBAC_DENIALS = _safe_create_metric(
    Counter,
...
class AuditLog:
    ...
    async def shutdown(self):
        ...
        try:
            ...
            if valid_tasks:
                    ...
                    valid_tasks, timeout=5, return_when=asyncio.ALL_COMPLETED
                )
    ...
    async def self_heal(self) -> None:
        ...
        try:
            ...
            entries = await self.backend.read_last_n(
                limit=10000
            )  # Assuming read_last_n returns raw encrypted strings
            ...
            for i, encrypted_entry_b64 in enumerate(entries):
                ...
                try:
                    ...
                    if not entry.get("signature") or not entry.get("signing_key_id"):
                        logger.warning(
                            f"Entry {i} is missing a signature or key ID. Flagging as potential tamper."
                        )
                        TAMPER_ALERTS.inc()
                        SELF_HEAL_EVENTS.labels(type="no_signature").inc()
                        issues_detected += 1
                        await self._execute_hooks(
                            "tamper_detected", entry=entry, issue="no_signature"
                        )
                        continue
                    ...
                    signed_data_bytes = (
                        base64.b64decode(entry["signed_data"])
                        if isinstance(entry["signed_data"], str)
                    ...
                    if verifier is None:
                        verifier = getattr(
                            self.crypto_provider, "verify_signature", None
                        )  # Mock convention

                    if verifier is None:
                        logger.critical(
                            "Crypto provider does not implement verify/verify_signature"
                        )
                        raise RuntimeError(
                            "Crypto provider does not implement verify/verify_signature"
                        )
                    ...
                    if not is_valid:
                        logger.warning(
                            f"Signature mismatch detected for entry {i} (ID: {entry.get('uuid')}). Possible tampering."
                        )
                        TAMPER_ALERTS.inc()
                        SELF_HEAL_EVENTS.labels(type="invalid_signature").inc()
                        issues_detected += 1
                        await self._execute_hooks(
                            "tamper_detected", entry=entry, issue="invalid_signature"
                        )

                except ValueError as e:
                    logger.error(
                        f"Failed to decrypt or parse entry {i}. This could indicate tampering or corruption. Error: {e}"
                    )
                    TAMPER_ALERTS.inc()
                    ...
                    logger.error(
                        f"Unexpected error during integrity check for entry {i}: {e}",
                        exc_info=True,
                    )
                    TAMPER_ALERTS.inc()
                    ...
                    await self._execute_hooks(
                        "tamper_detected",
                        entry=entry or encrypted_entry_b64,
                        issue="check_failed",
                    )
    ...
    def _authorize(
        self, op: str, credentials: Optional[HTTPAuthorizationCredentials] = None
        ...
        if ROLES[role].get(op):
            logger.info(
                f"Authorization successful for user '{username}' with role '{role}' for operation '{op}'."
            )
            return user
        else:
            logger.warning(
                f"Authorization denied for user '{username}' with role '{role}' for operation '{op}'."
            )
            RBAC_DENIALS.labels(user=username, role=role, operation=op).inc()
            return None
    ...
    async def _execute_hooks(self, event_type: str, *args, **kwargs):
        ...
        for hook in hooks.get(event_type, []):
            try:
                ...
                logger.error(
                    f"Failed to execute hook '{hook.__name__}' for event '{event_type}': {e}",
                    exc_info=True,
                )
    ...
    async def log_action(
        ...
        with LOG_LATENCY.labels(op="log_action").time():
            ...
            try:
                ...
                if not self.current_signing_key_id:
                    raise RuntimeError(
                        "No active signing key is available. Key initialization failed."
                    )
                ...
                entry["signed_data"] = base64.b64encode(payload).decode(
                    "utf-8"
                ...
                await self._execute_hooks(
                    "log_error",
                    error=e,
                    entry=locals().get("entry", {"action": action}),
                )
                raise HTTPException(
                    status_code=500,
                    detail=f"Internal server error: Failed to log action. {e}",
                )
            ...
            try:
                ...
                for entry_data in raw_entries:
                    try:
                        ...
                        if isinstance(entry_data, str) and entry_data.startswith(
                            ("gAAAA", "AQIC")
                        ):  # Heuristic for encrypted/encoded data
                ...
                logger.info(
                    f"Successfully queried recent history (limit={limit}, action={action})."
                )
    ...
    async def detect_oscillation(
        ...
        try:
            ...
            dummy_creds = HTTPAuthorizationCredentials(
                scheme="Bearer", credentials="admin_token" if USERS else "dummy"
            )
            ...
            if len(unique_hashes) == 1 and None not in unique_hashes:
                logger.warning(
                    "Oscillation detected: The same hash was repeated within the window."
                )
                return True
            ...
            if len(unique_hashes) == 2 and len(recent_hashes) == window:
                alternating = all(
                    recent_hashes[i] != recent_hashes[i + 1] for i in range(window - 1)
                )
                if alternating:
                    logger.warning(
                        "Oscillation detected: Alternating hashes found within the window."
                    )
                    return True
        except HTTPException:
            ...
            LOG_ERRORS.labels(
                type="oscillation_fail", user="system", action="detect_oscillation"
            ).inc()
    ...
    async def rotate_signing_key(
        ...
        try:
            ...
            await self._execute_hooks(
                "key_rotated", new_key_id=new_key_id, old_key_id=previous_key_id
            )
...
async def log_action(
    ...
    **kwargs,
    ...
    if (
        ...
        or not AUDIT_LOG._self_heal_task
        or AUDIT_LOG._self_heal_task.done()
    ):
...
def log(
    ...
    try:
        ...
        async def run_log():
            ...
            try:
                await cli_audit_log.log_action(
                    action=action,
...
def query(
    ...
    try:
        ...
        async def run_query():
            await cli_audit_log.start()
            try:
                return await cli_audit_log.get_recent_history(
                    action=action, limit=limit, credentials=creds
                )
            finally:
                await cli_audit_log.shutdown()
...
async def query_log_api(
    filters: QueryFilter, credentials: HTTPAuthorizationCredentials = Security(security)
):
    """Queries recent log entries based on filters."""
    try:
        entries = await AUDIT_LOG.get_recent_history(
            action=filters.action, limit=filters.limit, credentials=credentials
        )
        ...
        async def LogAction(self, request, context):
            ...
            if not self.audit_log._authorize("write", creds):
                ...
                return audit_log_pb2.LogActionResponse(
                    status="failed", error_message="Not authorized"
                )
            ...
            try:
                ...
                return audit_log_pb2.LogActionResponse(
                    status="failed", error_message=f"Invalid details_json: {e}"
                )
