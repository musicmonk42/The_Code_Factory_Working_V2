...
class SensitiveDataFilter(logging.Filter):
    ...
    SENSITIVE_FIELD_KEYWORDS: FrozenSet[str] = frozenset([
        "pin", "secret", "password", "token", "key", "credential",
        "auth", "apikey", "api_key", "private", "cipher", "encrypt"
    ])
...
def detect_runtime_environment() -> RuntimeEnvironment:
    ...
    # Staging indicators
    if any(env in ["staging", "stage", "pre-prod", "preprod"] 
    ...
    if any(env in ["development", "dev", "local"] 
           for env in [python_env, app_env, node_env, environment]):
