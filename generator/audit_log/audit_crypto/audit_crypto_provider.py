...
try:
    import pkcs11
    from pkcs11.constants import CKM_EDDSA, CKM_RSA_PKCS_PSS, CKM_ECDSA, CKM_SHA256, CKG_MGF1_SHA256, CKS_RW_USER_FUNCTIONS, CKM_EC_EDWARDS_KEY_PAIR_GEN, CKM_RSA_PKCS_KEY_PAIR_GEN, CKM_EC_KEY_PAIR_GEN
...
class SoftwareCryptoProvider(CryptoProvider):
            ...
            for metadata in key_metadata_list:
                ...
                try:
                    ...
                    if key_info:
                        ...
                        self.logger.warning(f"SoftwareCryptoProvider: KeyStore could not load key '{key_id}'. File might be corrupt or missing.",
                                            extra={"operation": "load_keys_keystore_load_fail", "key_id": key_id})
                        KEY_LOAD_COUNT.labels(provider_type="software", status="keystore_load_fail").inc()
                        # log_action is already called in KeyStore.load_key
                except Exception as e:
                    self.logger.error(f"SoftwareCryptoProvider: Failed to load or deserialize key '{key_id}': {e}. Skipping.", exc_info=True,
                                      extra={"operation": "load_keys_deserialize_fail", "key_id": key_id})
                    CRYPTO_ERRORS.labels(type="KeyLoadError", provider_type="software", operation="load_key_individual").inc()
                    KEY_LOAD_COUNT.labels(provider_type="software", status="deserialize_fail").inc()
                    asyncio.create_task(log_action("key_load", key_id=key_id, algo=metadata.get("algo", "unknown"), provider="software", status="deserialize_fail", error=str(e)))

            self.logger.info(f"SoftwareCryptoProvider: Loaded {loaded_count} keys into memory cache.",
                             extra={"operation": "load_keys_end", "loaded_count": loaded_count})
            asyncio.create_task(log_action("load_existing_keys_summary", provider="software", loaded_count=loaded_count, status="success"))
        except Exception as e:
            self.logger.error(f"SoftwareCryptoProvider: Unexpected error during key loading: {e}", exc_info=True,
                              extra={"operation": "load_keys_unexpected_fail"})
            CRYPTO_ERRORS.labels(type="KeyLoadError", provider_type="software", operation="load_all_keys").inc()
            asyncio.create_task(log_action("load_existing_keys_summary", provider="software", status="fail", error=str(e)))
        ...
        try:
            if HAS_OPENTELEMETRY and tracer:
                ...
                with tracer.start_as_current_span("software_sign") as span:
                    ...
                    signature = await asyncio.to_thread(self._sign_with_key_internal, data, key_obj, algo)
                    SIGN_OPERATIONS.labels(algo=algo, provider_type="software").inc()
                    span.set_status(trace.StatusCode.OK)
                    status_label = "success"
                    await log_action("sign", key_id=key_id, algo=algo, provider="software", success=True)
                    return signature
            else:
                signature = await asyncio.to_thread(self._sign_with_key_internal, data, key_obj, algo)
                SIGN_OPERATIONS.labels(algo=algo, provider_type="software").inc()
            ...
            self.logger.error(f"Error during software sign for key '{key_id}' ({algo}): {e}", exc_info=True,
                              extra={"operation": "sign_fail", "key_id": key_id, "algo": algo})
            CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="sign").inc()
    ...
    async def verify(self, signature: bytes, data: bytes, key_id: str) -> bool:
        ...
        if not key_info:
            ...
            VERIFY_OPERATIONS.labels(algo="unknown", provider_type="software", status="key_not_found").inc()
            await log_action("verify", key_id=key_id, provider="software", status="key_not_found", success=False)
            return False
        ...
        try:
            if HAS_OPENTELEMETRY and tracer:
                ...
                with tracer.start_as_current_span("software_verify") as span:
                    span.set_attribute("algo", algo)
                    span.set_attribute("key_id", key_id)
                    result = await asyncio.to_thread(self._verify_with_key_internal, signature, data, key_obj, algo)
                    if result:
                        status_label = "success"
                        span.set_status(trace.StatusCode.OK)
                    else:
                    ...
                    await log_action("verify", key_id=key_id, algo=algo, provider="software", success=result)
                    return result
            else:
                result = await asyncio.to_thread(self._verify_with_key_internal, signature, data, key_obj, algo)
                if result: status_label = "success"
                await log_action("verify", key_id=key_id, algo=algo, provider="software", success=result)
                return result
        except InvalidSignature:
            status_label = "fail"
            self.logger.info(f"Invalid signature detected for key '{key_id}' ({algo}).",
                             extra={"operation": "verify_invalid_sig", "key_id": key_id, "algo": algo})
            await log_action("verify", key_id=key_id, algo=algo, provider="software", success=False, error="InvalidSignature")
            return False
        except Exception as e:
            status_label = "error"
            self.logger.error(f"Error during software verify for key '{key_id}' ({algo}): {e}", exc_info=True,
                              extra={"operation": "verify_error", "key_id": key_id, "algo": algo})
            CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="verify").inc()
    ...
    def _verify_with_key_internal(self, signature: bytes, data: bytes, key_obj: Any, algo: str) -> bool:
        """
        Synchronous internal verification logic. Runs in a separate thread.
        Uses constant-time comparison for HMAC.
        """
        if algo == "rsa":
            key_obj.public_key().verify(signature, data, padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=padding.PSS.MAX_LENGTH), hashes.SHA256())
        elif algo == "ecdsa":
        ...
        try:
            if HAS_OPENTELEMETRY and tracer:
                ...
                with tracer.start_as_current_span("software_generate_key") as span:
                    ...
                    await self.key_store.store_key(key_id, key_data_bytes, algo, creation_time, status="active")
                    self.keys[key_id] = {"key_obj": key_obj, "algo": algo, "creation_time": creation_time, "status": "active"}

                    await log_action("generate_key", key_id=key_id, algo=algo, provider="software", success=True)
                    span.set_status(trace.StatusCode.OK)
                    return key_id
            else:
                key_obj = await asyncio.to_thread(self._generate_key_internal, algo)
            ...
            self.logger.error(f"Error generating new {algo} software key: {e}", exc_info=True,
                              extra={"operation": "generate_key_fail", "algo": algo})
            CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="generate_key").inc()
            await log_action("generate_key", key_id=key_id, algo=algo, provider="software", success=False, error=str(e))
    ...
    async def rotate_key(self, old_key_id: Optional[str], algo: str) -> str:
        ...
        if old_key_id and old_key_id in self.keys:
            ...
            try:
                ...
                self.logger.info(f"Software key rotation: Key '{old_key_id}' retired, new key '{new_key_id}' generated for algo '{algo}'. Old key remains for verification.",
                                 extra={"operation": "rotate_key_success", "old_key_id": old_key_id, "new_key_id": new_key_id, "algo": algo})
                await log_action('rotate_key', old_key_id=old_key_id, new_key_id=new_key_id, algo=algo, provider='software', success=True, action="retire_old_key")
            except Exception as e:
                self.logger.error(f"Failed to update status of old key '{old_key_id}' to retired: {e}", exc_info=True,
                                  extra={"operation": "rotate_key_retire_fail", "old_key_id": old_key_id})
                CRYPTO_ERRORS.labels(type="KeyRetireError", provider_type="software", operation="rotate_key").inc()
                await log_action('rotate_key', old_key_id=old_key_id, new_key_id=new_key_id, algo=algo, provider='software', success=False, error=str(e), action="retire_old_key")
                # Do not re-raise, as the new key was successfully generated. Log and continue.
        else:
            self.logger.info(f"Software key rotation: New key '{new_key_id}' generated (no old key specified or found to retire).",
                             extra={"operation": "rotate_key_success", "new_key_id": new_key_id, "algo": algo})
            await log_action('rotate_key', new_key_id=new_key_id, algo=algo, provider='software', success=True, message="No old key to retire.")
            ...
            try:
                ...
                for old_key_id, algo in keys_to_rotate:
                    try:
                        ...
                        CRYPTO_ERRORS.labels(type="AutoRotateError", provider_type="software", operation="rotate_keys_periodic").inc()
                        asyncio.create_task(send_alert(f"SoftwareCryptoProvider: Auto-rotation failed for key '{old_key_id}': {e}", severity="high"))
                        asyncio.create_task(log_action("auto_rotate", key_id=old_key_id, success=False, error=str(e)))
                    ...
                    try:
                        if key_id in self.keys:
                            ...
                            if isinstance(key_obj_to_zeroize, (bytes, bytearray)):
                                ...
                                self.logger.debug(f"SoftwareCryptoProvider: Zeroized key material for '{key_id}' in memory.",
                                                  extra={"operation": "key_zeroized_memory", "key_id": key_id})
                            ...
                            self.logger.info(f"SoftwareCryptoProvider: Permanently deleted retired key '{key_id}'.",
                                             extra={"operation": "auto_delete_key_success", "key_id": key_id})
                            KEY_CLEANUP_COUNT.labels(provider_type="software", status="success").inc()
                            await log_action("key_delete", key_id=key_id, provider="software", status="success")
                    except Exception as e:
                        self.logger.error(f"SoftwareCryptoProvider: Failed to delete retired key '{key_id}': {e}", exc_info=True,
                                          extra={"operation": "auto_delete_key_fail", "key_id": key_id})
                        CRYPTO_ERRORS.labels(type="AutoDeleteError", provider_type="software", operation="cleanup_keys_periodic").inc()
                        asyncio.create_task(send_alert(f"SoftwareCryptoProvider: Failed to delete retired key '{key_id}': {e}", severity="critical"))
...
class HSMCryptoProvider(CryptoProvider):
    ...
    def __init__(self,
        ...
        try:
            ...
            self.logger.info("HSM PIN fetched successfully.", extra={"operation": "hsm_pin_fetch_success"})
        except Exception as e:
    ...
    async def _initialize_hsm_session(self):
        ...
        async with self._init_lock:
            ...
            try:
                ...
                self.session = await asyncio.to_thread(token.open, rw=True, user_pin=self.hsm_user_pin)
                HSM_SESSION_HEALTH.labels(provider_type='hsm').set(1)
                self.logger.info(f"HSM session initialized successfully on token '{token.label}' in slot {self.hsm_slot_id}.",
                                 extra={"operation": "hsm_session_init_success", "token_label": token.label, "slot_id": self.hsm_slot_id})
                ...
                self.logger.critical(f"Failed to initialize HSM session: {e}. Check HSM device, PKCS#11 library, PIN, and slot ID.", exc_info=True,
                                     extra={"operation": "hsm_session_init_fail_pkcs11"})
                CRYPTO_ERRORS.labels(type="HSMInitError", provider_type="hsm", operation="init_session").inc()
                ...
                self.session = None
                HSM_SESSION_HEALTH.labels(provider_type='hsm').set(0)
                self.logger.critical(f"Unexpected error initializing HSM session: {e}.", exc_info=True,
    ...
    def _is_session_valid(self) -> bool:
        ...
        try:
            ...
            if not is_valid:
                self.logger.warning(f"HSM session state is {session_info.state}, expected {CKS_RW_USER_FUNCTIONS}. Session considered invalid.",
                                    extra={"operation": "hsm_session_state_mismatch", "current_state": session_info.state})
            return is_valid
        except pkcs11.exceptions.PKCS11Error as e:
            self.logger.warning(f"HSM session validation failed with PKCS#11 error: {e}. Session might be invalid or disconnected.",
                                extra={"operation": "hsm_session_validate_fail"})
            return False
        except Exception as e:
            self.logger.error(f"Unexpected error during HSM session validation: {e}", exc_info=True,
        ...
        while True:
            try:
                ...
                if not is_session_valid:
                    self.logger.warning("HSM session is not valid. Attempting to re-initialize.",
                    ...
                    try:
                        await retry_operation(
                            ...
                            backend_name=self.__class__.__name__, op_name="hsm_reinit_session"
                        )
                        await log_action("hsm_health_monitor", status="reinit_success")
                    except Exception as e:
                        self.logger.error(f"Failed to re-initialize HSM session after retries: {e}. HSM features remain unavailable.", exc_info=True,
                                          extra={"operation": "hsm_reinit_fail_final"})
                        asyncio.create_task(send_alert(f"HSM session for {self.__class__.__name__} failed to re-initialize. Crypto operations may be impacted.", severity="critical"))
                        await log_action("hsm_health_monitor", status="reinit_fail", error=str(e))
                else:
                    HSM_SESSION_HEALTH.labels(provider_type='hsm').set(1)
                    self.logger.debug("HSM session is healthy.", extra={"operation": "hsm_session_healthy"})
                    await log_action("hsm_health_monitor", status="healthy")
            except asyncio.CancelledError:
                self.logger.info("HSMCryptoProvider: Health monitor task cancelled.", extra={"operation": "hsm_monitor_cancelled"})
                await log_action("hsm_health_monitor", status="cancelled")
                break
            except Exception as e:
                self.logger.error(f"HSMCryptoProvider: Unexpected error in health monitor task: {e}", exc_info=True,
                                  extra={"operation": "hsm_monitor_unexpected_error"})
                CRYPTO_ERRORS.labels(type="HSMMonitorError", provider_type="hsm", operation="health_check").inc()
                asyncio.create_task(send_alert(f"HSM health monitor encountered an error: {e}", severity="high"))
    ...
    async def generate_key(self, algo: str) -> str:
        ...
        if algo not in self.settings.SUPPORTED_ALGOS or algo == "hmac":
            raise UnsupportedAlgorithmError(f"Unsupported or invalid algorithm for HSM key generation: {algo}")

        await self._initialize_hsm_session()
        if not self.session:
            CRYPTO_ERRORS.labels(type="HSMNotAvailable", provider_type="hsm", operation="generate_key").inc()
            await log_action("generate_key", key_id="N/A", algo=algo, provider="hsm", status="no_session", success=False)
        ...
        if HAS_OPENTELEMETRY and tracer:
            ...
            with tracer.start_as_current_span("hsm_generate_key") as span:
                ...
                try:
                    ...
                    await log_action('generate_key', key_id=key_id_hex, algo=algo, provider='hsm', success=True)
                    span.set_status(trace.StatusCode.OK)
                    return key_id_hex
                except Exception as e:
                    CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="hsm", operation="generate_key").inc()
                    span.set_status(trace.StatusCode.ERROR, description=str(e))
                    await log_action('generate_key', key_id=key_id_hex, algo=algo, provider='hsm', success=False, error=str(e))
    ...
    def _generate_key_internal_hsm(self, algo: str, key_id_label: str):
        ...
        if algo == 'ed25519':
            ...
            self.logger.info(f"Generated Ed25519 key on HSM with ID/Label: '{key_id_label}'", extra={"key_id": key_id_label, "algo": algo})

        elif algo == 'rsa':
            self.session.generate_key_pair(
                pkcs11.constants.CKM_RSA_PKCS_KEY_PAIR_GEN,
                private_template={**common_private_template, pkcs11.Attribute.MODULUS_BITS: 2048, pkcs11.Attribute.PUBLIC_EXPONENT: (0x01, 0x00, 0x01)},
                public_template={**common_public_template, pkcs11.Attribute.MODULUS_BITS: 2048, pkcs11.Attribute.PUBLIC_EXPONENT: (0x01, 0x00, 0x01)}
            )
            self.logger.info(f"Generated RSA key on HSM with ID/Label: '{key_id_label}'", extra={"key_id": key_id_label, "algo": algo})
            ...
            self.session.generate_key_pair(
                ...
                private_template=common_private_template,
                public_template={**common_public_template, pkcs11.Attribute.EC_PARAMS: P256_OID}
            )
            self.logger.info(f"Generated ECDSA key on HSM with ID/Label: '{key_id_label}'", extra={"key_id": key_id_label, "algo": algo})
    ...
    async def verify(self, signature: bytes, data: bytes, key_id: str) -> bool:
        ...
        if not self.session:
            ...
            CRYPTO_ERRORS.labels(type="HSMNotAvailable", provider_type="hsm", operation="verify").inc()
        ...
        try:
            if HAS_OPENTELEMETRY and tracer:
                ...
                with tracer.start_as_current_span("hsm_verify") as span:
                    ...
                    if result:
                        ...
                        status_label = "fail"
                        span.set_status(trace.StatusCode.ERROR, description="Invalid signature")
                    await log_action("verify", key_id=key_id, algo="hsm", provider="hsm", success=result)
                    return result
            else:
                result = await asyncio.to_thread(self._verify_internal_hsm, signature, data, key_id)
                if result: status_label = "success"
            ...
            status_label = "fail"
            self.logger.info(f"Invalid signature detected by HSM for key '{key_id}'.",
                             extra={"operation": "hsm_verify_invalid_sig", "key_id": key_id})
            await log_action("verify", key_id=key_id, algo="hsm", provider="hsm", success=False, error="InvalidSignature")
            return False
        except pkcs11.exceptions.PKCS11Error as e:
            status_label = "error"
            self.logger.error(f"PKCS#11 error during HSM verification for key '{key_id}': {e}", exc_info=True,
                              extra={"operation": "hsm_verify_error_pkcs11", "key_id": key_id})
            CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="hsm", operation="verify").inc()
            ...
            status_label = "error"
            self.logger.error(f"Unexpected error during HSM verification for key '{key_id}': {e}", exc_info=True,
    ...
    def _verify_internal_hsm(self, signature: bytes, data: bytes, key_id: str) -> bool:
        ...
        if not public_key_obj:
            raise HSMKeyError(f"Public key with label '{key_id}' not found on HSM for verification.")
        ...
        if key_algo_type == pkcs11.KeyType.EC_EDWARDS:
            ...
            raise UnsupportedAlgorithmError(f"Unsupported key type {key_algo_type} for HSM verification.")
    ...
    async def rotate_key(self, old_key_id: Optional[str], algo: str) -> str:
        ...
        if algo not in self.settings.SUPPORTED_ALGOS or algo == "hmac": # Enforce policy
            raise UnsupportedAlgorithmError(f"Unsupported or invalid algorithm for HSM key rotation: {algo}")
        ...
        if old_key_id:
            ...
            if self.session:
                try:
                    ...
                    self.logger.info(f"HSM key rotation: Successfully destroyed old key '{old_key_id}'.",
                                     extra={"operation": "rotate_key_old_key_destroyed", "old_key_id": old_key_id})
                    await log_action('rotate_key', old_key_id=old_key_id, new_key_id=new_key_id, algo=algo, provider='hsm', success=True, action="destroy_old_key")
                except Exception as e:
                    self.logger.error(f"HSM key rotation: Failed to destroy old key '{old_key_id}': {e}. Manual cleanup may be required.", exc_info=True,
                                      extra={"operation": "rotate_key_destroy_fail", "old_key_id": old_key_id})
                    CRYPTO_ERRORS.labels(type="KeyDestroyError", provider_type="hsm", operation="rotate_key").inc()
                    asyncio.create_task(send_alert(f"HSM key destruction failed for {old_key_id}. Manual intervention required.", severity="critical"))
                    await log_action('rotate_key', old_key_id=old_key_id, new_key_id=new_key_id, algo=algo, provider='hsm', success=False, error=str(e), action="destroy_old_key")
                    raise CryptoOperationError(f"HSM key rotation failed to destroy old key: {e}") from e
            else:
                self.logger.warning(f"HSM session not available, could not destroy old key '{old_key_id}' during rotation. Manual cleanup required.",
                                    extra={"operation": "rotate_key_no_session_destroy", "old_key_id": old_key_id})
                await log_action('rotate_key', old_key_id=old_key_id, new_key_id=new_key_id, algo=algo, provider='hsm', success=False, error="HSM session not available", action="destroy_old_key")
    ...
    def _destroy_hsm_key_internal(self, key_id: str):
        ...
        if private_key_obj:
            try:
                ...
                self.logger.info(f"Destroyed HSM private key with label: '{key_id}'",
                                 extra={"operation": "hsm_destroy_priv_key", "key_id": key_id})
            except pkcs11.exceptions.PKCS11Error as e:
                self.logger.error(f"PKCS#11 error destroying HSM private key '{key_id}': {e}", exc_info=True,
                                  extra={"operation": "hsm_destroy_priv_key_fail_pkcs11", "key_id": key_id})
        ...
        if public_key_obj:
            try:
                ...
                self.logger.info(f"Destroyed HSM public key with label: '{key_id}'",
                                 extra={"operation": "hsm_destroy_pub_key", "key_id": key_id})
            except pkcs11.exceptions.PKCS11Error as e:
                self.logger.error(f"PKCS#11 error destroying HSM public key '{key_id}': {e}", exc_info=True,
                                  extra={"operation": "hsm_destroy_pub_key_fail_pkcs11", "key_id": key_id})
    ...
    async def close(self):
        ...
        if self.session:
            try:
                self.session.logout()
                self.logger.info("HSM session logged out.", extra={"operation": "hsm_session_logout"})
                await asyncio.to_thread(self.session.close)
                self.logger.info("HSM session closed successfully.", extra={"operation": "hsm_session_close_success"})
                HSM_SESSION_HEALTH.labels(provider_type="hsm").set(0)
                await log_action("hsm_session_close", status="success")
            except pkcs11.exceptions.PKCS11Error as e:
                self.logger.error(f"Failed to close HSM session due to PKCS11 error: {e}", exc_info=True,
                                  extra={"operation": "hsm_session_close_fail_pkcs11"})
                CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="hsm", operation="close_session").inc()
                await log_action("hsm_session_close", status="fail", error=str(e))
            except Exception as e:
                self.logger.error(f"Unexpected error closing HSM session: {e}", exc_info=True,
                                  extra={"operation": "hsm_session_close_unexpected"})
                CRYPTO_ERRORS.labels(type="SessionCloseErrorOther", provider_type="hsm", operation="close_session").inc()
                await log_action("hsm_session_close", status="fail", error=str(e))
