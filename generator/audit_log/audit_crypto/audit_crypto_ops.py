...
from typing import Any, Callable, Dict, List, Optional, Union, AsyncIterable

# Import the crypto provider and related utilities from the factory
from .audit_crypto_factory import (
    crypto_provider_factory, settings, log_action, send_alert, CRYPTO_ERRORS,
    _FALLBACK_HMAC_SECRET, SensitiveDataFilter
)
from cryptography.exceptions import InvalidSignature
from .audit_crypto_provider import (
    CryptoOperationError, KeyNotFoundError, InvalidKeyStatusError,
    UnsupportedAlgorithmError, HSMError, HSMConnectionError, HSMKeyError
)
...
def compute_hash(data: bytes) -> str:
    ...
    if not isinstance(data, bytes):
        logger.error("TypeError: Data for hashing must be bytes.", extra={"operation": "compute_hash_fail", "reason": "invalid_input_type"})
...
async def sign_entry(entry: Dict[str, Any], key_id: str, prev_hash: str = '') -> str:
    ...
    if not isinstance(entry, dict):
        logger.error("TypeError: Entry must be a dictionary.", extra={"operation": "sign_entry_fail", "reason": "invalid_entry_type"})
        raise TypeError("Entry must be a dictionary.")
    if not isinstance(key_id, str) or not key_id:
        logger.error("TypeError: Key ID must be a non-empty string.", extra={"operation": "sign_entry_fail", "reason": "invalid_key_id_type"})
        raise TypeError("Key ID must be a non-empty string.")
    if not isinstance(prev_hash, str):
        logger.error("TypeError: Previous hash must be a string.", extra={"operation": "sign_entry_fail", "reason": "invalid_prev_hash_type"})
    ...
    if not all(k in entry for k in required_fields):
        ...
        logger.error(error_msg, extra={"operation": "sign_entry_fail", "reason": "missing_required_fields", "missing_fields": missing_fields})
    ...
    try:
        ...
        await log_action('crypto_key_operation', {'operation': 'sign', 'key_id': key_id, 'entry_hash_signed_content': compute_hash(data_to_sign), 'prev_hash_used': prev_hash, 'success': True})
        return base64.b64encode(sig_bytes).decode('utf-8')
    except (KeyNotFoundError, InvalidKeyStatusError, UnsupportedAlgorithmError, HSMError, CryptoOperationError) as e:
        logger.error(f"Signing failed for entry (ID: {entry.get('entry_id', 'N/A')}, Key: {key_id}): {e}", exc_info=True,
                     extra={"operation": "sign_entry_fail", "entry_id": entry.get('entry_id'), "key_id": key_id, "error_type": type(e).__name__})
        CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type=settings.PROVIDER_TYPE, operation="sign_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'sign', 'key_id': key_id, 'entry_hash_signed_content': compute_hash(data_to_sign), 'prev_hash_used': prev_hash, 'success': False, 'error': str(e)})
        raise
    except Exception as e:
        logger.critical(f"Unexpected error during signing for entry (ID: {entry.get('entry_id', 'N/A')}, Key: {key_id}): {e}", exc_info=True,
                        extra={"operation": "sign_entry_fail_unexpected", "entry_id": entry.get('entry_id'), "key_id": key_id})
        CRYPTO_ERRORS.labels(type="UnexpectedError", provider_type=settings.PROVIDER_TYPE, operation="sign_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'sign', 'key_id': key_id, 'entry_hash_signed_content': compute_hash(data_to_sign), 'prev_hash_used': prev_hash, 'success': False, 'error': str(e)})
...
async def stream_sign_entry(
    ...
    if not all(k in metadata for k in required_fields):
        ...
        raise ValueError(f"Metadata must contain all required fields: {', '.join(required_fields)}. Missing: {', '.join(missing_fields)}.")
    ...
    try:
        ...
        CRYPTO_ERRORS.labels(type="StreamingHashFail", provider_type="utility", operation="stream_sign").inc()
    ...
    try:
        ...
        await log_action('crypto_key_operation', {'operation': 'stream_sign', 'key_id': key_id, 'data_hash': data_hash, 'prev_hash_used': prev_hash, 'success': True})
        return base64.b64encode(sig_bytes).decode('utf-8')
    except (KeyNotFoundError, InvalidKeyStatusError, UnsupportedAlgorithmError, HSMError, CryptoOperationError) as e:
        logger.error(f"Streaming signing failed: {e}", exc_info=True,
                     extra={"operation": "stream_sign_fail", "key_id": key_id, "error_type": type(e).__name__})
        CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type=settings.PROVIDER_TYPE, operation="stream_sign").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_sign', 'key_id': key_id, 'data_hash': data_hash, 'prev_hash_used': prev_hash, 'success': False, 'error': str(e)})
        raise
    except Exception as e:
        logger.critical(f"Unexpected error during streaming signing: {e}", exc_info=True)
        CRYPTO_ERRORS.labels(type="UnexpectedError", provider_type=settings.PROVIDER_TYPE, operation="stream_sign").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_sign', 'key_id': key_id, 'data_hash': data_hash, 'prev_hash_used': prev_hash, 'success': False, 'error': str(e)})
...
async def verify_entry(entry: Dict[str, Any], signature_b64: str, key_id: str) -> bool:
    ...
    """
    if not isinstance(entry, dict):
        logger.error("TypeError: Entry must be a dictionary.", extra={"operation": "verify_entry_fail", "reason": "invalid_entry_type"})
        raise TypeError("Entry must be a dictionary.")
    if not isinstance(signature_b64, str) or not signature_b64:
        logger.error("TypeError: Signature (base64) must be a non-empty string.", extra={"operation": "verify_entry_fail", "reason": "invalid_signature_type"})
        raise TypeError("Signature (base64) must be a non-empty string.")
    if not isinstance(key_id, str) or not key_id:
        logger.error("TypeError: Key ID must be a non-empty string.", extra={"operation": "verify_entry_fail", "reason": "invalid_key_id_type"})
        raise TypeError("Key ID must be a non-empty string.")
    ...
    try:
        ...
        logger.error(f"Failed to base64 decode signature for key '{key_id}' (Entry ID: {entry.get('entry_id', 'N/A')}): {e}", exc_info=True,
                     extra={"operation": "verify_entry_fail", "reason": "base64_decode_fail", "key_id": key_id, "entry_id": entry.get('entry_id')})
        CRYPTO_ERRORS.labels(type="Base64DecodeError", provider_type="unknown", operation="verify_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'verify', 'key_id': key_id, 'success': False, 'entry_hash_verified_content': compute_hash(data_to_verify), 'error': str(e)})
        return False
    ...
    try:
        ...
        await log_action('crypto_key_operation', {'operation': 'verify', 'key_id': key_id, 'success': verified, 'entry_hash_verified_content': compute_hash(data_to_verify)})
        return verified
    except InvalidSignature:
        logger.info(f"Invalid signature detected for key '{key_id}' (Entry ID: {entry.get('entry_id', 'N/A')}).",
                    extra={"operation": "verify_entry_invalid_sig", "key_id": key_id, "entry_id": entry.get('entry_id')})
        CRYPTO_ERRORS.labels(type="InvalidSignature", provider_type=settings.PROVIDER_TYPE, operation="verify_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'verify', 'key_id': key_id, 'success': False, 'entry_hash_verified_content': compute_hash(data_to_verify), 'error': 'InvalidSignature'})
        return False
    except (KeyNotFoundError, UnsupportedAlgorithmError, HSMError, CryptoOperationError) as e:
        logger.error(f"Verification failed for entry (ID: {entry.get('entry_id', 'N/A')}, Key: {key_id}): {e}", exc_info=True,
                     extra={"operation": "verify_entry_fail", "entry_id": entry.get('entry_id'), "key_id": key_id, "error_type": type(e).__name__})
        CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type=settings.PROVIDER_TYPE, operation="verify_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'verify', 'key_id': key_id, 'success': False, 'entry_hash_verified_content': compute_hash(data_to_verify), 'error': str(e)})
        raise
    except Exception as e:
        logger.critical(f"Unexpected error during verification for entry (ID: {entry.get('entry_id', 'N/A')}, Key: {key_id}): {e}", exc_info=True,
                        extra={"operation": "verify_entry_fail_unexpected", "entry_id": entry.get('entry_id'), "key_id": key_id})
        CRYPTO_ERRORS.labels(type="UnexpectedError", provider_type=settings.PROVIDER_TYPE, operation="verify_entry").inc()
        await log_action('crypto_key_operation', {'operation': 'verify', 'key_id': key_id, 'success': False, 'entry_hash_verified_content': compute_hash(data_to_verify), 'error': str(e)})
...
async def stream_verify_entry(
    ...
    try:
        ...
        logger.error(f"Failed to compute hash from data stream for verification: {e}", exc_info=True)
        CRYPTO_ERRORS.labels(type="StreamingHashFail", provider_type="utility", operation="stream_verify").inc()
        return False
    ...
    try:
        ...
        logger.error(f"Failed to base64 decode signature for streaming verification: {e}", exc_info=True)
        CRYPTO_ERRORS.labels(type="Base64DecodeError", provider_type="unknown", operation="stream_verify").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_verify', 'key_id': key_id, 'success': False, 'data_hash_verified_content': data_hash, 'error': str(e)})
        return False
    ...
    try:
        ...
        await log_action('crypto_key_operation', {'operation': 'stream_verify', 'key_id': key_id, 'success': verified, 'data_hash_verified_content': data_hash})
        return verified
    except InvalidSignature:
        logger.info(f"Invalid signature detected during streaming verification for key '{key_id}'.")
        CRYPTO_ERRORS.labels(type="InvalidSignature", provider_type=settings.PROVIDER_TYPE, operation="stream_verify").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_verify', 'key_id': key_id, 'success': False, 'data_hash_verified_content': data_hash, 'error': 'InvalidSignature'})
        return False
    except (KeyNotFoundError, UnsupportedAlgorithmError, HSMError, CryptoOperationError) as e:
        logger.error(f"Streaming verification failed for key '{key_id}': {e}", exc_info=True,
                     extra={"operation": "stream_verify_fail", "key_id": key_id, "error_type": type(e).__name__})
        CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type=settings.PROVIDER_TYPE, operation="stream_verify").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_verify', 'key_id': key_id, 'success': False, 'data_hash_verified_content': data_hash, 'error': str(e)})
        raise
    except Exception as e:
        logger.critical(f"Unexpected error during streaming verification: {e}", exc_info=True)
        CRYPTO_ERRORS.labels(type="UnexpectedError", provider_type=settings.PROVIDER_TYPE, operation="stream_verify").inc()
        await log_action('crypto_key_operation', {'operation': 'stream_verify', 'key_id': key_id, 'success': False, 'data_hash_verified_content': data_hash, 'error': str(e)})
...
async def verify_chain(entries: List[Dict[str, Any]]) -> bool:
    ...
    if not isinstance(entries, list):
        logger.error("TypeError: Entries must be a list.", extra={"operation": "verify_chain_fail", "reason": "invalid_entries_type"})
        raise TypeError("Entries must be a list.")
    if not all(isinstance(e, dict) for e in entries):
        logger.error("TypeError: All entries in the list must be dictionaries.", extra={"operation": "verify_chain_fail", "reason": "invalid_entry_type_in_list"})
        raise TypeError("All entries in the list must be dictionaries.")

    if not entries:
        logger.warning("verify_chain called with empty entries list. Returning True.", extra={"operation": "verify_chain_empty"})
    ...
    for i, entry_original in enumerate(entries):
        ...
        if entry_current_prev_hash != prev_hash:
            logger.error(f"Chain integrity check failed at entry {i} (ID: {entry_copy.get('entry_id', 'N/A')}): expected prev_hash '{prev_hash}', got '{entry_current_prev_hash}'.",
                         extra={"operation": "verify_chain_fail", "entry_index": i, "expected_hash": prev_hash, "actual_hash": entry_current_prev_hash, "entry_id": entry_copy.get('entry_id')})
            CRYPTO_ERRORS.labels(type="ChainIntegrityFail", provider_type="unknown", operation="verify_chain").inc()
            await log_action("verify_chain", status="fail", reason="hash_mismatch", entry_index=i, entry_id=entry_copy.get('entry_id'))
            return False
        ...
        if signature_b64 is None or key_id is None:
            logger.error(f"Entry {i} (ID: {entry_copy.get('entry_id', 'N/A')}) missing signature or key_id for chain verification. Chain broken.",
                         extra={"operation": "verify_chain_missing_sig_key", "entry_index": i, "entry_id": entry_copy.get('entry_id')})
            CRYPTO_ERRORS.labels(type="MissingSigOrKey", provider_type="unknown", operation="verify_chain").inc()
            await log_action("verify_chain", status="fail", reason="missing_sig_or_key", entry_index=i, entry_id=entry_copy.get('entry_id'))
            return False
    ...
    try:
        ...
        if not all(results):
            logger.error("One or more entries failed cryptographic signature verification within the chain.",
                         extra={"operation": "verify_chain_cryptographic_fail"})
            await log_action("verify_chain", status="fail", reason="cryptographic_mismatch")
            return False

        logger.info("Audit log chain verification successful.", extra={"operation": "verify_chain_success"})
        await log_action("verify_chain", status="success")
        return True
    except CryptoOperationError as e:
        logger.error(f"Critical crypto error during chain verification: {e}", exc_info=True,
                     extra={"operation": "verify_chain_critical_error", "error": str(e)})
        CRYPTO_ERRORS.labels(type="CriticalCryptoError", provider_type="unknown", operation="verify_chain").inc()
        await log_action("verify_chain", status="fail", reason="critical_crypto_error", error=str(e))
        raise
...
async def rotate_key(algo: str, old_key_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.info(f"Key rotation initiated: New key '{new_id}' generated for algo '{algo}'.",
                    extra={"operation": "rotate_key_initiated", "old_key_id": old_key_id, "new_key_id": new_id, "algo": algo})
        await log_action('crypto_key_operation', {'operation': 'rotate_key_overall', 'old_key_id': old_key_id, 'new_key_id': new_id, 'algo': algo, 'provider': settings.PROVIDER_TYPE, 'success': True})
        return new_id
    except (KeyNotFoundError, UnsupportedAlgorithmError, HSMError, CryptoOperationError) as e:
        logger.error(f"Key rotation failed for algo '{algo}' (old key: {old_key_id}): {e}", exc_info=True,
                     extra={"operation": "rotate_key_fail", "old_key_id": old_key_id, "algo": algo, "error_type": type(e).__name__})
        CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type=settings.PROVIDER_TYPE, operation="rotate_key").inc()
        await log_action('crypto_key_operation', {'operation': 'rotate_key_overall', 'old_key_id': old_key_id, 'new_key_id': 'N/A', 'algo': algo, 'provider': settings.PROVIDER_TYPE, 'success': False, 'error': str(e)})
        raise
    except Exception as e:
        logger.critical(f"Unexpected error during key rotation for algo '{algo}' (old key: {old_key_id}): {e}", exc_info=True,
                        extra={"operation": "rotate_key_fail_unexpected", "old_key_id": old_key_id, "algo": algo})
        CRYPTO_ERRORS.labels(type="UnexpectedError", provider_type=settings.PROVIDER_TYPE, operation="rotate_key").inc()
        await log_action('crypto_key_operation', {'operation': 'rotate_key_overall', 'old_key_id': old_key_id, 'new_key_id': 'N/A', 'algo': algo, 'provider': settings.PROVIDER_TYPE, 'success': False, 'error': str(e)})
...
async def safe_sign(entry: Dict[str, Any], key_id: str, prev_hash: str = '') -> str:
    ...
    if _FALLBACK_ATTEMPT_COUNT.get('total', 0) >= max_fallback_disable:
        ...
        try:
            ...
            loop.create_task(send_alert("CRITICAL: HMAC fallback auto-disabled. Audit log cannot be signed!", severity="emergency"))
            loop.create_task(log_action('crypto_fallback_disabled', {'reason': 'max_attempts_reached'}))
        except RuntimeError:
            # If no event loop is running, we must run the async calls synchronously
            try:
                asyncio.run(send_alert("CRITICAL: HMAC fallback auto-disabled. Audit log cannot be signed!", severity="emergency"))
                asyncio.run(log_action('crypto_fallback_disabled', {'reason': 'max_attempts_reached'}))
            except Exception:
                logging.critical("No running event loop to send critical alert or log crypto fallback disable.")
    ...
    try:
        ...
        logger.error(f"Primary signing failed for entry (ID: {entry.get('entry_id', 'N/A')}, Action: {entry.get('action', 'N/A')}): {e}. Falling back to HMAC.", exc_info=True,
                     extra={"operation": "primary_sign_fail", "entry_id": entry.get('entry_id'), "action": entry.get('action')})
        CRYPTO_ERRORS.labels(type="PrimarySignFail", provider_type=settings.PROVIDER_TYPE, operation="safe_sign").inc()
        await log_action('crypto_primary_sign_fail', {'operation': 'sign', 'key_id': key_id, 'entry_id': entry.get('entry_id'), 'reason': str(e)})
        ...
        if (_FALLBACK_ATTEMPT_COUNT['since_alert'] >= max_fallback_alert and
            ...
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(send_alert(f"WARNING: Primary crypto system is experiencing issues. {_FALLBACK_ATTEMPT_COUNT['since_alert']} fallback attempts in a row. HMAC fallback is active.", severity="high"))
            except RuntimeError:
                # If no event loop is running, we must run the async calls synchronously
                try:
                    asyncio.run(send_alert(f"WARNING: Primary crypto system is experiencing issues. {_FALLBACK_ATTEMPT_COUNT['since_alert']} fallback attempts in a row. HMAC fallback is active.", severity="high"))
                except Exception:
        ...
        try:
            ...
            logger.warning(f"Successfully signed using HMAC fallback for entry (ID: {entry.get('entry_id', 'N/A')}). ALERT: Primary crypto system is down!",
                           extra={"operation": "hmac_fallback_success", "entry_id": entry.get('entry_id')})
            await log_action('crypto_fallback_sign', {'operation': 'sign', 'key_id': 'HMAC_FALLBACK', 'entry_id': entry.get('entry_id'), 'reason': str(e), 'success': True})
            return fallback_signature
        except Exception as fallback_e:
            logger.critical(f"HMAC fallback signing also failed for entry (ID: {entry.get('entry_id', 'N/A')}): {fallback_e}. Data will NOT be signed.", exc_info=True,
                            extra={"operation": "hmac_fallback_fail", "entry_id": entry.get('entry_id')})
            CRYPTO_ERRORS.labels(type="FallbackSignFail", provider_type="fallback", operation="safe_sign").inc()
            
            # Explicitly escalate the failure
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(send_alert(f"CRITICAL: HMAC fallback signing failed for {entry.get('entry_id')}. Audit log cannot be signed!", severity="emergency"))
                loop.create_task(log_action('crypto_fallback_sign', {'operation': 'sign', 'key_id': 'HMAC_FALLBACK', 'entry_id': entry.get('entry_id'), 'reason': str(fallback_e), 'success': False}))
            except RuntimeError:
                # If no event loop is running, we must run the async calls synchronously
                try:
                    asyncio.run(send_alert(f"CRITICAL: HMAC fallback signing failed for {entry.get('entry_id')}. Audit log cannot be signed!", severity="emergency"))
                    asyncio.run(log_action('crypto_fallback_sign', {'operation': 'sign', 'key_id': 'HMAC_FALLBACK', 'entry_id': entry.get('entry_id'), 'reason': str(fallback_e), 'success': False}))
                except Exception:
                    logging.critical("No running event loop to send critical alert or log fallback failure.")
...
def hmac_sign_fallback(entry: Dict[str, Any], prev_hash: str) -> str:
    ...
    """
    if not isinstance(entry, dict):
        logger.error("TypeError: Entry must be a dictionary for HMAC fallback.", extra={"operation": "hmac_sign_fallback_fail", "reason": "invalid_entry_type"})
        raise TypeError("Entry must be a dictionary.")
    if not isinstance(prev_hash, str):
        logger.error("TypeError: Previous hash must be a string for HMAC fallback.", extra={"operation": "hmac_sign_fallback_fail", "reason": "invalid_prev_hash_type"})
        raise TypeError("Previous hash must be a string.")

    if _FALLBACK_HMAC_SECRET is None:
        logger.critical("HMAC fallback secret not securely configured. Cannot perform fallback signing.",
                       extra={"operation": "hmac_fallback_secret_missing"})
        CRYPTO_ERRORS.labels(type="FallbackSecretMissing", provider_type="fallback", operation="hmac_sign").inc()
        raise RuntimeError("HMAC fallback secret not securely configured. Cannot perform fallback signing.")
