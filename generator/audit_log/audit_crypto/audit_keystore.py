...
class FileSystemKeyStorageBackend:
    ...
    async def _acquire_lock(self, filepath: str, shared: bool = False):
        ...
        self.logger.debug(f"Attempting to acquire {'shared' if shared else 'exclusive'} lock for {filepath}")
        ...
        try:
            ...
            self.logger.debug(f"Acquired {'shared' if shared else 'exclusive'} lock for {filepath}.")
    ...
    async def _verify_permissions(self, filepath: str):
        ...
        try:
            ...
            if current_permissions != expected_permissions:
                self.logger.warning(f"File '{filepath}' has incorrect permissions ({oct(current_permissions)}), expected {oct(expected_permissions)}. Attempting to correct.",
                                    extra={"operation": "permission_mismatch", "filepath": filepath, "current_perm": oct(current_permissions)})
                await asyncio.to_thread(os.chmod, filepath, expected_permissions)
                self.logger.info(f"Permissions for '{filepath}' corrected to {oct(expected_permissions)}.",
    ...
    async def load_key_data(self, key_id: str) -> Optional[Dict[str, Any]]:
        ...
        if not os.path.exists(filepath):
            self.logger.debug(f"File system backend: Key file '{filepath}' not found.", extra={"key_id": key_id})
            return None
                ...
                try:
                    ...
                    try:
                        ...
                        keys_metadata.append({
                            ...
                            "status": metadata_json.get("status", "active"), # Default to active if status is missing
                            "retired_at": metadata_json.get("retired_at"), # Include retired_at if present
                            "last_modified": metadata_json.get("last_modified") # Include last_modified
                        })
                    except json.JSONDecodeError as e:
                        self.logger.error(f"File system backend: Corrupted JSON during list for key '{key_id}': {e}. Skipping.", exc_info=True)
                        # Do not re-raise, continue processing other files
                    finally:
                        await self._release_lock(filepath)
                except Exception as e:
                    self.logger.error(f"File system backend: Failed to read metadata for key file '{filename}': {e}", exc_info=True)
...
class KeyStore:
    ...
    """
    def __init__(self, key_dir: str, master_key: bytes, backend: Optional[KeyStorageBackend] = None):
        """
        ...
        if len(master_key) != 32:
            logger.critical("KeyStore: Master key for at-rest encryption is not 32 bytes (AES-256). Data encryption will fail.",
        ...
        if backend is None:
            ...
            self.logger.info(f"KeyStore initialized with FileSystemKeyStorageBackend at '{key_dir}'.")
        else:
            self.backend = backend
            self.logger.info(f"KeyStore initialized with custom backend: {type(backend).__name__}.")


    async def store_key(self, key_id: str, key_data_bytes: bytes, algo: str, creation_time: float, status: str, retired_at: Optional[float] = None) -> None:
        """
        ...
        async with self.lock:
            try:
                ...
                self.logger.info(f"Key '{key_id}' stored securely via {type(self.backend).__name__}.",
                                 extra={"operation": "key_store_success", "key_id": key_id, "algo": algo, "status": status})
                ...
                await log_action("key_store", key_id=key_id, algo=algo, status=status, success=False, error=str(e))
    ...
    async def load_key(self, key_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self.lock:
            try:
                ...
                if metadata is None:
                    self.logger.debug(f"Key '{key_id}' not found in backend {type(self.backend).__name__}.", extra={"key_id": key_id})
                    await log_action("key_load", key_id=key_id, success=False, error="Key not found")
                    return None
                ...
                decryptor.authenticate_additional_data(str(metadata["creation_time"]).encode('utf-8'))
                decryptor.authenticate_additional_data(metadata.get("status", "active").encode('utf-8')) # Use .get with default for backward compatibility
                if metadata.get("retired_at") is not None:
                    decryptor.authenticate_additional_data(str(metadata["retired_at"]).encode('utf-8'))

                key_data = decryptor.update(ciphertext) + decryptor.finalize() # Finalize performs tag verification

                self.logger.info(f"Key '{key_id}' loaded and decrypted successfully.",
                                 extra={"operation": "key_load_success", "key_id": key_id, "algo": metadata["algo"]})
                KEY_LOAD_COUNT.labels(provider_type="software", status="success").inc()
                await log_action("key_load", key_id=key_id, algo=metadata["algo"], status=metadata.get("status"), success=True)
                ...
                self.logger.error(f"KeyStore: Integrity check failed for key '{key_id}'. Possible tampering or wrong master key. Data: {metadata.get('encrypted_payload_b64', 'N/A')}",
                ...
                CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="load_key").inc()
    ...
    async def list_keys(self) -> List[Dict[str, Any]]:
        ...
        async with self.lock:
            try:
                ...
                self.logger.info(f"Listed {len(keys_metadata)} keys from {type(self.backend).__name__}.",
                                 extra={"operation": "list_keys_success", "count": len(keys_metadata)})
                return keys_metadata
            except Exception as e:
                self.logger.error(f"Failed to list keys from backend {type(self.backend).__name__}: {e}", exc_info=True)
                CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="list_keys").inc()
    ...
    async def delete_key_file(self, key_id: str) -> bool:
        ...
        async with self.lock:
            try:
                ...
                if deleted:
                    self.logger.info(f"Securely deleted key '{key_id}' via {type(self.backend).__name__}.",
                                     extra={"operation": "delete_key_file_success", "key_id": key_id})
                    await log_action("key_delete", key_id=key_id, success=True)
                else:
                    self.logger.warning(f"Attempted to delete key '{key_id}', but it was not found or could not be deleted by backend {type(self.backend).__name__}.",
                                        extra={"operation": "delete_key_file_not_found_or_fail", "key_id": key_id})
                    await log_action("key_delete", key_id=key_id, success=False, error="Key not found or backend failed to delete")
                return deleted
            except Exception as e:
                self.logger.error(f"Failed to delete key file for '{key_id}' via {type(self.backend).__name__}: {e}", exc_info=True,
                                  extra={"operation": "delete_key_file_fail", "key_id": key_id})
                CRYPTO_ERRORS.labels(type=type(e).__name__, provider_type="software", operation="delete_key_file").inc()
