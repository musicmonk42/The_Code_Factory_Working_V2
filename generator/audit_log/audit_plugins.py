...
# Metrics
PLUGIN_INVOCATIONS = safe_counter('audit_plugin_invocations_total', 'Plugin calls', ['event', 'plugin'])
PLUGIN_ERRORS = safe_counter('audit_plugin_errors_total', 'Plugin errors', ['event', 'plugin', 'type'])
# --- START: FIX 1 (Apply safe_gauge) ---
PLUGIN_LATENCY = safe_gauge('audit_plugin_latency_seconds', 'Plugin execution time', ['event', 'plugin'])
# --- END: FIX 1 ---
PLUGIN_MODIFICATIONS = safe_counter('audit_plugin_modifications_total', 'Entry modifications', ['plugin', 'type'])  # modify/redact/augment
# New metric for commercial plugin usage
COMMERCIAL_PLUGIN_USAGE = safe_counter('audit_commercial_plugin_usage_total', 'Usage count for commercial plugins', ['plugin', 'feature'])
...
class DefaultPlugin(AuditPlugin):
    ...
    def process(self, event: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if POLICY_CONTROLS['redact'] and 'details' in data and isinstance(data['details'], dict) and 'password' in data['details']:
...
class BillingPlugin(CommercialPlugin):
    ...
    def process(self, event: str, data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        original_entry_json = json.dumps(data, sort_keys=True) # Ensure consistent sorting for comparison
...
def register_plugin(name: str, plugin: AuditPlugin, controls: Optional[Dict[str, bool]] = None):
    ...
    logger.info(f"Manually registered plugin '{name}' with controls: {getattr(plugin, 'policy_controls')}")
...
def _sandboxed_worker(q: multiprocessing.Queue, plugin: AuditPlugin, entry: Dict[str, Any], event: str):
    """
    ...
    try:
        ...
        if not plugin_instance:
            raise RuntimeError(f"Plugin instance not available in worker process.")
...
async def sandboxed_execute(plugin: AuditPlugin, entry: Dict[str, Any], event: str) -> Optional[Dict[str, Any]]:
    ...
    p = multiprocessing.Process(target=_sandboxed_worker, args=(q, plugin, entry, event), name=f"audit_plugin_{plugin_name}")
    p.start()
    ...
    try:
        while (time.perf_counter() - start_time) < MAX_PLUGIN_TIME_SECONDS:
            ...
            if not p.is_alive():
                result_or_exception = RuntimeError("Plugin process terminated before sending result.")
                break
        ...
        if p.exitcode is not None and p.exitcode != 0:
            result_or_exception = RuntimeError(f"Plugin process terminated abruptly (ExitCode: {p.exitcode}).")
...
async def trigger_event(event: str, data: Any) -> Any:
    ...
    try:
        ...
        logger.warning("Failed to deep copy data using JSON serialization. Falling back to shallow copy.")
