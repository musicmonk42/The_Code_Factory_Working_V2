...
from typing import Dict, Any, Annotated, List, Optional, Union, Callable
...
try:
    # FastAPI imports
    from fastapi import FastAPI, Depends, HTTPException, status, Body, WebSocket, WebSocketDisconnect, Security, Request, UploadFile, File, Form, Query, Path as FastAPIPath
    ...
    from sqlalchemy import create_engine, Column, String, Text, DateTime, Boolean, Integer, text  # ADDED 'text' as per Step 1
    ...
    logging.getLogger(__name__).info("Successfully imported heavy dependencies (FastAPI, SQLAlchemy, etc.)")

except ImportError as e:
    logging.getLogger(__name__).warning(f"Heavy dependencies (FastAPI, SQLAlchemy, etc.) not found. API functionality will be disabled. Error: {e}")
    
    # --- Dummies for Imports ---
    
    # Dummy function/decorators
    def Depends(callable=None): return callable
    def Security(callable=None): return callable
    ...
    def FastAPIPath(*args, **kwargs): return None
    ...
    # Dummy Classes
    class _DummyFastAPI:
        def __init__(self, *args, **kwargs): self.state = {}
        def __call__(self, *args, **kwargs): return self
        def __getattr__(self, name):
            if name in ("post", "get", "put", "delete", "on_event", "add_middleware", "add_exception_handler"):
    ...
    class CryptContext:
        ...
        def hash(self, *args, **kwargs): return "dummy_hash"
    ...
    class _DummyMetadata:
        def create_all(self, *args, **kwargs):
            logging.getLogger(__name__).info("Dummy Base.metadata.create_all() called (SQLAlchemy not available).")
    class _DummyBase:
        ...
        user_id = None
        def __init__(self, *args, **kwargs): pass
        def __repr__(self): return "DummyModelInstance"
    ...
    class FastAPIInstrumentor:
        def instrument_app(self, *args, **kwargs): pass
    ...
    class _DummyTracer:
        def start_as_current_span(self, *args, **kwargs): return _DummySpan()
        
    class _DummyTrace:
        def get_tracer(self, *args, **kwargs): return _DummyTracer()
        def get_tracer_provider(self, *args, **kwargs): return self
        def set_tracer_provider(self, *args, **kwargs): pass
...
try:
    ...
    from runner.runner_logging import logger as runner_logger, search_logs # Use alias to avoid name clash
    ...
    def load_config(config_file: str):
        ...
        logging.warning(f"Using DUMMY load_config due to ImportError. Could not load real config '{config_file}'.")
    ...
    logging.warning("Custom modules (runner, intent_parser, logging, metrics, utils) not found. Using dummy implementations.")
...
if not SECRET_KEY and not os.getenv("TESTING") and _FASTAPI_AVAILABLE: # Only raise if not testing AND fastapi is available
...
pwd_context = CryptContext(schemes=[API_KEY_HASH_SCHEME], deprecated="auto") # Uses dummy if not available
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/api/v1/token", auto_error=False) # <<< FIX: Added auto_error=False
...
if not DATABASE_URL:
    ...
    elif _FASTAPI_AVAILABLE and not os.getenv("TESTING"): # Only raise if not testing AND fastapi is available
        logger.critical("DATABASE_URL environment variable not set. This is required for production.")
...
class APIKey(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    api_key_id = Column(String, unique=True, index=True, nullable=False) # Public ID for the API key
...
def create_db_tables(bind_engine=None): # <<< FIX: Added bind_engine=None
    ...
    try:
        if Base.metadata is not None and hasattr(Base.metadata, 'create_all'):
            ...
            logger.warning("Dummy create_db_tables() called (SQLAlchemy not available or Base.metadata is None).")
...
async def get_current_user_from_token(
    token: Annotated[Optional[str], Depends(oauth2_scheme)], db: Session = Depends(get_db) # <<< FIX: Made token Optional
):
    ...
    if user is None or not (_FASTAPI_AVAILABLE and user.is_active): # Skip is_active check in dummy mode
        if _FASTAPI_AVAILABLE: # Only check if not dummy
             if user is None or not user.is_active:
                logger.warning(f"Authentication failed: User '{token_data.username}' not found or inactive.")
                raise credentials_exception
...
async def get_current_api_key(api_key: Annotated[str, Security(api_key_header)], db: Session = Depends(get_db)):
    ...
    if api_key_obj is None or not (_FASTAPI_AVAILABLE and api_key_obj.is_active):
        if _FASTAPI_AVAILABLE:
            if api_key_obj is None or not api_key_obj.is_active:
                logger.warning(f"API key authentication failed: Invalid or inactive API Key provided.")
...
async def get_current_active_entity(
    ...
    api_key: Optional[APIKey] = Security(get_current_api_key) # Use Security to make it optional if user auth succeeds
):
...
def require_scopes(required_scopes: List[str]):
    ...
    async def scopes_checker(current_entity: Union[User, APIKey] = Depends(get_current_active_entity)):
        if not _FASTAPI_AVAILABLE: # If in dummy mode, always pass authorization
             return {"username": "dummy_user", "scopes": required_scopes}
             
        entity_scopes = current_entity.scopes.split(',') if isinstance(current_entity.scopes, str) else current_entity.scopes

        for scope in required_scopes:
            if scope not in entity_scopes:
                entity_id = current_entity.username if isinstance(current_entity, User) else current_entity.api_key_id
                logger.warning(f"Authorization failed for entity '{entity_id}': Insufficient scopes. Required: {required_scopes}, Has: {entity_scopes}")
...
if _FASTAPI_AVAILABLE:
    ...
    Instrumentator().instrument(api).expose(api, endpoint="/api/v1/metrics", include_in_schema=True) # MODIFIED as per Step 4 (already present in file)
...
try:
    ...
    logger.critical(f"FATAL: Required configuration file 'config.yaml' not found during API import time: {e}. Using minimal dummy config.", exc_info=True)
    ...
    logger.critical(f"Failed to load config (likely dummy setup): {e}. Using minimal dummy config.", exc_info=True)
    _global_config_instance = {
        "version": 1,
        "backend": "dummy",
        "framework": "dummy",
...
def register_api_endpoint(path: str, method: str = "POST", status_code: int = 200, summary: Optional[str] = None, scopes: Optional[List[str]] = None):
    """
    ...
    def decorator(func: Callable):
        ...
        api_method(versioned_path, status_code=status_code, summary=summary, dependencies=dependencies_list)(func)

        logger.info(f"Registered API endpoint: {method.upper()} {versioned_path} (Scopes: {scopes})")
        return func
    return decorator
    ...
    with tracer.start_as_current_span("health_check") as span:
        try:
            ...
            if not _FASTAPI_AVAILABLE:
                ...
                span.record_exception(e)
                raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Unexpected health check error.")
        ...
        overall_status = "healthy" if all([db_status != "failed", runner_status == "ok", parser_status == "ok"]) else "degraded"
        
        span.set_status(Status(StatusCode.OK if overall_status == "healthy" else StatusCode.ERROR))
        
        return {"status": overall_status, "components": {"database": db_status, "runner": runner_status, "parser": parser_status}}
    # END REPLACED

# --- Authentication Endpoints ---
@api.post("/api/v1/token", response_model=Token, summary="Obtain JWT access token for user authentication")
async def login_for_access_token(
    ...
    with tracer.start_as_current_span("login_for_access_token") as span:
        ...
        if not user or not pwd_context.verify(password, user.hashed_password):
            ...
            span.set_status(Status(StatusCode.UNAUTHENTICATED, "Invalid credentials")) # FIX: Use Status object
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        access_token = create_access_token(
            data={"sub": user.username, "scopes": user.scopes.split(',') if _FASTAPI_AVAILABLE else user.scopes}, # Store scopes as list
...
@register_api_endpoint(path="/users", method="POST", status_code=status.HTTP_201_CREATED, summary="Register a new user", scopes=["admin"])
async def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    with tracer.start_as_current_span("register_user") as span:
        ...
        if existing_user:
            ...
            span.set_status(Status(StatusCode.ALREADY_EXISTS, "Username already exists")) # FIX: Use Status object
        ...
        try:
            ...
            logger.error(f"Database integrity error during user registration for '{user_data.username}': {e}", exc_info=True)
            span.set_status(Status(StatusCode.ALREADY_EXISTS, "Username already exists")) # FIX: Use Status object
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Username already registered (DB error)")
        except SQLAlchemyError as e:
            logger.error(f"Database error during user registration for '{user_data.username}': {e}", exc_info=True)
            span.set_status(Status(StatusCode.INTERNAL_ERROR, "Database error")) # FIX: Use Status object
            raise HTTPException(status_code=500, detail="Internal database error during user registration.")
        except Exception as e:
            # Catch errors from dummy session
            ...
            return UserResponse(
                ...
                last_login_at=None,
                model_config={'from_attributes': True} if not hasattr(UserResponse.Config, "orm_mode") else {}
            )


@register_api_endpoint(path="/api-keys", method="POST", status_code=status.HTTP_201_CREATED, summary="Create a new API key", scopes=["admin"])
async def create_api_key(api_key_data: APIKeyCreate, db: Session = Depends(get_db)):
    ...
    with tracer.start_as_current_span("create_api_key") as span:
        ...
        try:
            ...
            return {"api_key_id": api_key_id, "api_key": raw_api_key, "scopes": api_key_data.scopes}
        except SQLAlchemyError as e:
            logger.error(f"Database error during API key creation: {e}", exc_info=True)
            span.set_status(Status(StatusCode.INTERNAL_ERROR, "Database error")) # FIX: Use Status object
            raise HTTPException(status_code=500, detail="Internal database error during API key creation.")
        except Exception as e:
            # Catch errors from dummy session
...
@register_api_endpoint(path="/api-keys/{api_key_id}", method="DELETE", status_code=status.HTTP_204_NO_CONTENT, summary="Delete an API key", scopes=["admin"])
async def delete_api_key(api_key_id: str, db: Session = Depends(get_db)):
    ...
    with tracer.start_as_current_span("delete_api_key") as span:
        ...
        try:
            ...
            logger.error(f"Database error during API key deletion for '{api_key_id}': {e}", exc_info=True)
            span.set_status(Status(StatusCode.INTERNAL_ERROR, "Database error")) # FIX: Use Status object
            raise HTTPException(status_code=500, detail="Internal database error during API key deletion.")
        except Exception as e:
            # Catch errors from dummy session
...
@register_api_endpoint(path="/run", summary="Run the generator with the given payload", scopes=["run"])
async def api_run(
    ...
    with tracer.start_as_current_span("api_run") as span:
        entity_id = authenticated_entity.username if isinstance(authenticated_entity, User) else authenticated_entity.api_key_id
        ...
        try:
            ...
            span.record_exception(e)
            raise HTTPException(status_code=500, detail="An internal error occurred during the generator run.")
...
@register_api_endpoint(path="/parse/text", summary="Parse text content to extract intent", scopes=["parse"])
async def api_parse_text(
    ...
    with tracer.start_as_current_span("api_parse_text") as span:
        ...
        try:
            ...
            span.set_status(Status(StatusCode.ERROR, str(e))) # FIX: Use Status object
            span.record_exception(e)
            raise HTTPException(status_code=500, detail="An internal error occurred during text parsing.")

@register_api_endpoint(path="/parse/file", summary="Parse an uploaded file to extract intent", scopes=["parse"])
async def api_parse_file(
    file: UploadFile = File(..., description="The file to parse"), # Use File for file uploads
    format_hint: Optional[str] = Form(None, description="Hint about the file format (e.g., 'markdown', 'json')"),
    dry_run: bool = Form(False, description="If true, perform a dry run without persisting changes"),
    authenticated_entity: Union[User, APIKey] = Depends(require_scopes(["parse"]))
):
    """
    ...
    with tracer.start_as_current_span("api_parse_file") as span:
        ...
        try:
            ...
            span.set_status(Status(StatusCode.ERROR, str(e))) # FIX: Use Status object
            span.record_exception(e)
            raise HTTPException(status_code=500, detail="An internal error occurred during file parsing.")
        finally:
            if temp_file_path.exists():
                try:
                    ...
                    logger.error(f"Error cleaning up temporary file {temp_file_path}: {e}", exc_info=True)
...
@register_api_endpoint(path="/parse/feedback/{item_id}", summary="Submit feedback for a parsed item", scopes=["parse"])
async def api_submit_parse_feedback(
    item_id: str = FastAPIPath(..., description="ID of the parsed item to submit feedback for"),
    rating: float = Body(..., ge=0.0, le=1.0, description="Rating for the parsed item (0.0 to 1.0)"),
    authenticated_entity: Union[User, APIKey] = Depends(require_scopes(["parse"]))
):
    """
...
@register_api_endpoint(path="/parse/reload_config", summary="Reload parser configuration", scopes=["admin"])
async def api_reload_parser_config(authenticated_entity: Union[User, APIKey] = Depends(require_scopes(["admin"]))):
    """
    ...
    with tracer.start_as_current_span("api_reload_parser_config") as span:
        ...
        try:
            ...
            span.set_status(Status(StatusCode.OK)) # FIX: Use Status object
            return {"status": "success", "message": "IntentParser configuration reloaded."}
        except Exception as e:
            logger.error(f"Config reload error for entity {entity_id}: {e}", exc_info=True)
            span.set_status(Status(StatusCode.ERROR, str(e))) # FIX: Use Status object
            span.record_exception(e)
            raise HTTPException(status_code=500, detail="An internal error occurred while reloading configuration.")
...
class SubmitFeedbackRequest(BaseModel):
    ...
    comments: Optional[str] = Field(None, max_length=500, description="Optional comments for the feedback")

@register_api_endpoint(path="/feedback", summary="Submit feedback on a generator run", scopes=["feedback"])
async def api_submit_runner_feedback(
    ...
    with tracer.start_as_current_span("api_submit_runner_feedback") as span:
        ...
        try:
            ...
            logger.info(f"Feedback submitted for run {feedback_data.run_id} by entity {entity_id}: rating {feedback_data.rating}, comments: {feedback_data.comments}")
            span.set_status(Status(StatusCode.OK)) # FIX: Use Status object
...
async def api_search_logs(
    ...
    with tracer.start_as_current_span("api_search_logs") as span:
        ...
        try:
            ...
            span.set_status(Status(StatusCode.ERROR, str(e))) # FIX: Use Status object
            span.record_exception(e)
            raise HTTPException(status_code=500, detail="An internal error occurred during log search.")
...
if __name__ == "__main__":
    if _FASTAPI_AVAILABLE:
        ...
        print("Cannot run API server directly: FastAPI dependencies are not installed.", file=os.stderr)
        exit(1)