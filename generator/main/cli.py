...
try:
    ...
    from runner.runner_logging import logger, search_logs, log_action # Using runner's logger
    ...
    class DummyConfigWatcher:
        def __init__(self, path, callback): logging.warning("DummyConfigWatcher: Initialized."); self.path = path; self.callback = callback
        async def start(self): logging.warning("DummyConfigWatcher: Started.")
        async def _reload(self): logging.warning("DummyConfigWatcher: Reloading config (dummy)."); self.callback("new_config", {"dummy": "reloaded"})
        def stop(self): logging.warning("DummyConfigWatcher: Stopped.") # Add dummy stop
    
    AGENT_REGISTRY = {"dummy_agent": "DummyAgentClass"}
    def register_agent(name, agent_class): logging.warning(f"Dummy register_agent: {name}")
    def hot_swap_agent(name, new_agent_class): logging.warning(f"Dummy hot_swap_agent: {name}")

    # Dummy logging, metrics, utils if not found
    _cli_logger = logging.getLogger("cli")
    def logger_dummy_info(*args, **kwargs): _cli_logger.info(*args, **kwargs)
    def logger_dummy_error(*args, **kwargs): _cli_logger.error(*args, **kwargs)
    def logger_dummy_debug(*args, **kwargs): _cli_logger.debug(*args, **kwargs)
    def logger_dummy_warning(*args, **kwargs): _cli_logger.warning(*args, **kwargs)
    ...
    def search_logs(query: str, limit: int = 10):
        ...
        return [f"Dummy log entry for query: {query}"] * min(limit, 1) # Return at least one dummy log
    ...
    logger.warning(f"Failed to import core runner/engine modules: {e}. Using dummy implementations for CLI functionality.")
...
@click.group() 
@click.option('--config-file', 'config_path', default='config.yaml', type=click.Path(exists=True, readable=True, path_type=Path), help="Path to the configuration file (e.g., config.yaml).")
@click.pass_context
...
@click.option('--input', 'input_path', required=True, type=click.Path(exists=True, readable=True, path_type=Path), help="Path to the input README.md file.")
@click.option('--max-iterations', type=int, default=10, help="Maximum iterations for the workflow engine's refinement loop.")
@click.option('--dry-run', is_flag=True, help="Simulate workflow execution without making actual changes or external calls.")
@click.option('--output-dir', default='output', type=click.Path(path_type=Path), help="Directory to store generated artifacts and logs.")
# FIX: Removed standalone --config option, will use context
@click.option('--parallel', type=int, default=1, help="Number of workflow instances to run in parallel using multiprocessing (ignored if distributed).")
@click.option('--distributed', is_flag=True, help="Enable distributed backend for workflow execution (e.g., Kubernetes). Overrides --parallel.")
@click.option('--interactive', is_flag=True, help="Run in interactive mode, prompting for user confirmations and inputs for parameters.")
@click.option('--user-id', default='cli_user', help="User ID for audit logging and personalization.")
@click.pass_context # FIX: Added context pass
def run(ctx, input_path: Path, max_iterations: int, dry_run: bool, output_dir: Path, parallel: int, distributed: bool, interactive: bool, user_id: str):
    ...
    if interactive:
        console.print(Panel("Interactive Mode: Confirm workflow parameters and steps.", title="Workflow Configuration", style="bold green"))
    ...
    async def single_run_task_async(run_id: int):
        ...
        watcher = ConfigWatcher(config_path, lambda new, diff: console.print(f"[yellow]Config reloaded: {json.dumps(diff)}[/yellow]"))
        ...
        try:
            ...
            with console.status(f"[bold blue]Run {run_id}: Performing health check...[/bold blue]"):
                if not engine.health_check():
                    ...
                    return {"status": "failed", "run_id": run_id, "error": "Health check failed"}
            ...
            console.print(f"[bold magenta]Run {run_id}: Output will be saved to: {current_output_path}[/bold magenta]")
            
            # Use Rich Progress for a more granular progress bar within the task
            with Progress(console=console, transient=False, redirect_stdout=False, redirect_stderr=False) as progress:
                task = progress.add_task(f"[cyan]Run {run_id}: Running workflow...", total=max_iterations)
                ...
                try:
                    ...
                    progress.update(task, completed=max_iterations) # Ensure completion if early exit
                    console.print(f"[green]Run {run_id}: Workflow completed successfully! Final Status: {final_status.get('status', 'N/A')}[/green]")
                    logger.info(f"CLI Run {run_id} completed.", status="success", run_id=run_id, output_path=str(current_output_path), user_id=user_id)
                    return {"status": "completed", "run_id": run_id, "output_path": str(current_output_path), "final_status": final_status}
                except Exception as e:
                    progress.update(task, completed=max_iterations, description=f"[red]Run {run_id}: Workflow failed[/red]")
                    logger.error(f"CLI Run {run_id}: Workflow execution failed: {e}", exc_info=True, run_id=run_id, user_id=user_id)
    ...
    try:
        if parallel > 1:
            console.print(f"[bold yellow]Running {parallel} workflows in parallel using multiprocessing...[/bold yellow]")
            ...
            try:
                with ProcessPoolExecutor(max_workers=parallel) as executor:
                    futures = [loop.run_in_executor(executor, sync_worker_wrapper, i) for i in range(parallel)]
                    results = loop.run_until_complete(asyncio.gather(*futures))
                console.print(Panel(f"[green]Parallel runs completed. Results:[/green]\n{json.dumps(results, indent=2)}", title="Parallel Run Summary"))
            finally:
                loop.close()
        else:
...
def status():
    ...
    console.print(Panel(json.dumps(metrics, indent=2), title="Application Metrics", style="bold blue"))
...
@click.option('--query', default='', help="Search query to filter logs. (e.g., 'error', 'workflow_id:xyz')")
@click.option('--limit', type=int, default=10, help="Maximum number of log entries to display.")
@click.option('--follow', is_flag=True, help="Continuously stream new log entries (like `tail -f`).")
def logs(query, limit, follow):
    if follow:
        ...
        last_log_time = datetime.datetime.min.replace(tzinfo=datetime.timezone.utc) # Track last log time to avoid duplicates
        while True:
            # Fetch logs with a slightly higher limit to ensure we catch all new ones since last check
            new_logs_raw = search_logs(query, limit=limit*2) # Fetch more, then filter by timestamp
            
            # Sort logs by timestamp to process oldest new logs first
            new_logs_raw.sort(key=lambda x: datetime.datetime.fromisoformat(x.get('timestamp').replace('Z', '+00:00')) if x.get('timestamp') else datetime.datetime.min.replace(tzinfo=datetime.timezone.utc))

            for log_entry in new_logs_raw:
                log_time_str = log_entry.get('timestamp') # Assuming timestamp key in ISO format
                if log_time_str:
                    log_time = datetime.datetime.fromisoformat(log_time_str.replace('Z', '+00:00')) # Handle Z for UTC
                    if log_time > last_log_time:
                        console.print(Panel(json.dumps(log_entry, indent=2), style="cyan", title=f"Log Entry", expand=False))
        ...
        if not results:
            ...
            return
        for log in results:
            console.print(Panel(json.dumps(log, indent=2), style="cyan", title=f"Log Entry", expand=False))

@cli.command(help="Submit feedback to improve the generator.")
@click.option('--message', prompt="Your feedback message", help="The feedback message you want to submit.")
@click.option('--rating', type=click.FloatRange(0, 1), default=None, help="Optional: A rating from 0.0 to 1.0 for the last workflow run.")
@click.option('--run-id', default=None, help="Optional: ID of the specific run this feedback is for.")
@click.option('--user-id', default='cli_user', help="User ID for audit logging and personalization.")
@click.option('--api-endpoint', default='http://127.0.0.1:8000/api/v1/feedback', help="API endpoint for feedback submission.")
@click.pass_context # FIX: Added context pass
async def feedback(ctx, message, rating, run_id, user_id, api_endpoint):
    ...
    redacted_feedback = redact_secrets(copy.deepcopy(feedback_data)) # Redact before logging locally
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.post(api_endpoint, json=feedback_data, headers=headers, timeout=5)
            response.raise_for_status() # Raise an exception for HTTP errors (4xx or 5xx)
            
            response_json = await response.json()
            console.print(f"[green]Feedback submitted successfully! API Response: {response_json.get('message', 'N/A')}[/green]")
            logger.info("Feedback sent successfully via CLI API.", extra={"api_response": response_json, "user_id": user_id})
        ...
        if rating is not None:
            ...
            console.print(f"[green]Feedback rating {rating} locally applied to engine tuning.[/green]")
        ...
        logger.error(f"Feedback API returned invalid JSON.", exc_info=True, user_id=user_id)
    except Exception as e:
        console.print(f"[red]An unexpected error occurred during feedback submission: {e}[/red]")
        logger.critical(f"Unexpected error in CLI feedback command: {e}", exc_info=True, user_id=user_id)
...
def config_show(ctx, raw):
    ...
    if raw:
        console.print(Panel(yaml_content, title="Current Configuration (Raw)", style="bold magenta"))
    else:
...
@click.option('--value', help="The new value for the configuration key. (Optional, will prompt if not provided).")
        ...
        with open(lock_file, 'x') as f: # Use 'x' mode to create exclusively
            ...
            for i, k in enumerate(keys):
                if not isinstance(current_level, dict):
                    console.print(f"[red]Error: Key '{key}' invalid. '{'.'.join(keys[:i])}' is not a dictionary.[/red]")
                    logger.error(f"Config edit failed: Invalid path, non-dict element encountered for key '{key}' by user '{user_id}'.")
                    return
                if k not in current_level:
                    console.print(f"[red]Error: Key '{key}' not found. Path segment '{k}' missing.[/red]")
            ...
            if value is None: # If value not provided as CLI arg, prompt interactively
                value_str = Prompt.ask(f"Enter new value for '{key}' (current: [yellow]{repr(old_value)}[/yellow])")
            else:
                value_str = value
            ...
            try:
                ...
                if isinstance(old_value, bool):
                    if value_str.lower() in ('true', '1', 'yes'): new_value = True
                    elif value_str.lower() in ('false', '0', 'no'): new_value = False
                    else: raise ValueError(f"Cannot convert '{value_str}' to boolean.")
                elif isinstance(old_value, int): new_value = int(value_str)
                elif isinstance(old_value, float): new_value = float(value_str)
                elif isinstance(old_value, list): new_value = json.loads(value_str) if value_str.startswith('[') else [item.strip() for item in value_str.split(',')]
                elif isinstance(old_value, dict): new_value = json.loads(value_str)
            except ValueError:
                console.print(f"[yellow]Warning: Could not auto-cast '{value_str}' to original type '{type(old_value).__name__}'. Storing as string.[/yellow]")
            except json.JSONDecodeError:
                console.print(f"[yellow]Warning: Could not parse '{value_str}' as JSON for dict/list. Storing as string.[/yellow]")
            ...
            log_action("Config Edited", category="config", key=key, old_value=repr(old_value), new_value=repr(new_value), user_id=user_id)
            console.print(f"[green]Config key '[bold]{key}[/bold]' updated from '[dim]{repr(old_value)}[/dim]' to '[bold]{repr(new_value)}[/bold]'.[/green]")
            console.print("[yellow]Remember to run 'config reload' for changes to take effect if the app is already running.[/yellow]")

    except FileExistsError:
        console.print(f"[red]Error: Could not acquire config lock. Another process might be editing the config. Lock file exists: {lock_file}[/red]")
        logger.error(f"Config edit failed: Lock already exists for user '{user_id}'.", exc_info=True)
    except Exception as e:
        console.print(f"[red]An unexpected error occurred during config edit: {e}[/red]")
        logger.critical(f"Unexpected error in config edit command: {e}", exc_info=True, user_id=user_id)
...
@click.option('--api-endpoint', default='http://127.0.0.1:8000/api/v1/parse/reload_config', help="API endpoint to trigger config reload.")
...
async def config_reload(api_endpoint, user_id):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            response_json = await response.json()
            console.print(f"[green]Configuration reload triggered successfully via API. Response: {response_json.get('message', 'N/A')}[/green]")
            log_action("Config Reload Triggered", category="config", source="cli", api_response=response_json, user_id=user_id)
    except aiohttp.ClientError as e:
        console.print(f"[red]Error triggering config reload via API: Network or HTTP error: {e}[/red]")
        logger.error(f"Config reload CLI command failed via API: {e}", exc_info=True, user_id=user_id)
        await send_alert(f"CLI config reload failed to API: {e}", severity="critical")
    except json.JSONDecodeError:
        response_text = await response.text()
        console.print(f"[red]Error: API returned invalid JSON during config reload. Response: {response_text}[/red]")
        logger.error(f"Config reload API returned invalid JSON.", exc_info=True, user_id=user_id)
    except Exception as e:
        console.print(f"[red]An unexpected error occurred during config reload: {e}[/red]")
        logger.critical(f"Unexpected error in config reload CLI command: {e}", exc_info=True, user_id=user_id)
...
def config_audit(limit, user_id):
    ...
    for log_entry in audit_logs:
        ...
        panel_content = f"Action: {log_entry.get('action_type', 'N/A')}\n" \
...
async def health(ctx):
    ...
    console.print(f"[{'green' if overall_status else 'red'}]Overall Health: {'OK' if overall_status else 'FAIL'}[/{'green' if overall_status else 'red'}]")
...
@click.option('--verify-signature', is_flag=True, help="Verify digital signature of the plugin package (conceptual).")
async def plugin_install(plugin_identifier, verify_signature):
    ...
    try:
        if verify_signature:
            console.print("[red]Signature verification is conceptual and not fully implemented.[/red]")
        ...
        if Path(plugin_identifier).suffix == '.py' and Path(plugin_identifier).exists():
            ...
            spec = importlib.util.spec_from_file_location(Path(plugin_identifier).stem, plugin_identifier)
        ...
        if hasattr(module, 'register') and callable(module.register):
            ...
            console.print(f"[yellow]Module '{plugin_identifier}' loaded, but no 'register()' function found. Cannot install.[/yellow]")
            log_action("Plugin Install Failed", category="plugin", plugin_name=plugin_identifier, reason="No register() function")
            await send_alert(f"Plugin install failed: '{plugin_identifier}' no register() found", severity="medium")
    except ImportError as e:
        console.print(f"[red]Error: Plugin module '{plugin_identifier}' not found. Ensure it's in PYTHONPATH or correctly named: {e}[/red]")
        logger.error(f"Plugin installation failed: {e}", exc_info=True)
        log_action("Plugin Install Failed", category="plugin", plugin_name=plugin_identifier, reason=f"Module not found: {e}")
        await send_alert(f"Plugin install failed: '{plugin_identifier}' module not found", severity="critical")
    except Exception as e:
        console.print(f"[red]Error installing plugin '{plugin_identifier}': {e}[/red]")
        logger.error(f"Plugin installation failed: {e}", exc_info=True)
        log_action("Plugin Install Failed", category="plugin", plugin_name=plugin_identifier, reason=str(e))
...
def plugin_uninstall(plugin_name):
    ...
    if plugin_name in AGENT_REGISTRY: # Simple deregistration from AGENT_REGISTRY
        ...
        console.print(f"[red]Agent/Plugin '{plugin_name}' not found in registry. Cannot uninstall.[/red]")
        log_action("Plugin Uninstall Failed", category="plugin", plugin_name=plugin_name, reason="Not found in registry")
        asyncio.run(send_alert(f"Plugin uninstall failed: '{plugin_name}' not found", severity="medium"))
...
@click.option('--output-format', type=click.Choice(['markdown', 'html', 'pdf']), default='html', help="Format for the generated documentation.")
@click.option('--output-dir', default='docs_output', type=click.Path(path_type=Path), help="Directory to save the generated documentation.")
@click.option('--source-dir', default='src', type=click.Path(exists=True, readable=True, path_type=Path), help="Source directory for documentation content (e.g., code, Markdown files).")
@click.option('--user-id', default='cli_user', help="User ID for audit logging.")
async def docs_generate(output_format: str, output_dir: Path, source_dir: Path, user_id: str):
    ...
    try:
        ...
        logger.error(f"Doc gen failed: Output directory creation error: {e}", exc_info=True, user_id=user_id)
        await send_alert(f"CLI doc gen failed: Output directory not writable", severity="critical")
        sys.exit(1)

    console.print(f"[bold blue]Generating documentation in '{output_format}' format to '{output_path}' from '{source_dir}'...[/bold blue]")
    ...
    with Progress(console=console, transient=False) as progress:
        ...
        try:
            ...
            dummy_doc_file.write_text(f"<h1>Project Documentation ({output_format})</h1>\n\nThis is auto-generated content from source: {source_dir}", encoding='utf-8')
            
            generated_files = [dummy_doc_file] # List of generated files

            console.print(f"[green]Documentation generated successfully: {', '.join(str(f) for f in generated_files)}[/green]")
            logger.info(f"Documentation generated: format={output_format}, path={str(output_path)}, user_id={user_id}", category="docs")
            if output_format == 'html' and generated_files:
                console.print(f"[yellow]You can view it by opening: file://{generated_files[0].resolve()}[/yellow]")
...
@click.argument('path', type=click.Path(exists=True, dir_okay=False, readable=True, path_type=Path)) # FIX: Removed conflicting 'help' keyword
def docs_view(path: Path):
    ...
    if doc_path.suffix.lower() in ['.html', '.pdf']:
    ...
    elif doc_path.suffix.lower() in ['.md', '.txt', '.rst', '.yaml', '.json']: # Also display other text formats
        try:
            content = doc_path.read_text(encoding='utf-8')
            # Use Syntax highlighting for code/config files
            if doc_path.suffix.lower() in ['.py', '.js', '.json', 'yaml', '.yml']:
                syntax = Syntax(content, doc_path.suffix.lower().strip('.'), theme="monokai", line_numbers=True)
        ...
        console.print(f"[yellow]Unsupported format for direct viewing: {doc_path.suffix}. Displaying raw content.[/yellow]")
        try:
            content = doc_path.read_text(encoding='utf-8')
...
@cli.command(name="shell", help="Enter an interactive shell mode for continuous CLI command input.")
def shell_mode():
    console.print(Panel("[bold green]Entering interactive shell mode. Type 'exit' or 'quit' to leave. This shell only executes CLI commands, not arbitrary system commands.[/bold green]", style="green"))
    ...
    while True:
        try:
            ...
            try:
                ...
                cli.main(args=args, standalone_mode=False) # standalone_mode=False prevents sys.exit
...
def suggest_recovery_cli(error: Optional[Exception] = None):
    ...
    if error:
        ...
        if "FileNotFound" in error_type:
            suggestions.insert(0, "[bold red]File not found error:[/bold red] Double-check the input path or config file path.")
        elif "Permission" in error_type or "OSError" in error_type: # Catch general OS errors for permissions/disk issues
            suggestions.insert(0, "[bold red]Permission/OS error:[/bold red] Ensure the CLI has necessary read/write permissions for specified directories, or check disk space/integrity.")
        elif "yaml" in str(error).lower() or "Config" in error_type: # More specific for config errors
            suggestions.insert(0, "[bold red]Configuration error:[/bold red] Review 'config.yaml' for syntax mistakes (e.g., indentation, missing colons). Use an online YAML validator if unsure.")
        elif "HTTPException" in error_type or "ClientError" in error_type: # For API related errors
            suggestions.insert(0, "[bold red]API communication error:[/bold red] Verify the API service is running and accessible at the configured endpoint (e.g., 'http://127.0.0.1:8000').")
