...
def _ensure_initial_migration(
    ...
    model modules discovered in ``app/models/`` (using explicit module-level
    imports for their side-effect of registering models on ``Base.metadata``)
    and provides a functional ``upgrade()`` / ``downgrade()`` pair.

    The file is only created when no ``.py`` migration files already exist in
    the versions directory, so any richer LLM-generated migrations are never
    overwritten.
    ...
    # Discover model modules from app/models/ to import for their side-effect
    # of registering ORM classes on Base.metadata.  Explicit module imports are
    # preferred over wildcard ``import *`` for clarity and import hygiene.
    ...
    if models_dir.is_dir():
        for model_file in sorted(models_dir.glob("*.py")):
            ...
            # Import the module for its side-effect; the noqa suppresses the
            # "imported but unused" warning from flake8/ruff.
            model_import_lines.append(f"    import app.models.{stem}  # noqa: F401")

    model_imports_block = (
        "\n".join(model_import_lines)
        if model_import_lines
        else "    pass  # No model files found in app/models/ — add imports here"
    )
    ...
    migration_content = (
        ...
        "import sqlalchemy as sa\n\n"
        "# ---------------------------------------------------------------------------\n"
        "# Import all ORM model modules so that Base.metadata is fully populated.\n"
        "# Each import is for its side-effect only (model classes register themselves\n"
        "# on the shared DeclarativeBase when the module is loaded).\n"
        "# ---------------------------------------------------------------------------\n"
        "try:\n"
        "    from app.database import Base  # noqa: F401\n"
        f"{model_imports_block}\n"
        "except ImportError:\n"
        ...
        "        # In CI/CD without a DB connection this block is a safe no-op.\n"
        "        bind = op.get_bind()\n"
