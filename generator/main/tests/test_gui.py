...
async def mock_dependencies(): # <<< FIX: Made fixture async
    ...
    with patch('main.gui.Runner') as mock_runner, \
...
class TestTuiLogHandler:
    ...
    async def test_emit_log_record(self, log_widget, mock_app):
        ...
        record = logging.LogRecord(
            ...
            msg="Test log", args=(), exc_info=None
        )
...
class TestAPIInteraction:
    ...
    async def test_make_api_request_error_handling(self, app_instance):
        ...
        with patch('main.gui.aiohttp.ClientSession') as MockSession:
            ...
            mock_request_context = AsyncMock()
            mock_request_context.__aenter__ = AsyncMock(side_effect=aiohttp.ClientError("Network error"))
            mock_request_context.__aexit__ = AsyncMock()
...
class TestRunnerTab:
    ...
    async def test_submit_runner_input_invalid_json(self, app_instance):
        ...
        with patch.object(app_instance.runner_log, 'write') as mock_log_write, \
             patch.object(app_instance, '_set_error_message', new_callable=AsyncMock) as mock_set_error:
            ...
            mock_set_error.assert_called_with(app_instance.runner_error, pytest.string_containing("Invalid JSON payload"))
...
class TestMetricsTab:
    ...
    async def test_update_metrics_display_api(self, app_instance):
        ...
        with patch.object(app_instance, '_make_api_request') as mock_api:
            ...
            mock_api.side_effect = [
                {"cpu_usage": 45.2, "memory_usage": 62.1, "active_tasks": 3}, # Metrics call
                {"version": "1.2.3"} # Version call
            ]
            
            with patch.object(app_instance.metrics_display, 'update') as mock_display_update, \
                 patch.object(app_instance.metrics_display_api_version, 'update') as mock_version_update, \
                 patch.object(app_instance, '_set_error_message', new_callable=AsyncMock), \
                 patch.object(app_instance, '_set_success_message', new_callable=AsyncMock):
    ...
    async def test_update_metrics_local(self, app_instance):
        ...
        with patch('main.gui.RUN_QUEUE.get_size', return_value=5) as mock_q, \
    ...
    async def test_metrics_refresh_interval_change(self, app_instance):
        ...
        with patch.object(app_instance, 'set_interval') as mock_interval, \
             patch.object(app_instance, '_set_success_message', new_callable=AsyncMock) as mock_success:
...
class TestConfigReload:
    ...
    async def test_reload_parser_config(self, app_instance):
        ...
        with patch.object(app_instance, '_trigger_backend_config_reload', new_callable=AsyncMock) as mock_trigger, \
             patch.object(app_instance.runner_log, 'write') as mock_log_write, \
             patch.object(app_instance, '_set_success_message', new_callable=AsyncMock):
...
class TestKeyBindings:
    ...
    async def app_instance(self, mock_dependencies):
        """Create MainApp instance."""
        from main.gui import MainApp, Input
        app = MainApp()
        async with app.run_test() as pilot:
            await asyncio.sleep(0.01) # Allow on_mount to complete
            yield app
