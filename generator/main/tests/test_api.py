...
from fastapi import HTTPException, status, WebSocketDisconnect, Request # ADDED WebSocketDisconnect, Request as per Steps 4 & 5
...
def mock_dependencies():
    ...
    with patch('main.api.Runner') as mock_runner, \
         patch('main.api.IntentParser') as mock_parser, \
         patch('main.api.get_metrics_dict') as mock_metrics, \
         patch('main.api.search_logs') as mock_search_logs: # <<< FIX: Added patch for search_logs
...
def valid_jwt_token(test_user_credentials):
    ...
    from main.api import SECRET_KEY, ALGORITHM, create_access_token
    # Need to create the user first so login works
    token_data = {
        "sub": test_user_credentials["username"],
        "scopes": ["user", "run", "parse", "feedback", "logs"] # Give all scopes for testing
    }
...
class TestConcurrency:
    ...
    async def test_concurrent_requests(self, test_app):
        ...
        async with httpx.AsyncClient(transport=ASGITransport(app=api), base_url="http://test") as aclient:
