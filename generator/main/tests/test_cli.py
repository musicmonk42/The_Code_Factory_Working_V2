...
import json
import time
import gc # FIX: Added for GC fixture
...
def mock_dependencies():
    ...
    with patch('main.cli.WorkflowEngine') as mock_engine, \
...
class TestPluginCommands:
    ...
    def test_plugin_load_command(self, cli_runner, mock_dependencies):
        ...
        result = cli_runner.invoke(cli, [
            'plugin', 'install', # Command is 'install' not 'load'
            '/fake/path/plugin.py'
        ])
