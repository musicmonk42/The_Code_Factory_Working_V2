...
class TestEndToEndWorkflow:
    ...
    async def test_complete_generation_workflow(self, test_environment):
        ...
        with patch('main.main.Runner') as MockRunner, \
    ...
    async def test_cli_to_api_workflow(self, test_environment):
        ...
        with patch('main.cli.aiohttp.ClientSession') as MockSession:
            ...
            result = runner.invoke(cli, [
                'run',
                '--input', str(test_environment['readme']),
                '--dry-run'
            ])
...
class TestGUIIntegration:
    ...
    async def test_gui_config_reload(self, test_environment):
        ...
        with patch.object(app, '_trigger_backend_config_reload', new_callable=AsyncMock) as mock_reload:
...
class TestCLIIntegration:
    ...
    def test_cli_config_integration(self, test_environment):
        ...
        # Show config
        result_show = runner.invoke(cli, [
            'config', 'show',
            '--config-file', str(test_environment['runner_config'])
        ])
        ...
        assert result_show.exit_code in [0, 1, 2] or result_validate.exit_code in [0, 1, 2]
...
class TestConfigurationPropagation:
    ...
    async def test_config_reload_propagates(self, test_environment):
        ...
        with patch('main.main.ConfigWatcher') as MockWatcher, \
...
class TestSecurityIntegration:
    ...
    def test_jwt_security(self):
        ...
        from main.api import create_access_token, verify_token, SECRET_KEY
        import jwt
...
class TestRecoveryMechanisms:
    ...
    async def test_api_recovery_after_error(self):
        ...
        assert bad_response.status_code != 500 or good_response.status_code in [200, 404]
