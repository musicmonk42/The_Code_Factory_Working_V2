...
try:
    ...
    from textual.widgets import Header, Footer, RichLog, DataTable, Button, Input, TextArea, Label, ProgressBar, TabbedContent, TabPane, Static, Markdown, Select # Keep Select for interval
    from textual.events import Mount
    from textual.css.query import NoMatches
    _TEXTUAL_AVAILABLE = True
except ImportError:
    logging.getLogger(__name__).warning("Textual, aiohttp, or dependencies not found. GUI cannot run.")
    # Define dummy classes to allow file import
    class App:
        def __init__(self, *args, **kwargs): pass
        def run(self, *args, **kwargs): pass
        def query_one(self, *args, **kwargs): return self
        def focus(self, *args, **kwargs): pass
        def push_screen(self, *args, **kwargs): pass
        def pop_screen(self, *args, **kwargs): pass
        def set_interval(self, *args, **kwargs): return self
        ...
        @classes.setter
        def classes(self, val): pass
        @property
        def visible(self): return False
        @visible.setter
        def visible(self, val): pass
        @property
        def cursor_row(self): return 0
        @cursor_row.setter
        def cursor_row(self, val): pass
        @property
        def row_count(self): return 0
        @property
        def title(self): return ""
        @title.setter
        def title(self, val): pass
        @property
        def sub_title(self): return ""
        @sub_title.setter
        def sub_title(self, val): pass
    ...
    class Grid(Container): pass
    class Header: pass
    class Footer: pass
    class RichLog:
        def __init__(self, *args, **kwargs): pass
        def write(self, *args, **kwargs): pass # Note: This is NOT async
    class DataTable:
        def __init__(self, *args, **kwargs): pass
        def add_columns(self, *args, **kwargs): pass
        def clear(self, *args, **kwargs): pass
        ...
        @property
        def row_count(self): return 0
        def get_row_at(self, *args, **kwargs): return ["dummy_id"]
        def update_cell_at(self, *args, **kwargs): pass
    class Button: pass
    class Input:
        def __init__(self, *args, **kwargs): pass
...
try:
    ...
    from runner.runner_metrics import (
        get_metrics_dict, RUN_QUEUE, RUN_PASS_RATE, RUN_RESOURCE_USAGE, HEALTH_STATUS
    )
    from intent_parser.intent_parser import IntentParser
    _RUNNER_AVAILABLE = True
except ImportError:
    logging.getLogger(__name__).warning("Runner or IntentParser modules not found. GUI logic will be dummied.")
    _RUNNER_AVAILABLE = False
    # Dummy runner components
    class DummyRunner: pass
    class DummyConfigWatcher:
        def __init__(self, *args, **kwargs): pass
        async def start(self): pass
        def stop(self): pass
        def _reload(self, *args, **kwargs): pass
    class DummyIntentParser: pass
    def load_config(*args, **kwargs): return {}
    def get_metrics_dict(): return {"dummy_metric": 0}
    class DummyMetric:
        def get(self): return "N/A"
        def get_size(self): return 0
    RUN_QUEUE, RUN_PASS_RATE, RUN_RESOURCE_USAGE, HEALTH_STATUS = (DummyMetric(), DummyMetric(), DummyMetric(), DummyMetric())
...
class MainApp(App):
    ...
    async def on_mount(self) -> None:
        ...
        try:
            ...
            self.metrics_display_api_version = self.query_one("#metrics_display_api_version", Static)
            # Feedback inputs
        ...
        self.metrics_update_interval_task = self.set_interval(5, self._update_metrics) # Store interval task
    ...
    async def _on_config_reload(self, new_config, diff):
        ...
        if self.runner_log:
            await self.runner_log.write(_(f"[yellow]CLI/TUI config reloaded. Diff: {json.dumps(diff)}[/yellow]"))
    ...
    async def _on_parser_config_reload(self, new_config, diff):
        ...
        try:
            await self._trigger_backend_config_reload("parser", API_ENDPOINTS["parser_reload_config"])
            if self.runner_log: # Log to main log
                self.runner_log.write(_("[green]Intent Parser config reload triggered via API.[/green]"))
        except Exception as e:
            if self.runner_log: # Log to main log
                self.runner_log.write(_(f"[red]Error triggering parser config reload via API: {e}[/red]"))
            if self.parser_error:
                await self._set_error_message(self.parser_error, f"Parser config reload failed: {e}")

    async def _set_error_message(self, label_widget: Label, message: str, clear_after: Optional[float] = 5):
    ...
    async def _set_success_message(self, label_widget: Label, message: str, clear_after: Optional[float] = 3):
    ...
    async def _make_api_request(self, method: str, url: str, json_data: Optional[Dict] = None, files: Optional[Dict] = None, timeout: int = 30) -> Dict:
        """Centralized helper to make authenticated API requests."""
        if not _TEXTUAL_AVAILABLE: # aiohttp is guarded with textual
             raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="aiohttp not available")
             
        if not GENERATOR_API_KEY:
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="API Key not configured. Please set GENERATOR_API_KEY env var.")
        ...
        try:
            async with aiohttp.ClientSession(headers=headers) as session:
                if files: # For multipart form data (file uploads)
                    ...
                    for field_name, (filename, file_content, content_type) in files.items():
                        ...
                        data.add_field(field_name, file_content, filename=filename, content_type=content_type)

                    async with session.request(method.upper(), url, data=data, timeout=timeout) as response:
                        response.raise_for_status() # Raises for 4xx/5xx responses
                        return await response.json()
                else: # For JSON payloads
                    async with session.request(method.upper(), url, json=json_data, timeout=timeout) as response:
            ...
            if self.runner_error:
                ...
                self.call_later(self._set_error_message, self.runner_error, f"API request failed: {e}")
            raise # Re-raise original exception
        except json.JSONDecodeError as e:
            app_logger.error(f"API returned invalid JSON from {url}: {e}", exc_info=True)
            if self.runner_error:
                 self.call_later(self._set_error_message, self.runner_error, f"API response invalid: {e}")
            raise # Re-raise original exception
        except Exception as e:
            app_logger.critical(f"Unexpected error during API request to {url}: {e}", exc_info=True)
            if self.runner_error:
                self.call_later(self._set_error_message, self.runner_error, f"Unexpected API error: {e}")
            raise # Re-raise original exception
    ...
    async def _trigger_backend_config_reload(self, config_type: str, api_url: str):
        ...
        if self.runner_log:
            await self.runner_log.write(f"[yellow]Triggering {config_type} config reload on backend API...[/yellow]")
        try:
        ...
        with TabbedContent(initial="runner-tab"):
            with TabPane(_("Runner"), id="runner-tab"):
                yield Vertical(
                    ...
                    Horizontal(
                        ...
                        Button(_("Reload CLI/TUI Config"), id="reload-runner-config", variant="default"),
                        Button(_("Submit Feedback"), id="submit-runner-feedback-button", variant="default"),
                    ),
                    Label(id="runner_error"),
                    Vertical( # For feedback inputs
                        Label(_("[b]Submit Feedback[/b]")),
                        Input(placeholder=_("Run ID (optional)"), id="feedback_input_run_id"),
                        Input(placeholder=_("Rating (1-5)"), id="feedback_input_rating"),
                        TextArea(placeholder=_("Comments (optional)"), id="feedback_input_comments", classes="feedback-comments"), # Added class for potential styling
            ...
            with TabPane(_("Intent Parser"), id="parser-tab"):
                yield Vertical(
                    ...
                    Input(placeholder=_("Enter text or /path/to/file..."), id="intent_parser_input"),
                    Horizontal(
                        Button(_("Parse Text/File"), id="parse-button", variant="primary"),
                        Button(_("Reload Parser Config"), id="reload-parser-config", variant="default"),
                    ),
            ...
            with TabPane(_("Clarifier"), id="clarifier-tab"):
                yield Vertical(
                    ...
                    Input(placeholder=_("Enter clarification response..."), id="clarifier_input"),
                    Button(_("Submit Clarification"), id="clarify-button", variant="primary"),
            ...
            with TabPane(_("Metrics"), id="metrics-tab"):
                yield Vertical(
                    ...
                    Horizontal(
                        Button(_("Refresh Metrics"), id="refresh-metrics-button", variant="default"),
                        Select([(f"{s}s", s) for s in [5, 10, 30, 60]], value=5, id="metrics_refresh_interval")
                    ),
    ...
    def action_focus_runner(self) -> None:
        try:
            ...
            if self.runner_error:
                self.call_later(self._set_error_message, self.runner_error, f"Failed to focus input: {e}")
    ...
    def action_focus_parser(self) -> None:
        try:
            ...
            if self.parser_error:
                self.call_later(self._set_error_message, self.parser_error, f"Failed to focus input: {e}")
    ...
    def action_focus_clarifier(self) -> None:
        try:
            ...
            if self.clarifier_error:
                self.call_later(self._set_error_message, self.clarifier_error, f"Failed to focus input: {e}")
    ...
    async def run_workflow(self, payload_str: str):
        ...
        if self.runner_error:
            await self._set_error_message(self.runner_error, "", clear_after=None) # Clear previous errors

        try:
            payload = json.loads(payload_str)
            if self.runner_log:
                self.runner_log.write(f"[blue]Sending run request to API: {API_ENDPOINTS['run']}...[/blue]")
            
            # Call backend API /api/v1/run
            result = await self._make_api_request("POST", API_ENDPOINTS["run"], json_data=payload)

            if self.runner_log:
                self.runner_log.write(f"[green]Run Result from API:\n{json.dumps(result, indent=2)}[/green]")
            if self.runner_error:
                await self._set_success_message(self.runner_error, _("Workflow run submitted successfully!"))
            
            # Update progress based on API response if it provides progress
            if self.runner_progress:
                self.runner_progress.update(progress=100) # Assuming full completion for simple demo
    ...
    async def reload_runner_config_button(self):
        ...
        try:
            ...
            if self.runner_error:
                await self._set_error_message(self.runner_error, f"CLI/TUI config reload failed: {e}")
    ...
    async def submit_runner_feedback_button(self):
        ...
        await self._set_error_message(self.feedback_error, "", clear_after=None) # Clear previous errors
        if self.runner_log:
        ...
        try:
            ...
            result = await self._make_api_request("POST", API_ENDPOINTS["runner_feedback"], json_data=feedback_payload)
            if self.runner_log:
                self.runner_log.write(f"[green]Feedback submitted. API Response: {result.get('message', 'N/A')}[/green]")
            await self._set_success_message(self.feedback_error, _("Feedback submitted successfully!"))
            # Clear input fields on success
            if self.feedback_input_run_id.value == run_id: # Only clear if it was auto-generated or explicitly matches
    ...
    async def run_intent_parser(self, text_input: str):
        ...
        await self._set_error_message(self.parser_error, "", clear_after=None) # Clear previous errors

        if not text_input:
            await self._set_error_message(self.parser_error, _("Input text or file path cannot be empty."))
            return
        ...
        try:
            ...
            if os.path.sep in text_input or (os.path.altsep and os.path.altsep in text_input):
                if not path.exists():
                    ...
                    return
                if not path.is_file():
                    await self._set_error_message(self.parser_error, f"Path is a directory, not a file: {path}")
                    return
                is_file = True
        except (IOError, OSError) as e:
             # Handle invalid path characters (e.g., "a:b")
            app_logger.warning(f"Input '{text_input}' is not a valid path. Treating as text. Error: {e}")
            is_file = False
        ...
        try:
            if is_file:
                ...
                result = await self._make_api_request("POST", API_ENDPOINTS["parse_file"], json_data=form_data, files=files)
            else:
                # For text parsing, use JSON payload
                json_data = {"content": text_input, "format_hint": None, "dry_run": False}
                result = await self._make_api_request("POST", API_ENDPOINTS["parse_text"], json_data=json_data)

            if self.runner_log: # Log to main log
                self.runner_log.write(f"[green]Parsed Result from API:\n{json.dumps(result, indent=2)}[/green]")
            ...
            if 'ambiguities' in result and result['ambiguities']:
                ...
                for q in result['ambiguities']:
                    ...
                    self.clarifier_table.add_row(question_id, q.get('question', 'N/A'), "Pending", key=question_id)
                
                if self.clarifier_table.row_count > 0:
                    self.clarifier_table.cursor_row = 0 # Focus first question
                if self.runner_log:
                    self.runner_log.write(_("[yellow]Ambiguities detected! Check Clarifier tab.[/yellow]"))
                self.query_one(TabbedContent).active = "clarifier-tab" # Auto-switch to clarifier tab
        except (HTTPException, aiohttp.ClientError) as e: # Catch API errors
            detail = getattr(e, 'detail', str(e))
            if self.runner_log:
    ...
    async def reload_parser_config_button(self):
        ...
        try:
            if self.parser_config_watcher:
                self.parser_config_watcher._reload(force=True) # Force immediate reload of local config
            if self.runner_log:
                self.runner_log.write(f"\n[green]CLI/TUI parser config reloaded from {PARSER_CONFIG_PATH}.[/green]")
            await self._trigger_backend_config_reload("parser", API_ENDPOINTS["parser_reload_config"])
            await self._set_success_message(self.parser_error, _("Parser config reloaded and API triggered!"))
        except Exception as e:
            if self.runner_log:
    ...
    async def submit_clarification(self, response: str):
        ...
        await self._set_error_message(self.clarifier_error, "", clear_after=None) # Clear previous errors

        try:
            selected_row_index = self.clarifier_table.cursor_row
            if selected_row_index is None or self.clarifier_table.row_count == 0: # Handle no selection or empty table
                await self._set_error_message(self.clarifier_error, _("No question selected or no questions available to clarify."))
                return
        except NoMatches:
        ...
        try:
            ...
            rating_for_api = 1.0 if response.lower() == 'yes' else (0.0 if response.lower() == 'no' else 0.5)

            result = await self._make_api_request("POST", api_url, json_data={"rating": rating_for_api})

            if self.runner_log:
                self.runner_log.write(f"[green]Clarification submitted. API Response: {result.get('message', 'N/A')}[/green]")
            self.clarifier_table.update_cell_at((selected_row_index, 2), "Resolved [green]✓[/green]")
            await self._set_success_message(self.clarifier_error, _(f"Clarified question {question_id} successfully!"))
    ...
    async def on_metrics_refresh_interval_changed(self, event: Select.Changed):
        ...
        try:
            ...
            self.metrics_update_interval_task = self.set_interval(interval_seconds, self._update_metrics) # Set new interval
            await self._set_success_message(self.metrics_error, _(f"Metrics refresh interval set to {interval_seconds}s!"), clear_after=2)
    ...
    async def update_metrics_display(self):
        ...
        await self._set_error_message(self.metrics_error, "", clear_after=None) # Clear previous errors
        try:
            ...
            if self.metrics_display_api_version: # Check if widget exists
                self.metrics_display_api_version.update(_(f"API Version: [green]{api_version_str}[/green]"))
                ...
                for key, value in metrics_data.items():
                    if isinstance(value, dict): # For nested metrics
                        ...
                        for sub_key, sub_value in value.items():
                            display_text_parts.append(f"  - {sub_key}: [green]{sub_value}[/green]\n")
                    else:
            ...
            if self.metrics_display: # Check if widget exists
                ...
                if isinstance(local_metrics_text, str):
                     self.metrics_display.update(local_metrics_text + "\n" + "".join(display_text_parts))
                else:
            ...
            await self._set_error_message(self.metrics_error, f"Failed to load API metrics: {detail}")
