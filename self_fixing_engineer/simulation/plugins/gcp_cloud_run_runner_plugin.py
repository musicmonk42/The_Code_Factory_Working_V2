...
try:
    ...
    from google.api_core.exceptions import GoogleAPIError, NotFound, Conflict, QuotaExceeded, InvalidArgument
...
if os.path.exists(CONFIG_FILE):
    ...
    PLUGIN_MANIFEST = {
        ...
        "capabilities": ["gcp_cloud_run_execution", "cloud_bursting", "distributed_simulation"],
        "permissions_required": ["gcp_cloud_run_invoke", "gcp_gcs_read", "gcp_gcs_write", "gcp_logging_read"],
        "compatibility": {
...
JOB_SUBMISSIONS_TOTAL = _safe_counter(
    'gcp_cloud_run_job_submissions_total', 'Total Cloud Run job submissions', ('status',)
)
...
class JobConfig(BaseModel):
    ...
    job_definition_name: Optional[str] = Field(None, description="Full resource name of existing Cloud Run job")
    image_url: str = Field(..., description="Docker image URL (e.g., gcr.io/my-project/image:latest)")
    ...
    env_vars: List[EnvVar] = Field(default_factory=list, description="Environment variables for the container")
    ...
    def validate_image_url(cls, v):
        ...
        if not image_re.match(v):
            raise ValueError('Image URL must be from a trusted GCP registry (gcr.io or *-docker.pkg.dev)')
        return v
...
async def _load_credentials_from_vault() -> Optional['service_account.Credentials']:
    ...
    if not vault_url.lower().startswith("https://"):
        logger.warning("VAULT_URL is not HTTPS. Refusing to fetch credentials over insecure channel.")
        return None
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        try:
            ...
            async with session.get(f"{vault_url.rstrip('/')}/v1/secret/data/gcp-credentials", headers=headers) as response:
                ...
                if creds_path and os.path.exists(creds_path):
                    ...
                    CREDENTIAL_SOURCE_TOTAL.labels(source='vault').inc()
                    return creds
                logger.warning("Vault response did not include usable credentials (sa_json or credentials_path).")
...
async def plugin_health() -> Dict[str, Any]:
    ...
    if not GCP_AVAILABLE:
        status = "error"
        details.append("Google Cloud client libraries not found. GCP Cloud Run execution is impossible.")
    ...
    if not project_id or not location:
        status = "degraded"
        details.append("GCP_PROJECT_ID or GCP_LOCATION not set; limited health checks will be performed.")
...
def _tar_directory_to_temp(project_root: str) -> str:
    ...
    excludes = set(
        os.getenv("GCR_ARCHIVE_EXCLUDES", ".git,.venv,venv,node_modules,__pycache__,.mypy_cache,.pytest_cache,.tox,.DS_Store").split(",")
    )
...
async def run_cloud_run_job(
    ...
    **kwargs
    ...
    gcs_client = storage.Client(credentials=credentials)
    jobs_client = run_v2.JobsClient(credentials=credentials)
    exec_client = run_v2.ExecutionsClient(credentials=credentials) if hasattr(run_v2, "ExecutionsClient") else jobs_client
    ...
    try:
        ...
        @retry(stop=stop_after_attempt(5), wait=wait_exponential(min=2, max=10), retry=retry_if_exception_type(GoogleAPIError))
        ...
        job_name_to_use = config.job_definition_name or f"projects/{config.project_id}/locations/{config.location}/jobs/{archive_base_name}"
        ...
        env_list = [run_v2.EnvVar(name=ev.name, value=ev.value) for ev in config.env_vars] + [
            run_v2.EnvVar(name="SFE_JOB_INPUT_ARCHIVE_GCS_PATH", value=f"gs://{config.input_gcs_bucket}/{temp_gcs_input_key}"),
            run_v2.EnvVar(name="SFE_JOB_OUTPUT_GCS_BUCKET", value=config.output_gcs_bucket or config.input_gcs_bucket),
            run_v2.EnvVar(name="SFE_JOB_OUTPUT_GCS_KEY_PREFIX", value=f"{config.output_gcs_key_prefix}/outputs/{job_id}"),
        ...
        try:
            execution_template = run_v2.ExecutionTemplate(template=task_template, parallelism=config.parallelism, task_count=config.task_count)
        except Exception:
        ...
        if not config.job_definition_name:
            ...
            @retry(stop=stop_after_attempt(3), wait=wait_exponential(min=2, max=10), retry=retry_if_exception_type((GoogleAPIError, Conflict)))
        ...
        result.update({"executionName": execution_name, "jobId": job_name_to_use, "reason": "Job execution started."})
        ...
        while True:
            ...
            final_status_reason = (
                execution_info.conditions[0].message if getattr(execution_info, "conditions", None) else None
            )
            logger.info(f"Execution '{execution_name}' status: {exec_state} ({final_status_reason})")
            if exec_state in ["SUCCEEDED", "FAILED", "CANCELLED", "TIMED_OUT"]:
                started = getattr(execution_info, "start_time", None)
                completed = getattr(execution_info, "completion_time", None)
                try:
                    dur = (completed - started).total_seconds() if started and completed else (time.monotonic() - monitor_start)
                except Exception:
        ...
        if exec_state not in ["SUCCEEDED", "FAILED", "CANCELLED", "TIMED_OUT"]:
            ...
            result.update({"finalStatus": exec_state, "statusReason": final_status_reason, "reason": final_status_reason})
        ...
        if exec_state == "SUCCEEDED":
            ...
            result["output_gcs_location"] = f"gs://{config.output_gcs_bucket or config.input_gcs_bucket}/{output_gcs_key}"
            ...
            async def download_job_results():
                logger.info(f"Downloading results from gs://{config.output_gcs_bucket or config.input_gcs_bucket}/{output_gcs_key}...")
            ...
            try:
                ...
                GCS_OPERATION_LATENCY.labels(operation='download').observe(time.monotonic() - download_start)
            except NotFound:
                logger.warning(f"Output file {output_gcs_key} not found in GCS.")
                result.update({"reason": result["reason"] + " Output file not found.", "success": False})
            except GoogleAPIError as e:
                logger.error(f"Failed to download results: {e}")
                result.update({"reason": result["reason"] + f" Download failed: {e}", "success": False})
        ...
        if exec_state == "FAILED" and logging_client:
            try:
                ...
                logs_iter = await asyncio.to_thread(logging_client.list_entries, filter_=filter_str, page_size=10)
                raw_logs = []
        ...
        if temp_archive_path and os.path.exists(temp_archive_path) and not config.retain_temp_archive:
            try:
        ...
        if temp_gcs_input_key and config.cleanup_gcs_input:
            try:
                ...
                GCS_OPERATION_LATENCY.labels(operation='delete').observe(time.monotonic() - delete_start)
                logger.debug(f"Cleaned up GCS input: gs://{config.input_gcs_bucket}/{temp_gcs_input_key}")
            except NotFound:
        ...
        JOB_DURATION_SECONDS.labels(status=result.get("finalStatus", "UNKNOWN")).observe(float(result.get("duration_seconds") or 0.0))
