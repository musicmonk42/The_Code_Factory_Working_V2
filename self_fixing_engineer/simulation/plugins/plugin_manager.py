...
try:
    ...
    try:
        ...
        logging.getLogger(__name__).warning("No suitable file locking library found (portalocker or fcntl). Plugin manager file operations may not be atomic under heavy concurrency. Install 'portalocker' for robust locking.")
...
try:
    from tenacity import retry as _tenacity_retry, stop_after_attempt, wait_exponential, retry_if_exception_type
...
PYTHON_ISOLATION_MODE = os.getenv("PLUGIN_MANAGER_PYTHON_ISOLATION", "").lower() or ("process" if (os.getenv("CI") or os.getenv("GITHUB_ACTIONS")) else "process")
...
if prometheus_available:
    _metrics_singleton = sys.modules.setdefault("simulation._metrics_singleton", type(sys)("simulation._metrics_singleton"))
        ...
        if m is None:
            try:
                ...
                if m is None:
                    ...
                    m = Counter(f"{name}_dupe_suppressed", doc, tuple(labelnames), registry=registry)
    ...
    def _get_or_create_gauge(name: str, doc: str, labelnames=(), registry=REGISTRY):
        ...
        if m is None:
            try:
                ...
                m = _metrics_singleton.CACHE.get(k)
                if m is None:
                    m = Gauge(f"{name}_dupe_suppressed", doc, tuple(labelnames), registry=registry)
            _metrics_singleton.CACHE[k] = m
        return m

    # Define module metrics idempotently
    PLUGIN_LOADS_TOTAL  = _get_or_create_counter('plugin_loads_total',  'Total plugin load attempts',              ['plugin_type', 'status'])
    PLUGIN_ERRORS_TOTAL = _get_or_create_counter('plugin_errors_total', 'Total errors during plugin operations',   ['error_type', 'plugin_name'])
    PLUGIN_HEALTH_STATUS= _get_or_create_gauge  ('plugin_health_status','Binary plugin health (1=ok,0=bad)',       ['plugin_name'])

def retry_decorator(exc_type):
    """Clarity wrapper for tenacity decorator selection."""
    if tenacity_available:
        return with_retry(stop_after_attempt(3), wait_exponential(min=2, max=10), retry_if_exception_type(exc_type))
    return (lambda f: f)
...
try:
    ...
    class WasmPluginWrapper:
        ...
        async def health(self) -> Dict[str, Any]:
            ...
            try:
                return await asyncio.wait_for(self.runner.plugin_health(), timeout=HEALTH_TIMEOUT_SEC)
            except asyncio.TimeoutError:
                return {"status": "error", "message": f"WASM health timeout after {HEALTH_TIMEOUT_SEC}s"}
            except Exception as e:
                return {"status": "error", "message": f"WASM plugin health check failed: {e}"}
    ...
    plugin_logger.warning(f"wasm_runner.py not found or failed to import: {e}. WASM plugin support will be limited.")
...
try:
    ...
    class GrpcPluginWrapper:
        ...
        async def health(self) -> Dict[str, Any]:
            ...
            try:
                ...
                return {"status": "error", "message": f"gRPC health timeout after {HEALTH_TIMEOUT_SEC}s"}
            except Exception as e:
                return {"status": "error", "message": f"gRPC plugin health check failed: {e}"}
    ...
    plugin_logger.warning(f"grpc_runner.py not found or failed to import: {e}. gRPC plugin support will be limited.")
...
DANGEROUS_PYTHON_MODULES = set(sys.builtin_module_names).union({'os', 'sys', 'subprocess', 'shutil', 'importlib', 'socket', 'threading', 'multiprocessing', 'ctypes', 'site', 'inspect'})
    ...
    try:
        ...
        for node in tree.body:
            if isinstance(node, ast.Assign):
                ...
                if isinstance(node.target, ast.Name) and node.target.id == "PLUGIN_MANIFEST" and node.value is not None:
...
def _minimal_manifest_validate(manifest: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    required = ["name", "version", "type", "entrypoint", "api_version", "manifest_version", "min_core_version", "max_core_version"]
    for k in required:
        if k not in manifest:
            return False, f"Missing required manifest field: {k}"
    if not isinstance(manifest.get("name"), str) or not manifest["name"]:
        return False, "Invalid name"
    if not isinstance(manifest.get("type"), str) or manifest["type"] not in ("python", "wasm", "grpc"):
...
"""
        ...
        try:
            ...
            try:
                ...
                pass
            return {"status": "error", "message": f"Python plugin health timeout after {self.health_timeout}s"}
        if proc.returncode != 0 and not out:
            return {"status": "error", "message": f"Python plugin process failed: {err.decode(errors='replace')[:1024]}"}
        try:
            return json.loads(out.decode(errors="replace").strip() or "{}") or {"status": "error", "message": "Empty health response"}
        except Exception as e:
...
class PluginManager:
    ...
    def _validate_manifest_schema(self, plugin_path: Path, manifest: Dict[str, Any]) -> bool:
        ...
        try:
            if _parse_version(manifest.get("manifest_version", "0.0")) < _parse_version(MIN_MANIFEST_VERSION):
                plugin_logger.error(f"Manifest version {manifest.get('manifest_version')} < {MIN_MANIFEST_VERSION}. Rejected.")
                return False
        except Exception:
            pass
        ...
        if any(perm in manifest.get("permissions", []) for perm in ["execute_arbitrary_code", "network_unrestricted"]):
            plugin_logger.error(f"[{manifest.get('name','?')}] Dangerous permissions detected.")
            return False

        if plugin_type == "python":
            if manifest.get("sandbox", {}).get("enabled"):
                plugin_logger.warning(f"Python plugin '{manifest['name']}' has sandbox enabled. Python sandboxing is limited; consider WASM/gRPC for better isolation.")
            # Prevent shadowing key stdlib/builtin names
            if manifest["name"] in DANGEROUS_PYTHON_MODULES or manifest["name"] in sys.builtin_module_names:
                plugin_logger.error(f"Python plugin '{manifest['name']}' uses a disallowed module name. Rejected.")
                return False
            ...
            if plugin_path.is_dir() and not manifest.get("entrypoint", "").endswith(".py"):
                plugin_logger.error(f"Python plugin '{manifest['name']}' directory entrypoint must be a .py file path (not module path). Rejected.")
                return False

        elif plugin_type == "wasm":
            sandbox = manifest.get("sandbox", {})
            if not sandbox.get("enabled"):
                plugin_logger.error(f"WASM plugin '{manifest['name']}' must enable sandboxing. Rejected.")
                return False
            if "resource_limits" not in sandbox:
                plugin_logger.error(f"WASM plugin '{manifest['name']}' must specify 'resource_limits'. Rejected.")
                return False
            ...
            if plugin_path.is_dir():
                ...
                if ep.endswith(".wasm"):
                    ...
                    if not any(plugin_path.glob("*.wasm")):
                        plugin_logger.error(f"No .wasm binary found for WASM plugin '{manifest['name']}'. Rejected.")
                        return False

        elif plugin_type == "grpc":
            sandbox = manifest.get("sandbox", {})
            if not sandbox.get("enabled"):
                plugin_logger.error(f"gRPC plugin '{manifest['name']}' must enable sandboxing. Rejected.")
                return False
            ep = manifest.get("entrypoint", "")
            if not ep.startswith("grpc://"):
                plugin_logger.error(f"gRPC plugin '{manifest['name']}' requires 'entrypoint' to be a gRPC URL. Rejected.")
                return False
        ...
        try:
            if min_version and _parse_version(current_core_version) < _parse_version(min_version):
                plugin_logger.error(f"Plugin requires core version >= {min_version}, current is {current_core_version}. Rejected.")
                return False
            if max_version and _parse_version(current_core_version) > _parse_version(max_version):
                plugin_logger.warning(f"Plugin supports up to core version {max_version}, current is {current_core_version}. Compatibility issues may arise.")
        except Exception:
            pass

        return True

    def _import_python_module_inproc(self, plugin_name: str, plugin_path: Path, manifest: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
        """
        ...
        if plugin_path.is_file():
            ...
            if sandbox_enabled and restricted_python_available:
                with open(plugin_path, "r", encoding="utf-8") as f:
                    ...
                    module = importlib.util.module_from_spec(importlib.util.spec_from_loader(plugin_name, None))
    ...
    def _get_python_instance(self, plugin_name: str, plugin_path: Path, manifest: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
        """
        ...
        if mode not in ("process", "inproc"):
            mode = "process"
        if mode == "process":
            proxy = PythonSubprocessProxy(plugin_path, manifest, health_timeout=self._health_timeout_for(manifest))
    ...
    def load_plugin(self, plugin_path: Path, check_health: bool = False):
        ...
        try:
            ...
            with self._registry_lock:
                if plugin_name in self.registry and self.registry[plugin_name].get("status") in ("loaded", "warning", "disabled"):
                    raise ValueError(f"Duplicate plugin name '{plugin_name}' detected. Unload or rename the existing plugin.")
            ...
            plugin_logger.info(f"[{plugin_name}] Loaded {plugin_type} plugin (isolation={ 'process' if plugin_type=='python' and isinstance(instance, PythonSubprocessProxy) else 'inproc' }).")
            if prometheus_available:
            ...
            if check_health:
                ...
                with self._registry_lock:
                    if status in ["error", "warning", "fail", "not_serving"]:
                        plugin_logger.warning(f"[{plugin_name}] Initial health check reported: {initial_health.get('message', status)}. Status set to 'warning'.")
            ...
            with self._registry_lock:
                self.registry[plugin_name] = {"status": "error", "error": str(e), "path": str(plugin_path)}
    ...
    def reload_plugin(self, name: str):
        ...
        try:
            ...
            with self._registry_lock:
                if reloaded_health.get("status") in ["error", "warning", "fail", "not_serving"]:
                    plugin_logger.warning(f"[{name}] Post-reload health check reported: {reloaded_health.get('message', reloaded_health.get('status'))}. Status set to 'warning'.")
                    self.registry[name]["status"] = "warning"
                else:
                    plugin_logger.info(f"[{name}] Post-reload health check: {reloaded_health.get('status')}.")
    ...
    def enable_plugin(self, name: str):
        ...
        with self._registry_lock:
            ...
            if self.registry[name]["status"] != "disabled":
                plugin_logger.info(f"[{name}] Plugin not disabled; current status: {self.registry[name]['status']}")
                return
        ...
        try:
            ...
            with self._registry_lock:
                if health_status.get("status") in ["ok", "serving"]:
                    ...
                    self.registry[name]["status"] = "warning"
                    self.registry[name]["error"] = health_status.get("message", "Re-enable health check failed.")
                    plugin_logger.warning(f"[{name}] Enabled but health check failed: {self.registry[name]['error']}")
        except Exception as e:
            with self._registry_lock:
    ...
    def disable_plugin(self, name: str):
        ...
        with self._registry_lock:
            if name not in self.registry or self.registry[name]["status"] not in ("loaded", "warning"):
                plugin_logger.warning(f"[{name}] Cannot disable: status={self.registry.get(name,{}).get('status')}")
                return
            plugin_entry = self.registry[name]
        ...
        try:
            ...
            plugin_logger.error(f"[{name}] Failed to fully disable (close error): {e}\n{traceback.format_exc()}")
    ...
    async def health(self, name: str) -> Dict[str, Any]:
        ...
        with self._registry_lock:
            ...
            if entry.get("status") not in ("loaded", "warning", "disabled"):
                return {"status": "error", "message": f"Plugin '{name}' is not in a checkable state (status: {entry.get('status')})"}

            instance = entry.get("instance")
            if not instance:
                return {"status": "error", "message": f"Plugin '{name}' has no instance"}
        ...
        try:
            ...
            # For in-process Python modules
                ...
                if hasattr(instance, health_method_name):
                    ...
                    return {"status": "error", "message": f"Health method '{health_method_name}' not found on plugin instance"}
            else:
                return {"status": "error", "message": f"No health check available for plugin type '{plugin_type}'"}
            ...
            result = {"status": "error", "message": f"Health check timeout for plugin '{name}'"}
            if prometheus_available:
            ...
            if prometheus_available:
                PLUGIN_HEALTH_STATUS.labels(plugin_name=name).set(0.0)
                PLUGIN_ERRORS_TOTAL.labels(error_type='health_check_failure', plugin_name=name).inc()
            return result
