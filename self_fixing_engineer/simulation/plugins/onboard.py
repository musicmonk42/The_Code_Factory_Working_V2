...
try:
    ...
    sys.stderr.write("Error: onboard.py requires 'pydantic'. Please install it: pip install pydantic\n")
    sys.exit(97)
...
class OnboardConfig(BaseModel):
    ...
    notification_backend: dict = Field(default_factory=dict, description="Notification backend config")
    checkpoint_backend: dict = Field(default_factory=dict, description="Checkpoint backend config")
    environment_variables: dict = Field(default_factory=dict, description="Environment variables to set")
...
def setup_logging(verbose=False, quiet=False, json_format=False):
    ...
    file_handler = RotatingFileHandler(script_dir / "onboarding_wizard.log", maxBytes=1_000_000, backupCount=3, encoding="utf-8")
...
if prometheus_available:
    ONBOARDING_STEPS_TOTAL = Counter('onboarding_steps_total', 'Total onboarding steps completed', ['step'])
    ONBOARDING_ERRORS_TOTAL = Counter('onboarding_errors_total', 'Total errors during onboarding', ['error_type'])
    ONBOARDING_LATENCY_SECONDS = Histogram('onboarding_latency_seconds', 'Latency of onboarding operations', ['operation'])
    ONBOARDING_HEALTH_CHECK_STATUS = Gauge('onboarding_health_check_status', 'Status of health checks', ['check_name'])
else:
    class _NoopMetric:
    ...
    ONBOARDING_STEPS_TOTAL = ONBOARDING_ERRORS_TOTAL = ONBOARDING_LATENCY_SECONDS = ONBOARDING_HEALTH_CHECK_STATUS = _NoopMetric()
...
def _get_user_input(prompt: str, options: Optional[List[str]] = None, default: Optional[str] = None, secret: bool = False) -> str:
...
async def _generate_config(config_data: Dict[str, Any], filename: str = "config.json"):
    ...
    with ONBOARDING_LATENCY_SECONDS.labels(operation='generate_config').time():
        ...
        try:
            ...
            print_status(f"Failed to write config file {config_path}: {e}. Check permissions or disk space.", "err")
...
def _generate_secure_config(secrets: Dict[str, str], secrets_manager: str = "local"):
    ...
    with ONBOARDING_LATENCY_SECONDS.labels(operation='generate_secure_config').time():
        if secrets_manager == "vault" and vault_available:
            ...
            vault_token = os.environ.get("VAULT_TOKEN") or _get_user_input("Enter Vault Token", secret=True)
            try:
        ...
        if not crypto_available:
            print_status("cryptography library not available. Refusing to store secrets insecurely. Please install 'cryptography' or use a secret manager.", "err")
            raise RuntimeError("Cannot securely store secrets without 'cryptography' or a secrets manager.")
...
def _load_secure_config() -> Dict[str, str]:
    ...
    f = Fernet(key)
    try:
        return {k: f.decrypt(v.encode('utf-8')).decode('utf-8') for k, v in data.get("secrets", {}).items()}
    except Exception as e:
        logger.error(f"Failed to decrypt secrets: {e}", exc_info=True)
        print_status("Failed to decrypt secrets. Check your key.", "err")
        return {}

@retry(stop=stop_after_attempt(3), wait=wait_exponential(min=2, max=10), retry=retry_if_exception_type(requests.RequestException))
...
def _validate_plugin_syntax(plugin_file_path: Path):
    ...
    try:
        ...
        print_status(f"Plugin syntax validation failed for {plugin_file_path}: {e}. Check syntax errors in the file.", "err")
        raise
...
def _auto_format_plugin(plugin_file_path: Path):
    if black_available:
        try:
            ...
            subprocess.run([sys.executable, "-m", "black", str(plugin_file_path), "--quiet"], check=False)
...
async def _generate_plugin_manifest(plugin_type: str, plugin_name: str, plugins_dir: Path):
    ...
    with ONBOARDING_LATENCY_SECONDS.labels(operation='generate_plugin_manifest').time():
        ...
        if manifest["min_core_version"] > CORE_VERSION or manifest["max_core_version"] < CORE_VERSION:
            print_status(f"Generated plugin may not be compatible with core version {CORE_VERSION}", "warn")
...
"""
            ...
            try:
                ...
                print_status(f"Failed to write demo plugin file {plugin_filepath}: {e}. Check permissions or syntax in template.", "err")
            ...
            try:
                ...
                print_status(f"Generated demo {plugin_type} plugin manifest: {manifest_path}", "ok")
                ...
                if plugin_type == "wasm":
                    ...
                    print_status(f"Created placeholder WASM file: {wasm_filepath}. Replace with your compiled .wasm binary.", "info")
                ...
                print_status(f"Failed to write demo plugin manifest {manifest_path}: {e}. Check permissions.", "err")
...
async def _run_health_checks(config: Dict[str, Any]):
    ...
    if MESH_ADAPTER_AVAILABLE:
        ...
        if pubsub_backend_url:
            ...
            try:
                ...
                print_status(f"Pub/Sub Health: {health.get('status','unknown').upper()} - {health.get('message', '')}", "ok" if ok else "err")
        ...
        if checkpoint_backend_type:
            ...
            try:
                ...
                if loaded_data and loaded_data.get("status") == "healthy":
                    print_status(f"Checkpoint Health: OK (saved and loaded test data successfully for {checkpoint_backend_type}).", "ok")
                    ONBOARDING_HEALTH_CHECK_STATUS.labels(check_name='checkpoint').set(1)
                else:
                    print_status(f"Checkpoint Health FAILED: Data mismatch for {checkpoint_backend_type}.", "err")
                    ONBOARDING_HEALTH_CHECK_STATUS.labels(check_name='checkpoint').set(0)
            except Exception as e:
                print_status(f"Checkpoint Health Check FAILED for {checkpoint_backend_type}: {e}", "err")
    ...
    print_status("\nChecking installed demo plugins (requires PluginManager to be run separately for full check):", "info")
    for plugin_type in ["python", "wasm", "grpc"]:
        ...
        if plugin_file_path.exists() or plugin_manifest_path.exists():
            print_status(f"  - Demo {plugin_type} plugin detected. (Run your plugin manager for full health)", "info")
        else:
...
async def _safe_mode_profile():
    ...
    print_status("*** SAFE MODE / STARTER PROFILE: Local-only demo. No cloud/network required. ***", "ok")
    ...
    await _generate_readme(CONFIG_DIR, "Configs Directory", "Contains configuration files for your project (SAFE MODE).")

    plugin_name = "demo_python_plugin"
    await _generate_plugin_manifest("python", plugin_name, PLUGINS_DIR)

    await _generate_readme(PLUGINS_DIR, "Plugins Directory", "Drop Python plugins or plugin folders here. Each plugin should have a manifest or be a single .py file. (SAFE MODE)")
    await _generate_readme(RESULTS_DIR, "Simulation Results", "Simulation outputs and logs will be stored here. (SAFE MODE)")

    print_status("Safe Mode profile created. You can run a local-only demo without any cloud/network setup!", "ok")
    print_status("To reset at any time, run: python onboard.py --reset", "info")
    print_status("To run troubleshooting/diagnostics, run: python onboard.py --troubleshoot", "info")
...
async def _reset_to_safe_mode():
    ...
    if not _non_interactive():
        confirm = _get_user_input("This will delete configs/plugins/results/CI files in this repo. Type YES to confirm", default="no")
    else:
        ...
        if target_dir.exists():
            for fpath in target_dir.iterdir():
                try:
                    ...
                    elif fpath.name not in ('.gitkeep', '.gitignore') and not fpath.name.endswith('.md'):
                        fpath.unlink()
...
def _run_basic_onboarding_tests():
    ...
    if test_plugin_file_path.exists():
        ...
        print_status("Test 2 (Demo Plugin): Skipped, no demo plugin file/manifest found (expected if no plugin types were selected).", "warn")
...
"""
    try:
        ...
        if ci_env:
            print_status("Detected CI environment. Use the generated ci.yaml in your workflow.", "info")
    except (PermissionError, OSError) as e:
...
def _check_existing_configs():
    ...
    if existing:
        try:
            overwrite = _get_user_input("Existing configs/plugins detected. Continue and overwrite?", options=["yes", "no"], default="no")
        except RuntimeError:
            # Non-interactive: do not overwrite by default
            print_status("Existing configs/plugins detected. Use --reset or remove the directories to continue in non-interactive mode.", "err")
            sys.exit(2)
        if overwrite == "no":
            sys.exit(0)
...
def _detect_venv():
    if sys.prefix == sys.base_prefix:
        print_status("WARNING: Not running in a Python virtual environment. Consider using venv for isolation.", "warn")
...
def _print_security_checklist(config: Dict[str, Any]):
    ...
    if SECURE_CONFIG_PATH.exists():
        ...
        print_status("No encrypted secrets found - configure encryption or secrets manager.", "warn")
    print_status("Default config disables external network access in Safe Mode." if config.get("project_type") == "demo_safe_mode" else "External access enabled - review permissions.", "ok" if config.get("project_type") == "demo_safe_mode" else "warn")
...
def _auto_open_docs():
    ...
    if readme_path.exists() and not _non_interactive():
        open_docs = _get_user_input("Open README in default browser?", options=["yes", "no"], default="yes")
        if open_docs == "yes":
...
async def onboard(args):
    ...
    try:
        ...
        print_status("This wizard will help you set up your project configuration and a demo plugin.", "info")

        # --- Project type ---
        if args.project_type:
            project_type = args.project_type
        else:
            project_type = "agentic_swarm" if non_interactive else _get_user_input(
        ...
        if args.plugin_types:
            ...
            selected_plugin_types_input = "python" if non_interactive else _get_user_input(
                f"Which plugin types do you plan to use? (comma-separated, e.g., {','.join(plugin_types_options)})",
                default="python"
            )
        selected_plugin_types = [p.strip() for p in selected_plugin_types_input.split(',') if p.strip() in plugin_types_options]
        if not selected_plugin_types:
            print_status("No valid plugin types selected. Skipping demo plugin generation.", "warn")
        ...
        if MESH_ADAPTER_AVAILABLE:
            try:
                supported = getattr(MeshPubSub, "supported_backends", lambda: ["redis", "nats", "kafka", "rabbitmq", "aws", "gcs", "azure", "etcd"])()
                pubsub_backend_options = sorted(list(set(pubsub_backend_options + [b for b in supported if b != "local"])))
            except Exception as e:
                logger.warning(f"Could not get supported backends from MeshPubSub: {e}. Using default set.")
                pubsub_backend_options = sorted(list(set(pubsub_backend_options + ["redis", "nats", "kafka", "rabbitmq", "aws", "gcs", "azure", "etcd"])))

        if args.pubsub_backend:
            pubsub_backend = args.pubsub_backend
        else:
            pubsub_backend = "local" if non_interactive else _get_user_input(
                f"Choose your preferred notification backend: {pubsub_backend_options}",
                options=pubsub_backend_options,
                default="redis" if "redis" in pubsub_backend_options else pubsub_backend_options[0]
            )
        ...
        if pubsub_backend in ["aws", "gcs", "azure"]:
            if not non_interactive:
                use_secrets_manager = _get_user_input("Store secrets with advanced manager (vault/aws/azure)?", options=["yes", "no"], default="no")
                if use_secrets_manager == "yes":
                    secrets_manager = _get_user_input("Choose manager", options=["vault", "aws", "azure"], default="vault")
            else:
        ...
        if pubsub_backend == "redis":
            pubsub_config["url"] = "redis://localhost:6379/0" if non_interactive else _get_user_input("Enter Redis URL", default="redis://localhost:6379/0")
        elif pubsub_backend == "nats":
            pubsub_config["url"] = "nats://localhost:4222" if non_interactive else _get_user_input("Enter NATS URL", default="nats://localhost:4222")
        elif pubsub_backend == "kafka":
            pubsub_config["url"] = "localhost:9092" if non_interactive else _get_user_input("Enter Kafka Bootstrap Servers (e.g., localhost:9092)", default="localhost:9092")
        elif pubsub_backend == "rabbitmq":
            pubsub_config["url"] = "amqp://guest:guest@localhost:5672/" if non_interactive else _get_user_input("Enter RabbitMQ AMQP URL", default="amqp://guest:guest@localhost:5672/")
        elif pubsub_backend == "aws":
            pubsub_config["url"] = "aws://"
            secrets["AWS_ACCESS_KEY_ID"] = os.environ.get("AWS_ACCESS_KEY_ID") or (None if non_interactive else _get_user_input("Enter AWS Access Key ID"))
            secrets["AWS_SECRET_ACCESS_KEY"] = os.environ.get("AWS_SECRET_ACCESS_KEY") or (None if non_interactive else _get_user_input("Enter AWS Secret Access Key", secret=True))
            secrets["AWS_REGION_NAME"] = os.environ.get("AWS_REGION_NAME") or ("us-east-1" if non_interactive else _get_user_input("Enter AWS Region", default="us-east-1"))
            print_status("For AWS backend, secrets will be stored when provided.", "info")
            if not non_interactive:
                test_conn = _get_user_input("Test AWS connection now?", options=["yes", "no"], default="yes")
                if test_conn == "yes":
                    await _test_connection("aws", {'secrets': secrets})
        elif pubsub_backend == "gcs":
            pubsub_config["url"] = "gcs://"
            pubsub_config["gcs_bucket_name"] = "your-gcs-event-bucket" if non_interactive else _get_user_input("Enter GCS Bucket Name for events", default="your-gcs-event-bucket")
            secrets["GOOGLE_APPLICATION_CREDENTIALS"] = os.environ.get("GOOGLE_APPLICATION_CREDENTIALS") or (None if non_interactive else _get_user_input("Enter path to Google Service Account Key"))
            if not non_interactive:
                test_conn = _get_user_input("Test GCS connection now?", options=["yes", "no"], default="yes")
                if test_conn == "yes":
                    await _test_connection("gcs", {'secrets': secrets})
        elif pubsub_backend == "azure":
            pubsub_config["url"] = "azure://"
            pubsub_config["azure_connection_string"] = "DefaultEndpointsProtocol=..." if non_interactive else _get_user_input("Enter Azure Storage Connection String", default="DefaultEndpointsProtocol=...")
            pubsub_config["azure_container_name"] = "mesh-events" if non_interactive else _get_user_input("Enter Azure Blob Container Name for events", default="mesh-events")
        elif pubsub_backend == "etcd":
            pubsub_config["url"] = "etcd://"
            pubsub_config["etcd_host"] = "localhost" if non_interactive else _get_user_input("Enter etcd Host", default="localhost")
            pubsub_config["etcd_port"] = "2379" if non_interactive else _get_user_input("Enter etcd Port", default="2379")
        ...
        if CHECKPOINT_AVAILABLE:
            try:
                ...
                checkpoint_backend_options = sorted(list(set(checkpoint_backend_options + [b for b in additional if b != "fs"])))
            except Exception as e:
                logger.warning(f"Could not get supported backends from CheckpointManager: {e}. Falling back to defaults.")
                checkpoint_backend_options = sorted(list(set(checkpoint_backend_options + ["s3", "redis", "postgres", "gcs", "azure", "etcd"])))

        if args.checkpoint_backend:
            checkpoint_backend = args.checkpoint_backend
        else:
            checkpoint_backend = "fs" if non_interactive else _get_user_input(
        ...
        if checkpoint_backend == "fs":
            checkpoint_config["dir"] = "./checkpoints" if non_interactive else _get_user_input("Enter local directory for checkpoints", default="./checkpoints")
        elif checkpoint_backend == "s3":
            checkpoint_config["s3_config"] = {"bucket": "your-s3-checkpoint-bucket" if non_interactive else _get_user_input("Enter S3 Bucket Name", default="your-s3-checkpoint-bucket")}
            secrets["AWS_ACCESS_KEY_ID"] = secrets.get("AWS_ACCESS_KEY_ID") or os.environ.get("AWS_ACCESS_KEY_ID") or (None if non_interactive else _get_user_input("Enter AWS Access Key ID"))
            secrets["AWS_SECRET_ACCESS_KEY"] = secrets.get("AWS_SECRET_ACCESS_KEY") or os.environ.get("AWS_SECRET_ACCESS_KEY") or (None if non_interactive else _get_user_input("Enter AWS Secret Access Key", secret=True))
            secrets["AWS_REGION_NAME"] = secrets.get("AWS_REGION_NAME") or os.environ.get("AWS_REGION_NAME") or ("us-east-1" if non_interactive else _get_user_input("Enter AWS Region", default="us-east-1"))
        elif checkpoint_backend == "redis":
            checkpoint_config["redis_config"] = {"url": "redis://localhost:6379/1" if non_interactive else _get_user_input("Enter Redis URL for checkpoints", default="redis://localhost:6379/1")}
        elif checkpoint_backend == "postgres":
            checkpoint_config["postgres_config"] = {"dsn": "postgresql://user:password@localhost:5432/database" if non_interactive else _get_user_input("Enter Postgres DSN", default="postgresql://user:password@localhost:5432/database")}
            print_status("Ensure your Postgres database has the 'checkpoints' table created.", "warn")
        elif checkpoint_backend == "gcs":
            checkpoint_config["gcs_config"] = {"bucket": "your-gcs-checkpoint-bucket" if non_interactive else _get_user_input("Enter GCS Bucket Name for checkpoints", default="your-gcs-checkpoint-bucket")}
            secrets["GOOGLE_APPLICATION_CREDENTIALS"] = secrets.get("GOOGLE_APPLICATION_CREDENTIALS") or os.environ.get("GOOGLE_APPLICATION_CREDENTIALS") or (None if non_interactive else _get_user_input("Enter path to Google Service Account Key"))
        elif checkpoint_backend == "azure":
            checkpoint_config["azure_config"] = {
                "connection_string": "DefaultEndpointsProtocol=..." if non_interactive else _get_user_input("Enter Azure Storage Connection String for checkpoints", default="DefaultEndpointsProtocol=..."),
                "container_name": "checkpoints" if non_interactive else _get_user_input("Enter Azure Blob Container Name for checkpoints", default="checkpoints")
            }
        elif checkpoint_backend == "etcd":
            checkpoint_config["etcd_config"] = {
                "host": "localhost" if non_interactive else _get_user_input("Enter etcd Host for checkpoints", default="localhost"),
                "port": "2379" if non_interactive else _get_user_input("Enter etcd Port for checkpoints", default="2379")
            }
        ...
        core_config = {
            ...
            "environment_variables": {
                ...
                "CHECKPOINT_FS_DIR": checkpoint_config.get("dir", "") if checkpoint_config.get("type") == "fs" else ""
            },
            "generated_with": {
                "wizard_version": "1.0.0",
                "python_version": platform.python_version(),
        ...
        await _generate_config(core_config)
        await _generate_readme(CONFIG_DIR, "Configs Directory", "Contains configuration files for your project.")
        ...
        await _generate_readme(RESULTS_DIR, "Simulation Results", "Simulation outputs and logs will be stored here.")

        # --- Generate CI ---
        if non_interactive:
            _generate_ci_yaml(ci_env)
        else:
            generate_ci_opt = _get_user_input("Generate GitHub Actions CI YAML?", options=["yes", "no"], default="yes")
        ...
        if non_interactive:
            ...
            run_health_checks_opt = _get_user_input("Do you want to run health checks for the configured backends now?", options=["yes", "no"], default="yes")
        if run_health_checks_opt == "yes":
            await _run_health_checks(core_config)

        # --- Run Test Simulation (best-effort) ---
        if not non_interactive:
            run_test_sim = _get_user_input("Do you want to run a test simulation now?", options=["yes", "no"], default="no")
            if run_test_sim == "yes":
                try:
                    subprocess.run([sys.executable, "main_sim_runner.py", "--session", "test_onboard"], check=True)
        ...
        # --- Telemetry Opt-in ---
        if not non_interactive:
            telemetry_opt = _get_user_input("Opt-in to anonymized telemetry for improvement? (no data sent otherwise)", options=["yes", "no"], default="no")
        ...
        if not non_interactive:
            run_first_job_opt = _get_user_input("Do you want to see the command to run your first job?", options=["yes", "no"], default="yes")
            if run_first_job_opt == "yes":
                print_status("\n--- Command to Run Your First Job ---", "ok")
                print_status("To run a simulation using your new configuration and demo plugin, run:", "info")
                print_status(f"python your_main_simulation_script.py --config {CONFIG_DIR / 'config.json'}", "info")
                print_status("Make sure your main script initializes MeshPubSub and CheckpointManager with values from config.json.", "info")
                print_status(f"Example: python -m your_app.main --config {CONFIG_DIR / 'config.json'}", "info")
                print_status("Or, if using environment variables:", "info")
                print_status(f"export MESH_BACKEND_URL='{pubsub_config.get('url', '')}'", "info")
                print_status(f"export CHECKPOINT_BACKEND_TYPE='{checkpoint_config.get('type', '')}'", "info")
                if checkpoint_config.get('type') == 'fs':
                    print_status(f"export CHECKPOINT_FS_DIR='{checkpoint_config.get('dir', '')}'", "info")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--safe', action='store_true', help="Run onboarding in SAFE MODE / Starter Profile (local demo only)")
    parser.add_argument('--reset', action='store_true', help="Reset configs/plugins to safe mode / starter profile")
    parser.add_argument('--troubleshoot', action='store_true', help="Run health checks and onboarding diagnostics")
    parser.add_argument('--show-examples', action='store_true', help="Show example configs, plugins, jobs after onboarding")
    ...
    parser.add_argument('--plugin-types', type=str, help="Set plugin types (comma-separated, no prompt)")
    ...
    try:
        if args.reset:
            ...
            try:
                ...
                print_status("No config.json found. Please run onboarding first or use --safe.", "err")
