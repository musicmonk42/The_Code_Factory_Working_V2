...
if os.path.exists(CONFIG_FILE):
    ...
    PLUGIN_MANIFEST = {
        ...
        "capabilities": ["aws_batch_execution", "cloud_bursting", "distributed_simulation"],
        "permissions_required": ["aws_s3_read", "aws_s3_write", "aws_batch_submit", "aws_batch_monitor", "aws_logs_read"],
        "compatibility": {
...
JOB_SUBMISSIONS_TOTAL = Counter(
    'aws_batch_job_submissions_total',
    'Total AWS Batch job submissions',
    ['status']
)
JOB_DURATION_SECONDS = Histogram(
    'aws_batch_job_duration_seconds',
    'Duration of AWS Batch jobs',
    ['status']
)
S3_OPERATION_LATENCY = Histogram(
    'aws_s3_operation_latency_seconds',
    'Latency of S3 operations',
    ['operation']
)

# Retry helpers
RetryableNetworkErrors = (EndpointConnectionError, ReadTimeoutError, ConnectionClosedError)

async def _async_retry(func, *args, retries=5, min_delay=2, max_delay=10, jitter=True, exceptions=(Exception,), **kwargs):
    attempt = 0
    while True:
        try:
...
class JobConfig(BaseModel):
    ...
    environment: List[Dict[str, str]] = Field(default_factory=list, description="Environment variables for the container")
    resourceRequirements: List[Dict[str, str]] = Field(default_factory=list, description="CPU/Memory requirements")
    container_overrides: Dict[str, Any] = Field(default_factory=dict, description="Raw container overrides")
    ...
    include_patterns: Optional[List[str]] = Field(None, description="Glob patterns to include when archiving project_root")
    exclude_patterns: Optional[List[str]] = Field(
        default_factory=lambda: [
            '.git/**', '**/.git/**', '**/.env', '**/*.pem', '**/*.key',
            'node_modules/**', '**/__pycache__/**', '.aws/**', '**/.aws/**',
            '**/*.crt', '**/*.p12', '**/.docker/**', '**/.npmrc', '**/.pypirc'
        ],
        description="Glob patterns to exclude"
    )
    max_archive_size_mb: int = Field(500, ge=1, le=10_000, description="Maximum allowed archive size in MB")
    sse_enabled: bool = Field(False, description="Enable server-side encryption for S3 uploads (SSE-S3 by default)")
    sse_kms_key_id: Optional[str] = Field(None, description="KMS Key ID for SSE-KMS encryption")
    poll_interval_seconds: int = Field(15, ge=1, le=300, description="Polling interval for job status checks")
    poll_jitter_seconds: float = Field(2.0, ge=0.0, le=30.0, description="Max jitter added/subtracted to poll interval")
...
async def _load_credentials_from_vault(vault_url: str) -> Optional[Dict[str, str]]:
    ...
    async def _do_get():
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(f"{vault_url}/v1/secret/data/aws-credentials", headers=headers, ssl=True) as response:
...
def _should_include_file(path: str, root: str, include_patterns: Optional[List[str]], exclude_patterns: List[str]) -> bool:
...
async def _create_filtered_archive(project_root: str, dest_tar_path: str, include_patterns: Optional[List[str]], exclude_patterns: List[str]) -> str:
                ...
                for root, dirs, files in os.walk(project_root, followlinks=False):
                    ...
                    for name in files:
                        ...
                        if os.path.islink(fpath):
                            continue
                        if _should_include_file(fpath, project_root, include_patterns, exclude_patterns):
...
def _s3_extra_args_for_encryption(sse_enabled: bool, sse_kms_key_id: Optional[str]) -> Dict[str, Any]:
    if sse_kms_key_id:
    ...
    if vault_mode:
        ...
        if not creds:
            ...
            if env_key and env_secret:
                creds = {"aws_access_key_id": env_key, "aws_secret_access_key": env_secret}
...
async def run_batch_job(job_config: dict, project_root: str, output_dir: str) -> dict:
    ...
    try:
        if os.environ.get("PYTEST_CURRENT_TEST"):
            ...
            await _create_filtered_archive(project_root, temp_archive_path, cfg.include_patterns, cfg.exclude_patterns)
        ...
        if archive_size_mb > cfg.max_archive_size_mb:
            ...
            raise ValueError(f"Archive size {archive_size_mb:.2f} MB exceeds max {cfg.max_archive_size_mb} MB")
    ...
    uploaded = False
    logger.info("[%s] Uploading to s3://%s/%s...", job_name, cfg.input_s3_bucket, temp_s3_input_key)
    try:
        extra_args = _s3_extra_args_for_encryption(cfg.sse_enabled, cfg.sse_kms_key_id)
        with open(temp_archive_path, "rb") as fh:
            await asyncio.to_thread(s3_client.upload_fileobj, fh, cfg.input_s3_bucket, temp_s3_input_key, ExtraArgs=extra_args)
        uploaded = True
        logger.info("[%s] Uploaded to S3: s3://%s/%s", job_name, cfg.input_s3_bucket, temp_s3_input_key)
    except Exception as e:
    ...
    async def _cleanup_input():
        if uploaded and cfg.cleanup_s3_input:
            try:
                await asyncio.to_thread(
                    s3_client.delete_object, Bucket=cfg.input_s3_bucket, Key=temp_s3_input_key
                )
            except Exception as e:
    ...
    submit_job_params = {
        ...
        "containerOverrides": {
            "environment": [
                {"name": "SFE_JOB_INPUT_ARCHIVE_S3_PATH", "value": f"s3://{cfg.input_s3_bucket}/{temp_s3_input_key}"},
    ...
    while time.monotonic() - poll_start_time < cfg.max_duration_seconds:
        try:
            ...
            if not jobs:
                logger.warning(f"DescribeJobs returned empty list for job {job_id}, continuing to poll.")
                job_detail = {}
            else:
                job_detail = jobs[0]
        except Exception as e:
        ...
        poll_delay = cfg.poll_interval_seconds + random.uniform(-cfg.poll_jitter_seconds, cfg.poll_jitter_seconds)
        ...
        status = "TIMED_OUT"
        try:
            await asyncio.to_thread(batch_client.terminate_job, jobId=job_id, reason="Job exceeded max duration")
        except Exception as e:
    ...
    if status == "FAILED" or status == "TIMED_OUT":
        ...
        if log_stream:
            try:
                ...
                raw_log = [e.get("message") for e in (log_resp or {}).get("events", []) if "message" in e]
            except Exception as e:
