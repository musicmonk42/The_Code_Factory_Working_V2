...
try:
    ...
    from google.api_core.exceptions import GoogleAPIError, Forbidden, NotFound, GoogleAPICallError
    GCP_AVAILABLE = True
except ImportError as e:
    # Let the factory decide how to handle unavailable clients
    raise ImportError("google-cloud-logging and/or google-cloud-secret-manager not found. GCP Logging client is unavailable.") from e
...
_TRANSIENT_GCP_ERROR_NAMES = {
    ...
    "Unavailable", "ResourceExhausted", "Aborted", "InternalServerError",
...
class GCPSecretManagerBackend(SecretsBackend):
    ...
    def __init__(self, project_id: str):
        if not GCP_AVAILABLE:
            raise SIEMClientConfigurationError("GCP Secret Manager backend requested but Google Cloud SDK is not available.", "GCPLogging")
        if not project_id:
            raise SIEMClientConfigurationError("GCP Project ID is required for GCP Secret Manager.", "GCPLogging")
    ...
    async def get_secret(self, secret_id: str) -> str:
        try:
            ...
            raise SIEMClientConfigurationError(f"Failed to fetch secret from GCP Secret Manager: {e}", "GCPLogging", original_exception=e)
...
class GcpLoggingConfig(BaseModel):
    ...
    credentials_path: Optional[str] = None  # Path to service account key file (optional; prefer vault)
    credentials_secret_id: Optional[str] = None  # Secret ID for service account key JSON in secrets backend
    ...
    def validate_project_id_format(cls, v):
        if PRODUCTION_MODE and not re.match(r"^[a-z][a-z0-9-]{4,28}[a-z0-9]$", v):
            raise ValueError(f"Invalid GCP Project ID format for production: {v}. Must be 6-30 characters, lowercase letters, numbers, and hyphens, and start with a letter.")
        return v

    @validator("log_name")
    def validate_log_name_format(cls, v):
        if PRODUCTION_MODE and not re.match(r"^[a-zA-Z0-9-._/]+$", v):
            raise ValueError(f"Invalid GCP Log Name format for production: {v}. Must contain letters, numbers, hyphens, underscores, dots, or slashes.")
        return v
    ...
    def validate_credentials_source(cls, v, values):
        if PRODUCTION_MODE:
            if not values.get("credentials_secret_id"):
                raise ValueError("In PRODUCTION_MODE, 'credentials_secret_id' must be provided for GCP credentials. Direct path/ENV are forbidden.")
...
class GcpLoggingClient(BaseSIEMClient):
    ...
    def __init__(self, config: Dict[str, Any], metrics_hook: Optional[Callable] = None, paranoid_mode: bool = False):
        ...
        try:
            ...
            _base_logger.critical(f"CRITICAL: Invalid GCP Logging client configuration: {e}.", extra={'client_type': self.client_type})
            alert_operator(f"CRITICAL: Invalid GCP Logging client configuration: {e}.", level="CRITICAL")
            raise SIEMClientConfigurationError(f"Invalid GCP Logging client configuration: {e}", self.client_type, original_exception=e)
    ...
    async def _ensure_credentials_loaded(self):
        ...
        async with self._creds_lock:
            ...
            for provider_name in self.secrets_providers:
                ...
                try:
                    if provider_name == "gcp":
                        provider_proj = (self.secrets_provider_config.get("gcp") or {}).get("project_id") or self.project_id
                        backend = GCPSecretManagerBackend(provider_proj)
                    else:
                        _base_logger.warning(f"Secrets backend '{provider_name}' not supported for GCP credentials.", extra={'client_type': self.client_type})
                        continue

                    credentials_json = await backend.get_secret(self.credentials_secret_id)
                    self._temp_credentials_path = os.path.join(tempfile.gettempdir(), f"gcp_sa_key_{uuid.uuid4().hex}.json")
                    ...
                    self.logger.info(f"GCP credentials loaded from secrets backend: {provider_name} into temporary file.", extra=self.logger.extra)
                    return
                except Exception as e:
                    last_exc = e
                    _base_logger.warning(f"Failed to fetch GCP credentials from {provider_name}: {e}", exc_info=True, extra={'client_type': self.client_type})
                finally:
                    if backend:
                        try:
                            await backend.close()
            ...
            alert_operator("CRITICAL: Failed to load GCP credentials from secrets.", level="CRITICAL")
            raise SIEMClientConfigurationError("Failed to load GCP credentials from any configured secrets backend.", self.client_type, original_exception=last_exc)
    ...
    async def _get_gcp_client(self) -> gcp_logging_sdk.Client:
        ...
        if self._logging_client is None:
            ...
            if self.credentials_path:
                self.logger.debug(f"Initializing GCP client with credentials from {self.credentials_path}", extra=self.logger.extra)
                self._credentials = await asyncio.shield(self._run_blocking_in_executor(lambda: service_account.Credentials.from_service_account_file(self.credentials_path)))
                self._logging_client = await asyncio.shield(self._run_blocking_in_executor(lambda: gcp_logging_sdk.Client(project=self.project_id, credentials=self._credentials)))
            else:
                self.logger.debug("Initializing GCP client using default application credentials.", extra=self.logger.extra)
                self._logging_client = await asyncio.shield(self._run_blocking_in_executor(lambda: gcp_logging_sdk.Client(project=self.project_id)))
    ...
    async def _perform_health_check_logic(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            def _probe():
                it = client.list_entries(page_size=1, filter=f'logName="projects/{self.project_id}/logs/{encoded_log}"')
                for _ in it.pages:
                    break
                return True

            await asyncio.shield(self._run_blocking_in_executor(_probe))
            self.logger.info(f"GCP project '{self.project_id}' and log '{self.log_name}' are accessible.", extra=self.logger.extra)
            return True, "Successfully connected to GCP Cloud Logging."
        except Forbidden as e:
            alert_operator(f"CRITICAL: GCP permission denied during health check: {e}", level="CRITICAL")
            raise SIEMClientAuthError(f"GCP permission denied: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except NotFound as e:
            alert_operator(f"CRITICAL: GCP Project '{self.project_id}' or Log '{self.log_name}' not found during health check: {e}", level="CRITICAL")
            raise SIEMClientConfigurationError(f"GCP project or log not found: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except GoogleAPICallError as e:
            alert_operator(f"CRITICAL: GCP API error during health check: {e}", level="CRITICAL")
            raise SIEMClientConnectivityError(f"GCP API error: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except Exception as e:
            alert_operator(f"CRITICAL: Unexpected error during GCP Cloud Logging health check: {e}", level="CRITICAL")
            raise SIEMClientError(f"Unexpected error during GCP Cloud Logging health check: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_log_logic(self, log_entry: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        raise SIEMClientPublishError(f"Failed to send log to GCP Cloud Logging: {failed_logs[0]['error']}", self.client_type, details=failed_logs[0], correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_logs_batch_logic(self, log_entries: List[Dict[str, Any]]) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        batches = [log_entries[i:i + self.MAX_BATCH_SIZE] for i in range(0, len(log_entries), self.MAX_BATCH_SIZE)]
        ...
        for batch in batches:
            ...
            try:
                ...
                alert_operator(f"CRITICAL: GCP permission denied while sending batch: {e}", level="CRITICAL")
                failed_logs.extend([{"log": log, "error": str(e)} for log in batch])
            except GoogleAPICallError as e:
                if _is_transient_gcp_error(e):
                    alert_operator(f"ERROR: Transient GCP API error while sending batch. Will retry: {e}", level="ERROR")
                    raise SIEMClientConnectivityError(f"Transient GCP API error while sending batch: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
                alert_operator(f"CRITICAL: GCP API error while sending batch: {e}", level="CRITICAL")
                failed_logs.extend([{"log": log, "error": str(e)} for log in batch])
            except Exception as e:
                alert_operator(f"CRITICAL: GCP Cloud Logging batch send failed: {e}", level="CRITICAL")
                failed_logs.extend([{"log": log, "error": str(e)} for log in batch])
    ...
    async def _perform_query_logs_logic(self, query_string: str, time_range: str, limit: int) -> List[Dict[str, Any]]:
        ...
        def _fetch_entries():
            ...
            iterator = client.list_entries(filter=full_filter, order_by="timestamp desc", page_size=page_size)
            results = []
        ...
        try:
            ...
            raise SIEMClientAuthError(f"GCP permission denied during query: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except NotFound as e:
            alert_operator(f"CRITICAL: GCP Project '{self.project_id}' or Log '{self.log_name}' not found during query: {e}", level="CRITICAL")
            raise SIEMClientConfigurationError(f"GCP project or log not found during query: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except GoogleAPICallError as e:
            if _is_transient_gcp_error(e):
                alert_operator(f"ERROR: Transient GCP API error during query. Will retry: {e}", level="ERROR")
                raise SIEMClientConnectivityError(f"Transient GCP API error during query: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            alert_operator(f"CRITICAL: GCP API error during query: {e}", level="CRITICAL")
            raise SIEMClientQueryError(f"GCP API error during query: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
        except SIEMClientError:
            raise
        except Exception as e:
            alert_operator(f"CRITICAL: Unexpected error during GCP Cloud Logging query: {e}", level="CRITICAL")
            raise SIEMClientQueryError(f"Unexpected error during GCP Cloud Logging query: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def close(self):
        ...
        if self._temp_credentials_path and os.path.exists(self._temp_credentials_path):
            try:
                self.logger.info(f"Cleaning up temporary GCP credentials file: {self._temp_credentials_path}", extra=self.logger.extra)
                os.remove(self._temp_credentials_path)
            except Exception as e:
                _base_logger.warning(f"Failed to remove temporary GCP credentials file: {e}", extra=self.logger.extra)
            finally:
