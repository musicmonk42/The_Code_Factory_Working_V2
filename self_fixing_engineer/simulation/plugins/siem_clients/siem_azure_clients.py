...
try:
    ...
    # Let factory catch ImportError
    raise ImportError("azure-servicebus or azure-identity not found. Azure Service Bus client is unavailable.") from e
...
try:
    ...
    # Let factory catch ImportError
    raise ImportError("azure-monitor-query not found. Azure Sentinel KQL querying is unavailable.") from e
...
try:
    ...
    _base_logger.warning("azure-keyvault-secrets not found. Azure Key Vault secrets backend will not be available.")
...
class AzureKeyVaultBackend(SecretsBackend):
    ...
    async def get_secret(self, secret_id: str) -> str:
        try:
            ...
            raise SIEMClientConfigurationError(f"Failed to fetch secret from Azure Key Vault: {e}", "AzureClient", original_exception=e)
...
class AzureSentinelConfig(BaseModel):
    ...
    monitor_query_endpoint: Optional[HttpUrl] = Field(None, description="Endpoint for Azure Monitor Query API.")
    use_aad_for_query: bool = Field(True, description="Use Azure AD (Managed Identity/Service Principal) for KQL querying.")
    ...
    def validate_workspace_id_not_dummy(cls, v):
        if PRODUCTION_MODE and not re.match(r"^[0-9a-fA-F]{8}(-[0-9a-fA-F]{4}){3}-[0-9a-fA-F]{12}$", v):
    ...
    def validate_log_type_format(cls, v):
        ...
        if not re.match(r"^[a-zA-Z0-9_]+$", v):
            raise ValueError("Invalid Log Type format. Must contain only letters, numbers, and underscores.")
        return v
    ...
    def validate_shared_key_source(cls, v, values):
        if PRODUCTION_MODE:
            if not values.get("shared_key_secret_id"):
                raise ValueError("In PRODUCTION_MODE, 'shared_key' must be loaded via 'shared_key_secret_id'. Direct key/ENV are forbidden.")
            if v and ("dummy" in v.lower() or "fake" in v.lower() or "test" in v.lower() or "mock" in v.lower() or not re.match(r"^[A-Za-z0-9+/=]+$", v)):
                raise ValueError("Dummy/fake/invalid Shared Key format detected. Not allowed in production.")
        return v
    ...
    def validate_secrets_providers_list(cls, v, values):
        if values.get("shared_key_secret_id"):
            if not v:
                raise ValueError("secrets_providers list must not be empty if shared_key_secret_id is provided.")
            for provider in v:
...
class AzureEventGridConfig(BaseModel):
    ...
    def validate_endpoint_not_dummy(cls, v):
        if PRODUCTION_MODE and ("dummy" in str(v).lower() or "test" in str(v).lower() or "mock" in str(v).lower() or "example.com" in str(v).lower()):
    ...
    def validate_key_source(cls, v, values):
        if PRODUCTION_MODE:
            if not values.get("key_secret_id"):
                raise ValueError("In PRODUCTION_MODE, 'key' must be loaded via 'key_secret_id'. Direct key/ENV are forbidden.")
            if v and ("dummy" in v.lower() or "fake" in v.lower() or "test" in v.lower() or "mock" in v.lower()):
    ...
    def validate_secrets_providers_list(cls, v, values):
        if values.get("key_secret_id"):
            if not v:
                raise ValueError("secrets_providers list must not be empty if key_secret_id is provided.")
            for provider in v:
                if provider not in ("aws", "azure", "gcp"):
                    raise ValueError(f"Invalid secrets_provider: {provider}. Must be one of 'aws', 'azure', 'gcp'.")
        return v
    ...
    def validate_topic_name_format(cls, v):
        if PRODUCTION_MODE and not re.match(r"^[a-zA-Z0-9-]{3,50}$", v):
            raise ValueError(f"Invalid topic name format for production: {v}. Must be 3-50 characters, letters, numbers, and hyphens.")
        return v
...
class AzureServiceBusConfig(BaseModel):
    ...
    def validate_connection_string_source(cls, v, values):
        if PRODUCTION_MODE:
            if not values.get("connection_string_secret_id") and not values.get("namespace_fqdn"):
                raise ValueError("In PRODUCTION_MODE, either 'connection_string_secret_id' or 'namespace_fqdn' must be provided. Direct string/ENV are forbidden.")
            if v and ("dummy" in v.lower() or "test" in v.lower() or "mock" in v.lower()):
                raise ValueError("Dummy/test Connection String detected. Not allowed in production.")
        return v
    ...
    def validate_namespace_fqdn_not_dummy(cls, v):
        if PRODUCTION_MODE and v and ("dummy" in v.lower() or "test" in v.lower() or "mock" in v.lower() or "example.com" in v.lower()):
    ...
    def validate_name_format(cls, v, field):
        if PRODUCTION_MODE and v and not re.match(r"^[a-zA-Z0-9-._]{1,260}$", v):
            raise ValueError(f"Invalid {field.name} format for production: {v}. Must be 1-260 characters, letters, numbers, hyphens, dots, or underscores.")
        return v
...
class AzureSentinelClient(AiohttpClientMixin, BaseSIEMClient):
    ...
    def __init__(self, config: Dict[str, Any], metrics_hook: Optional[Callable] = None, paranoid_mode: bool = False):
        ...
        try:
            ...
            validated_config = AzureSentinelConfig(**azure_sentinel_config_data).dict(exclude_unset=True)
        except ValidationError as e:
            _base_logger.critical(f"CRITICAL: Invalid Azure Sentinel client configuration: {e}.")
            _notify_ops(f"CRITICAL: Invalid Azure Sentinel client configuration: {e}.", level="CRITICAL")
            raise SIEMClientConfigurationError(f"Invalid Azure Sentinel client configuration: {e}", self.client_type, original_exception=e)
    ...
    async def _ensure_shared_key_loaded(self):
        ...
        if not self.shared_key_secret_id:
            raise SIEMClientConfigurationError("Shared key not configured; shared_key_secret_id is required.", self.client_type)
        ...
        for provider in self.secrets_providers:
            try:
                if provider == "azure":
                    ...
                    if not vault_url:
                        raise ValueError("Azure secrets provider configured but vault_url is missing.")
                    ...
                    _base_logger.warning(f"Secrets backend '{provider}' not supported for Azure Sentinel shared key.", extra={'client_type': self.client_type})
            except Exception as e:
                last_exc = e
                _base_logger.warning(f"Failed to fetch Azure Sentinel shared key from {provider}: {e}", exc_info=True, extra={'client_type': self.client_type})
        _notify_ops("CRITICAL: Failed to load Azure Sentinel shared key from secrets.", level="CRITICAL")
        raise SIEMClientConfigurationError("Failed to load Azure Sentinel shared key from any configured secrets backend.", self.client_type, original_exception=last_exc)
    ...
    async def _get_logs_query_client(self) -> LogsQueryClient:
        ...
        if self._logs_query_client is None:
            if not self.use_aad_for_query:
                _notify_ops("CRITICAL: Azure Sentinel KQL querying requires Azure AD.", level="CRITICAL")
                raise SIEMClientConfigurationError("KQL querying without Azure AD is not supported for security reasons.", self.client_type)
            self._azure_monitor_credential = AzureMonitorCredential()
            self._logs_query_client = LogsQueryClient(self._azure_monitor_credential, endpoint=self.monitor_query_endpoint)
            self.logger.info("Azure Monitor Query client initialized with Azure AD credentials.", extra=self.logger.extra)
    ...
    async def _perform_health_check_logic(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            hashed_string = hmac.new(decoded_shared_key, string_to_sign.encode('utf-8'), hashlib.sha256).digest()
            ...
            async with asyncio.shield(session.post(api_url, headers=headers, data=dummy_body, timeout=self.timeout)) as response:
                if response.status not in [200, 202]:
                    response_text = await response.text()
                    _notify_ops(f"CRITICAL: Azure Sentinel Data Collector API health check failed: {response.status}: {response_text}", level="CRITICAL")
                    raise SIEMClientResponseError(f"Azure Sentinel Data Collector API responded with status {response.status}: {response_text}", self.client_type, response.status, response_text, correlation_id=self.logger.extra.get('correlation_id'))
            self.logger.info("Azure Sentinel Data Collector API is reachable.", extra=self.logger.extra)

            # Test KQL client connectivity (query path)
            client = await self._get_logs_query_client()
            await asyncio.shield(client.query_workspace(
                workspace_id=self.workspace_id,
                query="Heartbeat | take 1",
                timespan=(datetime.datetime.utcnow() - datetime.timedelta(minutes=5), datetime.datetime.utcnow()),
            ...
            raise SIEMClientConnectivityError(f"Azure Sentinel health check failed: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_log_logic(self, log_entry: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        raise SIEMClientPublishError(f"Failed to send log to Azure Sentinel: {failed_logs[0]['error']}", self.client_type, details=failed_logs[0], correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_logs_batch_logic(self, log_entries: List[Dict[str, Any]]) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        for batch_events in batches:
            ...
            try:
                ...
                _notify_ops(f"CRITICAL: Invalid Shared Key format during batch send: {e}.", level="CRITICAL")
                raise SIEMClientAuthError(f"Invalid Shared Key format: {e}. Must be Base64 encoded.", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            ...
            try:
                async with asyncio.shield(session.post(api_url, headers=headers, data=body, timeout=self.timeout)) as response:
                    status_code = response.status
                    response_text = await response.text()
                    if status_code >= 400:
                        _notify_ops(f"CRITICAL: Azure Sentinel Data Collector API rejected batch with status {status_code}: {response_text}", level="CRITICAL")
                        failed_logs.extend([{"log": log, "error": response_text} for log in batch_events])
                    else:
                        total_sent += len(batch_events)
            except Exception as e:
                _notify_ops(f"CRITICAL: Azure Sentinel batch log send failed: {e}", level="CRITICAL")
        ...
        return True, f"Batch of {len(log_entries)} logs sent to Azure Sentinel/Log Analytics.", []
    ...
    async def _perform_query_logs_logic(self, query_string: str, time_range: str, limit: int) -> List[Dict[str, Any]]:
        ...
        if not self.workspace_id:
            _notify_ops("CRITICAL: Azure Workspace ID is required for KQL querying.", level="CRITICAL")
            raise SIEMClientConfigurationError("Azure Workspace ID is required for KQL querying.", self.client_type, correlation_id=self.logger.extra.get('correlation_id'))
        if not self.use_aad_for_query:
            _notify_ops("CRITICAL: KQL querying without Azure AD is not supported.", level="CRITICAL")
            raise SIEMClientConfigurationError("KQL querying without Azure AD is not supported for security reasons.", self.client_type, correlation_id=self.logger.extra.get('correlation_id'))
        ...
        try:
            ...
            response = await asyncio.shield(client.query_workspace(
                workspace_id=self.workspace_id,
            ...
            if any(x in str(e) for x in ("AuthenticationFailed", "Unauthorized", "AADSTS")):
                _notify_ops(f"CRITICAL: Azure Monitor Query authentication failed: {e}", level="CRITICAL")
                raise SIEMClientAuthError(f"Azure Monitor Query authentication failed: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            elif any(x in str(e) for x in ("Connection refused", "Failed to establish a new connection")):
                _notify_ops(f"CRITICAL: Azure Monitor Query connection error: {e}", level="CRITICAL")
                raise SIEMClientConnectivityError(f"Azure Monitor Query connection error: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            else:
                _notify_ops(f"CRITICAL: Failed to query Azure Sentinel: {e}", level="CRITICAL")
                raise SIEMClientQueryError(f"Failed to query Azure Sentinel: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def close(self):
        ...
        if self._logs_query_client or self._azure_monitor_credential:
            self.logger.debug(f"{self.client_type} LogsQueryClient and credential closed.", extra=self.logger.extra)
...
class AzureEventGridClient(BaseSIEMClient):
    ...
    def __init__(self, config: Dict[str, Any], metrics_hook: Optional[Callable] = None, paranoid_mode: bool = False):
        ...
        try:
            ...
            validated_config = AzureEventGridConfig(**eventgrid_config_data).dict(exclude_unset=True)
        except ValidationError as e:
            _base_logger.critical(f"CRITICAL: Invalid Azure Event Grid client configuration: {e}.")
            _notify_ops(f"CRITICAL: Invalid Azure Event Grid client configuration: {e}.", level="CRITICAL")
            raise SIEMClientConfigurationError(f"Invalid Azure Event Grid client configuration: {e}", self.client_type, original_exception=e)
        ...
        self._event_grid_publisher_client = EventGridPublisherClient(
            ...
            credential=AzureKeyCredential(self.key or "DUMMY")  # placeholder; will set real key before send
        )
        self.logger.extra.update({'endpoint': str(self.endpoint), 'topic': validated_config["topic_name"]})
    ...
    async def _ensure_key_loaded(self):
        ...
        if not self.key_secret_id:
            raise SIEMClientConfigurationError("Event Grid key not configured; key_secret_id is required.", self.client_type)
        last_exc: Optional[Exception] = None
        for provider in self.secrets_providers:
            try:
                if provider == "azure":
                    ...
                    _base_logger.warning(f"Secrets backend '{provider}' not supported for Event Grid key.", extra={'client_type': self.client_type})
            except Exception as e:
                last_exc = e
                _base_logger.warning(f"Failed to fetch Event Grid key from {provider}: {e}", exc_info=True, extra={'client_type': self.client_type})
        _notify_ops("CRITICAL: Failed to load Event Grid key from secrets.", level="CRITICAL")
        raise SIEMClientConfigurationError("Failed to load Event Grid key from any configured secrets backend.", self.client_type, original_exception=last_exc)
    ...
    async def _perform_health_check_logic(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            await asyncio.shield(self._run_blocking_in_executor(lambda: self._event_grid_publisher_client.send([dummy_event])))
            self.logger.info(f"Successfully sent dummy CloudEvent for {self.client_type} health check.", extra=self.logger.extra)
            return True, "Azure Event Grid client initialized and connectivity tested via dummy send."
        except Exception as e:
            _notify_ops(f"CRITICAL: Azure Event Grid health check failed: {e}", level="CRITICAL")
            raise SIEMClientConnectivityError(f"Azure Event Grid health check failed: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_log_logic(self, log_entry: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            await asyncio.shield(self._run_blocking_in_executor(lambda: self._event_grid_publisher_client.send([cloud_event])))
            ...
            if "AuthenticationFailed" in str(e) or "Unauthorized" in str(e):
                raise SIEMClientAuthError(f"Azure Event Grid authentication failed: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            elif "Connection refused" in str(e) or "Failed to establish a new connection" in str(e):
                raise SIEMClientConnectivityError(f"Azure Event Grid connection error: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            else:
                raise SIEMClientPublishError(f"Failed to publish log to Azure Event Grid: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_logs_batch_logic(self, log_entries: List[Dict[str, Any]]) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        for log_entry in log_entries:
            ...
            subject = f"sfe/audit/{event_type}"
            event_time = datetime.datetime.utcnow().isoformat() + "Z"
            batch_events.append(CloudEvent(
                id=event_id,
                source=f"/sfesystem/client/{self.client_type}",
                data=log_entry,
                type=event_type,
        ...
        try:
            await asyncio.shield(self._run_blocking_in_executor(lambda: self._event_grid_publisher_client.send(batch_events)))
            return True, f"Batch of {len(log_entries)} logs published to Azure Event Grid.", []
        except Exception as e:
            _notify_ops(f"CRITICAL: Azure Event Grid batch log send failed: {e}", level="CRITICAL")
            raise SIEMClientPublishError(f"Failed to publish Azure Event Grid batch: {e}", self.client_type, original_exception=e, details={"batch_size": len(log_entries)}, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100, correlation_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        raise NotImplementedError(f"Querying is not supported by {self.client_type} client (event bus).")
...
class AzureServiceBusClient(BaseSIEMClient):
    ...
    def __init__(self, config: Dict[str, Any], metrics_hook: Optional[Callable] = None, paranoid_mode: bool = False):
        ...
        try:
            ...
            validated_config = AzureServiceBusConfig(**servicebus_config_data).dict(exclude_unset=True)
        except ValidationError as e:
            _base_logger.critical(f"CRITICAL: Invalid Azure Service Bus client configuration: {e}.")
            _notify_ops(f"CRITICAL: Invalid Azure Service Bus client configuration: {e}.", level="CRITICAL")
            raise SIEMClientConfigurationError(f"Invalid Azure Service Bus client configuration: {e}", self.client_type, original_exception=e)
    ...
    async def _ensure_connection_string_loaded(self):
        ...
        if not self.connection_string_secret_id:
            raise SIEMClientConfigurationError("Service Bus connection string not configured; connection_string_secret_id or namespace_fqdn required.", self.client_type)
        last_exc: Optional[Exception] = None
        for provider in self.secrets_providers:
            try:
                if provider == "azure":
                    ...
                    backend = AzureKeyVaultBackend(vault_url=vault_url)
                    try:
                        self.connection_string = await backend.get_secret(self.connection_string_secret_id)
                        return
                    finally:
                        await backend.close()
                else:
                    _base_logger.warning(f"Secrets backend '{provider}' not supported for Service Bus connection string.", extra={'client_type': self.client_type})
            except Exception as e:
                last_exc = e
                _base_logger.warning(f"Failed to fetch Service Bus connection string from {provider}: {e}", exc_info=True, extra={'client_type': self.client_type})
        _notify_ops("CRITICAL: Failed to load Service Bus connection string from secrets.", level="CRITICAL")
        raise SIEMClientConfigurationError("Failed to load Service Bus connection string from any configured secrets backend.", self.client_type, original_exception=last_exc)
    ...
    async def _get_servicebus_client(self) -> ServiceBusClient:
        ...
        if self._service_bus_client is None or self._service_bus_client.closed:
            ...
            if self.connection_string:
                self.logger.info("Initializing Azure Service Bus client with connection string.", extra=self.logger.extra)
                # Creation is cheap; do not offload to executor
                self._service_bus_client = ServiceBusClient.from_connection_string(self.connection_string)
            elif self.namespace_fqdn:
                self.logger.info("Initializing Azure Service Bus client with Azure AD credentials (namespace FQDN).", extra=self.logger.extra)
                credential = AioDefaultAzureCredential()
                self._service_bus_client = ServiceBusClient(fully_qualified_namespace=self.namespace_fqdn, credential=credential)
            else:
                _notify_ops("CRITICAL: Azure Service Bus client cannot be initialized: No connection string or namespace FQDN provided.", level="CRITICAL")
                raise SIEMClientConfigurationError("Azure Service Bus client cannot be initialized: No connection string or namespace FQDN provided.", self.client_type, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_health_check_logic(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.queue_name:
                ...
                self.logger.info(f"Azure Service Bus Queue '{self.queue_name}' is reachable.", extra=self.logger.extra)
                ...
                async with sender:
                    ...
                    pass
                self.logger.info(f"Azure Service Bus Topic '{self.topic_name}' is reachable.", extra=self.logger.extra)
            else:
                _notify_ops("CRITICAL: Azure Service Bus client configured without queue or topic name.", level="CRITICAL")
                return False, "No queue or topic configured."
            return True, "Azure Service Bus is reachable."
        except Exception as e:
            _notify_ops(f"CRITICAL: Azure Service Bus health check failed: {e}", level="CRITICAL")
            raise SIEMClientConnectivityError(f"Failed to connect to Azure Service Bus: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_log_logic(self, log_entry: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        try:
            if self.queue_name:
                ...
                _notify_ops("CRITICAL: Azure Service Bus client configured without queue or topic name for sending.", level="CRITICAL")
                raise SIEMClientConfigurationError("Neither queue_name nor topic_name is configured for Azure Service Bus.", self.client_type, correlation_id=self.logger.extra.get('correlation_id'))
        except Exception as e:
            ...
            if any(x in str(e) for x in ("Authentication", "Unauthorized", "AADSTS")):
                raise SIEMClientAuthError(f"Azure Service Bus authentication failed: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            elif any(x in str(e) for x in ("Connection", "Host")):
                raise SIEMClientConnectivityError(f"Azure Service Bus connection error: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
            else:
                raise SIEMClientPublishError(f"Failed to send log to Azure Service Bus: {e}", self.client_type, original_exception=e, correlation_id=self.logger.extra.get('correlation_id'))
    ...
    async def _perform_send_logs_batch_logic(self, log_entries: List[Dict[str, Any]]) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        try:
            if self.queue_name:
                ...
                _notify_ops("CRITICAL: Azure Service Bus client configured without queue or topic name for batch sending.", level="CRITICAL")
                raise SIEMClientConfigurationError("Neither queue_name nor topic_name is configured for Azure Service Bus.", self.client_type, correlation_id=self.logger.extra.get('correlation_id'))

            async with sender:
                messages = [ServiceBusMessage(json.dumps(log).encode('utf-8')) for log in log_entries]
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100, correlation_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        raise NotImplementedError(f"Querying is not supported by {self.client_type} client (message bus).")

    async def close(self):
        """Closes the Azure Service Bus client."""
        await super().close()
        if self._service_bus_client and not self._service_bus_client.closed:
            self.logger.info(f"Closing Service Bus client for {self.client_type}.", extra=self.logger.extra)
