...
try:
    ...
    _base_logger.error(f"Generic SIEM clients unavailable: {e}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    alert_operator(f"Generic SIEM clients unavailable: {e}", level="ERROR")

try:
    from .siem_aws_clients import AwsCloudWatchClient
except ImportError as e:
    _base_logger.error(f"AWS SIEM clients unavailable: {e}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    alert_operator(f"AWS SIEM clients unavailable: {e}", level="ERROR")

try:
    from .siem_gcp_clients import GcpLoggingClient
except ImportError as e:
    _base_logger.error(f"GCP SIEM clients unavailable: {e}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    alert_operator(f"GCP SIEM clients unavailable: {e}", level="ERROR")

try:
    from .siem_azure_clients import AzureSentinelClient, AzureEventGridClient, AzureServiceBusClient
except ImportError as e:
    _base_logger.error(f"Azure SIEM clients unavailable: {e}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
...
if SplunkClient:            SIEM_CLIENT_REGISTRY["splunk"] = SplunkClient
if ElasticClient:           SIEM_CLIENT_REGISTRY["elastic"] = ElasticClient
if DatadogClient:           SIEM_CLIENT_REGISTRY["datadog"] = DatadogClient
if AzureSentinelClient:     SIEM_CLIENT_REGISTRY["azure_sentinel"] = AzureSentinelClient
if AwsCloudWatchClient:     SIEM_CLIENT_REGISTRY["aws_cloudwatch"] = AwsCloudWatchClient
if GcpLoggingClient:        SIEM_CLIENT_REGISTRY["gcp_logging"] = GcpLoggingClient
if AzureEventGridClient:    SIEM_CLIENT_REGISTRY["azure_event_grid"] = AzureEventGridClient
if AzureServiceBusClient:   SIEM_CLIENT_REGISTRY["azure_service_bus"] = AzureServiceBusClient
...
def get_siem_client(
    ...
    if PRODUCTION_MODE and not paranoid_mode:
        ...
        _base_logger.critical(f"CRITICAL: {msg}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    ...
    try:
        client_instance = client_class(config, metrics_hook=metrics_hook, paranoid_mode=paranoid_mode)
        _base_logger.info(f"Successfully initialized SIEM client: {siem_type}", extra={'client_type': siem_type, 'correlation_id': 'N/A'})
        return client_instance
    except SIEMClientConfigurationError:
        # Already logged/alerted at origin
        _base_logger.critical(f"CRITICAL: SIEM client '{siem_type}' failed configuration.", exc_info=True, extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
        raise
    except Exception as e:
        msg = f"Failed to initialize '{siem_type}' client due to unexpected error: {e}"
        _base_logger.critical(f"CRITICAL: {msg}", exc_info=True, extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
        alert_operator(f"CRITICAL: {msg}", level="CRITICAL")
...
def list_available_siem_clients() -> List[Dict[str, Any]]:
    ...
    """
    _base_logger.info("Listing available SIEM clients and their dependency status.", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    ...
    _base_logger.info("SIEM Client Availability Report:", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
    for client_info in available_clients_info:
        status_msg = "Available" if client_info["is_available"] else "Unavailable"
        deps_msg = f" (Missing/Error: {', '.join(client_info['required_dependencies_status'])})" if not client_info["is_available"] else ""
        _base_logger.info(f"  - {client_info['type']}: {status_msg}{deps_msg}", extra={'client_type': 'SIEM_Factory', 'correlation_id': 'N/A'})
