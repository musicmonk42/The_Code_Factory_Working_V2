...
from .siem_factory import get_siem_client, list_available_siem_clients, SIEM_CLIENT_REGISTRY
from .siem_base import (
    SIEMClientConfigurationError, SIEMClientError, _base_logger, PRODUCTION_MODE,
    alert_operator, SECRETS_MANAGER,
)
...
try:
    ...
    _base_logger.warning("asyncclick library not found. Production CLI commands will not be available.")
...
_SCRUB_KEY_REGEX = re.compile(r"(password|passwd|secret|api[_-]?key|token|connection[_-]?string|shared[_-]?key|private[_-]?key|client[_-]?secret)", re.IGNORECASE)
...
def _scrub_obj(obj: Any) -> Any:
    ...
    try:
        ...
        if isinstance(obj, str):
            ...
            s = obj
            s = re.sub(r'(?i)(api[_-]?key|token|password|secret)\s*[:=]\s*["\']?([A-Za-z0-9_\-\.=/+]{6,})', r'\1=***REDACTED***', s)
            return s
        return obj
    except Exception:
        return obj
...
async def run_tests():
    ...
    if os.getenv("RUN_SIEM_TESTS", "false").lower() != "true":
        _base_logger.critical("CRITICAL: siem_main.py (test runner) is attempting to execute without 'RUN_SIEM_TESTS=true' environment flag. Aborting.")
        alert_operator("CRITICAL: siem_main.py (test runner) is attempting to execute without explicit test flag. Aborting.", level="CRITICAL")
        sys.exit(1)
    ...
    for client_info in list_available_siem_clients():
        print(f"  Type: {client_info['type']}, Available: {client_info['is_available']}, Requires: {', '.join(client_info['required_dependencies_status'])}")
    ...
    test_config = {
        ...
        "rate_limit_burst": 2,
        "secret_scrub_patterns": [r"password", r"api_key", r"secret_info", r"connection_string"],  # Example scrubbing
        "paranoid_mode": True,  # Ensure paranoid mode is ON for tests
        "splunk": {
            "url": os.getenv("SIEM_SPLUNK_HEC_URL", "http://dummy-splunk:8088/services/collector/event"),
        ...
        "datadog": {
            "url": os.getenv("SIEM_DATADOG_API_URL", "https://dummy-intake.logs.datadoghq.com/api/v2/logs"),
            "query_url": os.getenv("SIEM_DATADOG_QUERY_URL", "https://dummy-api.datadoghq.com/api/v1/logs-queries"),
        ...
        "azure_sentinel": {
            ...
            "shared_key": os.getenv("SIEM_AZURE_SHARED_KEY", base64.b64encode(b"dummy_shared_key").decode()),
        ...
        "azure_event_grid": {
            "endpoint": os.getenv("SIEM_AZURE_EVENTGRID_ENDPOINT", "https://dummy-topic.eastus-1.eventgrid.azure.net/api/events"),
            "key": os.getenv("SIEM_AZURE_EVENTGRID_KEY", "dummy_eventgrid_key"),
        },
        "azure_service_bus": {
            "connection_string": os.getenv("SIEM_AZURE_SERVICEBUS_CONNECTION_STRING", "Endpoint=sb://dummy.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=dummy_key"),
    ...
    for client_type, client_config in test_config.items():
        if isinstance(client_config, dict):
            for key, value in client_config.items():
                if isinstance(value, str) and ("dummy" in value.lower() or "mock" in value.lower() or "test" in value.lower() or "example.com" in value.lower()):
                    _base_logger.critical(f"CRITICAL: Dummy/test config value detected for {client_type}.{key}: {value}. Aborting test run.")
                    alert_operator(f"CRITICAL: Dummy/test config value detected in SIEM test config for {client_type}.{key}. Aborting.", level="CRITICAL")
                    sys.exit(1)
    ...
    test_log_entry = {
        ...
        "user_id": "testuser",
        "details": {"test_field": "test_value", "sensitive_info": "my_secret_password_123", "api_key_data": "ak-12345-xyz"}
    }
    ...
    for siem_type in SIEM_CLIENT_REGISTRY.keys():  # Iterate through registered types
        ...
        client_info = next((info for info in list_available_siem_clients() if info['type'] == siem_type), None)
        if client_info and not client_info['is_available']:
            print(f"Skipping {siem_type} client test: Dependencies not met. Required: {', '.join(client_info['required_dependencies_status'])}")
            continue

        try:
            correlation_id = str(uuid.uuid4())
            async with get_siem_client(siem_type, test_config, metrics_hook=test_metrics_hook) as client:
                ...
                send_success, send_msg = await client.send_log(test_log_entry, correlation_id=correlation_id)
                print(f"Single Log Send: {send_success} - {send_msg}")

                # Test send_logs (batch)
                batch_log_entries = [
                    {**test_log_entry, "message": "Batch log 1", "event_id": str(uuid.uuid4())},
                    {**test_log_entry, "message": "Batch log 2", "severity": "WARNING", "event_id": str(uuid.uuid4())},
                    {**test_log_entry, "message": "Batch log 3 (sensitive)", "details": {"payment_info": "1111-2222-3333-4444", "private_key": "abc-xyz-123"}, "event_id": str(uuid.uuid4())},
                ]
                batch_success, batch_msg, failed_logs = await client.send_logs(batch_log_entries, correlation_id=correlation_id)
                ...
                try:
                    ...
                    query_results = await client.query_logs(query_string_test, "1h", 2, correlation_id=correlation_id)
                    ...
                    print(f"Query Error for {siem_type}: {_scrub_and_dump({'error': qe.args[0], 'correlation_id': getattr(qe, 'correlation_id', None)})}")
                except Exception as exc:
                    print(f"Unexpected error during query for {siem_type}: {_scrub_and_dump(str(exc))}")
            ...
            print(f"SIEM Client Error for {siem_type}: {e.client_type} - {_scrub_and_dump(e.args[0])} (Correlation ID: {e.correlation_id})")
...
if CLICK_AVAILABLE:
    ...
    async def cli():
        ...
        if not PRODUCTION_MODE:
            _base_logger.critical("CRITICAL: Production CLI commands are restricted to PRODUCTION_MODE. Aborting.")
            alert_operator("CRITICAL: Production CLI commands are restricted to PRODUCTION_MODE. Aborting.", level="CRITICAL")
            # Using click's fail-fast
            raise click.ClickException("Production CLI commands are restricted to PRODUCTION_MODE.")

    @cli.command("health-check")
    @click.option("--siem-type", required=True, type=click.Choice(list(SIEM_CLIENT_REGISTRY.keys())), help="The type of SIEM client to check (e.g., 'splunk', 'aws_cloudwatch').")
    @click.option("--config-file", required=True, type=click.Path(exists=True, dir_okay=False, readable=True), help="Path to a JSON configuration file.")
    async def health_check_command(siem_type, config_file):
        ...
        try:
            ...
            if not os.path.exists(hmac_file):
                raise SIEMClientConfigurationError(f"HMAC signature file not found for config: {hmac_file}", "Main")
            ...
            if not isinstance(hmac_secret, (str, bytes)):
                raise SIEMClientConfigurationError("HMAC secret is not a string/bytes value.", "Main")
            if isinstance(hmac_secret, str):
                hmac_secret = hmac_secret.encode('utf-8')

            generated_hmac = hmac.new(hmac_secret, config_data.encode('utf-8'), hashlib.sha256).hexdigest()

            if not hmac.compare_digest(generated_hmac, expected_hmac):
                raise SIEMClientConfigurationError("Config file integrity check failed: HMAC mismatch.", "Main")

            config = json.loads(config_data)

        except Exception as e:
            _base_logger.critical(f"CRITICAL: Failed to load or validate config file '{config_file}': {e}. Aborting.")
            alert_operator(f"CRITICAL: Failed to load or validate config file '{config_file}': {e}. Aborting.", level="CRITICAL")
            raise click.ClickException("Failed to load or validate config file.")

        try:
            async with get_siem_client(siem_type, config, metrics_hook=lambda *args, **kwargs: None) as client:
    ...
    @click.option("--siem-type", required=True, type=click.Choice(list(SIEM_CLIENT_REGISTRY.keys())), help="The type of SIEM client to use.")
    @click.option("--config-file", required=True, type=click.Path(exists=True, dir_okay=False, readable=True), help="Path to a JSON configuration file.")
    @click.option("--log-file", required=True, type=click.Path(exists=True, dir_okay=False, readable=True), help="Path to a single JSON log entry file.")
    ...
    @click.option("--siem-type", required=True, type=click.Choice(list(SIEM_CLIENT_REGISTRY.keys())), help="The type of SIEM client to use.")
    @click.option("--config-file", required=True, type=click.Path(exists=True, dir_okay=False, readable=True), help="Path to a JSON configuration file.")
    @click.option("--log-file", required=True, type=click.Path(exists=True, dir_okay=False, readable=True), help="Path to a JSON file containing a list of log entries.")
    ...
    async def query_logs_command(siem_type, config_file, query_string, time_range, limit):
        ...
        try:
            ...
            click.echo(f"Error: {siem_type} client does not support querying. {_scrub_and_dump(str(e))}", err=True)
...
async def main():
    ...
    if PRODUCTION_MODE:
        _base_logger.critical("CRITICAL: siem_main.py (test runner) is attempting to execute in PRODUCTION_MODE. Aborting for security.")
        alert_operator("CRITICAL: siem_main.py (test runner) is attempting to execute in PRODUCTION_MODE. Aborting.", level="CRITICAL")
        sys.exit(1)
