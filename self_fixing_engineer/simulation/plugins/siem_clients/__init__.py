...
class MockSIEMClient(SIEMClientBase):
    ...
    async def send_events_batch(self, events: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"success": success_count, "failed": failed_count, "errors": []}
        ...
        try:
            ...
            async with self._session.get(url, ssl=False) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"Splunk health check failed with status {response.status}"
                    )
                    return False
            ...
            if self._session:
                ...
                async with self._session.post(url, json=payload, ssl=False) as response:
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Failed to send event to Splunk: {response.status}"
                        )
                        self.events_failed += 1
                        return False
            else:
...
class CloudWatchSIEMClient(SIEMClientBase):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all([self.log_group, self.log_stream]):
            raise ValueError(
                "CloudWatch client requires log_group and log_stream in config"
            )
        ...
        try:
            ...
            if self._client:
                ...
                self._client.put_log_events(
                    ...
                    logEvents=[
                        {"timestamp": timestamp, "message": json.dumps(formatted_event)}
                    ],
                )
                self.events_sent += 1
                return True
                ...
                # Mock mode
                logger.debug(
                    f"Mock CloudWatch: Would send event to {self.log_group}/{self.log_stream}"
                )
                self.events_sent += 1
                return True
...
class AzureSentinelSIEMClient(SIEMClientBase):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all([self.workspace_id, self.shared_key]):
            raise ValueError(
                "Azure Sentinel client requires workspace_id and shared_key"
            )

    async def connect(self) -> bool:
        """Establish connection to Azure Sentinel."""
        logger.info(
            f"Azure Sentinel client connecting to workspace {self.workspace_id}"
        )
