...
try:
    ...
    logging.warning(
        "FastAPI or Pydantic not found. Web UI Dashboard Plugin will not function in standalone API mode."
    )
    ...
    for key, field in DashboardConfig.__annotations__.items():
        ...
        if env_var:
            try:
                ...
                logger.warning(
                    f"Invalid type for environment variable DASHBOARD_{key.upper()}. Using default."
                )
    ...
    if PYDANTIC_AVAILABLE:
        try:
            ...
            return DashboardConfig(
                plugin_manifest=default_manifest
            )  # Fallback with default manifest
    else:
        cfg = DashboardConfig()
...
if PROMETHEUS_AVAILABLE:
    DASHBOARD_API_CALLS = _get_or_create_metric(
        Counter, "dashboard_api_calls_total", "Total API calls", ["endpoint"]
    )
    ...
    DASHBOARD_STATE_UPDATES = _get_or_create_metric(
        Counter, "dashboard_state_updates_total", "Total dashboard state updates"
    )
    DASHBOARD_COMPONENT_RENDERS = _get_or_create_metric(
        Counter,
        "dashboard_component_renders_total",
        "Total component data renders",
        ["component_name"],
    )
else:
...
async def get_dashboard_state() -> Dict[str, Any]:
    ...
    if CONFIG.state_storage == "redis" and REDIS_AVAILABLE and CONFIG.redis_url:
        try:
            ...
            logger.warning(
                f"Redis unavailable or error fetching state: {e}. Falling back to memory state."
            )
            if PROMETHEUS_AVAILABLE:
                DASHBOARD_API_CALLS.labels(endpoint="get_state_redis_fallback").inc()
...
async def update_dashboard_state(update_data: Dict[str, Any]):
    ...
    if CONFIG.state_storage == "redis" and REDIS_AVAILABLE and CONFIG.redis_url:
        try:
            ...
            logger.error(
                f"Failed to update Redis state: {e}. Updating memory state only.",
                exc_info=True,
            )
...
def get_example_chart(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    chart_data = [
        {
            "x": i,
            "y": current_metric - (10 - i) * 0.2 + (data["live_data_counter"] % 10),
        }
        for i in range(10)
    ]
...
if FASTAPI_AVAILABLE:
    ...
    def validate_component_name(name: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", name):
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid component name"
            )
        return name
    ...
    async def dashboard_manifest(api_key: str = Depends(get_api_key)) -> Dict[str, Any]:
        ...
        if PROMETHEUS_AVAILABLE:
            DASHBOARD_API_CALLS.labels(endpoint="manifest").inc()
    ...
    async def update_dashboard_api_state(
        request_data: UpdateStateRequest, api_key: str = Depends(get_api_key)
    ) -> Dict[str, Any]:
        """Allows the frontend or other systems to update parts of the dashboard state."""
        if PROMETHEUS_AVAILABLE:
            DASHBOARD_API_CALLS.labels(endpoint="state_update").inc()
        logger.info(
            f"Dashboard state update request received: {_scrub_secrets(request_data.update)}"
        )
    ...
    async def get_dashboard_component(
        component_name: str, api_key: str = Depends(get_api_key)
    ) -> Dict[str, Any]:
        """Fetches the configuration and data for a specific UI component."""
        if PROMETHEUS_AVAILABLE:
            DASHBOARD_API_CALLS.labels(endpoint=f"component_{component_name}").inc()
        ...
        try:
            ...
            if PROMETHEUS_AVAILABLE:
                DASHBOARD_COMPONENT_RENDERS.labels(component_name=component_name).inc()
            return _scrub_secrets(data)
        except Exception as e:
            logger.error(
                f"Error generating data for component '{component_name}': {e}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error generating component data: {e}",
            )
        ...
        try:
            ...
            try:
                ...
                await _send_websocket_update(
                    websocket,
                    {"type": "initial_state", "state": _scrub_secrets(initial_state)},
                )
                ...
                while not disconnect_event.is_set():
                    ...
                    current_state = await get_dashboard_state()
                    current_state["example_metric"] += 0.5 + (
                        current_state["live_data_counter"] * 0.01
                    )
                    ...
                    try:
                        await _send_websocket_update(
                            websocket,
                            {"type": "update", "state": _scrub_secrets(latest_state)},
                        )
                        logger.debug(
                            f"Pushed live update: example_metric={latest_state['example_metric']:.2f}"
                        )
...
if __name__ == "__main__":
    if not FASTAPI_AVAILABLE:
        print(
            "FastAPI is not installed. Cannot run the plugin's API in standalone mode.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    async def root():
        return {
            "message": "Web UI Dashboard Plugin Test Host is running. Access /plugin/dashboard/manifest"
        }
