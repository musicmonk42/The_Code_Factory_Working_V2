...
class SecretsManager:
    ...
    def get_secret(self, key: str, default: Optional[str] = None, required: bool = True) -> Optional[str]:
        ...
        if not secret_value and required:
            ...
            _schedule_alert(f"Critical: Missing required secret '{key}' for DLT client.", level="CRITICAL")
...
try:
    ...
    _base_logger.critical("CRITICAL: pydantic not found. Configuration validation is critical. Aborting startup.")
    _schedule_alert("CRITICAL: pydantic not found. DLT client cannot start without configuration validation.", level="CRITICAL")
    sys.exit(1)

try:
    from prometheus_client import Counter, Histogram, CollectorRegistry, generate_latest, Gauge
        ...
        with _metrics_lock:
            try:
                ...
                if isinstance(existing_metric, metric_type):
                    ...
                    _base_logger.warning(f"Metric '{name}' already registered with a different type. Reusing existing.")
                    return existing_metric
            except KeyError:
    ...
    operation_counter = get_or_create_metric(
        Counter, "dlt_client_operations_total", "Total number of DLT client operations",
        labelnames=["client_type", "operation", "status"]
    )
    operation_latency = get_or_create_metric(
        Histogram, "dlt_client_operation_latency_seconds", "Latency of DLT client operations in seconds",
        labelnames=["client_type", "operation"]
    )
    circuit_breaker_status = get_or_create_metric(
        Gauge, "dlt_client_circuit_breaker_status", "Status of DLT client circuit breaker (1=OPEN, 0=CLOSED)",
        labelnames=["client_type"]
    )
    circuit_breaker_failures = get_or_create_metric(
        Gauge, "dlt_client_circuit_breaker_failures", "Current consecutive failures for DLT client circuit breaker",
        labelnames=["client_type"]
    )
    audit_log_integrity_status = get_or_create_metric(
        Gauge, "dlt_audit_log_integrity_status", "Status of DLT audit log integrity (1=OK, 0=COMPROMISED)"
    )

except ImportError:
    _base_logger.critical("CRITICAL: prometheus-client not found. Metrics collection is critical. Aborting startup.")
    _schedule_alert("CRITICAL: prometheus-client not found. DLT client cannot start without metrics.", level="CRITICAL")
    sys.exit(1)
...
try:
    ...
    _otel_tracer_provider.add_span_processor(BatchSpanProcessor(JaegerExporter(agent_host_name="localhost", agent_port=6831)))
    trace.set_tracer_provider(_otel_tracer_provider)
    TRACER = trace.get_tracer(__name__)
    OTEL_AVAILABLE = True
    _base_logger.info("OpenTelemetry tracer initialized with Jaeger exporter for DLT clients.")
except ImportError:
    _base_logger.warning("OpenTelemetry or Jaeger not found. Tracing will be disabled.")
    class Status:
        def __init__(self, code, description=None): self.code = code; self.description = description
    class StatusCode:
    ...
    class DummyTracer:
        ...
        def get_current_span(self): return DummySpan()
    TRACER = DummyTracer()
...
class BaseDLTConfig(BaseModel):
    ...
    retry_policy: Dict[str, Any] = Field(default_factory=lambda: {"retries": 5, "delay": 1, "backoff": 2, "jitter": True})
    ...
    async def execute(self, operation: Callable, *args, **kwargs) -> Any:
        if self.state == "OPEN":
            if self.last_failure_time and (time.time() - self.last_failure_time) > self.reset_timeout:
                ...
                raise DLTClientCircuitBreakerError(f"Circuit breaker is OPEN for {self.client_type}", self.client_type)
        ...
        try:
            ...
            if self.failures >= self.failure_threshold:
                ...
                _schedule_alert(f"CRITICAL: Circuit breaker for DLT client '{self.client_type}' tripped to OPEN. Operations blocked.", level="CRITICAL")
                if PROMETHEUS_AVAILABLE:
...
class DLTClientError(Exception):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        ...
        _base_logger.error(
            ...
            extra={'client_type': client_type, 'details': scrubbed_details, 'correlation_id': correlation_id}
        )
        if PROMETHEUS_AVAILABLE:
            operation_counter.labels(client_type=client_type, operation="general", status="error").inc()
...
class DLTClientConfigurationError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        ...
        _schedule_alert(f"CRITICAL: DLT Client Configuration Error ({self.client_type}): {message}", level="CRITICAL")

class DLTClientConnectivityError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"ERROR: DLT Client Connectivity Error ({self.client_type}): {message}", level="ERROR")

class DLTClientAuthError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"CRITICAL: DLT Client Authentication Error ({self.client_type}): {message}", level="CRITICAL")

class DLTClientTransactionError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"ERROR: DLT Client Transaction Error ({self.client_type}): {message}", level="ERROR")

class DLTClientQueryError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"WARNING: DLT Client Query Error ({self.client_type}): {message}", level="WARNING")

class DLTClientResourceError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"ERROR: DLT Client Resource Error ({self.client_type}): {message}", level="ERROR")

class DLTClientTimeoutError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"WARNING: DLT Client Timeout Error ({self.client_type}): {message}", level="WARNING")

class DLTClientValidationError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"CRITICAL: DLT Client Validation Error ({self.client_type}): {message}", level="CRITICAL")

class DLTClientCircuitBreakerError(DLTClientError):
    def __init__(self, message: str, client_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None, correlation_id: Optional[str] = None):
        super().__init__(message, client_type, original_exception, details, correlation_id)
        _schedule_alert(f"WARNING: DLT Client Circuit Breaker Open ({self.client_type}): {message}", level="WARNING")

# --- Async Retry Decorator ---
def async_retry(catch_exceptions: Optional[Union[Type[Exception], Tuple[Type[Exception], ...]]] = None):
    def decorator(fn):
        @functools.wraps(fn)
        async def wrapper(self, *args, **kwargs):
            client_type = getattr(self, 'client_type', 'N/A')
            config = getattr(self, 'config', {})
            retry_policy = config.get('retry_policy', {'retries': 5, 'delay': 1, 'backoff': 2, 'jitter': True})
            ...
            for i in range(retries):
                try:
                    ...
                    if PROMETHEUS_AVAILABLE:
                        operation_latency.labels(client_type=client_type, operation=fn.__name__).observe(time.time() - start_time)
                        operation_counter.labels(client_type=client_type, operation=fn.__name__, status="success").inc()
                    return result
                    ...
                    if catch_exceptions is None or isinstance(exc, catch_exceptions):
                        ...
                        _base_logger.warning(
                            ...
                            extra={'client_type': client_type, 'correlation_id': correlation_id}
                        )
                        if i < retries - 1:
                            ...
                            if PROMETHEUS_AVAILABLE:
                                operation_counter.labels(client_type=client_type, operation=fn.__name__, status="error").inc()
                            _base_logger.error(
            ...
            if last_exc:
                ...
                raise RuntimeError(f"Operation {fn.__name__} failed without capturing an exception.")
        return wrapper
    return decorator
    ...
    def _scrub(item: Any) -> Any:
        ...
        if isinstance(item, dict):
            ...
            for k, v in item.items():
                ...
                if any(term in str(k).lower() for term in ['key', 'secret', 'password', 'token', 'private_key']):
                    scrubbed[k] = '***REDACTED***'
                else:
                    scrubbed[k] = _scrub(v)
            return scrubbed
...
def _get_dlt_audit_hmac_key() -> bytes:
    ...
    if _dlt_audit_hmac_key is None:
        ...
        if key_str:
            ...
            _base_logger.warning("DLT_AUDIT_HMAC_KEY_ENV not set. Generated a random key for audit log signing. THIS IS INSECURE FOR PRODUCTION.")
...
class AuditManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            _base_logger.critical(f"CRITICAL: Audit log file '{self.log_file_path}' is not writable or accessible: {e}. Aborting startup.")
            _schedule_alert(f"CRITICAL: DLT Audit log file '{self.log_file_path}' is not writable. Aborting.", level="CRITICAL")
            sys.exit(1)
    ...
    async def log_event(self, event_type: str, **kwargs):
        ...
        async with self._log_lock:
            try:
                ...
                _base_logger.critical(f"CRITICAL: Failed to write audit event to file: {e}", exc_info=True)
                _schedule_alert(f"CRITICAL: Failed to write DLT audit event to file: {e}", level="CRITICAL")
        ...
        try:
            ...
            if last_verified_time_str:
                ...
                if datetime.utcnow() - last_verified_time < timedelta(hours=max_age_hours):
                    ...
                    if PROMETHEUS_AVAILABLE: audit_log_integrity_status.set(1)
                    return True
                    ...
                    for line_num, line in enumerate(f, 1):
                        try:
                            ...
                            if not event_data or not signature:
                                _base_logger.error(f"Audit log line {line_num} is malformed (missing event/signature).")
                                mismatched_signatures += 1
                                continue
                            
                            event_json_recreated = json.dumps(event_data, sort_keys=True, ensure_ascii=False)
                            expected_signature = hmac.new(_get_dlt_audit_hmac_key(), event_json_recreated.encode('utf-8'), hashlib.sha256).hexdigest()

                            if signature != expected_signature:
                                _base_logger.critical(f"CRITICAL: Audit log integrity compromised: Signature mismatch on line {line_num}. Event: {event_data}")
                                _schedule_alert(f"CRITICAL: DLT Audit log integrity compromised: Signature mismatch on line {line_num}.", level="CRITICAL")
                                mismatched_signatures += 1
                            current_entry_count += 1
                        except json.JSONDecodeError as e:
                            _base_logger.error(f"Audit log line {line_num} is not valid JSON: {e}. Line: {line.strip()}")
                            mismatched_signatures += 1
                        except Exception as e:
                            _base_logger.error(f"Unexpected error during audit log verification on line {line_num}: {e}", exc_info=True)
                            mismatched_signatures += 1

            if mismatched_signatures > 0:
                _base_logger.critical(f"DLT Audit log integrity check FAILED. {mismatched_signatures} signature mismatches found.")
                if PROMETHEUS_AVAILABLE: audit_log_integrity_status.set(0)
                return False
            else:
                _base_logger.info(f"DLT Audit log integrity check PASSED. {current_entry_count} entries verified.")
                async with self._integrity_lock:
                    with open(self.integrity_file_path, "w") as f:
            ...
            _base_logger.warning("Audit log file or integrity meta file not found. Cannot verify integrity.")
            if PROMETHEUS_AVAILABLE: audit_log_integrity_status.set(0)
            return False
        except Exception as e:
            _base_logger.critical(f"CRITICAL: Error during audit log integrity verification: {e}", exc_info=True)
            _schedule_alert(f"CRITICAL: Error during DLT audit log integrity verification: {e}", level="CRITICAL")
            if PROMETHEUS_AVAILABLE: audit_log_integrity_status.set(0)
            return False
...
class BaseOffChainClient(ABC):
    ...
    def __init__(self, config: Dict[str, Any]):
        try:
            ...
            raise DLTClientValidationError(f"Invalid off-chain client configuration: {e}", self.__class__.__name__)
    ...
    async def _run_blocking_in_executor(self, func: Callable, *args: Any, **kwargs: Any) -> Any:
        ...
        if global_executor is None:
            global_executor = ThreadPoolExecutor(max_workers=self.config.get('executor_max_workers', 4))
    ...
    async def close(self) -> None:
        if not hasattr(self, '_closed') or not self._closed:
            self.logger.info(f"Closing {self.__class__.__name__} client.", extra={'client_type': self.__class__.__name__})
            self._closed = True
...
class BaseDLTClient(ABC):
    ...
    def __init__(self, config: Dict[str, Any], off_chain_client: BaseOffChainClient):
        try:
            ...
            raise DLTClientValidationError(f"Invalid DLT client configuration: {e}", self.__class__.__name__)
        self.config = validated_config
...
def register_plugin_entrypoints(register_func: Callable):
    ...
    register_func(
        ...
        executor_func=lambda client, *args, **kwargs: client.off_chain_client.save_blob(*args, **kwargs),
        capabilities=["off_chain_storage"]
    )
    register_func(
        name="off_chain_get_blob",
        executor_func=lambda client, *args, **kwargs: client.off_chain_client.get_blob(*args, **kwargs),
        capabilities=["off_chain_storage"]
    )

def create_dlt_client(client_type: str, dlt_config: Dict[str, Any], off_chain_config: Dict[str, Any]) -> BaseDLTClient:
    """
    ...
    if client_type.lower() == "fabric" and FABRIC_AVAILABLE:
        ...
        supported_backends = []
        if FABRIC_AVAILABLE: supported_backends.append("fabric")
        if WEB3_AVAILABLE: supported_backends.append("ethereum/evm")
        if S3_AVAILABLE: supported_backends.append("s3")
        if GCS_AVAILABLE: supported_backends.append("gcs")
        if AZURE_BLOB_AVAILABLE: supported_backends.append("azure")
...
try:
    ...
    def _register_with_plugin_manager():
        try:
            ...
            _base_logger.warning(f"Could not auto-register DLT base framework with plugin manager: {e}")
