...
import aiohttp
from pydantic import BaseModel, Field, validator, ValidationError, AnyHttpUrl, AnyUrl, model_validator
    ...
    def validate_url(cls, v):
        if isinstance(v, str):
            ...
            if parsed.scheme not in ('grpc', 'grpcs', 'http', 'https'):
                raise ValueError(f"URL scheme must be one of: grpc, grpcs, http, https. Got: {parsed.scheme}")
        return v
...
class FabricConfig(BaseModel):
    ...
    def validate_mode_dependencies(self):
        ...
        if self.mode == "sdk":
            ...
            required = ["msp_id", "channel", "chaincode_id", "user_name", "cert_path", "key_path", "peers"]
...
class FabricClientWrapper(BaseDLTClient):
    ...
    def __init__(self, config: Dict[str, Any], off_chain_client: 'BaseOffChainClient'):
        ...
        try:
            ...
            if fabric_cfg.get("mode") == "rest" and not fabric_cfg.get("rest_api_auth_token"):
                fabric_cfg["rest_api_auth_token"] = SECRETS_MANAGER.get_secret("FABRIC_REST_TOKEN", required=False)
            ...
            raise DLTClientValidationError(f"Failed to load Fabric client configuration: {e}", "Fabric", original_exception=e)

        # 2) Prepare base/common config for BaseDLTClient (timeouts/retry/circuit-breaker)
        base_cfg_keys = ("default_timeout_seconds", "retry_policy", "circuit_breaker_threshold", "circuit_breaker_reset_timeout")
        base_cfg = {k: config[k] for k in base_cfg_keys if k in config}
        # Use get() with default to safely access the timeout value
        base_cfg.setdefault("default_timeout_seconds", validated_fabric.get("default_timeout_seconds", 30))
        ...
        if self._log_format == "json":
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"fabric_client_error.{level.lower()}", message=message, details=safe_entry)
                    )
                except RuntimeError:
                    pass
        else:
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    ...
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"fabric_client_error.{level.lower()}", message=message, details=safe_extra)
                    )
                except RuntimeError:
                    pass
        ...
        async with self._session_lock:
            if self._session is None or self._session.closed:
                ...
                self._session = aiohttp.ClientSession(
                    timeout=aiohttp.ClientTimeout(total=self.client_config["default_timeout_seconds"]),
                    headers=headers,
    ...
    async def _init_sdk_client(self) -> None:
        ...
        try:
            ...
            raise DLTClientConfigurationError(f"Failed to initialize Fabric SDK client: {e}", 
                                           self.client_type, original_exception=e)
    ...
    def _create_fabric_sdk_client(self):
        ...
        try:
            ...
            raise DLTClientConfigurationError(f"Error creating Fabric SDK client: {e}", self.client_type)
            ...
            try:
                ...
                if self.mode == "sdk":
                    ...
                    try:
                        ...
                        span.set_status(Status(StatusCode.ERROR, description=f"Chaincode query failed: {e}"))
                        span.record_exception(e)
                        self._format_log("error", f"Fabric health check failed: {e}", {"correlation_id": correlation_id})
                        return {
                            "status": False,
                    ...
                    resp = await self._circuit_breaker.execute(
                        session.get, health_url, timeout=self.client_config["health_check_timeout"]
                    )
                    try:
                ...
                span.record_exception(e)
                self._format_log("error", f"Fabric health check failed with HTTP {e.status}: {e.message}", {"correlation_id": correlation_id})
                if e.status == 401:
                    raise DLTClientAuthError(f"Fabric authentication failed: {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
                if e.status in (429, 503):
                    raise DLTClientTransactionError(f"Fabric rate limit or service unavailable: {e.status} - {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
                raise DLTClientConnectivityError(f"Fabric REST API responded with error: {e.status} - {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
            except asyncio.TimeoutError as e:
                span.set_status(Status(StatusCode.ERROR, description="Timeout"))
                span.record_exception(e)
                self._format_log("error", f"Fabric health check timed out: {e}", {"correlation_id": correlation_id})
                raise DLTClientTimeoutError(f"Fabric health check timed out: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
                ...
                span.record_exception(e)
                self._format_log("error", f"Unexpected error during Fabric health check: {e}", {"correlation_id": correlation_id})
                raise DLTClientError(f"Unexpected error during Fabric health check: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                off_chain_id = await self._circuit_breaker.execute(
                    self.off_chain_client.save_blob, checkpoint_name, payload_blob, correlation_id=correlation_id
                )
                ...
                if self.mode == "sdk":
                    ...
                    resp = await self._circuit_breaker.execute(
                        session.post, invoke_url, json=payload, timeout=self.client_config["invoke_timeout"]
                    )
                    try:
                        resp.raise_for_status()
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_id": tx_id, "version": version}
                )
                ...
                span.record_exception(e)
                self._format_log("error", f"Fabric write_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientTransactionError(f"Fabric write_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)

    @async_retry(catch_exceptions=(DLTClientConnectivityError, DLTClientAuthError, DLTClientQueryError, FileNotFoundError, DLTClientTimeoutError, DLTClientCircuitBreakerError))
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                if self.mode == "sdk":
                    ...
                    resp = await self._circuit_breaker.execute(
                        session.post, query_url, json=payload, timeout=self.client_config["query_timeout"]
                    )
                    try:
                        resp.raise_for_status()
                        response_json = await resp.json()
                ...
                if not result_data or not isinstance(result_data, dict):
                    raise FileNotFoundError(f"Checkpoint '{name}' (version {version}) not found or query returned empty result.")
                ...
                payload_blob = await self._circuit_breaker.execute(
                    self.off_chain_client.get_blob, off_chain_id, correlation_id=correlation_id
                )
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_id": entry.get("tx_id"), "version": entry.get("version")}
                )
                ...
                span.set_status(Status(StatusCode.ERROR, description="Off-chain blob or entry not found"))
                ...
                span.record_exception(e)
                self._format_log("error", f"Fabric read_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientQueryError(f"Fabric read_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_id": tx_id, "version": new_version}
                )
                ...
                span.set_status(Status(StatusCode.ERROR, description=f"Fabric rollback failed: {e}"))
                span.record_exception(e)
                self._format_log("error", f"Fabric rollback_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientTransactionError(f"Fabric rollback_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
        ...
        if self.mode == "rest":
            async with self._session_lock:
                if self._session and not self._session.closed:
                    self._format_log("info", "Closing Fabric REST client session", {"client_type": self.client_type})
                    try:
                        if ASYNC_TIMEOUT_AVAILABLE:
                            ...
                            await self._session.close()
                        self._format_log("info", "Fabric REST client session closed", {"client_type": self.client_type})
                    except Exception as e:
                        self._format_log("warning", f"Failed to close Fabric REST client session cleanly: {e}", {"client_type": self.client_type})
                    finally:
                        self._session = None
                        
        # For SDK mode, no explicit cleanup needed (SDK doesn't expose close methods)
        if self.mode == "sdk" and self._sdk_client:
            self._format_log("info", "Fabric SDK client resources released", {"client_type": self.client_type})
            self._sdk_client = None
            self._sdk_initialized = False
    ...
    def __del__(self):
        ...
        if self.mode == "rest" and hasattr(self, '_session') and self._session and not self._session.closed:
            try:
...
def register_plugin_entrypoints(register_func):
    ...
    register_func(
        ...
        executor_func=lambda config, off_chain_client, **kwargs: FabricClientWrapper(config, off_chain_client),
        capabilities=["dlt_operations"]
    )
    register_func(
...
try:
    ...
    def _register_with_plugin_manager():
        try:
            ...
            _base_logger.warning(f"Could not auto-register Fabric DLT client with plugin manager: {e}")
