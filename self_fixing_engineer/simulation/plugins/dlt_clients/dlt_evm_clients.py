    ...
    class AWSSecretsBackend(SecretsBackend):
        def __init__(self):
            if not AWS_SECRETS_AVAILABLE:
                raise DLTClientConfigurationError("AWS Secrets Manager backend requested but boto3 is not available.", "EVM")
        ...
        async def get_secret(self, secret_id: str) -> str:
            try:
                ...
                raise DLTClientConfigurationError(f"Failed to fetch secret from AWS Secrets Manager: {e}", "EVM", original_exception=e)
except Exception:
    ...
    class AzureKeyVaultBackend(SecretsBackend):
        def __init__(self, vault_url: str):
            if not AZURE_KEYVAULT_AVAILABLE:
                raise DLTClientConfigurationError("Azure Key Vault backend requested but Azure SDK is not available.", "EVM")
            if not vault_url:
                raise DLTClientConfigurationError("Azure Key Vault URL is required.", "EVM")
            self.client = AsyncSecretClient(vault_url=vault_url, credential=DefaultAzureCredential())
        ...
        async def get_secret(self, secret_id: str) -> str:
            try:
                ...
                raise DLTClientConfigurationError(f"Failed to fetch secret from Azure Key Vault: {e}", "EVM", original_exception=e)
            finally:
                try:
    ...
    class GCPSecretManagerBackend(SecretsBackend):
        def __init__(self, project_id: str):
            if not GCP_SECRET_MANAGER_AVAILABLE:
                raise DLTClientConfigurationError("GCP Secret Manager backend requested but Google Cloud SDK is not available.", "EVM")
            if not project_id:
                raise DLTClientConfigurationError("GCP Project ID is required for GCP Secret Manager.", "EVM")
        ...
        async def get_secret(self, secret_id: str) -> str:
            try:
                ...
                raise DLTClientConfigurationError(f"Failed to fetch secret from GCP Secret Manager: {e}", "EVM", original_exception=e)
except Exception:
...
class EVMConfig(BaseModel):
    ...
    secrets_provider_config: Optional[Dict[str, Any]] = None  # e.g., {"vault_url":"..."}, {"project_id":"..."}
    ...
    def validate_rpc_url_scheme(cls, v, values):
        ...
        if PRODUCTION_MODE and parsed.scheme == "http" and not values.get("allow_insecure_http", False):
            raise ValueError("In PRODUCTION_MODE, HTTPS is required for rpc_url unless allow_insecure_http is true.")
        return v
    ...
    def validate_private_key_presence(cls, v, values):
        ...
        if PRODUCTION_MODE:
            if not values.get("secrets_provider") or not values.get("private_key_secret_id"):
                raise ValueError("In PRODUCTION_MODE, private_key must be loaded via 'secrets_provider' and 'private_key_secret_id'.")
            return None

        # Non-prod: allow direct key, env fallback, or secrets backend
        if v is None and not values.get("private_key_secret_id") and not values.get("secrets_provider"):
            env_key = os.getenv("ETHEREUM_PRIVATE_KEY")
            if not env_key:
                raise ValueError("Provide private_key, or secrets_provider + private_key_secret_id, or set ETHEREUM_PRIVATE_KEY.")
            return env_key

        if v and not re.match(r"^(0x)?[a-fA-F0-9]{64}$", v):
            raise ValueError("private_key must be a 64-character hex string (optionally prefixed with 0x).")
        return v
    ...
    def validate_rpc_url_not_mock(cls, v):
        s = str(v).lower()
        if PRODUCTION_MODE and any(tok in s for tok in ("mock", "test", "example.com", "localhost")):
...
class EthereumClientWrapper(BaseDLTClient):
    ...
    def __init__(self, config: Dict[str, Any], off_chain_client: 'BaseOffChainClient'):
        ...
        try:
            ...
            raise DLTClientValidationError(f"Failed to load EVM client configuration: {e}", "EVM", original_exception=e)

        # 2) Build base (common) config subset for BaseDLTClient
        base_keys = ("default_timeout_seconds", "retry_policy", "circuit_breaker_threshold", "circuit_breaker_reset_timeout")
        ...
        self.max_fee_per_gas_gwei: Optional[int] = self.client_config.get("default_max_fee_per_gas_gwei")
        self.max_priority_fee_per_gas_gwei: Optional[int] = self.client_config.get("default_max_priority_fee_per_gas_gwei")
        self.base_fee_multiplier: float = float(self.client_config.get("eip1559_base_fee_multiplier", 2.0))
        ...
        # Rate limiter
        self._rate_limit_delay: float = 1.0 / float(self.client_config["rate_limit_requests_per_second"])
        self._last_request_time: float = 0.0

        # Initialize web3 (sync provider) and contract ABI
        if not os.path.exists(self.contract_abi_path):
            raise DLTClientConfigurationError(f"Contract ABI not found at {self.contract_abi_path}.", self.client_type)
        try:
            with open(self.contract_abi_path, "r") as f:
                self.contract_abi = json.load(f)
        except Exception as e:
            raise DLTClientConfigurationError(f"Failed to load contract ABI: {e}", self.client_type, original_exception=e)
        ...
        if self._log_format == "json":
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"evm_client_error.{level.lower()}", message=message, details=safe_entry)
                    )
                except RuntimeError:
                    pass
        else:
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    ...
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"evm_client_error.{level.lower()}", message=message, details=safe_extra)
                    )
                except RuntimeError:
                    pass
    ...
    def _safe_copy_dict(self, obj: Any, visited: Optional[set] = None) -> Any:
        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                ...
                if isinstance(k, str) and any(sensitive in k.lower() for sensitive in 
        ...
        try:
            if provider:
                if provider == "aws" and AWS_SECRETS_AVAILABLE:
                    ...
                    backend = AzureKeyVaultBackend(cfg.get("secrets_provider_config", {}).get("vault_url"))
                elif provider == "gcp" and GCP_SECRET_MANAGER_AVAILABLE:
                    backend = GCPSecretManagerBackend(cfg.get("secrets_provider_config", {}).get("project_id"))
                else:
                    if EVM_METRICS:
                        EVM_METRICS["secrets_unavailable_total"].labels(client_type=self.client_type, backend=str(provider)).inc()
                ...
                if EVM_METRICS:
                    EVM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="none_provided").inc()
                raise DLTClientConfigurationError("No private key source configured for EVM client.", self.client_type)
            ...
            self._format_log("info", f"EVM wallet address initialized: {self.account.address}", {"wallet_address": self.account.address})

        except Exception as e:
            if EVM_METRICS:
                EVM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type=provider or "direct").inc()
            raise DLTClientConfigurationError(f"Failed to initialize EVM account: {e}", self.client_type, original_exception=e)
    ...
    async def health_check(self, correlation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with TRACER.start_as_current_span(f"{self.client_type}.health_check", attributes={"correlation_id": correlation_id or ""}) as span:
            try:
                await self._rate_limit()
                is_connected = await self._exec_w3(self.w3.is_connected)
                if not is_connected:
                    raise DLTClientConnectivityError("Not connected to EVM RPC.", self.client_type, correlation_id=correlation_id)
                ...
                if int(chain_id) != self.chain_id:
                    ...
                    raise DLTClientValidationError(msg, self.client_type, correlation_id=correlation_id)
                ...
                empty_code = (code in (b"", b"0x", bytes(), bytearray()) or (hasattr(code, "hex") and code.hex() in ("0x", "")))
                if empty_code:
                    msg = f"No contract code found at {self.contract_address}."
                    span.set_status(Status(StatusCode.ERROR, description=msg))
                    self._format_log("error", msg, {"correlation_id": correlation_id})
                    raise DLTClientResourceError(msg, self.client_type, correlation_id=correlation_id)
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "chain_id": int(chain_id), "balance_eth": balance_eth}
                )
                return {
                    "status": True,
                ...
                return {"status": False, "message": "Circuit breaker open", "details": {}}
            except DLTClientError as e:
                span.set_status(Status(StatusCode.ERROR, description=str(e)))
                span.record_exception(e)
                self._format_log("error", f"EVM health check failed: {e}", {"correlation_id": correlation_id})
                return {"status": False, "message": f"EVM health check failed: {e}", "details": {"error": str(e)}}
            except Exception as e:
                span.set_status(Status(StatusCode.ERROR, description=f"Unexpected error: {e}"))
                span.record_exception(e)
                self._format_log("error", f"EVM health check failed unexpectedly: {e}", {"correlation_id": correlation_id})
                return {"status": False, "message": f"EVM health check failed unexpectedly: {e}", "details": {"error": str(e)}}
    ...
    async def _build_and_send_tx(
        ...
        with TRACER.start_as_current_span(f"{self.client_type}.send_transaction", attributes={"correlation_id": correlation_id or ""}) as span:
            try:
                await self._rate_limit()
                ...
                if priority_gwei is not None and max_fee_gwei is not None:
                    ...
                    self._format_log("debug", "Using EIP-1559 gas parameters from config/args.", {"correlation_id": correlation_id})
                else:
                    ...
                    try:
                        ...
                        if base_fee is not None:
                            ...
                            if gas_price is None:
                                if EVM_METRICS:
                                    EVM_METRICS["gas_fallback_total"].labels(client_type=self.client_type).inc()
                ...
                signed_tx = await self._circuit_breaker.execute(
                    lambda: self._run_blocking_in_executor(self.w3.eth.account.sign_transaction, transaction, self.private_key)
                )
                ...
                try:
                    ...
                    try:
                        safe_tx_params = scrub_secrets({k: (str(v) if isinstance(v, bytes) else v) for k, v in transaction.items()})
                    except TypeError:
                        safe_tx_params = self._safe_copy_dict({k: (str(v) if isinstance(v, bytes) else v) for k, v in transaction.items()})
                ...
                tx_hash_hex = await self._circuit_breaker.execute(
                    lambda: self._run_blocking_in_executor(
                        lambda: self.w3.to_hex(self.w3.eth.send_raw_transaction(signed_tx.rawTransaction))
                    )
                )
                self._format_log(
                    "info",
                ...
                try:
                    receipt = await self._exec_w3(self.w3.eth.wait_for_transaction_receipt, tx_hash_hex, self.tx_confirm_timeout)
                ...
                if receipt_status == 0:
                    ...
                    try:
                        ...
                        pass
                    raise DLTClientTransactionError("Transaction failed on-chain.", self.client_type, details={}, correlation_id=correlation_id)
                ...
                span.record_exception(e)
                self._format_log("error", f"EVM contract/transaction error: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                    ...
                    pass
                raise DLTClientTransactionError(f"EVM contract/transaction error: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
            except DLTClientCircuitBreakerError:
                raise
            except Exception as e:
                span.set_status(Status(StatusCode.ERROR, description=f"Transaction failed: {e}"))
                span.record_exception(e)
                self._format_log("error", f"Failed to send EVM transaction: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                            "evm_tx.send_failure",
                    ...
                    pass
                raise DLTClientTransactionError(f"Failed to send EVM transaction: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def write_checkpoint(
        ...
        with TRACER.start_as_current_span(
            ...
            attributes={"checkpoint_name": checkpoint_name, "hash": hash, "correlation_id": correlation_id or "", "payload_size": len(payload_blob)}
        ) as span:
            try:
                off_chain_id = await self._circuit_breaker.execute(
                    lambda: self.off_chain_client.save_blob(checkpoint_name, payload_blob, correlation_id=correlation_id)
                )
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_hash": tx_hash, "version": version}
                )
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                ...
                span.record_exception(e)
                self._format_log("error", f"EVM write_checkpoint failed: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                    ...
                    pass
                raise DLTClientTransactionError(f"EVM write_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)

    @async_retry(catch_exceptions=(DLTClientConnectivityError, DLTClientQueryError, FileNotFoundError, DLTClientTimeoutError, DLTClientCircuitBreakerError))
    async def read_checkpoint(
        ...
        with TRACER.start_as_current_span(
            ...
            attributes={"checkpoint_name": name, "version": str(version or ""), "correlation_id": correlation_id or ""}
        ) as span:
            try:
                await self._rate_limit()
                if version is None or version == "latest":
                    entry_tuple = await self._exec_w3(self.contract.functions.getLatestCheckpoint(name).call)
                    retrieved_version = int(entry_tuple[4]) if len(entry_tuple) > 4 and entry_tuple[4] is not None else None
                elif isinstance(version, int):
                    entry_tuple = await self._exec_w3(self.contract.functions.readCheckpoint(name, version).call)
                    retrieved_version = int(version)
                else:
                    raise DLTClientValidationError("Version must be int or 'latest'.", self.client_type, correlation_id=correlation_id)
                ...
                payload_blob = await self._circuit_breaker.execute(
                    lambda: self.off_chain_client.get_blob(off_chain_id, correlation_id=correlation_id)
                )
                ...
                try:
                    ...
                    pass
                return {"metadata": entry, "payload_blob": payload_blob, "tx_id": entry.get("tx_id")}
            except FileNotFoundError:
                span.set_status(Status(StatusCode.ERROR, description="Off-chain blob not found"))
                self._format_log("error", f"Off-chain blob not found: {name} v{version}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                ...
                span.record_exception(e)
                self._format_log("error", f"EVM read_checkpoint failed: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                            "evm_checkpoint.read_failure",
                    ...
                    pass
                raise DLTClientQueryError(f"EVM read_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    @async_retry(catch_exceptions=(DLTClientConnectivityError, DLTClientAuthError, DLTClientTransactionError, DLTClientTimeoutError, DLTClientCircuitBreakerError))
    async def _rotate_credentials(self, new_private_key_hex: str, correlation_id: Optional[str] = None) -> None:
        """
        Rotates EVM private key at runtime. Intended for use by a rotation controller.
        """
        with TRACER.start_as_current_span(f"{self.client_type}.rotate_credentials", attributes={"correlation_id": correlation_id or ""}) as span:
            try:
                ...
                span.set_status(Status(StatusCode.ERROR, description=f"EVM credential rotation failed: {e}"))
                span.record_exception(e)
                self._format_log("error", f"EVM credential rotation failed: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                    ...
                    pass
                raise DLTClientAuthError(f"Failed to rotate EVM private key: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def rollback_checkpoint(self, name: str, rollback_hash: str, correlation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with TRACER.start_as_current_span(
            ...
            attributes={"checkpoint_name": name, "rollback_hash": rollback_hash, "correlation_id": correlation_id or ""}
        ) as span:
            try:
                entry_tuple = await self._exec_w3(self.contract.functions.getCheckpointByHash(self.w3.to_bytes(hexstr=rollback_hash)).call)
                entry_to_rollback_to = {
                    "hash": self.w3.to_hex(entry_tuple[0]),
                    "prev_hash": self.w3.to_hex(entry_tuple[1]),
                    "metadata": json.loads(entry_tuple[2]) if entry_tuple[2] else {},
                    "off_chain_ref": entry_tuple[3],
                    "version": int(entry_tuple[4]) if len(entry_tuple) > 4 and entry_tuple[4] is not None else None,
                }

                tx_builder = self.contract.functions.rollbackCheckpoint(name, self.w3.to_bytes(hexstr=rollback_hash))
                tx_hash = await self._build_and_send_tx(tx_builder, correlation_id=correlation_id)
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_id": tx_hash, "new_version": new_version}
                )
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                ...
                span.record_exception(e)
                self._format_log("error", f"EVM rollback_checkpoint failed: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(
                    ...
                    pass
                raise DLTClientTransactionError(f"EVM rollback_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def close(self) -> None:
        ...
        if provider and hasattr(provider, "close"):
            self._format_log("info", f"{self.client_type} web3.py provider closing", {"client_type": self.client_type})
            try:
                if ASYNC_TIMEOUT_AVAILABLE:
                    ...
                    if inspect.isawaitable(res):
                        await res
                self._format_log("info", f"{self.client_type} web3.py provider closed", {"client_type": self.client_type})
            except Exception as e:
                self._format_log("warning", f"Failed to close web3.py provider cleanly: {e}", {"client_type": self.client_type})
...
def register_plugin_entrypoints(register_func: Callable):
    ...
    register_func(
        ...
        executor_func=lambda config, off_chain_client, **kwargs: EthereumClientWrapper(config, off_chain_client),
        capabilities=["dlt_operations"]
    )
    register_func(
...
def create_ethereum_client(config: Dict[str, Any], off_chain_client: 'BaseOffChainClient') -> EthereumClientWrapper:
    """
...
try:
    ...
    def _register_with_plugin_manager():
        try:
            ...
            _base_logger.warning(f"Could not auto-register Ethereum/EVM DLT client with plugin manager: {e}")
    
    # Only register in production mode
    if PRODUCTION_MODE:
        _register_with_plugin_manager()
except ImportError:
    _base_logger.debug("Plugin manager not available, skipping auto-registration of Ethereum/EVM client.")