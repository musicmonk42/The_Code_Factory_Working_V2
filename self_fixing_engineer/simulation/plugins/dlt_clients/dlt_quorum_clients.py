...
try:
    ...
    class BotoClientError(Exception): pass  # Define for type hinting

AZURE_KEYVAULT_AVAILABLE = False
try:
    from azure.identity.aio import DefaultAzureCredential
    from azure.keyvault.secrets.aio import SecretClient as AsyncSecretClient  # Use async client + async credential
    AZURE_KEYVAULT_AVAILABLE = True
except ImportError:
    _base_logger.warning("azure-identity or azure-keyvault-secrets not found. Azure Key Vault integration will be disabled.")
...
try:
    ...
    _base_logger.warning("google-cloud-secret-manager not found. GCP Secret Manager integration will be disabled.")
...
def temp_file(content: str, ttl: float = 3600.0) -> str:
    ...
    try:
        ...
        _base_logger.critical(f"CRITICAL: Failed to create or write to temporary file: {e}.", exc_info=True)
        try:
            asyncio.get_running_loop().create_task(alert_operator(
                f"CRITICAL: Failed to create or write temporary file for Quorum ABI: {e}.", level="CRITICAL"
            ))
        except RuntimeError:
            pass  # No running loop
        raise DLTClientConfigurationError("Failed to create or write temporary file for Quorum ABI.", "Quorum", original_exception=e) from e
    finally:
...
class AWSSecretsBackend(SecretsBackend):
    ...
    def __init__(self):
        if not AWS_SECRETS_AVAILABLE:
            if QUORUM_METRICS: QUORUM_METRICS["secrets_unavailable_total"].labels(client_type="Quorum", backend="aws").inc()
            raise DLTClientConfigurationError("AWS Secrets Manager backend requested but boto3 is not available.", "Quorum")
    ...
    async def get_secret(self, secret_id: str) -> str:
        try:
            ...
            raise DLTClientConfigurationError(f"Failed to fetch secret from AWS Secrets Manager: {e}", "Quorum", original_exception=e)

class AzureKeyVaultBackend(SecretsBackend):
    """Azure Key Vault backend (async)."""
    def __init__(self, vault_url: str):
        if not AZURE_KEYVAULT_AVAILABLE:
            if QUORUM_METRICS: QUORUM_METRICS["secrets_unavailable_total"].labels(client_type="Quorum", backend="azure").inc()
            raise DLTClientConfigurationError("Azure Key Vault backend requested but Azure SDK is not available.", "Quorum")
        if not vault_url:
    ...
    async def get_secret(self, secret_id: str) -> str:
        try:
            ...
            raise DLTClientConfigurationError(f"Failed to fetch secret from Azure Key Vault: {e}", "Quorum", original_exception=e)
        finally:
...
class GCPSecretManagerBackend(SecretsBackend):
    ...
    def __init__(self, project_id: str):
        if not GCP_SECRET_MANAGER_AVAILABLE:
            if QUORUM_METRICS: QUORUM_METRICS["secrets_unavailable_total"].labels(client_type="Quorum", backend="gcp").inc()
            raise DLTClientConfigurationError("GCP Secret Manager backend requested but Google Cloud SDK is not available.", "Quorum")
        if not project_id:
            raise DLTClientConfigurationError("GCP Project ID is required for GCP Secret Manager.", "Quorum")
    ...
    async def get_secret(self, secret_id: str) -> str:
        try:
            ...
            raise DLTClientConfigurationError(f"Failed to fetch secret from GCP Secret Manager: {e}", "Quorum", original_exception=e)
...
class QuorumConfig(BaseModel):
    ...
    private_for: Optional[List[str]] = None  # List of public keys for private transaction recipients
    ...
    def validate_contract_abi_source(cls, v, values):
        ...
        if v and values.get("contract_abi_secret_id") and PRODUCTION_MODE:
            _base_logger.warning("Both contract_abi_path and contract_abi_secret_id are provided. Prioritizing secret ID in production.")
        return v
    ...
    def validate_private_key_source(cls, v, values):
        ...
        if v and values.get("private_key_secret_id") and PRODUCTION_MODE:
            _base_logger.warning("Both private_key and private_key_secret_id are provided. Prioritizing secret ID in production.")
    ...
    def validate_privacy_settings_completeness(cls, v, values):
        ...
        if (privacy_group_id or private_for) and not (privacy_group_id and private_for):
            if QUORUM_METRICS: QUORUM_METRICS["privacy_config_invalid"].labels(client_type="Quorum").inc()
            raise ValueError("Both privacy_group_id and private_for must be provided for private transactions, or neither.")
        ...
        if private_for:
            if not isinstance(private_for, list) or not all(re.match(r"^[A-Za-z0-9+/]{44}$", pk) for pk in private_for):
                if QUORUM_METRICS: QUORUM_METRICS["privacy_config_invalid"].labels(client_type="Quorum").inc()
                raise ValueError("private_for must be a list of valid 44-character base64 public keys.")
        return v
    ...
    def validate_secrets_providers_list(cls, v, values):
        if values.get("contract_abi_secret_id") or values.get("private_key_secret_id"):
            if not v:
                raise ValueError("secrets_providers list must not be empty if secret_ids are provided.")
            for provider in v:
...
class QuorumClientWrapper(EthereumClientWrapper):
    ...
    def __init__(self, config: Dict[str, Any], off_chain_client: 'BaseOffChainClient'):
        ...
        try:
            ...
            try:
                asyncio.get_running_loop().create_task(alert_operator(f"CRITICAL: Invalid Quorum client configuration: {e}.", level="CRITICAL"))
            except RuntimeError:
                pass
            raise DLTClientConfigurationError(f"Invalid Quorum client configuration: {e}", "Quorum", original_exception=e)
        ...
        self.poa_middleware: bool = validated_config.get("poa_middleware", True)  # Quorum often uses PoA
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise DLTClientConfigurationError(f"Failed to read contract ABI: {e}", self.client_type, original_exception=e)
        ...
        try:
            ...
            self._format_log("info", f"Quorum wallet address: {self.account.address}", {"wallet_address": self.account.address})
        except Exception as e:
            self._format_log("error", f"Failed to initialize contract/account: {e}")
            raise DLTClientConfigurationError(f"Failed to initialize contract/account: {e}", self.client_type, original_exception=e)
        ...
        if not hc.get("status"):
            ...
            raise DLTClientConfigurationError(f"Initial Quorum health check failed: {msg}", self.client_type)
            ...
            for provider_name in secrets_providers:
                try:
                    if provider_name == "aws" and AWS_SECRETS_AVAILABLE:
                        ...
                        if QUORUM_METRICS: QUORUM_METRICS["secrets_unavailable_total"].labels(client_type=self.client_type, backend=provider_name).inc()
                        self._format_log("warning", f"Secrets backend '{provider_name}' requested but unavailable for contract ABI.", {"secrets_provider": provider_name})
                        continue
                    ...
                    self._format_log("info", f"Contract ABI loaded from secrets backend: {provider_name} into temporary file.")
                    return self._temp_contract_abi_path
                except Exception as e:
                    if QUORUM_METRICS: QUORUM_METRICS["contract_abi_load_failure"].labels(client_type=self.client_type, source_type=provider_name).inc()
                    self._format_log("warning", f"Failed to fetch contract ABI from {provider_name}: {e}", {"secrets_provider": provider_name})
                    continue

            if QUORUM_METRICS: QUORUM_METRICS["contract_abi_load_failure"].labels(client_type=self.client_type, source_type="all_secrets_failed").inc()
            _base_logger.critical("CRITICAL: Failed to load contract ABI from any configured secrets backend.")
            try:
                asyncio.get_running_loop().create_task(alert_operator("CRITICAL: Failed to load Quorum contract ABI from secrets.", level="CRITICAL"))
            except RuntimeError:
                pass
            raise DLTClientConfigurationError("Failed to load contract ABI from any configured secrets backend.", self.client_type)

        if contract_abi_path:
            if not os.path.exists(contract_abi_path):
                if QUORUM_METRICS: QUORUM_METRICS["contract_abi_load_failure"].labels(client_type=self.client_type, source_type="file_not_found").inc()
                _base_logger.critical(f"CRITICAL: Contract ABI not found at {contract_abi_path}.")
                try:
                    asyncio.get_running_loop().create_task(alert_operator(f"CRITICAL: Contract ABI not found at {contract_abi_path}.", level="CRITICAL"))
                except RuntimeError:
                    pass
                raise DLTClientConfigurationError(f"Contract ABI not found at {contract_abi_path}.", self.client_type)
            self._format_log("warning", "Contract ABI loaded from local file path. Not recommended for production if secrets are available.")
            return contract_abi_path

        if QUORUM_METRICS: QUORUM_METRICS["contract_abi_load_failure"].labels(client_type=self.client_type, source_type="no_source").inc()
        _base_logger.critical("CRITICAL: No contract ABI path or secret ID provided.")
        try:
            asyncio.get_running_loop().create_task(alert_operator("CRITICAL: No Quorum contract ABI source configured.", level="CRITICAL"))
        except RuntimeError:
            pass
        raise DLTClientConfigurationError("No Quorum contract ABI source configured.", self.client_type)
    ...
    async def _load_private_key_quorum(self, config: Dict[str, Any]):
        ...
        if PRODUCTION_MODE and not private_key_secret_id:
            if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="missing_secret_id").inc()
            _base_logger.critical("CRITICAL: In PRODUCTION_MODE, private_key_secret_id must be provided for Quorum private key.")
            try:
                asyncio.get_running_loop().create_task(alert_operator("CRITICAL: Quorum private key secret ID not configured in production.", level="CRITICAL"))
            except RuntimeError:
                pass
            raise DLTClientConfigurationError("private_key_secret_id is required in PRODUCTION_MODE.", self.client_type)
            ...
            for provider_name in secrets_providers:
                try:
                    if provider_name == "aws" and AWS_SECRETS_AVAILABLE:
                        ...
                        if QUORUM_METRICS: QUORUM_METRICS["secrets_unavailable_total"].labels(client_type=self.client_type, backend=provider_name).inc()
                        self._format_log("warning", f"Secrets backend '{provider_name}' requested but unavailable for private key.", {"secrets_provider": provider_name})
                        continue

                    self.private_key = await backend.get_secret(private_key_secret_id)
                    self._format_log("info", f"Private key loaded from secrets backend: {provider_name}.")
                    return  # Successfully loaded
                except Exception as e:
                    if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type=provider_name).inc()
                    self._format_log("warning", f"Failed to fetch private key from {provider_name}: {e}", {"secrets_provider": provider_name})
                    continue

            if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="all_secrets_failed").inc()
            _base_logger.critical("CRITICAL: Failed to load private key from any configured secrets backend.")
            try:
                asyncio.get_running_loop().create_task(alert_operator("CRITICAL: Failed to load Quorum private key from secrets.", level="CRITICAL"))
            except RuntimeError:
                pass
            raise DLTClientConfigurationError("Failed to load private key from any configured secrets backend.", self.client_type)
        ...
        if private_key_direct:
            ...
            self._format_log("warning", "Private key loaded directly from config. Not recommended for production.")
        else:
            if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="no_source").inc()
            _base_logger.critical("CRITICAL: No private key source configured.")
            try:
                asyncio.get_running_loop().create_task(alert_operator("CRITICAL: No Quorum private key source configured.", level="CRITICAL"))
            except RuntimeError:
                pass
            raise DLTClientConfigurationError("No Quorum private key source configured.", self.client_type)
    ...
    def _format_log(self, level: str, message: str, extra: Dict[str, Any] = None) -> None:
        ...
        if self._log_format == "json":
            ...
            if level.upper() in ["ERROR", "CRITICAL"]:
                try:
                    asyncio.get_running_loop().create_task(AUDIT.log_event(f"quorum_client_error.{level.lower()}", message=message, details=scrub_secrets(extra)))
                except RuntimeError:
                    pass
        else:
    ...
    @async_retry(catch_exceptions=(DLTClientConfigurationError, asyncio.TimeoutError, DLTClientCircuitBreakerError))
    async def _rotate_credentials(self, new_private_key: str, correlation_id: Optional[str] = None) -> None:
        """
        Rotates Quorum private key at runtime. This method is internal and would be called
        by a secrets rotation service.
        """
        with TRACER.start_as_current_span("quorum.rotate_credentials", attributes={"correlation_id": correlation_id}) as span:
            try:
                if not new_private_key or not re.match(r"^(0x)?[a-fA-F0-9]{64}$", new_private_key):
                    if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="rotation_invalid_key").inc()
                    self._format_log(
                        "error",
                        "New private key is empty or invalid format for rotation",
                        {"correlation_id": correlation_id, "error_code": "INVALID_PRIVATE_KEY"}
                    )
                    raise DLTClientValidationError(
                        "New private key cannot be empty or invalid format",
                        self.client_type, correlation_id=correlation_id
                    )
                self._format_log("info", "Initiating Quorum private key rotation", {"correlation_id": correlation_id})
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "new_wallet_address": self.account.address}
                )
                try:
                    asyncio.get_running_loop().create_task(AUDIT.log_event(
                ...
                span.set_status(Status(StatusCode.ERROR, description=f"Quorum credential rotation error: {e}"))
                span.record_exception(e)
                if QUORUM_METRICS: QUORUM_METRICS["private_key_load_failure"].labels(client_type=self.client_type, source_type="rotation_failed").inc()
                self._format_log(
                    "error",
                    f"Failed to rotate Quorum private key: {e}",
                    {"correlation_id": correlation_id, "error_code": "QUORUM_ROTATE_FAILED"}
                )
                try:
                    asyncio.get_running_loop().create_task(AUDIT.log_event(
                ...
                raise DLTClientAuthError(
                    f"Failed to rotate Quorum private key: {e}",
                    self.client_type, original_exception=e, correlation_id=correlation_id
                )

    @async_retry(catch_exceptions=(DLTClientConnectivityError, DLTClientAuthError, DLTClientTransactionError, DLTClientQueryError, DLTClientCircuitBreakerError))
    async def health_check(self, correlation_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with TRACER.start_as_current_span(f"{self.client_type}.health_check_logic", attributes={"correlation_id": correlation_id}) as span:
            try:
                ...
                if self.privacy_group_id and self.private_for:  # Both must be present for privacy
                    try:
                        ...
                        self._format_log("info", "Quorum privacy configuration is valid.", {"correlation_id": correlation_id})
                    except Exception as e:
                        span.set_status(Status(StatusCode.ERROR, description=f"Privacy check failed: {e}"))
                        span.record_exception(e)
                        if QUORUM_METRICS: QUORUM_METRICS["privacy_config_invalid"].labels(client_type=self.client_type).inc()
                        self._format_log(
                            "error",
                            f"Quorum privacy check failed: {e}",
                            {"correlation_id": correlation_id, "error_code": "QUORUM_PRIVACY_CHECK_FAILED"}
                        )
                        return {
                            "status": False,
                    ...
                    span.set_status(Status(StatusCode.ERROR, description="Incomplete privacy configuration"))
                    if QUORUM_METRICS: QUORUM_METRICS["privacy_config_invalid"].labels(client_type=self.client_type).inc()
                    self._format_log(
                        "error",
                        "Incomplete Quorum privacy configuration: both privacy_group_id and private_for are required for private transactions.",
                        {"correlation_id": correlation_id, "error_code": "QUORUM_PRIVACY_INCOMPLETE"}
                    )
                    return {
                        "status": False,
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "error_code": "QUORUM_HEALTHCHECK_FAILED"}
                )
                return {
                    "status": False,
    ...
    @async_retry(catch_exceptions=(DLTClientConnectivityError, DLTClientAuthError, DLTClientTransactionError, DLTClientTimeoutError, DLTClientCircuitBreakerError))
    async def _send_transaction(self, tx_builder_method: Callable, gas_limit: Optional[int] = None,
                                gas_price: Optional[int] = None, max_fee_per_gas: Optional[int] = None,
                                max_priority_fee_per_gas: Optional[int] = None, correlation_id: Optional[str] = None) -> str:
        """
        Sends a transaction to the Quorum network, supporting private transactions when configured.
        """
        if not getattr(self, "account", None):
            raise DLTClientConfigurationError("Quorum account not initialized. Private key likely failed to load.", self.client_type)

        with TRACER.start_as_current_span(f"{self.client_type}.send_transaction", attributes={"correlation_id": correlation_id}) as span:
            try:
                ...
                if (max_fee_per_gas is not None and max_priority_fee_per_gas is not None) or \
                   (self.default_max_fee_per_gas_gwei is not None and self.default_max_priority_fee_per_gas_gwei is not None):
                    tx_params['maxFeePerGas'] = self.w3.to_wei(max_fee_per_gas or self.default_max_fee_per_gas_gwei, 'gwei')
                    tx_params['maxPriorityFeePerGas'] = self.w3.to_wei(max_priority_fee_per_gas or self.default_max_priority_fee_per_gas_gwei, 'gwei')
                    self._format_log("debug", "Using EIP-1559 gas parameters from config/args.")
                else:
                    try:
                        latest_block = await self._circuit_breaker.execute(lambda: self.w3.eth.get_block('latest', full_transactions=False))
                        if 'baseFeePerGas' in latest_block and latest_block.baseFeePerGas is not None:
                            ...
                            tx_params['maxPriorityFeePerGas'] = self.w3.to_wei(priority_fee_gwei, 'gwei')
                            tx_params['maxFeePerGas'] = base_fee_per_gas + tx_params['maxPriorityFeePerGas']
                            self._format_log(
                                "debug",
                            ...
                            tx_params['gasPrice'] = gas_price or await self._circuit_breaker.execute(lambda: self.w3.eth.gas_price)
                            self._format_log(
                                "debug",
                        ...
                        self._format_log(
                            ...
                            {"correlation_id": correlation_id, "error_code": "GAS_ESTIMATION_FAILED"}
                        )
                        tx_params['gasPrice'] = gas_price or self.w3.to_wei(self.fallback_gas_price_gwei, 'gwei')
                ...
                # Build transaction
                transaction = await self._circuit_breaker.execute(lambda: tx_builder_method.build_transaction(tx_params))

                # Audit: Transaction Signing (hash of unsigned tx for audit purposes)
                unsigned_tx_json = json.dumps(transaction, sort_keys=True, separators=(',', ':')).encode('utf-8')
                ...
                if is_private_tx:
                    ...
                    try:
                        result = await self._circuit_breaker.execute(
                            lambda: self.w3.manager.coro_request(
                                ...
                                [raw_tx_hex, {"privateFor": self.private_for, "privacyGroupId": self.privacy_group_id}]
                            )
                        )
                    ...
                    if not tx_hash_hex:
                        ...
                        tx_hash = await self._circuit_breaker.execute(lambda: self.w3.eth.send_raw_transaction(signed_tx.rawTransaction))
                ...
                try:
                    receipt = await self._circuit_breaker.execute(
                        lambda: self.w3.eth.wait_for_transaction_receipt(tx_hash_hex, timeout=self.tx_confirm_timeout)
                    )
                except TimeExhausted as e:
                    span.set_status(Status(StatusCode.ERROR, description="Timeout"))
                    span.record_exception(e)
                    self._format_log("error", f"Transaction confirmation timed out: {e}", {"correlation_id": correlation_id})
                    try:
                        asyncio.get_running_loop().create_task(AUDIT.log_event(
                        ...
                        pass
                    raise DLTClientTimeoutError(f"Transaction confirmation timed out: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)

                if getattr(receipt, "status", 1) == 0:
                    span.set_status(Status(StatusCode.ERROR, description="Transaction failed on-chain"))
                    self._format_log(
                        "error",
                        f"Transaction failed on-chain. Receipt: {receipt}",
                        {"correlation_id": correlation_id, "receipt": scrub_secrets(receipt)}
                    )
                    try:
                        asyncio.get_running_loop().create_task(AUDIT.log_event(
                    ...
                    raise DLTClientTransactionError(
                        f"Transaction failed on-chain. Receipt: {receipt}",
                        self.client_type, details=scrub_secrets(receipt), correlation_id=correlation_id
                    )
                ...
                span.record_exception(e)
                self._format_log("error", f"Failed to send Quorum transaction: {e}", {"correlation_id": correlation_id})
                try:
                    asyncio.get_running_loop().create_task(AUDIT.log_event(
                    ...
                    pass
                raise DLTClientTransactionError(f"Failed to send Quorum transaction: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
        ...
        try:
            ...
            if self._temp_contract_abi_path and os.path.exists(self._temp_contract_abi_path):
                try:
                    ...
                    _base_logger.info(f"Cleaned up temporary Quorum ABI file: {self._temp_contract_abi_path}")
                except OSError as e:
                    _base_logger.warning(f"Failed to delete temporary Quorum ABI file {self._temp_contract_abi_path}: {e}")
                finally:
            ...
            self._format_log("audit", "Quorum DLT Client cleanup attempted", {"client_type": self.client_type})