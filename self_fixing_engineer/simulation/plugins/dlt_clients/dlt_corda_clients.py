...
from pydantic import BaseModel, HttpUrl, Field, ValidationError, field_validator, model_validator
...
class CordaClientWrapper(BaseDLTClient):
    ...
    def __init__(self, config: Dict[str, Any], off_chain_client: 'BaseOffChainClient'):
        ...
        try:
            ...
            corda_cfg["user"] = corda_cfg.get("user") or SECRETS_MANAGER.get_secret("CORDA_USER", required=True)
            corda_cfg["password"] = corda_cfg.get("password") or SECRETS_MANAGER.get_secret("CORDA_PASSWORD", required=True)
            ...
            raise DLTClientValidationError(f"Failed to load Corda client secrets or configuration: {e}", "Corda", original_exception=e)
    ...
    def _format_log(self, level: str, message: str, extra: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self._log_format == "json":
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"corda_client_error.{level.lower()}", message=message, details=safe_entry)
                    )
                except RuntimeError:
            ...
            if level.upper() in ("ERROR", "CRITICAL"):
                try:
                    asyncio.get_running_loop().create_task(
                        AUDIT.log_event(f"corda_client_error.{level.lower()}", message=message, details=scrub_secrets(extra))
                    )
                except RuntimeError:
                    pass
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                resp = await self._circuit_breaker.execute(
                    session.get, health_url, timeout=self.client_config["timeout_seconds"]
                )
                try:
                ...
                self._format_log("error", f"Corda health check failed with HTTP {e.status}: {e.message}", {"correlation_id": correlation_id})
                if e.status == 401:
                    raise DLTClientAuthError(f"Corda authentication failed: {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
                if e.status in (429, 503):
                    raise DLTClientTransactionError(f"Corda rate limit or service unavailable: {e.status} - {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
                raise DLTClientConnectivityError(f"Corda RPC responded with error: {e.status} - {e.message}", self.client_type, original_exception=e, correlation_id=correlation_id)
            except asyncio.TimeoutError as e:
                span.set_status(Status(StatusCode.ERROR, description="Timeout"))
                span.record_exception(e)
                self._format_log("error", f"Corda health check timed out: {e}", {"correlation_id": correlation_id})
                raise DLTClientTimeoutError(f"Corda health check timed out: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
                ...
                span.record_exception(e)
                self._format_log("error", f"Unexpected error during Corda health check: {e}", {"correlation_id": correlation_id})
                raise DLTClientError(f"Unexpected error during Corda health check: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def write_checkpoint(
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                resp = await self._circuit_breaker.execute(
                    session.post, flow_url, json=flow_args, timeout=self.client_config["timeout_seconds"]
                )
                try:
                    ...
                    if resp.status >= 400:
                        ...
                        raise DLTClientTransactionError(
                            msg, self.client_type, details=scrub_secrets(response_json), correlation_id=correlation_id
                        )
                ...
                span.record_exception(e)
                self._format_log("error", f"Corda write_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientTransactionError(f"Corda write_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def read_checkpoint(
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                span.record_exception(e)
                self._format_log("error", f"Corda read_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientQueryError(f"Corda read_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
        ...
        with TRACER.start_as_current_span(
            ...
            try:
                ...
                self._format_log(
                    ...
                    {"correlation_id": correlation_id, "tx_id": tx_id, "version": rolled_back_entry.get("version", "N/A")}
                )
                return {
                ...
                span.record_exception(e)
                self._format_log("error", f"Corda rollback_checkpoint failed: {e}", {"correlation_id": correlation_id})
                raise DLTClientTransactionError(f"Corda rollback_checkpoint failed: {e}", self.client_type, original_exception=e, correlation_id=correlation_id)
    ...
    async def close(self) -> None:
        ...
        async with self._session_lock:
            if self._session and not self._session.closed:
                self._format_log("info", f"{self.client_type} aiohttp session closing", {"client_type": self.client_type})
                try:
                    if ASYNC_TIMEOUT_AVAILABLE:
                        ...
                        await self._session.close()
                    self._format_log("info", f"{self.client_type} aiohttp session closed", {"client_type": self.client_type})
                except Exception as e:
                    self._format_log("warning", f"Failed to close aiohttp session cleanly: {e}", {"client_type": self.client_type})
                finally:
                    self._session = None
...
def register_plugin_entrypoints(register_func):
    ...
    register_func(
        ...
        executor_func=lambda config, off_chain_client, **kwargs: CordaClientWrapper(config, off_chain_client),
        capabilities=["dlt_operations"]
    )
    register_func(
...
try:
    ...
    def _register_with_plugin_manager():
        try:
            ...
            _base_logger.warning(f"Could not auto-register Corda DLT client with plugin manager: {e}")
