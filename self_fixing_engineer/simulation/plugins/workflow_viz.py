...
if PROMETHEUS_AVAILABLE:
    VIZ_RENDER_TOTAL = _get_or_create_metric(Counter, 'workflow_viz_render_total', 'Total visualization renders', ['backend'])
    VIZ_EXPORT_TOTAL = _get_or_create_metric(Counter, 'workflow_viz_export_total', 'Total visualization exports', ['format', 'status'])
    VIZ_RENDER_LATENCY = _get_or_create_metric(Histogram, 'workflow_viz_render_latency_seconds', 'Visualization render latency', ['backend'])
    VIZ_RENDER_ERRORS = _get_or_create_metric(Counter, 'workflow_viz_render_errors_total', 'Total errors during rendering', ['backend', 'error_type'])
...
if PYDANTIC_AVAILABLE:
    class WorkflowVizConfig(BaseModel):
        ...
        colorblind_palette: List[str] = Field(default=["#0072B2", "#D55E00", "#CC79A7", "#F0E442", "#009E73", "#56B4E9", "#E69F00", "#000000", "#FFFFFF"])
    ...
    class WorkflowVizConfig:
        def __init__(self):
            ...
            self.colorblind_palette = ["#0072B2", "#D55E00", "#CC79A7", "#F0E442", "#009E73", "#56B4E9", "#E69F00", "#000000", "#FFFFFF"]
...
if CONFIG_FILE.exists():
    try:
        ...
        env_overrides = {k.lower(): os.getenv(f"VIZ_{k.upper()}") for k in file_config.keys() if os.getenv(f"VIZ_{k.upper()}") is not None}
        if PYDANTIC_AVAILABLE:
        ...
        viz_logger.warning(f"Failed to load or validate workflow_viz_config.json: {e}. Using defaults.")
...
class DashboardAPI:
    ...
    def markdown(self, text: str) -> None:
        if self.backend == "streamlit": st.markdown(text)
    def warning(self, text: str) -> None:
        if self.backend == "streamlit": st.warning(text)
    def plotly_chart(self, fig: Any, use_container_width: bool = True) -> None:
        if self.backend == "streamlit": st.plotly_chart(fig, use_container_width=use_container_width)
    def expander(self, label: str, expanded: bool = False) -> Any:
        if self.backend == "streamlit": return st.expander(label, expanded=expanded)
    def subheader(self, text: str) -> None:
        if self.backend == "streamlit": st.subheader(text)
    def info(self, text: str) -> None:
        if self.backend == "streamlit": st.info(text)
    def pyplot(self, fig: Any) -> None:
        if self.backend == "streamlit": st.pyplot(fig)
    def caption(self, text: str) -> None:
        if self.backend == "streamlit": st.caption(text)
    def button(self, label: str, key: Optional[str] = None) -> bool:
        if self.backend == "streamlit": return st.button(label, key=key)
    def download_button(self, label: str, data: bytes, file_name: str, mime: str) -> None:
        if self.backend == "streamlit": st.download_button(label, data, file_name, mime)
...
def detect_dashboard_backend() -> str:
    if "streamlit" in sys.modules: return "streamlit"
    return "unknown"
...
def render_workflow_viz(
    ...
    if not result:
        ...
        if PROMETHEUS_AVAILABLE: VIZ_RENDER_TOTAL.labels(backend=dashboard_api.backend).inc(); VIZ_RENDER_ERRORS.labels(backend=dashboard_api.backend, error_type='no_data').inc()
        return None
    ...
    if not PLOTLY_AVAILABLE and not MATPLOTLIB_AVAILABLE:
        dashboard_api.warning("No visualization libraries available (Plotly, Matplotlib). Displaying text summary.")
    ...
    if prefer_plotly and PLOTLY_AVAILABLE:
        try:
            ...
            node_trace = go.Scatter(x=node_x, y=node_y, mode='markers+text', text=[node_labels[n] for n in nodes], textposition='bottom center', marker=dict(color=color_map, size=60, line=dict(width=2, color='#222')), hoverinfo='text', hovertext=hover_texts)
            
            fig = go.Figure(data=[edge_trace, node_trace])
            fig.update_layout(showlegend=False, margin=dict(l=10, r=10, t=30, b=10), xaxis=dict(showgrid=False, zeroline=False, showticklabels=False), yaxis=dict(showgrid=False, zeroline=False, showticklabels=False), plot_bgcolor='#fff', title='Workflow Flowchart (Interactive)')
            ...
            dashboard_api.caption("This interactive diagram shows the workflow of the self-fixing engine from specification to output, highlighting phases and counts of findings and actions. All colors and labels are chosen for clarity and accessibility.")

            if dashboard_api.button("Export Plotly as PNG", key="export_plotly_png"):
                try:
                    import kaleido
                except ImportError:
                    dashboard_api.warning("Kaleido is not installed. Install with 'pip install kaleido' for PNG export.")
                    if PROMETHEUS_AVAILABLE: VIZ_EXPORT_TOTAL.labels(format='png', status='failed_dependency').inc()
                else:
                    export_png_path = validate_export_path(RESULTS_DIR / f"workflow_plotly_{int(time.time())}.png")
                    fig.write_image(export_png_path, format="png")
                    dashboard_api.success(f"Plot exported to {export_png_path}")
                    if PROMETHEUS_AVAILABLE: VIZ_EXPORT_TOTAL.labels(format='png', status='success').inc()
            
            if PROMETHEUS_AVAILABLE: VIZ_RENDER_TOTAL.labels(backend='plotly').inc(); VIZ_RENDER_LATENCY.labels(backend='plotly').observe(time.monotonic() - start_time)
            return fig

        except Exception as e:
            viz_logger.error(f"Plotly rendering failed: {e}. Falling back to Matplotlib.", exc_info=True)
            if PROMETHEUS_AVAILABLE: VIZ_RENDER_TOTAL.labels(backend='plotly').inc(); VIZ_RENDER_ERRORS.labels(backend='plotly', error_type='runtime_error').inc()
    ...
    if not prefer_plotly and MATPLOTLIB_AVAILABLE:
        try:
            ...
            nx.draw(nx.DiGraph(edges), pos, with_labels=True, labels=node_labels, node_color=color_map, node_size=3400, font_size=12, font_weight="bold", edge_color="#555", arrows=True, ax=ax)
            nx.draw_networkx_edge_labels(nx.DiGraph(edges), pos, edge_labels={e: "" for e in edges}, font_color="#333", font_size=9, ax=ax)
            ...
            ax.legend(loc="upper center", bbox_to_anchor=(0.5, 1.15), ncol=4, fontsize=10, title="Workflow Phases")
            plt.tight_layout()
            ...
            if summary_callback:
                ...
                _default_summary_and_details(scrubbed_result, dashboard_api)
            dashboard_api.caption("This diagram shows the workflow of the self-fixing engine from specification to output, highlighting phases and counts of findings and actions. All colors and labels are chosen for clarity and accessibility.")
            if dashboard_api.button("Export Matplotlib as PNG", key="export_matplotlib_png"):
                export_png_path = validate_export_path(RESULTS_DIR / f"workflow_matplotlib_{int(time.time())}.png")
                fig.savefig(export_png_path, format="png")
                dashboard_api.success(f"Plot exported to {export_png_path}")
                if PROMETHEUS_AVAILABLE: VIZ_EXPORT_TOTAL.labels(format='png', status='success').inc()
            
            if PROMETHEUS_AVAILABLE: VIZ_RENDER_TOTAL.labels(backend='matplotlib').inc(); VIZ_RENDER_LATENCY.labels(backend='matplotlib').observe(time.monotonic() - start_time)
            return fig
        except Exception as e:
            viz_logger.error(f"Matplotlib rendering failed: {e}. Displaying text summary as final fallback.", exc_info=True)
            if PROMETHEUS_AVAILABLE: VIZ_RENDER_TOTAL.labels(backend='matplotlib').inc(); VIZ_RENDER_ERRORS.labels(backend='matplotlib', error_type='runtime_error').inc()
