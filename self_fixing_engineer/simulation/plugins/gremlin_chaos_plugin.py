...
if not TENACITY_AVAILABLE:
    logger.warning("Tenacity not installed; API retries disabled. Install 'tenacity' for production resilience.")
...
CHAOS_ATTACKS_TOTAL = _safe_counter(
    'gremlin_chaos_attacks_total', 'Total chaos attacks events', ('experiment_type', 'status')
)
CHAOS_ATTACK_ERRORS_TOTAL = _safe_counter(
    'gremlin_chaos_attack_errors_total', 'Total chaos attack errors', ('experiment_type', 'error_type')
)
CHAOS_ATTACK_DURATION_SECONDS = _safe_hist(
    'gremlin_chaos_attack_duration_seconds', 'Duration of chaos attacks', ('experiment_type',)
)
...
PLUGIN_MANIFEST = {
    ...
    "entry_points": {
        "run_chaos_experiment": {
            ...
            "parameters": ["experiment_type", "target_type", "target_value", "duration_seconds", "intensity", "labels", "tags"]
        }
    },
...
GREMLIN_VERIFY_SSL = os.getenv("GREMLIN_VERIFY_SSL", "true").strip().lower() in ("1", "true", "yes", "on")
GREMLIN_CA_BUNDLE = os.getenv("GREMLIN_CA_BUNDLE", "").strip()  # optional custom CA bundle path
GREMLIN_ALLOW_ALL_TARGETS = os.getenv("GREMLIN_ALLOW_ALL_TARGETS", "false").strip().lower() in ("1", "true", "yes", "on")
...
ExperimentType = Literal["cpu_hog", "network_latency", "process_kill", "disk_io", "blackhole", "shutdown"]
    ...
    if PYDANTIC_V2:
        ...
        def _validate(self) -> "TargetSpec":
            ...
            if t in ("Host", "Container"):
                if not self.names or not isinstance(self.names, list) or not all(isinstance(x, str) and x for x in self.names):
                    raise ValueError(f"{t} target requires non-empty 'names' list.")
            elif t == "Kubernetes":
                if GREMLIN_K8S_ALLOWED_NAMESPACES and self.namespace and self.namespace not in GREMLIN_K8S_ALLOWED_NAMESPACES:
                    raise ValueError(f"Kubernetes namespace '{self.namespace}' not allowed by policy.")
                if not (self.namespace or self.labels or self.name_regex):
                    raise ValueError("Kubernetes target requires at least one of: namespace, labels, name_regex.")
            elif t == "Random":
        ...
        def _validate_v1(cls, values):
            ...
            if t in ("Host", "Container"):
                ...
                if not names or not isinstance(names, list) or not all(isinstance(x, str) and x for x in names):
                    raise ValueError(f"{t} target requires non-empty 'names' list.")
            elif t == "Kubernetes":
                ns = values.get("namespace")
                if GREMLIN_K8S_ALLOWED_NAMESPACES and ns and ns not in GREMLIN_K8S_ALLOWED_NAMESPACES:
                    raise ValueError(f"Kubernetes namespace '{ns}' not allowed by policy.")
                if not (values.get("namespace") or values.get("labels") or values.get("name_regex")):
                    raise ValueError("Kubernetes target requires at least one of: namespace, labels, name_regex.")
            elif t == "Random":
                c = values.get("count")
                if not c or c < 1:
...
class AttackSpec(BaseModel):
    ...
    duration_seconds: int = Field(60, ge=1, le=int(os.getenv("GREMLIN_MAX_DURATION_SECONDS", "3600")))
        ...
        def _validate(self) -> "AttackSpec":
            ...
            if et == "network_latency":
                if self.delay_milliseconds is None and self.intensity is None:
                    raise ValueError("network_latency requires delay_milliseconds or intensity (ms).")
...
class GremlinApiClient:
    def __init__(self, base_url: str, team_id: str, api_key: str, timeout: int, verify_ssl: bool = True):
        ...
        self._timeout = aiohttp.ClientTimeout(total=timeout, connect=connect_timeout, sock_connect=connect_timeout, sock_read=timeout)
        ...
        if self._verify_ssl and GREMLIN_CA_BUNDLE:
            try:
                ctx = ssl.create_default_context(cafile=GREMLIN_CA_BUNDLE if os.path.exists(GREMLIN_CA_BUNDLE) else None)
                if not os.path.exists(GREMLIN_CA_BUNDLE):
                    logger.warning(f"GREMLIN_CA_BUNDLE path not found: {GREMLIN_CA_BUNDLE}. Using system CAs.")
                self._ssl_context = ctx
            except Exception as e:
                logger.warning(f"Failed to load GREMLIN_CA_BUNDLE '{GREMLIN_CA_BUNDLE}': {e}. Using system CAs.")
        ...
        async with self._session_lock:
            if self._session is None or self._session.closed:
                if not self._verify_ssl:
                    logger.warning("GREMLIN_VERIFY_SSL=false; TLS verification disabled. Do not use in production.")
    ...
    def _build_attack_payload(attack: AttackSpec, target: TargetSpec) -> Dict[str, Any]:
        ...
        if et == "cpu_hog":
            ...
            payload["args"]["delay_ms"] = int(attack.delay_milliseconds or int(attack.intensity or 100))
            ...
            payload["args"]["read_bps"] = int(attack.read_bytes_per_sec or int((attack.intensity or 10) * 1024 * 1024))
        ...
        if tt in ("Host", "Container"):
            ...
            if target.namespace: payload["target"]["namespace"] = target.namespace
            if target.cluster_name: payload["target"]["cluster_name"] = target.cluster_name
            if target.labels: payload["target"]["labels"] = target.labels
            if target.field_selectors: payload["target"]["field_selectors"] = target.field_selectors
            if target.name_regex: payload["target"]["name_regex"] = target.name_regex
        elif tt == "Random":
    ...
    def _retry_decorator(self, op_name: str):
        ...
        return retry(
            ...
            retry=retry_if_exception_type((aiohttp.ClientError, asyncio.TimeoutError, GremlinApiRetryableError)),
            after=after_sleep
        )
    ...
    async def _request(self, op: str, method: str, path: str, json_body: Optional[Dict[str, Any]] = None) -> Any:
        session = await self._ensure_session()
        url = f"{self.base_url}{path}"
        start = time.monotonic()
        async with _api_semaphore:
            try:
                async with session.request(method=method, url=url, headers=self._headers(), json=json_body) as resp:
                    ...
                    if 500 <= resp.status < 600 or resp.status in (408, 429):
                        raise GremlinApiRetryableError(f"HTTP {resp.status} for {op}", status=resp.status, body=_scrub(text))
                    raise GremlinApiError(f"HTTP {resp.status} for {op}", status=resp.status, body=_scrub(text))
    ...
    async def create_attack(self, attack: AttackSpec, target: TargetSpec) -> str:
        ...
        raise GremlinApiError("Create attack did not return an attack id", body=_scrub(json.dumps(data, ensure_ascii=False)))
    ...
    async with _client_lock:
        if _client is None:
            ...
            if GREMLIN_ALLOWED_BASE_URLS and base_host not in GREMLIN_ALLOWED_BASE_URLS:
                raise GremlinApiError(f"Base URL host '{base_host}' not in GREMLIN_ALLOWED_BASE_URLS policy.")
    ...
    **kwargs: Any
    ...
    try:
        ...
        if target_type in ("Host", "Container"):
            ...
            target_payload.update({k: v for k, v in target_value.items() if k in ("namespace", "cluster_name", "labels", "field_selectors", "name_regex")})
    ...
    try:
        ...
        await _audit_event("chaos_experiment_attempt", {
            "experiment_type": experiment_type,
        ...
        await _audit_event("chaos_experiment_initiated", {
            "experiment_type": experiment_type,
            "attack_id": attack_id,
            "correlation_id": correlation_id,
        })
        logger.info(f"[cid={correlation_id}] Initiated chaos attack: {attack_id} (type={experiment_type})")
        ...
        while state not in terminal_states and (time.monotonic() - poll_start) < max_wait:
            await asyncio.sleep(GREMLIN_POLL_INTERVAL_SECONDS + random.uniform(0, GREMLIN_POLL_JITTER_SECONDS))
            ...
            logger.info(f"[cid={correlation_id}] Attack {attack_id} status={state} reason={status_reason}")
        ...
        if final_state not in terminal_states:
            ...
            CHAOS_ATTACKS_TOTAL.labels(experiment_type=experiment_type, status="monitoring_timeout").inc()
            await _audit_event("chaos_experiment_monitoring_timeout", {
                "experiment_type": experiment_type,
                "attack_id": attack_id,
            ...
            try:
                ...
                logger.warning(f"[cid={correlation_id}] Failed to halt timed-out attack {attack_id}: {e}")
        ...
        if final_state == "SUCCEEDED":
            ...
            await _audit_event("chaos_experiment_succeeded", {
                "experiment_type": experiment_type,
                "attack_id": attack_id,
            ...
            CHAOS_ATTACK_ERRORS_TOTAL.labels(experiment_type=experiment_type, error_type=final_state).inc()
            await _audit_event("chaos_experiment_failed", {
                "experiment_type": experiment_type,
                "attack_id": attack_id,
                "final_state": final_state,
        ...
        CHAOS_ATTACK_ERRORS_TOTAL.labels(experiment_type=experiment_type, error_type="api_error").inc()
        msg = f"[cid={correlation_id}] Gremlin API error: status={e.status}, body={e.body}"
        logger.error(_scrub(msg))
        await _audit_event("chaos_experiment_api_error", {
            "experiment_type": experiment_type,
            "target": target_value,
            "error": _scrub(str(e)),
            "correlation_id": correlation_id,
        ...
        CHAOS_ATTACK_ERRORS_TOTAL.labels(experiment_type=experiment_type, error_type="unexpected_exception").inc()
        logger.error(f"[cid={correlation_id}] Unexpected error in run_chaos_experiment: {e}", exc_info=True)
        await _audit_event("chaos_experiment_unexpected_error", {
            "experiment_type": experiment_type,
            "target": target_value,
            "error": str(e),
            "correlation_id": correlation_id,
