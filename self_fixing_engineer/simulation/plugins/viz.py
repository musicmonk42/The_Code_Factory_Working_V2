...
def _load_config() -> VizConfig:
    ...
    for key, field_type in config_fields.items():
        ...
        if env_var:
            try:
                if field_type == int or (hasattr(field_type, '__origin__') and int in field_type.__args__):
                    config_dict[key] = int(env_var)
                else:
                    config_dict[key] = env_var
            except (ValueError, TypeError, AttributeError):
                logger.warning(f"Invalid type for environment variable VIZ_{key.upper()}. Using default.")
...
if PROMETHEUS_AVAILABLE:
    PLOT_GENERATIONS = _get_or_create_metric(Counter, 'viz_plot_generations_total', 'Total plots generated', ('plot_type', 'status'))
    PLOT_EXPORTS = _get_or_create_metric(Counter, 'viz_plot_exports_total', 'Total plot exports', ('format', 'status'))
    PLOT_CACHE_HITS = _get_or_create_metric(Counter, 'viz_plot_cache_hits_total', 'Total plot cache hits')
    PLOT_CACHE_MISSES = _get_or_create_metric(Counter, 'viz_plot_cache_misses_total', 'Total plot cache misses')
else:
    # Create dummy metrics for when Prometheus is not available
    PLOT_GENERATIONS = _get_or_create_metric(None, 'viz_plot_generations_total', 'Total plots generated')
    PLOT_EXPORTS = _get_or_create_metric(None, 'viz_plot_exports_total', 'Total plot exports')
    PLOT_CACHE_HITS = _get_or_create_metric(None, 'viz_plot_cache_hits_total', 'Total plot cache hits')
    PLOT_CACHE_MISSES = _get_or_create_metric(None, 'viz_plot_cache_misses_total', 'Total plot cache misses')
...
def register_post_plot_hook(fn: Callable[[str, Any, Dict[str, Any]], Tuple[Any, Dict[str, Any]]]) -> None:
...
def register_viz_panel(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if validated_id in _registered_viz_panels and not overwrite:
            raise ValueError(f"Visualization panel ID '{validated_id}' already registered. Use overwrite=True to replace.")
        elif validated_id in _registered_viz_panels:
            logger.warning(f"Visualization panel ID '{validated_id}' already registered. Overwriting.")
        ...
        logger.info(f"Registered visualization panel: {title} (ID: {validated_id}, Type: {plot_type})")
...
def get_panels_for_role(role: str) -> List[Dict[str, Any]]:
    ...
    return [
        panel for panel in list_panels_metadata()
...
def post_plot_hook(plot_type: str, plot_object: Any, metadata: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
    """
...
def plot_flakiness_trend(runs: List[Dict[str, Any]], test_file_name: str) -> Optional[Any]:
    ...
    if not MATPLOTLIB_AVAILABLE:
        ...
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='flakiness', status='skipped_no_lib').inc()
        return None
    
    try:
        results = [r['returncode'] == 0 for r in runs]
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.plot([i + 1 for i in range(len(results))], results, marker='o', linestyle='-', color='blue')
        ...
        fig.tight_layout()
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='flakiness', status='success').inc()
        return fig
    except Exception as e:
        logger.error(f"Matplotlib error generating flakiness plot: {e}", exc_info=True)
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='flakiness', status='error').inc()
        return None
...
def plot_coverage_history(coverage_data: List[float], label: str = "Coverage") -> Optional[Any]:
    ...
    if not MATPLOTLIB_AVAILABLE:
        ...
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='coverage', status='skipped_no_lib').inc()
        return None
    
    if not coverage_data:
        logger.info("No coverage data to plot.")
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='coverage', status='no_data').inc()
        return None

    try:
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.plot([i + 1 for i in range(len(coverage_data))], [c * 100 for c in coverage_data], marker='o', linestyle='-', color='green')
        ...
        ax.grid(True, linestyle='--', alpha=0.7)
        fig.tight_layout()
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='coverage', status='success').inc()
        return fig
    except Exception as e:
        logger.error(f"Matplotlib error generating coverage plot: {e}", exc_info=True)
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='coverage', status='error').inc()
        return None
...
def plot_metric_trend(metrics: List[float], metric_name: str, unit: str = "", filename_suffix: str = "") -> Optional[Any]:
    """
    ...
    if not MATPLOTLIB_AVAILABLE:
        ...
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='metric_trend', status='skipped_no_lib').inc()
        return None

    if not metrics:
        logger.info(f"No data to plot for {metric_name}.")
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='metric_trend', status='no_data').inc()
        return None

    try:
        fig, ax = plt.subplots(figsize=(8, 4))
        ax.plot([i + 1 for i in range(len(metrics))], metrics, marker='o', linestyle='-', color='purple')
        ax.set_title(f"{metric_name} Trend Over Runs")
        ax.set_xlabel("Simulation Run")
        ax.set_ylabel(f"{metric_name} ({unit})")
        ax.grid(True, linestyle='--', alpha=0.7)
        fig.tight_layout()
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='metric_trend', status='success').inc()
        return fig
    except Exception as e:
        logger.error(f"Matplotlib error generating metric trend plot: {e}", exc_info=True)
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='metric_trend', status='error').inc()
        return None
...
def render_example_plugin_custom_metric_trend(st_dash_obj: Any, current_result: Dict[str, Any]) -> Optional[Any]:
    """
    ...
    if not PLOTLY_AVAILABLE:
        ...
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='plugin_custom', status='skipped_no_lib').inc()
        return None
    ...
    for p_run in custom_metrics:
        if p_run.get("plugin_name") == "ExampleChaosSecurityPlugin" and \
           p_run.get("function") == "perform_custom_security_audit" and \
           p_run.get("result", {}).get("status") == "FINDINGS_DETECTED":
            audit_data_points.append(len(p_run["result"].get("findings", [])))
        else:
            audit_data_points.append(0)

    if not audit_data_points:
        st_dash_obj.info("No relevant audit data from ExampleChaosSecurityPlugin found in this run.")
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='plugin_custom', status='no_data').inc()
        return None

    try:
        fig = go.Figure(data=[go.Scatter(y=audit_data_points, mode='lines+markers', name='Audit Findings Count')])
        fig.update_layout(
        ...
        if st_dash_obj.button("Export Plotly Plot as HTML", key="export_plotly_plot"):
            ...
            if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format='html', status='success').inc()
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='plugin_custom', status='success').inc()
        return fig
    except Exception as e:
        logger.error(f"Plotly error generating plugin custom plot: {e}", exc_info=True)
        if PROMETHEUS_AVAILABLE: PLOT_GENERATIONS.labels(plot_type='plugin_custom', status='error').inc()
        return None
...
async def _get_cached_plot_data(cache_key: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if cached_data:
            ...
            if PROMETHEUS_AVAILABLE: PLOT_CACHE_HITS.inc()
    ...
    if PROMETHEUS_AVAILABLE: PLOT_CACHE_MISSES.inc()
    return None
...
# --- Batch Export API ---
async def batch_export_panels(panel_ids: Optional[List[str]] = None, format: Optional[str] = None) -> Dict[str, Optional[str]]:
    """
    ...
    for pid in target_ids:
        ...
        if not panel or not panel["export_supported"]:
            exports[pid] = None
            if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='unsupported').inc()
            continue
        ...
        try:
            ...
            if plot_data_from_cache:
                ...
                if panel["plot_type"] == "plotly" and 'data' in plot_data_from_cache and 'layout' in plot_data_from_cache:
                    fig = go.Figure(data=plot_data_from_cache['data'], layout=plot_data_from_cache['layout'])
                elif panel["plot_type"] == "matplotlib":
                    logger.debug(f"Matplotlib plot {pid} not directly reconstructible from JSON cache. Re-rendering.")
            ...
            if fig is None:
                exports[pid] = None
                if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='render_failed').inc()
                continue
            ...
            if panel["plot_type"] == "matplotlib":
                ...
                if not PLOTLY_EXPORT_AVAILABLE:
                    ...
                    exports[pid] = None
                    if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='plotly_export_lib_missing').inc()
                    continue
                ...
                try:
                    ...
                    logger.warning(f"Export not supported for {panel['plot_type']} to format {export_format}.")
                    exports[pid] = None
                    if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='unsupported_type_format').inc()
                    continue
            exports[pid] = str(export_path)
            if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='success').inc()
        except Exception as e:
            logger.error(f"Failed to export {pid} to {export_format}: {e}", exc_info=True)
            exports[pid] = None
            if PROMETHEUS_AVAILABLE: PLOT_EXPORTS.labels(format=export_format, status='error').inc()
    return exports
...
if __name__ == "__main__":
    ...
    class MockDashboardObject:
        def markdown(self, text: str): print(f"Mock Markdown: {text}")
        def warning(self, text: str): print(f"Mock Warning: {text}")
        def plotly_chart(self, fig: Any, **kwargs): print(f"Mock Plotly Chart Rendered: {fig.layout.title.text if hasattr(fig, 'layout') else 'Unknown'}")
        def info(self, text: str): print(f"Mock Info: {text}")
        def button(self, label: str, key: str): return True # Simulate button click for export
        def success(self, text: str): print(f"Mock Success: {text}")

    mock_dash_obj = MockDashboardObject()
    mock_current_result = {
        "plugin_runs": [
            {"plugin_name": "ExampleChaosSecurityPlugin", "function": "perform_custom_security_audit", "result": {"status": "FINDINGS_DETECTED", "findings": ["vuln1", "vuln2"]}},
            {"plugin_name": "OtherPlugin", "function": "do_something", "result": {"status": "OK"}},
            {"plugin_name": "ExampleChaosSecurityPlugin", "function": "perform_custom_security_audit", "result": {"status": "FINDINGS_DETECTED", "findings": ["vuln3"]}},
        ]
    }
    ...
    async def run_batch_export_demo():
        ...
        exported_html_files = await batch_export_panels(panel_ids=["example_plugin_custom_metric_trend"], format="html")
