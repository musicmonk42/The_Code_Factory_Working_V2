...
try:
    ...
    logging.getLogger(__name__).warning(
        "Boto3 not available. AWS Secrets Manager integration will be disabled."
    )
...
if not _base_logger.handlers:
    ...
    formatter = logging.Formatter(
        "%(asctime)s - [%(levelname)s] - %(name)s - component:%(component)s - %(message)s"
    )
...
VALIDATE_PATHS = (
    os.getenv("DLT_VALIDATE_PATHS", "false").lower() == "true"
)  # validate paths even outside production
ENFORCE_PATHS_IN_PROD = (
    os.getenv("DLT_ENFORCE_PATHS_IN_PROD", "true").lower() == "true"
...
def get_or_create_metric(
    metric_type, name, documentation, labelnames: Tuple[str, ...] = (), buckets=None
):
...
def _get_metric_factory():
    ...
    return {
        "load_total": get_or_create_metric(
            Counter,
            "dlt_config_load_total",
            "Total DLT config load attempts",
            ("status",),
        ),
        "validation_errors": get_or_create_metric(
            Counter,
            "dlt_config_validation_errors_total",
            "Total DLT config validation errors",
            ("dlt_type",),
        ),
        "refresh_total": get_or_create_metric(
            Counter,
            "dlt_config_refresh_total",
            "Total DLT config refresh attempts",
            ("status",),
        ),
        "change_total": get_or_create_metric(
            Counter, "dlt_config_change_total", "Total DLT config changes detected"
        ),
        "load_latency": get_or_create_metric(
            Histogram,
            "dlt_config_load_latency_seconds",
            "Latency of DLT config loads",
            ("phase",),
        ),
    }
...
def _scrub_string_value(s: str) -> str:
    ...
    s = AZURE_CONN_RE.sub(
        "DefaultEndpointsProtocol=https;AccountName=[REDACTED];AccountKey=[REDACTED]", s
    )
        ...
        if isinstance(value, dict):
            ...
            sanitized_data[key] = [
                (
                    ...
                    else (_scrub_string_value(item) if isinstance(item, str) else item)
                )
                for item in value
            ]
...
def _load_secret_from_aws_secrets_manager(secret_name: str) -> Optional[str]:
    if not BOTO3_AVAILABLE:
        logger.warning(
            f"Boto3 not available. Cannot load secret '{secret_name}' from AWS Secrets Manager."
        )
        return None
    try:
        ...
        logger.error(
            f"Unexpected error retrieving secret '{secret_name}' from AWS Secrets Manager: {e}",
            exc_info=True,
        )
        return None
...
class S3OffChainConfig(BaseModel):
    ...
    aws_access_key_id: Optional[str] = Field(
        None, description="AWS Access Key ID, defaults to env AWS_ACCESS_KEY_ID."
    )
...
class GcsOffChainConfig(BaseModel):
    ...
    project_id: Optional[str] = Field(
        None, description="GCP project ID, defaults to env GCP_PROJECT_ID."
    )
    credentials_path: Optional[str] = Field(
        None,
        description="Path to GCP service account key file, defaults to env GOOGLE_APPLICATION_CREDENTIALS.",
    )
...
class IpfsOffChainConfig(BaseModel):
    api_url: AnyHttpUrl = Field(
        ..., description="URL of the IPFS API endpoint (e.g., 'http://127.0.0.1:5001')."
    )
...
class FabricDLTConfig(BaseModel):
    ...
    def _validate_paths(self) -> "FabricDLTConfig":
        ...
        if should_check and not os.path.exists(self.network_profile_path):
            raise ValueError(
                f"Fabric network_profile_path does not exist: {self.network_profile_path}"
            )
        return self
...
class EvmDLTConfig(BaseModel):
    ...
    private_key_secret_id: Optional[str] = Field(
        None, description="AWS Secrets Manager ID for the private key."
    )
    poa_middleware: bool = Field(
        False,
    ...
    default_max_priority_fee_per_gas: Optional[int] = Field(
        None,
        description="Default maxPriorityFeePerGas (in Gwei) for EIP-1559 transactions.",
    )
    tx_confirm_timeout: int = Field(
        120, description="Timeout for transaction confirmation in seconds."
    )
    ...
    def enforce_private_key_presence(self) -> "EvmDLTConfig":
        if not self.private_key and self.private_key_secret_id:
            ...
            if loaded_key:
                ...
                raise ValueError(
                    f"Private key secret '{self.private_key_secret_id}' not found in Secrets Manager in production mode."
                )
...
class CordaDLTConfig(BaseModel):
    ...
    password_secret_id: Optional[str] = Field(
        None, description="AWS Secrets Manager ID for the password."
    )
    ...
    def enforce_password_presence(self) -> "CordaDLTConfig":
        if self.password_secret_id:
            ...
            if loaded_password:
                ...
                raise ValueError(
                    f"Password secret '{self.password_secret_id}' not found in Secrets Manager in production mode."
                )

        if PRODUCTION_MODE and not (self.password or self.password_secret_id):
            raise ValueError(
                "In PRODUCTION_MODE, Corda config requires a password or password_secret_id."
            )
        return self
...
class DLTNetworkConfig(BaseModel):
    ...
    secret_scrub_patterns: List[str] = Field(
        default_factory=list,
        description="Regex patterns for scrubbing secrets from logs/exceptions for this specific network.",
    )
    ...
    def load_and_validate(cls, config_data: Dict[str, Any]) -> "DLTNetworkConfig":
        ...
        try:
            ...
            logger.error(
                f"DLT Network Config Validation Error: {e.errors()}",
                exc_info=True,
                extra={"config_name": migrated_data.get("name")},
            )
            raise DLTClientConfigurationError(
                f"Invalid DLT Network Configuration: {e.errors()}", "ConfigManager"
            )
    ...
    def _migrate_schema(cls, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_version < 1:
            logger.info(
                f"Migrating DLT Network Config '{migrated.get('name', 'unknown')}' from v0 to v1.",
                extra={"config_name": migrated.get("name")},
            )
...
class DLTNetworkConfigManager:
        ...
        if all_configs_json:
            try:
                ...
                if isinstance(parsed, list):
                    ...
                    logger.warning(
                        "DLT_NETWORK_CONFIGS_JSON must be a JSON array or object; ignoring."
                    )
            ...
            if env_var_name.startswith("DLT_NETWORK_CONFIG_") and env_var_name.endswith(
                ...
                try:
                    ...
                    if not isinstance(cfg, dict):
                        logger.warning(
                            f"Env var {env_var_name} does not contain a JSON object; skipping."
                        )
                        continue
        ...
        with self._state_lock:
            ...
            for name, raw_cfg in raw_map.items():
                try:
                    ...
                    logger.error(
                        f"Unexpected error validating config '{name}': {e}",
                        exc_info=True,
                    )
    ...
    def _add_config(
        ...
        try:
            ...
            if validated_config.name in target_dict:
                logger.warning(
                    f"Duplicate DLT network config name detected: '{validated_config.name}'. Overwriting existing.",
                    extra={"config_name": validated_config.name},
                )
            ...
            metrics["validation_errors"].labels(dlt_type=str(dlt_type)).inc()
            logger.error(
                f"Invalid DLT network configuration for '{config_data.get('name', 'unnamed')}': {e}",
                exc_info=True,
                extra={"config_data": scrubbed_config_data},
            )
            raise
    ...
    async def refresh_configs_if_changed(self) -> bool:
        ...
        with self._state_lock:
            if current_hash != self._last_config_hash:
                logger.info(
                    "Detected DLT network configuration change. Reloading configurations...",
                    extra={
                ...
                logger.info(
                    f"DLT network configurations reloaded. Now {len(self._configs)} active configs.",
    ...
    def start_background_refresh(
        self, stop_event: Optional[threading.Event] = None, jitter: float = 0.2
    ) -> threading.Thread:
        """
...
def get_dlt_network_config_manager(
        ...
        os.environ["DLT_NETWORK_CONFIG_DEV_FABRIC_JSON"] = json.dumps(
            {
                ...
                "fabric": {
                    ...
                    "chaincode_name": "dev-cc",
                    "org_name": "dev-org",
                    "user_name": "user1",
                    "network_profile_path": __file__,  # point to existing file for path check
                    "peer_names": ["peer0.dev.com"],
                    "invoke_timeout": 30,
                },
        ...
        os.environ["DLT_NETWORK_CONFIG_PROD_EVM_JSON"] = json.dumps(
            {
                ...
                "azure_blob": {
                    "connection_string": "DefaultEndpointsProtocol=https;AccountName=myacct;AccountKey=supersecret",
                    "container_name": "prod-blob-container",
                },
