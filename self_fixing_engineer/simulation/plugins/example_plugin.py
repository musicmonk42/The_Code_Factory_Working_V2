...
if not _boot_logger.handlers:
    ...
    _boot_handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(name)s: %(message)s"))
...
if not plugin_logger.handlers:
    plugin_logger.setLevel(os.getenv(f"{PLUGIN_MANIFEST['name'].upper()}_LOG_LEVEL", "INFO").upper())
...
def run_custom_chaos_experiment(target_id: str, intensity: float = 0.5) -> Dict[str, Any]:
    ...
    """
    correlation_id = hashlib.sha256(f"{target_id}:{time.time_ns()}".encode("utf-8")).hexdigest()[:12]
...
class SecurityAuditParams(BaseModel):
    code_path: str = Field(..., min_length=1, description="Path to the code file for security audit.")
...
def perform_custom_security_audit(code_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not candidate_real.startswith(sanctioned_real + os.sep) and candidate_real != sanctioned_real:
            raise PermissionError(f"File access denied: {code_path} is outside sanctioned directory.")
    ...
    try:
        ...
        if sz_bytes > max_mb * 1024 * 1024:
            plugin_logger.warning(f"Skipping audit for large file (> {max_mb} MB): {candidate_real}")
            return {
                "plugin_name": PLUGIN_MANIFEST["name"],
    ...
    try:
        ...
        with io.open(candidate_real, "r", encoding="utf-8", errors="ignore") as f:
            for line_num, line in enumerate(f, 1):
                ...
                if re.search(r"\bos\.system\s*\(", line) or re.search(r"\bsubprocess\.(Popen|call|run)\s*\(", line):
                    findings.append({
                        "line": line_num,
        ...
        result = {
            ...
            "message": "Security audit completed." if findings == [] else "Security audit completed with findings.",
        ...
        plugin_logger.info(f"Security audit on {code_path} completed in {duration:.2f}s with severity={overall_severity}.")
        return result
...
def example_plugin_dashboard_panel(st_dash_obj: Any, current_result: Dict[str, Any]):
    ...
    if last_chaos_run and last_chaos_run.get("plugin_name") == PLUGIN_MANIFEST["name"]:
        st_dash_obj.write(f"**Last Chaos Experiment Status:** {last_chaos_run.get('status', 'N/A')}")
        st_dash_obj.write(f"Target: {last_chaos_run.get('target', 'N/A')}, Intensity: {last_chaos_run.get('intensity', 'N/A')}")
    ...
    audit_findings = [
        ...
        {"file": "utils.py", "finding": "Insecure temporary file creation", "severity": "Medium"},
    ]

    if audit_findings:
        st_dash_obj.write("Potential Security Findings:")
        for finding in audit_findings:
            st_dash_obj.warning(f"- **{finding['severity']}**: {finding['finding']} in `{finding['file']}`")
    else:
    ...
    if st_dash_obj.session_state.get("enable_live_data", False):
        st_dash_obj.text(f"Live data enabled for this panel. Current time: {datetime.now().strftime('%H:%M:%S')}")
