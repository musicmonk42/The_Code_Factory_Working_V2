...
PLUGIN_MANIFEST = {
    ...
    "capabilities": [
        "scala_test_execution",
        "scala_coverage_analysis",
        "sbt_integration",
    ],
...
def _find_scoverage_xml(
    ...
    pattern = os.path.join(
        sbt_project_root, "target", "scala-*", "scoverage-report", "scoverage.xml"
    )
...
async def run_scala_tests(
    ...
    **kwargs,
    ...
    full_coverage_report_path = os.path.abspath(
        os.path.join(project_root, temp_coverage_report_path_relative)
    )
    ...
    if not os.path.exists(os.path.join(sbt_project_root, "build.sbt")):
        ...
        if target_identifier.endswith(".scala"):
            ...
            if os.path.isdir(src_main_scala):
                ...
                logger.info(
                    f"Copied {copied_files} source files ({copied_bytes/1024/1024:.2f} MB) to temp SBT project (bounded)."
                )
            else:
                ...
                if os.path.exists(full_original_target_path):
                    ...
                    logger.info(
                        f"Copied target source file to temp SBT project: {full_original_target_path} -> {target_file_in_temp}"
                    )
                else:
                    logger.warning(
                        f"Target source file {full_original_target_path} not found. Coverage might be inaccurate."
                    )
        ...
        logger.info(
            f"Copied generated test file to temp SBT project: {full_test_file_path} -> {temp_test_path}"
        )
    ...
    if (
        target_identifier
        and not target_identifier.endswith(".scala")
        and not target_identifier.startswith("src/")
    ):
        ...
        if safe_identifier:
            ...
            logger.warning(
                f"Skipping testOnly due to unsafe target identifier: {target_identifier}"
            )
    ...
    try:
        ...
        try:
            stdout_bytes, stderr_bytes = await asyncio.wait_for(
                proc.communicate(),
                timeout=kwargs.get("timeout_seconds", SCALA_RUNNER_TIMEOUT_SEC),
            )
        except asyncio.TimeoutError:
            logger.error(
                f"sbt timed out after {kwargs.get('timeout_seconds', SCALA_RUNNER_TIMEOUT_SEC)}s. Terminating..."
            )
            try:
                proc.terminate()
        ...
        if (
            ...
            result["success"] = True
            result["reason"] = (
                f"SBT tests passed. {test_summary['tests']} tests, {test_summary['skipped']} skipped."
            )
            logger.info(result["reason"])
        else:
            result["success"] = False
            result["reason"] = (
                f"SBT tests finished. {test_summary['failures']} failures, {test_summary['errors']} errors, {test_summary['tests']} total."
            )
            logger.warning(result["reason"])

        # Find Scoverage XML dynamically
        scoverage_xml_report_path = _find_scoverage_xml(
