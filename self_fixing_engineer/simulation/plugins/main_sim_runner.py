...
if prometheus_available:
    remote_jobs_submitted = Counter(
        "remote_jobs_submitted_total", "Number of remote jobs submitted"
    )
    ...
    simulation_run_duration_hist = Histogram(
        "simulation_run_duration_seconds",
        "Duration of entire simulation runs in seconds",
    )
...
def register_entrypoint(*args, **kwargs):
    ...
    if not fn:
        fn = kwargs.get("entrypoint_func") or kwargs.get("runner") or kwargs.get("func")
...
def _synthesize_kwargs_for_runner(
    rf: Callable, module_name: str, language_or_framework: str, args: argparse.Namespace
) -> Dict[str, Any]:
    """
    ...
    if "temp_coverage_report_path_relative" in params:
        defaults["temp_coverage_report_path_relative"] = os.path.join(
            coverage_dir_rel, coverage_filename
        )
...
def _plugin_register_adapter(module_name: str):
    ...
    def adapter(language_or_framework=None, runner_info: Dict[str, Any] | None = None, **kw):
        ...
        def _entrypoint(args: argparse.Namespace):
            ...
            if callable(rf):
                try:
                    synthesized = _synthesize_kwargs_for_runner(
                        rf, module_name, language_or_framework, args
                    )
...
def discover_and_register_plugin_entrypoints():
    ...
    for plugin_file in glob.glob(os.path.join(plugin_dir, "*.py")):
        ...
        try:
                ...
                if (
                    ...
                    if varname in ("PLUGIN_MANIFEST", "__manifest__", "manifest"):
                        try:
                            ...
                            continue
            manifest = (
                manifest_candidates.get("PLUGIN_MANIFEST")
                or manifest_candidates.get("__manifest__")
                or manifest_candidates.get("manifest")
                or {}
            )
            ...
            if hasattr(module, "register_plugin_entrypoints") and callable(
                ...
                main_runner_logger.warning(
                    f"Plugin module '{module_name}' has no register_plugin_entrypoints; skipping."
                )
                continue
            ...
            _registered_plugin_info[module_name] = {
                "version": version,
                "hash": plugin_hash,
            ...
            main_runner_logger.error(
                f"Failed to load or register entrypoints from plugin '{module_name}': {e}"
            )
            main_runner_logger.debug(
                f"Traceback for plugin '{module_name}': {traceback.format_exc()}"
            )
...
def validate_deployment_or_exit(remote: bool = False):
    ...
    if not remote and skip_local:
        main_runner_logger.info(
            "Skipping local environment validation (SIM_RUNNER_SKIP_VALIDATION_FOR_LOCAL=true)."
        )
        return
    ...
    with open(log_path, "w", encoding="utf-8") as validation_log:
        ...
        if missing_env:
            ...
            log_validation(
                "[VALIDATION PASS] Required environment variables present for this mode."
            )
                ...
                try:
                    ...
                    if i == attempts - 1:
                        if remote:
                            ...
                            log_validation(
                                f"[VALIDATION WARN] OPA health check failed (local mode): {e}"
                            )
                    else:
                        time.sleep(1.5 * (i + 1))
        except Exception as e:
...
def check_rbac_permission(actor: str, action: str, resource: str) -> bool:
    ...
    opa_url = os.environ.get("OPA_URL")
    fail_open = os.environ.get("SIM_RUNNER_RBAC_FAIL_OPEN", "false").lower() in (
        "1",
        "true",
        "yes",
    )
    if not opa_url:
        main_runner_logger.warning(
            "OPA_URL not set; RBAC check skipped. Returning {} by policy.".format(
                "ALLOW" if fail_open else "DENY"
            )
        )
...
def enforce_kernel_sandboxing(profile_path: str, cgroup: str = None, apparmor_profile: str = None):
    ...
    main_runner_logger.warning(
        "Seccomp enforcement not applied (allow-all). Configure container-level sandboxing (firejail/bwrap/AppArmor/SELinux) for real isolation."
    )
    ...
    try:
        ...
        with tarfile.open(tar_path, "w:gz") as tar:
            ...
            tar.add(
                ...
                arcname=".",
                filter=_tar_filter([f"./{p}" for p in exclude]),
            )
        if prometheus_available:
        ...
        resp = retry_op(
            lambda: batch_v1.create_namespaced_job(
                namespace=job_config.get("namespace", "default"), body=job
            )
        )
        ...
        while True:
            ...
            completed = (
                any(c.type == "Complete" and c.status == "True" for c in conditions)
                or succeeded > 0
            )
            has_failed = (
...
def run_plugin_in_sandbox(
    plugin_name: str, args: argparse.Namespace, sandbox: bool = True
) -> Dict[str, Any]:
    if sandbox:
...
def aggregate_simulation_results(
    core_result: Dict[str, Any], plugin_results: List[Dict[str, Any]]
) -> Dict[str, Any]:
...
def main():
    ...
    parser.add_argument(
        "--watch", action="store_true", help="Watch mode: auto-rerun on file changes."
    )
    parser.add_argument(
        "--dashboard",
        action="store_true",
        help="Launch Streamlit dashboard for result visualization.",
    )
    parser.add_argument(
        "--remote",
        action="store_true",
        help="Execute the simulation remotely, e.g., via Kubernetes.",
    )
    parser.add_argument(
        "--namespace", help="Kubernetes namespace for remote job.", default="default"
    )
    parser.add_argument(
        "--container-image",
    ...
    parser.add_argument(
        "--policy", help="Simulation policy to apply (e.g., strict, chaos, custom)."
    )
    ...
    parser.add_argument(
        "--notify",
        action="store_true",
        help="Enable notifications on errors or completion.",
    )
    ...
    parser.add_argument(
        "--insecure",
        action="store_true",
        help="Run in insecure mode without sandboxing (development only).",
    )
    parser.add_argument(
        "--sandbox-plugins",
        action="store_true",
        help="Run all plugins in sandbox during bulk execution loop.",
    )
    ...
    sandbox_env = os.environ.get("SIM_RUNNER_SANDBOX_PLUGINS", "false").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    if not sandbox_plugins:
        main_runner_logger.warning(
            "Bulk plugin execution is NOT sandboxed. Use --sandbox-plugins or SIM_RUNNER_SANDBOX_PLUGINS=true to enable sandboxing."
        )
    ...
    with tracer.start_as_current_span("main_run", context=otel_context):
        ...
        if args.run_plugin:
            ...
            with tracer.start_as_current_span(f"plugin_run_{plugin_name}"):
                try:
                    plugin_output = run_plugin_in_sandbox(
                        plugin_name, args, sandbox=not args.insecure
                    )
                    main_runner_logger.info(
                        f"Plugin '{plugin_name}' execution result: {plugin_output}"
                    )
            ...
            try:
                ...
                # object with attrs
                    ...
                    discovered_test = getattr(discovered, "test", None) or getattr(
                        discovered, "test_file", None
                    )
                    discovered_code = getattr(discovered, "code", None) or getattr(
                        discovered, "code_file", None
                    )
            except Exception:
                discovered_test = discovered_code = None
        ...
        def run_and_report():
            ...
            with tracer.start_as_current_span("core_simulation"):
                try:
                    if args.agentic:
                        ...
                        maybe_swarm = core.run_simulation_swarm(
                            test_file_path,
                            code_file_path,
                            config,
            ...
            final_result = aggregate_simulation_results(
                core_simulation_result, plugin_execution_results
            )
            out_path = utils.save_sim_result(
                session_name,
                os.path.basename(code_file_path or "no_code"),
                final_result,
            )
            ...
            # Color output toggle
            no_color = os.environ.get("NO_COLOR", "false").lower() in (
                "1",
                "true",
                "yes",
            )
        ...
        if args.remote:
            ...
            main_runner_logger.debug(
                f"Remote packaging excludes: {['.git', '.venv', 'venv', '__pycache__', 'node_modules']}"
            )
