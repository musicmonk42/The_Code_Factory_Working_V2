...
try:
    ...
    logging.warning("google-cloud-logging not found. GCP Cloud Logging integration will be disabled.")

try:
    from azure.monitor.query.aio import LogsQueryClient
    AZURE_MONITOR_QUERY_AVAILABLE = True
except ImportError:
    logging.warning("azure-monitor-query not found. Azure Monitor query functionality will be disabled.")
...
try:
    ...
    logging.warning("azure-monitor-ingestion not found. Azure Monitor log ingestion will be disabled.")

logger = logging.getLogger(__name__)

# --- Async Retry Helper ---
async def _async_retry(func, *args, retries=3, min_delay=1, max_delay=5, jitter=True, exceptions=(Exception,), **kwargs):
    """
...
class CloudLoggingError(Exception):
    ...
    def __init__(self, message: str, cloud_type: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None):
...
class CloudLoggingResponseError(CloudLoggingError):
    ...
    def __init__(self, message: str, cloud_type: str, status_code: int, response_text: str, original_exception: Optional[Exception] = None, details: Optional[Dict[str, Any]] = None):
...
class BaseCloudLogger:
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100) -> List[Dict[str, Any]]:
    ...
    def _parse_relative_time_range_to_ms(self, time_range_str: str) -> int:
        if not time_range_str or len(time_range_str) < 2: return 24 * 3600 * 1000
        unit = time_range_str[-1].lower()
        try: value = int(time_range_str[:-1])
        except ValueError: return 24 * 3600 * 1000
        if unit == 's': return value * 1000
        elif unit == 'm': return value * 60 * 1000
        elif unit == 'h': return value * 3600 * 1000
        elif unit == 'd': return value * 24 * 3600 * 1000
        else: return 24 * 3600 * 1000

    def _parse_relative_time_range_to_timedelta(self, time_range_str: str) -> datetime.timedelta:
        if not time_range_str or len(time_range_str) < 2: return datetime.timedelta(hours=24)
        unit = time_range_str[-1].lower()
        try: value = int(time_range_str[:-1])
        except ValueError: return datetime.timedelta(hours=24)
        ...
        else: return datetime.timedelta(hours=24)
...
class CloudWatchLogger(BaseCloudLogger):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.log_group_name:
            raise CloudLoggingConfigurationError("AWS Log Group Name must be configured.", self.cloud_type)
    ...
    async def _get_aws_client(self):
        if self._cw_logs_client is None:
            self._cw_logs_client = await self._to_thread(lambda: boto3.client('logs', region_name=self.region_name))
    ...
    async def flush(self):
        ...
        while self._log_buffer:
            batch = [self._log_buffer.popleft() for _ in range(min(self.batch_size, len(self._log_buffer)))]
            log_events = [{'timestamp': int(time.time() * 1000), 'message': json.dumps(event)} for event in batch]
            put_args = {'logGroupName': self.log_group_name, 'logStreamName': self.log_stream_name, 'logEvents': log_events}
            if self._sequence_token:
                put_args['sequenceToken'] = self._sequence_token

            try:
                # Use retry logic for the API call
                response = await _async_retry(_put_events, put_args, retries=3, min_delay=1, max_delay=5, exceptions=(AWSClientError,))
                self._sequence_token = response.get('nextSequenceToken')
                rejected_info = response.get('rejectedLogEventsInfo')
                if rejected_info and not isinstance(rejected_info, MagicMock):  # Avoid logging mocks
                ...
                logger.error(f"Failed to flush logs to CloudWatch after retries: {e}", exc_info=True)
                self._sequence_token = None # Invalidate token on failure
                raise CloudLoggingResponseError("AWS PutLogEvents failed", self.cloud_type, 500, str(e), e)

    async def _get_latest_sequence_token(self, client) -> Optional[str]:
        try:
            response = await self._to_thread(lambda: client.describe_log_streams(
    ...
    async def health_check(self) -> Tuple[bool, str]:
        ...
        try:
            client = await self._get_aws_client()
            await self._to_thread(lambda: client.describe_log_groups(logGroupNamePrefix=self.log_group_name, limit=1))
            ...
            if hasattr(e, 'response') and isinstance(e.response, dict):
                ...
                if error_code == 'AccessDeniedException':
                    ...
                    raise CloudLoggingAuthError(f"AWS authorization failed: {msg}", self.cloud_type, e) from e
            # For any other AWS error, wrap it in our custom generic error
            raise CloudLoggingError(f"Unexpected error during AWS health check: {e}", self.cloud_type, e) from e
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            query_id = (await self._to_thread(lambda: client.start_query(
                logGroupName=self.log_group_name, startTime=start_time_s, endTime=end_time_s, queryString=query_string, limit=limit
            ))).get('queryId')

            if not query_id: raise CloudLoggingQueryError("Failed to start CloudWatch Logs query.", self.cloud_type)
            
            started = time.time()
            while True:
                results_response = await self._to_thread(lambda: client.get_query_results(queryId=query_id))
                status = results_response.get('status')
                if status == 'Complete':
                    return [{field['field']: field['value'] for field in row} for row in results_response.get('results', [])]
...
class GCPLogger(BaseCloudLogger):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not GCP_AVAILABLE:
            raise CloudLoggingConfigurationError("google-cloud-logging not installed for GCP.", self.cloud_type)
    ...
    async def _get_gcp_client(self) -> gcp_logging_sdk.Client:
        if self._logging_client is None:
            if self.credentials_path:
                self._logging_client = await self._to_thread(lambda: gcp_logging_sdk.Client.from_service_account_json(self.credentials_path, project=self.project_id))
            else:
                self._logging_client = await self._to_thread(lambda: gcp_logging_sdk.Client(project=self.project_id))
    ...
    async def flush(self):
        ...
        while self._log_buffer:
            ...
            try:
                ...
                for event in reversed(batch): self._log_buffer.appendleft(event)
                logger.error(f"Failed to flush logs to GCP: {e}", exc_info=True)
                raise CloudLoggingResponseError("GCP API Error", self.cloud_type, getattr(e, "code", 500), str(e), e)
    ...
    async def health_check(self) -> Tuple[bool, str]:
        try:
            ...
            raise CloudLoggingError(f"Unexpected error during GCP health check: {e}", self.cloud_type, e)
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100) -> List[Dict[str, Any]]:
        ...
        filters = [f'logName="projects/{self.project_id}/logs/{self.log_name}"', f'timestamp >= "{start_time.isoformat()}Z"', f'timestamp <= "{end_time.isoformat()}Z"']
        if query_string: filters.append(query_string)
        
        try:
            def _collect():
                iterator = client.list_entries(filter=" AND ".join(filters), order_by=gcp_logging_sdk.DESCENDING, page_size=limit)
...
class AzureMonitorLogger(BaseCloudLogger):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.data_collection_endpoint = azure_config.get("data_collection_endpoint", os.getenv("AZURE_DCE"))
        ...
        if not all([AZURE_IDENTITY_AVAILABLE, AZURE_MONITOR_INGESTION_AVAILABLE, AZURE_MONITOR_QUERY_AVAILABLE]):
            raise CloudLoggingConfigurationError("Required Azure libraries not installed.", self.cloud_type)
        if not all([self.data_collection_endpoint, self.dcr_immutable_id, self.stream_name]):
            raise CloudLoggingConfigurationError("Azure config requires DCE, DCR_ID, and stream_name.", self.cloud_type)
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if self._ingestion_client: await self._ingestion_client.close()
        if self._query_client: await self._query_client.close()
        if self._credential: await self._credential.close()

    async def _get_credential(self) -> DefaultAzureCredential:
        if self._credential is None: self._credential = DefaultAzureCredential()
    ...
    async def _get_ingestion_client(self) -> LogsIngestionClient:
        if self._ingestion_client is None:
            ...
            self._ingestion_client = LogsIngestionClient(endpoint=self.data_collection_endpoint, credential=creds)
    ...
    async def flush(self):
        ...
        while self._log_buffer:
            batch = [self._log_buffer.popleft() for _ in range(min(self.batch_size, len(self._log_buffer)))]
            try:
                await client.upload(rule_id=self.dcr_immutable_id, stream_name=self.stream_name, logs=batch)
            except Exception as e:
                for event in reversed(batch): self._log_buffer.appendleft(event)
                logger.error(f"Failed to flush logs to Azure: {e}", exc_info=True)
                raise CloudLoggingResponseError("Azure ingestion API call failed", self.cloud_type, 500, str(e), e)
    ...
    async def health_check(self) -> Tuple[bool, str]:
        try:
            ...
            raise CloudLoggingAuthError(f"Azure authentication/configuration failed: {e}", self.cloud_type, e)
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = await client.query_workspace(
                workspace_id=self.workspace_id, query=full_kql_query, timespan=(start_time, end_time)
            )
            if response and response.tables:
                return [dict(zip([col.name for col in table.columns], row)) for table in response.tables for row in table.rows]
            return []
        except Exception as e:
            raise CloudLoggingQueryError(f"Failed to query Azure Monitor Logs: {e}", self.cloud_type, e)
