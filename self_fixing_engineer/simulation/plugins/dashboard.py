...
try:
    ...
    logging.warning(f"PluginManager is not available. Plugin Gallery functionality will be limited. Error: {e}")
finally:
...
try:
    ...
    logging.warning(f"MeshPubSub or CheckpointManager not found. Onboarding and full health checks will be limited. Error: {e}")
finally:
...
try:
    ...
    try:
        ...
        REDIS_AVAILABLE = False
        redis_client = None
        logging.warning(f"Could not connect to Redis at {REDIS_URL}: {e}. Live data streaming will be unavailable.")
except ImportError:
    pass
...
DANGEROUS_NAMES = {'os', 'sys', 'subprocess', 'shutil', 'importlib', 'socket', 'threading', 'multiprocessing', 'ctypes', 'builtins'}
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if STREAMLIT_AVAILABLE:
                st_dash.error(f"An error occurred in a component from plugin '{plugin_name}'. Please check logs for details. Error: {e}")
            return None
    return wrapper
...
def load_plugin_dashboard_panels_cached():
    ...
    if not os.path.exists(Config.PLUGINS_DIR):
        logging.warning(f"Plugins directory not found: {Config.PLUGINS_DIR}. No dashboard plugins will be loaded.")
        return
    ...
    for plugin_file in glob.glob(os.path.join(Config.PLUGINS_DIR, "*.py")):
        ...
        if module_name == "__init__" or module_name in DANGEROUS_NAMES:
            logging.warning(f"Skipping potentially dangerous or invalid plugin module: {module_name}")
            continue
        try:
            ...
            if not is_version_compatible(DASHBOARD_CORE_VERSION, min_core_version, max_core_version): # Use DASHBOARD_CORE_VERSION
                logging.warning(f"Plugin {module_name} (Core API v{min_core_version}-{max_core_version}) incompatible with Dashboard Core v{DASHBOARD_CORE_VERSION}. Skipping UI registration.")
                continue

            # Check manifest for dangerous names (if a manifest exists alongside single .py)
            manifest_path = os.path.join(Config.PLUGINS_DIR, module_name, "manifest.json") # Check for co-located manifest
            ...
            if os.path.exists(manifest_path):
                with open(manifest_path, 'r', encoding='utf-8') as f:
                    ...
                    if manifest_data.get('name') in DANGEROUS_NAMES or any(dep in DANGEROUS_NAMES for dep in manifest_data.get('dependencies', [])):
                        logging.warning(f"Manifest for {module_name} contains dangerous names. Skipping.")
                        continue


            if hasattr(module, 'register_my_dashboard_panels') and callable(module.register_my_dashboard_panels):
                ...
                logging.info(f"Plugin {module_name} does not have a 'register_my_dashboard_panels' function.")
            
            # Check for UI components in single-file plugins
            title = getattr(module, 'TITLE', module_name.capitalize()) if hasattr(module, 'TITLE') else module_name.capitalize()
            ...
            if hasattr(module, 'render_sidebar_component') and callable(module.render_sidebar_component):
                _dashboard_sidebar_components.append({
                    "plugin": module_name,
                    "title": title,
                    "func": plugin_callback_handler(module.render_sidebar_component), # Apply decorator
                    "description": description,
                    "roles": roles
                })
            ...
            if hasattr(module, 'render_main_component') and callable(module.render_main_component):
                _dashboard_main_components.append({
                    "plugin": module_name,
                    "title": title,
                    "func": plugin_callback_handler(module.render_main_component), # Apply decorator
                    "description": description,
                    "roles": roles
                })
                logging.info(f"Registered main component for plugin: {module_name}")
        except Exception as e:
            logging.error(f"Error loading or registering dashboard panels from plugin {plugin_file}: {e}", exc_info=True)
    ...
    for item in os.listdir(Config.PLUGINS_DIR):
        ...
        if not is_version_compatible(DASHBOARD_CORE_VERSION, min_core_version, max_core_version):
            logging.warning(f"Plugin {item} (Core API v{min_core_version}-{max_core_version}) incompatible with Dashboard Core v{DASHBOARD_CORE_VERSION}. Skipping UI registration.")
            continue
        ...
        try:
            ...
            if hasattr(module, 'register_my_dashboard_panels') and callable(module.register_my_dashboard_panels):
                ...
                module.register_my_dashboard_panels(register_dashboard_panel)
            title = getattr(module, 'TITLE', item.capitalize()) if hasattr(module, 'TITLE') else item.capitalize()
            description = getattr(module, 'DESCRIPTION', '')
            roles = getattr(module, 'ROLES', [])
...
def is_version_compatible(current_version: str, min_version: str, max_version: str) -> bool:
    ...
    try:
        ...
        logging.warning("Python 'packaging' library not found. Falling back to simple string comparison.")
        # Fallback to simple string comparison if 'packaging' is not available
        return min_version <= current_version <= max_version
    except Exception as e:
        logging.warning(f"Error parsing version string with 'packaging': {e}. Falling back to simple string comparison.")
        return min_version <= current_version <= max_version
...
def load_all_simulation_results(results_dir: str = Config.RESULTS_DIR) -> List[Dict[str, Any]]:
    all_results = []
    ...
    all_results.sort(key=lambda x: os.path.getmtime(x['_filepath']) if '_filepath' in x else 0, reverse=True)
    return all_results
...
def display_core_metrics(selected_result: Dict[str, Any]):
    ...
    with col1:
        ...
        failed_runs_count = sum(1 for r in selected_result.get("runs", []) if isinstance(r, dict) and r.get("returncode") != 0)
...
try:
    ...
    logging.warning("Workflow visualization (workflow_viz.py) not found. This feature will be unavailable.")
...
def display_plugin_gallery(plugin_manager: PluginManager, user_role: str):
    ...
    if error_plugins:
        ...
        for ep in error_plugins:
            st_dash.error(f"Plugin: {ep.get('name', 'Unnamed')} — {ep.get('error', 'Unknown error')}")
    ...
    for plugin in all_plugins:
        ...
        with st_dash.expander(f"**{name}** (Status: `{status}`)", expanded=False):
            ...
            st_dash.markdown(f"**Description:** {manifest.get('description', 'No description provided.')}")
            ...
            if not is_version_compatible(DASHBOARD_CORE_VERSION, min_core_ver, max_core_ver):
                st_dash.warning(f"Plugin might be incompatible with Dashboard Core v{DASHBOARD_CORE_VERSION}.")

            st_dash.markdown(f"**Permissions:** `{', '.join(manifest.get('permissions', ['None']))}`")
            st_dash.markdown(f"**Capabilities:** `{', '.join(manifest.get('capabilities', ['None']))}`")
            ...
            if user_role == "admin":
                if status == "disabled" or status == "error":
                    if col_buttons[0].button(f"✅ Enable {name}", key=f"enable_{name}", help=t("enable_plugin_tooltip")):
                        try:
                    ...
                    if col_buttons[0].button(f"❌ Disable {name}", key=f"disable_{name}", help=t("disable_plugin_tooltip")):
                        try:
            ...
            if manifest.get('type') == 'python' and user_role == "admin":
                if col_buttons[1].button(f"🔄 Reload {name}", key=f"reload_{name}", help=t("reload_plugin_tooltip")):
                    try:
            ...
            # Health Check button
            if col_buttons[2].button(f"❤️ Health Check {name}", key=f"health_{name}", help=t("run_health_check_tooltip")):
                try:
                    ...
                    if health_status.get('status') != 'ok' and user_role == 'admin':
                         if st_dash.button(f"Reset Plugin {name}", key=f"reset_plugin_{name}", help=t("reset_plugin_tooltip")):
                            ...
                            st_dash.info(f"Conceptual reset initiated for {name}. Actual reset logic needs implementation.")
                            # Example of a reload after conceptual reset
                            try:
                                plugin_manager.disable_plugin(name)
                                time.sleep(0.1) # brief pause
                                plugin_manager.enable_plugin(name)
                                st_dash.success(f"Plugin '{name}' re-enabled after conceptual reset.")
                                st_dash.rerun()
                            except Exception as re:
                                st_dash.error(f"Error during re-enable after conceptual reset: {re}")
                except Exception as e:
                    st_dash.error(f"Health check failed for '{name}': {e}")
                    st_dash.exception(e) # Show full traceback
                    if user_role == 'admin':
                        if st_dash.button(f"Reset Plugin {name}", key=f"reset_plugin_fail_{name}", help=t("reset_plugin_tooltip")):
                            st_dash.info(f"Conceptual reset initiated for {name}. Actual reset logic needs implementation.")
                            try:
                                plugin_manager.disable_plugin(name)
                                time.sleep(0.1)
...
def sanitize_plugin_name(plugin_name: str) -> str:
    ...
    plugin_name = re.sub(r'[^a-zA-Z0-9_]', '', plugin_name)  # Allow only alphanumeric and underscore
...
def _generate_plugin_manifest_gui(plugin_type: str, plugin_name: str, plugins_dir: str):
    ...
    if manifest['name'] in DANGEROUS_NAMES:
        st_dash.error(f"Generated plugin name '{plugin_name}' is a dangerous name. Please choose another.")
...
"""
        ...
        st_dash.info(f"Created placeholder WASM file: {wasm_filepath}. Replace with your compiled .wasm binary.")
...
async def _run_health_checks_gui(config: Dict[str, Any], test_all_plugins: bool = False):
    ...
    if ONBOARDING_BACKENDS_AVAILABLE:
        ...
        if pubsub_backend_url:
            ...
            try:
                ...
                if pubsub_backend_url.startswith("gcs://"):
                    mesh_kwargs["gcs_bucket_name"] = config["notification_backend"].get("gcs_bucket_name")
                elif pubsub_backend_url.startswith("azure://"):
                    mesh_kwargs["azure_connection_string"] = config["notification_backend"].get("azure_connection_string")
                    mesh_kwargs["azure_container_name"] = config["notification_backend"].get("azure_container_name")
                elif pubsub_backend_url.startswith("etcd://"):
                    mesh_kwargs["etcd_host"] = config["notification_backend"].get("etcd_host")
                    mesh_kwargs["etcd_port"] = int(config["notification_backend"].get("etcd_port", 2379))

                mesh = MeshPubSub(backend_url=pubsub_backend_url, **mesh_kwargs)
                await mesh.connect()
                health = await mesh.healthcheck()
                st_dash.success(f"Pub/Sub Health: {health['status'].upper()} - {health.get('message', '')}")
                await mesh.close()
            except Exception as e:
        ...
        st_dash.warning("Required backend modules for Pub/Sub not available. Cannot perform health check.")
    ...
    if ONBOARDING_BACKENDS_AVAILABLE:
        ...
        if checkpoint_backend_type:
            ...
            try:
                ...
                if backend_config_key in config["checkpoint_backend"]:
                    chk_manager_kwargs[backend_config_key] = config["checkpoint_backend"][backend_config_key]
                ...
                if loaded_data and loaded_data.get("status") == "healthy":
                    st_dash.success(f"Checkpoint Health: OK (saved and loaded test data successfully for {checkpoint_backend_type}).")
                else:
                    st_dash.error(f"Checkpoint Health FAILED: Data mismatch for {checkpoint_backend_type}.")
        ...
        st_dash.warning("Required backend modules for Checkpointing not available. Cannot perform health check.")
            ...
            if status == 'loaded':
                try:
                    ...
                    if health_status.get('status') == 'ok':
                        st_dash.success(f"Plugin '{name}' Health: OK - {health_status.get('message', '')}")
                    else:
                        st_dash.warning(f"Plugin '{name}' Health: {health_status.get('status', 'FAIL').upper()} - {health_status.get('message', '')}")
                        failed_checks.append(f"Plugin: {name}")
                except Exception as e:
                    plugin_health_results[name] = {"status": "error", "message": str(e), "traceback": traceback.format_exc()}
                    st_dash.error(f"Plugin '{name}' Health Check FAILED: {e}")
                    logging.error(f"Plugin '{name}' Health Check FAILED: {e}", exc_info=True)
                    failed_checks.append(f"Plugin: {name}")
            else:
                st_dash.info(f"Skipping health check for plugin '{name}' as it is not loaded (Status: {status}).")
...
def display_onboarding_wizard():
    ...
    with st_dash.form("onboarding_form"):
        ...
        project_type = st_dash.selectbox(
            ...
            index=0, key="onboard_project_type", help=t("project_type_tooltip")
        )
        ...
        selected_plugin_types = st_dash.multiselect(
            ...
            default=["python"], key="onboard_plugin_types", help=t("plugin_types_tooltip")
        )
        ...
        if ONBOARDING_BACKENDS_AVAILABLE:
            try:
                pubsub_backend_options.extend([b for b in MeshPubSub.supported_backends() if b != "local"])
                pubsub_backend_options = sorted(list(set(pubsub_backend_options)))
            except Exception:
                # Fallback list if supported_backends() fails
                pubsub_backend_options.extend(["redis", "nats", "kafka", "rabbitmq", "aws", "gcs", "azure", "etcd"])
                pubsub_backend_options = sorted(list(set(pubsub_backend_options)))

        pubsub_backend = st_dash.selectbox(
            "Choose your preferred notification backend:",
            options=pubsub_backend_options,
            index=pubsub_backend_options.index("redis") if "redis" in pubsub_backend_options else 0,
            key="onboard_pubsub_backend", help=t("pubsub_backend_tooltip")
        )
        pubsub_config = {"type": pubsub_backend}
        if pubsub_backend == "redis":
            pubsub_config["url"] = st_dash.text_input("Redis URL", value="redis://localhost:6379/0", key="pubsub_redis_url", help=t("redis_url_tooltip"))
        elif pubsub_backend == "nats":
            pubsub_config["url"] = st_dash.text_input("NATS URL", value="nats://localhost:4222", key="pubsub_nats_url", help=t("nats_url_tooltip"))
        elif pubsub_backend == "kafka":
            pubsub_config["url"] = st_dash.text_input("Kafka Bootstrap Servers (e.g., localhost:9092)", value="localhost:9092", key="pubsub_kafka_url", help=t("kafka_url_tooltip"))
        elif pubsub_backend == "rabbitmq":
            pubsub_config["url"] = st_dash.text_input("RabbitMQ AMQP URL", value="amqp://guest:guest@localhost:5672/", key="pubsub_rabbitmq_url", help=t("rabbitmq_url_tooltip"))
        elif pubsub_backend == "aws":
            pubsub_config["url"] = "aws://"
            st_dash.info("For AWS backend, ensure AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION_NAME are set as environment variables.")
        elif pubsub_backend == "gcs":
            pubsub_config["url"] = "gcs://"
            pubsub_config["gcs_bucket_name"] = st_dash.text_input("GCS Bucket Name for events", value="your-gcs-event-bucket", key="pubsub_gcs_bucket", help=t("gcs_bucket_tooltip"))
            st_dash.info("For GCS backend, ensure GOOGLE_APPLICATION_CREDENTIALS points to your service account key.")
        elif pubsub_backend == "azure":
            pubsub_config["url"] = "azure://"
            pubsub_config["azure_connection_string"] = st_dash.text_input("Azure Storage Connection String", value="DefaultEndpointsProtocol=...", key="pubsub_azure_conn_str", help=t("azure_conn_str_tooltip"))
            pubsub_config["azure_container_name"] = st_dash.text_input("Azure Blob Container Name for events", value="mesh-events", key="pubsub_azure_container", help=t("azure_container_tooltip"))
        elif pubsub_backend == "etcd":
            pubsub_config["url"] = "etcd://"
            pubsub_config["etcd_host"] = st_dash.text_input("etcd Host", value="localhost", key="pubsub_etcd_host", help=t("etcd_host_tooltip"))
            pubsub_config["etcd_port"] = st_dash.text_input("etcd Port", value="2379", key="pubsub_etcd_port", help=t("etcd_port_tooltip"))
        ...
        if ONBOARDING_BACKENDS_AVAILABLE:
            try:
                checkpoint_backend_options.extend([b for b in CheckpointManager._BACKENDS.keys() if b != "fs"])
                ...
                checkpoint_backend_options.extend(["s3", "redis", "postgres", "gcs", "azure", "etcd"])
                checkpoint_backend_options = sorted(list(set(checkpoint_backend_options)))

        checkpoint_backend = st_dash.selectbox(
            "Choose your preferred checkpoint backend:",
            options=checkpoint_backend_options,
            index=checkpoint_backend_options.index("fs") if "fs" in checkpoint_backend_options else 0,
            key="onboard_checkpoint_backend", help=t("checkpoint_backend_tooltip")
        )
        checkpoint_config = {"type": checkpoint_backend}
        if checkpoint_backend == "fs":
            checkpoint_config["dir"] = st_dash.text_input("Local directory for checkpoints", value="./checkpoints", key="chk_fs_dir", help=t("fs_dir_tooltip"))
        elif checkpoint_backend == "s3":
            checkpoint_config["s3_config"] = {"bucket": st_dash.text_input("S3 Bucket Name", value="your-s3-checkpoint-bucket", key="chk_s3_bucket", help=t("s3_bucket_tooltip"))}
            st_dash.info("For S3 backend, ensure AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION_NAME are set as environment variables.")
        elif checkpoint_backend == "redis":
            checkpoint_config["redis_config"] = {"url": st_dash.text_input("Redis URL for checkpoints", value="redis://localhost:6379/1", key="chk_redis_url", help=t("chk_redis_url_tooltip"))}
        elif checkpoint_backend == "postgres":
            checkpoint_config["postgres_config"] = {"dsn": st_dash.text_input("Postgres DSN", value="postgresql://user:password@localhost:5432/database", key="chk_pg_dsn", help=t("pg_dsn_tooltip"))}
            st_dash.info("Ensure your Postgres database has the 'checkpoints' table created.")
        elif checkpoint_backend == "gcs":
            checkpoint_config["gcs_config"] = {"bucket": st_dash.text_input("GCS Bucket Name for checkpoints", value="your-gcs-checkpoint-bucket", key="chk_gcs_bucket", help=t("chk_gcs_bucket_tooltip"))}
            st_dash.info("For GCS backend, ensure GOOGLE_APPLICATION_CREDENTIALS points to your service account key.")
        elif checkpoint_backend == "azure":
            checkpoint_config["azure_config"] = {
                "connection_string": st_dash.text_input("Azure Storage Connection String for checkpoints", value="DefaultEndpointsProtocol=...", key="chk_azure_conn_str", help=t("chk_azure_conn_str_tooltip")),
                "container_name": st_dash.text_input("Azure Blob Container Name for checkpoints", value="checkpoints", key="chk_azure_container", help=t("chk_azure_container_tooltip"))
            }
        elif checkpoint_backend == "etcd":
            checkpoint_config["etcd_config"] = {
                "host": st_dash.text_input("etcd Host for checkpoints", value="localhost", key="chk_etcd_host", help=t("chk_etcd_host_tooltip")),
                "port": st_dash.text_input("etcd Port for checkpoints", value="2379", key="chk_etcd_port", help=t("chk_etcd_port_tooltip"))
            }

        submitted = st_dash.form_submit_button("Complete Onboarding & Generate Config", help=t("generate_config_tooltip"))
        if submitted:
            ...
            for p_type in selected_plugin_types:
                ...
                _generate_plugin_manifest_gui(p_type, plugin_name, Config.PLUGINS_DIR) # Use Config.PLUGINS_DIR
            ...
            st_dash.session_state.generated_config_path = os.path.join(Config.CONFIG_DIR, "config.json") # Use Config.CONFIG_DIR
            st_dash.session_state.generated_plugins = [f"demo_{p_type}_plugin" for p_type in selected_plugin_types]
    ...
    if st_dash.session_state.get("onboarding_completed", False):
        ...
        if col_onboard_buttons[0].button("Run All Health Checks", key="run_all_health_checks", help=t("run_all_health_checks_tooltip")):
            if "onboarding_config" in st_dash.session_state:
                with st_dash.spinner("Running health checks... This might take a moment."):
                    run_async_streamlit(_run_health_checks_gui(st_dash.session_state.onboarding_config, test_all_plugins=True))
                st_dash.success("Health checks completed. See results above.")
            else:
                st_dash.error("Onboarding configuration not found. Please complete onboarding first.")

        if col_onboard_buttons[1].button("Run Demo Job (Conceptual)", key="run_demo_job", help=t("run_demo_job_tooltip")):
            st_dash.info("Running a demo job is conceptual and requires your main simulation script.")
            st_dash.code(f"python your_main_simulation_script.py --config {os.path.join(Config.CONFIG_DIR, 'config.json')}", language="bash") # Use Config.CONFIG_DIR
        ...
        if 'generated_config_path' in st_dash.session_state:
            st_dash.markdown(f"View generated config: `{st_dash.session_state.generated_config_path}`")
            with open(st_dash.session_state.generated_config_path, "r") as f:
                config_data = f.read()
            st_dash.download_button("Download Config", config_data, file_name="config.json", mime="application/json", key="download_config_btn", help=t("download_config_tooltip"))
...
def t(key: str) -> str:
    ...
    return st_dash.session_state.LOCALES.get(st_dash.session_state.lang, st_dash.session_state.LOCALES["en"]).get(key, key)
...
def display_simulation_dashboard():
    ...
    if not STREAMLIT_AVAILABLE:
        st_dash.error("Streamlit is not installed. Please install it (`pip install streamlit`) to view the dashboard.")
        return
    ...
    with st_dash.sidebar:
        ...
        st_dash.session_state.lang = st_dash.selectbox(
            ...
            key="language_selector", help=t("language_selector_tooltip")
        )

        # High Contrast Mode (Conceptual)
        high_contrast_mode = st_dash.checkbox(t('high_contrast_mode_label'), key="high_contrast_checkbox", help=t("high_contrast_mode_tooltip"))
        ...
        dark_mode = st_dash.checkbox(t('dark_mode_label'), key="dark_mode_checkbox", help=t("dark_mode_tooltip"))
        if dark_mode:
        ...
        sidebar_components = [c for c in get_registered_sidebar_components() if not c['roles'] or user_role in c['roles'] or user_role == 'admin']
        for scomp in sidebar_components:
            with st_dash.expander(scomp['title'], help=scomp['description'] if scomp['description'] else None):
                try:
                    ...
                    logging.error(f"Error rendering sidebar for {scomp['plugin']}: {e}", exc_info=True)
    ...
    if PLUGIN_MANAGER_AVAILABLE and 'plugin_manager_instance' not in st_dash.session_state:
        st_dash.session_state.plugin_manager_instance = PluginManager(plugins_dir=Config.PLUGINS_DIR)
    ...
    main_components = [c for c in get_registered_main_components() if not c['roles'] or user_role in c['roles'] or user_role == 'admin']
    tab_names = [
    ...
    with tabs[0]:
        ...
        if not results:
            ...
            search_query = st_dash.text_input(t("search_results_label"), "", help=t("search_results_tooltip"))
            
            session_names = sorted(list(set([
                os.path.basename(r.get('test_file', f"unknown_session_{idx}")).split('_tests')[0]
                for idx, r in enumerate(results)
            ] if isinstance(results, list) else [])))
            selected_session = st_dash.sidebar.selectbox(t("select_session_label"), ["All"] + session_names, key="session_selector", help=t("select_session_tooltip")) # Translated

            filtered_results_by_session = []
            if selected_session == "All":
                filtered_results_by_session = results
            else:
                filtered_results_by_session = [r for r in results if os.path.basename(r.get('test_file', '')).startswith(selected_session)]
            
            # Apply search filter
            if search_query:
                filtered_results = [
                    r for r in filtered_results_by_session
                    if search_query.lower() in json.dumps(r).lower() # Simple string search across JSON content
                ]
            ...
            if not filtered_results:
                ...
                for r in filtered_results:
                    ...
                    try:
                        mtime = datetime.fromtimestamp(os.path.getmtime(r['_filepath'])).strftime('%Y-%m-%d %H:%M:%S')
                    except (KeyError, FileNotFoundError):
                        mtime = "N/A"
                    run_options.append(f"{filename_base} (Generated: {mtime})")

                # Ensure selected_run_index is valid for filtered_results
                if 'selected_run_index' not in st_dash.session_state or \
                   st_dash.session_state.selected_run_index >= len(filtered_results) or \
                   st_dash.session_state.selected_run_index >= len(run_options): # defensive check
                ...
                selected_run_index = st_dash.selectbox(
                    ...
                    key="run_selector", help=t("select_run_tooltip")
                )
                ...
                if REDIS_AVAILABLE and live_job_id:
                    enable_live_data = st_dash.sidebar.checkbox(t("enable_live_data_updates_label"), key="live_data_checkbox", help=t("enable_live_data_tooltip"))
                    if enable_live_data:
                        live_update_interval = st_dash.sidebar.slider(t("live_update_interval_label"), 1, 10, 3, key="live_data_interval_slider", help=t("live_update_interval_tooltip"))
                        st_autorefresh(interval=live_update_interval * 1000, key="live_data_refresh")
                ...
                if selected_result:
                    with live_data_placeholder.container():
                        ...
                        if REDIS_AVAILABLE and live_job_id and st_dash.session_state.get('live_data_checkbox', False):
                        ...
                        if WORKFLOW_VIZ_AVAILABLE:
                            if st_dash.checkbox(t("show_workflow_flowchart_label"), value=True, key="show_workflow_flowchart", help=t("show_workflow_flowchart_tooltip")): # Translated
                                render_workflow_viz(selected_result, prefer_plotly=True, summary_callback=_display_summary_and_details)
                            else:
                        ...
                        for i, run in enumerate(selected_result.get("runs", [])):
                            if isinstance(run, dict):
                                with st_dash.expander(f"Run {run.get('run', i)} - {'PASS' if run.get('returncode') == 0 else 'FAIL'}", key=f"run_expander_{run.get('run', i)}_{i}", help=t("run_details_expander_tooltip")):
                                    st_dash.json(run)
                        ...
                        if PLOTLY_AVAILABLE:
                            ...
                            if runs:
                                df_data = {
                                    ...
                                    "CPU avg (%)": [r.get("metrics", {}).get("cpu_percent_avg", 0) for r in runs if isinstance(r, dict) and 'metrics' in r],
                                    "Memory max (MB)": [r.get("metrics", {}).get("memory_rss_max_mb", 0) for r in runs if isinstance(r, dict) and 'metrics' in r],
                                    "Duration (s)": [r.get("metrics", {}).get("duration_seconds", 0) for r in runs if isinstance(r, dict) and 'metrics' in r],
                                    "Return Code": [r.get("returncode") for r in runs if isinstance(r, dict)],
                                }
                                # Create basic Plotly graph objects figure
                                fig = go.Figure(data=[
                                    go.Bar(name='CPU avg (%)', x=df_data["Run"], y=df_data["CPU avg (%)"]),
                                    go.Bar(name='Memory max (MB)', x=df_data["Run"], y=df_data["Memory max (MB)"]),
                                    go.Bar(name='Duration (s)', x=df_data["Run"], y=df_data["Duration (s)"])
                                ])
                                fig.update_layout(barmode='group', title_text="Resource Usage per Run")
                            ...
                            if all(isinstance(r, dict) and 'metrics' in r for r in runs):
                                st_dash.bar_chart({
                                    "CPU Usage (%)": [r['metrics'].get('cpu_percent_avg', 0) for r in runs],
                                    "Max Memory (MB)": [r['metrics'].get('memory_rss_max_mb', 0) for r in runs]
                                })
                            else:
                                st_dash.warning(t("invalid_run_data_warning")) # New translation key
                            st_dash.info(t("install_plotly_info")) # Translated

                        if "flaky_plot" in selected_result and os.path.exists(selected_result["flaky_plot"]):
                        ...
                        if selected_result.get("agentic"):
                            ...
                            for agent_key in ["planner", "fault", "analyzer", "healer", "ethics", "sustainability"]:
                                if agent_key in selected_result:
                                    st_dash.json({agent_key.title(): selected_result[agent_key]}, expanded=False)
                            if selected_result.get("consensus"):
                                st_dash.json({"Consensus Protocol": {"decision": selected_result["consensus"]}}, expanded=False)
                            if selected_result.get("healer", {}).get("llm_suggestion"):
                                st_dash.markdown(f"**{t('ai_healer_suggestion_label')}**\n{selected_result['healer']['llm_suggestion']}") # Translated
                        ...
                        for panel_info in available_panels:
                            if panel_info["roles"] and user_role not in panel_info["roles"] and user_role != "admin":
                                st_dash.info(f"{t('panel_not_available_for_role_info')} ('{panel_info['title']}') ({user_role}).") # Translated
                                continue
                            with st_dash.expander(f"⚙️ {panel_info['title']}", expanded=False, help=panel_info['description'] if panel_info['description'] else None):
                                ...
                                try:
                                    ...
                                    st_dash.error(f"Error rendering plugin panel '{panel_info['title']}': {e}")
                                    st_dash.exception(e)
                        ...
                        with st_dash.form("custom_report_form"):
                            report_metrics = st_dash.multiselect(
                                ...
                                options=[t("total_runs"), t("failed_runs"), t("code_coverage"), t("rl_reward"), t("ethical_compliance"), t("energy_consumed"), t("carbon_emitted")], # Translated metrics
                                default=[t("total_runs"), t("failed_runs"), t("code_coverage")], # Default translated metrics
                                key="report_metrics_multiselect", help=t("select_metrics_tooltip")
                            )
                            time_range = st_dash.slider(
                                t("select_time_range_label"), # Translated
                                min_value=1, max_value=90, value=30,
                                key="report_time_range_slider", help=t("select_time_range_tooltip")
                            )
                            viz_type = st_dash.selectbox(
                                t("select_visualization_type_label"), # Translated
                                options=[t("bar_chart"), t("line_chart"), t("table")], # Translated viz types
                                key="report_viz_type_selectbox", help=t("select_visualization_type_tooltip")
                            )
                            submitted = st_dash.form_submit_button(t("generate_report_button"), key="generate_report_button", help=t("generate_report_button_tooltip")) # Translated
                        if submitted:
                            st_dash.success(f"{t('generated_report_success_message')} {viz_type} {t('for_metrics')} {', '.join(report_metrics)} {t('over_last')} {time_range} {t('days')}.") # Translated message components
                        ...
                        col_export1.download_button(
                            ...
                            key="download_json_btn", help=t("download_json_tooltip")
                        )
                        if col_export1.button(t("copy_json_to_clipboard_button"), key="copy_json_btn", help=t("copy_json_tooltip")): # Translated
                            st_dash.code(json_export_str, language='json')
                            st_dash.success(t("json_copied_to_clipboard_info"), icon="📋") # Translated
                        ...
                        col_export2.download_button(
                            ...
                            mime="text/html",
                            key="download_html_btn", help=t("download_html_tooltip")
                        )
                        ...
                        for run in selected_result.get("runs", []):
                            ...
                            duration_val = f"{metrics.get('duration_seconds', 'N/A'):.2f}" if isinstance(metrics.get('duration_seconds'), (int, float)) else 'N/A'
                            cpu_val = f"{metrics.get('cpu_percent_avg', 'N/A'):.2f}" if isinstance(metrics.get('cpu_percent_avg'), (int, float)) else 'N/A'
                            mem_val = f"{metrics.get('memory_rss_max_mb', 'N/A'):.2f}" if isinstance(metrics.get('memory_rss_max_mb'), (int, float)) else 'N/A'
                            csv_content += f"{run_val}," \
                                           f"{returncode_val}," \
                                           f"{duration_val}," \
                                           f"{cpu_val}," \
                                           f"{mem_val}\n"
                        ...
                        col_export3.download_button(
                            ...
                            mime="text/csv",
                            key="download_csv_btn", help=t("download_csv_tooltip")
                        )
    with tabs[1]:
