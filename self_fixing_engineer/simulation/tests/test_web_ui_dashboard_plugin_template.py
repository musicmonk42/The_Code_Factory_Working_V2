...
def mock_external_dependencies():
    ...
    """
    with patch('web_ui_dashboard_plugin_template.Redis') as mock_redis, \
        ...
        with patch('web_ui_dashboard_plugin_template.PROMETHEUS_AVAILABLE', True), \
             patch('web_ui_dashboard_plugin_template.REGISTRY', new=CollectorRegistry(auto_describe=True)), \
...
def test_get_component_endpoint_success(api_client):
    ...
    assert DASHBOARD_COMPONENT_RENDERS.labels(component_name='example_metric_panel')._value.get() == 1
...
async def test_websocket_workflow(api_client, mock_external_dependencies):
    ...
    with api_client.websocket_connect(websocket_uri) as websocket:
        ...
        assert WEBSOCKET_CONNECTIONS.labels(status='connected')._value.get() == initial_connected + 1
    ...
    assert WEBSOCKET_CONNECTIONS.labels(status='disconnected')._value.get() == initial_disconnected + 1
...
async def test_websocket_multiple_connections(api_client, mock_external_dependencies):
    ...
    with api_client.websocket_connect(websocket_uri) as ws1, \
        ...
        assert WEBSOCKET_CONNECTIONS.labels(status='connected')._value.get() == initial_connected + 2
    ...
    assert WEBSOCKET_CONNECTIONS.labels(status='disconnected')._value.get() == initial_disconnected + 2
