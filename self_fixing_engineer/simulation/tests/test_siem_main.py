    ...
    for siem_type in SIEM_CLIENT_REGISTRY.keys():
        ...
        try:
            ...
            async with get_siem_client(siem_type, test_config, metrics_hook=test_metrics_hook) as client:
                ...
                # Test query_logs
                try:
                    query_results = await client.query_logs("test query", "1h", 2, correlation_id=correlation_id)
...
if CLICK_AVAILABLE:
    ...
    @click.option("--siem-type", required=True, type=click.Choice(list(SIEM_CLIENT_REGISTRY.keys())))
    ...
    def health_check_command(siem_type, config_file):
        ...
        try:
            ...
            if os.path.exists(hmac_file):
                ...
                hmac_secret = SECRETS_MANAGER.get_secret("SIEM_CONFIG_HMAC_SECRET", required=True).encode('utf-8')
                with open(config_file, 'r') as f:
                    config_data = f.read()
...
class TestCLI:
    ...
    def test_cli_blocks_non_production(self):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["health-check", "--siem-type", "splunk", "--config-file", "dummy.json"])
    ...
    def test_health_check_command_success(self, tmp_path):
        ...
        expected_hmac = hmac.new(hmac_secret, config_data.encode('utf-8'), hashlib.sha256).hexdigest()
        hmac_file = tmp_path / "config.json.hmac"
        hmac_file.write_text(expected_hmac)
        
        runner = CliRunner()
        result = runner.invoke(cli, ["health-check", "--siem-type", "splunk", "--config-file", str(config_file)])
