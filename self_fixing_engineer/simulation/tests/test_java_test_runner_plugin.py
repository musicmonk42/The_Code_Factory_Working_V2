...
def mock_node_in_path():
    ...
    with (
        patch("plugins.jest_runner_plugin._which", new=AsyncMock()) as mock_which,
...
async def test_run_jest_tests_success_full_workflow(mock_temp_jest_project):
    ...
    with (
        patch(
            "plugins.jest_runner_plugin._which",
...
async def test_run_jest_tests_no_npx():
    ...
    with (
        patch("plugins.jest_runner_plugin._which", new=AsyncMock(return_value=None)),
        patch.object(Path, "exists", mock_path_exists),
    ):
