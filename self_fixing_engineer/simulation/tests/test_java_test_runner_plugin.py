...
async def test_run_jest_tests_success_full_workflow(mock_temp_jest_project):
    ...
    """
    with patch('plugins.jest_runner_plugin._which', new=AsyncMock(return_value="/usr/bin/npx")), \
         patch('plugins.jest_runner_plugin._get_package_version', new=AsyncMock(return_value="29.5.0")), \
         patch('asyncio.create_subprocess_exec', new=AsyncMock()) as mock_subprocess, \
         patch('shutil.copy2') as mock_copy2, \
         patch('shutil.copyfile') as mock_copyfile, \
         patch('plugins.jest_runner_plugin._copytree_compat') as mock_copytree, \
         patch('plugins.jest_runner_plugin._install_packages', new=AsyncMock(return_value=(True, ''))):
        ...
        def mock_path_exists(self):
            ...
            if "jest_run_" in path_str:
                ...
                if match:
                    temp_jest_dir = Path(path_str[:path_str.index(match.group(0)) + len(match.group(0))])
                return True
            return original_path_exists(self)
...
async def test_run_jest_tests_test_failure(mock_temp_jest_project):
    ...
    with patch('plugins.jest_runner_plugin._which', new=AsyncMock(return_value="/usr/bin/npx")), \
         ...
         patch('plugins.jest_runner_plugin._copytree_compat'), \
         patch('plugins.jest_runner_plugin._install_packages', new=AsyncMock(return_value=(False, "Failed to setup Jest environment: Failed to install packages"))):

        # Mock Jest subprocess output for a test failure
        mock_process = MagicMock()
        mock_process.communicate = AsyncMock(return_value=(
            b'{"success": false, "numFailedTests": 1}',
            b''
        ))
        ...
        with patch('builtins.open', mock_open(read_data='{"test": "data"}')), \
            ...
            with patch('os.path.exists', side_effect=lambda p: p != os.path.join(mock_temp_jest_project, 'package.json')):
                result = await run_jest_tests(
                    test_file_path='tests/sum.test.js',
...
async def test_which_command():
    ...
    with patch('plugins.jest_runner_plugin._shutil_which', return_value=None):
        ...
        with patch('os.name', 'nt'), \
            ...
            mock_process = MagicMock()
            mock_process.communicate = AsyncMock(return_value=(b'C:\\Program Files\\nodejs\\node.exe\n', b''))
            mock_process.returncode = 0
...
async def test_run_jest_tests_no_npx():
    ...
    with patch('plugins.jest_runner_plugin._which', new=AsyncMock(return_value=None)), \
