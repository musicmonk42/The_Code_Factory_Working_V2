...
def mock_external_dependencies():
    ...
    with patch(f'{plugin_path}.SIEM_CLIENTS_AVAILABLE', True), \
        ...
        with patch(f'{plugin_path}.prometheus_available', True), \
...
def test_config_model_validation_success():
    ...
    config_data = {
        ...
        "policy": {
            "rules": [{"conditions": [{"field": "event_type", "operator": "equals", "value": "alert"}], "action": "block"}]
        }
    }
...
def test_policy_enforcer_mask_rule_enforcement():
    ...
    policy_config = PolicyConfig(
        rules=[
            PolicyRule(
                conditions=[PolicyCondition(field="event_type", operator="equals", value="sensitive_event")],
                action="mask",
    ...
    event = {"event_type": "sensitive_event", "details": {"sensitive_info": "secret123"}}
...
def test_policy_enforcer_block_rule_enforcement():
    ...
    policy_config = PolicyConfig(
        rules=[
            PolicyRule(
                conditions=[PolicyCondition(field="event_type", operator="equals", value="blocked_event")],
                action="block"
            )
        ]
    )
...
async def test_send_siem_event_success(mock_external_dependencies):
    ...
    send_result = await plugin.send_siem_event(
        ...
        event_details={"message": "This is a test"}
    )
...
async def test_send_siem_event_policy_blocked(mock_external_dependencies):
    ...
    """
    plugin = GenericSIEMIntegrationPlugin(config=SIEM_CONFIG_MODEL)
    plugin.policy_enforcer = PolicyEnforcer(PolicyConfig(
        rules=[
            PolicyRule(
                conditions=[PolicyCondition(field="event_type", operator="equals", value="blocked_event")],
                action="block"
...
async def test_send_siem_event_backend_disabled(mock_external_dependencies):
    ...
    plugin.self_healing_manager.disabled_backends['splunk'] = {"consecutive_failures": 5, "disabled_until": time.monotonic() + 300}
    
    with patch.object(plugin.self_healing_manager, 'enqueue_for_retry', new=AsyncMock()) as mock_enqueue:
        send_result = await plugin.send_siem_event(
            event_type="test_event",
            event_details={"message": "This should be queued"}
        )
        ...
        from simulation.plugins.siem_integration_plugin import SIEM_SEND_ERRORS_TOTAL
        assert SIEM_SEND_ERRORS_TOTAL.labels(siem_type='splunk', error_type='backend_disabled')._value.get() == 1
...
async def test_query_siem_logs_success(mock_external_dependencies):
    ...
    query_result = await plugin.query_siem_logs(
        query_string="event_type='security_alert'",
        siem_type_override="splunk"
    )
