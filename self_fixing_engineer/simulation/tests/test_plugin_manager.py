    ...
    if not hasattr(PluginManager, 'health'):
        async def health(self, name: str):
            ...
            try:
                ...
                try:
                    from plugin_manager import prometheus_available, PLUGIN_HEALTH_STATUS
...
def mock_external_dependencies():
    ...
    with patch('plugin_manager.os.makedirs'), \
         patch('plugin_manager.shutil.rmtree'), \
         patch('plugin_manager.importlib.util.spec_from_file_location'), \
         patch('plugin_manager.importlib.util.module_from_spec') as mock_module_from_spec, \
         patch('plugin_manager.tenacity_available', True), \
         patch('plugin_manager.prometheus_available', True), \
         patch('plugin_manager.pydantic_available', True), \
         patch('plugin_manager.restricted_python_available', True), \
         patch('plugin_manager.detect_secrets_available', False), \
         patch.dict('plugin_manager.HANDLERS', {
             'wasm': MockWasmWrapper,
             'grpc': MockGrpcWrapper
         }, clear=True): # clear=True ensures only our mocks exist
        ...
        mock_module.PLUGIN_MANIFEST = {
            "name": "python_example_plugin", "version": "1.0.0", "type": "python",
            "entrypoint": "main", "health_check": "health", "api_version": "v1",
            "manifest_version": "2.0", "author": "test", "capabilities": [],
            "permissions": [], "dependencies": [], "min_core_version": "0.0.0",
            "max_core_version": "9.9.9", "sandbox": {"enabled": False}
        }
...
def mock_plugin_and_manifest_files():
    ...
    with open(python_plugin_path, "w") as f:
        f.write("PLUGIN_MANIFEST = {'name': 'python_plugin', 'type': 'python', 'version': '1.0', 'entrypoint': 'main.py', 'health_check': 'health', 'api_version': 'v1', 'manifest_version': '2.0', 'author': 'test', 'min_core_version': '0.0.0', 'max_core_version': '9.9.9'}\n")
    ...
    with open(wasm_dir / "manifest.json", "w") as f:
        f.write("""{"name": "wasm_plugin", "type": "wasm", "version": "1.0", "entrypoint": "main.wasm", "health_check": "health", "api_version": "v1", "manifest_version": "2.0", "author": "test", "min_core_version": "0.0.0", "max_core_version": "9.9.9", "sandbox": {"enabled": true, "resource_limits": {"memory": "64MB"}}}""")
    (wasm_dir / "main.wasm").touch() # Create dummy wasm file

    yield {"temp_dir": temp_dir, "python_plugin_path": python_plugin_path, "invalid_python_plugin_path": invalid_python_plugin_path, "wasm_dir": wasm_dir}
...
async def test_load_plugin_with_dangerous_permission(mock_plugin_and_manifest_files):
    ...
    manifest = {
        "name": "dangerous_plugin", "version": "1.0", "type": "python",
        "entrypoint": "main.py", "health_check": "health", "api_version": "v1",
        "manifest_version": "2.0", "author": "test", "capabilities": [],
        "permissions": ["execute_arbitrary_code"], "dependencies": [],
        "min_core_version": "0.0.0", "max_core_version": "9.9.9", "sandbox": {"enabled": False}
    }
...
async def test_enable_disable_reload_plugin_cycle(mock_plugin_and_manifest_files):
    ...
    initial_reload_errors = PLUGIN_ERRORS_TOTAL.labels(error_type='reload_failure', plugin_name='python_plugin')._value.get()
...
async def test_discover_plugins_and_load_all(mock_plugin_and_manifest_files):
    ...
    python_success_before = PLUGIN_LOADS_TOTAL.labels(plugin_type='python', status='success')._value.get()
    wasm_success_before = PLUGIN_LOADS_TOTAL.labels(plugin_type='wasm', status='success')._value.get()
    ...
    python_success_after = PLUGIN_LOADS_TOTAL.labels(plugin_type='python', status='success')._value.get()
    wasm_success_after = PLUGIN_LOADS_TOTAL.labels(plugin_type='wasm', status='success')._value.get()
