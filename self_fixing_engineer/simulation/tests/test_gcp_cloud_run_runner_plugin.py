...
try:
    from google.api_core.exceptions import GoogleAPIError, NotFound, Conflict, ResourceExhausted
...
def mock_jobs_client():
    ...
    with patch('plugins.gcp_cloud_run_runner_plugin.run_v2') as mock_run_v2:
        ...
        class MockExecutionTemplate:
            def __init__(self, template=None, task_template=None, parallelism=None, task_count=None, **kwargs):
...
class TestPluginHealth:
    ...
    async def test_health_check_success(self, mock_environment, mock_credentials, 
                                       mock_gcs_client, mock_jobs_client):
        """Test successful health check."""
        with patch('plugins.gcp_cloud_run_runner_plugin._get_credentials', 
                  return_value=mock_credentials):
            result = await plugin_health()
            
            assert result['status'] == 'ok'
            assert any('credentials loaded successfully' in d.lower() 
    ...
    async def test_health_check_no_credentials(self, mock_environment):
        ...
        with patch('plugins.gcp_cloud_run_runner_plugin._get_credentials', 
            ...
            assert any('credentials not configured' in d.lower() 
                      for d in result['details'])
    
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    async def test_health_check_gcs_failure(self, mock_environment, mock_credentials,
                                           mock_gcs_client):
...
class TestRunCloudRunJob:
    ...
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    async def test_successful_job_execution(self, valid_job_config, temp_project_dir,
                                           mock_environment, mock_credentials,
                                           mock_gcs_client, mock_jobs_client,
                                           mock_logging_client):
        ...
        with patch('plugins.gcp_cloud_run_runner_plugin._get_credentials',
                  return_value=mock_credentials), \
             patch('plugins.gcp_cloud_run_runner_plugin._tar_directory_to_temp',
    ...
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    async def test_failed_job_execution(self, valid_job_config, temp_project_dir,
                                       mock_environment, mock_credentials,
                                       mock_gcs_client, mock_jobs_client,
                                       mock_logging_client):
    ...
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    @patch('plugins.gcp_cloud_run_runner_plugin.retry', lambda *args, **kwargs: lambda f: f)  # Disable retry decorator
    async def test_quota_exceeded_retry(self, valid_job_config, temp_project_dir,
                                       mock_environment, mock_credentials,
                                       mock_gcs_client, mock_jobs_client):
    ...
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    async def test_gcs_download_failure(self, valid_job_config, temp_project_dir,
                                       mock_environment, mock_credentials,
                                       mock_gcs_client, mock_jobs_client):
...
class TestSecurity:
    ...
    async def test_vault_credentials_loading(self, mock_environment):
        ...
        with patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True), \
             patch('plugins.gcp_cloud_run_runner_plugin.service_account') as mock_service_account, \
             patch('plugins.gcp_cloud_run_runner_plugin.aiohttp.ClientSession') as mock_aiohttp_session:
...
class TestEdgeCases:
    ...
    @pytest.mark.asyncio
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    async def test_monitoring_timeout(self, valid_job_config, temp_project_dir,
                                     mock_environment, mock_credentials,
                                     mock_gcs_client, mock_jobs_client):
        ...
        def time_side_effect():
            nonlocal call_count
            call_count += 1
            if call_count <= 4:  # Initial calls before/during setup (overall start, upload start, observe, monitor start)
                return 0
            else:
        ...
        with patch('plugins.gcp_cloud_run_runner_plugin._get_credentials',
             ...
             patch('plugins.gcp_cloud_run_runner_plugin._tar_directory_to_temp',
                  return_value="/tmp/test-archive.tar.gz"), \
             patch('asyncio.sleep', side_effect=mock_sleep), \
             patch.object(gcp_plugin_module.time, 'monotonic', side_effect=time_side_effect), \
             patch('builtins.open', create=True), \
             patch('os.path.exists', return_value=True), \
             patch('os.remove'):
    ...
    @patch('plugins.gcp_cloud_run_runner_plugin.GCP_AVAILABLE', True)
    @patch('plugins.gcp_cloud_run_runner_plugin.retry', lambda *args, **kwargs: lambda f: f)  # Disable retry decorator
    async def test_job_conflict_retry(self, valid_job_config, temp_project_dir,
                                     mock_environment, mock_credentials,
                                     mock_gcs_client, mock_jobs_client):
