...
def create_mock_core_functions():
    ...
    if not hasattr(core, 'load_rbac_policy'):
        core.load_rbac_policy = lambda path: {
            'roles': [{'name': 'admin', 'permissions': [{'action': 'run:agent', 'resource': '*'}]}],
...
async def test_simulation_module_end_to_end(setup_test_environment):
    ...
    with patch('simulation.runners.process.os.execve', new=AsyncMock()), \
        ...
        with patch('getpass.getuser', return_value="test_user"):
            ...
            with patch.object(core, 'CONFIG_FILE', str(setup_test_environment["configs_dir"] / "config.yaml"), create=True), \
                 patch.object(core, 'RBAC_POLICY_FILE', str(setup_test_environment["configs_dir"] / "rbac_policy.yaml"), create=True):
                ...
                with patch.object(core, 'run_agent', return_value={"status": "SUCCESS"}, create=True):
