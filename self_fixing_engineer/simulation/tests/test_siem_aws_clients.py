...
class AwsCloudWatchConfig:
    def __init__(self, **kwargs):
        ...
        if os.getenv('PRODUCTION_MODE') == 'true':
            ...
            if (self.aws_access_key_id or self.aws_secret_access_key) and not self.aws_credentials_secret_id:
...
class AwsCloudWatchClient:
    ...
    async def health_check(self):
        ...
        try:
            ...
            raise SIEMClientResponseError("unexpected response", self.client_type, 500, str(response))
        except Exception as e:
    ...
    async def query_logs(self, query_string, time_range, limit):
        ...
        for attempt in range(max_attempts):
            ...
            if result.get('status') == 'Complete':
                ...
                raise SIEMClientQueryError(result.get('statusReason', 'Query failed'), self.client_type)
        
        # Timeout
        client.stop_query(queryId=query_id)
        raise SIEMClientQueryError(f"Query timed out after {max_attempts} attempts", self.client_type)
    ...
    async def _ensure_log_group_and_stream(self):
        ...
        try:
            ...
            if self.auto_create_log_group:
                ...
                raise SIEMClientConfigurationError("Log group not found and auto-creation is disabled", self.client_type)
        ...
        try:
            ...
            if self.auto_create_log_stream:
                ...
                raise SIEMClientConfigurationError("Log stream not found and auto-creation is disabled", self.client_type)
...
def mock_boto3_client():
    ...
    client = MagicMock()
    client.describe_log_groups = MagicMock(return_value={"logGroups": [{"logGroupName": "test-group"}]})
    client.describe_log_streams = MagicMock(return_value={
...
class TestErrorHandling:
    ...
    async def test_auth_error(self, cloudwatch_client):
        ...
        class MockClientError(Exception):
            def __init__(self):
                self.response = {"Error": {"Code": "AccessDeniedException", "Message": "Access denied"}}
    ...
    async def test_connectivity_error(self, cloudwatch_client):
        ...
        cloudwatch_client._cw_logs_client.describe_log_groups.side_effect = Exception("Network error")
