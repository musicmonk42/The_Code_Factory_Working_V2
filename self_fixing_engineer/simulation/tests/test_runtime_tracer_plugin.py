...
def mock_external_dependencies():
    ...
    with patch(
        ...
        mock_subprocess_run.return_value = MagicMock(
            returncode=0, stdout="Docker info output", stderr=""
        )
...
async def test_plugin_health_success(mock_external_dependencies):
    ...
    with patch.dict(
        "runtime_tracer_plugin.TRACER_CONFIG",
        {"use_docker_sandbox": True, "check_container_python": False},
    ):
    ...
    assert result["status"] in [
        "ok",
        "degraded",
    ]  # May be degraded if container check warnings
...
async def test_plugin_health_docker_not_found(mock_external_dependencies):
    ...
    mock_external_dependencies["mock_shutil_which"].return_value = (
        None  # Mock `which docker` failure
    )
...
@pytest.mark.asyncio
async def test_plugin_health_docker_not_found_but_unsafe_allowed(
    ...
    mock_log_content = [
        {
            ...
            "call_type": "eval/exec/__import__",
            "line": 5,
            "file": "my_app.py",
            "code_line": "exec('print(1)')",
        },
        {
            "type": "exception",
    ...
    with patch("runtime_tracer_plugin.asyncio.create_subprocess_exec") as mock_subprocess_exec:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(
                ...
                result = await analyze_runtime_behavior(
                    target_code_path=str(target_code_path), analysis_duration_seconds=1
                )
...
async def test_analyze_runtime_behavior_target_not_found():
    ...
    with patch("runtime_tracer_plugin.os.path.exists", return_value=False):
        result = await analyze_runtime_behavior(
            target_code_path="/nonexistent/file.py", analysis_duration_seconds=5
        )
...
async def test_analyze_runtime_behavior_malformed_json_in_trace():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            ...
            result = await analyze_runtime_behavior(
                target_code_path=str(target_code_path), analysis_duration_seconds=5
            )
