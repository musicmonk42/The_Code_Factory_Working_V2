...
try:
    from simulation.plugins.aws_batch_runner_plugin import (
        plugin_health, run_batch_job, JobConfig,
        AWS_AVAILABLE,
    )
except ImportError:
...
def mock_aws_clients():
    ...
    with patch('boto3.Session') as mock_session, \
         patch('simulation.plugins.aws_batch_runner_plugin._load_credentials_from_vault', new_callable=AsyncMock) as mock_load_vault:
        ...
        mock_load_vault.return_value = {'aws_access_key_id': 'vault_key', 'aws_secret_access_key': 'vault_secret'}

        with patch('simulation.plugins.aws_batch_runner_plugin._session_has_real_creds', return_value=True):
            yield {
...
def test_job_config_invalid_arn_format():
    with pytest.raises(ValidationError, match="Invalid Batch ARN or name format"):
        JobConfig(jobDefinition="invalid:arn!!", jobQueue="invalid-queue", input_s3_bucket="test")

def test_job_config_path_traversal_prevention():
    with pytest.raises(ValidationError, match="invalid characters"):
        JobConfig(jobDefinition="mock-def", jobQueue="mock-queue", input_s3_bucket="test/../bucket")
...
async def test_plugin_health_no_credentials_error(mock_aws_clients):
    ...
    with patch('simulation.plugins.aws_batch_runner_plugin._session_has_real_creds', return_value=False):
        result = await plugin_health()
...
async def test_run_batch_job_failure_workflow(mock_aws_clients, mock_job_config_valid):
    ...
    mock_batch_client.describe_jobs.side_effect = [
        {'jobs': [{'status': 'RUNNING'}]},
        {'jobs': [{'status': 'FAILED', 'statusReason': 'User Error', 'container': {'logStreamName': 'mock_stream'}}]},
    ]
...
async def test_run_batch_job_s3_download_failure(mock_aws_clients, mock_job_config_valid):
    ...
    mock_batch_client.describe_jobs.return_value = {'jobs': [{'status': 'SUCCEEDED', 'container': {}}]}
...
@pytest.mark.asyncio
async def test_run_batch_job_with_vault_credentials_failure(mock_aws_clients, mock_job_config_valid):
