...
async def test_run_jest_tests_success_full_workflow(mock_temp_jest_project):
    ...
    """
    with patch('simulation.plugins.jest_runner_plugin._which', new=AsyncMock(return_value="/usr/bin/npx")), \
...
async def test_run_jest_tests_test_failure(mock_temp_jest_project):
    ...
    with patch('simulation.plugins.jest_runner_plugin._which', new=AsyncMock(return_value="/usr/bin/npx")), \
         ...
         patch('shutil.copyfile'), \
         patch('simulation.plugins.jest_runner_plugin._copytree_compat'), \
         patch('simulation.plugins.jest_runner_plugin._install_packages', new=AsyncMock(return_value=(False, "Failed to setup Jest environment: Failed to install packages"))):
...
async def test_run_jest_tests_no_npx():
    ...
    with patch('simulation.plugins.jest_runner_plugin._which', new=AsyncMock(return_value=None)), \
