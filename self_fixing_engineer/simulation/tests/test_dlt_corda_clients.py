...
def create_mock_response(status, text=None, json_data=None, headers=None):
    ...
    if status >= 400:
        mock_resp.raise_for_status.side_effect = ClientResponseError(
            request_info=MagicMock(), history=(), status=status, message=text or "Error"
        )
    return mock_resp
...
def mock_secrets_manager(mocker):
    ...
    mock_get_secret.side_effect = lambda key, **kwargs: (
        "testuser" if "user" in key.lower() else "testpass"
    )
...
@pytest.mark.parametrize(
    "rpc_url,user,password,is_prod,should_fail",
    [
        (
            "http://localhost:8080",
            "user",
...
async def test_corda_init_production_mode_validation(
    rpc_url, user, password, is_prod, should_fail, mock_off_chain, mocker
):
    """
...
async def test_health_check_success(mock_off_chain, mock_aiohttp):
    ...
    mock_aiohttp["get"].assert_called_once_with(
        "http://localhost:8080/api/rest/corda/v4/me", timeout=30
    )


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "status,error_type,message_part",
    [
...
async def test_health_check_failures(
    mock_off_chain, mock_aiohttp, status, error_type, message_part
):
    """
...
async def test_write_checkpoint_success(mock_off_chain, mock_aiohttp):
    ...
    mock_response_data = {"id": "mock_tx_id", "returnValue": {"result": {"version": 1}}}

    # Create mock response
    mock_response = create_mock_response(
        200, json_data=mock_response_data, text=json.dumps(mock_response_data)
    )
...
async def test_write_checkpoint_retry_on_transient_error(mock_off_chain, mock_aiohttp, mocker):
    ...
    success_response = create_mock_response(
        200, json_data=mock_success_data, text=json.dumps(mock_success_data)
    )
