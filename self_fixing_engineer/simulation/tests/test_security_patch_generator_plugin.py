...
def mock_external_dependencies():
    ...
    """
    with patch(
        "security_patch_generator_plugin.aiohttp.ClientSession"
    ) as mock_aiohttp, patch(
        "security_patch_generator_plugin.Redis"
    ) as mock_redis, patch(
        "security_patch_generator_plugin.LANGCHAIN_AVAILABLE", True
    ), patch(
        ...
        mock_response.text = AsyncMock(
            return_value=json.dumps({"choices": [{"text": "mocked response"}]})
        )

        mock_async_response = AsyncMock()
        mock_async_response.text = "mocked response"
        mock_async_response.generations = [
            [
                MagicMock(
        ...
        with patch(
            ...
            new=AsyncMock(return_value=mock_llm_client),
        ):
            yield {
...
"""
    ...
    assert (
        "prevents sql injection" in explanation.lower()
        or "this prevents sql injection" in explanation.lower()
    )
...
async def test_generate_security_patch_llm_refusal(mock_external_dependencies):
    ...
    mock_external_dependencies["mock_llm_client"].generate_text = AsyncMock(
        return_value=(
            "I cannot generate a safe and effective fix. Manual fix required.",
            {"tokens": 10},
        )
    )
    ...
    result = await generate_security_patch(
        vulnerability_details=vuln_details, vulnerable_code_snippet=vulnerable_code
    )
...
async def test_generate_security_patch_invalid_input():
    ...
    with pytest.raises(TypeError):
        await generate_security_patch(
            vulnerability_details={}, vulnerable_code_snippet=123  # Should be string
        )
