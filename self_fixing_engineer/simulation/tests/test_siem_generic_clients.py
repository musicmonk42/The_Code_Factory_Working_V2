...
class SplunkClient(AiohttpClientMixin, BaseSIEMClient):
    ...
    async def health_check(self, correlation_id=None):
        ...
        async with response:
            ...
            raise SIEMClientConnectivityError(
                f"Health check failed: {response.status}", self.client_type
            )
    ...
    async def send_log(self, log_entry, validate_schema=True, correlation_id=None):
        ...
        raise SIEMClientPublishError(
            f"Failed: {failed[0]['error']}", self.client_type, details=failed[0]
        )
...
class ElasticClient(AiohttpClientMixin, BaseSIEMClient):
    ...
    async def send_logs(self, log_entries, validate_schema=True, correlation_id=None):
        ...
        session.post.return_value = MockAsyncResponse(
            status=200,
            json_data={
                "items": [{"index": {"status": 201}}] * len(log_entries),
                "errors": False,
            },
        )
    ...
    async def query_logs(
        ...
        session.post.return_value = MockAsyncResponse(
            status=200, json_data={"hits": {"hits": [{"_source": {"message": "test"}}]}}
        )
...
class DatadogClient(AiohttpClientMixin, BaseSIEMClient):
    ...
    async def _ensure_config_loaded(self):
        ...
        async with self._config_lock:
            ...
            config_data["url"] = await _get_secret(
                "SIEM_DATADOG_API_URL",
                "https://http-intake.logs.datadoghq.com/api/v2/logs",
            )
            config_data["query_url"] = await _get_secret(
    ...
    async def send_logs(self, log_entries, validate_schema=True, correlation_id=None):
        ...
        for batch in batches:
            ...
            response = await session.post(
                self.url, headers={"DD-API-KEY": self.api_key}, json=batch_payload
            )
            async with response:
                if response.status >= 400:
                    failed_logs.extend(
    ...
    async def query_logs(
        ...
        session.post.return_value = MockAsyncResponse(
            status=200, json_data={"data": [{"content": {"message": "test"}}]}
        )
