...
try:
    ...
    from workflow_viz import (
        WorkflowVizConfig, _scrub_secrets,
        validate_custom_phases, validate_export_path,
        render_workflow_viz,
        DashboardAPI,
        VIZ_RENDER_TOTAL, VIZ_EXPORT_TOTAL, VIZ_RENDER_ERRORS,
        WorkflowPhase,
        PLOTLY_AVAILABLE, MATPLOTLIB_AVAILABLE, RESULTS_DIR,
    )
except ImportError as e:
    print(f"Error importing workflow_viz: {e}")
    print(f"sys.path: {sys.path}")
    print(f"plugins_dir: {plugins_dir}")
    print(f"Files in plugins_dir: {os.listdir(plugins_dir) if os.path.exists(plugins_dir) else 'DIR NOT FOUND'}")
    raise
...
def mock_external_dependencies():
    ...
    with patch('workflow_viz.nx') as mock_nx:
        ...
        with patch('workflow_viz.retry', side_effect=lambda **kwargs: lambda func: func):
            ...
            with patch('workflow_viz.PROMETHEUS_AVAILABLE', True), \
                 patch('workflow_viz.REGISTRY', new=CollectorRegistry(auto_describe=True)):
...
async def test_render_workflow_viz_plotly_success(mock_external_dependencies, mock_result_data):
    ...
    """
    with patch('workflow_viz.PLOTLY_AVAILABLE', True), \
         patch('workflow_viz.go') as mock_go, \
...
@pytest.mark.asyncio
async def test_render_workflow_viz_matplotlib_fallback(mock_external_dependencies, mock_result_data):
    """
    Test that the system falls back to Matplotlib if Plotly rendering fails.
    """
    with patch('workflow_viz.PLOTLY_AVAILABLE', True), \
         patch('workflow_viz.MATPLOTLIB_AVAILABLE', True), \
         patch('workflow_viz.st') as mock_st:
        ...
        initial_plotly_errors = VIZ_RENDER_ERRORS.labels(backend='plotly', error_type='runtime_error')._value.get()
        ...
        assert VIZ_RENDER_ERRORS.labels(backend='plotly', error_type='runtime_error')._value.get() == initial_plotly_errors + 1
        
        # Should have succeeded with matplotlib
        assert fig is not None
        # Check that pyplot was called (matplotlib was used)
        mock_st.pyplot.assert_called_once()
        assert VIZ_RENDER_TOTAL.labels(backend='matplotlib')._value.get() == initial_matplotlib_renders + 1


@pytest.mark.asyncio
async def test_batch_export_panels_success(mock_external_dependencies, mock_result_data, mock_filesystem):
    """
        ...
        with patch('viz.get_registered_viz_panels') as mock_get_panels, \
            ...
            mock_get_panels.return_value = {
                "flakiness_trend": {
                    ...
                    "render_function": MagicMock(return_value=mock_external_dependencies['mock_fig']),
...
def test_render_workflow_viz_no_data(mock_external_dependencies):
    ...
    with patch('workflow_viz.st') as mock_st:
        ...
        initial_errors = VIZ_RENDER_ERRORS.labels(backend='streamlit', error_type='no_data')._value.get()
        ...
        assert VIZ_RENDER_ERRORS.labels(backend='streamlit', error_type='no_data')._value.get() == initial_errors + 1


def test_render_workflow_viz_no_libraries():
    """Test rendering when no visualization libraries are available."""
    with patch('workflow_viz.PLOTLY_AVAILABLE', False), \
         patch('workflow_viz.MATPLOTLIB_AVAILABLE', False), \
         patch('workflow_viz.st') as mock_st:
