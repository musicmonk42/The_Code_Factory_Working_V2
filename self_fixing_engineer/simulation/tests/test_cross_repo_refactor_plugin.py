...
try:
    ...
    from simulation.plugins.cross_repo_refactor_plugin import (
        plugin_health, perform_cross_repo_refactor,
        GitRepoManager, _validate_refactor_plan, _mask_token_in_url,
        _is_safe_path, TENACITY_AVAILABLE, GITPYTHON_AVAILABLE
    )
except ImportError as e:
    # If imports fail, provide a clear message. This can happen if the
    # project structure has changed unexpectedly.
    pytest.fail(f"Failed to import the plugin. Check project structure and sys.path "
                f"manipulation in the test file. Error: {e}", pytrace=False)
...
def mock_external_dependencies():
    ...
    with patch.object(cross_repo_refactor_plugin, 'Repo') as mock_repo_class, \
         patch('aiohttp.ClientSession') as mock_aiohttp, \
         patch.object(cross_repo_refactor_plugin._sfe_audit_logger, 'log_event', new=AsyncMock()) as mock_audit_log, \
         patch.object(cross_repo_refactor_plugin.shutil, 'rmtree') as mock_rmtree, \
         patch.dict(cross_repo_refactor_plugin.GIT_CONFIG, {
        ...
        mock_repo_instance.index.commit = MagicMock(return_value=MagicMock(hexsha='mock_commit_sha'))
...
@pytest.mark.parametrize("invalid_plan, expected_error", [
...
@pytest.mark.asyncio
async def test_perform_refactor_full_success_workflow(mock_external_dependencies, temp_repo_dir, valid_refactor_plan):
    """
    ...
    with patch('tempfile.mkdtemp', return_value=temp_repo_dir), \
         patch.object(cross_repo_refactor_plugin.GitRepoManager, 'create_pull_request', new=AsyncMock(return_value="https://github.com/mock/repo/pull/1")):
    ...
    mock_external_dependencies["mock_rmtree"].assert_called_once_with(temp_repo_dir) # Cleanup was triggered
    mock_external_dependencies["mock_audit_log"].assert_called()


@pytest.mark.asyncio
async def test_perform_refactor_dry_run(mock_external_dependencies, temp_repo_dir, valid_refactor_plan):
    """
    ...
    mock_external_dependencies["mock_aiohttp"].return_value.__aenter__.return_value.post.assert_not_called()

@pytest.mark.asyncio
async def test_perform_refactor_clone_failure(mock_external_dependencies, temp_repo_dir, valid_refactor_plan):
    """
    Test the workflow when a git clone operation fails.
    """
    # Simulate a clone failure
    mock_external_dependencies["mock_repo_class"].clone_from.side_effect = Exception("Clone failed: repository not found")
    ...
    mock_external_dependencies["mock_rmtree"].assert_called_once_with(temp_repo_dir) # Cleanup should still run

@pytest.mark.asyncio
async def test_perform_refactor_no_cleanup_on_failure(mock_external_dependencies, temp_repo_dir, valid_refactor_plan):
    """
    Test that temporary directories are retained for debugging when cleanup_on_failure is False.
    """
    mock_external_dependencies["mock_repo_class"].clone_from.side_effect = Exception("Simulated failure")
...
async def test_plugin_health_success():
    ...
    with patch('simulation.plugins.cross_repo_refactor_plugin.GITPYTHON_AVAILABLE', True), \
         patch('asyncio.create_subprocess_exec') as mock_subprocess, \
         patch.dict(cross_repo_refactor_plugin.GIT_CONFIG, {"pr_api_token": "mock_pr_token", "pr_api_base_url": "https://api.github.com"}):
