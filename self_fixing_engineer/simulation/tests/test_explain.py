...
async def reasoner_with_shutdown(mock_config, mock_settings):
    ...
    with patch("simulation.explain.AutoTokenizer.from_pretrained", MagicMock()), \
         patch("simulation.explain.AutoModelForCausalLM.from_pretrained", MagicMock()), \
         patch("simulation.explain.pipeline", MagicMock(return_value=MagicMock(tokenizer=MagicMock(pad_token_id=0, eos_token_id=1)))), \
        ...
        with patch.object(reasoner.executor, "submit", wraps=reasoner.executor.submit) as mock_submit:
            def side_effect(func, *args, **kwargs):
                return func(*args, **kwargs)
            mock_submit.side_effect = lambda func, *args, **kwargs: reasoner.executor.submit(partial(func, **kwargs) if kwargs else func, *args)
...
@pytest.mark.asyncio
async def test_explainable_reasoner_plugin_explain_result(reasoner_with_shutdown, mock_settings, mock_config):
...
@pytest.mark.asyncio
async def test_explainable_reasoner_plugin_execute(reasoner_with_shutdown, mock_settings, mock_config):
