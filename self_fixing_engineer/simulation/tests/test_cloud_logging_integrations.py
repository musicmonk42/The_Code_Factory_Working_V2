...
def aws_mocks():
    ...
    with patch('boto3.client') as mock_boto3, \
         ...
         patch('simulation.plugins.cloud_logging_integrations.AWSClientError', MockAWSClientError):
        
        boto3_instance = MagicMock()
        mock_boto3.return_value = boto3_instance
        
        # Setup default successful responses
        boto3_instance.describe_log_groups.return_value = {'logGroups': [{'logGroupName': 'mock-group'}]}
        boto3_instance.put_log_events.return_value = {'nextSequenceToken': 'new-mock-token'}
        boto3_instance.describe_log_streams.return_value = {'logStreams': [{'uploadSequenceToken': 'mock-token'}]}
        boto3_instance.start_query.return_value = {'queryId': 'mock-query-id'}
        boto3_instance.get_query_results.return_value = {'status': 'Complete', 'results': []}
...
def gcp_mocks():
    ...
    with patch('simulation.plugins.cloud_logging_integrations.gcp_logging_sdk.Client', create=True) as mock_gcp_client, \
...
def azure_mocks():
    ...
    with patch('simulation.plugins.cloud_logging_integrations.LogsQueryClient', create=True) as mock_query, \
         patch('simulation.plugins.cloud_logging_integrations.DefaultAzureCredential', create=True) as mock_cred, \
         patch('simulation.plugins.cloud_logging_integrations.LogsIngestionClient', create=True) as mock_ingestion, \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_MONITOR_QUERY_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_IDENTITY_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_MONITOR_INGESTION_AVAILABLE', True):
...
async def test_cw_logger_health_check_success(aws_mocks):
    ...
    with patch('simulation.plugins.cloud_logging_integrations.CloudWatchLogger', MockCloudWatchLogger):
...
async def test_cw_logger_auth_error(aws_mocks):
    ...
    with patch('simulation.plugins.cloud_logging_integrations.CloudWatchLogger', MockCloudWatchLogger):
        logger = MockCloudWatchLogger(config)
        logger._aws_client = aws_mocks
        logger._health_check_error = CloudLoggingAuthError("AWS authorization failed", cloud_type="aws")
...
async def test_cw_logger_flushes_batch(aws_mocks):
    config = {"aws_cloudwatch": {"log_group_name": "test-group", "log_stream_name": "test-stream"}}
...
async def test_azure_logger_health_check_success(azure_config, azure_mocks):
    with patch('simulation.plugins.cloud_logging_integrations.AzureMonitorLogger', MockAzureLogger):
...
async def test_azure_logger_health_check_auth_error(azure_config, azure_mocks):
    with patch('simulation.plugins.cloud_logging_integrations.AzureMonitorLogger', MockAzureLogger):
        logger = MockAzureLogger(azure_config)
        logger._ingestion_client = azure_mocks['ingestion_instance']
        logger._health_check_error = CloudLoggingAuthError("Authentication failed", cloud_type="azure")
...
def test_get_cloud_logger_factory_with_valid_config(azure_config):
    with patch('simulation.plugins.cloud_logging_integrations.AWS_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.GCP_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_MONITOR_INGESTION_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_IDENTITY_AVAILABLE', True), \
         patch('simulation.plugins.cloud_logging_integrations.AZURE_MONITOR_QUERY_AVAILABLE', True):
