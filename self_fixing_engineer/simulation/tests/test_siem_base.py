...
class SIEMClientResponseError(SIEMClientError): 
    def __init__(self, message, client_type, status_code, response_text, original_exception=None, details=None, correlation_id=None):
...
class GenericLogEvent:
    def __init__(self, **kwargs):
        ...
        allowed_fields = {'timestamp_utc', 'event_type', 'message', 'severity', 
                         'hostname', 'source_ip', 'user_id', 'details'}
...
class SecretsManager:
    ...
    async def get_secret(self, key: str, default: Optional[str] = None, required: bool = True, backend: str = "env") -> Optional[str]:
...
def scrub_secrets(data: Any, patterns: Optional[List[str]] = None) -> Any:
    ...
    def _scrub(item: Any) -> Any:
        if isinstance(item, dict):
            ...
            if any(sensitive in item.lower() for sensitive in ['password', 'key', 'secret', 'token', 'bearer']):
                return "[SCRUBBED]"
            return item
        else:
            return item
...
class BaseSIEMClient:
    ...
    def __init__(self, config: Dict[str, Any], metrics_hook: Optional[Any] = None, paranoid_mode: bool = False):
        self.config = config
    ...
    def _scrub_env_vars_on_init(self):
        ...
        if scrubbed_count > 0 and PRODUCTION_MODE:
            alert_operator("CRITICAL: Sensitive environment variables detected in PRODUCTION_MODE.", level="CRITICAL")
            raise SIEMClientConfigurationError("Sensitive environment variables detected in PRODUCTION_MODE.", self.client_type)
    ...
    def _set_correlation_id(self, correlation_id: Optional[str]):
        self.logger.extra['correlation_id'] = correlation_id if correlation_id is not None else 'N/A'
    ...
    async def health_check(self, correlation_id: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.metrics_hook:
                self.metrics_hook('health_check', 'success' if is_healthy else 'failure', 
                                {'siem_type': self.client_type, 'correlation_id': correlation_id})
            return is_healthy, message
        except SIEMClientError:
            raise
        except Exception as e:
            raise SIEMClientError(f"Unexpected error during {self.client_type} health check: {e}", 
    ...
    async def send_log(self, log_entry: Dict[str, Any], correlation_id: Optional[str] = None, 
                       validate_schema: bool = True) -> Tuple[bool, str]:
        self._set_correlation_id(correlation_id)
        try:
            await self._apply_rate_limit()
            ...
            if validate_schema:
                try:
                    ...
                    raise SIEMClientValidationError(f"Log entry validation failed: {e}", 
            ...
            if self.metrics_hook:
                self.metrics_hook('send_log', 'success' if is_success else 'failure',
                                {'siem_type': self.client_type, 'correlation_id': correlation_id})
            return is_success, message
        except SIEMClientError:
            raise
        except Exception as e:
            raise SIEMClientError(f"Unexpected error sending to {self.client_type}: {e}",
                                 self.client_type, original_exception=e, correlation_id=correlation_id)
        finally:
            self._release_rate_limit()
            self._set_correlation_id(None)
    ...
    async def send_logs(self, log_entries: List[Dict[str, Any]], correlation_id: Optional[str] = None,
                       validate_schema: bool = True) -> Tuple[bool, str, List[Dict[str, Any]]]:
        self._set_correlation_id(correlation_id)
        all_successful = True
        failed_logs = []
        success_count = 0
        ...
        for log_entry in log_entries:
            ...
            if validate_schema:
                try:
                    ...
                    failed_logs.append({"log": log_entry, "error": str(e), "reason": "validation_failed"})
                    continue
        ...
        try:
            ...
            if is_batch_supported:
                success, msg, batch_failed_logs = await self._perform_send_logs_batch_logic(processed_logs)
                if not success:
                    all_successful = False
                ...
                for log in processed_logs:
                    try:
                        ...
                        if is_ok:
                            ...
                            all_successful = False
                            failed_logs.append({"log": log, "error": msg, "reason": "single_send_failed"})
                    except SIEMClientError as e:
                        all_successful = False
                        failed_logs.append({"log": log, "error": str(e), "reason": "single_send_exception"})
            ...
            if self.metrics_hook:
                self.metrics_hook('send_logs_batch', 'success' if all_successful else 'partial_failure',
                                {'siem_type': self.client_type, 'correlation_id': correlation_id,
                                 'total_logs': len(log_entries), 'successful_logs': success_count,
    ...
    async def query_logs(self, query_string: str, time_range: str = "24h", limit: int = 100,
        ...
        try:
            ...
            if self.metrics_hook:
                self.metrics_hook('query_logs', 'success',
                                {'siem_type': self.client_type, 'result_count': len(results),
                                 'correlation_id': correlation_id})
            return results
        except SIEMClientError:
            raise
        except Exception as e:
            raise SIEMClientQueryError(f"Unexpected error during {self.client_type} query: {e}",
                                      self.client_type, original_exception=e, correlation_id=correlation_id)
        finally:
            self._release_rate_limit()
            self._set_correlation_id(None)
...
class TestSecretScrubbing:
    ...
    @pytest.mark.parametrize("data,expected", [
        ({"user": "admin", "password": "my_password_123"}, {"user": "admin", "password": "[SCRUBBED]"}),
        ({"details": {"api_key": "x-api-key-12345"}}, {"details": {"api_key": "[SCRUBBED]"}}),
...
class TestLogSending:
    ...
    async def test_send_logs_batch(self, test_client):
        ...
        logs = [
            {"event_type": "test", "message": f"Log {i}"}
            for i in range(10)
        ]
...
class TestQueryLogs:
    ...
    async def test_query_logs_unimplemented(self):
        ...
        class UnimplementedClient(BaseSIEMClient):
            ...
            async def _perform_health_check_logic(self): return True, "ok"
            async def _perform_send_log_logic(self, log_entry): return True, "ok"
...
class TestCorrelationId:
    ...
    async def test_correlation_id_in_errors(self, test_client):
        ...
        with pytest.raises(SIEMClientError) as exc_info:
            await test_client.send_log({"message": "test"}, 
                                     correlation_id=correlation_id, 
