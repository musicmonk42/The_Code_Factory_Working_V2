...
def mock_external_deps(mocker):
    ...
    mocker.patch('simulation.plugins.dlt_clients.dlt_base.os.path.abspath', side_effect=lambda x: f"/mock/path/{x}")
    mocker.patch('simulation.plugins.dlt_clients.dlt_base.os.urandom', return_value=b'test-hmac-key-for-testing-only-123')
    mocker.patch('simulation.plugins.dlt_clients.dlt_base.json.dump')
    mocker.patch('simulation.plugins.dlt_clients.dlt_base.json.load', return_value={"last_verified_entry_count": 0, "last_verification_time": "2025-01-01T00:00:00"})
...
async def test_audit_manager_integrity_check_success(mocker):
    ...
    mocker.patch('simulation.plugins.dlt_clients.dlt_base._get_dlt_audit_hmac_key', return_value=mock_hmac_key)
    
    event_data = {"event_type": "test", "payload": {"key": "value"}, "timestamp": "2025-01-01T00:00:00.000000"}
...
async def test_audit_manager_integrity_check_failure(mocker):
    ...
    tampered_event = {"event_type": "test", "payload": {"key": "malicious"}, "timestamp": "2025-01-01T00:00:00.000000"}
...
async def test_async_retry_decorator(mocker):
    ...
    class MockClient:
        def __init__(self):
            ...
            self.config = {'retry_policy': {'retries': 3, 'delay': 0.1, 'backoff': 2, 'jitter': False}}
    ...
    call_count = 0
    mock_client.config = {'retry_policy': {'retries': 2, 'delay': 0.1, 'backoff': 2, 'jitter': False}}
