...
def disable_info_logging():
    ...
    _base_logger.setLevel(logging.CRITICAL + 1)  # Suppress all logs below CRITICAL to reduce warning noise
    yield
...
def mock_dlt_client():
    ...
    mock.read_checkpoint = AsyncMock(return_value={"metadata": {}, "payload_blob": b"mock data", "tx_id": "mock_tx_id"})
    mock.rollback_checkpoint = AsyncMock(return_value={"metadata": {}, "tx_id": "mock_rollback_tx_id", "version": 2})
...
def test_cli_health_check_success(mock_factory, mock_config_file):
    ...
    """
    runner = CliRunner()
    result = runner.invoke(cli, ['health-check', '--dlt-type', 'simple', '--config-file', mock_config_file], standalone_mode=False)
...
def test_cli_health_check_failure(mock_factory, mock_config_file):
    ...
    """
    mock_factory.health_check.return_value = {"status": False, "message": "Failed", "details": {}}
...
def test_cli_write_checkpoint_success(mock_factory, mock_config_file, mock_payload_file):
    ...
    result = runner.invoke(cli, [
        ...
        '--checkpoint-name', 'test-chain',
        '--hash', 'test-hash',
        '--payload-file', mock_payload_file
...
def test_cli_rollback_checkpoint_success(mock_factory, mock_config_file):
    ...
    result = runner.invoke(cli, [
        ...
        '--config-file', mock_config_file,
        '--checkpoint-name', 'test-chain',
        '--rollback-hash', 'rollback-hash-value'
    ], standalone_mode=False)
...
def test_cli_invalid_config_file(mocker, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['health-check', '--dlt-type', 'simple', '--config-file', str(tmp_path / "non-existent.json")], standalone_mode=False, catch_exceptions=True)
...
def test_cli_invalid_json_in_config(mocker, tmp_path):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ['health-check', '--dlt-type', 'simple', '--config-file', str(invalid_json_file)], standalone_mode=False, catch_exceptions=True)
...
def test_cli_dlt_client_configuration_error(mocker, mock_config_file):
    ...
    """
    mocker.patch.object(DLTFactory, 'list_available_dlt_clients', return_value=['simple', 'evm'])
    mocker.patch.object(DLTFactory, 'get_dlt_client', new=AsyncMock(side_effect=DLTClientConfigurationError("Mock config error", "Main")))
    mocker.patch('simulation.plugins.dlt_clients.dlt_main.alert_operator', new=AsyncMock())
    mocker.patch('simulation.plugins.dlt_clients.dlt_main.scrub_secrets', side_effect=lambda x: x)
    
    runner = CliRunner()
    result = runner.invoke(cli, ['health-check', '--dlt-type', 'simple', '--config-file', mock_config_file], standalone_mode=False, catch_exceptions=True)
...
def test_cli_verbose_flag(mock_factory, mock_config_file):
    ...
    with patch('simulation.plugins.dlt_clients.dlt_main._base_logger') as mock_logger:
        result = runner.invoke(cli, ['--verbose', 'health-check', '--dlt-type', 'simple', '--config-file', mock_config_file], standalone_mode=False)
