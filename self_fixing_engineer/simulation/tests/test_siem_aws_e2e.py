...
class MockAwsCloudWatchClient:
    def __init__(self, config, metrics_hook=None):
        ...
        self.log_stream_name = config.get("awscloudwatch", {}).get("log_stream_name", LOG_STREAM_NAME)
    ...
    async def health_check(self, correlation_id=None):
        ...
        if self.metrics_hook:
            self.metrics_hook("health_check", "success", {
    ...
    async def send_logs(self, log_entries, correlation_id=None):
        ...
        if self.metrics_hook:
            self.metrics_hook("send_logs", "success", {
                "client_type": self.client_type,
                "latency": 0.1,
    ...
    async def _perform_send_log_logic(self, log_entry):
        ...
        if self.metrics_hook:
            self.metrics_hook("send_log", "success", {
                "client_type": self.client_type,
                "latency": 0.05
            })
...
async def aws_client(siem_config, metrics_collector):
    ...
    client = None
    try:
        client = await get_siem_client("aws_cloudwatch", siem_config, metrics_hook=metrics_collector)
        yield client
    finally:
        if client:
            await client.close()
