...
try:
    import web3
    from web3.exceptions import ContractLogicError, ContractCustomError, TransactionNotFound, TimeExhausted
...
if WEB3_AVAILABLE:
    ...
    class QuorumConfig(BaseModel):
        ...
        def validate_private_for(cls, v, values):
            ...
            if v:
                if not isinstance(v, list) or not all(re.match(r"^[A-Za-z0-9+/]{44}$", pk) for pk in v):
...
class TestQuorumConfigValidation:
    ...
    def test_missing_abi_source(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            ...
            QuorumConfig(**invalid_config)
        assert "Either contract_abi_path or contract_abi_secret_id must be provided" in str(exc_info.value)
...
class TestQuorumClient:
    ...
    def mock_off_chain(self):
        ...
        mock.health_check.return_value = {"status": True, "message": "OK", "details": {}}
    ...
    def mock_secrets_manager(self, mocker):
        ...
        SECRETS_MANAGER.get_secret.side_effect = lambda key, **kwargs: (
            "0x" + "1" * 64 if "private_key" in key.lower() else
            json.dumps([{"anonymous": False, "inputs": [], "name": "writeCheckpoint", "outputs": [], "stateMutability": "nonpayable", "type": "function"}]) if "abi" in key.lower() else
            "mock_secret"
        )
    ...
    def mock_web3_provider(self, mocker):
        ...
        mock_w3.eth.wait_for_transaction_receipt = AsyncMock(return_value=MagicMock(status=1, blockNumber=123))
        ...
        mock_account.sign_transaction = MagicMock(return_value=MagicMock(rawTransaction=b"mock_raw_tx"))
