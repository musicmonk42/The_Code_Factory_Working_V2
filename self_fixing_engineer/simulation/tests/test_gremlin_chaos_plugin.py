...
def mock_gremlin_and_env():
    ...
    with patch.dict(os.environ, {
        ...
        with patch('gremlin_chaos_plugin.GremlinApiClient') as MockGremlinApiClient, \
            ...
            with patch.object(gremlin_chaos_plugin.CHAOS_ATTACKS_TOTAL, 'labels', return_value=MagicMock()), \
                 patch.object(gremlin_chaos_plugin.CHAOS_ATTACK_ERRORS_TOTAL, 'labels', return_value=MagicMock()), \
                 patch.object(gremlin_chaos_plugin.CHAOS_ATTACK_DURATION_SECONDS, 'labels', return_value=MagicMock()), \
                 patch.object(gremlin_chaos_plugin.GREMLIN_INFLIGHT_ATTACKS, 'inc', return_value=MagicMock()), \
                 patch.object(gremlin_chaos_plugin.GREMLIN_INFLIGHT_ATTACKS, 'dec', return_value=MagicMock()), \
                 patch.object(gremlin_chaos_plugin.GREMLIN_HTTP_RESPONSES_TOTAL, 'labels', return_value=MagicMock()), \
...
async def test_plugin_health_api_error(mock_gremlin_and_env):
    plugin = mock_gremlin_and_env['plugin']
    mock_gremlin_and_env['mock_gremlin_client'].quick_check.side_effect = plugin.GremlinApiError("Unauthorized", status=401)
    result = await plugin.plugin_health()
    assert result['status'] == 'error'
...
@pytest.mark.asyncio
async def test_run_chaos_experiment_network_latency_kubernetes_success(mock_gremlin_and_env):
    plugin = mock_gremlin_and_env['plugin']
    mock_gremlin_and_env['mock_gremlin_client'].get_attack_status.side_effect = [
        {"state": "PENDING"},
        {"state": "RUNNING"},
...
async def test_run_chaos_experiment_api_error_on_initiation(mock_gremlin_and_env):
    plugin = mock_gremlin_and_env['plugin']
    mock_gremlin_and_env['mock_gremlin_client'].create_attack.side_effect = plugin.GremlinApiError("Bad Request", status=400)
    res = await plugin.run_chaos_experiment(
        experiment_type="cpu_hog",
        target_type="Host",
        target_value="mock-host",
    ...
    plugin.CHAOS_ATTACK_ERRORS_TOTAL.labels.assert_called_with(experiment_type='cpu_hog', error_type='api_error')
    assert plugin.CHAOS_ATTACKS_TOTAL.labels.return_value.inc.call_count == len(expected_labels_calls)
...
async def test_run_chaos_experiment_monitoring_timeout(mock_gremlin_and_env):
    ...
    with patch.dict(os.environ, {"GREMLIN_POLL_INTERVAL_SECONDS": "5", "GREMLIN_TIMEOUT_SECONDS": "10"}):
        res = await plugin.run_chaos_experiment(
            experiment_type="cpu_hog",
            target_type="Host",
            target_value="mock-host",
