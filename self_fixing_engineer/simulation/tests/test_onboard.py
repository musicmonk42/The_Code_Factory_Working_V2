...
def mock_external_dependencies():
    ...
    mock_mesh_pubsub.return_value.healthcheck = AsyncMock(return_value={"status": "ok", "message": "Mocked health check"})
    ...
    patches = [
        ...
        patch('simulation.plugins.onboard.hvac.Client') if hasattr(onboard_module, 'hvac') else None,
...
def mock_filesystem():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('simulation.plugins.onboard.script_dir', temp_path), \
             ...
             patch('simulation.plugins.onboard.SECURE_CONFIG_PATH', configs_dir / "secure.json"), \
             patch('simulation.plugins.onboard.SECURE_KEY_PATH', configs_dir / "secure.key"):
...
@pytest.mark.asyncio
async def test_onboarding_wizard_full_flow(mock_external_dependencies, mock_filesystem, mock_user_input):
    """
    Test the entire interactive onboarding wizard workflow.
    """
    with patch('builtins.input', side_effect=mock_user_input), \
        ...
        args = argparse.Namespace(
            help=False, reset=False, safe=False, troubleshoot=False,
            show_examples=False, verbose=False, quiet=False, json_log=False,
            project_type=None, plugin_types=None, pubsub_backend=None, checkpoint_backend=None
        )
        await onboard(args)
...
async def test_run_health_checks_with_failures(mock_external_dependencies, mock_filesystem):
    ...
    with patch('simulation.plugins.onboard.print_status') as mock_print_status:
        ...
        mock_print_status.assert_any_call("Checkpoint Health: OK (saved and loaded test data successfully for fs).", "ok")
...
def test_load_secure_config_local_decrypted(mock_filesystem, mock_external_dependencies):
    ...
    secure_data = {
        "secrets": {"TEST_SECRET": encrypted_secret.decode('utf-8')}
    }
    ...
    with patch('simulation.plugins.onboard.crypto_available', True), \
...
"""
    ...
    with patch('simulation.plugins.onboard.print_status') as mock_print_status:
        ...
        mock_print_status.assert_any_call("Test 1 (Config Content): Basic fields present. PASSED.", "ok")
        mock_print_status.assert_any_call("Test 2 (Python Plugin Content): Basic structure present. PASSED.", "ok")
