...
class SIEMClientResponseError(SIEMClientError): pass
    ...
    def __init__(self, **kwargs):
        ...
        if os.getenv('PRODUCTION_MODE') == 'true':
            if not self.shared_key_secret_id and self.shared_key:
                raise ValueError("In PRODUCTION_MODE, 'shared_key' must be loaded via 'shared_key_secret_id'")
...
class AzureSentinelClient:
    ...
    async def health_check(self):
        ...
        if hasattr(response, 'status') and response.status not in [200, 202]:
            raise SIEMClientResponseError(f"API responded with {response.status}", self.client_type, response.status, "")
    ...
    async def query_logs(self, query_string, time_range, limit):
        if not self.use_aad_for_query:
            raise SIEMClientConfigurationError("KQL querying without Azure AD is not supported", self.client_type)
...
class TestConfiguration:
    ...
    def test_eventgrid_valid_config(self):
        ...
        config = AzureEventGridConfig(
            endpoint="https://test.eventgrid.azure.net",
            key="test_key"
        )
...
class TestAzureSentinel:
    ...
    async def test_query_logs_success(self, sentinel_client):
        ...
        results = await sentinel_client.query_logs(
            "search *",
            "1h",
            10
        )
    ...
    async def test_query_logs_auth_failure(self, sentinel_client):
        ...
        with pytest.raises(SIEMClientAuthError):
            await sentinel_client.query_logs(
...
class TestAzureEventGrid:
    ...
    async def test_send_batch_events(self, eventgrid_client):
        ...
        log_entries = [
            {"event_type": f"Event{i}", "data": f"data{i}"}
            for i in range(500)
        ]
...
class TestIntegration:
    ...
    async def test_sentinel_full_pipeline(self, sentinel_client):
        ...
        log_entries = [
            {"id": test_id, "message": f"Integration test {i}"}
            for i in range(10)
        ]
    ...
    async def test_multi_client_workflow(self, sentinel_client, eventgrid_client, servicebus_client):
