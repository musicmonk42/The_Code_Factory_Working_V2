...
try:
    from prometheus_client import Counter, Histogram, Gauge, REGISTRY, generate_latest, CollectorRegistry
    PROMETHEUS_AVAILABLE = True
except ImportError:
    PROMETHEUS_AVAILABLE = False
    logging.getLogger(__name__).warning("Prometheus client not available. Metrics will not be collected in explain.py.")
...
try:
    ...
    logging.getLogger(__name__).warning("Pydantic not available. Input validation will be skipped in explain.py.")
...
try:
    ...
    PromptTemplate = None
    BaseChatModel = None
    logging.getLogger(__name__).warning("langchain_openai not available. LLM integration will be limited.")
...
try:
    ...
    logging.getLogger(__name__).warning("aiosqlite not available. Async DB access will not be available.")
...
try:
    ...
    logging.getLogger(__name__).warning("Warning: Hugging Face Transformers and/or PyTorch not found. The Reasoner will operate in fallback mode.")
...
METRICS = {
    "inference_total": get_or_create_metric(Counter, "reasoner_inference_total", "Total number of inferences processed", ["type"]),
    "inference_success": get_or_create_metric(Counter, "reasoner_inference_success_total", "Total number of successful inferences", ["type"]),
    "inference_errors": get_or_create_metric(Counter, "reasoner_inference_errors_total", "Total number of inference errors", ["type", "error_code"]),
    "inference_duration_seconds": get_or_create_metric(Histogram, "reasoner_inference_duration_seconds", "Inference duration in seconds", ["type"]),
    "history_entries_current": get_or_create_metric(Gauge, "reasoner_history_entries_current", "Current number of history entries"),
    "history_operations_total": get_or_create_metric(Counter, "reasoner_history_operations_total", "Total history database operations", ["operation", "status"]),
    "model_load_errors": get_or_create_metric(Counter, "reasoner_model_load_errors_total", "Total errors during model loading", ["model_name", "device"]),
    "model_pipeline_usage": get_or_create_metric(Counter, "reasoner_model_pipeline_usage_total", "Total usage of each model pipeline", ["model_name", "device"]),
    "health_status": get_or_create_metric(Gauge, "reasoner_health_status", "Health status of the reasoner (1=healthy, 0=unhealthy)", ["component"]),
    "last_health_check_timestamp": get_or_create_metric(Gauge, "reasoner_last_health_check_timestamp_seconds", "Timestamp of the last health check"),
    "explanation_latency": get_or_create_metric(Histogram, "explain_latency_seconds", "Explanation generation latency", ["result_id"]),
}
...
def _run_in_thread(fn: Callable, *args: Any, timeout: int = 15, **kwargs: Any) -> Any:
    if _executor is None:
        logger.critical("ThreadPoolExecutor is not initialized or is shut down when _run_in_thread was called synchronously.")
...
def _sanitize_context(context: Dict[str, Any], max_size_bytes: int = 4096) -> Dict[str, Any]:
    ...
    def _json_serializable_converter(obj: Any) -> Any:
        ...
        if isinstance(obj, collections.abc.Coroutine):
            logger.warning(f"Coroutine object found in context: {obj}. Converting to string. Caller should await this value.")
        ...
        logger.warning(f"Non-JSON-serializable object of type {type(obj)} found in context. Converting to string.")
        return str(obj)
    ...
    try:
        ...
        if len(context_json.encode('utf-8')) > max_size_bytes:
            logger.warning(f"Context size ({len(context_json.encode('utf-8'))} bytes) exceeds max_size ({max_size_bytes} bytes). Attempting to truncate.")
            ...
            while len(truncated_json_str.encode('utf-8')) > max_size_bytes:
                ...
                if cut_point == -1 or len(truncated_json_str) < 50:
                    logger.error("Could not truncate context while maintaining JSON structure. Returning minimal error context.")
                    return {"_truncated_context_error": f"Original context too large ({len(context_json.encode('utf-8'))} bytes) and could not be safely truncated. Content is likely incomplete."}
            ...
            try:
                ...
                return {"_truncated_context_error": f"Original context too large ({len(context_json.encode('utf-8'))} bytes) and truncated. Content may be incomplete."}
        return json.loads(context_json)
    except json.JSONDecodeError as e:
        raise ReasonerError(f"Invalid context format after processing: {e}", code="INVALID_CONTEXT_FORMAT", original_exception=e)
    except Exception as e:
        raise ReasonerError(f"Failed to sanitize context: {e}", code="CONTEXT_SANITIZATION_FAILED", original_exception=e)
...
class HistoryManager:
    ...
    async def init_db(self):
        ...
        try:
            async with self._db_lock:
                ...
                METRICS["history_operations_total"].labels(operation="init_db", status="success").inc()
            ...
            raise ReasonerError("Database initialization failed", code="DB_INIT_FAILED", original_exception=e)
    ...
    async def add_entry(self, entry: ReasoningHistory):
        try:
            async with self._db_lock:
                ...
                METRICS["history_operations_total"].labels(operation="add_entry", status="success").inc()
            ...
            raise ReasonerError("Failed to add history entry", code="DB_WRITE_FAILED", original_exception=e)
                ...
                async with aiosqlite.connect(str(self.db_path)) as conn:
                    ...
                    async for row in cursor:
                        try:
                            ...
                            logger.warning(f"Failed to decode context for history entry {row['id']}: {e}. Skipping this entry's context.")
                            entry = ReasoningHistory(
                                id=row["id"],
                                query=row["query"],
                            ...
                            entries.append(entry)
                    METRICS["history_operations_total"].labels(operation="get_entries", status="success").inc()
                return entries
        except (sqlite3.Error, aiosqlite.Error) as e:
            logger.error(f"Failed to retrieve history entries: {e}")
            METRICS["history_operations_total"].labels(operation="get_entries", status="error").inc()
            raise ReasonerError("Failed to retrieve history entries", code="DB_READ_FAILED", original_exception=e)
    ...
    async def clear(self):
        try:
            async with self._db_lock:
                ...
                METRICS["history_operations_total"].labels(operation="clear_history", status="success").inc()
                logger.info("History database cleared.")
        except (sqlite3.Error, aiosqlite.Error) as e:
            logger.error(f"Failed to clear history: {e}")
            METRICS["history_operations_total"].labels(operation="clear_history", status="error").inc()
            raise ReasonerError("Failed to clear history", code="DB_CLEAR_FAILED", original_exception=e)
    ...
    async def get_size(self) -> int:
        try:
            ...
            raise ReasonerError("Failed to get history size", code="DB_READ_FAILED", original_exception=e)
...
class ExplainableReasoner:
    def __init__(self, settings: Any, config: Optional[ReasonerConfig] = None):
        ...
        self.history = HistoryManager(db_path=history_db_full_path, max_size=self.config.max_history_size)
        ...
        self.executor: Optional[ThreadPoolExecutor] = ThreadPoolExecutor(max_workers=max(1, self.config.max_workers))
    ...
    async def _initialize_models_async(self):
        ...
        if not TRANSFORMERS_AVAILABLE:
            self.logger.warning("Transformers library not available. Models will not be loaded. Operating in fallback mode.")
            self.pipelines = []
        ...
        for model_cfg in model_configs_to_load:
            ...
            for attempt in range(retries):
                try:
                    self.logger.info(f"Attempting to load model: {model_name} on device {device} (attempt {attempt + 1}/{retries})")
                    model_kwargs = {
                    ...
                    if device >= 0 and 'torch' in sys.modules and torch.cuda.is_available():
                        current_quantization_config = BitsAndBytesConfig(
                            ...
                            bnb_4bit_compute_dtype=getattr(torch, quantization_cfg_params.get("bnb_4bit_compute_dtype", "float16")),
                            bnb_4bit_quant_type=quantization_cfg_params.get("bnb_4bit_quant_type", "nf4"),
                            bnb_4bit_use_double_quant=quantization_cfg_params.get("bnb_4bit_use_double_quant", True),
                            bnb_4bit_quant_storage=getattr(torch, quantization_cfg_params.get("bnb_4bit_quant_storage", "uint8"), torch.uint8),
                        )
                        model_kwargs["quantization_config"] = current_quantization_config
                        model_kwargs["torch_dtype"] = getattr(torch, quantization_cfg_params.get("torch_dtype", "float16"))
                        self.logger.info(f"Loading model '{model_name}' with 4-bit quantization on CUDA device {device}.")
                    elif device >= 0:
                        self.logger.warning(f"PyTorch and/or CUDA not found. Cannot apply BitsAndBytesConfig for quantization or load on GPU. Loading model '{model_name}' on CPU.")
                        device = -1

                    tokenizer = await asyncio.get_event_loop().run_in_executor(
                        self.executor, partial(AutoTokenizer.from_pretrained, model_name, **model_kwargs)
                    )
                    model_obj = await asyncio.get_event_loop().run_in_executor(
                        self.executor, partial(AutoModelForCausalLM.from_pretrained, model_name, **model_kwargs)
                    )
                    ...
                    if text_generation_pipeline.tokenizer.pad_token_id is None:
                        text_generation_pipeline.tokenizer.pad_token_id = text_generation_pipeline.tokenizer.eos_token_id
                    ...
                    self.logger.warning(f"Transformers library not available during retry for {model_name}. Operating in fallback mode.")
                    METRICS["model_load_errors"].labels(model_name=model_name, device=device).inc()
                    break
                except Exception as e:
                    self.logger.error(f"Failed to load NLP model '{model_name}' on device {device} (attempt {attempt + 1}): {e}", exc_info=True)
                    METRICS["model_load_errors"].labels(model_name=model_name, device=device).inc()
                    if attempt < retries - 1:
                        ...
                        self.logger.error(f"Max retries ({retries}) exceeded for model loading '{model_name}'. Operating in fallback mode for this model.")
                        if self.config.strict_mode:
                            raise ReasonerError(f"Model initialization failed after retries for {model_name}: {e}", code="MODEL_INIT_FAILED", original_exception=e)

        if not self.pipelines:
            self.logger.warning("No models were successfully loaded. Reasoner will operate in fallback mode.")
            METRICS["health_status"].labels(component="llm_models").set(0)
        else:
            METRICS["health_status"].labels(component="llm_models").set(1)
    ...
    async def _get_next_pipeline(self) -> Optional[Dict[str, Any]]:
        async with self._pipeline_lock:
            ...
            METRICS["model_pipeline_usage"].labels(model_name=pipeline_info['model_name'], device=pipeline_info['device']).inc()
    ...
    def _generate_text_sync(self, pipeline_info: Dict[str, Any], prompt: str, max_new_tokens: int, temperature: float) -> str:
        ...
        if prompt_length_tokens >= model_max_length:
            self.logger.warning(f"Prompt (tokens: {prompt_length_tokens}) exceeds or fills model's absolute max input length ({model_max_length}). Truncating prompt.")
            target_length = max(10, model_max_length - max_new_tokens)
            if target_length <= 0:
                self.logger.warning(f"Model max length ({model_max_length}) too small to even fit a truncated prompt for generation. Returning empty string.")
                return ""
            ...
            if prompt_length_tokens >= model_max_length:
                self.logger.warning("Prompt still fills entire model context after truncation. Cannot generate new tokens. Returning empty string.")
                return ""
        ...
        result_raw = model_pipeline(
            prompt,
            **generation_kwargs
        )
        
        if isinstance(result_raw, list) and len(result_raw) > 0 and 'generated_text' in result_raw[0]:
            generated_text = result_raw[0]["generated_text"]
            if generated_text.startswith(prompt):
                return generated_text[len(prompt):].strip()
            return generated_text.strip()
        else:
            self.logger.error(f"Unexpected model output format from {pipeline_info['model_name']} on device {pipeline_info['device']}: {result_raw}")
            raise ReasonerError("Model returned an unexpected output format.", code="BAD_MODEL_OUTPUT")
    ...
    async def _async_generate_text(self, prompt: str, max_length: int, temperature: float) -> str:
        ...
        if pipeline_info is None:
            raise ReasonerError("No text generation models are loaded or available.", code="MODEL_NOT_INITIALIZED")
        ...
        try:
            ...
            if "index out of range in self" in str(e) or "The attention mask is not all zeros" in str(e):
            ...
            self.logger.error(f"Error during text generation for model {pipeline_info['model_name']} on device {pipeline_info['device']}: {message}", exc_info=True)
        ...
        with METRICS["inference_duration_seconds"].labels(type="explain").time():
            ...
            try:
                ...
                if self.config.mock_mode or not self.pipelines:
                    response_text = _rule_based_fallback(sanitized_query, sanitized_context, "explain")
                    ...
                    prompt_template = self._create_explanation_prompt(sanitized_query, sanitized_context)
                ...
                METRICS["inference_errors"].labels(type="explain", error_code="UNEXPECTED_ERROR").inc()
                raise ReasonerError(f"An unexpected error occurred during reasoning: {e}", code="UNEXPECTED_ERROR", original_exception=e)
    ...
    async def reason(self, query: str, context: Dict[str, Any] = None) -> ReasoningResult:
        ...
        with METRICS["inference_duration_seconds"].labels(type="reason").time():
            ...
            try:
                ...
                if self.config.mock_mode or not self.pipelines:
                    response_text = _rule_based_fallback(sanitized_query, sanitized_context, "reason")
                    ...
                    prompt_template = self._create_reasoning_prompt(sanitized_query, sanitized_context)
                    prompt = prompt_template.format(history_placeholder=history_str)
                ...
                METRICS["inference_errors"].labels(type="reason", error_code="UNEXPECTED_ERROR").inc()
                raise ReasonerError(f"An unexpected error occurred during reasoning: {e}", code="UNEXPECTED_ERROR", original_exception=e)
...
if PYDANTIC_AVAILABLE:
    class ExplanationInput(BaseModel):
        result_id: str = Field(..., pattern=r"^[a-zA-Z0-9_-]+$", description="Unique ID of the simulation result.")
        status: str = Field(..., pattern=r"^(COMPLETED|FAILED|ERROR|SKIPPED|TIMEOUT)$", description="Status of the simulation result.")
        details: Dict[str, Any] = Field(default_factory=dict, description="Detailed results of the simulation.")
else:
    ...
    async def explain_result(self, result: Dict[str, Any]) -> str:
        ...
        try:
            if PYDANTIC_AVAILABLE:
                ...
                if not isinstance(validated_result, dict) or "id" not in validated_result or "status" not in validated_result:
                    raise ValueError("Invalid result input format.")
                # Manual regex check for status
                if not re.match(r"^(COMPLETED|FAILED|ERROR|SKIPPED|TIMEOUT)$", validated_result.get("status", "UNKNOWN")):
            ...
            if self.dlt_logger:
                await self.dlt_logger.add_entry(
                    ...
                    detail={"result_id": result.get("id"), "error": str(e), "reason": "input_validation_failed"},
                    agent_id="explain",
                    correlation_id=correlation_id
                )
            raise ReasonerError(f"Input validation failed for explanation: {e}", code="INPUT_VALIDATION_ERROR")
        ...
        with METRICS["explanation_latency"].labels(result_id=validated_result.result_id).time():
            try:
                if not LANGCHAIN_OPENAI_AVAILABLE:
                    explanation_text = _rule_based_fallback(f"explain result {validated_result.result_id}", validated_result.details, "explain")
                    logger.warning("langchain_openai not available. Using fallback for explanation generation.")
                else:
                    grok_api_key = None
                    if self.secrets_manager:
                         grok_api_key = self.secrets_manager.get_secret("GROK_API_KEY", required=False)
                    
                    if not grok_api_key:
                        logger.warning("GROK_API_KEY not found. Using fallback for explanation generation.")
                        explanation_text = _rule_based_fallback(f"explain result {validated_result.result_id}", validated_result.details, "explain")
                    else:
                ...
                if self.dlt_logger:
                    await self.dlt_logger.add_entry(
                        ...
                        detail={"result_id": validated_result.result_id, "explanation": explanation_text[:200] + "...", "status": validated_result.status},
                        agent_id="explain",
                        correlation_id=correlation_id
                    )
                return explanation_text
            except Exception as e:
                logger.error(f"Failed to generate explanation for result {validated_result.result_id}: {e}", exc_info=True)
                if self.dlt_logger:
                    await self.dlt_logger.add_entry(
                        kind="explain",
                        name="explanation_error",
                        detail={"result_id": validated_result.result_id, "error": str(e), "reason": "llm_generation_failed"},
                        agent_id="explain",
                        correlation_id=correlation_id
                    )
                raise ReasonerError(f"Failed to generate explanation: {e}", code="EXPLANATION_GENERATION_FAILED")
        ...
        try:
            if action == "explain":
                ...
                if not PROMETHEUS_AVAILABLE:
                    raise ReasonerError("Prometheus client not available.", code="METRICS_UNAVAILABLE")
                return generate_latest(registry=_metrics_registry).decode('utf-8')
            elif action == "get_health":
                if not PROMETHEUS_AVAILABLE:
                    return {"status": "warning", "message": "Prometheus client not available, limited health status provided."}
                ...
                for metric in _metrics_registry.collect():
                    if metric.name == "reasoner_health_status":
                        for sample in metric.samples:
                            ...
                            if component:
                                health_status[component] = "healthy" if sample.value == 1.0 else "unhealthy"
                return {"status": "success", "health": health_status, "timestamp": datetime.utcnow().isoformat()}
            else:
                raise ValueError(f"Unknown action for ExplainableReasonerPlugin: {action}")
        except ReasonerError as e:
            self.logger.critical(f"Critical ReasonerError during execute action '{action}': {e.message} (Code: {e.code})", exc_info=True)
            return {"status": "error", "message": e.message, "code": e.code}
        except Exception as e:
            self.logger.critical(f"Unexpected critical error during execute action '{action}': {e}", exc_info=True)
            return {"status": "error", "message": f"An unexpected error occurred: {e}", "code": "UNEXPECTED_CRITICAL_ERROR"}
