...
try:
    from prometheus_client import make_asgi_app, Counter, Gauge, Histogram, CollectorRegistry
...
def get_or_create_metric(metric_type, name, documentation, labelnames=None, buckets=None):
    ...
    if metric_type == Histogram and buckets:
        return metric_type(name, documentation, labelnames, buckets=buckets, registry=_metrics_registry)
    else:
        return metric_type(name, documentation, labelnames, registry=_metrics_registry)

if PROMETHEUS_AVAILABLE:
    PARALLEL_METRICS = {
        "backend_usage_total": get_or_create_metric(Counter, "parallel_backend_usage_total", "Total usage of each parallel backend", ["backend_name"]),
        "simulation_tasks_total": get_or_create_metric(Counter, "parallel_simulation_tasks_total", "Total simulation tasks processed", ["status"]),
        "rl_tuner_heartbeat": get_or_create_metric(Gauge, "parallel_rl_tuner_heartbeat_timestamp", "Timestamp of last RL tuner heartbeat"),
        "rl_tuner_status": get_or_create_metric(Gauge, "parallel_rl_tuner_status", "Status of RL tuner (1=alive, 0=dead)"),
        "backend_availability": get_or_create_metric(Gauge, "parallel_backend_availability", "Availability of parallel backends (1=available, 0=unavailable)", ["backend_name"]),
        "job_latency": get_or_create_metric(Histogram, "parallel_job_latency_seconds", "Latency of individual jobs", ["backend_name"]),
    }
else:
...
if PYDANTIC_AVAILABLE:
    ...
    class ParallelConfig(BaseModel):
        ...
        def validate_backend_configs(self):
            ...
            if self.default_backend == "aws_batch" and (not self.backend_configs.aws_batch_job_queue or not self.backend_configs.aws_batch_job_definition):
                 raise ValueError("AWS Batch job queue and definition must be specified if it's the default backend.")
            return self
        ...
        def load_from_yaml(cls, file_path: str):
            try:
                ...
                parallel_logger.critical(f"Critical Error: Failed to load or validate parallel configuration from {file_path}: {e}. Aborting startup.")
                sys.exit(1)
            except Exception as e:
                parallel_logger.critical(f"Critical Error: An unexpected error occurred while loading parallel config from {file_path}: {e}. Aborting startup.")
                sys.exit(1)
else:
    ...
    class ParallelConfig:
        def __init__(self):
            ...
            self.rl_tuner = type('RLTunerConfig', (object,), {
                'enabled': False,
            ...
            self.backend_configs = type('BackendSpecificConfig', (object,), {
            ...
            self.secrets = type('SecretsConfig', (object,), {
        ...
        def load_from_yaml(cls, file_path: str):
            ...
            try:
                with open(file_path, 'r') as f:
                    config_data = yaml.safe_load(f)
                instance = cls()
                if 'default_backend' in config_data: instance.default_backend = config_data['default_backend']
                if 'max_local_workers' in config_data: instance.max_local_workers = config_data['max_local_workers']
                ...
                parallel_logger.critical(f"Critical Error: Failed to load parallel configuration from {file_path}: {e}. Aborting startup.")
                sys.exit(1)
            except Exception as e:
                parallel_logger.critical(f"Critical Error: An unexpected error occurred while loading parallel config from {file_path}: {e}. Aborting startup.")
                sys.exit(1)
...
PARALLEL_CONFIG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'config', 'parallel_config.yaml')
...
def load_secret(secret_name: str) -> str:
    ...
    try:
        ...
        parallel_logger.critical(f"Unexpected error loading secret {secret_name}: {e}", exc_info=True)
        raise
...
def register_backend(name: str, is_available: bool = True):
    def decorator(func: Callable):
        ...
        if PROMETHEUS_AVAILABLE:
            PARALLEL_METRICS["backend_availability"].labels(backend_name=name).set(1 if is_available else 0)
        return func
    return decorator
...
class RayRLlibConcurrencyTuner:
    def __init__(self, config: RLTunerConfig):
        ...
        if not RLLIB_AVAILABLE or not self.config.enabled:
            parallel_logger.warning("Ray RLlib not available or RL tuner disabled. Concurrency tuner will be disabled.")
            self.policy = None
            return
    ...
    def _init_policy(self):
        ...
        try:
            ...
            parallel_logger.warning(f"Could not load policy from checkpoint (error: {e}). Initializing a new policy.")
        ...
        def training_loop():
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    parallel_logger.info(f"Training RL policy with {experiences['obs'].shape[0]} new experiences.")
    ...
    def _collect_experiences(self) -> Dict[str, np.ndarray]:
        experiences = {"obs": [], "actions": [], "rewards": [], "new_obs": [], "terminateds": []}
        try:
            with self.log_lock:
            ...
            for line in new_lines:
                try:
                    ...
                    experiences["new_obs"].append(np.zeros_like(np.array(data["obs"], dtype=np.float32)))
    ...
    def check_liveness(self) -> bool:
        if not self.training_thread or not self.training_thread.is_alive():
            ...
            alert_ops("RL Training Thread is dead. Concurrency tuning will fall back to heuristics.", level="CRITICAL")
            if PROMETHEUS_AVAILABLE:
                PARALLEL_METRICS["rl_tuner_status"].set(0)
            return False
        
        if time.time() - self.last_heartbeat_time > self.config.heartbeat_timeout_seconds:
            parallel_logger.warning(f"RL training thread heartbeat is stale (last update {time.time() - self.last_heartbeat_time:.2f}s ago).")
            alert_ops("RL Training Thread heartbeat stale. Concurrency tuning may be degraded.", level="WARNING")
            if PROMETHEUS_AVAILABLE:
                PARALLEL_METRICS["rl_tuner_status"].set(0)
            return False
    ...
    def record_feedback(self, throughput: float, failures: int):
        if self.last_obs is not None and self.last_action is not None and self.policy is not None:
            ...
            parallel_logger.info(f"RL feedback recorded: action={self.last_action}, reward={reward:.2f}")
...
def get_available_resources() -> Dict[str, Any]:
    return {
        ...
        "gpus": len(os.environ.get("NVIDIA_VISIBLE_DEVICES", "").split(',')) if "NVIDIA_VISIBLE_DEVICES" in os.environ else 0
    }
    ...
    def task_completed(self, success: bool = True, job_latency: Optional[float] = None):
        with self._lock:
            ...
            if PROMETHEUS_AVAILABLE and job_latency is not None:
                PARALLEL_METRICS["job_latency"].labels(backend_name="local_asyncio").observe(job_latency)
...
async def execute_local_asyncio(simulation_function: Callable, configurations: List[Dict[str, Any]], **kwargs) -> List[Dict[str, Any]]:
    ...
    async def run_task(config):
        async with semaphore:
            ...
            try:
                ...
                parallel_logger.error(f"Task failed for config {config.get('id', 'N/A')}: {e}", exc_info=True)
...
async def execute_kubernetes(simulation_function: Callable, configurations: List[Dict[str, Any]], config: ParallelConfig, **kwargs) -> List[Dict[str, Any]]:
    ...
    if not image:
        alert_ops("Kubernetes simulation image not configured. Cannot run K8s jobs.", level="CRITICAL")
        raise ValueError("Kubernetes simulation image (K8S_SIMULATION_IMAGE env var or config) must be set.")
    ...
    async def run_k8s_job(cfg: Dict, job_idx: int):
        ...
        resources = cfg.get("resources", {"requests": {"cpu": "1", "memory": "1Gi"}, "limits": {"cpu": "2", "memory": "2Gi"}})
        ...
        try:
            ...
            parallel_logger.info(f"Submitted Kubernetes Job '{job_name}' in namespace '{namespace}'.")
            ...
            while time.time() - start_time < job_timeout:
                ...
                if job_status.status.succeeded:
                    pod_list = core_v1.list_namespaced_pod(namespace, label_selector=f"job-name={job_name}", limit=1)
                    if not pod_list.items:
                        parallel_logger.error(f"Could not find pod for successful job {job_name}")
                        reporter.task_completed(success=False)
                        return {"status": "RESULT_ERROR", "job_name": job_name, "error": "Could not find pod to retrieve logs."}
                    pod_name = pod_list.items[0].metadata.name
                    logs = core_v1.read_namespaced_pod_log(pod_name, namespace)
                    
                    try:
                        result_json = json.loads(logs)
                        reporter.task_completed(success=True, job_latency=time.time() - start_job_time)
                        return result_json
                    except json.JSONDecodeError:
                        reporter.task_completed(success=False, job_latency=time.time() - start_job_time)
                        parallel_logger.error(f"Failed to parse JSON from pod logs for job {job_name}. Logs: {logs[:200]}...")
                        return {"status": "RESULT_ERROR", "job_name": job_name, "error": "Failed to parse JSON from pod logs", "logs_snippet": logs[:500]}

                if job_status.status.failed:
                    reason = job_status.status.conditions[0].message if job_status.status.conditions else "Unknown failure"
                    reporter.task_completed(success=False, job_latency=time.time() - start_job_time)
                    parallel_logger.warning(f"Kubernetes Job '{job_name}' failed. Reason: {reason}")
                    return {"status": "FAILED", "job_name": job_name, "reason": reason, "config": cfg}
            
            reporter.task_completed(success=False, job_latency=time.time() - start_job_time)
            parallel_logger.warning(f"Kubernetes Job '{job_name}' timed out after {job_timeout} seconds.")
            return {"status": "TIMEOUT", "job_name": job_name, "config": cfg}
        
        except K8sApiException as e:
            reporter.task_completed(success=False, job_latency=time.time() - start_job_time)
            parallel_logger.error(f"Kubernetes API error for job {job_name}: {e.reason}", exc_info=True)
            alert_ops(f"Kubernetes API error for job {job_name}: {e.reason}", level="ERROR")
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="k8s_job_error", detail={"job_name": job_name, "error": str(e)}, agent_id="parallel_k8s")
            return {"status": "ERROR", "job_name": job_name, "error": f"Kubernetes API error: {e.reason}", "config": cfg}
        except Exception as e:
            reporter.task_completed(success=False, job_latency=time.time() - start_job_time)
            parallel_logger.error(f"Unexpected error running K8s job {job_name}: {e}", exc_info=True)
            alert_ops(f"Unexpected error running K8s job {job_name}: {e}", level="ERROR")
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="k8s_job_error", detail={"job_name": job_name, "error": str(e)}, agent_id="parallel_k8s")
            return {"status": "ERROR", "job_name": job_name, "error": f"Unexpected error: {e}", "config": cfg}
...
async def execute_aws_batch(simulation_function: Callable, configurations: List[Dict[str, Any]], config: ParallelConfig, **kwargs) -> List[Dict[str, Any]]:
    if not BOTO3_AVAILABLE:
    ...
    if not all([job_queue, job_definition, s3_bucket]):
        alert_ops("Missing required AWS Batch configuration (job_queue, job_definition, s3_bucket).", level="CRITICAL")
        raise ValueError("Missing required AWS Batch configuration (job_queue, job_definition, s3_bucket).")
    ...
    for i, cfg in enumerate(configurations):
        ...
        try:
            ...
            parallel_logger.info(f"Submitted AWS Batch Job '{job_name}' with ID: {response['jobId']}")
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="aws_job_submission", detail={"job_name": job_name, "job_id": response['jobId']}, agent_id="parallel_aws")
        except ClientError as e:
            reporter.task_completed(success=False)
            ...
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="aws_job_error", detail={"job_name": job_name, "error": str(e)}, agent_id="parallel_aws")
            ...
            for job in desc['jobs']:
                ...
                if job['status'] == 'SUCCEEDED':
                    ...
                    try:
                        ...
                        if AUDIT_LOGGER_AVAILABLE:
                            dlt_logger = DLTLogger.from_environment()
                            await dlt_logger.add_entry(kind="parallel", name="aws_job_success", detail={"job_id": job_id}, agent_id="parallel_aws")
                    except (ClientError, json.JSONDecodeError) as e:
                        results.append({"status": "RESULT_ERROR", "error": str(e), "jobId": job_id})
                        reporter.task_completed(success=False)
                        parallel_logger.error(f"Failed to retrieve/parse result from S3 for job {job_id}: {e}")
                        alert_ops(f"Failed to retrieve/parse S3 result for job {job_id}: {e}", level="ERROR")
                        if AUDIT_LOGGER_AVAILABLE:
                            dlt_logger = DLTLogger.from_environment()
                            await dlt_logger.add_entry(kind="parallel", name="aws_job_error", detail={"job_id": job_id, "error": str(e)}, agent_id="parallel_aws")
                    ...
                    alert_ops(f"AWS Batch Job '{job_id}' failed. Reason: {reason}", level="WARNING")
                    if AUDIT_LOGGER_AVAILABLE:
                        dlt_logger = DLTLogger.from_environment()
                        await dlt_logger.add_entry(kind="parallel", name="aws_job_failure", detail={"job_id": job_id, "reason": reason}, agent_id="parallel_aws")
        except ClientError as e:
            parallel_logger.error(f"Error describing AWS Batch jobs: {e}. Assuming jobs failed.", exc_info=True)
            alert_ops(f"Error describing AWS Batch jobs: {e}. Assuming jobs failed.", level="ERROR")
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="aws_job_error", detail={"error": str(e)}, agent_id="parallel_aws")
    ...
    if submitted_jobs:
        for job_id, (_, cfg) in submitted_jobs.items():
            ...
            reporter.task_completed(success=False)
            parallel_logger.warning(f"AWS Batch Job '{job_id}' timed out after {job_timeout} seconds.")
            alert_ops(f"AWS Batch Job '{job_id}' timed out.", level="WARNING")
            if AUDIT_LOGGER_AVAILABLE:
                dlt_logger = DLTLogger.from_environment()
                await dlt_logger.add_entry(kind="parallel", name="aws_job_timeout", detail={"job_id": job_id}, agent_id="parallel_aws")
...
async def run_parallel_simulations(
    ...
    **kwargs
    ...
    if not available_backends:
        alert_ops("No parallel execution backends are available. Aborting startup.", level="CRITICAL")
        raise RuntimeError("No parallel execution backends are available. Please check dependencies and configuration.")
    
    if not executor or not _backend_availability.get(selected_backend, False):
        parallel_logger.error(f"Selected backend '{selected_backend}' is not available or unknown. Falling back to default: '{GLOBAL_PARALLEL_CONFIG.default_backend}'.")
        alert_ops(f"Selected backend '{selected_backend}' is unavailable. Falling back to '{GLOBAL_PARALLEL_CONFIG.default_backend}'.", level="WARNING")
        selected_backend = GLOBAL_PARALLEL_CONFIG.default_backend
        executor = _parallel_backends.get(selected_backend)
        if not executor or not _backend_availability.get(selected_backend, False):
            alert_ops(f"Default backend '{selected_backend}' is also unavailable. Aborting.", level="CRITICAL")
            raise RuntimeError(f"Neither selected backend '{parallel_backend}' nor default backend '{selected_backend}' is available.")

    if PROMETHEUS_AVAILABLE:
        PARALLEL_METRICS["backend_usage_total"].labels(backend_name=selected_backend).inc()

    parallel_logger.info(f"Executing {len(configurations)} tasks with backend: '{selected_backend}'.")
    start_time = time.time()
    
    results = await executor(simulation_function, configurations, config=GLOBAL_PARALLEL_CONFIG, **kwargs)
...
if __name__ == "__main__":
    ...
    async def example_simulation(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"status": "completed", "id": sim_id, "output": f"result_{sim_id}", "duration": duration}

    async def main():
        import argparse
        parser = argparse.ArgumentParser(description="Parallel Simulation Runner")
        parser.add_argument("--backend", type=str, help="Backend to use (e.g., local_asyncio, kubernetes, aws_batch). Overrides default config.")
        parser.add_argument("--tasks", type=int, default=10, help="Number of tasks to run")
        parser.add_argument("--fail-rate", type=float, default=0.1, help="Approximate fraction of tasks to fail")
