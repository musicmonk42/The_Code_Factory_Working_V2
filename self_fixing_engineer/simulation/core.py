...
def load_config(config_path: str) -> Dict[str, Any]:
    if not os.path.exists(config_path):
        logger.critical(f"Critical Error: Configuration file not found at {config_path}. Aborting startup.")
        sys.exit(1)
    try:
        ...
        logger.critical(f"Critical Error: Failed to load or validate configuration from {config_path}: {e}. Aborting startup.")
        raise
    except Exception as e:
        logger.critical(f"Critical Error: An unexpected error occurred while loading configuration from {config_path}: {e}. Aborting startup.")
        raise

def load_rbac_policy(rbac_path: str) -> Dict[str, Any]:
    if not os.path.exists(rbac_path):
        logger.critical(f"Critical Error: RBAC policy file not found at {rbac_path}. Aborting startup.")
        sys.exit(1)
    try:
        ...
        logger.critical(f"Critical Error: Failed to load or validate RBAC policy from {rbac_path}: {e}. Aborting startup.")
        raise
    except Exception as e:
        logger.critical(f"Critical Error: An unexpected error occurred while loading RBAC policy from {rbac_path}: {e}. Aborting startup.")
        raise
...
def check_permission(action: str, resource: str = "*") -> bool:
    ...
    if not user_roles:
        logger.warning(f"Permission Denied: User '{CURRENT_USER}' has no roles assigned. Access denied for action '{action}' on resource '{resource}'.")
        return False
    ...
    for role_name in user_roles:
        ...
        for perm in permissions:
            if _matches(perm['action'], action) and _matches(perm.get('resource', "*"), resource):
                logger.debug(f"Permission Granted: User '{CURRENT_USER}' with role '{role_name}' has '{action}' permission on '{resource}'.")
                return True
    logger.warning(f"Permission Denied: User '{CURRENT_USER}' lacks '{action}' permission on '{resource}'.")
    return False


class CircuitBreaker:
    def __init__(self, failure_threshold: int, recovery_timeout: int, channel_name: str, ops_channel_notifier: Callable):
    ...
    def _open(self):
        ...
        logger.error(f"Circuit Breaker OPEN for {self.channel_name}. Notifications to this channel are temporarily disabled.")
        self.ops_channel_notifier(f"CRITICAL: Circuit Breaker OPEN for {self.channel_name}. Notifications disabled. Failures: {self.failures}")

    def _half_open(self):
        self.is_open = False
        logger.warning(f"Circuit Breaker HALF-OPEN for {self.channel_name}. Attempting to send one notification.")

    def _close(self):
        self.is_open = False
        self.failures = 0
        self.last_failure_time = 0
        logger.info(f"Circuit Breaker CLOSED for {self.channel_name}. Notifications to this channel are re-enabled.")
        self.ops_channel_notifier(f"INFO: Circuit Breaker CLOSED for {self.channel_name}. Notifications re-enabled.")

    def _permanent_failure(self):
        self.permanent_failure = True
        logger.critical(f"PERMANENT FAILURE for {self.channel_name}. Notification channel is permanently disabled due to excessive failures.")
        self.ops_channel_notifier(f"CRITICAL: PERMANENT FAILURE for {self.channel_name}. This notification channel is permanently disabled. Manual intervention required.")


    def attempt_operation(self, func: Callable, *args, **kwargs):
        if self.permanent_failure:
            logger.warning(f"Skipping notification to {self.channel_name}: Channel is in permanent failure state.")
        ...
        if self.is_open:
            if time.time() - self.last_failure_time > self.recovery_timeout:
                ...
                self.failures += 1
                logger.warning(f"Skipping notification to {self.channel_name}: Circuit breaker is open.")
        ...
        try:
            ...
            self.failures += 1
            logger.error(f"Notification to {self.channel_name} failed (attempt {self.failures}): {e}")
...
class NotificationManager:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.slack_webhook_url = self.secrets_manager.get_secret("SLACK_WEBHOOK_URL", required=False) if self.secrets_manager else None
        self.email_smtp_password = self.secrets_manager.get_secret("EMAIL_SMTP_PASSWORD", required=False) if self.secrets_manager else None

        if self.slack_webhook_url:
            self.circuit_breakers['slack'] = CircuitBreaker(
                failure_threshold=5, recovery_timeout=300, channel_name='slack',
                ops_channel_notifier=self.ops_channel_notifier
            )
        if self.config.get('email_smtp_server') and self.config.get('email_sender') and self.config.get('email_recipients'):
            self.circuit_breakers['email'] = CircuitBreaker(
                failure_threshold=3, recovery_timeout=600, channel_name='email',
                ops_channel_notifier=self.ops_channel_notifier
            )
    ...
    def notify(self, channel: str, message: str, subject: Optional[str] = None):
        ...
        if not cb:
            logger.warning(f"Notification channel '{channel}' is not configured or enabled. Skipping.")
            return
...
class RedactingFormatter(logging.Formatter):
    def format(self, record):
        ...
        for keyword in REDACT_KEYWORDS:
            message = re.sub(rf"({keyword}[\"']?:\s*['\"]?)([^,\s\"']*)(['\"]?)", r"\1[REDACTED]\3", message, flags=re.IGNORECASE)
        return message

log_handler.setFormatter(RedactingFormatter('%(asctime)s - %(levelname)s - %(correlation_id)s - %(message)s'))


@correlated
def execute_remotely(job_config: Dict[str, Any], backend: str) -> Dict[str, Any]:
    logger.info(f"Attempting to execute job '{job_config.get('name')}' on remote backend '{backend}'.")
    if backend == 'kubernetes' and not KUBERNETES_AVAILABLE:
        if UNDER_PYTEST:
            logger.info(f"Kubernetes client not available; simulating submit in test mode for '{job_config.get('name')}'.")
            return {"status": "SUBMITTED", "job_id": "k8s-job-simulated", "message": "Simulated submission (pytest)."}
        logger.critical(f"Critical Error: Kubernetes backend requested but Kubernetes client not available. Aborting job '{job_config.get('name')}'.")
        NOTIFICATION_MANAGER.notify('ops_channel', f"CRITICAL: Kubernetes backend requested but client not available for job '{job_config.get('name')}'.")
        return {"status": "ERROR", "message": "Kubernetes client not available."}
    if backend == 'ray' and not RAY_AVAILABLE:
        if UNDER_PYTEST:
            logger.info(f"Ray not available; simulating submit in test mode for '{job_config.get('name')}'.")
            return {"status": "SUBMITTED", "job_id": "ray-job-simulated", "message": "Simulated submission (pytest)."}
        logger.critical(f"Critical Error: Ray backend requested but Ray not available. Aborting job '{job_config.get('name')}'.")
        NOTIFICATION_MANAGER.notify('ops_channel', f"CRITICAL: Ray backend requested but not available for job '{job_config.get('name')}'.")
    ...
    try:
        ...
        return {"status": "SUBMITTED", "job_id": f"{backend}-job-123", "message": "Job submitted successfully."}
    except Exception as e:
        logger.error(f"Failed to submit job '{job_config.get('name')}' to '{backend}': {e}")
        try:
            NOTIFICATION_MANAGER.notify('email', f"Failed to submit job '{job_config.get('name')}' to '{backend}': {e}", subject="Job Submission Failure")
        except Exception:
            pass
...
def run_job(job_config: Dict[str, Any]):
    ...
    if not check_permission(action="run:agent", resource=job_config.get('agent_type', '*')):
        logger.error(f"Permission denied to run agent type '{job_config.get('agent_type')}' for job '{job_name}'.")
        NOTIFICATION_MANAGER.notify('email', f"Permission denied to run agent type '{job_config.get('agent_type')}' for job '{job_name}'.", subject="Permission Denied")
        return {"status": "PERMISSION_DENIED", "message": "User lacks necessary permissions."}
    ...
    try:
        ...
        if job_config.get('notifications'):
            for channel in job_config['notifications']:
                NOTIFICATION_MANAGER.notify(channel, f"Job '{job_name}' completed successfully. Status: {result.get('status')}")
        ...
        if job_config.get('notifications'):
            for channel in job_config['notifications']:
                NOTIFICATION_MANAGER.notify(channel, f"Job '{job_name}' failed: {e}", subject=f"Job Failure: {job_name}")
        return {"status": "ERROR", "message": str(e)}
...
def watch_mode(files_to_watch: List[str], callback: Callable):
    if not WATCHDOG_AVAILABLE:
        logger.critical("Critical Error: Watchdog package not found. Watch mode cannot be enabled. "
                        "Please install 'watchdog' (pip install watchdog) for this feature.")
        NOTIFICATION_MANAGER.notify('ops_channel', "CRITICAL: Watchdog package not found. Watch mode disabled.")
...
async def main(args=None):
    ...
    parser.add_argument('--watch', action='store_true', help='Enable watch mode to monitor config changes.')
    ...
    parser.add_argument('--remote-backend', type=str, help='Specify a remote backend for execution (e.g., kubernetes, ray).')
    parser.add_argument('--agentic', action='store_true', help='Force agentic mode for all jobs (overrides config).')
    if args is None:
    ...
    if APP_CONFIG.get('watch_mode_enabled') or args.watch:
        ...
        if not files_to_watch:
            logger.warning("Watch mode enabled but no watch patterns specified in config. Defaulting to config.yaml and rbac_policy.yaml.")
        ...
        if not files_to_watch:
            logger.critical("Critical Error: Watch mode enabled but no files to watch found based on patterns. Aborting startup.")
            NOTIFICATION_MANAGER.notify('ops_channel', "CRITICAL: Watch mode enabled but no files found to watch. Aborting.")
            sys.exit(1)
        ...
        def run_callback():
            ...
            try:
                ...
                logger.error("Failed to reload configuration/RBAC. Previous configuration remains active or system will exit.")
                return

            current_jobs_to_run = [job for job in APP_CONFIG['jobs'] if job.get('enabled', True)]
            if args.job:
                current_jobs_to_run = [job for job in current_jobs_to_run if job['name'] == args.job]
            ...
            for job_idx, job_config in enumerate(current_jobs_to_run):
                logger.info(f"--- Running job {job_idx + 1}/{len(current_jobs_to_run)} (triggered by watch) ---")
                job_config["agentic"] = args.agentic if "agentic" in vars(args) else job_config.get("agentic", False)
                perm_action = f"run:{args.remote_backend}" if args.remote_backend else f"run:{job_config.get('agent_type', '*')}"
                perm_resource = job_config.get('agent_type', '*')
                if not check_permission(perm_action, perm_resource):
                    logger.error(f"Permission denied: Role needs '{perm_action}' permission to execute job '{job_config.get('name')}'.")
                    NOTIFICATION_MANAGER.notify('email', f"Permission denied for job '{job_config.get('name')}' (action: {perm_action}).", subject="Permission Denied on Watch Trigger")
                    continue
                if args.remote_backend: result = execute_remotely(job_config, args.remote_backend)
                else: result = run_job(job_config)
                if not args.summary or len(current_jobs_to_run) > 1:
                    print(f"\033[32mResults for Job {job_idx + 1} Status: {result.get('status')} ({result.get('job_id', 'local')})\033[0m\n")
                    if result.get("status") not in ["SUBMITTED", "ERROR"]: print("\033[35m" + json.dumps(summarize_result(result), indent=2) + "\033[0m")
                    else: print(f"Message: {result.get('message', 'No details.')}")
    ...
    for job_idx, job_config in enumerate(all_jobs_to_run):
        ...
        if not check_permission(perm_action, perm_resource):
            logger.error(f"Permission denied: Role needs '{perm_action}' permission to execute job '{job_config.get('name')}'.")
            NOTIFICATION_MANAGER.notify('email', f"Permission denied for job '{job_config.get('name')}' (action: {perm_action}).", subject="Permission Denied on Startup")
            continue
        if args.remote_backend: result = execute_remotely(job_config, args.remote_backend)
        else: result = run_job(job_config)
