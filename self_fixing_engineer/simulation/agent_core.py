...
class MetaLearningBase(ABC):
    ...
    @abstractmethod
    def learn(self, experiences: List[Dict[str, Any]], **kwargs) -> Optional[Dict[str, Any]]:
        """
        ...
        for exp_type, type_experiences in experience_types.items():
            if len(type_experiences) >= 3:  # Need at least 3 for a pattern
                patterns.append({
                    ...
                    "avg_reward": sum(e.get("reward", 0) for e in type_experiences) / len(type_experiences)
                })
        
        return patterns
    
    def _generate_insights(self, patterns: List[Dict[str, Any]]) -> List[LearningInsight]:
            ...
            for policy in sorted_policies:
                try:
                    if policy.condition(context):
                        ...
                        self.stats["policy_hits"][policy_name] = self.stats["policy_hits"].get(policy_name, 0) + 1
