...
AUDIT_LOG_INTEGRITY_FILE = os.getenv("AUDIT_LOG_INTEGRITY_FILE", str(Path(tempfile.gettempdir()) / "sandbox_audit_integrity.json"))
...
def _get_audit_hmac_key() -> bytes:
    ...
    if key_str:
        ...
        sandbox_logger.warning("AUDIT_HMAC_KEY_ENV not set. Generated a random key for audit log signing. THIS IS INSECURE FOR PRODUCTION.")
...
try:
    ...
    logging.getLogger(__name__).warning("Pydantic not available. Configuration and input validation will be skipped in sandbox.py.")
...
try:
    ...
    setattr(sys.modules.get(__name__, None) or sys.modules["simulation.sandbox"], "docker", _docker_stub)
...
try:
    ...
    setattr(sys.modules.get(__name__, None) or sys.modules["simulation.sandbox"], "podman", _podman_stub)
...
try:
    ...
    class _CoreV1Api:
        def create_namespaced_pod(self, namespace, body):
            return types.SimpleNamespace(metadata=types.SimpleNamespace(name=body.get("metadata", {}).get("name", "mock-pod")))
        ...
        def list_namespaced_pod(self, namespace, label_selector):
            return types.SimpleNamespace(items=[types.SimpleNamespace(metadata=types.SimpleNamespace(name="mock-pod"))])


    class _BatchV1Api:
        def create_namespaced_job(self, namespace, body):
            return types.SimpleNamespace(metadata=types.SimpleNamespace(name=body.get("metadata", {}).get("name", "mock-job")))
    ...
    class _NetworkingV1Api:
        def create_namespaced_network_policy(self, namespace, body):
            return types.SimpleNamespace(metadata=types.SimpleNamespace(name=body.get("metadata", {}).get("name", "mock-np")))
...
try:
    ...
    class _GremlinClient:  # minimal placeholder
        ...
        def run_experiment(self, *a, **k): return {"status": "simulated"}
...
try:
    ...
    class _Boto3Stub:
        def client(self, *_a, **_k): raise RuntimeError("boto3 not available")
    boto3 = _Boto3Stub()
...
try:
    ...
    logging.getLogger(__name__).warning("python-seccomp not installed; direct seccomp enforcement unavailable.")
...
try:
    ...
    logging.getLogger(__name__).warning("Firejail not found. Enhanced local process isolation unavailable.")
...
try:
    ...
    logging.getLogger(__name__).warning("Capsh not found. Capability dropping for local processes unavailable.")
...
if not os.path.exists(DEFAULT_SECCOMP_PROFILE_PATH):
    with open(DEFAULT_SECCOMP_PROFILE_PATH, "w") as f:
        json.dump({
            ...
            "syscalls": [
                {"names": ["exit", "exit_group", "read", "write", "close", "fstat", "lseek", "brk", "mmap", "munmap", "rt_sigaction", "rt_sigprocmask", "ioctl", "access", "openat", "newfstatat", "prlimit64", "set_tid_address", "set_robust_list", "rseq"], "action": "SCMP_ACT_ALLOW"}
            ]
        }, f, indent=2)
    logging.getLogger(__name__).info(f"Created default minimal seccomp profile at {DEFAULT_SECCOMP_PROFILE_PATH}")
...
if PYDANTIC_AVAILABLE:
    class SandboxPolicy(BaseModel):
        network_disabled: bool = Field(default=False, description="Disable network access for the sandbox")
        ...
        run_as_user: str = Field(default=DEFAULT_CONTAINER_USER, description="User ID and group ID in 'uid:gid' format")
        seccomp_profile: Optional[str] = Field(default=DEFAULT_SECCOMP_PROFILE_PATH, description="Path to seccomp profile")
        apparmor_profile: Optional[str] = Field(default=DEFAULT_APPARMOR_PROFILE_NAME, description="AppArmor profile name")
...
if PYDANTIC_AVAILABLE:
    class ContainerValidationConfig(BaseModel):
        ...
        kubernetes_pod_manifest: Optional[Dict[str, Any]] = Field(None, description="Kubernetes pod manifest.")
    ...
    class ContainerValidationConfig:
        def __init__(self, image: str, command: List[str], kubernetes_pod_manifest: Optional[Dict[str, Any]] = None):
            self.image = image
...
try:
    ...
    if not os.path.exists(AUDIT_LOG_INTEGRITY_FILE):
        with open(AUDIT_LOG_INTEGRITY_FILE, "w") as f:
            json.dump({"last_verified_entry_count": 0, "last_verification_time": datetime.utcnow().isoformat()}, f)
    ...
    with log_path.open("r", encoding="utf-8") as f:
        for idx, line in enumerate(f, 1):
            ...
            try:
                ...
                sandbox_logger.error("Audit log integrity compromised: Non-JSON line %s in %s", idx, log_path)
                ok = False
                continue
            sig = evt.get("signature")
            calc = _sign_event(evt["event"])
            if not sig or sig != calc:
                sandbox_logger.error("Audit log integrity compromised: Signature mismatch on line %s. Event: %s", idx, evt)
                ok = False
    return ok
...
def verify_audit_log_integrity() -> bool:
    ...
    candidate_logs.update(Path(p) for p in glob.glob(str(temp_root / "**" / "sandbox_audit.log"), recursive=True))
    ...
    for log in sorted(candidate_logs):
        ...
        if integ.exists():
            ...
            if last and last >= cutoff and integ.stat().st_mtime >= log.stat().st_mtime:
                sandbox_logger.info("Audit log integrity recently verified for %s. Skipping full check.", log)
                continue
        ...
        if ok:
            try:
                with integ.open("w", encoding="utf-8") as f:
                    json.dump({"last_verification_time": datetime.utcnow().isoformat()}, f, indent=2)
            except Exception:
                pass
...
def register_sandbox_backend(name: str, *, secure: bool = True):
    def decorator(executor_func: Callable):
        if not secure:
            ...
            alert_operator(f"Security Alert: Refused registration of insecure sandbox backend: {name}", level="CRITICAL")
...
async def check_external_services_async():
    ...
    if GCP_AVAILABLE:
        if not os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
            errors.append("GCP credentials (GOOGLE_APPLICATION_CREDENTIALS) not found in environment.")
    ...
    if errors:
        ...
        raise RuntimeError("One or more required external services/configurations are unavailable:\n" + "\n".join(errors))
...
async def _periodic_external_service_check(interval_seconds: int = 300):
    while True:
        try:
            ...
            sandbox_logger.error(f"Unexpected error during periodic external service check: {e}", exc_info=True)
            alert_operator(f"Unexpected error during periodic external service check: {e}", level="ERROR")
        await asyncio.sleep(interval_seconds)
...
if os.name == 'posix':
    try:
        ...
        libc = MockLibc()
        sandbox_logger.warning(f"Failed to load libc.so.6: {e}. Kernel-level sandboxing is disabled.")
else:
    class MockLibc:
        def prctl(self, *args, **kwargs):
...
def _apply_kernel_sandboxing_preexec(policy: SandboxPolicy, resource_limits: Optional[Dict[str, Any]] = None):
    def preexec():
        ...
        try:
            if os.geteuid() == 0:
                sandbox_logger.critical("SECURITY VIOLATION: Attempting to execute as root. Aborting child process.")
                alert_operator("SECURITY VIOLATION: Attempting to execute as root in local sandbox. Aborting.", level="CRITICAL")
                os._exit(1)
            if policy.run_as_user != DEFAULT_CONTAINER_USER:
                try:
                    ...
                    sandbox_logger.info(f"Dropped privileges to UID={os.getuid()}, GID={os.getgid()}.")
                except Exception as e:
                    sandbox_logger.critical(f"SECURITY VIOLATION: Failed to drop privileges to {policy.run_as_user}: {e}. Aborting child process.")
                    alert_operator(f"SECURITY VIOLATION: Failed to drop privileges to {policy.run_as_user} in local sandbox: {e}. Aborting.", level="CRITICAL")
                    os._exit(1)
                ...
                if SECCOMP_AVAILABLE:
                    try:
                        ...
                        if not os.path.isfile(filter_file):
                            sandbox_logger.critical(f"SECURITY VIOLATION: Seccomp profile file {filter_file} not found. Aborting child process.")
                            alert_operator(f"SECURITY VIOLATION: Seccomp profile {filter_file} not found. Aborting local sandbox.", level="CRITICAL")
                            os._exit(1)
                        ...
                        sandbox_logger.critical(f"SECURITY VIOLATION: Failed to apply seccomp via python-seccomp: {e}. Aborting child process.")
                        alert_operator(f"SECURITY VIOLATION: Failed to apply seccomp in local sandbox: {e}. Aborting.", level="CRITICAL")
                        os._exit(1)
                else:
                    sandbox_logger.critical("SECURITY VIOLATION: python-seccomp not installed; cannot enforce seccomp profile for local process. Aborting child process.")
                    alert_operator("SECURITY VIOLATION: python-seccomp not installed; cannot apply seccomp profile for local process. Aborting.", level="CRITICAL")
                    os._exit(1)
            try:
                drop_capabilities()
                sandbox_logger.info("Dropped all Linux capabilities.")
            except Exception as e:
                sandbox_logger.warning(f"Failed to drop capabilities: {e}. Proceeding with caution.")
        except Exception as e:
            sandbox_logger.critical(f"CRITICAL: Unexpected error during preexec_fn sandboxing setup: {e}. Aborting child process.", exc_info=True)
            alert_operator(f"CRITICAL: Unexpected error during local sandbox preexec_fn setup: {e}. Aborting.", level="CRITICAL")
            os._exit(1)
    return preexec
...
async def cleanup_sandbox(sandbox_id: str):
    ...
    if sandbox_info:
        ...
        try:
            if sandbox_type == 'docker' and DOCKER_AVAILABLE:
                ...
                if container_id:
                    try:
                        ...
                        sandbox_logger.error(f"Error cleaning up Docker container {container_id}: {e}")
                        alert_operator(f"Error cleaning up Docker container {container_id}: {e}", level="ERROR")
                ...
                if container_id:
                    try:
                        ...
                        sandbox_logger.error(f"Error cleaning up Podman container {container_id}: {e}")
                        alert_operator(f"Error cleaning up Podman container {container_id}: {e}", level="ERROR")
                ...
                if pod_name:
                    try:
                        await asyncio.to_thread(kube_config.load_kube_config)
                        v1 = client.CoreV1Api()
                        await asyncio.to_thread(v1.delete_namespaced_pod, name=pod_name, namespace=namespace, body=client.V1DeleteOptions(grace_period_seconds=10))
                        sandbox_logger.info(f"Kubernetes pod {pod_name} in namespace {namespace} deleted.")
                    except Exception as e:
                        sandbox_logger.error(f"Error cleaning up Kubernetes pod {pod_name}: {e}")
                        alert_operator(f"Error cleaning up Kubernetes pod {pod_name}: {e}", level="ERROR")
                if network_policy_name:
                    try:
                        networking = client.NetworkingV1Api()
                        await asyncio.to_thread(networking.delete_namespaced_network_policy, name=network_policy_name, namespace=namespace, body=client.V1DeleteOptions())
                        sandbox_logger.info(f"NetworkPolicy {network_policy_name} deleted in namespace {namespace}.")
                    except Exception as e:
                        sandbox_logger.error(f"Error deleting NetworkPolicy {network_policy_name}: {e}")
                        alert_operator(f"Error deleting NetworkPolicy {network_policy_name}: {e}", level="ERROR")
            ...
            sandbox_logger.error(f"Exception during cleanup_sandbox for {sandbox_id}: {e}", exc_info=True)
            alert_operator(f"Unexpected exception during sandbox cleanup for {sandbox_id}: {e}", level="CRITICAL")
    else:
...
def _validate_pod_manifest_internal(manifest: Dict[str, Any]) -> bool:
    ...
    for container in spec.get("containers", []):
        ...
        if security_context.get("privileged", False) or security_context.get("allowPrivilegeEscalation", False):
            return False
    return True
...
async def run_in_docker_sandbox(
    ...
    **kwargs
    ...
    if not DOCKER_AVAILABLE or not _backend_health_status.get("docker", False):
        result = {"status": "NOT_AVAILABLE", "reason": "Docker daemon not available or unhealthy."}
    ...
    try:
        if PYDANTIC_AVAILABLE:
            ...
            if policy_validated.privileged and not ALLOW_PRIVILEGED_CONTAINERS_GLOBAL: raise ValueError("Privileged containers forbidden by global policy.")
            if policy_validated.run_as_user.startswith("0:") or "root" in policy_validated.run_as_user.lower() or ":0" in policy_validated.run_as_user: raise ValueError("Running as root (UID 0) is forbidden by policy.")
    ...
    _active_sandboxes[container_name] = {"type": "docker", "start_time": time.time(), "container_id": None}
    monitor_task = asyncio.create_task(_monitor_sandbox_health(container_name))
    result_data = {}
    try:
        client = docker.from_env()
        volumes, container_workdir = _validate_and_bind_workdir(workdir, policy_validated.allow_write)
        container_kwargs = {
            "image": image, "command": command, "working_dir": container_workdir, "volumes": volumes,
            "detach": True, "remove": True, "name": container_name, "cap_drop": ["ALL"],
            "read_only": True, "user": policy_validated.run_as_user, "environment": minimal_env,
        }
        if resource_limits:
            if "cpu_shares" in resource_limits: container_kwargs["cpu_shares"] = resource_limits["cpu_shares"]
            if "memory" in resource_limits: container_kwargs["mem_limit"] = resource_limits["memory"]
            if "pids" in resource_limits: container_kwargs["pids_limit"] = resource_limits["pids"]
            if "max_duration_seconds" in resource_limits: _active_sandboxes[container_name]['max_run_time'] = resource_limits["max_duration_seconds"]
        ...
        result_data = {
            "status": "COMPLETED", "returncode": result["StatusCode"],
            "stdout": stdout.decode('utf-8', errors='replace'), "stderr": stderr.decode('utf-8', errors='replace'),
            "sandbox_id": container_name, "image": image
        }
    except Exception as e:
        result_data = {"status": "ERROR", "exception": str(e), "sandbox_id": container_name}
...
async def run_in_podman_sandbox(
    ...
    **kwargs
    ...
    if not PODMAN_AVAILABLE or not _backend_health_status.get("podman", False):
        result = {"status": "NOT_AVAILABLE", "reason": "Podman daemon not available or unhealthy."}
    ...
    container_name = f"sim_sandbox_{secrets.token_hex(4)}"
    _active_sandboxes[container_name] = {"type": "podman", "start_time": time.time(), "container_id": None}
    monitor_task = asyncio.create_task(_monitor_sandbox_health(container_name))
    result_data = {}
    try:
        client = podman.Client()
        ...
        security_opts = []
        if policy_validated.seccomp_profile: security_opts.append(f"seccomp={policy_validated.seccomp_profile}")
        if policy_validated.apparmor_profile: security_opts.append(f"apparmor={policy_validated.apparmor_profile}")
        if security_opts: container_kwargs["security_opt"] = security_opts
        if policy_validated.network_disabled:
            container_kwargs["network_mode"] = "none"
...
async def deploy_to_kubernetes(command: list[str], workdir: str, policy: Optional[SandboxPolicy] = None, kubernetes_pod_manifest: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    spec["template"].setdefault("metadata", {"name": f"{job_name}-pod", "labels": {"job-name": job_name}})
    ...
    containers = pod_spec.setdefault("containers", [{
        ...
        "image": "python:3.9-slim",
        "command": command,
        "securityContext": {"allowPrivilegeEscalation": False, "readOnlyRootFilesystem": True, "privileged": False}
    }])
    ...
    try:
        ...
        _ = net.create_namespaced_network_policy("default", {
            "metadata": {"name": f"{job_name}-np"},
            "spec": {"podSelector": {"matchLabels": {"job-name": job_name}}, "policyTypes": ["Ingress", "Egress"], "ingress": [], "egress": []}
        })
    except Exception as e:
        sandbox_logger.error(f"Failed to create NetworkPolicy for {job_name}: {e}")
        alert_operator(f"Network isolation for job {job_name} failed. Proceeding without isolation.", level="WARNING")
    ...
    try:
        ...
        v1 = client.CoreV1Api()
        pod_list = v1.list_namespaced_pod(namespace="default", label_selector=f"job-name={job_name}")
    ...
    # cleanup best-effort
    try:
        batch_v1.delete_namespaced_job(job_name, "default", body=client.V1DeleteOptions(propagation_policy="Background"))
    except Exception:
        pass
...
async def run_in_local_process_sandbox(command: list[str], workdir: str, policy: Optional[SandboxPolicy] = None) -> Dict[str, Any]:
...
async def run_in_sandbox(backend: str, command: List[str], workdir: str, image: Optional[str] = None, policy: Optional[Dict[str, Any]] = None, resource_limits: Optional[Dict[str, Any]] = None, kubernetes_pod_manifest: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    try:
        if PYDANTIC_AVAILABLE:
            validated_policy = SandboxPolicy(**policy) if policy else _default_policy()
            if validated_policy.privileged and not ALLOW_PRIVILEGED_CONTAINERS_GLOBAL: raise ValueError("Privileged containers forbidden by global policy.")
            if backend in ["docker", "podman"]:
                ...
                ContainerValidationConfig(image="python:3.9-slim", command=command, kubernetes_pod_manifest=kubernetes_pod_manifest)
        else:
            validated_policy = policy if policy else _default_policy()
            sandbox_logger.warning("Pydantic not available. Skipping detailed policy and input validation.")
            if validated_policy.privileged and not ALLOW_PRIVILEGED_CONTAINERS_GLOBAL: raise ValueError("Privileged containers forbidden by global policy.")
            if validated_policy.run_as_user.startswith("0:") or "root" in validated_policy.run_as_user.lower() or ":0" in validated_policy.run_as_user: raise ValueError("Running as root (UID 0) is forbidden by policy.")
            if backend in ["docker", "podman"]:
                if image is None:
                    image = "python:3.9-slim"  # Default to a whitelisted image
                _validate_container_config(image, command)
        ...
        sandbox_logger.critical(f"CRITICAL: Sandbox policy or input validation failed: {e}. Aborting operation.")
        alert_operator(f"CRITICAL: Sandbox policy or input validation failed: {e}. Aborting.", level="CRITICAL")
        log_audit({"event": "rejection", "backend": backend, "reason": f"Policy/Input validation failed: {e}"})
        return {"status": "REJECTED", "reason": f"Policy/Input validation failed: {e}"}

    executor = _sandbox_backends.get(backend)
    if not executor:
        sandbox_logger.error(f"Unsupported sandbox backend: {backend}. Available: {list(_sandbox_backends.keys())}")
        result = {"status": "ERROR", "reason": f"Unsupported backend: {backend}"}
        log_audit({"event": "error", "backend": backend, "reason": result["reason"]})
        alert_operator(f"Unsupported sandbox backend '{backend}'. Available: {list(_sandbox_backends.keys())}", level="CRITICAL")
        return result
    ...
    if PRODUCTION_MODE or not hasattr(executor, '_is_coroutine'):
        if not any([DOCKER_AVAILABLE, PODMAN_AVAILABLE, KUBERNETES_AVAILABLE, SECCOMP_AVAILABLE]):
            sandbox_logger.critical("NO SUPPORTED SANDBOX TECHNOLOGIES AVAILABLE. Refusing to execute any code.")
            result = {"status": "ERROR", "reason": "No sandbox available. Refusing execution (fail closed)."}
            log_audit({"event": "error", "backend": backend, "reason": result["reason"]})
            alert_operator("CRITICAL: No supported sandbox technologies available. Refusing execution.", level="CRITICAL")
            return result
        if not _backend_health_status.get(backend, False):
            sandbox_logger.critical(f"CRITICAL: Selected backend '{backend}' is unhealthy. Refusing to execute.")
            result = {"status": "ERROR", "reason": f"Selected backend '{backend}' is unhealthy. Refusing execution."}
            log_audit({"event": "error", "backend": backend, "reason": result["reason"]})
            alert_operator(f"CRITICAL: Selected sandbox backend '{backend}' is unhealthy. Refusing execution.", level="CRITICAL")
            return result
    ...
    log_audit({"event": "start", "backend": backend, "command": " ".join(command), "workdir": workdir, "image": image, "policy_applied": validated_policy.model_dump() if PYDANTIC_AVAILABLE else validated_policy.__dict__, "resource_limits": resource_limits, "kubernetes_pod_manifest_present": bool(kubernetes_pod_manifest), "run_by_user": getpass.getuser(), "run_as_uid": os.getuid() if hasattr(os, "getuid") else None, "pid": os.getpid()})
    result = await executor(command=command, workdir=workdir, image=image, policy=validated_policy, resource_limits=resource_limits, kubernetes_pod_manifest=kubernetes_pod_manifest)
    log_audit({"event": "end", "backend": backend, "result": result, "run_by_user": getpass.getuser(), "run_as_uid": os.getuid() if hasattr(os, "getuid") else None, "pid": os.getpid()})
    return result

async def _cleanup_all_active_sandboxes():
    sandbox_ids_to_clean = list(_active_sandboxes.keys())
    if sandbox_ids_to_clean:
        sandbox_logger.info(f"Initiating graceful cleanup of {len(sandbox_ids_to_clean)} active sandboxes on exit.")
        await asyncio.gather(*[cleanup_sandbox(s_id) for s_id in sandbox_ids_to_clean], return_exceptions=True)
    else:
...
def _run_async_cleanup_on_exit():
    try:
        ...
        if loop.is_running(): loop.create_task(_cleanup_all_active_sandboxes())
        else: loop.run_until_complete(_cleanup_all_active_sandboxes())
    except Exception as e:
...
async def _initial_external_service_check():
    try:
        ...
        sandbox_logger.critical(f"CRITICAL: Initial external service check failed: {e}. Aborting startup.")
...
async def check_rate_limit() -> bool:
    ...
    if _sandbox_execution_count > _sandbox_rate_limit:
        sandbox_logger.warning(f"Rate limit exceeded: {_sandbox_execution_count} executions in the last minute")
        return False
    return True
...
async def initialize_sandbox_system():
    ...
    if not verify_audit_log_integrity():
        ...
        alert_operator("CRITICAL: Audit log integrity check failed during sandbox initialization.", level="CRITICAL")
        return False
    ...
    try:
        ...
        sandbox_logger.critical(f"CRITICAL: Failed to initialize sandbox system: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--backend", choices=list(_sandbox_backends.keys()), default="docker", help="Sandbox backend to use")
    ...
    parser.add_argument("--disable-network", action="store_true", dest="disable_network", help="Disable network access")
    parser.add_argument("--read-only", action="store_true", dest="read_only", help="Make filesystem read-only")
    
    args = parser.parse_args()
    
    policy = SandboxPolicy(
        network_disabled=args.disable_network,
    ...
    result = loop.run_until_complete(run_in_sandbox(
        backend=args.backend,
        command=args.command,
        workdir=args.workdir,
        image=args.image,
        policy=policy.__dict__ if PYDANTIC_AVAILABLE else policy # Pass as dict if Pydantic is used
    ))
