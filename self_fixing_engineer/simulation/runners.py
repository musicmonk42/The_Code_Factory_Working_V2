...
try:
    ...
    logging.getLogger(__name__).warning("Pydantic not available. Configuration and input validation will be skipped in runners.py.")
...
try:
        ...
        with _metrics_lock:
            try:
                ...
                if isinstance(existing_metric, metric_type):
                    ...
                    runners_logger.warning(f"Metric '{name}' already registered with a different type. Reusing existing.")
                    return existing_metric
            except KeyError:
    ...
    RUNNER_METRICS = {
        "runner_execution_total": get_or_create_metric(Counter, "runner_execution_total", "Total runner executions", ["runner_type", "status"]),
        "runner_errors_total": get_or_create_metric(Counter, "runner_errors_total", "Total runner errors", ["runner_type", "error_type"]),
        "runner_duration_seconds": get_or_create_metric(Histogram, "runner_duration_seconds", "Runner execution duration in seconds", ["runner_type"]),
        "runner_dependency_missing": get_or_create_metric(Counter, "runner_dependency_missing_total", "Total times a runner dependency was missing", ["runner_type", "dependency"]),
        "runner_health": get_or_create_metric(Gauge, "runner_health", "Health status of runners (1=healthy, 0=unhealthy)", ["runner_type"]),
        "subprocess_latency_seconds": get_or_create_metric(Histogram, "runner_subprocess_latency_seconds", "Subprocess execution latency", ["runner_type"])
    }
except ImportError:
    PROMETHEUS_AVAILABLE = False
    runners_logger = logging.getLogger("simulation.runners")
    runners_logger.warning("Prometheus client not available. Metrics will not be collected in runners.py.")
    RUNNER_METRICS = {}
...
try:
    ...
    DLTLogger = None
    runners_logger.warning("DLT/Boto3 not available. Audit logging and secure credential handling will be disabled.")
...
def check_runner_dependencies(runner_name: str) -> bool:
    ...
    if missing_deps:
        runners_logger.error(f"Missing dependencies for runner '{runner_name}': {', '.join(missing_deps)}. This runner will not function.")
        if PROMETHEUS_AVAILABLE:
            for dep in missing_deps:
                RUNNER_METRICS["runner_dependency_missing"].labels(runner_type=runner_name, dependency=dep).inc()
...
if PYDANTIC_AVAILABLE:
    class PythonRunnerConfig(BaseModel):
        script_path: str = Field(..., min_length=1, description="Path to the Python script to execute.")
        ...
        timeout_seconds: int = Field(300, ge=1, description="Timeout for script execution in seconds.")
    ...
    class ContainerRunnerConfig(BaseModel):
        ...
        args: List[str] = Field(default_factory=list, description="Arguments for the container command.")
        env_vars: Dict[str, str] = Field(default_factory=dict, description="Environment variables for the container.")
        timeout_seconds: int = Field(600, ge=1, description="Timeout for container execution in seconds.")
        resource_limits: Dict[str, Any] = Field(default_factory=dict, description="CPU/memory limits for the container.")
        allow_network: bool = False
    ...
    class AgentConfig(BaseModel):
        runner_type: str = Field(..., description="The type of runner to use (e.g., 'python_script', 'container').")
        runner_config: Dict[str, Any] = Field(default_factory=dict, description="Configuration specific to the chosen runner type.")
        user_id: str = Field("system", description="ID of the user initiating the run.")
        job_id: str = Field(default_factory=lambda: f"job-{os.urandom(4).hex()}", description="Unique ID for the job.")
...
def _execute_subprocess_safely(
    ...
    if env:
        redacted_env = {k: "[REDACTED]" if any(s in k.lower() for s in ['key', 'secret', 'token', 'password']) else v for k, v in env.items()}
    ...
    try:
        if PROMETHEUS_AVAILABLE:
            with time_metric(RUNNER_METRICS["subprocess_latency_seconds"], {"runner_type": runner_type}):
        ...
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="timeout").inc()
        return {
            "status": "TIMEOUT",
            "stdout": e.stdout,
            "stderr": e.stderr,
        ...
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="subprocess_error").inc()
        raise e
        ...
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="command_not_found").inc()
        return {
            "status": "FAILED",
        ...
        runners_logger.error(
            ...
            f"error={e}. Duration={duration:.2f}s.", exc_info=True
        )
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="unexpected_error").inc()
        return {
            "status": "ERROR",
...
def _perform_integrity_check(file_path: str, expected_mime_type: Optional[str] = None) -> bool:
    ...
    if expected_mime_type and '.' in file_path:
        ...
        if 'python' in expected_mime_type.lower() and ext not in ['py', 'pyc', 'pyo']:
            runners_logger.warning(f"File extension mismatch for {file_path}. Expected Python, got .{ext}.")
    runners_logger.info(f"Performing conceptual integrity check for {file_path}. (Actual checks not implemented)")
    return True
...
def run_python_script(config: PythonRunnerConfig, job_id: str, user_id: str) -> Dict[str, Any]:
    if not check_runner_dependencies("python_script"):
        return {"status": "DEPENDENCY_MISSING", "error": "Required Python environment for script execution is not available."}

    if not _perform_integrity_check(config.script_path, expected_mime_type="text/x-python"):
        runners_logger.error(f"Integrity check failed for Python script: {config.script_path}. Aborting.")
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type="python_script", error_type="integrity_check_failed").inc()
        return {"status": "SECURITY_ERROR", "error": "Integrity check failed for script."}
    ...
    try:
        ...
        result = {"status": "FAILED", "return_code": e.returncode, "error": f"Command failed with exit code {e.returncode}."}
    
    duration = time.time() - start_time
    if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_duration_seconds"].labels(runner_type="python_script").observe(duration)
    if result["status"] == "SUCCESS":
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_execution_total"].labels(runner_type="python_script", status="success").inc()
    else:
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_execution_total"].labels(runner_type="python_script", status="failed").inc()
        alert_operator(f"Python script runner failed for job {job_id}: {result.get('error', 'Unknown error')}", level="ERROR")
    return result
...
def run_container(config: ContainerRunnerConfig, job_id: str, user_id: str) -> Dict[str, Any]:
    if not check_runner_dependencies("container"):
        return {"status": "DEPENDENCY_MISSING", "error": "Required container runtime environment is not available."}
    ...
    try:
        ...
        return {"status": "AUTH_ERROR", "error": "Failed to load container credentials."}
    ...
    duration = time.time() - start_time
    if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_duration_seconds"].labels(runner_type="container").observe(duration)
    if result["status"] == "SUCCESS":
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_execution_total"].labels(runner_type="container", status="success").inc()
    else:
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_execution_total"].labels(runner_type="container", status="failed").inc()
        alert_operator(f"Container runner failed for job {job_id}: {result.get('error', 'Unknown error')}", level="ERROR")
    return result
...
def run_agent(agent_config: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type="agent_config", error_type="validation_error").inc()
        alert_operator(f"Agent configuration validation failed: {e}", level="ERROR")
        return {"status": "ERROR", "message": f"Agent configuration validation failed: {e}"}
    ...
    if not runner_func:
        runners_logger.error(f"Unknown runner type: {runner_type}. Available runners: {list(_runners.keys())}")
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="unknown_runner_type").inc()
        alert_operator(f"Unknown runner type '{runner_type}' requested for job {job_id}.", level="CRITICAL")
        return {"status": "ERROR", "message": f"Unknown runner type: {runner_type}"}

    if not check_runner_dependencies(runner_type):
        runners_logger.error(f"Runner '{runner_type}' cannot be executed due to missing dependencies.")
        alert_operator(f"Runner '{runner_type}' missing dependencies for job {job_id}.", level="CRITICAL")
        return {"status": "ERROR", "message": f"Runner '{runner_type}' missing dependencies."}

    runners_logger.info(f"Executing agent with runner '{runner_type}' for job '{job_id}' by user '{user_id}'.")
    ...
    async def _execute_with_audit():
        if dlt_logger:
            await dlt_logger.add_entry(
                ...
                detail={"runner_type": runner_type, "job_id": job_id, "user_id": user_id},
                agent_id="runner",
        ...
        if dlt_logger and result:
            await dlt_logger.add_entry(
                ...
                detail={"runner_type": runner_type, "job_id": job_id, "status": result.get("status")},
                agent_id="runner",
                correlation_id=f"runner-{job_id}"
            )
        return result
    ...
    try:
        ...
        runners_logger.error(f"Unhandled exception during execution of runner '{runner_type}' for job '{job_id}': {e}", exc_info=True)
        if PROMETHEUS_AVAILABLE: RUNNER_METRICS["runner_errors_total"].labels(runner_type=runner_type, error_type="unhandled_exception").inc()
        alert_operator(f"Unhandled exception in runner '{runner_type}' for job {job_id}: {e}", level="CRITICAL")
        
        if DLT_LOGGER_AVAILABLE:
            try:
                dlt_logger = DLTLogger.from_environment()
                asyncio.run(dlt_logger.add_entry(
                    kind="runner",
                    name="agent_error",
                    detail={"runner_type": runner_type, "job_id": job_id, "error": str(e)},
                    agent_id="runner",
                    correlation_id=f"runner-{job_id}"
                ))
...
""")
    ...
    try:
        ...
        runners_logger.warning(f"Skipping container example: Docker not running or command not found. Error: {e}")
