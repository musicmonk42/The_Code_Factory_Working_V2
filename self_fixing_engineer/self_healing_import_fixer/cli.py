...
def setup_logging(verbose: bool, log_format: str) -> None:
    ...
    formatter = JsonFormatter() if final_format == 'json' else logging.Formatter("%(asctime)s - %(levelname)s: %(message)s")
    handler.setFormatter(formatter)
    if logger.hasHandlers():
        logger.handlers.clear()
    logger.addHandler(handler)
    if is_ci:
        logger.info("CI environment detected. Adjusting logging defaults.", extra={'cli_mode': 'ci'})
    if PRODUCTION_MODE:
...
def _get_plugin_signature_key() -> bytes:
    ...
    if _plugin_signature_key is None:
        key_str = SECRETS_MANAGER.get_secret(PLUGIN_SIGNATURE_KEY_ENV, required=True if PRODUCTION_MODE else False)
        if not key_str:
            if PRODUCTION_MODE:
                raise AnalyzerCriticalError("PLUGIN_SIGNATURE_KEY not set in PRODUCTION_MODE. Plugin verification is mandatory. Aborting startup.")
            else:
                logger.warning("PLUGIN_SIGNATURE_KEY not set. Plugin verification will be skipped (INSECURE).")
...
def _verify_plugin_integrity(file_path: str, expected_signature: str) -> bool:
    ...
    try:
        ...
        if hmac.compare_digest(calculated_signature, expected_signature):
            ...
            logger.error(f"Plugin integrity MISMATCH for {file_path}. Calculated: {calculated_signature}, Expected: {expected_signature}. Possible tampering detected!")
            cli_audit_logger.log_event("plugin_integrity_mismatch", file=file_path, calculated=calculated_signature, expected=expected_signature)
            alert_operator(f"CRITICAL: Plugin integrity mismatch for {file_path}. Possible tampering! Aborting.", level="CRITICAL")
            return False
    except Exception as e:
        logger.error(f"Error during plugin integrity verification for {file_path}: {e}", exc_info=True)
        cli_audit_logger.log_event("plugin_integrity_error", file=file_path, error=str(e))
        alert_operator(f"CRITICAL: Plugin integrity verification failed for {file_path}: {e}. Aborting.", level="CRITICAL")
        return False

class PluginManager:
    """Discovers, loads, and executes plugins and hooks."""
    def __init__(self, plugin_dirs: Optional[List[str]] = None, approved_plugins: Optional[Dict[str, str]] = None):
        self.plugins: Dict[str, Any] = {}
        self.plugin_dirs = plugin_dirs if plugin_dirs is not None else [os.path.join(Path(__file__).parent, 'plugins')]
        ...
        if PRODUCTION_MODE and not self.whitelisted_plugin_dirs:
            logger.critical("CRITICAL: In PRODUCTION_MODE, 'whitelisted_plugin_dirs' must be configured for PluginManager. Aborting startup.")
            alert_operator("CRITICAL: PluginManager: No whitelisted plugin directories configured in PRODUCTION_MODE. Aborting.", level="CRITICAL")
            sys.exit(1)
    ...
    async def _load_plugin_file_async(self, full_plugin_path: str, parser: argparse.ArgumentParser):
        ...
        if module_name in self.plugins:
            return
        cli_audit_logger.log_event("plugin_load_attempt", module_name=module_name, path=scrub_secrets(full_plugin_path))
        if PRODUCTION_MODE:
            expected_signature = self.approved_plugins.get(module_name)
            if not expected_signature:
                logger.critical(f"CRITICAL: Plugin '{module_name}' from '{full_plugin_path}' is not in approved_plugins list or has no expected signature. Aborting startup.")
                cli_audit_logger.log_event("plugin_load_forbidden", file=full_plugin_path, reason="not_approved_or_unsigned")
                alert_operator(f"CRITICAL: Unapproved/unsigned plugin '{module_name}' found. Aborting.", level="CRITICAL")
                sys.exit(1)
            if not await asyncio.to_thread(_verify_plugin_integrity, full_plugin_path, expected_signature):
                logger.critical(f"CRITICAL: Plugin '{module_name}' integrity verification failed. Aborting startup.")
                sys.exit(1)
        try:
            ...
            if spec and spec.loader:
                ...
                if hasattr(module, 'register_command'):
                    try:
                        ...
                        cli_audit_logger.log_event("plugin_loaded", module_name=module_name, path=full_plugin_path, status="success")
                    except Exception as e:
                        logger.critical(f"CRITICAL: Plugin '{module_name}' failed to register command: {e}. Aborting startup.", exc_info=True)
                        cli_audit_logger.log_event("plugin_load_failure", module_name=module_name, path=full_plugin_path, reason="command_registration_failed", error=str(e))
                        alert_operator(f"CRITICAL: Plugin '{module_name}' failed to register command. Aborting.", level="CRITICAL")
                        sys.exit(1)
        except Exception as e:
            logger.critical(f"CRITICAL: Failed to load plugin '{module_name}' from '{full_plugin_path}': {e}. Aborting startup.", exc_info=True)
            cli_audit_logger.log_event("plugin_load_failure", module_name=module_name, path=full_plugin_path, reason="import_failed", error=str(e))
            alert_operator(f"CRITICAL: Failed to load plugin '{module_name}'. Aborting.", level="CRITICAL")
            sys.exit(1)
    ...
    async def discover_and_load(self, parser: argparse.ArgumentParser):
        ...
        for plugin_dir in self.plugin_dirs:
            ...
            if PRODUCTION_MODE and not any(abs_plugin_dir.startswith(d) for d in self.whitelisted_plugin_dirs):
                logger.critical(f"CRITICAL: Attempted to load plugins from non-whitelisted directory: {abs_plugin_dir}. Aborting startup.")
                cli_audit_logger.log_event("plugin_load_forbidden", dir=abs_plugin_dir, reason="not_whitelisted", whitelisted_dirs=self.whitelisted_plugin_dirs)
                alert_operator(f"CRITICAL: Plugin load from non-whitelisted dir '{abs_plugin_dir}' forbidden in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                sys.exit(1)
            if not os.path.isdir(abs_plugin_dir):
                logger.warning(f"Plugin directory '{abs_plugin_dir}' does not exist. Skipping.")
                cli_audit_logger.log_event("plugin_load_skipped", dir=abs_plugin_dir, reason="directory_not_found")
                continue
        ...
        tasks = [asyncio.wait_for(self._load_plugin_file_async(p, parser), timeout=10) for p in plugin_files_to_load]
        await asyncio.gather(*tasks, return_exceptions=True)
        cli_audit_logger.log_event("plugin_discovery_complete", loaded_plugins=list(self.plugins.keys()), plugin_versions=self.plugin_versions)
        ...
        for name, plugin in list(self.plugins.items()):
            if hasattr(plugin, hook_name):
                try:
                    ...
                    logger.critical(f"CRITICAL: Error in hook '{hook_name}' for plugin '{name}': {e}. Unloading plugin and aborting.", exc_info=True)
                    cli_audit_logger.log_event("hook_run_failure", hook_name=hook_name, plugin=name, error=str(e))
                    del self.plugins[name]
                    alert_operator(f"CRITICAL: Plugin hook '{hook_name}' failed for '{name}' and was unloaded. Aborting.", level="CRITICAL")
                    sys.exit(1)

    def list_plugins(self, log_format="text"):
        plugins_list = [
            {"name": name, "doc": doc or "(no docstring)", "examples": self.examples.get(name, []), "version": self.plugin_versions.get(name, "unknown")}
            for name, doc in self.docs.items()
        ]
        if is_ci_environment() or log_format == "json":
            print(json.dumps(plugins_list, indent=2))
            cli_audit_logger.log_event("list_plugins_command", format=log_format, count=len(plugins_list))
            return
        try:
            from rich.console import Console
            from rich.table import Table
        except ImportError:
            logger.error("The 'rich' library is required for text-based plugin listings. Please install it.")
            return
        console = Console()
        ...
        for p in plugins_list:
            examples = "\n".join(p.get("examples", []))
            table.add_row(p['name'], p.get('version', "unknown"), p['doc'], examples or "(no examples)")
        console.print(table)
...
def load_config(config_path: str) -> Dict[str, Any]:
    ...
    if not os.path.exists(config_file_abs_path):
        logger.critical(f"CRITICAL: Configuration file not found at: {config_file_abs_path}. Aborting startup.")
        cli_audit_logger.log_event("config_load_failure", path=config_file_abs_path, reason="file_not_found")
        alert_operator(f"CRITICAL: Config file not found: {config_file_abs_path}. Aborting.", level="CRITICAL")
        sys.exit(1)
    try:
        with open(config_file_abs_path, 'r', encoding='utf-8') as f:
            if config_file_abs_path.endswith(('.json', '.JSON')):
                ...
                logger.critical(f"CRITICAL: Unsupported configuration file format: {config_file_abs_path}. Must be .json or .yaml. Aborting startup.")
                cli_audit_logger.log_event("config_load_failure", path=config_file_abs_path, reason="unsupported_format")
                alert_operator(f"CRITICAL: Config file has unsupported format: {config_file_abs_path}. Aborting.", level="CRITICAL")
                sys.exit(1)
        ...
        cli_audit_logger.log_event("config_loaded", path=config_file_abs_path, config_keys=list(config_data.keys()))
        return config_data
    except Exception as e:
        logger.critical(f"CRITICAL: Failed to load config {config_file_abs_path}: {e}. Aborting startup.", exc_info=True)
        cli_audit_logger.log_event("config_load_failure", path=config_file_abs_path, reason="parsing_error", error=str(e))
        alert_operator(f"CRITICAL: Failed to load config {config_file_abs_path}: {e}. Aborting.", level="CRITICAL")
        sys.exit(1)
...
def _validate_path_argument(path: str, arg_name: str, is_dir: bool = True, allow_symlink: bool = False, allowlist: Optional[List[str]] = None) -> str:
    abs_path = os.path.abspath(path)
    if ".." in path.split(os.sep):
        logger.critical(f"CRITICAL: Path traversal attempt detected in '{arg_name}' argument: {path}. Aborting.")
        cli_audit_logger.log_event("security_violation", type="path_traversal", arg_name=arg_name, path=path)
        alert_operator(f"CRITICAL: Path traversal detected in CLI arg '{arg_name}'. Aborting.", level="CRITICAL")
        sys.exit(1)
    if not allow_symlink and os.path.islink(abs_path):
        logger.critical(f"CRITICAL: Symlink detected for '{arg_name}' argument: {path}. Symlinks are forbidden. Aborting.")
        cli_audit_logger.log_event("security_violation", type="symlink_forbidden", arg_name=arg_name, path=path)
        alert_operator(f"CRITICAL: Symlink detected for CLI arg '{arg_name}'. Aborting.", level="CRITICAL")
        sys.exit(1)
    if is_dir and not os.path.isdir(abs_path):
        logger.critical(f"CRITICAL: Directory specified for '{arg_name}' does not exist or is not a directory: {path}. Aborting.")
        cli_audit_logger.log_event("path_validation_failure", type="dir_not_found", arg_name=arg_name, path=path)
        alert_operator(f"CRITICAL: Directory for CLI arg '{arg_name}' not found. Aborting.", level="CRITICAL")
        sys.exit(1)
    elif not is_dir and not os.path.exists(abs_path):
        logger.critical(f"CRITICAL: File specified for '{arg_name}' does not exist: {path}. Aborting.")
        cli_audit_logger.log_event("path_validation_failure", type="file_not_found", arg_name=arg_name, path=path)
        alert_operator(f"CRITICAL: File for CLI arg '{arg_name}' not found. Aborting.", level="CRITICAL")
        sys.exit(1)
    ...
    if not any(abs_path.startswith(os.path.abspath(scope)) for scope in project_scope):
        logger.critical(f"CRITICAL: Path '{path}' for '{arg_name}' is outside project scope/allowlist. Aborting.")
        cli_audit_logger.log_event("security_violation", type="path_outside_scope", arg_name=arg_name, path=path, allowlist=project_scope)
        alert_operator(f"CRITICAL: Path '{path}' for CLI arg '{arg_name}' is outside project scope. Aborting.", level="CRITICAL")
        sys.exit(1)
...
def create_parser(plugin_manager: PluginManager) -> CustomArgumentParser:
    ...
    parser.add_argument("--log-format", choices=['text', 'json'], default='text', help="Set log output format.")
    parser.add_argument("--ci", action="store_true", help="Force CI mode for structured output and defaults.")
    parser.add_argument("--list-plugins", action="store_true", help="List all installed plugins and their docs/examples.")
    parser.add_argument("--list-hooks", action="store_true", help="List all registered hooks.")
    parser.add_argument("--undo", type=int, help="Undo last [n] change sets (requires audit log or git).")
    parser.add_argument("--plugins-dir", help="Directory to auto-load plugins from. Can be specified multiple times.", action="append")
    subparsers = parser.add_subparsers(dest="command", title="commands", help="Available commands", required=True)
    analyze_parser = subparsers.add_parser("analyze", help="Perform deep analysis of import and call graphs.")
    analyze_parser.add_argument("root", help="The root directory of the Python project to analyze.")
    analyze_parser.add_argument("--output-format", choices=['text', 'json-ide', 'markdown', 'html'], default='text', help="Format for analysis output.")
    analyze_parser.add_argument("--no-suggestions", action="store_true", help="Omit cycle fix suggestions.")
    analyze_parser.add_argument("--ai-suggestions", action="store_true", help="Enable AI-powered refactoring suggestions.")
    analyze_parser.add_argument("--graph-type", choices=["import", "call"], default="import", help="Type of dependency graph to analyze.")
    analyze_parser.add_argument("--include-function-imports", action="store_true", help="Include imports inside functions.")

    heal_parser = subparsers.add_parser("heal", help="Automatically fix import statements and project structure.")
    heal_parser.add_argument("roots", nargs='+', help="One or more project root directories to heal.")
    heal_parser.add_argument("--dry-run", action="store_true", help="Show proposed changes without applying them.")
    heal_parser.add_argument("--fix-cycles", action="store_true", help="Attempt to automatically resolve circular import dependencies.")
    heal_parser.add_argument("--fix-deps", action="store_true", help="Find and add missing dependencies to pyproject.toml and requirements.txt.")
    heal_parser.add_argument("--interactive", "-i", action="store_true", help="Prompt for confirmation before applying each change.")
    heal_parser.add_argument("--run-tests", action="store_true", help="Run a test suite after applying fixes to validate them.")
    heal_parser.add_argument("--use-git-branch", action="store_true", help="Create a git branch for changes instead of .bak files.")
    heal_parser.add_argument("--pre-hook", help="A shell command to run before the healing process.")
    heal_parser.add_argument("--post-hook", help="A shell command to run after the healing process.")
    heal_parser.add_argument("--yes", "-y", action="store_true", help="Auto-approve all changes (non-interactive).")
    heal_parser.add_argument("--backup", action="store_true", help="Create .bak files for backups (even with git).")
    heal_parser.add_argument("--create-pr", action="store_true", help="Create a pull request after changes (requires git and gh).")
    heal_parser.add_argument("--python-version", default=f"{sys.version_info.major}.{sys.version_info.minor}", help="Python version for stdlib detection (e.g., '3.9').")

    visualize_parser = subparsers.add_parser("visualize", help="Export the dependency graph to various formats.")
    visualize_parser.add_argument("root", help="The root directory of the Python project.")
    visualize_parser.add_argument("format", choices=["dot", "json", "mermaid", "graphml"], help="The desired output format.")
    visualize_parser.add_argument("output_file", help="The path to write the output file to.")
    visualize_parser.add_argument("--graph-type", choices=["import", "call"], default="import", help="Type of graph to export.")

    serve_parser = subparsers.add_parser("serve", help="Serve an interactive web dashboard for analysis.")
    ...
    serve_parser.add_argument("--port", type=int, default=5000, help="Port for the dashboard server.")
    serve_parser.add_argument("--require-auth", action="store_true", help="Require authentication for dashboard access.")
    ...
    trigger_parser = subparsers.add_parser("trigger", help="Send a webhook-style trigger to a running dashboard.")
...
async def main_async():
    ...
    cli_audit_logger.log_event("cli_start", cli_args=scrub_secrets(vars(args)), production_mode=PRODUCTION_MODE)
    ...
    if PRODUCTION_MODE and os.getenv("PRODUCTION_MODE", "false").lower() != "true":
        logger.critical("CRITICAL: PRODUCTION_MODE is ON but not set via environment variable. Aborting for consistency.")
    ...
    try:
        ...
        if getattr(args, 'list_plugins', False):
            ...
            cli_audit_logger.log_event("command_executed", command="list_plugins", args=scrub_secrets(vars(args)))
            sys.exit(0)
        if getattr(args, 'list_hooks', False):
            ...
            cli_audit_logger.log_event("command_executed", command="list_hooks", args=scrub_secrets(vars(args)))
            sys.exit(0)
        if args.command == "analyze":
            ...
            if args.output_format == 'json-ide':
                report = {"cycles": analyzer.detect_cycles(), "dead_nodes": analyzer.detect_dead_nodes()}
            ...
            cli_audit_logger.log_event("command_executed", command="analyze", args=scrub_secrets(vars(args)))
            ...
            if PRODUCTION_MODE and not args.dry_run and not args.interactive:
                logger.critical("CRITICAL: In PRODUCTION_MODE, 'heal' command requires '--interactive' flag for explicit operator approval when not in dry-run. Aborting.")
                cli_audit_logger.log_event("command_forbidden", command="heal", reason="no_interactive_in_prod")
                alert_operator("CRITICAL: 'heal' command requires '--interactive' in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                sys.exit(1)
            if PRODUCTION_MODE and args.yes:
                logger.critical("CRITICAL: '--yes' flag is forbidden in PRODUCTION_MODE for 'heal' command. Aborting.")
                cli_audit_logger.log_event("command_forbidden", command="heal", reason="yes_forbidden_in_prod")
                alert_operator("CRITICAL: '--yes' flag forbidden in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                sys.exit(1)
            load_fixer()
            ...
            cli_audit_logger.log_event("command_executed", command="visualize", args=scrub_secrets(vars(args)))
            ...
            if PRODUCTION_MODE:
                if not args.require_auth:
                    logger.critical("CRITICAL: In PRODUCTION_MODE, 'serve' command requires '--require-auth'. Aborting.")
                    cli_audit_logger.log_event("command_forbidden", command="serve", reason="auth_not_required_in_prod")
                    alert_operator("CRITICAL: Dashboard requires authentication in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                    sys.exit(1)
                if not args.https_cert or not args.https_key:
                    logger.critical("CRITICAL: In PRODUCTION_MODE, 'serve' command requires '--https-cert' and '--https-key' for HTTPS. Aborting.")
                    cli_audit_logger.log_event("command_forbidden", command="serve", reason="https_not_enforced_in_prod")
                    alert_operator("CRITICAL: Dashboard requires HTTPS in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                    sys.exit(1)
                if args.host == "0.0.0.0" or args.host == "::":
                    logger.critical("CRITICAL: In PRODUCTION_MODE, 'serve' command cannot bind to public interfaces (0.0.0.0 or ::). Aborting.")
                    cli_audit_logger.log_event("command_forbidden", command="serve", reason="public_bind_forbidden_in_prod")
                    alert_operator("CRITICAL: Dashboard cannot bind to public interfaces in PRODUCTION_MODE. Aborting.", level="CRITICAL")
                    sys.exit(1)
            ...
            cli_audit_logger.log_event("command_executed", command="serve", args=scrub_secrets(vars(args)))
            ...
            try:
                ...
                logger.info(f"Successfully triggered '{args.event}' event. Server response: {response.json()}")
                cli_audit_logger.log_event("command_executed", command="trigger", args=scrub_secrets(vars(args)), status="success")
            except requests.RequestException as e:
                logger.critical(f"CRITICAL: Failed to trigger event on '{args.target}': {e}", exc_info=True)
                cli_audit_logger.log_event("command_executed", command="trigger", args=scrub_secrets(vars(args)), status="failure", error=str(e))
                alert_operator(f"CRITICAL: Failed to trigger event on '{args.target}': {e}. Aborting.", level="CRITICAL")
                sys.exit(1)
            except ValueError as e:
                logger.critical(f"CRITICAL: Invalid target URL for trigger: {e}", exc_info=True)
                cli_audit_logger.log_event("command_executed", command="trigger", args=scrub_secrets(vars(args)), status="failure", error=str(e))
                alert_operator(f"CRITICAL: Invalid target URL for trigger: {e}. Aborting.", level="CRITICAL")
                sys.exit(1)
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    with open(dummy_file_path, "w") as f:
                        f.write("import os\ndef func(): pass\n")
                    test_analyzer = ImportGraphAnalyzer(tmpdir, config=global_config.get('analyzer', {}))
            ...
            if not plugin_manager.plugins:
                logger.warning("No plugins were loaded. Check 'plugins' directory or --plugins-dir.")
            else:
        ...
        logger.critical(f"CRITICAL: An unexpected error occurred during CLI execution: {e}", exc_info=True)
        cli_audit_logger.log_event("cli_execution_failure", error=str(e), traceback=traceback.format_exc(), cli_args=scrub_secrets(vars(args)))
        alert_operator(f"CRITICAL: Unexpected error during CLI execution: {e}. Aborting.", level="CRITICAL", details={'traceback': traceback.format_exc()})
        sys.exit(1)
...
def main():
    ...
    try:
        ...
        logger.critical(f"CRITICAL: An unhandled exception occurred in the async main loop: {e}", exc_info=True)
        sys.exit(1)
