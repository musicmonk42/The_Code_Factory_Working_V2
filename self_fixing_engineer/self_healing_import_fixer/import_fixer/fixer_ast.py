...
try:
    from self_healing_import_fixer.import_fixer.compat_core import alert_operator, scrub_secrets, audit_logger, SECRETS_MANAGER
    ...
    try:
        alert_operator(f"CRITICAL: AST healing missing core dependency: {e}. Aborting.", level="CRITICAL")
    except Exception:
        pass
...
try:
    from self_healing_import_fixer.import_fixer.fixer_ai import get_ai_suggestions as get_ai_suggestions_real
...
def get_ai_refactoring_suggestion(context: str) -> str:
    ...
    try:
        ...
        audit_logger.log_event("ai_suggestion_failure", error=str(e), context_snippet=scrubbed_context[:200])
        alert_operator(f"CRITICAL: AI refactoring suggestion failed: {e}. Aborting healing.", level="CRITICAL")
...
class ImportResolver(NodeTransformer):
    ...
    """
    def __init__(self, current_module_path: str, project_root: str, whitelisted_paths: List[str], root_package_names: List[str]):
        """
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> ast.ImportFrom:
        ...
        current_file_path = os.path.join(self.project_root, *self.current_module_path.split('.')) + ".py"
        if not any(current_file_path.startswith(wp) for wp in self.whitelisted_paths):
            logger.critical(f"CRITICAL: Attempted to resolve imports in '{current_file_path}' which is outside whitelisted paths: {self.whitelisted_paths}. Aborting operation.")
            audit_logger.log_event("security_violation", type="path_traversal_attempt", file=current_file_path, whitelisted_paths=self.whitelisted_paths)
            alert_operator(f"CRITICAL: ImportResolver operating outside whitelisted paths: {current_file_path}. Aborting.", level="CRITICAL")
            raise AnalyzerCriticalError(f"ImportResolver operating outside whitelisted paths: {current_file_path}.")
        ...
        if node.level > 0: # It's a relative import
            ...
            logger.debug(f"Resolving in '{self.current_module_path}': from {'.' * node.level}{node.module or ''} -> {new_module_path}")
...
class CycleHealer:
    ...
    """
    def __init__(self, file_path: str, cycle: List[str], graph: nx.DiGraph, project_root: str, whitelisted_paths: List[str]):
        """
        ...
        if not any(Path(self.file_path).is_relative_to(wp) for wp in map(Path, whitelisted_paths)):
            logger.critical(f"CRITICAL: Attempted to heal file '{self.file_path}' which is outside whitelisted paths: {whitelisted_paths}. Aborting operation.")
            audit_logger.log_event("security_violation", type="path_traversal_attempt", file=self.file_path, whitelisted_paths=whitelisted_paths)
            alert_operator(f"CRITICAL: CycleHealer operating outside whitelisted paths: {self.file_path}. Aborting.", level="CRITICAL")
            raise AnalyzerCriticalError(f"CycleHealer operating outside whitelisted paths: {self.file_path}.")
    ...
    async def _parse_ast_and_cache(self) -> None:
        ...
        try:
            ...
            raise AnalyzerCriticalError(f"Syntax error in file {self.file_path} during AST parsing for CycleHealer: {e}.")
        except Exception as e:
            raise AnalyzerCriticalError(f"Unexpected error parsing file {self.file_path} for CycleHealer: {e}.")

        self.current_module_name = self._get_module_name_from_path(self.file_path)
        if not self.current_module_name:
            raise NonCriticalError(f"Could not determine module name for {self.file_path}. Cycle healing limited.")
    ...
    async def find_problematic_import(self) -> Optional[Tuple[ast.AST, Set[str]]]:
        ...
        if not self.current_module_name:
            raise NonCriticalError(f"Cannot find problematic import: current module name for {self.file_path} is unknown.")
        ...
        try:
            ...
            logger.error(f"Current module '{self.current_module_name}' not found in cycle {self.cycle}.")
            return None

        logger.debug(f"Looking for import from '{self.current_module_name}' to '{next_mod_name_in_cycle}' in {self.file_path}")

        for node in ast.walk(self.tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name == next_mod_name_in_cycle or alias.name.split('.')[0] == next_mod_name_in_cycle.split('.')[0]:
                        imported_names = {a.asname or a.name for a in node.names}
                        logger.debug(f"Found problematic ast.Import: {alias.name} at line {node.lineno}")
                ...
                if absolute_import_module == next_mod_name_in_cycle:
                    imported_names = {a.asname or a.name for a in node.names}
                    logger.debug(f"Found problematic ast.ImportFrom: {node.module} at line {node.lineno}")
                    return node, imported_names
        logger.debug(f"No direct problematic import found in {self.file_path} for cycle {self.cycle}.")
        return None
    ...
    async def heal(self) -> Optional[str]:
        ...
        audit_logger.log_event("cycle_heal_attempt", file=self.file_path, cycle=scrub_secrets(self.cycle))
        ...
        if result:
            ...
            if transformer.modified:
                ...
                logger.info(f"Cycle fix applied to '{self.file_path}' by moving import into a function.")
                audit_logger.log_event("cycle_heal_mechanical_success", file=self.file_path, cycle=scrub_secrets(self.cycle))
                return new_code
            else:
                logger.info(f"Mechanical import move failed for '{self.file_path}'. Consulting AI for advanced strategies.")
        else:
            logger.info(f"No direct problematic import found in '{self.file_path}' for mechanical healing. Consulting AI.")

        # If mechanical fix failed or not applicable, consult AI
        ai_suggestion = self.extract_interface()
        if ai_suggestion and "AI features are unavailable" not in ai_suggestion and "failed due to LLM API issue" not in ai_suggestion:
            audit_logger.log_event("cycle_heal_ai_suggestion", file=self.file_path, cycle=scrub_secrets(self.cycle), suggestion=scrub_secrets(ai_suggestion))
        ...
        logger.warning(f"Could not mechanically fix cycle in '{self.file_path}' and no AI suggestion generated.")
        audit_logger.log_event("cycle_heal_failure", file=self.file_path, cycle=scrub_secrets(self.cycle), reason="no_mechanical_or_ai_fix")
        return None
    ...
    def extract_interface(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error getting AI suggestion for extracting interface: {e}", exc_info=True)
            ...
            if not self.import_moved:
                ...
                for sub_node in ast.walk(node):
                    if isinstance(sub_node, ast.Name) and sub_node.id in self.usage_names:
                        logger.info(f"Moving cycle-causing import into function '{node.name}' in file.")
...
class DynamicImportHealer:
    ...
    def __init__(self, file_path: str, project_root: str, whitelisted_paths: List[str]):
        ...
        if not Path(self.file_path).is_file():
            raise AnalyzerCriticalError(f"File not found for DynamicImportHealer: {self.file_path}.")

        if not any(Path(self.file_path).is_relative_to(wp) for wp in map(Path, whitelisted_paths)):
            logger.critical(f"CRITICAL: Attempted to analyze file '{self.file_path}' which is outside whitelisted paths: {whitelisted_paths}. Aborting operation.")
            audit_logger.log_event("security_violation", type="path_traversal_attempt", file=self.file_path, whitelisted_paths=whitelisted_paths)
            alert_operator(f"CRITICAL: DynamicImportHealer operating outside whitelisted paths: {self.file_path}. Aborting.", level="CRITICAL")
            raise AnalyzerCriticalError(f"DynamicImportHealer operating outside whitelisted paths: {self.file_path}.")
    ...
    async def _parse_ast_and_cache(self) -> None:
        ...
        try:
            ...
            raise AnalyzerCriticalError(f"Syntax error in file {self.file_path} during AST parsing for DynamicImportHealer: {e}.")
        except Exception as e:
            raise AnalyzerCriticalError(f"Unexpected error parsing file {self.file_path} for DynamicImportHealer: {e}.")
        ...
        logger.debug(f"DynamicImportHealer initialized for file: {self.file_path}. Found {len(self.dynamic_nodes)} dynamic import candidates.")
    ...
    def heal(self) -> List[Tuple[ast.AST, str]]:
        ...
        for node in self.dynamic_nodes:
            ...
            audit_logger.log_event("dynamic_import_found", file=self.file_path, line=node.lineno,
                                   dynamic_call=node.func.id, suggestion=scrub_secrets(suggestion))

        if not fixes:
            audit_logger.log_event("dynamic_import_analysis_complete", file=self.file_path, status="no_dynamic_imports_found")
        else:
            audit_logger.log_event("dynamic_import_analysis_complete", file=self.file_path, status="dynamic_imports_found", count=len(fixes))
...
if __name__ == "__main__":
    ...
    resolver = ImportResolver(current_module_path, test_project_root, whitelisted_paths, root_package_names)
    ...
    with open(unwhitelisted_file, "w") as f: f.write("import os\n")
    try:
        # Simulate PRODUCTION_MODE for this test
        original_production_mode = PRODUCTION_MODE
        os.environ["PRODUCTION_MODE"] = "true"

        ImportResolver("unwhitelisted_file", os.path.dirname(unwhitelisted_file), [test_project_root], ["dummy"])
        ...
        if os.path.exists(unwhitelisted_file): os.remove(unwhitelisted_file)
    ...
    async def run_cycle_healing_test():
        cycle_healer_a = CycleHealer(cycle_file_a, ["module_a", "module_b"], simulated_graph, test_project_root, whitelisted_paths)
        cycle_healer_b = CycleHealer(cycle_file_b, ["module_a", "module_b"], simulated_graph, test_project_root, whitelisted_paths)
        ...
        if healed_code_a:
            ...
            print(f"Could not heal {os.path.basename(cycle_file_a)} mechanically or with AI suggestion.")
    ...
    try:
        ...
        CycleHealer(syntax_error_file, ["syntax_error_file"], nx.DiGraph(), test_project_root, whitelisted_paths)
    except AnalyzerCriticalError:
