...
def _get_plugin_signature_key(production_mode: bool) -> bytes:
    ...
    if _plugin_signature_key is None:
        ...
        if key_str:
            ...
            logger.warning("FIXER_PLUGIN_SIGNATURE_KEY_ENV not set. Generated a random key for plugin signing. THIS IS INSECURE FOR PRODUCTION.")
            alert_operator("WARNING: Plugin signature key not set. Using insecure random key. IMMEDIATE ACTION REQUIRED.", level="HIGH")
    else:
        if production_mode and _plugin_key_source != "secret":
            raise AnalyzerCriticalError("Random HMAC key was initialized in a non-production session; refusing to use in production.")
    return _plugin_signature_key

async def _verify_plugin_signature_async(file_path: Path, expected_signature: str, production_mode: bool) -> bool:
    """
    ...
    try:
        ...
        if hmac.compare_digest(calculated_signature, expected_signature):
            ...
            audit_logger.log_event("plugin_signature_mismatch", file=str(file_path), calculated=calculated_signature, expected=expected_signature)
            return False
        ...
        logger.error(f"Error during plugin signature verification for {file_path}: {e}", exc_info=True)
        alert_operator(f"ERROR: Plugin signature verification failed for {file_path}: {e}", level="ERROR")
        return False
...
class PluginManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.production_mode = bool(self.config.get("production_mode", os.getenv("PRODUCTION_MODE", "false").lower() == "true"))
        ...
        whitelisted_dirs = [Path(d).resolve() for d in self.config.get("whitelisted_plugin_dirs", [])]
        ...
        if self.production_mode and not self.whitelisted_plugin_dirs:
            raise AnalyzerCriticalError("In PRODUCTION_MODE, 'whitelisted_plugin_dirs' must be configured for PluginManager. Aborting startup.")
        ...
        audit_logger.log_event("plugin_manager_init", 
    ...
    def _validate_approved_plugins(self, approved_plugins: Mapping[str, str]) -> None:
        for module_name, signature in approved_plugins.items():
            if not self._SAFE_MOD_RE.match(module_name):
                raise PluginValidationError(f"Invalid plugin name in approved_plugins list: {module_name!r}")
            if not self._HEX_DIGEST_RE.match(signature):
                raise PluginValidationError(f"Invalid signature for plugin {module_name!r}. Must be a 64-char lowercase hex SHA-256 digest.")
    ...
    def register_hook(self, hook_name: str, func: Callable) -> None:
        if self.production_mode and not self._in_plugin_registration:
            raise PluginValidationError(f"Runtime hook registration for '{hook_name}' is forbidden in production.")
        if func not in self.hooks[hook_name]:
            self.hooks[hook_name].append(func)
            logger.debug(f"Registered hook '{hook_name}': {func.__name__}")
            audit_logger.log_event("plugin_registered", type="hook", hook_name=hook_name, func_name=func.__name__, module=_callable_module_name(func))

    def register_healer(self, healer: Callable) -> None:
        if self.production_mode and not self._in_plugin_registration:
            raise PluginValidationError(f"Runtime healer registration for '{healer.__name__}' is forbidden in production.")
        if healer not in self.healers:
            self.healers.append(healer)
            logger.debug(f"Registered healer: {healer.__name__}")
            audit_logger.log_event("plugin_registered", type="healer", func_name=healer.__name__, module=_callable_module_name(healer))

    def register_validator(self, validator: Callable) -> None:
        if self.production_mode and not self._in_plugin_registration:
            raise PluginValidationError(f"Runtime validator registration for '{validator.__name__}' is forbidden in production.")
        if validator not in self.validators:
            self.validators.append(validator)
            logger.debug(f"Registered validator: {validator.__name__}")
            audit_logger.log_event("plugin_registered", type="validator", func_name=validator.__name__, module=_callable_module_name(validator))

    def register_diff_viewer(self, viewer: Callable) -> None:
        if self.production_mode and not self._in_plugin_registration:
            raise PluginValidationError(f"Runtime diff viewer registration for '{viewer.__name__}' is forbidden in production.")
        if viewer not in self.diff_viewers:
            self.diff_viewers.append(viewer)
            logger.debug(f"Registered diff viewer: {viewer.__name__}")
            audit_logger.log_event("plugin_registered", type="diff_viewer", func_name=viewer.__name__, module=_callable_module_name(viewer))
    ...
    def run_hook(self, hook_name: str, *args, **kwargs) -> None:
        ...
        audit_logger.log_event("plugin_hook_run",
                               hook_name=hook_name,
        ...
        for func in self.hooks.get(hook_name, []):
            try:
                ...
                audit_logger.log_event("plugin_hook_failure", hook_name=hook_name, func_name=func.__name__, module=_callable_module_name(func), error=str(e))
                if stop_on_error:
                    raise AnalyzerCriticalError(f"Plugin hook '{hook_name}' failed in '{func.__name__}'. Aborting due to 'stop_on_hook_error' config.", alert_level="CRITICAL")
                errors.append((func.__name__, str(e)))

        if errors:
            alert_operator(f"{len(errors)} hook(s) failed in '{hook_name}'. See logs for details.", level="ERROR")
    ...
    async def load_plugin(self, module_name: str) -> None:
        ...
        async with lock:
            ...
            if not plugin_found:
                raise NonCriticalError(f"Plugin '{module_name}' not found in any whitelisted directory.")
    ...
    def unload_plugin(self, module_name: str) -> None:
        ...
        audit_logger.log_event("plugin_unloaded", module_name=module_name, components_unregistered=removed)
    ...
    async def _load_plugin_file_async(self, module_name: str, full_plugin_path: Path) -> None:
        ...
        audit_logger.log_event("plugin_load_attempt", module_name=module_name, path=scrub_secrets(str(full_plugin_path)))

        max_size = int(self.config.get("max_plugin_bytes", 0))
        if max_size and full_plugin_path.stat().st_size > max_size:
            raise PluginValidationError(f"Plugin '{module_name}' exceeds size limit of {max_size} bytes.")

        if self.production_mode and self.config.get("enforce_posix_perms", False):
            try:
                st = os.stat(full_plugin_path)
                if st.st_mode & stat.S_IWOTH:
                    raise PluginValidationError(f"Plugin '{module_name}' at {full_plugin_path} is world-writable.")
                if hasattr(os, "getuid") and st.st_uid not in {os.getuid(), 0}:
                    raise PluginValidationError(f"Plugin '{module_name}' at {full_plugin_path} not owned by current user/root.")
            except OSError as e:
                raise AnalyzerCriticalError(f"Failed to stat plugin file {full_plugin_path}: {e}", level="CRITICAL")

        expected_signature = self.approved_plugins.get(module_name)
        if self.production_mode and not expected_signature:
            raise PluginValidationError(f"Plugin '{module_name}' is not in approved_plugins or has no signature.")
        if self.production_mode and expected_signature and not self._HEX_DIGEST_RE.match(expected_signature):
            raise PluginValidationError(f"Plugin signature for '{module_name}' must be a 64-char lowercase hex SHA-256 digest.")

        if self.production_mode and expected_signature:
            if not await _verify_plugin_signature_async(full_plugin_path, expected_signature, self.production_mode):
                raise PluginValidationError(f"Plugin '{module_name}' signature verification failed.")
        
        spec = importlib.util.spec_from_file_location(module_name, full_plugin_path)
        if spec is None or spec.loader is None:
            raise PluginLoadError(f"Could not get module spec or loader for plugin: {full_plugin_path}")
        ...
        try:
            if self.production_mode and expected_signature:
                ...
                if not await _verify_plugin_signature_async(full_plugin_path, expected_signature, self.production_mode):
                    raise PluginValidationError(f"Plugin '{module_name}' changed on disk during load.")
            ...
            audit_logger.log_event("plugin_loaded", module_name=module_name, path=str(full_plugin_path))
        except Exception:
...
"""
    ...
    with open(healer_plugin_path, "w") as f: f.write(healer_plugin_content)
    with open(validator_plugin_path, "w") as f: f.write(validator_plugin_content)

    async def main_test():
        _reset_plugin_key_for_tests()
        healer_signature = hmac.new(_get_plugin_signature_key(production_mode=False), healer_plugin_content.encode('utf-8'), hashlib.sha256).hexdigest()
        validator_signature = hmac.new(_get_plugin_signature_key(production_mode=False), validator_plugin_content.encode('utf-8'), hashlib.sha256).hexdigest()
        ...
        print("\n--- Running Hooks ---")
        plugin_manager.run_hook("pre_healing", context="initial scan context", sensitive_info="my_secret_data")
        ...
        try:
            plugin_manager_unapproved = make_plugin_manager(config={"whitelisted_plugin_dirs": [], "approved_plugins": {}})
            await plugin_manager_unapproved.load_plugin("plugin_unapproved")
        except NonCriticalError as e:
            print(f"Caught expected NonCriticalError for loading from non-whitelisted directory: {e}")
        ...
        unapproved_plugin_path.write_text("from fixer_plugins import Plugin, PluginManager\nclass TestPlugin(Plugin):\n\tdef register(self, manager: PluginManager):\n\t\tpass\n")
        try:
            _reset_plugin_key_for_tests()
            plugin_manager_unsigned = make_plugin_manager(config={"whitelisted_plugin_dirs": [str(unapproved_plugin_dir)], "approved_plugins": { "plugin_unapproved_signed": "a" * 64 }, "production_mode": True})
        ...
        tampered_signature_original = hmac.new(_get_plugin_signature_key(production_mode=False), healer_plugin_content.encode('utf-8'), hashlib.sha256).hexdigest()
        
        try:
            _reset_plugin_key_for_tests()
            plugin_manager_tampered = make_plugin_manager(config={
