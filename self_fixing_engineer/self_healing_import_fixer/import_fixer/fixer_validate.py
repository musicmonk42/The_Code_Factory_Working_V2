...
class CodeValidator:
    ...
    def __init__(
        ...
        self.whitelisted_paths = [
            Path(p).resolve() for p in (whitelisted_paths or [self.project_root])
        ]
        self._tool_versions = {}
        ...
        if not self.project_root.is_dir():
            raise AnalyzerCriticalError(
                f"Project root '{self.project_root}' is not a valid directory. Aborting validation."
            )

        if PRODUCTION_MODE and not self.whitelisted_paths:
            raise AnalyzerCriticalError(
                "In PRODUCTION_MODE, 'whitelisted_paths' must be configured for CodeValidator. Aborting startup."
            )
        ...
        try:
            audit_logger.log_event(
                ...
                whitelisted_paths=[str(p) for p in self.whitelisted_paths],
            )
        except Exception:
    ...
    def _assert_whitelisted(self, p: Path) -> None:
        ...
        if not self._is_under(p):
            raise AnalyzerCriticalError(
                f"Attempted to operate on file '{p}' which is outside whitelisted paths: {self.whitelisted_paths}. Aborting operation."
            )
            ...
            try:
                ...
                if not tool_path:
                    raise NonCriticalError(
                        f"Required tool '{command[0]}' not found for {description}. Skipping validation."
                    )
                ...
                if process.returncode == 0:
                    logger.debug(
                        ...
                        extra={"command": " ".join(command), "cwd": str(cwd)},
                    )
                ...
                logger.critical(
                    f"CRITICAL: Unexpected error running {description}: {e}",
                    exc_info=True,
                    extra={"command": " ".join(command), "cwd": str(cwd)},
                )
                try:
                    ...
                    pass
                raise AnalyzerCriticalError(
                    f"CodeValidator: Unexpected error running {description}: {e}."
                )
    ...
    def compile_file(self, file_path: Path) -> StageResult:
        ...
        try:
            ...
            try:
                audit_logger.log_event(
                    "validation_step_success", step="compile", file=str(file_path)
                )
            except Exception:
                pass
            ...
            error_msg = (
                ...
                else f"Failed to read/parse {file_path}: {e}"
            )
            logger.error(f"Compilation FAILED for {file_path}: {error_msg}")
            try:
                audit_logger.log_event(
                    "validation_step_failure",
                    step="compile",
                    file=str(file_path),
                    error=error_msg,
                )
    ...
    async def _run_tool_stage(
        ...
        try:
            ...
            success, stdout, stderr = await self._run_command_async(
                command,
                description=f"{tool} {stage_name}",
                timeout_s=timeout_s,
    ...
    async def run_tests(
        ...
        if not shutil.which("pytest"):
            return StageResult(
                ...
                tool="pytest",
                version=None,
                stderr_tail="pytest not found but tests were requested.",
            )
    ...
    def show_diff(self, file_path: Path, new_code: str, interactive: bool) -> bool:
        ...
        for line in diff:
            if line.startswith(("+++", "---", "@@")):
                ...
                colored_diff.append(colored(line, "blue"))
            else:
                colored_diff.append(line)
    ...
    async def validate_and_commit_file(
        self,
        file_path: str,
        new_code: str,
        original_code: str,
        ...
        if PRODUCTION_MODE and interactive:
            raise AnalyzerCriticalError(
                "Interactive prompts are forbidden in PRODUCTION_MODE. Aborting validation."
            )
        ...
        if interactive:
            if not self.show_diff(file_path_obj, new_code, interactive):
                ...
                try:
                    audit_logger.log_event(
                        "single_file_validation_declined", file=str(file_path_obj)
                    )
                except Exception:
                    pass
        ...
        try:
            ...
            try:
                audit_logger.log_event(
                    "file_backup", file=str(file_path_obj), backup_path=str(backup_path)
                )
            except Exception:
                pass
        except Exception as e:
            raise AnalyzerCriticalError(
                f"Failed to create backup for {file_path_obj}: {e}. Aborting validation."
            )
        ...
        try:
            ...
            raise AnalyzerCriticalError(
                f"Failed to write temporary changes to {file_path_obj}: {e}. Aborting validation."
            )
        ...
        try:
                ...
                for validator_func in custom_validators:
                    try:
                        if not validator_func(file_path_obj):
                            logger.error(
                                f"Custom validator '{validator_func.__name__}' FAILED for {file_path_obj}."
                            )
                            try:
                                audit_logger.log_event(
                                    "validation_step_failure",
                                    step="custom_validator",
                        ...
                        logger.error(
                            f"Error running custom validator '{validator_func.__name__}' for {file_path_obj}: {e}",
                            exc_info=True,
                        )
                        try:
                            audit_logger.log_event(
            ...
            if not validation_passed:
                ...
                logger.info(
                    f"Validation PASSED for {file_path_obj}. Changes are committed (persisted)."
                )
                try:
                    audit_logger.log_event(
                        "single_file_validation_success", file=str(file_path_obj)
                    )
                except Exception:
                    pass
                try:
    ...
    async def validate_and_commit_batch(
        self,
        ...
        try:
            audit_logger.log_event(
                ...
                files_count=len(files_to_validate_obj),
                run_tests=run_tests,
            )
        except Exception:
            pass
        ...
        if PRODUCTION_MODE:
            raise AnalyzerCriticalError(
                "Batch validation is forbidden in PRODUCTION_MODE. Aborting."
            )
        ...
        try:
                    ...
                    for validator_func in custom_validators:
                        try:
                            if not validator_func(file_path):
                                logger.error(
                                    f"Custom validator '{validator_func.__name__}' FAILED for {file_path} in batch."
                                )
                                try:
                                    audit_logger.log_event(
                                        "validation_step_failure",
                            ...
                            logger.error(
                                f"Error running custom validator '{validator_func.__name__}' for {file_path} in batch: {e}",
                                exc_info=True,
                            )
                            try:
                                audit_logger.log_event(
            ...
            for file_path in files_to_validate_obj:
                self.rollback_change(
                    file_path,
                    original_contents.get(str(file_path), ""),
            ...
            if not validation_passed:
                ...
                logger.info(
                    f"Batch validation PASSED for {len(files_to_validate_obj)} files. Changes are committed (persisted)."
                )
                try:
                    audit_logger.log_event(
                        "batch_file_validation_success",
                        files_count=len(files_to_validate_obj),
                    )
                except Exception:
                    pass
    ...
    def rollback_change(
        self, file_path: Path, original_code: str, is_critical_failure: bool = False
    ):
        """
        ...
        try:
            ...
            logger.critical(
                f"CRITICAL: Failed to rollback changes for {file_path}: {e}",
                exc_info=True,
            )
            try:
                audit_logger.log_event(
            ...
            if is_critical_failure:
                alert_operator(
                    f"CRITICAL: Failed to rollback changes for {file_path}. Manual intervention REQUIRED! Aborting.",
                    level="CRITICAL",
                )
                raise
        except IOError as e:
            logger.critical(
                f"CRITICAL: Failed to rollback changes for {file_path}: {e}. Manual intervention REQUIRED!",
                exc_info=True,
            )
            try:
                audit_logger.log_event(
            ...
            logger.critical(
                f"CRITICAL: An unexpected error occurred during rollback for {file_path}: {e}. Manual intervention REQUIRED!",
                exc_info=True,
            )
            try:
                audit_logger.log_event(
            ...
            if is_critical_failure:
                alert_operator(
                    f"CRITICAL: Unexpected error during rollback for {file_path}. Manual intervention REQUIRED! Aborting.",
                    level="CRITICAL",
                )
                raise
...
async def main():
    ...
    code_validator = make_validator(
        project_root=str(test_project_root), whitelisted_paths=[str(test_project_root)]
    )
