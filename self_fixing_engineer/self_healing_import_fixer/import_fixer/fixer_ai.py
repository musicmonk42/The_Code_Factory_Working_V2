...
def _redis_alert_on_failure(e):
    ...
    if _redis_failure_count >= REDIS_ALERT_THRESHOLD and not _redis_failure_alerted:
        alert_operator("WARNING: Redis cache is persistently failing. Caching is degraded and performance/costs may suffer.", level="WARNING")
...
def _sanitize_response(response: str) -> str:
    ...
    scrubbed = re.sub(r"<\s*script.*?>.*?<\s*/\s*script\s*>", "[REDACTED]", scrubbed, flags=re.DOTALL|re.IGNORECASE)
    return scrubbed
...
class AIManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if PRODUCTION_MODE:
            if not self.llm_api_key:
                raise AnalyzerCriticalError("LLM API key is not configured. AI features cannot function.")
            ...
            if not self.proxy_url:
                raise AnalyzerCriticalError("In PRODUCTION_MODE, 'proxy_url' for LLM traffic is required but not configured.")
            if self.allow_auto_apply_patches:
                raise AnalyzerCriticalError("'allow_auto_apply_patches' is enabled in PRODUCTION_MODE. This is forbidden.")

        assert self.llm_endpoint.startswith("https://") or not PRODUCTION_MODE, "LLM endpoint must use HTTPS in production."
        ...
        logger.info(f"AIManager initialized for model: {self.model_name}, endpoint: {self.llm_endpoint}")
        audit_logger.log_event("ai_manager_init", model=self.model_name, endpoint=self.llm_endpoint,
    ...
    async def _enforce_token_quota(self, tokens_to_use: int):
        async with self._token_usage_lock:
            ...
            if current_minute_usage + tokens_to_use > self.token_quota_per_minute:
                ...
                logger.warning(f"LLM token quota exceeded. Current usage: {current_minute_usage}, requested: {tokens_to_use}. Waiting for {sleep_duration:.2f}s.", extra={'current_usage': current_minute_usage, 'requested_tokens': tokens_to_use})
                audit_logger.log_event("llm_token_quota_exceeded", current_usage=current_minute_usage, requested_tokens=tokens_to_use, wait_time=sleep_duration)
    ...
    async def _call_llm_api(self, prompt: str) -> Optional[str]:
        ...
        audit_logger.log_event("llm_api_call_start", model=self.model_name, endpoint=self.llm_endpoint,
                               prompt_length=len(prompt), max_tokens_requested=self.max_tokens)
        ...
        try:
            async with self._api_semaphore:
                ...
                if response_content:
                    ...
                    audit_logger.log_event("llm_api_call_output", response=scrub_secrets(response_content[:500]))
                    if cache:
                        try:
            ...
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type='RateLimitError', error_message=str(e))
            raise NonCriticalError(f"LLM rate limit exceeded: {e}")
        except APIError as e:
            logger.error(f"LLM API error: {e}")
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type='APIError', error_message=str(e))
            raise NonCriticalError(f"LLM API error: {e}")
        except httpx.TimeoutException as e:
            logger.error(f"LLM API timeout: {e}")
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type='TimeoutException', error_message=str(e))
            raise NonCriticalError(f"LLM API timeout: {e}")
        except Exception as e:
            logger.error(f"An error occurred during LLM API call: {e}", exc_info=True)
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type=type(e).__name__, error_message=str(e))
    ...
    async def get_cycle_breaking_suggestion(self, cycle_path: List[str], relevant_code_snippets: Dict[str, str]) -> str:
...
async def _get_ai_manager_instance(config: Optional[Dict[str, Any]] = None) -> AIManager:
...
def get_ai_suggestions(codebase_context: str, config: Optional[Dict[str, Any]] = None) -> List[str]:
    async def _async_suggestions():
        ...
        if suggestions_str and "AI features are unavailable" not in suggestions_str and "failed due to LLM API issue" not in suggestions_str and "failed after multiple retries" not in suggestions_str:
            return [s.strip() for s in suggestions_str.split('\n') if s.strip()]
        return []
    
    return _run_async_in_sync(_async_suggestions())

def get_ai_patch(problem_description: str, relevant_code: str, suggestions: List[str], config: Optional[Dict[str, Any]] = None) -> List[str]:
    async def _async_patch():
        ...
        if patch_str and "AI features are unavailable" not in patch_str and "failed due to LLM API issue" not in patch_str and "failed after multiple retries" not in patch_str:
...
async def main_test():
    ...
    class DummySecretsManager:
        def get_secret(self, key: str, default: Optional[str] = None, required: bool = True) -> Optional[str]:
            if key == "LLM_API_KEY": return os.getenv("OPENAI_API_KEY", "sk-dummy-test-key")
            if required: raise ValueError(f"Missing required secret for test: {key}")
            return default
    ...
    try:
        ...
        await asyncio.run(_ai_manager_instance._call_llm_api("Generate a very long suggestion about complex architecture patterns and microservices."))
    ...
    try:
        ...
        print(f"Caught expected AnalyzerCriticalError due to PRODUCTION_MODE + auto_apply_patches=True: {e}")
    finally:
