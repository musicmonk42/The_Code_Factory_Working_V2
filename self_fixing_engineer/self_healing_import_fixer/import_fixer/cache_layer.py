...
# tenacity (optional)
try:
    from tenacity import retry, stop_after_attempt, wait_exponential, retry_if_exception_type, RetryError, after_log
    _HAS_TENACITY = True
except Exception:
    _HAS_TENACITY = False
...
cache_misses = _safe_metric(metrics.Counter, "cache_layer_misses_total", "Cache misses", ["backend"])
cache_op_latency = _safe_metric(metrics.Histogram, "cache_layer_op_latency_seconds", "Cache operation latency", ["backend", "operation"])
redis_connection_failures = _safe_metric(metrics.Counter, "cache_layer_redis_connection_failures_total", "Redis connection failures")
file_hmac_failures = _safe_metric(metrics.Counter, "cache_layer_file_hmac_failures_total", "File cache HMAC verification failures")

_retry_on_redis = retry(
...
) if _HAS_TENACITY else (lambda f: f)
...
class _BaseCache:
    ...
    async def get(self, key: str) -> Optional[Any]:
        ...
        with tracer.start_as_current_span(f"{self.backend}_cache_get", attributes={"backend": self.backend, "key": key, "span_id": span_id}) as span:
            with cache_op_latency.labels(self.backend, "get").time():
                try:
                    ...
                    audit_logger.info("Cache operation", operation="get", backend=self.backend, key=key, status="success", span_id=span_id, data_classification="internal")
                    return result
                except Exception as e:
                    json_logger.error("Cache GET failed", extra={"data": {"backend": self.backend, "key": key, "span_id": span_id, "error": str(e)}}, exc_info=True)
                    span.record_exception(e)
                    audit_logger.error("Cache operation failed", operation="get", backend=self.backend, key=key, status="failure", error=str(e), span_id=span_id, data_classification="internal")
                    return None
    ...
    async def setex(self, key: str, ttl: int, val: Any) -> bool:
        ...
        span_id = str(uuid.uuid4())
        with tracer.start_as_current_span(f"{self.backend}_cache_setex", attributes={"backend": self.backend, "key": key, "ttl": ttl, "span_id": span_id}) as span:
            with cache_op_latency.labels(self.backend, "setex").time():
                try:
                    if sys.version_info >= (3, 11):
                        ...
                        success = await asyncio.wait_for(self._setex_impl(key, ttl, val), timeout=5.0)

                    audit_logger.info("Cache operation", operation="setex", backend=self.backend, key=key, status="success" if success else "failure", span_id=span_id, data_classification="internal")
                    return success
                except Exception as e:
                    json_logger.error("Cache SETEX failed", extra={"data": {"backend": self.backend, "key": key, "span_id": span_id, "error": str(e)}}, exc_info=True)
                    span.record_exception(e)
                    audit_logger.error("Cache operation failed", operation="setex", backend=self.backend, key=key, status="failure", error=str(e), span_id=span_id, data_classification="internal")
                    return False
    ...
    async def incr(self, key: str) -> int:
        ...
        span_id = str(uuid.uuid4())
        with tracer.start_as_current_span(f"{self.backend}_cache_incr", attributes={"backend": self.backend, "key": key, "span_id": span_id}) as span:
            with cache_op_latency.labels(self.backend, "incr").time():
                try:
                    ...
                    audit_logger.info("Cache operation", operation="incr", backend=self.backend, key=key, status="success", new_value=new_val, span_id=span_id, data_classification="internal")
                    return new_val
                except Exception as e:
                    json_logger.error("Cache INCR failed", extra={"data": {"backend": self.backend, "key": key, "span_id": span_id, "error": str(e)}}, exc_info=True)
                    span.record_exception(e)
                    audit_logger.error("Cache operation failed", operation="incr", backend=self.backend, key=key, status="failure", error=str(e), span_id=span_id, data_classification="internal")
                    return 0
...
class _FileCache(_BaseCache):
    def __init__(self, root: Path, secrets_manager: Any):
        ...
        if not self.hmac_key:
            if PRODUCTION_MODE:
                json_logger.critical("FILE_CACHE_HMAC_KEY not found in production. This is a security risk.")
            else:
    ...
    def _verify_payload(self, payload: Dict[str, Any]) -> bool:
        ...
        if not hmac_sig:
            json_logger.warning("File cache entry missing HMAC signature", extra={"data": {"path": str(self._p('')), "payload": payload}})
            return False
        payload_str = json.dumps(payload, sort_keys=True).encode('utf-8')
        h = hashlib.sha256(self.hmac_key.encode() + payload_str).hexdigest()
        return hmac_sig == h
    ...
    async def _get_impl(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            if not self._verify_payload(payload):
                ...
                json_logger.error("File cache entry HMAC validation failed. Potential tampering.", extra={"data": {"path": str(p)}})
            ...
            json_logger.error("Error reading file cache entry", extra={"path": str(p), "exc_info": True})
            return None
...
async def _connect_redis():
    ...
    redis_host = _validate_env_var("REDIS_HOST", os.getenv("REDIS_HOST", "localhost"), re.compile(r"^[a-zA-Z0-9\.\-_]+$"))
    redis_port = _validate_env_var("REDIS_PORT", os.getenv("REDIS_PORT", "6379"), re.compile(r"^\d{1,5}$"))
    redis_ssl = _validate_env_var("REDIS_SSL", os.getenv("REDIS_SSL", "false"), re.compile(r"^(true|false)$")) == "true" and PRODUCTION_MODE

    password = SECRETS_MANAGER.get_secret("REDIS_PASSWORD", required=PRODUCTION_MODE)
    ssl_cert_file = SECRETS_MANAGER.get_secret("REDIS_SSL_CERT", required=redis_ssl and PRODUCTION_MODE)
    ssl_key_file = SECRETS_MANAGER.get_secret("REDIS_SSL_KEY", required=redis_ssl and PRODUCTION_MODE)

    with contextlib.suppress(Exception):
        json_logger.info("Attempting to connect to Redis", extra={"host": redis_host, "port": redis_port, "ssl": redis_ssl, "data_classification": "internal"})
    ...
    try:
        ...
        json_logger.info("Successfully connected to Redis", extra={"host": redis_host, "port": redis_port, "data_classification": "internal"})
        return c
...
async def _check_fallback_usage(backend_name: str, message: str):
    ...
    if current_time - _last_fallback_alert_time > 3600 or not PRODUCTION_MODE:
        ...
        json_logger.warning("Using fallback cache", extra={"backend": backend_name, "reason": message, "data_classification": "internal"})
    ...
    *,
    ...
    if _HAS_REDIS:
        if _CACHED_REDIS_CLIENT:
            try:
                ...
                json_logger.warning("Stale Redis client detected. Disconnecting and retrying.", extra={"data_classification": "internal"})
        ...
        try:
            ...
            redis_connection_failures.inc()
            alert_operator(f"CRITICAL: Failed to connect to Redis after multiple retries: {e}. Using fallback cache.", level="CRITICAL")
            json_logger.critical("Redis connection failed. Falling back.", exc_info=True, extra={"data_classification": "internal"})
            _CACHED_REDIS_CLIENT = None
        except Exception as e:
            redis_connection_failures.inc()
            await _check_fallback_usage("file_cache", f"Redis not available: {e}. Using fallback cache.")
    ...
    if project_root:
        try:
            ...
            await _check_fallback_usage("in_memory_cache", f"File cache unavailable at {project_root}: {e}. Using in-memory cache.")
