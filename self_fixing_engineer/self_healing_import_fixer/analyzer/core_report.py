...
try:
    from flask import Flask, jsonify, render_template_string, request, send_file, current_app
...
def encrypt_report_content(content: bytes) -> bytes:
    ...
    try:
        ...
        alert_operator(f"CRITICAL: Report encryption failed: {e}. Report not encrypted.", level="CRITICAL")
...
def decrypt_report_content(encrypted_content: bytes) -> bytes:
    ...
    try:
        ...
        alert_operator(f"CRITICAL: Report decryption failed: {e}. Report inaccessible.", level="CRITICAL")
...
class ReportGenerator:
    ...
    """
    def __init__(self, output_dir: str = "reports", approved_report_dirs: Optional[List[str]] = None):
        ...
        if self.production_mode and not self._is_approved_dir(self.output_dir):
            raise AnalyzerCriticalError("[CRITICAL][REPORT] Output directory is not within approved paths.")
        ...
        try:
            ...
            raise AnalyzerCriticalError(f"Report output directory '{self.output_dir}' is not writable or accessible: {e}.")
    ...
    def _format_html_report(self, results):
        ...
        for section, data in results.items():
            ...
            if isinstance(data, (dict, list)):
                html.append(f"<pre>{json.dumps(scrub_secrets(data), indent=2, ensure_ascii=False)}</pre>")
            else:
    ...
    def _format_pdf_report(self, results: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            raise ImportError("weasyprint library not found. Please install it to generate PDF reports.")
    ...
    def generate_report(self,
        ...
        if self.production_mode:
            try:
                ...
                logger.critical(f"CRITICAL: Report encryption failed: {e}. Report will NOT be saved.", exc_info=True)
                alert_operator(f"CRITICAL: Report encryption failed: {e}. Aborting report save.", level="CRITICAL")
        ...
        try:
            ...
            audit_logger.log_event(
                ...
                summary={"overall_status": results.get("overall_status", "N/A"), "violations_count": len(results.get("policy_violations", []))}
            )
            return file_path
        except IOError as e:
            ...
            logger.critical(f"CRITICAL: An unexpected error occurred during report generation: {e}", exc_info=True)
            alert_operator(f"CRITICAL: Unexpected error during report generation: {e}", level="CRITICAL")
...
if FLASK_AVAILABLE:
    ...
    def login():
        ...
        if username == "admin" and password == admin_password:
            access_token = create_access_token(identity=username, additional_claims={"roles": ["admin"]})
            audit_logger.log_event("user_login_success", username=username)
            return jsonify(access_token=access_token)
        audit_logger.log_event("user_login_failure", username=username, reason="invalid_credentials")
    ...
    def get_report_endpoint(report_name_with_ext: str):
        ...
        if "admin" not in claims.get("roles", []):
            audit_logger.log_event("report_access_denied", report_name=report_name_with_ext, user=claims['sub'], reason="admin_access_required")
        ...
        audit_logger.log_event("report_access_attempt", report_name=sanitized_report_name, user=claims['sub'])
        ...
        if os.path.exists(report_path):
            try:
                ...
                audit_logger.log_event("report_viewed", report_name=sanitized_report_name, user=claims['sub'])
                return send_file(io.BytesIO(decrypted_content), mimetype=content_type, as_attachment=False, download_name=sanitized_report_name)
            except Exception as e:
                logger.error(f"Error serving report {sanitized_report_name}: {e}", exc_info=True)
                audit_logger.log_event("report_access_failure", report_name=sanitized_report_name, user=claims['sub'], error=str(e))
                alert_operator(f"ERROR: Failed to serve report {sanitized_report_name}: {e}", level="ERROR")
                return jsonify({"message": "Error serving report"}), 500
        audit_logger.log_event("report_access_failure", report_name=sanitized_report_name, user=claims['sub'], reason="not_found")
...
def generate_report(results: Dict[str, Any], report_name: str = "analysis_report", report_format: Literal["text", "markdown", "html", "pdf", "json"] = "html", user_id: str = "system") -> str:
...
if __name__ == "__main__":
    ...
    class DummySecretsManager:
        def get_secret(self, key: str, default: Optional[str] = None, required: bool = True) -> Optional[str]:
            if key == "JWT_SECRET_KEY": return "super-secret-jwt-key"
            if key == "DASHBOARD_ADMIN_PASSWORD": return "admin_secure_password"
            if key == "REPORT_KMS_KEY_ALIAS": return "alias/analyzer-report-key"
    ...
    sample_results = {
        ...
        "policy_violations": [
            {"rule_id": "PR001", "severity": "high", "message": "Denied import detected.", "sensitive_field": "secret_token_abc"},
            {"rule_id": "DL001", "severity": "medium", "message": "Too many dependencies."},
            "A list of strings with secret_token_abc"
        ],
        "security_scan_results": {
            "bandit_issues_count": 5,
            "pip_audit_vulnerabilities_count": 3,
            "critical_vulnerabilities": ["CVE-2023-1234", {"id": "CVE-2023-4567", "details": "This has a secret_token_abc."}]
        },
        "ai_suggestions": [
    ...
    test_approved_dirs = [os.path.abspath("test_reports_approved"), os.path.abspath("/var/log/analyzer_reports")]
    os.makedirs(test_approved_dirs[0], exist_ok=True)

    report_generator = ReportGenerator(output_dir=test_approved_dirs[0], approved_report_dirs=test_approved_dirs)

    print("\n--- Generating Text Report ---")
    text_report_path = report_generator.generate_report(sample_results, "my_text_report", "text", user_id="test_user_1")
    ...
    markdown_report_path = report_generator.generate_report(sample_results, "my_markdown_report", "markdown", user_id="test_user_2")
    ...
    html_report_path = report_generator.generate_report(sample_results, "my_html_report", "html", user_id="test_user_3")
    ...
    json_report_path = report_generator.generate_report(sample_results, "my_json_report", "json", user_id="test_user_4")
    ...
    try:
        pdf_report_path = report_generator.generate_report(sample_results, "my_pdf_report", "pdf", user_id="test_user_5")
