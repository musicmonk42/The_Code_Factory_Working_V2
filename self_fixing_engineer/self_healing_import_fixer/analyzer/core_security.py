...
try:
    ...
    try:
        ...
        alert_operator(
            f"CRITICAL: Security analysis missing core dependency: {e}. Aborting.",
            level="CRITICAL",
        )
    except Exception:
        pass
...
def _run_cmd(argv: List[str], timeout: int = 300) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        argv, timeout=timeout, capture_output=True, text=True, check=False, shell=False
    )
...
class SecurityAnalyzer:
    ...
    def __init__(self, project_root: str):
        if not os.path.isdir(project_root):
            raise AnalyzerCriticalError(
                f"Project root '{project_root}' is not a valid directory. Aborting security analysis."
            )
    ...
    def _check_tool_availability_on_init(self):
        ...
        for tool, command in tool_checks.items():
            ...
            if not tool_path:
                ...
                logger.critical(
                    f"CRITICAL: {tool} tool is NOT available in PATH.",
                    extra={"component": tool, "status": "Unavailable"},
                )
                continue
            try:
                ...
                if required_versions.get(tool) and required_versions[tool] not in version:
                    ...
                    logger.info(
                        f"{tool} tool is available (version: {version}).",
                        extra={"component": tool, "status": "Available"},
                    )
            except subprocess.TimeoutExpired:
                tools_missing.append(f"{tool} (Error: Timeout)")
                logger.critical(
                    f"CRITICAL: {tool} tool timed out during availability check.",
                    extra={"component": tool, "status": "Unavailable"},
                )
            except Exception as e:
                tools_missing.append(f"{tool} (Error: {e})")
                logger.critical(
                    f"CRITICAL: {tool} tool check failed. {e}",
                    extra={"component": tool, "status": "Unavailable"},
                )
        if tools_missing:
            logger.critical(
                f"CRITICAL: Required security analysis tools are missing or incorrect: {', '.join(tools_missing)}. Aborting startup."
            )
            alert_operator(
    ...
    def _run_subprocess_safely(self, command: List[str], description: str) -> Tuple[bool, str]:
        ...
        if not tool_path:
            logger.error(
                f"Security tool '{command[0]}' not found in PATH for {description}.",
        ...
        try:
            ...
            logger.error(
                f"{description} timed out after {300} seconds.",
                extra={"command": " ".join(sanitized_command)},
            )
            alert_operator(
            ...
            alert_operator(
                f"CRITICAL: Unexpected error during {description}: {e}.",
                level="CRITICAL",
            )
    ...
    def _run_bandit(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise SecurityAnalysisError(
                f"Unexpected error running Bandit: {e}",
                tool="Bandit",
                original_exception=e,
            )
    ...
    def _run_pip_audit(self) -> List[Dict[str, Any]]:
        ...
        try:
            success, output = self._run_subprocess_safely(
                ["pip-audit", "--json"], description="pip-audit dependency scan"
            )
            if not success:
                try:
                    ...
                    if pip_audit_output.get("vulnerabilities"):
                        logger.info(
                            "pip-audit found vulnerabilities (non-zero exit code but valid results)."
                        )
                        return pip_audit_output.get("vulnerabilities", [])
                except json.JSONDecodeError:
                    raise SecurityAnalysisError(
                        f"pip-audit scan failed: {output}", tool="pip-audit"
                    )
            pip_audit_output = json.loads(output)
            ...
            logger.error(
                f"Failed to parse pip-audit JSON output: {e}. Raw output: {output[:500]}..."
            )
            raise SecurityAnalysisError(
            ...
            raise SecurityAnalysisError(
                f"Unexpected error running pip-audit: {e}",
                tool="pip-audit",
                original_exception=e,
            )
        ...
        try:
            ...
            if not success:
                try:
                    ...
                    logger.info(
                        f"Snyk found {len(vulnerabilities)} vulnerabilities (non-zero exit code but valid results)."
                    )
                    return vulnerabilities
                except json.JSONDecodeError:
            ...
            raise SecurityAnalysisError(
                f"Unexpected error running Snyk: {e}", tool="Snyk", original_exception=e
            )
    ...
    async def perform_security_scan(self) -> Dict[str, Any]:
        ...
        if REDIS_CLIENT:
            try:
                ...
                if cached_results:
                    ...
                    audit_logger.log_event(
                        "security_scan_cache_hit", project_root=self.project_root
                    )
        ...
        bandit_issues, pip_audit_vulnerabilities, snyk_vulnerabilities = (
            await self._run_tools_in_parallel()
        )
        try:
            if bandit_issues:
                ...
                if critical_bandit_issues:
                    alert_operator(
                        f"CRITICAL: Bandit detected {len(critical_bandit_issues)} HIGH/CRITICAL issues in {self.project_root}.",
                        level="CRITICAL",
                    )
                    audit_logger.log_event(
                        "critical_bandit_issues",
                        project_root=self.project_root,
                        issues=_deep_scrub(critical_bandit_issues),
                    )
            if pip_audit_vulnerabilities:
                ...
                logger.warning(
                    f"pip-audit detected {len(pip_audit_vulnerabilities)} vulnerabilities."
                )
                critical_pip_audit_vulns = [
                    vuln
                    for vuln in pip_audit_vulnerabilities
                    if vuln.get("vulnerability_id", "").startswith("CVE-")
                ]
                if critical_pip_audit_vulns:
                    alert_operator(
                        f"CRITICAL: pip-audit detected {len(critical_pip_audit_vulns)} CVEs in dependencies for {self.project_root}.",
                        level="CRITICAL",
                    )
                    audit_logger.log_event(
                        "critical_pip_audit_vulns",
                        project_root=self.project_root,
                        vulns=_deep_scrub(critical_pip_audit_vulns),
                    )
            if snyk_vulnerabilities:
                ...
                if critical_snyk_vulns:
                    alert_operator(
                        f"CRITICAL: Snyk detected {len(critical_snyk_vulns)} HIGH/CRITICAL vulnerabilities in {self.project_root}.",
                        level="CRITICAL",
                    )
                    audit_logger.log_event(
                        "critical_snyk_vulns",
                        project_root=self.project_root,
                        vulns=_deep_scrub(critical_snyk_vulns),
                    )
            if results["overall_status"] == "FAIL":
                alert_operator(
                    f"CRITICAL: Security scan for {self.project_root} FAILED. Issues detected.",
                    level="CRITICAL",
                )
                audit_logger.log_event(
            ...
            logger.critical(
                f"CRITICAL: Security scan aborted due to tool failure: {e}",
                exc_info=True,
            )
            alert_operator(
                f"CRITICAL: Security scan for {self.project_root} ABORTED due to tool failure: {e.tool}: {e}.",
                level="CRITICAL",
            )
            results["overall_status"] = "ABORTED"
            results["summary"] = f"Scan aborted due to tool failure: {e.tool}"
            audit_logger.log_event(
                "security_scan_aborted",
                project_root=self.project_root,
                error=str(e),
                tool=e.tool,
            )
            ...
            audit_logger.log_event(
                "security_scan_error", project_root=self.project_root, error=str(e)
            )
        return results
    ...
    def security_health_check(self, check_only: bool = False) -> bool:
        ...
        for tool, command in tool_checks.items():
            ...
            if not tool_path:
                ...
                logger.error(
                    f"{tool} is NOT available in PATH.",
                    extra={"component": tool, "status": "Unavailable"},
                )
                continue
            try:
                ...
                version = result.stdout.strip()
                logger.info(
                    f"{tool} is available (version: {version}).",
                    extra={"component": tool, "status": "Available"},
                )
                ...
                health_status["overall_healthy"] = False
                logger.error(
                    f"{tool} timed out during availability check.",
                    extra={"component": tool, "status": "Unavailable"},
                )
            except Exception as e:
                health_status[f"{tool.lower().replace('-', '_')}_available"] = False
                health_status["overall_healthy"] = False
                logger.error(
                    f"{tool} check failed with unexpected error: {e}.",
                    extra={"component": tool, "status": "Unavailable"},
                )
        if not health_status["overall_healthy"]:
            logger.critical(
                "CRITICAL: Security health check failed. Some security analysis tools are missing or unhealthy."
            )
            audit_logger.log_event(
            ...
            alert_operator(
                "CRITICAL: Security health check failed. Some security analysis tools are missing.",
                level="CRITICAL",
            )
            if not check_only:
                raise RuntimeError("[CRITICAL][SECURITY] Security health check failed")
        logger.info(
            f"Security component health check complete. Overall healthy: {health_status['overall_healthy']}"
        )
        audit_logger.log_event(
            "security_health_check_complete",
            project_root=self.project_root,
...
if __name__ == "__main__":
    ...
    is_healthy_only = asyncio.run(
        security_health_check(project_root=test_project_root, check_only=True)
    )
    print(f"Security components healthy (check_only): {is_healthy_only}")

    print("\n--- Running Full Security Scan ---")
    overall_scan_status = asyncio.run(
        security_health_check(project_root=test_project_root, check_only=False)
    )
