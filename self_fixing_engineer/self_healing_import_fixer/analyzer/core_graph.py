...
class ImportGraphAnalyzer:
    ...
    def __init__(self, project_root: str, config: Optional[Dict[str, Any]] = None):
        ...
        if not os.path.isdir(self.project_root):
            raise AnalyzerCriticalError(
                f"Project root '{self.project_root}' is not a valid directory. Aborting graph analysis."
            )
        ...
        if not any(self.project_root.startswith(wp) for wp in self.whitelisted_paths):
            raise AnalyzerCriticalError(
                f"Project root '{self.project_root}' is not within whitelisted paths: {self.whitelisted_paths}. Aborting graph analysis."
            )
        ...
        try:
            ...
            logger.warning(
                f"Could not check read access on {self.project_root}: {e}. Proceeding but be aware of potential risks."
            )
        ...
        audit_logger.log_event(
            ...
            max_files=self.max_python_files,
    ...
    def _set_memory_limit(self):
        ...
        if resource is not None and self.max_memory_mb > 0:
            try:
                ...
                logger.warning(
                    f"Failed to set memory limit: {e}. Graph analysis might consume more memory than desired.",
                    exc_info=True,
                )
                alert_operator(
                    f"WARNING: Graph Analyzer failed to set memory limit: {e}. Monitor resource usage.",
                    level="WARNING",
                )
        else:
            logger.info(
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    if len(python_files) > self.max_python_files:
                        logger.warning(
                            f"Max Python files limit ({self.max_python_files}) reached. Skipping remaining files."
                        )
                        from .core_audit import audit_logger

                        audit_logger.log_event(
                            "graph_analysis_limit_reached",
                            limit_type="max_python_files",
                            limit_value=self.max_python_files,
                        )
                        alert_operator(
                            f"WARNING: Graph Analyzer hit max Python files limit ({self.max_python_files}). Analysis may be incomplete.",
                            level="WARNING",
                        )
                        return python_files
                ...
                if isinstance(node, ast.Import):
                    ...
                    if node.module:
                        if node.level > 0:
                            ...
                            full_module_name = (
                                ".".join(base_module_parts + [node.module])
                                if node.module
                                else ".".join(base_module_parts)
                            )
        ...
        if redis_client:
            try:
                await redis_client.setex(
                    cache_key, 86400, json.dumps(list(imported_modules))
                )  # Cache for 24 hours
    ...
    def build_graph(self) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to find all Python files due to permissions: {e}. Aborting graph build."
            )
            alert_operator(
                f"CRITICAL: Graph Analyzer: File system permission error during scan: {e}. Aborting.",
                level="CRITICAL",
            )
        ...
        for i, result in enumerate(parsing_results):
            ...
            for imported_module_name in imports:
                if imported_module_name in self.module_paths or any(
                    ...
                    logger.debug(
                        f"Skipping external/unresolved import: {module_name} -> {imported_module_name}"
                    )

            if len(self.graph) > self.max_graph_nodes:
                logger.warning(
                    f"Max graph nodes limit ({self.max_graph_nodes}) reached. Stopping graph construction."
                )
                audit_logger.log_event(
                    "graph_analysis_limit_reached",
                    limit_type="max_graph_nodes",
                    limit_value=self.max_graph_nodes,
                )
                alert_operator(
                    f"WARNING: Graph Analyzer hit max graph nodes limit ({self.max_graph_nodes}). Analysis may be incomplete.",
                    level="WARNING",
                )
                break
        ...
        if python_files:
            ...
            if error_rate > self.parsing_error_threshold:
                if PRODUCTION_MODE:
                    raise AnalyzerCriticalError(
                        f"High parsing error rate ({error_rate:.2%}) detected in graph analysis. Aborting due to potentially corrupted codebase or severe parsing issues."
                    )
                else:
                    logger.warning(
                        f"Parsing errors in {len(self.syntax_error_files)} files ({error_rate:.2%}), continuing with partial graph (non-production mode)."
                    )
                audit_logger.log_event(
                    "graph_parsing_high_error_rate",
                    error_rate=error_rate,
                    total_files=len(python_files),
                    failed_files=len(self.syntax_error_files),
                )
                alert_operator(
                    f"CRITICAL: Graph Analyzer: High parsing error rate ({error_rate:.2%}). Aborting in production.",
                    level="CRITICAL",
                )

        logger.info(
            f"Import graph built with {len(self.graph)} nodes and {sum(len(v) for v in self.graph.values())} edges."
        )
        audit_logger.log_event(
                ...
                if neighbor not in visited:
                    ...
                    try:
                        ...
                        audit_logger.log_event(
                            ...
                            cycle=scrub_secrets(list(cycle)),
                            project_root=self.project_root,
                        )
                    except ValueError:
    ...
    def detect_dead_nodes(
        ...
        for node in dead_nodes:
            audit_logger.log_event(
                "dead_node_detected", module=node, project_root=self.project_root
            )

        logger.info(
            f"Dead code detection complete. Found {len(dead_nodes)} potentially dead nodes."
        )
        return dead_nodes
    ...
    def visualize_graph(
        ...
        if not allow_graphviz_spawn and PRODUCTION_MODE:
            logger.warning(
                "Graphviz rendering is disabled in PRODUCTION_MODE unless 'allow_graphviz_spawn' is explicitly True in config. Skipping visualization."
            )
            audit_logger.log_event(
                "graph_visualization_skipped",
                reason="graphviz_spawn_disabled_in_prod",
                project_root=self.project_root,
            )
            alert_operator(
                "WARNING: Graphviz rendering skipped in PRODUCTION_MODE. Set 'allow_graphviz_spawn=True' in config to enable.",
                level="WARNING",
            )
            return
        ...
        if not shutil.which("dot"):
            logger.error(
                "Graphviz 'dot' command not found. Please install Graphviz to enable visualization."
            )
            audit_logger.log_event(
                "graph_visualization_failed",
                reason="graphviz_not_found",
                project_root=self.project_root,
            )
            alert_operator(
                "ERROR: Graphviz 'dot' command not found. Graph visualization disabled.",
                level="ERROR",
            )
            return
        ...
        try:
            ...
            alert_operator(
                f"ERROR: Failed to render graph visualization for {self.project_root}. Error: {e}",
                level="ERROR",
            )
...
if __name__ == "__main__":
    ...
    try:
        print(
            f"Attempting to build graph with {num_bad_files}/{num_total_files} bad files ({num_bad_files/num_total_files:.2%})..."
        )
