...
try:
    ...
    try:
        from .core_utils import alert_operator
        alert_operator(f"CRITICAL: Policy management missing core dependency: {e}. Aborting.", level="CRITICAL")
    except Exception:
        pass
    ...
    if _policy_hmac_key is None:
        ...
        if not key_str:
            if PRODUCTION_MODE:
                ...
                warnings.warn("ANALYZER_POLICY_HMAC_KEY_ENV not set. Generated a random key for policy file signing. THIS IS INSECURE FOR PRODUCTION.")
                alert_operator("WARNING: Policy HMAC key not set. Using insecure random key. IMMEDIATE ACTION REQUIRED.", level="WARNING")
        else:
...
class PolicyRule(BaseModel):
    ...
    type: Literal["import_restriction", "dependency_limit", "cycle_prevention", "naming_convention"] = Field(..., description="Type of policy rule.")
    severity: Literal["low", "medium", "high", "critical"] = Field("medium", description="Severity of violation.")
    allow_imports: Optional[List[str]] = Field(None, description="List of allowed top-level imports (regex or module name).")
    deny_imports: Optional[List[str]] = Field(None, description="List of denied top-level imports (regex or module name).")
    target_modules: Optional[List[str]] = Field(None, description="Modules to which this rule applies (regex or module name).")
    max_dependencies: Optional[int] = Field(None, ge=0, description="Maximum allowed direct dependencies for a module.")
    pattern: Optional[str] = Field(None, description="Regex pattern for naming convention.")
    apply_to: Optional[Literal["modules", "functions", "classes"]] = Field(None, description="Apply naming convention to modules, functions, or classes.")

    @validator('allow_imports', 'deny_imports', 'target_modules', 'pattern', pre=True, each_item=False)
...
def _validate_and_apply(policy_data_bytes: bytes, expect_hmac: bool) -> ArchitecturalPolicy:
    ...
    calculated_signature = hmac.new(_get_policy_hmac_key(), policy_content_str, hashlib.sha256).hexdigest()
    ...
    policies.policies = [PolicyRule(**rule) if not isinstance(rule, PolicyRule) else rule for rule in policies.policies]
    ...
    while not _watcher_stop_event.is_set():
        try:
            ...
            if last_mtime is None:
                ...
                logger.info(f"Detected change in policy file {policy_file_path}, attempting reload.")
            ...
            alert_operator(f"ERROR: Policy hot-reload failed: {e}. Keeping last valid version.", level="ERROR")
...
def start_policy_watcher(policy_file_path: str, poll_interval: float = 10.0):
    ...
    if _watcher_thread is None:
        _watcher_thread = threading.Thread(
            target=_watch_loop, 
            args=(policy_file_path, poll_interval), 
            daemon=True
        )
...
# --- Policy Manager ---
class PolicyManager:
    def __init__(self, policy_file_path: str, enable_hot_reload: bool = True, reload_poll_interval: float = 10.0):
        from .core_audit import audit_logger
        self.policy_file_path = policy_file_path
        self._redis_cache_key = f"policy:{hashlib.sha256(policy_file_path.encode('utf-8')).hexdigest()}"
        self._load_policies_sync()
        logger.info(f"PolicyManager initialized with policies from: {self.policy_file_path}")
        audit_logger.log_event("policy_manager_init", policy_file=self.policy_file_path,
        ...
        if redis_client:
            try:
                ...
                if cached_policy_bytes:
                    ...
                    audit_logger.log_event("policy_load_cache_hit", policy_file=self.policy_file_path)
            except Exception as e:
        ...
        if policy_data_bytes is None:
            try:
                if s3_bucket and PRODUCTION_MODE:
                    s3 = boto3.client("s3")
                    response = await asyncio.to_thread(s3.get_object, Bucket=s3_bucket, Key=self.policy_file_path)
                ...
                raise AnalyzerCriticalError(f"S3 ClientError for policy file '{self.policy_file_path}': {e.response['Error']['Message']}.")
            except Exception as e:
    ...
    def check_architectural_policies(self,
                                     ...
                                     dead_nodes: Set[str]) -> List[PolicyViolation]:
        with _policy_lock:
            ...
            from .core_audit import audit_logger
            audit_logger.log_event("policy_enforcement_start", policy_version=_last_good_policy.version)
            ...
            for rule in _last_good_policy.policies:
                try:
                    ...
                    alert_operator(f"CRITICAL: Failed to enforce policy rule {rule.id} due to an internal error: {e}. Check logs.", level="CRITICAL")

            if violations:
                logger.warning(f"Policy enforcement complete. Found {len(violations)} violations.")
                for violation in violations:
                    violation_extra = {k: v for k, v in violation.model_dump().items() if k not in ('message', 'levelname', 'levelno', 'name', 'msg', 'args')}
                    audit_logger.log_event("policy_violation_detailed", **scrub_secrets(violation.model_dump()))
                    if violation.severity in ["high", "critical"]:
                        logger.error(f"CRITICAL POLICY VIOLATION: {violation.rule_name} - {violation.message}", extra=violation_extra)
                        alert_operator(f"CRITICAL POLICY VIOLATION: {violation.rule_name} (Severity: {violation.severity}) in {violation.offending_item}: {violation.message}", level="CRITICAL")
                    else:
                        logger.warning(f"POLICY VIOLATION: {violation.rule_name} - {violation.message}", extra=violation_extra)
            else:
    ...
    def _enforce_import_restriction(self, rule: PolicyRule, code_graph: Dict[str, Set[str]]) -> List[PolicyViolation]:
        violations = []
        ...
        for importer_module, imported_modules in code_graph.items():
            ...
            for imported_module in imported_modules:
                ...
                is_allowed = allow_imports_regex and any(pattern.match(imported_module) for pattern in allow_imports_regex)
                if is_denied and not is_allowed:
                    violations.append(PolicyViolation(
                        ...
                        details={"importer": importer_module, "imported": imported_module, "reason": "denied_import"}
                    ))
        return violations

    def _enforce_dependency_limit(self, rule: PolicyRule, code_graph: Dict[str, Set[str]]) -> List[PolicyViolation]:
        violations = []
        if rule.max_dependencies is None:
            logger.warning(f"Rule {rule.id}: max_dependencies is not set for dependency_limit rule. Skipping.")
            return []
        ...
        for module, dependencies in code_graph.items():
            ...
            if num_dependencies > rule.max_dependencies:
                violations.append(PolicyViolation(
                    ...
                    details={"module": module, "num_dependencies": num_dependencies, "limit": rule.max_dependencies}
                ))
        return violations

    def _enforce_cycle_prevention(self, rule: PolicyRule, detected_cycles: List[List[str]]) -> List[PolicyViolation]:
        violations = []
        if detected_cycles:
    ...
    def _enforce_naming_convention(self, rule: PolicyRule, module_paths: Dict[str, str]) -> List[PolicyViolation]:
        violations = []
        if not rule.pattern or not rule.apply_to:
            logger.warning(f"Rule {rule.id}: Naming convention rule missing pattern or apply_to. Skipping.")
            return []
            ...
            if rule.apply_to == "modules":
                if not naming_pattern.match(module_name):
                    violations.append(PolicyViolation(
                        ...
                        details={"item_type": "module", "name": module_name, "pattern": rule.pattern}
                    ))
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        if len(file_content) > 1024 * 1024:
                            logger.warning(f"Skipping naming convention check for {file_path}: file too large (>1MB).")
                            continue
                        tree = None
                        try:
                            import ast
                            ...
                            logger.warning(f"Syntax error or AST parsing error in {file_path}: {e}. Skipping naming convention check.")
                            continue
                            
                    for node in ast.walk(tree):
                        if (rule.apply_to == "functions" and getattr(node, "name", None) and isinstance(node, ast.FunctionDef)) or \
                           (rule.apply_to == "classes" and getattr(node, "name", None) and isinstance(node, ast.ClassDef)):
                            if not naming_pattern.match(node.name):
                                violations.append(PolicyViolation(
                                    ...
                                    details={"item_type": rule.apply_to, "name": node.name, "module": module_name, "pattern": rule.pattern}
                                ))
                except SyntaxError as e:
                    logger.warning(f"Syntax error in {file_path} for naming convention check: {e}. Skipping AST parsing.")
                except Exception as e:
                    logger.error(f"Error parsing {file_path} for naming convention check: {e}", exc_info=True)
        return violations