...
class AlertConfig:
    ...
    pagerduty_routing_key: Optional[str] = field(default_factory=lambda: os.getenv("PAGERDUTY_ROUTING_KEY"))
    ...
    webhook_urls: List[str] = field(default_factory=lambda: os.getenv("WEBHOOK_URLS", "").split(","))
...
if PROMETHEUS_AVAILABLE:
    ...
    operation_histogram = Histogram('analyzer_operation_duration_seconds', 'Operation duration', ['operation'])
...
def alert_operator(message: str, level: Union[str, AlertLevel] = AlertLevel.ERROR,
...
def scrub_secrets(data: Any, max_depth: int = 10) -> Any:
    ...
    sensitive_keys = {
        ...
        'refresh_token', 'bearer', 'session', 'cookie', 'jwt',
        'aws_access_key_id', 'aws_secret_access_key', 'database_url',
        'connection_string', 'dsn', 'encryption_key', 'salt', 'hash'
    }
        ...
        if field in data:
            ...
            if 'min_length' in requirements and isinstance(value, str):
                if len(value) < requirements['min_length']:
                    return False, f"Field '{field}' must be at least {requirements['min_length']} characters"
            
            if 'max_length' in requirements and isinstance(value, str):
                if len(value) > requirements['max_length']:
                    return False, f"Field '{field}' must be at most {requirements['max_length']} characters"
            ...
            if 'enum' in requirements:
                if value not in requirements['enum']:
                    return False, f"Field '{field}' must be one of {requirements['enum']}"
