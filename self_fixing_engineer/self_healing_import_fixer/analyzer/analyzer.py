...
try:
    ...
    try:
        ...
        from .core_utils import alert_operator
        alert_operator(f"CRITICAL: Analyzer missing core dependency: {e}. Aborting.", level="CRITICAL")
    except Exception:
        pass
...
class AnalyzerConfig(BaseModel):
    ...
    def validate_paths(cls, config_data):
        ...
        allowed_roots = [ar for ar in os.environ.get("ANALYZER_PROJECT_ALLOWLIST", "").split(":") if ar]
        if allowed_roots and not any(root.startswith(os.path.abspath(ar)) for ar in allowed_roots):
            raise AnalyzerCriticalError(f"Project root {root} not in allowed project allowlist {allowed_roots}.")
        if ".." in root or os.path.islink(root) or not os.path.isdir(root):
            raise AnalyzerCriticalError("Invalid project_root: must be a real directory, no symlinks/traversal.")
        if config_data.get("policy_rules_file"):
            ...
            if ".." in pf or os.path.islink(pf) or not os.path.isfile(pf):
                raise AnalyzerCriticalError("Invalid policy_rules_file: must be an existing file, no symlinks/traversal.")
        return config_data
    ...
    try:
        if PRODUCTION_MODE:
            ...
            try:
                ...
                logger.error(f"CONFIG LOAD FAILURE: AWS SSM unavailable or misconfigured: {e}", exc_info=True)
            ...
            if not os.path.exists(config_path):
                ...
                raise AnalyzerCriticalError(f"Configuration file not found at: {config_path}. Aborting startup.")
            with open(config_path, 'r', encoding='utf-8') as f:
                if config_path.endswith('.json'):
                    ...
                    logger.error(f"CONFIG LOAD FAILURE: Unsupported format: {config_path}", exc_info=True)
                    raise AnalyzerCriticalError(f"Unsupported configuration file format: {config_path}. Must be .json or .yaml. Aborting startup.")

        config_data = AnalyzerConfig.validate_paths(config_data)
        config = AnalyzerConfig(**config_data)
        logger.info("Configuration loaded successfully.")
        audit_logger.log_event("config_loaded", path=config_path, config_summary=list(config_data.keys()))
        if PRODUCTION_MODE and "mock" in config.llm_endpoint:
            raise AnalyzerCriticalError("Mock LLM endpoint detected in PRODUCTION_MODE. Aborting for security.")
        return config
    except (ClientError, json.JSONDecodeError, yaml.YAMLError, ValidationError) as e:
        logger.error(f"CONFIG LOAD FAILURE. Check file format, permissions, and AWS SSM configuration: {e}", exc_info=True)
        raise AnalyzerCriticalError(f"Failed to load/validate configuration: {e}. Aborting startup.")
    except Exception as e:
        logger.error(f"Unexpected error loading configuration: {e}", exc_info=True)
        raise AnalyzerCriticalError(f"Unexpected error loading configuration: {e}. Aborting startup.")

def validate_output_dir(output_dir: str, project_root: str):
    abs_out = os.path.abspath(output_dir)
    abs_root = os.path.abspath(project_root)
    if not abs_out.startswith(abs_root):
        raise AnalyzerCriticalError(f"Output directory {abs_out} must be within project root {abs_root}.")
    if os.path.islink(abs_out) or ".." in abs_out:
        raise AnalyzerCriticalError(f"Output directory {abs_out} cannot be a symlink or contain traversal.")
    try:
...
def _handle_analyze(project_root, app_config, output_dir):
    ...
    try:
        ...
        logger.info(f"Graph analysis complete. Cycles found: {len(cycles)}, Dead nodes: {len(dead_nodes)}")
        audit_logger.log_event("graph_analysis_complete", project_root=project_root, cycles_found=len(cycles), dead_nodes_found=len(dead_nodes))
        report_generator = ReportGenerator(output_dir)
        report_path = report_generator.generate_report(
            report_name="graph_analysis_report",
            results={"cycles": cycles, "dead_nodes": dead_nodes, "graph_summary": "Graph built successfully."}
        )
...
def _handle_check_policy(project_root, app_config, output_dir):
    from .core_audit import audit_logger
    try:
        audit_logger.log_event("action_check_policy_start", project_root=project_root, policy_file=app_config.policy_rules_file)
        logger.info(f"Checking architectural policies for {project_root}...")
        if not app_config.policy_rules_file:
            raise AnalyzerCriticalError("Policy rules file not specified in configuration for policy check.")
        ...
        audit_logger.log_event("policy_check_complete", project_root=project_root, violations_found=len(policy_violations), details=[v.model_dump() for v in policy_violations])
        if policy_violations:
            critical_violations = [v for v in policy_violations if v.severity in ["high", "critical"]]
            if critical_violations:
                alert_operator(f"CRITICAL: Policy check FAILED for {project_root}. {len(critical_violations)} critical violations found.", level="CRITICAL")
                raise AnalyzerCriticalError("Critical policy violations detected.")
            else:
                logger.warning(f"Policy check completed with {len(policy_violations)} non-critical violations for {project_root}.")
        report_generator = ReportGenerator(output_dir)
        report_path = report_generator.generate_report(
...
def _handle_security_scan(project_root, app_config, output_dir):
    ...
    try:
        ...
        logger.info(f"Security scan complete. Overall status: {scan_results.get('overall_status','UNKNOWN')}")
        audit_logger.log_event("security_scan_complete", project_root=project_root, status=scan_results.get("overall_status","UNKNOWN"), summary=scan_results.get("summary"))
        if scan_results.get("overall_status") == "FAIL":
            alert_operator(f"CRITICAL: Security scan FAILED for {project_root}. Issues detected.", level="CRITICAL")
            raise AnalyzerCriticalError("Security scan failed with detected issues.")
        report_generator = ReportGenerator(output_dir)
        report_path = report_generator.generate_report(
            report_name="security_scan_report",
...
def _handle_suggest_patch(project_root, app_config, output_dir, dry_run):
    from .core_audit import audit_logger
    try:
        audit_logger.log_event("action_suggest_patch_start", project_root=project_root, dry_run=dry_run)
        ...
        logger.info(f"AI suggestions and patches generated. Suggestions: {len(suggestions)}, Patches: {len(patches)}")
        audit_logger.log_event("ai_suggestions_patches_generated", project_root=project_root, suggestions_count=len(suggestions), patches_count=len(patches))
        if not suggestions and not patches:
            alert_operator(f"WARNING: AI suggestions/patches generation failed for {project_root}. No output.", level="WARNING")
        if not dry_run:
            logger.info("Patch application is not supported in production. Aborting apply.")
            raise AnalyzerCriticalError("Patch application is not implemented or is disabled for safety in production.")
        report_generator = ReportGenerator(output_dir)
        report_path = report_generator.generate_report(
    ...
    try:
        ...
        async def check_components():
            tasks = [
                ...
                asyncio.to_thread(lambda: PolicyManager(app_config.policy_rules_file) if app_config.policy_rules_file else True),
                asyncio.to_thread(lambda: SecurityAnalyzer(project_root).security_health_check(check_only=True)),
                asyncio.to_thread(lambda: get_ai_suggestions("health check", config=app_config.ai_config)),
            ]
            results = await asyncio.gather(*tasks, return_exceptions=True)
            return results

        results = await check_components()
        component_names = ["graph_analyzer", "policy_manager", "security_analyzer", "ai_manager"]
        for name, result in zip(component_names, results):
            if isinstance(result, Exception):
                ...
                alert_operator(f"CRITICAL: Analyzer {name.replace('_', ' ').title()} component unhealthy: {result}.", level="CRITICAL")
            else:
        ...
        audit_logger.log_event("health_check_complete", overall_status=overall_status, **health_statuses)
        if not overall_status:
            alert_operator("CRITICAL: Overall Analyzer health check FAILED. Some components are unhealthy.", level="CRITICAL")
...
@click.command()
@click.argument("action", type=click.Choice(["analyze", "check-policy", "security-scan", "suggest-patch", "health-check"]))
@click.option("--path", default=".", help="Path to the codebase to analyze (default: current directory).")
@click.option("--config", default="config.yaml", help="Path to the configuration file (default: config.yaml).")
...
@click.option("--dry-run", is_flag=True, help="Simulate execution without making changes or generating final reports.")
def main(action, path, config, output_dir, verbose, production_mode, dry_run):
    ...
    audit_logger.log_event("action_metadata", action=action, project_root=path, config=scrub_secrets(app_config.model_dump()))

    # Mandatory CLI Input Validation
    project_root = os.path.abspath(path)
    if not os.path.isdir(project_root):
        logger.critical(f"Project root path '{project_root}' is not a valid directory. Aborting startup.")
        logger.error("For directory errors: ensure the path exists and permissions are correct, and not a symlink/traversal.")
    ...
    audit_logger.log_event("analyzer_end", action=action, status="success", duration_seconds=duration, end_time=end_time.isoformat() + "Z")
...
if __name__ == "__main__":
    try:
        ...
        from .core_audit import audit_logger
        audit_logger.log_event("analyzer_execution_failure", action="startup", error=str(e), end_time=datetime.datetime.utcnow().isoformat() + "Z")
        sys.exit(1)