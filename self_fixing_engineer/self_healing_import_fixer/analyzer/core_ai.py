...
from openai import AsyncOpenAI, APIError, RateLimitError  # Production-grade LLM client with specific error handling
import hashlib
import uuid
import re
import warnings
...
try:
    ...
    try:
        ...
        alert_operator(f"CRITICAL: AI features missing core dependency: {e}. Aborting.", level="CRITICAL")
    except Exception:
        pass
...
class AIManager:
    ...
    def __init__(self, config: Dict[str, Any], trace_id: Optional[str] = None):
        ...
        if PRODUCTION_MODE:
            if not self.config.get("llm_endpoint") or not self.config.get("llm_endpoint").startswith("https://"):
                raise RuntimeError("[CRITICAL][AI] LLM endpoint must use HTTPS in production.")
            if not self.config.get("proxy_url"):
                raise RuntimeError("[CRITICAL][AI] In PRODUCTION_MODE, 'proxy_url' for LLM traffic is required but not configured.")
            if self.config.get("allow_auto_apply_patches", False):
                raise RuntimeError("[CRITICAL][AI] 'allow_auto_apply_patches' is enabled in PRODUCTION_MODE. This is forbidden.")
        ...
        if not self.llm_api_key:
            raise RuntimeError("[CRITICAL][AI] LLM API key is not configured. AI features cannot function.")
        
        self.llm_endpoint = self.config.get("llm_endpoint")
        if not self.llm_endpoint:
            raise RuntimeError("[CRITICAL][AI] LLM API endpoint is not configured. AI features cannot function.")
        ...
        logger.info(f"AIManager initialized for model: {self.model_name}, endpoint: {self.llm_endpoint}, trace_id: {self.trace_id}")
        audit_logger.log_event(
            "ai_manager_init", model=self.model_name, endpoint=self.llm_endpoint,
            concurrency_limit=self.api_concurrency_limit, token_quota=self.token_quota_per_minute,
            proxy_configured=bool(self.proxy_url), auto_apply_patches_allowed=self.allow_auto_apply_patches,
    ...
    def _sanitize_prompt(self, prompt: str) -> str:
        ...
        # Long prompt
        if len(prompt) > 4096:
            logger.warning("Prompt length is unusually long (>{} chars). Truncating for safety.".format(4096))
            prompt = prompt[:4096]
        ...
        if prompt.count('```') % 2 != 0:
            logger.warning("Unmatched code block delimiter '```' in prompt. Potential injection attempt.")
    ...
    async def _enforce_token_quota(self, tokens_to_use: int, timeout: float = 30.0):
        ...
        async with self._token_usage_lock:
            ...
            if current_minute_usage + tokens_to_use > self.token_quota_per_minute:
                oldest_timestamp = self._token_usage_history[0][0] if self._token_usage_history else current_time - 60
                wait_until = oldest_timestamp + 60
                sleep_duration = max(0, wait_until - current_time) + 1
                if sleep_duration > timeout:
                    logger.critical(f"CRITICAL: LLM token quota would require waiting {sleep_duration:.2f}s (>{timeout}s max). Aborting API call.")
                    audit_logger.log_event("llm_token_quota_timeout_abort", current_usage=current_minute_usage, requested_tokens=tokens_to_use, trace_id=self.trace_id)
                    alert_operator(f"CRITICAL: LLM token quota overrun; would require excessive wait. Aborting API call.", level="CRITICAL")
                    raise RuntimeError("LLM token quota overrun. Aborting API call (wait too long).")
                logger.warning(f"LLM token quota exceeded. Waiting for {sleep_duration:.2f}s.", extra={'current_usage': current_minute_usage, 'requested_tokens': tokens_to_use})
                audit_logger.log_event("llm_token_quota_exceeded", current_usage=current_minute_usage, requested_tokens=tokens_to_use, wait_time=sleep_duration, trace_id=self.trace_id)
                alert_operator(f"WARNING: LLM token quota exceeded. Waiting {sleep_duration:.2f}s. Current usage: {current_minute_usage}", level="WARNING")
                await asyncio.sleep(sleep_duration)
                self._token_usage_history = [
                    (ts, tokens) for ts, tokens in self._token_usage_history if time.time() - ts < 60
                ]
                current_minute_usage = sum(tokens for _, tokens in self._token_usage_history)
                if current_minute_usage + tokens_to_use > self.token_quota_per_minute:
                    logger.critical(f"CRITICAL: LLM token quota overrun after waiting. Aborting API call.", extra={'current_usage': current_minute_usage, 'requested_tokens': tokens_to_use})
                    audit_logger.log_event("llm_token_quota_overrun_abort", current_usage=current_minute_usage, requested_tokens=tokens_to_use, trace_id=self.trace_id)
    ...
    async def _call_llm_api(self, prompt: str, trace_id: Optional[str] = None) -> Optional[str]:
        ...
        if REDIS_CLIENT:
            try:
                ...
                if cached_response:
                    ...
                    audit_logger.log_event("llm_api_cache_hit", cache_key=cache_key, trace_id=trace_id)
                    return cached_response
            except Exception as e:
                logger.warning(f"Redis cache unavailable for get: {e}")
        logger.debug(f"Sending prompt to LLM (first 100 chars): {prompt[:100]}...")
        audit_logger.log_event("llm_api_call_start", model=self.model_name, endpoint=self.llm_endpoint,
                               prompt_length=len(prompt), max_tokens_requested=self.max_tokens, trace_id=trace_id)
        audit_logger.log_event("llm_api_call_input", prompt=scrub_secrets(prompt[:500]), trace_id=trace_id)
        ...
        try:
            async with self._api_semaphore:
                ...
                response_content = response.choices[0].message.content.strip() if response.choices else None
                if response_content:
                    logger.debug("Successfully received response from LLM.")
                    audit_logger.log_event("llm_api_call_success", model=self.model_name, response_length=len(response_content),
                                           tokens_used=getattr(response.usage, "total_tokens", None), trace_id=trace_id)
                    audit_logger.log_event("llm_api_call_output", response=scrub_secrets(response_content[:500]), trace_id=trace_id)
                    if REDIS_CLIENT:
                        try:
                            await REDIS_CLIENT.setex(cache_key, 3600, response_content)
                            audit_logger.log_event("llm_api_cache_set", cache_key=cache_key, trace_id=trace_id)
                        except Exception as e:
            ...
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type='RateLimitError', error_message=str(e), trace_id=trace_id)
            raise NonCriticalError(f"LLM rate limit exceeded: {e}")
        except APIError as e:
            logger.error(f"LLM API error: {e}")
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type='APIError', error_message=str(e), trace_id=trace_id)
            raise NonCriticalError(f"LLM API error: {e}")
        except Exception as e:
            logger.error(f"An error occurred during LLM API call: {e}", exc_info=True)
            audit_logger.log_event("llm_api_call_failure", model=self.model_name, error_type=type(e).__name__, error_message=str(e), trace_id=trace_id)
    ...
    async def get_cycle_breaking_suggestion(self, cycle_path: List[str], relevant_code_snippets: Dict[str, str], trace_id: Optional[str] = None) -> str:
        logger.info(f"Generating AI cycle-breaking suggestion for cycle: {' -> '.join(cycle_path)}...")
        code_context_str = "\n\n".join([f"Module: {mod_name}\n```python\n{code}\n```" for mod_name, code in relevant_code_snippets.items()])
        prompt = (
...
async def get_ai_manager_instance(config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> AIManager:
...
async def get_ai_suggestions(codebase_context: str, config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> List[str]:
...
async def get_ai_patch(problem_description: str, relevant_code: str, suggestions: List[str], config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> List[str]:
    manager = await get_ai_manager_instance(config, trace_id, tenant_id)
    patch_str = await manager.get_cycle_breaking_suggestion(
        cycle_path=[],
        relevant_code_snippets={"problem_context": f"Problem: {problem_description}\nSuggestions: {'; '.join(suggestions)}\nCode:\n{relevant_code}"},
        trace_id=trace_id
    )
...
async def ai_health_check(config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> Dict[str, Any]:
    manager = await get_ai_manager_instance(config, trace_id, tenant_id)
    return await manager.health_check()

# Sync wrappers using the new _run_async helper
def get_ai_suggestions_sync(codebase_context: str, config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> List[str]:
    return _run_async(get_ai_suggestions(codebase_context, config, trace_id, tenant_id))

def get_ai_patch_sync(problem_description: str, relevant_code: str, suggestions: List[str], config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> List[str]:
    return _run_async(get_ai_patch(problem_description, relevant_code, suggestions, config, trace_id, tenant_id))

def ai_health_check_sync(config: Optional[Dict[str, Any]] = None, trace_id: Optional[str] = None, tenant_id: Optional[str] = None) -> Dict[str, Any]:
...
async def main_test():
    ...
    try:
        ...
        """
        suggestion = await manager.get_refactoring_suggestion(sample_refactoring_context, trace_id=trace_id)
        print(f"Refactoring Suggestion:\n{suggestion}\n")

        print("\n--- Testing AI Cycle-Breaking Suggestion ---")
        patch = await manager.get_cycle_breaking_suggestion(
            cycle_path=[],
            relevant_code_snippets={"problem_context": "Circular import between module_a and module_b.\nSuggestions: Extract interfaces; Use lazy import.\nCode:\nimport module_b\nimport module_a"},
            trace_id=trace_id
        )
