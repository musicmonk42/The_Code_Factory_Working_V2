        ...
        async with self._token_usage_lock:
            ...
            if current_minute_usage + tokens_to_use > self.token_quota_per_minute:
                ...
                if sleep_duration > timeout:
                    ...
                    alert_operator("CRITICAL: LLM token quota overrun; would require excessive wait. Aborting API call.", level="CRITICAL")
                ...
                current_minute_usage = sum(tokens for _, tokens in self._token_usage_history)
                if current_minute_usage + tokens_to_use > self.token_quota_per_minute:
                    logger.critical("CRITICAL: LLM token quota overrun after waiting. Aborting API call.", extra={'current_usage': current_minute_usage, 'requested_tokens': tokens_to_use})
