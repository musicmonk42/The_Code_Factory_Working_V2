...
class SecretConfig:
    ...
    local_key_file: Optional[str] = field(default_factory=lambda: os.getenv("LOCAL_KEY_FILE", ".secrets.key"))
...
class SecretsManager:
    ...
    def _initialize_providers(self):
        ...
        if provider == SecretProvider.AWS_SECRETS_MANAGER and AWS_AVAILABLE:
            ...
            if not self.config.vault_url or not self.config.vault_token:
                raise SecurityAnalysisError("Vault URL and token must be configured for HashiCorp Vault provider.")
            ...
            if not self.config.azure_vault_url:
                raise SecurityAnalysisError("Azure Vault URL must be configured for Azure Key Vault provider.")
            try:
                ...
                raise SecurityAnalysisError(f"Failed to authenticate with Azure Key Vault: {e}") from e
        elif provider == SecretProvider.GCP_SECRET_MANAGER and GCP_AVAILABLE:
            if not self.config.gcp_project_id:
                raise SecurityAnalysisError("GCP Project ID must be configured for GCP Secret Manager provider.")
    ...
    def get_secret(self, secret_name: str, version: Optional[str] = None, required: bool = False) -> Optional[str]:
        """
        ...
        except (ValueError, ClientError, hvac.exceptions.InvalidRequest, Exception) as e:
    ...
    def _clear_secret_from_cache(self, secret_name: str):
        ...
        with self._lock:
            ...
            if keys_to_remove:
                logger.debug(f"Cleared {len(keys_to_remove)} cache entries for secret '{secret_name}'")
    ...
    def _get_local_encrypted_secret(self, secret_name: str) -> Optional[str]:
        ...
        try:
            ...
            raise SecurityAnalysisError(f"Failed to decrypt local secret '{secret_name}': {e}") from e
    ...
    def _set_local_encrypted_secret(self, secret_name: str, secret_value: str) -> bool:
        ...
        try:
            ...
            raise SecurityAnalysisError(f"Failed to encrypt and store local secret '{secret_name}': {e}") from e
    ...
    def list_secrets(self, prefix: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            if provider == SecretProvider.ENV_VARS:
                secrets = [k for k in os.environ.keys() 
    ...
    def validate_secret_policy(self, secret_value: str) -> Tuple[bool, Optional[str]]:
        ...
        if not (has_upper and has_lower and has_digit and has_special):
            return False, "Secret must contain uppercase, lowercase, digit, and special character"
...
_default_config = SecretConfig(
    provider=SecretProvider.AWS_SECRETS_MANAGER if PRODUCTION_MODE and AWS_AVAILABLE 
