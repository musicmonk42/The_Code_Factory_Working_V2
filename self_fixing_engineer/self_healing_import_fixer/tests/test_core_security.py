...
def mock_alert_operator_security(mocker):
    return mocker.patch("self_healing_import_fixer.analyzer.core_utils.alert_operator")
...
def test_run_subprocess_safely_success(monkeypatch, test_security_project):
    from self_healing_import_fixer.analyzer import core_security
    ...
    analyzer = SecurityAnalyzer(test_security_project)
    success, output = analyzer._run_subprocess_safely(["echo", "hello"], "test command")
...
def test_run_subprocess_safely_failure_raises_exception(
    ...
    analyzer = SecurityAnalyzer(test_security_project)
    success, output = analyzer._run_subprocess_safely(["bad-command"], "bad command")
