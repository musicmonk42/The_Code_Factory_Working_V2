...
def mock_core_dependencies():
    ...
    with patch('fixer_ai.alert_operator') as mock_alert, \
         patch('fixer_ai.scrub_secrets', side_effect=lambda x: str(x) if x else '') as mock_scrub, \
         patch('fixer_ai.audit_logger') as mock_audit, \
         patch('fixer_ai.SECRETS_MANAGER') as mock_secrets:
...
@pytest.mark.asyncio
async def test_token_quota_enforcement(mock_core_dependencies, mock_httpx_client, mock_openai_client):
...
@pytest.mark.asyncio
async def test_call_llm_api_uses_cache(mock_core_dependencies, mock_openai_client, mock_redis_client):
