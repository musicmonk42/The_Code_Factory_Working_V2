...
def policy_file_with_signature(tmp_path):
    ...
    signature = hmac.new(
        test_key,
        policy_content_str.encode('utf-8'),
        hashlib.sha256
    ).hexdigest()
...
def test_init_with_valid_policy_succeeds(policy_file_with_signature, mock_audit_logger_policy, mock_secrets_manager_policy):
...
def test_enforcement_error_handling(policy_file_with_signature, mock_codebase_data, mock_alert_operator_policy):
    with patch.dict(os.environ, {"PRODUCTION_MODE": "false"}):
        core_policy_module._last_good_policy = None
        manager = PolicyManager(policy_file_with_signature, enable_hot_reload=False)
        with patch.object(manager, '_enforce_import_restriction', side_effect=Exception("Test error")):
            violations = manager.check_architectural_policies(**mock_codebase_data)
