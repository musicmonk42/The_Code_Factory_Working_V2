...
class FakeRedis:
    ...
    async def incr(self, key): self._store[key] = int(self._store.get(key, 0) or 0) + 1; return self._store[key]
...
def _patch_infra(monkeypatch, modules: Dict[str, Any]) -> None:
    ...
    if ai_mod:
        ...
        monkeypatch.setattr(ai_mod, "get_ai_suggestions", lambda context: ["Move import inside function to break cycle"], raising=False)
        monkeypatch.setattr(ai_mod, "get_ai_patch", lambda *args: ["# patch:\n# (no-op for test)"], raising=False)
