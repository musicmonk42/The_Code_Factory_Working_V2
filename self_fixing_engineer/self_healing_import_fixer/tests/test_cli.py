...
def mock_core_dependencies():
    ...
    with patch('self_healing_import_fixer.cli.alert_operator', new=MagicMock()) as mock_alert, \
         patch('self_healing_import_fixer.cli.scrub_secrets', new=MagicMock(side_effect=lambda x: x)) as mock_scrub, \
         patch('self_healing_import_fixer.cli.SECRETS_MANAGER', new=MagicMock()) as mock_secrets, \
         patch('self_healing_import_fixer.cli.cli_audit_logger', new=mock_audit_logger_instance) as mock_logger:
...
@pytest.mark.parametrize("path_arg, is_dir, allow_symlink, should_raise", [
    ...
    ("/etc/passwd", False, False, True),  # Outside project scope (simulated by allowlist)
])
def test_validate_path_argument_security_checks(path_arg, is_dir, allow_symlink, should_raise, tmp_path):
    """
    ...
    if should_raise:
        with pytest.raises(SystemExit):
            _validate_path_argument(path_arg, "test_arg", is_dir=is_dir, allow_symlink=allow_symlink, allowlist=allowlist)
    else:
        ...
        result = _validate_path_argument(test_path, "test_arg", is_dir=is_dir, allow_symlink=allow_symlink, allowlist=allowlist)
        assert result == str(Path(test_path).resolve())

def test_main_handles_analyze_command(test_project_setup, mock_plugin_manager, mock_core_dependencies):
    ...
    with patch('sys.argv', ['self_healing_import_fixer.cli', 'analyze', test_project_setup["project_root"], '--output-format', 'text']), \
         patch('self_healing_import_fixer.cli.ImportGraphAnalyzer', create=True) as MockAnalyzer, \
...
def test_heal_command_in_prod_requires_interactive(test_project_setup, mock_plugin_manager, mock_core_dependencies):
    """
    ...
    with patch('sys.argv', ['self_healing_import_fixer.cli', 'heal', test_project_setup["project_root"]]), \
         ...
         patch('sys.exit', side_effect=lambda code: (_ for _ in ()).throw(TestExit(code))):
...
def test_heal_command_in_prod_forbids_yes_flag(test_project_setup, mock_plugin_manager, mock_core_dependencies):
    """
    ...
    # Mock the heal_entrypoint - it should NOT be called if security check works
    with patch('sys.argv', ['self_healing_import_fixer.cli', 'heal', test_project_setup["project_root"], '-i', '-y']), \
         patch('self_healing_import_fixer.cli.heal_entrypoint', create=True) as mock_heal, \
         patch('self_healing_import_fixer.cli.PRODUCTION_MODE', True), \
         patch('self_healing_import_fixer.cli.load_fixer') as mock_load_fixer, \
         patch('sys.exit', side_effect=lambda code: (_ for _ in ()).throw(TestExit(code))):
...
def test_serve_command_in_prod_enforces_security(test_project_setup, mock_plugin_manager, mock_core_dependencies):
    """
    ...
    with patch('sys.argv', ['self_healing_import_fixer.cli', 'serve', test_project_setup["project_root"]]), \
...
async def test_main_loads_plugins_from_config(mock_plugin_manager, test_project_setup):
    ...
    with patch('self_healing_import_fixer.cli.load_config', return_value=config_data), \
         patch('sys.argv', ['self_healing_import_fixer.cli', '--config', str(config_path), 'analyze', test_project_setup["project_root"], '--output-format', 'text']), \
         patch('self_healing_import_fixer.cli.ImportGraphAnalyzer', create=True, return_value=mock_analyzer), \
    ...
    MockPluginManager.assert_called_with(
        plugin_dirs=None,
        approved_plugins=config_data['plugins']['approved_plugins']
    )
...
def test_cli_execution_failure_logs_and_aborts(mock_plugin_manager, mock_core_dependencies):
    ...
    with patch('self_healing_import_fixer.cli.ImportGraphAnalyzer', create=True, side_effect=ValueError("An unexpected error occurred.")), \
