...
class _SecretsMgr:  # pragma: no cover
    def get_secret(self, *a, **k): return "dummy_secret_value"
...
def project(tmp_path):
    ...
    file_type_fail.write_text("def bad_add(a: int, b: str) -> int:\n    return a + b\n", encoding="utf-8")
...
async def test_run_linting_with_ruff_success(project, monkeypatch):
    ...
    # Pretend ruff exists
    monkeypatch.setattr("import_fixer.fixer_validate.shutil.which",
                        lambda cmd: "/usr/bin/ruff" if cmd == "ruff" else None, raising=False)

    # Mock subprocess
    proc = MagicMock()
    proc.returncode = 0
    proc.communicate = AsyncMock(return_value=(b"", b""))
    monkeypatch.setattr("import_fixer.fixer_validate.asyncio.create_subprocess_exec",
                        AsyncMock(return_value=proc), raising=False)
...
async def test_validate_and_commit_file_no_write_access_raises_error(project, monkeypatch):
    ...
    monkeypatch.setattr("import_fixer.fixer_validate.os.access", lambda p, m: m != os.W_OK, raising=False)
...
async def test_validate_and_commit_file_pipeline_success(project, monkeypatch, tmp_path):
    ...
    # Safe backup + write
    monkeypatch.setattr("import_fixer.fixer_validate.shutil.copy", lambda *a, **k: None, raising=False)
    ...
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.compile_file", lambda *a, **k: ok_stage, raising=False)
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.run_linting", AsyncMock(return_value=ok_stage), raising=False)
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.run_type_checking", AsyncMock(return_value=ok_stage), raising=False)
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.run_static_analysis", AsyncMock(return_value=ok_stage), raising=False)
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.run_tests", AsyncMock(return_value=ok_stage), raising=False)
...
async def test_validate_and_commit_file_pipeline_failure_rolls_back(project, monkeypatch):
    ...
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.compile_file", lambda *a, **k: ok_stage, raising=False)
    monkeypatch.setattr("import_fixer.fixer_validate.CodeValidator.run_linting", AsyncMock(return_value=bad_stage), raising=False)
