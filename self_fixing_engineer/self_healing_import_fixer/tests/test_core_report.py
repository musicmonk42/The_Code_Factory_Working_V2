...
def test_init_with_unapproved_dir_in_prod_exits(tmp_path):
    with patch.dict(os.environ, {"PRODUCTION_MODE": "true"}):
        with pytest.raises(AnalyzerCriticalError) as excinfo:
            ReportGenerator(str(tmp_path / "unapproved_dir"), approved_report_dirs=[str(tmp_path / "approved_dir")])
    assert "is not within approved paths" in str(excinfo.value)

@pytest.mark.skipif(sys.platform == "win32", reason="os.chmod does not restrict permissions on Windows as on Unix")
...
@pytest.mark.parametrize("report_format, file_ext", [
    ("text", ".txt"),
    ("markdown", ".md"),
    ("html", ".html"),
    ("json", ".json"),
...
def test_generate_pdf_report_calls_weasyprint(tmp_path, monkeypatch):
    ...
    sample_results = {
        "section": [{"field": "secret_token_abc"}]
    }
...
def test_dashboard_login_success(monkeypatch):
    ...
    with app.test_client() as client:
        resp = client.post('/login', json={"username": "admin", "password": "admin_secure_password"})
...
def test_get_report_endpoint_success_no_encryption(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(crmod, "SECRETS_MANAGER", MagicMock(get_secret=lambda k: "admin_secure_password" if k == "DASHBOARD_ADMIN_PASSWORD" else "jwtsecret"))

    with app.test_client() as client:
        login_resp = client.post('/login', json={"username": "admin", "password": "admin_secure_password"})
