...
async def test_ai_manager_init_success(valid_ai_config, mock_secrets_manager, mock_audit_logger_ai, mock_httpx_client, mock_openai_client):
...
async def test_ai_manager_init_missing_api_key_exits(valid_ai_config, mock_secrets_manager, mock_alert_operator, mock_sys_exit):
...
async def test_ai_manager_init_missing_endpoint_exits(valid_ai_config, mock_alert_operator, mock_sys_exit, mock_secrets_manager):
...
async def test_ai_manager_init_no_https_in_prod_exits(valid_ai_config, mock_alert_operator, mock_sys_exit, mock_secrets_manager):
...
async def test_ai_manager_init_no_proxy_in_prod_exits(valid_ai_config, mock_alert_operator, mock_sys_exit, mock_secrets_manager):
...
async def test_ai_manager_init_auto_apply_in_prod_exits(valid_ai_config, mock_alert_operator, mock_sys_exit, mock_secrets_manager):
        ...
        with patch('analyzer.core_ai.AsyncOpenAI', return_value=mock_llm_client):
            ...
            with patch.object(manager, '_sanitize_prompt', return_value="test prompt"):
                ...
                with patch('analyzer.core_ai.REDIS_CLIENT', None):
                    ...
                    assert 'llm_api_call_success' in event_types or 'llm_api_call_output' in event_types

async def test_call_llm_api_failure_and_retry(mock_llm_client, mock_audit_logger_ai, mock_secrets_manager, valid_ai_config, mock_alert_operator):
        ...
        with patch('analyzer.core_ai.AsyncOpenAI') as mock_openai:
            ...
            with patch.object(manager, '_sanitize_prompt', return_value="test prompt"):
                ...
                with patch('analyzer.core_ai.REDIS_CLIENT', None):
                    ...
                    mock_audit_logger_ai.log_event.assert_any_call('llm_api_call_failure', model='gpt-3.5-turbo', error_type='Exception', error_message='API failed', trace_id=manager.trace_id)
                    mock_alert_operator.assert_called_with('ERROR: LLM API call failed: API failed.', level='ERROR')


# --- Token Quota Enforcement Tests ---
async def test_token_quota_enforcement_waits(mock_llm_client, mock_secrets_manager, valid_ai_config, mock_audit_logger_ai, mock_alert_operator):
...
async def test_token_quota_overrun_aborts(mock_llm_client, mock_secrets_manager, valid_ai_config, mock_audit_logger_ai, mock_alert_operator):
...
async def test_get_ai_suggestions_success(mock_llm_client, mock_secrets_manager, valid_ai_config):
    ...
    with patch('analyzer.core_ai.get_ai_manager_instance') as mock_get_manager:
        ...
        from unittest.mock import AsyncMock
        mock_manager.get_refactoring_suggestion = AsyncMock(return_value="Suggestion 1\nSuggestion 2")
...
async def test_ai_public_functions_handle_empty_response(mock_llm_client, mock_secrets_manager, valid_ai_config):
    ...
    with patch('analyzer.core_ai.get_ai_manager_instance') as mock_get_manager:
        ...
        mock_manager.get_cycle_breaking_suggestion = AsyncMock(return_value="AI features are unavailable.")
        patches = await get_ai_patch("Problem", "Code", ["S1"], valid_ai_config)
        assert patches == []