...
class _SecretsMgr:  # pragma: no cover
    def get_secret(self, *a, **k): return "test_hmac_key"
...
def hmac_key(monkeypatch):
    ...
    monkeypatch.setattr("import_fixer.fixer_plugins.SECRETS_MANAGER.get_secret",
                        lambda *a, **k: "test_hmac_key", raising=True)
    _reset_plugin_key_for_tests()
...
@pytest.mark.asyncio
async def test_load_plugin_from_unwhitelisted_dir_raises_noncritical(tmp_path, plugin_dir, hmac_key):
...
def test_run_hook_with_exception_raises_and_alerts(monkeypatch):
    ...
    alerted = {}
    def _alert(msg, level="INFO"): alerted["msg"] = msg; alerted["level"] = level
