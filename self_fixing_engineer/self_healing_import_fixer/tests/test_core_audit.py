    ...
    try:
        ...
        if 'def __init__' in content:
            ...
            if 'asyncio.create_task' in init_method:
                issues.append("__init__ calls asyncio.create_task - causes 'was never awaited' warning")
    except Exception as e:
...
def test_summary_of_issues():
    ...
    for method in ['_write_initial_log_entry', '_initialize_integrity_file', 
                   '_write_integrity_violation', '_update_integrity_metadata']:
        if not hasattr(RegulatoryAuditLogger, method):
    ...
    try:
        ...
        if init_start != -1:
            ...
            if 'asyncio.create_task' in init_source:
                issues["Initialization Issues"].append("asyncio.create_task in __init__ causes warnings")
    except:
