...
class EnvironmentConfig:
    ...
    observation_keys: List[str] = field(default_factory=lambda: [
        "pass_rate", "latency", "alert_ratio", "code_coverage", "complexity"
    ])
    max_steps: int = 100
    ...
    action_cooldowns: Dict[int, int] = field(default_factory=lambda: {
...
class CodeHealthEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=0, high=1,
            shape=(len(self.config.observation_keys),),
            dtype=np.float32
        )
        ...
        with self._state_lock:
            try:
                ...
                if isinstance(metrics, SystemMetrics):
                    ...
                    raise ValueError(f"get_metrics must return SystemMetrics or array, got {type(metrics)}")
            except Exception as e:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict[str, Any]]:
        ...
        with self._state_lock:
            ...
            if not can_execute:
                logger.debug(f"Action {action_name} on cooldown for {cooldown_remaining} more steps")
                info['cooldown'] = True
            ...
            if self.done:
                ...
                logger.debug(f"Step {self.steps}: Action={action_name}, Reward={reward:.2f}, State={self.state.round(2).tolist()}")
    ...
    def _check_and_handle_rollback(self, current_action: int) -> Optional[Dict[str, Any]]:
        ...
        pass_rate_idx = self.config.observation_keys.index("pass_rate") if "pass_rate" in self.config.observation_keys else -1
        ...
        logger.warning(f"[SAFEGUARD] {severity.upper()} state detected (pass_rate: {pass_rate:.2f}). Initiating automatic rollback.")
    ...
    def _record_step(self, action: int, action_name: str, reward: float, info: Dict[str, Any]):
        ...
        step_record = {
            ...
            "action": action_name,
            "action_id": action,
            "state_before": self.last_state_before_action.tolist() if self.last_state_before_action is not None else [],
    ...
    def _handle_episode_end(self):
        ...
        logger.info(f"Episode ended - Session: {self.session_id}, Steps: {self.steps}, Reward: {self.cumulative_reward:.2f}, Reason: {reason}")
    ...
    def reset(self, seed=None, options=None) -> Tuple[np.ndarray, Dict[str, Any]]:
        ...
        with self._state_lock:
            ...
            logger.info(f"Environment reset - Session: {self.session_id}, Initial state: {self.state.round(2).tolist()}")
    ...
    def _render_rgb_array(self) -> np.ndarray:
        ...
        if len(self.metrics_history) > 1:
            history_array = np.array([m.to_array(self.config.observation_keys) 
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        metrics_array = np.array([m.to_array(self.config.observation_keys) 
                                 for m in self.metrics_history])
        ...
        return {
            ...
            "improvement": dict(zip(self.config.observation_keys, 
                                   self.state - metrics_array[0] if len(metrics_array) > 0 else np.zeros_like(self.state)))
        }
...
def run_code_health_simulation():
    ...
    class SimulationState:
        def __init__(self):
            ...
            self.action_effects = {
                ActionType.RESTART.value: {
                    'pass_rate': 0.05, 'latency': -0.03, 'alert_ratio': -0.01
                },
                ActionType.ROLLBACK.value: {
                    'pass_rate': 0.10, 'latency': -0.05, 'alert_ratio': -0.02
                },
                ActionType.APPLY_PATCH.value: {
                    'pass_rate': 0.08, 'latency': -0.04, 'alert_ratio': -0.015,
                    'success_rate': 0.7
                },
        ...
        def degrade_metrics(self):
            ...
            self.metrics.pass_rate = max(0, self.metrics.pass_rate - np.random.uniform(0.005, 0.015))
            self.metrics.latency = min(1, self.metrics.latency + np.random.uniform(0.002, 0.008))
            self.metrics.alert_ratio = min(1, self.metrics.alert_ratio + np.random.uniform(0.001, 0.003))
            self.metrics.complexity = min(1, self.metrics.complexity + np.random.uniform(0.001, 0.002))
        ...
        def apply_action_effects(self, action_id: int) -> Dict[str, Any]:
            ...
            if action_id in self.action_effects:
                ...
                result["effects_applied"] = {k: v for k, v in effects.items() 
    ...
    def apply_action_simulated(action_id: int) -> Dict[str, Any]:
        ...
        logger.info(f"Simulating action: {ActionType(action_id).name if action_id < len(ActionType) else 'unknown'}")
...
if __name__ == "__main__":
    ...
    if os.environ.get("SANDBOXED_ENV", "") == "1":
        ...
        print(f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Launching sandboxed process...")
