...
if os.getenv("PYTEST_COLLECTING"):
    # Provide minimal stubs to avoid expensive imports (deap, etc.)
    ...
    def evolve_configs(*args, **kwargs):
        """Stub for pytest collection (deprecated function)"""
        pass

    # Stop here during collection
    __all__ = ['ConfigurationSpace', 'EvolutionConfig', 'FitnessEvaluator',
               'GeneticOptimizer', 'DEAP_AVAILABLE', 'run_test_evaluation', 'evolve_configs']
else:
        ...
        def _start_worker_pool(self):
            ...
            for i in range(self.config.max_parallel_evaluations):
                worker = threading.Thread(
                    target=self._evaluation_worker, daemon=True, name=f"EvalWorker-{i}"
                )
                worker.start()
            ...
            try:
                ...
                try:
                    stdout, stderr = process.communicate(
                        input=json.dumps(config), timeout=self.config.timeout_seconds
                    )
    ...
    class GeneticOptimizer:
        ...
        def _setup_deap(self):
            ...
            self.toolbox.register(
                ...
                tools.mutGaussian,
                mu=0,
                sigma=self.evolution_config.mutation_sigma,
                indpb=0.2,
            )
            self.toolbox.register(
                "select",
        ...
        def evolve(
            ...
            logbook.header = (
                ["gen", "nevals"] + stats.fields
                if hasattr(stats, "fields")
                else ["gen", "nevals", "avg", "min", "max", "std"]
            )
            ...
            for gen in range(1, self.evolution_config.generations + 1):
                ...
                offspring = elite + tools.selBest(
                    offspring, len(population) - self.evolution_config.elite_size
                )
        ...
        def get_evolution_summary(self) -> Dict[str, Any]:
            ...
            return {
                "best_fitness": self.best_fitness,
                "best_config": (
                    self.evaluator._map_genes_to_config(self.best_individual)
    ...
    def run_test_evaluation(config: Dict[str, Any]) -> Dict[str, float]:
        ...
        is_good_config = (
            400 < config.get("max_connections", 0) < 800
            and 2 < config.get("timeout_sec", 10) < 6
            and config.get("retry_count", 1) == 3
            and config.get("alert_threshold", 1.0) < 0.7
        )
