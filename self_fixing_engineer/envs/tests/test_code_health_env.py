...
class TestAutomaticRollback:
    ...
    def test_automatic_rollback_triggered(self, mock_action):
        ...
        def degrading_metrics():
            ...
            if call_count <= 2:
                ...
                return [0.15, 0.8, 0.5]  # Bad metrics (pass_rate below critical threshold)
    ...
    def test_rollback_disabled(self, mock_metrics, mock_action):
        ...
        def bad_metrics():
            return [0.09, 0.9, 0.9]  # Very bad metrics (below default critical threshold of 0.1)
        
        env = CodeHealthEnv(
            get_metrics=bad_metrics,
            apply_action=mock_action,
            config=config
        )
...
class TestAuditLogging:
    ...
    def test_audit_logging_lifecycle(self, basic_config, mock_metrics, mock_action, mock_audit_logger):
        ...
        # Reset
        env.reset()
        reset_calls = get_event_types(mock_audit_logger.log_event.call_args_list, "rl_episode_reset")
...
class TestIntegration:
    ...
    def test_full_episode(self, mock_action):
        ...
        env = CodeHealthEnv(
            get_metrics=sequential_metrics,
            apply_action=mock_action,
            config=config
        )
        ...
        training_data = env.get_training_data()
        assert any("automatic_rollback" in record.get("info", {}) 
                  for record in training_data)
