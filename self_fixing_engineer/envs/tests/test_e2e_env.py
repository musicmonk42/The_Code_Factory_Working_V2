...
class SimulatedSystem:
    ...
    def degrade(self):
        ...
        self.current_metrics.pass_rate = max(0, 
            self.current_metrics.pass_rate - self.degradation_rate + 
            np.random.normal(0, self.volatility))
        self.current_metrics.latency = min(1,
            self.current_metrics.latency + self.degradation_rate/2 +
            np.random.normal(0, self.volatility/2))
        self.current_metrics.alert_ratio = min(1,
            self.current_metrics.alert_ratio + self.degradation_rate/3 +
    ...
    def apply_action(self, action: ActionType) -> Dict[str, Any]:
        ...
        if action == ActionType.RESTART:
            ...
            self.current_metrics.pass_rate = min(1, 
                self.current_metrics.pass_rate + self.recovery_rate)
            self.current_metrics.latency = max(0,
                self.current_metrics.latency - self.recovery_rate/2)
            result["effect"] = "System restarted"
            
        elif action == ActionType.ROLLBACK:
            # Rollback significantly improves metrics
            self.current_metrics.pass_rate = min(1, 
                self.current_metrics.pass_rate + self.recovery_rate * 2)
            self.current_metrics.latency = max(0,
                self.current_metrics.latency - self.recovery_rate)
            self.current_metrics.alert_ratio = max(0,
                self.current_metrics.alert_ratio - self.recovery_rate/2)
            ...
            if np.random.random() < 0.7:
                self.current_metrics.pass_rate = min(1,
                    self.current_metrics.pass_rate + self.recovery_rate * 1.5)
                self.current_metrics.code_coverage = min(1,
                    self.current_metrics.code_coverage + 0.02)
            ...
            self.current_metrics.complexity = max(0,
                self.current_metrics.complexity - 0.02)
            result["effect"] = "Code linted"
            
        elif action == ActionType.RUN_TESTS:
            self.current_metrics.code_coverage = min(1,
                self.current_metrics.code_coverage + 0.01)
...
class TestE2EAsyncOperations:
    ...
    async def test_async_action_pipeline(self):
        ...
        env = CodeHealthEnv(
            get_metrics=lambda: system.current_metrics,
            apply_action=async_apply_action
        )
...
class TestE2EPerformanceAndScale:
    ...
    def test_high_frequency_monitoring(self):
        ...
        config = EnvironmentConfig(
            max_steps=1000,
            max_action_history=100  # Limited history
        )
