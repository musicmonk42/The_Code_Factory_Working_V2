        ...
        if cls.PROD_MODE:
            ...
            if cls.FIPS_MODE:
                if cls.ENCRYPTION_ALGORITHM not in ["AES-256-GCM", "AES-256-CBC"]:
                    errors.append(
                        f"FIPS mode requires AES encryption, got {cls.ENCRYPTION_ALGORITHM}"
                    )
                if cls.HASH_ALGORITHM not in [
                    "SHA256",
                    "SHA384",
                    "SHA512",
                    "SHA3-256",
        ...
        with tracer.start_as_current_span("crypto.generate_key") as span:
            ...
            if PROMETHEUS_AVAILABLE:
                CRYPTO_OPERATIONS.labels(
                    operation="generate_key", algorithm="CSPRNG", status="success"
                ).inc()
            ...
            try:
                ...
                if PROMETHEUS_AVAILABLE:
                    ...
                    CRYPTO_LATENCY.labels(
                        operation="derive_key", algorithm=SecurityConfig.KEY_DERIVATION
...
def compute_hmac(data: bytes, key: bytes, algorithm: Optional[str] = None) -> str:
    """
...
def verify_hmac(
    data: bytes, key: bytes, expected_hmac: str, algorithm: Optional[str] = None
) -> bool:
    """
    ...
    if PROMETHEUS_AVAILABLE:
        DATA_INTEGRITY_CHECKS.labels(result="valid" if result else "invalid").inc()
...
def compress_data(
    data: bytes, algorithm: Optional[str] = None, level: Optional[int] = None
) -> bytes:
    """
...
def compress_json(
    data: Dict[str, Any], algorithm: Optional[str] = None, level: Optional[int] = None
) -> bytes:
    """
...
def decompress_json(data: bytes, algorithm: Optional[str] = None) -> Dict[str, Any]:
    """
        ...
        if isinstance(obj, dict):
            ...
            for key, value in obj.items():
                ...
                if _is_sensitive_field(key):
                    ...
                    audit_logger.info("Scrubbed sensitive field", extra={"field": key})
                else:
...
def anonymize_data(
    data: Dict[str, Any], fields_to_anonymize: List[str], method: str = "hash"
) -> Dict[str, Any]:
    """
...
def rotate_fernet_keys(
    current_keys: List[bytes], new_key: Optional[bytes] = None
    ...
    audit_logger.info(
        ...
        extra={"key_count": len(rotated_keys), "max_keys": max_keys},
    )
...
def validate_checkpoint_data(
    data: Dict[str, Any], schema: Optional[Any] = None, max_size: Optional[int] = None
) -> bool:
    """
