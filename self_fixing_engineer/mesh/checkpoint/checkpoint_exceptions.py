...
try:
    ...
    if otel_version < MIN_VERSIONS["opentelemetry.trace"]:
        logging.warning(f"OpenTelemetry version {otel_version} is outdated. Update to >= {MIN_VERSIONS['opentelemetry.trace']} is recommended.")
...
log_handler = RotatingFileHandler('exceptions.log', maxBytes=10*1024*1024, backupCount=5) # 10MB per file, 5 backups
structlog.configure(
    processors=[
...
if PYBREAKER_AVAILABLE:
    BREAKER = CircuitBreaker(fail_max=5, reset_timeout=60) # Opens after 5 failures, closes after 60s
...
class CheckpointError(Exception):
    ...
    def __init__(
        ...
        self.error_code = error_code.value if isinstance(error_code, CheckpointErrorCode) else CheckpointErrorCode.GENERIC_ERROR.value
        ...
        if OPENTELEMETRY_AVAILABLE and trace:
            ...
            if span and span.is_recording():
                ...
                span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"{self.__class__.__name__}: {message}"))
    ...
    def sign_context(self, secret: Optional[str] = None) -> str:
        ...
        if not secret:
            raise ValueError("HMAC secret not provided and EXCEPTION_HMAC_SECRET env var is not set.")
    ...
    async def raise_with_alert(
        ...
        if ALERT_CACHE is not None:
            ...
            if count > 5:
                logger.warning("Alert flood detected, suppressing notification.", alert_key=alert_key, count=count)
                raise instance # Raise without alerting to avoid spam

        logger.critical("Raising exception with operator alert", message=message, context=instance.context)
...
class CheckpointAuditError(CheckpointError):
    ...
    def __init__(self, message: str, context: Optional[Dict[str, Any]] = None):
        super().__init__(f"Audit Failure: {message}", context, CheckpointErrorCode.AUDIT_FAILURE, severity="critical")
        audit_logger.critical("Security incident detected", audit_message=message, context=self.context)

class CheckpointBackendError(CheckpointError):
    """Raised when an underlying storage backend operation fails."""
    def __init__(self, message: str, context: Optional[Dict[str, Any]] = None, error_code: CheckpointErrorCode = CheckpointErrorCode.BACKEND_UNAVAILABLE):
...
class CheckpointValidationError(CheckpointError):
    ...
    def __init__(self, message: str, context: Optional[Dict[str, Any]] = None):
        super().__init__(f"Validation Error: {message}", context, CheckpointErrorCode.VALIDATION_FAILURE, severity="warning")
