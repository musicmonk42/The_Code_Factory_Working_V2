...
try:
    ...
    PUB_COUNT = Counter("mesh_pub_count", "Total published messages", ["backend", "channel", "env", "tenant"])
    PUB_FAIL_COUNT = Counter("mesh_pub_fail_count", "Total failed publish attempts", ["backend", "channel", "reason", "env", "tenant"])
    SUB_COUNT = Counter("mesh_sub_count", "Total messages processed by subscriber", ["backend", "channel", "env", "tenant"])
    SUB_FAIL_COUNT = Counter("mesh_sub_fail_count", "Total failed subscriber message processing", ["backend", "channel", "reason", "env", "tenant"])
    DLQ_COUNT = Counter("mesh_dlq_events_total", "Total events written to the dead-letter queue", ["backend", "channel", "reason", "env", "tenant"])
    DLQ_REPLAY_COUNT = Counter("mesh_dlq_replay_total", "Total events replayed from DLQ", ["backend", "status", "env", "tenant"])
    CONNECT_STATUS = Gauge("mesh_backend_connect_status", "Connection status to the backend (1=connected, 0=disconnected)", ["backend", "env", "tenant"])
    CONNECT_LATENCY = Histogram("mesh_connect_latency_seconds", "Latency of backend connection attempts", ["backend", "env", "tenant"])
    REQUEST_LATENCY = Summary("mesh_request_latency_seconds", "Latency of pub/sub requests", ["backend", "channel", "op", "env", "tenant"])
    PROMETHEUS_AVAILABLE = True
except ImportError:
    PUB_COUNT, PUB_FAIL_COUNT, SUB_COUNT, SUB_FAIL_COUNT, DLQ_COUNT, DLQ_REPLAY_COUNT, CONNECT_STATUS, CONNECT_LATENCY, REQUEST_LATENCY = (None,) * 9
...
try:
    ...
    resource = Resource.create({SERVICE_NAME: "mesh-adapter", "env": os.getenv("ENV", "unknown"), "tenant": os.getenv("TENANT", "unknown")})
...
TOP_CHANNELS = {"user_events", "order_events", "payment_updates"} # Example high-volume channels
...
def _enforce_prod_requirements():
    ...
    if HAS_FCNTL is False and HAS_MSVCRT is False:
        logging.warning("WARNING: Neither fcntl (Unix) nor msvcrt (Windows) available. File locking may not work properly.")
    
    if not MultiFernet:
        logging.critical("CRITICAL: `cryptography` module not available. Encryption is required in production mode. Exiting.")
        sys.exit(1)
    if not ENCRYPTION_KEY:
        logging.critical("CRITICAL: `MESH_ENCRYPTION_KEY` environment variable is not set, but is required for production mode. Exiting.")
        sys.exit(1)
    if not HMAC_KEY:
        logging.critical("CRITICAL: `MESH_HMAC_KEY` environment variable is not set, but is required for production mode. Exiting.")
        sys.exit(1)
    if not aiofiles:
        logging.critical("CRITICAL: `aiofiles` module is not available, but is required for the DLQ in production mode. Exiting.")
        sys.exit(1)
    if not os.environ.get("OTEL_EXPORTER_OTLP_TRACES_ENDPOINT"):
        logging.critical("CRITICAL: OTEL_EXPORTER_OTLP_TRACES_ENDPOINT is required in production for tracing. Exiting.")
        sys.exit(1)
    ...
    if nats and hasattr(nats, '__version__') and nats.__version__ < '2.8.0':
        logging.critical("CRITICAL: nats-py version vulnerable to JAAS RCE. Upgrade to >=2.8.0. Exiting.")
        sys.exit(1)
    ...
    if aiobotocore and hasattr(aiobotocore, '__version__') and aiobotocore.__version__ < '2.5.0':
        logging.warning("WARNING: aiobotocore version may be vulnerable to aiohttp-related smuggling. Upgrade to >= 2.5.0.")
...
class CircuitBreakerWrapper:
    ...
    failures: int = 0
    is_open: bool = False
    breaker: Any = field(default_factory=lambda: CircuitBreaker(fail_max=5, reset_timeout=60) if CircuitBreaker else None)
...
class MeshPubSub:
    ...
    def __init__(
        ...
        self.azure_container_name = azure_container_name or os.getenv("AZURE_STORAGE_CONTAINER_NAME", "mesh-events")
        ...
        self.multi_fernet = MultiFernet([Fernet(k.encode()) for k in ENCRYPTION_KEY.split(",")]) if ENCRYPTION_KEY and MultiFernet else None
        self.hmac_key = HMAC_KEY.encode() if HMAC_KEY else None
        
    @staticmethod
    def detect_backend(url: str) -> str:
        if url.startswith(("redis://", "rediss://")): return "redis"
        elif url.startswith(("nats://", "tls://")): return "nats"
        elif url.startswith(("kafka://", "kafka+ssl://")): return "kafka"
        elif url.startswith(("amqp://", "amqps://")): return "rabbitmq"
        elif url.startswith("aws://"): return "aws"
        elif url.startswith("gcs://"): return "gcs"
        elif url.startswith("azure://"): return "azure"
        elif url.startswith("etcd://"): return "etcd"
        else: raise ValueError(f"Unknown or unsupported mesh backend URL: {url}")

    def _sign_payload(self, payload: bytes) -> str:
        if not self.hmac_key: return ""
        return hmac.new(self.hmac_key, payload, hashlib.sha256).hexdigest()

    def _prepare_payload(self, message: Any) -> bytes:
        payload = json.dumps(message).encode('utf-8')
        signature = self._sign_payload(payload)
        signed_payload = json.dumps({"sig": signature, "data": payload.decode('utf-8')}).encode('utf-8')
        ...
        with get_tracing_context()("mesh_connect") as span:
            ...
            try:
                if self.backend_type == "redis":
                    ...
                    if not nats: raise ImportError("nats not installed")
                    self._client = await nats.connect(self.backend_url, tls=PROD_MODE)
                elif self.backend_type == "kafka":
                    if not AIOKafkaProducer: raise ImportError("aiokafka not installed")
                    parsed_url = urlparse(self.backend_url)
                    ...
                    if not aio_pika: raise ImportError("aio_pika not installed")
                    if PROD_MODE:
                        url = self.backend_url.replace("amqp://", "amqps://")
                        if not RABBITMQ_USER or not RABBITMQ_PASSWORD:
                            raise RuntimeError("RABBITMQ_USER and RABBITMQ_PASSWORD required in production.")
                    else: url = self.backend_url
                    ...
                    if not aiobotocore: raise ImportError("aiobotocore not installed")
                    ...
                    if not storage or not pubsub_v1: raise ImportError("google-cloud-storage or google-cloud-pubsub not installed")
                    ...
                    if not ServiceBusClient: raise ImportError("azure-servicebus not installed")
                    if not self.azure_connection_string:
                        raise ValueError("AZURE_STORAGE_CONNECTION_STRING must be configured.")
                    self._azure_servicebus_client = ServiceBusClient.from_connection_string(self.azure_connection_string)
                elif self.backend_type == "etcd":
                    if not etcd3: raise ImportError("etcd3 not installed")
                    ...
                    self._etcd_client = etcd3.client(
                        host=self.etcd_host, port=self.etcd_port, secure=PROD_MODE,
                        user=self.etcd_user, password=self.etcd_password
                    )
                else: raise NotImplementedError(f"Backend '{self.backend_type}' not implemented.")
                
                await self.healthcheck()
                logger.info("Connected to mesh backend.", backend=self.backend_type, env=ENV, tenant=TENANT)
                if CONNECT_STATUS: CONNECT_STATUS.labels(backend=self.backend_type, env=ENV, tenant=TENANT).set(1)
                if CONNECT_LATENCY: CONNECT_LATENCY.labels(backend=self.backend_type, env=ENV, tenant=TENANT).observe(time.time() - start_time)
                if self._auto_replay_dlq: await self.replay_dlq()
            except Exception as e:
                if CONNECT_STATUS: CONNECT_STATUS.labels(backend=self.backend_type, env=ENV, tenant=TENANT).set(0)
                if CONNECT_LATENCY: CONNECT_LATENCY.labels(backend=self.backend_type, env=ENV, tenant=TENANT).observe(time.time() - start_time)
                logger.critical("Failed to connect to mesh backend.", backend=self.backend_type, error=str(e))
                raise

    def _scrub_payload(self, payload: Dict) -> Dict:
        def scrub(data):
            if isinstance(data, dict):
                return {k: "[REDACTED]" if self.SENSITIVE_KEYS.match(k) else scrub(v) for k, v in data.items()}
    ...
    async def _write_to_dlq(self, payload: dict, native: bool = False):
        with get_tracing_context()("mesh_dlq_write") as span:
            ...
            try:
                if native and self.use_native_dlq:
                    ...
                    async with self._dlq_lock:
                        ...
                        if self.multi_fernet:
                            ...
                            payload_to_write = {"encrypted": encrypted, "sig": self._sign_payload(payload_str.encode())}
                        
                        if self.enable_dlq_rotation:
                            handler = TimedRotatingFileHandler(self.dead_letter_path, when="midnight", backupCount=7)
                            handler.doRollover()
                            ...
                            if not os.path.exists(self.dead_letter_path):
                                if aiofiles:
                                    async with aiofiles.open(self.dead_letter_path, 'a'): pass
                                else:
                                    with open(self.dead_letter_path, 'a'): pass
                            ...
                            if HAS_FCNTL:
                                ...
                                if stat.st_mode & 0o777 != 0o600:
                                    logger.critical("CRITICAL: DLQ file permissions are not 0600. Exiting.")
                                    sys.exit(1)
                ...
                logger.warning("Event written to DLQ.", dlq_path=self.dead_letter_path, reason=payload.get('exc'))
                if DLQ_COUNT: DLQ_COUNT.labels(backend=self.backend_type, channel=_cap_label(payload.get('channel'), self.TOP_CHANNELS), reason=payload.get('exc'), env=ENV, tenant=TENANT).inc()
            except Exception as e:
    ...
    async def _write_to_dlq_native(self, payload: dict):
        if self.backend_type == "kafka":
            ...
            try:
                ...
                logger.error("Failed to send to native Kafka DLQ. Falling back to file DLQ.", error=str(e))
                await self._write_to_dlq(payload, native=False)
        elif self.backend_type == "azure":
            try:
                sender = self._azure_servicebus_client.get_topic_sender(topic_name=payload['channel'])
                msg = ServiceBusMessage(json.dumps(payload).encode('utf-8'))
                await sender.send_messages(msg)
            except Exception as e:
                logger.error("Failed to send to native Azure DLQ. Falling back to file DLQ.", error=str(e))
                await self._write_to_dlq(payload, native=False)
        elif self.backend_type == "gcs":
            try:
                dead_letter_topic = self._gcs_pubsub_publisher.topic_path(self.gcs_project_id, f"{payload['channel']}_dead_letter")
                await self._loop.run_in_executor(None, lambda: self._gcs_pubsub_publisher.publish(dead_letter_topic, json.dumps(payload).encode()).result())
            except Exception as e:
                logger.error("Failed to send to native GCS DLQ. Falling back to file DLQ.", error=str(e))
                await self._write_to_dlq(payload, native=False)
        elif self.backend_type == "aws":
            try:
                ...
                await self._aws_sqs_client.send_message(QueueUrl=dlq_url, MessageBody=json.dumps(payload))
            except Exception as e:
                logger.error("Failed to send to native AWS DLQ. Falling back to file DLQ.", error=str(e))
                await self._write_to_dlq(payload, native=False)
        elif self.backend_type == "rabbitmq":
            try:
                exchange = await self._rabbitmq_channel.declare_exchange('dead-letter-exchange', 'fanout', durable=True)
                msg = aio_pika.Message(body=json.dumps(payload).encode())
                await exchange.publish(msg, routing_key="")
            except Exception as e:
                logger.error("Failed to send to native RabbitMQ DLQ. Falling back to file DLQ.", error=str(e))
                await self._write_to_dlq(payload, native=False)
        else:
    ...
    async def publish(self, channel: str, message: Any):
        if self._throttle: await self._throttle.acquire()
            ...
            with get_tracing_context()("mesh_publish") as span:
                ...
                if self.event_schema:
                    try:
                        ...
                        await self._write_to_dlq({"channel": channel, "message": message, "exc": f"schema: {ve}", "time": time.time()})
                        raise
                ...
                try:
                    ...
                    logger.error("Message is not JSON serializable.", channel=channel, error=str(te))
                    await self._write_to_dlq({"channel": channel, "message": message, "exc": f"serialization: {te}", "time": time.time()})
                    raise
                ...
                if self.backend_type == "redis":
                    ...
                    if channel.startswith("arn:aws:sns:"):
                        await self._aws_sns_client.publish(TopicArn=channel, Message=data_bytes.decode())
                    else:
                        resp = await self._aws_sqs_client.get_queue_url(QueueName=channel)
                        q_url = resp["QueueUrl"]
                        await self._aws_sqs_client.send_message(QueueUrl=q_url, MessageBody=data_bytes.decode())
                elif self.backend_type == "gcs":
                    topic_path = self._gcs_pubsub_publisher.topic_path(self.gcs_project_id, channel)
                    await self._loop.run_in_executor(None, lambda: self._gcs_pubsub_publisher.publish(topic_path, data_bytes).result())
                elif self.backend_type == "azure":
                    async with self._azure_servicebus_client.get_topic_sender(topic_name=channel) as sender:
                        msg = ServiceBusMessage(data_bytes)
                        await sender.send_messages(msg)
                elif self.backend_type == "etcd":
                    etcd_key = f"/mesh/events/{channel}/{int(time.time() * 1000)}"
                    await self._loop.run_in_executor(None, lambda: self._etcd_client.put(etcd_key, data_bytes.decode()))
                else: raise NotImplementedError(f"Publish not implemented for backend '{self.backend_type}'.")
                
                if PUB_COUNT: PUB_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), env=ENV, tenant=TENANT).inc()
        ...
        try:
            ...
            if isinstance(e, CircuitBreakerError):
                logger.critical("Circuit breaker open. Skipping publish.", backend=self.backend_type, channel=channel)
                if PUB_FAIL_COUNT:
                    PUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="circuit_breaker", env=ENV, tenant=TENANT).inc()
            raise
            ...
            if self.backend_type == "redis":
                ...
                async for msg in pubsub.listen():
                    if msg["type"] == "message":
                        try:
                            ...
                            if self.event_schema: self.event_schema(decoded_msg)
                            yield decoded_msg
                            if SUB_COUNT: SUB_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), env=ENV, tenant=TENANT).inc()
                        except (InvalidToken, json.JSONDecodeError, AttributeError) as e:
                            logger.error("Failed to process Redis message.", error=str(e), channel=channel)
                            await self._write_to_dlq({"channel": channel, "message": msg["data"], "exc": str(e), "time": time.time()})
                            if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
                            continue
                ...
                async def _consumer_task(msg):
                    try:
                        ...
                        logger.error("Failed to process NATS message.", error=str(e), channel=channel)
                        await self._write_to_dlq({"channel": channel, "message": msg.data, "exc": str(e), "time": time.time()}, native=self.use_native_dlq)
                        if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
                    ...
                    async for msg in self._consumer:
                        async with semaphore:
                            ...
                            if delivery_count > self.MAX_REDELIVERIES:
                                logger.warning("Message exceeded max redeliveries.", channel=channel, redeliveries=delivery_count)
                                await self._write_to_dlq({"channel": channel, "message": msg.value.decode(), "exc": "Max redeliveries", "time": time.time()}, native=self.use_native_dlq)
                            ...
                            try:
                                ...
                                logger.error("Failed to process Kafka message.", error=str(e), channel=channel)
                                msg.headers["delivery_count"] = str(delivery_count)
                                await self._write_to_dlq({"channel": channel, "message": msg.value.decode(), "exc": str(e), "time": time.time()}, native=self.use_native_dlq)
                                if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
                ...
                queue = await self._rabbitmq_channel.declare_queue(channel, arguments={'x-dead-letter-exchange': 'dead-letter-exchange'})
                    ...
                    async for message in queue_iter:
                        async with semaphore:
                            try:
                                ...
                                logger.error("Failed to process RabbitMQ message.", error=str(e), channel=channel)
                                await self._write_to_dlq({"channel": channel, "message": message.body.decode(), "exc": str(e), "time": time.time()})
                                if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
                                await message.nack()
                ...
                q_url = resp["QueueUrl"]
                while True:
                    resp = await self._aws_sqs_client.receive_message(QueueUrl=q_url, MaxNumberOfMessages=10, WaitTimeSeconds=20)
                    for msg in resp.get("Messages", []):
                        async with semaphore:
                            delivery_count = int(msg.get('Attributes', {}).get('ApproximateReceiveCount', 0))
                            if delivery_count > self.MAX_REDELIVERIES:
                                logger.warning("Message exceeded max redeliveries.", channel=channel, redeliveries=delivery_count)
                                await self._write_to_dlq({"channel": channel, "message": msg["Body"], "exc": "Max redeliveries", "time": time.time()}, native=self.use_native_dlq)
                                continue
                            ...
                            try:
                                ...
                                yield decoded_msg
                                if SUB_COUNT: SUB_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), env=ENV, tenant=TENANT).inc()
                                await self._aws_sqs_client.delete_message(QueueUrl=q_url, ReceiptHandle=msg["ReceiptHandle"])
                            except Exception as e:
                                logger.error("Failed to process AWS message.", error=str(e), channel=channel)
                                await self._write_to_dlq({"channel": channel, "message": msg["Body"], "exc": str(e), "time": time.time()})
                                if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
            
            elif self.backend_type == "gcs":
                subscription_path = self._gcs_pubsub_subscriber.subscription_path(self.gcs_project_id, f"{channel}-sub")
                flow_control = pubsub_v1.types.FlowControl(max_messages=10)
                async for message in self._gcs_pubsub_subscriber.yield_messages(subscription_path, flow_control=flow_control):
                    async with semaphore:
                        try:
                            ...
                            logger.error("Failed to process GCS message.", error=str(e), channel=channel)
                            await self._write_to_dlq({"channel": channel, "message": message.data, "exc": str(e), "time": time.time()}, native=self.use_native_dlq)
                            if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
            
            elif self.backend_type == "azure":
                receiver = self._azure_servicebus_client.get_subscription_receiver(topic_name=channel, subscription_name=f"{channel}-sub")
                async with receiver:
                    async for msg in receiver:
                        async with semaphore:
                            try:
                                ...
                                logger.error("Failed to process Azure Service Bus message.", error=str(e), channel=channel)
                                await self._write_to_dlq({"channel": channel, "message": str(msg), "exc": str(e), "time": time.time()}, native=self.use_native_dlq)
                                if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()
                ...
                self._etcd_watch_cancel, _ = await self._loop.run_in_executor(None, lambda: self._etcd_client.watch_prefix(etcd_prefix, _watch_callback))
                ...
                while True:
                    ...
                    async with semaphore:
                        try:
                            ...
                            logger.error("Failed to process Etcd message.", error=str(e), channel=channel)
                            await self._write_to_dlq({"channel": channel, "message": value.decode(), "exc": str(e), "time": time.time()})
                            if SUB_FAIL_COUNT: SUB_FAIL_COUNT.labels(backend=self.backend_type, channel=_cap_label(channel, self.TOP_CHANNELS), reason="processing_error", env=ENV, tenant=TENANT).inc()

            else:
                raise NotImplementedError(f"Subscribe not implemented for backend '{self.backend_type}'.")

    async def replay_dlq(self):
        if not os.path.exists(self.dead_letter_path): return
                ...
                for line in lines:
                    ...
                    try:
                        ...
                        if "encrypted" in rec:
                             ...
                             payload_str = self.multi_fernet.decrypt(rec['encrypted'].encode()).decode()
                        ...
                        if DLQ_REPLAY_COUNT: DLQ_REPLAY_COUNT.labels(backend=self.backend_type, status="success", env=ENV, tenant=TENANT).inc()
                    except Exception as e:
                        logger.error("Failed to replay DLQ event.", error=str(e))
                        remaining.append(line)
                        if DLQ_REPLAY_COUNT: DLQ_REPLAY_COUNT.labels(backend=self.backend_type, status="fail", env=ENV, tenant=TENANT).inc()
        ...
        with get_tracing_context()("mesh_close") as span:
            ...
            try:
                if self.backend_type == "redis":
                    ...
                    if self._rabbitmq_conn: await self._rabbitmq_conn.close()
                elif self.backend_type == "aws":
                    if self._aws_sqs_client: await self._aws_sqs_client.close()
                    if self._aws_sns_client: await self._aws_sns_client.close()
                elif self.backend_type == "gcs":
                    if self._gcs_pubsub_publisher: await self._gcs_pubsub_publisher.transport.close()
                    if self._gcs_pubsub_subscriber: await self._gcs_pubsub_subscriber.transport.close()
                elif self.backend_type == "azure":
                    if self._azure_servicebus_client: await self._azure_servicebus_client.close()
                elif self.backend_type == "etcd":
                    if self._etcd_client: self._etcd_client.close()
                    if self._etcd_watch_cancel: await self._loop.run_in_executor(None, self._etcd_watch_cancel)
            except Exception as e:
    ...
    async def healthcheck(self) -> dict:
        try:
            ...
            if self.backend_type == "redis":
                ...
                await self._loop.run_in_executor(None, lambda: self._gcs_client.get_bucket(self.gcs_bucket_name))
            elif self.backend_type == "azure":
                pass
            elif self.backend_type == "etcd":
            ...
            status = {"backend": self.backend_type, "status": "ok", "latency": time.time() - start_time}
            if CONNECT_STATUS: CONNECT_STATUS.labels(backend=self.backend_type, env=ENV, tenant=TENANT).set(1)
            if CONNECT_LATENCY: CONNECT_LATENCY.labels(backend=self.backend_type, env=ENV, tenant=TENANT).observe(time.time() - start_time)
            return status
        except Exception as e:
    ...
    async def run_harness():
        try:
            class MockClient:
                ...
                async def ping(self): return "PONG"
                @property
                def is_connected(self): return True
                async def publish(self, channel, message): logger.info("Mock publish.", backend=self.name)
                def pubsub(self): return self
                async def subscribe(self, channel, **kwargs): pass
                def listen(self):
                    async def mock_listen():
                        yield {"type": "message", "data": json.dumps({"sig": "", "data": json.dumps({"test": "data"})}).encode()}
                    return mock_listen()
                async def connect(self, *args, **kwargs): pass
                async def close(self, *args, **kwargs): pass
                async def wait_closed(self): pass
                async def list_queues(self): return []
                async def get_queue_url(self, QueueName): return {"QueueUrl": "mock_url"}
                async def send_message(self, QueueUrl, MessageBody): pass
                @property
                def transport(self): return self
                async def transport_close(self): pass
                async def start(self): pass
                async def stop(self): pass
                async def partitions_for_topic(self, topic): return []
                @property
                def is_closed(self): return False
                def get_subscription_receiver(self, **kwargs): return self
                async def __aenter__(self): return self
                async def __aexit__(self, exc_type, exc_val, exc_tb): pass
                async def receive_message(self, **kwargs): return {"Messages": []}
                async def delete_message(self, **kwargs): pass
                def channel(self): return self
                async def declare_queue(self, channel, **kwargs): return self
                async def declare_exchange(self, name, type, **kwargs): return self
                def iterator(self): return self
                async def __aiter__(self): yield self
                async def ack(self): pass
                def __next__(self): return self
                @property
                def body(self): return json.dumps({"sig": "", "data": json.dumps({"test": "data"})}).encode()
                @property
                def value(self): return json.dumps({"sig": "", "data": json.dumps({"test": "data"})}).encode()
                @property
                def headers(self): return {"delivery_count": "1"}
                @property
                def default_exchange(self): return self
                async def publish(self, msg, **kwargs): pass
            ...
            async def test_backend(backend_type):
                ...
                adapter = MeshPubSub(backend_url=f"{backend_type}://mock_url", use_native_dlq=True, enable_dlq_rotation=True, log_payloads=True)
                
                # Mock client for each backend
                if backend_type == "redis": adapter._client = MockClient("redis")
                elif backend_type == "nats": adapter._client = MockClient("nats")
                elif backend_type == "kafka": adapter._producer = MockClient("kafka"); adapter._consumer = MockClient("kafka")
                elif backend_type == "rabbitmq": adapter._rabbitmq_conn = MockClient("rabbitmq"); adapter._rabbitmq_channel = MockClient("rabbitmq")
                elif backend_type == "aws": adapter._aws_sqs_client = MockClient("aws_sqs"); adapter._aws_sns_client = MockClient("aws_sns")
                elif backend_type == "gcs": adapter._gcs_pubsub_publisher = MockClient("gcs_pubsub"); adapter._gcs_pubsub_subscriber = MockClient("gcs_pubsub")
                elif backend_type == "azure": adapter._azure_servicebus_client = MockClient("azure")
                elif backend_type == "etcd": adapter._etcd_client = MockClient("etcd")

                await adapter.connect()
                await adapter.publish(f"test_{backend_type}_channel", {"msg": "hello", "timestamp": time.time()})
                await adapter.close()
