...
async def mock_kafka_adapter():
    ...
    with patch("mesh.mesh_adapter.AIOKafkaProducer") as mock_producer, \
...
def mock_metrics():
    ...
    metrics = {}
    metric_names = [
        "PUB_COUNT", "PUB_FAIL_COUNT", "SUB_COUNT", "SUB_FAIL_COUNT",
        "DLQ_COUNT", "DLQ_REPLAY_COUNT", "CONNECT_STATUS", "CONNECT_LATENCY"
    ]
...
class TestPublishing:
    ...
    async def test_publish_redis(self, redis_adapter, test_message):
        ...
        if msg is None:
            ...
            with patch.object(redis_adapter._client, 'publish', new_callable=AsyncMock) as mock_publish:
                await redis_adapter.publish("test_channel", test_message)
...
class TestDeadLetterQueue:
    ...
    async def test_dlq_write(self, redis_adapter):
        ...
        with open(dlq_path, 'r') as f:
            ...
            if "encrypted" in entry:
                # Decrypt the entry
                decrypted_str = redis_adapter.multi_fernet.decrypt(entry['encrypted'].encode()).decode()
    ...
    async def test_dlq_replay(self, redis_adapter):
        ...
        # Create DLQ entries
        dlq_entries = [
            {"channel": f"replay_{i}", "message": {"id": i}, "exc": "test", "time": time.time()}
            for i in range(3)
        ]
...
class TestReliability:
    ...
    async def test_circuit_breaker(self, redis_adapter):
        ...
        if "redis" in circuit_breakers and circuit_breakers["redis"].breaker:
            ...
            # Simulate failures
            failure_count = 0
            with patch.object(redis_adapter._client, 'publish', side_effect=ConnectionError("Failed")):
    ...
    async def test_rate_limiting(self, redis_adapter):
        ...
        tasks = [
            redis_adapter.publish(f"rate_test_{i}", {"id": i})
            for i in range(20)
        ]
