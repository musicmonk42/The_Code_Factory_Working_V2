...
async def local_backend():
    ...
    backend = MeshPolicyBackend(
        backend_type="local",
        policy_schema=TestPolicySchema,
...
class TestPolicyOperations:
    ...
    async def test_batch_operations(self, local_backend, test_policy):
        ...
        policies = [
            {"policy_id": f"batch_{i}", "policy_data": {**test_policy, "id": f"batch_{i}"}}
            for i in range(5)
        ]
...
"""
        ...
        assert result.returncode == 100, f"Expected exit code 100, got {result.returncode}. Output: {result.stdout} Error: {result.stderr}"
