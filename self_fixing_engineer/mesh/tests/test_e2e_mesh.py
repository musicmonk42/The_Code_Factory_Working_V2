...
from mesh import event_bus, mesh_policy, mesh_adapter, checkpoint_manager
...
async def services():
    ...
    policy_enforcer = mesh_policy.MeshPolicyEnforcer(policy_id=TEST_POLICY_ID, backend=policy_backend)
...
class TestFailureAndRecovery:
    ...
    async def test_dlq_recovery_workflow(self, services):
        ...
        with patch.object(services['checkpoint_manager'], '_local_save', 
                         side_effect=Exception("Simulated failure")):
            try:
                await services['checkpoint_manager'].save(
                    name="failing_checkpoint",
                    state={"data": "test"}
                )
            except Exception:
...
class TestSecurityIntegration:
    ...
    async def test_encryption_key_rotation(self, services):
        ...
        hash2 = await services['checkpoint_manager'].save(
            name="post_rotation_checkpoint",
            state=new_checkpoint_data
        )
