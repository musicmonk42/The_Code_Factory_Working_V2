...
@pytest.fixture
async def crew_manager(mock_policy, mock_metrics_hook, mock_audit_hook, mock_sandbox_runner, mock_agent_health_poller, mock_agent_stop_commander, monkeypatch):
...
async def test_sanitize_dict():
    ...
    data = {"password": "secret", "safe": "value", "nested": {"data": "ok", "secret_key": "hidden"}}
...
async def test_add_agent_success(crew_manager):
    ...
    agent_info = await crew_manager.add_agent("test_agent", TestAgent, config={"key": "value"}, tags=["tag1"], metadata={"meta": "data"})
...
async def test_start_agent_resource_error(crew_manager, monkeypatch):
    ...
    if _PSUTIL_AVAILABLE:
        ...
        monkeypatch.setattr(crew_manager, "_start_sandbox_with_retries", mock_start_sandbox_with_retries)
