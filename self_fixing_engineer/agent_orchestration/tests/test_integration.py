...
def temp_config(tmp_path):
    ...
    config = {
        "agents": [
            {"id": "agent1", "name": "agent_one", "manifest": "test", "entrypoint": "run", "agent_type": "ai"}  # Changed to "agent_one"
        ],
        "compliance_controls": {
...
@pytest.mark.asyncio
async def test_integration_load_config_and_manage_agents(temp_config, mock_sandbox_runner, mock_agent_health_poller):
...
@pytest.mark.asyncio
async def test_integration_scale_with_config(temp_config, mock_sandbox_runner, mock_agent_health_poller):
