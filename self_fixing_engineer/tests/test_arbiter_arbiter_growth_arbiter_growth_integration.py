...
async def test_integration_full_event_flow(
    ...
    # Verify logging
    assert (
        "Processing growth event" in caplog.text
        or "Event skill_improved for test_arbiter" in caplog.text
    )
...
async def test_integration_concurrent_events(arbiter_manager_factory):
    ...
    tasks = [manager.improve_skill(f"skill_{i}", 0.01) for i in range(10)]
...
async def test_integration_anomaly_detection(arbiter_manager_factory):
    ...
    await manager.record_growth_event(
        "skill_improved", {"skill_name": "anomaly_test", "improvement_amount": 10.0}
    )
