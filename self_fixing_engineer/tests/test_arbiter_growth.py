...
create_module_stub("etcd3", {"client": lambda **kwargs: MagicMock()})
...
async def test_custom_growth_event(basic_manager):
    ...
    await basic_manager.record_growth_event(
        "custom_achievement", {"achievement": "first_pr_merged", "repo": "test-repo"}
    )
...
async def test_concurrent_operations():
    ...
    tasks = [mgr.acquire_skill(f"skill_{i}", 0.1 * i) for i in range(10)]
