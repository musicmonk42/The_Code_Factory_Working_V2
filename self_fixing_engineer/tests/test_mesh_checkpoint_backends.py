...
def mock_encryption():
    ...
    with patch("mesh.checkpoint.checkpoint_backends.encryption_mgr") as mock_mgr:
        mock_mgr.encrypt.side_effect = lambda x: b"encrypted_" + (
            x if isinstance(x, bytes) else x.encode()
        )
        mock_mgr.decrypt.side_effect = lambda x: (
            x.replace(b"encrypted_", b"") if b"encrypted_" in x else x
        )
...
class TestS3Backend:
    ...
    @pytest.mark.asyncio
    async def test_s3_save(
        self, backend_registry, s3_client_mock, test_data, mock_metrics, mock_encryption
    ):
...
class TestRedisBackend:
    ...
    @pytest.mark.asyncio
    async def test_redis_save(
        self, backend_registry, redis_client_mock, test_data, mock_encryption
    ):
    ...
    @pytest.mark.asyncio
    async def test_redis_load(
        self, backend_registry, redis_client_mock, test_data, mock_encryption
    ):
        """Test Redis load operation."""
        from mesh.checkpoint.checkpoint_backends import redis_load

        checkpoint_data = {"state": test_data.state, "metadata": {"hash": "test_hash"}}
...
class TestPostgresBackend:
    ...
    async def postgres_pool_mock(self):
        ...
        with patch(
            "mesh.checkpoint.checkpoint_backends.asyncpg.create_pool",
    ...
    async def test_postgres_save(
        self, backend_registry, postgres_pool_mock, test_data, mock_encryption
    ):
    ...
    async def test_postgres_load(
        ...
        with patch(
            "mesh.checkpoint.checkpoint_backends.decompress_json",
            side_effect=lambda x: json.loads(x),
        ):
