...
def mock_secrets_manager(mocker):
    mocker.patch.object(
        SECRETS_MANAGER,
        "get_secret",
        side_effect=lambda key, **kwargs: f"mock_secret_{key.lower()}",
    )
    # Default to returning a valid private key
    SECRETS_MANAGER.get_secret.side_effect = lambda key, **kwargs: (
        "0x" + "1" * 64 if "private_key" in key.lower() else "mock_secret"
    )
...
def mock_web3_provider(mocker):
    ...
    mock_w3.eth.get_block = MagicMock(
        return_value=MagicMock(baseFeePerGas=20000000000)
    )  # EIP-1559 mock
    mock_w3.eth.gas_price = 30000000000  # 30 Gwei
    mock_w3.eth.account.sign_transaction = MagicMock(
        return_value=MagicMock(rawTransaction=b"signed_tx")
    )

    # Mock the contract
    mock_contract = MagicMock()
    mock_contract.functions.writeCheckpoint = MagicMock(
        return_value=MagicMock(build_transaction=MagicMock())
    )
    ...
    mock_contract.functions.rollbackCheckpoint = MagicMock(
        return_value=MagicMock(build_transaction=MagicMock())
    )
...
@pytest.mark.parametrize(
    "chain_id,connected,contract_code,balance,expected_exception,message_part",
    [
        ...
        (
            ...
            None,
            "connected",
        ),  # Low balance - should warn, not fail
    ],
)
...
async def test_write_checkpoint_success(mock_off_chain, mock_web3_provider, mocker):
    ...
    mock_web3_provider.eth.wait_for_transaction_receipt.return_value = MagicMock(
        status=1, blockNumber=123
    )
    mock_web3_provider.eth.get_transaction_receipt.return_value = MagicMock(
        status=1, blockNumber=123
    )
    ...
    assert version == 123
    mock_off_chain.save_blob.assert_called_with(
        "test-checkpoint", b"mock_data", correlation_id=None
    )
...
async def test_read_checkpoint_success(mock_off_chain, mock_web3_provider, mocker):
    ...
    mock_web3_provider.eth.contract().functions.getLatestCheckpoint.return_value.call.return_value = (
        mock_entry_tuple
    )
