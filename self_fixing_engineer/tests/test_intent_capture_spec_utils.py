...
def mock_prometheus():
    ...
    mock_counter = MagicMock(
        inc=MagicMock(), labels=MagicMock(return_value=MagicMock(inc=MagicMock()))
    )
...
def mock_memory():
    """Mock agent memory."""
    mock_mem = MagicMock(
        load_memory_variables=MagicMock(return_value={"history": "mock_transcript"})
    )
    yield mock_mem
...
def mock_checklist():
    ...
    with patch(
        "intent_capture.spec_utils.get_checklist",
        AsyncMock(return_value=[{"name": "req", "weight": 1, "description": "desc"}]),
    ):
        yield
...
def test_load_ambiguous_words_retry():
    ...
    with patch(
        "intent_capture.spec_utils.requests.get",
        side_effect=[requests.exceptions.ConnectionError, mock_resp_success],
    ) as mock_get:
        with patch(
            "intent_capture.spec_utils.os.environ.get",
            side_effect=lambda k, d=None: (
    ...
    with patch("intent_capture.spec_utils.PromptTemplate") as mock_prompt_template:
        ...
        with patch(
            ...
            spec, notes = await auto_fix_spec(
                ...
                mock_llm,
                "json",
                [{"line": 1, "sentence": "sent", "vague_elements": ["word"]}],
            )
...
@pytest.mark.asyncio
async def test_generate_spec_from_memory_success(
    mock_memory, mock_tracer, mock_prometheus, mock_checklist, temp_locales
):
...
@pytest.mark.asyncio
async def test_generate_spec_from_memory_auto_fix(
    mock_memory, mock_tracer, mock_prometheus, mock_checklist, temp_locales
):
