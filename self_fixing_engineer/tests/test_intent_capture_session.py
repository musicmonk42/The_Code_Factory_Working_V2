...
def mock_asyncpg():
    ...
    with patch(
        "intent_capture.requirements.asyncpg.create_pool",
...
def mock_prometheus():
    ...
    with (
        patch("intent_capture.requirements.REQ_SUGGESTIONS_TOTAL", mock_counter),
        patch(
            "intent_capture.requirements.REQ_SUGGESTIONS_LATENCY_SECONDS",
            mock_histogram,
        ),
                ...
                "proj": [
                    {
                        ...
                        "covered_items": 5,
                        "total_items": 10,
                        "timestamp": "2025-09-11T19:00:25.233082",
                    }
                ]
            }
        )
...
async def test_update_item_status_custom(mock_asyncpg):
    ...
    with patch(
        ...
        with patch(
            "intent_capture.requirements.RequirementsManager.set_global_custom_checklists",
...
async def test_suggest_requirements_ml(mock_sentence_transformer, mock_llm):
    ...
    with patch(
        ...
        suggestions = await suggest_requirements(
            "fintech", "snippet", REQUIREMENTS_CHECKLIST, mock_llm
        )
...
async def test_generate_coverage_report():
    ...
    with patch(
        "intent_capture.requirements.RequirementsManager.get_coverage_history",
        AsyncMock(
            return_value=[
                {
                    "domain": "dom",
