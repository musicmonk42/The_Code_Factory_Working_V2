...
def test_histogram_buckets_and_idempotency():
    h1 = get_or_create_metric(
        Histogram,
        "test_histogram_idem",
        "Idempotent histogram",
        ("label3",),
        buckets=(0.1, 1, 10),
    )
    h2 = get_or_create_metric(
        Histogram,
        "test_histogram_idem",
        "Idempotent histogram",
        ("label3",),
...
def test_dynamic_compliance_metrics_exist(mock_config):
    ...
    COMPLIANCE_CONTROL_STATUS.labels(
        control_id="FAKE-1", status_detail="enforced", required="true"
    ).set(1)
    COMPLIANCE_CONTROL_STATUS.labels(
        control_id="FAKE-2", status_detail="logged", required="false"
    ).set(0)
...
def test_prometheus_scrape_for_all_metrics(mock_config):
    ...
    # Force metrics to be registered if they aren't already
    # by accessing them from the metrics module
