...
async def mock_ml_platform_client(mocker: MockerFixture):
    ...
    client.get_training_job_status = mocker.AsyncMock(
        return_value={
            "status": "completed",
            "model_id": "mock_model",
...
async def orchestrator(
    ...
    mocker.patch.object(
        orchestrator,
        "_verify_leadership_and_fencing",
        new_callable=mocker.AsyncMock,
        return_value=True,
    )
            ...
            try:
                ...
                if not orchestrator._is_leader:
                    await orchestrator._become_leader(
                        {"instance_id": orchestrator._instance_id, "token": 123}
                    )
                # Always ensure training check task exists and is running for leader
                if orchestrator._is_leader and (
                    not orchestrator._training_check_task
                    or orchestrator._training_check_task.done()
                ):
...
async def test_ingest_learning_record_success(orchestrator, mocker: MockerFixture):
    ...
    mocker.patch.object(
        orchestrator.ingestor, "ingest_learning_record", new_callable=mocker.AsyncMock
    )
...
async def test_training_check_core(orchestrator, mocker: MockerFixture):
    ...
    mocker.patch.object(
        ...
        "trigger_model_training_and_deployment",
        new_callable=mocker.AsyncMock,
...
async def test_health_status(orchestrator, mocker: MockerFixture):
    ...
    mocker.patch.object(
        orchestrator.ingestor.kafka_producer, "bootstrap_connected", return_value=True
    )
