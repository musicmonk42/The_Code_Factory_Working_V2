...
def test_circuit_breaker_open_error_init(caplog):
    ...
    assert (
        str(error)
        == "Circuit breaker open (Details: {'breaker_name': 'snapshot', 'reset_timeout': 60})"
    )
...
def test_audit_chain_tampered_error_init(caplog):
    ...
    assert (
        str(error)
        == "Audit chain validation failed (Details: {'expected_hash': 'abc123', 'actual_hash': 'xyz789'})"
    )
...
def test_arbiter_growth_error_complex_details(caplog):
    ...
    assert (
        "Exception raised: ArbiterGrowthError, Details: {'nested': {'key': 'value', 'list_of_ints': [1, 2, 3]}}"
        in caplog.text
    )
...
def test_rate_limit_error_metrics(caplog):
    ...
    assert (
        "Exception raised: RateLimitError, Details: {'limit': 10, 'current_rate': 12, 'exceeds_by': 2}"
        in caplog.text
    )
