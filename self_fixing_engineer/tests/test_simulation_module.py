...
SAMPLE_SIMULATION_CONFIG = {
    "type": "agent",
    "id": "test-sim-001",
    "parameters": {"iterations": 100, "target_accuracy": 0.95},
}
...
SAMPLE_QUANTUM_PARAMS = {"circuit_type": "variational", "qubit_count": 5, "shots": 1000}
...
def mock_reasoner():
    ...
    mock.explain_result = AsyncMock(
        return_value="This is an explanation of the simulation results."
    )
...
def mock_quantum_api():
    ...
    mock = AsyncMock()
    mock.perform_quantum_operation = AsyncMock(
        return_value={
            "status": "SUCCESS",
...
def mock_agent_runners():
    ...
    with (
        patch("simulation_module.run_agent") as mock_run_agent,
        ...
        mock_parallel.return_value = {
            "status": "success",
            "results": [
                {"id": "task-1", "accuracy": 0.91},
                {"id": "task-2", "accuracy": 0.93},
            ],
        }

        yield {"agent": mock_run_agent, "swarm": mock_swarm, "parallel": mock_parallel}
...
async def test_health_check_failure_reasoner(
    ...
    mock_reasoner.execute.return_value = {
        "status": "error",
...
@pytest.mark.asyncio
async def test_explain_result_invalid_input(simulation_module_instance, mock_reasoner):
...
@pytest.mark.asyncio
async def test_explain_result_reasoner_error(simulation_module_instance, mock_reasoner):
...
@pytest.mark.asyncio
async def test_run_in_secure_sandbox(simulation_module_instance, mock_sandbox):
...
@pytest.mark.asyncio
async def test_register_message_handlers(simulation_module_instance, mock_message_bus):
...
@pytest.mark.asyncio
async def test_register_message_handlers_not_initialized(mock_db, mock_message_bus):
...
async def test_handle_simulation_request_success(
    ...
    mock_message_bus.publish.assert_awaited_once_with(
        topic="responses.simulation.agent", payload=ANY
    )
