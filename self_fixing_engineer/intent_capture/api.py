...
from agent_core import (AgentError, ConfigurationError, InvalidSessionError,
                        get_or_create_agent)
...
class AppConfig(BaseSettings):
    ...
    def __init__(self, **values: Any):
        ...
        self.JWT_SECRET_KEY = self._get_secret("JWT_SECRET", "secrets/data/api", "a_very_strong_and_long_secret_key_for_demo_thirty_two_chars_or_more")
...
class PredictRequest(BaseModel):
    ...
    def validate_and_sanitize_input(cls, v):
        if not v or not v.strip(): raise ValueError("User input cannot be empty.")
...
SAFETY_VIOLATIONS_TOTAL = Counter("agent_safety_violations_total", "Total responses blocked by safety guardrails")
...
async def get_current_user(token: str = Depends(oauth2_scheme), redis_client: aredis.Redis = Depends(get_redis_client)):
    credentials_exception = HTTPException(status.HTTP_401_UNAUTHORIZED, "Could not validate credentials", headers={"WWW-Authenticate": "Bearer"})
    try:
        if await redis_client.sismember("jwt_blocklist", token):
            raise credentials_exception
        payload = jwt.decode(token, config.JWT_SECRET_KEY, algorithms=[config.JWT_ALGORITHM], audience="agent_core_user", issuer="agent_core_auth")
        return payload
    except (JOSEError, InvalidSessionError):
        raise credentials_exception

async def set_user_state_for_limiter(request: Request, current_user: dict = Depends(get_current_user)) -> dict:
...
async def lifespan(app: FastAPI):
    if dsn := os.getenv("SENTRY_DSN"): sentry_sdk.init(dsn=dsn, traces_sample_rate=0.2)
    redis_client = await get_redis_client()
    await FastAPICache.init(RedisBackend(redis_client), prefix="api-cache")
    if hf_pipeline: app.state.safety_pipeline = hf_pipeline('text-classification', model='unitaryai/toxic-bert')
    ...
    async def refresh_secrets_loop():
        while True:
            ...
            config.__init__()
    if config.USE_VAULT: app.state.secret_refresher = asyncio.create_task(refresh_secrets_loop())

    logger.info("Application startup complete.")
    yield
    if hasattr(app.state, 'secret_refresher'): app.state.secret_refresher.cancel()
...
def create_app() -> FastAPI:
    app = FastAPI(title=config.APP_NAME, version=config.API_VERSION, lifespan=lifespan, exception_handlers={Exception: global_exception_handler})
    ...
    async def login_for_access_token():
        ...
        to_encode = { "sub": "demo_user", "tier": "standard", "consent_prune": True, "session_id": session_id, "exp": datetime.utcnow() + timedelta(hours=1), "iss": "agent_core_auth", "aud": "agent_core_user" }
        return {"access_token": jwt.encode(to_encode, config.JWT_SECRET_KEY, algorithm=config.JWT_ALGORITHM), "token_type": "bearer"}

    @app.post("/predict", response_model=PredictResponse, tags=["Agent Interaction"], summary="Get a response from the AI agent", dependencies=[Depends(dynamic_rate_limiter)])
    @cache(expire=300, key_builder=lambda f, *args, **kwargs: f"predict:{hash(kwargs['request_data'].user_input + kwargs['current_user']['session_id'])}")
    async def predict_agent_response(request: Request, request_data: PredictRequest, current_user: dict = Depends(set_user_state_for_limiter)):
        span = trace.get_current_span(); span.set_attribute('user.id', current_user.get("sub")); span.set_attribute('request.id', request.state.request_id)
        ...
        try:
            ...
            if os.getenv('USE_QUEUE', 'false').lower() == 'true':
                ...
                return {"response": "Prediction task has been queued.", "trace": {"status": "queued"}}
            else:
                agent = await get_or_create_agent(request_data.session_token)
                prediction_result = await agent.predict(request_data.user_input, timeout=request_data.timeout)
    ...
    @app.post("/prune_sessions", status_code=204, tags=["Compliance"], summary="Prune user data based on retention policy")
    async def prune_old_sessions(current_user: dict = Depends(get_current_user)):
        if not current_user.get('consent_prune', False):
            raise HTTPException(status.HTTP_403_FORBIDDEN, "User has not consented to data pruning.")
