...
def mock_prometheus():
    ...
    with patch('intent_capture.io_utils.FILE_OPS_TOTAL', mock_counter), \
         patch('intent_capture.io_utils.FILE_OPS_LATENCY_SECONDS', mock_histogram), \
         patch('intent_capture.io_utils.DOWNLOAD_LATENCY_SECONDS', mock_histogram), \
         patch('intent_capture.io_utils.DOWNLOAD_BYTES_TOTAL', mock_counter), \
         patch('intent_capture.io_utils.IN_PROGRESS_DOWNLOADS', mock_gauge), \
         patch('intent_capture.io_utils.SAFETY_VIOLATIONS_TOTAL', mock_counter):
        yield
...
# --- Tests for Hash File ---
@pytest.mark.asyncio
async def test_hash_file_distributed_cache_success(file_manager, temp_workspace, mock_redis, mock_prometheus):
...
@pytest.mark.asyncio
async def test_hash_file_distributed_cache_cached(file_manager, temp_workspace, mock_redis, mock_prometheus):
...
@pytest.mark.asyncio
async def test_download_file_to_temp_success(file_manager, mock_circuit_breaker, mock_prometheus, monkeypatch):
...
def test_prune_audit_logs(mock_boto3, monkeypatch):
    ...
    mock_boto3.list_objects_v2.return_value = {
        'Contents': [
            {'Key': 'old_log.json', 'LastModified': old_date}
        ]
    }
