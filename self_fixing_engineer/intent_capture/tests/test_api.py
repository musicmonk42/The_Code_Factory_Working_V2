...
def mock_get_or_create_agent():
    ...
    mock_agent.predict = AsyncMock(return_value={"response": "mocked agent response", "trace": {"status": "mocked_success"}})
    
    with patch('intent_capture.api.get_or_create_agent', AsyncMock(return_value=mock_agent)) as mock_func:
        yield mock_func
...
@pytest.mark.asyncio
async def test_predict_success(async_client: AsyncClient, valid_token: str, mock_get_or_create_agent: AsyncMock, test_secret_key: str):
    ...
    auth_token_payload = {
        ...
        "iss": "agent_core_auth", "aud": "agent_core_user"
    }
...
@pytest.mark.asyncio
async def test_predict_agent_error(async_client: AsyncClient, valid_token: str, mock_get_or_create_agent: AsyncMock, test_secret_key: str):
...
@pytest.mark.asyncio
async def test_predict_timeout_error(async_client: AsyncClient, valid_token: str, mock_get_or_create_agent: AsyncMock, test_secret_key: str):
...
async def test_prune_sessions_success(async_client: AsyncClient, test_secret_key: str):
    ...
    token_payload = {
        "sub": "test_user", "consent_prune": True, "exp": datetime.utcnow() + timedelta(hours=1),
        "iss": "agent_core_auth", "aud": "agent_core_user"
    }
...
async def test_prune_sessions_forbidden(async_client: AsyncClient, test_secret_key: str):
    ...
    token_payload = {
        "sub": "test_user", "consent_prune": False, "exp": datetime.utcnow() + timedelta(hours=1),
        "iss": "agent_core_auth", "aud": "agent_core_user"
    }
    token = jwt.encode(token_payload, test_secret_key, algorithm="HS512")
    headers = {"Authorization": f"Bearer {token}"}
...
def test_app_config_secret_handling(monkeypatch, test_secret_key):
    ...
    assert config._get_secret("NON_EXISTENT_SECRET", "dummy/path", default="default_val") == "default_val"
