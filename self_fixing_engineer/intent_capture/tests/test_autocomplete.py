...
def test_log_audit_event():
    ...
    with patch.dict(os.environ, {'ENABLE_AUDIT': 'true', 'AUDIT_BUCKET': 'test-bucket', 'AWS_REGION': 'us-east-1'}):
        with patch('intent_capture.autocomplete.boto3.client') as mock_boto:
            with patch('intent_capture.autocomplete.os.getlogin', return_value='testuser'):
                ...
                assert 'test command' in call_args.kwargs['Body'] or '[REDACTED' in call_args.kwargs['Body']