...
sys.modules['intent_capture.requirements'] = MagicMock(
    get_coverage_history=AsyncMock(),
    generate_coverage_report=AsyncMock()
)
...
class TestAuthenticationSecurity:
    ...
    def test_password_hashing_on_load(self, tmp_path):
        ...
        plaintext_config = {
            "credentials": {
                "usernames": {
                    "user1": {"password": "plaintext_password"}
                }
            }
        }
...
class TestMetrics:
    ...
    def test_metrics_increment_on_page_view(self):
        ...
        with patch.object(web_app.st, 'session_state', mock_state):
            with patch.object(web_app, 'LOCALES', {"en": {"chat_header": "Chat", "page_dashboard": "Dashboard"}}):
