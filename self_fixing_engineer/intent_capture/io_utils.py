...
if PROMETHEUS_AVAILABLE:
    ...
    FILE_OPS_LATENCY_SECONDS = Histogram("io_file_ops_latency_seconds", "File operation latency", ["operation"])
    ...
    FILE_OPS_TOTAL = FILE_OPS_LATENCY_SECONDS = DOWNLOAD_LATENCY_SECONDS = DOWNLOAD_BYTES_TOTAL = IN_PROGRESS_DOWNLOADS = SAFETY_VIOLATIONS_TOTAL = None

# --- OTEL Tracing ---
telemetry_tracer = trace.get_tracer(__name__) if OTEL_AVAILABLE else None

# --- Circuit Breakers ---
download_breaker = CircuitBreaker(fail_max=5, timeout_duration=120) if AIOBREAKER_AVAILABLE else None
...
class FileManager:
    ...
    def validate_path(self, path: str) -> str:
        ...
        if not real_path.startswith(self.workspace):
            raise PermissionError(f"Path traversal attempt detected: '{path}' resolves outside workspace.")
        return real_path
...
async def get_redis_client():
    ...
    if REDIS_AVAILABLE and os.getenv("REDIS_URL"):
        try:
            if redis_breaker:
                client = await redis_breaker.call_async(aredis.from_url, os.getenv("REDIS_URL"), decode_responses=True)
            else:
...
async def hash_file_distributed_cache(path: str, file_manager: FileManager) -> str:
    ...
    if os.getenv('CHECK_BIAS', 'false') == 'true':
        def is_biased(text): return False # Placeholder
        with open(validated_path, 'r', encoding="utf-8", errors="ignore") as f:
            content = f.read()
            if is_biased(content):
                raise Exception('File content blocked for bias')

    with FILE_OPS_LATENCY_SECONDS.labels(operation="hash").time() if FILE_OPS_LATENCY_SECONDS else contextmanager(lambda: (yield))():
...
async def download_file_to_temp(url: str, file_manager: FileManager) -> Optional[str]:
    ...
    if os.getenv('USE_QUEUE', 'false') == 'true':
        try:
            ...
            channel.basic_publish(
                exchange='', routing_key='io_downloads',
    ...
    async def _perform_download():
        with IN_PROGRESS_DOWNLOADS.track_inprogress() if IN_PROGRESS_DOWNLOADS else contextmanager(lambda: (yield))():
            if telemetry_tracer:
    ...
    try:
        with DOWNLOAD_LATENCY_SECONDS.time() if DOWNLOAD_LATENCY_SECONDS else contextmanager(lambda: (yield))():
...
def log_audit_event(event_type: str, data: Dict):
    ...
    try:
        log_data = {'timestamp': datetime.now().isoformat(), 'user': os.getlogin(), 'event_type': event_type, 'data': json.dumps(data)}
...
def prune_audit_logs(retention_days: int = 90):
    if os.getenv('CONSENT_PRUNE', 'true').lower() != 'true' or not boto3: return
    try:
        s3 = boto3.client('s3', aws_access_key_id=os.getenv('AWS_ACCESS_KEY_ID'), aws_secret_access_key=os.getenv('AWS_SECRET_KEY'), region_name=os.getenv('AWS_REGION', 'us-east-1'))
        bucket = os.getenv('AUDIT_BUCKET', 'io-audit-logs')
        ...
        if 'Contents' in response:
            ...
            keys = [obj['Key'] for obj in response['Contents'] if obj['LastModified'].replace(tzinfo=None) < cutoff]
            if keys:
