...
# Prometheus Metrics
COMPLETION_LATENCY_SECONDS = Histogram("cli_completion_latency_seconds", "CLI autocomplete latency", ["user", "operation"])
...
SAFETY_VIOLATIONS_TOTAL = Counter("cli_safety_violations_total", "Safety violations in CLI suggestions")
KEY_REFRESH_SUCCESS_TIMESTAMP = Gauge("cli_key_refresh_success_timestamp_seconds", "Timestamp of last successful key refresh")
...
class JsonFormatter(logging.Formatter):
    def format(self, record):
        log_record = {"timestamp": time.time(), "level": record.levelname,
                      "message": self._mask_pii(record.getMessage()), "name": record.name}
...
class FernetEncryptor:
    ...
    def decrypt(self, token: bytes) -> str:
        try: return self.fernet.decrypt(token).decode()
...
class CommandRegistry:
    ...
    def update_all_commands(self):
        cmd_set = {"help", "exit", "quit", "ai:", "set provider", "prune_history", "fetch_suggestion"}
...
class AutocompleteState:
    ...
    @retry(retry=retry_if_exception_type(Exception), stop=stop_after_attempt(3), wait=wait_exponential(multiplier=2, min=2, max=10))
    ...
    async def _fetch_key_from_vault(self) -> Optional[bytes]:
        if os.getenv("USE_VAULT", "false").lower() != "true": return None
        try:
            client = hvac.Client(url=os.getenv("VAULT_URL"), token=os.getenv("VAULT_TOKEN"))
            if client.is_authenticated():
                secret = client.secrets.kv.v2.read_secret_version(path="secret/data/autocomplete/encryption")
            ...
            if os.getenv("SENTRY_DSN"): sentry_sdk.capture_exception(e)
        return None
...
async def get_ai_suggestions(text: str, state: AutocompleteState) -> List[str]:
    if not state.llm_instance: return []
    ...
    async def _invoke_llm(prompt: str) -> List[str]:
        ...
        TOKEN_USAGE.labels(user=os.getlogin(), provider=state.llm_provider).inc(getattr(response, "token_usage", 0))
...
@retry(retry=retry_if_exception_type(RedisConnectionError), stop=stop_after_attempt(3), wait=wait_exponential(multiplier=2, min=2, max=10))
async def fuzzy_matches(text: str, candidates: List[str], state: AutocompleteState) -> List[str]:
    if not text: return sorted(candidates)
...
class CommandCompleter:
    ...
    def complete(self, text: str, state_index: int) -> Optional[str]:
        with tracer.start_as_current_span("cli_completion") as span, \
             COMPLETION_LATENCY_SECONDS.labels(user=os.getlogin(), operation="total").time(), \
    ...
    async def _async_complete(self, text: str, state_index: int) -> Optional[str]:
        ...
        if line.startswith("ai: "):
            if os.getenv('USE_QUEUE', 'false').lower() == 'true':
                ...
                try:
                    for _ in range(3):  # UPGRADE: Circuit breaker for RabbitMQ - [Date: August 19, 2025]
                        try:
                            ...
                            channel.basic_publish(
                                ...
                                body=json.dumps({'query': query, 'session': os.getpid(), 'state_index': state_index}),
                                properties=pika.BasicProperties(delivery_mode=2, content_type='application/json', priority=0)
                            )
                            connection.close()
                            return 'AI suggestion queued (use "fetch_suggestion")' if state_index == 0 else None
            ...
            with COMPLETION_LATENCY_SECONDS.labels(user=os.getlogin(), operation="ai_suggestion").time():
...
def log_audit_event(line: str, result: str):
    if os.getenv("ENABLE_AUDIT", "false").lower() != "true": return
    try:
        ...
        if not bucket or not region:
            ...
            return
        log_data = {'timestamp': time.time(), 'user': os.getlogin(), 'input': anonymize_pii(line), 'result': anonymize_pii(result)}
        s3 = boto3.client('s3',
    ...
    async def _async_setup():
        ...
        if state.encryptor and os.path.exists(histfile):
            with open(histfile, "rb") as f:
                for line_token in f:
                    if not line_token.strip(): continue
                    decrypted = state.encryptor.decrypt(line_token.strip())
                    if decrypted: readline.add_history(anonymize_pii(decrypted))
        def save_encrypted_history():
            if not state.encryptor: return
...
def startup_validation():
    ...
    if dsn := os.getenv("SENTRY_DSN"):
        sentry_sdk.init(dsn=dsn, traces_sample_rate=0.1, environment=os.getenv("ENVIRONMENT", "production"))
        logger.info('Sentry SDK initialized.')

if __name__ == "__main__":
    setup_logging()
    startup_validation()
    if p := os.getenv("METRICS_PORT"): start_http_server(int(p))
    ...
    try:
        while True:
            ...
            state = asyncio.run(AutocompleteState.instance())
            if line.split()[0] not in state.command_registry.all_commands and not line.startswith("ai: "):
