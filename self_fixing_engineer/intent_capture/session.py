...
try:
    ...
    SESSION_SAVE_ATTEMPTS = Counter("session_save_attempts_total", "Total attempts to save session", ["session_name"])
    SESSION_LOAD_ATTEMPTS = Counter("session_load_attempts_total", "Total attempts to load session", ["session_name"])
    SESSION_ERRORS = Counter("session_errors_total", "Total session errors", ["session_name", "error_type"])
    SESSION_SAVE_LATENCY = Histogram("session_save_latency_seconds", "Latency of saving session", ["session_name"])
    SESSION_LOAD_LATENCY = Histogram("session_load_latency_seconds", "Latency of loading session", ["session_name"])
    SESSIONS_PRUNED_TOTAL = Counter("sessions_pruned_total", "Total sessions pruned", ["reason"])
    SESSION_PRUNE_LATENCY_SECONDS = Histogram("session_prune_latency_seconds", "Latency of session pruning operation")
except ImportError:
...
try:
    ...
    def log_action(*args, **kwargs): pass # Dummy log_action
    ...
    with _config_lock:
        if _CONFIG_INSTANCE is None:
            try:
                ...
                logger.warning("Config module not found. Session management will use hardcoded defaults for storage paths and history sizes.")
                _CONFIG_INSTANCE = None
            except ValidationError as e:
                logger.error(f"Config validation failed at session module load: {e}. Ensure all required environment variables are set for Config. Using defaults.")
                _CONFIG_INSTANCE = None
            except Exception as e:
                logger.error(f"Unexpected error loading Config module in session.py: {e}. Using defaults.", exc_info=True)
                _CONFIG_INSTANCE = None
        ...
        if _CONFIG_INSTANCE:
            session_storage_root = getattr(_CONFIG_INSTANCE, 'SESSION_STORAGE_PATH', session_storage_root)
            session_max_history_size = getattr(_CONFIG_INSTANCE, 'SESSION_MAX_HISTORY_SIZE', session_max_history_size)
            session_archive_enabled = getattr(_CONFIG_INSTANCE, 'SESSION_ARCHIVE_ENABLED', session_archive_enabled)
            session_archive_path = getattr(_CONFIG_INSTANCE, 'SESSION_ARCHIVE_PATH', session_archive_path)
            session_max_age_days = getattr(_CONFIG_INSTANCE, 'SESSION_MAX_AGE_DAYS', session_max_age_days)
        ...
        if _FERNET_INSTANCE is None and CRYPTOGRAPHY_AVAILABLE and encryption_key_str:
            try:
                ...
                logger.error(f"Failed to initialize Fernet with provided key: {e}. Session encryption disabled.", exc_info=True)
...
@backoff.on_exception(backoff.expo,
                      (portalocker.exceptions.LockException, FileExistsError, FileNotFoundError, IOError, json.JSONDecodeError),
                      max_tries=5,
    ...
    try:
        ...
        if config_params["encryption_enabled"] and _FERNET_INSTANCE:
            ...
            if isinstance(memory_obj, dict): # Ensure it's a dict before dumping
                ...
                logger.warning(f"Memory for session {session_name} is not a dict, skipping encryption.")
        ...
        SESSION_SAVE_LATENCY.observe({"session_name": session_name}, latency) # Updated for Prometheus client
        log_action("session_saved", {"session_id": session_name, "path": session_path, "size_bytes": await aiofiles.os.path.getsize(session_path), "latency_seconds": latency})
        ...
        log_action("session_save_failed", {"session_id": session_name, "error": str(e), "reason": "validation_failed"})
        return False
    except Exception as e:
        ...
        log_action("session_save_failed", {"session_id": session_name, "error": str(e), "reason": "io_error"})
        return False
...
async def load_session(session_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        session_path = _get_session_path(session_name)
        logger.debug(f"Attempting to load session from: {session_path} (exists: {await aiofiles.os.path.exists(session_path)})")
        ...
        if config_params["encryption_enabled"] and _FERNET_INSTANCE:
            ...
            if isinstance(encrypted_memory, str):
                try:
                    decrypted_memory_bytes = _FERNET_INSTANCE.decrypt(encrypted_memory.encode('utf-8'))
                    raw_data["memory"] = json.loads(decrypted_memory_bytes.decode('utf-8'))
                except Exception as e:
                    logger.error(f"Failed to decrypt memory for session {session_name}: {e}. Data might be corrupted.", exc_info=True)
                    SESSION_ERRORS.inc(session_name=session_name, error_type="decryption_failed")
                    return None # Fail to load if decryption fails
            else:
                logger.warning(f"Memory for session {session_name} is not encrypted string, skipping decryption.")
        ...
        latency = time.perf_counter() - start_time
        SESSION_LOAD_LATENCY.observe({"session_name": session_name}, latency) # Updated for Prometheus client
        ...
        SESSION_ERRORS.inc(session_name=session_name, error_type="validation_error")
        logger.error(f"Session data validation failed for loaded session '{session_name}': {e}. Data might be corrupted or outdated.")
        log_action("session_load_failed", {"session_id": session_name, "error": str(e), "reason": "validation_failed"})
        return None
    except Exception as e:
        SESSION_ERRORS.inc(session_name=session_name, error_type=type(e).__name__)
        logger.error(f"Failed to load session '{session_name}': {e}.", exc_info=True)
        log_action("session_load_failed", {"session_id": session_name, "error": str(e), "reason": "io_error"})
        return None
...
async def export_spec(spec_content: Union[str, Dict[str,Any]], file_format: str, output_path: str) -> bool:
    """
    ...
    try:
        ...
        content_to_write = spec_content if isinstance(spec_content, str) else json.dumps(spec_content, indent=2) # Ensure dict is dumped to string
        await _atomic_write_json(output_path, {"format": file_format, "content": content_to_write})
        
        log_action("spec_exported", {"format": file_format, "path": output_path, "size_bytes": len(content_to_write.encode('utf-8'))})
...
async def save_session_history(session_name: str, history_data: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        if len(history_data) > config_params["max_history_size"]:
            ...
            logger.warning(f"Truncated history for '{session_name}' to {config_params['max_history_size']} items.")

        await _atomic_write_json(history_path, history_data)
        
        log_action("session_history_saved", {"session_id": session_name, "path": history_path, "items_count": len(history_data)})
        ...
        logger.error(f"History data validation failed for '{session_name}': {e}. Save aborted.", exc_info=True)
        return False
    except Exception as e:
        SESSION_ERRORS.inc(error_type=type(e).__name__)
...
async def load_session_history(session_name: str) -> List[Dict[str, Any]]:
    ...
    try:
        history_path = _get_history_path(session_name)
        logger.debug(f"Attempting to load history from: {history_path} (exists: {await aiofiles.os.path.exists(history_path)})")
        
        history_data = await _read_json_file(history_path)
        if not isinstance(history_data, list):
            raise ValueError("History data is not a list.")
        
        log_action("session_history_loaded", {"session_id": session_name, "path": history_path, "items_count": len(history_data)})
        logger.info(f"History for session '{session_name}' loaded successfully from {history_path}.")
        return history_data
    except FileNotFoundError:
        logger.warning(f"History for session '{session_name}' not found at {_get_history_path(session_name)}. Returning empty list.")
        log_action("session_history_load_failed", {"session_id": session_name, "reason": "not_found"})
        return []
    except Exception as e:
        SESSION_ERRORS.inc(error_type=type(e).__name__)
        logger.error(f"Failed to load history for session '{session_name}': {e}.", exc_info=True)
        log_action("session_history_load_failed", {"session_id": session_name, "error": str(e), "reason": "io_error"})
        return []
...
async def delete_session(session_name: str) -> bool:
    ...
    if config_params["archive_enabled"]:
        ...
        if await aiofiles.os.path.exists(history_path):
            archive_history_path = os.path.join(archive_dir, f"{session_name}_history.{timestamp}.json")
            try:
    ...
    try:
        ...
        if not deleted_files:
            ...
            log_action("session_delete_failed", {"session_id": session_name, "reason": "not_found"})
            return False
        
        log_action("session_deleted", {"session_id": session_name, "deleted_files": deleted_files, "archived": config_params["archive_enabled"]})
        return True
    except Exception as e:
        SESSION_ERRORS.inc(session_name=session_name, error_type=type(e).__name__)
...
async def get_session_metadata(session_name: str) -> Optional[SessionMetadata]:
    ...
    try:
        ...
        if config_params["encryption_enabled"] and _FERNET_INSTANCE:
            ...
            if isinstance(encrypted_memory, str):
                try:
                    ...
                    logger.error(f"Failed to decrypt memory for metadata of session {session_name}: {e}. Skipping decryption.", exc_info=True)
        ...
        logger.error(f"Metadata validation failed for session '{session_name}': {e}. File might be corrupted.")
        return None
    except Exception as e:
        logger.error(f"Failed to retrieve metadata for session '{session_name}': {e}", exc_info=True)
        return None
...
async def prune_old_sessions(max_age_days: Optional[int] = None) -> int:
        ...
        if metadata:
            try:
                ...
                if age > timedelta(days=age_threshold):
                    logger.warning(f"Pruning session '{session_name}' as it is {age.days} days old.")
                    if await delete_session(session_name):
                        pruned_count += 1
                        SESSIONS_PRUNED_TOTAL.labels(reason="age_exceeded").inc()
                    else:
                        SESSIONS_PRUNED_TOTAL.labels(reason="delete_failed").inc() # Count failures to delete
            except Exception as e:
                logger.error(f"Error processing session '{session_name}' for pruning: {e}", exc_info=True)
    ...
    if pruned_count > 0:
        log_action("sessions_pruned", {"count": pruned_count, "max_age_days": age_threshold, "latency_seconds": latency})
