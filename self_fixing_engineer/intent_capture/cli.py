...
COMMAND_EXECUTION_TOTAL = Counter("cli_command_execution_total", "Commands executed", ["command", "status"])
COMMAND_LATENCY_SECONDS = Histogram("cli_command_latency_seconds", "Command execution latency", ["command"])
...
SAFETY_VIOLATIONS_TOTAL = Counter("cli_safety_violations_total", "Safety violations in CLI responses")
...
class SessionState:
    def __init__(self):
        self._state: Dict[str, Any] = {
            "agent": None, "collab_mode": "inactive", "collab_server": None,
            "collab_uri": None, "collab_queue": asyncio.Queue(), "undo_stack": deque(maxlen=100)
        }
        ...
        try:
            async for message in websocket:
                ...
                await asyncio.gather(
                    *[client.send(sanitized_message) for client in self.clients if client != websocket]
                )
...
def validate_environment():
    ...
    JWT_SECRET = fetch_jwt_from_vault() or os.getenv("CLI_JWT_SECRET", "default-insecure-secret-key-for-dev")
...
class JsonFormatter(logging.Formatter):
    def format(self, record):
        ...
        log_record = {"timestamp": datetime.datetime.now().isoformat(), "level": record.levelname,
                      "message": message, "name": record.name}
        return json.dumps(log_record)

def setup_logging():
    for handler in logging.root.handlers[:]:
        logging.root.removeHandler(handler)
    log_handler = logging.handlers.RotatingFileHandler("cli.log", maxBytes=10*1024*1024, backupCount=5)
...
def global_exception_handler(exc_type, exc_value, exc_traceback):
    ...
    CONSOLE.print("[bold red]A critical unhandled error occurred. The application will now exit. See cli.log for details.[/bold red]")
    sys.exit(1)
...
- `exit` / `quit`: Exits the CLI.
    ...
    async def _handle_collab_start(self, args: List[str]):
        ...
        if mode == "host":
            ...
            CONSOLE.print("[red]Usage: `collab start host` or `collab start client <uri> <token>`[/red]")

    async def _collab_client_listener(self, uri, queue):
        try:
            async with websockets.connect(uri) as websocket:
                await self.state.set("collab_uri", websocket)
                CONSOLE.print(f"[green]Connected to collaboration session at {uri.split('?')[0]}[/green]")
                while True:
                    message = await websocket.recv()
                    await queue.put(message)
        except (ConnectionClosed, websockets.exceptions.InvalidURI, websockets.exceptions.InvalidHandshake) as e:
            ...
            await queue.put(json.dumps({"type": "error", "payload": "An unexpected error occurred."}))
...
def log_audit_event(event_type: str, data: Dict):
    if os.getenv("ENABLE_AUDIT", "false").lower() != "true" or not boto3: return
    try:
        log_data = {'timestamp': datetime.datetime.now().isoformat(), 'user': os.getlogin(), 'event_type': event_type, 'data': json.dumps(data)}
        s3 = boto3.client('s3', aws_access_key_id=os.getenv('AWS_ACCESS_KEY_ID'), aws_secret_access_key=os.getenv('AWS_SECRET_KEY'), region_name=os.getenv('AWS_REGION', 'us-east-1'))
        s3.put_object(
...
async def main_cli_loop():
    ...
    try:
        ...
        from autocomplete import add_to_history, execute_macro, handle_command_not_found, setup_autocomplete
    except ImportError:
        def get_or_create_agent(*a, **k): 
            ...
            return Dummy()
        def add_to_history(x): pass
        def execute_macro(x): return x
        def handle_command_not_found(x,y): CONSOLE.print(f"Unknown command: {x}")
        def setup_autocomplete(llm=None): pass
    ...
    threading.Thread(target=lambda: _local_input_worker(asyncio.get_event_loop(), local_input_queue), daemon=True).start()
    ...
    while not _shutdown_event.is_set():
        try:
            ...
            if not sanitized_input: continue
            ...
            try:
                ...
                log_audit_event('command', {'input': sanitized_input, 'command': command, 'args': args})
            except ValueError:
                ...
                if os.getenv('USE_QUEUE', 'false') == 'true':
                    try:
                        ...
                        channel.basic_publish(
                            exchange='', routing_key='cli_tasks',
                            body=json.dumps({'sanitized_input': sanitized_input, 'session_id': 'default_cli_session'}),
                            properties=pika.BasicProperties(delivery_mode=2, content_type='application/json', priority=0)
                        )
                        connection.close()
                ...
                safe_response = bleach.clean(response.get('response', 'No response text.'), strip=True)
                TOKEN_USAGE.labels(user=os.getlogin(), provider="openai").inc(response.get("token_usage", 0))
