...
def setup_logging():
    ...
    handler.setFormatter(PiiMaskingFormatter('%(asctime)s - [%(levelname)s] - (Config) %(message)s'))
...
if PROMETHEUS_AVAILABLE:
    ...
    SAFETY_VIOLATIONS_TOTAL = Counter("config_safety_violations_total", "Safety violations in config")
else:
...
def _fetch_config_from_service() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return None
    except (requests.Timeout, requests.ConnectionError, json.JSONDecodeError, CircuitBreakerError) as e:
        config_logger.error(f"Failed to fetch config from service: {e}")
        return None

class ConfigEncryptor:
    def __init__(self, key: Optional[str]):
        if not key: raise ValueError("CONFIG_ENCRYPTION_KEY is not set for encryptor.")
...
class Config(BaseSettings):
    ...
    """
    model_config = SettingsConfigDict(env_prefix='INTENT_AGENT_', case_sensitive=False, extra='ignore', strict=True)
...
class GlobalConfigManager:
    ...
    def reload_config(cls):
        ...
        with cls._lock:
            ...
            try:
                ...
                log_audit_event('config_reload', {'success': True, 'failure_count': cls._reload_failure_count})
            except Exception as e:
                cls._reload_failure_count += 1
                CONFIG_RELOADS_TOTAL.labels(status="failed").inc()
                if cls._reload_failure_count > 3:
                    config_logger.critical("Config reload has failed 3+ times. Maintaining stale config.")
                else:
                    config_logger.error(f"Config reload failed. Keeping old config. Error: {e}")
                log_audit_event('config_reload', {'success': False, 'failure_count': cls._reload_failure_count})
    ...
    def _start_watcher(cls):
        ...
        observer = Observer()
        observer.schedule(event_handler, path=os.path.dirname(CUSTOM_CONFIG_PATH) or '.', recursive=False)
        observer.start()
