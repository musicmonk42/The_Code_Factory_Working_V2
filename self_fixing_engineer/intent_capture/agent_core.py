...
class StateManagementError(AgentError):
    pass


class InvalidSessionError(AgentError):
    pass
...
class SafetyViolationError(AgentError):
    pass  # UPGRADE: Custom exception for safety guardrail violations
...
class MockLLM:
    ...
    async def ainvoke(self, *args, **kwargs) -> AIMessage:
        return AIMessage(
            content="The AI systems are currently under heavy load. Please try again shortly."
        )
...
class FallbackLLM:
    ...
    async def ainvoke(self, *args, **kwargs) -> AIMessage:
        return AIMessage(
            content="All primary AI models are currently unavailable. While I cannot process your request fully, please check our status page or try again in a few minutes."
        )


# --- Asynchronous LLM Provider Factory ---
class LLMProviderFactory:
    _available_llm_classes = {
        "xai": ChatXAI,
        "openai": ChatOpenAI,
    ...
    async def get_llm(
        ...
        candidates = [primary_provider] + [
            p.lower() for p in retry_providers if p.lower() != primary_provider
        ]
            ...
            with LLM_RESPONSE_LATENCY_SECONDS.labels(llm_provider=prov).time():
                try:
                    ...
                    init_params = (
                        ...
                        if prov == "openai"
                        else {"model": model, "temperature": temperature}
                    )

                    if prov in ["openai", "anthropic", "xai"]:
                        init_params["api_key"] = key
                    elif prov == "google":
                        init_params["google_api_key"] = key
                    ...
                    AGENT_PREDICTION_ERRORS_TOTAL.labels(
                        agent_id="N/A", error_type="llm_initialization"
                    ).inc()
...
class AgentResponse(BaseModel):
    ...
    cited_sources: List[str] = Field(
        description="A list of sources used to formulate the response, if any.",
    ...
    def moderate(self, text: str):
        ...
        if any(word in text.lower() for word in self.banned_words):
            raise SafetyViolationError(
                "Response contains potentially harmful content and has been blocked."
            )
    ...
    async def create(
        ...
        if os.getenv("USE_VECTOR_MEMORY", "false").lower() == "true":
            try:
                ...
                logger.error(
                    f"Failed to initialize Pinecone vector store: {e}. RAG will be disabled."
                )
                kwargs["use_vector_memory"] = False
                AGENT_PREDICTION_ERRORS_TOTAL.labels(
                    agent_id=agent_id, error_type="rag_initialization"
                ).inc()
    ...
    def _setup_runnable(self) -> Runnable:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    "You are an expert AI assistant. If context is provided, use it to inform your answer. Respond with perfect JSON matching the requested schema.",
                ),
    ...
    async def predict(self, user_input: str, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            self.chat_history.extend(
                [
                    HumanMessage(content=anonymized_input),
                    AIMessage(content=final_response),
                ]
            )
            if self.use_vector_memory:
                self.vector_memory.save_context(
                    {"input": anonymized_input}, {"output": final_response}
                )
...
async def get_or_create_agent(session_token: str) -> CollaborativeAgent:
    ...
    if not redis_url:
        raise ConfigurationError("REDIS_URL is not configured.")
    ...
    async with AGENT_CREATION_SEMAPHORE:
        ...
        agent = await CollaborativeAgent.create(
            f"agent_{session_id}", session_id, llm_config, state_backend
        )
...
def validate_environment():
    ...
    required = [
        "JWT_SECRET",
        f"{os.environ.get('LLM_PROVIDER', 'openai').upper()}_API_KEYS",
        "REDIS_URL",
    ]
    if os.environ.get("USE_VECTOR_MEMORY", "false").lower() == "true":
...
async def main():
    ...
    try:
        ...
        print(
            f"Agent created for session: {agent.session_id} (Vector Memory: {agent.use_vector_memory})"
        )
...
class VaultSecretManager:
    ...
    def __init__(self):
        """Initialize Vault client with environment-based configuration."""
        ...
        try:
            ...
            if self._production_mode and not self._fallback_mode:
                raise RuntimeError(
                    "CRITICAL: hvac library required in production mode. "
