...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record):
        log_record = {
            ...
            "username": st.session_state.get("username", "anonymous"),  # P3: Add user/session to logs
        }
...
st.set_page_config(page_title="Intent Capture Agent", layout="wide", initial_sidebar_state="expanded")

# P5: Observability - Prometheus Metrics
PROMETHEUS_AVAILABLE = True
try:
    HTTP_REQUESTS_TOTAL = Counter(
        "streamlit_http_requests_total", "Total HTTP requests to Streamlit app", ["path"]
    )
    APP_ERRORS_TOTAL = Counter(
        "streamlit_app_errors_total", "Total errors in Streamlit app", ["component", "error_type"]
    )
except Exception:
...
if PROMETHEUS_AVAILABLE and os.getenv("PROMETHEUS_PORT"):
    try:
        ...
        logger.info("Prometheus metrics server started.", extra={"port": os.getenv("PROMETHEUS_PORT")})
            ...
            def get_cached_agent(session_id: str):
                ...
                if getattr(app_config, "REDIS_URL", None):
                    try:
                        ...
                        logger.warning("RedisStateBackend not available, falling back to InMemoryStateBackend.")
...
try:
    ...
    for user in auth_config.get("credentials", {}).get("usernames", {}).values():
        if not str(user.get("password", "")).startswith("$2b$"):  # bcrypt prefix
            salt = bcrypt.gensalt()
            user["password"] = bcrypt.hashpw(str(user["password"]).encode("utf-8"), salt).decode("utf-8")
...
def generate_captcha():
    ...
    st.session_state.captcha_text = "".join(random.choices(string.ascii_uppercase + string.digits, k=6))
...
if not st.session_state.get("authenticated", False):
    ...
    if st.session_state.failed_login_attempts.get(username, 0) >= 5:
        if datetime.now() < st.session_state.failed_login_attempts.get(f"{username}_blocked_until", datetime.now()):
    ...
    # CAPTCHA display
    captcha_input = st.text_input(f"{t('captcha_label')}: `{st.session_state.captcha_text}`", key="login_captcha")
    ...
    if st.button(t("login_button")):
        ...
        # CAPTCHA validation
        if datetime.now() > st.session_state.captcha_expiry or captcha_input.upper() != st.session_state.captcha_text:
            logger.warning(
        ...
        if user_info and bcrypt.checkpw(sanitized_password.encode("utf-8"), user_info["password"].encode("utf-8")):
            ...
            logger.warning(f"Failed login attempt for username: {sanitized_username} (credentials incorrect)")
            st.error(t("login_error"))
            st.session_state.failed_login_attempts[sanitized_username] = (
                st.session_state.failed_login_attempts.get(sanitized_username, 0) + 1
            )
            if st.session_state.failed_login_attempts[sanitized_username] >= 5:
                st.session_state.failed_login_attempts[f"{sanitized_username}_blocked_until"] = datetime.now() + timedelta(
                    minutes=10
                )
            generate_captcha()
            st.rerun()
    ...
    if redis_url:
        try:
            ...
            if PROMETHEUS_AVAILABLE:
                APP_ERRORS_TOTAL.labels(component="redis_connection", error_type=type(e).__name__).inc()
            return None
...
with st.sidebar:
    ...
    page = st.radio("Navigate", [t("page_chat"), t("page_dashboard"), t("page_specs"), t("page_collab"), t("page_plugins"), "Health"])
            ...
            if plugin["status"] == "enabled":
                try:
                    ...
                    if os.path.exists(plugin_ui_path):
                        spec = importlib.util.spec_from_file_location(plugin["name"], plugin_ui_path)
                        module = importlib.util.module_from_spec(spec)
                        assert spec.loader is not None
                        spec.loader.exec_module(module)
                        if hasattr(module, "render_sidebar_component"):
                            with st.expander(f"{t('sidebar_plugin_expander_title')}: {plugin['name'].replace('_', ' ').title()}", expanded=True):
                                module.render_sidebar_component(st)
                except Exception as e:
                    logger.error(f"{t('sidebar_plugin_error')} '{plugin['name']}': {e}", exc_info=True)
                    if PROMETHEUS_AVAILABLE:
                        APP_ERRORS_TOTAL.labels(component="plugin_sidebar_ui", error_type=type(e).__name__).inc()
...
def render_chat_page():
        ...
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "assistant" and "trace" in message:
                with st.expander(t("reasoning_expander"), expanded=False):
                    tabs = st.tabs([t("initial_thought_tab"), t("reflection_tab"), t("critique_tab")])
                    with tabs[0]:
    ...
    if prompt := st.chat_input("Describe your project or requirements...", max_chars=MAX_CHAT_INPUT_LENGTH):
        ...
        if not is_safe:
            ...
            logger.warning(f"User '{st.session_state.username}' input failed safety check: {safety_reason}")
            st.stop()
        ...
        with st.spinner(t("agent_thinking")):
            try:
                ...
                st.session_state.messages.append(
                    {"role": "assistant", "content": response_data["response"], "trace": response_data["trace"]}
                )
                logger.info(f"Agent response generated for user '{st.session_state.username}'.")
            except Exception as e:
                logger.error(f"Agent prediction failed for user '{st.session_state.username}': {e}", exc_info=True)
                if PROMETHEUS_AVAILABLE:
                    APP_ERRORS_TOTAL.labels(component="agent_predict", error_type=type(e).__name__).inc()
...
def render_specs_page():
    ...
    supported_spec_formats = getattr(
        config, "SUPPORTED_SPEC_FORMATS", ["markdown", "gherkin", "json", "yaml", "user_story"]
    )
    ...
    if st.button(t("generate_spec_button"), use_container_width=True):
        logger.info(f"User '{st.session_state.username}' attempting to generate spec in {spec_format} format.")
        with st.spinner("Generating specification..."):
            try:
                ...
                if spec_data:
                    ...
                    logger.info(f"Spec generated successfully for user '{st.session_state.username}'.")
                else:
                    st.error(t("spec_generated_fail"))
                    logger.error(f"Failed to generate spec for user '{st.session_state.username}'.")
                    if PROMETHEUS_AVAILABLE:
                        APP_ERRORS_TOTAL.labels(component="generate_spec", error_type="GenerationFailed").inc()
            except Exception as e:
                logger.error(f"Error generating spec for user '{st.session_state.username}': {e}", exc_info=True)
                if PROMETHEUS_AVAILABLE:
                    APP_ERRORS_TOTAL.labels(component="generate_spec", error_type=type(e).__name__).inc()
...
def render_collab_page():
    ...
    if not redis_client:
        ...
        if PROMETHEUS_AVAILABLE:
            APP_ERRORS_TOTAL.labels(component="collab_redis", error_type="RedisConnectionError").inc()
        st.stop()
    ...
    try:
        ...
        for msg_json in message_history:
            ...
            with st.chat_message(msg.get("role", "user"), avatar="🧑‍💻" if msg.get("role") == "user" else "🤖"):
...
def render_plugins_page():
    ...
    enabled_plugins = [p for p in PluginManager.get_plugin_diagnostics() if p["status"] == "enabled"]
    if not enabled_plugins:
        ...
        for plugin in enabled_plugins:
            try:
                ...
                logger.error(f"{t('error_rendering_plugin')} '{plugin['name']}': {e}", exc_info=True)
                if PROMETHEUS_AVAILABLE:
                    APP_ERRORS_TOTAL.labels(component="plugin_main_ui", error_type=type(e).__name__).inc()
...
def render_health_page():
    ...
    st.markdown("This page provides a simple health check for the application and its dependencies.")
