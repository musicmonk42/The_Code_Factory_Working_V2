...
try:
    ...
    SPEC_GEN_LATENCY_SECONDS = Histogram("spec_gen_latency_seconds", "Spec generation latency in seconds", ["format"])
    # Metrics for validation
    SPEC_VALIDATION_TOTAL = Counter("spec_validation_total", "Total spec validations", ["format", "is_valid"])
...
try:
    ...
    logger.warning("NLTK data not found. Attempting to download... This might fail in production containers if not pre-downloaded.")
    try:
        nltk.download('punkt')
        nltk.download('stopwords')
        logger.info("NLTK data downloaded successfully.")
    except Exception as e:
        logger.error(f"Failed to download NLTK data: {e}. Please ensure 'punkt' and 'stopwords' are available.", exc_info=True)
...
def _load_locales():
    ...
    if os.path.exists(LOCALES_FILE):
        try:
            ...
            logger.error(f"Failed to load locales from {LOCALES_FILE}: {e}. Using default English prompts.", exc_info=True)
    else:
...
def get_localized_prompt(key: str, lang: str = "en") -> str:
    ...
    return _LOCALES.get(lang.lower(), {}).get(key, _LOCALES.get("en", {}).get(key, f"Prompt not found for {key} in {lang}."))
...
def load_ambiguous_words(lang: str) -> List[str]:
    ...
    ambiguous = []
    try:
        @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, max=10), retry=retry_if_exception_type(requests.exceptions.RequestException), before_sleep=before_sleep_log(logger, logging.WARNING))
        def _fetch_words():
...
def register_spec_handler(format_name: str, validator: Callable[[str], Tuple[bool, str]], generator: Callable[..., Optional[str]]):
    """
...
def validate_spec(spec: str, format: str, version: str = "1.0", schema: Optional[dict]=None) -> Tuple[bool, str]:
    """
    ...
    with get_tracing_context("validate_spec") as span:
        ...
        # YAML: parse + (optional) schema (reuse JSON schema)
            ...
            try:
                ...
                is_valid, validation_message = False, f"YAML (JSON Schema) violation: {ve}"
        # Gherkin: regex + advanced lint rules
        elif format_lower == "gherkin":
            if not re.search(r"^\s*Feature:", spec, re.MULTILINE):
                is_valid, validation_message = False, "Gherkin: Missing 'Feature' keyword."
            elif not re.search(r"^\s*Scenario:", spec, re.MULTILINE):
                is_valid, validation_message = False, "Gherkin: Missing 'Scenario' keyword."
            else:
                scenarios = re.split(r"^\s*Scenario:", spec, flags=re.MULTILINE)[1:]
                for idx, sc in enumerate(scenarios, 1):
                    if not any(re.search(rf"^\s*{step} ", sc, re.MULTILINE) for step in ["Given", "When", "Then"]):
                        is_valid, validation_message = False, f"Gherkin: Scenario {idx} missing Given/When/Then."
                        break
        ...
        # User story strict check
            ...
            if re.search(r"As a .* I want .* so that .*", spec, re.IGNORECASE | re.DOTALL):
                ...
                is_valid, validation_message = False, "User story must contain 'As a..., I want..., so that...'."
        else:
            logger.warning(f"No validator found for format '{format}'. Assuming valid if not empty.")
            is_valid, validation_message = bool(spec.strip()), "No specific validator for this format."
        
        if OPENTELEMETRY_AVAILABLE:
            span.set_attribute("validation.is_valid", is_valid)
            span.set_attribute("validation.message", validation_message)
            span.set_status(trace.Status(trace.StatusCode.OK if is_valid else trace.StatusCode.ERROR, description=validation_message))
...
def migrate_spec(spec: str, format: str, from_version: str, to_version: str) -> Tuple[Optional[str], str]:
    """
    ...
    """
    logger.warning(f"Migration from v{from_version} to v{to_version} is not implemented for format '{format}'.")
...
def detect_ambiguity(text: str, language: str = 'english') -> List[Dict[str, Any]]:
    ...
    with get_tracing_context("detect_ambiguity") as span:
        ...
        for i, sentence in enumerate(sentences):
            found_words = {word for word in nltk.word_tokenize(sentence.lower()) if word in ambiguous_words}
            if found_words:
                ambiguities.append({
...
async def auto_fix_spec(spec: str, llm: BaseChatModel, format: str, issues: List[Dict[str, Any]], language: str = "en") -> Tuple[Optional[str], str]:
    """
    ...
    try:
        with get_tracing_context("auto_fix_spec") as span:
            ...
            try:
                ...
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"Auto-fix generation failed: {e}"))
                if PROMETHEUS_AVAILABLE and SPEC_AUTO_FIX_TOTAL: 
...
class TraceableArtifact:
    ...
    """
    def __init__(self, content: str, artifact_type: str, source_spec_id: str, generation_prompt: str):
        ...
        self.history = [{"timestamp": self.timestamp, "action": "created", "notes": "Initial generation."}]
        self.persist_metadata()

    def update(self, new_content: str, notes: str):
        self.content = new_content
        self.history.append({"timestamp": datetime.utcnow().isoformat(), "action": "updated", "notes": notes})
        self.persist_metadata()
    ...
    def persist_metadata(self):
        ...
        try:
            ...
            @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, max=10), retry=retry_if_exception_type(requests.exceptions.RequestException), before_sleep=before_sleep_log(logger, logging.WARNING))
            def _send_payload():
                requests.post(provenance_url, json=payload, timeout=5) # 5s timeout for external service
            _send_payload()
...
async def _generate_downstream_artifact(spec_id: str, spec_content: str, llm: BaseChatModel, prompt_template_key: str, artifact_type: str, language: str = "en", **kwargs) -> TraceableArtifact:
...
async def generate_code_stub(spec_id: str, spec_content: str, llm: BaseChatModel, language: str = "python") -> TraceableArtifact:
    # P9: Accessibility - Ensure generated code stubs have comments for clarity.
    # The prompt itself ensures this.
    return await _generate_downstream_artifact(spec_id, spec_content, llm, "generate_code_stub_prompt", "code", language=language, language_name=language)

async def generate_test_stub(spec_id: str, spec_content: str, llm: BaseChatModel, framework: str = "pytest", language: str = "en") -> TraceableArtifact:
    # P9: Accessibility - Ensure generated test stubs have clear structure and placeholder comments.
    # The prompt itself ensures this.
    return await _generate_downstream_artifact(spec_id, spec_content, llm, "generate_test_stub_prompt", "test", language=language, framework=framework)

async def generate_security_review(spec_id: str, spec_content: str, llm: BaseChatModel, language: str = "en") -> TraceableArtifact:
    return await _generate_downstream_artifact(spec_id, spec_content, llm, "generate_security_review_prompt", "security_review", language=language)

# --- Core Spec Generation, Gap Analysis, and Refinement ---

async def generate_spec_from_memory(memory: Any, llm: BaseChatModel, format: str = "gherkin", persona: str = "", language: str = "en", version: str = "1.0", schema: Optional[dict]=None, **kwargs) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        with get_tracing_context("generate_spec_from_memory") as span:
            ...
            if not transcript:
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description="Empty transcript"))
            ...
            if format in SPEC_HANDLERS:
                spec_content = await SPEC_HANDLERS[format]["generator"](transcript, llm, persona, language, **kwargs)
            else:
                prompt_text = get_localized_prompt(f"generate_spec_prompt_{format.lower()}", language)
                if prompt_text.startswith("Prompt not found"):
                    logger.error(f"No localized prompt found for spec format: {format} in language: {language}")
                    if OPENTELEMETRY_AVAILABLE:
                        span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"No prompt for format {format}"))
                    if PROMETHEUS_AVAILABLE and SPEC_GEN_TOTAL: 
                        SPEC_GEN_TOTAL.labels(format=format, status="no_prompt").inc()
                    return None

                spec_prompt = PromptTemplate.from_template(prompt_text)
                response = await (spec_prompt | llm).ainvoke({"transcript": transcript, "persona": persona, "language": language})
                spec_content = response.content

            if not spec_content:
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description="LLM returned empty content"))
                if PROMETHEUS_AVAILABLE and SPEC_GEN_TOTAL: 
            ...
            if not is_valid or ambiguities:
                logger.warning(f"Initial spec has issues. Validation: {validation_msg}. Ambiguities: {len(ambiguities)}. Attempting auto-fix.")
                if OPENTELEMETRY_AVAILABLE:
                    span.add_event("initial_spec_issues", {"validation_msg": validation_msg, "ambiguities_count": len(ambiguities)})
                fixed_spec, fix_notes = await auto_fix_spec(spec_content, llm, format, ambiguities, language=language)
                if fixed_spec:
                    ...
                    print("\033[33mWarning: Auto-fix failed. The generated spec may have issues.\033[0m")
                    if OPENTELEMETRY_AVAILABLE:
                        span.set_attribute("auto_fix.applied", False)
                        span.set_status(trace.Status(trace.StatusCode.ERROR, description="Auto-fix failed, spec may have issues"))
                    if PROMETHEUS_AVAILABLE and SPEC_GEN_TOTAL: 
...
async def generate_gaps(spec_content: str, transcript: str, llm: BaseChatModel, domain: Optional[str] = None, project: Optional[str] = None, language: str = "en") -> Optional[str]:
    """
    ...
    with get_tracing_context("generate_gaps") as span:
        ...
        if not checklist:
            ...
            return None
        checklist_str = "\n".join([f"- {item['name']} (Weight: {item['weight']}): {item['description']}" for item in checklist])
        ...
        try:
            ...
            if OPENTELEMETRY_AVAILABLE:
                span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"Gaps analysis failed: {e}"))
            return None

async def refine_spec(last_spec: str, instruction: str, llm: BaseChatModel, language: str = "en") -> Optional[str]:
    """
    ...
    with get_tracing_context("refine_spec") as span:
        ...
        try:
            ...
            if OPENTELEMETRY_AVAILABLE:
                span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"Refine spec failed: {e}"))
            return None
...
async def review_spec(spec_content: str, llm: BaseChatModel, language: str = "en") -> Optional[str]:
    ...
    with get_tracing_context("review_spec") as span:
        ...
        try:
            ...
            if OPENTELEMETRY_AVAILABLE:
                span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"Review spec failed: {e}"))
            return None
...
def diff_specs(spec1: str, spec2: str) -> str:
    ...
    with get_tracing_context("diff_specs") as span:
        if not isinstance(spec1, str) or not isinstance(spec2, str):
            if OPENTELEMETRY_AVAILABLE:
                span.set_status(trace.Status(trace.StatusCode.ERROR, description="Inputs must be strings"))
