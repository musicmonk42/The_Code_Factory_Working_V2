...
try:
    ...
    REQ_SUGGESTIONS_TOTAL = Counter("req_suggestions_total", "Total requirement suggestions", ["domain", "status"])
    REQ_SUGGESTIONS_LATENCY_SECONDS = Histogram("req_suggestions_latency_seconds", "Requirement suggestion latency in seconds", ["domain"])
    # Metrics for DB operations
    DB_OPS_TOTAL = Counter("req_db_ops_total", "Total DB operations", ["operation", "status"])
    DB_OPS_LATENCY_SECONDS = Histogram("req_db_ops_latency_seconds", "DB operation latency in seconds", ["operation"])
    # Metrics for ML model loading
    ML_MODEL_LOAD_LATENCY_SECONDS = Histogram("req_ml_model_load_latency_seconds", "ML model load latency in seconds")
except ImportError:
...
try:
    ...
    trace_provider.add_span_processor(BatchSpanProcessor(ConsoleSpanExporter())) # Use ConsoleSpanExporter for local debugging
...
try:
    ...
    asyncpg = None
    DB_AVAILABLE = False
    logging.warning("asyncpg not found. PostgreSQL integration will be skipped. Install with 'pip install asyncpg==0.30.0'.")
...
try:
    ...
    logging.warning("redis.asyncio not found. Redis features will be skipped. Install with 'pip install redis[async]==6.4.0'.")
...
try:
    ...
    util = None
    ML_ENABLED = False
    logging.warning("sentence-transformers not found. ML-driven suggestions will be disabled. Install with 'pip install sentence-transformers==5.1.0 torch==2.8.0'.")
...
try:
    ...
    logging.warning("pandas not found. Advanced coverage reports will be limited. Install with 'pip install pandas==2.3.1'.")
...
REQUIREMENTS_CHECKLIST: List[Dict[str, Any]] = [
    {"id": "REQ001", "name": "user roles", "weight": 3, "description": "Define different user types and their permissions."},
    {"id": "REQ002", "name": "error states", "weight": 2, "description": "Handle and document possible error conditions and messages."},
    # ... and so on
]
DOMAIN_SPECIFIC: Dict[str, List[Dict[str, Any]]] = {
    "fintech": [
        {"id": "FIN001", "name": "AML/KYC compliance", "weight": 3, "description": "Anti-Money Laundering and Know Your Customer regulations."},
        # ...
    ],
    "ui": [
        {"id": "UI001", "name": "accessibility standards", "weight": 3, "description": "Compliance with WCAG, ARIA for screen readers and assistive technologies."},
        # ...
    ],
    # ...
}
...
async def _load_json_file(file_path: str) -> Dict:
    ...
    if not aiofiles:
        logger.error("aiofiles is not installed. File operations will fail. Run 'pip install aiofiles'.")
        return {}
...
class RequirementsManager:
            ...
            async with self._model_lock: # Acquire async lock
                if self._embedding_model is None:
                    ...
                    with get_tracing_context("load_embedding_model") as span:
                        logger.info("Loading SentenceTransformer model for requirements: 'all-MiniLM-L6-v2'")
                        # P3: Run blocking model load in a thread pool
                        self._embedding_model = await asyncio.to_thread(SentenceTransformer, 'all-MiniLM-L6-v2')
    ...
    async def get_db_conn_pool(self) -> asyncpg.Pool: # P3: Make async and return a pool
        ...
        if self._db_pool is None:
            ...
            if not all([db_name, db_user, db_pass, db_host]):
                logger.critical("CRITICAL: Missing database connection environment variables. Exiting.")
                sys.exit(1)
            if db_host == "localhost" and os.environ.get("PROD_MODE", "false").lower() == "true":
                logger.critical("CRITICAL: Localhost DB connection not allowed in production. Exiting.")
                sys.exit(1)
    ...
    @retry(
        ...
        retry=retry_if_exception_type((asyncpg.exceptions.PostgresError, RateLimitError)) if DB_AVAILABLE else retry_if_exception_type(RateLimitError), # Retry on specific DB errors
        ...
        with get_tracing_context("db_get_custom_checklists") as span:
            ...
            try:
                ...
                for proj, domain, checklist in rows:
                    result.setdefault(proj, {})[domain] = json.loads(checklist) if isinstance(checklist, str) else checklist
                ...
                if PROMETHEUS_AVAILABLE and DB_OPS_TOTAL:
                    ...
                    DB_OPS_LATENCY_SECONDS.labels(operation="get_checklists").observe(time.perf_counter() - start_time)
                return result
    ...
    @retry(
        stop=stop_after_attempt(3), # P6: Retries for DB operations
        wait=wait_exponential(multiplier=1, min=1, max=10),
        retry=retry_if_exception_type((asyncpg.exceptions.PostgresError, RateLimitError)) if DB_AVAILABLE else retry_if_exception_type(RateLimitError),
        before_sleep=before_sleep_log(logger, logging.WARNING)
    )
                    ...
                    async with conn.transaction(): # P2: Atomic transaction
                        for project, domains in customs.items():
                            for domain, checklist in domains.items():
                                ...
                                """, project, domain, json.dumps(checklist))
                logger.info("Custom checklists saved to DB.")
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.OK))
                if PROMETHEUS_AVAILABLE and DB_OPS_TOTAL:
                    DB_OPS_TOTAL.labels(operation="save_checklists", status="success").inc()
                    DB_OPS_LATENCY_SECONDS.labels(operation="save_checklists").observe(time.perf_counter() - start_time)
            except asyncpg.exceptions.PostgresError as e:
                if 'rate limit' in str(e).lower():
                    logger.warning(f"Database rate limit detected: {e}. Retrying via tenacity.")
                    raise RateLimitError("DB rate limit exceeded") from e
    ...
    async def get_global_custom_checklists(self) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    ...
    async def get_checklist(self, domain: Optional[str] = None, project: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def add_item(self, domain: str, item_name: str, weight: int = 1, description: str = "", project: Optional[str] = None) -> str:
        ...
        new_item = {"id": f"CUST-{uuid.uuid4()}", "name": sanitized_item_name, "weight": weight, "description": sanitized_description}
        ...
        logger.info(f"AUDIT: User '{user}' added item '{sanitized_item_name}' to {domain} for project '{project_key}'.")

        return f"Added '{sanitized_item_name}' to {domain} checklist for project '{project_key}'."

    async def update_item_status(self, item_id: str, status: str, project: Optional[str] = None, domain: Optional[str] = None) -> bool:
        ...
        if domain and domain in project_customs:
            for item in project_customs[domain]:
                if item.get("id") == item_id:
                    ...
                    await self.set_global_custom_checklists(customs)
                    logger.info(f"AUDIT: User '{user}' updated status of item '{item_id}' to '{status}' in domain '{domain}' for project '{project_key}'.")
                    return True
        ...
        for checklist in [REQUIREMENTS_CHECKLIST] + list(DOMAIN_SPECIFIC.values()):
            for item in checklist:
                if item.get("id") == item_id:
                    item["status"] = status
                    item["updated_at"] = datetime.datetime.utcnow().isoformat()
                    logger.info(f"Updated status of global/domain item '{item_id}' to '{status}' in memory.")
        ...
        logger.warning(f"Item '{item_id}' not found in any checklist for project '{project_key}' and domain '{domain}'.")
        return False
    ...
    async def _generate_novel_requirements(self, context: str, llm: BaseLanguageModel) -> List[Dict[str, Any]]:
        ...
        with get_tracing_context("generate_novel_requirements") as span:
            ...
            try:
                ...
                response = await asyncio.wait_for(llm.ainvoke(prompt), timeout=LLM_GEN_TIMEOUT_SECONDS)
                    ...
                    for s in suggestions:
                        if isinstance(s, dict) and "name" in s and "description" in s:
                            sanitized_suggestions.append({
                                "name": sanitize_text(s["name"], allow_punct=False, max_length=256),
                                "description": sanitize_text(s["description"], allow_punct=True, max_length=2048)
                            })
                    logger.info(f"Generated {len(sanitized_suggestions)} novel requirements from LLM.")
                    if OPENTELEMETRY_AVAILABLE:
                ...
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"LLM parse failed: {e}"))
                raise e # Re-raise for tenacity
            except Exception as e:
                logger.error(f"An unexpected error occurred during LLM generation: {e}", exc_info=True)
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"LLM generation failed: {e}"))
                return [] # Don't retry on unknown errors
            finally:
                if PROMETHEUS_AVAILABLE and REQ_SUGGESTIONS_TOTAL:
                    status = "success" if (OPENTELEMETRY_AVAILABLE and hasattr(span, 'status') and span.status.status_code == trace.StatusCode.OK) else "failed"
                    REQ_SUGGESTIONS_TOTAL.labels(domain="novel", status=status).inc()
                    REQ_SUGGESTIONS_LATENCY_SECONDS.labels(domain="novel").observe(time.perf_counter() - start_time)

    async def _suggest_via_embeddings(self, domain: str, transcript_snippet: str, existing_checklist: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Suggests requirements via cosine similarity of embeddings."""
        try:
            model = await self.get_embedding_model()
            transcript_embedding = await asyncio.to_thread(model.encode, transcript_snippet, convert_to_tensor=True)
            existing_names = {item["name"].lower() for item in existing_checklist}
            candidates = [req for req in REQUIREMENTS_CHECKLIST + DOMAIN_SPECIFIC.get(domain, []) if req["name"].lower() not in existing_names]
            ...
            if candidates:
                ...
                candidate_embeddings = await asyncio.to_thread(model.encode, candidate_descs, convert_to_tensor=True)
                scores = util.pytorch_cos_sim(transcript_embedding, candidate_embeddings)[0]
                return [candidates[i] for i, score in enumerate(scores) if score.item() > COSINE_SIM_THRESHOLD]
        except Exception as e:
            logger.error(f"Error during similarity-based suggestion: {e}", exc_info=True)
        return []

    async def suggest_requirements(self, domain: str, transcript_snippet: str, existing_checklist: List[Dict[str, Any]], llm: Optional[BaseLanguageModel] = None) -> List[Dict[str, Any]]:
        ...
        with get_tracing_context("suggest_requirements") as span:
            ...
            suggestions = []
            if ML_ENABLED:
                embedding_suggestions = await self._suggest_via_embeddings(domain, transcript_snippet, existing_checklist)
            ...
            if llm:
                try:
                    novel_suggestions = await self._generate_novel_requirements(transcript_snippet, llm)
            ...
            if PROMETHEUS_AVAILABLE and REQ_SUGGESTIONS_TOTAL:
                ...
                REQ_SUGGESTIONS_LATENCY_SECONDS.labels(domain=domain).observe(time.perf_counter() - start_time)
    ...
    async def propose_checklist_updates(self, transcript: str, existing_checklist: List[Dict[str, Any]], llm: BaseLanguageModel) -> List[Dict[str, Any]]:
        ...
        with get_tracing_context("propose_checklist_updates") as span:
            ...
            try:
                    ...
                    for p in proposals:
                        if isinstance(p, dict) and "name" in p and "description" in p:
                            sanitized_proposals.append({
                                "name": sanitize_text(p["name"], allow_punct=False, max_length=256),
                                "description": sanitize_text(p["description"], allow_punct=True, max_length=2048)
                            })
                    logger.info(f"Proposed {len(sanitized_proposals)} new checklist items for self-updating.")
                    if OPENTELEMETRY_AVAILABLE:
                ...
                if OPENTELEMETRY_AVAILABLE:
                    span.set_status(trace.Status(trace.StatusCode.ERROR, description=f"LLM proposal failed: {e}"))
                raise e # Re-raise for tenacity
            finally:
                if PROMETHEUS_AVAILABLE and REQ_SUGGESTIONS_TOTAL:
                    status = "success" if (OPENTELEMETRY_AVAILABLE and hasattr(span, 'status') and span.status.status_code == trace.StatusCode.OK) else "failed"
                    REQ_SUGGESTIONS_TOTAL.labels(domain="checklist_update", status=status).inc()
                    REQ_SUGGESTIONS_LATENCY_SECONDS.labels(domain="checklist_update").observe(time.perf_counter() - start_time)

    async def log_coverage_snapshot(self, project: str, domain: str, coverage_percent: float, covered_items: int, total_items: int):
        ...
        if REDIS_AVAILABLE:
            try:
                r = await redis.Redis.from_url(os.environ.get("REDIS_URL", "redis://localhost:6379/0"))
                key = f"coverage:{project}"
                await r.rpush(key, json.dumps(snapshot))
                logger.info(f"Coverage snapshot logged to Redis for project {project}, domain {domain}.")
                return
            except Exception as e:
        ...
        logger.info(f"Coverage snapshot logged to local file for project {project}, domain {domain}.")
    ...
    async def generate_coverage_report(self, project: str) -> str:
        ...
        for domain in latest_by_domain:
            ...
            if len(domain_history) > 1:
                ...
                trend = "📈 Increased" if end_cov > start_cov else "📉 Decreased" if end_cov < start_cov else " stagnant"
    ...
    async def compute_coverage(self, gaps_table_markdown: str, llm: Optional[BaseLanguageModel]=None) -> Dict[str, float]:
        ...
        if llm:
            ...
            try:
                # P6: Add timeout to LLM call
                resp = await asyncio.wait_for(llm.ainvoke(prompt), timeout=LLM_PARSE_TIMEOUT_SECONDS)
        ...
        for item in requirements:
            ...
            try:
                if 'PLUG-' in item_id and not validate_uuid(item_id.split('-')[-1]):
                    logger.warning(f"Invalid UUID in provided item_id '{item_id}'. Generating new one.")
            ...
            item_description = sanitize_text(item.get("description", ""), allow_punct=True, max_length=2048)
            item_weight = item.get("weight", 1)

            if not item_name:
                logger.warning(f"Skipping plugin requirement with empty name in domain '{sanitized_domain_name}'.")
                continue

            sanitized_item = {"id": item_id, "name": item_name, "weight": item_weight, "description": item_description}

            is_duplicate = any(
                existing.get("id") == sanitized_item["id"] or existing["name"].lower() == sanitized_item["name"].lower()
            ...
            if not is_duplicate:
                ...
                logger.info(f"Plugin registered requirement '{sanitized_item['name']}' for domain '{sanitized_domain_name}'.")
...
async def add_item(domain: str, item_name: str, weight: int = 1, description: str = "", project: Optional[str] = None):
    return await manager.add_item(domain, item_name, weight, description, project)

async def update_item_status(item_id: str, status: str, project: Optional[str] = None, domain: Optional[str] = None):
...
async def suggest_requirements(domain: str, transcript_snippet: str, existing_checklist: List[Dict[str, Any]], llm: Optional[BaseLanguageModel] = None):
    return await manager.suggest_requirements(domain, transcript_snippet, existing_checklist, llm)

async def propose_checklist_updates(transcript: str, existing_checklist: List[Dict[str, Any]], llm: BaseLanguageModel):
    return await manager.propose_checklist_updates(transcript, existing_checklist, llm)

async def log_coverage_snapshot(project: str, domain: str, coverage_percent: float, covered_items: int, total_items: int):
    return await manager.log_coverage_snapshot(project, domain, coverage_percent, covered_items, total_items)
