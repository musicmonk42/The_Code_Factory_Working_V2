...
try:
    ...
    from feast.errors import (FeastObjectNotFoundException, FeastProviderError,
                              FeastResourceError)
    # Production-ready sources
    from feast.infra.offline_stores.bigquery_source import BigQuerySource
    FEAST_AVAILABLE = True
except ImportError:
    logging.getLogger(__name__).critical("Feast library not found. FeatureStoreClient cannot operate in real mode.")
    ...
    class ValueType:
        ...
        DOUBLE = "DOUBLE"
        DATETIME = "DATETIME"
    class FeastField: pass
    class DataSource: pass
    class FileSource: pass
    class BigQuerySource: pass
    class FeastObjectNotFoundException(Exception): pass
    class FeastProviderError(Exception): pass
    class FeastResourceError(Exception): pass
...
try:
    ...
    GX_AVAILABLE = False
    logging.getLogger(__name__).warning("great-expectations not found. Feature validation unavailable.")
...
try:
    ...
    logging.getLogger(__name__).warning("postgres_client.py not found. Redaction storage unavailable.")
...
try:
    ...
    logging.getLogger(__name__).warning("audit_ledger_client.py not found. Audit logging unavailable.")
    class AuditLedgerClient:
        async def __aenter__(self): return self
        async def __aexit__(self, exc_type, exc_val, exc_tb): pass
        async def log_event(self, event_type, details, operator):
            logger.info(f"Stubbed AuditLedgerClient.log_event called for event_type: {event_type}, details: {details}, operator: {operator}")
...
logger.setLevel(os.getenv("LOG_LEVEL", "INFO").upper()) # Allow log level to be configured via env var
...
class FeatureEntityModel(BaseModel):
    ...
    value_type: str = PydanticField(..., description="Value type (e.g., INT64, STRING, DOUBLE, DATETIME).")
    description: Optional[str] = PydanticField(None, max_length=500, description="Entity description.")
    
    @field_validator('value_type')
    @classmethod
    def validate_value_type(cls, v):
        valid_types = set(getattr(ValueType, '__members__', {}).keys()) or {
            'INT64','STRING','DOUBLE','DATETIME'
        }
        if v not in valid_types:
            raise ValueError(f"Invalid value_type: {v}. Must be one of {sorted(list(valid_types))}.")
        return v
...
class FeatureViewModel(BaseModel):
    ...
    name: str = PydanticField(..., min_length=1, max_length=100, description="Unique feature view name.")
    ...
    schema: List[Dict[str, str]] = PydanticField(..., min_items=1, description="Schema fields with name and dtype.")
...
class FeatureSourceModel(BaseModel):
    ...
    config: Dict[str, Any] = PydanticField(..., description="Source configuration (e.g., table, path).")
    timestamp_field: Optional[str] = PydanticField(None, description="Timestamp field for time-based joins.")
...
class FeatureStoreClient:
    ...
    def __init__(self, repo_path: Optional[str] = None):
        ...
        if os.getenv("ENV", "dev") != "dev" and not os.getenv("USE_SECRETS_MANAGER", "false").lower() == "true":
            raise ValueError("Secrets Manager required in production for GCP_CREDENTIALS and REDIS_URL.")
            
        self._fs: Optional[FeatureStore] = None
        self._is_connected = False
        self.semaphore = asyncio.Semaphore(5)  # Limit concurrent operations
        self.metric_labels = {"env": os.getenv("ENV", "dev"), "cluster": os.getenv("CLUSTER", "default")}
    ...
    def _get_credentials(self, key: str) -> str:
        ...
        if os.getenv("USE_SECRETS_MANAGER", "false").lower() == "true" and os.getenv("ENV", "dev") != "dev" and BOTO3_AVAILABLE:
            try:
        ...
        if env_value and os.getenv("ENV", "dev") != "dev":
            logger.warning(f"Using env var for {key} in production; prefer Secrets Manager for security.")
        return env_value or ""
    ...
    async def connect(self) -> None:
        ...
        with tracer.start_as_current_span("feast_connect") as span:
            ...
            try:
                ...
                FS_CALLS_TOTAL.labels(operation="connect", status="success", **self.metric_labels).inc()
                span.set_status(Status(StatusCode.OK))
                logger.info(f"Connected to Feast Feature Store at {self.repo_path}")
            except Exception as e:
                FS_CALLS_TOTAL.labels(operation="connect", status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="connect", error_type=type(e).__name__, **self.metric_labels).inc()
                ...
                FS_CALL_LATENCY_SECONDS.labels(operation="connect", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def health_check(self) -> bool:
        ...
        with tracer.start_as_current_span("feast_health_check") as span:
            ...
            try:
                ...
                FS_CALLS_TOTAL.labels(operation="health_check", status="success", **self.metric_labels).inc()
                return True
            except Exception as e:
                FS_CALLS_ERRORS.labels(operation="health_check", error_type=type(e).__name__, **self.metric_labels).inc()
                return False
    ...
    async def log_operation(self, operation: str, details: Dict[str, Any]) -> str:
        ...
        if not AUDIT_LEDGER_AVAILABLE:
            logger.warning(f"Audit logging skipped for operation '{operation}': audit_ledger_client.py not available.")
            return "not_available"
        
        with tracer.start_as_current_span("feast_log_operation") as span:
            span.set_attribute("feast.operation", operation)
            FS_AUDIT_LOGS_TOTAL.labels(operation=operation, status="attempt", **self.metric_labels).inc()
            try:
                ...
                FS_AUDIT_LOGS_TOTAL.labels(operation=operation, status="success", **self.metric_labels).inc()
                span.set_status(Status(StatusCode.OK))
                logger.info(f"Logged operation '{operation}' with details: {details}")
                return tx_hash
            except Exception as e:
                FS_AUDIT_LOGS_TOTAL.labels(operation=operation, status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="log_operation", error_type=type(e).__name__, **self.metric_labels).inc()
                span.record_exception(e)
    ...
    @retry(
        ...
        retry=retry_if_exception_type((FeastProviderError, FeastResourceError, FeastObjectNotFoundException)),
        reraise=True
    )
    async def apply_feature_definitions(self, definitions: List[Union[Entity, FeatureView]]) -> None:
        """
        ...
        with tracer.start_as_current_span("feast_apply_definitions") as span:
            ...
            start_time = time.monotonic()
            FS_CALLS_TOTAL.labels(operation="apply_definitions", status="attempt", **self.metric_labels).inc()
            try:
                for defn in definitions:
                    if isinstance(defn, Entity):
                        FeatureEntityModel(name=defn.name, value_type=defn.value_type.name, description=defn.description)
                        ...
                        FeatureSourceModel(type=defn.source.__class__.__name__, config=defn.source.__dict__, timestamp_field=defn.source.timestamp_field)
                loop = asyncio.get_running_loop()
                await loop.run_in_executor(None, self._fs.apply, definitions)
                
                await self.log_operation("apply_definitions", {"num_definitions": len(definitions), "definitions": [d.name for d in definitions]})
                
                FS_CALLS_TOTAL.labels(operation="apply_definitions", status="success", **self.metric_labels).inc()
                span.set_status(Status(StatusCode.OK))
                logger.info(f"Applied {len(definitions)} Feast definitions.")
            except Exception as e:
                FS_CALLS_TOTAL.labels(operation="apply_definitions", status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="apply_definitions", error_type=type(e).__name__, **self.metric_labels).inc()
                span.record_exception(e)
                ...
                FS_CALL_LATENCY_SECONDS.labels(operation="apply_definitions", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def wait_for_ingestion(self, feature_view_name: str, timeout: int = 30) -> None:
        ...
        with tracer.start_as_current_span("feast_wait_for_ingestion") as span:
            ...
            FS_CALLS_TOTAL.labels(operation="wait_for_ingestion", status="attempt", **self.metric_labels).inc()
            try:
                while time.monotonic() - start_time < timeout:
                    features = await self.get_online_features(
                        [f"{feature_view_name}:daily_login_count"],
                        [{"user_id": 101}]
                    )
                    if features and features[0].get('daily_login_count') is not None:
                        FS_CALLS_TOTAL.labels(operation="wait_for_ingestion", status="success", **self.metric_labels).inc()
                        span.set_status(Status(StatusCode.OK))
                        return
                    await asyncio.sleep(1)
                raise TimeoutError(f"Ingestion for {feature_view_name} not completed within {timeout}s")
            except Exception as e:
                FS_CALLS_TOTAL.labels(operation="wait_for_ingestion", status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="wait_for_ingestion", error_type=type(e).__name__, **self.metric_labels).inc()
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to wait for ingestion: {e}"))
                logger.error(f"Failed to wait for ingestion of '{feature_view_name}': {e}", exc_info=True)
                raise
            finally:
                FS_CALL_LATENCY_SECONDS.labels(operation="wait_for_ingestion", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def ingest_features(self, feature_view_name: str, data_df: Any) -> None:
        ...
        if os.getenv("ENV", "dev") != "dev" and os.getenv("USE_SECRETS_MANAGER", "false").lower() != "true":
        ...
        for col in sensitive_cols:
            if col in data_df.columns:
                data_df[col + '_hash'] = data_df[col].apply(lambda x: hashlib.sha256(str(x).encode()).hexdigest())
        ...
        async with self.semaphore:
            with tracer.start_as_current_span("feast_ingest_features") as span:
                ...
                start_time = time.monotonic()
                FS_CALLS_TOTAL.labels(operation="ingest_features", status="attempt", **self.metric_labels).inc()
                try:
                    ...
                    if RAY_AVAILABLE and os.getenv("FEAST_PROVIDER", "local") != "local":
                        ...
                        batches = [data_df[i:i + batch_size] for i in range(0, len(data_df), batch_size)]
                        futures = [ingest_batch.remote(self.repo_path, feature_view_name, batch) for batch in batches]
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                            batches = [data_df[i:i + batch_size] for i in range(0, len(data_df), batch_size)]
                            futures = [executor.submit(self._fs.ingest, feature_view, batch) for batch in batches]
                            concurrent.futures.wait(futures)
                    
                    await self.wait_for_ingestion(feature_view_name)
                    FS_FEATURE_FRESHNESS_SECONDS.labels(feature_view=feature_view_name, **self.metric_labels).set(0)

                    await self.log_operation("ingest_features", {"feature_view": feature_view_name, "rows": len(data_df)})

                    FS_CALLS_TOTAL.labels(operation="ingest_features", status="success", **self.metric_labels).inc()
                    span.set_status(Status(StatusCode.OK))
                    logger.info(f"Ingested {len(data_df)} rows into FeatureView '{feature_view_name}'.")
                except Exception as e:
                    FS_CALLS_TOTAL.labels(operation="ingest_features", status="failure", **self.metric_labels).inc()
                    FS_CALLS_ERRORS.labels(operation="ingest_features", error_type=type(e).__name__, **self.metric_labels).inc()
                    span.record_exception(e)
                    span.set_status(Status(StatusCode.ERROR, f"Failed to ingest features: {e}"))
                    logger.error(f"Failed to ingest data into FeatureView '{feature_view_name}': {e}", exc_info=True)
                    raise
                finally:
                    FS_CALL_LATENCY_SECONDS.labels(operation="ingest_features", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def get_online_features(self, feature_refs: List[str], entity_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        async with self.semaphore:
            with tracer.start_as_current_span("feast_get_online_features") as span:
                ...
                start_time = time.monotonic()
                FS_CALLS_TOTAL.labels(operation="get_online_features", status="attempt", **self.metric_labels).inc()
                try:
                    ...
                    FS_CALLS_TOTAL.labels(operation="get_online_features", status="success", **self.metric_labels).inc()
                    span.set_status(Status(StatusCode.OK))
                    logger.info(f"Retrieved online features for {len(entity_rows)} entities.")
                    return all_results
                except Exception as e:
                    FS_CALLS_TOTAL.labels(operation="get_online_features", status="failure", **self.metric_labels).inc()
                    FS_CALLS_ERRORS.labels(operation="get_online_features", error_type=type(e).__name__, **self.metric_labels).inc()
                    span.record_exception(e)
                    ...
                    FS_CALL_LATENCY_SECONDS.labels(operation="get_online_features", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def get_historical_features(self, entity_df: Any, feature_refs: List[str]) -> Any:
        ...
        async with self.semaphore:
            with tracer.start_as_current_span("feast_get_historical_features") as span:
                ...
                span.set_attribute("feast.feature_refs", str(feature_refs))
                start_time = time.monotonic()
                FS_CALLS_TOTAL.labels(operation="get_historical_features", status="attempt", **self.metric_labels).inc()
                try:
                    loop = asyncio.get_running_loop()
                    job = await loop.run_in_executor(None, self._fs.get_historical_features, entity_df, feature_refs)
                    feature_data = await loop.run_in_executor(None, job.to_df)
                    
                    if 'email' in feature_data.columns:
                        feature_data['email_hash'] = feature_data['email'].apply(lambda x: hashlib.sha256(str(x).encode()).hexdigest())
                        feature_data = feature_data.drop(columns=['email'])
                        
                    await self.log_operation("get_historical_features", {"num_entities": len(entity_df), "feature_refs": feature_refs})
                        
                    FS_CALLS_TOTAL.labels(operation="get_historical_features", status="success", **self.metric_labels).inc()
                    span.set_status(Status(StatusCode.OK))
                    logger.info(f"Retrieved historical features for {len(entity_df)} entities.")
                    return feature_data
                except Exception as e:
                    FS_CALLS_TOTAL.labels(operation="get_historical_features", status="failure", **self.metric_labels).inc()
                    FS_CALLS_ERRORS.labels(operation="get_historical_features", error_type=type(e).__name__, **self.metric_labels).inc()
                    span.record_exception(e)
                    span.set_status(Status(StatusCode.ERROR, f"Failed to get historical features: {e}"))
                    logger.error(f"Failed to retrieve historical features: {e}", exc_info=True)
                    raise
                finally:
                    FS_CALL_LATENCY_SECONDS.labels(operation="get_historical_features", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def validate_features(self, feature_view_name: str) -> Dict[str, Any]:
        ...
        with tracer.start_as_current_span("feast_validate_features") as span:
            span.set_attribute("feast.feature_view", feature_view_name)
            start_time = time.monotonic()
            FS_CALLS_TOTAL.labels(operation="validate_features", status="attempt", **self.metric_labels).inc()
            try:
                import pandas as pd
                loop = asyncio.get_running_loop()
                feature_view = self._fs.get_feature_view(feature_view_name)
                ...
                if not GX_AVAILABLE:
                    ...
                    ge_results = await loop.run_in_executor(None, lambda: context.run_checkpoint(checkpoint.name, batch_request={"dataframe": df}))
                    results["success"] = ge_results.success
                    results["validation_results"] = ge_results.to_dict()

                freshness = (datetime.now(timezone.utc) - df.get('event_timestamp', pd.Series([datetime.now(timezone.utc)])).max()).total_seconds() if 'event_timestamp' in df else 0
                FS_FEATURE_FRESHNESS_SECONDS.labels(feature_view=feature_view_name, **self.metric_labels).set(freshness)

                # Statistical drift detection (e.g., KL divergence for numerical features)
                drift_stats = {}
                if SCIPY_AVAILABLE:
                    numeric_type_names = {t for t in ("INT64", "DOUBLE", "FLOAT") if hasattr(ValueType, t)}
                    for feature in feature_view.schema:
                        if feature.dtype.name in numeric_type_names:
                            baseline_data = await self.get_historical_features(
                                entity_df=pd.DataFrame({"user_id": [101], "event_timestamp": [datetime.now(timezone.utc) - timedelta(days=7)]}),
                            ...
                            if not baseline_data.empty and not df.empty:
                                ...
                                baseline_values = baseline_data.get(feature.name, pd.Series([])).dropna().values
                                if len(current_values) > 0 and len(baseline_values) > 0:
                                    current_dist = pd.Series(current_values).value_counts(normalize=True).sort_index()
                                    baseline_dist = pd.Series(baseline_values).value_counts(normalize=True).sort_index()
                                    common_indices = current_dist.index.intersection(baseline_dist.index)
                                    if not common_indices.empty:
                                        kl_div = scipy.stats.entropy(current_dist[common_indices], baseline_dist[common_indices])
                                        drift_stats[feature.name] = kl_div > 0.1

                await self.log_operation("validate_features", {
                ...
                FS_CALLS_TOTAL.labels(operation="validate_features", status="success", **self.metric_labels).inc()
                span.set_status(Status(StatusCode.OK))
                logger.info(f"Validated FeatureView '{feature_view_name}' with Great Expectations and drift detection.")
                return {
                    "freshness_ok": freshness < 3600,
                    "drift_detected": not results["success"] or (any(drift_stats.values()) if drift_stats else False),
                    "validation_results": results,
                    "drift_stats": drift_stats
                }
            except Exception as e:
                FS_CALLS_TOTAL.labels(operation="validate_features", status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="validate_features", error_type=type(e).__name__, **self.metric_labels).inc()
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to validate features: {e}"))
                logger.error(f"Failed to validate FeatureView '{feature_view_name}': {e}", exc_info=True)
                raise ValueError(f"Failed to validate features: {e}") from e
            finally:
                FS_CALL_LATENCY_SECONDS.labels(operation="validate_features", **self.metric_labels).observe(time.monotonic() - start_time)
    ...
    async def flag_for_redaction(self, feature_view_name: str, reason: str) -> None:
        ...
        with tracer.start_as_current_span("feast_flag_for_redaction") as span:
            span.set_attribute("feast.feature_view", feature_view_name)
            FS_CALLS_TOTAL.labels(operation="flag_for_redaction", status="attempt", **self.metric_labels).inc()
            try:
                ...
                if not POSTGRES_AVAILABLE:
                    ...
                    await self.log_operation("redaction_flag", {"feature_view": feature_view_name, "reason": reason, "timestamp": datetime.now(timezone.utc).isoformat()})
                else:
                    pg_client = PostgresClient()
                    async with pg_client:
                        await pg_client.save("feature_redactions", {
                            "id": view_hash,
                            "feature_view": feature_view_name,
                            "reason": reason,
                ...
                FS_REDACTIONS_TOTAL.labels(feature_view=feature_view_name, **self.metric_labels).inc()
                FS_CALLS_TOTAL.labels(operation="flag_for_redaction", status="success", **self.metric_labels).inc()
                span.set_status(Status(StatusCode.OK))
                logger.info(f"Flagged FeatureView '{feature_view_name}' for redaction: {reason}")
            except Exception as e:
                FS_CALLS_TOTAL.labels(operation="flag_for_redaction", status="failure", **self.metric_labels).inc()
                FS_CALLS_ERRORS.labels(operation="flag_for_redaction", error_type=type(e).__name__, **self.metric_labels).inc()
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Failed to flag for redaction: {e}"))
                logger.error(f"Failed to flag FeatureView '{feature_view_name}' for redaction: {e}", exc_info=True)
                raise ValueError(f"Failed to flag for redaction: {e}") from e
            finally:
                FS_CALL_LATENCY_SECONDS.labels(operation="flag_for_redaction", **self.metric_labels).observe(time.monotonic() - start_time)
...
async def main():
    ...
    if provider == "gcp":
        ...
        if not all([project_id, bigquery_dataset, redis_url]):
            raise ValueError("GCP configuration requires GCP_PROJECT, BIGQUERY_DATASET, and REDIS_URL.")
        yaml_content = f"""
project: {project_id}
...
"""
    ...
    try:
        ...
        subprocess.run(
            ["feast", "materialize-incremental", datetime.now(timezone.utc).isoformat()],
            cwd=repo_path,
            check=True
        )
        # Get online features
        ...
        assert historical_features[historical_features['user_id'] == 101]['daily_login_count'].iloc[0] == 5, "Point-in-time join failed"
        await client.log_operation("get_historical_features", {"num_entities": len(entity_df_historical)})
        # Validate features
        logger.info("\nValidating features...")
        ...
        await client.log_operation("validate_features", {"feature_view": "user_daily_logins", "success": validation_results['freshness_ok']})
        # Test edge cases
        try:
            invalid_fv = FeatureView(name="invalid", entities=["missing"], ttl=timedelta(days=0), schema=[])
        ...
        try:
            ...
            validation_results = await client.validate_features("user_daily_logins")
            assert not validation_results['freshness_ok'] or validation_results['drift_detected'], "Expected validation failure for invalid data"
            logger.info("Caught expected GX validation failure")
            await client.log_operation("validate_features", {"feature_view": "user_daily_logins", "success": False})
        except ValueError:
        ...
        await client.log_operation("ingest_features", {"feature_view": "user_daily_logins", "rows": len(extreme_df)})
        ...
        await client.log_operation("flag_for_redaction", {"feature_view": "user_daily_logins", "reason": "GDPR"})
    except Exception as e:
