...
REDIS_CALLS_TOTAL = _get_or_create_metric(Counter, "redis_calls_total", "Total Redis calls", ["operation", "status"])
REDIS_CALLS_ERRORS = _get_or_create_metric(Counter, "redis_calls_errors", "Redis call errors", ["operation", "error_type"])
REDIS_CALL_LATENCY_SECONDS = _get_or_create_metric(Histogram, "redis_call_latency_seconds", "Redis call latency in seconds", ["operation"])
REDIS_CONNECTIONS_CURRENT = _get_or_create_metric(Gauge, "redis_connections_current", "Current number of active Redis connections")
REDIS_LOCK_ACQUIRED_TOTAL = _get_or_create_metric(Counter, "redis_lock_acquired_total", "Total Redis locks acquired")
REDIS_LOCK_RELEASED_TOTAL = _get_or_create_metric(Counter, "redis_lock_released_total", "Total Redis locks released")
REDIS_LOCK_FAILED_TOTAL = _get_or_create_metric(Counter, "redis_lock_failed_total", "Total Redis lock acquisition failures")
REDIS_MEMORY_USAGE = _get_or_create_metric(Gauge, "redis_memory_usage_mb", "Redis memory usage in megabytes", ["instance"])
REDIS_KEYSPACE_SIZE = _get_or_create_metric(Gauge, "redis_keyspace_size", "Number of keys in Redis database", ["instance"])
...
class RedisClient:
    ...
    def __init__(self, redis_url: Optional[str] = None):
        ...
        self.use_ssl = self.redis_url.startswith("rediss://") or os.getenv("REDIS_USE_SSL", "false").lower() == "true"
        
        env = os.getenv("ENV", "dev").lower()
        if env == "prod" and not self.use_ssl:
            logger.error("SSL is required in production (ENV=prod). Use rediss:// or set REDIS_USE_SSL=true.")
        ...
        logger.info(f"RedisClient initialized for URL: {_redact_key(self.redis_url)}, SSL: {self.use_ssl}")
    ...
    async def connect(self) -> None:
        ...
        with tracer.start_as_current_span("redis_connect") as span:
            ...
            try:
                ...
                logger.error(f"Failed to connect to Redis at {_redact_key(self.redis_url)}: {e}", exc_info=True)
                raise ConnectionError(f"Failed to connect to Redis: {e}") from e
            finally:
                REDIS_CALL_LATENCY_SECONDS.labels(operation="connect").observe(time.monotonic() - start_time)
    ...
    async def _start_health_check(self, interval: float = float(os.getenv("REDIS_HEALTH_CHECK_INTERVAL", "60.0"))) -> None:
    ...
    async def disconnect(self) -> None:
        ...
        with tracer.start_as_current_span("redis_disconnect") as span:
            ...
            try:
                ...
                REDIS_CALL_LATENCY_SECONDS.labels(operation="disconnect").observe(time.monotonic() - start_time)

    async def _execute_operation(self, operation: str, key: str, func: callable, *args, **kwargs) -> Any:
        """
        ...
        with tracer.start_as_current_span(span_name) as span:
            ...
            for attempt in range(2):
                try:
                    ...
                    if attempt == 0:
                        logger.warning(f"Transient error during {operation} on key '{_redact_key(key)}': {e}. Attempting reconnect.")
                        await self.reconnect()
                        continue
                    REDIS_CALLS_TOTAL.labels(operation=operation, status="failure").inc()
                    REDIS_CALLS_ERRORS.labels(operation=operation, error_type=type(e).__name__).inc()
                    span.record_exception(e)
                    span.set_status(trace.Status(trace.StatusCode.ERROR, f"Redis {operation.upper()} error: {e}"))
                    logger.error(f"Redis {operation.upper()} operation failed for key '{_redact_key(key)}': {e}", exc_info=True)
                    raise
                finally:
                    REDIS_CALL_LATENCY_SECONDS.labels(operation=operation).observe(time.monotonic() - start_time)
    ...
    async def set(self, key: str, value: Any, ex: Optional[int] = None, px: Optional[int] = None) -> bool:
        ...
        if not isinstance(value, (str, bytes, int, float)):
            try:
                ...
                logger.error(f"Value for key '{_redact_key(key)}' is not JSON serializable: {e}", exc_info=True)
    ...
    async def mset(self, mapping: Dict[str, Any]) -> bool:
        ...
        return await self._execute_operation("mset", "multiple", self.client.mset, sanitized_mapping)
    ...
    def lock(self, name: str, timeout: int = 10, blocking_timeout: int = 5) -> RedisLock:
        ...
        logger.debug(f"Creating RedisLock for '{_redact_key(name)}' with timeout={timeout}, blocking_timeout={blocking_timeout}")
...
async def main():
    ...
    try:
        ...
        test_json_value = {"data": "complex_object", "timestamp": datetime.now().isoformat()}
        ...
        lock = client.lock(test_lock_name, timeout=5, blocking_timeout=2) # Lock for max 5s, wait max 2s
        
        async with lock:
            logger.info(f"Successfully acquired lock '{_redact_key(test_lock_name)}'. Holding for 1 second...")
        ...
        logger.info(f"Attempting to acquire lock '{_redact_key(test_lock_name)}' concurrently (should fail)...")
        concurrent_lock = client.lock(test_lock_name, timeout=1, blocking_timeout=0.5)
        try:
            async with concurrent_lock:
                logger.warning("Concurrent lock acquired (this should ideally not happen in a real concurrent test).")
        except LockError:
