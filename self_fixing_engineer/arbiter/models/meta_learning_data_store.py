...
try:
    ...
    logging.getLogger(__name__).warning("cryptography library not found. Sensitive fields will not be encrypted.")
...
try:
    ...
    logging.getLogger(__name__).warning("redis.asyncio library not found. Redis backend will not be available.")
    REDIS_AVAILABLE = False
    class redis:
        class Redis:
            def __init__(self, *args, **kwargs): pass
            async def ping(self): raise ConnectionError("Redis not available")
            async def hset(self, *args, **kwargs): raise ConnectionError("Redis not available")
            async def hget(self, *args, **kwargs): raise ConnectionError("Redis not available")
            async def hgetall(self, *args, **kwargs): raise ConnectionError("Redis not available")
...
MLDS_OPS_TOTAL = _get_or_create_metric(Counter, "meta_learning_ops_total", "Total MetaLearningDataStore operations", ["operation", "status"])
MLDS_OPS_LATENCY = _get_or_create_metric(Histogram, "meta_learning_ops_latency_seconds", "Latency of MetaLearningDataStore ops", ["operation"])
MLDS_DATA_SIZE = _get_or_create_metric(Gauge, "meta_learning_data_size", "Current number of records in MetaLearningDataStore", ["backend"])
...
class MetaLearningRecord(BaseModel):
    experiment_id: str = Field(..., description="Unique identifier for the meta-learning experiment.")
    ...
    meta_features: Dict[str, Any] = Field(..., description="Meta-features extracted from the dataset.")
    ...
    model_artifact_uri: Optional[str] = Field(None, description="URI/path to the trained model artifact (can be encrypted).")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="UTC timestamp of record creation.")
    tags: Optional[List[str]] = Field(default_factory=list, description="User-defined tags/annotations.")
    ...
    def validate_tags(cls, tag):
        ...
        if not tag.replace('-', '').replace('_', '').isalnum(): # Allow alphanumeric, hyphens, underscores
...
class BaseMetaLearningDataStore:
    ...
    async def update_record(self, experiment_id: str, updates: Dict[str, Any]) -> MetaLearningRecord:
        raise NotImplementedError
    ...
    async def _encrypt_field(self, data: Optional[str]) -> Optional[str]:
        ...
        if not key:
            if REQUIRE_ENCRYPTION:
                raise MetaLearningEncryptionError("Encryption key not set and encryption is required.")
...
class InMemoryMetaLearningDataStore(BaseMetaLearningDataStore):
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10),
           retry=retry_if_exception_type(TRANSIENT_ERRORS), reraise=True)
    async def add_record(self, record: Union[MetaLearningRecord, Dict[str, Any]]) -> str:
        op = "add_record"
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                async with self._lock:
                    if record.experiment_id in self._store:
                        raise MetaLearningDataStoreError(f"Experiment ID {record.experiment_id} already exists.")
                    
                    if len(self._store) >= self.max_records:
                        raise MetaLearningDataStoreError(f"Max records limit ({self.max_records}) reached for in-memory store.")
            ...
            except (ValidationError, MetaLearningDataStoreError, MetaLearningEncryptionError) as e:
            ...
            try:
                async with self._lock:
                    if experiment_id not in self._store:
                        raise MetaLearningRecordNotFound(f"Experiment ID {experiment_id} not found.")
                    record = self._store[experiment_id].copy(deep=True) # Return a copy to prevent external modification
                ...
                logger.error(f"Unexpected error getting record {experiment_id}: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
            ...
            try:
                ...
                for r in records:
                    ...
                    if record_copy.model_artifact_uri:
                        record_copy.model_artifact_uri = await self._decrypt_field(record_copy.model_artifact_uri)
            ...
            try:
                async with self._lock:
                    ...
                    if 'model_artifact_uri' in updates and updates['model_artifact_uri'] is not None:
                        updates['model_artifact_uri'] = await self._encrypt_field(updates['model_artifact_uri'])
                    ...
                    if updated.model_artifact_uri:
                        updated.model_artifact_uri = await self._decrypt_field(updated.model_artifact_uri)

                    MLDS_OPS_TOTAL.labels(operation=op, status="success").inc()
                    return updated
            except (ValidationError, MetaLearningRecordNotFound, MetaLearningEncryptionError) as e:
                logger.error(f"Error updating record {experiment_id}: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise e
            except Exception as e:
                logger.error(f"Unexpected error updating record {experiment_id}: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
    ...
    async def delete_record(self, experiment_id: str) -> None:
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                logger.error(f"Unexpected error deleting record {experiment_id}: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
...
class RedisMetaLearningDataStore(BaseMetaLearningDataStore):
    ...
    def __init__(self):
        ...
        logger.info(f"RedisMetaLearningDataStore initialized with URL: {self.redis_url}, Hash Key: {self.redis_hash_key}")
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                if current_size >= self.max_records:
                    raise MetaLearningDataStoreError(f"Max records limit ({self.max_records}) reached for Redis store.")

                result = await self._redis.hset(self.redis_hash_key, record.experiment_id, record_json)
                if result == 0: # Key already existed and was updated
                    raise MetaLearningDataStoreError(f"Experiment ID {record.experiment_id} already exists (was updated instead of added).")
    ...
    async def get_record(self, experiment_id: str) -> MetaLearningRecord:
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                if not record_json:
                    raise MetaLearningRecordNotFound(f"Experiment ID {experiment_id} not found in Redis.")
                ...
                logger.error(f"Error getting record {experiment_id} from Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise e
            except ConnectionError as e:
                ...
                logger.error(f"Unexpected error getting record {experiment_id} from Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise MetaLearningDataStoreError(f"Failed to get record: {e}") from e
            finally:
                MLDS_OPS_LATENCY.labels(operation=op).observe(time.monotonic() - start)
            ...
            try:
                ...
                for _, record_json in all_records_data.items():
                    try:
                        ...
                        logger.warning(f"Skipping invalid record from Redis: {e}. Data: {record_json[:100]}...", exc_info=True)
                    except MetaLearningEncryptionError as e:
                        logger.warning(f"Skipping record due to decryption error: {e}. Data: {record_json[:100]}...", exc_info=True)
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                if not existing_record_json:
                    raise MetaLearningRecordNotFound(f"Experiment ID {experiment_id} not found in Redis for update.")
                ...
                if 'model_artifact_uri' not in updates and base_record.model_artifact_uri:
                    base_record.model_artifact_uri = await self._decrypt_field(base_record.model_artifact_uri)
                ...
                if updated_record.model_artifact_uri and 'model_artifact_uri' not in updates:
                     updated_record.model_artifact_uri = await self._encrypt_field(updated_record.model_artifact_uri)
                ...
                if updated_record.model_artifact_uri:
                    updated_record.model_artifact_uri = await self._decrypt_field(updated_record.model_artifact_uri)

                MLDS_OPS_TOTAL.labels(operation=op, status="success").inc()
                return updated_record
            except (ValidationError, MetaLearningRecordNotFound, MetaLearningEncryptionError) as e:
                logger.error(f"Error updating record {experiment_id} in Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise e
            except ConnectionError as e:
                ...
                logger.error(f"Unexpected error updating record {experiment_id} in Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise MetaLearningDataStoreError(f"Failed to update record: {e}") from e
            finally:
                MLDS_OPS_LATENCY.labels(operation=op).observe(time.monotonic() - start)
    ...
    async def delete_record(self, experiment_id: str) -> None:
        ...
        with tracer.start_as_current_span(f"meta_learning_{op}") as span:
            ...
            try:
                ...
                if result == 0:
                    raise MetaLearningRecordNotFound(f"Experiment ID {experiment_id} not found in Redis for deletion.")
                
                MLDS_DATA_SIZE.labels(backend=self._backend_label).dec()
                logger.info(f"Deleted meta-learning record from Redis: {experiment_id}")
                MLDS_OPS_TOTAL.labels(operation=op, status="success").inc()
            except MetaLearningRecordNotFound as e:
                logger.error(f"Error deleting record {experiment_id} from Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise e
            except ConnectionError as e:
                ...
                logger.error(f"Unexpected error deleting record {experiment_id} from Redis: {e}", exc_info=True)
                MLDS_OPS_TOTAL.labels(operation=op, status="failure").inc()
                raise MetaLearningDataStoreError(f"Failed to delete record: {e}") from e
            finally:
                MLDS_OPS_LATENCY.labels(operation=op).observe(time.monotonic() - start)
...
def get_meta_learning_data_store(backend: Optional[str] = None, **kwargs) -> BaseMetaLearningDataStore:
    """
...
async def main():
    ...
    try:
        ...
        for i in range(in_memory_store.max_records):
            await in_memory_store.add_record(MetaLearningRecord(experiment_id=f"limit_exp_{i}", task_type="test", dataset_name="test", meta_features={}, hyperparameters={}, metrics={}))
        
        try:
            await in_memory_store.add_record(MetaLearningRecord(experiment_id="over_limit", task_type="test", dataset_name="test", meta_features={}, hyperparameters={}, metrics={}))
    ...
    if REDIS_AVAILABLE:
        ...
        try:
            ...
            async with redis_store:
                ...
                MLDS_DATA_SIZE.labels(backend=redis_store._backend_label).set(0) # Reset metric after flush
                ...
                assert rec_redis.model_artifact_uri == "s3://models/boston/v2" # Should be decrypted
                ...
                for i in range(redis_store.max_records):
                    await redis_store.add_record(MetaLearningRecord(experiment_id=f"redis_limit_exp_{i}", task_type="test", dataset_name="test", meta_features={}, hyperparameters={}, metrics={}))
                
                try:
                    await redis_store.add_record(MetaLearningRecord(experiment_id="redis_over_limit", task_type="test", dataset_name="test", meta_features={}, hyperparameters={}, metrics={}))
