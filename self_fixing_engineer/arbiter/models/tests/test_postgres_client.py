...
async def test_connect_success(pg_client):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="connect", table="n/a", status="success") == 1
...
async def test_connect_failure(mocker: MockerFixture):
    ...
    mocker.patch("postgres_client.asyncpg.create_pool", 
    ...
    assert get_metric_value(DB_CALLS_ERRORS, db_type="postgresql", operation="connect", table="n/a", error_type="PostgresError") >= 1
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="connect", table="n/a", status="failure") >= 1
...
async def test_disconnect_success(pg_client):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="disconnect", table="n/a", status="success") == 1
...
async def test_save_success(pg_client, mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="save", table="feedback", status="success") == 1
...
async def test_save_many_success(pg_client, mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="save_many", table="feedback", status="success") == 1
...
async def test_load_success(pg_client, mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="load", table="feedback", status="success") == 1
...
async def test_load_all_success(pg_client, mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="load_all", table="feedback", status="success") == 1
...
async def test_update_success(pg_client, mocker: MockerFixture):
    ...
    updated = await pg_client.update("feedback", {"id": SAMPLE_FEEDBACK_DATA["id"]}, {"data": {"new_comment": "Updated"}})
    assert updated
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="update", table="feedback", status="success") == 1
...
async def test_delete_success(pg_client, mocker: MockerFixture):
    ...
    assert deleted
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="delete", table="feedback", status="success") == 1
...
async def test_retry_on_connect_failure(mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_ERRORS, db_type="postgresql", operation="connect", table="n/a", error_type="PostgresError") == 2
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="connect", table="n/a", status="success") == 1
...
async def test_concurrent_save(pg_client, mocker: MockerFixture):
    ...
    assert get_metric_value(DB_CALLS_TOTAL, db_type="postgresql", operation="save", table="feedback", status="success") == 5
...
async def test_jsonb_handling(pg_client, mocker: MockerFixture):
    ...
    mock_conn = pg_client._pool.acquire.return_value.__aenter__.return_value
    mock_conn.fetch = mocker.AsyncMock(side_effect=[
...
async def test_ssl_mode(mocker: MockerFixture):
    ...
    create_pool_mock = mocker.patch("postgres_client.asyncpg.create_pool", 
...
async def test_agent_knowledge_operations(pg_client, mocker: MockerFixture):
    ...
    # Mock for save
    mock_conn = pg_client._pool.acquire.return_value.__aenter__.return_value
    mock_conn.fetch = mocker.AsyncMock(return_value=[{"domain": knowledge_data["domain"], "key": knowledge_data["key"]}])
    ...
    loaded = await pg_client.load("agent_knowledge", f"{knowledge_data['domain']}:{knowledge_data['key']}", query_field="domain_key")
    ...
    deleted = await pg_client.delete("agent_knowledge", f"{knowledge_data['domain']}:{knowledge_data['key']}", query_field="domain_key")
