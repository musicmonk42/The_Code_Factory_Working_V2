...
from multi_modal_schemas import (
    BaseConfig, ImageOCRResult, ImageCaptioningResult, ImageAnalysisResult,
    AudioTranscriptionResult, AudioAnalysisResult, VideoSummaryResult,
    VideoAnalysisResult, Sentiment, Severity, to_camel, MultiModalAnalysisResult
)
...
SAMPLE_IMAGE_ANALYSIS = {
    ...
    "captioning_result": {"caption": "A person standing in front of a building.", "confidence": 0.88},
...
SAMPLE_AUDIO_ANALYSIS = {
    ...
    "transcription": {"text": "This is a test.", "language": "en", "duration_seconds": 15.5, "speakers": ["speaker1"]},
...
SAMPLE_VIDEO_ANALYSIS = {
    ...
    "summary_result": {"summary_text": "Python tutorial.", "key_moments_timestamps": [10.5, 45.2], "chapters": [{"title": "Intro", "start": 0.0}]},
...
def test_camel_case_deserialization():
    ...
    assert str(image.source_url) == "http://example.com/camel.jpg/"  # Note: Pydantic may normalize URLs
...
def test_list_max_length():
    ...
    image = ImageAnalysisResult(
        image_id="img_list",
        detected_objects=["obj"] * 500  # Max is 500
    )
    ...
    with pytest.raises(ValidationError) as exc_info:
        ImageAnalysisResult(
            image_id="img_list_exceed",
            detected_objects=["obj"] * 501
        )
