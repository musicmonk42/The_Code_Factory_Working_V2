...
def mock_web3_dependencies(mocker: MockerFixture):
    ...
    mock_eth.wait_for_transaction_receipt = AsyncMock(return_value=MockReceipt(
        status=1, blockNumber=100, gasUsed=150000
    ))
    mock_eth.get_transaction_receipt = AsyncMock(return_value=MockReceipt(
        status=1, blockNumber=100, gasUsed=150000
    ))
    ...
    mock_contract.functions.logEvent.return_value.build_transaction.return_value = {
        "chainId": 1337, "gas": 200000, "gasPrice": 2000000000, "nonce": 1
    }
    ...
    mocker.patch("arbiter.models.audit_ledger_client.AsyncWebsocketProvider", mock_async_websocket_provider)
    ...
    def mock_checksum(addr):
        return addr
    mocker.patch("arbiter.models.audit_ledger_client.to_checksum_address", mock_checksum, create=True)
...
async def audit_client(mock_web3_dependencies, mocker: MockerFixture):
    ...
    original_connect = AuditLedgerClient.connect.__wrapped__ if hasattr(AuditLedgerClient.connect, '__wrapped__') else AuditLedgerClient.connect
    original_log_event = AuditLedgerClient.log_event.__wrapped__ if hasattr(AuditLedgerClient.log_event, '__wrapped__') else AuditLedgerClient.log_event
    original_batch = AuditLedgerClient.batch_log_events.__wrapped__ if hasattr(AuditLedgerClient.batch_log_events, '__wrapped__') else AuditLedgerClient.batch_log_events
...
class TestAuditLedgerClientInit:
    ...
    def test_init_with_production_env_requires_secrets_manager(self, mock_web3_dependencies, mocker: MockerFixture):
...
class TestAuditLedgerClientEventLogging:
    ...
    async def test_log_event_transaction_failure(self, audit_client, mock_web3_dependencies):
        ...
        mock_web3_dependencies["contract"].functions.logEvent.return_value.estimate_gas.return_value = 200000
        mock_web3_dependencies["contract"].functions.logEvent.return_value.build_transaction.return_value = {
            "chainId": 1337,
            "gas": 200000,
            "gasPrice": 2000000000,
            "nonce": 1
        }
        mock_web3_dependencies["eth"].account.sign_transaction.return_value = MagicMock(rawTransaction=b"raw_tx_data")
        ...
        mock_web3_dependencies["eth"].wait_for_transaction_receipt.side_effect = DLTTransactionError(
            f"Transaction {b'tx_hash_123'.hex()} reverted."
        )
        mock_web3_dependencies["eth"].get_transaction_receipt.return_value = mock_receipt
        
        with pytest.raises(DLTTransactionError, match="An unexpected error occurred logging DLT event to ethereum"):
            await audit_client.log_event("test:event", {})
...
class TestAuditLedgerClientBatchOperations:
    ...
    async def test_batch_log_events_not_supported(self, audit_client, mock_web3_dependencies):
        ...
        events = [
            AuditEvent(event_type="test:batch", details={"id": i})
            for i in range(3)
        ]
...
class TestAuditLedgerClientHealthCheck:
    ...
    @pytest.mark.asyncio
    async def test_is_connected_updates_state_on_disconnect(self, audit_client, mock_web3_dependencies):
...
class TestAuditLedgerClientGasManagement:
    ...
    async def test_gas_estimation_failure_uses_default(self, audit_client, mock_web3_dependencies):
        ...
        mock_web3_dependencies["contract"].functions.logEvent.return_value.estimate_gas.side_effect = Exception("Gas estimation failed")
