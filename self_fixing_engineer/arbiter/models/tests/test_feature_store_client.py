...
from arbiter.models.feature_store_client import (
    FeatureStoreClient, ConnectionError
)
...
async def feature_client(mocker: MockerFixture):
    ...
    try:
        from feast import FeatureStore, Entity, FeatureView, ValueType, Field as FeastField
        ...
        mock_fv.source.__dict__ = {"path": "test.parquet", "timestamp_field": "event_timestamp"}
        ...
        mock_online_response.to_dict.return_value = {"daily_login_count": [2.0], "user_id": [101]}
...
async def test_connect_failure(mocker: MockerFixture):
    ...
    mocker.patch("arbiter.models.feature_store_client.FeatureStore", side_effect=FeastProviderError("Connection failed"))
    mocker.patch("arbiter.models.feature_store_client.AUDIT_LEDGER_AVAILABLE", False)
    mocker.patch("arbiter.models.feature_store_client.POSTGRES_AVAILABLE", False)
    ...
    assert get_metric_value(FS_CALLS_ERRORS, operation="connect", error_type="FeastProviderError") >= 1
    spans = in_memory_exporter.get_finished_spans()
...
async def test_apply_feature_definitions_failure(feature_client, mocker: MockerFixture):
    ...
    assert get_metric_value(FS_CALLS_ERRORS, operation="apply_definitions", error_type="FeastProviderError") >= 1
...
async def test_ingest_features_failure(feature_client, mocker: MockerFixture):
    ...
    mocker.patch.object(feature_client._fs, "ingest", side_effect=FeastResourceError("Ingest failed"))
    
    with pytest.raises(FeastResourceError):
        await feature_client.ingest_features("mock_fv", mock_df)
    assert get_metric_value(FS_CALLS_ERRORS, operation="ingest_features", error_type="FeastResourceError") >= 1
...
async def test_get_historical_features_success(feature_client):
    ...
    assert get_metric_value(FS_CALLS_TOTAL, operation="get_historical_features", status="success") == 1
...
async def test_get_historical_features_failure(feature_client, mocker: MockerFixture):
    ...
    mocker.patch.object(feature_client._fs, "get_historical_features", 
    ...
    assert get_metric_value(FS_CALLS_ERRORS, operation="get_historical_features", 
                           error_type="FeastObjectNotFoundException") >= 1
...
async def test_get_online_features_failure(feature_client, mocker: MockerFixture):
    ...
    from feast.errors import FeastProviderError
    mocker.patch.object(feature_client._fs, "get_online_features", 
    ...
    assert get_metric_value(FS_CALLS_ERRORS, operation="get_online_features", 
                           error_type="FeastProviderError") >= 1
...
async def test_retry_on_connect_failure(mocker: MockerFixture):
    ...
    mocker.patch("arbiter.models.feature_store_client.FeatureStore", 
                side_effect=[
    ...
    assert get_metric_value(FS_CALLS_ERRORS, operation="connect", error_type="FeastProviderError") == 2
    assert get_metric_value(FS_CALLS_TOTAL, operation="connect", status="success") == 1
...
async def test_retry_on_ingest_failure(feature_client, mocker: MockerFixture):
    ...
    mocker.patch.object(feature_client._fs, "ingest", 
                       side_effect=[
    ...
    await feature_client.ingest_features("mock_fv", mock_df)
    assert get_metric_value(FS_CALLS_ERRORS, operation="ingest_features", error_type="FeastResourceError") == 2
    assert get_metric_value(FS_CALLS_TOTAL, operation="ingest_features", status="success") == 1
...
async def test_validate_features_basic(feature_client):
    ...
    async def mock_get_online(refs, entities):
        return [{"user_id": 101, "daily_login_count": 5, "event_timestamp": datetime.now(timezone.utc)}]
