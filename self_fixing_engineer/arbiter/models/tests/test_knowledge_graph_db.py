...
async def kg_client(mocker: MockerFixture):
    ...
    try:
        ...
        from neo4j import AsyncManagedTransaction, AsyncSession, exceptions as neo4j_exceptions
        ...
        mock_result.single = mocker.AsyncMock(return_value={"nodeId": "mock_node_id", "relId": "mock_rel_id"})
...
async def test_health_check_failure(kg_client, mocker: MockerFixture):
    ...
    await kg_client.connect()
    from neo4j import exceptions as neo4j_exceptions
    mocker.patch.object(kg_client._driver, "verify_connectivity", 
                       side_effect=neo4j_exceptions.ServiceUnavailable("Unavailable"))
    is_healthy = await kg_client.health_check()
    assert not is_healthy
    assert get_metric_value(KG_ERRORS, operation="health_check", error_type="ServiceUnavailable") >= 1
    assert get_metric_value(KG_OPS_TOTAL, operation="health_check", status="failure") >= 1
...
async def test_export_graph_success(kg_client, tmp_path, mocker: MockerFixture):
    ...
    mock_rel = {"rid": "rel1", "type": "TEST_REL", "start_eid": "node1", "end_eid": "node2", "props": {}}
...
async def test_import_graph_success(kg_client, tmp_path, mocker: MockerFixture):
    ...
    with gzip.open(f"{import_file}.rels.jsonl.gz", "wt") as f:
        f.write(json.dumps({"rid": "rel1", "type": "TEST_REL", "start_eid": "node1", "end_eid": "node2", "props": {}}) + "\n")
