...
try:
    ...
    logging.getLogger(__name__).critical("merklelib library not found. MerkleTree functionality will not be available.")
    ...
    class MerkleLibTree:
        def __init__(self, leaves: Optional[List[bytes]] = None):
            ...
            logging.getLogger(__name__).warning("Mock MerkleLibTree initialized. Merkle operations will be conceptual.")
        ...
        def get_proof(self, index: int) -> List[Tuple[bytes, str]]:
            if not (0 <= index < len(self._hashed_leaves_mock)):
                raise IndexError(f"Mock MerkleLibTree: Leaf index {index} out of bounds for {len(self._hashed_leaves_mock)} leaves.")
            # Return a mock proof that can still be used conceptually
            return [(hashlib.sha256(b"mock_sibling").digest(), "right")] if self._hashed_leaves_mock else []
...
def _get_or_create_metric(metric_class: Union[Type[Counter], Type[Gauge], Type[Histogram]],
    ...
    try:
        if buckets:
            return metric_class(name, documentation, labelnames=labelnames, buckets=buckets, registry=METRICS_REGISTRY)
...
MERKLE_OPS_TOTAL = _get_or_create_metric(Counter, "merkle_ops_total", "Total Merkle tree operations", ["operation", "status"])
MERKLE_OPS_LATENCY_SECONDS = _get_or_create_metric(Histogram, "merkle_ops_latency_seconds", "Merkle tree operation latency in seconds", ["operation"], buckets=(.001, .01, .1, 1, 2, 5, 10))
MERKLE_TREE_SIZE = _get_or_create_metric(Gauge, "merkle_tree_size", "Current number of leaves in Merkle tree")
MERKLE_TREE_DEPTH = _get_or_create_metric(Gauge, "merkle_tree_depth", "Current depth of the Merkle tree")
...
class MerkleTree:
    ...
    async def _update_tree(self) -> None:
        ...
        if not MERKLELIB_AVAILABLE: 
            logger.warning("merklelib not available. Cannot rebuild tree."); return
        with self._rwlock:
            leaves = self._leaves
    ...
    def _proof_for_index(self, idx: int) -> List[Tuple[bytes, str]]:
        ...
        if hasattr(self._tree, "get_proof"):
            try:
                ...
                target = self._leaves[idx] if not self._store_raw else self._hash_leaf(self._leaves[idx])
                ...
                with tracer.start_as_current_span("merkle_add_leaf") as span:
                    ...
                    try:
                        ...
                        if not MERKLELIB_AVAILABLE or not self._tree:
                            raise MerkleTreeError("MerkleTree not initialized or merklelib not available.")
                        ...
                        if MERKLELIB_AVAILABLE and hasattr(self._tree, "add_leaf"):
                            ...
                            if hasattr(self._tree, "make_tree"): self._tree.make_tree()
                            self._update_metrics()
                        else:
                        ...
                        logger.debug(f"Added leaf to Merkle tree. Current leaves: {len(self._leaves)}")
                        ...
                        MERKLE_OPS_LATENCY_SECONDS.labels(operation="add_leaf").observe(time.monotonic() - start_time)
            ...
            with self._rwlock:
                with tracer.start_as_current_span("merkle_add_leaves") as span:
                    ...
                    try:
                        ...
                        new_items_raw = [d.encode('utf-8') if isinstance(d, str) else d for d in data_list]
                        ...
                        span.set_status(Status(StatusCode.OK))
                        logger.debug(f"Added {len(data_list)} leaves to Merkle tree. Current leaves: {len(self._leaves)}")
                    except Exception as e:
                        ...
                        MERKLE_OPS_LATENCY_SECONDS.labels(operation="add_leaves").observe(time.monotonic() - start_time)
        ...
        with self._rwlock:
            with tracer.start_as_current_span("merkle_get_root") as span:
                ...
                try:
                    ...
                    MERKLE_OPS_LATENCY_SECONDS.labels(operation="get_root").observe(time.monotonic() - start_time)
    ...
    def get_proof(self, index: int) -> List[Dict[str, str]]:
        with self._rwlock:
            with tracer.start_as_current_span("merkle_get_proof") as span:
                ...
                try:
                    if not self._leaves:
                        raise MerkleTreeEmptyError("Attempted to get proof from an empty Merkle tree.")
                    if not (0 <= index < len(self._leaves)):
                        raise IndexError(f"Leaf index {index} out of bounds for {len(self._leaves)} leaves.")
                    
                    if not MERKLELIB_AVAILABLE or not self._tree:
                        raise MerkleTreeError("MerkleTree not properly initialized (merklelib not available). Cannot get proof.")

                    proof_tuples = self._proof_for_index(index)
                    
                    formatted_proof = [{"node": node.hex(), "position": pos} for node, pos in proof_tuples]
                    ...
                    MERKLE_OPS_LATENCY_SECONDS.labels(operation="get_proof").observe(time.monotonic() - start_time)
    ...
    def verify_proof(root: str, leaf_data: Union[str, bytes], proof: List[Dict[str, str]]) -> bool:
        ...
        with tracer.start_as_current_span("merkle_verify_proof") as span:
            ...
            try:
                ...
                for node_data in proof:
                    if 'node' in node_data and 'position' in node_data:
                        try:
                            ...
                            raise MerkleProofError(f"Invalid hex string in proof node: {node_data['node']}")
                    else:
                        raise MerkleProofError(f"Malformed proof node: {node_data}. Expected 'node' and 'position'.")
                ...
                MERKLE_OPS_TOTAL.labels(operation="verify_proof", status="success" if is_valid else "failure").inc()
                span.set_status(Status(StatusCode.OK if is_valid else StatusCode.ERROR))
                if not is_valid:
                    logger.warning(f"Merkle proof verification failed for leaf '{leaf_data}'. Root: {root}, Proof: {proof}")
                ...
                MERKLE_OPS_LATENCY_SECONDS.labels(operation="verify_proof").observe(time.monotonic() - start_time)
            ...
            with self._rwlock:
                with tracer.start_as_current_span("merkle_save_tree") as span:
                    ...
                    try:
                        ...
                        logger.error(f"Failed to save Merkle tree state to {filepath}: {e}", exc_info=True)
                        raise
                    finally:
                        MERKLE_OPS_LATENCY_SECONDS.labels(operation="save_tree").observe(time.monotonic() - start_time)
    ...
    async def load(cls, filepath: Optional[str] = None) -> 'MerkleTree':
        ...
        with tracer.start_as_current_span("merkle_load_tree") as span:
            ...
            try:
                ...
                logger.warning(f"Merkle tree state file not found at {filepath}. Initializing empty tree.")
                ...
                logger.error(f"Merkle tree state file at {filepath} is corrupted: {e}. Initializing empty tree.", exc_info=True)
                MERKLE_OPS_TOTAL.labels(operation="load_tree", status="failure").inc()
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, "File corrupted"))
                return cls()
            except Exception as e:
                logger.error(f"Failed to load Merkle tree state from {filepath}: {e}", exc_info=True)
                MERKLE_OPS_TOTAL.labels(operation="load_tree", status="failure").inc()
                span.record_exception(e)
                span.set_status(Status(StatusCode.ERROR, f"Unexpected load error: {e}"))
                raise
            finally:
                MERKLE_OPS_LATENCY_SECONDS.labels(operation="load_tree").observe(time.monotonic() - start_time)
...
async def main():
    ...
    if MERKLELIB_AVAILABLE:
        ...
        logger.info(f"Proof verification for tampered leaf '{tampered_leaf_data}' is: {is_valid_tampered}")
    ...
    logger.info(f"Proof verification on loaded tree for leaf '{leaf_data_to_prove}' is: {is_valid_loaded}")
