...
from arbiter.models.meta_learning_data_store import MetaLearningDataStoreConfig, get_meta_learning_data_store
...
try:
    ...
    logger.warning("Failed to import 'arbiter.audit_log'. Using dummy fallbacks. "
...
class Learner:
    ...
    def __init__(self, arbiter: Arbiter, redis: Redis, db_url: Optional[str] = None, merkle_tree_class: Optional[Callable] = None):
        """
        ...
        self.event_hooks: Dict[str, List[Callable]] = {
            'pre_learn': [], 'post_learn': [], 'pre_forget': [], 'post_forget': [], 'on_schema_reload': []
        }
    ...
    async def learn_new_thing(self, domain: str, key: str, value: Any, user_id: Optional[str] = None, source: str = "system", write_to_disk: bool = True, explanation_quality_score: Optional[int] = None, agent_id: Optional[str] = None, session_id: Optional[str] = None, decision_trace: Optional[Dict[str, Any]] = None, event_type: str = "knowledge_update") -> Dict[str, Any]:
        """
                ...
                try:
                    ...
                    async with self.redis.lock(lock_key, timeout=10, blocking_timeout=5):
                        for hook in self.event_hooks['pre_learn']:
                            await (hook(domain, key, value) if asyncio.iscoroutinefunction(hook) else asyncio.to_thread(hook, domain, key, value))

                        result = await self._process_learn(domain, key, value, user_id, source, write_to_disk)
                        
                        if explanation_quality_score is not None:
                            await record_explanation_quality(self, domain, key, result.get('version'), explanation_quality_score)
                        ...
                        if result.get("status") == "learned":
                            ...
                            try:
                                learning_record = LearningRecord(
                                    ...
                                    old_value_summary=str(result.get("previous_value_summary"))[:200] if result.get("previous_value_summary") else None,
                                    version=result.get('version'),
                                    diff_applied=result.get('diff_applied'),
                                    explanation=result.get('explanation')
                                )
                                await self.meta_data_store.write_record(learning_record.model_dump())
                                logger.info("Meta-learning record written", domain=domain, key=key)
                    ...
                    await self.arbiter.bug_manager.bug_detected(
                        ...
                        {"domain": domain, "key": key, "value": str(value), "user_id": user_id, "source": source}
                    )
                    return {"status": "failed", "reason": f"unexpected_error: {e}"}
                finally:
                    learn_duration_seconds.labels(**get_labels(domain=domain)).observe(time.monotonic() - start_time)

    async def _process_learn(self, domain: str, key: str, value: Any, user_id: Optional[str], source: str, write_to_disk: bool) -> Dict[str, Any]:
        ...
        if previous_entry.get("value") == stored_value and previous_entry.get("source") == source:
            ...
            return {"status": "unchanged", "reason": "value_identical", "previous_value_summary": str(previous_value)}
        ...
        logger.info("Learned in-memory", domain=domain, key=key, version=value_with_metadata['version'], source=source)
        ...
        await self.redis.setex(f"knowledge:{domain}:{key}", ArbiterConfig.KNOWLEDGE_REDIS_TTL_SECONDS, json.dumps(redis_value))

        learn_counter.labels(**get_labels(domain=domain, source=source)).inc()
        explanation = await generate_explanation(self, domain, key, value, previous_value, value_diff)

        return {"status": "learned", "version": value_with_metadata['version'], "diff_applied": value_diff is not None, "explanation": explanation, "previous_value_summary": str(previous_value)}
    ...
    async def _get_previous_value(self, previous_entry, domain):
        ...
        if previous_entry.get("value"):
            try:
                ...
                logger.error("Failed to decrypt previous value. Assuming no previous value.", domain=domain, key=previous_entry.get('key'))
        return None
        
    async def learn_batch(self, facts: List[Dict[str, Any]], user_id: Optional[str] = None, source: str = "batch_import", write_to_disk: bool = True, agent_id: Optional[str] = None, session_id: Optional[str] = None, event_type: str = "knowledge_update_batch") -> List[Dict[str, Any]]:
        """
        ...
        with tracer.start_as_current_span("learn_batch"):
            ...
            for i in range(0, len(facts), chunk_size):
                ...
                validation_tasks = [validate_data(self, fact.get("domain", ""), fact.get("value", None)) for fact in chunk]
                ...
                for j, fact in enumerate(chunk):
                    ...
                    if isinstance(validation_res, Exception) or not validation_res["is_valid"]:
                        reason_code = validation_res['reason_code'] if isinstance(validation_res, dict) else "internal_validation_error"
                    ...
                    individual_learn_tasks.append(
                        self._prepare_and_process_single_fact_for_batch(domain, key, value, user_id, source, decision_trace)
                    )

                processed_facts_details = await asyncio.gather(*individual_learn_tasks, return_exceptions=True)
                    ...
                    if isinstance(res, Exception):
                        ...
                        if res["result"].get("status") == "learned":
                            batch_db_entries.append(
                                (res["domain"], res["key"], res["value_with_metadata"], res["timestamp"],
                                 res["leaf_hash"], res["merkle_proof"], res["merkle_root"])
                            )
                            try:
                                meta_learning_records.append(LearningRecord(
                                    ...
                                    old_value_summary=str(res.get("previous_value_summary"))[:200] if res.get("previous_value_summary") else None,
                                    version=res["value_with_metadata"]["version"],
                                    diff_applied=res["result"].get('diff_applied'),
                                    explanation=res["result"].get('explanation')
                                ).model_dump())
                            except Exception as meta_e:
                                logger.error("Failed to prepare meta-learning record for batch fact", domain=domain, key=key, error=str(meta_e), exc_info=True)
                                learn_error_counter.labels(**get_labels(domain=domain, error_type='meta_learning_batch_prep_error')).inc()
            ...
            if meta_learning_records:
                try:
                    ...
                    logger.info("Meta-learning records written for batch", count=len(meta_learning_records))
    ...
    async def _prepare_and_process_single_fact_for_batch(self, domain: str, key: str, value: Any, user_id: Optional[str], source: str, decision_trace: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if previous_entry.get("value") == stored_value and previous_entry.get("source") == source:
            ...
            return {"result": {"status": "unchanged", "reason": "value_identical", "fact": {"domain": domain, "key": key}}, "domain": domain, "key": key, "value": value, "previous_value_summary": str(previous_value), "decision_trace": decision_trace}
        ...
        return {
            "result": {"status": "learned", "version": value_with_metadata['version'], "diff_applied": value_diff is not None, "explanation": explanation, "fact": {"domain": domain, "key": key}},
            "domain": domain,
            "key": key,
            "value": value,
            "value_with_metadata": value_with_metadata,
    ...
    async def forget_fact(self, domain: str, key: str, user_id: Optional[str] = None, reason: str = "manual_deletion", agent_id: Optional[str] = None, session_id: Optional[str] = None, decision_trace: Optional[Dict[str, Any]] = None, event_type: str = "knowledge_deletion") -> Dict[str, Any]:
        """
        ...
        with tracer.start_as_current_span("forget_fact"):
            ...
            async with self.redis.lock(lock_key, timeout=10, blocking_timeout=5):
                for hook in self.event_hooks['pre_forget']:
                    await (hook(domain, key) if asyncio.iscoroutinefunction(hook) else asyncio.to_thread(hook, domain, key))
                    ...
                    if domain in ArbiterConfig.ENCRYPTED_DOMAINS:
                        try:
                            decrypted_value_for_audit = await decrypt_value(
                                bytes.fromhex(fact_to_forget.get("value")), 
                                self.ciphers
                            )
                        except ValueError:
                ...
                try:
                    if domain in self.arbiter.state.get("memory", {}) and key in self.arbiter.state["memory"][domain]:
                    ...
                    if decrypted_value_for_audit is None:
                        logger.warning("Could not decrypt value for audit log.", domain=domain, key=key)
                        leaf_hash_for_audit = hashlib.sha256(b"DECRYPTION_FAILED_FOR_AUDIT").hexdigest()
                        merkle_proof_for_audit = []
                        merkle_root_for_audit = "0"*64
                        audit_notes = "Decryption failed during audit preparation."
                    else:
                        leaf_data_for_merkle_audit = json.dumps(decrypted_value_for_audit, sort_keys=True, default=str).encode('utf-8')
                        leaf_hash_for_audit = hashlib.sha256(leaf_data_for_merkle_audit).hexdigest()
                        tree_for_audit = self.merkle_tree_class([hashlib.sha256(leaf_data_for_merkle_audit).digest()])
                    ...
                    if await self.db_circuit_breaker.can_proceed():
                        ...
                        if await self.audit_circuit_breaker.can_proceed():
                            await self.audit_logger.log_event("knowledge_forget", "Fact forgotten.", audit_event_data, user_id or "system")
                    ...
                    try:
                        learning_record = LearningRecord(
                            ...
                            old_value_summary=str(decrypted_value_for_audit)[:200] if decrypted_value_for_audit else None,
                            version=fact_to_forget.get('version'),
                            diff_applied=None,
                            explanation=f"Fact forgotten due to: {reason}"
                        )
                        await self.meta_data_store.write_record(learning_record.model_dump())
                        logger.info("Meta-learning record written for forgetting", domain=domain, key=key)
                    except Exception as meta_e:
    ...
    async def retrieve_knowledge(self, domain: str, key: str, decrypt: bool = True) -> Optional[Dict[str, Any]]:
        """
        ...
        if cached_data_json:
            try:
                ...
                if domain in ArbiterConfig.ENCRYPTED_DOMAINS and isinstance(cached_data.get("value"), str):
                    cached_data["value"] = bytes.fromhex(cached_data["value"])
                self.arbiter.state.setdefault("memory", {}).setdefault(domain, {})[key] = cached_data.copy()
                retrieve_hit_miss.labels(**get_labels(domain=domain, cache_status='redis_hit')).inc()
                ...
                learn_error_counter.labels(**get_labels(domain=domain, error_type='redis_corrupted_entry')).inc()
        ...
        if not await self.db_circuit_breaker.can_proceed():
            logger.warning("Database read skipped due to open DB circuit breaker.", domain=domain, key=key)
            learn_error_counter.labels(**get_labels(domain=domain, error_type='db_read_circuit_open')).inc()
            return None
        ...
        try:
            ...
            if db_data:
                await self.db_circuit_breaker.record_success()
                if domain in ArbiterConfig.ENCRYPTED_DOMAINS and isinstance(db_data.get("value"), str):
                    db_data["value"] = bytes.fromhex(db_data["value"])
                redis_value = db_data.copy()
                if isinstance(redis_value.get("value"), bytes):
                    redis_value["value"] = redis_value["value"].hex()
                await self.redis.setex(redis_key, ArbiterConfig.KNOWLEDGE_REDIS_TTL_SECONDS, json.dumps(redis_value))
                self.arbiter.state.setdefault("memory", {}).setdefault(domain, {})[key] = db_data.copy()
            ...
            await self.db_circuit_breaker.record_failure()
            learn_error_counter.labels(**get_labels(domain=domain, error_type='db_load_failure')).inc()

        logger.debug("Knowledge not found.", domain=domain, key=key)
        return None

    async def _process_retrieved_data(self, data: Dict[str, Any], domain: str, decrypt: bool) -> Dict[str, Any]:
        """Process retrieved data, handling decryption."""
        if decrypt and domain in ArbiterConfig.ENCRYPTED_DOMAINS and isinstance(data.get('value'), bytes):
            try:
    ...
    def _compute_diff(self, old_value: Any, new_value: Any) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            from jsonpatch import JsonPatch, JsonPointerException
            old_json = json.loads(json.dumps(old_value, default=str)) if old_value is not None else {}
            new_json = json.loads(json.dumps(new_value, default=str)) if new_value is not None else {}
            ...
            learn_error_counter.labels(**get_labels(domain='diff_computation', error_type='jsonpatch_error')).inc()
            logger.warning("Failed to compute JSON diff", error=str(e))
            return [{"op": "replace", "path": "/", "value": new_value}]
        except Exception as e:
            learn_error_counter.labels(**get_labels(domain='diff_computation', error_type='unexpected_error')).inc()
