...
schema_reload_total = Counter(
    "arbiter_learner_schema_reload_total", "Total schema reload attempts", ["status"]
)
        ...
        try:
            ...
            if schema_info:
                try:
                    ...
                    validation_failure_total.labels(
                        domain=domain, reason_code="invalid_schema"
                    ).inc()
                    return {
                        "is_valid": False,
            ...
            if hook:
                try:
                    ...
                    if not is_valid:
                        ...
                        validation_failure_total.labels(
                            domain=domain, reason_code="custom_validation_failed"
                        ).inc()
                        return {
                            "is_valid": False,
                            "reason_code": "custom_validation_failed",
                            "reason": f"Custom validation failed for '{domain}'.",
                        }
                except Exception as e:
                    logger.exception("Custom validation hook error", domain=domain)
                    validation_failure_total.labels(
                        domain=domain, reason_code="custom_validation_error"
                    ).inc()
                    return {
                        "is_valid": False,
                        "reason_code": "custom_validation_error",
                        "reason": f"Error in custom validation hook: {str(e)}",
                    }
            ...
            validation_latency_seconds.labels(domain=domain).observe(
...
def register_validation_hook(
    learner: Any,
    domain: str,
    hook_func: Callable[[Any], Union[bool, Coroutine[Any, Any, bool]]],
) -> None:
    """
    ...
    with tracer.start_as_current_span("register_validation_hook") as span:
        ...
        try:
            ...
            logger.info(
                "Validation hook registered (audit logging skipped in sync context)",
                domain=domain,
...
@retry(
    stop=stop_after_attempt(SCHEMA_RELOAD_RETRIES),
