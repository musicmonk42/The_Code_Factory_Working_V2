...
# Prometheus metrics
key_rotation_counter = Counter('key_rotation_total', 'Total number of key rotation events', ['version'])
learn_error_counter = Counter('arbiter_learn_errors_total', 'Total number of learning errors', ['domain', 'error_type'])
        ...
        if aws_region:
            try:
                ...
                for version in keys_to_load:
                    ...
                    if key_param_name:
                        response = ssm_client.get_parameter(Name=key_param_name, WithDecryption=True)
                        key_value = response['Parameter']['Value']
                        cls.ENCRYPTION_KEYS[version] = Fernet(key_value.encode('utf-8'))
                        logger.info("Loaded encryption key from SSM", version=version, key_param_name=key_param_name)
                    else:
                        logger.warning("SSM parameter path not set for key version", version=version)
                ...
                logger.error("Failed to load keys from AWS SSM. Falling back to in-memory key.", error=str(e), exc_info=True)
...
async def decrypt_value(encrypted: bytes, ciphers: Dict[str, Fernet]) -> Any:
    ...
    try:
        ...
        key_id, encrypted_data = (parts[0].decode('utf-8'), parts[1]) if len(parts) == 2 else ("v1", encrypted)
