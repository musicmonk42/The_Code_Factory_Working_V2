...
from arbiter.audit_log import TamperEvidentLogger as AuditLogger, log_event as audit_log, verify_log_integrity as verify_audit_chain
from .metrics import learn_error_counter, learn_duration_seconds, forget_duration_seconds

logger = structlog.get_logger(__name__)

# Metric for CB state
circuit_breaker_state = Gauge('circuit_breaker_state', 'Circuit breaker state (1=open, 0=closed)', ['name'])
...
class CircuitBreaker:
    ...
    def __init__(self,
        ...
        logger.info("CircuitBreaker initialized", name=self.name, threshold=self.failure_threshold, cooldown=self.cooldown_seconds)
    ...
    async def can_proceed(self) -> bool:
        ...
        if self.last_failure:
            ...
            if elapsed > self.cooldown_seconds:
                ...
                self.last_failure = None
                circuit_breaker_state.labels(name=self.name).set(0)
                logger.info("Circuit breaker closed after cooldown", name=self.name, elapsed=elapsed)
                return True
        remaining = max(0, self.cooldown_seconds - ((datetime.now(timezone.utc) - self.last_failure).total_seconds() if self.last_failure else 0))
...
class MerkleTree:
    ...
    def __init__(self, leaves: List[bytes]):
        ...
        self._root = self._tree_levels[-1][0] if self._tree_levels and self._tree_levels[-1] else self._hash(b'')
...
async def _persist_knowledge_inner(db: Any, circuit_breaker: CircuitBreaker, domain: str, key: str, value_with_metadata: Dict[str, Any], user_id: Optional[str], leaf_hash: str, merkle_proof: List[Tuple[str, str]], merkle_root: str):
    """Inner function that performs the actual persistence logic."""
    try:
        await db.save_agent_knowledge(domain, key, value_with_metadata, value_with_metadata["timestamp"])
        ...
        if await circuit_breaker.can_proceed():
            ...
            await audit_log("knowledge_learning", audit_event_data)  # Fixed: audit_log is a function, not an object
            await circuit_breaker.record_success()
        else:
            logger.error("Audit log for learning skipped due to open circuit breaker", name=circuit_breaker.name)
...
async def persist_knowledge(db: Any, circuit_breaker: CircuitBreaker, domain: str, key: str, value_with_metadata: Dict[str, Any], user_id: Optional[str], leaf_hash: str, merkle_proof: List[Tuple[str, str]], merkle_root: str):
    """Persist knowledge and create an audit event with retry logic."""
    return await _persist_knowledge_inner(db, circuit_breaker, domain, key, value_with_metadata, user_id, leaf_hash, merkle_proof, merkle_root)


async def _persist_knowledge_batch_inner(db: Any, circuit_breaker: CircuitBreaker, entries: List[Tuple[str, str, Dict[str, Any], str, str, List[Tuple[str, str]], str]], user_id: Optional[str]):
    """Inner function that performs the actual batch persistence logic."""
    try:
        db_entries = [(domain, key, value_with_metadata, timestamp) for domain, key, value_with_metadata, timestamp, _, _, _ in entries]
        ...
        if await circuit_breaker.can_proceed():
            ...
            await audit_log("knowledge_learning_batch", {"entries": audit_events, "user_id": user_id})  # Fixed: audit_log is a function, not an object
            await circuit_breaker.record_success()
        else:
            logger.error("Batch audit skipped due to open circuit breaker", name=circuit_breaker.name)
            learn_error_counter.labels(domain="batch", error_type='audit_circuit_open_batch_learn').inc()
    except Exception as e:
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=10))
async def persist_knowledge_batch(db: Any, circuit_breaker: CircuitBreaker, entries: List[Tuple[str, str, Dict[str, Any], str, str, List[Tuple[str, str]], str]], user_id: Optional[str]):
