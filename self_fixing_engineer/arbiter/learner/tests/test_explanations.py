...
def ensure_templates_loaded():
    ...
    from arbiter.learner.explanations import _load_prompt_templates, EXPLANATION_PROMPT_TEMPLATES
...
def setup_opentelemetry(mocker):
    ...
    mocker.patch("opentelemetry.exporter.otlp.proto.http.trace_exporter.OTLPSpanExporter", return_value=in_memory_exporter)
    yield
    in_memory_exporter.clear()
...
async def test_load_prompt_templates_file_not_found(mocker, tmp_path):
    ...
    mocker.patch.dict(os.environ, {"EXPLANATION_PROMPT_TEMPLATE_PATH": str(tmp_path / "nonexistent.json")})
...
async def test_generate_text_with_retry_success(mock_learner):
    ...
    mock_learner.llm_explanation_client.generate_text.side_effect = [Exception("Fail1"), "Success on retry"]
...
async def test_get_explanation_quality_report_all(mock_learner):
    ...
    mock_learner.explanation_feedback_log = [
        {"domain": "TestDomain1", "key": "key1", "version": 1, "score": 4, "timestamp": "2025-08-05T12:00:00Z"},
        {"domain": "TestDomain2", "key": "key2", "version": 2, "score": 5, "timestamp": "2025-08-05T12:01:00Z"}
    ]
...
async def test_get_explanation_quality_report_filtered(mock_learner):
    ...
    mock_learner.explanation_feedback_log = [
        {"domain": "TestDomain", "key": "key1", "version": 1, "score": 4, "timestamp": "2025-08-05T12:00:00Z"},
        {"domain": "OtherDomain", "key": "key2", "version": 2, "score": 5, "timestamp": "2025-08-05T12:01:00Z"}
    ]
