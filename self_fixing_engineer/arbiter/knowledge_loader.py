...
def save_knowledge_atomic(
    filename: Union[str, os.PathLike], knowledge_data: Dict[str, Any]
) -> None:
    """
...
class KnowledgeLoader:
    ...
    def load_all(self) -> None:
        ...
        with self._lock:
            ...
            if os.path.exists(self.knowledge_data_path) and os.path.isdir(self.knowledge_data_path):
                ...
                logger.warning(
                    f"  Knowledge data path '{self.knowledge_data_path}' not found or is not a directory. Using internal canonical knowledge."
                )
    ...
    def inject_to_arbiter(self, arbiter_instance: Any) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Injecting knowledge into {getattr(arbiter_instance, 'name', 'unnamed-arbiter')}..."
            )
            ...
            if not isinstance(mem, dict):
                logger.error(
                    "Arbiter state['memory'] is not a dictionary. Cannot inject knowledge."
                )
                return
