...
from .utils import (  # or appropriate relative/absolute path
    ...
    redact_pii, validate_input_details
)
...
# Prometheus Metrics
NOTIFICATION_SEND = get_or_create_metric(Counter, 'notification_send', 'Total number of notification send attempts', ['channel'])
NOTIFICATION_SEND_SUCCESS = get_or_create_metric(Counter, 'notification_send_success', 'Total number of successful notifications sent', ['channel'])
NOTIFICATION_SEND_FAILED = get_or_create_metric(Counter, 'notification_send_failed', 'Total number of failed notifications sent', ['channel', 'reason'])
NOTIFICATION_CIRCUIT_BREAKER_OPEN = get_or_create_metric(Counter, 'notification_circuit_breaker_open', 'Total number of times a notification circuit breaker was open', ['channel'])
NOTIFICATION_RATE_LIMITED = get_or_create_metric(Counter, 'notification_rate_limited', 'Total number of times a notification was rate-limited', ['channel'])
NOTIFICATION_CURRENT_FAILURES_GAUGE = get_or_create_metric(Gauge, 'notification_current_failures', 'Current consecutive failures for a channel', ['channel'])
NOTIFICATION_SEND_DURATION_SECONDS = get_or_create_metric(Histogram, 'notification_send_duration_seconds', 'Send duration', ['channel'])
...
class CircuitBreaker:
    ...
    """
    def __init__(self, failure_threshold: int, recovery_timeout: int, half_open_attempts: int = 1, redis_url: Optional[str] = None, escalation_handler: Optional[Callable] = None):
        ...
        self._state: Dict[str, tuple] = {}  # In-memory state: {channel: (state, failures, last_failure, attempts)}
    ...
    async def initialize(self):
        ...
        if self.redis_url and redis:
            try:
                ...
                logger.error(f"Failed to connect to Redis for CircuitBreaker: {e}. Falling back to in-memory state.")
                self.redis = None
    ...
    async def _set_state(self, channel: str, state: str, failures: int, last_failure: float, attempts: int):
        """Sets the state of the circuit breaker for a channel."""
        if self.redis:
            await self.redis.hset(f"circuit_breaker:{channel}", mapping={
                "state": state,
                "failures": failures,
                ...
                async with self._local_lock:
                    ...
                    if state == "OPEN":
                        if now - last_failure > self.recovery_timeout:
                            ...
                            logger.info(json.dumps({"event": "circuit_breaker_state_change", "channel": channel, "old_state": "OPEN", "new_state": "HALF_OPEN"}))
                            ...
                            logger.warning(json.dumps({"event": "circuit_breaker_open", "channel": channel}))
                    ...
                    if state == "HALF_OPEN":
                        if attempts >= self.half_open_attempts:
                            ...
                            logger.warning(json.dumps({"event": "circuit_breaker_state_change", "channel": channel, "old_state": "HALF_OPEN", "new_state": "OPEN", "reason": "failed_half_open_attempts"}))
                            await self._set_state(channel, state, failures, last_failure, attempts)
                            NOTIFICATION_CIRCUIT_BREAKER_OPEN.labels(channel=channel).inc()
                            raise CircuitBreakerOpenError(f"Circuit breaker for {channel} is OPEN.")
                        else:
                            attempts += 1
                            logger.info(json.dumps({"event": "circuit_breaker_half_open_attempt", "channel": channel, "attempt": attempts, "max_attempts": self.half_open_attempts}))
                            await self._set_state(channel, state, failures, last_failure, attempts)
                ...
                try:
                    ...
                    async with self._local_lock:
                        ...
                        if current_state != "CLOSED":
                            logger.info(json.dumps({"event": "circuit_breaker_state_change", "channel": channel, "old_state": current_state, "new_state": "CLOSED", "reason": "successful_call"}))
                    ...
                    async with self._local_lock:
                        ...
                        if state == "HALF_OPEN" or failures >= self.failure_threshold:
                            state = "OPEN"
                            logger.error(json.dumps({"event": "circuit_breaker_state_change", "channel": channel, "old_state": state, "new_state": "OPEN", "reason": "failure_threshold_exceeded", "failures": failures}))
                            if self.escalation_handler:
                                try:
                                    await self.escalation_handler(channel, failures)
                                except Exception as handler_e:
                                    logger.error(json.dumps({"event": "escalation_handler_failed", "channel": channel, "error": str(handler_e)}))
                        await self._set_state(channel, state, failures, last_failure, attempts)
                    raise e
            return wrapper
        return decorator
    ...
    async def initialize(self):
        ...
        if self.redis_url and redis:
            try:
                ...
                logger.error(f"Failed to connect to Redis for RateLimiter: {e}. Falling back to in-memory state.")
                self.redis = None
        ...
        def decorator(func: Callable[..., Coroutine]) -> Callable[..., Coroutine]:
            ...
            async def wrapper(*args, **kwargs):
                ...
                if self.redis:
                    ...
                    count = results[1]
                    if count >= max_calls:
                        logger.warning(json.dumps({"event": "rate_limit_exceeded", "channel": channel, "strategy": "redis", "count": count, "limit": max_calls}))
                    ...
                    async with self._local_lock:
                        ...
                        if len(self._call_timestamps) > self._max_size:
                            ...
                            logger.warning(json.dumps({"event": "rate_limiter_cache_evicted", "oldest_key": oldest_key}))
                            
                        if len(timestamps) >= max_calls:
                            logger.warning(json.dumps({"event": "rate_limit_exceeded", "channel": channel, "strategy": "in_memory", "count": len(timestamps), "limit": max_calls}))
                            NOTIFICATION_RATE_LIMITED.labels(channel=channel).inc()
                            raise RateLimitExceededError(f"Rate limit exceeded for channel {channel}.")
...
def default_escalation_handler(channel: str, failures: int) -> None:
    ...
    logger.critical(f"Escalating: Circuit breaker for channel '{channel}' is OPEN after {failures} failures.")
...
class NotificationService:
    ...
    """
    _critical_notification_handler: Optional[Callable[[str, int, str], Coroutine[Any, Any, None]]] = None

    @classmethod
    def register_critical_notification_handler(cls, handler: Callable[[str, int, str], Coroutine[Any, Any, None]]):
        """
        Registers a handler for critical notifications, such as when an escalation is needed.
        """
        if cls._critical_notification_handler is not None and cls._critical_notification_handler != handler:
    ...
    def __init__(self, settings: Any):
        ...
        self._notification_failures: Dict[str, collections.deque] = collections.defaultdict(collections.deque)
        ...
        self.circuit_breaker = CircuitBreaker(
            ...
            escalation_handler=getattr(settings, 'NOTIFICATION_CIRCUIT_BREAKER_ESCALATION_HANDLER', default_escalation_handler)
        )
    ...
    def _validate_settings(self):
        ...
        if "email" in enabled_channels:
            ...
            if not all([self.settings.EMAIL_SMTP_SERVER, self.settings.EMAIL_SENDER, self.settings.EMAIL_SMTP_USERNAME, self.settings.EMAIL_SMTP_PASSWORD]):
                raise ValueError("Email SMTP settings incomplete but channel is enabled.")
        if "pagerduty" in enabled_channels and not getattr(self.settings, 'PAGERDUTY_ROUTING_KEY', None):
    ...
    async def _check_and_escalate(self, channel: str, message: str) -> None:
        ...
        async with self._escalation_lock:
            ...
            while timestamps and timestamps[0] <= now - self.settings.NOTIFICATION_FAILURE_WINDOW_SECONDS:
                timestamps.popleft()
            ...
            if consecutive_failures >= self.settings.NOTIFICATION_FAILURE_THRESHOLD:
                if now - self._last_escalation_time[channel] > self.settings.NOTIFICATION_FAILURE_WINDOW_SECONDS * 2:
                    if NotificationService._critical_notification_handler:
                        logger.critical(json.dumps({"event": "notification_escalation_triggered", "channel": channel, "failures": consecutive_failures, "window_seconds": self.settings.NOTIFICATION_FAILURE_WINDOW_SECONDS}))
                        try:
                            await NotificationService._critical_notification_handler(channel, consecutive_failures, message)
                            self._last_escalation_time[channel] = now
                        except Exception as e:
                            logger.error(f"Critical notification handler failed: {e}", exc_info=True)
                    else:
                        logger.critical(json.dumps({"event": "notification_escalation_suppressed", "channel": channel, "reason": "no_handler_registered"}))
                else:
                    logger.warning(json.dumps({"event": "notification_escalation_suppressed", "channel": channel, "reason": "recent_escalation"}))

    async def _record_notification_failure(self, channel: str, message: str, error_code: str) -> None:
    ...
    def _simulate_failure(self, channel: str) -> None:
        ...
        if random.random() < failure_rate:
            logger.warning(json.dumps({"event": "simulated_failure", "channel": channel, "rate": failure_rate}))
    ...
    def _notify_slack_with_decorators(self):
        ...
        async def _notify_slack(self, message: str, timeout: float) -> bool:
            ...
            if not self.settings.SLACK_WEBHOOK_URL:
                ...
                await self._record_notification_failure("slack", "Slack webhook URL not configured.", "CONFIG_MISSING")
                raise NotificationError("Slack webhook URL not configured.", "slack", "CONFIG_MISSING")
            ...
            if hasattr(self.settings, 'SLACK_AUTH_TOKEN'):
                headers['Authorization'] = f'Bearer {self.settings.SLACK_AUTH_TOKEN.get_secret_value()}'
            
            with NOTIFICATION_SEND_DURATION_SECONDS.labels(channel='slack').time():
                try:
                    async with self._session.post(self.settings.SLACK_WEBHOOK_URL, json=payload, timeout=timeout, headers=headers, ssl=True) as response:
                    ...
                    logger.error(json.dumps({"event": "send_failure", "channel": "slack", "error_code": error_code, "error_message": error_message}))
                    ...
                    await self._record_notification_failure("slack", f"Unexpected Slack error: {e}", "UNEXPECTED_ERROR")
                    raise NotificationError(f"Unexpected error sending Slack notification: {e}", "slack", "UNEXPECTED_ERROR") from e
    ...
    def _notify_email_with_decorators(self):
        ...
        @tenacity.retry(
            ...
            retry=(tenacity.retry_if_exception_type(aiohttp.ClientError) | tenacity.retry_if_exception_type(asyncio.TimeoutError) | tenacity.retry_if_exception_type(aiosmtplib.SMTPException)),
            before_sleep=tenacity.before_sleep_log(logger, logging.WARNING),
            reraise=True
        )
        async def _notify_email(self, subject: str, body: str, recipients: List[str], timeout: float) -> bool:
            """Sends an email notification."""
            NOTIFICATION_SEND.labels(channel='email').inc()
            self._simulate_failure('email')
            
            # Pre-flight checks
            if not self.settings.EMAIL_ENABLED or "email" not in self.settings.ENABLED_NOTIFICATION_CHANNELS:
                logger.debug("Email notifications disabled.")
                return False
            if not all([self.settings.EMAIL_SMTP_SERVER, self.settings.EMAIL_SENDER, self.settings.EMAIL_SMTP_USERNAME, self.settings.EMAIL_SMTP_PASSWORD]):
                await self._record_notification_failure("email", "Email SMTP settings incomplete.", "CONFIG_MISSING")
                raise NotificationError("Email SMTP settings incomplete.", "email", "CONFIG_MISSING")
            ...
            with NOTIFICATION_SEND_DURATION_SECONDS.labels(channel='email').time():
                try:
                    ...
                    smtp = aiosmtplib.SMTP(hostname=self.settings.EMAIL_SMTP_SERVER, port=self.settings.EMAIL_SMTP_PORT, use_tls=False, timeout=timeout)
                    await smtp.connect()
                    ...
                    await smtp.login(self.settings.EMAIL_SMTP_USERNAME, self.settings.EMAIL_SMTP_PASSWORD.get_secret_value())
                    await smtp.send_message(msg)
                    await smtp.quit()
                    
                    redacted_recipients = redact_pii({"recipients": ", ".join(recipients)})["recipients"]
                    logger.info(json.dumps({"event": "send_success", "channel": "email", "recipients": redacted_recipients}))
                    await self._record_notification_success("email")
                    return True
                except aiosmtplib.SMTPException as e:
                    await self._record_notification_failure("email", f"SMTP error: {e}", "SMTP_ERROR")
                    raise NotificationError(f"Failed to send email via SMTP: {e}", "email", "SMTP_ERROR") from e
                except Exception as e:
                    error_code = "TIMEOUT" if isinstance(e, asyncio.TimeoutError) else "UNEXPECTED_ERROR"
                    await self._record_notification_failure("email", f"Unexpected email error: {e}", error_code)
                    raise NotificationError(f"Unexpected error sending email: {e}", "email", error_code) from e
    ...
    def _notify_pagerduty_with_decorators(self):
        ...
        async def _notify_pagerduty(self, event_type: str, description: str, timeout: float, details: Optional[Dict[str, Any]] = None) -> bool:
            """Triggers an event in PagerDuty."""
            if not self.settings.PAGERDUTY_ROUTING_KEY or not self.settings.PAGERDUTY_ROUTING_KEY.get_secret_value():
                await self._record_notification_failure("pagerduty", "PagerDuty routing key not configured.", "CONFIG_MISSING")
                raise NotificationError("PagerDuty routing key not configured.", "pagerduty", "CONFIG_MISSING")
            ...
            payload = {
                ...
                "payload": {
                    ...
                    "custom_details": validate_input_details(redacted_details) if redacted_details else {}
                }
            }
            
            headers = {}
            if getattr(self.settings, 'PAGERDUTY_API_KEY', None):
                headers['Authorization'] = f'Token token={self.settings.PAGERDUTY_API_KEY.get_secret_value()}'
                
            with NOTIFICATION_SEND_DURATION_SECONDS.labels(channel='pagerduty').time():
                try:
                    async with self._session.post("https://events.pagerduty.com/v2/enqueue", json=payload, timeout=timeout, headers=headers, ssl=True) as response:
                        response.raise_for_status()
                    ...
                    logger.error(json.dumps({"event": "send_failure", "channel": "pagerduty", "error_code": error_code, "error_message": error_message}))
                    ...
                    await self._record_notification_failure("pagerduty", f"Unexpected PagerDuty error: {e}", "UNEXPECTED_ERROR")
                    raise NotificationError(f"Unexpected error triggering PagerDuty event: {e}", "pagerduty", "UNEXPECTED_ERROR") from e
    ...
    async def notify_batch(self, notifications: List[Dict[str, Any]]) -> List[Any]:
        ...
        for notif in notifications:
            ...
            if channel == "slack":
                tasks.append(self._notify_slack_with_decorators(notif.get("message", ""), self.settings.SLACK_API_TIMEOUT_SECONDS))
            elif channel == "email":
                tasks.append(self._notify_email_with_decorators(
                    notif.get("subject", ""), notif.get("body", ""), 
                    notif.get("recipients", self.settings.EMAIL_RECIPIENTS), 
                    self.settings.EMAIL_API_TIMEOUT_SECONDS
                ))
            elif channel == "pagerduty":
                tasks.append(self._notify_pagerduty_with_decorators(
                    notif.get("event_type", "trigger"), notif.get("description", ""), 
                    self.settings.PAGERDUTY_API_TIMEOUT_SECONDS, notif.get("details")
                ))
            else:
                ...
                async def failed_task():
                    raise NotificationError(f"Unknown channel: {channel}", str(channel), "INVALID_CHANNEL")
