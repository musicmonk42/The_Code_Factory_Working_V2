...
# Prometheus Metrics
AUDIT_LOG_FLUSH = get_or_create_metric(Counter, 'audit_log_flush', 'Total number of times the audit log buffer was flushed')
AUDIT_LOG_WRITE_SUCCESS = get_or_create_metric(Counter, 'audit_log_write_success', 'Total number of successful audit log writes to local file')
AUDIT_LOG_WRITE_FAILED = get_or_create_metric(Counter, 'audit_log_write_failed', 'Total number of failed audit log writes to local file')
AUDIT_LOG_DEAD_LETTER = get_or_create_metric(Counter, 'audit_log_dead_letter', 'Total number of audit entries sent to dead-letter queue', ['reason'])
AUDIT_LOG_ROTATION = get_or_create_metric(Counter, 'audit_log_rotation', 'Total number of audit log file rotations')
AUDIT_LOG_BUFFER_SIZE_GAUGE = get_or_create_metric(Gauge, 'audit_log_buffer_size', 'Current size of the audit log buffer')
AUDIT_LOG_REMOTE_SEND_SUCCESS = get_or_create_metric(Counter, 'audit_log_remote_send_success', 'Total successful remote audit log sends')
AUDIT_LOG_REMOTE_SEND_FAILED = get_or_create_metric(Counter, 'audit_log_remote_send_failed', 'Total failed remote audit log sends')
AUDIT_LOG_DISK_CHECK_FAILED = get_or_create_metric(Counter, 'audit_log_disk_check_failed', 'Total number of disk space check failures')
AUDIT_LOG_FLUSH_DURATION_SECONDS = get_or_create_metric(Histogram, 'audit_log_flush_duration_seconds', 'Duration of audit log buffer flush operations.')
AUDIT_LOG_DROPPED = get_or_create_metric(Counter, 'audit_log_dropped', 'Total dropped audit entries due to buffer overflow')
...
class AuditLogManager:
    ...
    def __init__(
        ...
        self.temp_cleanup_timeout = getattr(self.settings, 'AUDIT_LOG_TEMP_CLEANUP_TIMEOUT_SECONDS', 10)
        ...
        self.log_path = log_path if log_path is not None else getattr(settings, 'AUDIT_LOG_FILE_PATH', 'sfe_bug_manager_audit.log')
        self.dead_letter_log_path = dead_letter_log_path if dead_letter_log_path is not None else getattr(settings, 'AUDIT_DEAD_LETTER_FILE_PATH', 'sfe_bug_manager_dead_letter.log')
        self.enabled = enabled if enabled is not None else getattr(settings, 'AUDIT_LOG_ENABLED', True)
        ...
        if not os.access(dead_letter_dir, os.W_OK):
            raise ValueError(f"Dead letter log path parent directory '{dead_letter_dir}' is not writable.")
        ...
        if self.encryption_key and not isinstance(self.encryption_key, (bytes, str)):
            logger.warning("AUDIT_LOG_ENCRYPTION_KEY is not a valid type (bytes/str). Encryption will be disabled.")
    ...
    async def _write_to_dead_letter_queue(self, entry: Dict[str, Any], reason: str) -> None:
        ...
        try:
            ...
            await asyncio.get_running_loop().run_in_executor(
                self._io_executor,
                self._sync_write_to_dead_letter, json.dumps(dead_letter_entry) + "\n"
            )
    ...
    def _sync_write_to_dead_letter(self, data: str) -> None:
        try:
            ...
            logger.critical(f"Could not write to dead-letter file '{self.dead_letter_log_path}': {e}", exc_info=True)
    ...
    async def _periodic_flush(self) -> None:
        while True:
            try:
                ...
                logger.critical(f"Unhandled exception in periodic audit log flush: {e}", exc_info=True)
    ...
    async def _flush_buffer(self, final_flush: bool = False) -> None:
        ...
        with AUDIT_LOG_FLUSH_DURATION_SECONDS.time():
            ...
            try:
                ...
                await asyncio.get_running_loop().run_in_executor(
                    self._io_executor,
                    self._sync_atomic_write_with_retry, processed_logs
                )
                ...
                logger.warning("Could not acquire file lock for audit log. Re-buffering entries for next flush.")
                async with self._buffer_lock:
                ...
                for entry in processed_logs:
                    await self._write_to_dead_letter_queue(entry, f"Local audit file write failed: {e}")
                AUDIT_LOG_WRITE_FAILED.inc()
                raise AuditLogError(f"Local audit log write failed: {e}")

            if getattr(self.settings, 'REMOTE_AUDIT_SERVICE_ENABLED', False) and local_write_successful:
        ...
        for i in range(backup_count - 1, -1, -1):
            ...
            if os.path.exists(src):
                try:
                    ...
                    logger.error(f"Failed to rotate/compress '{src}' to '{dst}': {e}", exc_info=True)
    ...
    def _sync_atomic_write_with_retry(self, entries: List[Dict[str, Any]]) -> None:
        temp_filepath = Path(self.log_path).parent / f"{Path(self.log_path).name}.tmp_{os.getpid()}_{time.monotonic()}"
        ...
        try:
            ...
            if os.path.exists(temp_filepath):
                try:
                    ...
                    logger.error(f"Failed to remove temporary audit file {temp_filepath}: {e_remove}")
    ...
    async def _send_to_remote_audit_service(self, entries: List[Dict[str, Any]]) -> None:
        ...
        if not hasattr(self, '_session') or self._session is None or self._session.closed:
            logger.error("Cannot send to remote audit service: aiohttp ClientSession is not available.")
        ...
        try:
            async with self._session.post(remote_url, json=entries, timeout=timeout) as response:
                if 200 <= response.status < 300:
                    logger.info(f"Sent {len(entries)} audit entries to remote service successfully.")
                    AUDIT_LOG_REMOTE_SEND_SUCCESS.inc()
                else:
                    response_text = await response.text()
                    logger.error(f"Failed to send audit entries. Status: {response.status}, Response: {response_text}")
                    await self._handle_remote_send_failure(entries, f"Remote send failed: HTTP {response.status}")
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            error_reason = "Timeout" if isinstance(e, asyncio.TimeoutError) else f"ClientError - {str(e)}"
            ...
            await self._handle_remote_send_failure(entries, f"Remote send failed: Unexpected error - {e}")
    ...
    async def audit(self, event_type: str, details: Dict[str, Any]) -> None:
        ...
        if not isinstance(event_type, str) or not event_type:
            logger.error(f"Invalid event_type: must be a non-empty string, got {type(event_type).__name__}")
