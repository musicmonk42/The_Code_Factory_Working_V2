...
class TestMLRemediationModel:
    ...
    @pytest.mark.asyncio
    async def test_predict_api_error_raises_custom_exception(
...
class TestRemediationStep:
    ...
    async def test_precondition_skip(self, bug_details):
        ...
        step = RemediationStep(
            name="PreconditionStep",
            action_name="precondition_action",
            pre_condition=mock_precondition,
        )
...
class TestRemediationPlaybook:
    ...
    async def test_successful_run(self, bug_details):
        ...
        step1 = RemediationStep(
            name="Step1", action_name="success_playbook_action1", on_success="Step2"
        )
        step2 = RemediationStep(
            name="Step2", action_name="success_playbook_action2", on_success="FINISH"
        )
    ...
    async def test_run_with_step_failure(self, bug_details):
        ...
        step2 = RemediationStep(
            name="SuccessfulStep", action_name="failure_playbook_action2"
        )  # Should not be called
        step_notify = RemediationStep(
            name="Notify",
...
class TestBugFixerRegistry:
    ...
    async def test_rule_based_selection_priority(self, bug_details):
        ...
        pb_specific = RemediationPlaybook(
        ...
        pb_global = RemediationPlaybook(
            "Global", [RemediationStep("s3", action_name="global_action")]
        )

        BugFixerRegistry.register_playbook(
            pb_specific, location="database.connector", bug_signature_prefix="db_conn"
        )
        BugFixerRegistry.register_playbook(
        ...
        with patch.object(
            pb_specific, "execute", new=AsyncMock(return_value=True)
        ) as mock_exec_spec, patch.object(
            pb_general_loc, "execute", new=AsyncMock(return_value=True)
        ) as mock_exec_gen, patch.object(
            pb_global, "execute", new=AsyncMock(return_value=True)
        ) as mock_exec_glob:

            # Test specific match
            await BugFixerRegistry.run_remediation(
                "database.connector", bug_details, "db_conn_timeout_sig"
            )
            ...
            await BugFixerRegistry.run_remediation(
                "database.connector", bug_details, "some_other_sig"
            )
    ...
    async def test_ml_selection_override(self, bug_details, ml_model):
        ...
        rule_playbook = RemediationPlaybook(
        ...
        BugFixerRegistry.register_playbook(
            rule_playbook, location="database.connector", bug_signature_prefix="*"
        )
        BugFixerRegistry.register_playbook(
            ml_playbook, location="global", bug_signature_prefix="*"
        )  # Register the ML playbook
        BugFixerRegistry.set_ml_model(ml_model)

        with patch.object(
            ml_playbook, "execute", new=AsyncMock(return_value=True)
        ) as mock_exec_ml, patch.object(
            rule_playbook, "execute", new=AsyncMock(return_value=True)
        ) as mock_exec_rule:
