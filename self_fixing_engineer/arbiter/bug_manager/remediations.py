...
from .utils import (  # or from arbiter.bug_manager.utils import (
    RemediationError, MLRemediationError, validate_input_details, redact_pii
)
...
REMEDIATION_PLAYBOOK_EXECUTION = get_or_create_metric(
    Counter, 'remediation_playbook_execution',
    'Total number of remediation playbook executions', ['playbook_name', 'source']
)
REMEDIATION_STEP_EXECUTION = get_or_create_metric(
    Counter, 'remediation_step_execution',
    'Total number of remediation step executions', ['playbook_name', 'step_name', 'outcome']
)
REMEDIATION_STEP_DURATION_SECONDS = get_or_create_metric(
    Histogram, 'remediation_step_duration_seconds',
    'Step execution duration', ['playbook_name', 'step_name']
)
REMEDIATION_SUCCESS = get_or_create_metric(
    Counter, 'remediation_success',
    'Total number of successful remediations', ['playbook_name']
)
REMEDIATION_FAILURE = get_or_create_metric(
    Counter, 'remediation_failure',
    'Total number of failed remediations', ['playbook_name']
)
ML_REMEDIATION_PREDICTION = get_or_create_metric(
    Counter, 'ml_remediation_prediction',
    'Total number of ML remediation prediction requests'
)
ML_REMEDIATION_PREDICTION_SUCCESS = get_or_create_metric(
    Counter, 'ml_remediation_prediction_success',
    'Total number of successful ML remediation predictions'
)
ML_REMEDIATION_PREDICTION_FAILED = get_or_create_metric(
    Counter, 'ml_remediation_prediction_failed',
    'Total number of failed ML remediation predictions', ['reason']
)
ML_REMEDIATION_FEEDBACK = get_or_create_metric(
    Counter, 'ml_remediation_feedback',
    'Total number of ML remediation feedback submissions'
)
ML_REMEDIATION_FEEDBACK_FAILED = get_or_create_metric(
    Counter, 'ml_remediation_feedback_failed',
    'Total number of failed ML remediation feedback submissions', ['reason']
)
...
class MLRemediationModel:
    ...
    @tenacity.retry(
        ...
        wait=tenacity.wait.wait_exponential(multiplier=lambda retry_state: retry_state.args[0].retry_delay_seconds, min=1, max=10),
        retry=tenacity.retry_if_exception_type((aiohttp.ClientError, asyncio.TimeoutError)),
        before_sleep=tenacity.before_sleep_log(logger, logging.WARNING),
        reraise=True
    )
    async def predict_remediation_strategy(self, bug_details: Dict[str, Any]) -> Tuple[Optional[str], float]:
        """
        ...
        logger.info(json.dumps({"event": "ml_prediction_request", "message_preview": model_input.get("message", "")[:50]}))
        
        headers = {"Authorization": f"Bearer {self.auth_token.get_secret_value()}"} if self.auth_token else {}

        try:
            session = await self._get_session()
            async with session.post(self.model_endpoint, json=model_input, timeout=aiohttp.ClientTimeout(total=self.request_timeout), headers=headers, ssl=True) as response:
            ...
            logger.info(json.dumps({"event": "ml_prediction_success", "predicted_playbook": predicted_playbook, "confidence": confidence}))
            ML_REMEDIATION_PREDICTION_SUCCESS.inc()
            return predicted_playbook, confidence
        except aiohttp.ClientError as e:
            logger.error(json.dumps({"event": "ml_prediction_failed", "reason": "api_error", "error_message": str(e)}))
            ...
            logger.error(json.dumps({"event": "ml_prediction_failed", "reason": "timeout", "timeout_seconds": self.request_timeout}))
            ML_REMEDIATION_PREDICTION_FAILED.labels(reason='TIMEOUT').inc()
            raise MLRemediationError("ML prediction timed out.")
        except Exception as e:
            logger.error(json.dumps({"event": "ml_prediction_failed", "reason": "unexpected_error", "error_message": str(e)}), exc_info=True)
    ...
    async def record_remediation_outcome(self, bug_details: Dict[str, Any], playbook_name: str, outcome: str) -> None:
        """
        ...
        logger.info(json.dumps({"event": "ml_feedback_submission", "playbook": playbook_name, "outcome": outcome}))

        try:
            session = await self._get_session()
            feedback_endpoint = f"{self.model_endpoint.rstrip('/')}/feedback"
            headers = {"Authorization": f"Bearer {self.auth_token.get_secret_value()}"} if self.auth_token else {}
            async with session.post(feedback_endpoint, json=feedback_data, timeout=aiohttp.ClientTimeout(total=2.0), headers=headers, ssl=True) as response:
                response.raise_for_status()
            ...
            logger.warning(json.dumps({"event": "ml_feedback_failed", "reason": reason, "error_message": str(e)}))
            ML_REMEDIATION_FEEDBACK_FAILED.labels(reason=reason).inc()
            raise
        except Exception as e:
            logger.warning(json.dumps({"event": "ml_feedback_failed", "reason": "unexpected_error", "error_message": str(e)}), exc_info=True)
...
class RemediationStep:
    ...
    def __init__(self, name: str, action_name: str,
                 ...
                 on_success: Optional[str] = "FINISH", on_failure: Optional[str] = "ABORT",
                 description: Optional[str] = None, idempotent: bool = True,
                 retries: int = 0, retry_delay_seconds: float = 1.0, timeout_seconds: float = 30.0):
        self.name = name
    ...
    async def execute(self, bug_details: Dict[str, Any], playbook_name: str) -> bool:
        ...
        """
        logger.info(json.dumps({"event": "step_execution_start", "playbook": playbook_name, "step": self.name, "description": self.description}))
        
        if not self.idempotent and self.retries > 0:
            logger.warning(json.dumps({"event": "non_idempotent_step_with_retries", "playbook": playbook_name, "step": self.name}))

        if self.pre_condition:
            try:
                if not await self.pre_condition(bug_details):
                    logger.info(json.dumps({"event": "step_skipped", "reason": "pre_condition_not_met", "playbook": playbook_name, "step": self.name}))
                    REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='skipped_precondition').inc()
                    return False
            except Exception as e:
                logger.error(json.dumps({"event": "pre_condition_failed", "playbook": playbook_name, "step": self.name, "error": str(e)}), exc_info=True)
                REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='failed_precondition').inc()
                raise RemediationError(f"Error in pre-condition for step '{self.name}'", self.name, original_exception=e)

        with REMEDIATION_STEP_DURATION_SECONDS.labels(playbook_name=playbook_name, step_name=self.name).time():
            try:
                ...
                result = await asyncio.wait_for(
                    action_with_retries(bug_details),
                    timeout=self.timeout_seconds
                )
                
                if result:
                    logger.info(json.dumps({"event": "step_succeeded", "playbook": playbook_name, "step": self.name}))
                    REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='success').inc()
                else:
                    logger.warning(json.dumps({"event": "step_failed", "playbook": playbook_name, "step": self.name, "reason": "action_returned_false"}))
                    REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='failure').inc()
                return result

            except asyncio.TimeoutError:
                logger.error(json.dumps({"event": "step_timeout", "playbook": playbook_name, "step": self.name, "timeout_seconds": self.timeout_seconds}))
                REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='timeout').inc()
                raise RemediationError(f"Step '{self.name}' timed out", self.name, playbook_name=playbook_name)
            except Exception as e:
                logger.error(json.dumps({"event": "step_error", "playbook": playbook_name, "step": self.name, "error": str(e)}), exc_info=True)
                REMEDIATION_STEP_EXECUTION.labels(playbook_name=playbook_name, step_name=self.name, outcome='error').inc()
                raise RemediationError(f"Error in step '{self.name}'", self.name, playbook_name=playbook_name, original_exception=e)
...
class RemediationPlaybook:
    ...
    async def execute(self, location: str, bug_details: Dict[str, Any]) -> bool:
        ...
        """
        logger.info(json.dumps({"event": "playbook_execution_start", "playbook": self.name, "location": location}))
        ...
        try:
            while current_step_name:
                ...
                if not step:
                    logger.error(json.dumps({"event": "playbook_invalid_step", "playbook": self.name, "step": current_step_name}))
                    is_fixed = False
                    break
                ...
                try:
                    ...
                    logger.error(json.dumps({"event": "playbook_step_exception", "playbook": self.name, "step": e.step_name, "error": str(e)}))
                    current_step_name = step.on_failure
                
                if current_step_name == "FINISH":
                    logger.info(json.dumps({"event": "playbook_finished_explicitly", "playbook": self.name}))
                    break
        ...
        logger.info(json.dumps({"event": "playbook_execution_end", "playbook": self.name, "outcome": "success" if is_fixed else "failure"}))
        return is_fixed
...
class BugFixerRegistry:
    ...
    @classmethod
    def register_playbook(cls, playbook: RemediationPlaybook, location: str, bug_signature_prefix: str = "*"):
        """
        ...
        if bug_signature_prefix in cls._playbooks[location]:
            logger.warning(f"Overwriting existing playbook for location '{location}' with signature prefix '{bug_signature_prefix}'.")
        cls._playbooks[location][bug_signature_prefix] = playbook
        logger.info(f"Remediation playbook '{playbook.name}' registered for '{location}' with signature '{bug_signature_prefix}'.")

    @classmethod
    async def run_remediation(cls, location: str, bug_details: Dict[str, Any], bug_signature: str) -> bool:
        """
        ...
        ml_confidence_threshold = getattr(cls._settings, 'ML_CONFIDENCE_THRESHOLD', 0.75) if cls._settings else 0.75
        if cls._ml_remediation_model:
            try:
                predicted_name, confidence = await cls._ml_remediation_model.predict_remediation_strategy(validated_details)
                    ...
                    for loc, sig_map in cls._playbooks.items():
                        for sig, pb in sig_map.items():
                            if pb.name == predicted_name:
                                ...
                                break
                        if chosen_playbook: break
                    if chosen_playbook:
                        logger.info(f"ML model selected playbook '{chosen_playbook.name}'.")
                    else:
                        logger.warning(f"ML model recommended playbook '{predicted_name}' but it was not found in registry.")
        ...
        if chosen_playbook:
            logger.info(json.dumps({"event": "playbook_selected", "playbook": chosen_playbook.name, "source": playbook_source}))
            REMEDIATION_PLAYBOOK_EXECUTION.labels(playbook_name=chosen_playbook.name, source=playbook_source).inc()
...
BugFixerRegistry.register_playbook(restart_service_playbook, location="*", bug_signature_prefix="500_error")