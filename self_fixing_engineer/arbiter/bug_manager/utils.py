...
PII_REDACTION_COUNT = get_or_create_metric(Counter, 'pii_redaction_count', 'Total PII redactions', ['type'])
SETTINGS_VALIDATION_ERRORS = get_or_create_metric(Counter, 'settings_validation_errors', 'Total settings validation errors', ['setting_name'])
...
try:
    ...
    logger.warning("Pydantic not available; using dummy SecretStr. Install Pydantic for production security.")
    class SecretStrBase:
        """
...
def redact_pii(details: Dict[str, Any], settings: Any = None) -> Dict[str, Any]:
    ...
    """
    sensitive_keywords = getattr(settings, 'PII_SENSITIVE_KEYWORDS', {
        "token", "key", "password", "secret", "api_key", "webhook_url", "routing_key",
        "address", "phone", "email", "ssn", "credit_card", "account_number",
        "dob", "username", "ip_address", "geolocation", "auth_header", "bearer"
    })
        ...
        for key, (pattern, replacement) in patterns.items():
            if pattern.search(value):
                ...
                if mask_level == 'partial':
                    value = pattern.sub(lambda m: m.group(0)[:4] + "[...REDACTED...]" + m.group(0)[-4:], value)
                else:
        ...
        if origin_type is Union and type(None) in type_args:
            ...
            if value is None or isinstance(value, actual_types):
                is_valid = True
                 ...
                 errors.append(f"Setting '{field}' has incorrect type. Expected one of {actual_types} or None, got {type(value)}.")
        # Handle regular types (int, str, bool, list, etc.)
        elif origin_type is None:
             if isinstance(value, expected_type):
                 is_valid = True
             else:
                errors.append(f"Setting '{field}' has incorrect type. Expected {expected_type}, got {type(value)}.")
        # Handle other generics like list, tuple, etc.
        else:
            if isinstance(value, origin_type):
                is_valid = True
            else:
                errors.append(f"Setting '{field}' has incorrect container type. Expected {origin_type}, got {type(value)}.")
...
def apply_settings_validation(settings_obj: Any) -> None:
    ...
    required_fields = {
        "DEBUG_MODE": bool, "SLACK_WEBHOOK_URL": Optional[str], "EMAIL_RECIPIENTS": List[str],
        "EMAIL_ENABLED": bool, "EMAIL_SENDER": str, "EMAIL_SMTP_SERVER": Optional[str],
        "EMAIL_SMTP_PORT": int, "EMAIL_USE_STARTTLS": bool, "EMAIL_SMTP_USERNAME": Optional[str],
        "EMAIL_SMTP_PASSWORD": Optional[SecretStr], "PAGERDUTY_ENABLED": bool,
        "PAGERDUTY_ROUTING_KEY": Optional[SecretStr], "ENABLED_NOTIFICATION_CHANNELS": tuple,
        "AUDIT_LOG_FILE_PATH": str, "AUDIT_DEAD_LETTER_FILE_PATH": str,
        "AUTO_FIX_ENABLED": bool, "NOTIFICATION_FAILURE_THRESHOLD": int,
        "NOTIFICATION_FAILURE_WINDOW_SECONDS": int, "RATE_LIMIT_ENABLED": bool,
        "RATE_LIMIT_WINDOW_SECONDS": int, "RATE_LIMIT_MAX_REPORTS": int,
        "AUDIT_LOG_ENABLED": bool, "AUDIT_LOG_FLUSH_INTERVAL_SECONDS": float,
        "AUDIT_LOG_BUFFER_SIZE": int, "AUDIT_LOG_MAX_FILE_SIZE_MB": int,
        "AUDIT_LOG_BACKUP_COUNT": int, "REMOTE_AUDIT_SERVICE_ENABLED": bool,
        "REMOTE_AUDIT_SERVICE_URL": Optional[str], "REMOTE_AUDIT_SERVICE_TIMEOUT": float,
        "REMOTE_AUDIT_DEAD_LETTER_ENABLED": bool, "SLACK_API_TIMEOUT_SECONDS": float,
        "EMAIL_API_TIMEOUT_SECONDS": float, "PAGERDUTY_API_TIMEOUT_SECONDS": float,
        "SLACK_FAILURE_RATE": float, "EMAIL_FAILURE_RATE": float,
        "PAGERDUTY_FAILURE_RATE": float, "ML_REMEDIATION_ENABLED": bool,
    ...
    for field, expected_type in required_fields.items():
        ...
        if env_var in os.environ:
            try:
                ...
                logger.error(json.dumps({"event": "env_parse_error", "field": field, "env_var": env_var, "error": str(e)}))
...
class BugManagerError(Exception):
    ...
    def __init__(self, message: str, error_id: Optional[str] = None, timestamp: Optional[str] = None):
        if not isinstance(message, str):
            raise TypeError("Error message must be a string.")
        super().__init__(message)
        self.error_id = error_id if error_id is not None else hashlib.sha256(message.encode()).hexdigest()[:8]
        self.timestamp = timestamp if timestamp is not None else datetime.datetime.now(datetime.timezone.utc).isoformat()
...
class NotificationError(BugManagerError):
    ...
    def __init__(self, message: str, channel: str, error_code: str = "GENERIC_ERROR", **kwargs: Any):
...
class RemediationError(BugManagerError):
    ...
    def __init__(self, message: str, step_name: Optional[str] = None, playbook_name: Optional[str] = None,
