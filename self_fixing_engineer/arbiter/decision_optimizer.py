...
from arbiter.metrics import get_or_create_counter, get_or_create_histogram, get_or_create_gauge
...
class MetaLearningService:
    ...
    async def get_latest_plugin_version(self, kind: str, name: str) -> Optional[str]:
        self.logger.debug(f"Fetching latest version for plugin {kind}:{name} from meta-learning service.")
        await asyncio.sleep(0.05)
        return None

    async def get_plugin_code(self, kind: str, name: str, version: str) -> Optional[Callable]:
        self.logger.debug(f"Fetching code for plugin {kind}:{name} version {version} from meta-learning service.")
...
# Prometheus Metrics
TASK_PRIORITIZATION_COUNT = get_or_create_counter('sfe_decision_optimizer_prioritization_total', 'Total task prioritizations for SFE')
ALLOCATION_LATENCY = get_or_create_histogram('sfe_decision_optimizer_allocation_latency_seconds', 'SFE task allocation latency')
COORDINATION_SUCCESS = get_or_create_counter('sfe_decision_optimizer_coordination_success_total', 'Successful SFE coordination actions')
AGENT_ACTIVE_GAUGE = get_or_create_gauge('sfe_decision_optimizer_active_agents', 'Number of active SFE agents')
EXPLANATION_EVENTS = get_or_create_counter('sfe_decision_optimizer_explanation_events_total', 'Total SFE explanation requests')
ERRORS_CRITICAL = get_or_create_counter('sfe_decision_optimizer_critical_errors_total', 'Critical errors caught in SFE DecisionOptimizer')
PLUGIN_EXECUTION_LATENCY = get_or_create_histogram('sfe_decision_optimizer_plugin_execution_latency_seconds', 'Latency of SFE plugin executions', ('plugin_name',))
DB_OPERATION_LATENCY = get_or_create_histogram('sfe_decision_optimizer_db_operation_latency_seconds', 'Latency of SFE database operations', ('operation_type',))
STRATEGY_REFRESH_COUNT = get_or_create_counter('sfe_decision_optimizer_strategy_refresh_total', 'Total strategy refresh attempts')
STRATEGY_REFRESH_SUCCESS = get_or_create_counter('sfe_decision_optimizer_strategy_refresh_success_total', 'Successful strategy refreshes')
...
class DecisionOptimizer:
    ...
    def __init__(
        ...
        self.config = {
            'default_weights': (config.get('default_weights', {'priority': 0.5, 'deadline': 0.3, 'risk': 0.15, 'context': 0.05})
                                if config else {'priority': 0.5, 'deadline': 0.3, 'risk': 0.15, 'context': 0.05}),
            'max_tasks_per_agent': (config.get('max_tasks_per_agent', 10) if config else 10),
            'encryption_key': (config.get('encryption_key', settings.ENCRYPTION_KEY.get_secret_value() if hasattr(settings, 'ENCRYPTION_KEY') and settings.ENCRYPTION_KEY else None)
                               if config else (settings.ENCRYPTION_KEY.get_secret_value() if hasattr(settings, 'ENCRYPTION_KEY') and settings.ENCRYPTION_KEY else None)),
            'redis_url': (config.get('redis_url', settings.REDIS_URL or 'redis://localhost:6379') if config else (settings.REDIS_URL or 'redis://localhost:6379')),
            'redis_pool_size': (config.get('redis_pool_size', getattr(settings, 'REDIS_POOL_SIZE', 100)) if config else getattr(settings, 'REDIS_POOL_SIZE', 100)),
            'db_pool_size': (config.get('db_pool_size', getattr(settings, 'DB_POOL_SIZE', 50)) if config else getattr(settings, 'DB_POOL_SIZE', 50)),
            'policy_check': (config.get('policy_check', True) if config else True),
            'human_approval_threshold': (config.get('human_approval_threshold', 'high') if config else 'high'),
            'batch_size': (config.get('batch_size', getattr(settings, 'DB_BATCH_SIZE', 100)) if config else getattr(settings, 'DB_BATCH_SIZE', 100)),
            'strategy_refresh_interval_seconds': (config.get('strategy_refresh_interval_seconds', 300) if config else 300)
        }
        ...
        if self.config['encryption_key']:
            try:
                ...
                self.logger.error(f"Failed to initialize Fernet encrypter for SFE: {e}. Encryption features will be disabled.", exc_info=True)
                self.encrypter = None
        else:
            self.logger.warning("Encryption key not provided in SFE config or settings. Fernet will not be initialized.")
            self.encrypter = None

        self.event_log: List[Dict[str, Any]] = []

        self.db: Optional[Database] = sfe_core_engine.database if sfe_core_engine and hasattr(sfe_core_engine, 'database') else None
        self.feedback_manager: Optional[FeedbackManager] = sfe_core_engine.feedback_manager if sfe_core_engine and hasattr(sfe_core_engine, 'feedback_manager') else None
        self.knowledge_graph: Optional["KnowledgeGraph"] = sfe_core_engine.knowledge_graph if sfe_core_engine and hasattr(sfe_core_engine, 'knowledge_graph') else None
        self.explainable_reasoner: Optional[ExplainableReasoner] = sfe_core_engine.explainable_reasoner if sfe_core_engine and hasattr(sfe_core_engine, 'explainable_reasoner') else None
        self.policy_engine: Optional[PolicyEngine] = sfe_core_engine.policy_engine if sfe_core_engine and hasattr(sfe_core_engine, 'policy_engine') else None
        self.bug_manager: Optional[BugManager] = sfe_core_engine.bug_manager if sfe_core_engine and hasattr(sfe_core_engine, 'bug_manager') else None
        self.monitor: Optional[Monitor] = sfe_core_engine.monitor if sfe_core_engine and hasattr(sfe_core_engine, 'monitor') else None
        self.human_in_loop: Optional[HumanInLoop] = sfe_core_engine.human_in_loop if sfe_core_engine and hasattr(sfe_core_engine, 'human_in_loop') else None
        ...
        if not self.feedback_manager:
            self.logger.warning("SFE FeedbackManager not provided. Some SFE metrics and error reporting will be degraded.")
        if not self.knowledge_graph:
            self.logger.warning("SFE KnowledgeGraph not provided. SFE graph-based reasoning and auditing will be degraded.")
        if not self.explainable_reasoner:
            self.logger.warning("SFE ExplainableReasoner not provided. SFE explanations will not be generated.")
        if not self.policy_engine:
            self.logger.warning("SFE PolicyEngine not provided. SFE policy checks will be bypassed.")
        if not self.bug_manager:
            self.logger.warning("SFE BugManager not provided. SFE critical error reporting will be degraded.")
    ...
    async def __aenter__(self):
        ...
        if not self.db and hasattr(self.settings, 'DATABASE_URL'):
            try:
                ...
                self.logger.critical(f"Failed to establish standalone SFE database connection: {e}", exc_info=True)
            ...
            self.logger.warning("No database URL provided and no SFECoreEngine.database instance. DB operations will be skipped.")
    ...
    async def _periodic_strategy_refresh(self):
        while True:
            try:
                ...
                self.logger.info(f"Initiating periodic strategy refresh for SFE DecisionOptimizer. Next refresh in {interval} seconds.")
                await self.refresh_strategies()
            except asyncio.CancelledError:
                break
            except Exception as e:
                self.logger.error(f"Error during periodic strategy refresh for SFE: {e}", exc_info=True)
    ...
    async def refresh_strategies(self):
        ...
        try:
            ...
            if new_weights:
                ...
                self.logger.info(f"SFE Prioritization weights updated: {old_weights} -> {new_weights}")
                await self._log_event('sfe_prioritization_weights_updated', {'old_weights': old_weights, 'new_weights': new_weights})
            ...
            if self.policy_engine:
                ...
                if new_policy_rules:
                    if hasattr(self.policy_engine, 'update_rules'):
                        ...
                        await self._log_event('sfe_policy_rules_updated', {'new_rules': new_policy_rules})
                    else:
                        self.logger.warning("SFE PolicyEngine does not support dynamic rule updates. Skipping.")

            new_prioritizer_version = await self.meta_learning_service.get_latest_plugin_version('PRIORITIZER', 'advanced_prioritizer')
            if new_prioritizer_version:
                prioritizer_plugin = self.plugin_registry.get('PRIORITIZER', 'advanced_prioritizer')
                if prioritizer_plugin and getattr(prioritizer_plugin, '_version', '0.0') != new_prioritizer_version:
                    self.logger.info(f"New version of 'advanced_prioritizer' plugin available: {new_prioritizer_version}. Attempting to load.")
                    self.logger.warning("Could not load new version of '%s' SFE plugin. Sticking to current.", 'advanced_prioritizer')
                elif not prioritizer_plugin:
                     self.logger.warning("No 'advanced_prioritizer' SFE plugin registered to update.")
    ...
    async def prioritize_and_allocate(
        ...
        sorted_tasks = sorted(
            tasks,
            key=lambda t: (-getattr(t, "priority", 0), getattr(t, "estimated_compute", 1.0))
        )
    ...
    async def process_remediation_proposal(self, proposal: Dict[str, Any]):
        ...
        if not self.policy_engine:
            self.logger.warning("SFE PolicyEngine not available, bypassing policy check for remediation.")
        else:
            ...
            if not is_allowed:
                await self._log_event('remediation_denied_by_policy', {"proposal": proposal, "reason": reason})
                return
        ...
        if proposal.get("risk_level") == "low":
            ...
            if not self.human_in_loop:
                self.logger.error("HumanInLoop component not available. Cannot escalate high-risk remediation. Denying action.")
                await self._log_event('remediation_escalation_failed', {"proposal": proposal, "reason": "HumanInLoop not configured."})
                return
            ...
            if approval_response.get("approved"):
                await self._log_event('remediation_human_approved', {"proposal": proposal, "response": approval_response})
                await self._execute_fix(proposal)
            else:
                await self._log_event('remediation_human_denied', {"proposal": proposal, "response": approval_response})
    ...
    async def _execute_fix(self, proposal: Dict[str, Any]):
        ...
        if fixer_name == "self_healing_import_fixer":
            self.logger.info(f"Simulating execution of fixer '{fixer_name}' for proposal type '{proposal.get('type')}'.")
            result = {"status": "success", "details": "Simulated fix applied successfully."}
        else:
            self.logger.error(f"Execution failed: Unknown fixer '{fixer_name}' for proposal type '{proposal.get('type')}'.")
        ...
        if self.sfe_core_engine and hasattr(self.sfe_core_engine, 'audit'):
            ...
            await self._log_event('remediation_executed', {
                'proposal_type': proposal.get('type'),
                'details': {"proposal": proposal, "result": result},
                'agent_id': 'sfe_system'
            })
        
        self.logger.info(f"Completed execution of fix for proposal '{proposal.get('type')}'. Status: {result.get('status')}")

    async def _log_event(self, event_type: str, details: Dict[str, Any]):
        audit_instance = self.sfe_core_engine.audit if self.sfe_core_engine and hasattr(self.sfe_core_engine, 'audit') else None
        ...
        if self.knowledge_graph:
            try:
                await self.knowledge_graph.add_fact(
                    'SFEDecisionOptimizerEvents', event['id'], event, source='sfe_decision_optimizer',
                    timestamp=timestamp_iso
                )
            except Exception as e:
        ...
        if self.monitor:
            try:
                self.monitor.log_action({
                    'type': event_type, 'decision_id': event['id'], 'description': details.get('description', ''),
                    'details': details
                })
            except Exception as e:
        ...
        if audit_instance and hasattr(audit_instance, 'add_entry'):
            try:
                await audit_instance.add_entry(
                    kind='sfe_decision_optimizer', name=event_type, detail=details, sim_id=details.get('sim_id'),
                    agent_id=details.get('user_id', 'system')
                )
            except Exception as e:
                self.logger.error(f"Failed to add audit entry for SFE event {event_type}: {e}", exc_info=True)

        if self.feedback_manager:
            try:
                await self.feedback_manager.record_metric(f'sfe_{event_type}_count', 1, {'event_id': event['id']})
            except Exception as e:
                self.logger.error(f"Failed to record metric via SFE FeedbackManager for event {event_type}: {e}", exc_info=True)

        if self.redis_client:
            try:
                await self._redis_publish('sfe_decision_optimizer_events', json.dumps(safe_serialize(event)))
            except Exception as e:
    ...
    def critical_alert_decorator(method: Callable) -> Callable:
        ...
        async def wrapper(self: DecisionOptimizer, *args, **kwargs):
            try:
                ...
                ERRORS_CRITICAL.inc()
                self.logger.exception(f"Critical error in SFE DecisionOptimizer method {method.__name__} (ID: {error_id}): {e}")
                
                error_log_data = {
                    'type': 'sfe_critical_error', 'method': method.__name__,
                    'error': str(e), 'error_id': error_id, 'traceback': traceback.format_exc()
                }
                    ...
                    try:
                        if hasattr(self.bug_manager, 'bug_detected'):
                            await self.bug_manager.bug_detected(
                                'sfe_critical_error', f"Critical error in SFE {method.__name__}: {e}", error_id=error_id
                            )
                        else:
                            self.logger.warning("BugManager instance does not have a 'bug_detected' method.")
                    except Exception as be:
                        self.logger.error(f"Failed to log SFE bug to BugManager: {be}")
                
                if self.sfe_core_engine and hasattr(self.sfe_core_engine, 'notification_service') and self.sfe_core_engine.notification_service:
                    try:
                        if hasattr(self.sfe_core_engine.notification_service, 'send_alert'):
                            self.sfe_core_engine.notification_service.send_alert(
                                f"Critical error in SFE {method.__name__}: {e}", 'critical',
                                time.time(), {'error_id': error_id}
                            )
                        else:
                            self.logger.warning("SFECoreEngine.notification_service does not have a 'send_alert' method.")
    ...
    async def load_strategy_plugin(self, kind: str, name: str, strategy_type: str):
        async with self.lock:
            ...
            if not hasattr(plugin_instance, 'execute') or not callable(plugin_instance.execute):
                raise TypeError(f"SFE Plugin {kind}:{name} does not have a callable 'execute' method.")
            ...
            if strategy_type == 'prioritizer':
                ...
                raise ValueError(f"Unknown strategy_type: {strategy_type}. Must be 'prioritizer', 'allocator', or 'coordinator'.")
            
            setattr(getattr(self, strategy_type), '_plugin_name', f"{kind}:{name}")

            if self.knowledge_graph:
                await self.knowledge_graph.add_fact(
                    'SFEStrategyVersions', f"{strategy_type}_{name}_{time.time()}",
                    {'plugin': f"{kind}:{name}", 'strategy_type': strategy_type}, source='sfe_decision_optimizer'
                )
            await self._log_event('sfe_strategy_plugin_loaded', {
    ...
    async def safe_execute(self, callable: Callable, *args, **kwargs):
        if self.policy_engine:
            allowed, reason = await self.policy_engine.should_auto_learn(
                'SFEStrategyExecution', callable.__name__, 'system', {'args': args, 'kwargs': kwargs}
            )
            if not allowed:
                await self._log_event('sfe_strategy_execution_denied', {
                    'strategy_name': callable.__name__, 'reason': reason,
                    'args': safe_serialize(args), 'kwargs': safe_serialize(kwargs)
                })
        ...
        try:
            if asyncio.iscoroutinefunction(callable):
                ...
                loop = asyncio.get_running_loop()
                result = await loop.run_in_executor(self.array_backend.executor, callable, *args, **kwargs)
            ...
            if self.bug_manager:
                callable_name = getattr(callable, '_plugin_name', callable.__name__)
                await self.bug_manager.bug_detected(
                    'sfe_strategy_execution_error', f"SFE Strategy '{callable_name}' execution failed: {e}",
                    {'strategy': callable_name, 'error': str(e), 'args': safe_serialize(args), 'kwargs': safe_serialize(kwargs)}
                )
            raise
    ...
    async def anonymize_task(self, task: Task):
        ...
        if self.knowledge_graph:
            await self.knowledge_graph.add_fact(
                'SFEAnonymizedTasks', task.id, task.metadata, source='sfe_decision_optimizer'
            )

    async def _handle_failed_task(self, task: Task, error: str):
        if self.redis_client:
            await self.redis_client.lpush('sfe_dead_letter_queue', json.dumps({
                'task_id': task.id, 'error': error, 'metadata': safe_serialize(task.metadata)
            }))
        await self._log_event('sfe_task_failed', {
    ...
    async def _redis_publish(self, channel: str, message: str):
        if self.redis_client:
            ...
            self.logger.warning("Redis client not initialized for SFE. Cannot publish to channel %s.", channel)
        ...
        try:
            ...
            for task in task_queue:
                ...
                for dep_id in task.dependencies:
                    if not self.task_graph.has_node(dep_id):
                        self.logger.warning(f"SFE task dependency {dep_id} for task {task.id} not found in current task queue. Skipping edge.")
                        continue
            ...
            if self.config['policy_check'] and self.policy_engine:
                policy_checks = await asyncio.gather(*[
                    self.policy_engine.should_auto_learn('SFETasks', task.id, 'system', task.metadata)
                    for task in task_queue
                ])
                for i, (allowed, reason) in enumerate(policy_checks):
                    ...
                    if allowed:
                        ...
                        await self._log_event('sfe_task_policy_denied', {
                            'task_id': task.id, 'reason': reason, 'sim_id': task.metadata.get('sim_id')
                        })
                        if self.feedback_manager:
                            await self.feedback_manager.log_error({
                                'type': 'sfe_policy_denial', 'task_id': task.id, 'reason': reason
                            })
            else:
            ...
            for i in range(0, len(allowed_tasks), batch_size):
                ...
                batch_prioritized = await self.safe_execute(
                    self.prioritizer, batch, criteria or {'weights': self.config['default_weights']}, agent_pool, explorer_context
                )
            ...
            if self.explainable_reasoner and criteria and criteria.get('explain', False):
                try:
                    ...
                    await self._log_event('sfe_prioritization_explained', {
                        'explanation': explanation, 'task_ids': [t.id for t in prioritized]
                    })
                    EXPLANATION_EVENTS.inc()
                except Exception as e:
                    self.logger.error(f"Failed to generate explanation for SFE prioritization: {e}", exc_info=True)
            ...
            if self.db:
                ...
                try:
                    ...
                    for task in prioritized:
                        task_sim_id = task.sim_request.id if task.sim_request and hasattr(task.sim_request, 'id') else task.id
                    ...
                    DB_OPERATION_LATENCY.labels(operation_type='sfe_save_simulation_batch').observe(time.monotonic() - db_start_time)
                except Exception as e:
                    self.logger.error(f"Failed to save SFE simulation status for prioritized tasks: {e}", exc_info=True)
                    DB_OPERATION_LATENCY.labels(operation_type='sfe_save_simulation_batch_error').observe(time.monotonic() - db_start_time)


            await self._log_event('sfe_prioritize_tasks', {
            ...
            await self._handle_failed_task(task_queue[0] if task_queue else Task(id='unknown', priority=0), str(e))
            raise

    async def _default_prioritize(self, task_queue: List[Task], criteria: Dict[str, Any], agent_pool: List[Agent], explorer_context: Dict[str, Any]) -> List[Task]:
        ...
        if criteria.get('use_dream_mode', False):
            ...
            if dream_plugin:
                try:
                    dream_strategy_scores = await self.safe_execute(dream_plugin.execute, {'tasks': [t.metadata for t in task_queue]})
                    return sorted(task_queue, key=lambda t: dream_strategy_scores.get(t.id, 0), reverse=True)
                except Exception as e:
                    self.logger.warning(f"SFE Dream mode plugin execution failed: {e}. Proceeding with default prioritization.")
            else:
                self.logger.warning("SFE Dream mode requested but 'dream_mode' plugin not found. Proceeding with default prioritization.")

        async def score(task: Task) -> float:
            deadline_score = 1.0 / (task.deadline - now + 1) if task.deadline and (task.deadline - now > 0) else 0
            ...
            if explorer_context:
                if explorer_context.get('code_vulnerabilities_found') and task.action_type == 'security_fix':
                    context_score += 0.8
                elif explorer_context.get('test_coverage_issues') and task.action_type == 'test_generation':
                    context_score += 0.5
        ...
        if not hasattr(numpy_scores, 'argsort'):
            self.logger.warning("Array backend did not return a NumPy-like array. Converting manually.")
    ...
    async def _get_knowledge_graph_context_score(self, task: Task) -> float:
        ...
        try:
            ...
            self.logger.warning(f"Error getting SFE KnowledgeGraph context for task {task.id}: {e}", exc_info=True)
            return 0.0
    ...
    async def allocate_resources(
        ...
        try:
            ...
            if metrics.get('cpu_percent', 0) > 80:
                await self._log_event('sfe_high_system_load', {
            ...
            for agent_id, tasks in assignments.items():
                approved_tasks = []
                for task in tasks:
                    if self.human_in_loop and \
                       self.config.get('human_approval_threshold') and \
                       self.config['human_approval_threshold'] != 'none' and \
                       task.risk_level.lower() == self.config['human_approval_threshold'].lower():
                        try:
                            response = await self.human_in_loop.request_approval({
                                'decision_id': task.id, 'risk_level': task.risk_level, 'action': task.action_type,
                                'task_details': safe_serialize(task.metadata), 'sim_id': task.metadata.get('sim_id')
                            })
                            if response.get('approved', False):
                                approved_tasks.append(task)
                                await self._log_event('sfe_task_human_approved', {
                                    'task_id': task.id, 'agent_id': agent_id, 'sim_id': task.metadata.get('sim_id')
                                })
                            else:
                                await self._log_event('sfe_task_allocation_denied_human', {
                                    'task_id': task.id, 'reason': response.get('comment', 'Human denial'),
                                    'sim_id': task.metadata.get('sim_id')
                                })
                                if self.feedback_manager:
                                    await self.feedback_manager.log_error({
                                        'type': 'sfe_human_denial', 'task_id': task.id, 'reason': response.get('comment')
                                    })
                        except Exception as e:
                            self.logger.error(f"Error requesting SFE human approval for task {task.id}: {e}", exc_info=True)
                            await self._log_event('sfe_human_approval_error', {
                                'task_id': task.id, 'error': str(e), 'sim_id': task.metadata.get('sim_id')
                            })
                ...
                for task in approved_tasks:
                    ...
                    try:
                        if self.sfe_core_engine and hasattr(self.sfe_core_engine, 'execute_task'):
                            result = await self.sfe_core_engine.execute_task(task.action_type, task.sim_request, task_id=task.id)
                            await self._log_event('sfe_task_executed_via_core_engine', {
                                'task_id': task.id, 'result': safe_serialize(result), 'sim_id': task.id, 'agent_id': agent_id
                            })
                        else:
                            plugin_instance = None
                            if hasattr(self.plugin_registry, 'get_plugin_by_action_name'):
                                plugin_instance = self.plugin_registry.get_plugin_by_action_name(task.action_type)
                            else:
                                plugin_instance = self.plugin_registry.get('ACTION', task.action_type)

                            if plugin_instance and hasattr(plugin_instance, 'execute'):
                                result = await self.safe_execute(plugin_instance.execute, task.sim_request, task_id=task.id)
                                await self._log_event('sfe_task_executed_direct_plugin', {
                                    'task_id': task.id, 'result': safe_serialize(result), 'sim_id': task.id, 'agent_id': agent_id
                                })
                            else:
                                self.logger.warning(f"No executable SFE plugin found for action_type '{task.action_type}'. Task {task.id} not executed.")
                                await self._log_event('sfe_plugin_not_found', {
                                    'task_id': task.id, 'action_type': task.action_type,
                                    'description': 'No executable SFE plugin found for action_type'
                                })
                                continue

                        PLUGIN_EXECUTION_LATENCY.labels(plugin_name=task.action_type).observe(time.monotonic() - plugin_execution_start_time)

                    except Exception as e:
                        PLUGIN_EXECUTION_LATENCY.labels(plugin_name=task.action_type).observe(time.monotonic() - plugin_execution_start_time)
                        if self.bug_manager:
                            await self.bug_manager.bug_detected(
                                'sfe_plugin_execution_error', f"SFE Plugin {task.action_type} for task {task.id} failed: {e}",
                                {'task_id': task.id, 'error': str(e)}
                            )
                        await self._handle_failed_task(task, str(e))

            await self._log_event('sfe_allocate_resources', {
    ...
    async def _default_allocate(
        ...
        for task in task_queue:
            ...
            if not candidates:
                await self._log_event('sfe_no_suitable_agent', {
                    'task_id': task.id, 'description': 'No suitable SFE agent found for skills/load/role',
                    'sim_id': task.metadata.get('sim_id')
                })
                if self.feedback_manager:
                    await self.feedback_manager.log_error({
                        'type': 'sfe_allocation_failure', 'task_id': task.id, 'reason': 'No suitable SFE agent'
                    })
                continue
    ...
    async def coordinate_arbiters(
        ...
        try:
            if self.encrypter is None:
                self.logger.error("Fernet encrypter not initialized for SFE. Cannot perform secure coordination.")
                raise RuntimeError("Fernet encrypter is not initialized. Cannot coordinate SFE arbiters securely.")
            ...
            if self.redis_client:
                await self._redis_publish('sfe_arbiter_coordination', json.dumps({
                    'event_type': 'coordination', 'context': safe_serialize(shared_context), 'timestamp': time.time()
                }))
            COORDINATION_SUCCESS.inc()
            await self._log_event('sfe_coordinate_arbiters', {
                'agent_count': len(agent_pool),
                'latency': time.monotonic() - start_time
            })
            return coordination
    ...
    async def _default_coordinate(self, agent_pool: List[Agent], encrypted_context: bytes) -> Dict[str, Any]:
        coordination_results = {}
        
        if self.encrypter is None:
            self.logger.error("Fernet encrypter not initialized for SFE. Cannot decrypt context for coordination.")
            raise RuntimeError("Fernet encrypter is not initialized. Cannot decrypt SFE context.")

        try:
            decrypted_context = json.loads(self.encrypter.decrypt(encrypted_context).decode('utf-8'))
        except InvalidToken as e:
            self.logger.error(f"Failed to decrypt context during SFE coordination: {e}. Invalid encryption token.", exc_info=True)
            raise RuntimeError("Failed to decrypt SFE coordination context due to invalid token.") from e
        except Exception as e:
            self.logger.error(f"Failed to decrypt or deserialize context during SFE coordination: {e}", exc_info=True)
        ...
        for agent in agent_pool:
            if agent.arbiter_instance and hasattr(agent.arbiter_instance, 'propose_action'):
                try:
                    ...
                    self.logger.warning(f"SFE Agent {agent.id} failed to propose action: {e}", exc_info=True)
                    coordination_results[agent.id] = 'proposal_failed'
                    if self.feedback_manager:
                        await self.feedback_manager.log_error({
                            'type': 'sfe_proposal_failure', 'agent_id': agent.id, 'reason': str(e)
                        })

        if proposals and decrypted_context and 'action_schema' in decrypted_context:
            actions = [p['action'] for p in proposals.values() if isinstance(p, dict) and 'action' in p]
            if actions:
                majority_action = Counter(actions).most_common(1)[0][0]
                decrypted_context['agreed_action'] = majority_action
                if self.knowledge_graph:
                    await self.knowledge_graph.add_fact(
                        'SFECoordinationResults', f'coord_{uuid.uuid4()}', {
                ...
                self.logger.info("No valid actions proposed for SFE consensus or no 'action' key in proposals.")
        elif not proposals:
            self.logger.info("No proposals received from agents for SFE coordination.")
        else:
            self.logger.warning("Decrypted context or action_schema missing for SFE consensus evaluation.")
        ...
        for agent in agent_pool:
            if agent.arbiter_instance and hasattr(agent.arbiter_instance, 'receive_context'):
                try:
                    ...
                    self.logger.warning(f"SFE Agent {agent.id} failed to receive context: {e}", exc_info=True)
                    coordination_results[agent.id] = 'context_update_failed'
                    if self.feedback_manager:
                        await self.feedback_manager.log_error({
                            'type': 'sfe_context_update_failure', 'agent_id': agent.id, 'reason': str(e)
                        })
    ...
    async def share_learning(self, agent: Agent, strategy: Dict[str, Any]):
        ...
        if self.knowledge_graph:
            await self.knowledge_graph.add_fact(
                'SFEAgentLearning', f"{agent.id}_{uuid.uuid4()}",
                {'strategy': strategy, 'agent_id': agent.id}, source='sfe_decision_optimizer'
            )
        else:
            self.logger.warning(f"SFE KnowledgeGraph not initialized, skipping learning share for agent {agent.id}.")

    async def rollback_allocation(self, assignments: Dict[str, List[Task]], agent_pool: List[Agent]):
        ...
        for agent_id, tasks in assignments.items():
            ...
            if agent:
                ...
                self.logger.warning(f"SFE Agent {agent_id} not found in agent_pool during rollback. Possible stale agent reference.")
    ...
    async def explain_decision(self, decision_id: str) -> Dict[str, Any]:
        ...
        if not decision_event:
            if self.knowledge_graph:
                kg_fact = await self.knowledge_graph.get_fact('SFEDecisionOptimizerEvents', decision_id)
                if kg_fact and 'value' in kg_fact:
                    decision_event = kg_fact['value']
                else:
                    return {'error': f'SFE Decision {decision_id} not found in memory or KnowledgeGraph.'}
            else:
                return {'error': f'SFE Decision {decision_id} not found in memory. KnowledgeGraph not initialized to search further.'}
        ...
        if self.explainable_reasoner:
            try:
                ...
                self.logger.error(f"Failed to generate explanation for SFE decision {decision_id}: {e}", exc_info=True)
    ...
    async def stream_events(self, websocket: WebSocket):
        ...
        try:
            ...
            if self.feedback_manager:
                await self.feedback_manager.log_error({'type': 'sfe_websocket_error', 'error': str(e)})

    async def compute_trust_score(self, context: Dict[str, Any], user_id: Optional[str] = None) -> float:
        """
        ...
        try:
            if self.policy_engine and self.config["policy_check"]:
                ...
                DB_OPERATION_LATENCY.labels(operation_type='sfe_policy_check').observe(time.monotonic() - policy_start_time)
                
                if not allowed:
                    await self._log_event("sfe_trust_score_policy_denied", {
                        "task_id": task.id, "reason": reason, "user_id": user_id, "score_result": 0.0
                    })
                    if self.feedback_manager:
                        await self.feedback_manager.log_error({
                            "type": "sfe_policy_denial", "task_id": task.id, "reason": reason, "user_id": user_id
                        })
                    return 0.0
            ...
            if self.knowledge_graph:
                ...
                try:
                    ...
                    self.logger.warning(f"SFE KnowledgeGraph context enrichment failed for trust score task {task.id}: {kg_e}", exc_info=True)
                    context_score = 0.0
                DB_OPERATION_LATENCY.labels(operation_type='sfe_knowledge_graph_lookup').observe(time.monotonic() - kg_start_time)
            ...
            if self.human_in_loop and task.risk_level.lower() == "high":
                ...
                try:
                    ...
                    if not response.get("approved", False):
                        score *= 0.5
                        await self._log_event("sfe_trust_score_human_denial", {
                            "task_id": task.id, "reason": response.get("comment", "Human denial"),
                            "user_id": user_id, "score_result": score
                        })
                        if self.feedback_manager:
                            await self.feedback_manager.log_error({
                                "type": "sfe_human_denial_trust_score", "task_id": task.id,
                                "reason": response.get("comment"), "user_id": user_id
                            })
                    else:
                        await self._log_event("sfe_trust_score_human_approved", {
                            "task_id": task.id, "user_id": user_id, "score_result": score
                        })
                except Exception as hitl_e:
                    self.logger.error(f"SFE Human-in-loop approval failed for trust score task {task.id}: {hitl_e}", exc_info=True)
                DB_OPERATION_LATENCY.labels(operation_type='sfe_human_in_loop_approval').observe(time.monotonic() - human_loop_start_time)

            await self._log_event("sfe_trust_score_computed", {
                "task_id": task.id, "score": score, "anomalies": anomalies, "user_id": user_id,
                "context_factors": {k: context.get(k) for k in weights.keys()}
            })
            
            if self.knowledge_graph:
                await self.knowledge_graph.add_fact(
                    "SFETrustScores", task.id, {"score": score, "context": safe_serialize(context), "user_id": user_id},
                    source="sfe_decision_optimizer_trust_score"
                )
            
            if self.feedback_manager:
                await self.feedback_manager.record_metric("sfe_trust_score", score, {"task_id": task.id, "user_id": user_id})
            ...
            self.logger.error(f"SFE Trust score computation for user {user_id} failed unexpectedly: {e}", exc_info=True)
            return 0.0