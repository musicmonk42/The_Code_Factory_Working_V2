...
# --- Metrics Definitions ---
MQ_PUBLISH_TOTAL = _get_or_create_metric(
    Counter,
    "mq_publish_total",
    "Total messages published",
    ("backend", "event_type", "status"),
)
MQ_CONSUME_TOTAL = _get_or_create_metric(
    Counter,
    "mq_consume_total",
    "Total messages consumed",
    ("backend", "event_type", "status"),
)
MQ_PUBLISH_LATENCY = _get_or_create_metric(
    Histogram,
    "mq_publish_latency_seconds",
    "Latency of message publishing",
    ("backend", "event_type"),
)
MQ_CONSUME_LATENCY = _get_or_create_metric(
    Histogram,
    "mq_consume_latency_seconds",
    "Latency of message consumption",
    ("backend", "event_type"),
)
MQ_DLQ_TOTAL = _get_or_create_metric(
    Counter,
    "mq_dlq_total",
    "Total messages sent to Dead Letter Queue",
    ("backend", "event_type", "reason"),
)
MQ_ENCRYPTION_ERRORS = _get_or_create_metric(
    Counter,
    "mq_encryption_errors_total",
    "Total encryption/decryption errors",
    ("backend", "event_type"),
)
MQ_CONNECTION_STATUS = _get_or_create_metric(
    Gauge,
    "mq_connection_status",
    "Status of message queue connection (1=up, 0=down)",
    ("backend",),
)
...
class BackendNotAvailableError(MessageQueueServiceError):
    pass


class SerializationError(MessageQueueServiceError):
    pass


class DecryptionError(MessageQueueServiceError):
    pass


class PermissionError(MessageQueueServiceError):
    pass
...
class MessageQueueService:
    ...
    def __init__(
        ...
        if backend_type not in self.SUPPORTED_BACKENDS:
            raise ValueError(
                f"Unsupported backend type: {backend_type}. Must be one of {self.SUPPORTED_BACKENDS}"
            )
        ...
        if encryption_key:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid Fernet encryption key format in config. Key must be 32 url-safe base64-encoded bytes: {e}"
                ) from e
        ...
        if self.backend_type == "redis_streams":
            if not REDIS_STREAMS_AVAILABLE:
                raise BackendNotAvailableError(
                    "redis-py (asyncio) is not installed for Redis Streams backend."
                )
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error disconnecting from {self.backend_type} backend: {e}",
                exc_info=True,
            )
        ...
        try:
            ...
            if self.audit_hook:
                self.audit_hook(
                    ...
                    data,
                    "failed",
                    f"Serialization/Encryption error: {e}",
                )
            raise MessageQueueServiceError(
                f"Failed to publish '{event_type}' due to serialization/encryption error."
            ) from e
        ...
        try:
            ...
            if self.backend_type == "redis_streams" and self._redis_client:
                ...
                await self.memory_queue.put(
                    {"event_type": event_type, "data": data, "raw_data": encrypted_data}
                )
            else:
                raise MessageQueueServiceError(
                    f"Publish not implemented for {self.backend_type} or client not ready."
                )
            ...
            logger.debug(
                f"Published event '{event_type}' to {self.backend_type} topic '{topic_name}'. Critical: {is_critical}"
            )
            return
        except Exception as e:
            MQ_PUBLISH_TOTAL.labels(
    ...
    async def subscribe(
        ...
        logger.info(
            f"Subscribed to '{event_type}' on {self.backend_type} backend. Consumer group: {self.consumer_group_id}"
        )
            ...
            try:
                ...
                if message.get("event_type") == event_type:
                    ...
                    try:
                        ...
                        await self._send_to_dlq(
                            event_type, message.get("data"), f"Processing error: {e}"
                        )
                    except Exception as e:
                        logger.error(
                            f"In-memory message handler failed: {e}. Sending to DLQ.",
                            exc_info=True,
                        )
                        await self._send_to_dlq(
    ...
    async def _redis_stream_consumer(
        ...
        try:
            await self._redis_client.xgroup_create(
                stream_name, self.consumer_group_id, id="0", mkstream=True
            )
            logger.info(
                f"Redis Stream consumer group '{self.consumer_group_id}' created or already exists for stream '{stream_name}'."
            )
        except Exception as e:
                        ...
                        for message_id, message_data in messages:
                            ...
                            try:
                                ...
                                if self.policy_hook and not self.policy_hook(
                                    stream_name, "consume", deserialized_data
                                ):
                                    logger.warning(
                                        f"Policy denied consume for event '{stream_name}'. Skipping."
                                    )
                                ...
                                MQ_CONSUME_LATENCY.labels(
                                    backend="redis_streams", event_type=stream_name
                                ).observe(time.monotonic() - start_time)
                            except (SerializationError, DecryptionError) as e:
                                logger.error(
                                    f"Redis Stream message processing failed for ID {message_id}: {e}. Sending to DLQ.",
                                    exc_info=True,
                                )
                                await self._send_to_dlq(
                                    stream_name,
                                    {
                                        "id": message_id,
                                        "data": (
                                            message_data.get(b"payload").decode(
                                                "latin-1", errors="replace"
                                            )
                                            if message_data.get(b"payload")
                                            else "N/A"
                                        ),
                                    },
                                ...
                                logger.error(
                                    f"Redis Stream message handler failed for ID {message_id}: {e}. Sending to DLQ.",
                                    exc_info=True,
                                )
                                await self._send_to_dlq(
                                    stream_name,
                ...
                try:
                    async for msg in consumer:
                        ...
                        try:
                            ...
                            # Policy and audit hooks for consume
                            if self.policy_hook and not self.policy_hook(
                                topic_name, "consume", deserialized_data
                            ):
                                logger.warning(
                                    f"Policy denied consume for event '{topic_name}'. Skipping."
                                )
                                continue
                            if self.audit_hook:
                                self.audit_hook(
                            ...
                            await handler(deserialized_data)
                            MQ_CONSUME_TOTAL.labels(
                                backend="kafka", event_type=topic_name, status="success"
                            ).inc()
                            MQ_CONSUME_LATENCY.labels(
                                backend="kafka", event_type=topic_name
                            ).observe(time.monotonic() - start_time)
                        except (SerializationError, DecryptionError) as e:
                            logger.error(
                                f"Kafka message processing failed for offset {msg.offset}: {e}. Sending to DLQ.",
                                exc_info=True,
                            )
                            await self._send_to_dlq(
                                topic_name,
                            ...
                            logger.error(
                                f"Kafka message handler failed for offset {msg.offset}: {e}. Sending to DLQ.",
                                exc_info=True,
                            )
                            await self._send_to_dlq(
                                topic_name,
                    ...
                    logger.error(
                        f"Kafka consumer error: {e}. Attempting reconnection...",
                        exc_info=True,
                    )
                    ...
                    logger.critical(
                        f"Unhandled error in Kafka consumer for '{topic_name}': {e}",
                        exc_info=True,
                    )
                    await asyncio.sleep(self.retry_delay_base * 5)
        finally:
    ...
    async def _send_to_dlq(
        self, event_type: str, original_data: Dict[str, Any], reason: str
    ) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Message for '{event_type}' sent to DLQ topic '{dlq_topic}' due to: {reason}"
            )
        except Exception as e:
            logger.critical(
                f"Failed to send message to DLQ for '{event_type}': {e}. Message lost.",
                exc_info=True,
            )
            if self.audit_hook:
        ...
        if self.backend_type == "redis_streams" and self._redis_client:
            ...
            for message_id, message_data in messages:
                try:
                    ...
                    if not payload_bytes:
                        raise SerializationError(
                            "Missing 'payload' field in Redis Stream DLQ message."
                        )
                    decrypted_data = self._decrypt_payload(payload_bytes)
                    ...
                    if original_event_type and original_data:
                        ...
                        logger.warning(
                            f"Malformed DLQ entry {message_id}: {dlq_entry}. Skipping replay and deleting from DLQ."
                        )
                        await self._redis_client.xdel(dlq_topic, message_id)
                except Exception as e:
                    logger.error(
                ...
                async for msg in consumer:
                    try:
                        ...
                        if original_event_type and original_data:
                            ...
                            logger.warning(
                                f"Malformed Kafka DLQ entry at offset {msg.offset}: {dlq_entry}. Skipping replay and committing offset."
                            )
                            await consumer.commit()
                    except Exception as e:
                        logger.error(
                            f"Error replaying Kafka DLQ message at offset {msg.offset}: {e}. Will retry on next replay_dlq call.",
                            exc_info=True,
                        )
            finally:
                await consumer.stop()
