...
"""
Persistent array store for the Arbiter/SFE platform.

This module provides the canonical implementation. The old name
(``arbiter_array_backend``) is a deprecated compatibility shim that
...
USAGE:
    ...
    from self_fixing_engineer.arbiter.persistent_array_store import PersistentArrayStore

    # Register backend as a plugin
    @registry.register(kind=PlugInKind.CORE_SERVICE, name="PersistentArrayStore", version="1.0.0", author="Arbiter Team")
    class MyPersistentArrayStore(PersistentArrayStore):
        pass

    # In an async function:
    backend = MyPersistentArrayStore(name="my_array", storage_path="arrays.json")
...
class PersistentArrayStore(PluginBase):
...
class ConcretePersistentArrayStore(PersistentArrayStore):
    ...
    async def start(self) -> None:
        ...
        await self.initialize()
        logger.info(f"PersistentArrayStore {self.name} started")
    ...
    def stop(self) -> None:
        ...
        logger.info(f"PersistentArrayStore {self.name} stopped")
...
try:
    existing = registry.get(PlugInKind.CORE_SERVICE, "PersistentArrayStore")
    # If we get here without an exception, the plugin is already registered
    logger.debug(
        f"PersistentArrayStore already registered with version {existing.version if hasattr(existing, 'version') else 'unknown'}"
    )
    ...
    try:
        registry.register(
            ...
            name="PersistentArrayStore",
            version="1.0.0",
            author="Arbiter Team",
        )(ConcretePersistentArrayStore)
        logger.info("PersistentArrayStore registered successfully")
    except ValueError as e:
        if "not newer than existing version" in str(e):
            # Already registered, which is fine
            logger.debug("PersistentArrayStore already registered")
        else:
            raise
...
# ---------------------------------------------------------------------------
# These names were used in the original arbiter_array_backend module.
# They are preserved here so that code that imports directly from
# persistent_array_store (rather than through the shim) can still use the
# old class names if needed.  New code should use the canonical names above.
ArrayBackend = PersistentArrayStore
ConcreteArrayBackend = ConcretePersistentArrayStore
