...
SCHEMA_VALIDATION_ERRORS = get_or_create_metric(
    'Counter', 'schema_validation_errors_total',
...
class AgentState(Base):
    ...
    def _validate_inventory(self, inventory):
        ...
        if parsed is None and isinstance(inventory, str) and inventory not in ['[]', '']:
            SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='invalid_inventory').inc()
    ...
    def _validate_language(self, language):
        ...
        if parsed is None and isinstance(language, str) and language not in ['[]', '']:
            SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='invalid_language').inc()
    ...
    def _validate_personality(self, personality):
        ...
        if parsed is None and isinstance(personality, str) and personality not in ['{}', '']:
            SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='invalid_personality').inc()
    ...
    def validate_world_size(self, key, value):
        ...
        if not isinstance(value, int) or value <= 0:
            SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='invalid_world_size').inc()
    ...
    async def _validate_json_fields(mapper, connection, target):
        ...
        with tracer.start_as_current_span("validate_agent_state", attributes={"table": "agent_state"}):
            try:
                await asyncio.wait_for(
                    AgentState._validate_fields(target),
                    timeout=timeout
                )
            except asyncio.TimeoutError:
                SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='validation_timeout').inc()
        ...
        try:
            ...
            if personality_schema:
                ...
                if personality and not all(
                    ...
                    SCHEMA_VALIDATION_ERRORS.labels(table='agent_state', error_type='invalid_personality_schema').inc()
                    raise ValueError(f"AgentState.personality does not match schema: {personality_schema}")
...
class AgentMetadata(Base):
    ...
    def _validate_value(self, value):
        ...
        if parsed is None and isinstance(value, str) and value not in ['{}', '']:
            SCHEMA_VALIDATION_ERRORS.labels(table='agent_metadata', error_type='invalid_value').inc()
    ...
    async def _validate_json_fields(mapper, connection, target):
        ...
        with tracer.start_as_current_span("validate_agent_metadata", attributes={"table": "agent_metadata"}):
            try:
                await asyncio.wait_for(
                    AgentMetadata._validate_fields(target),
                    timeout=timeout
                )
            except asyncio.TimeoutError:
                SCHEMA_VALIDATION_ERRORS.labels(table='agent_metadata', error_type='validation_timeout').inc()
        ...
        try:
            ...
            if metadata_schema:
                ...
                if value and not all(
                    ...
                    SCHEMA_VALIDATION_ERRORS.labels(table='agent_metadata', error_type='invalid_value_schema').inc()
                    raise ValueError(f"AgentMetadata.value does not match schema: {metadata_schema}")
        except AttributeError:
