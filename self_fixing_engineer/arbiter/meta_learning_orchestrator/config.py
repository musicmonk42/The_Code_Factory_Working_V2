...
class MetaLearningConfig(BaseSettings):
    ...
    # Security Mode
    SECURE_MODE: bool = Field(default=False, description="If true, enforce presence of security keys.")

    # Data Lake Configuration - Changed from DirectoryPath to str with validation
    DATA_LAKE_PATH: str = Field(default="data/data_lake.jsonl", description="Path for data lake file.")
    ...
    LOCAL_AUDIT_LOG_PATH: str = Field(default="data/meta_learning_audit.jsonl", description="Local file path for audit log.")
    AUDIT_ENCRYPTION_KEY: Optional[str] = Field(default=None, description="Fernet encryption key for audit logs.")
    AUDIT_SIGNING_PRIVATE_KEY: Optional[str] = Field(default=None, description="ECDSA private key for signing audit logs (PEM format).")
    AUDIT_SIGNING_PUBLIC_KEY: Optional[str] = Field(default=None, description="ECDSA public key for verifying audit logs (PEM format).")
    AUDIT_LOG_ROTATION_SIZE_MB: int = Field(default=100, ge=1, description="Size in MB before audit log rotation.")
    AUDIT_LOG_MAX_FILES: int = Field(default=10, ge=1, description="Maximum number of rotated audit log files.")
    ...
    REDIS_URL: str = Field(default="redis://localhost:6379/0", description="Redis URL for distributed lock.")
    ...
    TRAINING_CHECK_INTERVAL_SECONDS: int = Field(default=3600, ge=60, description="Interval to check for new training data.")
    DEPLOYMENT_CHECK_INTERVAL_SECONDS: int = Field(default=1800, ge=60, description="Interval to check for new models to deploy.")
    ...
    ML_PLATFORM_ENDPOINT: str = Field(default="http://localhost:8081/ml-platform", description="Endpoint for the ML Platform service.")
    AGENT_CONFIG_SERVICE_ENDPOINT: str = Field(default="http://localhost:8082/agent-config", description="Endpoint for Agent Config service.")
    POLICY_ENGINE_ENDPOINT: str = Field(default="http://localhost:8083/policy-engine", description="Endpoint for Policy Engine service.")
    ...
    DEPLOYMENT_RETRY_DELAY_SECONDS: int = Field(default=60, ge=1, description="Delay between deployment retries.")
    ...
    CONFIG_RELOAD_INTERVAL_SECONDS: int = Field(default=0, ge=0, description="Interval to check for config reloads. 0 to disable.")
    CONFIG_SOURCE: str = Field(default="env", description="Source for dynamic config ('env', 'file', 'etcd').")
    CONFIG_FILE_PATH: Optional[str] = Field(default=None, description="Path to a config file if CONFIG_SOURCE is 'file'.")
    ...
    ETCD_PREFIX: str = Field(default="/meta_learning/config/", description="Etcd key prefix for dynamic configuration.")
    ...
    @field_validator("AUDIT_ENCRYPTION_KEY", "AUDIT_SIGNING_PRIVATE_KEY", "AUDIT_SIGNING_PUBLIC_KEY")
    ...
    def validate_kafka_brokers(cls, v: str, info: ValidationInfo) -> str:
        ...
        if (use_kafka_ingestion or use_kafka_audit):
            ...
            for broker in brokers:
                broker = broker.strip()
                if ':' not in broker:
                    raise ValueError(f"Invalid Kafka broker format: '{broker}'. Expected 'host:port'.")
        return v
    ...
    def validate_retention(cls, v: int, info: ValidationInfo) -> int:
        ...
        if use_s3 and v > 365:
            logger.warning("High retention days (>365) with S3 may incur significant costs. Consider S3 Lifecycle Policies.")
        return v
