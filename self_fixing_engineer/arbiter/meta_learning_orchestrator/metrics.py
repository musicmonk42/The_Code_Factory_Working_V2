...
if multiproc_dir:
    ...
    logger.info(f"Initialized Prometheus REGISTRY with multiprocess support using dir: {multiproc_dir}")
else:
    logger.warning("PROMETHEUS_MULTIPROC_DIR not set. Using single-process mode—metrics may corrupt in multi-worker apps.")
...
class LabeledMetricWrapper:
    ...
    """
    def __init__(self, metric, global_labels: Dict[str, str], extra_labelnames: Tuple[str, ...] = ()):
        self._metric = metric
    ...
    def inc(self, amount=1):
        ...
        if self._extra_labelnames:
            raise ValueError(f"{self._name} has additional labels {self._extra_labelnames} that must be specified")
...
def _get_or_create_metric_internal(metric_class, name, documentation, labelnames=(), buckets=None):
    ...
    # Create the metric
    if buckets:
        metric = metric_class(name, documentation, labelnames=all_labelnames, buckets=buckets, registry=REGISTRY)
    else:
...
class MetricRegistry:
    ...
    def get_or_create(self, metric_class, name: str, documentation: str,
                      labelnames: Tuple[str, ...] = (), buckets: Optional[Tuple[float, ...]] = None):
        """
        ...
        metric = _get_or_create_metric_internal(metric_class, name, documentation, labelnames, buckets)
...
METRIC_CONFLICTS = registry.get_or_create(
    Counter, "ml_metric_conflicts_total", "Total metric type mismatches requiring unregistration"
)
...
ML_TRAINING_LATENCY = registry.get_or_create(
    Histogram, "ml_training_latency_seconds", "Latency of ML training jobs",
    buckets=(0.1, 1, 5, 10, 30, 60, 300, 600, 1800)
)
ML_EVALUATION_LATENCY = registry.get_or_create(
    Histogram, "ml_evaluation_latency_seconds", "Latency of ML model evaluations",
    buckets=(0.1, 1, 5, 10, 30, 60, 300, 600)
)
ML_DEPLOYMENT_LATENCY = registry.get_or_create(
    Histogram, "ml_deployment_latency_seconds", "Latency of ML model deployments",
    buckets=(0.1, 1, 5, 10, 30, 60, 300, 600)
)
...
ML_DEPLOYMENT_RETRIES_EXHAUSTED = registry.get_or_create(
    Counter, "ml_deployment_retries_exhausted_total", "Deployments that exhausted all retries"
)
...
ML_AUDIT_EVENTS_TOTAL = registry.get_or_create(
    Counter, "ml_audit_events_total", "Total audit events logged", labelnames=("event_type",)
)
...
ML_AUDIT_SIGNATURE_MISMATCH = registry.get_or_create(
    Counter, "ml_audit_signature_mismatch_total", "Total audit signature mismatches detected"
)
...
if __name__ == "__main__":
    ...
    audit_metric = registry.get_or_create(
        Counter, "ml_audit_events_total", "Total audit events", labelnames=("event_type",)
    )
