...
from .models import (
    ...
    LeaderElectionError, ValidationError
)
...
async def create_task_with_supervision(coro: Awaitable, task_name: str, restart_on_error: bool = True, restart_delay: int = 5):
    """
    ...
    while True:
        ...
        try:
            ...
            _log_structured(logging.INFO, f"Task '{task_name}' was cancelled gracefully.", task_name=task_name)
            break
        except Exception as e:
            _log_structured(logging.CRITICAL, f"Task '{task_name}' crashed with unhandled exception.",
                            task_name=task_name, error=str(e), exc_info=True)
            ML_ORCHESTRATOR_ERRORS.inc()
            if restart_on_error:
                _log_structured(logging.INFO, f"Restarting task '{task_name}' in {restart_delay} seconds...",
                                task_name=task_name, restart_delay=restart_delay)
                await asyncio.sleep(restart_delay)
            else:
                _log_structured(logging.ERROR, f"Task '{task_name}' failed and will NOT be restarted.",
                                task_name=task_name, restart_on_error=restart_on_error)
                break

# --- Sub-classes for Modular Orchestration Logic ---
class Ingestor:
    """Handles data ingestion from various sources (Kafka, S3, local file)."""
    def __init__(self, config: MetaLearningConfig, kafka_producer: Optional[AIOKafkaProducer] = None):
        self.config = config
    ...
    async def initialize(self):
        ...
        if self.config.USE_KAFKA_INGESTION and not self.kafka_producer:
            ...
            self.kafka_producer = AIOKafkaProducer(bootstrap_servers=self.config.KAFKA_BOOTSTRAP_SERVERS)
        ...
        with trace.get_tracer(__name__).start_as_current_span("ingest_learning_record"):
            ...
            try:
                ...
                if self.config.USE_KAFKA_INGESTION:
                    ...
                    _log_structured(logging.DEBUG, "Ingested learning record to Kafka.", agent_id=record.agent_id)
                    ...
                    _log_structured(logging.DEBUG, "Ingested learning record to local file.", path=self.config.DATA_LAKE_PATH)

            except ValidationError as e:
                _log_structured(logging.ERROR, "Failed to ingest record due to validation error.",
                                error=e.errors(), exc_info=True)
                ML_ORCHESTRATOR_ERRORS.inc()
                raise DataIngestionError(f"Invalid learning record data: {e.errors()}") from e
            except Exception as e:
                _log_structured(logging.ERROR, "Failed to ingest learning record.",
...
class Trainer:
    ...
    def __init__(self, config: MetaLearningConfig, ml_platform_client: MLPlatformClient, agent_config_service: AgentConfigurationService):
        self.config = config
    ...
    async def _evaluate_model(self, model_version: ModelVersion) -> bool:
        ...
        with trace.get_tracer(__name__).start_as_current_span("evaluate_model"):
            ...
            try:
                _log_structured(logging.INFO, "Evaluating model...", model_id=model_version.model_id, version=model_version.version)
                ...
                if primary_metric >= self.config.MODEL_BENCHMARK_THRESHOLD:
                    _log_structured(logging.INFO, "Model passed evaluation.", model_id=model_version.model_id, version=model_version.version,
                                    metric=primary_metric, threshold=self.config.MODEL_BENCHMARK_THRESHOLD)
                    return True
                else:
                    _log_structured(logging.WARNING, "Model failed evaluation.", model_id=model_version.model_id, version=model_version.version,
                                    metric=primary_metric, threshold=self.config.MODEL_BENCHMARK_THRESHOLD)
                    return False
            except Exception as e:
                _log_structured(logging.ERROR, "Error during model evaluation.", model_id=model_version.model_id, error=str(e), exc_info=True)
                ML_ORCHESTRATOR_ERRORS.inc()
                return False
            finally:
    ...
    @retry(
        ...
        reraise=True,
        before_sleep=lambda retry_state: _log_structured(logging.WARNING, "Deployment failed, retrying...",
                                                         attempt=retry_state.attempt_number, max_attempts=5)
    )
    async def _deploy_model(self, model_version: ModelVersion):
        ...
        with trace.get_tracer(__name__).start_as_current_span("deploy_model"):
            ...
            start_time = time.monotonic()
            try:
                _log_structured(logging.INFO, "Attempting to deploy model.", model_id=model_version.model_id, version=model_version.version)

                new_prioritization_weights = model_version.evaluation_metrics.get("new_prioritization_weights")
                if new_prioritization_weights:
                    success = await self.agent_config_service.update_prioritization_weights(new_prioritization_weights, model_version.version)
                    if not success:
                        raise ModelDeploymentError("Failed to update prioritization weights.")
                    _log_structured(logging.INFO, "Prioritization weights updated.", model_id=model_version.model_id)

                new_policy_rules = model_version.evaluation_metrics.get("new_policy_rules")
                if new_policy_rules:
                    success = await self.agent_config_service.update_policy_rules(new_policy_rules, model_version.version)
                    if not success:
                        raise ModelDeploymentError("Failed to update policy rules.")
                    _log_structured(logging.INFO, "Policy rules updated.", model_id=model_version.model_id)

                if model_version.evaluation_metrics.get("is_rl_policy", False):
                    success = await self.agent_config_service.update_rl_policy(model_version.model_id, model_version.version)
                    if not success:
                        raise ModelDeploymentError("Failed to deploy RL policy.")
                    _log_structured(logging.INFO, "RL policy deployed.", model_id=model_version.model_id)

                ML_DEPLOYMENT_SUCCESS_COUNT.inc()
                _log_structured(logging.INFO, "Model successfully deployed.", model_id=model_version.model_id, version=model_version.version)
                
            except Exception as e:
                ML_DEPLOYMENT_FAILURE_COUNT.inc()
                _log_structured(logging.ERROR, "Deployment failed for model.", model_id=model_version.model_id, error=str(e), exc_info=True)
    ...
    async def trigger_model_training_and_deployment(self, data_location: str) -> Optional[ModelVersion]:
        """
        ...
        with trace.get_tracer(__name__).start_as_current_span("trainer_full_cycle"):
            ...
            try:
                ...
                _log_structured(logging.INFO, "ML training job triggered on platform.", job_id=job_id)
                ...
                while status_result["status"] == "running":
                    ...
                    _log_structured(logging.DEBUG, "Polling training job status.", job_id=job_id, status=status_result['status'])

                if status_result["status"] != "completed":
                    ML_TRAINING_FAILURE_COUNT.inc()
                    _log_structured(logging.ERROR, "ML training job failed.", job_id=job_id, error=status_result.get('error', 'Unknown error'))
                    return None
                ...
                ML_TRAINING_FAILURE_COUNT.inc()
                _log_structured(logging.ERROR, "Error during training/deployment cycle.", error=str(e), exc_info=True)
                ML_ORCHESTRATOR_ERRORS.inc()
                return None
            finally:
...
class MetaLearningOrchestrator:
    ...
    def __init__(
        ...
        self.ml_platform_client = ml_platform_client or MLPlatformClient(self.config.ML_PLATFORM_ENDPOINT)
        self.agent_config_service = agent_config_service or AgentConfigurationService(self.config.AGENT_CONFIG_SERVICE_ENDPOINT)
        ...
        _log_structured(logging.INFO, "MetaLearningOrchestrator initialized.", instance_id=self._instance_id)
    ...
    def _validate_local_dir(self, path: str):
        ...
        if not os.path.exists(path):
            try:
                ...
                _log_structured(logging.CRITICAL, "Failed to create local directory. Check permissions.", path=path, error=str(e), exc_info=True)
                raise SystemExit(f"Startup failed: Cannot create directory {path}") from e
        elif not os.access(path, os.W_OK):
            _log_structured(logging.CRITICAL, "Local directory is not writable. Check permissions.", path=path)
    ...
    async def start(self):
        ...
        if not self.redis_client:
            try:
                ...
                _log_structured(logging.CRITICAL, "Failed to connect to Redis for leader election. HA features will be disabled.", error=str(e), exc_info=True)
        ...
        for task in tasks_to_cancel:
            if task and not task.done():
                ...
                try:
                    ...
                    _log_structured(logging.ERROR, "Timeout waiting for task to cancel.", task_name=task.get_name())
                except Exception as e:
                    _log_structured(logging.ERROR, "Error while waiting for task to stop.", task_name=task.get_name(), error=str(e), exc_info=True)
    ...
    async def _run_periodic_leader_task(self, coro: Callable[[], Awaitable], task_name: str, interval_seconds: int):
        """A generic supervised loop for leader-only tasks."""
        while self._running:
            try:
                if self._is_leader:
                    if not await self._verify_leadership_and_fencing():
                        _log_structured(logging.WARNING, "Lost leadership during task execution. Stepping down.",
                                        instance_id=self._instance_id, task_name=task_name)
                        continue # Leadership will be re-evaluated by the main leader election loop.
                    
                    await coro()
                else:
                    _log_structured(logging.DEBUG, "Follower instance skipping leader task.",
                                    instance_id=self._instance_id, task_name=task_name)
            except asyncio.CancelledError:
                _log_structured(logging.INFO, "Periodic leader task cancelled.", task_name=task_name, instance_id=self._instance_id)
                break
            except Exception as e:
                _log_structured(logging.ERROR, "Error during periodic leader task.", task_name=task_name, error=str(e), exc_info=True)
                ML_ORCHESTRATOR_ERRORS.inc()
            finally:
            ...
            try:
                if self.redis_client:
                    ...
                    if acquired and not self._is_leader:
                        ...
                        _log_structured(logging.DEBUG, "Instance is a FOLLOWER.", instance_id=self._instance_id, lock_holder=holder_id)
                else:
                    # Non-HA mode: always become leader if not already.
                    if not self._is_leader:
                        await self._become_leader({"instance_id": self._instance_id, "token": "non-ha-mode"})
                        _log_structured(logging.WARNING, "Redis not available. Running in non-HA mode (always leader).", instance_id=self._instance_id)
            except asyncio.CancelledError:
                break
            except Exception as e:
                _log_structured(logging.ERROR, "Error during leader election cycle.", error=str(e), exc_info=True)
                ML_ORCHESTRATOR_ERRORS.inc()
                if self._is_leader:
    ...
    async def _become_leader(self, lock_info: Dict[str, Any]):
        ...
        if not self._training_check_task or self._training_check_task.done():
            self._training_check_task = asyncio.create_task(
                create_task_with_supervision(self._run_periodic_leader_task(self._training_check_core, "training_check_loop", self.config.TRAINING_CHECK_INTERVAL_SECONDS), "training_check_loop")
            )
        if not self._data_cleanup_task or self._data_cleanup_task.done():
            self._data_cleanup_task = asyncio.create_task(
                create_task_with_supervision(self._run_periodic_leader_task(self._data_cleanup_core, "data_cleanup_loop", 24 * 3600), "data_cleanup_loop")
            )
    ...
    async def _acquire_leader_lock(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        fencing_token = int(current_time * 1000) # High-resolution timestamp as a simple fencing token.
        ...
        is_acquired = await self.redis_client.set(
            self.config.REDIS_LOCK_KEY, lock_value_str,
            nx=True, ex=self.config.REDIS_LOCK_TTL_SECONDS
        )
        ...
        if is_acquired:
            ...
            if current_holder_str:
                try:
                    ...
                    _log_structured(logging.ERROR, "Failed to decode Redis lock value. Possible corruption.", raw_value=current_holder_str)
    ...
    async def _verify_leadership_and_fencing(self) -> bool:
        ...
        try:
            ...
            if (current_lock_value.get("instance_id") == self._instance_id and
                current_lock_value.get("token") == self._fencing_token):
                # We are still the rightful leader.
                return True
            else:
                _log_structured(logging.WARNING, "Fencing token mismatch or new leader elected. Lost leadership.",
                                instance_id=self._instance_id, my_token=self._fencing_token, current_lock_value=current_lock_value)
                await self._step_down_leadership("fencing_token_mismatch")
                return False
        except (json.JSONDecodeError, KeyError) as e:
            _log_structured(logging.ERROR, "Corrupted lock value in Redis. Stepping down to be safe.", error=str(e))
            await self._step_down_leadership("corrupted_lock_value")
            return False
        except Exception as e:
            _log_structured(logging.ERROR, "Error verifying leadership. Stepping down to be safe.", error=str(e), exc_info=True)
    ...
    async def _get_local_file_records_count(self) -> int:
        ...
        try:
            ...
            _log_structured(logging.ERROR, "Failed to count records in local file.", path=path, error=str(e), exc_info=True)
            ML_ORCHESTRATOR_ERRORS.inc()
        return count
    
    async def _get_kafka_new_records_count(self) -> int:
        """Gets the count of new records in the Kafka topic by checking offsets."""
        if not self.config.USE_KAFKA_INGESTION: return 0
        consumer = None
        try:
            ...
            _log_structured(logging.ERROR, "Failed to get Kafka new records count.", topic=self.config.KAFKA_TOPIC, error=str(e), exc_info=True)
            ML_ORCHESTRATOR_ERRORS.inc()
    ...
    async def _training_check_core(self):
        ...
        if current_records_count >= self.config.MIN_RECORDS_FOR_TRAINING:
            _log_structured(logging.INFO, "Threshold met. Triggering model training.",
                            instance_id=self._instance_id, record_count=current_records_count,
            ...
            if new_model and new_model.deployment_status == "deployed":
                if self._current_active_model:
                    self._current_active_model = self._current_active_model.model_copy(update={"is_active": False})
                ...
                try:
                    ...
                    _log_structured(logging.WARNING, "Could not convert model version to numeric for gauge.", version=new_model.version)

    async def _data_cleanup_core(self):
        """Core logic for the periodic data cleanup task, executed only by the leader."""
        _log_structured(logging.INFO, "Initiating data cleanup.",
                        instance_id=self._instance_id, retention_days=self.config.DATA_RETENTION_DAYS)
        
        if self.config.USE_S3_DATA_LAKE:
            await self._cleanup_s3_data_lake()
        elif self.config.USE_KAFKA_INGESTION:
            _log_structured(logging.INFO, "Kafka topic retention is managed by Kafka broker configuration (log.retention.ms). No direct cleanup performed by orchestrator.")
        else:
    ...
    async def _cleanup_local_data_lake(self):
        ...
        try:
            async with aiofiles.open(self.config.DATA_LAKE_PATH, "r") as infile, \
                ...
                async for line in infile:
                    try:
                        ...
                        _log_structured(logging.WARNING, "Skipping corrupted line during cleanup.", line=line.strip())
            
            # Atomic move operation
            await asyncio.to_thread(os.replace, temp_path, self.config.DATA_LAKE_PATH)
            self._new_records_count = retained_count
            ML_DATA_QUEUE_SIZE.set(self._new_records_count)
            _log_structured(logging.INFO, "Local data cleanup complete.", retained_records=retained_count)
        except Exception as e:
            _log_structured(logging.ERROR, "Error during local data cleanup.", error=str(e), exc_info=True)
            ML_ORCHESTRATOR_ERRORS.inc()
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        is_healthy = self._running and self._leader_election_task and not self._leader_election_task.done()
        ...
        status = {
            ...
            "tasks_status": {
                "leader_election": "running" if self._leader_election_task and not self._leader_election_task.done() else "stopped",
                "training_check": "running" if self._training_check_task and not self._training_check_task.done() else "stopped",
                "data_cleanup": "running" if self._data_cleanup_task and not self._data_cleanup_task.done() else "stopped",
            },
            "new_records_count": self._new_records_count,
            "current_active_model": self._current_active_model.model_dump() if self._current_active_model else None
        }
        
        # Check external dependencies with a short timeout
        try:
            if self.redis_client: await asyncio.wait_for(self.redis_client.ping(), timeout=1.0)
            status["redis_connected"] = True
        except Exception: status["redis_connected"] = False
        
        status["kafka_connected"] = bool(self.ingestor.kafka_producer and self.ingestor.kafka_producer.bootstrap_connected())
    ...
    async def is_ready(self) -> bool:
        ...
        try:
            ...
            if not all(task and not task.done() for task in [self._leader_election_task, self._training_check_task]):
                return False
            ...
            if self.config.USE_KAFKA_INGESTION and not (self.ingestor.kafka_producer and self.ingestor.kafka_producer.bootstrap_connected()):
                raise Exception("Kafka producer is not connected.")

            if self.config.USE_S3_DATA_LAKE:
                async with self.ingestor.s3_session.client("s3") as s3:
                    await asyncio.wait_for(s3.head_bucket(Bucket=self.config.DATA_LAKE_S3_BUCKET), timeout=timeout)
            
            # Check dependent microservices are healthy.
            async with aiohttp.ClientSession() as session:
                ml_resp = await asyncio.wait_for(session.get(f"{self.ml_platform_client.endpoint}/health"), timeout=timeout)
                ml_resp.raise_for_status()
                if (await ml_resp.json()).get("status") != "healthy":
                    raise Exception("Dependency 'ML Platform' is unhealthy.")

                agent_resp = await asyncio.wait_for(session.get(f"{self.agent_config_service.endpoint}/health"), timeout=timeout)
