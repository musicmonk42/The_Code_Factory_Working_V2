...
async def agent_client(mock_session):
    ...
    client = AgentConfigurationService(endpoint="http://mock-agent-config.com", session=mock_session)
    yield client
    if hasattr(client, 'close'):
        await client.close()
...
async def test_pii_redaction(ml_client, mocker: MockerFixture):
    ...
    data = {"data_path": "/path", "params": {"email": "test@example.com", "password": "secret", "user_id": "user123"}}
    ...
    mocker.patch("arbiter.meta_learning_orchestrator.clients.PIIRedactorFilter", return_value=mock_redactor_instance)
...
async def test_timeout_handling(ml_client, mocker: MockerFixture, caplog):
    ...
    mocker.patch.object(ml_client.session, "request", side_effect=asyncio.TimeoutError("Connection timed out"))
