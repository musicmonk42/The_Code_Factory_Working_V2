...
@pytest.mark.parametrize("metric_class, name, doc, labelnames, buckets", [
...
@pytest.mark.parametrize("metric_class, name, doc", [
...
def test_metric_registry_global_labels(metric_registry):
    ...
    counter = metric_registry.get_or_create(Counter, "test_counter_with_labels", "Test counter", ("event_type",))
...
def test_metric_operations(metric_idx):
    ...
    assert expected_label_string in metrics, f"Expected {expected_label_string} not found in metrics"
...
def test_metric_registry_thread_safety(metric_registry, mocker: MockerFixture):
    ...
    for i in range(10):
        metric = metric_registry.get_or_create(Counter, f"test_thread_counter_{i}", f"Test counter {i}")
...
def test_invalid_label_names(metric_registry, caplog):
    ...
    counter = metric_registry.get_or_create(Counter, "test_valid_label_counter", "Test counter", ("valid_label",))
...
def test_metrics_exposition_format():
    ...
    from arbiter.meta_learning_orchestrator.metrics import (
        ML_INGESTION_COUNT, ML_AUDIT_EVENTS_TOTAL, 
        ML_CURRENT_MODEL_VERSION, ML_EVALUATION_LATENCY
    )
    ...
    assert metrics_after[ingestion_key] == initial_ingestion + 2.0, f"Expected {initial_ingestion + 2.0}, got {metrics_after[ingestion_key]}"
    
    # Check other metrics exist with expected values
    assert 'ml_audit_events_total{cluster="test-cluster",environment="test",event_type="test_event"}' in metrics_after
    assert metrics_after['ml_audit_events_total{cluster="test-cluster",environment="test",event_type="test_event"}'] >= 1.0
    
    assert 'ml_current_model_version{cluster="test-cluster",environment="test"}' in metrics_after
    assert metrics_after['ml_current_model_version{cluster="test-cluster",environment="test"}'] == 1.5
    
    assert 'ml_evaluation_latency_seconds_sum{cluster="test-cluster",environment="test"}' in metrics_after