...
def logger_with_filters(caplog, mocker: MockerFixture):
    ...
    mocker.patch.dict(
        os.environ,
        {
            "PII_SENSITIVE_KEYS": "custom_key1,custom_key2,user_id,email,ip_address,phone_number,session_id,agent_id"
        },
    )
...
@pytest.mark.asyncio
async def test_log_correlation_filter_invalid_span(
...
async def test_pii_redaction_filter_msg_string(logger_with_filters, caplog):
    ...
    test_logger, _ = logger_with_filters
    test_logger.info(
        "User agent_id: agent-123, email: test@example.com, ip_address: 192.168.1.1, phone: 555-123-4567"
    )
    assert len(caplog.records) > 0
...
async def test_pii_redaction_filter_args_tuple(logger_with_filters, caplog):
    ...
    test_logger.info(
        "Log with args: %s, %s",
        {"session_id": "sess-abc", "ip_address": "10.0.0.1"},
