...
async def orchestrator(mocker: MockerFixture, tmp_path):
    ...
    mock_ml_client.get_training_job_status = mocker.AsyncMock(side_effect=[
        {"status": "running"},
        {"status": "running"},
        {"status": "completed", "model_id": "mock_model", "metrics": {"accuracy": 0.95, "new_policy_rules": {"rule": "new_rule"}}}
    ])
    ...
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.Ingestor.ingest_learning_record", 
    ...
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.Trainer.trigger_model_training_and_deployment", 
                 mocker.AsyncMock(return_value=mock_trainer_result))
    
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.AuditUtils.add_audit_event", mocker.AsyncMock())
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.AuditUtils.validate_audit_chain", mocker.AsyncMock(
        return_value={"is_valid": True, "total_events": 10, "message": "Audit chain valid."}
    ))
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.MetaLearningOrchestrator._cleanup_s3_data_lake", mocker.AsyncMock())
    # Don't mock _cleanup_local_data_lake for the cleanup test
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.MetaLearningOrchestrator._acquire_leader_lock", mocker.AsyncMock(
        return_value=(True, {"instance_id": str(uuid.uuid4()), "token": 12345})
    ))
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.MetaLearningOrchestrator._verify_leadership_and_fencing", mocker.AsyncMock(
        return_value=True
    ))
    
    # Mock the background task creation to prevent infinite loops
    mocker.patch("arbiter.meta_learning_orchestrator.orchestrator.create_task_with_supervision", 
