...
async def test_config_secure_mode_enforces_keys(mocker: MockerFixture, tmp_path):
    ...
    for env_key, field_name in test_cases:
        mocker.patch.dict(os.environ, {
            ...
            "ML_AUDIT_ENCRYPTION_KEY": Fernet.generate_key().decode() if env_key != "ML_AUDIT_ENCRYPTION_KEY" else "",
            "ML_AUDIT_SIGNING_PRIVATE_KEY": "key" if env_key != "ML_AUDIT_SIGNING_PRIVATE_KEY" else "",
            "ML_AUDIT_SIGNING_PUBLIC_KEY": "key" if env_key != "ML_AUDIT_SIGNING_PUBLIC_KEY" else "",
        })
