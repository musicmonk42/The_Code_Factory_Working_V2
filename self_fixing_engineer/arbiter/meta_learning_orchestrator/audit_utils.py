...
ML_AUDIT_EVENTS_TOTAL = Counter(
    "ml_audit_events_total", "Total number of audit events logged.", ["event_type"]
)
...
class AuditUtils:
    ...
    def __init__(
        ...
        try:
            ...
            if not self.private_key:
                logger.critical(
                    "AUDIT_SIGNING_PRIVATE_KEY missing. Logs will be unsigned—a major security risk!"
                )
            if not self.public_key:
                logger.warning(
                    "AUDIT_SIGNING_PUBLIC_KEY missing. Signature verification will not be possible."
                )
        ...
        if USE_KAFKA_AUDIT:
            try:
                ...
                self.kafka_producer = AIOKafkaProducer(
                    loop=self.loop, bootstrap_servers=KAFKA_BROKERS
                )
                ...
                logger.error(
                    f"Failed to connect to Kafka: {e}. Falling back to file-based logging.",
                    exc_info=True,
                )
                self.kafka_producer = None
    ...
    async def _get_last_hash(self) -> str:
        ...
        if self.kafka_producer:
            try:
                ...
                if not consumer.partitions_for_topic(
                    KAFKA_TOPIC
                ) or tp not in await consumer.end_offsets([tp]):
                    await consumer.stop()
                    return "genesis_hash"
    ...
    def _verify_signature(self, digest: bytes, signature: str) -> bool:
        ...
        try:
            self.public_key.verify(
                bytes.fromhex(signature), digest, ec.ECDSA(Prehashed(hashes.SHA256()))
            )
            return True
                    ...
                    try:
                        ...
                        if (
                            ...
                            report["mismatches"].append(
                                {"line": line_number, "type": "hash_mismatch"}
                            )
                        ...
                        if not self._verify_signature(
                            ...
                            report["mismatches"].append(
                                {"line": line_number, "type": "signature_mismatch"}
                            )
    ...
    async def _validate_kafka_chain(self) -> Dict[str, Any]:
        ...
        try:
            ...
            report["mismatches"].append(
                {"offset": offset, "type": "processing_error", "error": str(e)}
            )
            report["is_valid"] = False
        finally:
            await consumer.stop()
