...
class CircuitBreakerListener:
    ...
    def success(self, cb):
        ...
        logger.debug(f"Circuit breaker '{cb.name}' call succeeded for arbiter '{self.arbiter_name}'")
    
    def failure(self, cb, exc):
        """Called when a circuit breaker protected call fails."""
        logger.warning(f"Circuit breaker '{cb.name}' recorded failure for arbiter '{self.arbiter_name}': {exc}")
    
    def state_change(self, cb, old_state, new_state):
        """Called when the circuit breaker changes state."""
        logger.warning(f"Circuit breaker '{cb.name}' for arbiter '{self.arbiter_name}' changed state from {old_state} to {new_state}")
        if str(new_state) == 'open':
            GROWTH_CIRCUIT_BREAKER_TRIPS.labels(arbiter=self.arbiter_name, breaker_name=cb.name).inc()
...
class Neo4jKnowledgeGraph:
    ...
    async def add_fact(self, arbiter_id: str, event_type: str, event_details: Dict[str, Any]) -> None:
        ...
        logger.debug(f"[KnowledgeGraph] Added fact for {arbiter_id}: {event_type} -> {event_details.get('skill_name')}")
...
class LoggingFeedbackManager:
    ...
    async def record_feedback(self, arbiter_id: str, event_type: str, event_details: Dict[str, Any]) -> None:
...
class ArbiterGrowthManager:
    ...
    def __init__(
        ...
        self._idempotency_salt = self.config_store.get("security.idempotency_salt", os.urandom(16).hex())
        ...
        self._snapshot_breaker = CircuitBreaker(fail_max=5, reset_timeout=60, name=f"{self.arbiter}_snapshot")
        self._push_event_breaker = CircuitBreaker(fail_max=10, reset_timeout=30, name=f"{self.arbiter}_push_event")
    ...
    async def stop(self) -> None:
        ...
        tasks = [self._flush_task, self._evolution_task, self._load_task, self._process_ops_task]
        for task in tasks:
        ...
        logger.info(f"Processing {self._pending_operations.qsize()} remaining operations before shutdown...")
    ...
    async def _periodic_flush(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.debug(f"Arbiter '{self.arbiter}' queue size: {self._pending_operations.qsize()}")
                GROWTH_PENDING_QUEUE.labels(arbiter=self.arbiter).set(self._pending_operations.qsize())
        ...
        with tracer.start_as_current_span("validate_audit_chain"):
            ...
            for log in all_logs:
                ...
                if log["previous_log_hash"] != last_hash:
                    raise AuditChainTamperedError("Hash chain mismatch.", details={"log_timestamp": log["timestamp"]})
                ...
                if log["log_hash"] != recalculated_hash:
                    raise AuditChainTamperedError("Log entry hash is corrupt.", details={"log_timestamp": log["timestamp"]})

                # 3. Verify timestamps are monotonic
                current_timestamp = datetime.fromisoformat(log["timestamp"])
                if current_timestamp < last_timestamp:
                    raise AuditChainTamperedError("Timestamp out of order.", details={"log_timestamp": log["timestamp"]})
    ...
    def _recalculate_log_hash(self, log: Dict[str, Any]) -> str:
        ...
        details_str = json.dumps(log["details"], sort_keys=True) if isinstance(log["details"], dict) else log["details"]
        return _create_hmac_hash(key, log['arbiter_id'], log['operation'], log['timestamp'], details_str, log['previous_log_hash'])
    ...
    async def _load_state_and_replay_events(self) -> None:
        ...
        with tracer.start_as_current_span("load_state_and_replay_events"):
            ...
            events_to_replay = await self.storage_backend.load_events(self.arbiter, from_offset=self._state.event_offset)
            ...
            for event_dict in events_to_replay:
                try:
                    ...
                    self._state.event_offset = event_dict.get("canonical_offset", self._state.event_offset)
                except Exception as e:
                    # Skip invalid or corrupt events during replay but log them.
                    logger.error(f"Skipping invalid event during replay for '{self.arbiter}': {event_dict}. Error: {e}")
    ...
    async def _apply_event(self, event: GrowthEvent, is_replay: bool = False) -> None:
        ...
        with tracer.start_as_current_span(f"apply_event_{event.type}"):
            ...
            # Call before-hooks
            if not is_replay:
                for hook in self._before_hooks: await hook.on_growth_event(event, self._state)
            ...
            if event.type == "skill_improved":
                ...
                if not is_replay:
                    GROWTH_SKILL_IMPROVEMENT.labels(arbiter=self.arbiter, skill=skill_name).observe(amount)
                    ...
                    if amount > anomaly_threshold:
                        GROWTH_ANOMALY_SCORE.labels(arbiter=self.arbiter, event_type=event.type).set(amount)
                        logger.warning(f"Anomaly detected: Large skill improvement of {amount} for {skill_name}.")
            ...
            # Call after-hooks
            if not is_replay:
                for hook in self._after_hooks: await hook.on_growth_event(event, self._state)
    ...
    async def _save_if_dirty(self, force: bool = False) -> None:
        ...
        snapshot_interval = await self.config_store.get_config("arbiter.snapshot_interval_events", 100)
        ...
        try:
            ...
            async def _push():
                for event in events:
                    ...
                    await self.feedback_manager.record_feedback(self.arbiter, event.type, event.details)
    ...
    async def record_growth_event(self, event_type: str, details: Dict[str, Any]) -> None:
        ...
        with tracer.start_as_current_span("record_growth_event_api"):
            async def operation():
                ...
                if not is_new_event:
                    logger.info(f"Skipping duplicate event due to idempotency key: {idempotency_key}")
                    return
    ...
    async def improve_skill(self, skill_name: str, improvement_amount: float = 0.01) -> None:
        ...
        await self.record_growth_event("skill_improved", {"skill_name": skill_name, "improvement_amount": improvement_amount})

    async def level_up(self) -> None:
        """A convenience API to record a level up event."""
        await self.record_growth_event("level_up", {"new_level": self._state.level + 1, "old_level": self._state.level})
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        """
        uptime = (self.clock() - self._start_time).total_seconds() if hasattr(self, '_start_time') else 0
