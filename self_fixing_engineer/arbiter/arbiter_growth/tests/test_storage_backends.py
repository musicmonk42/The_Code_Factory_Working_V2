...
async def test_redis_save_and_load_snapshot(redis_backend, mock_redis_client):
    ...
    mock_redis_client.hgetall.return_value = {
        b'level': b'3',
        b'skills_encrypted': redis_backend.cipher.encrypt(json.dumps({"python": 0.7}).encode('utf-8')),
...
def test_storage_backend_factory_sqlite(mock_config_store):
    ...
    mock_config_store.get.side_effect = lambda k, d=None: "sqlite" if k == "storage.backend" else d
...
async def test_redis_stream_operations(redis_backend, mock_redis_client):
    ...
    # Create events
    events = [
        GrowthEvent(type="learning", timestamp="2024-01-01T00:00:00+00:00", details={"skill_name": "python"}),
        GrowthEvent(type="achievement", timestamp="2024-01-01T00:01:00+00:00", details={"skill_name": "rust"})
    ]
    ...
    with patch('sqlalchemy.ext.asyncio.AsyncSession.commit', side_effect=SQLAlchemyError("DB Error")):
        with caplog.at_level(logging.ERROR):
            with pytest.raises(ArbiterGrowthError):
                await sqlite_backend.save_snapshot("test", ArbiterState(arbiter_id="test").model_dump())
...
async def test_redis_consumer_group(redis_backend, mock_redis_client):
    ...
    event_payload = {"type": "test", "timestamp": "2024-01-01T00:00:00+00:00", "details": {}}
...
async def test_kafka_offset_management(kafka_backend, mock_kafka_producer):
    ...
    async def mock_load_events(*args, **kwargs):
        encrypted_payload = kafka_backend.cipher.encrypt(json.dumps({
            "event_offset": 0,
            "type": "test",
            "timestamp": "2024-01-01T00:00:00+00:00",
            "details": {}
        ...
        return [{"event_offset": 0, "type": "test", "timestamp": "2024-01-01T00:00:00+00:00", "details": {}}]

    with patch.object(kafka_backend, 'load_events', side_effect=mock_load_events):
        await kafka_backend.save_event("test_arbiter", {"type": "test", "timestamp": "2024-01-01T00:00:00+00:00", "details": {}})
