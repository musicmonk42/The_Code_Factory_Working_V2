...
from arbiter.arbiter_growth.idempotency import IdempotencyStore, IdempotencyStoreError, IDEMPOTENCY_HITS_TOTAL
...
async def test_start_idempotent(set_env_redis_url):
    ...
    with patch('arbiter.arbiter_growth.idempotency.redis.from_url', return_value=mock_redis_client) as mock_from_url:
        await store.start()
        await store.start()
