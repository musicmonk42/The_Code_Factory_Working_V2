...
def mock_config_store():
    ...
    def get_side_effect(key, default=None):
        configs = {
            ...
            "metrics.growth_skill_improvement_value.buckets": [0.1, 0.5, 1.0, 5.0, 10.0, float('inf')],
        }
...
def test_concurrent_metric_creation(isolated_registry):
    ...
    def create_metric():
        metric = get_or_create_metric(
            Counter, "concurrent_metric", "Concurrent metric", registry=isolated_registry
        )
...
def test_metric_usage_integration(isolated_registry):
    ...
    # Verify the values
    assert isolated_registry.get_sample_value("test_counter_total", {"status": "success", "method": "GET"}) == 1.0
    assert isolated_registry.get_sample_value("test_counter_total", {"status": "success", "method": "POST"}) == 2.0
    assert isolated_registry.get_sample_value("test_counter_total", {"status": "error", "method": "GET"}) == 1.0
...
def test_metric_labels(isolated_registry):
    ...
    metric = get_or_create_metric(
        Counter, "test_counter_labels", "Test counter with labels", labelnames=("arbiter",), registry=isolated_registry
    )
    metric.labels(arbiter="test_instance").inc()
    assert isolated_registry.get_sample_value("test_counter_labels_total", {"arbiter": "test_instance"}) == 1
...
async def test_concurrent_metric_updates(isolated_registry):
    ...
    counter = get_or_create_metric(
        Counter, "concurrent_counter", "Test concurrent counter", registry=isolated_registry
    )
