...
async def test_anomaly_detection_sets_metric(manager_factory):
    ...
    await manager.record_growth_event(
        "skill_improved",
        {"skill_name": "test", "improvement_amount": 10.0}
    )
...
@pytest.mark.asyncio
async def test_snapshot_interval(manager_factory, mock_storage_backend, mock_clock, mock_config_store):
    """Test that a snapshot is saved after the configured number of events."""
    manager = manager_factory()
    
    # Mock config to ensure proper rate limiting values
    mock_config_store.get_config = AsyncMock(side_effect=lambda key, default=None: {
