...
class GrowthEvent(BaseModel):
    ...
    event_version: float = Field(
        1.0,
        description="The schema version of the event payload."
    )
...
class ArbiterState(BaseModel):
    ...
    arbiter_id: str = Field(
        ...,
        description="The unique identifier for the arbiter."
    )
    level: int = Field(
        1,
        ge=1,
        description="The current level of the arbiter."
    )
    ...
    schema_version: float = Field(
        1.0,
        description="The schema version of this state object."
    )
    experience_points: float = Field(
        0.0,
        description="The total experience points accumulated by the arbiter."
    )
    ...
    def validate_skill_scores(cls, v: Dict[str, float]) -> Dict[str, float]:
        ...
        for skill, score in v.items():
            if not 0.0 <= score <= 1.0:
                raise ValueError(f"Skill '{skill}' has an invalid score: {score}. Must be between 0.0 and 1.0.")
        return v
...
class GrowthSnapshot(Base):
    ...
    level = Column(Integer, server_default='1', nullable=False, comment="The arbiter's level at the time of the snapshot.")
    skills_encrypted = Column(Text, comment="Encrypted JSON dictionary of skill scores.")
    user_preferences_encrypted = Column(Text, comment="Encrypted JSON dictionary of user preferences.")
    experience_points = Column(Float, server_default='0.0', nullable=False, comment="The arbiter's XP at the time of the snapshot.")
    schema_version = Column(Float, server_default='1.0', nullable=False, comment="The schema version of the snapshotted state.")
    event_offset = Column(String, server_default='0', nullable=False, comment="The event offset corresponding to this state snapshot.")
...
class GrowthEventRecord(Base):
    ...
    arbiter_id = Column(String, index=True, nullable=False, comment="The arbiter this event belongs to.")
    ...
    event_version = Column(Float, server_default='1.0', nullable=False, comment="The schema version of the event.")

    __table_args__ = (
        Index('idx_arbiter_growth_events_arbiter_id', 'arbiter_id'),
    )
...
class AuditLog(Base):
    ...
    id = Column(Integer, primary_key=True)
    arbiter_id = Column(String, index=True, nullable=False, comment="The arbiter this log entry belongs to.")
    operation = Column(String, nullable=False, comment="The operation being audited (e.g., 'event_recorded').")
    timestamp = Column(String, nullable=False, comment="The ISO 8601 timestamp of the audited operation.")
    details = Column(Text, comment="JSON string containing details of the operation.")
    previous_log_hash = Column(String, nullable=False, comment="The hash of the preceding log entry in the chain.")
    log_hash = Column(String, nullable=False, unique=True, comment="The hash of this log entry, creating the chain link.")
