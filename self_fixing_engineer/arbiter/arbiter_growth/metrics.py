        ...
        if existing_metric:
            ...
            # If the type is wrong, unregister the old one before creating a new one
                ...
                try:
                    ...
                    logger.error(f"Failed to unregister conflicting metric '{name}': {e}", exc_info=True)
...
GROWTH_EVENTS = get_or_create_metric(
    Counter, "growth_events_total", "Total number of growth events recorded.", ("arbiter",)
)
GROWTH_SAVE_ERRORS = get_or_create_metric(
    Counter, "growth_save_errors_total", "Total number of errors encountered while saving arbiter state.", ("arbiter",)
)
GROWTH_PENDING_QUEUE = get_or_create_metric(
    Gauge, "growth_pending_queue_size", "The current size of the pending operations queue.", ("arbiter",)
)
GROWTH_SKILL_IMPROVEMENT = get_or_create_metric(
    Histogram, "growth_skill_improvement_value", "Distribution of skill improvement amounts.", ("arbiter", "skill")
)
GROWTH_SNAPSHOTS = get_or_create_metric(
    Counter, "growth_snapshots_total", "Total number of state snapshots created.", ("arbiter",)
)
GROWTH_CIRCUIT_BREAKER_TRIPS = get_or_create_metric(
    Counter, "growth_circuit_breaker_trips_total", "Total number of times a circuit breaker has tripped (opened).", ("arbiter", "breaker_name")
)
GROWTH_ANOMALY_SCORE = get_or_create_metric(
    Gauge, "growth_anomaly_score", "A score indicating the anomalousness of a growth event.", ("arbiter", "event_type")
)

# --- Latency Metrics ---

GROWTH_EVENT_PUSH_LATENCY = get_or_create_metric(
    Histogram, "growth_event_push_latency_seconds", "Latency of pushing an event to external systems (e.g., Knowledge Graph).", ("arbiter",)
)
GROWTH_OPERATION_QUEUE_LATENCY = get_or_create_metric(
    Histogram, "growth_operation_queue_latency_seconds", "Time an operation spends in the pending queue before execution.", ("arbiter",)
)
GROWTH_OPERATION_EXECUTION_LATENCY = get_or_create_metric(
    Histogram, "growth_operation_execution_latency_seconds", "Time it takes to execute a queued operation.", ("arbiter",)
)
STORAGE_LATENCY_SECONDS = get_or_create_metric(
    Histogram, "storage_latency_seconds", "Latency of storage backend operations.", ("backend", "operation")
)

# --- Security and Auditing Metrics ---

AUDIT_VALIDATION_ERRORS_TOTAL = get_or_create_metric(
    Counter, "audit_validation_errors_total", "Total number of audit chain validation failures.", ("arbiter",)
)
GROWTH_AUDIT_ANCHORS_TOTAL = get_or_create_metric(
    Counter, "growth_audit_anchors_total", "Total number of audit chain hashes anchored to an external ledger.", ("arbiter",)
)

# --- Idempotency and Rate Limiting Metrics ---

IDEMPOTENCY_HITS_TOTAL = get_or_create_metric(
    Counter, "idempotency_hits_total", "Total number of idempotency check hits and misses.", ("arbiter", "hit")
)
RATE_LIMIT_REJECTIONS_TOTAL = get_or_create_metric(
    Counter, "rate_limit_rejections_total", "Total number of operations rejected due to rate limiting.", ("arbiter",)
)

# --- Configuration Metrics ---

CONFIG_FALLBACK_USED = get_or_create_metric(
    Counter, "config_fallback_used_total", "Total number of times a fallback default config was used.", ("config_key",)
)