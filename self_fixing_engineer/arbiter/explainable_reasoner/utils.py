...
try:
    ...
    _utils_logger.warning("Warning: arbiter.models.multi_modal_schemas not found. Using dummy MultiModalData/Schemas for standalone mode in utils.")
    ...
    class MultiModalData(BaseModel):
        ...
        def dict(self, exclude_unset=False) -> Dict[str, Any]:
            ...
            return {"data_type": self.data_type, "data_preview": data_snippet, "metadata": self.metadata}
...
def _format_multimodal_for_prompt(data: Union[ImageAnalysisResult, AudioAnalysisResult, VideoAnalysisResult]) -> str:
    """
    ...
    if isinstance(data, ImageAnalysisResult):
        if hasattr(data, 'image_id'): parts.append(f"--- Image Analysis (ID: {data.image_id}) ---")
        if data.captioning_result and hasattr(data.captioning_result, 'caption') and data.captioning_result.caption:
        ...
        if data.ocr_result and hasattr(data.ocr_result, 'text') and data.ocr_result.text:
            truncated_ocr = data.ocr_result.text[:500] + "..." if len(data.ocr_result.text) > 500 else data.ocr_result.text
        ...
        if hasattr(data, 'audio_id'): parts.append(f"--- Audio Analysis (ID: {data.audio_id}) ---")
        if data.transcription and hasattr(data.transcription, 'text') and data.transcription.text:
            truncated_transcript = data.transcription.text[:1000] + "..." if len(data.transcription.text) > 1000 else data.transcription.text
        ...
        if hasattr(data, 'video_id'): parts.append(f"--- Video Analysis (ID: {data.video_id}) ---")
...
async def _sanitize_context(context: Dict[str, Any], config: ReasonerConfig) -> Dict[str, Any]:
    ...
    compiled_patterns = [re.compile(p, re.IGNORECASE) for p in redact_patterns + default_redact_patterns]
    ...
    def _json_serializable_converter(obj: Any, current_depth: int = 0, visited: set = None) -> Any:
        ...
        if current_depth >= config.sanitization_options.get("max_nesting_depth", 10):
            _utils_logger.warning(f"Max context nesting depth ({config.sanitization_options.get('max_nesting_depth', 10)}) exceeded.")
            METRICS["context_validation_errors"].labels(error_code=ReasonerErrorCode.CONTEXT_MAX_DEPTH_EXCEEDED).inc()
            return "[MAX_DEPTH_EXCEEDED]"

        new_visited = visited | {id(obj)}
        allowed_types = config.sanitization_options.get("allowed_primitive_types", (str, int, float, bool, type(None)))
        ...
        if isinstance(obj, list):
            return [_json_serializable_converter(elem, current_depth + 1, new_visited) for elem in obj]
        
        if MULTI_MODAL_SCHEMAS_AVAILABLE and isinstance(obj, (ImageAnalysisResult, AudioAnalysisResult, VideoAnalysisResult)):
        ...
        METRICS["context_validation_errors"].labels(error_code=ReasonerErrorCode.CONTEXT_UNSUPPORTED_TYPE).inc()
        return str(obj)
    ...
    try:
        ...
        if config.sanitization_options.get("context_schema_model"):
            try:
                validated_model = config.sanitization_options["context_schema_model"].model_validate(context)
                context = validated_model.model_dump()
            except Exception as e:
                raise ReasonerError(f"Context schema validation failed: {e}", code=ReasonerErrorCode.CONTEXT_SCHEMA_VIOLATION, original_exception=e)
        ...
        raise ReasonerError(f"Failed to sanitize context: {e}", code=ReasonerErrorCode.CONTEXT_SANITIZATION_FAILED, original_exception=e)
...
def rate_limited(calls_per_second: float, key_extractor: Optional[Callable[[Any, Any], str]] = None):
    """
        ...
        async def wrapper(*args: P.args, **kwargs: P.kwargs) -> Any:
            ...
            if key_extractor:
                try:
                    ...
                    _utils_logger.error(f"Error in rate_limited key_extractor: {e}. Using 'global' key.")
            ...
            if redis_client:
                try:
                    ...
                    """
                    wait_time = await redis_client.eval(lua_script, 1, rate_limit_key, time.monotonic(), _interval)
                    if wait_time > 0:
                        _utils_logger.warning(f"Distributed rate limit hit for key '{key}'. Waiting {wait_time:.2f}s.")
                        await asyncio.sleep(wait_time)
                except Exception as e:
                    # Fall back to local rate limiting if Redis fails
                    _utils_logger.warning(f"Redis rate limiting failed: {e}. Falling back to local rate limiting.")
                    async with _local_locks[key]:
                        ...
                        if time_since_last_call < _interval:
                            ...
                            _utils_logger.warning(f"Local rate limit hit for key '{key}'. Waiting {wait_time:.2f}s.")
                            await asyncio.sleep(wait_time)
