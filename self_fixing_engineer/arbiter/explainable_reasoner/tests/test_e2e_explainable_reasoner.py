...
def mock_external_deps():
    ...
    with patch("arbiter.explainable_reasoner.explainable_reasoner.TRANSFORMERS_AVAILABLE", True), \
         patch("arbiter.explainable_reasoner.explainable_reasoner.pipeline") as mock_pipeline, \
         patch("arbiter.explainable_reasoner.explainable_reasoner.AutoModelForCausalLM") as mock_model, \
         patch("arbiter.explainable_reasoner.explainable_reasoner.AutoTokenizer") as mock_tokenizer, \
         patch("arbiter.explainable_reasoner.explainable_reasoner.pybreaker") as mock_breaker, \
         ...
         patch("arbiter.explainable_reasoner.explainable_reasoner.get_or_create_metric") as mock_get_metric:
        ...
        mock_pipeline_instance.tokenizer.encode.return_value = list(range(100))  # Return list of token IDs
        mock_pipeline_instance.__call__ = lambda prompt, **kwargs: [{"generated_text": f"{prompt} mock_local_response"}]
        ...
        mock_tracemalloc.take_snapshot.return_value = MagicMock(statistics=MagicMock(return_value=[]))
...
def prod_config(tmp_path):
    ...
    return ReasonerConfig(
        ...
        sanitization_options={
            ...
            "allowed_primitive_types": (str, int, float, bool, type(None), SensitiveValue)
        }
    )
...
async def plugin(prod_config):
    ...
    with patch("arbiter.explainable_reasoner.explainable_reasoner.ReasonerConfig.from_env", return_value=prod_config):
...
async def test_e2e_full_lifecycle(reasoner):
    ...
    if "explain" in result:
        ...
        assert result["explain"] in ["mock_cloud_adapter_response", "[Fallback] Based on available information (topic: AI, user_role: developer), the requested explanation for 'Explain the importance of AI ethics in modern applications.' could not be generated by the primary model."]
    ...
    assert health["status"] in ["healthy", "degraded", "unhealthy"]  # Allow unhealthy for no models
...
async def test_e2e_plugin_rbac_and_admin_tasks(plugin):
    ...
    with patch("arbiter.explainable_reasoner.explainable_reasoner.jwt") as mock_jwt_module:
        ...
        mock_jwt_module.decode.return_value = {"role": "admin", "exp": time.time() + 3600}
        ...
        assert "error" not in purge_result or purge_result["error"]["code"] != ReasonerErrorCode.PERMISSION_DENIED
        ...
        mock_jwt_module.decode.return_value = {"role": "user", "exp": time.time() + 3600}
...
async def test_e2e_performance_under_load(reasoner):
    ...
    tasks = [
        reasoner.explain(f"perf_test_{i}", {"index": i})
        for i in range(10)
    ]
...
def test_e2e_metrics_exposition():
    ...
    with patch("arbiter.explainable_reasoner.metrics.METRICS", {
        ...
        "reasoner_inference_duration_seconds": MagicMock(_name="reasoner_inference_duration_seconds")
    }):
...
"""
            ...
            for metric in expected_metrics:
                assert metric in content_str, f"Expected metric '{metric}' not found in metrics output"
