...
def mock_dependencies():
    with patch("arbiter.explainable_reasoner.metrics.get_or_create_metric") as mock_get_metric, \
         patch("arbiter.explainable_reasoner.prompt_strategies._simple_text_sanitize") as mock_sanitize, \
         patch("arbiter.explainable_reasoner.prompt_strategies._format_multimodal_for_prompt") as mock_format, \
         patch("arbiter.explainable_reasoner.prompt_strategies.trace") as mock_trace, \
         patch("arbiter.explainable_reasoner.prompt_strategies._prompt_strategy_logger") as mock_module_logger:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DefaultPromptStrategy, ["goal in detail:", "Based on this context:", "Explanation:"], ["Reason step-by-step about:", "Using this context:", "Reasoning:"]),
        (ConcisePromptStrategy, ["Explain", "briefly"], ["Reason about", "Context:"]),
        (VerbosePromptStrategy, ["Provide a detailed explanation", "Full context provided:", "Complete conversation history:"], ["Provide detailed, step-by-step reasoning", "Full context provided:"]),
        (StructuredPromptStrategy, ["task", "goal", "context"], ["task", "reasoning", "context"]),
    ]
)
@pytest.mark.asyncio
async def test_strategy_prompt_generation(
    ...
    explain_prompt = await strategy.create_explanation_prompt(
        dummy_context, 
        "test goal", 
        history_str="previous interaction"
    )
...
def test_factory_re_registration_warning(clean_factory, mock_dependencies):
    ...
    mock_module_logger.warning.assert_called_with(
        "strategy_re-registration",
        name="default",
        new_class="ConcisePromptStrategy"
    )
...
async def test_prompt_strategy_with_special_characters(mock_logger):
    ...
    prompt = await strategy.create_explanation_prompt(special_context, "goal with 'quotes' and \"double quotes\"")
