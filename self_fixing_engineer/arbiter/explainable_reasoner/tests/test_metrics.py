...
def test_get_metrics_content_failure(mock_structlog):
    ...
    with patch(
        ...
        mock_structlog.error.assert_called_with(
            "metrics_exposition_failed", error="Exposition fail", exc_info=True
        )
...
def test_default_metrics_exist():
    ...
    assert (
        "requests_total" in METRICS
        or "prompt_size_bytes" in METRICS
        or "inference_duration_seconds" in METRICS
        or len(METRICS) > 0
    )  # At least some metrics exist
