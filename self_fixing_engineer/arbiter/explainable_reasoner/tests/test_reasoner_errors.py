...
def mock_sentry():
    ...
    with patch(
        "arbiter.explainable_reasoner.reasoner_errors.SENTRY_AVAILABLE", True
    ), patch(
...
def test_error_code_values():
    ...
    error_codes = [
        getattr(ReasonerErrorCode, attr)
        for attr in dir(ReasonerErrorCode)
        if not attr.startswith("_") and attr.isupper()
    ]
...
def test_sentry_not_available(mock_logger):
    ...
    with patch(
        "arbiter.explainable_reasoner.reasoner_errors.SENTRY_AVAILABLE", False
...
def test_reasoner_error_no_message_or_code():
    ...
    with pytest.raises(
        TypeError, match="missing 2 required positional arguments: 'message' and 'code'"
    ):
        ReasonerError()
