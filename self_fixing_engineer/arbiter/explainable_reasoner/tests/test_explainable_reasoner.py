...
def mock_dependencies():
    ...
    patches = [
        patch("arbiter.explainable_reasoner.explainable_reasoner.TRANSFORMERS_AVAILABLE", True),
    ...
    pipeline_instance.__call__ = lambda prompt, **kwargs: [{"generated_text": f"{prompt} generated"}]
    ...
    breaker_instance.call_async = AsyncMock(side_effect=lambda func, *args, **kwargs: func(*args, **kwargs))
...
async def reasoner_instance(mock_config):
    ...
    with patch("arbiter.explainable_reasoner.explainable_reasoner._format_multimodal_for_prompt") as mock_format:
...
async def test_init_with_invalid_jwt_secret():
    ...
    config = ReasonerConfig(
        jwt_secret_key=SensitiveValue("default-secret-key-change-me"),
        mock_mode=False
    )
...
async def test_batch_explain_with_exceptions(reasoner_instance):
    ...
    with patch.object(reasoner_instance, 'explain', side_effect=[
...
async def test_handle_request_context_too_large(reasoner_instance):
    ...
    assert exc_info.value.code in [ReasonerErrorCode.INVALID_INPUT, ReasonerErrorCode.CONTEXT_SIZE_EXCEEDED]
...
async def test_health_check_healthy(reasoner_instance):
    ...
    assert health["status"] in ["healthy", "degraded", "unhealthy"]  # May vary due to mock setup
...
async def test_plugin_initialize():
    with patch("arbiter.explainable_reasoner.explainable_reasoner.ReasonerConfig.from_env") as mock_from_env:
        mock_from_env.return_value = ReasonerConfig(
            mock_mode=True,
            jwt_secret_key=SensitiveValue("test_key")
        )
    ...
    with patch.object(ReasonerErrorCode, 'PERMISSION_DENIED', 'PERMISSION_DENIED'):
        with patch("arbiter.explainable_reasoner.explainable_reasoner.ReasonerConfig.from_env") as mock_from_env:
            ...
            with patch("arbiter.explainable_reasoner.explainable_reasoner.jwt") as mock_jwt:
                ...
                with patch.object(plugin, 'purge_history', return_value={"status": "success", "message": "History purged"}):
                    # Mock the executor
...
async def test_plugin_execute_rbac_failure():
    ...
    with patch.object(ReasonerErrorCode, 'PERMISSION_DENIED', 'PERMISSION_DENIED'):
        with patch("arbiter.explainable_reasoner.explainable_reasoner.ReasonerConfig.from_env") as mock_from_env:
            ...
            with patch("arbiter.explainable_reasoner.explainable_reasoner.jwt") as mock_jwt:
                mock_jwt.decode.return_value = {"role": "user", "exp": 9999999999}  # Non-admin role
                # Create proper exception class
                class InvalidTokenError(Exception):
                    pass
                mock_jwt.InvalidTokenError = InvalidTokenError
