...
def mock_metrics():
    ...
    with patch("arbiter.explainable_reasoner.adapters.INFERENCE_LATENCY") as mock_latency, \
         ...
         patch("arbiter.explainable_reasoner.adapters.HEALTH_CHECK_ERRORS") as mock_health:
...
@pytest.mark.asyncio
async def test_openai_adapter_generate_success(mock_httpx_client, mock_sensitive_value, mock_metrics):
    adapter = OpenAIGPTAdapter("gpt-4", mock_sensitive_value, "https://api.openai.com/v1")
...
@pytest.mark.asyncio
async def test_openai_adapter_generate_with_multimodal(mock_httpx_client, mock_sensitive_value, dummy_multimodal_data):
...
@pytest.mark.asyncio
async def test_openai_adapter_generate_error_handling(mock_httpx_client, mock_sensitive_value, mock_metrics):
    adapter = OpenAIGPTAdapter("gpt-4", mock_sensitive_value, "https://api.openai.com/v1")
...
@pytest.mark.asyncio
async def test_gemini_adapter_generate_success(mock_httpx_client, mock_sensitive_value, mock_metrics):
    adapter = GeminiAPIAdapter("gemini-pro", mock_sensitive_value, "https://generativelanguage.googleapis.com/v1beta/models")
...
@pytest.mark.asyncio
async def test_anthropic_adapter_generate_success(mock_httpx_client, mock_sensitive_value, mock_metrics):
