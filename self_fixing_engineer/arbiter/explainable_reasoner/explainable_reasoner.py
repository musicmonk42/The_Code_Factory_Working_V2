...
try:
    ...
    if os.getenv("REASONER_SENTRY_DSN"):
        sentry_sdk.init(
            dsn=os.getenv("REASONER_SENTRY_DSN"),
            traces_sample_rate=0.1,
            release=f"explainable_reasoner@{__version__}",
        )
except ImportError:
    sentry_sdk = None
...
class ExplainableReasoner:
    ...
    def __init__(
        ...
        if settings and not isinstance(settings, collections.namedtuple):
            self.logger.warning(
                "non_dummy_settings_provided",
                message="Ensure compatibility with ReasonerConfig",
            )
        ...
        self._session_auth_enabled: bool = (
        ...
        if self.config.audit_log_enabled and self.config.audit_ledger_url:
            self.audit_ledger_client = AuditLedgerClient(
                ...
                api_key=(
                    self.config.audit_api_key.get_actual_value()
                    if self.config.audit_api_key
                    else None
                ),
            )
        ...
        self._prompt_strategy = PromptStrategyFactory.get_strategy(
            prompt_strategy_name, self.logger
        )
        ...
        if PYBREAKER_AVAILABLE:
            ...
            self.logger.warning(
                "pybreaker_unavailable",
                message="Circuit breaker for model reloading is disabled.",
            )
        ...
        get_or_create_metric(
            Gauge,
            "reasoner_instances",
            "Number of active reasoner instances",
            labelnames=("instance_id",),
        ).labels(instance_id=self._instance_id).inc()
    ...
    def _initialize_history_manager(self) -> None:
        ...
        if (
            ...
            self.history: BaseHistoryManager = RedisHistoryManager(
                self.config.redis_url.get_actual_value(),
                self.config.max_history_size,
                self.config.history_retention_days,
                self.audit_ledger_client,
            )
    ...
    async def async_init(self) -> None:
        ...
        try:
            ...
            get_or_create_metric(
                ...
                "Duration of reasoner initialization",
        ...
        try:
            if self.config.redis_url:
                ...
                self._redis_pool = aioredis.ConnectionPool.from_url(
                    redis_url_str, max_connections=pool_size
                )
    ...
    async def _init_history_db(self):
        ...
        for attempt in range(retries):
            try:
                ...
                if attempt == retries - 1:
                    raise ReasonerError(
                        f"History DB init failed after {retries} retries",
        ...
        try:
            ...
            get_or_create_metric(
                Gauge,
                "reasoner_history_entries_current",
                "Current number of history entries",
                labelnames=("backend",),
            ).labels(backend=self._backend_name).set(size)
        except Exception as e:
            self.logger.error("history_size_fetch_failed", error=str(e), exc_info=True)
            raise ReasonerError(
                f"Failed to get history size: {e}",
    ...
    async def _run_model_readiness_test(self):
        async with self._inference_semaphore:
            try:
                result = await self._async_generate_text(
                    "Health check", max_length=5, temperature=0.5
                )
                get_or_create_metric(
                    Counter,
                    "reasoner_health_check_success",
                ...
                self._last_health_check = {
                    "status": "healthy",
                    "messages": ["Model readiness test passed"],
                }
            except Exception as e:
                get_or_create_metric(
                    Counter,
                    "reasoner_health_check_errors",
    ...
    async def _initialize_models_async(self) -> None:
        ...
        if self.config.cloud_fallback_model_enabled and not (
            ...
            and self.config.cloud_fallback_api_key.get_actual_value()
        ):
        ...
        for i, result in enumerate(loaded_pipelines_info):
            ...
            if isinstance(result, dict):
                ...
                get_or_create_metric(
                    Counter,
                    "reasoner_model_load_success",
                    "Successful model loads",
                    labelnames=("model_name", "device"),
                ).labels(model_name=model_name, device=str(device)).inc()
                ...
                get_or_create_metric(
                    Counter,
                    "reasoner_model_load_errors",
                    "Failed model loads",
                    labelnames=("model_name", "device"),
                ).labels(model_name=model_name, device=str(device)).inc()

    async def _load_single_model(
        self, model_cfg: Dict[str, Any], is_reload: bool = False
        ...
        try:
            if (
                ...
                adapter_config = {
                    "model_name": model_name,
                    "api_key": (
                        self.config.cloud_fallback_api_key.get_actual_value()
                        if self.config.cloud_fallback_api_key
                        else None
                    ),
            ...
            await asyncio.wait_for(
                self._execute_in_thread(
                    lambda: pipeline_info["pipeline"]("test", max_new_tokens=5)
                ),
                timeout=15,
            )
            self.logger.info(
            ...
            if self.config.transformers_offline and "offline" in str(e).lower():
                raise ReasonerError(
                    f"Model '{model_name}' not in cache for offline mode",
                    code=ReasonerErrorCode.MODEL_LOAD_FAILED,
                    original_exception=e,
                ) from e
            if self.config.strict_mode and not is_reload:
                raise ReasonerError(
                    f"Model initialization failed for {model_name}",
                    code=ReasonerErrorCode.MODEL_LOAD_FAILED,
                    original_exception=e,
                ) from e
            return None
    ...
    def _load_hf_pipeline_sync(self, model_cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_kwargs = {
            ...
            "local_files_only": self.config.transformers_offline,
    ...
    async def _execute_in_thread(
        self, fn: Callable, *args: Any, timeout: int = 30, **kwargs: Any
    ) -> Any:
        """Runs a synchronous function in the ThreadPoolExecutor."""
        if not self._executor or self._executor._shutdown:
            get_or_create_metric(
                Counter,
                "reasoner_executor_restarts_total",
                "Total number of executor restarts",
            ).inc()
            self._executor = ThreadPoolExecutor(max_workers=self.config.max_workers)
        ...
        try:
            get_or_create_metric(
                Gauge,
                "reasoner_executor_queue_size",
                "Current size of the executor queue",
            ).set(self._executor._work_queue.qsize())
            future = loop.run_in_executor(self._executor, lambda: fn(*args, **kwargs))
            return await asyncio.wait_for(future, timeout=timeout)
        except FuturesTimeoutError as e:
            raise ReasonerError(
                f"Operation '{fn.__name__}' timed out",
                code=ReasonerErrorCode.TIMEOUT,
                original_exception=e,
            ) from e
        except Exception as e:
            raise ReasonerError(
                f"Operation '{fn.__name__}' failed: {e}",
                code=ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                original_exception=e,
            ) from e
    ...
    async def _get_next_pipeline(self) -> Optional[Dict[str, Any]]:
        ...
        async with self._pipeline_lock:
            ...
            for _ in range(num_pipelines):
                ...
                if (
                    last_failed
                    and (datetime.now(timezone.utc) - last_failed).total_seconds()
                    < self.config.model_cooldown_period
                ):
                    if model_key not in self._model_reload_tasks:
                        self.logger.warning(
                            "model_in_cooldown_initiating_reload", model_key=model_key
                        )
                        task = asyncio.create_task(
                            self._attempt_reload_model(
                                pipeline_info,
                                initial_delay=self.config.model_cooldown_period,
                            )
                        )
                ...
                get_or_create_metric(
                    Counter,
                    "reasoner_model_pipeline_usage",
                    "Usage count per model pipeline",
                    labelnames=("model_name", "device"),
                ).labels(
            ...
            if (
                ...
                if fallback:
                    self.logger.warning(
                        "using_cloud_fallback_all_local_failed",
    ...
    async def _unload_model(self, model_key: str) -> None:
        ...
        async with self._pipeline_lock:
            found_idx = next(
                (
                    i
                    for i, p in enumerate(self._model_pipelines)
                    if f"{p['model_name']}-{p['device']}" == model_key
                ),
                -1,
            )
            if found_idx != -1:
    ...
    async def _attempt_reload_model(
        self,
        ...
        try:
            ...
            self.logger.error(
                "model_reload_circuit_open",
                model_key=key,
                message="Circuit breaker is open, skipping reload attempt.",
            )
        except Exception as e:
            self.logger.error(
                "model_reload_unhandled_exception",
                model_key=key,
                error=str(e),
                exc_info=True,
            )
    ...
    async def _reload_model_with_retries(
        self,
        model_info: Dict[str, Any],
        initial_delay: int,
        ...
        for attempt in range(self.config.model_reload_retries):
            try:
                config_to_use = new_config or next(
                    (
                        cfg
                        for cfg in self.config.model_configs
                        if cfg.get("model_name") == model_info["model_name"]
                    ),
                    {},
                )
                if not config_to_use:
                    raise ReasonerError(
                        f"No configuration found for reloading model {key}",
                        ReasonerErrorCode.CONFIGURATION_ERROR,
                    )

                new_pipeline_info = await asyncio.wait_for(
                    self._load_single_model(config_to_use, is_reload=True), timeout=600
                )
                if new_pipeline_info:
                    async with self._pipeline_lock:
                        found_idx = next(
                            (
                                i
                                for i, p in enumerate(self._model_pipelines)
                                if p["model_name"] == model_info["model_name"]
                            ),
                            -1,
                        )
                        if found_idx != -1:
                    ...
                    get_or_create_metric(
                        Counter,
                        "reasoner_model_reload_success",
                        "Successful model reloads",
                        labelnames=("model_name", "device"),
                    ).labels(
                        model_name=new_pipeline_info["model_name"],
                        device=str(new_pipeline_info["device"]),
                    ).inc()
                    return
            except Exception as e:
                self.logger.error(
                    "model_reload_attempt_failed",
                    model_key=key,
                    attempt=attempt + 1,
                    error=str(e),
                    exc_info=True,
    ...
    async def _truncate_prompt_if_needed(
        self, prompt: str, tokenizer: Any, max_new_tokens: int
    ) -> str:
        ...
        try:
            ...
            if prompt_length_tokens >= model_max_length - max_new_tokens:
                ...
                get_or_create_metric(
                    Counter,
                    "reasoner_prompt_truncations",
                    "Total number of prompt truncations",
                    labelnames=("model_name",),
                ).labels(model_name=tokenizer.name_or_path).inc()

                if new_length <= 50:
                    raise ReasonerError(
                        f"Prompt is too long for model's context ({model_max_length} tokens) to be truncated meaningfully.",
    ...
    async def _async_generate_text(
        ...
        if not self._inference_semaphore:
            raise ReasonerError(
                "Inference semaphore not initialized",
                code=ReasonerErrorCode.CONFIGURATION_ERROR,
            )
        async with self._inference_semaphore:
            start_time = time.monotonic()
            ...
            if not pipeline_info:
                raise ReasonerError(
                    "No model available for inference",
                    code=ReasonerErrorCode.MODEL_NOT_INITIALIZED,
                )
            ...
            try:
                if isinstance(model_pipeline, LLMAdapter):
                    ...
                    if TRANSFORMERS_AVAILABLE:
                        prompt = await self._truncate_prompt_if_needed(
                            prompt, model_pipeline.tokenizer, max_length
                        )
                ...
                raise ReasonerError(
                    f"HTTP error in inference: {e}",
                    code=ReasonerErrorCode.SERVICE_UNAVAILABLE,
                    original_exception=e,
                ) from e
                ...
                raise ReasonerError(
                    f"Inference failed for {model_name}",
                    code=ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                    original_exception=e,
                ) from e
    ...
    def _generate_text_sync(
        ...
        try:
            ...
            if (
                ...
                processed_text = (
                    _simple_text_sanitize(processed_text)
                    if processed_text
            ...
            if torch and hasattr(torch.cuda, "OutOfMemoryError")
            else Exception
        ) as e:
            if "OutOfMemoryError" in str(type(e).__name__):
                raise ReasonerError(
                    "GPU out of memory",
                    code=ReasonerErrorCode.CUDA_OOM,
                    original_exception=e,
                ) from e
    ...
    async def _prepare_prompt_with_history(
        self,
        prompt_type: str,
        ...
        recent_history = await self.history.get_entries(
            limit=self.config.max_history_size, session_id=session_id
        )
        history_str = self._build_history_string(recent_history, session_id)
        formatted_context = {
            k: (
                _format_multimodal_for_prompt(v)
                if isinstance(v, (MultiModalData, MultiModalAnalysisResult))
                else v
            )
            for k, v in context.items()
        }
        if prompt_type == "explain":
            prompt = await self._prompt_strategy.create_explanation_prompt(
                formatted_context, query, history_str=history_str
            )
        else:
            prompt = await self._prompt_strategy.create_reasoning_prompt(
                formatted_context, query, history_str=history_str
            )
        ...
        get_or_create_metric(
            ...
            labelnames=("type",),
        ).labels(type=prompt_type).observe(len(prompt.encode("utf-8")))
        return prompt
        ...
        if self._session_auth_enabled:
            if not self._redis_client:
                ...
                if self.config.strict_mode:
                    raise ReasonerError(
                        "Session validation is required, but backend (Redis) is not available.",
                        code=ReasonerErrorCode.CONFIGURATION_ERROR,
                    )
                return True
            ...
            try:
                ...
                if not exists:
                    get_or_create_metric(
                        Counter,
                        "reasoner_session_validation_errors",
                        "Session validation errors",
                        labelnames=("type",),
                    ).labels(type="invalid_session").inc()
                    return False
                return True
            except Exception as e:
                get_or_create_metric(
    ...
    async def _validate_request_inputs(
        ...
        if context and not isinstance(context, dict):
            raise ReasonerError(
                "Context must be a dictionary.", code=ReasonerErrorCode.INVALID_INPUT
            )
        ...
        if self._session_auth_enabled and session_id:
            if not await self._validate_session(session_id):
                raise ReasonerError(
                    ...
                    code=ReasonerErrorCode.PERMISSION_DENIED,
                )

        sanitized_query = _simple_text_sanitize(query)
        if not sanitized_query:
            raise ReasonerError(
                "Query was entirely sanitized, resulting in an empty string.",
                code=ReasonerErrorCode.INVALID_INPUT,
            )
        ...
        if sanitized_context != (context or {}):
            ...
            ).labels(redaction_type="context").inc()

        try:
            context_bytes = json.dumps(sanitized_context, default=str).encode("utf-8")
            if len(context_bytes) > self.config.max_context_bytes:
                raise ReasonerError(
                    f"Context size ({len(context_bytes)} bytes) exceeds limit of {self.config.max_context_bytes}",
                    code=ReasonerErrorCode.CONTEXT_SIZE_EXCEEDED,
                )
            ...
            for key, data in multi_modal_data.items():
                if isinstance(data, MultiModalData) and data.data_type == "image":
                    ...
                    try:
                        ...
                        raise ReasonerError(
                            f"Invalid image data for key '{key}': {e}",
                            code=ReasonerErrorCode.INVALID_INPUT,
                            original_exception=e,
                        ) from e
    ...
    async def _perform_inference_with_fallback(
        ...
        try:
            temperature = (
                ...
                else self.config.temperature_reason
            )
    ...
    async def _finalize_request(
        ...
        if self._redis_client and "fallback" not in response_type:
            try:
                ...
                get_or_create_metric(
                    Counter,
                    "reasoner_cache_errors",
                    "Cache operation errors",
                    labelnames=("type",),
                ).labels(type=task_type).inc()
    ...
    async def _handle_request(
        ...
        with tracer.start_as_current_span(f"reasoner.{task_type}"):
            ...
            get_or_create_metric(
                ...
                "Total requests",
                ("user_id", "task_type"),
            ).labels(user_id=user_id or "anon", task_type=task_type).inc()

            sanitized_query, sanitized_context, multi_modal_data = (
                await self._validate_request_inputs(query, context, session_id)
            )
            ...
            if self._redis_client:
                try:
                    ...
                    if cached_response:
                        ...
                        return {
                            "result": cached_response.decode("utf-8"),
                            "source": "cache",
                            "id": str(uuid.uuid4()),
                        }
                    get_or_create_metric(
                        Counter,
                        "reasoner_cache_misses",
                        "Cache misses",
                        labelnames=("type",),
                    ).labels(type=task_type).inc()
                except Exception as e:
            ...
            prompt = await self._prepare_prompt_with_history(
                task_type, sanitized_context, sanitized_query, session_id
            )
    ...
    def _build_history_string(
        self, history_entries: List[Dict[str, Any]], session_id: Optional[str]
    ) -> str:
        ...
        get_or_create_metric(
            Histogram,
            "reasoner_history_entries_used",
            "Number of history entries used in a prompt",
        ).observe(len(history_entries))
        ...
        for entry in reversed(history_entries):
            context_str = "; ".join(
                [f"{k}: {str(v)[:100]}" for k, v in entry.get("context", {}).items()]
            )
            ...
            if entry["response"] != response_sanitized:
                get_or_create_metric(
                    ...
                    "Total redactions of sensitive data",
                    labelnames=("redaction_type",),
                ).labels(redaction_type="history").inc()
    ...
    async def batch_explain(
        self, queries: List[str], contexts: List[Optional[Dict[str, Any]]], **kwargs
    ) -> List[Union[Dict[str, Any], Dict[str, str]]]:
        if not (len(queries) == len(contexts)):
            raise ReasonerError(
                "Length of 'queries' and 'contexts' lists must be equal.",
                code=ReasonerErrorCode.INVALID_INPUT,
            )
        ...
        for res in results:
            if isinstance(res, ReasonerError):
                ...
                processed_results.append(
                    ReasonerError(
                        f"An unexpected error occurred: {res}",
                        code=ReasonerErrorCode.UNEXPECTED_ERROR,
                        original_exception=res,
                    ).to_api_response()
                )
            else:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            queue_size = (
                ...
                if self._executor and hasattr(self._executor, "_work_queue")
                else "unknown"
            )
        ...
        if not self._is_ready:
            ...
            if failed_models == len(self._model_pipelines):
                ...
                status = "degraded"
                messages.append(
                    f"WARNING: {failed_models}/{len(self._model_pipelines)} models failed"
                )
    ...
    async def shutdown(self) -> None:
        ...
        if tracemalloc.is_tracing():
            ...
            self.logger.info(
                "memory_snapshot_at_shutdown",
                top_10_leaks=[str(stat) for stat in top_stats[:10]],
            )
            tracemalloc.stop()
        ...
        get_or_create_metric(
            ...
            "Number of active reasoner instances",
            labelnames=("instance_id",),
        ).labels(instance_id=self._instance_id).dec()
        self.logger.info("shutdown_completed")
        get_or_create_metric(
            Histogram,
            "reasoner_shutdown_duration_seconds",
            "Duration of reasoner shutdown",
        ).observe(time.monotonic() - start_time)
...
@plugin(
    ...
    name="explainable_reasoner",
    description="Generates explanations and reasoning with transformer-based models",
    version="1.2.0",
)
    ...
    async def execute(self, action: str, **kwargs: Any) -> Any:
        ...
        if required_role:
            if (
                ...
                self.logger.critical(
                    "rbac_misconfigured",
                    action=action,
                    reason="JWT secret key is not set or is the default.",
                )
                return ReasonerError(
                    "Security feature misconfigured.",
                    code=ReasonerErrorCode.CONFIGURATION_ERROR,
                ).to_api_response()
            ...
            if not token:
                self.logger.warning(
                    "rbac_token_missing", action=action, required_role=required_role
                )
                return ReasonerError(
                    f"Authentication token required for action '{action}'.",
                    code=ReasonerErrorCode.SECURITY_VIOLATION,
                ).to_api_response()
            ...
            try:
                if not JWT_AVAILABLE:
                    raise ReasonerError(
                        "JWT library not installed, cannot perform RBAC.",
                        code=ReasonerErrorCode.CONFIGURATION_ERROR,
                    )
        ...
        try:
            if action in action_map:
                ...
                raise ReasonerError(
                    f"Invalid action: {action}", code=ReasonerErrorCode.INVALID_INPUT
                )
        except ReasonerError as e:
            self.logger.error(
                "plugin_error", action=action, error_code=e.code, message=str(e.message)
            )
            return e.to_api_response()
        except Exception as e:
            error = ReasonerError(
                f"Unexpected error in action '{action}': {type(e).__name__}",
                code=ReasonerErrorCode.UNEXPECTED_ERROR,
                original_exception=e,
            )
            self.logger.error(
                "plugin_unexpected_error", action=action, message=str(e), exc_info=True
            )
            return error.to_api_response()
