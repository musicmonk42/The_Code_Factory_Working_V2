...
def get_or_create_metric(metric_type: Type, name: str, description: str, labelnames: Tuple[str, ...] = (), buckets: Optional[Tuple[float, ...]] = None):
    """
    ...
    try:
        if metric_type is Counter:
            metric = Counter(name, description, labelnames=labelnames or (), registry=METRICS_REGISTRY)
        elif metric_type is Gauge:
            metric = Gauge(name, description, labelnames=labelnames or (), registry=METRICS_REGISTRY)
        elif metric_type is Histogram:
            metric = Histogram(name, description, labelnames=labelnames or (), buckets=buckets or Histogram.DEFAULT_BUCKETS, registry=METRICS_REGISTRY)
        elif metric_type is Summary:
            metric = Summary(name, description, labelnames=labelnames or (), registry=METRICS_REGISTRY)
        else:
...
def initialize_metrics():
    ...
    if PROMETHEUS_MULTIPROC_DIR:
        try:
            ...
            _metrics_logger.warning("fallback_to_in_memory_registry", reason="Multiprocess setup failed.")
    else:
