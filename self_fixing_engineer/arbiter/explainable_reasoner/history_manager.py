...
try:
    ...
    logger.warning("Warning: arbiter.models.multi_modal_schemas not found. Using dummy MultiModalData/Schemas for standalone mode.")
...
class BaseHistoryManager(ABC):
    ...
    def __init__(self, max_history_size: int, retention_days: int, audit_client: Optional[AuditLedgerClient] = None):
    ...
    async def _pre_add_entry_checks(self, entry: Dict[str, Any]) -> None:
        ...
        if binary_data_present:
            ...
            raise ReasonerError("Binary data detected in history entry", code=ReasonerErrorCode.SENSITIVE_DATA_LEAK)
        
        for key, value in entry.items():
            if isinstance(value, str) and any(re.search(pattern, value) for pattern in sensitive_patterns):
                self._logger.warning("sensitive_data_detected", key=key)
                raise ReasonerError("Sensitive data detected in history entry", code=ReasonerErrorCode.SENSITIVE_DATA_LEAK)
    ...
    def _decrypt(self, encrypted_text: str) -> str:
        ...
        if self._encryption_key:
            try:
                ...
                self._logger.warning("decryption_failed_invalid_token", message="Could not decrypt field. Key may have changed or data is corrupt.")
    ...
    def _record_op_success(self, operation: str, start_time: float):
        ...
        if "reasoner_history_operations_total" in METRICS:
            METRICS["reasoner_history_operations_total"].labels(operation=operation, status="success").inc()
        if "reasoner_history_operation_latency_seconds" in METRICS:
            METRICS["reasoner_history_operation_latency_seconds"].labels(operation=operation).observe(time.monotonic() - start_time)

    def _record_op_error(self, operation: str, start_time: float, e: Exception):
        """Records metrics and logs for a failed operation."""
        # FIX: Check if metric exists before accessing.
        if "reasoner_history_operations_total" in METRICS:
            METRICS["reasoner_history_operations_total"].labels(operation=operation, status="error").inc()
        if "reasoner_history_operation_latency_seconds" in METRICS:
            METRICS["reasoner_history_operation_latency_seconds"].labels(operation=operation).observe(time.monotonic() - start_time)
    ...
    async def _log_audit_event(self, event_type: str, details: Dict, operator: str):
        ...
        if self.audit_client:
            await self.audit_client.log_event(event_type=event_type, details=details, operator=operator)
    ...
    @abstractmethod
    async def get_entries(self, limit: int = 10, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
...
class SQLiteHistoryManager(BaseHistoryManager):
    ...
    def __init__(self, db_path: Path, max_history_size: int, retention_days: int, audit_client: Optional[AuditLedgerClient] = None):
        super().__init__(max_history_size, retention_days, audit_client)
        self.db_path = db_path
        self._conn: Optional[aiosqlite.Connection] = None
        if not SQLITE_AVAILABLE:
            self._logger.error("aiosqlite_missing", message="SQLite backend unavailable. Please install 'aiosqlite'.")
            raise ReasonerError("aiosqlite not installed", code=ReasonerErrorCode.CONFIGURATION_ERROR)
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            if "reasoner_history_db_connection_failures_total" in METRICS:
                METRICS["reasoner_history_db_connection_failures_total"].labels(backend=self._backend_name).inc()
            self._record_op_error("init_db", start_time, e)
            raise ReasonerError("Failed to initialize SQLite DB", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def add_entry(self, entry: Dict[str, Any]) -> None:
        ...
        try:
            await self._conn.execute(
                ...
                (entry["id"], entry["query"], json.dumps(entry["context"]), entry["response"], entry["response_type"], entry["timestamp"], entry.get("session_id"))
            )
            await self._conn.commit()
            self._record_op_success("add_entry", start_time)
            await self._log_audit_event(
                event_type="history_add_entry",
                details={"entry_id": entry["id"], "timestamp": entry["timestamp"], "session_id": entry.get("session_id")},
                operator="system"
            )
        except Exception as e:
            self._record_op_error("add_entry", start_time, e)
            raise ReasonerError("Failed to add history entry", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def add_entries_batch(self, entries: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            data_to_insert = [
                (e["id"], e["query"], json.dumps(e["context"]), e["response"], e["response_type"], e["timestamp"], e.get("session_id"))
                for e in entries
            ]
            ...
            raise ReasonerError("Failed to add history entries in batch", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def get_entries(self, limit: int = 10, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with self._conn.execute(query, params) as cursor:
                ...
                entries = [
                    {
                        "id": row[0], "query": row[1], "context": json.loads(row[2]),
                        "response": row[3], "response_type": row[4], "timestamp": row[5],
                        "session_id": row[6]
                    } for row in rows
                ]
            ...
            raise ReasonerError("Failed to retrieve history entries", code=ReasonerErrorCode.HISTORY_READ_FAILED, original_exception=e)
    ...
    async def get_size(self) -> int:
        ...
        try:
            ...
            raise ReasonerError("Failed to get history size", code=ReasonerErrorCode.HISTORY_READ_FAILED, original_exception=e)

    async def prune_old_entries(self) -> None:
        """Prunes old entries based on retention days."""
        start_time = time.monotonic()
        try:
            cutoff_timestamp = (datetime.now(timezone.utc) - timedelta(days=self.retention_days)).isoformat()
            async with self._conn.execute("SELECT COUNT(*) FROM history WHERE timestamp < ?", (cutoff_timestamp,)) as cursor:
                count = (await cursor.fetchone())[0]

            if count > 0:
                await self._conn.execute("DELETE FROM history WHERE timestamp < ?", (cutoff_timestamp,))
                await self._conn.commit()
                self._logger.info("sqlite_prune_completed", count=count, cutoff_timestamp=cutoff_timestamp)
                ...
                await self._log_audit_event(
                    ...
                    details={"db_path": str(self.db_path), "count": count, "cutoff_timestamp": cutoff_timestamp},
            ...
            if "reasoner_history_entries_current" in METRICS:
                METRICS["reasoner_history_entries_current"].labels(backend=self._backend_name).set(await self.get_size())
        except Exception as e:
            self._record_op_error("prune_old_entries", start_time, e)
            raise ReasonerError("Failed to prune old history entries", code=ReasonerErrorCode.HISTORY_PRUNING_FAILED, original_exception=e)

    async def clear(self, session_id: Optional[str] = None) -> None:
        """Clears history entries, optionally for a session."""
        start_time = time.monotonic()
        try:
            query = "DELETE FROM history" if not session_id else "DELETE FROM history WHERE session_id = ?"
            params = [] if not session_id else [session_id]
            cursor = await self._conn.execute(query, params)
            deleted_count = cursor.rowcount
            await self._conn.commit()

            self._logger.info("sqlite_clear_completed", session_id=session_id or "all", count=deleted_count)
            ...
            raise ReasonerError("Failed to clear history", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def purge_all(self, operator_id: str = "system_api_request") -> None:
        ...
        try:
            ...
            if "reasoner_history_entries_current" in METRICS:
                METRICS["reasoner_history_entries_current"].labels(backend=self._backend_name).set(0)
        except Exception as e:
            self._record_op_error("purge_all", start_time, e)
            raise ReasonerError("Failed to purge all history", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def export_history(self, output_format: str = "json", operator_id: str = "system_api_request") -> AsyncGenerator[Union[str, bytes], None]:
        ...
        try:
            if output_format not in ["json", "csv"]:
                raise ReasonerError(f"Unsupported format: {output_format}", code=ReasonerErrorCode.INVALID_INPUT)
            
            async with self._conn.execute("SELECT * FROM history ORDER BY timestamp DESC") as cursor:
            ...
            self._logger.info("sqlite_export_completed", format=output_format, operator_id=operator_id)
            ...
            raise ReasonerError("Failed to export history", code=ReasonerErrorCode.HISTORY_READ_FAILED, original_exception=e)
    ...
    async def aclose(self) -> None:
        ...
        try:
            ...
            raise ReasonerError("Failed to close SQLite connection", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
...
class PostgresHistoryManager(BaseHistoryManager):
    ...
    def __init__(self, db_url: str, max_history_size: int, retention_days: int, audit_client: Optional[AuditLedgerClient] = None):
        super().__init__(max_history_size, retention_days, audit_client)
        self.db_url = db_url
        self._pool: Optional[asyncpg.Pool] = None

        if not POSTGRES_AVAILABLE:
            self._logger.error("asyncpg_missing", message="Postgres backend unavailable. Please install 'asyncpg'.")
            raise ReasonerError("asyncpg not installed", code=ReasonerErrorCode.CONFIGURATION_ERROR)
        if not CRYPTOGRAPHY_AVAILABLE:
            self._logger.error("cryptography_missing", message="Cryptography backend unavailable. Please install 'cryptography'.")
            raise ReasonerError("cryptography not installed", code=ReasonerErrorCode.CONFIGURATION_ERROR)

        encryption_key = os.getenv("REASONER_ENCRYPTION_KEY")
        if encryption_key:
            self._encryption_key = Fernet(encryption_key.encode('utf-8'))
        else:
            self._logger.warning("encryption_key_missing", message="REASONER_ENCRYPTION_KEY not set. History responses will not be encrypted.")
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            if "reasoner_history_db_connection_failures_total" in METRICS:
                METRICS["reasoner_history_db_connection_failures_total"].labels(backend=self._backend_name).inc()
            self._record_op_error("init_db", start_time, e)
            raise ReasonerError("Failed to initialize Postgres DB", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
        ...
        try:
            async with self._pool.acquire() as conn:
                await conn.execute(
                    ...
                    entry_to_store["id"], entry_to_store["query"], json.dumps(entry_to_store["context"]), entry_to_store["response"], 
                    entry_to_store["response_type"], datetime.fromisoformat(entry_to_store["timestamp"]), entry_to_store.get("session_id")
                )
            self._record_op_success("add_entry", start_time)
            await self._log_audit_event(
                event_type="history_add_entry",
        ...
        try:
            ...
            for entry in entries:
                ...
                data_to_insert.append(
                    (
                        entry_to_store["id"], entry_to_store["query"], json.dumps(entry_to_store["context"]),
                        entry_to_store["response"], entry_to_store["response_type"],
                        datetime.fromisoformat(entry_to_store["timestamp"]), entry_to_store.get("session_id")
                    )
                )
            ...
            await self._log_audit_event("history_add_entries_batch", {"count": len(entries)}, "system")
        except Exception as e:
            self._record_op_error("add_entries_batch", start_time, e)
            raise ReasonerError("Failed to add history entries in batch", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def get_entries(self, limit: int = 10, session_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            async with self._pool.acquire() as conn:
                ...
                entries = [
                    {
                        "id": row["id"], "query": row["query"], "context": row["context"],
                        "response": self._decrypt(row["response"]), "response_type": row["response_type"],
                        "timestamp": row["timestamp"].isoformat(), "session_id": row["session_id"]
                    } for row in rows
                ]
            self._record_op_success("get_entries", start_time)
            return entries
    ...
    async def prune_old_entries(self) -> None:
        ...
        try:
            ...
            async with self._pool.acquire() as conn:
                ...
                result = await conn.execute("DELETE FROM history WHERE timestamp < $1", cutoff_timestamp)
                count = int(result.split(" ")[1]) if result.startswith("DELETE") else 0
            
            if count > 0:
                self._logger.info("postgres_prune_completed", count=count, cutoff_timestamp=cutoff_timestamp.isoformat())
                if "reasoner_history_pruned_entries_total" in METRICS:
                    METRICS["reasoner_history_pruned_entries_total"].inc(count)
                await self._log_audit_event(
                    "history_prune", {"count": count, "cutoff_timestamp": cutoff_timestamp.isoformat()}, "system_scheduled_task"
                )
            else:
    ...
    async def clear(self, session_id: Optional[str] = None) -> None:
        ...
        try:
            async with self._pool.acquire() as conn:
                if session_id:
                    result = await conn.execute("DELETE FROM history WHERE session_id = $1", session_id)
                else:
                    result = await conn.execute("DELETE FROM history")
                deleted_count = int(result.split(" ")[1]) if result.startswith("DELETE") else 0

            self._logger.info("postgres_clear_completed", session_id=session_id or "all", count=deleted_count)
            self._record_op_success("clear", start_time)
            if deleted_count > 0:
                await self._log_audit_event("history_clear", {"session_id": session_id or "all", "count": deleted_count}, "system")
            if "reasoner_history_entries_current" in METRICS:
                METRICS["reasoner_history_entries_current"].labels(backend=self._backend_name).set(await self.get_size())
        except Exception as e:
            self._record_op_error("clear", start_time, e)
    ...
    async def purge_all(self, operator_id: str = "system_api_request") -> None:
        ...
        try:
            ...
            self._record_op_success("purge_all", start_time)
            if count > 0:
                await self._log_audit_event("history_purge", {"count": count, "operator_id": operator_id}, operator_id)
            if "reasoner_history_entries_current" in METRICS:
                METRICS["reasoner_history_entries_current"].labels(backend=self._backend_name).set(0)
        except Exception as e:
            self._record_op_error("purge_all", start_time, e)
    ...
    async def export_history(self, output_format: str = "json", operator_id: str = "system_api_request") -> AsyncGenerator[Union[str, bytes], None]:
        ...
        try:
            ...
            self._logger.info("postgres_export_completed", format=output_format, operator_id=operator_id)
            self._record_op_success("export_history", start_time)
            await self._log_audit_event("history_export", {"format": output_format, "operator_id": operator_id}, operator_id)
        except Exception as e:
            self._record_op_error("export_history", start_time, e)
            raise ReasonerError("Failed to export history", code=ReasonerErrorCode.HISTORY_READ_FAILED, original_exception=e)
    ...
    async def aclose(self) -> None:
        ...
        try:
            ...
            self._record_op_error("aclose", start_time, e)
            raise ReasonerError("Failed to close Postgres connection", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
...
class RedisHistoryManager(BaseHistoryManager):
    ...
    def __init__(self, redis_url: str, max_history_size: int, retention_days: int, audit_client: Optional[AuditLedgerClient] = None):
        super().__init__(max_history_size, retention_days, audit_client)
        ...
        if not REDIS_AVAILABLE:
            self._logger.error("redis_missing", message="Redis backend unavailable. Please install 'redis'.")
    ...
    async def init_db(self) -> None:
        ...
        start_time = time.monotonic()
        try:
            self._pool = aioredis.ConnectionPool.from_url(self.redis_url, max_connections=10, decode_responses=True)
            ...
            if "reasoner_history_db_connection_failures_total" in METRICS:
                METRICS["reasoner_history_db_connection_failures_total"].labels(backend=self._backend_name).inc()
            self._record_op_error("init_db", start_time, e)
            raise ReasonerError("Failed to initialize Redis DB", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def add_entry(self, entry: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._record_op_success("add_entry", start_time)
            await self._log_audit_event("history_add_entry", {"entry_id": entry["id"], "session_id": entry.get("session_id")}, "system")
        except Exception as e:
            self._record_op_error("add_entry", start_time, e)
            raise ReasonerError("Failed to add history entry", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)
    ...
    async def prune_old_entries(self) -> None:
        ...
        start_time = time.monotonic()
        try:
            cutoff_timestamp = (datetime.now(timezone.utc) - timedelta(days=self.retention_days)).timestamp()
            # Corrected: remove by score (timestamp)
            removed_count = await self._redis.zremrangebyscore(self._history_key, -float('inf'), cutoff_timestamp)
            
            if removed_count > 0:
                self._logger.info("redis_prune_completed", count=removed_count, cutoff_timestamp=cutoff_timestamp)
                if "reasoner_history_pruned_entries_total" in METRICS:
                    METRICS["reasoner_history_pruned_entries_total"].inc(removed_count)
                await self._log_audit_event(
                    "history_prune", {"count": removed_count, "cutoff_timestamp": cutoff_timestamp}, "system_scheduled_task"
                )
            else:
    ...
    async def clear(self, session_id: Optional[str] = None) -> None:
        ...
        try:
            ...
            self._record_op_success("clear", start_time)
            if count > 0:
                await self._log_audit_event("history_clear", {"session_id": session_id or "all", "count": count}, "system")
            if "reasoner_history_entries_current" in METRICS:
                METRICS["reasoner_history_entries_current"].labels(backend=self._backend_name).set(await self.get_size())
        except Exception as e:
            self._record_op_error("clear", start_time, e)
    ...
    async def export_history(self, output_format: str = "json", operator_id: str = "system_api_request") -> AsyncGenerator[Union[str, bytes], None]:
        ...
        try:
            ...
            self._logger.info("redis_export_completed", format=output_format, operator_id=operator_id)
            self._record_op_success("export_history", start_time)
            await self._log_audit_event("history_export", {"format": output_format, "operator_id": operator_id}, operator_id)
        except Exception as e:
            self._record_op_error("export_history", start_time, e)
    ...
    async def aclose(self) -> None:
        ...
        try:
            ...
            self._record_op_error("aclose", start_time, e)
            raise ReasonerError("Failed to close Redis connection", code=ReasonerErrorCode.HISTORY_WRITE_FAILED, original_exception=e)