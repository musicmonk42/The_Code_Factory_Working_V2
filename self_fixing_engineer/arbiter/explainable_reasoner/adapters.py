...
try:
    INFERENCE_ERRORS = Counter(
        ...
        "Inference errors",
        labelnames=["adapter", "endpoint", "code"],
    )
except ValueError:
    # Metric already exists, find it in registry
    for collector in REGISTRY._collector_to_names:
        if "model_inference_errors_total" in REGISTRY._collector_to_names[collector]:
            ...
            break
STREAM_CHUNKS = get_or_create_metric(
    Counter, "model_stream_chunks_total", "Streamed chunks", ("adapter", "endpoint")
)
HEALTH_CHECK_ERRORS = get_or_create_metric(
    Counter, "adapter_health_check_errors_total", "Health check failures", ("adapter")
)
        ...
        async def wrapper(self, *args, **kwargs):
                        ...
                        if status_code == 429:
                            retry_after = float(
                                e.response.headers.get(
                                    "Retry-After", initial_backoff_delay * (2**attempt)
                                )
                            )
                            _logger.warning(
                                "rate_limit_hit",
                            ...
                            if attempt == max_retries - 1:
                                raise ReasonerError(
                                    f"API error after {max_retries} retries: {str(e)}",
                                    ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                                    e,
                                )
            ...
            if self._breaker:
                try:
                    ...
                    raise ReasonerError(
                        "Circuit breaker open", ReasonerErrorCode.SERVICE_UNAVAILABLE, e
                    )
            else:
...
class OpenAIGPTAdapter(LLMAdapter):
    ...
    def _build_openai_messages(
        self, prompt: str, multi_modal_data: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if multi_modal_data:
            if not isinstance(multi_modal_data, dict):
                raise ReasonerError(
                    "Multimodal data must be a dictionary",
            ...
            for key, data in multi_modal_data.items():
                ...
                if data.get("data_type") == "image":
                    ...
                    if mime not in [
                        ...
                        raise ReasonerError(
                            f"Unsupported image type {mime} for key {key}",
                            ReasonerErrorCode.INVALID_INPUT,
                        )
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            INFERENCE_ERRORS.labels(
                adapter=self.__class__.__name__, endpoint=endpoint, code="unexpected"
            ).inc()
            raise ReasonerError(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            async with client.stream(
                "POST", f"{self.base_url}/{endpoint}", json=payload, headers=headers
            ) as response:
                response.raise_for_status()
            ...
            raise ReasonerError(
                f"Stream timed out after {self.timeout}s", ReasonerErrorCode.TIMEOUT, e
            )
        except httpx.HTTPStatusError as e:
            _logger.error(
                "openai_stream_error",
            ...
            raise ReasonerError(
                f"OpenAI API stream error: {e.response.text}",
                ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                e,
            )
        except Exception as e:
...
class GeminiAPIAdapter(LLMAdapter):
            ...
            for key, data in multi_modal_data.items():
                ...
                if data.get("data_type") == "image":
                    ...
                    if mime_type not in [
                        ...
                        raise ReasonerError(
                            f"Unsupported image type {mime_type} for key {key}",
                            ReasonerErrorCode.INVALID_INPUT,
                        )
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        if max_tokens > 8192:
            raise ReasonerError(
                "maxOutputTokens exceeds limit (8192)", ReasonerErrorCode.INVALID_INPUT
            )
        ...
        try:
            ...
            response = await client.post(
                ...
                params={"key": self._api_key_param},
                json=payload,
            )
            response.raise_for_status()
            result = response.json()
            ...
            raise ReasonerError(
                f"Gemini API returned an error: {e.response.text}",
                code=ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                original_exception=e,
            ) from e
        except Exception as e:
    ...
    async def stream_generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ReasonerError(
                f"Gemini API stream error: {e.response.text}",
                ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                e,
            )
        except Exception as e:
            _logger.critical(
                "gemini_stream_unexpected_error",
                exc_info=True,
                error=str(e),
                endpoint=endpoint,
            )
            ...
            for key, data in multi_modal_data.items():
                ...
                if data.get("data_type") == "image":
                    ...
                    if media_type not in [
                        ...
                        raise ReasonerError(
                            f"Unsupported image type {media_type} for key {key}",
                            ReasonerErrorCode.INVALID_INPUT,
                        )
                    b64_image = base64.b64encode(data["data"]).decode("utf-8")
                    content.insert(
    ...
    async def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ReasonerError(
                f"Anthropic API returned an error: {e.response.text}",
                code=ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                original_exception=e,
            ) from e
        except Exception as e:
    ...
    async def stream_generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise ReasonerError(
                f"Anthropic API stream error: {e.response.text}",
                ReasonerErrorCode.MODEL_INFERENCE_FAILED,
                e,
            )
        except Exception as e:
...
class LLMAdapterFactory:
    ...
    def get_adapter(cls, model_config_json: str) -> LLMAdapter:
        ...
        if adapter_name not in cls._adapters:
            ...
            raise ValueError(
                f"Unknown adapter type: {adapter_name}. Available: {list(cls._adapters.keys())}"
            )
        ...
        if not api_key_value:
            ...
            raise ReasonerError(
                f"API key missing for {adapter_name} in config or env var {key_env_var}",
                ReasonerErrorCode.CONFIGURATION_ERROR,
            )
