...
# Define metrics
AUDIT_SEND_LATENCY = get_or_create_metric(Histogram, 'audit_send_latency_seconds', 'Audit event send latency', ('status',))
AUDIT_ERRORS = get_or_create_metric(Counter, 'audit_errors_total', 'Audit errors', ('code',))
AUDIT_BATCH_SIZE = get_or_create_metric(Histogram, 'audit_batch_size_total', 'Number of events in batch', ('status',))
AUDIT_RATE_LIMIT_HITS = get_or_create_metric(Counter, 'audit_rate_limit_hits_total', 'Rate limit hits', ('endpoint',))
...
class AuditLedgerClient:
    ...
    """
    def __init__(self, ledger_url: str = os.getenv('AUDIT_LEDGER_URL', "https://localhost:8080/audit"),
        ...
        self._logger.info("audit_client_init", ledger_url=self.ledger_url, max_retries=self.max_retries)
    ...
    @tenacity.retry(
        ...
        retry=tenacity.retry_if_exception_type((httpx.HTTPStatusError, httpx.TimeoutException, asyncio.TimeoutError)),
        before_sleep=lambda retry_state: _logger.warning("retry_send", attempt=retry_state.attempt_number, error=str(retry_state.outcome.exception()))
    )
        ...
        if tracer:
            with tracer.start_as_current_span("audit_send_event", kind=SpanKind.CLIENT) as span:
                ...
                try:
                    ...
                    if self._breaker:
                        response = await self._breaker.call_async(client.post, self.ledger_url, json=audit_record, headers=headers)
                    else:
                        response = await client.post(self.ledger_url, json=audit_record, headers=headers)
                    response.raise_for_status()
                    self._logger.info("audit_event_sent_success", event_type=audit_record["event_type"])
                    if AUDIT_SEND_LATENCY:
                        AUDIT_SEND_LATENCY.labels(status='success').observe(time.monotonic() - start_time)
                    if span:
                    ...
                    if status_code == 429:
                        retry_after = float(e.response.headers.get('Retry-After', self.initial_backoff_delay))
                        remaining = e.response.headers.get('x-ratelimit-remaining', None)
                        if remaining and int(remaining) < 5:
                            self._logger.warning("low_rate_limit", endpoint=self.ledger_url, remaining=remaining, retry_after=retry_after)
                            AUDIT_RATE_LIMIT_HITS.labels(endpoint=self.ledger_url).inc()
                            raise ReasonerError(f"Rate limit hit; retry after {retry_after}s", ReasonerErrorCode.SERVICE_UNAVAILABLE, e)
                    ...
                    raise ReasonerError("Audit log failed after retries", ReasonerErrorCode.AUDIT_LOG_FAILED, e)
        else:
    ...
    async def log_event(self, event_type: str, details: Dict[str, Any], operator: str = "system") -> bool:
        ...
        audit_record = {
            ...
            "record_hash": hashlib.sha256(json.dumps(redacted_details, sort_keys=True, default=str).encode('utf-8')).hexdigest()
        }
        self._logger.info("auditing_event", event_type=event_type, operator=operator, record_hash=audit_record["record_hash"])
        try:
            ...
            self._logger.error("audit_log_structured_error", event_type=event_type, message=e.message)
            return False
        except Exception as e:
            self._logger.critical("audit_log_unhandled_exception", event_type=event_type, error=str(e), exc_info=True)
            return False
    ...
    async def log_batch_events(self, events: List[Dict[str, Any]]) -> bool:
        ...
        if tracer:
            with tracer.start_as_current_span("audit_log_batch", kind=SpanKind.CLIENT) as span:
                ...
                results = await asyncio.gather(*[log_single_event(event) for event in events], return_exceptions=True)
        else:
            results = await asyncio.gather(*[log_single_event(event) for event in events], return_exceptions=True)
        ...
        for result, event in zip(results, events):
            if isinstance(result, Exception) or not result:
                success = False
                self._logger.warning("batch_event_failed", event_type=event.get("event_type", "unknown"), error=str(result) if isinstance(result, Exception) else "failed")
        if AUDIT_BATCH_SIZE:
            ...
            with tracer.start_as_current_span("audit_health_check", kind=SpanKind.CLIENT) as span:
                try:
                    ...
                    if self.health_endpoint:
                        response = await client.get(f"{self.ledger_url}/{self.health_endpoint.lstrip('/')}", headers=headers)
                        span.set_attribute("http.method", "GET")
                        span.set_attribute("http.url", f"{self.ledger_url}/{self.health_endpoint.lstrip('/')}")
                    else:
                        response = await client.post(self.ledger_url, json={"ping": "true"}, headers=headers)
    ...
    async def close(self):
        ...
        if self._client and not self._client.is_closed:
            try:
                ...
                raise ReasonerError(f"Failed to close client: {str(e)}", ReasonerErrorCode.UNEXPECTED_ERROR, e)
    ...
    async def test_client():
        ...
        try:
            ...
            success = await client.log_event(
                "test_event",
                {"detail": "Test event", "email": "user@example.com"}
            )
            ...
            batch = [
                ...
                {"event_type": "batch_event_2", "details": {"data": "Batch 2", "email": "user2@example.com"}}
            ]
