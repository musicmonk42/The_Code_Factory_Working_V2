...
from pydantic import BaseModel, Field, ValidationError, HttpUrl, ConfigDict, model_validator, field_validator
import logging
import structlog
import yaml
...
class ReasonerConfig(BaseModel):
    ...
    def validate_dependencies(self):
        ...
        if self.distributed_history_backend == "postgres" and not (self.postgres_db_url and self.postgres_db_url.get_actual_value()):
            raise ValueError("Postgres URL is required when 'distributed_history_backend' is 'postgres'.")
    ...
    def from_file(cls, file_path: Union[str, Path]) -> "ReasonerConfig":
        ...
        try:
            if file_path.suffix.lower() in ['.yaml', '.yml']:
                ...
                raise ValueError(f"Unsupported file format: {file_path.suffix}. Use .json or .yaml/.yml.")
