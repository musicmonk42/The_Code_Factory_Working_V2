...
if PYDANTIC_V2:
   ...
   class SettingsConfigDict(dict): pass  # Dummy for v1
...
def get_or_create_histogram(name: str, documentation: str, labelnames: Tuple[str, ...] = (), buckets: Tuple[float, ...] = (0.001, 0.01, 0.1, 0.5, 1, 2, 5, 10)):
    with _metrics_lock:
        try:
...
# Prometheus metrics
CONFIG_ACCESS = get_or_create_counter('config_access_total', 'Total configuration accesses', ('setting',))
CONFIG_ERRORS = get_or_create_counter('config_errors_total', 'Total configuration errors', ('error_type',))
CONFIG_OPS_TOTAL = get_or_create_counter("config_ops_total", "Total config operations", ["operation"])
...
class LLMSettings(BaseSettings):
    ...
    api_key: Optional[SecretStr] = Field(default=SecretStr("sk-dummy-llm-key-for-tests"), env="OPENAI_API_KEY")
...
class ArbiterConfig(BaseSettings):
    ...
    ENCRYPTION_PASSWORD: str = Field(default="darshan", env="ENCRYPTION_PASSWORD", description="Password for API encryption (default: 'darshan')")
    ENCRYPTION_KEY: Optional[SecretStr] = Field(default=SecretStr("default-encryption-key-for-tests-only-must-be-32-bytes"), env="ENCRYPTION_KEY")
    ...
    ADMIN_API_KEY: Optional[SecretStr] = Field(default=SecretStr("dummy-admin-key-for-tests"), env="ADMIN_API_KEY")
    ...
    ARENA_JWT_SECRET: SecretStr = Field(default=SecretStr("default-arena-jwt-secret"), env="ARENA_JWT_SECRET")
    ...
    MERKLE_TREE_PRIVATE_KEY: Optional[SecretStr] = Field(default=None, env="MERKLE_TREE_PRIVATE_KEY")
    ...
    DREAM_MODE_HISTORY_DB: str = Field(default="sqlite:///./dream_history.db", env="DREAM_MODE_HISTORY_DB")
    ...
    REASONER_HISTORY_DB: str = Field(default="sqlite:///./reasoner_history.db", env="REASONER_HISTORY_DB")
    ...
    KAFKA_SCHEMA_REGISTRY_URL: Optional[HttpUrl] = Field(default=None, env="KAFKA_SCHEMA_REGISTRY_URL")
    GROWTH_MAX_OPERATIONS: int = Field(1000, description="Max pending operations for growth manager")
    ARRAY_STORAGE_TYPE: str = Field("json", description="Array storage type: json, sqlite, redis, postgres")
    ...
    OMNICORE_URL: HttpUrl = Field(default="https://api.example.com", description="OmniCore API endpoint")
    ROLE_MAP: Dict[str, int] = Field(default_factory=lambda: {"guest": 0, "user": 1, "explorer_user": 2, "admin": 3})
    ...
    RL_MODEL_PATH: str = Field(default="./models/ppo_model.zip", description="Path to save/load RL model")
    ...
    @field_validator("OMNICORE_URL", "SLACK_WEBHOOK_URL", "ALERT_WEBHOOK_URL", "PROMETHEUS_GATEWAY", "KAFKA_SCHEMA_REGISTRY_URL", mode="before")
    ...
    def initialize(cls) -> "ArbiterConfig":
        ...
        with cls._singleton_lock:
            ...
            try:
                ...
                encryption_key_val = instance.ENCRYPTION_KEY.get_secret_value() if instance.ENCRYPTION_KEY else ""
                if encryption_key_val and encryption_key_val != "default-encryption-key-for-tests-only-must-be-32-bytes":
                    try:
                    ...
                    instance.ENCRYPTION_KEY = SecretStr(instance.ENCRYPTION_KEY_BYTES.decode('utf-8'))
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                if file_path.endswith('.json'):
                    try:
                        ...
                        logger.error(f"Invalid JSON in config file {file_path}: {e}", exc_info=True)
                        raise ValueError(f"Invalid JSON in config file: {e}")
                else:
                    try:
                        data = yaml.safe_load(f)
                    except yaml.YAMLError as e:
                        logger.error(f"Invalid YAML in config file {file_path}: {e}", exc_info=True)
    ...
    def encrypt_sensitive_fields(self) -> None:
        ...
        sensitive_field_names = [
            'EMAIL_SMTP_PASSWORD', 'ADMIN_API_KEY', 'JWT_SECRET_KEY',
    ...
    def _validate_custom_settings(self):
        ...
        if not llm_api_key or llm_api_key == "sk-dummy-llm-key-for-tests":
            logger.warning("LLM_API_KEY is missing or set to a dummy value. LLM functionality may be limited.")
        ...
        if isinstance(current_email_recipients, str):
            self.EMAIL_RECIPIENTS_LIST = [r.strip() for r in current_email_recipients.split(',') if r.strip()]
        else:
        ...
        for key_name, secret_field in critical_secrets.items():
            if secret_field and isinstance(secret_field, SecretStr) and secret_field.get_secret_value().strip() == "":
                logger.warning(f"CRITICAL: {key_name} is not set or empty. This may impact core functionality.")
    ...
    async def stream_config_change(cls, key: str, value: Any):
        ...
        try:
            ...
            async with redis.from_url(redis_url_str, decode_responses=True) as redis_client:
                ...
                await redis_client.publish('config_events', json.dumps({
                    'key': key,
                    'value': safe_value,
    ...
    async def rotate_encryption_key(self) -> None:
        ...
        try:
            ...
            logger.warning("Encryption key rotated. Remember to re-encrypt all data and update a secure vault.")
    ...
    if os.path.exists(persona_file_path):
        try:
            with open(persona_file_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(personas, dict) and all(isinstance(k, str) and isinstance(v, str) for k, v in personas.items()):
                    logger.info(f"Loaded personas from {persona_file_path}.")
                    CONFIG_OPS_TOTAL.labels(operation="load_persona").inc()
                    return personas
                else:
                    logger.warning(f"Persona file {persona_file_path} content is invalid. Expected Dict[str, str]. Using empty dict.")
                    return {}
        except (json.JSONDecodeError, IOError) as e:
            logger.error(f"Failed to load personas from {persona_file_path}: {e}. Using empty dict.", exc_info=True)
...
registry.register(kind=PlugInKind.CORE_SERVICE, name="ArbiterConfig", version="1.0.0", author="Arbiter Team")(ArbiterConfig)