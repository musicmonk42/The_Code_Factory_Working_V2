...
# Prometheus Metrics
explorer_ops_total = Counter("explorer_ops_total", "Total explorer operations", ["explorer_id", "operation"])
explorer_errors_total = Counter("explorer_errors_total", "Total explorer errors", ["explorer_id", "error_type"])
...
class LogDB:
        ...
        async with self._lock:
            try:
                ...
                async with self.db_client.get_session() as session:
                    session.add(ExperimentLog(id=log_entry["id"], data=log_entry, timestamp=datetime.now(timezone.utc)))
    ...
    async def get_experiment_log(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._lock:
            try:
                async with self.db_client.get_session() as session:
                    result = await session.execute(select(ExperimentLog).filter_by(id=experiment_id))
        ...
        async with self._lock:
            try:
                async with self.db_client.get_session() as session:
                    ...
                    logs = [log.data for log in results.scalars()
                            if all(key in log.data and log.data[key] == value for key, value in query.items())]
...
class Explorer:
    ...
    """
    def __init__(self, sandbox_env: Any, log_db: Optional[LogDB] = None, config: Optional[ArbiterConfig] = None):
        """
        ...
        logger.info(f"Explorer '{self.explorer_id}' initialized with sandbox environment and log DB.")
    ...
    async def execute(self, action: str, **kwargs) -> Dict[str, Any]:
            ...
            try:
                ...
                async with asyncio.timeout(30):
                    if action == "get_status":
                        ...
                        return {"urls": await self.discover_urls(kwargs.get("html_discovery_dir", "public"))}
                ...
                explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="timeout").inc()
        ...
        with tracer.start_as_current_span(f"explorer_execute_{action}"):
            try:
                ...
                explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="execute").inc()
    ...
    async def run_experiment(self, experiment_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for variant in variants:
                result = await self.sandbox_env.evaluate(variant, metric=experiment_config.get("metric"))
            ...
            explorer_ops_total.labels(explorer_id=self.explorer_id, operation="run_experiment").inc()
            ...
            explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="run_experiment").inc()
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_crawl": {"errors": [], "timestamp": datetime.now(timezone.utc).isoformat()}
            }
        except Exception as e:
            ...
            return {"health": "degraded", "last_crawl": {"errors": [str(e)], "timestamp": datetime.now(timezone.utc).isoformat()}}
    ...
    async def discover_urls(self, html_discovery_dir: str) -> List[str]:
        ...
        try:
            ...
            explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="discover_urls").inc()
            ...
            for url in urls:
                async with limiter:
                    try:
                        ...
                        logger.error(f"[{self.explorer_id}] Error crawling {url}: {e}", exc_info=True)
                        results.append({"url": url, "status": "error", "error": str(e)})
                        explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="crawl").inc()
    ...
    async def explore_and_fix(self, arbiter, fix_paths: Optional[List[str]]) -> Dict[str, Any]:
        ...
        try:
                ...
                if arbiter.analyzer:
                    ...
                    for issue in issues:
                        if issue["suggested_fixer"] == "self_healing_import_fixer":
                            ...
                            logger.info(f"[{self.explorer_id}] Fixed issue {issue['type']} at {path}")
            explorer_ops_total.labels(explorer_id=self.explorer_id, operation="explore_and_fix").inc()
            ...
            explorer_errors_total.labels(explorer_id=self.explorer_id, error_type="explore_and_fix").inc()
            raise ExperimentExecutionError(f"Explore and fix failed: {e}") from e

    async def replay_experiment(self, experiment_id: str, new_sandbox_env: Optional[Any] = None) -> Dict[str, Any]:
        """
        ...
        if initial_random_state_str:
            try:
                ...
                logger.warning(f"Could not reset random state for replay {experiment_id} due to malformed state: {e}")
    ...
    async def _run_ab_test(self, experiment_id: str, variant_a_agent: Any, variant_b_agent: Any, runs: int, metrics: Optional[List[str]] = None, **kwargs) -> Dict[str, Any]:
        """
        Internal implementation for A/B testing two agent variants.
        """
        logger.info(f"Running A/B test for experiment {experiment_id} with {runs} runs per variant.")
        ...
        # Compare
        results["summary"] = self._compare_variants(results["variant_a"]["metrics"], results["variant_b"]["metrics"])
        logger.info(f"A/B test summary for {experiment_id}: {results['summary']}")
        return results

    async def _run_evolution_experiment(self, experiment_id: str, initial_population: List[Any], generations: int, **kwargs) -> Dict[str, Any]:
        """
        Internal implementation for an evolutionary experiment.
        """
        logger.info(f"Running evolution experiment for {experiment_id} over {generations} generations.")
        ...
        for gen in range(generations):
            ...
            if not current_population:
                logger.warning(f"Population became empty at generation {gen} for experiment {experiment_id}. Stopping evolution.")
                break

            fitness_scores = await asyncio.gather(*[
                self.sandbox_env.evaluate(agent_variant, **kwargs) for agent_variant in current_population
            ])
            sorted_population = sorted(zip(fitness_scores, current_population), key=lambda x: x[0], reverse=True)
            ...
            logger.debug(f"Generation {gen} - Best fitness: {best_agent_fitness}, Population size: {len(current_population)}")

            if not best_agent:
                logger.error(f"No best agent found in generation {gen} for experiment {experiment_id}.")
                break
        ...
        logger.info(f"Evolution experiment {experiment_id} finished. Final best agent: {final_best_agent_name}, Final best fitness: {final_best_fitness}")

        return {
            "evolution_trace": evolution_trace,
            "final_best_agent_details": {"name": final_best_agent_name, "fitness": final_best_fitness},
    ...
    def _calculate_metrics(self, runs_data: List[Dict[str, Any]], metrics: Optional[List[str]]) -> Dict[str, Any]:
        """
        ...
        for metric_name in metrics:
            ...
            if values and all(isinstance(v, (int, float)) for v in values):
                try:
                    ...
                    logger.warning(f"Failed to calculate numerical statistics for metric '{metric_name}': {e}")
    ...
    def _compare_variants(self, metrics_a: Dict[str, Any], metrics_b: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for metric_name_a, value_a in metrics_a.items():
            if "_avg" in metric_name_a and metric_name_a in metrics_b:
                ...
                if avg_a is not None and avg_b is not None:
                    ...
                    summary["comparison"][metric_name_a] = {"verdict": "One or both variants lack metric value"}
        logger.debug(f"Comparison summary: {summary}")
        return summary

# Register as a plugin
registry.register(kind=PlugInKind.CORE_SERVICE, name="Explorer", version="1.0.0", author="Arbiter Team")(Explorer)
...
class ArbiterExplorer:
    ...
    async def run_ab_test(self, experiment_name: str, variant_a: Any, variant_b: Any, 
                          num_runs: int = 1, metric: str = "perf") -> Dict[str, Any]:
        """
    ...
    async def run_evolutionary_experiment(self, experiment_name: str, initial_agent: Any,
                                          num_generations: int = 1, population_size: int = 1,
                                          metric: str = "perf") -> Dict[str, Any]:
        """
    ...
    async def _run_experiment(self, name: str, experiment_func: Callable, 
    ...
    async def _log_experiment(self, entry: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Failed to log experiment {entry.get('experiment_id')}: {str(e)}", 
                         exc_info=True)
    ...
    def _compare_variants(self, metrics_a: Dict[str, Any], 
