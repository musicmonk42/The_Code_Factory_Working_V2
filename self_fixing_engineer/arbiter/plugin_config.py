...
# Prometheus metrics
plugin_config_ops_total = Counter(
    "plugin_config_ops_total", "Total plugin config operations", ["operation"]
)
plugin_config_errors_total = Counter(
    "plugin_config_errors_total", "Total plugin config errors", ["operation"]
)
        ...
        with tracer.start_as_current_span("validate_plugin_registry"):
            ...
            for key, value in plugins.items():
                if not isinstance(key, str) or not isinstance(value, str):
                    ...
                    raise TypeError(
                        f"Plugin registry keys and values must be strings. Found key: {key}, value: {value}"
                    )
        ...
        with tracer.start_as_current_span("register_plugin"):
            async with cls._REGISTRY_LOCK:
                if not isinstance(name, str) or not isinstance(import_path, str):
                    ...
                    raise TypeError(
                        f"Plugin name and import path must be strings: {name}, {import_path}"
                    )
