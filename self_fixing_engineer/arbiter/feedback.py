...
def _get_or_create_metric(
    ...
    # Must be checked before any isinstance() calls with metric_class
...
feedback_received_total = _get_or_create_metric(
    Counter, "feedback_received_total", "Total feedback received", ("type",)
)
feedback_errors_total = _get_or_create_metric(
    Counter, "feedback_errors_total", "Total errors logged", ("component",)
)
feedback_metrics_recorded_total = _get_or_create_metric(
    Counter,
    "feedback_metrics_recorded_total",
    "Total metrics recorded",
    ("metric_name",),
)
feedback_processing_time = _get_or_create_metric(
    Histogram,
    "feedback_processing_time",
    "Time spent processing feedback",
    buckets=(0.001, 0.01, 0.1, 1, 10),
)
human_in_loop_approvals = _get_or_create_metric(
    Counter, "human_in_loop_approvals_total", "Total human approvals"
)
human_in_loop_denials = _get_or_create_metric(
    Counter, "human_in_loop_denials_total", "Total human denials"
)
last_feedback_timestamp = _get_or_create_metric(
    Gauge, "last_feedback_timestamp", "Timestamp of the last feedback received"
)
feedback_ops_total = _get_or_create_metric(
    Counter, "feedback_ops_total", "Total feedback operations", ["operation"]
)
...
class SQLiteClient:
    ...
    def __init__(self, db_file: str = "feedback.db"):
        if not SQLLITE_AVAILABLE:
            raise ImportError(
                "aiosqlite is required for SQLiteClient. Install with 'pip install aiosqlite'"
            )
...
class FeedbackManager:
    ...
    def __init__(
        self,
        db_client: Optional[Union[SQLiteClient, PostgresClient]] = None,
        config: Optional[Any] = None,
        log_file: str = "feedback_log.json",
        ...
        if db_client:
            ...
            if (
                ...
                if IS_PRODUCTION:
                    raise RuntimeError(
                        f"FeedbackManager: In production, DATABASE_URL '{db_url}' is not supported or its client/driver is not available. Refusing to start."
                    )
                else:
                    self.db_client = SQLiteClient(db_file="feedback.db")
                    logger.warning(
                        f"FeedbackManager: Development mode: DATABASE_URL '{db_url}' not recognized or driver not available. Falling back to SQLiteClient (feedback.db)."
                    )
        else:
            if IS_PRODUCTION:
                raise RuntimeError(
                    "FeedbackManager: In production, DATABASE_URL is not set. Refusing to start without a real database."
                )
            else:
                self.db_client = SQLiteClient(db_file="feedback.db")
                logger.warning(
                    "FeedbackManager: Development mode: No DATABASE_URL found in config. Falling back to SQLiteClient (feedback.db)."
                )
    ...
    async def _sync_and_filter_to_logfile(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to write all data to feedback log file {self.log_file} from DB: {e}",
                exc_info=True,
            )
    ...
    async def add_feedback(self, decision_id: str, feedback: Dict[str, Any]) -> None:
        ...
        with tracer.start_as_current_span("add_feedback"):
            ...
            try:
                ...
                logger.error(
                    f"Failed to store feedback for decision {decision_id}: {e}",
                    exc_info=True,
                )
    ...
    async def record_metric(
        ...
        if not isinstance(value, (int, float)) or isinstance(value, bool):
            logger.error(
                f"Invalid metric value: {value} (type: {type(value)}) for metric {name}. Must be int or float, not bool."
            )
        ...
        metric_entry = {
            # Security: Use SHA-256 instead of MD5 for hashing
            "id": hashlib.sha256(
                f"{name}-{value}-{datetime.now(timezone.utc).isoformat()}-{random.randint(0, 100000)}".encode(
                    "utf-8"
                )
            ).hexdigest(),
            "type": "metric",
    ...
    async def add_user_feedback(self, feedback: Dict[str, Any]):
        ...
        try:
            ...
            last_feedback_timestamp.set(datetime.now(timezone.utc).timestamp())
            logger.info(
                f"User feedback added: Decision ID={feedback.get('decision_id', 'N/A')}, Approved={feedback.get('approved', 'N/A')}"
            )
        except Exception as e:
    ...
    async def get_summary(self) -> Dict[str, Any]:
        ...
        with feedback_processing_time.time():
            try:
                ...
                all_approval_requests = await self.db_client.get_feedback_entries(
                    {"type": "approval_request"}
                )
                all_approval_responses = await self.db_client.get_feedback_entries(
                ...
                summary = {
                    ...
                    "approval_requests_summary": {
                        ...
                        "approved_count": sum(
                            1
                            for r in all_approval_responses
                            if r.get("response", {}).get("approved") is True
                        ),
                        "denied_count": sum(
                            1
                            for r in all_approval_responses
                            if r.get("response", {}).get("approved") is False
                        ),
                    },
                }
    ...
    async def log_approval_response(self, decision_id: str, response: Dict[str, Any]):
        ...
        try:
            ...
            await self.db_client.update_feedback_entry(
                ...
                {
                    "status": "resolved",
                    "resolution_timestamp": response_timestamp,
                    "response_details": response,
                },
            )
            logger.info(
                f"Approval response logged for {decision_id}: Approved={response.get('approved')}, User={response.get('user_id', 'N/A')}"
            )
    ...
    async def get_approval_stats(
        ...
        try:
            ...
            if group_by_reviewer:
                reviewer_stats: Dict[str, Dict[str, int]] = collections.defaultdict(
