...
def _sanitize_label(value: Any) -> str:
    ...
    with tracer.start_as_current_span(
        "sanitize_label", attributes={"raw_value": str(value)}
    ) as span:
...
COMPLIANCE_CONTROL_ACTIONS_TOTAL = get_or_create_metric(
    ...
    (
        ...
        "result",
        "action_type",
    ),  # e.g., control_id='NIST_AC-1', result='passed', action_type='auto_learn'
)
...
COMPLIANCE_METRIC_INIT_ERRORS = get_or_create_metric(
    Counter,
    "compliance_metric_init_errors_total",
...
COMPLIANCE_CONTROL_ACTIVE = get_or_create_metric(
    Gauge,
    "compliance_control_active_total",
...
METRIC_REFRESH_STATE_TRANSITIONS = get_or_create_metric(
    Counter,
    "metric_refresh_state_transitions_total",
...
METRIC_REFRESH_LATENCY = get_or_create_metric(
    Histogram,
    "metric_refresh_latency_seconds",
    "Latency of compliance metric refresh task",
...
def record_policy_decision(
    ...
    with tracer.start_as_current_span(
        ...
        sanitized_allowed = (
            _sanitize_label(allowed) if allowed in VALID_ALLOWED_VALUES else "invalid"
        )
        if sanitized_allowed == "invalid":
            _log_error_rate_limited(
                f"Invalid allowed value: {allowed}. Must be one of {VALID_ALLOWED_VALUES}",
                "invalid_allowed",
            )
            span.record_exception(ValueError(f"Invalid allowed: {allowed}"))
        sanitized_user_type = (
            _sanitize_label(user_type) if user_type in VALID_USER_TYPES else "invalid"
        )
...
def record_llm_call_latency(provider: str, latency: float) -> None:
    ...
    with tracer.start_as_current_span(
        "record_llm_call_latency", attributes={"provider": provider, "latency": latency}
    ) as span:
...
def record_compliance_violation(control_id: str, violation_type: str) -> None:
    ...
    with tracer.start_as_current_span(
        ...
        attributes={"control_id": control_id, "violation_type": violation_type},
    ) as span:
        sanitized_violation_type = (
            _sanitize_label(violation_type)
            if violation_type in VALID_VIOLATION_TYPES
            else "invalid"
        )
        ...
        try:
            ...
            if hasattr(load_compliance_map, "_redis_pool"):
                try:
                    ...
                    _log_error_rate_limited(
                        f"Error cleaning up Redis pool: {e}", "redis_cleanup_failed"
                    )
                    span.record_exception(e)
...
@retry(
    ...
    before_sleep=lambda retry_state: logger.debug(
        f"Retrying compliance metric initialization: attempt {retry_state.attempt_number}"
    ),
)
    ...
    with tracer.start_as_current_span("initialize_compliance_metrics") as span:
        ...
        for control_id, control_info in ALL_COMPLIANCE_CONTROLS.items():
            ...
            if (
                ...
                or not control_info.get("name")
                or not isinstance(control_info.get("status"), str)
            ):
                _log_error_rate_limited(
    ...
    while True:
        with tracer.start_as_current_span("refresh_compliance_metrics") as span:
            ...
            if pause_value not in ("true", "false"):
                _log_error_rate_limited(
                    f"Invalid PAUSE_METRIC_REFRESH_TASKS: {pause_value}",
                    "invalid_config",
                )
                span.record_exception(
                    ValueError(f"Invalid PAUSE_METRIC_REFRESH_TASKS: {pause_value}")
                )
                pause_value = "false"
            ...
            if is_paused != _last_pause_state:
                logger.info(
                    f"Compliance metric refresh task {'paused' if is_paused else 'resumed'}"
                )
                METRIC_REFRESH_STATE_TRANSITIONS.labels(
            ...
            try:
                ...
                _log_error_rate_limited(
                    f"Error refreshing compliance metrics: {e}", "refresh_failed"
                )
                span.record_exception(e)
                span.set_attribute("refresh_status", "failed")

            _last_refresh_time = time.monotonic()
            METRIC_REFRESH_LATENCY.labels(operation="refresh").observe(
