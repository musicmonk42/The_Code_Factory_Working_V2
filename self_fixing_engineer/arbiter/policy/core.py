...
SQLITE_QUERY_LATENCY = get_or_create_metric(
    Histogram,
    "sqlite_query_latency_seconds",
    "Latency of SQLite database queries",
...
POLICY_REFRESH_STATE_TRANSITIONS = get_or_create_metric(
    Counter,
    "policy_refresh_state_transitions_total",
...
POLICY_ENGINE_INIT_ERRORS = get_or_create_metric(
    Counter,
    "policy_engine_init_errors_total",
...
POLICY_ENGINE_RESET_ERRORS = get_or_create_metric(
    Counter,
    "policy_engine_reset_errors_total",
...
class SQLiteClient:
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying SQLite connection: attempt {retry_state.attempt_number}"
        ),
    )
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying save_feedback_entry: attempt {retry_state.attempt_number}"
        ),
    )
    async def save_feedback_entry(self, entry: Dict[str, Any]):
        """Saves a feedback entry with rate-limiting and JSON sanitization."""
        with tracer.start_as_current_span(
            "save_feedback_entry", attributes={"entry_id": entry.get("id")}
        ) as span:
            ...
            if "id" not in entry_copy:
                ...
                entry_copy["id"] = hashlib.sha256(
                    json.dumps(entry_copy, sort_keys=True).encode("utf-8")
            ...
            if self._conn:
                ...
                """,
                    (
                        entry_copy["id"],
                        entry_copy["type"],
                        json.dumps(entry_copy, default=str),
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying get_feedback_entries: attempt {retry_state.attempt_number}"
        ),
    )
    async def get_feedback_entries(
        ...
        with tracer.start_as_current_span(
            "get_feedback_entries", attributes={"query": str(query)}
        ) as span:
                ...
                for row in rows:
                    ...
                    for k, v in updates.items():
                        if not isinstance(k, str) or not isinstance(
                            ...
                            span.record_exception(
                                ValueError(f"Invalid update key or value: {k}={v}")
                            )
                    ...
                    updated = True
                await self._conn.commit()
                SQLITE_QUERY_LATENCY.labels(operation="update").observe(
                    time.monotonic() - start_time
                )
                return updated
            return False
    ...
    async def close(self):
        ...
        with tracer.start_as_current_span(
            "sqlite_close", attributes={"db_file": self.db_file}
        ) as span:
            try:
                if self._conn:
...
class BasicDecisionOptimizer:
    ...
    def __init__(self, settings: Optional[Dict] = None):
        ...
        self.score_rules = self.settings.get(
            "score_rules",
            {
        ...
        required_keys = [
            "login_attempts_penalty",
            "device_trusted_bonus",
            "recent_login_bonus",
            "admin_user_bonus",
            "default_score",
        ]
    ...
    async def compute_trust_score(
        ...
        with tracer.start_as_current_span(
            ...
            if "login_attempts" in auth_context and not isinstance(
                auth_context["login_attempts"], int
            ):
                span.record_exception(ValueError("login_attempts must be an integer"))
                raise ValueError("login_attempts must be an integer")
            if "device_trusted" in auth_context and not isinstance(
                auth_context["device_trusted"], bool
            ):
...
class PolicyEngine:
    def __init__(self, arbiter_instance: Any, config: ArbiterConfig):
        with tracer.start_as_current_span("policy_engine_init") as span:
            ...
            required_config_keys = [
                ...
                "CIRCUIT_BREAKER_MIN_OPERATION_INTERVAL",
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying LLMClient creation: attempt {retry_state.attempt_number}"
        ),
    )
    ...
    async def _call_llm_for_policy_evaluation(
        ...
        with tracer.start_as_current_span(
            ...
            attributes={"prompt": self._sanitize_prompt(prompt)},
        ) as span:
            ...
            if len(prompt) > 10000:
                span.record_exception(
                    ValueError("Prompt exceeds maximum length of 10000 characters")
                )
                return "NO", "Prompt too long.", 0.0
            current_time = time.monotonic()
            if current_time - self._last_llm_call_time < self._llm_call_interval:
                await asyncio.sleep(
                    self._llm_call_interval - (current_time - self._last_llm_call_time)
                )
            ...
            try:
                ...
                LLM_CALL_LATENCY.labels(provider=llm_provider).observe(
                    time.monotonic() - start_time
                )
                ...
                logger.error(
                    f"LLM call timed out after {self.config.LLM_API_TIMEOUT_SECONDS} seconds"
                )
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying policy load: attempt {retry_state.attempt_number}"
        ),
    )
    def _load_policies_from_file(self):
        ...
        with tracer.start_as_current_span(
            ...
            attributes={"file_path": self.config.POLICY_CONFIG_FILE_PATH},
        ) as span:
            try:
                ...
                with open(
                    ...
                    if old_policies and self._policies != old_policies:
                        asyncio.create_task(
                            self._audit_policy_changes(old_policies, self._policies)
                        )
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying compliance controls load: attempt {retry_state.attempt_number}"
        ),
    )
    def _load_compliance_controls(self):
        ...
        with tracer.start_as_current_span("load_compliance_controls") as span:
            try:
                ...
                logger.error(
                    f"Error loading compliance controls via compliance_mapper: {e}",
                    exc_info=True,
                )
    ...
    def _get_default_policies(self) -> Dict[str, Any]:
        ...
        return {
            "domain_rules": {
                ...
                "*": {
                    "allow": True,
                    "reason": "Default policy for unspecified domains",
                },
            },
            "user_rules": {
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying custom rule registration: attempt {retry_state.attempt_number}"
        ),
    )
        ...
        with self._lock:
            ...
            if not self.validate_policies(proposed_policies):
                ...
                policy_decision_total.labels(
                    allowed="false",
                    domain="policy_adaptation",
                    user_type="evolution_strategy",
                    reason_code="invalid_proposed_policies",
                ).inc()
            ...
            try:
                ...
                await self._audit_policy_changes(
                    old_policies, proposed_policies, path="policy_adaptation"
                )
            ...
            await self._audit_policy_decision(
                ...
                proposed_policies,
                True,
                "Policy update applied successfully from evolution strategy.",
                control_tag="PC-1",
                is_compliant=True,
            )
            policy_decision_total.labels(
    ...
    def validate_policies(policies: Dict[str, Any]) -> bool:
        ...
        for domain, rule in policies["domain_rules"].items():
            if (
                ...
                or "allow" not in rule
                or not isinstance(rule["allow"], bool)
            ):
                return False
        ...
        if not isinstance(llm_rules.get("enabled", False), bool) or not isinstance(
            llm_rules.get("prompt_template", ""), str
        ):
            return False
        trust_rules = policies["trust_rules"]
        if not isinstance(trust_rules.get("enabled", False), bool) or not isinstance(
            trust_rules.get("threshold", 0.0), (int, float)
        ):
            return False
        return True

    async def _audit_policy_changes(
        self, old_policies: Dict[str, Any], new_policies: Dict[str, Any], path: str = ""
    ):
        for k, v in new_policies.items():
            ...
            if k not in old_policies:
                await self._audit_policy_decision(
                    ...
                    None,
                    True,
                    f"Policy added: {current_path} = {json.dumps(v, default=str)}",
                )
                ...
                await self._audit_policy_decision(
                    ...
                    None,
                    True,
                    f"Policy changed: {current_path} from {json.dumps(old_policies[k], default=str)} to {json.dumps(v, default=str)}",
                )
        for k, v in old_policies.items():
            if k not in new_policies:
                ...
                await self._audit_policy_decision(
                    ...
                    None,
                    True,
                    f"Policy removed: {removed_path} = {json.dumps(v, default=str)}",
                )
    ...
    async def _enforce_compliance(
        ...
        with tracer.start_as_current_span(
            ...
            if not control_tag:
                ...
                return (
                    False,
                    f"Action '{action_name}' is not mapped to any compliance control. Blocking due to undefined compliance.",
                )
            ...
            if not control_info:
                ...
                return (
                    False,
                    f"Compliance control '{control_tag}' for action '{action_name}' is not defined. Blocking due to unknown control.",
                )
            ...
            if required and status == "not_implemented":
                ...
                return (
                    False,
                    f"Compliance control '{control_tag}' for action '{action_name}' is required but marked 'not_implemented'.",
                )
            if required and status == "partially_enforced":
                span.set_attribute("compliance_status", "partially_enforced")
                return (
                    False,
                    f"Compliance control '{control_tag}' for action '{action_name}' is required but only 'partially_enforced'.",
                )
            if required and status != "enforced":
                span.set_attribute("compliance_status", status)
                return (
                    False,
                    f"Compliance control '{control_tag}' for action '{action_name}' is required but not 'enforced'. Current status: '{status}'",
                )
            # Allow optional controls with warnings
            if not required and status in ["not_implemented", "partially_enforced"]:
                logger.warning(
                    f"Optional compliance control '{control_tag}' for action '{action_name}' is '{status}'. Proceeding but compliance may be incomplete."
                )
                span.set_attribute("compliance_status", status)
                return (
                    True,
                    f"Optional control '{control_tag}' is '{status}' (allowed to proceed).",
                )
            span.set_attribute("compliance_status", "enforced")
            return (
                True,
                f"Compliance control '{control_tag}' is 'enforced' for action '{action_name}'.",
            )

    async def should_auto_learn(
        self, domain: str, key: str, user_id: Optional[str], value: Optional[Any] = None
    ) -> Tuple[bool, str]:
        with tracer.start_as_current_span(
            "should_auto_learn",
            attributes={"domain": domain, "key": key, "user_id": user_id},
        ) as span:
            if not isinstance(domain, str) or not re.match(
                self.config.VALID_DOMAIN_PATTERN, domain
            ):
            ...
            with self._lock:
                domain_rule = self._policies["domain_rules"].get(
                    domain,
                ...
                if not domain_rule["allow"]:
                    ...
                    await self._audit_policy_decision(
                        ...
                        False,
                        reason,
                        control_tag=domain_control_tag,
                    )
                    policy_decision_total.labels(
                        allowed="false",
                        domain=domain,
                        user_type=effective_user_id_str,
                        reason_code="domain_rule_denied",
                    ).inc()
                    return False, reason
                ...
                if (
                    ...
                    if not any(
                        ...
                        policy_decision_total.labels(
                            ...
                            domain=domain,
                            user_type=effective_user_id_str,
                            reason_code="insufficient_roles",
                        ).inc()
                        return False, reason
                ...
                if not user_rule["allow"]:
                    ...
                    await self._audit_policy_decision(
                        ...
                        False,
                        reason,
                        control_tag=user_control_tag,
                    )
                    policy_decision_total.labels(
                        allowed="false",
                        domain=domain,
                        user_type=effective_user_id_str,
                        reason_code="user_rule_denied",
                    ).inc()
                    return False, reason
                ...
                if (
                    ...
                    policy_decision_total.labels(
                        ...
                        domain=domain,
                        user_type=effective_user_id_str,
                        reason_code="user_restricted_domain",
                    ).inc()
                    return False, reason
                    ...
                    try:
                        ...
                        if llm_trust_score < self.config.LLM_POLICY_MIN_TRUST_SCORE:
                            ...
                            await self._audit_policy_decision(
                                ...
                                False,
                                reason,
                                control_tag=llm_control_tag,
                            )
                            policy_decision_total.labels(
                                allowed="false",
                                domain=domain,
                                user_type=effective_user_id_str,
                                reason_code="llm_low_trust_score",
                            ).inc()
                            return False, reason
                    ...
                    try:
                        ...
                        if not allowed:
                            ...
                            policy_decision_total.labels(
                                ...
                                domain=domain,
                                user_type=effective_user_id_str,
                                reason_code="custom_rule_denied",
                            ).inc()
                            return (
                                False,
                                f"Custom rule '{custom_rule_func.__name__}' denied: {reason}",
            ...
            compliance_check_passed, compliance_reason = await self._enforce_compliance(
                action_id_for_compliance_check, final_control_tag_for_action
            )
            ...
            if not compliance_check_passed:
                ...
                policy_decision_total.labels(
                    ...
                    domain=domain,
                    user_type=effective_user_id_str,
                    reason_code="compliance_enforcement_blocked",
                ).inc()
    ...
    @retry(
        ...
        retry=retry_if_exception_type(Exception),
        before_sleep=lambda retry_state: logger.debug(
            f"Retrying audit_policy_decision: attempt {retry_state.attempt_number}"
        ),
    )
    async def _audit_policy_decision(
        ...
        if control_tag:
            audit_entry_details["control_tag"] = control_tag
        if is_compliant is not None:
            audit_entry_details["is_compliant"] = is_compliant
        ...
        try:
            ...
            logger.debug(
                f"Audited: {decision_type} - Allowed: {allowed} - Reason: {reason} - Control Tag: {control_tag} - Compliant: {is_compliant}"
            )
        except Exception as e:
    ...
    async def _get_user_roles(self, user_id: str) -> List[str]:
        ...
        with tracer.start_as_current_span(
            "get_user_roles", attributes={"user_id": user_id}
        ) as span:
            try:
    ...
    def _validate_llm_policy_output(
        self, llm_response_text: str, valid_responses: List[str]
    ) -> Tuple[str, str, float]:
        ...
        try:
            ...
            logger.warning(
                "LLM_VALID_RESPONSES not found in ArbiterConfig. Using provided valid_responses."
            )
        decision, reason, trust_score = (
            "NO",
            "LLM response format invalid or ambiguous.",
            0.0,
        )
        try:
            ...
            if len(parts) > 0:
                ...
                if parsed_decision in valid_responses:
                    ...
                    reason, trust_score = (
                        f"LLM decision '{parsed_decision}' not in valid responses.",
                        0.2,
                    )
            else:
            ...
            logger.error(
                f"Error validating LLM policy output: {e}. Raw response: {llm_response_text}"
            )
            reason, trust_score = f"Error processing LLM response: {e}", 0.0
        return decision, reason, trust_score

    async def trust_score_rule(
        self, domain: str, key: str, user_id: Optional[str], value: Optional[Dict]
    ) -> Tuple[bool, str]:
            ...
            try:
                ...
                if score < threshold:
                    ...
                    await self._audit_policy_decision(
                        ...
                        False,
                        reason,
                        control_tag=trust_control_tag,
                        is_compliant=False,
                    )
                    policy_decision_total.labels(
                        allowed="false",
                        domain=domain,
                        user_type=effective_user_id_for_stats,
                        reason_code="trust_score_low",
                    ).inc()
                    span.set_attribute("trust_score", score)
                    return False, reason
                ...
                policy_decision_total.labels(
                    ...
                    domain=domain,
                    user_type=effective_user_id_for_stats,
                    reason_code="trust_score_sufficient",
                ).inc()
                span.set_attribute("trust_score", score)
                return True, reason
            except ImportError:
                ...
                if score < threshold:
                    ...
                    policy_decision_total.labels(
                        ...
                        domain=domain,
                        user_type=effective_user_id_for_stats,
                        reason_code="trust_score_low_fallback",
                    ).inc()
                    span.set_attribute("trust_score", score)
                    return False, reason
                ...
                policy_decision_total.labels(
                    ...
                    domain=domain,
                    user_type=effective_user_id_for_stats,
                    reason_code="trust_score_sufficient_fallback",
                ).inc()
                span.set_attribute("trust_score", score)
                return True, reason
            except Exception as e:
...
async def should_auto_learn(
    domain: str, key: str, user_id: Optional[str], value: Optional[Any] = None
) -> Tuple[bool, str]:
    global _policy_engine_instance
    if _policy_engine_instance is None:
        with _policy_engine_lock:
