...
def get_or_create_metric(metric_class: type, name: str, documentation: str, labelnames: Union[tuple, list] = (), initial_value: float = 0.0, buckets=None):
    """
...
LLM_API_FAILURE_COUNT = get_or_create_metric(
    Counter, 'llm_api_failure_count', 'Total failures for LLM policy API calls.',
    labelnames=('provider',)
)
LLM_CIRCUIT_BREAKER_STATE = get_or_create_metric(
    Gauge, 'llm_circuit_breaker_state', 'Current state of the LLM policy API circuit breaker (0=closed, 1=open).',
    labelnames=('provider',)
)
LLM_CIRCUIT_BREAKER_TRIPS = get_or_create_metric(
    Counter, 'llm_circuit_breaker_trips_total', 'Total number of times the circuit breaker has tripped.',
    labelnames=('provider',)
)
LLM_CIRCUIT_BREAKER_ERRORS = get_or_create_metric(
    Counter, 'llm_circuit_breaker_errors_total', 'Total errors in circuit breaker operations',
    labelnames=('error_type',)
)
LLM_CIRCUIT_BREAKER_TRANSITIONS = get_or_create_metric(
    Counter, 'llm_circuit_breaker_transitions_total', 'Total circuit breaker state transitions',
    labelnames=('provider', 'from_state', 'to_state')
)
CIRCUIT_BREAKER_CLEANUP_OPERATIONS = get_or_create_metric(
    Counter, 'circuit_breaker_cleanup_operations_total', 'Total circuit breaker state cleanup operations',
    labelnames=('provider', 'result')
)
REDIS_OPERATION_LATENCY = get_or_create_metric(
    Histogram, 'circuit_breaker_redis_operation_latency_seconds', 'Latency of Redis operations for circuit breaker',
    labelnames=('provider', 'operation'), buckets=(0.001, 0.01, 0.1, 0.5, 1, 2, 5)
)
CONFIG_REFRESH_OPERATIONS = get_or_create_metric(
    Counter, 'circuit_breaker_config_refresh_operations_total', 'Total configuration refresh operations',
    labelnames=('result',)
)
TASK_STATE_TRANSITIONS = get_or_create_metric(
    Counter, 'circuit_breaker_task_state_transitions_total', 'Total task state transitions (pause/resume)',
...
def get_global_connection_pool(config: 'ArbiterConfig') -> Optional[redis.ConnectionPool]:
    """
    ...
    with _connection_pool_lock:
        if _global_connection_pool is None and getattr(config, 'REDIS_URL', None):
            try:
                ...
                max_connections_str = os.getenv('REDIS_MAX_CONNECTIONS', str(getattr(config, 'REDIS_MAX_CONNECTIONS', 100)))
                socket_timeout_str = os.getenv('REDIS_SOCKET_TIMEOUT', str(getattr(config, 'REDIS_SOCKET_TIMEOUT', 5.0)))
                socket_connect_timeout_str = os.getenv('REDIS_SOCKET_CONNECT_TIMEOUT', str(getattr(config, 'REDIS_SOCKET_CONNECT_TIMEOUT', 5.0)))
                ...
                try:
                    ...
                    _log_validation_error(f"Invalid REDIS_MAX_CONNECTIONS: {max_connections_str}. Using default: 100", 'invalid_config')
                    max_connections = 100
                try:
                    ...
                    _log_validation_error(f"Invalid REDIS_SOCKET_TIMEOUT: {socket_timeout_str}. Using default: 5.0", 'invalid_config')
                    socket_timeout = 5.0
                try:
                    ...
                    _log_validation_error(f"Invalid REDIS_SOCKET_CONNECT_TIMEOUT: {socket_connect_timeout_str}. Using default: 5.0", 'invalid_config')
                ...
                _log_validation_error(f"Failed to create global Redis connection pool: {e}", 'redis_pool_creation_failed')
                _global_connection_pool = None
...
class CircuitBreakerState:
    ...
    def __init__(self, provider: str, config: 'ArbiterConfig'):
        ...
        try:
            ...
            _log_validation_error(f"Invalid CIRCUIT_BREAKER_MIN_OPERATION_INTERVAL: {min_interval_str}. Using default: 0.1", 'invalid_config')
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.debug(f"Retrying Redis connection for {retry_state.args[0].provider if retry_state.args else 'unknown'}: attempt {retry_state.attempt_number}")
    )
        ...
        if redis_url:
            try:
                ...
                if pool:
                    ...
                    logger.debug(f"Connected to Redis for circuit breaker: {_sanitize_provider(self.provider)}")
                else:
                    self.redis_client = None
                    logger.warning(f"No Redis connection pool available for {_sanitize_provider(self.provider)}. Using in-memory state.")
            except redis.ConnectionError as e:
                _log_validation_error(f"Redis connection pool exhausted or failed for {_sanitize_provider(self.provider)}: {e}", 'redis_connection_pool_exhausted')
                self.redis_client = None
                raise redis.RedisError(f"Failed to connect to Redis for {_sanitize_provider(self.provider)}: {e}")
            except Exception as e:
                _log_validation_error(f"Failed to connect to Redis for {_sanitize_provider(self.provider)}: {e}", 'redis_connection_failed')
                self.redis_client = None
                raise redis.RedisError(f"Failed to connect to Redis for {_sanitize_provider(self.provider)}: {e}")
    ...
    async def close(self) -> None:
        ...
        if self.redis_client:
            ...
            logger.debug(f"Closed Redis connection for circuit breaker: {_sanitize_provider(self.provider)}")
            self.redis_client = None
    ...
    async def _rate_limit(self) -> None:
        ...
        with tracer.start_as_current_span("rate_limit", attributes={"provider": self.provider}) as span:
            current_time = time.monotonic()
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=1, max=10),
        retry=retry_if_exception_type(redis.RedisError),
        before_sleep=lambda retry_state: logger.debug(f"Retrying Redis get operation for provider: attempt {retry_state.attempt_number}")
    )
    async def get_state(self) -> Dict[str, Any]:
        ...
        if not await self._check_redis_health():
            REDIS_OPERATION_LATENCY.labels(provider=self.provider, operation='get_state').observe(time.monotonic() - start_time)
        ...
        try:
            ...
            state = {
                ...
                "last_failure_time": datetime.fromisoformat(state_data.get("last_failure_time", datetime.min.isoformat())).replace(tzinfo=timezone.utc),
                "next_try_after": datetime.fromisoformat(state_data.get("next_try_after", datetime.min.isoformat())).replace(tzinfo=timezone.utc)
            }

            # State validation
            if not isinstance(state["failures"], int) or state["failures"] < 0:
                _log_validation_error(f"Invalid failures value for {_sanitize_provider(self.provider)}: {state['failures']}. Resetting to 0.", 'invalid_state')
                state["failures"] = 0
            for key in ("last_failure_time", "next_try_after"):
                if not isinstance(state[key], datetime) or state[key].tzinfo != timezone.utc:
                    _log_validation_error(f"Invalid {key} for {_sanitize_provider(self.provider)}: {state[key]}. Resetting to min value.", 'invalid_state')
            ...
            REDIS_OPERATION_LATENCY.labels(provider=self.provider, operation='get_state').observe(time.monotonic() - start_time)
            _log_validation_error(f"Invalid datetime format in Redis state for {_sanitize_provider(self.provider)}: {e}", 'invalid_datetime_format')
            return self._in_memory_state.copy()
        except Exception as e:
            REDIS_OPERATION_LATENCY.labels(provider=self.provider, operation='get_state').observe(time.monotonic() - start_time)
            _log_validation_error(f"Error fetching state from Redis for {_sanitize_provider(self.provider)}: {e}", 'redis_get_failed')
            return self._in_memory_state.copy()
    ...
    @retry(
        ...
        wait=wait_exponential(multiplier=1, min=1, max=10),
        retry=retry_if_exception_type(redis.RedisError),
        before_sleep=lambda retry_state: logger.debug(f"Retrying Redis set operation for provider: attempt {retry_state.attempt_number}")
    )
    async def set_state(self, state: Dict[str, Any]) -> None:
        ...
        if not isinstance(state["failures"], int) or state["failures"] < 0 or state["failures"] > 1000:
            _log_validation_error(f"Invalid failures value for {_sanitize_provider(self.provider)}: {state['failures']}. Clamping to [0, 1000].", 'invalid_state')
        ...
        if not await self._check_redis_health():
            ...
            REDIS_OPERATION_LATENCY.labels(provider=self.provider, operation='set_state').observe(time.monotonic() - start_time)
            return
        ...
        try:
            ...
            async with self.redis_client.pipeline() as pipe:
                ...
                expiry_seconds_str = os.getenv('CIRCUIT_BREAKER_STATE_TTL_SECONDS', str(getattr(self.config, 'CIRCUIT_BREAKER_STATE_TTL_SECONDS', 86400)))
                try:
                    expiry_seconds = int(expiry_seconds_str)
                except ValueError:
                    _log_validation_error(f"Invalid CIRCUIT_BREAKER_STATE_TTL_SECONDS: {expiry_seconds_str}. Using default: 86400", 'invalid_config')
            ...
            REDIS_OPERATION_LATENCY.labels(provider=self.provider, operation='set_state').observe(time.monotonic() - start_time)
            _log_validation_error(f"Error setting state in Redis for {_sanitize_provider(self.provider)}: {e}", 'redis_set_failed')
            self._in_memory_state.update(state)
...
def validate_config(config: 'ArbiterConfig') -> None:
    ...
    for field, type_, default in required_fields:
        ...
        if not isinstance(value, type_):
            logger.warning(f"Invalid {field}: expected {type_.__name__}, got {type(value).__name__}. Using default: {default}")
...
async def get_breaker_state(provider: str, config: 'ArbiterConfig') -> BreakerStateManager:
    ...
    with tracer.start_as_current_span("get_breaker_state", attributes={"provider": provider}):
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", provider) or len(provider) > 50:
            _log_validation_error(f"Invalid provider name: '{_sanitize_provider(provider)}'", 'invalid_provider_name')
            raise ValueError("Provider name must be alphanumeric with underscores or hyphens and <= 50 characters")
        ...
        if redis_url:
            ...
            try:
                ...
                logger.warning("Falling back to in-memory behavior for %s due to Redis connection failure.", _sanitize_provider(provider))
        else:
        ...
        with _breaker_states_lock:
            if len(_breaker_states) >= _MAX_PROVIDERS and provider not in _breaker_states:
                _log_validation_error(f"Maximum provider limit ({_MAX_PROVIDERS}) reached. Cannot create state for {_sanitize_provider(provider)}.", 'max_providers_exceeded')
    ...
    while True:
        with tracer.start_as_current_span("cleanup_breaker_states") as span:
            ...
            if pause_value not in ('true', 'false'):
                _log_validation_error(f"Invalid PAUSE_CIRCUIT_BREAKER_TASKS: {pause_value}. Using default: false", 'invalid_config')
                pause_value = 'false'
            ...
            if is_paused != _last_pause_state:
                ...
                TASK_STATE_TRANSITIONS.labels(task='cleanup', state='paused' if is_paused else 'resumed').inc()
                ...
                with _breaker_states_lock:
                    ...
                    for provider, state in list(_breaker_states.items()):
                        ...
                        if provider in critical_providers:
                            logger.debug(f"Skipping cleanup for critical provider: {_sanitize_provider(provider)}")
                            continue
                        current_state = await state.get_state()
                        # Add provider-specific tracing attributes
                        span.set_attribute(f"provider.{provider}.failures", current_state.get("failures", 0))
                        span.set_attribute(f"provider.{provider}.next_try_after", current_state["next_try_after"].isoformat())
                        
                        # Validate state to ensure data integrity
                        if not isinstance(current_state.get("failures"), int) or current_state.get("failures", -1) < 0:
                            _log_validation_error(f"Invalid state for {_sanitize_provider(provider)} during cleanup: {current_state}", 'invalid_state')
                            CIRCUIT_BREAKER_CLEANUP_OPERATIONS.labels(provider=provider, result='invalid_state').inc()
                            expired.append(provider)
                            continue
                        
                        # Check if state is stale (no activity for 1 day)
                        if current_state["next_try_after"] < datetime.now(timezone.utc) - timedelta(days=1):
                             expired.append(provider)
                             CIRCUIT_BREAKER_CLEANUP_OPERATIONS.labels(provider=provider, result='expired').inc()
                    ...
                    for provider in expired:
                        ...
                        logger.debug(f"Cleaned up expired circuit breaker state for {_sanitize_provider(provider)}")
                
                # Use environment variable as fallback for cleanup interval
                try:
                    config = get_config()
                    cleanup_interval_str = os.getenv('CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS', str(getattr(config, 'CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS', 3600)))
                except Exception:
                    cleanup_interval_str = os.getenv('CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS', '3600')
                try:
                    cleanup_interval = int(cleanup_interval_str)
                    if cleanup_interval < 60:
                        raise ValueError("CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS must be at least 60")
                except ValueError:
                    _log_validation_error(f"Invalid CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS: {cleanup_interval_str}. Using default: 3600", 'invalid_config')
...
async def periodic_config_refresh() -> None:
    ...
    while True:
        with tracer.start_as_current_span("periodic_config_refresh") as span:
            ...
            is_paused = _pause_tasks or pause_value == 'true'
            if is_paused != _last_pause_state:
                logger.info(f"Circuit breaker config refresh task {'paused' if is_paused else 'resumed'}")
                TASK_STATE_TRANSITIONS.labels(task='config_refresh', state='paused' if is_paused else 'resumed').inc()
                _last_pause_state = is_paused
                span.set_attribute("task_state", "paused" if is_paused else "resumed")
            if is_paused:
            ...
            try:
                ...
                try:
                    ...
                    _log_validation_error(f"Invalid CONFIG_REFRESH_INTERVAL_SECONDS: {refresh_interval_str}. Using default: 300", 'invalid_config')
        ...
        with _breaker_states_lock:
            for state in _breaker_states.values():
                if hasattr(state, 'config'):
                    ...
                    logger.debug(f"Refreshed configuration for circuit breaker: {_sanitize_provider(state.provider)}")
                    span.set_attribute(f"provider.{state.provider}.refreshed", True)

async def is_llm_policy_circuit_breaker_open(provider: str = "default", config: Optional['ArbiterConfig'] = None) -> bool:
    """
    ...
    """
    with tracer.start_as_current_span("is_llm_policy_circuit_breaker_open", attributes={"provider": provider}) as span:
        ...
        async with breaker_state_manager.state_lock():
            ...
            if state["failures"] >= failure_threshold:
                if datetime.now(timezone.utc) < state["next_try_after"]:
                    LLM_CIRCUIT_BREAKER_STATE.labels(provider=provider).set(1) # Reflects open state
                    LLM_CIRCUIT_BREAKER_TRANSITIONS.labels(provider=provider, from_state=current_state_str, to_state="open").inc()
            ...
            LLM_CIRCUIT_BREAKER_TRANSITIONS.labels(provider=provider, from_state=current_state_str, to_state="closed").inc()
            span.set_attribute("breaker_state", "closed")
            return False

async def record_llm_policy_api_success(provider: str = "default", config: Optional['ArbiterConfig'] = None) -> None:
    """
    ...
    Args:
        ...
        config: An optional ArbiterConfig object.
    """
    with tracer.start_as_current_span("record_llm_policy_api_success", attributes={"provider": provider}) as span:
        config = config or get_config()
        validate_config(config)
        breaker_state_manager = await get_breaker_state(provider, config)
        ...
        async with breaker_state_manager.state_lock():
            state = await breaker_state_manager.get_state()
            if state["failures"] > 0:
                logger.debug("LLM policy API call successful for '%s'. Resetting circuit breaker.", _sanitize_provider(provider))
...
async def record_llm_policy_api_failure(provider: str = "default", error_message: Optional[str] = None, config: Optional['ArbiterConfig'] = None) -> None:
    """
    ...
    Args:
        ...
        config: An optional ArbiterConfig object.
    """
    with tracer.start_as_current_span("record_llm_policy_api_failure", attributes={"provider": provider}) as span:
        config = config or get_config()
        validate_config(config)
        breaker_state_manager = await get_breaker_state(provider, config)
