...
CONFIG_ERRORS = Counter(
    'arbiter_config_errors_total', 'Total configuration errors',
    labelnames=('error_type',)
)
CONFIG_INITIALIZATIONS = Counter(
    'arbiter_config_initializations_total', 'Total configuration initializations',
    labelnames=('result',)
)
CONFIG_RELOAD_FREQUENCY = Counter(
    'arbiter_config_reload_frequency_total', 'Total number of configuration reloads',
    labelnames=('result',)
)
CONFIG_VALIDATION_DURATION = Histogram(
    'arbiter_config_validation_duration_seconds', 'Duration of configuration validation',
    labelnames=('operation',), buckets=(0.001, 0.01, 0.1, 0.5, 1, 2, 5)
)
CONFIG_TO_DICT_CACHE_HITS = Counter(
    'arbiter_config_to_dict_cache_hits_total', 'Total cache hits/misses for to_dict calls',
    labelnames=('result',)
)
CONFIG_REDIS_VALIDATION_DURATION = Histogram(
    'arbiter_config_redis_validation_duration_seconds', 'Duration of Redis URL validation',
    labelnames=('operation',), buckets=(0.001, 0.01, 0.1, 0.5, 1, 2, 5)
)
...
class ArbiterConfig(BaseSettings):
    ...
    _redis_pools: dict = PrivateAttr(default_factory=dict) # Supporting multiple pools for different URLs

    # Core Settings
    POLICY_CONFIG_FILE_PATH: str = Field(
        default_factory=lambda: os.getenv('POLICY_CONFIG_FILE_PATH', os.path.abspath(os.path.join(os.path.dirname(__file__), "../policies.json"))),
        description="Absolute path to the policy configuration file."
    )
    AUDIT_LOG_FILE_PATH: str = Field(
        default_factory=lambda: os.getenv('AUDIT_LOG_FILE_PATH', os.path.abspath(os.path.join(os.path.dirname(__file__), "policy_audit.log"))),
        description="Absolute path to the audit log file."
    )
    DEFAULT_AUTO_LEARN_POLICY: bool = Field(True, description="Default policy for auto-learning if not specified in rules.")
    POLICY_REFRESH_INTERVAL_SECONDS: int = Field(300, description="Interval for policy refresh in seconds")
    LLM_POLICY_EVALUATION_ENABLED: bool = Field(True, description="Enable LLM-based policy evaluation.")
    VALID_DOMAIN_PATTERN: str = Field(r"^[a-zA-Z0-9_.-]+$", description="Regex pattern for validating policy domain names.")
    MAX_LEARN_RETRIES: int = Field(5, ge=0, description="Maximum number of retries for a failed learning attempt.")
    REDIS_URL: Optional[str] = Field(
        default_factory=lambda: os.getenv('REDIS_URL', None),
        description="URL for the Redis instance, required in production."
    )
    CONFIG_REFRESH_INTERVAL_SECONDS: int = Field(300, description="Config refresh interval in seconds")

    # Circuit Breaker Settings
    CIRCUIT_BREAKER_STATE_TTL_SECONDS: int = Field(86400, description="Redis key TTL in seconds")
    CIRCUIT_BREAKER_CLEANUP_INTERVAL_SECONDS: int = Field(3600, description="Interval for cleanup task in seconds")
    ...
    REDIS_SOCKET_CONNECT_TIMEOUT: float = Field(5.0, description="Redis socket connect timeout in seconds")
    CIRCUIT_BREAKER_MIN_OPERATION_INTERVAL: float = Field(0.1, description="Minimum interval between Redis operations in seconds")
    CIRCUIT_BREAKER_CRITICAL_PROVIDERS: str = Field('', description="Comma-separated list of providers exempt from cleanup")
    CIRCUIT_BREAKER_VALIDATION_ERROR_INTERVAL: float = Field(300.0, description="Interval for logging validation errors in seconds")
    CIRCUIT_BREAKER_MAX_PROVIDERS: int = Field(1000, description="Maximum number of providers")
    PAUSE_CIRCUIT_BREAKER_TASKS: str = Field('false', description="Set to 'true' to pause cleanup and refresh tasks")
    POLICY_PAUSE_POLLING_INTERVAL: float = Field(60.0, description="Polling interval when policy refresh is paused")
    
    # Secret Settings
    ENCRYPTION_KEY: SecretStr = Field(SecretStr(os.getenv("ENCRYPTION_KEY", "")), description="A 32-byte URL-safe base64-encoded key for Fernet encryption.")
    OPENAI_API_KEY: SecretStr = Field(SecretStr(os.getenv("OPENAI_API_KEY", "")), description="API key for OpenAI.")
    ANTHROPIC_API_KEY: SecretStr = Field(SecretStr(os.getenv("ANTHROPIC_API_KEY", "")), description="API key for Anthropic.")
    GEMINI_API_KEY: SecretStr = Field(SecretStr(os.getenv("GOOGLE_API_KEY", "")), description="API key for Google Gemini.")

    # LLM Settings
    LLM_PROVIDER: str = Field("openai", description="Default LLM provider to use (openai, anthropic, gemini).")
    LLM_MODEL: str = Field("gpt-4o-mini", description="Default model name for LLM.")
    LLM_API_URL: Optional[str] = Field(None, description="Custom API URL for LLM provider, must be a valid URL.")
    LLM_API_TIMEOUT_SECONDS: float = Field(30, gt=0, description="Timeout for LLM API calls in seconds.")
    LLM_API_BACKOFF_MAX_SECONDS: float = Field(60.0, description="Maximum backoff seconds for LLM API circuit breaker")
    LLM_API_FAILURE_THRESHOLD: int = Field(3, description="Failure threshold for LLM API circuit breaker")
    LLM_POLICY_MIN_TRUST_SCORE: float = Field(0.5, ge=0, le=1, description="Minimum trust score for LLM policy decisions to be considered valid.")
    LLM_VALID_RESPONSES: list = Field(default_factory=lambda: ["YES", "NO"], description="Valid responses from LLM")
    ...
    DECISION_OPTIMIZER_SETTINGS: Dict[str, Any] = Field(
        default_factory=lambda: {
            ...
            "feedback_db_path": os.getenv('FEEDBACK_DB_PATH', os.path.abspath(os.path.join(os.path.dirname(__file__), "feedback.db"))),
            "llm_feedback_enabled": False,
            "llm_feedback_model": os.getenv('LLM_FEEDBACK_MODEL', "gpt-4o-mini"),
            "llm_feedback_api_url": os.getenv('LLM_FEEDBACK_API_URL', "http://localhost:11434/api/generate"),
    ...
    def parse_optimizer_settings(cls, v):
        ...
        if isinstance(v, str):
            try:
                ...
                raise ValueError("DECISION_OPTIMIZER_SETTINGS must be a dict or a JSON string representing a dict")
        if v is None:
    ...
    def validate_secrets(cls, values: dict) -> dict:
        ...
        with tracer.start_as_current_span("validate_secrets") as span:
            ...
            if is_production:
                ...
                for key in ("OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GEMINI_API_KEY"):
                    if not values.get(key):
                        logger.warning(f"{key} is not set. LLM functionality may be disabled for {key.split('_')[0].lower()}.")
                        span.set_attribute(f"{key.lower()}_status", "missing")
                if values.get("ENCRYPTION_KEY"):
                    try:
                        key = values["ENCRYPTION_KEY"].encode('utf-8')
                        if len(key) != 44:  # Fernet keys are 32 bytes, base64-encoded to 44 characters
            ...
            # Validate file paths
            for path_key in ("POLICY_CONFIG_FILE_PATH", "AUDIT_LOG_FILE_PATH", "DECISION_OPTIMIZER_SETTINGS"):
                ...
                if path and not os.path.isabs(path):
                    ...
                    if path_key == "POLICY_CONFIG_FILE_PATH":
                        values[path_key] = os.path.abspath(os.path.join(os.path.dirname(__file__), "../policies.json"))
                    elif path_key == "AUDIT_LOG_FILE_PATH":
                        values[path_key] = os.path.abspath(os.path.join(os.path.dirname(__file__), "policy_audit.log"))
                    else:
                        values[path_key]["feedback_db_path"] = os.path.abspath(os.path.join(os.path.dirname(__file__), "feedback.db"))
                else:
            ...
            if critical_providers:
                ...
                if invalid_providers:
                    ...
                    span.record_exception(ValueError(f"Invalid CIRCUIT_BREAKER_CRITICAL_PROVIDERS: {invalid_providers}"))
            ...
            if llm_provider not in valid_providers:
                logger.error(f"Invalid LLM_PROVIDER: {llm_provider}. Must be one of {valid_providers}")
            ...
            if llm_model not in valid_models.get(llm_provider, set()):
                logger.error(f"Invalid LLM_MODEL: {llm_model} for provider {llm_provider}. Must be one of {valid_models.get(llm_provider, [])}")
                CONFIG_ERRORS.labels(error_type='invalid_llm_model').inc()
                span.record_exception(ValueError(f"Invalid LLM_MODEL: {llm_model} for provider {llm_provider}"))
                span.set_attribute("llm_model_status", "invalid")
                values["LLM_MODEL"] = "gpt-4o-mini" if llm_provider == "openai" else ("claude-3-sonnet" if llm_provider == "anthropic" else "gemini-1.5-flash")
            else:
            ...
            CONFIG_VALIDATION_DURATION.labels(operation='validate_secrets').observe(time.monotonic() - start_time)
            return values
        ...
        if url:
            ...
            with tracer.start_as_current_span("validate_redis_url") as span:
                try:
                    ...
                    CONFIG_REDIS_VALIDATION_DURATION.labels(operation='validate_redis_url').observe(time.time() - start_time)
        return self
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        with tracer.start_as_current_span("to_dict") as span:
            with self._cache_lock:
                ...
                if self._config_cache is not None and (current_time - self._config_cache_timestamp) < self._config_cache_ttl:
                ...
                for key, value in result.items():
                    if isinstance(self.model_fields[key].default, SecretStr) or key in ["ENCRYPTION_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GEMINI_API_KEY"]:
                         result[key] = "[REDACTED]"
    ...
    def get_api_key_for_provider(provider: str) -> Optional[str]:
        ...
        with tracer.start_as_current_span("get_api_key_for_provider", attributes={"provider": provider}) as span:
            if provider == "openai":
                ...
                logger.warning(f"No specific API key found for provider: {provider}. Falling back to LLM_API_KEY.")
