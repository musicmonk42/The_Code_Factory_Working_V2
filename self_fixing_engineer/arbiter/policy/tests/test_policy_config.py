...
def test_config_defaults_and_field_types(monkeypatch):
    ...
    with patch('redis.asyncio.Redis.from_url') as mock_redis:
        ...
        assert cfg.DECISION_OPTIMIZER_SETTINGS.get("llm_call_latency_buckets") == (0.1, 0.5, 1, 2, 5, 10, 30, 60)
        assert cfg.DECISION_OPTIMIZER_SETTINGS.get("feedback_processing_buckets") == (0.001, 0.01, 0.1, 1, 10)
...
def test_env_loading_and_override(monkeypatch, tmp_path):
    ...
    with patch('redis.asyncio.Redis.from_url'):
        ...
        assert cfg.DECISION_OPTIMIZER_SETTINGS["llm_call_latency_buckets"] == [0.2, 0.6, 1.2]
...
def test_model_validator_enforces_secrets(monkeypatch):
    ...
    pytest.skip("ENCRYPTION_KEY validation has known issue in config.py - empty SecretStr not detected as missing")
