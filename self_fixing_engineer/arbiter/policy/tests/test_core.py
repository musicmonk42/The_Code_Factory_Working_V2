...
from arbiter.policy.core import (
    PolicyEngine, BasicDecisionOptimizer, SQLiteClient,
    initialize_policy_engine, get_policy_engine_instance, reset_policy_engine
)
...
def create_mock_enforce_compliance():
    ...
    async def mock_enforce_compliance(self, action_name: str, control_tag: Optional[str]) -> Tuple[bool, str]:
        if not control_tag:
...
async def policy_engine(minimal_arbiter, arbiter_config, monkeypatch):
    ...
    monkeypatch.setattr(engine, "_enforce_compliance", create_mock_enforce_compliance().__get__(engine, PolicyEngine))
...
class TestBasicDecisionOptimizer:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("context,user_id,expected_range", [
        ({"login_attempts": 0, "device_trusted": True, "last_login": datetime.now(timezone.utc).isoformat()}, 
         "adminUser", (0.7, 1.0)),
        ({"login_attempts": 5, "device_trusted": False, "last_login": (datetime.now(timezone.utc) - timedelta(days=90)).isoformat()}, 
         "guestUser", (0.0, 0.5)),
        ({"login_attempts": 2, "device_trusted": True}, 
         "user", (0.4, 0.8)),
        ({"login_attempts": 0, "device_trusted": True, "last_login": "invalid-date"}, 
         "user", (0.0, 1.0)),
    ])
...
class TestPolicyEngine:
    ...
    async def test_sensitive_keys(self, policy_engine):
        ...
        policy_engine._policies["domain_rules"]["test"]["sensitive_keys"] = ["password", "ssn"]
    ...
    async def test_policy_reload(self, policy_engine):
        ...
        new_policies["domain_rules"]["new_domain"] = {"allow": False, "reason": "New restriction"}
    ...
    async def test_policy_evolution_update(self, policy_engine):
        ...
        policy_engine._policies["domain_rules"]["evolved"] = {"allow": True, "reason": "Evolved rule"}
...
class TestIntegration:
    ...
    async def test_full_workflow(self, minimal_arbiter, arbiter_config):
        ...
        with patch("arbiter.policy.core.get_config", return_value=arbiter_config):
            ...
            engine._enforce_compliance = create_mock_enforce_compliance().__get__(engine, PolicyEngine)
...
def test_all_public_apis_exported():
    ...
    for name in expected_exports:
        ...
        assert callable(getattr(core, name)) or isinstance(getattr(core, name), type), f"Export not callable or class: {name}"
