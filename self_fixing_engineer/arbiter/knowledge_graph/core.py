...
try:
    ...
    logging.getLogger(__name__).warning(
        "Warning: PyJWT not found. JWT validation will be disabled."
    )
    jwt = None
        ...
        async def load(self, table: str, session_id: str) -> Optional[Dict[str, Any]]:
            async with self.pool.acquire() as conn:
                result = await conn.fetchrow(
                    f"SELECT state FROM {table} WHERE session_id = $1", session_id
                )
                return {"state": result["state"]} if result else None

except ImportError:
    logging.getLogger(__name__).warning(
        "Warning: asyncpg not found. Postgres client functionality will be unavailable."
    )
    PostgresClient = None
    ...
    async def init_client(self):
        try:
            ...
            raise AgentCoreException(
                ...
                code=AgentErrorCode.STATE_LOAD_FAILED,
        ...
        with tracer.start_as_current_span("redis_save_state") as span:
            ...
            try:
                await asyncio.wait_for(
                    self.client.set(f"agent_state:{session_id}", json.dumps(state)),
                    timeout=5,
                )
                AGENT_METRICS["state_backend_operations_total"].labels(
                    operation="save", backend_type="redis"
                ).inc()
                AGENT_METRICS["state_backend_latency_seconds"].labels(
                    operation="save", backend_type="redis"
                ).observe(time.monotonic() - start_time)
                logger.debug(
                    f"Saved state for session {session_id} in Redis. Trace ID: {trace_id_var.get()}"
                )
            except (asyncio.TimeoutError, Exception) as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    operation="save",
                    backend_type="redis",
                ...
                logger.error(
                    f"Failed to save state for session {session_id} in Redis: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Redis save state failed: {e}",
                    code=AgentErrorCode.STATE_SAVE_FAILED,
                    original_exception=e,
                ) from e
        ...
        with tracer.start_as_current_span("redis_load_state") as span:
            ...
            try:
                data = await asyncio.wait_for(
                    self.client.get(f"agent_state:{session_id}"), timeout=5
                )
                if data:
                    AGENT_METRICS["state_backend_operations_total"].labels(
                        operation="load", backend_type="redis"
                    ).inc()
                    AGENT_METRICS["state_backend_latency_seconds"].labels(
                        operation="load", backend_type="redis"
                    ).observe(time.monotonic() - start_time)
                    logger.debug(
                        f"Loaded state for session {session_id} from Redis. Trace ID: {trace_id_var.get()}"
                    )
                    return json.loads(data)
                else:
                    logger.info(
                        f"No state found for session {session_id} in Redis. Trace ID: {trace_id_var.get()}"
                    )
                    return None
                ...
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to decode state for session {session_id} from Redis: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Redis load state failed: {e}",
                    code=AgentErrorCode.STATE_LOAD_FAILED,
                    original_exception=e,
                ) from e
            except (asyncio.TimeoutError, Exception) as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    operation="load",
                    backend_type="redis",
                    error_code="redis_load_failed",
                ).inc()
                span.record_exception(e)
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to load state for session {session_id} from Redis: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Redis load state failed: {e}",
...
class PostgresStateBackend(StateBackend):
    ...
    async def save_state(self, session_id: str, state: Dict[str, Any]):
        ...
        with tracer.start_as_current_span("postgres_save_state") as span:
            ...
            try:
                ...
                AGENT_METRICS["state_backend_operations_total"].labels(
                    operation="save", backend_type="postgres"
                ).inc()
                AGENT_METRICS["state_backend_latency_seconds"].labels(
                    operation="save", backend_type="postgres"
                ).observe(time.monotonic() - start_time)
                logger.debug(
                    f"Saved state for session {session_id} in Postgres. Trace ID: {trace_id_var.get()}"
                )
            except (asyncio.TimeoutError, Exception) as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    operation="save",
                    backend_type="postgres",
                    error_code="postgres_save_failed",
                ).inc()
                span.record_exception(e)
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to save state for session {session_id} in Postgres: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Postgres save state failed: {e}",
                    code=AgentErrorCode.STATE_SAVE_FAILED,
                    original_exception=e,
                ) from e
    ...
    async def load_state(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        with tracer.start_as_current_span("postgres_load_state") as span:
            ...
            try:
                data = await asyncio.wait_for(
                    self.client.load("agent_states", session_id), timeout=5
                )
                if data:
                    AGENT_METRICS["state_backend_operations_total"].labels(
                        operation="load", backend_type="postgres"
                    ).inc()
                    AGENT_METRICS["state_backend_latency_seconds"].labels(
                        operation="load", backend_type="postgres"
                    ).observe(time.monotonic() - start_time)
                    logger.debug(
                        f"Loaded state for session {session_id} from Postgres. Trace ID: {trace_id_var.get()}"
                    )
                    return json.loads(data["state"])
                else:
                    logger.info(
                        f"No state found for session {session_id} in Postgres. Trace ID: {trace_id_var.get()}"
                    )
                    return None
            except json.JSONDecodeError as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                ...
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to decode state for session {session_id} from Postgres: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Postgres load state failed: {e}",
                    code=AgentErrorCode.STATE_LOAD_FAILED,
                    original_exception=e,
                ) from e
            except (asyncio.TimeoutError, Exception) as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    operation="load",
                    backend_type="postgres",
                    error_code="postgres_load_failed",
                ).inc()
                span.record_exception(e)
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to load state for session {session_id} from Postgres: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Postgres load state failed: {e}",
        ...
        with tracer.start_as_current_span("inmemory_load_state") as span:
            ...
            try:
                async with self._lock:
                    ...
                    if data:
                        AGENT_METRICS["state_backend_operations_total"].labels(
                            operation="load", backend_type="inmemory"
                        ).inc()
                        AGENT_METRICS["state_backend_latency_seconds"].labels(
                            operation="load", backend_type="inmemory"
                        ).observe(time.monotonic() - start_time)
                        logger.debug(
                            f"Loaded state for session {session_id} from InMemory. Trace ID: {trace_id_var.get()}"
                        )
                        ...
                        logger.info(
                            f"No state found for session {session_id} in InMemory. Trace ID: {trace_id_var.get()}"
                        )
                        return None
            except Exception as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    ...
                    error_code="inmemory_load_failed",
                ).inc()
                span.record_exception(e)
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to load state for session {session_id} from InMemory: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"InMemory load state failed: {e}",
                    code=AgentErrorCode.STATE_LOAD_FAILED,
                    original_exception=e,
                ) from e
    ...
    async def save_state(self, session_id: str, state: Dict[str, Any]) -> None:
        ...
        with tracer.start_as_current_span("inmemory_save_state") as span:
            ...
            try:
                ...
                AGENT_METRICS["state_backend_operations_total"].labels(
                    operation="save", backend_type="inmemory"
                ).inc()
                AGENT_METRICS["state_backend_latency_seconds"].labels(
                    operation="save", backend_type="inmemory"
                ).observe(time.monotonic() - start_time)
                logger.debug(
                    f"Saved state for session {session_id} in InMemory. Trace ID: {trace_id_var.get()}"
                )
            except Exception as e:
                AGENT_METRICS["state_backend_errors_total"].labels(
                    operation="save",
                    backend_type="inmemory",
                    error_code="inmemory_save_failed",
                ).inc()
                span.record_exception(e)
                span.set_status(trace.Status(trace.StatusCode.ERROR, str(e)))
                logger.error(
                    f"Failed to save state for session {session_id} in InMemory: {e}. Trace ID: {trace_id_var.get()}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"InMemory save state failed: {e}",
                    code=AgentErrorCode.STATE_SAVE_FAILED,
                    original_exception=e,
                ) from e
...
class MetaLearning:
    ...
    def log_correction(
        ...
        if (
            len(input_text) + len(initial_response) + len(corrected_response)
            > Config.MAX_CORRECTION_ENTRY_SIZE
        ):
    ...
    def train_model(self):
        ...
        try:
            ...
            if "meta_learning_train_duration_seconds" in AGENT_METRICS:
                AGENT_METRICS["meta_learning_train_duration_seconds"].observe(
        ...
        try:
            with open("meta_learning.pkl", "rb") as f:
                ...
                if isinstance(model_pipeline, Pipeline):
                    ...
                    logger.warning(
                        "Loaded object is not a scikit-learn pipeline. Re-initializing model."
                    )
                    self.model_pipeline = Pipeline(
                        [
                            ("tfidf", TfidfVectorizer()),
...
class CollaborativeAgent:
    ...
    async def load_state(self, operator_id: str = "system"):
        ...
        try:
            ...
            if state:
                ...
                logger.debug(
                    f"State loaded for agent {self.agent_id}, session {self.session_id}. Trace ID: {current_trace_id}"
                )
            ...
            logger.error(
                f"State load failed for agent {self.agent_id}: {e}. Trace ID: {current_trace_id}",
                exc_info=True,
            )
            raise AgentCoreException(
                f"State load failed: {e}",
                code=AgentErrorCode.STATE_LOAD_FAILED,
                original_exception=e,
            ) from e
        finally:
    ...
    async def save_state(self, operator_id: str = "system"):
        ...
        try:
            ...
            logger.error(
                f"State save failed for agent {self.agent_id}: {e}. Trace ID: {current_trace_id}",
                exc_info=True,
            )
            raise AgentCoreException(
    ...
    async def _call_llm_with_retries(
        self, llm_instance: Any, messages: List[Any], provider: str, model: str
    ) -> Any:
        """
    ...
    async def predict(
        ...
        if jwt and Config.AUDIT_SIGNING_PUBLIC_KEY and operator_id.startswith("JWT:"):
            try:
                decoded = jwt.decode(
                    operator_id[4:],
                    Config.AUDIT_SIGNING_PUBLIC_KEY.get_actual_value(),
                    algorithms=["HS256"],
                )
                logger.debug(
                    f"JWT validated for operator: {decoded.get('sub')}. Trace ID: {current_trace_id}"
                )
            except jwt.InvalidTokenError as e:
                logger.error(
                    f"Invalid operator JWT: {e}. Trace ID: {current_trace_id}",
                    exc_info=True,
                )
                raise AgentCoreException(
                    f"Invalid operator JWT: {e}",
        ...
        if Config.REDIS_URL and RedisClient:
            try:
                ...
                if hasattr(redis_client, "connect") and asyncio.iscoroutinefunction(
                ...
                if cached_result:
                    logger.info(
                        f"Cache hit for input. Session: {self.session_id}. Trace ID: {current_trace_id}"
                    )
        ...
        try:
            async with async_timeout(timeout):
                ...
                for item in multi_modal_context:
                    if not isinstance(item, MultiModalData):
                        logger.warning(
                            f"Invalid multimodal data type: {type(item)}. Skipping. Trace ID: {current_trace_id}"
                        )
                        continue
                ...
                with tracer.start_as_current_span("llm_call_reflection") as span:
                    reflection_prompt = REFLECTION_PROMPT_TEMPLATE.format(
                        input=user_input, ai_response=response_text
                    )
                    reflection = await self._call_llm_with_retries(
                        self.llm,
                ...
                with tracer.start_as_current_span("llm_call_critique") as span:
                    critique_prompt = CRITIQUE_PROMPT_TEMPLATE.format(
                        persona=self.persona, ai_response=response_text
                    )
                    critique = await self._call_llm_with_retries(
                        self.llm,
                ...
                with tracer.start_as_current_span("llm_call_correction") as span:
                    correct_prompt = SELF_CORRECT_PROMPT_TEMPLATE.format(
                        ai_response=response_text,
                        reflection=reflection_text,
                        critique=critique_text,
                    )
                    corrected = await self._call_llm_with_retries(
                        self.llm,
                ...
                if Config.REDIS_URL and RedisClient and redis_client is not None:
                    try:
                        ...
                        logger.debug(
                            f"Result cached for key: {cache_key}. Trace ID: {current_trace_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Failed to cache result in Redis: {e}. Trace ID: {current_trace_id}"
                        )
            ...
            raise AgentCoreException(
                f"Unexpected error during agent prediction: {e}",
                code=AgentErrorCode.UNEXPECTED_ERROR,
                original_exception=e,
            ) from e
        finally:
...
class AgentTeam:
    ...
    def __init__(
        ...
        if not all([llm_config, state_backend, meta_learning]):
            raise ValueError(
                "llm_config, state_backend, and meta_learning are required for AgentTeam."
            )
        self.agents = {
    ...
    async def delegate_task(
        ...
        await self.agents["requirements"].audit_ledger.log_event(
            ...
            details={
                "session_id": self.session_id,
                "initial_input_preview": initial_input[:100],
        ...
        try:
            reqs_output = await self.agents["requirements"].predict(
                initial_input, context=context, timeout=timeout, operator_id=operator_id
            )
            refinement_context = {
            ...
            """
            final_output = await self.agents["refiner"].predict(
                refinement_prompt,
            ...
            if "agent_team_task_duration_seconds" in AGENT_METRICS:
                AGENT_METRICS["agent_team_task_duration_seconds"].observe(
            ...
            await self.agents["requirements"].audit_ledger.log_event(
                ...
                details={
                    "session_id": self.session_id,
                    "final_response_preview": final_output["response"][:100],
                },
                operator=operator_id,
            )
            return {
            ...
            if "agent_team_task_errors_total" in AGENT_METRICS:
                AGENT_METRICS["agent_team_task_errors_total"].labels(
                    error_code=AgentErrorCode.UNEXPECTED_ERROR.value
                ).inc()
            logger.error(
                f"AgentTeam task failed: {e}. Trace ID: {current_trace_id}",
                exc_info=True,
            )
            await self.agents["requirements"].audit_ledger.log_event(
...
async def get_or_create_agent(
    ...
    if "api_key" in llm_conf:
        ...
        if isinstance(api_key_value, str):
            ...
            logger.warning(
                f"Unexpected type for api_key in llm_conf: {type(api_key_value)}. Skipping SensitiveValue wrapping."
            )
    ...
    if final_state_backend is None:
        if Config.REDIS_URL and RedisClient is not None:
            try:
                ...
                logger.error(
                    f"Failed to initialize RedisStateBackend: {e}. Falling back to InMemoryStateBackend.",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to initialize PostgresStateBackend: {e}. Falling back to InMemoryStateBackend.",
                    exc_info=True,
                )
                final_state_backend = InMemoryStateBackend()
        else:
            final_state_backend = InMemoryStateBackend()
            logger.info(
                "Using InMemoryStateBackend for state management (no Redis/Postgres configured or available)."
            )
...
async def setup_conversation(
    llm: Any, persona: str = "", language: str = Config.DEFAULT_LANGUAGE
) -> Tuple[ConversationChain, ConversationBufferWindowMemory]:
    """
    ...
    """
    logger.warning(
        "Legacy function 'setup_conversation' called. Use get_or_create_agent instead, which is now async and handles state persistence, meta-learning, and multimodal data. This function will be deprecated in a future release."
    )
    ...
    if hasattr(llm, "__class__") and llm.__class__.__name__ == "ChatOpenAI":
        ...
        llm_config_from_langchain = {
            ...
            "model": llm.model_name,
            "temperature": llm.temperature,
            "api_key": getattr(llm, "xai_api_key", None),
        }
    else:
        llm_config_from_langchain = {
            "provider": "unknown",
            "model": getattr(llm, "model_name", "unknown_model"),
    ...
    conversation_chain = ConversationChain(
        llm=agent.llm, memory=agent.memory, prompt=prompt_template
    )
    return conversation_chain, agent.memory


# ---------------------------------------------------------------------------
# In-memory KnowledgeGraph — default implementation when a graph database
# (e.g. Neo4j) is not available.  Provides the full interface expected by
# file_watcher.py, arbiter_growth_manager.py and stubs.KnowledgeGraphStub.
# ---------------------------------------------------------------------------

class KnowledgeGraph:
    """Lightweight in-memory knowledge graph used when no backend DB is configured."""
