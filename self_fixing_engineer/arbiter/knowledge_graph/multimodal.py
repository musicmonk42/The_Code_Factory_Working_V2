...
class DefaultMultiModalProcessor(MultiModalProcessor):
    ...
    def __init__(self, logger: logging.Logger):
        ...
        if Config.REDIS_URL:
            try:
                ...
                redis_url = str(Config.REDIS_URL) if hasattr(Config.REDIS_URL, '__str__') else Config.REDIS_URL
    ...
    async def summarize(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        if self.redis_client:
            try:
                cached_result = await async_with_retry(lambda: self.redis_client.get(cache_key), retries=3, delay=1)
                if cached_result:
        ...
        if len(item.data) > Config.MAX_MM_DATA_SIZE_MB * 1024 * 1024:
            self._logger.warning(f"MultiModalData too large ({len(item.data) / (1024*1024):.2f}MB). Trace ID: {trace_id_var.get()}")
            AGENT_METRICS["mm_processor_failures_total"].labels(data_type=item.data_type, error_type=AgentErrorCode.MM_DATA_TOO_LARGE.value).inc()
            await audit_ledger_client.log_event(event_type="multimodal:size_exceeded", details={"data_type": item.data_type, "size_bytes": len(item.data)})
            return {"status": "failed", "summary": "Data exceeds maximum size limit.", "data_hash": data_hash}
        ...
        try:
            ...
            if processor:
                ...
                if self.redis_client and result.get("status") == "success":
                    try:
                        await async_with_retry(lambda: self.redis_client.setex(cache_key, Config.CACHE_EXPIRATION_SECONDS, json.dumps(result)), retries=3, delay=1)
                    except Exception as e:
                        self._logger.warning(f"Failed to cache result: {e}")
                return result
            
            self._logger.warning(f"Unsupported data type: {item.data_type}. Trace ID: {trace_id_var.get()}")
            AGENT_METRICS["mm_processor_failures_total"].labels(data_type=item.data_type, error_type=AgentErrorCode.MM_UNSUPPORTED_DATA.value).inc()
            await audit_ledger_client.log_event(event_type="multimodal:unsupported", details={"data_type": item.data_type})
            return {"status": "failed", "summary": "Unsupported data type.", "data_hash": data_hash}
        except asyncio.TimeoutError:
            self._logger.error(f"Timeout processing {item.data_type}. Trace ID: {trace_id_var.get()}")
            AGENT_METRICS["mm_processor_failures_total"].labels(data_type=item.data_type, error_type=AgentErrorCode.TIMEOUT.value).inc()
            await audit_ledger_client.log_event(event_type="multimodal:timeout", details={"data_type": item.data_type})
            return {"status": "failed", "summary": "Processing timed out.", "data_hash": data_hash}
        except Exception as e:
            self._logger.error(f"Unexpected error processing {item.data_type}: {e}. Trace ID: {trace_id_var.get()}", exc_info=True)
            AGENT_METRICS["mm_processor_failures_total"].labels(data_type=item.data_type, error_type=AgentErrorCode.MM_PROCESSING_FAILED.value).inc()
            await audit_ledger_client.log_event(event_type="multimodal:failed", details={"data_type": item.data_type, "error": str(e)})
            raise AgentCoreException(f"Multi-modal processing failed: {e}", code=AgentErrorCode.MM_PROCESSING_FAILED, original_exception=e) from e
    ...
    async def _process_image(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        if not self._image_processing_available:
            ...
            try:
                await audit_ledger_client.log_event(event_type="multimodal:skipped", details={"data_type": "image", "reason": "missing library"})
            except Exception as e:
                self._logger.warning(f"Failed to log audit event: {e}")
            return {"status": "skipped", "summary": "Image processing not available.", "data_hash": data_hash}
        ...
        try:
            ...
            return {"status": "failed", "summary": f"Failed to identify image: {e}", "data_hash": data_hash}
        except Exception as e:
            raise AgentCoreException(f"Error processing image: {e}", code=AgentErrorCode.MM_PROCESSING_FAILED, original_exception=e) from e
    ...
    async def _process_audio(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        if not self._audio_processing_available:
            ...
            try:
                await audit_ledger_client.log_event(event_type="multimodal:skipped", details={"data_type": "audio", "reason": "missing library"})
            except Exception as e:
                self._logger.warning(f"Failed to log audit event: {e}")
            return {"status": "skipped", "summary": "Audio processing not available.", "data_hash": data_hash}
        ...
        try:
            ...
            return {"status": "failed", "summary": f"Failed to decode audio: {e}", "data_hash": data_hash}
        except Exception as e:
            raise AgentCoreException(f"Error processing audio: {e}", code=AgentErrorCode.MM_PROCESSING_FAILED, original_exception=e) from e
    ...
    async def _process_video(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        if not self._video_processing_available:
            ...
            try:
                await audit_ledger_client.log_event(event_type="multimodal:skipped", details={"data_type": "video", "reason": "missing library"})
            except Exception as e:
                self._logger.warning(f"Failed to log audit event: {e}")
    ...
    async def _process_text_file(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "failed", "summary": f"Failed to decode text: {e}", "data_hash": data_hash}
        except Exception as e:
            raise AgentCoreException(f"Error processing text file: {e}", code=AgentErrorCode.MM_PROCESSING_FAILED, original_exception=e) from e
    ...
    async def _process_pdf_file(self, item: MultiModalData) -> Dict[str, Any]:
        ...
        if not self._pdf_processing_available:
            ...
            try:
                await audit_ledger_client.log_event(event_type="multimodal:skipped", details={"data_type": "pdf_file", "reason": "missing library"})
            except Exception as e:
                self._logger.warning(f"Failed to log audit event: {e}")
            return {"status": "skipped", "summary": "PDF processing not available.", "data_hash": data_hash}
        ...
        try:
            ...
            return {"status": "failed", "summary": f"Failed to process PDF: {e}", "data_hash": data_hash}