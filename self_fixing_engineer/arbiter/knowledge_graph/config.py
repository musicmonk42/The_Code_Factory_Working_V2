...
class MetaLearningConfig(BaseSettings):
    ...
    DATA_LAKE_PATH: str = Field(
        default="data/meta_learning_data.jsonl",
        description="Local file path for data lake fallback.",
    )
    ...
    AUDIT_LEDGER_URL: AnyUrl = Field(
        default="http://localhost:8000/audit_ledger",
        description="URL for the audit ledger service.",
    )
    LOCAL_AUDIT_LOG_PATH: str = Field(
        default="data/meta_learning_audit.jsonl",
        description="Local file path for audit log fallback.",
    )
    AUDIT_ENCRYPTION_KEY: Optional[SensitiveValue] = Field(
        default=None, description="Fernet encryption key for audit logs."
    )
    AUDIT_SIGNING_PRIVATE_KEY: Optional[SensitiveValue] = Field(
        default=None,
        description="ECDSA private key for signing audit logs (PEM format).",
    )
    AUDIT_SIGNING_PUBLIC_KEY: Optional[SensitiveValue] = Field(
        default=None,
    ...
    AUDIT_LOG_MAX_FILES: int = Field(
        default=10,
        ge=1,
        description="Maximum number of rotated audit log files to keep.",
    )
    ...
    LLM_RATE_LIMIT_CALLS: int = Field(
        default=100,
        ge=1,
        description="Number of LLM calls allowed per rate limit period.",
    )
    LLM_RATE_LIMIT_PERIOD: int = Field(
        default=60, ge=1, description="The period in seconds for the LLM rate limit."
    )

    # Agent Configuration
    DEFAULT_LANGUAGE: str = Field(
        default="en",
        description="Default language for agent communication (e.g., 'en' for English).",
    )
    ...
    MAX_META_LEARNING_CORRECTIONS: int = Field(
        default=10, description="Maximum corrections to store"
    )
    ...
    TRAINING_CHECK_INTERVAL_SECONDS: int = Field(
        default=3600,
        ge=60,
        description="Interval to check for new training data in seconds.",
    )
    DEPLOYMENT_CHECK_INTERVAL_SECONDS: int = Field(
        default=1800,
        ge=60,
        description="Interval to check for new models to deploy in seconds.",
    )
    ...
    # Service Endpoints
    ML_PLATFORM_ENDPOINT: AnyUrl = Field(
        default="http://localhost:8081/ml-platform",
        description="Endpoint for the ML Platform service.",
    )
    AGENT_CONFIG_SERVICE_ENDPOINT: AnyUrl = Field(
        default="http://localhost:8082/agent-config",
        description="Endpoint for the Agent Configuration service.",
    )
    POLICY_ENGINE_ENDPOINT: AnyUrl = Field(
        default="http://localhost:8083/policy-engine",
        description="Endpoint for the Policy Engine service.",
    )
    ...
    CONFIG_RELOAD_INTERVAL_SECONDS: int = Field(
        default=300,
        ge=0,
        description="Interval to check for config reloads (e.g., from Etcd). Set to 0 to disable.",
    )
    CONFIG_SOURCE: str = Field(
        default="env",
        description="Source for dynamic configuration (e.g., 'env', 'file', 'etcd').",
    )
    CONFIG_FILE_PATH: Optional[str] = Field(
        default=None,
        description="Path to a configuration file if CONFIG_SOURCE is 'file'.",
    )
    ...
    def validate_redis_url(cls, v):
        ...
        if v.scheme not in ("http", "https", "redis", "rediss"):
            raise ValueError(
                "REDIS_URL must be a valid HTTP or Redis URL scheme (http, https, redis, rediss)."
            )
        return v
    ...
    def reload_config(self):
        ...
        if self.CONFIG_SOURCE == "file" and self.CONFIG_FILE_PATH:
            try:
                ...
                logger.error(
                    f"An unexpected error occurred during config file reload: {str(e)}",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.error(
                    f"Failed to reload configuration from Etcd: {str(e)}", exc_info=True
                )
    ...
    def _reload_from_etcd(self):
        ...
        try:
            ...
            raise ImportError(
                "etcd3 library not installed. " "Install it with: pip install etcd3"
            )

        try:
            # Connect to Etcd
            etcd_port = int(os.getenv("ETCD_PORT", "2379"))
            etcd = etcd3.client(host=self.ETCD_HOST, port=etcd_port, timeout=5)
            ...
            if not config_updates:
                logger.warning(f"No configuration found in Etcd with prefix: {prefix}")
                return
            ...
            for key, value in config_updates.items():
                ...
                if hasattr(self, attr_name):
                    try:
                        ...
                        logger.warning(f"Failed to convert Etcd value for {key}: {e}")
                else:
                    logger.debug(
            ...
            logger.error(f"Failed to connect to Etcd at {self.ETCD_HOST}: {e}")
            raise
        except Exception as e:
            logger.error(f"Error fetching configuration from Etcd: {e}", exc_info=True)
            raise
