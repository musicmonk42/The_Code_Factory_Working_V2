...
def mock_multimodal_processors(monkeypatch):
    ...
    mock_concise_strategy.return_value.create_agent_prompt = AsyncMock(return_value="Concise Prompt")
    
    monkeypatch.setattr('arbiter.knowledge_graph.core.DefaultMultiModalProcessor', mock_default_processor)
...
def mock_langchain_components(monkeypatch):
    ...
    monkeypatch.setattr('arbiter.knowledge_graph.core.messages_from_dict', MagicMock(return_value=[]))
    monkeypatch.setattr('arbiter.knowledge_graph.core.load_persona_dict', 
                       MagicMock(return_value={"default": "Test persona", "expert": "Expert persona", "scrum_master": "Scrum Master persona"}))
...
@pytest.fixture(autouse=True)
def setup_test_environment(mock_config, mock_external_clients, mock_llm_providers, 
                          mock_multimodal_processors, mock_langchain_components,
                          mock_agent_metrics):
