        ...
        with patch('arbiter.knowledge_graph.core.RedisClient') as mock_redis:
            with patch('arbiter.knowledge_graph.core.PostgresClient') as mock_postgres:
                with patch('arbiter.knowledge_graph.core.AuditLedgerClient') as mock_audit:
                    with patch('arbiter.knowledge_graph.core.DefaultMultiModalProcessor') as mock_mm:
                        with patch('arbiter.knowledge_graph.core.DefaultPromptStrategy') as mock_prompt:
                            with patch('arbiter.knowledge_graph.core.load_persona_dict', return_value={"default": "Test persona", "expert": "Expert persona"}):
                                with patch('arbiter.knowledge_graph.core.AGENT_METRICS', {
    ...
    async def test_team_delegate_task(self, mock_team_config, mock_all_external_services):
        ...
        with patch('builtins.open', side_effect=FileNotFoundError):
            ...
            mock_req_response = {
                ...
                "trace": {"initial": "init", "reflection": "ref", "critique": "crit", "corrected": "corr"}
            }
            mock_ref_response = {
                "response": "Refined response",
                "trace": {"initial": "init2", "reflection": "ref2", "critique": "crit2", "corrected": "corr2"}
            }
...
class TestFactoryFunctions:
    ...
    async def test_get_or_create_agent_with_postgres(self, mock_all_external_services):
        ...
        with patch('arbiter.knowledge_graph.core.Config.POSTGRES_DB_URL', "postgresql://localhost/testdb"):
...
class TestUtilityFunctions:
    ...
    def test_get_transcript(self):
        ...
        with patch('arbiter.knowledge_graph.core._sanitize_user_input', side_effect=lambda x: f"sanitized_{x}"):
