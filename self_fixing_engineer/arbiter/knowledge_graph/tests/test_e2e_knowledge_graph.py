...
class TestKnowledgeGraphE2EWorkflow:
    ...
    def setup_environment(self, tmp_path):
        ...
        prompt_templates_file.write_text(json.dumps({
        ...
        with patch.dict(os.environ, env_vars):
            with patch('arbiter.knowledge_graph.core.ChatOpenAI') as mock_llm:
                with patch('arbiter.knowledge_graph.config.load_persona_dict') as mock_personas:
                    ...
                    assert any(phrase in agent.persona.lower() for phrase in ["test", "knowledge", "assistant"])
        ...
        with patch.dict(os.environ, env_vars):
            with patch('arbiter.knowledge_graph.core.ChatOpenAI') as mock_llm:
                ...
                for agent_name in team.agents:
                    team.agents[agent_name].predict = AsyncMock(return_value={
    ...
    async def test_config_validation_and_loading(self, setup_environment):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            with patch('builtins.open', mock_open(read_data=json.dumps({
                "knowledge": "Knowledge Graph Specialist"
            }))):
...
class TestKnowledgeGraphSecurity:
    ...
    async def test_pii_redaction(self):
        ...
        with patch('arbiter.knowledge_graph.utils.Config.PII_SENSITIVE_KEYS', ['password', 'ssn']):
