...
class TestDefaultMultiModalProcessor:
    ...
    def test_processor_initialization_no_libraries(self, mock_logger, mock_config):
        ...
        with patch.multiple('arbiter.knowledge_graph.multimodal',
    ...
    def test_processor_initialization_with_transformers(self, mock_logger, mock_config):
        ...
        with patch('arbiter.knowledge_graph.multimodal.TRANSFORMERS_AVAILABLE', True):
            with patch('arbiter.knowledge_graph.multimodal.pipeline') as mock_pipeline:
                ...
                mock_pipeline.assert_any_call("image-to-text", model="Salesforce/blip-image-captioning-base")
                mock_pipeline.assert_any_call("automatic-speech-recognition", model="openai/whisper-tiny")
                mock_pipeline.assert_any_call("summarization", model="facebook/bart-large-cnn")
    
    @pytest.mark.asyncio
    async def test_summarize_with_cache_hit(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics, mock_audit):
        ...
        cached_result = {"status": "success", "summary": "Cached result", "data_hash": data_hash}
        
        with patch('arbiter.knowledge_graph.multimodal.async_with_retry', AsyncMock(return_value=json.dumps(cached_result))):
            item = mock_multimodal_data("image", test_data)
            result = await processor.summarize(item)
            
            assert result == cached_result
            mock_metrics["multimodal_data_processed_total"].labels.assert_called_once_with(data_type="image")
    
    @pytest.mark.asyncio
    async def test_summarize_data_too_large(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics, mock_audit):
    ...
    @pytest.mark.asyncio
    async def test_summarize_unsupported_type(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics, mock_audit):
    ...
    @pytest.mark.asyncio
    async def test_summarize_timeout(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics, mock_audit):
        ...
        with patch('asyncio.wait_for', side_effect=asyncio.TimeoutError()):
            ...
            mock_metrics["mm_processor_failures_total"].labels.assert_called_once_with(
                data_type="image",
                error_type=AgentErrorCode.TIMEOUT.value
            )
    ...
    @pytest.mark.asyncio
    async def test_process_image_not_available(self, mock_logger, mock_config, mock_multimodal_data, mock_audit):
    ...
    @pytest.mark.asyncio
    async def test_process_text_file_decode_error(self, mock_logger, mock_config, mock_multimodal_data):
    ...
    @pytest.mark.asyncio
    async def test_caching_successful_result(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics):
        ...
        with patch('arbiter.knowledge_graph.multimodal.async_with_retry', AsyncMock(side_effect=[None, None])):
            result = await processor.summarize(item)
    ...
    @pytest.mark.asyncio
    async def test_exception_handling(self, mock_logger, mock_config, mock_multimodal_data, mock_metrics, mock_audit):
    ...
    async def test_concurrent_processing(self):
        ...
        with patch('arbiter.knowledge_graph.multimodal.Config') as mock_config:
            ...
            results = await asyncio.gather(
                *[processor.summarize(item) for item in items],
    ...
    async def test_audit_logging_failure(self):
        ...
        with patch('arbiter.knowledge_graph.multimodal.Config') as mock_config:
            ...
            with patch('arbiter.knowledge_graph.multimodal.audit_ledger_client') as mock_audit:
                ...
                assert any("Failed to log audit" in str(call) for call in logger.warning.call_args_list)
