...
GROWTH_ANOMALY_SCORE = get_or_create_metric(
    Gauge, "growth_anomaly_score", "Anomaly score for growth events", ["arbiter"]
)
CONFIG_FALLBACK_USED = get_or_create_metric(
    Counter,
    "config_fallback_used_total",
    "Total times fallback config was used",
    ["arbiter"],
)
GROWTH_AUDIT_ANCHORS_TOTAL = get_or_create_metric(
    Counter,
    "growth_audit_anchors_total",
    "Total audit chain anchors created",
    ["arbiter"],
)
GROWTH_ERRORS_TOTAL = get_or_create_metric(
    Counter,
    "growth_errors_total",
    "Total errors in growth operations",
    ["arbiter", "error_type"],
)
    ...
    async def get_config(self, key: str) -> Any:
        async with self._cache_lock:
            ...
            if self.client:
                try:
                    with tracer.start_as_current_span(
                        f"etcd_get_config_{key}", attributes={"config.key": key}
                    ):
                        value_bytes, _ = await asyncio.get_event_loop().run_in_executor(
                            None, lambda: self.client.get(key)
                        )
                        if value_bytes:
            ...
            if key in self.defaults:  # Added check for key in self.defaults
                ...
                self._cache[key] = value
                logger.debug(
                    f"Config '{key}' not found in etcd or fallback, using default: {value}"
                )
                return value
...
class SQLiteStorageBackend:
    ...
    async def save(self, arbiter_id: str, data: Dict[str, Any]) -> None:
        async with self.session_factory() as session:
            async with session.begin():
                ...
                record.skills_encrypted = self.cipher.encrypt(
                    json.dumps(data["skills"]).encode("utf-8")
                )
                record.user_preferences_encrypted = self.cipher.encrypt(
                    json.dumps(data.get("user_preferences", {})).encode("utf-8")
                )
    ...
    async def save_event(self, arbiter_id: str, event: Dict[str, Any]) -> None:
        async with self.session_factory() as session:
            async with session.begin():
                details_encrypted = self.cipher.encrypt(
        ...
        async with self.session_factory() as session:
            async with session.begin():
                ...
                current_hash = hashlib.sha256(
                    f"{arbiter_id}{operation}{timestamp}{details_str}{previous_hash}".encode()
                ).hexdigest()
...
class RedisStreamsStorageBackend:
    ...
    async def _get_stream_key(self, arbiter_id: str) -> str:
        return f"arbiter:{arbiter_id}:events"

    async def _get_snapshot_key(self, arbiter_id: str) -> str:
        return f"arbiter:{arbiter_id}:snapshot"

    async def _get_audit_key(self, arbiter_id: str) -> str:
        return f"arbiter:{arbiter_id}:audit"
    ...
    async def save(self, arbiter_id: str, data: Dict[str, Any]) -> None:
        ...
        if self.cipher:
            save_data[b"skills_encrypted"] = self.cipher.encrypt(
                json.dumps(data["skills"]).encode("utf-8")
            )
            save_data[b"user_preferences_encrypted"] = self.cipher.encrypt(
                json.dumps(data.get("user_preferences", {})).encode("utf-8")
            )
        else:
    ...
    async def save_event(self, arbiter_id: str, event: Dict[str, Any]) -> None:
        ...
        if self.cipher:
            event_data_to_store[b"details_encrypted"] = self.cipher.encrypt(
                json.dumps(event["details"]).encode("utf-8")
            )
        else:
    ...
    async def load_events(
        ...
        while True:
            ...
            response = await self.redis.xread(
                ...
                count=self.config_store.defaults.get(
                    "redis_batch_size", 1000
                ),  # Use a configured batch size
                block=0,  # Do not block
            )

            if (
                not response or not response[0][1]
            ):  # Check if response is empty or stream has no messages
                break

            for msg_id, msg_data in response[0][1]:
                event = {}
                event["canonical_offset"] = msg_id.decode(
                    "utf-8"
                )  # Redis Stream ID as canonical offset
                ...
                event["event_version"] = float(
                    msg_data.get(b"event_version", b"1.0").decode("utf-8")
                )
                ...
                if b"details_encrypted" in msg_data and self.cipher:
                    try:
                        ...
                        logger.error(
                            f"Failed to decrypt Redis event details for {arbiter_id} (ID: {msg_id.decode('utf-8')}): {e}"
                        )
            ...
            last_id = events_with_offsets[-1][
                "canonical_offset"
            ]  # Next read starts after the last message received
...
class KafkaStorageBackend:
    ...
    async def stop(self):
        ...
        if self.zookeeper:
            await asyncio.get_event_loop().run_in_executor(None, self.zookeeper.stop)
    ...
    async def load(self, arbiter_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.warning(
            f"KafkaStorageBackend: Direct 'load' of latest snapshot for {arbiter_id} is not efficient. Consider a dedicated snapshot storage for this operation."
        )
                        ...
                        if self.snapshot_deserializer:
                            ...
                            if self.cipher:
                                if snapshot_data.get("skills_encrypted"):
                                    snapshot_data["skills"] = json.loads(
                                        self.cipher.decrypt(
                                            snapshot_data["skills_encrypted"]
                                        ).decode("utf-8")
                                    )
                                if snapshot_data.get("user_preferences_encrypted"):
                                    snapshot_data["user_preferences"] = json.loads(
                                        self.cipher.decrypt(
                                            snapshot_data["user_preferences_encrypted"]
                                        ).decode("utf-8")
                                    )
                            ...
                            logger.info(
                                f"Loaded snapshot for {arbiter_id} from offset {latest_message.offset}"
                            )
    ...
    async def save(self, arbiter_id: str, data: Dict[str, Any]) -> None:
        ...
        if self.cipher:
            save_data_avro["skills_encrypted"] = self.cipher.encrypt(
                json.dumps(data["skills"]).encode("utf-8")
            )
            save_data_avro.pop("skills", None)  # Remove unencrypted field
            save_data_avro["user_preferences_encrypted"] = self.cipher.encrypt(
                json.dumps(data.get("user_preferences", {})).encode("utf-8")
            )
    ...
    async def save_event(self, arbiter_id: str, event: Dict[str, Any]) -> None:
        ...
        if self.cipher:
            event_data_avro["details_encrypted"] = self.cipher.encrypt(
                json.dumps(event["details"]).encode("utf-8")
            )
        ...
        try:
            ...
            async with (
                self.producer.transaction()
            ):  # This ensures atomicity for batching
                serialized_event = self.event_serializer(event_data_avro)
                await self.producer.send(
                    event_topic, serialized_event, key=arbiter_id.encode("utf-8")
                )
    ...
    async def load_events(
        ...
        try:
            ...
            if not partitions:
                logger.warning(
                    f"No partitions found for event topic {event_topic}. Cannot load events."
                )
                return []
            ...
            if isinstance(from_offset, str) and ":" in from_offset:
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid 'from_offset' format: {from_offset}. Starting from earliest."
                    )
            else:
            ...
            while True:
                ...
                for tp, msgs in messages.items():
                    for message in msgs:
                        ...
                        if self.event_deserializer:
                            ...
                            if self.cipher and deserialized_event.get(
                                ...
                                try:
                                    deserialized_event["details"] = json.loads(
                                        self.cipher.decrypt(
                                            deserialized_event["details_encrypted"]
                                        ).decode("utf-8")
                                    )
                                except Exception as e:
                                    logger.error(
                            ...
                            deserialized_event["canonical_offset"] = (
                ...
                if (
                    sum(len(m) for m in messages.values()) < 100
                ):  # If we got a partial batch, assume end
                    break
    ...
    async def save_audit_log(
        ...
        try:
            ...
            await self.producer.send_and_wait(
                audit_topic,
                json.dumps(audit_entry).encode("utf-8"),
                key=arbiter_id.encode("utf-8"),
            )
            logger.info(
                f"Audit log (Kafka backend) saved: {arbiter_id}: {operation} - Hash: {current_hash}"
            )
        except Exception as e:
    ...
    async def get_last_audit_hash(self, arbiter_id: str) -> str:
        ...
        try:
            ...
            for tp in tps:
                consumer.seek(
                    tp, end_offsets[tp] - 1 if end_offsets[tp] > 0 else 0
                )  # Seek to the last message if exists

            # Poll messages for a short period
            messages = await consumer.getmany(
                timeout_ms=1000, max_records=len(tps)
            )  # Try to get 1 message per partition
    ...
    async def load_all_audit_logs(self, arbiter_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            while True:
                messages = await consumer.getmany(
                    timeout_ms=100, max_records=500
                )  # Fetch in batches
                if not messages:
                    break  # No more messages
                ...
                for tp, msgs in messages.items():
                    for message in msgs:
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to decode audit log JSON from Kafka message at offset {message.offset}: {e}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Error processing audit log message from Kafka at offset {message.offset}: {e}"
                            )
...
class ArbiterState(BaseModel):
    ...
    event_offset: Union[int, str] = Field(
        "0"
    )  # This will store the last processed Kafka offset (e.g., "partition:offset") or SQLite ID
...
class ArbiterGrowthManager:
    ...
    async def anchor_audit_chain_periodically(
        self, external_ledger_api: Callable[[str, str], Awaitable[None]]
    ):
        """
        ...
        with tracer.start_as_current_span("anchor_audit_chain"):
            try:
                latest_hash = await self._call_maybe_async(
                    self.storage_backend.get_last_audit_hash, self.arbiter
                )
                if latest_hash != "genesis_hash":
                    ...
                    logger.info(
                        f"Anchoring latest audit hash {latest_hash} for {self.arbiter} to external ledger."
                    )
        ...
        with tracer.start_as_current_span(
            ...
            if snapshot_data:
                ...
                if snapshot_data.get("schema_version", 1.0) < self.SCHEMA_VERSION:
                    logger.info(
                        f"Applying schema migration for {self.arbiter}. Old version: {snapshot_data.get('schema_version')}, New version: {self.SCHEMA_VERSION}"
                    )
            ...
            events_to_replay = await self._call_maybe_async(
                self.storage_backend.load_events, self.arbiter, from_offset=from_offset
            )
            logger.info(
                f"Replaying {len(events_to_replay)} events for {self.arbiter} from offset {self._state.event_offset}"
            )
            ...
            for event_dict in events_to_replay:
                ...
                self._state.event_offset = event.details.get(
                    "canonical_offset", self._state.event_offset
                )  # Use canonical offset if provided by backend
            ...
            logger.info(
                f"Finished replaying events for {self.arbiter}. Current level: {self._state.level}, Events since last snapshot: {self._event_count_since_snapshot}"
            )
    ...
    async def _apply_event(self, event: GrowthEvent) -> None:
        ...
        """
        with tracer.start_as_current_span(
            f"apply_event_{event.type}", attributes={"event.type": event.type}
        ):
        ...
        if hasattr(self._snapshot_breaker, "__aenter__"):
            try:
                ...
                GROWTH_CIRCUIT_BREAKER_TRIPS.labels(
                    arbiter=self.arbiter, breaker_name="snapshot"
                ).inc()
                logger.error(
                    f"Failed to save snapshot for {self.arbiter}: Circuit breaker is open."
                )
    ...
    async def __do_save_snapshot(self):
        async with self._save_lock:
            ...
            logger.info(
                f"Snapshot saved for {self.arbiter}. Current event offset: {self._state.event_offset}"
            )
    ...
    async def _audit_log(self, operation: str, details: Dict[str, Any]) -> None:
        ...
        previous_hash = await self._call_maybe_async(
            self.storage_backend.get_last_audit_hash, self.arbiter
        )
        new_hash = await self._call_maybe_async(
            self.storage_backend.save_audit_log,
            self.arbiter,
            operation,
            details,
            previous_hash,
        ...
        logger.debug(
            f"Audit logged operation '{operation}' for {self.arbiter}. New hash: {new_hash}"
        )
        ...
        try:
            ...
            if self.feedback_manager:
                ...
                if await self._call_maybe_async(
                    ...
                    logger.debug(
                        f"Event '{event.type}' pushed to Feedback Manager for {self.arbiter}."
                    )
                else:
                    logger.info(
                        f"Feedback Manager update for event '{event.type}' (key: {fm_idempotency_key}) skipped due to idempotency."
                    )
            ...
            GROWTH_EVENT_PUSH_LATENCY.labels(arbiter=self.arbiter).observe(
    ...
    async def _queue_operation(
        ...
        if self._load_task and self._load_task.done():
            ...
            logger.debug(
                f"Queueing operation for {self.arbiter} (load task pending). Queue size: {self._pending_operations.qsize() + 1}"
            )
    ...
    async def acquire_skill(
        ...
        try:
            ...
            logger.error(
                f"[{self.arbiter}] Failed to queue skill acquisition event for {skill_name}: {e}"
            )
            raise
    ...
    async def improve_skill(
        ...
        try:
            ...
            logger.error(
                f"[{self.arbiter}] Failed to queue skill improvement event for {skill_name}: {e}"
            )
            raise
    ...
    async def gain_experience(
        self, amount: float, context: Optional[Dict[str, Any]] = None
    ) -> None:
        """
    ...
    async def health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_data = {
                ...
                "snapshot_breaker_state": getattr(
                    self._snapshot_breaker, "current_state", "unknown"
                ),
                "push_event_breaker_state": getattr(
                    self._push_event_breaker, "current_state", "unknown"
                ),
                "storage_backend_status": storage_status,
                "idempotency_store_status": idempotency_status,
                "config_store_status": config_status,
                "last_audit_hash": await self._call_maybe_async(
                    self.storage_backend.get_last_audit_hash, self.arbiter
                ),
                "up_time_seconds": (
    ...
    async def rotate_encryption_key(self, new_key: bytes) -> None:
        ...
        """
        logger.warning(
            f"[{self.arbiter}] Encryption key rotation is a conceptual method. A full, production-grade implementation would require changes to the StorageBackend interface to support re-encryption of all data."
        )
        GROWTH_ERRORS_TOTAL.labels(
            arbiter=self.arbiter, error_type="key_rotation_not_implemented"
        ).inc()
