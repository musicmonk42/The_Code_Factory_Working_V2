...
try:
    from arbiter.message_queue_service import MessageQueueService, MessageQueueServiceError, SerializationError, DecryptionError
...
CONSUMER_MESSAGES_PROCESSED_TOTAL = _get_metric(Counter, 'consumer_messages_processed_total', 'Total messages processed by consumer', ['event_type', 'status'])
CONSUMER_DELIVERY_ATTEMPTS_TOTAL = _get_metric(Counter, 'consumer_delivery_attempts_total', 'Delivery attempts to external webhooks', ['event_type', 'webhook_url_hash'])
CONSUMER_DELIVERY_SUCCESS_TOTAL = _get_metric(Counter, 'consumer_delivery_success_total', 'Successful messages delivered externally', ['event_type', 'webhook_url_hash'])
CONSUMER_DELIVERY_FAILURE_TOTAL = _get_metric(Counter, 'consumer_delivery_failure_total', 'Failed external deliveries', ['event_type', 'webhook_url_hash', 'error_code'])
CONSUMER_DELIVERY_LATENCY_SECONDS = _get_metric(Histogram, 'consumer_delivery_latency_seconds', 'Latency for external delivery', ['event_type', 'webhook_url_hash'])
CONSUMER_POISON_MESSAGES_TOTAL = _get_metric(Counter, 'consumer_poison_messages_total', 'Poison messages quarantined', ['event_type'])
CONSUMER_OPS_TOTAL = _get_metric(Counter, 'consumer_ops_total', 'Total consumer operations', ['event_type'])
CONSUMER_ERRORS_TOTAL = _get_metric(Counter, 'consumer_errors_total', 'Total consumer errors', ['event_type', 'error_type'])
...
def redact_sensitive(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        return data
    sensitive_keys = {"token", "key", "password", "secret", "api_key", "webhook_url", "routing_key"}
    out = {}
...
async def initialize_handlers():
    ...
    if getattr(settings_inst, "SLACK_WEBHOOK_URL", None):
        try:
            ...
            _EXTERNAL_NOTIFIER_HANDLERS["slack"] = SlackAuditHook(webhook_url=str(settings_inst.SLACK_WEBHOOK_URL))
...
async def send_to_external_notifier(event_type: str, data: Dict[str, Any]) -> bool:
    ...
    with tracer.start_as_current_span(f"send_notification_{event_type}", attributes={"event.type": event_type, "correlation.id": correlation_id}):
        for handler_name, handler_instance in _EXTERNAL_NOTIFIER_HANDLERS.items():
            ...
            try:
                ...
                if hasattr(handler_instance, 'audit_hook') and asyncio.iscoroutinefunction(handler_instance.audit_hook):
                    await handler_instance.audit_hook(event_type, outgoing_data)
                # Check for a partial function wrapped with a handler
                elif isinstance(handler_instance, partial) and asyncio.iscoroutinefunction(handler_instance.func):
                ...
                logger.info(f"Delivered {event_type} via {handler_name} [correlation_id={correlation_id}]")
            except Exception as e:
                CONSUMER_DELIVERY_FAILURE_TOTAL.labels(event_type, url_hash, type(e).__name__).inc()
                CONSUMER_ERRORS_TOTAL.labels(event_type=event_type, error_type="external_delivery").inc()
                logger.error(f"Delivery to '{handler_name}' failed: {e} [correlation_id={correlation_id}]")
...
async def process_event(event_type: str, data: Dict[str, Any], mq_service: MessageQueueService, audit_logger: AuditLogManager):
    """
    ...
    correlation_id = data.get("correlation_id", str(uuid.uuid4()))
    message_id = data.get("event_id") or data.get("id") or hashlib.sha256(json.dumps(data, sort_keys=True).encode()).hexdigest()
    ...
    if SFE_CORE_AVAILABLE and getattr(mq_service, 'redis_client', None):
        try:
            ...
            if val: current_retries = int(val)
        except Exception as e:
    ...
    if current_retries >= POISON_MESSAGE_THRESHOLD:
        logger.critical(f"Poison message for {event_type} [{correlation_id}] exceeded retries; moving to DLQ.")
        ...
        await audit_logger.audit("poison_message_quarantined", {
    ...
    try:
        ...
        if delivered:
            ...
            await audit_logger.audit("message_delivery_failed", {
                "event_type": event_type,
                "message_id": message_id,
                "correlation_id": correlation_id,
        ...
        await audit_logger.audit("poison_message_unhandled_error", {
            "event_type": event_type,
            "message_id": message_id,
            "error": str(e),
            "correlation_id": correlation_id,
        ...
        url_hash = hashlib.sha256(str(data.get('webhook_url', 'generic')).encode()).hexdigest()[:8]
        CONSUMER_DELIVERY_LATENCY_SECONDS.labels(event_type, url_hash).observe(time.monotonic() - start_time)
...
async def handle_message(event_type: str, data: Dict[str, Any], mq_service: MessageQueueService, audit_logger: AuditLogManager):
    """
    Processes a single message, handles delivery, and manages poison messages.
...
class QueueConsumerWorker:
    ...
    async def run(self):
        ...
        for event_type in event_types:
            handler_with_ctx = partial(handle_message, event_type=event_type, mq_service=self.mq_service, audit_logger=self.audit_logger)
...
if SFE_CORE_AVAILABLE:
    registry.register(kind=PlugInKind.CORE_SERVICE, name="QueueConsumerWorker", version="1.0.0", author="Arbiter Team")(QueueConsumerWorker)