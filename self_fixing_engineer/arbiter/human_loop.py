...
try:
    ...
    logging.getLogger(__name__).warning("aiosmtplib not available. Asynchronous email functionality will be disabled.")
...
try:
    ...
    def get_or_create_counter(*args, **kwargs): return DummyCounter()
...
try:
    ...
    logging.getLogger(__name__).warning("Warning: models.db_clients not found. DummyDBClient will be used as a fallback.")
    ...
    class DummyDBClient:
        ...
        async def get_feedback_entries(self, query: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
            ...
            return [e for e in self.feedback_entries if isinstance(e, dict) and all(e.get(k) == v for k, v in query.items())]
...
class HumanInLoopConfig(BaseModel):
    ...
    EMAIL_SMTP_PASSWORD: Optional[str] = Field(None, description="SMTP password for authentication.")
    ...
    EMAIL_RECIPIENTS: Dict[str, str] = Field(default_factory=dict, description="Map of roles to email addresses.")
    ...
    MAX_NOTIFICATION_RETRIES: int = Field(3, description="Maximum number of retries for a notification.")
    ...
    def validate_production_email_config(cls, values):
        if values.get('IS_PRODUCTION') and values.get('EMAIL_ENABLED'):
            if not all([values.get('EMAIL_SMTP_SERVER'), values.get('EMAIL_SMTP_USER'), values.get('EMAIL_SMTP_PASSWORD')]):
                raise ValueError("In production, EMAIL_ENABLED requires all SMTP configuration fields (server, user, password).")
        return values
    
    @field_validator('DATABASE_URL')
    def validate_database_url_in_production(cls, v, info):
        if info.data.get('IS_PRODUCTION') and not v:
            raise ValueError("In production, DATABASE_URL must be set and point to a real database.")
        return v
    
    @model_validator(mode='after')
    def validate_salt_in_production(self):
        if self.IS_PRODUCTION and "default-dev-salt" in SECRET_SALT:
            logger.critical("SECURITY ALERT: Using default SECRET_SALT in a production environment is insecure. Please set the HITL_SECRET_SALT environment variable.")
        return self
...
class FeedbackManager:
    ...
    async def log_approval_request(self, decision_id: str, decision_context: Dict[str, Any]) -> None:
        log_entry = {
    ...
    async def log_approval_response(self, decision_id: str, response: Dict[str, Any]) -> None:
        ...
        await self.db_client.update_feedback_entry(
            ...
            {"status": "resolved", "resolution_timestamp": ts, "response_details": response}
        )
    ...
    async def log_error(self, error_details: Dict[str, Any]) -> None:
        await self.db_client.save_feedback_entry({**error_details, "timestamp": datetime.now(timezone.utc).isoformat(), "type": "error_log"})
...
class HumanInLoop:
        ...
        if db_url:
            if db_url.startswith("postgresql") and DB_CLIENTS_AVAILABLE:
                ...
                if self.config.IS_PRODUCTION:
                    raise RuntimeError(f"HumanInLoop: In production, DATABASE_URL '{db_url}' is not supported or its client is not available. Refusing to start.")
                else:
                    self._db_client = DummyDBClient()
                    self.logger.warning(f"HumanInLoop: Development mode: DATABASE_URL '{db_url}' not recognized or driver not available. Falling back to DummyDBClient.")
        else:
            if self.config.IS_PRODUCTION:
                raise RuntimeError("HumanInLoop: In production, DATABASE_URL is not set. Refusing to start without a real database.")
            else:
                self._db_client = DummyDBClient()
                self.logger.warning("HumanInLoop: Development mode: No DATABASE_URL found in config. Falling back to DummyDBClient.")
        
        self.feedback_manager = feedback_manager if feedback_manager else FeedbackManager(db_client=self._db_client)
    ...
    async def _handle_hook(self, hook: Optional[Callable[[Dict[str, Any]], Awaitable[None]]], event_data: Dict[str, Any]) -> None:
        """
        ...
        if hook:
            try:
                ...
                self.logger.error(f"Error executing hook for event type {event_data.get('event_type')}: {e}")
    ...
    async def request_approval(self, decision: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._handle_hook(self.error_hook, {"event_type": "invalid_request_schema", "errors": e.errors(), "request_data": decision})
            return {"approved": False, "comment": "Invalid request schema."}

        decision_id = validated_request.decision_id or validated_request.cycle or f"decision_{hashlib.sha256(json.dumps(validated_request.model_dump(), sort_keys=True).encode()).hexdigest()}"
        timeout = validated_request.timeout_seconds if validated_request.timeout_seconds is not None else self.config.DEFAULT_TIMEOUT_SECONDS
        context = validated_request.model_dump()
        context["decision_id"] = decision_id
        context["timeout_seconds"] = timeout

        await self.feedback_manager.log_approval_request(decision_id, context)
        await self._handle_hook(self.audit_hook, {"event_type": "approval_requested", "decision_id": decision_id, "context": context})

        async with self._lock:
            approval_future = asyncio.Future()
            self._pending_approvals[decision_id] = approval_future

        notification_tasks = self._get_notification_tasks(decision_id, context, context.get("required_role", "reviewer"))
        if not notification_tasks:
            self.logger.warning(f"No notification channel configured for {decision_id}; using mock approval.")
            await self._mock_user_approval(decision_id, context)
        else:
            results = await asyncio.gather(*notification_tasks, return_exceptions=True)
            if all(isinstance(res, Exception) for res in results):
                self.logger.warning(f"All notification channels failed for {decision_id}; falling back to mock approval.")
                await self._mock_user_approval(decision_id, context)

        try:
            return await asyncio.wait_for(approval_future, timeout=timeout)
        except asyncio.TimeoutError:
            timeout_response = {"approved": False, "user_id": "system", "comment": "Approval request timed out."}
            await self._handle_hook(self.error_hook, {"event_type": "request_timeout", "decision_id": decision_id})
            await self.receive_human_feedback({"decision_id": decision_id, "signature": "N/A_timeout", **timeout_response})
            return timeout_response
        finally:
            async with self._lock:
                self._pending_approvals.pop(decision_id, None)

    def _get_notification_tasks(self, decision_id: str, context: Dict[str, Any], role: str) -> List[Awaitable[None]]:
    ...
    async def receive_human_feedback(self, feedback: Dict[str, Any]) -> None:
        ...
        with tracer.start_as_current_span("receive_human_feedback"):
            ...
            try:
                ...
                await self._handle_hook(self.error_hook, {"event_type": "invalid_feedback_schema", "errors": e.errors(), "feedback_data": feedback})
                return
            ...
            if not await self._validate_user_signature(validated.user_id, validated.signature, validated.decision_id, validated.approved, validated.comment, validated.timestamp):
                self.logger.error(f"SECURITY ALERT: Invalid signature for feedback on {validated.decision_id}. Rejected.")
                await self._handle_hook(self.error_hook, {"event_type": "invalid_signature", "decision_id": validated.decision_id, "user_id": validated.user_id})
                return

            await self.feedback_manager.log_approval_response(validated.decision_id, validated.model_dump())
            await self._handle_hook(self.audit_hook, {"event_type": "feedback_received", "feedback": validated.model_dump()})
            
            # Record Prometheus metrics
            if validated.approved:
                human_in_loop_approvals.labels(decision_id=validated.decision_id).inc()
                await self.feedback_manager.record_metric("human_approval", 1.0, {"decision_id": validated.decision_id, "status": "approved"})
            else:
                human_in_loop_denials.labels(decision_id=validated.decision_id).inc()
                await self.feedback_manager.record_metric("human_approval", 0.0, {"decision_id": validated.decision_id, "status": "denied"})
    ...
    async def _validate_user_signature(self, user_id: str, signature: str, decision_id: str, approved: bool, comment: str, timestamp: str) -> bool:
        """
        Validates human feedback signature.
        """
        expected = hashlib.sha256(f"{decision_id}{user_id}{approved}{comment}{timestamp}{SECRET_SALT}".encode()).hexdigest()
        return signature == expected

    async def _send_email_approval(self, decision_id: str, context: Dict[str, Any], recipient: str) -> None:
        ...
        if not self.config.EMAIL_ENABLED:
            ...
            await self._handle_hook(self.error_hook, {"event_type": "email_notification_skipped", "decision_id": decision_id, "reason": "email disabled in config"})
            return
...
"""
        ...
        while retries < self.config.MAX_NOTIFICATION_RETRIES:
            try:
                if self.config.IS_PRODUCTION and AIOSMTPLIB_AVAILABLE:
                    async with aiosmtplib.SMTP(hostname=self.config.EMAIL_SMTP_SERVER, port=self.config.EMAIL_SMTP_PORT, use_tls=self.config.EMAIL_USE_TLS) as server:
                        await server.login(self.config.EMAIL_SMTP_USER, self.config.EMAIL_SMTP_PASSWORD)
                        await server.send_message(msg)
                else:
                    loop = asyncio.get_running_loop()
                    await loop.run_in_executor(
                        None,
                        lambda: self._send_sync_email(self.config, recipient, msg)
                    )
                self.logger.info(f"Email notification for {decision_id} sent to {recipient}.")
                await self._handle_hook(self.audit_hook, {"event_type": "email_notification_sent", "decision_id": decision_id, "recipient": recipient})
                return
            except (smtplib.SMTPException, aiosmtplib.SMTPException) as e:
                self.logger.error(f"SMTP error sending email for {decision_id} (attempt {retries+1}/{self.config.MAX_NOTIFICATION_RETRIES}): {e}")
                retries += 1
                await self.sleeper(self.config.RETRY_DELAY_SECONDS)
            except Exception as e:
                self.logger.error(f"Failed to send approval email for {decision_id}: {e}", exc_info=True)
                await self._handle_hook(self.error_hook, {"event_type": "email_notification_failed", "decision_id": decision_id, "error": str(e)})
                return
        
        self.logger.error(f"Max retries reached: Failed to send approval email for {decision_id} after {self.config.MAX_NOTIFICATION_RETRIES} attempts.")
        await self._handle_hook(self.error_hook, {"event_type": "email_notification_failed_max_retries", "decision_id": decision_id, "error": "Max retries reached"})
    ...
    async def _post_slack_approval(self, decision_id: str, context: Dict[str, Any]) -> None:
        ...
        if not self.config.SLACK_WEBHOOK_URL:
            ...
            await self._handle_hook(self.error_hook, {"event_type": "slack_notification_skipped", "decision_id": decision_id, "reason": "slack webhook not configured"})
            return

        payload = {
            "text": f"Approval Required for action: {context.get('action')}",
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*Approval Required: `{context.get('action')}`*"}},
                {"type": "section", "fields": [
                ...
                {"type": "divider"},
                {"type": "section", "text": {"type": "mrkdwn", "text": f"This request will time out in *{context.get('timeout_seconds')} seconds*."}}
            ]
        }
        retries = 0
        while retries < self.config.MAX_NOTIFICATION_RETRIES:
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(self.config.SLACK_WEBHOOK_URL, json=payload, timeout=10) as response:
                        response.raise_for_status()
                        self.logger.info(f"Slack notification for {decision_id} sent successfully.")
                        await self._handle_hook(self.audit_hook, {"event_type": "slack_notification_sent", "decision_id": decision_id})
                        return
            except aiohttp.ClientError as e:
                self.logger.error(f"Slack API error sending notification for {decision_id} (attempt {retries+1}/{self.config.MAX_NOTIFICATION_RETRIES}): {e}")
                retries += 1
                await self.sleeper(self.config.RETRY_DELAY_SECONDS)
            except asyncio.TimeoutError:
                self.logger.error(f"Slack notification timeout for {decision_id} (attempt {retries+1}/{self.config.MAX_NOTIFICATION_RETRIES}).")
                retries += 1
                await self.sleeper(self.config.RETRY_DELAY_SECONDS)
            except Exception as e:
                self.logger.error(f"Failed to send Slack notification for {decision_id}: {e}", exc_info=True)
                await self._handle_hook(self.error_hook, {"event_type": "slack_notification_failed", "decision_id": decision_id, "error": str(e)})
                return
        
        self.logger.error(f"Max retries reached: Failed to send Slack notification for {decision_id} after {self.config.MAX_NOTIFICATION_RETRIES} attempts.")
        await self._handle_hook(self.error_hook, {"event_type": "slack_notification_failed_max_retries", "decision_id": decision_id, "error": "Max retries reached"})
    ...
    async def _notify_ui_approval(self, decision_id: str, context: Dict[str, Any]) -> None:
        ...
        if not self.websocket_manager:
            ...
            await self._handle_hook(self.error_hook, {"event_type": "websocket_notification_skipped", "decision_id": decision_id, "reason": "websocket manager not configured"})
            return
        ...
        while retries < self.config.MAX_NOTIFICATION_RETRIES:
            try:
                ...
                await self._handle_hook(self.audit_hook, {"event_type": "websocket_notification_sent", "decision_id": decision_id})
                return
            except Exception as e:
                self.logger.error(f"Failed to send WebSocket notification for {decision_id} (attempt {retries+1}/{self.config.MAX_NOTIFICATION_RETRIES}): {e}")
                retries += 1
                await self.sleeper(self.config.RETRY_DELAY_SECONDS)
        
        self.logger.error(f"Max retries reached: Failed to send WebSocket notification for {decision_id} after {self.config.MAX_NOTIFICATION_RETRIES} attempts.")
        await self._handle_hook(self.error_hook, {"event_type": "websocket_notification_failed_max_retries", "decision_id": decision_id, "error": "Max retries reached"})
    ...
    async def _mock_user_approval(self, decision_id: str, decision_context: Dict[str, Any]) -> None:
        ...
        timestamp = datetime.now(timezone.utc).isoformat()
        signature = hashlib.sha256(f"{decision_id}{user}{approved}{comment}{timestamp}{SECRET_SALT}".encode()).hexdigest()
...
mock_registry.register(kind=MockPlugInKind.CORE_SERVICE, name="HumanInLoop", version="1.0.0", author="Arbiter Team")(HumanInLoop)
...
if not arbiter_registry.get_metadata(PlugInKind.CORE_SERVICE, "human_in_loop"):
    register(kind=PlugInKind.CORE_SERVICE, name="human_in_loop", version="1.0.0", author="Arbiter Team")(get_human_approval)
