...
self_healing_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'self_healing_import_fixer'))
...
def reset_registry(tmp_path):
    ...
    root_plugins_json = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'plugins.json'))
...
class MockPlugin(PluginBase):
    ...
    async def get_capabilities(self): return ["test"]
...
async def test_health_check_all(reset_registry):
    ...
    reg.register_instance(kind, "UnhealthyPlugin", unhealthy_instance, version="1.0.0", author="Test")
...
def test_dependency_validation_missing(reset_registry):
    ...
    with pytest.raises(PluginDependencyError, match="not found"):
        @reg.register(kind=kind, name="NeedsDep", version="1.0.0", author="Test", dependencies=deps)
...
def test_dependency_version_conflict(reset_registry):
    ...
    with pytest.raises(PluginDependencyError, match="does not satisfy"):
        @reg.register(kind=kind, name="NeedsNewDep", version="1.0.0", author="Test", dependencies=deps)
...
async def test_reload(reset_registry):
    ...
    mock_module.__spec__ = ModuleSpec("test_module", mock_module.__loader__, origin="/fake/path/test_module.py")
    ...
    try:
        ...
        with patch('arbiter_plugin_registry.importlib.import_module', return_value=mock_module), \
