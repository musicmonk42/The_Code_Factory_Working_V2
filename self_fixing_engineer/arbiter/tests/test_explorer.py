...
async def test_run_ab_test_success(explorer):
    ...
    result = await explorer.run_ab_test("test_ab", variant_a="A", variant_b="B", num_runs=2, metric="perf")
...
async def test_run_evolutionary_experiment_success(explorer):
    ...
    explorer.sandbox_env.test_agent = AsyncMock(return_value=True)
    result = await explorer.run_evolutionary_experiment("test_evo", initial_agent="init", num_generations=2, population_size=2, metric="perf")
    assert result["status"] == "completed"
...
async def test_run_evolutionary_experiment_failure(explorer):
    ...
    with pytest.raises(ExperimentExecutionError):
        await explorer.run_evolutionary_experiment("test_fail", "init", num_generations=1, population_size=1, metric="perf")
...
async def test_concurrent_experiments(explorer):
    ...
    explorer.sandbox_env.test_agent = AsyncMock(return_value=True)
    tasks = [
        asyncio.create_task(explorer.run_ab_test(f"concurrent_{i}", "A", "B", num_runs=1, metric="perf"))
        for i in range(5)
    ]
