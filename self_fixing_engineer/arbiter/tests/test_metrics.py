...
@pytest.fixture
def mock_logger():
    with patch.object(_metrics_logger, 'info') as mock_info, \
...
@pytest.mark.parametrize("name, doc, labels", [
...
@pytest.mark.parametrize("name, doc, labels, buckets", [
...
@pytest.mark.parametrize("metric_type, kwargs", [
    (Counter, {}),
    (Gauge, {}),
...
def test_metric_registration_time(mock_time):
    ...
    with patch('arbiter.metrics.METRIC_REGISTRATION_TIME') as mock_metric:
        ...
        mock_metric.labels.assert_called_with(
            metric_name="arbiter_timed_counter",
            metric_type="Counter"
        )
