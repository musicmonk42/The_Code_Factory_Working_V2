...
class MessageQueueService:
    def __init__(self, backend_type='redis', settings=None):
        ...
        self.critical_events_count = Counter('mq_critical_events_total', 'Critical events', ['event_type'])
    ...
    async def subscribe(self, event_type, handler):
        ...
        if self.backend_type == 'redis':
            await self.redis_client.xgroup_create(f'{self.settings.MQ_TOPIC_PREFIX}_{event_type}', self.settings.MQ_CONSUMER_GROUP_ID)

    async def _consume_loop(self):
        while True:
            for event_type, handler in self.handlers.items():
                topic = f'{self.settings.MQ_TOPIC_PREFIX}_{event_type}'
                messages = await self.redis_client.xreadgroup(self.settings.MQ_CONSUMER_GROUP_ID, 'consumer', {topic: '>'}, count=1, block=0)
                for _, msgs in messages:
                    for msg_id, msg in msgs:
                        await self._process_message(msg, event_type)
                        await self.redis_client.xack(topic, self.settings.MQ_CONSUMER_GROUP_ID, msg_id)
    ...
    async def _process_message(self, message, event_type):
        try:
            ...
            self.message_latency.labels(event_type=event_type).observe(time.time() - metadata['timestamp'])
        except Exception as e:
            metadata['attempts'] += 1
...
async def test_process_message_success():
    ...
    message = {b'data': service._serialize_message({'payload': encrypted.decode(), 'metadata': {'attempts': 0, 'timestamp': time.time()}})}
...
async def test_process_message_failure():
    service = MessageQueueService(backend_type='redis', settings=MockSettings())
    async def h(m): raise Exception("fail")
    await service.subscribe('test', h)
    serialized = service._serialize_message({'data': {}})
    encrypted = service._encrypt_payload(serialized)
    message = {b'data': service._serialize_message({'payload': encrypted.decode(), 'metadata': {'attempts': 0, 'timestamp': time.time()}})}
...
async def test_poison_message():
    ...
    serialized = service._serialize_message({'data': {}})
    encrypted = service._encrypt_payload(serialized)
    message = {b'data': service._serialize_message({'payload': encrypted.decode(), 'metadata': {'attempts': 5, 'timestamp': time.time()}})}
...
async def test_replay_dlq():
    ...
    service.redis_client.xread.return_value = [(b'test_events_test_dlq', [(b'id', {b'data': encrypted})])]
