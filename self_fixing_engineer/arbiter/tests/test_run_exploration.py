...
from arbiter.run_exploration import (
    setup_logging, load_config, notify_critical_error, 
    load_plugins, run_agent_task, run_agentic_workflow, main
)
...
@patch('pkgutil.iter_modules', return_value=[(None, 'plugin1', True), (None, 'plugin2', True)])
...
async def test_run_agentic_workflow_success(mock_file_open, mock_makedirs, 
                                           mock_health_server, mock_arena_class, 
                                           mock_config):
...
@patch('arbiter.run_exploration.start_health_server')
@patch('os.makedirs')
async def test_run_agentic_workflow_with_errors(mock_makedirs, mock_health_server, 
                                               mock_arena_class, mock_config):
...
@patch('asyncio.Event')
async def test_main_no_args(mock_event, mock_workflow, mock_health, 
                           mock_setup):
