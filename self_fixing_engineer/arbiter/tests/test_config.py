...
@patch.dict(os.environ, {"ENCRYPTION_KEY": Fernet.generate_key().decode(), "DB_PATH": "sqlite:///./test.db"})
...
def test_to_dict(mock_access):
    ...
    if "EMAIL_SMTP_PASSWORD" in config_dict:
        assert config_dict["EMAIL_SMTP_PASSWORD"] == "[REDACTED]" or config_dict["EMAIL_SMTP_PASSWORD"] is None
...
def test_encrypt_sensitive_fields():
    ...
    decrypted = config._cipher.decrypt(config._sensitive_fields["EMAIL_SMTP_PASSWORD"].encode()).decode()
