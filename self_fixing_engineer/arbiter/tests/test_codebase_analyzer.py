...
def mock_metrics():
    with patch('codebase_analyzer.analyzer_ops_total') as mock_ops, \
...
@pytest.mark.parametrize("dep, flag", [
...
async def test_generate_report_markdown(temp_dir):
    analyzer = CodebaseAnalyzer(root_dir=str(temp_dir))
    analyzer.summary = {"defects": [{"file": "test.py", "line": 1, "column": 1, "message": "error", "source": "test"}]}
    ...
    with patch('aiofiles.open', return_value=mock_file):
        await analyzer.generate_report(output_format="markdown", output_path=str(temp_dir / "report.md"))
...
async def test_generate_report_json(temp_dir):
    ...
    with patch('aiofiles.open', return_value=mock_file):
        await analyzer.generate_report(output_format="json", output_path=str(temp_dir / "report.json"))
        mock_file.write.assert_called_once()
...
async def test_generate_report_junit(temp_dir):
    ...
    with patch('aiofiles.open', return_value=mock_file):
        await analyzer.generate_report(output_format="junit", output_path=str(temp_dir / "report.xml"))
        mock_file.write.assert_called_once()
...
def test_generate_junit_xml_report():
    analyzer = CodebaseAnalyzer(root_dir=".")
    summary = {"defects": [{"file": "test.py", "line": 1, "column": 1, "message": "error", "source": "test"}]}
...
def test_cli_tools():
    ...
    with patch('codebase_analyzer.CodebaseAnalyzer') as mock_analyzer_class:
        ...
        mock_instance.__aenter__ = AsyncMock(return_value=mock_instance)
        mock_instance.__aexit__ = AsyncMock()
        mock_instance.audit_repair_tools = AsyncMock(return_value=[
            {"name": "test", "type": "test", "available": True, "installed_via": "pip"}
        ])
        mock_analyzer_class.return_value = mock_instance
