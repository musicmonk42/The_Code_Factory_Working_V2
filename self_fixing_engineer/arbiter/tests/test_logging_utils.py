...
class TestPIIRedactorFilter(unittest.TestCase):
    ...
    def test_api_key_redaction(self):
        ...
        for text in texts:
            redacted = self.filter._redact_text(text)
            self.assertTrue(
                any(marker in redacted for marker in [
                    "[API_KEY_REDACTED]",
                    "[TOKEN_REDACTED]",
                    "[PASSWORD_REDACTED]",
                    "[SECRET_REDACTED]"
    ...
    def test_jwt_token_redaction(self):
        ...
        self.assertTrue(
            "[JWT_REDACTED]" in redacted or 
            "[TOKEN_REDACTED]" in redacted or
            "[AWS_SECRET_REDACTED]" in redacted
        )
    ...
    def test_file_path_redaction(self):
        ...
        for path in paths:
            ...
            self.assertTrue(
                "[USER_PATH_REDACTED]" in redacted or 
                "[WIN_PATH_REDACTED]" in redacted
            )
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_large_text_redaction(self):
        ...
        large_text = " ".join([
            f"User{i}@example.com has SSN {i:03d}-{i:02d}-{i:04d}"
            for i in range(100)
        ])
        ...
        self.assertLess(len(remaining_emails), original_count * 0.2, 
