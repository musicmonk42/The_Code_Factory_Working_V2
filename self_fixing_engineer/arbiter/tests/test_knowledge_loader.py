...
@pytest.mark.parametrize("orig, new, expected", [
    ({}, {"a": 1}, {"a": 1}),
    ({"a": 1}, {"b": 2}, {"a": 1, "b": 2}),
...
@patch('os.replace')
@patch('os.remove')
def test_save_knowledge_atomic_success(mock_remove, mock_replace, mock_dump, mock_fdopen, mock_mkstemp, mock_makedirs, mock_logger):
...
def test_load_knowledge_invalid_json(mock_json_load, mock_open, mock_logger):
    ...
    assert result is None
    error.assert_called_with("Malformed JSON in knowledge file invalid.json: Expecting value: line 1 column 1 (char 0). Returning None.")

# Test KnowledgeLoader initialization
@patch('os.walk')
@patch('knowledge_loader.load_knowledge', side_effect=lambda f: {os.path.basename(f): {"data": "test"}} if 'test' in f else None)
def test_knowledge_loader_init(mock_load, mock_walk, mock_logger):
    ...
    assert loader.loaded_knowledge == {'test1': {'data': 'test'}, 'test2': {'data': 'test'}}
...
async def test_load_and_aggregate(mock_load, mock_gather):
    ...
    assert loader.loaded_knowledge == {'file1': {'data': 'test'}, 'file2': {'data': 'test'}}
