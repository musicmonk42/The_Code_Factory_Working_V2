...
sys.modules['arbiter.arbiter_array_backend'].ConcreteArrayBackend = MagicMock(return_value=mock_array_backend)

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..')))

from arbiter.decision_optimizer import (
    DecisionOptimizer,
    Task,
    Agent,
    safe_serialize
...
async def test_allocate_resources_simple(optimizer):
    ...
    agents = [
        Agent(id="a1", skills={"python"}, max_compute=10.0, current_load=0.0)
    ]
    tasks = [
        Task(id="t1", priority=1.0, required_skills={"python"}, estimated_compute=2.0)
    ]
    
    with patch('arbiter.decision_optimizer.get_system_metrics_async', AsyncMock(return_value={"cpu_percent": 50})):
