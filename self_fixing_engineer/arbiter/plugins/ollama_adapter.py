...
class OllamaAdapter:
    ...
    def __init__(self, settings: Dict[str, Any]):
        ...
        if not model_name:
            self.logger.critical(
                "LLM_MODEL is missing from settings. Cannot initialize OllamaAdapter."
            )
    ...
    def _check_circuit_breaker(self):
        ...
        if self._circuit_breaker_state == "open":
            if (
                time.monotonic() - self._circuit_breaker_last_failure_time
    ...
    def _update_circuit_breaker(self, success: bool):
        ...
        if success:
            ...
            if self._circuit_breaker_state == "half-open":
                ...
                self.circuit_breaker_state_gauge.set(2)  # 2 for "open"
                self.logger.error(
                    f"Circuit breaker is now 'open' after {self._circuit_breaker_failures} failures."
                )
    ...
    async def generate(
        ...
        try:
            ...
            if isinstance(
                ...
                self.logger.error(
                    f"Ollama generation failed due to connection error or timeout: {original_exception} [Correlation ID: {correlation_id}]"
                )
                raise TimeoutError(
                    f"Ollama API call timed out or failed to connect: {original_exception}"
                ) from original_exception
            elif isinstance(original_exception, aiohttp.ClientResponseError):
                self.logger.error(
                    f"Ollama API status error: {original_exception.status} - {original_exception.message} [Correlation ID: {correlation_id}]"
                )
                if original_exception.status in [401, 403]:
                    # This is unlikely for a local Ollama but included for consistency
                    raise AuthError(
                        f"Ollama authentication error: {original_exception.status} - {original_exception.message}"
                    ) from original_exception
                elif original_exception.status == 429:
                    # This is unlikely for a local Ollama but included for consistency
                    raise RateLimitError(
                        f"Ollama rate limit exceeded: {original_exception.message}"
                    ) from original_exception
                else:
                    raise APIError(
                        f"Ollama API error (status {original_exception.status}): {original_exception.message}"
                    ) from original_exception
            else:
                self.logger.error(
                    f"Unexpected error during Ollama generation: {original_exception} [Correlation ID: {correlation_id}]",
                    exc_info=True,
                )
                raise APIError(
                    f"Unexpected Ollama API error: {original_exception}"
                ) from original_exception
        except Exception as e:
