...
class AnthropicAdapter:
    ...
    def __init__(self, settings: Dict[str, Any]):
        ...
        if not api_key:
            self.logger.critical("ANTHROPIC_API_KEY is missing from settings. Cannot initialize AnthropicAdapter.")
    ...
    def _update_circuit_breaker(self, success: bool):
        ...
        if success:
            ...
            if self.circuit_breaker_failures >= self.circuit_breaker_threshold:
                ...
                self.logger.warning(f"Circuit breaker opened due to {self.circuit_breaker_failures} failures.")
    ...
    def _sanitize_prompt(self, prompt: str) -> str:
        ...
        # Email addresses
        sanitized_prompt = re.sub(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', '[EMAIL]', sanitized_prompt)
        # Phone numbers (common formats)
        sanitized_prompt = re.sub(r'\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}', '[PHONE]', sanitized_prompt)
        # SSNs (simplified)
    ...
    async def generate(
        ...
        if self.circuit_breaker_state == "open":
            if time.time() - self.circuit_breaker_last_failure_time > self.circuit_breaker_timeout:
                ...
                self.logger.warning(f"Circuit breaker is open. Not attempting Anthropic API call. [Correlation ID: {correlation_id}]")
        ...
        try:
            ...
            success = True
            self.logger.info(f"Anthropic generation successful for correlation_id: {correlation_id}")
        ...
        # 2. Handle RetryError Explicitly
            ...
            if isinstance(original_exception, (anthropic.APITimeoutError, asyncio.TimeoutError)):
                self.logger.error(f"Anthropic generation timed out: {original_exception} [Correlation ID: {correlation_id}]")
                error_type = "timeout"
                self._update_circuit_breaker(success=False)
                raise TimeoutError(f"Anthropic API call timed out: {original_exception}") from original_exception
            elif isinstance(original_exception, anthropic.APIStatusError) or (hasattr(original_exception, 'status_code')):
                self.logger.error(f"Anthropic API status error: {original_exception.status_code} - {original_exception.message} [Correlation ID: {correlation_id}]")
                if original_exception.status_code in [401, 403]:
                    error_type = "authentication"
                    self._update_circuit_breaker(success=False)
                    raise AuthError(f"Anthropic authentication error: {original_exception.status_code} - {original_exception.message}") from original_exception
                elif original_exception.status_code == 429:
                    error_type = "rate_limit"
                    self._update_circuit_breaker(success=False)
                    raise RateLimitError(f"Anthropic rate limit exceeded: {original_exception.message}") from original_exception
                else:
                    error_type = "api_status_error"
                    self._update_circuit_breaker(success=False)
                    raise APIError(f"Anthropic API error (status {original_exception.status_code}): {original_exception.message}") from original_exception
            else:
                self.logger.error(f"Unexpected error during Anthropic generation: {original_exception} [Correlation ID: {correlation_id}]", exc_info=True)
                error_type = "unexpected_llm_client_error"
                self._update_circuit_breaker(success=False)
                raise APIError(f"Unexpected Anthropic API error: {original_exception}") from original_exception
        
        # 3. Validate LLMClient Dependency
        # Generic catch-all for any other unhandled exceptions
        except Exception as e:
            self.logger.critical(f"A critical, unhandled error occurred in AnthropicAdapter: {e} [Correlation ID: {correlation_id}]", exc_info=True)
