...
from .llm_client import LLMClient, LLMClientError, AuthError, RateLimitError, TimeoutError, APIError, CircuitBreakerOpenError, get_or_create_metric
...
class GeminiAdapter:
    ...
    def __init__(self, settings: Dict[str, Any]):
        ...
        if not api_key:
            self.logger.critical("GEMINI_API_KEY is missing from settings. Cannot initialize GeminiAdapter.")
    ...
    async def generate(
        ...
        if len(prompt) > 100000:  # Gemini's typical context limit
            raise ValueError(f"Prompt is too long. Max length is 100,000 characters, but got {len(prompt)}.")
        ...
        if self.circuit_breaker_state == "open":
            if time.monotonic() - self.circuit_breaker_last_failure_time > self.circuit_breaker_timeout:
                ...
                self.logger.error(f"Circuit breaker is 'open'. Blocking API call to prevent cascading failures. [Correlation ID: {correlation_id}]")
                gemini_call_errors_total.labels(
                    provider=self.provider, model=self.model, correlation_id=correlation_id, error_type="circuit_breaker"
                ).inc()
        ...
        try:
            ...
            if isinstance(original_exception, asyncio.TimeoutError):
                error_type = "timeout"
                self.logger.error(f"Gemini generation timed out: {original_exception} [Correlation ID: {correlation_id}]")
                self._update_circuit_breaker(success=False)
                raise TimeoutError(f"Gemini API call timed out: {original_exception}") from original_exception
                ...
                if status_code is not None:
                    if status_code in [401, 403]:
                        ...
                        self._update_circuit_breaker(success=False)
                        raise AuthError(f"Gemini authentication error: {status_code} - {str(original_exception)}") from original_exception
                    elif status_code == 429:
                        error_type = "rate_limit_exceeded"
                        self._update_circuit_breaker(success=False)
                        raise RateLimitError(f"Gemini rate limit exceeded: {str(original_exception)}") from original_exception
                    else:
                        error_type = f"api_error_{status_code}"
                        self._update_circuit_breaker(success=False)
                        raise APIError(f"Gemini API error (status {status_code}): {str(original_exception)}") from original_exception
                else:
                    # If no status code can be found, log the full error message.
                    error_type = "api_error_no_code"
                    self.logger.error(f"GoogleAPICallError without a status code. Message: {original_exception} [Correlation ID: {correlation_id}]", exc_info=True)
                    self._update_circuit_breaker(success=False)
                    raise APIError(f"Gemini API error: {str(original_exception)}") from original_exception
            else:
                # Catch any other unexpected LLMClient errors.
                error_type = "unexpected_llm_client_error"
                self.logger.error(
                    f"Unexpected error during Gemini generation: {original_exception} [Correlation ID: {correlation_id}]", exc_info=True
                )
                self._update_circuit_breaker(success=False)
                raise APIError(f"Unexpected Gemini API error: {original_exception}") from original_exception
        
        except Exception as e:
            # This is a critical, top-level catch for any unhandled exceptions.
            error_type = "critical_unhandled_error"
            self.logger.critical(
                f"A critical, unhandled error occurred in GeminiAdapter: {e} [Correlation ID: {correlation_id}]", exc_info=True
            )
            self._update_circuit_breaker(success=False)
            ...
            if error_type != "unknown":
                gemini_call_errors_total.labels(
                    provider=self.provider, model=self.model, correlation_id=correlation_id, error_type=error_type
                ).inc()
    ...
    def _update_circuit_breaker(self, success: bool):
        ...
        if success:
            if self.circuit_breaker_state == "half-open":
                ...
                self.logger.info("Circuit breaker is now 'closed' after a successful call in 'half-open' state.")
                self.circuit_breaker_state = "closed"
                self.circuit_breaker_failures = 0
            ...
            if self.circuit_breaker_failures >= self.circuit_breaker_threshold:
                ...
                self.logger.error("Circuit breaker is now 'open' due to too many consecutive failures.")