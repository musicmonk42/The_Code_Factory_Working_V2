...
class ProcessorConfig(BaseModel):
    ...
    default_provider: str = Field("default", description="The default provider to use for this modality.")
...
class SecurityConfig(BaseModel):
    ...
    sandbox_enabled: bool = Field(False, description="Enable a sandboxed environment for execution (if available).")
...
class AuditLogConfig(BaseModel):
    ...
    destination: str = Field("console", description="The destination for audit logs ('console', 'file', or 'kafka').")
        ...
        for modality, controls in v.items():
            for control_id in controls:
                if not control_id_pattern.match(control_id):
                    raise ValueError(f"Invalid compliance control ID for modality '{modality}': '{control_id}'.")
        return v
...
class MultiModalConfig(BaseModel):
    ...
    """
    image_processing: ProcessorConfig = Field(default_factory=ProcessorConfig, description="Configuration for image processing.")
    audio_processing: ProcessorConfig = Field(default_factory=ProcessorConfig, description="Configuration for audio processing.")
    video_processing: ProcessorConfig = Field(default_factory=ProcessorConfig, description="Configuration for video processing.")
    text_processing: ProcessorConfig = Field(default_factory=ProcessorConfig, description="Configuration for text processing.")
    security_config: SecurityConfig = Field(default_factory=SecurityConfig, description="Global security and validation configuration.")
    audit_log_config: AuditLogConfig = Field(default_factory=AuditLogConfig, description="Global audit logging configuration.")
    metrics_config: MetricsConfig = Field(default_factory=MetricsConfig, description="Global metrics configuration.")
    cache_config: CacheConfig = Field(default_factory=CacheConfig, description="Global caching configuration.")
    compliance_config: ComplianceConfig = Field(default_factory=ComplianceConfig, description="Global compliance mapping configuration.")
    circuit_breaker_config: CircuitBreakerConfig = Field(default_factory=CircuitBreakerConfig, description="Global circuit breaker configuration.")
    user_id_for_auditing: str = Field("system_user", description="The user ID to use for audit logs when one is not provided.")
    ...
    def load_config(cls, config_file: str = None) -> 'MultiModalConfig':
        ...
        env_vars = {
            'MULTI_MODAL_IMAGE_PROCESSING_ENABLED': ('image_processing', 'enabled', bool),
            'MULTI_MODAL_AUDIO_PROCESSING_ENABLED': ('audio_processing', 'enabled', bool),
            'MULTI_MODAL_VIDEO_PROCESSING_ENABLED': ('video_processing', 'enabled', bool),
            'MULTI_MODAL_TEXT_PROCESSING_ENABLED': ('text_processing', 'enabled', bool),
            'MULTI_MODAL_SECURITY_MASK_PII': ('security_config', 'mask_pii_in_logs', bool),
            'MULTI_MODAL_SECURITY_SANDBOX_ENABLED': ('security_config', 'sandbox_enabled', bool),
            'MULTI_MODAL_AUDIT_LOG_CONFIG_ENABLED': ('audit_log_config', 'enabled', bool),
            'MULTI_MODAL_AUDIT_LOG_CONFIG_LOG_LEVEL': ('audit_log_config', 'log_level', str),
            'MULTI_MODAL_AUDIT_LOG_CONFIG_DESTINATION': ('audit_log_config', 'destination', str),
            'MULTI_MODAL_METRICS_CONFIG_ENABLED': ('metrics_config', 'enabled', bool),
            'MULTI_MODAL_METRICS_CONFIG_EXPORTER_PORT': ('metrics_config', 'exporter_port', int),
            ...
            'MULTI_MODAL_CACHE_CONFIG_TTL_SECONDS': ('cache_config', 'ttl_seconds', int),
            'MULTI_MODAL_CIRCUIT_BREAKER_CONFIG_ENABLED': ('circuit_breaker_config', 'enabled', bool),
            'MULTI_MODAL_CIRCUIT_BREAKER_CONFIG_THRESHOLD': ('circuit_breaker_config', 'threshold', int),
            'MULTI_MODAL_CIRCUIT_BREAKER_CONFIG_TIMEOUT_SECONDS': ('circuit_breaker_config', 'timeout_seconds', int),
        }
        ...
        if modalities_env:
            ...
            config_data['circuit_breaker_config']['modalities'] = [m.strip() for m in modalities_env.split(',')]
        ...
        for env_var, (section, key, var_type) in env_vars.items():
            ...
            if value is not None:
                ...
                try:
                    ...
                    logging.warning(f"Could not convert environment variable {env_var} value '{value}' to type {var_type.__name__}. Skipping.")
