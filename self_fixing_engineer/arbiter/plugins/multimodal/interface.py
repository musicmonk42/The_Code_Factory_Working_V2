...
try:
    ...
    raise ImportError("Pydantic and prometheus_client are required. Please install them with 'pip install pydantic prometheus_client'.")
...
class ProcessingResult(BaseModel, Generic[T]):
    ...
    data: Optional[T] = Field(None, description="The processed data or results, type-hinted by Generic[T].")
    summary: Optional[str] = Field(None, description="A human-readable summary of the processing result.")
    operation_id: str = Field(..., description="A unique identifier for this specific processing operation.")
    model_confidence: Optional[float] = Field(None, ge=0.0, le=1.0, description="Confidence score of the model's output (0.0 to 1.0), if applicable.")
...
class MultiModalAnalysisResult(BaseModel, Generic[T], ABC):
    ...
    """
    raw_data: T = Field(..., description="The raw result data (e.g., prediction, embedding, transcript). Type-hinted by Generic[T].")
    meta: Dict[str, Any] = Field(default_factory=dict, description="Standardized metadata dictionary (model, timestamp, confidence, provenance, etc).")
    result_type: AnalysisResultType = Field(..., description="The specific type of analysis result (e.g., 'image', 'audio', 'text').")
    success: bool = Field(True, description="Indicates if the analysis was successful.")
    error_message: Optional[str] = Field(None, description="Detailed error message if analysis failed.")
    confidence: Optional[float] = Field(None, ge=0.0, le=1.0, description="Overall confidence score of the analysis result, if applicable.")
    model_id: Optional[str] = Field(None, description="Identifier of the model used for analysis.")
    timestamp_utc: datetime.datetime = Field(default_factory=datetime.datetime.utcnow, description="UTC timestamp of when the analysis was performed.")
    # Add fields for provenance, audit trail, PII redaction status, etc.
    data_provenance: Optional[Dict[str, Any]] = Field(None, description="Information about the source data (e.g., file hash, origin URL, capture device).")
    audit_id: Optional[str] = Field(None, description="Unique identifier for the audit trail entry associated with this analysis.")


    class Config:
        """Pydantic configuration for the base model."""
        arbitrary_types_allowed = True # Allow 'raw_data' to be flexible (e.g., numpy arrays)
...
class ImageAnalysisResult(MultiModalAnalysisResult[Union[Dict[str, Any], List[Dict[str, Any]]]]):
    """
    ...
    classifications: Optional[List[Dict[str, Union[str, float]]]] = Field(None, description="List of detected classes and their probabilities.")
    objects: Optional[List[Dict[str, Any]]] = Field(None, description="List of detected objects with bounding boxes, labels, and confidence.")
    ocr_text: Optional[str] = Field(None, description="Extracted text from OCR.")
    embedding: Optional[List[float]] = Field(None, description="Vector embedding of the image.")
    segmentation_masks: Optional[Any] = Field(None, description="Raw segmentation mask data (e.g., binary masks or RLE).")

    def summary(self) -> str:
        description_parts = []
        if self.classifications:
            top_class = sorted(self.classifications, key=lambda x: x.get('score', 0), reverse=True)
            if top_class:
                description_parts.append(f"Class: {top_class[0].get('label')} ({top_class[0].get('score', 0):.2f})")
        if self.objects:
        ...
        base_summary = self.meta.get('description', '') # Still allow for an optional descriptive meta tag
        if not description_parts:
            description_parts.append("No specific analysis results.")
        
        # Consolidate description and add confidence if present
        final_summary = f"Image analysis: {base_summary}. " if base_summary else "Image analysis: "
...
class AudioAnalysisResult(MultiModalAnalysisResult[Union[str, Dict[str, Any]]]):
    ...
    speakers: Optional[List[Dict[str, Union[str, float]]]] = Field(None, description="List of identified speakers and their segments.")
    sentiment: Optional[Dict[str, float]] = Field(None, description="Sentiment analysis scores (e.g., {'positive': 0.8, 'negative': 0.1}).")
    audio_events: Optional[List[Dict[str, Union[str, float]]]] = Field(None, description="Detected audio events (e.g., music, siren, speech) and their confidence.")
    ...
    def summary(self) -> str:
        ...
        if self.sentiment:
            ...
            description_parts.append(f"Sentiment: {top_sentiment} ({self.sentiment[top_sentiment]:.2f})")
        ...
        final_summary = f"Audio analysis: {base_summary}. " if base_summary else "Audio analysis: "
        final_summary += ' '.join(description_parts)
        if self.confidence is not None:
            final_summary += f". Confidence: {self.confidence:.2f}"
        return final_summary.strip()
...
class VideoAnalysisResult(MultiModalAnalysisResult[Union[Dict[str, Any], List[Dict[str, Any]]]]):
    """
    ...
    scene_changes: Optional[List[float]] = Field(None, description="Timestamps of detected scene changes.")
    tracked_objects: Optional[List[Dict[str, Any]]] = Field(None, description="List of objects tracked across frames with their trajectories.")
    actions: Optional[List[Dict[str, Union[str, float, Tuple[float, float]]]]] = Field(None, description="Detected actions with timestamps/durations and confidence.")
    summary_transcript: Optional[str] = Field(None, description="Summarized transcript from video's audio.")
    key_frames_analysis: Optional[List[ImageAnalysisResult]] = Field(None, description="Analysis results for key frames within the video.")
    overall_sentiment: Optional[Dict[str, float]] = Field(None, description="Overall sentiment derived from video content (visual + audio).")

    def summary(self) -> str:
        description_parts = []
        if self.summary_transcript:
            description_parts.append(f"Summarized Audio: {self.summary_transcript[:50]}...")
        ...
        final_summary = f"Video analysis: {base_summary}. " if base_summary else "Video analysis: "
        final_summary += ' '.join(description_parts)
        if self.confidence is not None:
            final_summary += f". Confidence: {self.confidence:.2f}"
        return final_summary.strip()
...
class TextAnalysisResult(MultiModalAnalysisResult[str]):
    ...
    classification: Optional[List[Dict[str, Union[str, float]]]] = Field(None, description="Text classification results (e.g., categories, topics).")
    sentiment: Optional[Dict[str, float]] = Field(None, description="Sentiment analysis scores for the text.")
    entities: Optional[List[Dict[str, Union[str, str]]]] = Field(None, description="Extracted named entities (e.g., people, organizations, locations).")
    summary_text: Optional[str] = Field(None, description="A summarized version of the input text.")
    translation: Optional[str] = Field(None, description="Translated text, if translation was performed.")
    language: Optional[str] = Field(None, description="Detected language of the text.")

    def summary(self) -> str:
        description_parts = []
        if self.classification:
            top_class = sorted(self.classification, key=lambda x: x.get('score', 0), reverse=True)
            if top_class:
        ...
        final_summary = f"Text analysis: {base_summary}. " if base_summary else "Text analysis: "
        final_summary += ' '.join(description_parts)
        if self.confidence is not None:
            final_summary += f". Confidence: {self.confidence:.2f}"
        return final_summary.strip()
...
class MultiModalPluginInterface(ABC):
    ...
    async def analyze_image_async(self, image_data: Union[bytes, str, Any], **kwargs) -> ImageAnalysisResult:
        ...
        """
        raise NotImplementedError("Asynchronous image analysis not implemented by this plugin.")
    ...
    async def analyze_audio_async(self, audio_data: Union[bytes, str, Any], **kwargs) -> AudioAnalysisResult:
        ...
        """
        raise NotImplementedError("Asynchronous audio analysis not implemented by this plugin.")

    async def analyze_video_async(self, video_data: Union[bytes, str, Any], **kwargs) -> VideoAnalysisResult:
        """
        ...
        """
        raise NotImplementedError("Asynchronous video analysis not implemented by this plugin.")
    ...
    async def analyze_text_async(self, text_data: str, **kwargs) -> TextAnalysisResult:
        ...
        """
        raise NotImplementedError("Asynchronous text analysis not implemented by this plugin.")
...
class DummyMultiModalPlugin(MultiModalPluginInterface):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.dummy_model_id = config.get("model_id", "dummy-v1.0") if config else "dummy-v1.0"
        self.call_count = 0 # For demonstrating resource management/tracking
        self.requests_total = get_or_create(
            Counter('dummy_plugin_requests_total', 'Total dummy plugin requests', ['modality', 'status'])
        )
        self.processing_latency_seconds = get_or_create(
            Histogram('dummy_plugin_processing_latency_seconds', 'Dummy plugin processing latency', ['modality'])
        )

    def analyze_image(self, image_data: Union[bytes, str, Any], **kwargs) -> ImageAnalysisResult:
        start_time = time.monotonic()
        try:
            if image_data is None or (isinstance(image_data, str) and not os.path.exists(image_data)):
                raise ValueError("Invalid image_data: must be non-empty bytes or valid file path")
            ...
            result = ImageAnalysisResult(
                raw_data={"processed_bytes": len(image_data) if isinstance(image_data, bytes) else "N/A"},
                meta={"custom_meta": kwargs.get('context', 'no_context'), "description": kwargs.get('description', 'Dummy image analysis')},
                result_type=AnalysisResultType.IMAGE,
                success=True,
                confidence=0.75,
                model_id=self.dummy_model_id,
                classifications=[{"label": "dummy_category", "score": 0.9}],
                objects=[{"box": [10, 20, 30, 40], "label": "dummy_object", "confidence": 0.8}],
                ocr_text="dummy text from image",
                data_provenance={"input_type": type(image_data).__name__, "size": len(image_data) if isinstance(image_data, bytes) else "N/A"},
            )
    ...
    def analyze_audio(self, audio_data: Union[bytes, str, Any], **kwargs) -> AudioAnalysisResult:
        start_time = time.monotonic()
        try:
            if audio_data is None or (isinstance(audio_data, str) and not os.path.exists(audio_data)):
                raise ValueError("Invalid audio_data: must be non-empty bytes or valid file path")
            ...
            result = AudioAnalysisResult(
                raw_data={"processed_samples": len(audio_data) if isinstance(audio_data, bytes) else "N/A"},
                meta={"custom_meta": kwargs.get('context', 'no_context'), "description": kwargs.get('description', 'Dummy audio analysis')},
                ...
                data_provenance={"input_type": type(audio_data).__name__, "size": len(audio_data) if isinstance(audio_data, bytes) else "N/A"},
            )
    ...
    def analyze_video(self, video_data: Union[bytes, str, Any], **kwargs) -> VideoAnalysisResult:
        start_time = time.monotonic()
        try:
            if video_data is None or (isinstance(video_data, str) and not os.path.exists(video_data)):
                raise ValueError("Invalid video_data: must be non-empty bytes or valid file path")
            ...
            result = VideoAnalysisResult(
                ...
                meta={"custom_meta": kwargs.get('context', 'no_context'), "description": kwargs.get('description', 'Dummy video analysis')},
                ...
                tracked_objects=[{"id": 1, "label": "person", "trajectory": [[0,0],[1,1]]}],
                actions=[{"action": "running", "confidence": 0.9, "timestamp": (1.0, 2.0)}],
                key_frames_analysis=[],
                overall_sentiment={"positive": 0.7, "negative": 0.1},
                data_provenance={"input_type": type(video_data).__name__, "size": len(video_data) if isinstance(video_data, bytes) else "N/A"},
            )
    ...
    def analyze_text(self, text_data: str, **kwargs) -> TextAnalysisResult:
        ...
        try:
            ...
            result = TextAnalysisResult(
                raw_data=text_data,
                meta={"custom_meta": kwargs.get('context', 'no_context'), "description": kwargs.get('description', 'Dummy text analysis')},
                ...
                language="en",
                data_provenance={"input_type": type(text_data).__name__, "length": len(text_data)},
            )
    ...
    def shutdown(self) -> None:
        ...
        """
        print(f"DummyMultiModalPlugin (ID: {self.dummy_model_id}) shutting down after {self.call_count} calls.")

    async def shutdown_async(self) -> None:
        """
        Dummy async shutdown for demonstration.
        """
        print(f"DummyMultiModalPlugin (ID: {self.dummy_model_id}) async shutting down after {self.call_count} calls.")
...
if __name__ == "__main__":
    ...
    with DummyMultiModalPlugin(config={"model_id": "dummy-test-v1.1"}) as plugin:
        # Test Image
        img_result = plugin.analyze_image(b"fake_image_bytes", description="family photo", context="from_user_upload")
        ...
        print(f"Image Provenance: {json.dumps(img_result.get_provenance_info(), indent=2)}")
        print(f"Image repr: {repr(img_result)}") # Test __repr__

        # Test Audio
        audio_result = plugin.analyze_audio(b"fake_audio_bytes", description="meeting recording")
        ...
        # Test Video
        video_result = plugin.analyze_video(b"fake_video_bytes", description="security footage")
        ...
        # Test Text
        text_result = plugin.analyze_text("The quick brown fox jumps over the lazy dog.", description="user query")
    ...
    try:
        ...
        print("Configuration validation test conceptual. See `TestConfig(threshold=1.5)` example.")
    ...
    print(f"Error Result Provenance: {json.dumps(error_simulated_result.get_provenance_info(), indent=2)}")
    ...
    try:
        ...
        print("Direct instantiation of MultiModalAnalysisResult (abstract base class) is prevented at runtime.")
        print("Uncomment `MultiModalAnalysisResult(raw_data=\"test\", result_type=AnalysisResultType.GENERIC)` to see TypeError.")
