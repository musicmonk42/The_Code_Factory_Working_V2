...
class LLMClient:
    ...
    def __init__(self, provider: str, api_key: Optional[str], model: str, base_url: Optional[str] = None,
                 timeout: int = 60, retry_attempts: int = 3, retry_backoff_factor: float = 2.0):
        """
        ...
        if not self.model:
            if self.provider == "openai":
                ...
                logger.warning(f"Model not specified for provider {provider} and no default is known. This may lead to errors.")
        ...
        if provider == "openai":
            ...
            try:
                ...
                logger.error(f"Failed to initialize Gemini GenerativeModel({self.model}): {e}. Check API key and model availability.")
                self.client = None 
        ...
        with self._circuit_breaker_lock:
            if success:
                ...
                if self.circuit_breaker_failures >= self.circuit_breaker_threshold:
                    ...
                    logger.warning(f"Circuit breaker for {self.provider}/{self.model} is now OPEN after {self.circuit_breaker_failures} consecutive failures.")
        ...
        with self._circuit_breaker_lock:
            if self.circuit_breaker_state == "open":
                ...
                if elapsed_time > self.circuit_breaker_timeout:
                    ...
                    logger.info(f"Circuit breaker for {self.provider}/{self.model} is now HALF-OPEN. Next request will be a probe.")
                else:
                    raise CircuitBreakerOpenError(f"Circuit breaker for {self.provider}/{self.model} is OPEN. Not attempting call.")
    ...
    async def _handle_llm_call(self, coro_producer: Callable[[], Awaitable], prompt: str, is_streaming: bool, correlation_id: Optional[str] = None) -> Union[str, AsyncGenerator[str, None]]:
        """
        ...
        async def _call_with_retry():
            with tracer.start_as_current_span(f"llm_api_call_{self.provider}") as span:
                ...
                span.set_attribute("llm.call_type", "streaming" if is_streaming else "non_streaming")
                ...
                try:
                    ...
                    LLM_CALL_SUCCESS.labels(provider=self.provider, model=self.model, correlation_id=correlation_id).inc()
                    LLM_CALL_LATENCY.labels(provider=self.provider, model=self.model, correlation_id=correlation_id).observe(time.monotonic() - start_time)
                    
                    logger.info(f"LLM call {self.provider}/{self.model} successful for prompt hash: {hashlib.sha256(prompt.encode()).hexdigest()[:8]}... [{correlation_id}]")
                    return result
                except Exception as e:
                    error_type = type(e).__name__
                    status_code = getattr(e, "status_code", getattr(e, "code", getattr(e, "status", None))) 
                    ...
                    if isinstance(e, openai.BadRequestError) or \
                       (isinstance(e, anthropic.APIStatusError) and status_code and 400 <= status_code < 500 and status_code != 429) or \
                       (isinstance(e, google_exceptions.InvalidArgument)) or \
                       (isinstance(e, aiohttp.ClientResponseError) and status_code and 400 <= status_code < 500 and status_code != 429):
                        is_non_retryable = True
                    
                    if is_non_retryable:
                        self._update_circuit_breaker(success=False)
                        LLM_CALL_ERRORS.labels(provider=self.provider, model=self.model, error_type=f"ClientError_{status_code if status_code else 'UNKNOWN'}", correlation_id=correlation_id).inc()
                        span.record_exception(e)
                        span.set_status(trace.Status(trace.StatusCode.ERROR, f"Non-Retryable API Error: {str(e)}"))
                        logger.error(f"{log_message} [Non-Retryable] [{correlation_id}]", exc_info=True)
                        raise # Re-raise immediately, tenacity won't retry this
                    else: 
                        LLM_CALL_ERRORS.labels(provider=self.provider, model=self.model, error_type=error_type, correlation_id=correlation_id).inc()
                        span.record_exception(e)
                        span.set_status(trace.Status(trace.StatusCode.ERROR, f"Transient Error: {str(e)}"))
                        logger.warning(f"{log_message} [Transient, Retrying] [{correlation_id}]", exc_info=True)
        ...
        try:
            ...
            if isinstance(final_e, openai.AuthenticationError) or (isinstance(final_e, anthropic.APIStatusError) and getattr(final_e, 'status_code', None) in [401, 403]):
                raise AuthError(final_log_message) from final_e
            elif isinstance(final_e, openai.RateLimitError) or (isinstance(final_e, anthropic.APIStatusError) and getattr(final_e, 'status_code', None) == 429):
            ...
            elif isinstance(final_e, (openai.APIStatusError, google_exceptions.GoogleAPICallError, aiohttp.ClientResponseError)):
    ...
    def _generate_prompt(self, messages: List[Union[Dict[str, str], Any]]) -> str: # Use Any for BaseMessage types
        """
    ...
    async def generate_text(self, prompt: str, max_tokens: int = 1000, temperature: float = 0.7, correlation_id: Optional[str] = None) -> str:
        """
        ...
        if not prompt or not isinstance(prompt, str) or len(prompt) > 100000:
            raise InputValidationError("Prompt must be a non-empty string and under 100,000 characters.")
    ...
    async def _generate_core(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float) -> str:
        ...
        if self.provider == "openai":
            response = await self.client.chat.completions.create(
                model=self.model, messages=messages, max_tokens=max_tokens, temperature=temperature, timeout=self.timeout
            )
            ...
            if not self.client: raise RuntimeError("Gemini client not initialized.")
            generation_config = {"temperature": temperature}
            if max_tokens: generation_config["max_output_tokens"] = max_tokens
            response = await self.client.generate_content(
                messages, generation_config=generation_config, request_options={'timeout': self.timeout}
            )
            result = response.text
        elif self.provider == "ollama":
            session = await self._get_ollama_session()
            ollama_payload = {
                "model": self.model, "prompt": self._generate_prompt(messages),
                "options": {"temperature": temperature, "num_predict": max_tokens}
            }
            async with session.post(f"{self.base_url}/api/generate", json=ollama_payload, timeout=self.timeout) as resp:
    ...
    async def async_stream_text(self, prompt: str, max_tokens: int = 1000, temperature: float = 0.7, correlation_id: Optional[str] = None) -> AsyncGenerator[str, None]:
        """
        ...
        return await self._handle_llm_call(_stream_coro_producer, prompt, is_streaming=True, correlation_id=correlation_id)

    async def _stream_core(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float) -> AsyncGenerator[str, None]:
        """Core logic for streaming generation, separated for _handle_llm_call."""
        if self.provider == "openai":
            stream = await self.client.chat.completions.create(
                model=self.model, messages=messages, max_tokens=max_tokens, temperature=temperature, stream=True, timeout=self.timeout
            )
            ...
            try:
                stream = await self.client.generate_content(
                    messages, generation_config=generation_config, stream=True, request_options={'timeout': self.timeout}
                )
                async for chunk in stream:
                    yield chunk.text
            except (google_exceptions.FailedPrecondition, google_exceptions.InvalidArgument) as e:
                logger.error(f"Gemini streaming not supported for model {self.model} or invalid request: {e}", exc_info=True)
                raise RuntimeError(f"Gemini streaming not supported or failed for {self.model}: {e}") 

        elif self.provider == "ollama":
            session = await self._get_ollama_session()
            ollama_payload = {
                "model": self.model, "prompt": self._generate_prompt(messages),
                "options": {"temperature": temperature, "num_predict": max_tokens}, "stream": True
            }
            async with session.post(f"{self.base_url}/api/generate", json=ollama_payload, timeout=self.timeout) as resp:
                resp.raise_for_status()
                async for line in resp.content:
...
class LoadBalancedLLMClient:
    ...
    def __init__(self, providers_config: List[Dict[str, Any]]):
        ...
        for i, config in enumerate(providers_config):
            ...
            if not provider_name or not model:
                logger.error(f"Invalid provider config at index {i}: 'provider' and 'model' are required. Skipping.")
                continue
            ...
            try:
                ...
                logger.error(f"Failed to initialize LLMClient for {provider_name}/{model}: {e}. Marking as unavailable.", exc_info=True)
                self.provider_status[provider_name] = {
                    "status": "unavailable", 
        ...
        if not self.providers:
            raise ValueError("No LLM providers successfully initialized. Cannot create LoadBalancedLLMClient.")
        ...
        for p_client in self.active_providers:
            ...
            self._weighted_providers_list.extend([p_client] * int(weight * 10)) # Scale weights to integers for repetition
    ...
    def _select_provider(self) -> LLMClient:
        ...
        while attempt_count < max_selection_attempts:
            ...
            if status_info["status"] == "unavailable": 
                if status_info["last_error_time"] and (time.monotonic() - status_info["last_error_time"] > self.QUARANTINE_DURATION_SECONDS):
            ...
            if status_info["status"] == "degraded": 
                if status_info["last_selection_penalty_time"] and (time.monotonic() - status_info["last_selection_penalty_time"] < self.RETRYABLE_FAILURE_PENALTY_TIME):
                    attempt_count += 1
        ...
        logger.warning("All LLM providers are currently unhealthy or in backoff. Choosing next available provider (may fail).")
    ...
    def _update_provider_status(self, provider_name: str, success: bool, is_retryable_error: bool = False):
        """
        ...
        if success:
            ...
            if is_retryable_error:
                ...
                status_info["consecutive_failures"] = 0 # Reset consecutive failures for retryable errors
            else: 
                status_info["consecutive_failures"] += 1
                if status_info["consecutive_failures"] >= self.FAILURE_QUARANTINE_THRESHOLD:
                    status_info["status"] = "unavailable"
                    logger.error(f"Provider {provider_name} quarantined after {status_info['consecutive_failures']} consecutive non-retryable failures.")
                else:
                    status_info["status"] = "degraded"
                status_info["last_selection_penalty_time"] = time.monotonic() 
    ...
    async def generate_text(self, prompt: str, max_tokens: int = 1000, temperature: float = 0.7, correlation_id: Optional[str] = None) -> str:
        ...
        max_attempts = sum(p.retry_attempts + 1 for p in self.providers if self.provider_status[p.provider]["status"] != "unavailable") or 1
        if max_attempts == 0 and self.providers: # If all are unavailable, still try once per provider
        ...
        while current_attempt < max_attempts:
            ...
            try:
                ...
                is_retryable_error = isinstance(e, (RateLimitError, TimeoutError)) # These are retryable by load balancer
                self._update_provider_status(selected_provider.provider, success=False, is_retryable_error=is_retryable_error)
                
                logger.warning(f"Load Balancer: Provider {selected_provider.provider} failed after its internal retries (Error: {type(e).__name__}). Attempting fallback. [{correlation_id}]")
                ...
                if current_attempt >= max_attempts or not self.active_providers:
                    LLM_PROVIDER_FAILOVERS_TOTAL.labels(failed_provider=selected_provider.provider, fallback_provider="none").inc()
                    raise LLMClientError(f"All configured LLM providers failed to generate text after all retries and fallbacks. Last error: {e} [{correlation_id}]") from e
                else:
                    # Log failover and continue to next iteration to try another provider
                    next_provider_candidate = self._select_provider() # Get the next provider to log for failover metric
                    LLM_PROVIDER_FAILOVERS_TOTAL.labels(failed_provider=selected_provider.provider, fallback_provider=next_provider_candidate.provider).inc()
        
        # This part should ideally not be reached if max_attempts logic is sound, but as a safeguard
        raise LLMClientError("Load balancing logic exhausted all active providers without success (unexpected path).")
    ...
    async def async_stream_text(self, prompt: str, max_tokens: int = 1000, temperature: float = 0.7, correlation_id: Optional[str] = None) -> AsyncGenerator[str, None]:
        ...
        while current_attempt < max_attempts:
            ...
            try:
                ...
                stream_gen = await selected_provider._handle_llm_call(_stream_coro_producer, prompt, is_streaming=True, correlation_id=correlation_id)
                ...
                logger.warning(f"Load Balancer: Streaming provider {selected_provider.provider} failed after internal retries (Error: {type(e).__name__}). Attempting fallback. [{correlation_id}]")
                
                if current_attempt >= max_attempts or not self.active_providers:
                    LLM_PROVIDER_FAILOVERS_TOTAL.labels(failed_provider=selected_provider.provider, fallback_provider="none").inc()
                    raise LLMClientError(f"All configured LLM providers failed to stream text after all retries and fallbacks. Last error: {e} [{correlation_id}]") from e
                else:
                    next_provider_candidate = self._select_provider()
                    LLM_PROVIDER_FAILOVERS_TOTAL.labels(failed_provider=selected_provider.provider, fallback_provider=next_provider_candidate.provider).inc()
        
        raise LLMClientError("Load balancing logic exhausted all active providers for streaming without success (unexpected path).")
...
async def main():
    ...
    for provider, status in lb_client.provider_status.items():
        print(f"  {provider}: {status['status']} (Errors: {status['error_count']}, Consecutive: {status['consecutive_failures']})")
    ...
    try:
        response_ns = await lb_client.generate_text(prompt_ns, max_tokens=50, correlation_id=corr_id_ns)
    ...
    try:
        ...
        async for chunk in lb_client.async_stream_text(prompt_s, max_tokens=100, correlation_id=corr_id_s):
    ...
    if lb_client.active_providers:
        ...
        async def mock_fail_generate_non_retryable(*args, **kwargs):
            raise openai.BadRequestError("Simulated bad request (non-retryable)", response=None, body=None, status_code=400)
        async def mock_fail_stream_non_retryable(*args, **kwargs):
            raise anthropic.APIStatusError("Simulated streaming client error (non-retryable)", response=None, body=None, status_code=400)
        ...
        try:
            ...
            print(f"Fallback Non-Streaming ultimately failed as expected after quarantining primary ({corr_id_ns}): {e}")
        ...
        try:
            ...
            async for chunk in lb_client.async_stream_text(prompt_stream_fail_s, max_tokens=50, correlation_id=corr_id_s):
