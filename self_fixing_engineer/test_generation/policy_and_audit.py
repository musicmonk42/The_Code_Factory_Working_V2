...
class Constants:
    DEFAULT_POLICIES = {
        "generation_rules": {
            ...
            "allowed_languages": ["python", "javascript", "java", "typescript", "rust", "go"],
        ...
        "integration_rules": {
            ...
            "human_review_required_languages": ["javascript", "java", "typescript", "rust", "go"],
    ...
    SENSITIVE_KEYS = {
        "password", "api_key", "token", "secret", "credentials", "jwt", "auth",
    }
...
class Configuration:
    ...
    def from_env(cls, project_root: Optional[str] = None):
        ...
        return cls(
            ...
            critical_events_for_mq=os.getenv("CRITICAL_EVENTS_FOR_MQ", "").split(",") if os.getenv("CRITICAL_EVENTS_FOR_MQ") else [],
            webhook_hooks=json.loads(os.getenv("WEBHOOK_HOOKS", "{}")),
            slack_webhook_url=os.getenv("SLACK_WEBHOOK_URL"),
            slack_events=os.getenv("SLACK_EVENTS", "").split(",") if os.getenv("SLACK_EVENTS") else [],
            webhook_events=os.getenv("WEBHOOK_EVENTS", "").split(",") if os.getenv("WEBHOOK_EVENTS") else [],
            allowed_notification_hosts=set(os.getenv("ALLOWED_NOTIFICATION_HOSTS", "").split(",")) if os.getenv("ALLOWED_NOTIFICATION_HOSTS") else None
        )
...
if os.getenv("AUDIT_ENABLED", "true").lower() == "false":
    ...
    if not AUDIT_LOGGER_AVAILABLE:
        warnings.warn("test_generation.orchestrator.audit is not installed. Running with fallback logger.", RuntimeWarning)
...
def redact_sensitive(obj: Any) -> Any:
    ...
    url_param_regex = re.compile(
        r'(?i)(?:api_key|token|secret|password)=[^&"\'\s]+'
    )
    redacted_items = 0
        ...
        if isinstance(item, dict):
            ...
            for k, v in item.items():
                if isinstance(v, str) and re.search(url_param_regex, v):
                    ...
                    redacted_items += 1
                elif k.lower() in Constants.SENSITIVE_KEYS or "pass" in k.lower() or (isinstance(v, str) and re.match(api_key_regex, v)):
                    new_dict[k] = "[REDACTED]"
                    redacted_items += 1
                else:
...
try:
    ...
    logger.warning(f"Warning: An error occurred during prometheus_client version check: {e}. Metrics will be disabled.")
...
class MetricsClient:
    ...
    def __init__(self):
        ...
        if self.enabled:
            self.policy_evaluations_total = prometheus_client.Counter("atco_policy_evaluations_total", "Total policy evaluations", ["result", "rule"])
            self.notification_failures_total = prometheus_client.Counter("atco_notification_failures_total", "Failed notifications", ["service", "event_name"])
        else:
...
try:
    ...
    try:
        ...
        logger.warning(f"Warning: An error occurred during tenacity version check: {e}. Retries disabled.")
except ImportError:
...
class LocalFileSystem(FileSystem):
    ...
    def generate_file_hash(self, path: str, project_root: str) -> str:
        try:
            ...
            logger.warning("Could not import generate_file_hash from utils.py. Using local placeholder.")
            return "NO_HASH"
    ...
    async def cleanup_temp_dir(self, path: str) -> None:
        try:
            ...
            logger.warning("Could not import cleanup_temp_dir from utils.py. Using local placeholder.")
...
class PolicyClient(ABC):
    ...
    @abstractmethod
    async def evaluate_policy(self, policy_path: str, input_data: Dict[str, Any]) -> Tuple[bool, str]:
...
class OPAPolicyClient(PolicyClient):
    ...
    async def evaluate_policy(self, policy_path: str, input_data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not opa_server_base_url or (not self.config.demo_mode and not opa_server_base_url.startswith("https://")):
            raise ValueError("OPA URL is not HTTPS or is missing.")
        
        full_opa_url = f"{opa_server_base_url}/v1/data/{policy_path}"
        logger.debug(f"Querying OPA at {full_opa_url} with input (truncated): {json.dumps(input_data)[:200]}...")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.debug(f"OPA response for {policy_path}: Allowed={allowed}, Reason='{reason}'")
                    if self.audit_logger:
                        redacted_input, redacted_count = redact_sensitive(input_data)
                        if redacted_count > 0:
                            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
                        await self.audit_logger.log_event(
                            Constants.AUDIT_EVENT_TYPES["policy_decision"],
                            {"input": redacted_input, "policy": policy_path, "result": "allowed" if allowed else "denied", "reason": reason}
                        )
            ...
            if self.audit_logger:
                ...
                await self.audit_logger.log_event(
                    ...
                    {"policy_path": policy_path, "input": redacted_input, "error": str(e), "traceback": traceback.format_exc()},
                    critical=True
                )
            raise
    ...
    def _parse_opa_response(self, opa_response: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if isinstance(result, list) and result:
            ...
            if isinstance(first_decision, dict):
                ...
                return allowed, first_decision.get("reason", "OPA allowed" if allowed else "OPA denied")
...
class PolicyEngine:
    ...
    """
    def __init__(self, policy_config_path: Optional[str], config: Configuration, audit_logger: Any, metrics_client: MetricsClient, filesystem: FileSystem, policy_client: PolicyClient):
        """
    ...
    @classmethod
    async def create(cls, policy_config_path: Optional[str], config: Configuration, audit_logger: Any, metrics_client: MetricsClient, filesystem: FileSystem, policy_client: PolicyClient):
        self = cls(policy_config_path, config, audit_logger, metrics_client, filesystem, policy_client)
        await self._load_policies()
        return self

    async def _load_policies(self):
        """Loads and validates policies from the configuration file."""
        if self.policy_config_path and (".." in self.policy_config_path or not self.policy_config_path.endswith(".json")):
            raise ValueError("Invalid policy_config_path")

        if not self.policy_config_path:
            logger.info("PolicyEngine initialized without a valid config file path. Using built-in defaults.")
        ...
        if not self.filesystem.file_exists(full_policy_config_path):
            logger.critical(f"CRITICAL: PolicyEngine initialized without a config file at {full_policy_config_path}. Using hardcoded defaults.")
            self.policies = Constants.DEFAULT_POLICIES.copy()
            if self.audit_logger:
                await self.audit_logger.log_event(
                    Constants.AUDIT_EVENT_TYPES["policy_reloaded"],
            ...
            try:
                ...
                self.policy_hash = self.filesystem.generate_file_hash(full_policy_config_path, self.config.project_root)
                logger.info(f"PolicyEngine loaded policies from: {full_policy_config_path} (Hash: {self.policy_hash})")
                if self.audit_logger:
                    await self.audit_logger.log_event(
                        Constants.AUDIT_EVENT_TYPES["policy_reloaded"],
                ...
                logger.critical(f"CRITICAL: Error loading or validating policies from {full_policy_config_path}: {e}. Aborting execution.", exc_info=True)
                if self.audit_logger:
                    await self.audit_logger.log_event(
                        Constants.AUDIT_EVENT_TYPES["policy_reload_failure"],
                        {"path": full_policy_config_path, "error": str(e), "traceback": traceback.format_exc()},
                        critical=True
                    )
                raise
    ...
    def _validate_policy_schema(self, policies: Dict[str, Any]):
        ...
        required_top_level_keys = ["generation_rules", "integration_rules", "security_scan_threshold"]
        ...
        required_integ = ["min_test_quality_score", "deny_integrate_modules", "human_review_required_languages"]
    ...
    async def reload_policies(self):
        ...
        try:
            ...
            if self.audit_logger:
                await self.audit_logger.log_event(
                    Constants.AUDIT_EVENT_TYPES["policy_reload_failure"],
                    {"path": self.policy_config_path, "error": str(e), "traceback": traceback.format_exc()},
                    critical=True
                )
            raise

    async def _deny(self, rule: str, input_data: Dict[str, Any], reason: str, critical: bool = False) -> Tuple[bool, str]:
        """
        ...
        if self.audit_logger:
            if redacted_count > 0:
                await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["policy_denied"], {"input": redacted_input, "policy": rule, "reason": reason}, critical=critical)
        return False, reason
    ...
    async def _allow(self, rule: str, input_data: Dict[str, Any], reason: str) -> Tuple[bool, str]:
        ...
        if self.audit_logger:
            if redacted_count > 0:
                await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["policy_decision"], {"input": redacted_input, "policy": rule, "result": "allowed", "reason": reason})
        return True, reason

    async def should_generate_tests(self, module_identifier: str, language: str) -> Tuple[bool, str]:
        """
    ...
    async def should_integrate_test(self, module_identifier: str, test_quality_score: float, language: str, has_security_issues: bool = False, security_severity_str: str = "NONE") -> Tuple[bool, str]:
        ...
        """
        cache_key = (module_identifier, test_quality_score, language, has_security_issues, security_severity_str)
        async with self._integrate_lock:
            if cache_key in self._integrate_cache:
                return self._integrate_cache[cache_key]
        
        input_data = {"module_identifier": module_identifier, "test_quality_score": test_quality_score, "language": language, "has_security_issues": has_security_issues, "security_severity": security_severity_str}
        
        if self.config.opa_integration_enabled:
            try:
                allowed, reason = await self.policy_client.evaluate_policy(Constants.OPA_POLICY_PATHS["integration_check"], input_data)
                self.metrics_client.policy_evaluations_total.labels(result="allowed" if allowed else "denied", rule="integration_check").inc()
                if not allowed:
                    if self.audit_logger:
                        redacted_input, redacted_count = redact_sensitive(input_data)
                        if redacted_count > 0:
                            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
                        await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["policy_denied"], {"input": redacted_input, "policy": "integration_check_opa", "reason": reason}, critical=True)
                async with self._integrate_lock:
                    self._integrate_cache[cache_key] = (allowed, reason)
                return allowed, reason
            except Exception as e:
                self.metrics_client.policy_evaluations_total.labels(result="denied", rule="integration_check").inc()
                return False, f"OPA policy evaluation failed: {e}. Defaulting to deny."
        ...
        if test_quality_score < min_quality:
            result = await self._deny("integration_rules_local", input_data, f"Test quality score ({test_quality_score:.2f}) below minimum required ({min_quality:.2f}).")
            async with self._integrate_lock:
                self._integrate_cache[cache_key] = result
            return result
            
        if any(deny_mod in module_identifier for deny_mod in integration_rules.get("deny_integrate_modules", [])):
            result = await self._deny("integration_rules_local", input_data, "Test integration explicitly forbidden for this module.")
            async with self._integrate_lock:
                self._integrate_cache[cache_key] = result
            return result
        ...
        if security_severity_str not in severity_levels:
            if security_severity_str not in self._warned_severities:
                logger.warning(f"Unknown security severity '{security_severity_str}'. Mapping to 'CRITICAL'.")
                self._warned_severities.add(security_severity_str)
            security_severity_str = "CRITICAL"

        if has_security_issues and severity_levels.get(security_severity_str, 0) >= severity_levels.get(security_threshold_level, 0):
            result = await self._deny("integration_rules_local", input_data, f"Security scan detected issues of '{security_severity_str}' severity, which meets or exceeds threshold '{security_threshold_level}'.")
            async with self._integrate_lock:
                self._integrate_cache[cache_key] = result
            return result
            
        result = await self._allow("integration_rules_local", input_data, "Policy allows test integration.")
        async with self._integrate_lock:
            self._integrate_cache[cache_key] = result
        return result
    
    async def requires_pr_for_integration(self, module_identifier: str, language: str, test_quality_score: float) -> Tuple[bool, str]:
        """
        ...
        input_data = {"module_identifier": module_identifier, "language": language, "test_quality_score": test_quality_score}
        
        if self.config.opa_integration_enabled:
            try:
                requires_pr_by_opa, reason_opa = await self.policy_client.evaluate_policy(Constants.OPA_POLICY_PATHS["pr_required_check"], input_data)
                self.metrics_client.policy_evaluations_total.labels(result="requires_pr" if requires_pr_by_opa else "allowed", rule="pr_required_check").inc()
                async with self._pr_lock:
                    self._pr_cache[cache_key] = (requires_pr_by_opa, reason_opa)
                return requires_pr_by_opa, reason_opa
            except Exception as e:
                logger.warning(f"OPA check for PR requirement failed: {e}. Falling back to local policy.", exc_info=True)
                if self.audit_logger:
                    redacted_input, redacted_count = redact_sensitive(input_data)
                    if redacted_count > 0:
                        await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
                    await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["opa_failure"], {"input": redacted_input, "policy": "pr_required_check", "error": str(e)}, critical=True)
                result = (True, f"OPA check for PR failed: {e}. Defaulting to require PR.")
                async with self._pr_lock:
        ...
        if lang in hr_langs:
            ...
            self.metrics_client.policy_evaluations_total.labels(result="requires_pr", rule="integration_rules_local").inc()
            if self.audit_logger:
                redacted_input, redacted_count = redact_sensitive(input_data)
                if redacted_count > 0:
                    await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
                await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["policy_decision"], {"input": redacted_input, "policy": "pr_check_local", "result": "requires_pr", "reason": reason})
            async with self._pr_lock:
                self._pr_cache[cache_key] = (True, reason)
            return True, reason
            
        if any(mod in module_identifier for mod in integration_rules.get("require_human_review_modules", [])):
        ...
        self.metrics_client.policy_evaluations_total.labels(result="allowed", rule="integration_rules_local").inc()
        if self.audit_logger:
            redacted_input, redacted_count = redact_sensitive(input_data)
            if redacted_count > 0:
                await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["policy_decision"], {"input": redacted_input, "policy": "pr_check_local", "result": "allowed", "reason": reason})
        async with self._pr_lock:
...
class EventBus:
    ...
    """
    def __init__(self, config: Configuration, audit_logger: Any, metrics_client: MetricsClient, message_queue_service=None, session_factory: Optional[Callable[[], aiohttp.ClientSession]] = None):
        """
        ...
        if event_name in self.config.critical_events_for_mq and self.message_queue_service:
            try:
                ...
                if self.metrics_client:
                    self.metrics_client.notification_failures_total.labels(service="message_queue", event_name=event_name).inc()
                if self.audit_logger:
                    await self.audit_logger.log_event(
                        Constants.AUDIT_EVENT_TYPES["notification_failure"],
                        {"service": "message_queue", "event_name": event_name, "error": "MessageQueue publish failed"}
                    )
    ...
    async def _publish_to_slack(self, event_name: str, data: Dict[str, Any], slack_url: str, slack_events: set) -> bool:
        ...
        try:
            ...
            self.metrics_client.notification_failures_total.labels(service="Slack", event_name=event_name).inc()
            logger.error(f"Failed to send notification to Slack for {event_name}.", exc_info=True, extra={"service": "Slack", "event": event_name})
            if self.audit_logger:
                await self.audit_logger.log_event(
                    Constants.AUDIT_EVENT_TYPES["notification_failure"],
                    {"service": "Slack", "event": event_name, "error": traceback.format_exc()},
                    critical=True
                )
            return False

    async def _publish_to_webhook(self, event_name: str, data: Dict[str, Any], webhook_hooks: dict, webhook_events: set) -> bool:
        ...
        try:
            redacted_data, _ = redact_sensitive(data)
            await self._send_notification_with_retry(webhook_url, redacted_data, f"webhook:{hook_name}")
            return True
        except Exception:
            self.metrics_client.notification_failures_total.labels(service=f"webhook:{hook_name}", event_name=event_name).inc()
            logger.error(f"Failed to send notification to webhook {hook_name} for {event_name}.", exc_info=True, extra={"service": f"webhook:{hook_name}", "event": event_name})
            if self.audit_logger:
                await self.audit_logger.log_event(
                    Constants.AUDIT_EVENT_TYPES["notification_failure"],
                    {"service": f"webhook:{hook_name}", "event": event_name, "error": traceback.format_exc()},
                    critical=True
                )
            return False
        
    @retry(stop=RETRY_STOP, wait=RETRY_WAIT)
    async def _send_notification_with_retry(self, url: str, payload: Dict[str, Any], service_name: str):
        ...
        if self.config.allowed_notification_hosts and parsed_url.netloc not in self.config.allowed_notification_hosts:
            raise ValueError(f"Host {parsed_url.netloc} not in allowed hosts for {service_name}")
        
        async with self.session_factory() as session:
            async with session.post(url, json=payload, timeout=self.config.notification_timeout_seconds) as response:
                response.raise_for_status()
        ...
        if self.audit_logger:
            ...
            if redacted_count > 0:
                await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["sensitive_data_redacted"], {"count": redacted_count})
            await self.audit_logger.log_event(Constants.AUDIT_EVENT_TYPES["notification_sent"], {"service": service_name, "payload": redacted_payload})

# Final list of exposed symbols
__all__ = ["PolicyEngine", "EventBus", "redact_sensitive", "AuditLogger", "Constants", "Configuration", "MetricsClient", "FileSystem", "LocalFileSystem", "OPAPolicyClient", "PolicyClient"]
...
if __name__ == "__main__":
    ...
    async def run_test_case():
        ...
        policy_engine = await PolicyEngine.create(None, config, audit_logger, metrics_client, filesystem, policy_client)
