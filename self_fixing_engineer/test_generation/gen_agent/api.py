...
try:
    ...
    logging.warning("Flask extension 'flask_jwt_extended' not installed. JWT authentication disabled.")
    pass
...
try:
    ...
    logging.warning("Flask extension 'prometheus_flask_exporter' not installed. Prometheus metrics disabled.")
    pass
...
TestAgentState = TypedDict('TestAgentState', {
    'spec': str,
    'spec_format': str,
    'language': str,
    'framework': str,
...
def create_app(config: Dict[str, Any]) -> Flask:
    ...
    def _load_swagger() -> Dict[str, Any]:
        ...
        return {
            ...
            "paths": {
                ...
                "/status": {"get": {"summary": "Status", "responses": {"200": {"description": "OK"}}}},
                "/generate-tests": {
                    "post": {"summary": "Generate tests", "responses": {"200": {"description": "OK"}}}
                },
            },
        }
    ...
    if get_swaggerui_blueprint:
        swaggerui_blueprint = get_swaggerui_blueprint(
            SWAGGER_URL,
            API_URL,
            config={'app_name': "Test Generation Agent API"}
        )
    ...
    if Limiter and LIMITER_AVAILABLE:
        ...
        if os.getenv("ENV", "dev") == "prod" and storage_uri.startswith("memory://"):
            logger.warning("Rate limit storage is in-memory in production; use a shared backend (e.g., redis://).")
    ...
    if os.getenv("ENV", "dev") == "prod" and not JWT_AVAILABLE:
        raise RuntimeError("JWT authentication is required in production environment but flask_jwt_extended is not available.")
    ...
    def add_security_headers(response):
        if os.getenv("ENV", "dev") == "prod":
            response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; font-src 'self' data:;"
    ...
    def after_request_log(response):
        ...
        logger.info(f"rid={g.request_id} {request.path} {response.status_code} in {elapsed_time:.4f}s")
        return response
    ...
    def generate_tests_endpoint():
        try:
            ...
            if current_app.config["_GRAPH"] is None:
                return jsonify({"error": "Service unavailable", "message": "LLM agent failed to initialize at startup."}), 503
            ...
            if isinstance(e, ValidationError):
                return jsonify({"error": "Bad Request", "message": "Validation failed", "details": e.errors()}), 400
            else:
                return jsonify({"error": "Bad Request", "message": str(e)}), 400
        except Exception as e:
    ...
    def _register_swagger_mocks():
        ...
        for path, methods in (spec.get("paths") or {}).items():
            for m, op in (methods or {}).items():
                ...
                try:
                    app.add_url_rule(path, endpoint=endpoint_name, view_func=_mk_handler(), methods=[http_method])
                    logger.info("Registered mock endpoint from swagger: %s %s", http_method, path)
                except Exception as e:
                    logger.warning("Failed to register mock endpoint %s %s: %s", http_method, path, e)
...
def serve_api(host: str, port: int) -> int:
    if not Flask or not FLASK_AVAILABLE:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            asyncio.run(audit_logger.log_event(
                ...
                details={"action": "serve_api", "result": "failure", "reason": "flask not installed"},
                critical=True
            ))
        return 1
    ...
    if os.getenv("ENV", "dev") == "prod" and _GUNICORN_OK:
        ...
        _monkey.patch_all()
        sys.argv = ["gunicorn", "-w", "4", "-k", "gevent", f"--bind={host}:{port}", f"{__name__}:app"]
