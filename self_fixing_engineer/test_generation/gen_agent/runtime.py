...
with contextlib.suppress(ImportError):
    ...
    class TestAgentConfig(BaseSettings):
        model_config = SettingsConfigDict(env_file='.env', extra='ignore', env_file_encoding='utf-8', env_prefix="TEST_AGENT_")
        sessions_dir: Path = Field(default=Path(__file__).parent.parent.resolve() / "sessions")
        tests_output_dir: Path = Field(default=Path(__file__).parent.parent.resolve() / "generated_tests")
...
if not config:
    ...
    config = types.SimpleNamespace(
        ...
        sessions_dir=Path(os.getenv("TEST_AGENT_SESSIONS_DIR", os.path.join(os.path.dirname(os.path.dirname(__file__)), "sessions"))).resolve(),
        tests_output_dir=Path(os.getenv("TEST_AGENT_TESTS_OUTPUT_DIR", os.path.join(os.path.dirname(os.path.dirname(__file__)), "generated_tests"))).resolve(),
...
def _load_config(config_file: Optional[str] = None) -> Dict[str, Any]:
    ...
    if os.environ:
        ...
        for k, v in os.environ.items():
            ...
            redacted = "***" if any(s in target_key.lower() for s in ("api_key", "token", "password")) else v
...
try:
    ...
    try:
        dependency_status = Gauge('dependency_status', 'Status of optional dependencies', ['package'])
    except ValueError:
        class _DG:  # tiny no-op
            def labels(self, **_: str): return self
            def set(self, *_: Any): pass
    ...
    try:
        dependency_check_total = Counter('dependency_check_total', 'Total number of dependency checks')
    except ValueError:
        class _DC:
            def inc(self, *_: Any, **__: Any): pass
...
def redact_sensitive(data: Any, extra_keys: Optional[set[str]] = None) -> Any:
    ...
    sensitive_key_names = {"api_key", "secret", "password", "token", "auth", "authorization", "key"}
    if extra_keys:
    ...
    if isinstance(data, str):
        ...
        redacted_str = re.sub(r'("Authorization"\s*:\s*)"Bearer [^"]+"', r'\1"[REDACTED]"', redacted_str)
...
def setup_logging(is_ci: bool = False, log_level: int = logging.INFO, log_file_path: str = "test_gen_agent.log", enable_file: bool = True) -> None:
    ...
    if enable_file:
        file_handler = logging.handlers.RotatingFileHandler(Path(log_file_path).resolve(), maxBytes=1024*1024*5, backupCount=5, encoding='utf-8')
...
def is_ci_environment() -> bool:
    ...
    return any(os.environ.get(key) for key in ['CI', 'GITHUB_ACTIONS', 'JENKINS_URL', 'GITLAB_CI', 'TF_BUILD', 'BUILDKITE', 'TRAVIS', 'CIRCLECI', 'TEAMCITY_VERSION', 'APPVEYOR'])
...
def install_package(package_name: str) -> bool:
    ...
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", package_name], timeout=300, env=env)
        version_output = subprocess.check_output([sys.executable, "-m", "pip", "show", package_name]).decode()
        version_line = next((line for line in version_output.splitlines() if line.startswith("Version:")), None)
...
async def run_dependency_check(provider: Optional[str] = None, is_ci: bool = False):
    ...
    try:
        ...
        raise ImportError(f"Missing package for '{provider}'. Install with: {hints.get(provider, 'pip install <package>')}") from e
...
async def ensure_package(package_name: str, is_ci: bool) -> bool:
    ...
    if importlib.util.find_spec(_normalize_pkg_name(package_name)) is None:
        ...
        if is_ci:
            logging.critical(f"In CI mode, cannot auto-install. Please add '{package_name}' to your project dependencies.")
            return False
        else:
...
async def run_dependency_check_async(is_ci: bool) -> None:
    ...
    _load_and_check_deps.cache_clear()
    check_tasks = [ensure_package(pkg, is_ci) for pkg in _optional_packages.keys() if importlib.util.find_spec(_normalize_pkg_name(pkg)) is None]
...
def health_check() -> Dict[str, Any]:
    ...
    _load_and_check_deps.cache_clear()
    _load_and_check_deps()
    valid_config_types = tuple(t for t in (Dynaconf, BaseSettings, types.SimpleNamespace) if t is not None)
    health_status = {
        "status": "ok",
...
async def interactive_session_creator(session_name: str) -> Optional[Dict[str, Any]]:
    ...
    spec_format = input("Enter spec format (e.g., gherkin, openapi, user_story): ").strip() or "gherkin"
    print("Paste your specification below. Press Ctrl+D (Unix) or Ctrl+Z then Enter (Windows) when done.")
    try:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False, encoding="utf-8", dir=str(session_path.parent)) as tmp_file:
    ...
    if session_path.exists():
        try:
            ...
            if is_ci:
                ...
                response = input(f"Failed to read session file '{session_path}'. Overwrite? [y/N]: ").lower()
                if response == 'y':
    ...
    def _init_llm_with_retry(provider: str, model: str) -> Any:
        ...
        if provider == "openai":
            try:
                ...
                raise ImportError("Missing 'langchain-openai'. Install with: pip install langchain-openai") from e
            api_key = _cfg("openai_api_key", "OPENAI_API_KEY")
            if not api_key:
                raise ValueError("OpenAI API key not found in CONFIG['openai_api_key'] or OPENAI_API_KEY.")
            ...
            try:
                ...
                raise ImportError("Missing 'langchain-google-genai'. Install with: pip install langchain-google-genai") from e
            api_key = _cfg("gemini_api_key", "GEMINI_API_KEY")
            if not api_key:
                raise ValueError("Gemini API key not found in CONFIG['gemini_api_key'] or GEMINI_API_KEY.")
            ...
            try:
                ...
                raise ImportError("Missing 'langchain-anthropic'. Install with: pip install langchain-anthropic") from e
            api_key = _cfg("anthropic_api_key", "ANTHROPIC_API_KEY")
            if not api_key:
                raise ValueError("Anthropic API key not found in CONFIG['anthropic_api_key'] or ANTHROPIC_API_KEY.")
            ...
            try:
                ...
                raise ImportError("Missing 'langchain-community'. Install with: pip install langchain-community") from e
        ...
        if llm_instance is None:
            raise ImportError(f"Failed to initialize LLM with provider '{provider}' and model '{model}'.")
