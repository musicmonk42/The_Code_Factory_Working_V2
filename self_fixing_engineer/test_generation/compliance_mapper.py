...
try:
    ...
    class StubAsyncFile:
        ...
        async def __aexit__(self, exc_type, exc_val, exc_tb):
            self.file.close()
    aiofiles = type('aiofiles', (), {'open': lambda path, mode='r', encoding='utf-8': StubAsyncFile(path, mode, encoding)})
...
class ComplianceConfig:
    ...
    file_extensions: List[str] = field(default_factory=lambda: ['.py', '.js', '.ts', '.java', '.go'])
    ...
    def __post_init__(self):
        ...
        if self.timeout_per_file <= 0:
            raise ValueError(f"Invalid timeout_per_file: {self.timeout_per_file}. Must be positive.")
        if self.timeout_per_rule <= 0:
            raise ValueError(f"Invalid timeout_per_rule: {self.timeout_per_rule}. Must be positive.")
        ...
        if self.encryption_key is None and CRYPTOGRAPHY_AVAILABLE:
            ...
            self.encryption_key = key.decode("utf-8") if isinstance(key, (bytes, bytearray)) else str(key)
        
        if not CRYPTOGRAPHY_AVAILABLE and self.encryption_key:
            logger.warning({"message": "Encryption key provided but cryptography module not available. Encryption disabled."})
...
class ComplianceRule:
    ...
    async def check(self, file_path: Path, content: str, config: ComplianceConfig) -> List[ComplianceIssue]:
...
class GDPRDataProtectionRule(ComplianceRule):
    ...
    def _sync_check(self, file_path: Path, content: str, config: ComplianceConfig) -> List[ComplianceIssue]:
        issues = []
        sensitive_patterns = {
            ...
            for pattern, issue_type in sensitive_patterns.items():
                ...
                if match:
                    ...
                    if config.encryption_key and CRYPTOGRAPHY_AVAILABLE and Fernet is not None:
                        try:
                            f = Fernet(config.encryption_key.encode() if isinstance(config.encryption_key, str) else config.encryption_key)
                            code_snippet = f.encrypt(matched_snippet.encode("utf-8")).decode("utf-8", errors="ignore")
                            encrypted = True
                        except Exception:
                            encrypted = False
                            code_snippet = matched_snippet
    ...
    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10)) if TENACITY_AVAILABLE else lambda x: x
    async def check(self, file_path: Path, content: str, config: ComplianceConfig) -> List[ComplianceIssue]:
...
class CustomComplianceRule(ComplianceRule):
    ...
    def __init__(self, frameworks: List[ComplianceFramework], patterns: Dict[str, str], description: str, severity: str):
...
class ComplianceRuleRegistry:
    ...
    async def _discover_rules(self):
        ...
        try:
            entry_points = importlib.metadata.entry_points(group='compliance_rules') if sys.version_info >= (3, 10) else []
            for entry_point in entry_points:
                try:
                    ...
                    logger.error({"message": f"Failed to load rule {entry_point.name}", "error": repr(e)})
        except Exception as e:
    ...
    async def register_rule(self, rule: ComplianceRule):
        ...
        for framework in rule.frameworks:
            ...
            logger.info({"message": f"Registered rule for {framework.value}: {rule.__class__.__name__}"})
...
async def generate_report_async(project_root: str, user_id: str = "unknown", custom_config: Optional[str] = None) -> ComplianceReport:
    """
    ...
    try:
        ...
        config = ComplianceConfig(project_root=Path(project_root), user_id=user_id, custom_framework_config=custom_config_path)
    except ValueError as e:
        logger.error({"message": f"Configuration error: {repr(e)}"})
        await audit_event("compliance_config_error", {"error": repr(e), "user_id": user_id}, critical=True)
        raise
    ...
    ctx = tracer.start_as_current_span(f"process_file_{Path(project_root).name}") if TRACING_AVAILABLE else nullcontext()
    with ctx as span:
        if span:
        ...
        await audit_event("compliance_check_started", {"project_root": str(project_root), "user_id": user_id})

        files = [f for f in config.project_root.rglob("*") if f.is_file() and f.suffix in config.file_extensions]
        ...
        with ThreadPoolExecutor(max_workers=config.max_workers) as executor:
            for batch in batches:
                tasks = [
                    _process_file(file, config, executor)
                    for file in batch
                ]
                ...
                for result in results:
                    if isinstance(result, list):
                        ...
                        logger.error({"message": f"File processing timed out after {config.timeout_per_file} seconds", "file": "unknown"})
                        ...
                        compliance_file_errors.labels(error_type="timeout", framework="unknown").inc()
                        await audit_event("compliance_file_error", {"error": "timeout", "user_id": user_id}, critical=True)
                    elif isinstance(result, Exception):
                        # FIX: Treat exceptions as non-compliant issues
                        logger.error({"message": f"File processing failed with {type(result).__name__}", "error": repr(result), "file": "unknown"})
                        issue = ComplianceIssue(
                            file_path=Path("unknown"),
                            framework=ComplianceFramework.GDPR,
                        ...
                        all_issues.append(issue)
                        compliance_file_errors.labels(error_type="unexpected", framework="unknown").inc()
                        await audit_event("compliance_file_error", {"error": repr(result), "user_id": user_id}, critical=True)
        ...
        for issue in all_issues:
            ...
            await audit_event("compliance_issue_detected", {
                "file": str(issue.file_path), "framework": issue.framework.value,
        ...
        if not is_compliant and config.alert_callback:
            await maybe_await(config.alert_callback(f"Found {len(all_issues)} compliance issues in {project_root}"))

        if span:
            span.set_attribute("issue_count", len(all_issues))
            span.set_attribute("is_compliant", is_compliant)

        await audit_event("compliance_check_completed", {
            "project_root": str(project_root), "issue_count": len(all_issues),
            "is_compliant": is_compliant, "user_id": user_id
        })

        logger.info({"message": f"Compliance check finished. Issues: {len(all_issues)}, Compliant: {is_compliant}"})
...
def generate_report_sync(project_root: str, *, user_id: str, custom_config: Optional[str] = None):
    ...
    """
    return asyncio.run(generate_report_async(project_root, user_id=user_id, custom_config=custom_config))
...
async def _process_file(file_path: Path, config: ComplianceConfig, executor: ThreadPoolExecutor) -> List[ComplianceIssue]:
    """Processes a single file for compliance issues."""
    if file_path.suffix not in config.file_extensions or file_path.stat().st_size > config.max_file_size:
        return []
    ...
    try:
        ...
        await audit_event("compliance_file_error", {"file": str(file_path), "error": repr(e), "user_id": config.user_id}, critical=True)
    ...
    ctx = tracer.start_as_current_span(f"process_file_{file_path.name}") if TRACING_AVAILABLE else nullcontext()
    with ctx as span:
        if span:
        ...
        try:
            ...
            for result in results:
                if isinstance(result, list):
                    ...
                    logger.error({"message": f"Rule timed out on {file_path}", "error": repr(result)})
                    compliance_file_errors.labels(error_type="rule_timeout", framework="unknown").inc()
                elif isinstance(result, Exception):
                    logger.error({"message": f"Rule check failed for {file_path}", "error": repr(result)})
                    compliance_file_errors.labels(error_type="rule_error", framework="unknown").inc()
        except Exception as e:
            ...
            file_issues.append(ComplianceIssue(
                file_path=file_path,
                framework=ComplianceFramework.GDPR,
                issue_type="processing_error",
...
async def _run_rule_check(
    ...
    with compliance_check_duration.labels(framework=framework.value).time() if PROMETHEUS_AVAILABLE else nullcontext():
        try:
            issues = await asyncio.wait_for(
                rule.check(file_path, content, config),
                timeout=config.timeout_per_rule,
            )
            compliance_checks_total.labels(framework=framework.value, status="success").inc() if PROMETHEUS_AVAILABLE else None
            return issues
            ...
            compliance_checks_total.labels(framework=framework.value, status="timeout").inc() if PROMETHEUS_AVAILABLE else None
            compliance_file_errors.labels(error_type="rule_timeout", framework=framework.value).inc()
            return [ComplianceIssue(
                file_path=file_path,
                framework=framework,
                issue_type="rule_timeout",
            ...
            logger.error({"message": f"Rule {rule.__class__.__name__} failed on {file_path}", "error": repr(e)})
            compliance_checks_total.labels(framework=framework.value, status="failure").inc() if PROMETHEUS_AVAILABLE else None
            return [ComplianceIssue(
                file_path=file_path,
                framework=framework,
    ...
    async def main():
        ...
        try:
            ...
            if not project_path.exists():
                project_path.mkdir()
                (project_path / "gdpr_violation.py").write_text("def process_user_data(email, ssn):\n    print(f'User email is {email}')\n")
            ...
            await reporter.generate_report(overall_results=report.to_json(), policy_engine=SimpleNamespace(policy_hash="mock_hash_123"))
