...
try:
    ...
    try:
        ...
        if Version(rich_version_str) >= Version('14.0.0'):
            ...
            logging.getLogger(__name__).warning("Warning: Rich version < 14.0.0 detected. Rich console output will be disabled.")
    except PackageNotFoundError:
        RICH_AVAILABLE = False
        logging.getLogger(__name__).warning("Warning: 'rich' not installed. Rich console output will be disabled.")
except ImportError:
    RICH_AVAILABLE = False
    Console = None
    logging.getLogger(__name__).warning("Warning: 'rich' not available. Rich console output will be disabled.")
...
try:
    ...
    logging.getLogger(__name__).warning("Warning: 'aiofiles' not available. Falling back to synchronous file operations.")
    ...
    aiofiles = SimpleNamespace(open=_SyncFileWrapper, os=SimpleNamespace(
...
try:
    ...
    try:
        ...
        if Version(prom_version) >= Version('0.22.1'):
            ...
            test_execution_duration = Histogram("atco_test_execution_seconds", "Time taken for test execution", ["language"])
            test_execution_errors = Counter("atco_test_execution_errors_total", "Failed test executions", ["language"])
            file_operations_total = Counter("atco_file_operations_total", "File operations by type and status", ["operation", "status"])
            process_executions_total = Counter("atco_process_executions_total", "Process executions by command and status", ["command", "status"])
            METRICS_AVAILABLE = True
        else:
            METRICS_AVAILABLE = False
            logging.getLogger(__name__).warning("Warning: prometheus_client version < 0.22.1 detected. Metrics will be disabled.")
    except PackageNotFoundError:
        METRICS_AVAILABLE = False
        logging.getLogger(__name__).warning("Warning: 'prometheus_client' not installed. Metrics will be disabled.")
    except Exception:
        METRICS_AVAILABLE = False
        logging.getLogger(__name__).warning("Warning: prometheus_client has no __version__ attribute. Metrics will be disabled.")
except ImportError:
    METRICS_AVAILABLE = False
    class DummyMetric:
    ...
    logging.getLogger(__name__).warning("Warning: 'prometheus_client' not available. Metrics will be disabled.")
...
try:
    ...
    try:
        ...
        if Version(ten_version) >= Version('9.1.2'):
            ...
            logging.getLogger(__name__).warning("Warning: tenacity version < 9.1.2 detected. Retries may be disabled.")
    except PackageNotFoundError:
        TENACITY_AVAILABLE = False
        logging.getLogger(__name__).warning("Warning: 'tenacity' not available. Retries will be disabled.")
    except Exception:
        TENACITY_AVAILABLE = False
        logging.getLogger(__name__).warning("Warning: tenacity has no __version__ attribute. Retries may be disabled.")
except ImportError:
    TENACITY_AVAILABLE = False
...
try:
    ...
    logging.getLogger(__name__).warning(f"Warning: Arbiter audit_log import failed ({e}). Using stub.")
...
def validate_and_resolve_path(base_path: str, user_input_path: str, allow_outside_base: bool = False) -> str:
    """
    ...
    if not within:
        if not allow_outside_base:
            raise PathError(f"Path '{user_input_path}' is outside the base directory '{base_path}'.")
    ...
    if os.path.islink(combined_path):
        ...
        try:
            link_within = os.path.commonpath([link_target, abs_base_path]) == os.path.realpath(abs_base_path)
        except ValueError:
            link_within = False
        if not link_within:
            if not allow_outside_base:
                raise PathError(f"Symbolic link '{user_input_path}' points to an unauthorized location '{link_target}'.")
    ...
    def __init__(self, config: Dict[str, Any], project_root: str):
        ...
        def validate_and_create_dir(dir_path_key: str, default_path: str) -> str:
            ...
            if os.getenv("DEMO_MODE", "False").lower() == "true":
                logger.warning(f"DEMO_MODE: Writable path '{abs_path}' is being used. In a production environment, this might be restricted.")
            
            return str(abs_path)

        self.QUARANTINE_DIR: str = validate_and_create_dir("quarantine_dir", "atco_artifacts/quarantined_tests")
        self.GENERATED_OUTPUT_DIR: str = validate_and_create_dir("generated_output_dir", "atco_artifacts/generated")
        self.SARIF_EXPORT_DIR: str = validate_and_create_dir("sarif_export_dir", "atco_artifacts/sarif_reports")
        self.AUDIT_LOG_FILE: str = os.path.join(self.project_root, config.get("audit_log_file", "atco_artifacts/atco_audit.log"))
        self.COVERAGE_REPORTS_DIR: str = validate_and_create_dir("coverage_reports_dir", "atco_artifacts/coverage_reports")
        ...
        if sys.version_info >= (3, 9):
            if not full_suite_dir.is_relative_to(self.project_root):
                raise PathError(f"Configured test suite path '{full_suite_dir}' is outside the project root.")
        else:
...
def log(msg: str, level: str = "INFO", style: str = "green") -> None:
    ...
    if console:
        color_map = {
            ...
            "CRITICAL": "bold red on white", "SUCCESS": "bold green"
        }
...
def _is_path_allowed_for_write(target_path: str, config: 'ATCOConfig') -> bool:
    ...
    for allowed_path_root in config.ALLOWED_WRITE_PATHS:
        try:
            if os.path.commonpath([abs_target_path, os.path.abspath(allowed_path_root)]) == os.path.abspath(allowed_path_root):
                return True
        except ValueError:
            continue
    return False
...
async def monitor_and_prioritize_uncovered_code(coverage_file: str, policy_engine, project_root: str, config: Dict) -> List:
...
@zero_trust_guard
def generate_file_hash(filepath_relative: str, project_root: str, hash_algorithm: str = "sha256") -> str:
    ...
    try:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            _fire_and_forget(audit_logger.log_event(event_type="file_hash_failure", details={"path": filepath_relative, "error": str(e), "traceback": traceback.format_exc()}, critical=True))
        return "HASH_ERROR"

@zero_trust_guard
async def secure_write_file(filepath_relative: str, project_root: str, content: str, mode: str = 'w', permissions: int = 0o600) -> None:
    """
    ...
    try:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(event_type="file_write_failure", details={"path": filepath_relative, "error": str(e), "traceback": traceback.format_exc()}, critical=True)
...
async def backup_existing_test(dst_relative_path: str, project_root: str) -> str:
    ...
    try:
        if AIOFILES_AVAILABLE:
            async with aiofiles.open(full_dst_path, 'rb') as src, aiofiles.open(full_bak_dst_path, 'wb') as dst:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(
                ...
                details={"original_file": dst_relative_path, "backup_file": bak_dst_relative},
                critical=False
            )
        ...
        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(event_type="file_backup_failure", details={"original_file": dst_relative_path, "error": str(e), "traceback": traceback.format_exc()}, critical=True)
...
def compare_files(file1_full_path: str, file2_full_path: str) -> bool:
    ...
    try:
        ...
        logger.error(f"Error comparing files {file1_full_path} and {file2_full_path}: {e}", exc_info=True)
        return False
    ...
    try:
        ...
        try:
            await audit_logger.log_event(
                ...
                details={"path": path, "error": str(e), "traceback": traceback.format_exc()},
                critical=True,
            )
        except Exception:
...
class SecurityScanner:
    ...
    @zero_trust_guard
    async def scan_test_file(self, file_path_relative: str, language: str) -> Tuple[bool, List[str], str]:
        ...
        if language == "python":
            try:
                ...
                cmd = ["bandit", "-r", full_file_path, "-f", "json", "-q", "--configfile", os.devnull] # Use a null config to prevent custom rules
                ...
                process_executions_total.labels(command="bandit", status="success" if proc.returncode in [0, 1] else "failure").inc()
                ...
                if issues:
                    ...
                    ranked_severities = (self.SEV_RANK.get(r.get("severity", "NONE"), 0) for r in issues)
                    max_rank = max(ranked_severities, default=0)
                    severity = {v: k for k, v in self.SEV_RANK.items()}.get(max_rank, "NONE")
            except FileNotFoundError:
                logger.warning("Security scan skipped for '%s': Bandit not available.", file_path_relative)
                return False, [], "NONE"
            except Exception as e:
                logger.error(f"Bandit scan failed: {e}", exc_info=True)
                process_executions_total.labels(command="bandit", status="error").inc()
                if AUDIT_LOGGER_AVAILABLE:
                    await audit_logger.log_event(event_type="security_scan_failure", details={"file": file_path_relative, "error": str(e), "traceback": traceback.format_exc()}, critical=True)
                raise
        else:
            logger.warning(f"Security scan skipped for '{full_file_path}': Bandit not available or unsupported language.")
        ...
        if issues:
            ...
            if AUDIT_LOGGER_AVAILABLE:
                await audit_logger.log_event(
                    ...
                    details={"file": full_file_path, "issues": issues, "severity": severity},
            ...
            if AUDIT_LOGGER_AVAILABLE:
                await audit_logger.log_event(
                    event_type="security_scan",
                    details={"file": full_file_path, "issues": "none", "severity": "NONE"},
                    critical=False
                )
            return False, [], "NONE"
...
class KnowledgeGraphClient:
    ...
    async def update_module_metrics(self, module_identifier: str, metrics: Dict[str, Any]):
        ...
        logger.debug(f"Conceptual: Updating Knowledge Graph for '{module_identifier}' with metrics: {json.dumps(metrics)}")
...
class PRCreator:
    ...
    @zero_trust_guard
    async def create_pr(self, branch_name: str, title: str, description: str, files_to_add: List[str]) -> Tuple[bool, str]:
        ...
        result_message = f"https://github.com/org/repo/pull/{random.randint(100, 999)}" if success else "Simulated PR creation failed."
        ...
        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(
                ...
                details={"branch": branch_name, "title": title, "files": redacted_files, "result": "success" if success else "failure", "message": result_message},
    ...
    @zero_trust_guard
    async def create_jira_ticket(self, title: str, description: str, project_key: str = "ATCO") -> Tuple[bool, str]:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(
                ...
                details={"project": project_key, "title": title, "result": "success" if success else "failure", "message": result_message},
                critical=not success
            )
    ...
    def __init__(self, project_root: str, config: Dict[str, Any]):
        ...
        if TORCH_AVAILABLE and config.get('mutation_testing', {}).get('ml_fault_injection', False):
            try:
                ...
                logger.warning(f"Failed to load ML model for fault injection: {e}. Falling back to standard mutation testing.")
                self.ml_model = None

    @zero_trust_guard
    async def run_mutations(self, source_file_relative: str, test_file_relative: str, language: str) -> Tuple[bool, float, str]:
        ...
        result_message = "Simulated mutation testing successful." if success else "Simulated mutation testing failure."

        if AUDIT_LOGGER_AVAILABLE:
            await audit_logger.log_event(
                event_type="mutation_test",
                details={"source_file": source_file_relative, "test_file": test_file_relative, "language": language, "result": "success" if success else "failure", "score": mutation_score},
                critical=not success
            )
        ...
        for plugin in self.plugins:
            ...
            try:
                if asyncio.iscoroutinefunction(plugin):
                    ...
                    modified_code = await asyncio.to_thread(plugin, modified_code, language, project_root)
                logger.debug(f"Applied enrichment plugin: {plugin_name}")
            except Exception as e:
                logger.error(f"Error applying enrichment plugin {plugin_name}: {e}. Skipping plugin for this test.", exc_info=True)
                if AUDIT_LOGGER_AVAILABLE:
                    await audit_logger.log_event(event_type="test_enrichment_failure", details={"plugin": plugin_name, "error": str(e), "traceback": traceback.format_exc()}, critical=False)
...
def add_mocking_framework_import(test_code: str, language: str, project_root: str) -> str:
    ...
    if language == "python" and "unittest.mock" not in test_code and "mock" in test_code.lower() and "import mock" not in test_code:
...
async def llm_refine_test_plugin(test_code: str, language: str, project_root: str) -> str:
    ...
    if language == "python":
        ...
        return test_code.replace("expect(true).toBe(true);", "expect(true).toBe(true); // LLM-enhanced assertion clarity")
    return test_code
    ...
    def executable_to_symlink(self):
        ...
        if sys.platform == "win32":
            ...
            executables = [
                ...
                if os.path.exists(os.path.join(src_dir, exe))
            ]
        return executables
...
async def create_and_install_venv(venv_rel_or_root: str, project_root_or_deps: Optional[str] = None, deps: Optional[List[str]] = None, config: Dict[str, Any] = {}) -> Tuple[bool, Optional[str]]:
    ...
    if not os.path.isfile(python_exec):
        logger.warning(
            ...
            "Continuing with system interpreter.", python_exec
        )
    ...
    if deps:
        try:
            process = await asyncio.create_subprocess_exec(
                python_exec, "-m", "pip", "install", *deps,
            ...
            if process.returncode != 0:
                logger.warning(
                    ...
                    ", ".join(deps), stderr.decode('utf-8', errors='ignore')
                )
                return False, None
...
async def run_pytest_and_coverage(
    ...
    full_coverage_report_path = validate_and_resolve_path(project_root, coverage_report_path_relative)
    
    logger.info(f"Running pytest on '{test_path_relative}' for module '{target_module_identifier}' in venv...")
    ...
    try:
        ...
        if os.path.exists(full_coverage_report_path):
            coverage_increase = await parse_coverage_delta(full_coverage_report_path, target_module_identifier, language="python")
        else:
            logger.warning(f"Coverage report not generated at {full_coverage_report_path} for '{target_module_identifier}'.")
    ...
    if AUDIT_LOGGER_AVAILABLE:
        await audit_logger.log_event(
            ...
            details={"test_file": test_path_relative, "language": "python", "status": "passed" if test_passed else "failed", "coverage_increase": coverage_increase},
...
async def run_jest_and_coverage(
    ...
    if not npm_or_yarn:
        return False, 0.0, "Node.js package manager (npm/yarn) not found. Cannot run Jest."

    cmd = [
        npm_or_yarn, "test", os.path.abspath(full_test_path),
        "--",
        "--coverage",
    ...
    try:
        ...
        if os.path.exists(jest_json_summary_file):
            coverage_increase = await parse_coverage_delta(jest_json_summary_file, target_file_path_relative, language="javascript")
        else:
            logger.warning(f"Jest coverage report not generated at {jest_json_summary_file} for '{target_file_path_relative}'.")
    ...
    if AUDIT_LOGGER_AVAILABLE:
        await audit_logger.log_event(
            event_type="test_execution",
            details={"test_file": test_path_relative, "language": "javascript", "status": "passed" if test_passed else "failed", "coverage_increase": coverage_increase},
            critical=not test_passed
        )
    status_marker = "SUCCESS" if test_passed else "FAILURE"
    execution_log = f"{status_marker}\n{execution_log}"
...
async def run_junit_and_coverage(
    ...
    build_tool = None
    if os.path.exists(os.path.join(project_root, "pom.xml")): build_tool = "maven"
    elif os.path.exists(os.path.join(project_root, "build.gradle")): build_tool = "gradle"

    if not build_tool:
        return False, 0.0, "No Maven (pom.xml) or Gradle (build.gradle) found. Cannot run Java tests."
    ...
    try:
        async with _observe_duration(test_execution_duration, language="java"):
            ...
            stdout, stderr = await asyncio.wait_for(process.communicate(), timeout=exec_timeout_java)
        ...
        jacoco_report_full_path = os.path.join(project_root, "target", "site", "jacoco", "jacoco.xml")
        if build_tool == "gradle":
             jacoco_report_full_path = os.path.join(project_root, "build", "reports", "jacoco", "test", "jacocoTestReport.xml")

        if os.path.exists(jacoco_report_full_path):
            coverage_increase = await parse_coverage_delta(jacoco_report_full_path, target_class_identifier, language="java")
        else:
    ...
    if AUDIT_LOGGER_AVAILABLE:
        await audit_logger.log_event(
            event_type="test_execution",
            details={"test_file": test_path_relative, "language": "java", "status": "passed" if test_passed else "failed", "coverage_increase": coverage_increase},
            critical=not test_passed
        )
...
@zero_trust_guard
async def parse_coverage_delta(coverage_report_full_path: str, target_identifier: str, language: str) -> float:
    """Parses a coverage report file to find the line coverage percentage."""
    try:
        if not os.path.exists(coverage_report_full_path):
            logger.error(f"Coverage report file not found at '{coverage_report_full_path}' for delta parsing.")
            return 0.0
        ...
        if language == "python":
            ...
            for pkg in root.findall(".//package"):
                for clazz in pkg.findall("classes/class"):
                    ...
                    file_path_in_report = clazz.attrib["filename"].replace('/', '.').replace('\\', '.')
                    module_name_from_report = os.path.splitext(file_path_in_report)[0]
                    
                    if module_name_from_report == target_identifier or module_name_from_report.startswith(f"{target_identifier}."):
                        line_rate = float(clazz.attrib.get("line-rate", "0"))
                        return line_rate * 100
            logger.warning(f"No specific coverage data found for Python module '{target_identifier}' in report.")
            return 0.0
            ...
            if AIOFILES_AVAILABLE:
                ...
                content = await asyncio.to_thread(lambda: open(coverage_report_full_path, 'r', encoding='utf-8').read())
            ...
            if isinstance(file_block, dict) and 'lines' in file_block and 'pct' in file_block['lines']:
            ...
            logger.warning(f"No specific coverage data found for JS/TS file '{target_identifier}' in report.")
            return 0.0
            ...
            for counter_element in root.findall(".//class[@name='{}']/counter[@type='LINE']".format(target_identifier.replace('.', '/'))):
            ...
            logger.warning(f"No specific coverage data found for Java class '{target_identifier}' in report.")
            return 0.0
        ...
        # --- fix Rust LCOV parsing ---
            ...
            logger.error(f"Unsupported coverage report format/language for parsing: {language}/{os.path.basename(coverage_report_full_path)}")
            return 0.0

    except (FileNotFoundError, ET.ParseError, json.JSONDecodeError) as e:
        logger.error(f"Failed to parse coverage report '{coverage_report_full_path}': {e}", exc_info=True)
        return 0.0
    except Exception as e:
        logger.error(f"An unexpected error occurred while parsing coverage report delta for {target_identifier}: {e}", exc_info=True)
        return 0.0

@zero_trust_guard
def scan_for_uncovered_code_from_xml(coverage_xml_relative_path: str, project_root: str) -> List[str]:
    ...
    try:
        if not os.path.exists(full_coverage_xml_path):
            logger.error(f"Coverage XML file not found at '{full_coverage_xml_path}' for initial scan.")
            return []
        ...
        logger.error(f"Failed to parse coverage XML '{full_coverage_xml_path}': {e}", exc_info=True)
        return []
    except Exception as e:
        logger.error(f"An unexpected error occurred while scanning coverage XML: {e}", exc_info=True)
...
@zero_trust_guard
async def prioritize_test_targets(coverage_report_path: str, project_root: str, uncovered_python_modules: List[str], policy_engine) -> List[Dict[str, Any]]:
    """
    ...
    if coverage_report_path.endswith(".xml"):
        ...
        try:
            ...
            for identifier in uncovered_python_modules:
                ...
                allowed, reason = await policy_engine.should_generate_tests(target["identifier"], target["language"])
                if allowed:
                    prioritized_targets.append(target)
                    logger.debug(f"Module '{target['identifier']}' selected for test generation (Priority: {priority_score:.2f}, Current Coverage: {target['current_line_coverage']:.1f}%).")
                else:
                    logger.info(f"Module '{target['identifier']}' skipped due to policy: {reason}")
                    if AUDIT_LOGGER_AVAILABLE:
                        await audit_logger.log_event(
                            event_type="test_generation_skipped",
                            details={"identifier": identifier, "reason": reason, "policy_check": "should_generate_tests"},
                            critical=False
                        )
        except Exception as e:
    ...
    # Rust (conceptual)
        ...
        logger.warning("Rust LCOV parsing is not yet implemented. Skipping Rust coverage prioritization.")
        return []
    else:
        logger.warning(f"Unsupported coverage report format for prioritization: {coverage_report_path}")
...
def init_llm(model_name: str = "gpt-4o", temperature: float = 0.7, api_key: str = None, backend: str = "openai"):
    """
    ...
    if not api_key:
        ...
        if not api_key:
            logger.warning(f"API key for backend '{backend}' not found in '{env_var_name}'. Falling back to mock backend.")
            if AUDIT_LOGGER_AVAILABLE:
                _fire_and_forget(audit_logger.log_event(
    ...
    try:
        if backend == "openai":
            ...
            logger.info(f"Initialized OpenAI client for model '{model_name}' with temperature {temperature}.")
            return client
        elif backend == "anthropic":
            from anthropic import AsyncAnthropic
            client = AsyncAnthropic(api_key=api_key)
            logger.info(f"Initialized Anthropic client for model '{model_name}' with temperature {temperature}.")
            return client
        elif backend == "google":
            from google.generativeai.client import get_default_async_client
            client = get_default_async_client()
            logger.info(f"Initialized Google Generative AI client for model '{model_name}' with temperature {temperature}.")
            return client
            
    except ImportError:
        logger.error(f"LLM backend '{backend}' is not installed. Please run `pip install {backend}`.")
        return init_llm(backend="mock")
    except Exception as e:
        ...
        if AUDIT_LOGGER_AVAILABLE:
            _fire_and_forget(audit_logger.log_event(
                ...
                details={"backend": backend, "model": model_name, "error": str(e), "traceback": traceback.format_exc()},
                critical=True
            ))
        return init_llm(backend="mock")