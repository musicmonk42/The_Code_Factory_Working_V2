...
class TestLoggingInitialization:
    def test_dictconfig_failure_falls_back_with_glyph_parity(self, monkeypatch: pytest.MonkeyPatch, caplog):
        ...
        fake_out = types.SimpleNamespace(encoding="ascii", write=sys.__stdout__.write, flush=lambda: None)
        ...
        with caplog.at_level(logging.INFO):
            ...
            assert "Failed to set up structured logging: Invalid logging_config schema: missing 'handlers'." in text
...
class TestAuditHandler:
    def test_audit_file_path_patched_and_writable(self, tmp_path: pathlib.Path, reload_console, monkeypatch):
        console = reload_console()
...
class TestThreadSafety:
    def test_backend_switch_is_thread_safe(self, monkeypatch: pytest.MonkeyPatch, reload_console):
        ...
        monkeypatch.setattr(f"{console.__name__}.RICH_AVAILABLE", importlib.util.find_spec("rich") is not None)
...
class TestGlyphMapping:
    def test_ascii_vs_utf8_glyphs_via_reload(self, monkeypatch: pytest.MonkeyPatch, caplog):
        ...
        ascii_out = types.SimpleNamespace(encoding="ascii", write=sys.__stdout__.write, flush=lambda: None)
        ...
        utf_out = types.SimpleNamespace(encoding="utf-8", write=sys.__stdout__.write, flush=lambda: None)
...
class TestProgressReporting:
    @pytest.mark.skipif(
        not importlib.util.find_spec("rich"),
        reason="rich library is not installed"
    )
    def test_progress_bar_rich_available(self, reload_console, monkeypatch):
        ...
        with console.log_progress_bars("Overall Progress", tasks) as progress_map:
            ...
            mock_progress_instance.add_task.assert_any_call("Task 1", total=10, description="Overall Progress")
            mock_progress_instance.add_task.assert_any_call("Task 2", total=20, description="Overall Progress")
