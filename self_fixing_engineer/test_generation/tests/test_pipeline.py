...
def orchestrator(project: Path, monkeypatch):
    ...
    monkeypatch.setattr("test_generation.orchestrator.orchestrator._write_sarif_atomically", AsyncMock())
...
async def test_quarantine_on_failing_test(orchestrator, project: Path, monkeypatch):
    gen_summary = {"module1": {"generation_success": True, "generated_test_path": "atco_artifacts/generated/test_module1.py", "language": "python"}}
    ...
    monkeypatch.setattr("test_generation.orchestrator.orchestrator.run_pytest_and_coverage", AsyncMock(return_value=(False, 0.0, "Test failed")))
...
async def test_quarantine_on_low_coverage(orchestrator, project: Path, monkeypatch):
    ...
    test_path = sanitize_path("atco_artifacts/generated/test_module1.py", str(project))
    (project / test_path).write_text("def test_dummy(): assert True")
    monkeypatch.setattr("test_generation.orchestrator.orchestrator.run_pytest_and_coverage", AsyncMock(return_value=(True, 0.0, "Passed")))
    result = await orchestrator.integrate_and_validate_generated_tests(gen_summary)
    assert result["summary"]["total_quarantined"] == 1
...
async def test_quarantine_on_security_issues(orchestrator, project: Path, monkeypatch):
    ...
    test_path = sanitize_path("atco_artifacts/generated/test_module1.py", str(project))
    (project / test_path).write_text("def test_dummy(): assert True")
    mock_scanner = Mock(spec=SecurityScanner, scan_test_file=AsyncMock(return_value=(True, [{"text": "Issue"}], "HIGH")))
...
async def test_quarantine_on_low_mutation_score(orchestrator, project: Path, monkeypatch):
    ...
    test_path = sanitize_path("atco_artifacts/generated/test_module1.py", str(project))
    (project / test_path).write_text("def test_dummy(): assert True")
    mock_tester = Mock(spec=MutationTester, run_mutations=AsyncMock(return_value=(True, 40.0, "Low score")))
    monkeypatch.setattr(orchestrator, "mutation_tester", mock_tester)
    orchestrator.config["mutation_testing"] = {"enabled": True, "min_score_for_integration": 50.0}
    monkeypatch.setattr("test_generation.orchestrator.orchestrator.run_pytest_and_coverage", AsyncMock(return_value=(True, 10.0, "Passed")))
    result = await orchestrator.integrate_and_validate_generated_tests(gen_summary)
    assert result["summary"]["total_quarantined"] == 1
...
async def test_pr_required_stages_file(orchestrator, project: Path, monkeypatch):
    ...
    monkeypatch.setattr(orchestrator.policy_engine, "requires_pr_for_integration", AsyncMock(return_value=(True, "PR required")))
