...
async def test_main_invalid_paths(project: Path, monkeypatch):
    args = Mock(project_root=str(project), config_file="../evil.json", suite_dir="tests", coverage_xml="coverage.xml",
                treat_review_required_as_success=False, abort_on_critical=False, enable_html_report=False)
...
async def test_main_config_loading(project: Path, monkeypatch):
    args = Mock(project_root=str(project), config_file="atco_config.json", suite_dir="tests", coverage_xml="coverage.xml",
                treat_review_required_as_success=False, abort_on_critical=False, enable_html_report=False)
    ...
    mock_orchestrator.return_value.integrate_and_validate_generated_tests = AsyncMock(return_value={"summary": {}})
    monkeypatch.setattr("test_generation.orchestrator.cli.GenerationOrchestrator", mock_orchestrator)
...
def test_graceful_shutdown(monkeypatch):
    ...
    mock_log.assert_called_with("Received signal 2. Initiating graceful shutdown...", level="WARNING")
