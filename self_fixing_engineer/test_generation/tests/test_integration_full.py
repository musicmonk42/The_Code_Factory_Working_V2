...
async def test_full_cli_run_with_mocked_agents(tmp_path, runner):
    ...
    with \
        patch("test_generation.gen_agent.cli.run_dependency_check", new=AsyncMock(return_value=None)), \
        patch("test_generation.gen_agent.cli.init_llm", new=AsyncMock(return_value=MagicMock())), \
        patch("test_generation.gen_agent.cli.ensure_session_file", new=AsyncMock(return_value=mocked_session_state)), \
        ...
        patch("test_generation.gen_agent.io_utils.append_to_feedback_log", new=AsyncMock()) as mock_append, \
        patch("sys.exit") as mock_exit:  # Prevent actual sys.exit calls

        # Ensure sys.exit doesn't actually exit
        mock_exit.side_effect = lambda code: None

        args = [
            "--project-root", str(tmp_path),   # group option must come before subcommand
            "generate",
            "--session", session_id,
...
async def test_graph_with_real_agent_mocks(tmp_path):
    ...
    with patch("test_generation.gen_agent.graph.planner_agent", fake_planner), \
         ...
         patch("test_generation.gen_agent.graph.adaptive_test_executor_agent", fake_executor), \
