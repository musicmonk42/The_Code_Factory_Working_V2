...
async def test_e2e_happy_and_quarantine_paths(project: Path, config, monkeypatch):
    # Mock dependencies
    mock_policy = Mock(spec=PolicyEngine, should_integrate_test=AsyncMock(return_value=(True, "Allowed")), requires_pr_for_integration=AsyncMock(return_value=(False, "No PR")))
    mock_scanner = Mock(spec=SecurityScanner, scan_test_file=AsyncMock(return_value=(False, [], "NONE")))
    mock_pr = Mock(spec=PRCreator, create_pr=AsyncMock(return_value=(True, "http://pr")))
    mock_tester = Mock(spec=MutationTester, run_mutations=AsyncMock(return_value=(True, 80.0, "Passed")))
    ...
    monkeypatch.setattr("test_generation.utils.create_and_install_venv", AsyncMock(return_value=(True, "path/to/python")))
...
async def test_e2e_cli_main(project: Path, config, monkeypatch):
    ...
    monkeypatch.setattr("test_generation.utils.monitor_and_prioritize_uncovered_code", AsyncMock(return_value=[{"identifier": "module1", "language": "python", "priority": 100, "current_line_coverage": 0.0}]))
    
    # We must mock the run_pytest function to prevent it from trying to run
    monkeypatch.setattr("test_generation.utils.run_pytest_and_coverage", AsyncMock(return_value=(True, 10.0, "Passed")))
    
    monkeypatch.setattr("test_generation.utils.SecurityScanner.scan_test_file", AsyncMock(return_value=(False, [], "NONE")))
    # Standardize mock path for PRCreator
    monkeypatch.setattr("test_generation.utils.PRCreator.create_pr", AsyncMock(return_value=(True, "http://pr")))
    ...
    with patch('test_generation.orchestrator.cli.GenerationOrchestrator', return_value=mock_orchestrator):
