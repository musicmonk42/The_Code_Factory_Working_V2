...
def mock_subprocess_exec_with_json_output():
    ...
    mock_proc.communicate.return_value = (
        b'{"results": "SECURITY REPORT"}',
        b''
    )
    mock_proc.returncode = 0
    with patch("test_generation.gen_agent.agents.asyncio.create_subprocess_exec", return_value=mock_proc) as mock_exec:
        yield mock_exec
...
async def test_generator_agent_strips_code_fences():
    ...
    state = {"language": "python", "framework": "pytest", "code_under_test": "print(123)"}
...
async def test_security_agent_runs_bandit(mock_subprocess_exec_with_json_output):
    ...
    with patch("test_generation.gen_agent.agents.BANDIT_AVAILABLE", True), \
         patch("test_generation.gen_agent.agents.shutil.which", return_value="/usr/bin/bandit"):
...
async def test_sanitization_fallback(monkeypatch):
    ...
    state = {"code_under_test": "<script>alert(1)</script>", "language": "python", "framework": "pytest"}

    # Patch the audit_logger to prevent errors
    with patch("test_generation.gen_agent.agents.audit_logger.log_event", new_callable=AsyncMock), \
