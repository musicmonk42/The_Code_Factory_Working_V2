...
async def test_audit_with_arbiter(monkeypatch):
    ...
    mock_arbiter.log_event.assert_called_once_with(event_type="test_event", details=ANY, critical=False, run_id=RUN_ID)
...
async def test_audit_fallback(monkeypatch, caplog):
    ...
    assert any(f'{{"event": "test_event", "level": "INFO", "key": "value", "run_id": "{RUN_ID}"' in record.message for record in caplog.records)
