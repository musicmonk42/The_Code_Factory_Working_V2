...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"retry_count": 0, "timeout": 60},
            ValueError,
        ),  # Invalid char in target_id
        (
            "valid.module",
            "../invalid",
            {"retry_count": 0, "timeout": 60},
            ValueError,
        ),  # Path traversal in output_path
        (
            "valid.module",
            "",
            {"retry_count": 0, "timeout": 60},
        ...
        (
            "valid.module",
            "output/dir",
            {"retry_count": "invalid", "timeout": 60},
            ValueError,
        ),  # Non-int retry_count
    ],
)
...
async def test_jest_llm_backend_generate_success(
    ...
    with (
        patch.object(backend.llm, "ainvoke") as mock_ainvoke,
...
@pytest.mark.asyncio
async def test_jest_llm_backend_generate_retry_exceeded(
    mock_config, temp_project_root, monkeypatch
):
...
@pytest.mark.asyncio
async def test_diffblue_backend_generate_simulated_failure(
    mock_config, temp_project_root, monkeypatch
):
