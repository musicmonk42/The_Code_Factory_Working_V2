...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"api_key": "abc123", "nested": {"secret": "hidden"}}, {"api_key": "[REDACTED]", "nested": {"secret": "[REDACTED]"}}),  # Nested
...
def test_policy_engine_init_missing_file(temp_project_root):
    ...
    assert engine.policies["generation_rules"]["allowed_languages"] == ["python", "javascript", "java", "typescript", "rust", "go"]

@pytest.mark.asyncio
async def test_policy_engine_should_generate_tests_local_allowed(mock_policy_file, temp_project_root):
...
@pytest.mark.asyncio
async def test_policy_engine_should_generate_tests_local_denied_regulated(mock_policy_file, temp_project_root):
...
@pytest.mark.asyncio
async def test_policy_engine_should_generate_tests_opa_enabled(mock_policy_file, temp_project_root, mock_config):
...
async def test_policy_engine_should_generate_tests_opa_failure(mock_policy_file, temp_project_root):
    ...
    with patch("test_generation.policy_and_audit.aiohttp.ClientSession.post", side_effect=Exception("OPA error")):
        allowed, reason = await engine.should_generate_tests("module.py", "python")
        assert not allowed
        assert "OPA policy evaluation failed" in reason

@pytest.mark.asyncio
async def test_policy_engine_should_integrate_test_local_allowed(mock_policy_file, temp_project_root):
...
@pytest.mark.asyncio
async def test_policy_engine_should_integrate_test_local_denied_quality(mock_policy_file, temp_project_root):
...
@pytest.mark.asyncio
async def test_policy_engine_requires_pr_for_integration_local_required(mock_policy_file, temp_project_root):
...
@pytest.mark.asyncio
async def test_policy_engine_requires_pr_for_integration_local_not_required(mock_policy_file, temp_project_root):
...
async def test_audit_logger_log_event(temp_project_root):
    ...
    with patch.object(logger.dlt_logger, "add_entry", AsyncMock()) as mock_dlt_add, \
         patch("logging.getLogger") as mock_local_logger:
...
async def test_audit_logger_log_event_redaction(temp_project_root):
    ...
    with patch.object(logger.dlt_logger, "add_entry", AsyncMock()) as mock_dlt_add:
        await logger.log_event("test_event", {"password": "secret", "nested": {"api_key": "key123"}})
        
        call_args = mock_dlt_add.call_args[0]
        assert call_args[2] == {"password": "[REDACTED]", "nested": {"api_key": "[REDACTED]"}}
...
async def test_event_bus_publish_metrics_failure(mock_config):
    ...
    with patch("test_generation.policy_and_audit.aiohttp.ClientSession.post", side_effect=Exception("Network error")), \
...
async def test_event_bus_publish_redaction(mock_config):
    ...
    with patch("test_generation.policy_and_audit.aiohttp.ClientSession.post") as mock_post:
        ...
        await bus.publish("test_event", {"password": "secret", "module": "test_module", "reason": "test_reason"})
        call_args = mock_post.call_args[1]["json"]
