...
class TestSanitizePath:
    ...
    def test_symlink_escape_rejected(self, venvs, project: pathlib.Path, tmp_path: pathlib.Path, monkeypatch):
...
# -----------------------------
@pytest.mark.asyncio
async def test_retry_and_jitter_bounds(monkeypatch: pytest.MonkeyPatch, venvs, project: pathlib.Path):
...
# -----------------------------
@pytest.mark.asyncio
async def test_cancel_during_backoff_sleep(monkeypatch: pytest.MonkeyPatch, venvs, project: pathlib.Path):
    # Mock venv.create to succeed, so we get to the `pip install` part
    def mock_venv_create(path, *args, **kwargs):
        pathlib.Path(path, "Scripts").mkdir(parents=True)
        pathlib.Path(path, "Scripts", "pip.exe").touch()
    ...
    cfg = dict(venvs.CONFIG)
    cfg.update({"venv_creation_retries": 2, "venv_retry_backoff_min": 0.01, "venv_retry_backoff_max": 0.02})
    monkeypatch.setattr(venvs, "CONFIG", cfg, raising=False)
    monkeypatch.setattr(venvs.utils, "cleanup_temp_dir", AsyncMock())
    ...
    assert any(
        "venv_creation_cancelled" in str(r) for r in records
    )
...
async def test_persist_vs_cleanup(monkeypatch: pytest.MonkeyPatch, venvs, project: pathlib.Path):
    ...
    # FIX: Correct the typo from `temporary_venv` to `temporary_env`
    temp_dir_path = ""
    async with venvs.temporary_env(str(project), language="python", required_deps=[], persist=True) as py:
    ...
    temp_dir_path = ""
    async with venvs.temporary_env(str(project), language="python", required_deps=[], persist=False) as py:
        assert isinstance(py, venvs.EnvHandle)
        temp_dir_path = py.root_path
...
# -----------------------------
@pytest.mark.asyncio
async def test_keep_on_failure_prevents_cleanup(monkeypatch: pytest.MonkeyPatch, venvs, project: pathlib.Path):
    calls = {"cleanup": 0}
    ...
    monkeypatch.setattr(subprocess, "run", Mock(side_effect=subprocess.CalledProcessError(1, 'cmd', stderr='mocked failure')))

    # `keep_on_failure=True`
    with pytest.raises(RuntimeError):
        # FIX: Correct the typo from `temporary_venv` to `temporary_env`
        async with venvs.temporary_env(str(project), language="python", required_deps=["non-existent-package"], keep_on_failure=True):
            pass
    ...
    with pytest.raises(RuntimeError):
        # FIX: Correct the typo from `temporary_venv` to `temporary_env`
        async with venvs.temporary_env(str(project), language="python", required_deps=["non-existent-package"], keep_on_failure=False):
            pass
...
# -----------------------------
@pytest.mark.asyncio
async def test_timeout_and_dependency_sanitation(monkeypatch: pytest.MonkeyPatch, venvs, project: pathlib.Path):
    ...
    # FIX: Correct the typo from `temporary_venv` to `temporary_env`
    async with venvs.temporary_env(str(project), language="python", required_deps=bad_deps) as handle:
