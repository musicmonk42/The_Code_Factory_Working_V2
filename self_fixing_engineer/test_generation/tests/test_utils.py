...
def test_atco_config_init_success(temp_project_root, mock_config):
    ...
    assert config.QUARANTINE_DIR == str(Path(temp_project_root) / "atco_artifacts/quarantined_tests")
...
async def test_security_scanner_bandit_success(temp_project_root, mock_config):
    ...
    with patch("asyncio.create_subprocess_exec") as mock_exec:
        ...
        mock_process.communicate.return_value = (
            json.dumps({"results": [{"issue_severity": "HIGH", "issue_text": "Hardcoded secret", "line_number": 1}]}).encode(),
            b""
        )
...
async def test_mutation_tester_success(temp_project_root, mock_config):
    ...
    with patch('random.random', return_value=0.0):
        success, score, log = await tester.run_mutations("source.py", "test.py",
    "python")
        assert success
        assert score >= 0
...
async def test_run_pytest_and_coverage_success(temp_project_root, mock_config):
    ...
    with patch("os.path.exists", return_value=True), \
         patch("asyncio.create_subprocess_exec") as mock_exec, \
         patch("test_generation.utils.parse_coverage_delta", AsyncMock(return_value=80.0)):
        mock_process = AsyncMock()
        ...
        success, coverage, log = await run_pytest_and_coverage(
            "/mock/venv/bin/python", "test.py", "module1", temp_project_root,
            "coverage.xml", mock_config
        )
        assert success
...
async def test_run_jest_and_coverage_success(temp_project_root, mock_config):
    ...
    with patch("shutil.which", return_value="/usr/bin/npm"), \
         patch("os.path.exists", return_value=True), \
         patch("asyncio.create_subprocess_exec") as mock_exec, \
         patch("test_generation.utils.parse_coverage_delta", AsyncMock(return_value=75.0)):
        mock_process = AsyncMock()
        mock_process.communicate.return_value = (b"", b"")
        mock_process.returncode = 0
        mock_exec.return_value = mock_process
...
async def test_run_junit_and_coverage_success(temp_project_root, mock_config):
    ...
    with patch("os.path.exists", return_value=True), \
         patch("asyncio.create_subprocess_exec") as mock_exec, \
         patch("test_generation.utils.parse_coverage_delta", AsyncMock(return_value=85.0)):
        mock_process = AsyncMock()
...
@pytest.mark.asyncio
async def test_monitor_and_prioritize_uncovered_code(temp_project_root, mock_policy_engine, mock_config):
    ...
    with patch("test_generation.utils.scan_for_uncovered_code_from_xml", return_value=["module1"]):
        targets = await monitor_and_prioritize_uncovered_code("coverage.xml", mock_policy_engine, temp_project_root, mock_config)
...
@pytest.mark.asyncio
async def test_monitor_and_prioritize_uncovered_code_policy_denied(temp_project_root, mock_policy_engine, mock_config):
