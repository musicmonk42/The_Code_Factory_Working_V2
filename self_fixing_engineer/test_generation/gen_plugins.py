...
def _limit_tests_per_function(blocks: Dict[str, List[str]], max_per_fn: int) -> List[str]:
    ...
    for fname, cases in blocks.items():
        ...
        ranked = sorted(
            cases,
            key=lambda s: hashlib.sha256(f"{fname}::{s}".encode()).hexdigest()
        )
...
class PythonTestGenerator(BaseTestGenerator):
    def generate(self, code: str, config: dict) -> List[str]:
        ...
        def _generate_cached(code: str, cfg_key: str) -> Dict[str, List[str]]:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef) and isinstance(parent.get(node), ast.Module):
                    ...
                    class_name = "Test" + "".join(part.capitalize() for part in sanitized_fname.split("_"))
                    ...
                    if framework == "unittest":
                        ...
                        body.append(f"        self.assertIsNotNone({fname}({', '.join(arg_values)}))")
                        ...
                        if any(a.arg == "num" for a in node.args.args):
                            logger.info(f"Generating numeric boundary tests for {fname}", extra={"code_hash": code_hash})
                        ...
                        if any(a.arg == "text" for a in node.args.args):
                            logger.info(f"Generating string boundary tests for {fname}", extra={"code_hash": code_hash})
        ...
        header_template = "import pytest" if config.get("test_framework", DEFAULT_PYTHON_TEST_FRAMEWORK) == "pytest" else "import unittest"
        ...
        needs_mocking = any(dep in code for dep in config.get("dependencies_to_mock", ["requests.get"]))
        if needs_mocking and config.get("test_framework", DEFAULT_PYTHON_TEST_FRAMEWORK) == "unittest":
            header_template += "\nfrom unittest.mock import patch"
        
        if needs_mocking:
            limited_blocks.append("class MockResponse:\n    def json(self): return {}\n    def raise_for_status(self): pass")
                ...
                for node in getattr(tree, "body", []):
                    if getattr(node, "type", "") == "FunctionDeclaration":
                        ...
                        if framework == "jest":
                            ...
                            if any(dep in code for dep in dependencies_to_mock) and "fetch" in dependencies_to_mock:
                                test_blocks[fname].append(
                                    f'describe("{fname}", () => {{\n'
                                    f'  test("mocked fetch", () => {{\n'
                            ...
                            if any(dep in code for dep in dependencies_to_mock) and "axios" in dependencies_to_mock:
                                test_blocks[fname].append(
                                    f'describe("{fname}", () => {{\n'
                                    f'  test("mocked axios", () => {{\n'
                        ...
                        for decl in node.declarations:
                            if getattr(decl, "type", "") == "VariableDeclarator" and \
                               getattr(getattr(decl, "init", None), "type", "") in ("FunctionExpression", "ArrowFunctionExpression"):
                                fname = decl.id.name
                                if fname:
                                    if framework == "jest":
                                        test_blocks[fname].append(
...
def _call_ai_for_tests(code: str, language: str, config: dict) -> List[str]:
    ...
    try:
        ...
        logger.info("Successfully generated tests via AI.", extra={"test_count": len(response.get("tests", []))})
...
def generate_tests(
    ...
    if len(code) > 10_000:
        return {"status": "error", "tests": [], "count": 0, "error": "Input code too large", "metrics": {"generation_time": time.time() - start}}
    
    code_hash = hashlib.sha256(code.encode('utf-8')).hexdigest()
    
    dangerous_constructs = ["__import__", "eval", "exec", "open(", "os.system", "subprocess.run"]
    if config.get("strict_source_filter", False):
        if any(x in code for x in dangerous_constructs):
            logger.warning("Code contains potentially dangerous constructs.", extra={"code_hash": code_hash})
            return {"status": "error", "tests": [], "count": 0, "error": "Code contains potentially dangerous constructs", "metrics": {"generation_time": time.time() - start}}
    
    if language not in SUPPORTED_LANGUAGES:
        return {"status": "error", "tests": [], "count": 0, "error": f"Language '{language}' is not supported. Supported: {list(SUPPORTED_LANGUAGES)}.", "metrics": {"generation_time": time.time() - start}}

    framework = config.get("test_framework", DEFAULT_PYTHON_TEST_FRAMEWORK if language == "python" else DEFAULT_JS_TEST_FRAMEWORK)
    if framework not in SUPPORTED_FRAMEWORKS.get(language, frozenset()):
        return {"status": "error", "tests": [], "count": 0, "error": f"Unsupported test framework '{framework}' for language '{language}'.", "metrics": {"generation_time": time.time() - start}}
    ...
    module_path = config.get("module_path")
    if module_path:
        if config.get("strict_source_filter", False) and any(x in module_path for x in dangerous_constructs):
            logger.error("Invalid module_path due to dangerous constructs.", extra={"code_hash": code_hash, "module_path": module_path})
            return {"status": "error", "tests": [], "count": 0, "error": "Invalid module_path due to dangerous constructs", "metrics": {"generation_time": time.time() - start}}
        safe_name = module_path.replace(".", "_").replace("/", "_")
        if not safe_name.isidentifier():
            logger.error("Invalid module_path due to non-identifier characters.", extra={"code_hash": code_hash, "module_path": module_path})
        ...
        try:
            ...
            logger.error("Invalid module path.", extra={"code_hash": code_hash, "module_path": module_path})
            return {"status": "error", "tests": [], "count": 0, "error": "Invalid module path", "metrics": {"generation_time": time.time() - start}}
    ...
    if not tests and config.get("fallback_to_internal", True):
        ...
        try:
            ...
            return {"status": "error", "tests": [], "count": 0, "error": error_reason, "error_details": details, "metrics": {"generation_time": time.time() - start}}
        except Exception:
            error_reason = "An unexpected internal error occurred."
            logger.exception(error_reason)
            return {"status": "error", "tests": [], "count": 0, "error": error_reason, "metrics": {"generation_time": time.time() - start}}
    ...
    logger.info(f"Test generation request processed. Source hash: {code_hash}, Test count: {len(valid_tests)}, Status: {status}")
