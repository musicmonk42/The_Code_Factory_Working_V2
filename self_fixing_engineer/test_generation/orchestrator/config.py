...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "source_dirs": {"python": ["src", "app"], "javascript": ["src/js", "src/ts"], "java": ["src/main/java"], "rust": ["src"], "go": ["src"]},
    ...
    "slack_webhook_url": None, "slack_events": ["test_quarantined", "test_requires_pr", "security_alert"],
    "webhook_hooks": {}, "webhook_events": ["atco_run_finished"],

    # --- Policies & Scans ---
    "policy_config_path": "atco_policies.json",
    "security_scan_threshold": "NONE",
    "enrichment_plugins": {"header_enabled": True, "mocking_import_enabled": True, "llm_refinement_enabled": False},
    ...
    "test_generation_backends": {
        "python": {"module": "test_generation.backends", "class": "PynguinBackend", "isolation_strategy": "virtualenv", "required_deps": ["pytest", "pytest-cov"]},
        "javascript": {"module": "test_generation.backends", "class": "JestLLMBackend", "isolation_strategy": "default", "required_deps": ["jest", "ts-jest"]},
        "typescript": {"module": "test_generation.backends", "class": "JestLLMBackend", "isolation_strategy": "default", "required_deps": ["jest", "ts-jest"]},
        "java": {"module": "test_generation.backends", "class": "DiffblueBackend", "isolation_strategy": "default", "required_deps": []},
        "rust": {"module": "test_generation.backends", "class": "CargoBackend", "isolation_strategy": "default", "required_deps": []},
        "go": {"module": "test_generation.backends", "class": "GoBackend", "isolation_strategy": "default", "required_deps": []}
    },
...
CONFIG_SCHEMA = {
    ...
    "properties": {
        ...
        "compliance_reporting": {"type": "object", "properties": {"enabled": {"type": "boolean"}}, "additionalProperties": False},
        "backend_timeouts": {"type": "object", "patternProperties": {".*": {"type": "number", "minimum": 1}}},
        "source_dirs": {"type": "object", "patternProperties": {".*": {"type": "array", "items": {"type": "string"}}}},
        "prioritization_rules": {"type": "object", "properties": {"language_boosts": {"type": "object", "patternProperties": {".*": {"type": "number", "minimum": 0}}, "additionalProperties": False}}, "additionalProperties": False},
        "jira_integration": {"type": "object", "properties": {"enabled": {"type": "boolean"}, "api_url": {"type": "string"}, "project_key": {"type": "string"}}, "additionalProperties": False},
        "pr_integration": {"type": "object", "properties": {"enabled": {"type": "boolean"}}, "additionalProperties": False},
        ...
        "enrichment_plugins": {"type": "object", "properties": {"header_enabled": {"type": "boolean"}, "mocking_import_enabled": {"type": "boolean"}, "llm_refinement_enabled": {"type": "boolean"}}, "additionalProperties": False},
        "security_scan_sim_critical_issue_chance": {"type": "number"},
        "mutation_testing": {"type": "object", "properties": {"enabled": {"type": "boolean"}, "min_score_for_integration": {"type": "number", "minimum": 0, "maximum": 100}}, "additionalProperties": False},
        "python_venv_deps": {"type": "array", "items": {"type": "string"}},
        "test_generation_backends": {"type": "object", "patternProperties": {".*": {"type": "object", "properties": {"module": {"type": "string"}, "class": {"type": "string"}, "isolation_strategy": {"type": "string"}, "required_deps": {"type": "array", "items": {"type": "string"}}}, "required": ["module", "class"], "additionalProperties": False}}},
        ...
        "console_glyphs": {"type": "object", "properties": {"check": {"type": "string"}, "warn": {"type": "string"}, "x": {"type": "string"}}, "additionalProperties": False},
        "console_ascii_glyphs": {"type": "object", "properties": {"check": {"type": "string"}, "warn": {"type": "string"}, "x": {"type": "string"}}, "additionalProperties": False},
        "log_styles": {"type": "object", "patternProperties": {".*": {"type": "object", "properties": {"console_style": {"type": "string"}}, "required": ["console_style"]}}},
    },
    "additionalProperties": False
}

__all__ = [
    'CONFIG', 'PROJECT_ROOT', 'LOGGING_CONFIG', 'QUARANTINE_DIR',
    'GENERATED_OUTPUT_DIR', 'SARIF_EXPORT_DIR', 'AUDIT_LOG_FILE',
    'COVERAGE_REPORTS_DIR', 'VENV_TEMP_DIR', 'HTML_REPORTS_DIR',
    '_ensure_dir', '_ensure_artifact_dirs', 'load_config'
]
...
def load_config(project_root: str, config_file: str) -> MappingProxyType:
    ...
    try:
        ...
        logging.warning(f"Failed to load or parse '{config_file}'. Using full default configuration. Error: {e}")
    ...
    if user_config_was_loaded:
        try:
            ...
            if errors:
                logging.warning(f"Found {len(errors)} validation error(s) in configuration. Reverting invalid fields to their defaults.")
                ...
                for err in errors:
                    ...
                    if err.validator == "type":
                        ...
                        if unexpected:
                            logging.warning(f"Unexpected config keys at '{'.'.join(map(str, err.path)) or '<root>'}': {unexpected}")
                        else:
                            logging.warning(f"Validation error at '{'.'.join(map(str, err.path)) or '<root>'}': {err.message}")
                    else:
                        logging.warning(f"Validation error at '{'.'.join(map(str, err.path)) or '<root>'}': {err.message}")
    ...
    LOGGING_CONFIG['handlers']['audit_file']['filename'] = os.path.join(
        PROJECT_ROOT, merged_config.get("audit_log_file", DEFAULT_CONFIG["audit_log_file"])
    )
    ...
    def _ensure_artifact_dirs_safe(project_root: str, cfg: Dict[str, Any]) -> None:
        for key in ("generated_output_dir","quarantine_dir","sarif_export_dir",
                    "coverage_reports_dir","html_reports_dir","venv_temp_dir"):
            rel = cfg.get(key)
            if rel:
...
class TestConfigLoading(unittest.TestCase):
    ...
    def test_invalid_config_schema_falls_back(self):
        ...
        with self.assertLogs(level='WARNING') as cm:
            ...
            self.assertTrue(any("Unexpected config keys at '<root>': ['new_key']" in log for log in cm.output))
    ...
    def test_invalid_top_level_config_type(self):
        ...
        with self.assertLogs(level='WARNING') as cm:
            cfg = load_config(self.test_dir, self.config_file)
            self.assertTrue(any("Top-level config must be a JSON object" in log for log in cm.output))
    ...
    def test_non_dict_with_additional_properties_true(self):
        ...
        with self.assertLogs(level='WARNING') as cm:
            cfg = load_config(self.test_dir, self.config_file)
            self.assertTrue(any("'jira_integration' should be of type 'object'" in log for log in cm.output))
        self.assertNotEqual(cfg['jira_integration'], 'not-a-dict')
        self.assertEqual(cfg['jira_integration']['enabled'], DEFAULT_CONFIG['jira_integration']['enabled'])
    ...
    def test_invalid_sub_field_reverts_top_level_field_only(self):
        ...
        with self.assertLogs(level='WARNING') as cm:
            cfg = load_config(self.test_dir, self.config_file)
            self.assertTrue(any("'enabled' should be of type 'boolean'" in log for log in cm.output))
        self.assertEqual(cfg['jira_integration']['enabled'], DEFAULT_CONFIG['jira_integration']['enabled'])
    ...
    def test_deep_merge_partial_override(self):
        ...
        self.assertEqual(cfg["backend_timeouts"]["pynguin"], DEFAULT_CONFIG["backend_timeouts"]["pynguin"])
    ...
    def test_deep_merge_with_invalid_subfield(self):
        ...
        self.assertEqual(cfg['prioritization_rules']['language_boosts']['python'], DEFAULT_CONFIG['prioritization_rules']['language_boosts']['python'])
