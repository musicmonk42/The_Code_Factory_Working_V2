...
from .console import (
    log, RICH_AVAILABLE, console,
    Progress, BarColumn, TimeElapsedColumn, TimeRemainingColumn, TextColumn,
)
# Import venv utilities from same package for isolation.
try:
    from .venvs import sanitize_path as venv_sanitize_path, temporary_env, create_and_install_venv
except ImportError as e:
    logging.getLogger(__name__).error(f"Venvs import failed: {e}. Using stubs.")
    # Basic fallback stubs to prevent crashing
    def venv_sanitize_path(path: str, root: str) -> str: return os.path.abspath(os.path.join(root, path))
    @asynccontextmanager
    async def temporary_env(*args, **kwargs): yield sys.executable
    async def create_and_install_venv(*args, **kwargs): return False, str(sys.exc_info()[1])

# Metrics from same package for monitoring.
try:
    from .metrics import generation_duration, integration_success, integration_failure, METRICS_AVAILABLE
except ImportError:
    class DummyMetric:
        def labels(self, **kwargs): return self
        def observe(self, *args): pass
        def inc(self): pass
        def time(self): return self # Mock the context manager behavior
        def __enter__(self): pass # Add __enter__ for context manager
        def __exit__(self, exc_type, exc_val, exc_tb): pass # Add __exit__ for context manager
...
try:
    ...
    async def _write_sarif_atomically(path, data): return True
    async def cleanup_old_temp_files(*args, **kwargs): pass
...
from test_generation.utils import (
    SecurityScanner, KnowledgeGraphClient, PRCreator, MutationTester, CodeEnricher,
    add_atco_header, add_mocking_framework_import, llm_refine_test_plugin,
    cleanup_path_safe, run_jest_and_coverage, run_junit_and_coverage,
...
def _append_local_audit_log(project_root, config, event_name, detail):
    try:
        ...
        log(f"Arbiter audit fallback file write failed for event '{event_name}': {e}", level="WARNING")
...
async def _execute_test_command(
    ...
    if language == 'python':
        return await run_pytest_and_coverage(
            venv_path, test_path_relative, target_identifier, project_root, coverage_report_path, config
        )
    elif language in ['javascript', 'typescript']:
        return await run_jest_and_coverage(
            project_root, test_path_relative, target_identifier, coverage_report_path, config
        )
    elif language == 'java':
...
class GenerationOrchestrator:
    ...
    def __init__(self, config: Dict[str, Any], project_root: str, suite_dir: str):
        ...
        self.language_semaphores = defaultdict(lambda: asyncio.Semaphore(self.config.get('demo_per_lang_concurrency', 2) if self.config.get('is_demo_mode') else self.config.get('per_lang_concurrency', 4)))
        ...
        self.knowledge_graph_client = self._load_component("knowledge_graph_client", "knowledge_graph_config")
    ...
    async def run_pipeline(self, coverage_xml: str) -> dict:
        ...
        targets = await utils.monitor_and_prioritize_uncovered_code(coverage_xml, self.policy_engine, self.project_root, self.config)
    ...
    def _load_component(self, component_name: str, config_key: str, **kwargs) -> Any:
        ...
        try:
            ...
            if component_name == "policy_engine":
                ...
                log(f"Warning: Unknown component '{component_name}' requested in _load_component. Returning Mock.", level="WARNING")
                return Mock()
        except Exception as e:
            log(f"CRITICAL: Failed to initialize {component_name}: {e}. This is a critical dependency.", level="CRITICAL")
    ...
    def _load_test_enricher(self):
        ...
        try:
            ...
            log(f"Warning: Failed to initialize CodeEnricher: {e}. Using a pass-through stub.", level="WARNING")
    ...
    async def generate_tests_for_targets(
        ...
        if not targets:
            ...
            await _maybe_await(audit_event("pipeline_step", {
                "step": "generation",
                "result": "skipped",
                "reason": "no targets",
        ...
        log(f"Starting test generation for {len(targets)} targets in parallel...", level="INFO")
        await _maybe_await(audit_event("pipeline_step", {
            "step": "generation",
            "result": "started",
        ...
        if RICH_AVAILABLE and console:
            ...
            self.progress_task_id = self.rich_progress.add_task("Generating tests...", total=len(targets))
        ...
        async def _handle_single_test_generation(target_data: Dict[str, Any]) -> tuple[str, bool, str, Any | None]:
            backend_class = BackendRegistry.get_backend(target_data["language"])
            if not backend_class:
                return target_data["identifier"], False, f"No backend registered for language {target_data['language']}", None
            ...
            for i in range(retry_settings["generation_retries"] + 1):
                with generation_duration.labels(language=target_data["language"]).time():
                    try:
                        sanitized_output_path = venv_sanitize_path(output_base_relative, self.project_root)
                        ...
                        success, err, test_path_relative = await backend_instance.generate_tests(
                            ...
                            params={"retry_count": i, "timeout": timeout, **target_data}
                        )
                    except Exception as e:
                        success, err, test_path_relative = False, f"Backend failed with unhandled exception: {e}", None
                        log(f"Backend failed for '{target_data['identifier']}': {e}", level="ERROR")
                        await _maybe_await(audit_event("backend_run_failure", {
                            "target": target_data['identifier'],
                            "error": str(e),
                            "traceback": traceback.format_exc(),
                            "timestamp": datetime.now(timezone.utc).isoformat()
                        }, run_id=RUN_ID, critical=True))
                        continue

                if success and test_path_relative:
                    log(f"Generated test for '{target_data['identifier']}' ({target_data['language']}) successfully.", level="SUCCESS")
                ...
                if i < retry_settings["generation_retries"]:
                    log(f"Generation failed for '{target_data['identifier']}' ({target_data['language']}). Retrying ({i+1}/{retry_settings['generation_retries']}): {err}", level="WARNING")
                    integration_failure.labels(language=target_data["language"]).inc()
                    await asyncio.sleep(random.uniform(retry_settings["retry_backoff_min"], retry_settings["retry_backoff_max"]) * (i + 1))

            log(f"Test generation ultimately failed for '{target_data['identifier']}' ({target_data['language']}) after all retries.", level="ERROR")
            integration_failure.labels(language=target_data["language"]).inc()
            return target_data["identifier"], False, f"Failed after {retry_settings['generation_retries']+1} attempts: {err}", None
        ...
        for target in targets:
            ...
            task = asyncio.create_task(_generate_with_semaphore(lang_semaphore, _handle_single_test_generation(target)))
            tasks.append(task)
        ...
        for res in results:
            if isinstance(res, Exception):
                ...
                await _maybe_await(audit_event("generation_task", {
            ...
            original_target = next((t for t in targets if t['identifier'] == module_identifier), None)
    ...
    async def integrate_and_validate_generated_tests(
        ...
        try:
            ...
            sarif_dir = venv_sanitize_path(self.config.get('sarif_export_dir', SARIF_EXPORT_DIR), self.project_root)
            ...
            try:
                ...
                log(f"Warning: Could not initialize audit log file at '{audit_rel}': {_e}", level="WARNING")
            # ---------------------------------------------------------------------------

        except Exception as e:
            log(f"Failed to create required directories or cleanup temporary files: {e}", level="CRITICAL")
            await _maybe_await(audit_event("dir_creation_failure", {
                "error": str(e),
                "traceback": traceback.format_exc(),
                "timestamp": datetime.now(timezone.utc).isoformat()
            }, run_id=RUN_ID, critical=True))
        ...
        for module_identifier, gen_data in generation_summary.items():
            if not gen_data.get("generation_success"):
                overall_results["details"][module_identifier] = {
                    ...
                    "reason": gen_data.get("generation_error", "Generated test file not found or empty."),
                    "test_passed": False, "coverage_increase_percent": 0.0,
                    "security_issues_found": False, "security_issues_list": [], "security_max_severity": "NONE",
                    "backup_path": "", "sarif_artifact_path": "", "final_integrated_test_hash": "N/A"
                }
                overall_results["summary"]["total_not_generated"] += 1
                log(f"Skipping integration for '{module_identifier}': Test not generated successfully or file missing.", level="WARNING")
                continue
            ...
            try:
                needs_pr, pr_msg = await self.policy_engine.requires_pr_for_integration(generated_test_path_relative)
            except Exception as e:
            ...
            if needs_pr:
                overall_results["details"][module_identifier] = {
                    ...
                    "security_issues_found": False, "security_issues_list": [], "security_max_severity": "NONE",
                    "backup_path": "", "sarif_artifact_path": "", "final_integrated_test_hash": "N/A",
                    "staged_path": None, "audit_event_id": str(uuid.uuid4()), "quarantine_log_path": None
                }
        ...
        for e in errors:
            ...
            await _maybe_await(audit_event("integration_task_error", {
                "error": str(e),
                "traceback": "".join(traceback.format_exception(type(e), e, e.__traceback__)),
                "timestamp": datetime.now(timezone.utc).isoformat()
            }, run_id=RUN_ID, critical=True))
        integration_results_list = [r for r in integration_results_list if not isinstance(r, Exception)]
        ...
        refinement_success_rate = (
            ...
            if total_refinement_attempts > 0 else 0.0
        )
        ...
        await _maybe_await(audit_event("integration_results", {
            ...
            "total_targets_considered": overall_results["summary"]["total_targets_considered"],
        }, run_id=RUN_ID))
        try:
            _append_local_audit_log(
                self.project_root, self.config, "integration_results",
                {
                    ...
                    "total_targets_considered": overall_results["summary"].get("total_targets_considered", 0),
                },
            )
        except Exception:
            pass
    ...
    def _calculate_test_quality_score(self, test_passed: bool, coverage_increase: float, mutation_score: float) -> float:
    ...
    async def _handle_single_test_integration(
        ...
        try:
            ...
            log(f"Failed to validate path for {target_identifier}: {e}", level="CRITICAL")
            await _maybe_await(audit_event("test_integration_failure", {
        ...
        if missing_or_empty:
            ...
            await _maybe_await(audit_event("test_quarantined", {
                 "module": target_identifier,
            ...
            try:
                _append_local_audit_log(
                    self.project_root, self.config, "test_quarantined",
                    {
                        "module": target_identifier,
        ...
        try:
            async with aiofiles.open(full_src_test_path, 'r', encoding='utf-8', errors='ignore') as f:
                original_test_content = await f.read()
            enriched_test_content = await _maybe_await(self.test_enricher.enrich_test(original_test_content, language, self.project_root))
            if enriched_test_content != original_test_content:
                ...
                log(f"Test file '{os.path.basename(full_src_test_path)}' enriched.", level="INFO")
            else:
                log(f"Test file '{os.path.basename(full_src_test_path)}' not modified by enrichers.", level="INFO")
            await _maybe_await(audit_event("test_enrichment", {
                 "module": target_identifier,
                 "action": "enrichment_completed",
                 "timestamp": datetime.now(timezone.utc).isoformat()
            }, run_id=RUN_ID))
        except Exception as e:
            log(f"Error enriching test {os.path.basename(full_src_test_path)}: {e}", level="ERROR")
            log(f"Enrichment Traceback:\n{traceback.format_exc()}", level="ERROR")
            await _maybe_await(audit_event("test_enrichment_failure", {
                "module": target_identifier,
                "error": str(e),
                "traceback": traceback.format_exc(),
                "timestamp": datetime.now(timezone.utc).isoformat()
        ...
        if language in ["javascript", "typescript"]: coverage_report_filename = f"coverage_{os.path.basename(src_test_path_relative).replace('.py','').replace('.js','').replace('.java','')}_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}_{random.randint(0,9999)}.json"

        temp_coverage_report_path_relative = os.path.join(COVERAGE_REPORTS_DIR, coverage_report_filename)
        full_temp_coverage_report_path = venv_sanitize_path(temp_coverage_report_path_relative, self.project_root)
        os.makedirs(os.path.dirname(full_temp_coverage_report_path), exist_ok=True)

        test_passed, coverage_increase, exec_log = False, 0.0, f"No test runner for language: {language}"
        ...
        try:
            ...
            log(f"Exception during test execution for {target_identifier}: {e}", level="ERROR")
            exec_log = f"Exception during execution: {e}"
            test_passed = False
            coverage_increase = 0.0
            await _maybe_await(audit_event("test_execution_failure", {
                "module": target_identifier,
                "error": str(e),
                "traceback": traceback.format_exc(),
                "timestamp": datetime.now(timezone.utc).isoformat()
        ...
        if self.config.get('mutation_testing', {}).get('enabled'):
            try:
                mutation_success, mutation_score, mutation_log = await _maybe_await(self.mutation_tester.run_mutations(
                    target_identifier, src_test_path_relative, language
                ))
            except Exception as e:
                log(f"Exception during mutation testing for {target_identifier}: {e}", level="ERROR")
                mutation_log = f"Exception during mutation testing: {e}"
                mutation_score = -1.0
                mutation_success = False
                await _maybe_await(audit_event("mutation_test_failure", {
                    "module": target_identifier,
                    "error": str(e),
                    "traceback": traceback.format_exc(),
                    "timestamp": datetime.now(timezone.utc).isoformat()
        ...
        if not mutation_success and self.config.get('mutation_testing', {}).get('enabled'):
            log(f"Mutation testing failed for {target_identifier}: {mutation_log}", level="WARNING")


        has_security_issues, security_issues_list, security_max_severity = await _maybe_await(self.security_scanner.scan_test_file(src_test_path_relative, language))
        ...
        if has_security_issues:
            ...
            await _maybe_await(audit_event("security_scan_issues", {
                "module": target_identifier,
                "issues": truncated_issues_list,
                "severity": security_max_severity,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }, run_id=RUN_ID))
            await _maybe_await(self.event_bus.publish("security_alert", {"module": target_identifier, "issues": security_issues_list, "severity": security_max_severity}))
        ...
        test_quality_score = self._calculate_test_quality_score(test_passed, coverage_increase, mutation_score)

        try:
            fn = self.policy_engine.should_integrate_test
            res = fn(target_identifier, test_quality_score, language, has_security_issues, security_max_severity)
            ...
            log(f"Error during policy enforcement for {target_identifier}: {e}", level="CRITICAL")
            integrate_allowed = False
            policy_reason_integrate = f"Policy engine failed: {e}"
            requires_pr = False
            await _maybe_await(audit_event("policy_check_failure", {
                "module": target_identifier,
                "error": str(e),
                "traceback": traceback.format_exc(),
                "timestamp": datetime.now(timezone.utc).isoformat()
            ...
            if self.config.get('abort_on_critical', False):
                raise OrchestrationPipelineError(f"Policy engine failed for {target_identifier}") from e


        min_coverage_gain = self.config.get('policy', {}).get('min_coverage_gain_for_integration', 1.0)
        min_mutation_score = self.config.get('mutation_testing', {}).get('min_score_for_integration', 50.0)

        # FIX: Implement a guard to quarantine the test if the mutation score is too low.
        if not integrate_allowed or not test_passed or coverage_increase < min_coverage_gain or has_security_issues or (mutation_score != -1.0 and mutation_score < min_mutation_score):
            final_quarantine_reason = ""
            if not integrate_allowed: final_quarantine_reason = f"Policy denied integration: {policy_reason_integrate}"
            elif not test_passed: final_quarantine_reason = "Test failed during execution."
            elif coverage_increase < min_coverage_gain: final_quarantine_reason = f"Test generated insufficient coverage ({coverage_increase:.2f}%). Minimum required is {min_coverage_gain}%."
            ...
            result_summary["integration_status"] = "QUARANTINED"
            result_summary["reason"] = final_quarantine_reason or "Test did not meet integration criteria."
            ...
            quarantine_dst_path = venv_sanitize_path(os.path.join(QUARANTINE_DIR, quarantine_name), self.project_root)

            try:
                shutil.move(full_src_test_path, quarantine_dst_path)
            except Exception as e:
                log(f"Error moving file to quarantine for {target_identifier}: {e}. File may be lost.", level="CRITICAL")
                await _maybe_await(audit_event("test_quarantined_failure", {
                     "module": target_identifier,
                     "reason": "move_error",
                     "error": str(e),
                     "traceback": traceback.format_exc(),
            ...
            try:
                ...
                log(f"Full logs for quarantined test saved to: {quarantine_log_path}", level="INFO")
            except Exception as e:
                log(f"Failed to write quarantine log file for {target_identifier}: {e}", level="WARNING")
                await _maybe_await(audit_event("quarantine_log_failure", {
                    "module": target_identifier,
                    "error": str(e),
                    "timestamp": datetime.now(timezone.utc).isoformat()
                }, run_id=RUN_ID))
            ...
            log(f"Test for '{target_identifier}' quarantined: {result_summary['reason']}", level="WARNING")
            ...
            try:
                ...
                pass
            await _maybe_await(self.event_bus.publish("test_quarantined", {"module": target_identifier, "reason": result_summary['reason']}))
            ...
            pr_stg_path_dir_full = venv_sanitize_path(os.path.join(QUARANTINE_DIR, "for_pr"), self.project_root)
            os.makedirs(pr_stg_path_dir_full, exist_ok=True)
            staged_path = venv_sanitize_path(os.path.join(pr_stg_path_dir_full, os.path.basename(full_src_test_path)), self.project_root)
            try:
                shutil.move(full_src_test_path, staged_path)
                result_summary["staged_path"] = staged_path
            except Exception as e:
                log(f"Error moving file to PR staging for {target_identifier}: {e}", level="CRITICAL")
                await _maybe_await(audit_event("test_pr_staging_failure", {
                     "module": target_identifier,
                     "reason": "move_error",
                     "error": str(e),
                     "traceback": traceback.format_exc(),
            ...
            log(f"Test for '{target_identifier}' requires PR: {result_summary['reason']}. Staged for PR.", level="INFO")

            await _maybe_await(audit_event("test_staged_for_pr", {
                "module": target_identifier,
                "reason": result_summary['reason'],
                "staged_path": staged_path,
                "timestamp": datetime.now(timezone.utc).isoformat()
            }, run_id=RUN_ID))
            await _maybe_await(self.event_bus.publish("test_requires_pr", {"module": target_identifier, "reason": result_summary['reason']}))
            ...
            if self.config.get('pr_integration', {}).get('enabled'):
                try:
                    ...
                    log(f"Error creating/updating PR for {target_identifier}: {e}", level="ERROR")
                    result_summary["integration_status"] = "PR_CREATION_FAILED"
                    await _maybe_await(audit_event("pr_creation_failure", {
                        "module": target_identifier,
                        "error": str(e),
        ...
        dst_test_path_relative = os.path.join(self.suite_dir, os.path.basename(src_test_path_relative))
        full_dst_test_path = venv_sanitize_path(dst_test_path_relative, self.project_root)

        try:
            if os.path.isfile(full_dst_test_path):
                if compare_files(venv_sanitize_path(src_test_path_relative, self.project_root), full_dst_test_path):
                    ...
                    if inspect.isawaitable(res):
                        await res
                    log(f"Deduplicated test for '{target_identifier}' (skipped copy).", level="INFO")
                else:
                    result_summary["backup_path"] = await _maybe_await(backup_existing_test(dst_test_path_relative, self.project_root))
            ...
            log(f"Error during file integration for {target_identifier}: {e}", level="CRITICAL")
            result_summary["integration_status"] = "QUARANTINED"
            result_summary["reason"] = f"Failed to integrate test file: {e}"
            try:
                shutil.move(full_src_test_path, venv_sanitize_path(os.path.join(QUARANTINE_DIR, os.path.basename(full_src_test_path)), self.project_root))
            except Exception as move_e:
                log(f"Critical error: Could not move failed test to quarantine for {target_identifier}. File may be lost: {move_e}", level="CRITICAL")
                await _maybe_await(audit_event("test_integration_failure", {
                    "module": target_identifier,
                    "error": str(move_e),
                    "traceback": traceback.format_exc(),
        ...
        sarif_data = {
            ...
            "backup_path_relative": result_summary['backup_path'] if result_summary['backup_path'] else None,
        ...
        if await _maybe_await(_write_sarif_atomically(full_sarif_path, sarif_data)):
            ...
            await _maybe_await(audit_event("sarif_exported", {
                "module": target_identifier,
            ...
            try:
                _append_local_audit_log(
                    self.project_root, self.config, "sarif_exported",
                    {
                        "module": target_identifier,
                        "sarif_path": sarif_path_relative,
                        "test_hash": integrated_test_hash,
        ...
        try:
            await _maybe_await(self.knowledge_graph_client.update_module_metrics(
                target_identifier,
                {
            ...
            log(f"Error updating Knowledge Graph for {target_identifier}: {e}", level="CRITICAL")
            await _maybe_await(audit_event("knowledge_graph_update_failure", {
                "module": target_identifier,
                "error": str(e),
                "traceback": traceback.format_exc(),
                "timestamp": datetime.now(timezone.utc).isoformat()
        ...
        await _maybe_await(audit_event("test_integrated", {
            "kind": "atco",
        ...
        try:
            _append_local_audit_log(
                self.project_root, self.config, "test_integrated",
                {
                    "run_id": RUN_ID,
                    "module": target_identifier,
