...
class DummyMetric:
    ...
    def __enter__(self): return self._Timer().__enter__()
    def __exit__(self, exc_type, exc, tb): return self._Timer().__exit__(exc_type, exc, tb)
...
_ENABLE_PROM = os.getenv("ATCO_ENABLE_PROMETHEUS", "true").lower() not in ("0", "false", "no")
...
if not _ENABLE_PROM:
    ...
    try:
        ...
        if prom_ver >= Version('0.22.1') and prom_ver < Version('0.22.2'):
            logging.warning("Warning: Known CPU degradation issue with prometheus_client==0.22.1. Metrics may be disabled.")
        ...
        report_generation_duration = DummyMetric()
        report_generation_errors = DummyMetric()
        logging.getLogger(__name__).warning("Warning: 'prometheus_client' not available or bugged. Metrics disabled.")
...
try:
    ...
    logging.critical("CRITICAL: The 'arbiter.audit_log' library is a required dependency but was not found. Aborting.")
    raise
...
async def _write_sarif_atomically(path: str, data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode="w", delete=False, dir=os.path.dirname(path), encoding='utf-8') as tmp:
            tmp_name = tmp.name
...
class HTMLReporter:
    ...
    def __init__(self, project_root: str, report_dir: str = "atco_artifacts/sarif", sarif_dir: str = None):
    ...
    def _validate_report_schema(self, overall_results: Dict[str, Any]):
        ...
        if "summary" not in overall_results or "details" not in overall_results or "ai_metrics" not in overall_results:
            raise ReportValidationError("Overall results must contain 'summary', 'details', and 'ai_metrics' keys.")
        ...
        required_summary_keys = ["total_integrated", "total_quarantined", "total_requires_pr", "total_deduplicated", "total_not_generated", "total_targets_considered"]
        for key in required_summary_keys:
            if key not in overall_results["summary"]:
                raise ReportValidationError(f"Missing required key in summary: '{key}'")

        required_ai_metrics_keys = ["refinement_success_rate_percent", "total_refinement_attempts", "total_generations"]
        ...
        for identifier, detail in overall_results["details"].items():
            ...
            required_detail_keys = ["integration_status", "test_passed", "coverage_increase_percent", "security_issues_found", "security_max_severity", "reason", "language", "mutation_score_percent"]
            for key in required_detail_keys:
                if key not in detail:
                    raise ReportValidationError(f"Missing required key in detail for '{identifier}': '{key}'")

            if detail.get('sarif_artifact_path') and not re.match(r'^[a-zA-Z0-9_/.\-]+$', detail['sarif_artifact_path']):
                raise ReportValidationError(f"Invalid characters in sarif_artifact_path for '{identifier}'")
            if detail.get('final_integrated_test_hash') and not re.match(r'^[a-f0-9]{64}$', detail['final_integrated_test_hash']):
                pass
        ...
        * **Total Test Generations:** {ai_metrics.get('total_generations', 0)}
        ...
        for module_id, detail in sanitized_results['details'].items():
            ...
            if bool(detail.get("security_issues_found")):
                severity_map = {'LOW': '⚠️', 'MEDIUM': '⚠️', 'HIGH': '❌', 'CRITICAL': '❌'}
    ...
    async def generate_html_report(self, overall_results: Dict[str, Any], policy_engine: Any) -> str:
        with report_generation_duration.time():
            if os.getenv("DEMO_MODE", "False").lower() == "true":
                await maybe_await(audit_logger.log_event(
                    event_type="report_generation",
                    details={"action": "demo_mode_report", "message": "Forced audit log entry due to DEMO_MODE"},
                    critical=False
                ))
            ...
            try:
                ...
                await maybe_await(audit_logger.log_event(
                    ...
                    details={"error": f"Schema validation failed: {e}", "inputs": overall_results, "traceback": traceback.format_exc()},
                    critical=True
                ))
                raise
            ...
            for module_id, detail in sorted_details:
                ...
                status_class = status_class_map.get(detail.get('integration_status'), 'not_generated')

                test_pass_icon = '<span class="icon success">&#x2714;</span>' if bool(detail.get('test_passed')) else '<span class="icon fail">&#x2716;</span>'
                ...
                if security_issues_found:
                    severity_map = {'LOW': 'warn', 'MEDIUM': 'warn', 'HIGH': 'fail', 'CRITICAL': 'fail'}
                    icon_map = {'warn': '&#9888;', 'fail': '&#x2716;'}
                ...
                if 'mutation_score_percent' in detail:
                    mutation_score_display = f'{float(detail.get("mutation_score_percent", 0) or 0):.1f}%'
...
"""
            ...
            final_html_content = report_html_content.format(
                ...
                full_date_time=datetime.now(timezone.utc).strftime('%A, %B %d, %Y at %I:%M:%S %p %Z'),
            ...
            try:
                ...
                if AUDIT_LOGGER_AVAILABLE:
                    await maybe_await(audit_logger.log_event(
                        ...
                        details={
                            ...
                            "latest_alias": Path(os.path.relpath(latest_report_path, self.project_root)).as_posix(),
                ...
                logger.critical(f"CRITICAL: Failed to write HTML report to {full_report_path}: {e}", exc_info=True)
    ...
    def build(self, overall_results: Dict[str, Any], policy_engine: Any) -> str:
        ...
        raise RuntimeError("HTMLReporter.build() called from async context. Use await generate_html_report(...) instead.")
