...
def _check_disk_space(path: Path, min_mb: int) -> bool:
    ...
    try:
        ...
        if free_mb < min_mb:
            log(f"Disk space check failed for '{path}': only {free_mb:.2f}MB free, but {min_mb}MB is required.", level="CRITICAL")
            return False
        return True
    except OSError as e:
...
def normalize_results(obj: Any) -> Dict[str, Any]:
    ...
    return {"summary": {
        "total_targets_considered": 0, "total_integrated": 0,
        "total_quarantined": 0, "total_requires_pr": 0,
        "total_deduplicated": 0, "total_not_generated": 0,
...
async def _amain(argv: Optional[Union[Sequence[str], argparse.Namespace]] = None) -> int:
    """
    ...
    treat_review_required_as_success_arg = bool(getattr(args, "treat_review_required_as_success", False))
    ...
    try:
        ...
        print(f"CRITICAL: Failed to load config or initialize logging: {e}", file=sys.stderr)
    ...
    try:
        ...
        log(f"CRITICAL: Pre-flight path validation failed: {e}. Use a path under PROJECT_ROOT.", level="CRITICAL")
        return EXIT_FATAL_ERROR
    ...
    for path_str_rel in required_dirs:
        if path_str_rel:
            ...
            if not _check_permissions(path):
                log(f"CRITICAL: Insufficient permissions for directory '{path}'. Exiting.", level="CRITICAL")
                return EXIT_FATAL_ERROR
            if not _check_disk_space(path, disk_space_required_mb):
                log(f"CRITICAL: Insufficient disk space for directory '{path}'. Exiting.", level="CRITICAL")
                return EXIT_FATAL_ERROR
    ...
    try:
        ...
        if hasattr(orchestrator, "run_pipeline"):
            ...
            integration_results = await rp(coverage_xml=args.coverage_xml) if inspect.iscoroutinefunction(rp) else rp(coverage_xml=args.coverage_xml)
        else:
            try:
                targets = await atco_utils.monitor_and_prioritize_uncovered_code(args.coverage_xml, orchestrator.policy_engine, PROJECT_ROOT, CONFIG)
            except Exception as e:
                log(f"Warning: Failed to identify uncovered code targets due to an error: {e}. Proceeding without new targets.", level="WARNING")
                log(f"Traceback:\n{traceback.format_exc()}", level="DEBUG")
                targets = []
            
            generation_results = await orchestrator.generate_tests_for_targets(targets, CONFIG.get('generated_output_dir', "atco_artifacts/generated"))
            integration_results = await orchestrator.integrate_and_validate_generated_tests(generation_results)
        ...
        treat_review_as_success = CONFIG.get("treat_review_required_as_success", False) or treat_review_required_as_success_arg
        ...
        if enable_html_report:
            ...
            reporter.build(overall_results=integration_results, policy_engine=orchestrator.policy_engine)
            log("HTML report generated successfully.", level="SUCCESS")

        sarif_dir = sanitize_path(CONFIG.get("sarif_export_dir", "atco_artifacts/sarif"), PROJECT_ROOT)
        ...
        audit_detail = {
            ...
            "status": run_status_message, "exit_code": exit_code,
            "results_summary": integration_results["summary"], "html_report": html_report_path,
        }
        if policy_hash:
            audit_detail["policy_hash"] = policy_hash

        await audit_mod.audit_event("run_completed", {
            "kind": "atco", "name": "run_completed", "detail": audit_detail,
            "agent_id": "orchestrator", "correlation_id": f"atco_run_completed_{RUN_ID}",
            "timestamp": datetime.now(timezone.utc).isoformat()
        }, run_id=RUN_ID)
        
        publish_payload = {
            "timestamp": datetime.now(timezone.utc).isoformat(), "status": run_status_message, 
            "exit_code": exit_code, "results_summary": integration_results["summary"],
            "html_report_path": html_report_path, "run_id": RUN_ID
        }
        if policy_hash:
        ...
        print(f"CRITICAL: A fatal error occurred during CLI execution: {e}", file=sys.stderr)
        traceback.print_exc()
        return EXIT_FATAL_ERROR
