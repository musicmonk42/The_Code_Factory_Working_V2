...
GRPC_ENDPOINT_ALLOWLIST_SECRET = (
    "GRPC_ENDPOINT_ALLOWLIST"  # comma-separated host:port  # nosec B105
)
...
def _get_tls_credentials() -> Optional["grpc_types.ChannelCredentials"]:
    ...
    try:
        ...
        logger.critical(
            f"CRITICAL: Failed to load gRPC TLS credentials: {e}. Aborting startup.",
            exc_info=True,
        )
        alert_operator(
            f"CRITICAL: Failed to load gRPC TLS credentials: {e}. Aborting.",
            level="CRITICAL",
        )
        sys.exit(1)
...
def _is_endpoint_allowed(address: str) -> bool:
    ...
    allowlist_str = SECRETS_MANAGER.get_secret(
        GRPC_ENDPOINT_ALLOWLIST_SECRET, required=PRODUCTION_MODE
    )
    if not allowlist_str:
        if PRODUCTION_MODE:
            logger.critical(
                "CRITICAL: gRPC endpoint allowlist not set in PRODUCTION_MODE. Aborting startup."
            )
            alert_operator(
...
async def plugin_health(
    ...
    try:
        ...
        response = await asyncio.wait_for(
            getattr(stub, health_check_method_name)(request), timeout=5.0
        )
        ...
        alert_operator(
            f"CRITICAL: Plugin '{plugin_name}' health check FAILED: {e.details()} (Code: {e.code().name}).",
            level="CRITICAL",
        )
        return status_str
    except Exception as e:
        ...
        alert_operator(
            f"CRITICAL: Plugin '{plugin_name}' health check unexpected error: {e}.",
            level="CRITICAL",
        )
        return status_str
...
async def connect(
    ...
    for attempt in range(1, max(1, int(retries)) + 1):
        try:
            ...
            logger.warning(
                f"Connection failed to {address} (attempt {attempt}/{retries}): {last_error} "
            ...
            last_error = str(e)
            logger.error(
                f"Unexpected error connecting to {address} (attempt {attempt}/{retries}): {e}. "
                f"Retrying in {delay:.1f}s...",
                exc_info=True,
            )
            audit_logger.log_event(
                "grpc_connect_error", endpoint=address, attempt=attempt, error=str(e)
            )
    ...
    logger.critical(
        f"CRITICAL: Failed to connect to {address} after {retries} attempts: {last_error}"
    )
    audit_logger.log_event(
        "grpc_connect_critical_failure",
        endpoint=address,
        retries=retries,
...
async def run_method(stub: Any, method_name: str, request: Any, timeout: float) -> Any:
    ...
    try:
        ...
        alert_operator(
            f"CRITICAL: gRPC call to {method_name} failed: {e.details()} (Code: {e.code().name}).",
            level="CRITICAL",
        )
        raise
    except asyncio.TimeoutError:
        ...
        alert_operator(
            f"CRITICAL: Unexpected error during gRPC call to {method_name}: {e}.",
            level="CRITICAL",
        )
        raise
...
def validate_manifest(manifest_data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if not manifest_hmac_key:
            if PRODUCTION_MODE:
                raise AnalyzerCriticalError(
                    "Manifest HMAC key is required in PRODUCTION_MODE but not found."
                )
            else:
                logger.warning(
                    "Manifest HMAC key missing. Skipping signature validation in non-prod."
                )
                PluginManifest(**manifest_data)
                return

        calculated_signature = hmac.new(
            manifest_hmac_key.encode("utf-8"), manifest_content_str, hashlib.sha256
        ).hexdigest()
        ...
        logger.critical(
            f"CRITICAL: Unexpected error during manifest validation: {e}.",
            exc_info=True,
        )
        audit_logger.log_event(
            "manifest_validation",
...
def list_plugins(directory_path: str) -> List[Dict[str, Any]]:
    ...
    for filename in os.listdir(abs_directory_path):
        ...
        try:
            ...
            if PRODUCTION_MODE and manifest.get("is_demo_plugin", False):
                logger.critical(
                    f"CRITICAL: Demo plugin '{manifest.get('name')}' detected in PRODUCTION_MODE: {file_path}."
                )
                audit_logger.log_event(
                    "demo_plugin_detected_in_prod",
                    file=file_path,
                    plugin_name=manifest.get("name"),
                )
                alert_operator(
                    f"CRITICAL: Demo plugin '{manifest.get('name')}' detected in PRODUCTION_MODE.",
                    level="CRITICAL",
                )
                sys.exit(1)
    ...
    audit_logger.log_event(
        ...
        valid_plugins_count=len(valid_plugins),
    )
...
def generate_plugin_docs(manifest: Dict[str, Any], output_path: str) -> None:
    ...
    try:
        ...
        alert_operator(
            f"ERROR: Failed to write plugin documentation to {output_path}: {e}.",
            level="ERROR",
        )
