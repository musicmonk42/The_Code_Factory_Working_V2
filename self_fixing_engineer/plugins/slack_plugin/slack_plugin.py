...
class AuditJsonFormatter(jsonlogger.JsonFormatter):
    def __init__(self, *args, **kwargs):
        ...
        self._hmac_key = SECRETS_MANAGER.get_secret("SLACK_AUDIT_LOG_HMAC_KEY", required=PROD_MODE).encode()
...
try:
    ...
    logging.getLogger("slack_plugin").warning(f"Could not set up audit log file at {AUDIT_LOG_PATH}: {e}. Using a dummy handler.")
    ...
    def load_from_secure_vault(cls) -> "SlackGatewaySettings":
        ...
        try:
            settings_dict = {
                ...
                'encryption_key': SECRETS_MANAGER.get_secret("SLACK_GATEWAY_ENCRYPTION_KEY", required=False),
            ...
            for key in ['persistence_dir', 'min_workers', 'max_workers', 'max_queue_size', 'admin_api_port', 'admin_api_host', 'requests_per_second_limit', 'circuit_breaker_threshold', 'circuit_breaker_reset_sec', 'dry_run', 'dry_run_failure_rate', 'url_allowlist']:
            ...
            if targets_json:
                settings_dict['targets'] = [SlackTarget.model_validate(t) for t in json.loads(targets_json)]
            ...
            if PROD_MODE:
                ...
                for target in settings.targets:
                    if not any(re.match(pattern, target.webhook_url) for pattern in settings.url_allowlist):
...
class SlackMetrics:
    NOTIFICATIONS_QUEUED = Counter("slack_notifications_queued_total", "Notifications placed into the send queue.", ["target_name", "event_name", "severity"])
    NOTIFICATIONS_DROPPED = Counter("slack_notifications_dropped_total", "Notifications dropped due to a full queue.", ["target_name", "event_name"])
    NOTIFICATIONS_SENT_SUCCESS = Counter("slack_notifications_sent_success_total", "Notifications successfully sent to Slack.", ["target_name"])
    NOTIFICATIONS_FAILED_PERMANENTLY = Counter("slack_notifications_failed_permanently_total", "Notifications that failed to send after all retries.", ["target_name", "reason"])
    DEAD_LETTER_NOTIFICATIONS = Counter("slack_dead_letter_notifications_total", "Notifications sent to the dead-letter handler.", ["target_name", "reason"])
    SEND_LATENCY = Histogram("slack_send_latency_seconds", "Latency of a successful batch send operation.", ["target_name"])
    CIRCUIT_BREAKER_STATUS = Gauge("slack_circuit_breaker_status", "The status of the circuit breaker (1 for open, 0 for closed).", ["target_name"])
    RATE_LIMIT_THROTTLED_SECONDS = Counter("slack_rate_limit_throttled_seconds_total", "Total time in seconds spent waiting for the rate limiter.", ["target_name"])
    ACTIVE_WORKERS = Gauge("slack_active_workers", "Number of active worker tasks for a target.", ["target_name"])
    NON_TRACED_NOTIFICATIONS = Counter("slack_non_traced_notifications_total", "Events sent without OpenTelemetry tracing.", ["target_name"])
    QUEUE_SIZE = Gauge("slack_notifications_queue_size", "The current size of the in-memory event queue.", ["target_name"])
    QUEUE_LATENCY = Histogram("slack_queue_latency_seconds", "Time from enqueue to processing.", ["target_name"])
    RETRY_ATTEMPTS = Histogram("slack_retry_attempts", "Number of retry attempts per batch.", ["target_name"])
...
class SlackEvent(BaseModel):
    ...
    SENSITIVE_KEYS: ClassVar[re.Pattern] = re.compile(r".*(password|secret|key|token|pii|ssn|credit_card|credentials).*", re.IGNORECASE)
...
def load_template(event_name: str) -> str:
    ...
    return templates.get(event_name, "{event.event_name} alert from {hostname}. Details: ```{event.details}```")


class Serializer(Protocol):
    def encode_payload(self, event: SlackEvent, target_config: SlackTarget, hostname: str) -> Dict[str, Any]: ...


class SlackBlockKitSerializer:
    SEVERITY_COLORS = {
        "info": "#36a64f", "warning": "#ffa500", "error": "#d50000", "critical": "#b71c1c",
    }
    
    def encode_payload(self, event: SlackEvent, target_config: SlackTarget, hostname: str) -> Dict[str, Any]:
        template_str = load_template(event.event_name) if target_config.template_name is None else load_template(target_config.template_name)
        message_text = template_str.format(event=event, hostname=hostname, target_config=target_config)
        ...
        payload = {
            ...
            "attachments": [{
                ...
                "blocks": [
                    {"type": "header", "text": {"type": "plain_text", "text": f"{event.severity.upper()}: {event.event_name}"}},
                    {"type": "section", "fields": [
                    ...
                    {"type": "context", "elements": [
                        {"type": "mrkdwn", "text": f"CorrID: `{event.correlation_id[:8]}` | Seq: {event.sequence_id} | Sig: `{event.signature[:8]}...`"}
                    ]}
                ]
            }]
        }
...
class PersistentWALQueue(EventQueue):
    def __init__(self, target_name: str, persistence_dir: str, max_in_memory_size: int, encryption_key: Optional[str] = None):
        ...
        self._hmac_key = SECRETS_MANAGER.get_secret("SLACK_WAL_HMAC_KEY", required=PROD_MODE).encode()
    ...
    async def startup(self):
        try:
                    ...
                    async for line in f:
                        if line.strip(): 
                            try:
                                ...
                                if not hmac.compare_digest(sig, hmac.new(self._hmac_key, data.encode('utf-8'), hashlib.sha256).hexdigest()):
                                    raise AnalyzerCriticalError("WAL integrity check failed on decrypt.")
                                
                                decrypted_line = self._cipher.decrypt(data.encode('utf-8')) if self._cipher else data.encode('utf-8')
            ...
            main_logger.info(f"Loaded {self._mem_queue.qsize()} events from disk for target {os.path.basename(self._dir)}.")
            audit_logger.info("WAL loaded from disk.", extra={"context": {"target": os.path.basename(self._dir), "event_count": self._mem_queue.qsize()}})
        except OSError as e:
            main_logger.critical(f"Failed to load WAL from disk: {e}. Exiting.", extra={"context": {"target": os.path.basename(self._dir)}})
    ...
    def _create_signature(self, event: SlackEvent) -> str:
        ...
        return hmac.new(SECRETS_MANAGER.get_secret("SLACK_GATEWAY_SIGNING_SECRET").encode(), canonical_event.encode(), hashlib.sha256).hexdigest()
    ...
    async def shutdown(self):
        ...
        audit_logger.info("WAL compacted and queues flushed on shutdown.", extra={"context": {"target": os.path.basename(self._dir)}})

# ---- 5. Advanced Resilience Patterns ----
class CircuitBreaker:
    def __init__(self, threshold: int, reset_seconds: int, metrics: SlackMetrics, target_name: str):
    ...
    def record_failure(self):
        ...
        if self._failure_count >= self._threshold and not self._is_open:
            ...
            alert_operator(f"CRITICAL: Slack circuit breaker tripped for {self._target_name}.", "CRITICAL")
...
class SlackGateway:
    def __init__(self, target_config: SlackTarget, global_settings: SlackGatewaySettings, metrics: SlackMetrics, serializer: Serializer, rate_limiter: TokenBucket, dead_letter_hook: Optional[DeadLetterHook]):
        ...
        self._event_queue: EventQueue = PersistentWALQueue(target_config.name, global_settings.persistence_dir, global_settings.max_queue_size, encryption_key=global_settings.encryption_key)
    ...
    async def _send_event(self, event: SlackEvent):
        with tracer.start_as_current_span("slack_send_event", attributes={
            ...
            while attempt < self.global_settings.max_retries:
                ...
                async with self._concurrency_limiter:
                    ...
                    try:
                        ...
                        async with session.post(self.target_config.webhook_url, json=payload, ssl=self.global_settings.verify_ssl, timeout=timeout) as resp:
                            ...
                            if resp.status == 200:
                                ...
                                self.metrics.NOTIFICATIONS_SENT_SUCCESS.labels(target_name=self.target_config.name).inc()
                                self.circuit_breaker.record_success()
                                audit_logger.info("notification_sent_success", extra={"context": {"target": self.target_config.name, "event_name": event.event_name}})
                                return True
                            elif resp.status == 429:
                                retry_after = int(resp.headers.get("Retry-After", "5"))
                                main_logger.warning(f"Rate limited by Slack API. Backing off for {retry_after} seconds.", extra={"context": {"target": self.target_config.name}})
                                await asyncio.sleep(retry_after)
                                continue
                            elif 400 <= resp.status < 500:
                                error_text = await resp.text()
                                main_logger.error("Permanent failure sending to Slack (client error).", extra={"context": {"target": self.target_config.name, "error": error_text, "status_code": resp.status}})
                                await self._handle_dead_letter(event, "client_error")
                                self.metrics.NOTIFICATIONS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason=f"client_error_{resp.status}").inc()
                                return False
                            else: resp.raise_for_status()
                    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
                        main_logger.warning("Temporary failure sending to Slack.", extra={"context": {"target": self.target_config.name, "attempt": attempt + 1, "error": str(e)}})
                        if attempt + 1 >= self.global_settings.max_retries:
                            self.circuit_breaker.record_failure()
                            await self._handle_dead_letter(event, f"service_unavailable: {type(e).__name__}")
                            self.metrics.NOTIFICATIONS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason="service_unavailable").inc()
                            return False
                attempt += 1
    ...
    async def _worker(self, worker_id: int):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if event is None: self._event_queue.task_done(); await self._event_queue.put(None); break
                ...
                if self.global_settings.dry_run:
                    main_logger.info("[DRY RUN] Would send Slack notification.", extra={"context": {"target": self.target_config.name, "event_name": event.event_name, "simulated_failure": self.global_settings.dry_run_failure_rate > 0 and random.random() < self.global_settings.dry_run_failure_rate}})
                ...
                else: success = await self._send_event(event)
                ...
                main_logger.critical("Unhandled exception in Slack worker.", extra={"context": {"error": str(e), "target": self.target_config.name, "worker_id": worker_id}})
                audit_logger.critical("worker_unhandled_exception", extra={"context": {"target": self.target_config.name, "worker_id": worker_id, "error": str(e)}})
...
class SlackGatewayManager:
    def __init__(self, settings: SlackGatewaySettings, metrics: SlackMetrics, dead_letter_hook: Optional[DeadLetterHook] = None):
        self.settings = settings
        self.metrics = metrics
        self.dead_letter_hook = dead_letter_hook
        self._gateways: Dict[str, SlackGateway] = {}
        self._serializers: Dict[str, Serializer] = {"block_kit_serializer": SlackBlockKitSerializer()}
    ...
    async def _load_sequence_counters(self):
        for target in self.settings.targets:
            try:
                ...
                main_logger.warning("Failed to load sequence counter, starting from 0.", extra={"context": {"target": target.name, "error": str(e)}})
    ...
    async def _save_sequence_counter(self, target_name: str, seq_id: int):
        try:
            ...
            main_logger.error("Failed to persist sequence counter.", extra={"context": {"target": target_name, "error": str(e)}})
        ...
        async with self._lock:
            ...
            for target in new_settings.targets:
                ...
                if ws_id not in self._rate_limiters:
                    self._rate_limiters[ws_id] = TokenBucket(new_settings.requests_per_second_limit, new_settings.requests_per_second_limit, self.metrics, ws_id)
            ...
            for name, target_config in new_targets_map.items():
                ...
                new_gateways[name] = SlackGateway(target_config, new_settings, self.metrics, serializer, rate_limiter, self.dead_letter_hook)
    ...
    async def publish(self, target_name: str, event_name: str, details: Dict[str, Any], **kwargs):
        ...
        try:
            event = SlackEvent(event_name=event_name, details=details, trace_context=trace_context, sequence_id=seq_id, signature=signature, **kwargs)
            await gateway.publish(event)
        except ValidationError as e: 
            main_logger.error("Invalid Slack event schema.", extra={"context": {"error": str(e), "event_name": event_name}})
...
DEAD_LETTER_DIR = os.environ.get("SLACK_GATEWAY_DEAD_LETTER_DIR", "/var/lib/slack_gateway_dead_letters")
...
async def dead_letter_to_file(event: SlackEvent, reason: str):
    ...
    encryption_key = SECRETS_MANAGER.get_secret("SLACK_GATEWAY_DEAD_LETTER_ENCRYPTION_KEY", required=False)
    ...
    try:
        if PROD_MODE:
            ...
            slack_settings = SlackGatewaySettings(
                signing_secret=os.environ.get("SLACK_GATEWAY_SIGNING_SECRET", "non-prod-signing-secret"),
                admin_api_key=os.environ.get("SLACK_GATEWAY_ADMIN_API_KEY", "non-prod-admin-key"),
                targets=[
                    SlackTarget(name="alerts", webhook_url=os.environ.get("SLACK_ALERTS_URL", "https://localhost/alerts")),
                    SlackTarget(name="audit", webhook_url=os.environ.get("SLACK_AUDIT_URL", "https://localhost/audit")),
                ],
                url_allowlist=['^https://localhost', '^https://hooks.slack.com']
            )

        slack_metrics = SlackMetrics()
        slack_gateway_manager = SlackGatewayManager(slack_settings, slack_metrics, dead_letter_hook=dead_letter_to_file)
...
if __name__ == "__main__":
    ...
    async def main_example():
        ...
        await slack_gateway_manager.publish("alerts", "database_connection_failed", {"db_host": "prod-db-1"}, severity="critical")
        await slack_gateway_manager.publish("audit", "user_logged_in", {"user_id": "user-123"}, severity="info")
        await slack_gateway_manager.publish("alerts", "sensitive_info_exposed", {"user_id": "user-456", "token": "super-secret-token-123"}, severity="critical")
