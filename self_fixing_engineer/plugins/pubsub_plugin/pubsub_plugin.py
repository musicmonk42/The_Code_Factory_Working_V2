...
try:
    ...
    _span_processor = BatchSpanProcessor(ConsoleSpanExporter()) # For dev/local. In prod, use a real exporter (e.g., OTLP)
...
class PubSubSettings(BaseSettings):
    ...
    batch_max_bytes: int = Field(1024 * 1024, ge=1, description="Maximum bytes per batch (1MB default).")
    batch_max_latency_sec: float = Field(0.5, ge=0.01, description="Maximum time to wait for a batch to fill.")
    ...
    worker_batch_size: int = Field(500, ge=1, description="How many messages the worker pulls from queue at once.")
    worker_linger_sec: float = Field(0.2, ge=0.01, description="How long the worker waits to build a batch.")
    ...
    allowed_project_ids: Optional[List[str]] = Field(None, description="List of allowed GCP Project IDs in production.")
    allowed_topic_ids: Optional[List[str]] = Field(None, description="List of allowed Pub/Sub Topic IDs in production.")
    ...
    gcp_credentials_secret_id: Optional[str] = Field(None, description="Secret ID for GCP credentials (e.g., service account key JSON) in a secure vault.")
    ...
    def validate_project_id_in_prod(cls, v, values):
        ...
        async def _get_allowed_project_ids():
            ...
            allowed_ids_str = SECRETS_MANAGER.get_secret("PUBSUB_ALLOWED_PROJECT_IDS", required=PRODUCTION_MODE)
        ...
        if PRODUCTION_MODE:
            if allowed_project_ids:
                if v not in allowed_project_ids:
                    raise ValueError(f"Project ID '{v}' is not in the 'allowed_project_ids' list: {allowed_project_ids}.")
            else:
                raise ValueError("In PRODUCTION_MODE, 'allowed_project_ids' list must be configured and non-empty.")
            
            if "dummy" in v.lower() or "test" in v.lower() or "mock" in v.lower():
                raise ValueError(f"Dummy/test Project ID '{v}' detected in PRODUCTION_MODE. Not allowed.")
        return v
    ...
    def validate_topic_id_in_prod(cls, v, values):
        ...
        async def _get_allowed_topic_ids():
            ...
            allowed_ids_str = SECRETS_MANAGER.get_secret("PUBSUB_ALLOWED_TOPIC_IDS", required=PRODUCTION_MODE)
            if not allowed_ids_str:
                return []
        ...
        if PRODUCTION_MODE:
            if allowed_topic_ids:
                if v not in allowed_topic_ids:
                    raise ValueError(f"Topic ID '{v}' is not in the 'allowed_topic_ids' list: {allowed_topic_ids}.")
            else:
                raise ValueError("In PRODUCTION_MODE, 'allowed_topic_ids' list must be configured and non-empty.")
            
            if "dummy" in v.lower() or "test" in v.lower() or "mock" in v.lower():
                raise ValueError(f"Dummy/test Topic ID '{v}' detected in PRODUCTION_MODE. Not allowed.")
        return v
    ...
    def validate_gcp_credentials_source(cls, v):
        ...
        if PRODUCTION_MODE and not v:
            raise ValueError("In PRODUCTION_MODE, 'gcp_credentials_secret_id' must be provided for GCP credentials. Default ADC/ENV is forbidden.")
        return v
...
try:
    ...
    logger.critical(f"CRITICAL: PubSubSettings validation failed: {e}. Aborting startup.", exc_info=True)
    alert_operator(f"CRITICAL: PubSubSettings validation failed: {e}. Aborting.", level="CRITICAL")
    sys.exit(1)
except Exception as e:
    logger.critical(f"CRITICAL: Unexpected error loading PubSubSettings: {e}. Aborting startup.", exc_info=True)
    alert_operator(f"CRITICAL: Unexpected error loading PubSubSettings: {e}. Aborting.", level="CRITICAL")
    sys.exit(1)
...
class PubSubMetrics:
    ...
    def __init__(self, registry: CollectorRegistry):
        self.EVENTS_QUEUED = Counter("pubsub_audit_events_queued_total", "Events placed into the send queue.", ["event_name"], registry=registry)
        self.EVENTS_DROPPED = Counter("pubsub_audit_events_dropped_total", "Events dropped due to a full queue.", ["event_name"], registry=registry)
        self.EVENTS_PUBLISHED_SUCCESS = Counter("pubsub_audit_events_published_success_total", "Events successfully published to Pub/Sub.", ["topic"], registry=registry)
        self.EVENTS_FAILED_PERMANENTLY = Counter("pubsub_audit_events_failed_permanently_total", "Events that failed to publish after all retries.", ["topic", "reason"], registry=registry)
        self.PUBLISH_LATENCY = Histogram("pubsub_audit_publish_latency_seconds", "Latency of a successful batch publish operation.", ["topic"], registry=registry)
        self.CIRCUIT_BREAKER_STATUS = Gauge("pubsub_audit_circuit_breaker_status", "The status of the circuit breaker (1 for open, 0 for closed).", registry=registry)
        self.QUEUE_SIZE = Gauge("pubsub_audit_queue_current_size", "Current number of events in the Pub/Sub send queue.", registry=registry)
    ...
    def check(self):
        if self._is_open:
            if time.monotonic() - self._last_failure_time > self._reset_seconds:
                ...
                alert_operator("INFO: Pub/Sub Circuit Breaker RESET. Resuming publishes.", level="INFO")
            else:
    ...
    def record_failure(self):
        ...
        if self._failure_count >= self._threshold:
            if not self._is_open:
                ...
                audit_logger.log_event("pubsub_circuit_breaker_tripped", status="open", failure_count=self._failure_count)
                alert_operator(f"CRITICAL: Pub/Sub Circuit Breaker TRIPPED. Publishes suspended. Failures: {self._failure_count}", level="CRITICAL")
...
class PubSubGateway:
    ...
    def __init__(self, settings: PubSubSettings, metrics: PubSubMetrics):
        ...
        logger.info("Pub/Sub Gateway initialized.", extra={"context": {"project_id": self.settings.project_id, "topic_id": self.settings.topic_id}})
        audit_logger.log_event("pubsub_gateway_init", project_id=self.settings.project_id, topic_id=self.settings.topic_id, max_queue_size=self.settings.max_queue_size)
    ...
    async def _load_gcp_credentials(self):
        ...
        if PRODUCTION_MODE:
            ...
            if not gcp_credentials_secret_id:
                raise AnalyzerCriticalError("In PRODUCTION_MODE, 'gcp_credentials_secret_id' must be provided for GCP credentials. Aborting startup.")
            try:
                credentials_json = SECRETS_MANAGER.get_secret(gcp_credentials_secret_id, required=True)
                self._gcp_credentials = service_account.Credentials.from_service_account_info(json.loads(credentials_json))
                ...
                raise AnalyzerCriticalError(f"Failed to load GCP credentials from secret manager '{gcp_credentials_secret_id}': {e}")
        else:
            logger.warning("Using Google Application Default Credentials (ADC). Not recommended for production without explicit control.")
    ...
    async def startup(self):
        ...
        if not self._worker_tasks:
            ...
            self._topic_path = self._publisher_client.topic_path(self.settings.project_id, self.settings.topic_id)

            try:
                await asyncio.to_thread(self._publisher_client.get_topic, request={"topic": self._topic_path})
                logger.info(f"Pub/Sub Topic '{self.settings.topic_id}' validated successfully.")
            except google_exceptions.NotFound:
                raise AnalyzerCriticalError(f"Pub/Sub Topic '{self.settings.topic_id}' not found. Aborting startup.")
            except Exception as e:
                raise AnalyzerCriticalError(f"Failed to validate Pub/Sub Topic '{self.settings.topic_id}': {e}. Aborting startup.")

            self._worker_tasks = [asyncio.create_task(self._worker()) for _ in range(self.settings.num_workers)]
            logger.info(f"Pub/Sub Gateway started with {self.settings.num_workers} workers.", extra={"context": {"topic": self._topic_path}})
            audit_logger.log_event("pubsub_gateway_startup", status="success", num_workers=self.settings.num_workers)
    ...
    async def shutdown(self):
        ...
        audit_logger.log_event("pubsub_gateway_shutdown_start", queue_size=self._event_queue.qsize())
        ...
        try:
            await asyncio.wait_for(asyncio.gather(*self._worker_tasks), timeout=self.settings.batch_max_latency_sec * self.settings.num_workers * 2 + 10)
            logger.info("Pub/Sub Gateway worker tasks finished.")
            audit_logger.log_event("pubsub_gateway_worker_finished")
        except asyncio.TimeoutError:
            raise AnalyzerCriticalError("Pub/Sub Gateway worker tasks timed out during shutdown. Some messages may be lost.")
        except Exception as e:
    ...
    def publish(self, event_name: str, service_name: str, details: Dict[str, Any]):
        ...
        try:
            ...
            audit_logger.log_event("pubsub_event_queued", event_name=event_name, queue_size=self._event_queue.qsize())
        except asyncio.QueueFull:
            self.metrics.EVENTS_DROPPED.labels(event_name=event_name).inc()
            logger.critical("CRITICAL: Pub/Sub event queue is full. Dropping event.", extra={"context": {"event_name": event_name, "queue_size": self.settings.max_queue_size}})
            audit_logger.log_event("pubsub_event_dropped", event_name=event_name, reason="queue_full", queue_size=self.settings.max_queue_size)
            alert_operator(f"CRITICAL: Pub/Sub event queue is FULL ({self.settings.max_queue_size} events). Events are being dropped. IMMEDIATE ACTION REQUIRED!", level="CRITICAL")
        except ValidationError as e:
            logger.error(f"Invalid event schema for event '{event_name}': {e}", extra={"context": {"error": str(e)}})
            audit_logger.log_event("pubsub_event_validation_failed", event_name=event_name, error=str(e))
            alert_operator(f"CRITICAL: Pub/Sub event validation failed for '{event_name}': {e}. Aborting.", level="CRITICAL")
            ...
            alert_operator(f"CRITICAL: Pub/Sub event PII validation failed for '{event_name}': {e}.", level="CRITICAL")
        except Exception as e:
    ...
    async def _publish_batch(self, batch: List[AuditEvent]):
        with _tracer.start_as_current_span("pubsub_publish_batch", attributes={"batch.size": len(batch), "topic": self.settings.topic_id}) as span:
            try:
                self.circuit_breaker.check()
            except ConnectionAbortedError as e:
                logger.warning("Publish skipped due to open circuit breaker.", extra={"context": {"error": str(e)}})
                self.metrics.EVENTS_FAILED_PERMANENTLY.labels(topic=self.settings.topic_id, reason='circuit_breaker').inc(len(batch))
                audit_logger.log_event("pubsub_publish_skipped", reason="circuit_breaker_open", batch_size=len(batch))
                return
            ...
            async with self._sem:
                for event in batch:
                    ...
                    future = self._publisher_client.publish(self._topic_path, data, retry=self.custom_retry)
            ...
            try:
                ...
                self.metrics.EVENTS_PUBLISHED_SUCCESS.labels(topic=self.settings.topic_id).inc(len(batch))
                self.circuit_breaker.record_success()
                if span: span.set_status(trace.Status(trace.StatusCode.OK))
                logger.info(f"Successfully published batch of {len(batch)} events to Pub/Sub.", extra={"topic": self.settings.topic_path})
                audit_logger.log_event("pubsub_publish_success", batch_size=len(batch), topic=self.settings.topic_id, duration=duration)
            except (google_exceptions.RetryError, google_exceptions.ServiceUnavailable, google_exceptions.DeadlineExceeded) as e:
                self.circuit_breaker.record_failure()
                self.metrics.EVENTS_FAILED_PERMANENTLY.labels(topic=self.settings.topic_id, reason='service_unavailable').inc(len(batch))
                logger.critical(f"CRITICAL: Failed to publish batch after retries (service unavailable/timeout): {e}.", exc_info=True)
                if span: span.record_exception(e)
                if span: span.set_status(trace.Status(trace.StatusCode.ERROR, "Publish failed"))
                audit_logger.log_event("pubsub_publish_failure", batch_size=len(batch), topic=self.settings.topic_id, error=str(e), reason="service_unavailable")
                alert_operator(f"CRITICAL: Pub/Sub publish failed after retries for topic {self.settings.topic_id}: {e}.", level="CRITICAL")
                raise AnalyzerCriticalError("Pub/Sub publish failed after retries.")
            except Exception as e:
                self.circuit_breaker.record_failure()
                logger.critical(f"CRITICAL: Unhandled exception during Pub/Sub batch publish: {e}.", exc_info=True)
                if span: span.record_exception(e)
                if span: span.set_status(trace.Status(trace.StatusCode.ERROR, "Unhandled publish error"))
                audit_logger.log_event("pubsub_publish_failure", batch_size=len(batch), topic=self.settings.topic_id, error=str(e), reason="unhandled_exception")
                alert_operator(f"CRITICAL: Unhandled exception during Pub/Sub publish: {e}.", level="CRITICAL")
        ...
        while True:
            try:
                ...
                while len(batch) < self.settings.worker_batch_size:
                    try:
                        event = await asyncio.wait_for(self._event_queue.get(), self.settings.worker_linger_sec)
                        if event is None:
                            self._event_queue.task_done()
...
try:
    ...
    async def health(_):
        return web.json_response({
            ...
            "circuit_breaker_status": "closed" if not pubsub_gateway.circuit_breaker._is_open else "open"
        })
...
if not PRODUCTION_MODE:
    ...
    async def main():
        async with app_lifecycle():
            ...
            pubsub_gateway.publish("user_logged_in", "auth-service", {"user_id": "user-123", "ip_address": "192.168.1.1"})
            pubsub_gateway.publish("payment_processed", "billing-service", {"amount": 99.99, "currency": "USD"})
    ...
    if __name__ == "__main__":
        try:
            ...
            alert_operator(f"CRITICAL: Unhandled exception during Pub/Sub example run: {e}.", level="CRITICAL")
    ...
    if __name__ == "__main__":
        ...
        alert_operator("CRITICAL: Pub/Sub plugin example code executed in PRODUCTION_MODE. Aborting.", level="CRITICAL")
        sys.exit(1)