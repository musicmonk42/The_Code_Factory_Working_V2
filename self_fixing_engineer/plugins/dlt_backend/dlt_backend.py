        ...
        async def wrapper(*args, **kwargs):
            ...
            for i in range(retries):
                try:
                    ...
                    last_exc = exc
                    logger.warning(f"[DLT Retry] Attempt {i+1}/{retries} failed: {type(exc).__name__}: {exc}", exc_info=True)
                    audit_logger.log_event("dlt_retry_attempt", operation=fn.__name__, attempt=i+1, max_retries=retries, error=str(exc))
                    if i < retries - 1:
                        await asyncio.sleep(delay * (backoff ** i))
            
            logger.critical(f"[DLT Retry] Operation '{fn.__name__}' failed after {retries} attempts: {last_exc}", exc_info=True)
            audit_logger.log_event("dlt_retry_final_failure", operation=fn.__name__, max_retries=retries, final_error=str(last_exc))
            if await _should_alert(f"{fn.__name__}:{type(last_exc).__name__}"):
                alert_operator(f"CRITICAL: DLT operation '{fn.__name__}' failed after {retries} attempts. Error: {last_exc}", level="CRITICAL", details={'traceback': traceback.format_exc()})
            raise last_exc
...
def _verify_hash_chain(expected_prev_hash: str, actual_prev_hash: str, checkpoint_name: str, version: int):
    """Verifies the hash chain integrity."""
    if expected_prev_hash != actual_prev_hash:
        audit_logger.log_event("dlt_hash_chain_failure", checkpoint=checkpoint_name, version=version, expected=expected_prev_hash, actual=actual_prev_hash)
...
def encrypt(plaintext: bytes, key: bytes) -> bytes:
    ...
    if not HAVE_AESGCM:
        raise AnalyzerCriticalError("Encryption requested but 'cryptography' (AESGCM) is not installed.")
...
def decrypt(ciphertext: bytes, key: bytes) -> bytes:
    ...
    if not HAVE_AESGCM:
        raise AnalyzerCriticalError("Decryption requested but 'cryptography' (AESGCM) is not installed.")
    if PRODUCTION_MODE and (not isinstance(key, bytes) or len(key) != 32):
...
try:
    ...
    if PRODUCTION_MODE:
        ...
        class S3OffChainClient: # Dummy for non-prod
            def __init__(self, config): self.store = {}; logger.warning("Using dummy S3OffChainClient.")
            ...
            async def get_blob(self, off_chain_id, correlation_id=None):
                if off_chain_id not in self.store: raise FileNotFoundError(f"Dummy blob {off_chain_id} not found.")
                return self.store[off_chain_id]
            async def health_check(self, correlation_id=None): return {"status": True, "message": "Dummy S3 is healthy."}
...
try:
    ...
    if PRODUCTION_MODE:
        ...
        class FabricClientWrapper: # Dummy for non-prod
            def __init__(self, config, off_chain_client): self.chain = {}; self.off_chain_client = off_chain_client; logger.warning("Using dummy FabricClientWrapper.")
            async def write_checkpoint(self, checkpoint_name, hash, prev_hash, metadata, payload_blob, correlation_id=None):
                ...
                entry = {"hash": hash, "prev_hash": prev_hash, "metadata": metadata, "off_chain_ref": off_chain_id, "tx_id": tx_id, "version": version}
            ...
            async def read_checkpoint(self, name, version=None, correlation_id=None):
                chain = self.chain.get(name, []); entry = chain[-1] if version is None or version == "latest" else next((e for e in chain if e['version'] == version), None)
                if not entry: raise FileNotFoundError(f"Dummy checkpoint {name} v{version} not found.")
            ...
            async def get_version_tx(self, name, version, correlation_id=None): return await self.read_checkpoint(name, version)
            async def rollback_checkpoint(self, name, rollback_hash, correlation_id=None):
                chain = self.chain.get(name, []); entry = next((e for e in chain if e['hash'] == rollback_hash), None)
                if not entry: raise FileNotFoundError(f"Dummy checkpoint {name} hash {rollback_hash} not found.")
                new_version = len(chain) + 1; tx_id = f"{name}-rollback-tx{new_version}-{int(time.time())}"
                new_entry = {"hash": entry["hash"], "prev_hash": entry["prev_hash"], "metadata": entry["metadata"], "off_chain_ref": entry["off_chain_ref"], "tx_id": tx_id, "version": new_version, "rollback_of_version": entry["version"]}
                chain.append(new_entry); return new_entry
            async def health_check(self, correlation_id=None): return {"status": True, "message": "Dummy Fabric is healthy."}
            async def close(self): pass
...
async def initialize_dlt_backend(config: Dict[str, Any]) -> None:
    ...
    if PRODUCTION_MODE:
        if off_chain_type == "in_memory":
            raise AnalyzerCriticalError("'in_memory' off-chain storage is forbidden in PRODUCTION_MODE.")
        if dlt_type == "simple":
    ...
    try:
        if off_chain_type == "s3":
            ...
            raise AnalyzerCriticalError(f"Unsupported off-chain storage type '{off_chain_type}'. Aborting startup.")
        
        health_result = await off_chain_client.health_check()
        if not health_result["status"]:
            raise AnalyzerCriticalError(f"Off-chain client '{off_chain_type}' failed initial health check: {health_result['message']}.")
        logger.info(f"Off-chain client '{off_chain_type}' initialized and healthy.")
    except AnalyzerCriticalError as e:
        logger.critical(f"CRITICAL: {e}.")
        alert_operator(f"CRITICAL: Off-chain client '{off_chain_type}' failed health check. Aborting.", level="CRITICAL")
        raise e
    except Exception as e:
        logger.critical(f"CRITICAL: Failed to initialize off-chain client '{off_chain_type}': {e}.", exc_info=True)
        alert_operator(f"CRITICAL: Failed to initialize off-chain client '{off_chain_type}': {e}. Aborting.", level="CRITICAL")
        raise AnalyzerCriticalError(f"Failed to initialize off-chain client '{off_chain_type}': {e}.")
    ...
    try:
        if dlt_type == "fabric":
            ...
            raise AnalyzerCriticalError(f"Unsupported DLT client type '{dlt_type}'. Aborting startup.")
        
        health_result = await fabric_client.health_check()
        if not health_result["status"]:
            raise AnalyzerCriticalError(f"DLT client '{dlt_type}' failed initial health check: {health_result['message']}.")
        logger.info(f"DLT client '{dlt_type}' initialized and healthy.")
    except AnalyzerCriticalError as e:
        logger.critical(f"CRITICAL: {e}.")
        alert_operator(f"CRITICAL: DLT client '{dlt_type}' failed health check. Aborting.", level="CRITICAL")
        raise e
    except Exception as e:
        logger.critical(f"CRITICAL: Failed to initialize DLT client '{dlt_type}': {e}.", exc_info=True)
        alert_operator(f"CRITICAL: Failed to initialize DLT client '{dlt_type}': {e}. Aborting.", level="CRITICAL")
        raise AnalyzerCriticalError(f"Failed to initialize DLT client '{dlt_type}': {e}.")

    logger.info("DLT Backend initialized successfully.")
    audit_logger.log_event("dlt_backend_initialized", off_chain_type=off_chain_type, dlt_type=dlt_type)
    ...
    if REDIS_CLIENT:
        ...
        try:
            ...
            if ok:
                ...
                logger.warning(f"Failed to acquire distributed lock for '{name}'. Another process may be writing.")
    ...
    async def from_blob(blob: bytes) -> Any:
        try:
            ...
            if await _should_alert(f"payload_corrupt:{name}"):
                alert_operator(f"CRITICAL: Off-chain payload corrupt for '{name}'.", level="CRITICAL")
    ...
    if op == "save":
        async with _save_locks[name]:
            async with _maybe_dist_lock(name):
                ...
                latest_tx = None
                try:
                    latest_tx = await async_retry()(fabric_client.read_checkpoint)(name, version="latest")
                ...
                if latest_tx and latest_tx["metadata"].get("payload_hash") == state_hash:
                    logger.info(f"DLT checkpoint save skipped: {name} is identical to latest version.")
                    if REDIS_CLIENT:
                        try:
                            # Prime cache with the latest state if it was a cache miss
                            await REDIS_CLIENT.setex(f"dlt_checkpoint:{name}:latest", CACHE_TTL, json.dumps({"state": state, "hash": latest_tx["metadata"]["hash"]}))
                        except Exception as e:
                ...
                base = {"name": name, "payload_hash": state_hash, "prev_hash": prev_hash}
                sig = _maybe_sign_checkpoint(base)

                metadata.update({
                    "compression_algo": "gzip",
                    "encryption_algo": "AES-256-GCM" if getattr(self, "encrypt_key", None) else "none",
                    "prev_hash": prev_hash,
                ...
                audit_logger.log_event(
                    ...
                    name=name, hash=version_hash, prev_hash=prev_hash, tx_id=tx_id, version=version,
                    ts=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                    metadata=scrub_sensitive_data(metadata), off_chain_id=off_chain_id,
                ...
                if REDIS_CLIENT:
                    try:
                        await REDIS_CLIENT.setex(
                            f"dlt_checkpoint:{name}:latest", CACHE_TTL,
        ...
        if is_latest and REDIS_CLIENT:
            try:
                ...
                if cached_data:
                    ...
                    latest_tx_on_chain = await async_retry()(fabric_client.read_checkpoint)(name, version="latest")
                    if latest_tx_on_chain["metadata"]["hash"] == cached_obj.get("hash"):
                        logger.info(f"DLT checkpoint loaded from cache and verified: {name} v{version}")
                        return cached_obj["state"]
                    else:
                        logger.warning("Cached data hash mismatch with latest ledger entry. Bypassing cache.")
            except Exception as e:
                logger.warning(f"Cache fast-path failed: {e}")

        tx = await async_retry()(fabric_client.read_checkpoint)(name, version=version, correlation_id=kwargs.get("correlation_id"))
        ...
        if self.enable_hash_chain:
            ...
            expected_current_hash = _calculate_hash({"payload_hash": payload_hash, "prev_hash": expected_prev})
            if expected_current_hash != tx["metadata"]["hash"]:
                hash_chain_ok = False
                msg = (f"Tamper detected for '{name}' v{version_num}: "
                       f"stored hash {tx['metadata']['hash']} != recomputed {expected_current_hash}")
                logger.critical("CRITICAL: " + msg)
                audit_logger.log_event("dlt_hash_current_mismatch",
                                       checkpoint=name, version=version_num,
            ...
            if sig:
                key = SECRETS_MANAGER.get_secret("DLT_HMAC_KEY", required=PRODUCTION_MODE)
                if key:
                    base = {"name": name, "payload_hash": payload_hash, "prev_hash": expected_prev}
        ...
        audit_logger.log_event(
            ...
            name=name, version=version, hash=tx["metadata"]["hash"], tx_id=tx.get("tx_id"),
            ts=datetime.datetime.now(datetime.timezone.utc).isoformat(),
            metadata=scrub_sensitive_data(tx.get("metadata")), hash_chain_ok=hash_chain_ok,
            user=getpass.getuser() if 'getpass' in sys.modules else 'unknown_user',
            correlation_id=kwargs.get("correlation_id")
        )
        ...
        prev_tx = await async_retry()(fabric_client.get_version_tx)(name, version, correlation_id=kwargs.get("correlation_id"))
        
        # Create a new DLT entry that points back to the previous hash
        rollback_tx = await async_retry()(fabric_client.rollback_checkpoint)(name, prev_tx["metadata"]["hash"], correlation_id=kwargs.get("correlation_id"))
        
        audit_logger.log_event(
            "dlt_checkpoint_rollback",
            name=name, to_version=version, rollback_tx_id=rollback_tx["tx_id"],
            ts=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        ...
        logger.info(f"DLT checkpoint rolled back: {name} -> v{version} [rollback_tx={rollback_tx['tx_id']}]")
        return rollback_tx

    elif op == "diff":
        v1, v2 = args
        tx1 = await async_retry()(fabric_client.get_version_tx)(name, v1, correlation_id=kwargs.get("correlation_id"))
        tx2 = await async_retry()(fabric_client.get_version_tx)(name, v2, correlation_id=kwargs.get("correlation_id"))
...
async def _run_initialization_and_test() -> None:
    ...
    try:
        ...
        state1 = {"value": 10, "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "metadata": {"source": "test"}}
        ...
        state2 = {"value": 20, "timestamp": datetime.datetime.now(datetime.timezone.utc).isoformat(), "metadata": {"source": "test"}}
        ...
        if os.getenv("FABRIC_NETWORK_PROFILE") and os.path.exists(os.getenv("FABRIC_NETWORK_PROFILE")):
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"An unexpected error occurred during application runtime: {e}", exc_info=True)
        sys.exit(1)
