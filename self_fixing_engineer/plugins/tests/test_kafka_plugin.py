...
def test_kafka_settings_missing_ssl_cafile_prod(set_env, sample_settings_dict):
    ...
    with pytest.raises(ValidationError, match="ssl_cafile or ssl_cafile_secret_id must be provided"):
        KafkaSettings(**sample_settings_dict)
...
def test_kafka_settings_invalid_brokers_allowlist(set_env, sample_settings_dict):
    ...
    with pytest.raises(ValidationError, match="configured brokers .* not in the 'allowed_brokers' list"):
        KafkaSettings(**sample_settings_dict)
...
async def test_producer_start_success(mock_aiokafka_producer, mock_prometheus_registry):
    ...
    producer = KafkaAuditProducer(KafkaSettings(**{"brokers": "localhost:9092", "audit_topic": "audit"}), AuditMetrics(mock_prometheus_registry))
...
async def test_producer_start_failure(mock_aiokafka_producer, mock_alert_operator):
    ...
    producer = KafkaAuditProducer(KafkaSettings(**{"brokers": "localhost:9092", "audit_topic": "audit"}), AuditMetrics(CollectorRegistry()))
    producer._producer = mock_producer
...
async def test_producer_send_retry_failure(mock_aiokafka_producer, mock_alert_operator):
    ...
    settings = KafkaSettings(**{"brokers": "localhost:9092", "audit_topic": "audit", "audit_max_retries": 2})
