...
def reset_mocks():
    ...
    mock_secrets_manager.get_secret = MagicMock(side_effect=lambda key, required=True: 
...
def sample_settings_dict():
    ...
    return {
        ...
        "targets": [
            {"name": "security", "url": "https://siem.example.com", "token": "test-token"},
...
def test_json_hec_serializer():
    ...
    event = siem_plugin.SIEMEvent(event_name="test", source="app", details={"key": "value"}, signature="sig")
    batch = [event]
...
def test_siem_gateway_init(sample_settings_dict, sample_metrics):
    ...
    settings = siem_plugin.SIEMGatewaySettings(**sample_settings_dict)
    gateway = siem_plugin.SIEMGateway(target, settings, sample_metrics, siem_plugin.JsonHecSerializer(), None)
...
@pytest.mark.asyncio
async def test_siem_gateway_manager_startup_prod_checks(monkeypatch, sample_settings_dict, sample_metrics):
...
async def test_dead_letter_to_file(temp_dir):
    ...
    mock_secrets_manager.get_secret = MagicMock(side_effect=lambda key, required=False: 
