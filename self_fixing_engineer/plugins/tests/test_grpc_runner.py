...
def mock_prometheus():
    ...
    with patch("grpc_runner.PLUGIN_HEALTH_GAUGE") as mock_gauge, \
...
def test_get_tls_credentials_production_missing(mock_secrets_manager, mock_audit_logger, mock_alert_operator, set_env):
...
def test_is_endpoint_allowed_forbidden(mock_secrets_manager, mock_audit_logger, mock_alert_operator):
...
async def test_plugin_health_success(mock_health_stub, mock_prometheus):
    ...
    mock_health_stub.return_value = health_pb2.HealthCheckResponse(status=health_pb2.HealthCheckResponse.SERVING)
...
async def test_plugin_health_timeout(mock_health_stub, mock_prometheus):
    ...
    mock_counter.labels.assert_called_with(plugin_name="test_plugin", operation_type='health_check_timeout')
...
async def test_plugin_health_grpc_error_unavailable(mock_health_stub, mock_prometheus):
    ...
    mock_health_stub.side_effect = grpc.aio.AioRpcError(grpc.StatusCode.UNAVAILABLE, "details", "trailers")
    mock_gauge, mock_counter, _ = mock_prometheus
    status = await plugin_health(MagicMock(), "test_plugin")
    assert status == "NOT_SERVING"
    mock_gauge.labels().set.assert_called_with(0)
    mock_counter.labels.assert_called_with(plugin_name="test_plugin", operation_type='health_check_failed')
...
async def test_plugin_health_unhandled_error(mock_health_stub, mock_prometheus):
    ...
    assert status == "ERROR"
    mock_gauge.labels().set.assert_called_with(0)
    mock_counter.labels.assert_called_with(plugin_name="test_plugin", operation_type='health_check_error')
...
async def test_connect_success_secure(mock_secrets_manager, mock_grpc_channel, set_env):
    ...
    set_env({"PRODUCTION_MODE": "true"})
    mock_secrets_manager.get_secret.side_effect = lambda x, **kw: "/path/to/cert" if "CERT" in x else "/path/to/key" if "KEY" in x else "/path/to/ca" if "CA" in x else "allowed_endpoint"
...
def test_emit_metric_operations_counter(mock_prometheus):
    ...
    mock_gauge, mock_counter, _ = mock_prometheus
    emit_metric("plugin_operations_total", 1.0, {"plugin_name": "test_plugin", "operation_type": "test_op"}, "counter")
...
def test_validate_manifest_invalid_format(mock_secrets_manager):
    ...
    with pytest.raises(RuntimeError): # Using RuntimeError as a placeholder for a more specific exception
        validate_manifest(manifest)
...
def test_generate_plugin_docs_success(temp_dir):
    ...
    manifest = {"name": "test", "version": "1.0.0", "author": "Author", "description": "Desc"}
...
async def test_start_prometheus_exporter_failure(mock_alert_operator):
    ...
    with patch("prometheus_client.start_http_server", side_effect=Exception("Bind error")):
        ...
        assert "CRITICAL: Failed to start Prometheus exporter" in mock_alert_operator.call_args[0][0]
