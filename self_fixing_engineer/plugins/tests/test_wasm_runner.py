...
def generate_plugin_docs(plugin_dir, whitelist_dirs, output_path):
    with open(output_path, "w") as f:
        ...
        for name, manifest in plugins.items():
            ...
            f.write(f"**Description**: {manifest.get('description', 'No description provided.')}\n\n")
...
def mock_wasmtime(monkeypatch):
    ...
    return mock_config, mock_engine, mock_store, mock_linker, mock_module, mock_instance, mock_func
...
def temp_plugins_dir(tmp_path):
    ...
    manifest_content = {
        ...
        "sandbox": {"enabled": True, "resource_limits": {"memory": "64MB", "runtime_seconds": 5, "network": False}},
...
# --- WasmRunner Tests ---
@pytest.mark.asyncio
async def test_wasm_runner_init_success(mock_secrets_manager, mock_wasmtime, temp_wasm_file, mock_alert_operator):
...
@pytest.mark.asyncio
async def test_wasm_runner_init_demo_prod(set_env, mock_wasmtime, temp_wasm_file, mock_alert_operator):
...
@pytest.mark.asyncio
async def test_wasm_runner_init_outside_whitelist(mock_wasmtime, mock_alert_operator, temp_wasm_file):
...
def test_generate_plugin_docs_non_whitelisted(temp_plugins_dir, set_env):
    ...
    set_env({"PRODUCTION_MODE": "true"})
    with pytest.raises(SystemExit):
        generate_plugin_docs(str(temp_plugins_dir), ["/other_dir"], str(temp_plugins_dir / "docs.md"))
