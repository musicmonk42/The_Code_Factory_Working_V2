...
sys.modules['plugins.core_utils'] = MagicMock(
    ...
    scrub_secrets=mock_scrub_sensitive_data
)
sys.modules['plugins.core_audit'] = MagicMock(
    audit_logger=mock_audit_logger
)
sys.modules['plugins.core_secrets'] = MagicMock(
...
async def test_send_request_circuit_breaker(mock_aiohttp_session, gateway, monkeypatch):
    ...
    pagerduty_plugin.logger.debug(f"Circuit breaker test completed, state: {gateway._circuit_state}")
...
async def test_enqueue_request_queue_full(gateway, sample_settings_dict):
    ...
    async def patched_enqueue(req):
        try:
            ...
            pagerduty_plugin.logger.critical("PagerDuty event queue is full. Dropping event.", 
                                            extra={"dedup_key": req.dedup_key, 
                                                   "queue_size": test_gateway.settings.max_queue_size})
            pagerduty_plugin.audit_logger.log_event("pagerduty_event_dropped", 
                                                   dedup_key=req.dedup_key, 
                                                   reason="queue_full", 
                                                   queue_size=test_gateway.settings.max_queue_size)
            mock_alert_operator(f"CRITICAL: PagerDuty event queue is FULL ({test_gateway.settings.max_queue_size} events). Events are being dropped. IMMEDIATE ACTION REQUIRED!", 
                              level="CRITICAL")
