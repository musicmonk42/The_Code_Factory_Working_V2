...
def setup_environment(monkeypatch):
    ...
    if hasattr(monkeypatch, 'setenv'):
        ...
        monkeypatch.setenv("SLACK_GATEWAY_TARGETS", json.dumps([
            {"name": "alerts", "webhook_url": "https://hooks.slack.com/alerts"},
            {"name": "audit", "webhook_url": "https://hooks.slack.com/audit"}
        ]))
...
def test_slack_gateway_settings_default_secret(sample_settings_dict):
    ...
    with pytest.raises(ValidationError, match="CRITICAL: The signing_secret must not be the default value"):
...
@pytest.mark.asyncio
async def test_slack_gateway_settings_dry_run_prod(monkeypatch, sample_settings_dict, mock_secrets_manager):
...
def test_slack_block_kit_serializer():
    ...
    event = SlackEvent(event_name="test", details={"data": "value"}, severity="info", sequence_id=1, signature="sig")
...
async def test_slack_gateway_init(sample_settings_dict, sample_metrics, mock_secrets_manager):
    ...
    settings = SlackGatewaySettings(**sample_settings_dict)
    rate_limiter = TokenBucket(rate=1.0, capacity=10.0, metrics=sample_metrics, target_name="alerts")
    gateway = SlackGateway(target, settings, sample_metrics, SlackBlockKitSerializer(), rate_limiter, None)
...
@pytest.mark.asyncio
async def test_slack_gateway_manager_startup_prod_no_opentelemetry(monkeypatch, sample_settings_dict, sample_metrics):
...
async def test_slack_gateway_manager_health_check(sample_settings_dict, sample_metrics, temp_dir):
    ...
    with patch.object(SlackGateway, 'startup', new_callable=AsyncMock), \
...
async def test_dead_letter_to_file(temp_dir, mock_secrets_manager):
    ...
    file_operations = {"open_called": False, "write_called": False, "chmod_called": False}
    ...
    # Apply mocks
    with patch("plugins.slack_plugin.slack_plugin.DEAD_LETTER_DIR", str(temp_dir)), \
