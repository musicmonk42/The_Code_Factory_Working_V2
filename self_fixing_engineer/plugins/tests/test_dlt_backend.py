...
def dummy_checkpoint_manager(dummy_state):
    cm = dlt_backend.CheckpointManager(
        backend="dlt", enable_hash_chain=True, state_schema=None, encrypt_key=None
    )
    return cm
...
@pytest.mark.asyncio
async def test_initialize_dlt_backend_success(
    monkeypatch, s3_and_fabric_dummy, dummy_audit_logger, dummy_alert_operator
):
    config = {
    ...
    dummy_audit_logger.log_event.assert_called_with(
        "dlt_backend_initialized", off_chain_type="s3", dlt_type="fabric"
    )
...
@pytest.mark.asyncio
async def test_initialize_dlt_backend_dlt_fail(
    monkeypatch, s3_and_fabric_dummy, dummy_alert_operator
):
    class BadFabric:
...
@pytest.mark.asyncio
async def test_save_and_load_cycle(
    monkeypatch, redis_mock, scrub_patch, dummy_audit_logger, dummy_alert_operator
):
...
@pytest.mark.asyncio
async def test_off_chain_payload_corrupt(
    monkeypatch, redis_mock, scrub_patch, dummy_audit_logger, dummy_alert_operator
):
    await dlt_backend.initialize_dlt_backend(
        {
    ...
    await cm.save(
        "corrupt",
        {"value": 123, "timestamp": "2022-01-01T00:00:00Z", "metadata": {}},
        metadata={},
    )
    ...
    monkeypatch.setattr(
        dlt_backend,
        "decompress_json",
        lambda b: (_ for _ in ()).throw(ValueError("corrupt")),
    )
    with pytest.raises(dlt_backend.AnalyzerCriticalError):
