...
class CircuitBreaker:
    def __init__(self, threshold, reset_seconds, metrics):
        ...
        self._metrics.CIRCUIT_BREAKER_STATUS.labels = MagicMock(return_value=self._metrics.CIRCUIT_BREAKER_STATUS)
...
class PubSubGateway:
    ...
    def publish(self, event_name, service_name, details):
        ...
        self._event_queue.put_nowait(AuditEvent(event_name=event_name, service_name=service_name, details=details))
    ...
    async def _publish_batch(self, batch):
        ...
        for event in batch:
            self._publisher_client.publish("topic_path", json.dumps(event.model_dump()).encode('utf-8'))
...
def mock_secrets_manager():
    ...
    with patch("pubsub_plugin.SECRETS_MANAGER", mock):
        mock.get_secret.return_value = json.dumps({"type": "service_account", "project_id": "test-project"})
        yield mock
...
@pytest.mark.asyncio
async def test_gateway_startup_success(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_gateway_startup_topic_not_found(mock_pubsub_publisher, sample_settings_dict, mock_alert_operator, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_gateway_startup_credentials_failure(mock_pubsub_publisher, sample_settings_dict, mock_secrets_manager, mock_alert_operator):
...
@pytest.mark.asyncio
async def test_gateway_shutdown_success(mock_pubsub_publisher, sample_settings_dict, sample_metrics):
...
@pytest.mark.asyncio
async def test_gateway_shutdown_timeout(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_alert_operator):
...
@pytest.mark.asyncio
async def test_publish_success(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_publish_queue_full(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_alert_operator, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_publish_batch_success(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_tracer, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_publish_batch_service_unavailable(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_alert_operator, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_worker_success(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_secrets_manager):
...
@pytest.mark.asyncio
async def test_worker_dry_run(mock_pubsub_publisher, sample_settings_dict, sample_metrics, mock_secrets_manager):
...
def test_main_block_prod(set_env, mock_alert_operator):
    ...
    alert_args, _ = mock_alert_operator.call_args
    assert "CRITICAL: Pub/Sub plugin example code executed in PRODUCTION_MODE. Aborting." in alert_args[0]
    assert alert_args[1] == "CRITICAL"
