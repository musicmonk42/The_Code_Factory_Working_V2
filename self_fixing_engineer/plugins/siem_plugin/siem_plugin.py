...
class AuditJsonFormatter(jsonlogger.JsonFormatter):
    def __init__(self, *args, **kwargs):
        ...
        self._hmac_key = SECRETS_MANAGER.get_secret("SIEM_AUDIT_LOG_HMAC_KEY", required=PROD_MODE).encode()
...
try:
    ...
    logging.getLogger("siem_gateway").warning(f"Could not set up audit log file at {AUDIT_LOG_PATH}: {e}. Using a dummy handler.")
    ...
    def load_from_secure_vault(cls) -> "SIEMGatewaySettings":
        ...
        try:
            settings_dict = {
                ...
                'encryption_key': SECRETS_MANAGER.get_secret("SIEM_GATEWAY_ENCRYPTION_KEY", required=False),
            ...
            for key in ['persistence_dir', 'min_workers', 'max_workers', 'max_queue_size', 'worker_batch_size', 'admin_api_port', 'admin_api_host', 'requests_per_second_limit', 'circuit_breaker_threshold', 'circuit_breaker_reset_sec', 'dry_run', 'url_allowlist']:
            ...
            if targets_json:
                settings_dict['targets'] = [SIEMTarget.model_validate(t) for t in json.loads(targets_json)]
...
class SIEMMetrics:
    EVENTS_QUEUED = Counter("siem_audit_events_queued_total", "Events placed into the send queue.", ["target_name", "event_name"])
    EVENTS_DROPPED = Counter("siem_audit_events_dropped_total", "Events dropped due to a full queue.", ["target_name", "event_name"])
    EVENTS_SENT_SUCCESS = Counter("siem_audit_events_sent_success_total", "Events successfully sent to the SIEM.", ["target_name"])
    EVENTS_FAILED_PERMANENTLY = Counter("siem_audit_events_failed_permanently_total", "Events that failed to send after all retries.", ["target_name", "reason"])
    DEAD_LETTER_EVENTS = Counter("siem_audit_dead_letter_events_total", "Events sent to the dead-letter handler.", ["target_name", "reason"])
    SEND_LATENCY = Histogram("siem_audit_send_latency_seconds", "Latency of a successful batch send operation.", ["target_name"])
    CIRCUIT_BREAKER_STATUS = Gauge("siem_audit_circuit_breaker_status", "The status of the circuit breaker (1 for open, 0 for closed).", ["target_name"])
    RATE_LIMIT_THROTTLED_SECONDS = Counter("siem_audit_rate_limit_throttled_seconds_total", "Total time in seconds spent waiting for the rate limiter.", ["target_name"])
    ACTIVE_WORKERS = Gauge("siem_audit_active_workers", "Number of active worker tasks for a target.", ["target_name"])
    QUEUE_SIZE = Gauge("siem_audit_queue_size", "Current size of the in-memory event queue.", ["target_name"])
    QUEUE_LATENCY = Histogram("siem_audit_queue_latency_seconds", "Time from enqueue to processing.", ["target_name"])
...
class Serializer(Protocol):
    def encode_batch(self, batch: List[SIEMEvent], hostname: str, default_index: Optional[str]) -> bytes: ...
    @property
    def content_type(self) -> str: ...

class JsonHecSerializer:
    @property
    def content_type(self) -> str: return "application/json"
    def encode_batch(self, batch: List[SIEMEvent], hostname: str, default_index: Optional[str]) -> bytes:
        payload_str = "".join([json.dumps({
            "time": event.timestamp, "host": hostname, "source": event.source,
            "sourcetype": event.sourcetype, "index": event.index or default_index,
...
class PersistentWALQueue(EventQueue):
    def __init__(self, target_name: str, persistence_dir: str, max_in_memory_size: int, encryption_key: Optional[str] = None):
        ...
        self._hmac_key = SECRETS_MANAGER.get_secret("SIEM_WAL_HMAC_KEY", required=PROD_MODE).encode()
    ...
    async def _open_next_log_segment(self):
        async with self._write_lock:
            ...
            try:
                ...
                main_logger.critical(f"Failed to open next log segment: {e}. Exiting.", extra={"context": {"target": os.path.basename(self._dir)}})
                raise RuntimeError("Critical startup failure: WAL log segment could not be opened.") from e
...
class CircuitBreaker:
    ...
    def record_failure(self):
        ...
        if self._failure_count >= self._threshold and not self._is_open:
            ...
            alert_operator(f"CRITICAL: SIEM circuit breaker tripped for {self._target_name}.", "CRITICAL")
...
class SIEMGateway:
    def __init__(self, target_config: SIEMTarget, global_settings: SIEMGatewaySettings, metrics: SIEMMetrics, serializer: Serializer, dead_letter_hook: Optional[DeadLetterHook]):
        ...
        self.rate_limiter = TokenBucket(global_settings.requests_per_second_limit, global_settings.requests_per_second_limit, metrics, target_config.name)
    ...
    async def startup(self):
        ...
        self.metrics.ACTIVE_WORKERS.labels(target_name=self.target_config.name).set(len(self._workers))
    ...
    async def shutdown(self):
        ...
        await self._event_queue.flush(timeout=self.global_settings.max_queue_size / self.global_settings.worker_batch_size + 10)
    ...
    async def _get_session(self) -> aiohttp.ClientSession:
        async with self._session_lock:
            if self._session is None or self._session.closed:
                ...
                self._session = aiohttp.ClientSession(timeout=timeout, ssl=ssl_context if PROD_MODE else False)
        return self._session

    async def _handle_dead_letter(self, event: SIEMEvent, reason: str):
        self.metrics.DEAD_LETTER_EVENTS.labels(target_name=self.target_config.name, reason=reason).inc()
    ...
    async def publish(self, event: SIEMEvent):
        try:
            ...
            self.metrics.EVENTS_QUEUED.labels(target_name=self.target_config.name, event_name=event.event_name).inc()
            ...
            try:
                ...
                self.metrics.EVENTS_DROPPED.labels(target_name=self.target_config.name, event_name=event.event_name).inc()
    ...
    async def _publish_batch(self, batch: List[SIEMEvent]):
        ...
        with tracer.start_as_current_span("siem_publish_batch", attributes={
            ...
            payload = self.serializer.encode_batch(deduped_batch, self._hostname, self.target_config.index)
            headers = {"Authorization": f"Splunk {self.target_config.token}", "Content-Type": self.serializer.content_type}
            attempt = 0
            while attempt < self.global_settings.max_retries:
                ...
                async with self._concurrency_limiter:
                    ...
                    try:
                        ...
                        async with session.post(self.target_config.url, data=payload, headers=headers, ssl=self.global_settings.verify_ssl, timeout=timeout) as resp:
                            ...
                            if resp.status == 200:
                                ...
                                self.metrics.EVENTS_SENT_SUCCESS.labels(target_name=self.target_config.name).inc(len(deduped_batch))
                                self.circuit_breaker.record_success()
                                audit_logger.info("batch_sent_success", extra={"context": {"target": self.target_config.name, "batch_size": len(deduped_batch)}})
                                return True
                            elif resp.status == 429:
                                retry_after = int(resp.headers.get("Retry-After", "5"))
                                main_logger.warning(f"Rate limited by API. Backing off for {retry_after} seconds.", extra={"context": {"target": self.target_config.name}})
                                await asyncio.sleep(retry_after)
                                continue
                            elif 400 <= resp.status < 500:
                                error_text = await resp.text()
                                main_logger.error(f"Permanent failure sending to SIEM (client error): {error_text}", extra={"context": {"target": self.target_config.name, "status": resp.status}})
                                for event in deduped_batch: await self._handle_dead_letter(event, "client_error")
                                self.metrics.EVENTS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason=f"client_error_{resp.status}").inc(len(deduped_batch))
                                return False
                            else: resp.raise_for_status()
                    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
                        main_logger.warning(f"Temporary failure sending to SIEM: {e}", extra={"context": {"target": self.target_config.name, "attempt": attempt + 1}})
                        if attempt + 1 >= self.global_settings.max_retries:
                            ...
                            self.metrics.EVENTS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason="service_unavailable").inc(len(deduped_batch))
                            return False
                attempt += 1
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                if self.global_settings.dry_run:
                    main_logger.info("[DRY RUN] Would publish batch.", extra={"context": {"target": self.target_config.name, "batch_size": len(batch)}})
                    success = True
                else:
    ...
    async def _worker_manager(self):
        ...
        while not self._shutdown_event.is_set():
            ...
            # Scale up
            if avg_queue > self.global_settings.queue_size_per_worker * len(active_workers) and len(active_workers) < self.global_settings.max_workers and cpu_usage < 80:
    ...
    async def _heartbeat(self):
        while not self._shutdown_event.is_set():
            try:
                await self._publish_batch([SIEMEvent(event_name="heartbeat", details={"ping": "pong"}, source="siem-gateway-internal", sequence_id=0, signature="")])
                self.circuit_breaker.record_success()
            except Exception as e:
                self.circuit_breaker.record_failure()
...
class SIEMGatewayManager:
    def __init__(self, settings: SIEMGatewaySettings, metrics: SIEMMetrics, dead_letter_hook: Optional[DeadLetterHook] = None):
        self.settings = settings
        self.metrics = metrics
        self.dead_letter_hook = dead_letter_hook
        self._gateways: Dict[str, SIEMGateway] = {}
        self._serializers: Dict[str, Serializer] = {"json_hec": JsonHecSerializer(), "gzip_json_hec": GzipJsonHecSerializer()}
    ...
    async def startup(self):
        if PROD_MODE and self.settings.dry_run:
            main_logger.critical("DRY_RUN is enabled in production mode. This is a critical error. Exiting.")
            sys.exit(1)
        ...
        audit_logger.info("startup", extra={"context": {"status": "starting", "config_version": self._config_version}})
    ...
    async def reload_config(self, new_settings: SIEMGatewaySettings):
        async with self._lock:
            ...
            for name, target_config in new_targets_map.items():
                ...
                new_gateways[name] = SIEMGateway(target_config, new_settings, self.metrics, serializer, self.dead_letter_hook)
    ...
    def publish(self, target_name: str, event_name: str, details: Dict[str, Any], **kwargs):
        ...
        try:
            event = SIEMEvent(event_name=event_name, details=details, source=kwargs.get("source", gateway.target_config.source), index=kwargs.get("index", gateway.target_config.index), trace_context=trace_context, sequence_id=seq_id, signature=signature)
            asyncio.create_task(gateway.publish(event))
        except ValidationError as e: 
            main_logger.error("Invalid SIEM event schema.", extra={"context": {"error": str(e), "event_name": event_name}})
    ...
    async def _run_admin_api_server(self):
        ...
        async def handle_pause(request):
            ...
            if gw := self._gateways.get(target_name):
                gw.pause()
                audit_logger.info("api_pause_target", extra={"context": {"target": target_name, "source_ip": request.remote}})
        ...
        async def handle_resume(request):
            ...
            if gw := self._gateways.get(target_name):
                gw.resume()
                audit_logger.info("api_resume_target", extra={"context": {"target": target_name, "source_ip": request.remote}})
...
DEAD_LETTER_DIR = os.environ.get("SIEM_GATEWAY_DEAD_LETTER_DIR", "/var/lib/siem_gateway_dead_letters")
...
async def dead_letter_to_file(event: SIEMEvent, reason: str):
    ...
    encryption_key = SECRETS_MANAGER.get_secret("SIEM_GATEWAY_DEAD_LETTER_ENCRYPTION_KEY", required=False)
    ...
    try:
        if PROD_MODE:
            ...
            siem_settings = SIEMGatewaySettings(
                signing_secret=os.environ.get("SIEM_GATEWAY_SIGNING_SECRET", "non-prod-signing-secret"),
                admin_api_key=os.environ.get("SIEM_GATEWAY_ADMIN_API_KEY", "non-prod-admin-key"),
                targets=[
                    SIEMTarget(name="security", url=os.environ.get("SIEM_SEC_URL", "http://localhost:8088/services/collector"), token=os.environ.get("SIEM_SEC_TOKEN", "token-sec")),
                    SIEMTarget(name="ops", url=os.environ.get("SIEM_OPS_URL", "http://localhost:8088/services/collector"), token=os.environ.get("SIEM_OPS_TOKEN", "token-ops")),
                ],
                url_allowlist=['^http://localhost', '^https://example.com']
            )

        siem_metrics = SIEMMetrics()
        siem_gateway_manager = SIEMGatewayManager(siem_settings, siem_metrics, dead_letter_hook=dead_letter_to_file)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        siem_gateway_manager.publish("security", "user_login_failed", {"user": "hacker", "ip": "1.1.1.1"})
        siem_gateway_manager.publish("ops", "system_status_ok", {"service": "web-server", "uptime_minutes": 300})
        siem_gateway_manager.publish("security", "new_user_created", {"user": "alice", "password": "super-secret-password"})
