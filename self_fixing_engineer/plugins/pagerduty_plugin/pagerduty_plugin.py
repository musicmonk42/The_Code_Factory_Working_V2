...
try:
    ...
    raise StartupCriticalError(f"CRITICAL: Missing core dependency for PagerDuty plugin: {e}. Aborting startup.")
...
if not logger.handlers:
    ...
    if PRODUCTION_MODE:
        try:
            ...
            alert_operator(f"CRITICAL: PagerDuty plugin failed to configure file logging to {LOG_FILE_PATH}: {e}. Aborting.", level="CRITICAL")
            raise StartupCriticalError(f"CRITICAL: PagerDuty plugin failed to configure file logging: {e}.")
    else:
...
try:
    ...
    from prometheus_client import Counter, Gauge, Histogram, CollectorRegistry, REGISTRY, Info
except ImportError as e:
    logger.critical(f"CRITICAL: Missing core dependency for PagerDuty plugin: {e}. Aborting startup.")
    alert_operator(f"CRITICAL: PagerDuty plugin missing core dependency: {e}. Aborting.", level="CRITICAL")
...
class PagerDutySettings(BaseSettings):
    ...
    routing_key_secret_id: str = Field(..., description="Secret ID for routing_key in a secure vault.")
    timeout_seconds: float = Field(10.0, ge=1.0, description="Timeout for HTTP requests to PagerDuty API.")
    ...
    dry_run: bool = Field(False, description="If true, events are logged but not sent to PagerDuty.")
    max_concurrent_requests: int = Field(10, ge=1, description="Maximum concurrent HTTP requests to PagerDuty API.")
    max_queue_size: int = Field(1000, ge=0, description="Maximum number of events to buffer in the internal queue.")
    circuit_breaker_threshold: int = Field(5, ge=1, description="Number of consecutive failures to trip the circuit breaker.")
    ...
    pagerduty_api_url: str = Field("https://events.pagerduty.com/v2/enqueue", description="PagerDuty Events API endpoint.")
    ...
    def validate_production_mode_settings(self) -> 'PagerDutySettings':
        if PRODUCTION_MODE:
            ...
            if not self.pagerduty_api_url.lower().startswith("https://"):
                raise ValueError(f"Non-HTTPS endpoint '{self.pagerduty_api_url}' detected in PRODUCTION_MODE. HTTPS is mandatory.")
            ...
            if self.pagerduty_api_url not in allowed_endpoints:
                raise ValueError(f"PagerDuty API URL '{self.pagerduty_api_url}' is not in the 'allowed_pagerduty_api_urls' list.")
            
            if any(term in self.pagerduty_api_url.lower() for term in ["dummy", "test", "mock", "example.com"]):
                raise ValueError(f"Dummy/test PagerDuty API URL '{self.pagerduty_api_url}' detected in PRODUCTION_MODE.")
...
try:
    ...
    logger.critical(f"CRITICAL: PagerDutySettings validation failed: {e}. Aborting startup.", extra={'validation_errors': e.errors()})
    alert_operator(f"CRITICAL: PagerDutySettings validation failed: {e}. Aborting.", level="CRITICAL")
    raise StartupCriticalError("PagerDutySettings validation failed.")
except Exception as e:
    logger.critical(f"CRITICAL: Unexpected error loading PagerDutySettings: {e}. Aborting startup.", exc_info=True)
    alert_operator(f"CRITICAL: Unexpected error loading PagerDutySettings: {e}. Aborting.", level="CRITICAL")
...
class PagerDutyEventPayload(BaseModel):
    ...
    def validate_and_scrub_pii(cls, data: Any) -> Any:
        ...
        if scrubbed_details != custom_details:
            logger.warning("Sensitive fields scrubbed from custom_details. PagerDuty event will proceed.")
            audit_logger.log_event("pagerduty_custom_details_scrubbed", details_snippet=scrub_sensitive_data(str(custom_details)[:100]))
...
class PagerDutyAPIRequest(BaseModel):
    ...
    def _sign_request(self) -> str:
        ...
        request_json_str = json.dumps(request_payload, sort_keys=True, ensure_ascii=False).encode('utf-8')
        pagerduty_hmac_key = SECRETS_MANAGER.get_secret("PAGERDUTY_HMAC_KEY", required=PRODUCTION_MODE)
        
        if PRODUCTION_MODE and not pagerduty_hmac_key:
            raise PagerDutyEventError("Missing 'PAGERDUTY_HMAC_KEY' for event signing. Aborting send.")
        
        return hmac.new(pagerduty_hmac_key.encode('utf-8'), request_json_str, hashlib.sha256).hexdigest()
...
class PagerDutyGateway:
    ...
    def __init__(self, settings: PagerDutySettings, metrics: PagerDutyMetrics):
        ...
        self._routing_key: Optional[str] = SECRETS_MANAGER.get_secret(self.settings.routing_key_secret_id, required=True)
        ...
        logger.info("PagerDuty Gateway initialized.", extra={'api_url': self.settings.pagerduty_api_url, 'max_queue_size': self.settings.max_queue_size})
        audit_logger.log_event("pagerduty_gateway_init", api_url=self.settings.pagerduty_api_url, max_queue_size=self.settings.max_queue_size)
    ...
    async def startup(self):
        ...
        if not self._workers:
            ...
            logger.info(f"PagerDuty Gateway started with {len(self._workers)} background event processors.")
            audit_logger.log_event("pagerduty_gateway_startup", status="success", worker_count=len(self._workers))
    ...
    async def shutdown(self):
        ...
        audit_logger.log_event("pagerduty_gateway_shutdown_start", queue_size=self._event_queue.qsize())
        
        for _ in self._workers:
            await self._event_queue.put(None)
        
        try:
            await asyncio.wait_for(self._event_queue.join(), timeout=self.settings.timeout_seconds * self.settings.max_retries * 2 + 10)
            logger.info("PagerDuty Gateway queue drained successfully.")
            audit_logger.log_event("pagerduty_gateway_queue_drained", remaining_events=self._event_queue.qsize())
        except asyncio.TimeoutError:
            remaining_events = self._event_queue.qsize()
            logger.critical(f"CRITICAL: PagerDuty Gateway failed to drain queue within timeout. {remaining_events} events remain unsent.")
            audit_logger.log_event("pagerduty_gateway_shutdown_failure", reason="queue_drain_timeout", remaining_events=remaining_events)
            alert_operator(f"CRITICAL: PagerDuty Gateway queue NOT drained. {remaining_events} events unsent. Aborting.", level="CRITICAL")
            raise StartupCriticalError(f"Queue drain timed out with {remaining_events} events remaining.")
        except Exception as e:
            logger.critical(f"CRITICAL: Unexpected error during queue drain: {e}.", exc_info=True)
            audit_logger.log_event("pagerduty_gateway_shutdown_failure", reason="queue_drain_error", error=str(e))
            alert_operator(f"CRITICAL: PagerDuty Gateway queue drain failed unexpectedly: {e}.", level="CRITICAL")
    ...
    async def _check_circuit_breaker(self):
        ...
        if self._circuit_state == "open":
            if time.monotonic() - self._last_failure_time > self.settings.circuit_breaker_reset_sec:
                ...
                raise PagerDutyEventError("Circuit breaker is open. PagerDuty requests are temporarily suspended.")
    ...
    def _handle_failure(self, event_action: str, dedup_key: str):
        ...
        self._last_failure_time = time.monotonic()
        if self._failure_count >= self.settings.circuit_breaker_threshold and self._circuit_state == "closed":
            self._circuit_state = "open"
            self.metrics.CIRCUIT_BREAKER_STATUS.set(1)
            logger.error("Circuit breaker tripped due to excessive failures.")
            audit_logger.log_event("pagerduty_circuit_breaker_tripped",
                                   failure_count=self._failure_count,
                                   event_action=event_action, dedup_key=dedup_key)
            alert_operator(f"CRITICAL: PagerDuty Circuit Breaker TRIPPED. Requests suspended. Failures: {self._failure_count}", level="CRITICAL")
    ...
    async def _send_request(self, request: PagerDutyAPIRequest):
        ...
        while attempt <= self.settings.max_retries:
            ...
            log_context = {"dedup_key": request.dedup_key, "action": request.event_action, "attempt": attempt + 1}
            audit_logger.log_event("pagerduty_send_attempt", **log_context, api_url=self.settings.pagerduty_api_url)
            ...
            try:
                ...
                async with session.post(self.settings.pagerduty_api_url, json=request_body) as resp:
                    ...
                    if resp.status in [200, 202]:
                        ...
                        logger.info("Successfully sent PagerDuty event.", extra={"context": log_context, "status": resp.status})
                        audit_logger.log_event("pagerduty_send_success", **log_context, status=resp.status)
                        return
                    ...
                    if resp.status == 429:
                        ...
                        try:
                            ...
                            delay = 1.0 + jitter
                        logger.warning(f"Rate limited by PagerDuty. Retrying in {delay:.2f} seconds.", extra={"context": log_context, "status": resp.status})
                    ...
                    if 400 <= resp.status < 500:
                        ...
                        logger.error("Permanent PagerDuty client error. Dropping event.", extra={"context": log_context, "status": resp.status, "response": scrub_sensitive_data(text)})
                        audit_logger.log_event("pagerduty_send_permanent_failure", **log_context, status=resp.status, response=scrub_sensitive_data(text))
                        return
                    
                    resp.raise_for_status()
            except (aiohttp.ClientError, asyncio.TimeoutError) as exc:
                logger.warning("Temporary failure sending to PagerDuty. Retrying...", extra={"context": log_context, "error": str(exc)})
                audit_logger.log_event("pagerduty_send_retriable_failure", **log_context, error=str(exc))
                if attempt == self.settings.max_retries:
                    self._handle_failure(request.event_action, request.dedup_key)
                    self.metrics.EVENTS_FAILED_PERMANENTLY.labels(reason='server_error').inc()
                    logger.error("Failed to send PagerDuty event after all retries.", extra={"context": log_context})
                    audit_logger.log_event("pagerduty_send_final_failure", **log_context, error=str(exc))
                else:
                    await asyncio.sleep(self.settings.retry_backoff_factor ** attempt)
            except Exception as e:
                logger.error("Unhandled exception in _send_request.", extra={"context": log_context, "error": str(e)}, exc_info=True)
                self._handle_failure(request.event_action, request.dedup_key)
                audit_logger.log_event("pagerduty_send_unhandled_exception", **log_context, error=str(e))
                alert_operator(f"CRITICAL: Unhandled exception sending PagerDuty event: {e}.", level="CRITICAL")
                return
            attempt += 1
    ...
    async def _event_processor_task(self, worker_id: int):
        ...
        while True:
            ...
            if self.settings.dry_run:
                ...
                safe_body['payload']['custom_details'] = scrub_sensitive_data(safe_body['payload']['custom_details'])
                logger.info("[DRY RUN] Would send PagerDuty event.", extra={"context": {"event_body": safe_body}})
                audit_logger.log_event("pagerduty_dry_run_event", dedup_key=request.dedup_key, action=request.event_action)
                self._event_queue.task_done()
                continue
            ...
            try:
                ...
                logger.warning("Event dropped due to error.", extra={"context": {"dedup_key": request.dedup_key, "error": str(e)}})
                audit_logger.log_event("pagerduty_event_dropped", dedup_key=request.dedup_key, reason="payload_error", error=str(e))
            except Exception as e:
                logger.error("Unhandled exception in event processor.", extra={"context": {"error": str(e)}}, exc_info=True)
                audit_logger.log_event("pagerduty_event_processor_unhandled_exception", dedup_key=request.dedup_key, error=str(e))
                alert_operator(f"CRITICAL: Unhandled exception in PagerDuty event processor: {e}.", level="CRITICAL")
            finally:
                self._event_queue.task_done()
    ...
    async def _enqueue_request(self, request: PagerDutyAPIRequest):
        ...
        try:
            ...
            self.metrics.EVENTS_DROPPED.inc()
            logger.critical("PagerDuty event queue is full. Dropping event.", extra={"dedup_key": request.dedup_key, "queue_size": self.settings.max_queue_size})
            audit_logger.log_event("pagerduty_event_dropped", dedup_key=request.dedup_key, reason="queue_full", queue_size=self.settings.max_queue_size)
            alert_operator(f"CRITICAL: PagerDuty event queue is FULL ({self.settings.max_queue_size} events). Events are being dropped. IMMEDIATE ACTION REQUIRED!", level="CRITICAL")

    async def trigger(self, event_name: str, details: Dict[str, Any], severity: Literal["critical", "error", "warning", "info"], source: str, dedup_key: str):
        try:
            timestamp_utc = datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
            payload = PagerDutyEventPayload(summary=f"[{severity.upper()}] {event_name}", source=source, severity=severity, custom_details=details, timestamp=timestamp_utc)
            request = PagerDutyAPIRequest(routing_key=self._routing_key, event_action="trigger", dedup_key=dedup_key, payload=payload)
        except (ValidationError, PagerDutyEventError) as e:
            logger.error(f"PagerDuty event payload validation failed for trigger: {e}", extra={"event_name": event_name, "details": scrub_sensitive_data(details)})
            audit_logger.log_event("pagerduty_payload_validation_failed", event_name=event_name, action="trigger", error=str(e), details_snippet=scrub_sensitive_data(str(details)[:100]))
            alert_operator(f"CRITICAL: PagerDuty event payload validation failed for trigger '{event_name}': {e}.", level="CRITICAL")
            return
    ...
    async def acknowledge(self, dedup_key: str):
        try:
            request = PagerDutyAPIRequest(routing_key=self._routing_key, event_action="acknowledge", dedup_key=dedup_key)
        except (ValidationError, PagerDutyEventError) as e:
            logger.error(f"PagerDuty event payload validation failed for acknowledge: {e}", extra={"dedup_key": dedup_key})
            audit_logger.log_event("pagerduty_payload_validation_failed", dedup_key=dedup_key, action="acknowledge", error=str(e))
            alert_operator(f"CRITICAL: PagerDuty event payload validation failed for acknowledge '{dedup_key}': {e}.", level="CRITICAL")
            return
        await self._enqueue_request(request)

    async def resolve(self, dedup_key: str):
        try:
            request = PagerDutyAPIRequest(routing_key=self._routing_key, event_action="resolve", dedup_key=dedup_key)
        except (ValidationError, PagerDutyEventError) as e:
            logger.error(f"PagerDuty event payload validation failed for resolve: {e}", extra={"dedup_key": dedup_key})
            audit_logger.log_event("pagerduty_payload_validation_failed", dedup_key=dedup_key, action="resolve", error=str(e))
            alert_operator(f"CRITICAL: PagerDuty event payload validation failed for resolve '{dedup_key}': {e}.", level="CRITICAL")
            return
        await self._enqueue_request(request)
...
if not PRODUCTION_MODE:
    ...
    async def main():
        async with app_lifecycle():
            ...
            await pagerduty_gateway.trigger(event_name="database_connection_failed", details={"db_host": "prod-db-1", "db_pass": "secret_db_pass"}, severity="critical", source="db-monitor", dedup_key=incident_key)
            await pagerduty_gateway.trigger(event_name="user_login_brute_force", details={"ip": "1.2.3.4"}, severity="warning", source="auth-service", dedup_key="brute-force-1.2.3.4")
    ...
    if __name__ == "__main__":
        try:
            ...
            print(f"Failed to run example: {e}. Please install aiohttp (`pip install aiohttp`) to run the usage example.")
        except StartupCriticalError as e:
            print(f"Example execution aborted due to critical startup error: {e}", file=sys.stderr)
            sys.exit(1)
else:
    if __name__ == "__main__":
        ...
        alert_operator("CRITICAL: PagerDuty plugin example code executed in PRODUCTION_MODE. Aborting.", level="CRITICAL")
