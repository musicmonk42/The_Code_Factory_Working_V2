...
if PRODUCTION_MODE:
    logger.critical("CRITICAL: Attempting to load 'demo_python_plugin.py' in PRODUCTION_MODE. This plugin is for demo/test purposes only and is forbidden in production builds. Aborting startup.")
    alert_operator("CRITICAL: Demo plugin 'demo_python_plugin.py' detected in PRODUCTION_MODE. Aborting.", level="CRITICAL")
    sys.exit(1)
...
def plugin_health():
    ...
    """
    health_status = {"status": "ok", "message": "Demo Python plugin is healthy!", "details": {}}
    ...
    if missing_deps:
        ...
        audit_logger.log_event("plugin_health_degraded", plugin=PLUGIN_MANIFEST["name"], reason="missing_dependencies", details=scrub_secrets(missing_deps))
    ...
    try:
        ...
        logger.error(f"Plugin health check: {health_status['message']}", exc_info=True, extra=health_status)
        audit_logger.log_event("plugin_health_unhealthy", plugin=PLUGIN_MANIFEST["name"], reason="runtime_failure", error=scrub_secrets(str(e)))
        alert_operator(f"WARNING: Demo plugin '{PLUGIN_MANIFEST['name']}' health check failed: {e}", level="WARNING")
    ...
    audit_logger.log_event("plugin_health_check", plugin=PLUGIN_MANIFEST["name"], status=health_status["status"], details=scrub_secrets(health_status))
    return health_status

@plugin(kind=PlugInKind.EXECUTION, name="demo_python_plugin", description="A simple demo Python plugin.", version="0.0.1", safe=True)
class PLUGIN_API:
    def hello(self):
        ...
        if os.getenv("RUN_QA_TESTS", "false").lower() == "true":
            audit_logger.log_event("demo_plugin_hello_executed_in_qa", plugin=PLUGIN_MANIFEST["name"])
