...
try:
    ...
    try:
        ...
        fcntl = None
        def file_lock(fd): pass
        def file_unlock(fd): pass
    ...
    class DummySecretsManager:
        def get_secret(self, key, required=True):
            ...
            if required and not value:
                alert_operator(f"CRITICAL: Failed to fetch required secret '{key}'. Aborting.", level="CRITICAL")
                sys.exit(1)
            return value
...
class AuditJsonFormatter(jsonlogger.JsonFormatter):
    def __init__(self, *args, **kwargs):
        ...
        self._hmac_key = SECRETS_MANAGER.get_secret("SNS_AUDIT_LOG_HMAC_KEY", required=PROD_MODE).encode()
    ...
    def add_fields(self, log_record, message_dict):
        ...
        signature = hmac.new(self._hmac_key, payload_str.encode('utf-8'), hashlib.sha256).hexdigest()
        log_record['signature'] = signature

AUDIT_LOG_PATH = os.environ.get("SNS_AUDIT_LOG_FILE", "/var/log/sns_audit.log")
try:
    os.makedirs(os.path.dirname(AUDIT_LOG_PATH), exist_ok=True)
    audit_log_handler = RotatingFileHandler(AUDIT_LOG_PATH, maxBytes=10*1024*1024, backupCount=5)
    os.chmod(AUDIT_LOG_PATH, 0o600)
except Exception as e:
    main_logger = logging.getLogger("sns_plugin")
    main_logger.warning(f"Could not set up audit log file at {AUDIT_LOG_PATH}: {e}. Using a dummy handler.")
    class DummyHandler(logging.Handler):
        def emit(self, record): pass
    audit_log_handler = DummyHandler()
    
audit_log_handler.setFormatter(AuditJsonFormatter('%(timestamp)s %(hostname)s %(service_name)s %(levelname)s %(message)s'))
...
class SNSGatewaySettings(BaseSettings):
    ...
    def validate_secrets(cls, v: str, info) -> str:
        if v in ("default-sns-secret-key-change-me", ""):
            raise ValueError(f"CRITICAL: The {info.field_name} must not be the default value or empty. Use a secure vault.")
        return v
    ...
    def load_from_secure_vault(cls) -> "SNSGatewaySettings":
        ...
        try:
            settings_dict = {
                ...
                'encryption_key': SECRETS_MANAGER.get_secret("SNS_GATEWAY_ENCRYPTION_KEY", required=False),
            ...
            key_expiry = SECRETS_MANAGER.get_secret("SNS_GATEWAY_ENCRYPTION_KEY_EXPIRY", required=False)
            if key_expiry and time.time() > float(key_expiry):
                raise ValueError("Encryption key expired. Aborting startup.")
            
            for key in ['persistence_dir', 'min_workers', 'max_workers', 'max_queue_size', 'worker_batch_size', 'worker_linger_sec', 'max_concurrent_requests', 'requests_per_second_limit', 'max_retries', 'retry_backoff_factor', 'circuit_breaker_threshold', 'circuit_breaker_reset_sec', 'admin_api_port', 'admin_api_host', 'dry_run', 'dry_run_failure_rate', 'verify_ssl', 'admin_api_enabled', 'strict_plugins', 'url_allowlist', 'admin_api_ip_allowlist', 'heartbeat_interval', 'compaction_interval', 'max_concurrent_per_region']:
                env_key = f'SNS_GATEWAY_{key.upper()}'
                if env_key in os.environ:
                    if PROD_MODE:
                        main_logger.warning(f"Env override for {key} in production. Use a secure vault for full compliance.")
                    settings_dict[key] = os.environ[env_key]

            targets_json = SECRETS_MANAGER.get_secret("SNS_GATEWAY_TARGETS", required=False)
            if targets_json:
                settings_dict['targets'] = [SNSTarget.model_validate(t) for t in json.loads(targets_json)]
            ...
            if PROD_MODE:
                ...
                for target in settings.targets:
                    if not target.access_key_id.startswith('AKIA'):
                        raise ValueError(f"Invalid AWS access key format for '{target.name}'. Must start with 'AKIA'.")

                    endpoint = target.url_endpoint or f"https://sns.{target.region}.amazonaws.com"
                    if not settings.url_allowlist and not re.match(r"^https://sns\..*\.amazonaws\.com", endpoint):
                        raise ValueError(f"URL for SNS target '{target.name}' is not a valid AWS SNS endpoint and no allowlist is provided.")
                    if settings.url_allowlist and not any(re.match(pattern, endpoint) for pattern in settings.url_allowlist):
                        raise ValueError(f"URL for SNS target '{target.name}' not in allowed_urls list.")
            
            return settings
        except (KeyError, json.JSONDecodeError, ValidationError) as e:
            main_logger.critical(f"Failed to load production configuration from secure source. Error: {e}")
            raise RuntimeError("Critical startup failure: Secure configuration could not be loaded.") from e

# ---- 2. Granular, Labeled Metrics ----
class SNSMetrics:
    NOTIFICATIONS_QUEUED = Counter("sns_notifications_queued_total", "Notifications placed into the send queue.", ["target_name", "event_name", "severity"])
    NOTIFICATIONS_DROPPED = Counter("sns_notifications_dropped_total", "Notifications dropped due to a full queue.", ["target_name", "event_name"])
    NOTIFICATIONS_SENT_SUCCESS = Counter("sns_notifications_sent_success_total", "Notifications successfully sent to SNS.", ["target_name"])
    NOTIFICATIONS_FAILED_PERMANENTLY = Counter("sns_notifications_failed_permanently_total", "Notifications that failed to send after all retries.", ["target_name", "reason"])
    DEAD_LETTER_NOTIFICATIONS = Counter("sns_dead_letter_notifications_total", "Notifications sent to the dead-letter handler.", ["target_name", "reason"])
    SEND_LATENCY = Histogram("sns_send_latency_seconds", "Latency of a successful batch send operation.", ["target_name"])
    CIRCUIT_BREAKER_STATUS = Gauge("sns_circuit_breaker_status", "The status of the circuit breaker (1 for open, 0 for closed).", ["target_name"])
    RATE_LIMIT_THROTTLED_SECONDS = Counter("sns_rate_limit_throttled_seconds_total", "Total time in seconds spent waiting for the rate limiter.", ["target_name"])
    ACTIVE_WORKERS = Gauge("sns_active_workers", "Number of active worker tasks for a target.", ["target_name"])
    NON_TRACED_NOTIFICATIONS = Counter("sns_non_traced_notifications_total", "Events sent without OpenTelemetry tracing.", ["target_name"])
    QUEUE_SIZE = Gauge("sns_queue_size", "Current size of the event queue.", ["target_name"])
    WAL_COMPACTIONS = Counter("sns_wal_compactions_total", "Number of WAL compactions performed.", ["target_name"])
    QUEUE_LATENCY = Histogram("sns_queue_latency_seconds", "Time from enqueue to processing.", ["target_name"])
    RETRY_ATTEMPTS = Histogram("sns_retry_attempts", "Number of retry attempts per batch.", ["target_name"])
...
class SNSEvent(BaseModel):
    ...
    SENSITIVE_KEYS = re.compile(r".*(password|secret|key|token|pii|ssn|credit_card|credentials).*", re.IGNORECASE)
    SENSITIVE_PATTERNS = [
            ...
            if isinstance(data, dict):
                ...
                for key, value in data.items():
                    if cls.SENSITIVE_KEYS.match(key):
                        audit_logger.critical(f"Detected and scrubbed sensitive key '{key}' from event.")
...
class PersistentWALQueue(EventQueue):
    def __init__(self, target_name: str, persistence_dir: str, max_in_memory_size: int, metrics: SNSMetrics, dead_letter_hook: Callable, encryption_key: Optional[str] = None):
        self._dir = os.path.join(persistence_dir, f"{target_name}_{hashlib.sha256(target_name.encode()).hexdigest()}")
    ...
    async def startup(self):
        ...
        try:
            log_files = sorted([f for f in os.listdir(self._dir) if f.startswith("events.") and f.endswith(".log")])
                    ...
                    async for line in f:
                        if line.strip():
                            try:
                                ...
                                sig, data = line.strip().split(b':', 1) if self._cipher else line.strip().split(':', 1)
                                
                                data_to_check = data.encode('utf-8') if isinstance(data, str) else data
                                
                                if not hmac.compare_digest(sig.decode('utf-8'), hmac.new(self._hmac_key, data_to_check, hashlib.sha256).hexdigest()):
                                    raise ValueError("WAL integrity check failed.")

                                decrypted_line = self._cipher.decrypt(data) if self._cipher else data.encode('utf-8')
                                event = SNSEvent.model_validate_json(decrypted_line)
                                await self._mem_queue.put(event)
                            except Exception as e:
                                main_logger.warning(f"Skipping corrupt WAL line: {e}", extra={"context": {"line_snippet": line[:50]}})
                                await self._dead_letter_hook(SNSEvent(event_name="corrupt_wal", details={"line": str(line[:50])}), "wal_corrupt")
                    file_unlock(f)
            main_logger.info(f"Loaded {self._mem_queue.qsize()} events from disk for target {self._target_name}.")
            audit_logger.info("WAL loaded from disk.", extra={"context": {"target": self._target_name, "event_count": self._mem_queue.qsize()}})
        except OSError as e:
            main_logger.critical(f"Failed to load WAL from disk: {e}. Exiting.", extra={"context": {"target": self._target_name}})
    ...
    def _create_signature(self, event: SNSEvent) -> str:
        ...
        return hmac.new(SECRETS_MANAGER.get_secret("SNS_GATEWAY_SIGNING_SECRET").encode(), canonical_event.encode(), hashlib.sha256).hexdigest()
    ...
    async def _open_next_log_segment(self):
        async with self._write_lock:
            ...
            self._current_log_path = os.path.join(self._dir, f"events.{time.strftime('%Y%m%d_%H%M%S')}.log")
            os.rename(temp_path, self._current_log_path)
            self._last_rotation_time = time.time()

    async def put(self, item: SNSEvent):
        async with self._write_lock:
            if not self._current_write_log or await aiofiles.os.stat(self._current_log_path).st_size > self._max_log_size or time.time() - self._last_rotation_time > self._log_rotation_interval:
                await self._open_next_log_segment()
    ...
    async def get(self): return await self._mem_queue.get()
    def qsize(self): return self._mem_queue.qsize()
    async def task_done(self): self._mem_queue.task_done()
    ...
    async def flush(self, timeout: int = 60):
        try:
            ...
            audit_logger.critical("shutdown_timeout", extra={"context": {"remaining_events": self._mem_queue.qsize()}})
            ...
            if len(log_files) > 2:
                for old_file in log_files[:-2]:
                    try:
                        ...
                        audit_logger.info("WAL file compacted.", extra={"context": {"file": old_file}})
                    except OSError as e:
                        main_logger.error(f"Failed to compact WAL file {old_file}: {e}")
            if self._compactor_task.done(): break

    async def shutdown(self):
        await self.flush()
        if self._compactor_task:
            self._compactor_task.cancel()
            try: await self._compactor_task
            except asyncio.CancelledError: pass
        if self._current_write_log: await self._current_write_log.close()
        audit_logger.info("WAL compacted and queues flushed on shutdown.", extra={"context": {"target": self._target_name}})
...
class CircuitBreaker:
    ...
    def check(self):
        if self._is_open:
            ...
            if time.monotonic() - self._last_failure_time > (self._reset_seconds + jitter):
                ...
                self._metrics.CIRCUIT_BREAKER_STATUS.labels(target_name=self._target_name).set(0)
                main_logger.warning("Circuit breaker has been reset.", extra={"context": {"target": self._target_name}})
                audit_logger.info("circuit_breaker_reset", extra={"context": {"target": self._target_name}})
            else:
    ...
    def record_failure(self):
        ...
        if self._failure_count >= self._threshold and not self._is_open:
            ...
            main_logger.critical("Circuit breaker tripped. Escalating.", extra={"context": {"target": self._target_name}})
            audit_logger.critical("circuit_breaker_tripped", extra={"context": {"target": self._target_name, "threshold": self._threshold}})
            alert_operator(f"CRITICAL: SNS circuit breaker tripped for {self._target_name}.", "CRITICAL")
    
    def record_success(self):
        if self._failure_count > 0:
            self._failure_count = 0
            main_logger.info("Circuit breaker failure count reset due to success.", extra={"context": {"target": self._target_name}})
...
class SNSGateway:
    def __init__(self, target_config: SNSTarget, global_settings: SNSGatewaySettings, metrics: SNSMetrics, serializer: Serializer, rate_limiter: TokenBucket, dead_letter_hook: Optional[DeadLetterHook]):
        ...
        self.circuit_breaker = CircuitBreaker(global_settings.circuit_breaker_threshold, global_settings.circuit_breaker_reset_sec, metrics, target_config.name)
        
        self._event_queue: EventQueue = PersistentWALQueue(target_config.name, global_settings.persistence_dir, global_settings.max_queue_size, metrics, dead_letter_hook, encryption_key=global_settings.encryption_key)
    ...
    async def shutdown(self):
        ...
        for _ in self._workers: await self._event_queue.put(None)
        await asyncio.gather(*self._workers, return_exceptions=True)
        
        await self._event_queue.flush(timeout=self.global_settings.max_queue_size / self.global_settings.max_workers + 10)
    ...
    async def _load_sequence_counter(self):
        path = os.path.join(self.global_settings.persistence_dir, f"{self.target_config.name}_seq.txt")
        try:
            ...
            self._sequence_id = 0
            main_logger.warning("Failed to load sequence counter, starting from 0.", extra={"context": {"target": self.target_config.name}})
    ...
    async def _save_sequence_counter(self, seq_id: int):
        ...
        try:
            ...
            main_logger.critical("Failed to persist sequence counter.", extra={"context": {"target": self.target_config.name, "error": str(e)}})
            audit_logger.critical("sequence_counter_persistence_failure", extra={"context": {"target": self.target_config.name, "error": str(e)}})
            raise
        ...
        async with self._session_lock:
            if self._session is None or self._session.closed:
                ...
                if PROD_MODE and self.global_settings.cert_path and self.global_settings.key_path:
                    ssl_context.load_cert_chain(self.global_settings.cert_path, self.global_settings.key_path)
    ...
    async def _handle_dead_letter(self, event: SNSEvent, reason: str):
        self.metrics.DEAD_LETTER_NOTIFICATIONS.labels(target_name=self.target_config.name, reason=reason).inc()
        audit_logger.warning("dead_letter_event", extra={"context": {"target": self.target_config.name, "reason": reason, "event_name": event.event_name}})
        if self.dead_letter_hook:
            try: await self.dead_letter_hook(event, reason)
            except Exception as e: main_logger.error("Dead-letter hook failed.", extra={"context": {"error": str(e), "target": self.target_config.name}})

    async def publish(self, event: SNSEvent):
        try:
            event.enqueue_time = time.time()
            await self._event_queue.put(event)
            self.metrics.NOTIFICATIONS_QUEUED.labels(target_name=self.target_config.name, event_name=event.event_name, severity=event.severity).inc()
            self.metrics.QUEUE_SIZE.labels(target_name=self.target_config.name).set(self._event_queue.qsize())
            audit_logger.debug("event_queued", extra={"context": {"target": self.target_config.name, "event_name": event.event_name}})
        except asyncio.QueueFull:
            try:
                await self._fallback_queue.put(event)
                main_logger.warning("Primary queue full, redirected to fallback queue.", extra={"context": {"target": self.target_config.name}})
                audit_logger.warning("event_redirected_to_fallback", extra={"context": {"target": self.target_config.name, "event_name": event.event_name}})
            except asyncio.QueueFull:
                self.metrics.NOTIFICATIONS_DROPPED.labels(target_name=self.target_config.name, event_name=event.event_name).inc()
                main_logger.warning("Primary and fallback queues are full. Event dropped.", extra={"context": {"target": self.target_config.name, "event_name": event.event_name}})
    ...
    async def _send_batch(self, batch: List[SNSEvent]):
        ...
        if not deduped_batch: return True

        with tracer.start_as_current_span("sns_send_batch", attributes={
            ...
            while attempt < self.global_settings.max_retries:
                self.metrics.RETRY_ATTEMPTS.labels(target_name=self.target_config.name).observe(attempt)
                await self.rate_limiter.acquire()
                async with self._concurrency_limiter:
                    start_time = time.monotonic()
                    try:
                        session = await self._get_session()
                        timeout = aiohttp.ClientTimeout(total=self.global_settings.retry_backoff_factor ** attempt * 5)
                        
                        endpoint = self.target_config.url_endpoint or f"https://sns.{self.target_config.region}.amazonaws.com"
                        
                        sns_payload = {
                            "Action": "PublishBatch",
                            "TopicArn": self.target_config.topic_arn,
                            "PublishBatchRequestEntries": [
                                {"Id": event.correlation_id, "Message": data} for event, data in zip(deduped_batch, payload_data)
                            ]
                        }
                        
                        async with session.post(endpoint, json=sns_payload, headers=headers, ssl=self.global_settings.verify_ssl, timeout=timeout) as resp:
                            self.rate_limiter.record_status(resp.status)
                            if resp.status == 200:
                                self.metrics.SEND_LATENCY.labels(target_name=self.target_config.name).observe(time.monotonic() - start_time)
                                self.metrics.NOTIFICATIONS_SENT_SUCCESS.labels(target_name=self.target_config.name).inc(len(deduped_batch))
                                self.circuit_breaker.record_success()
                                audit_logger.info("notification_sent_success", extra={"context": {"target": self.target_config.name, "batch_size": len(deduped_batch)}})
                                return True
                            elif resp.status == 429:
                                retry_after = int(resp.headers.get("Retry-After", "5"))
                                main_logger.warning(f"Rate limited by SNS API. Backing off for {retry_after} seconds.", extra={"context": {"target": self.target_config.name}})
                                await asyncio.sleep(retry_after)
                                continue
                            elif 400 <= resp.status < 500:
                                error_text = await resp.text()
                                main_logger.error("Permanent failure sending to SNS (client error).", extra={"context": {"target": self.target_config.name, "error": error_text, "status_code": resp.status}})
                                for event in deduped_batch: await self._handle_dead_letter(event, "client_error")
                                self.metrics.NOTIFICATIONS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason=f"client_error_{resp.status}").inc(len(deduped_batch))
                                return False
                            else: resp.raise_for_status()
                    except (aiohttp.ClientError, asyncio.TimeoutError) as e:
                        main_logger.warning("Temporary failure sending to SNS.", extra={"context": {"target": self.target_config.name, "attempt": attempt + 1, "error": str(e)}})
                        if attempt + 1 >= self.global_settings.max_retries:
                            self.circuit_breaker.record_failure()
                            for event in deduped_batch: await self._handle_dead_letter(event, "service_unavailable")
                            self.metrics.NOTIFICATIONS_FAILED_PERMANENTLY.labels(target_name=self.target_config.name, reason="service_unavailable").inc(len(deduped_batch))
                            return False
                attempt += 1
                if attempt < self.global_settings.max_retries: await asyncio.sleep(self.global_settings.retry_backoff_factor ** attempt)
            return False

    async def _worker(self, worker_id: int):
        main_logger.info(f"Starting worker {worker_id} for target {self.target_config.name}")
        audit_logger.info("worker_started", extra={"context": {"target": self.target_config.name, "worker_id": worker_id}})
        while not self._shutdown_event.is_set():
            try:
                self.metrics.QUEUE_SIZE.labels(target_name=self.target_config.name).set(self._event_queue.qsize())
                if self._is_paused: await asyncio.sleep(1); continue
                batch = []
                first_event = await self._event_queue.get()
                if first_event is None: self._event_queue.task_done(); await self._event_queue.put(None); break
                self.metrics.QUEUE_LATENCY.labels(target_name=self.target_config.name).observe(time.time() - first_event.enqueue_time)
                batch.append(first_event)
                while len(batch) < self.global_settings.worker_batch_size:
                    try:
                        event = await asyncio.wait_for(self._event_queue.get(), self.global_settings.worker_linger_sec)
                        if event is None: self._event_queue.put_nowait(None); break
                        self.metrics.QUEUE_LATENCY.labels(target_name=self.target_config.name).observe(time.time() - event.enqueue_time)
                        batch.append(event)
                    except asyncio.TimeoutError: break
                
                success = False
                if self.global_settings.dry_run:
                    main_logger.info("[DRY RUN] Would send SNS notification.", extra={"context": {"target": self.target_config.name, "batch_size": len(batch)}})
                    if self.global_settings.dry_run_failure_rate > 0 and random.random() < self.global_settings.dry_run_failure_rate:
                        for event in batch: await self._handle_dead_letter(event, "dry_run_simulated_failure")
                        success = False
                    else: success = True
                else: success = await self._send_batch(batch)
                
                if success:
                    self._health_stats["processed_count"] += len(batch)
                    self._health_stats["last_processed_time"] = time.time()
                for _ in batch: await self._event_queue.task_done()
        ...
        audit_logger.info("worker_stopped", extra={"context": {"target": self.target_config.name, "worker_id": worker_id}})

    async def _worker_manager(self):
        active_workers = [asyncio.create_task(self._worker(i)) for i in range(self.global_settings.min_workers)]
        ...
        while not self._shutdown_event.is_set():
            ...
            if avg_queue > self.global_settings.queue_size_per_worker * len(active_workers) and len(active_workers) < self.global_settings.max_workers and cpu_usage < 80 and mem_usage < 80:
                worker_id = len(active_workers)
                main_logger.info(f"Queue size high ({queue_size}), scaling up worker for {self.target_config.name} to {worker_id + 1}")
                audit_logger.info("worker_scale_up", extra={"context": {"target": self.target_config.name, "new_worker_count": worker_id + 1, "queue_size": queue_size}})
                ...
                if self._scale_down_timer >= self.global_settings.worker_scaling_interval * 3:
                    main_logger.info(f"Queue empty, scaling down worker for {self.target_config.name} to {len(active_workers) - 1}")
                    audit_logger.info("worker_scale_down", extra={"context": {"target": self.target_config.name, "new_worker_count": len(active_workers) - 1}})
            ...
            self.metrics.ACTIVE_WORKERS.labels(target_name=self.target_config.name).set(len(active_workers))
...
class SNSGatewayManager:
    def __init__(self, settings: SNSGatewaySettings, metrics: SNSMetrics, dead_letter_hook: Optional[DeadLetterHook] = None):
    ...
    async def startup(self):
        if PROD_MODE:
            if not fcntl:
                main_logger.critical("`fcntl` module not available. File locking is required for persistence in production mode. Exiting.")
                sys.exit(1)
            if self.settings.dry_run or self.settings.dry_run_failure_rate > 0:
                main_logger.critical("DRY_RUN or DRY_RUN_FAILURE_RATE is enabled in production mode. This is a critical error. Exiting.")
                sys.exit(1)
    ...
    async def shutdown(self):
        ...
        if self._http_server_task:
            ...
            try: await self._http_server_task
            except asyncio.CancelledError: pass

        async with self._lock:
            await asyncio.gather(*(gw.shutdown() for gw in self._gateways.values()))
        if self._admin_audit_log: await self._admin_audit_log.close()
        ...
        try:
            for entry_point in importlib.metadata.entry_points(group=group):
                try:
                    ...
                    main_logger.error(f"Failed to load serializer plugin '{entry_point.name}'.", extra={"context": {"error": str(e)}})
                    if self.settings.strict_plugins:
                        raise RuntimeError(f"Critical serializer plugin '{entry_point.name}' failed to load.")
        except TypeError:
            main_logger.warning(f"Falling back to legacy entry_points due to Python version {sys.version}")
    ...
    async def reload_config(self, new_settings: SNSGatewaySettings):
        async with self._lock:
            ...
            audit_logger.info("reload_config_initiated", extra={"context": {"new_target_count": len(new_settings.targets), "config_version": self._config_version}})
            ...
            for target in new_settings.targets:
                ...
                if limiter_key not in self._rate_limiters:
                    self._rate_limiters[limiter_key] = TokenBucket(new_settings.requests_per_second_limit, new_settings.requests_per_second_limit, self.metrics, limiter_key)

            for name, target_config in new_targets_map.items():
                serializer = self._serializers.get(target_config.serializer)
                if not serializer:
                    main_logger.error(f"Unknown serializer '{target_config.serializer}' for target '{name}'. Skipping.")
                    audit_logger.error("reload_config_failed_unknown_serializer", extra={"context": {"target": name, "serializer": target_config.serializer}})
                    continue
                
                limiter_key = f"{target_config.region}_{new_settings.requests_per_second_limit}"
                rate_limiter = self._rate_limiters[limiter_key]
                new_gateways[name] = SNSGateway(target_config, new_settings, self.metrics, serializer, rate_limiter, self.dead_letter_hook)
            ...
            main_logger.info("Configuration reloaded. Draining old gateways.", extra={"context": {"active_targets": list(self._gateways.keys())}})
            audit_logger.info("reload_config_success", extra={"context": {"active_targets": list(self._gateways.keys()), "config_version": self._config_version}})
    ...
    async def publish(self, target_name: str, event_name: str, details: Dict[str, Any], **kwargs):
        ...
        if not gateway:
            ...
            audit_logger.warning("publish_to_unknown_target", extra={"context": {"target": target_name, "event_name": event_name}})
            return
        ...
        async with gateway._sequence_lock:
            ...
            try:
                ...
                main_logger.critical("Failed to save sequence counter. This may lead to duplicate events on restart.")
        
        canonical_event = f"{seq_id}|{event_name}|{json.dumps(details, sort_keys=True)}"
        signature = hmac.new(self.settings.signing_secret.encode(), canonical_event.encode(), hashlib.sha256).hexdigest()
        ...
        try:
            event = SNSEvent(event_name=event_name, details=details, trace_context=trace_context, sequence_id=seq_id, signature=signature, **kwargs)
            await gateway.publish(event)
        except ValidationError as e:
            main_logger.error("Invalid SNS event schema.", extra={"context": {"error": str(e), "event_name": event_name}})
            audit_logger.error("invalid_event_schema", extra={"context": {"event_name": event_name, "error": str(e)}})
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for name, gw in self._gateways.items():
            targets_health[name] = {
                ...
                "wal_files": len([f for f in os.listdir(gw._event_queue._dir) if f.endswith('.log')]),
                "status": "paused" if gw._is_paused else ("healthy" if not gw.circuit_breaker._is_open else "unhealthy_circuit_open"),
                **gw._health_stats
            }
        return {"status": "ok", "targets": targets_health, "version": self._config_version}
    ...
    async def _run_admin_api_server(self):
        ...
        async def auth_middleware(request, handler):
            ...
            if request.path.startswith('/admin'):
                ...
                if not auth_header or auth_header != f"Bearer {self.settings.admin_api_key}":
                    audit_logger.warning("unauthorized_admin_api_access", extra={"context": {"path": request.path, "source_ip": request.remote}})
        ...
        async def handle_reload(request):
            try:
                ...
                audit_logger.critical("admin_api_reload_critical_failure", extra={"context": {"error": str(e), "source_ip": request.remote}})
        ...
        async def handle_pause(request):
            ...
            if gw := self._gateways.get(target_name):
                gw.pause()
                await self._log_admin_action("api_pause", {"target": target_name, "source_ip": request.remote})
        ...
        async def handle_resume(request):
            ...
            if gw := self._gateways.get(target_name):
                gw.resume()
                await self._log_admin_action("api_resume", {"target": target_name, "source_ip": request.remote})
        ...
        site = web.TCPSite(runner, self.settings.admin_api_host, self.settings.admin_api_port, ssl_context=ssl_context)
        
        try:
            await site.start()
            main_logger.info(f"Admin API server started on {'https' if ssl_context else 'http'}://{self.settings.admin_api_host}:{self.settings.admin_api_port}")
            audit_logger.info("admin_api_started", extra={"context": {"host": self.settings.admin_api_host, "port": self.settings.admin_api_port, "protocol": "https" if ssl_context else "http"}})
            await asyncio.Event().wait()
        except asyncio.CancelledError: pass
        except OSError as e:
            raise AnalyzerCriticalError(f"Failed to start admin API server on port {self.settings.admin_api_port}: {e}.")
...
async def dead_letter_to_file(event: SNSEvent, reason: str):
    log_line = json.dumps({"event": event.model_dump(), "failure_reason": reason, "timestamp": time.time()})
    filepath = os.path.join(DEAD_LETTER_DIR, f"sns_dead_letters.{time.strftime('%Y%m%d')}.log")
    
    encryption_key = SECRETS_MANAGER.get_secret("SNS_GATEWAY_DEAD_LETTER_ENCRYPTION_KEY", required=False)
    ...
    try:
        if PROD_MODE:
            ...
            sns_settings = SNSGatewaySettings(
                signing_secret=os.environ.get("SNS_GATEWAY_SIGNING_SECRET", "non-prod-signing-secret"),
                admin_api_key=os.environ.get("SNS_GATEWAY_ADMIN_API_KEY", "non-prod-admin-key"),
                targets=[
                    SNSTarget(name="alerts", topic_arn="arn:aws:sns:us-east-1:123456789012:alerts-topic", region="us-east-1", access_key_id="dummy-key-alerts", secret_access_key="dummy-secret-alerts"),
                    SNSTarget(name="audit", topic_arn="arn:aws:sns:us-east-2:123456789012:audit-topic", region="us-east-2", access_key_id="dummy-key-audit", secret_access_key="dummy-secret-audit"),
                ],
                url_allowlist=['^https://sns\\..*\\.amazonaws\\.com']
            )

        sns_metrics = SNSMetrics()
        sns_gateway_manager = SNSGatewayManager(sns_settings, sns_metrics, dead_letter_hook=dead_letter_to_file)
...
if __name__ == "__main__":
    ...
    async def main_example():
        ...
        await sns_gateway_manager.publish("alerts", "database_connection_failed", {"db_host": "prod-db-1"}, severity="critical")
        await sns_gateway_manager.publish("audit", "user_logged_in", {"user_id": "user-123"}, severity="info")
        await sns_gateway_manager.publish("alerts", "sensitive_info_exposed", {"user_id": "user-456", "token": "super-secret-token-123"}, severity="critical")

        main_logger.info("Published example events. Waiting for a few seconds to let workers process.")
