...
try:
    ...
    aiofiles = None
    logging.getLogger(__name__).warning("aiofiles library not found. Asynchronous file I/O may be limited.")
...
try:
    ...
    portalocker = None
    logging.getLogger(__name__).warning("portalocker library not found. File locking will be disabled.")
...
try:
    ...
    logging.getLogger(__name__).warning("elasticsearch library not found. Elasticsearch integration will be disabled.")
...
try:
    ...
    logging.getLogger(__name__).warning("kafka-python library not found. Kafka integration will be disabled.")
...
try:
    ...
    logging.getLogger(__name__).warning("web3.py library not found. Ethereum integration will be disabled.")
...
try:
    from simulation.plugins.dlt_clients.dlt_base import initialize_dlt_backend_clients, _dlt_client_instance
    from simulation.plugins.dlt_clients.dlt_base import ProductionDLTClient, EVMDLTClient, SimpleDLTClient
    from simulation.plugins.dlt_clients.dlt_base import ProductionOffChainClient
    DLT_BACKEND_AVAILABLE = True
except ImportError as e:
    _base_logger.warning(f"DLT backend (simulation.plugins.dlt_clients) not found: {e}. DLT integration will be disabled.", exc_info=True)
...
class MockConfig:
    ...
    DLT_BACKEND_CONFIG = {
        ...
        "fabric": {
            ...
            "peer_names": os.environ.get("FABRIC_PEER_NAMES", "").split(',') if os.environ.get("FABRIC_PEER_NAMES") else []
        },
        "evm": {
            ...
            "private_key": os.environ.get("ETHEREUM_PRIVATE_KEY") or os.environ.get("AVALANCHE_PRIVATE_KEY"),
            ...
            "default_max_priority_fee_per_gas": int(os.environ.get("EVM_MAX_PRIORITY_FEE_PER_GAS", "2")),
        },
        "corda": {
...
try:
    ...
    config = MockConfig()
    logger.warning("Using MockConfig for audit_log.py. Please ensure app.config is set up correctly in production.")
...
def sanitize_log(msg: str) -> str:
    ...
    msg = re.sub(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', 'REDACTED_EMAIL', msg)  # Email
    return msg
...
def validate_sensitive_env_vars() -> None:
    ...
    if app_env != "production":
        return
    sensitive_vars = [
        'PRIVATE_KEY_B64', 'PRIVATE_KEY_PASSWORD', 'AWS_SECRET_ACCESS_KEY',
        'ETHEREUM_PRIVATE_KEY', 'AVALANCHE_PRIVATE_KEY', 'CORDA_PASS',
        'ETHEREUM_AUDIT_CONTRACT', 'ETHEREUM_ABI_PATH'
    ]
    for var in sensitive_vars:
        value = os.environ.get(var, "")
        if value and ("dummy" in value.lower() or "mock" in value.lower()):
            logger.critical(f"Dummy value detected in sensitive env var {var}. Aborting.", extra={"context": "startup"})
            sys.exit(1)
    ...
    if CRYPTO_AVAILABLE and os.environ.get("PUBLIC_KEY_B64"):
        try:
            for key_b64 in os.environ["PUBLIC_KEY_B64"].split(","):
                ...
                key_id = hashlib.sha256(pub_key.public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)).hexdigest()
...
def _strip_signatures(entry: dict) -> dict:
    ...
    return {k: v for k, v in entry.items() if not k.startswith("signature") and not k.startswith("merkle_") and k != 'hash'}
...
def append_distributed_log(entry: dict, correlation_id: Optional[str] = None) -> bool:
    ...
    if not (KAFKA_AVAILABLE and hasattr(config, 'KAFKA_BOOTSTRAP_SERVERS') and config.KAFKA_BOOTSTRAP_SERVERS):
        logger.debug(sanitize_log("Kafka append skipped: kafka-python library or config missing."), extra=log_context)
        return False
    producer = None
    try:
        ...
        future = producer.send(config.KAFKA_AUDIT_TOPIC, key=entry_hash.encode('utf-8'), value=entry)
        record_metadata = future.get(timeout=10)
        logger.info(sanitize_log(f"[AUDIT] Appended to Kafka topic '{record_metadata.topic}' partition {record_metadata.partition}"), extra=log_context)
        return True
    except Exception as e:
        level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
        logger.log(level, sanitize_log(f"[AUDIT] Kafka append failed: {e}"), exc_info=True, extra=log_context)
    ...
    with AUDIT_LOCK:
        ...
        try:
            ...
            if not key_b64 or not password:
                logger.warning("Private key environment variables (PRIVATE_KEY_B64, PRIVATE_KEY_PASSWORD) not set. Digital signing will be disabled.")
                return None
            ...
            level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
            logger.log(level, f"Failed to load private key from environment: {e}", exc_info=True, extra=log_context)
            return None

async def key_rotation(audit_logger_instance: 'AuditLogger', correlation_id: Optional[str] = None) -> bool:
    """Initiates cryptographic key rotation."""
    log_context = {"correlation_id": correlation_id, "context": "key_rotation"}
    if not CRYPTO_AVAILABLE:
        logger.warning("Key rotation skipped: cryptography library not available.", extra=log_context)
        return False
    ...
    try:
        ...
        new_key_id = hashlib.sha256(new_private_key.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)).hexdigest()
        
        # a. Secure key rotation
        logger.critical(f"New key generated. IMMEDIATELY update your secret store with this new PRIVATE_KEY_B64: {pem_b64}. New Public Key ID: {new_key_id}", extra=log_context)
        ...
        if audit_logger_instance.signers:
            for old_signer in audit_logger_instance.signers:
                if isinstance(old_signer, Ed25519PrivateKey):
                    pub_key_bytes = old_signer.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)
        ...
        await audit_logger_instance.add_entry(
            kind="key_management",
            name="key_rotation_event",
            detail={"status": "failed", "error": str(e), "message": "Key rotation process failed."},
            agent_id="system",
            correlation_id=correlation_id
        )
        return False
...
def key_revocation(key_id: str, correlation_id: Optional[str] = None):
    ...
    with AUDIT_LOCK:
        ...
        logger.warning(f"SECURITY: Key revoked: {key_id}. Signatures from this key will now be invalid.", extra=log_context)
...
class AuditLogger:
    ...
    def __init__(self, log_path: Optional[str] = None, signers: Optional[List] = None, hash_algo: str = "sha256",
                 dlt_backend_enabled: bool = False, dlt_backend_config: Optional[Dict[str, Any]] = None):
        ...
        if not CRYPTO_AVAILABLE:
            logger.warning("Digital signing is disabled as cryptography library is not available.", extra=log_context)
        if not portalocker:
            logger.warning("File locking is disabled as portalocker is not available. Concurrent file writes may be unsafe.", extra=log_context)
        
        # e. Run chain verification at startup in production
        if os.environ.get("APP_ENV", "development").lower() == "production":
            if not verify_audit_chain(self.log_path):
                logger.critical("Audit chain invalid at startup. Aborting.", extra={"context": "startup"})
                sys.exit(1)
        ...
        if self.dlt_backend_enabled and not _initialized_dlt_backend:
            ...
            try:
                if DLT_BACKEND_AVAILABLE and callable(initialize_dlt_backend_clients):
                    ...
                    logger.info("DLT backend clients initialized successfully.", extra=log_context)
                else:
                    logger.critical("DLT backend initialization failed. Module not available or not callable. DLT integration will be disabled.", extra=log_context)
                    self.dlt_backend_enabled = False
            except Exception as e:
                logger.critical(f"Failed to initialize DLT backend clients: {e}. DLT integration will be disabled. Mitigation: Install required plugins or check configuration.", exc_info=True, extra=log_context)
                self.dlt_backend_enabled = False
    ...
    def _load_last_hashes(self):
        ...
        if not os.path.exists(self.log_path):
            self._last_entry_hash = "genesis_hash"
            logger.info("Audit log file not found. Starting new chain from 'genesis_hash'.", extra={"context": "load_hash"})
            return
        ...
        with AUDIT_LOCK:
            ...
            try:
                ...
                level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
                logger.log(level, f"Error reading last line of audit log: {e}", exc_info=True, extra={"context": "load_hash"})
                self._last_entry_hash = "genesis_hash"

        logger.info(f"Resumed audit chain from hash: {self._last_entry_hash[:10]}...", extra={"context": "load_hash"})
        ...
        with AUDIT_LOCK:
            with open(filepath, "rb") as f:
                try:
                    ...
                    level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
                    logger.log(level, f"Error reading last line from {filepath}: {e}", exc_info=True)
                    return None
    ...
    async def add_entry(self, kind: str, name: str, detail: Dict[str, Any], agent_id: str,
        ...
        with AUDIT_LOCK:
            ...
            if self.signers and CRYPTO_AVAILABLE:
                for signer in self.signers:
                    try:
                        pub_key_bytes = signer.public_key().public_bytes(serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)
                        key_id = hashlib.sha256(pub_key_bytes).hexdigest()
                        
                        if key_id in REVOKED_KEYS:
                            logger.warning(sanitize_log(f"Attempted to sign audit entry with revoked key: {key_id}. Signature will be skipped."), extra=log_context)
                            entry["signatures"].append({"key_id": key_id, "signature": "SKIPPED_REVOKED_KEY", "status": "revoked"})
                            continue
                        ...
                        entry["signatures"].append({"key_id": "error", "signature": f"error:{str(e)}", "status": "signing_failed"})
        ...
        try:
            ...
            logger.critical(f"Failed to write to primary audit log file after retries: {e}", exc_info=True, extra=log_context)
            # g. Optional: Alerting
            if os.environ.get('APP_ENV') == "production" and REQUESTS_AVAILABLE and os.environ.get('ALERT_WEBHOOK'):
                requests.post(os.environ['ALERT_WEBHOOK'], json={'msg': f"CRITICAL: Audit log file write failed after retries: {e}"})
        ...
        if self.dlt_backend_enabled and _dlt_client_instance:
            ...
            try:
                ...
                logger.info(f"Audit entry written to DLT backend. Tx ID: {tx_id}, DLT Version: {dlt_version}", extra=log_context)
            except Exception as e:
                logger.critical(f"Failed to write audit entry to DLT backend after retries: {e}. Mitigation: Check DLT server connectivity, configuration, and API keys.", exc_info=True, extra=log_context)
        elif self.dlt_backend_enabled and not _dlt_client_instance:
            logger.warning("DLT backend enabled in config but not initialized. Audit entry not sent to DLT.", extra=log_context)
        ...
        try:
            ...
            level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
            logger.log(level, sanitize_log(f"Error dispatching audit entry to Kafka: {e}"), exc_info=True, extra=log_context)
            if os.environ.get("APP_ENV", "development").lower() == "production":
                raise # Fail fast

        for plugin_func in self.plugins:
            try:
                logger.debug("Dispatching to plugin. Ensure plugin is thread-safe for concurrent execution.", extra=log_context)
                if asyncio.iscoroutinefunction(plugin_func):
                    asyncio.create_task(plugin_func(entry.copy(), correlation_id=log_context['correlation_id']))
                else:
                    asyncio.get_running_loop().run_in_executor(None, plugin_func, entry.copy(), log_context['correlation_id'])
            except Exception as e:
                logger.error(sanitize_log(f"Error dispatching audit entry to plugin {getattr(plugin_func, '__name__', str(plugin_func))}: {e}"), exc_info=True, extra=log_context)
        
        logger.info(sanitize_log(f"Audit entry processed: {event_type} (Hash: {entry_hash[:10]}...)"), extra=log_context)

    @retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=1, max=10), before_sleep=before_sleep_log(logger, logging.WARNING))
    async def _async_file_write(self, filepath: str, entry: Dict[str, Any], log_context: Dict[str, Any]):
        """
        ...
        if free < 100 * 1024 * 1024:  # Less than 100MB free
            logger.critical(f"Low disk space ({free/1024/1024:.2f}MB free) for {filepath}. Risk of write failure.", extra=log_context)
            if os.environ.get('APP_ENV') == "production" and REQUESTS_AVAILABLE and os.environ.get('ALERT_WEBHOOK'):
                requests.post(os.environ['ALERT_WEBHOOK'], json={'msg': f"CRITICAL: Low disk space ({free/1024/1024:.2f}MB) for audit log {filepath}"})
    ...
    def _sync_file_write(self, filepath: str, entry: Dict[str, Any], log_context: Dict[str, Any]):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not portalocker:
                    logger.warning("Portalocker is not available. Using basic file write without locking. This is not thread/process-safe for concurrent writes.", extra=log_context)
                ...
                if attempt == max_retries - 1:
                    level = logging.CRITICAL if os.environ.get("APP_ENV", "development").lower() == "production" else logging.ERROR
                    logger.log(level, f"File write failed after {max_retries} attempts to {filepath}: {e}", exc_info=True, extra=log_context)
                    raise
                time.sleep(2 ** attempt)  # Exponential backoff: 1s, 2s, 4s
                logger.warning(f"Retry {attempt+1}/{max_retries} for locked file write to {filepath}: {e}", exc_info=True, extra=log_context)
    ...
    async def close(self):
        ...
        if _dlt_client_instance and hasattr(_dlt_client_instance, 'close'):
            try:
                ...
                logger.error(f"Error closing DLT backend client: {e}", exc_info=True, extra=log_context)
            _dlt_client_instance = None
...
def verify_audit_chain(log_path: Optional[str] = None) -> bool:
    ...
    if not os.path.exists(path):
        logger.info(f"Audit log file not found at {path}. Chain considered valid (empty).", extra=log_context_base)
        return True
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                for i, line in enumerate(f):
                    ...
                    log_context = {"context": "verification", "entry_index": i, "correlation_id": current_correlation_id}
                    
                    with TRACER.start_as_current_span(f"audit_chain.verify_entry_{i}", attributes={"entry_index": i, "correlation_id": current_correlation_id}) as span:
                        try:
                            entry = json.loads(line)
            ...
            logger.critical(f"Error accessing audit log file during verification: {e}", exc_info=True, extra=log_context_base)
            is_valid = False
    
    if is_valid:
        logger.info("Audit chain successfully verified.", extra=log_context_base)
    else:
        logger.critical("Audit chain verification FAILED. Tampering or corruption detected!", extra=log_context_base)

    return is_valid

async def audit_log_event_async(event_type: str, message: str, data: Optional[Dict[str, Any]] = None, agent_id: str = "system",
                    correlation_id: Optional[str] = None,
                    compliance_control_id: Optional[str] = None,
                    is_compliant: Optional[bool] = None):
    ...
    try:
        ...
        log_context = {"correlation_id": correlation_id}
        logging.getLogger(__name__).error(f"Failed to log audit event via helper: {e}", exc_info=True, extra=log_context)
    finally:
...
if __name__ == "__main__":
    async def run_tests():
        ...
        await audit_logger.add_entry("system", "startup", {"message": "System initialized"}, "sfe_core_agent", correlation_id="cid-test1-1", compliance_control_id="NIST_CM-2", is_compliant=True)
        await audit_logger.add_entry("agent", "action", {"action": "refactor", "file": "main.py"}, "sfe_dev_agent", correlation_id="cid-test1-2", compliance_control_id="NIST_AC-6", is_compliant=True)
        await audit_logger.add_entry("security", "alert", {"vulnerability": "XSS"}, "sfe_sec_agent", correlation_id="cid-test1-3", compliance_control_id="NIST_RA-5", is_compliant=False)
        ...
        if DLT_BACKEND_AVAILABLE:
            ...
            if os.path.exists(test_dlt_log_path): os.remove(test_dlt_log_path)
            ...
            try:
                await audit_logger_dlt.add_entry("deployment", "start", {"app_version": "1.0.0"}, "sfe_deployer", correlation_id="cid-test2-1", compliance_control_id="NIST_CM-2", is_compliant=True)
                await audit_logger_dlt.add_entry("config", "update", {"param": "timeout", "value": 30}, "sfe_ops_agent", correlation_id="cid-test2-2", compliance_control_id="NIST_AC-3", is_compliant=True)
                
                if CRYPTO_AVAILABLE:
                    print("\nTest 2.1: Key Rotation with DLT audit")
                    initial_key_id = hashlib.sha256(audit_logger_dlt.signers[0].public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)).hexdigest() if audit_logger_dlt.signers else "N/A"
                    print(f"Initial Key ID: {initial_key_id}")
                    key_rotation_success = await key_rotation(audit_logger_dlt, correlation_id="cid-test2-key-rotate")
                    print(f"Key Rotation Success: {key_rotation_success}")
                    if key_rotation_success:
                        new_key_id = hashlib.sha256(audit_logger_dlt.signers[0].public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw)).hexdigest()
                ...
                await audit_logger_dlt.add_entry("system", "shutdown", {"reason": "test_complete"}, "sfe_core_agent", correlation_id="cid-test2-3", compliance_control_id="NIST_PL-2", is_compliant=True)
                
                await asyncio.sleep(1)

                print("\nTest 2 Verification (Local Chain):")
                if verify_audit_chain(test_dlt_log_path):
                    print("Test 2 PASSED: Local audit chain (with DLT attempts) verified successfully.")
                else:
                ...
                if isinstance(_dlt_client_instance, SimpleDLTClient):
                    ...
                    dlt_entry_hash = _dlt_client_instance.chain.get('audit_deployment:start')[0]['hash']
                    retrieved_tx_info = await _dlt_client_instance.read_checkpoint('audit_deployment:start', version=1)
