...
codegen-multipass-status: ## Show current multi-pass code-generation thresholds and timeout budgets
	...
	@echo ""
	@echo "$(YELLOW)Docgen / RAG / OTEL:$(NC)"
	@echo "  DOCGEN_MAX_CONTEXT_CHARS             = $${DOCGEN_MAX_CONTEXT_CHARS:-30000}  (pre-summarisation threshold; chars)"
	@echo "  DOCGEN_TRUNCATION_LINES              = $${DOCGEN_TRUNCATION_LINES:-100}   (lines per file when context is truncated)"
	@echo "  RAG_ENABLED                          = $${RAG_ENABLED:-false}  (set true + VECTOR_DB_* to enable RAG critique)"
	@echo "  OTEL_BATCH_LINT_SPANS                = $${OTEL_BATCH_LINT_SPANS:-true}  (batch lint spans to avoid Railway 500/s limit)"
	@echo ""
	@echo "$(YELLOW)SFE / Migration:$(NC)"
	@echo "  SFE_SKIP_MIGRATE_ON_ANALYSIS         = $${SFE_SKIP_MIGRATE_ON_ANALYSIS:-false}  (skip Alembic on analysis-only runs)"
	@echo "  AUTO_MIGRATE                         = $${AUTO_MIGRATE:-0}  (0=disabled, 1=run Alembic on startup)"
	@echo ""
